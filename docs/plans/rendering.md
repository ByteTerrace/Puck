...
**The frame graph and nesting.** P11's CPU half has landed, and so have
P11b's first items: the first-class package pass kind, a steady-state schedule
that allocates nothing, the document pass kind, the fold of the pipeline
document into the graph document, planned buffer edges, the graph runtime and
its engine packages, and the main view through the runtime. The rest of P11b runs
