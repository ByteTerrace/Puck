...
project. See [Contributing to Puck](development/contributing.md) for verification procedures.
...
project's own `<PuckLayer>`/`<PuckKind>` declaration, and
`puck architecture --check` (run by `verify.yml`'s `ledgers` job and
`Puck.Cli.Tests`) fails when the checked-in block disagrees with the
declarations. Do not hand-edit it; change the declaration and paste the
regenerated block over it.

```text
Optional extensions      Puck.Mcp  Puck.World.AgentBridge
...
The build enforces the rules above. `build/Puck.Architecture.targets`
...
| `PUCKARCH006` | A resolved Puck assembly that arrives by a route other than a project reference (a raw `<Reference>`, or a package)—the route is refused before its layer is ever considered. |
| `PUCKARCH007` | A resolved reference that declares no `<PuckKind>` at all—outside the gate's scope, so depending on it imports the freedom the gate never checked. |
| `PUCKARCH008` | A project's compiled output references an assembly `PuckArchitectureDeniedApi` denies it, even though nothing in its declared or resolved references would have caught the edge. |
...
| `Puck.Storage` | The Azure object-blob store behind owned-world cloud sync: a routed byte store with a version-token seam (opaque read token + optional if-match write) for conditional overwrites, and an address projection for the platform's edge namespace versus a raw dev/emulator account. Also owns the storage-account address routing both platform composition roots share: `PartitioningOptions` (the bucket count, account-index offset, endpoint format, and the anchor partition every user's published content lives on), `PublicStorageOptions`, and the `IPartitionResolver`/`DefaultPartitionResolver` pair that routes an oid through `Puck.Maths`' `MonotonicPartitioner`—one router for the silo, the edge, and the browser's WASM build. |
| `Puck.Azure` | The Azure hosting glue both platform composition roots share, and nothing else: `TryAddDataProtection` (the shared key ring persisted to blob storage and wrapped by a Key Vault key, falling back to an ephemeral local-development ring), its `DataProtectionHealthCheck` round-trip probe and `StaticKeyResolver`, and the on-behalf-of credential surface (`OnBehalfOfOptions`, `ToOnBehalfOfCredential`, and the `IdentityUtilities.ClientAssertionCredentialKey` the assertion credential is registered under). Carries the Data Protection, Key Vault, and `Azure.Identity` packages so neither `Puck.Storage` nor `Puck.Maths` has to. |

## Shared substrate

| Project | Responsibility |
|---|---|
| `Puck.Hosting` | Recursive `IRenderNode` hosting, capability propagation, terminal ownership, fixed-step simulation context, frame timing, and cross-thread publish buffers. Authenticated local Console sessions and completed capture use Networking framing and capability authentication. |
...
| `Puck.State` | The state and rule engine as a standalone deterministic library, with no world, body, rendering, or presentation concept: the state section contract and vocabulary (`IStateSection`, `StateRow`/`StateCell`, the advance/dynamics/cycle traits, `StateDomain`, phases, visibility, the catalog and its handles, `StateReader`, `StateCellWriter`), the lattice topologies and their compiled adjacency (`LatticeTopology`/`CompiledTopology`/`TopologyCompilation`), authored randomness (`Draw`/`StateGenerator`/`GeneratorEngine`), patterns (`PatternRow`/`CompiledPattern`), `DynamicsRow`, `CompiledTable`, the postfix expression vocabulary (`ValueExpression`/`ValueToken`) and its infix spelling (`ExpressionSpelling`), the compiled opcode enum and the allocation-free Int/Q48.16 evaluator (`ExpressionOp`/`ExpressionArithmetic`), the `puck.table.v1` lookup-table document and its canonicalizer, the `StateTransform` union, the validated-identifier family (`SafeName`/`CellName` and the owned-world file-name mapping), the reserved fact channels (`RuleFacts`) a host answers, and the comparison/trigger-mode vocabulary (`ActionStateComparison`, `ActionTriggerMode`) every gated trigger in the engine shares. `Puck.World.Schema` consumes and extends it for the world document through registries and derived records, never through edits inside it. Packable as `ByteTerrace.Puck.State`. |
| `Puck.Transpiler` | The Puck DSL's language core, with no knowledge of any particular document schema: the syntax tree (`Ast/`), the Parlot-based parser (`Parsing/`), diagnostics and source spans (`Diagnostics/`), the text formatter (`Formatting/`), the import resolver (`Modules/`), the extension seam (`Extensions/`), `CompilationResult<T>`, `PuckDslVocabulary` (the DSL's spelling of `Puck.State`'s comparison and cell-kind enums), and `UnitConversion`/`UnitDimension`—what a `deg`, `ms`, `cm` or `%` suffix is WORTH, which is the same wherever a number is authored. It names `Puck.State` because the language hands an operand span to `ExpressionSpelling` rather than re-parsing one (rule 8), and nothing else. Which of a document's own fields carries which dimension is that document vocabulary's table, never this one's. Packable as `ByteTerrace.Puck.Transpiler`. |
| `Puck.GamingBricks.Transpiler` | The `puck.cartridge.v1` vocabulary for the same language: `CartridgeDocumentEmitter` lowers a parsed document to canonical cartridge JSON both brick forges consume, `CartridgeDecompiler` writes one back out as source, and `CartridgeOperand` is the one reader and writer of a cartridge operand—an expression in its infix spelling for a read, a state-and-key pair for a write. It references `Puck.GamingBricks.Forge` and `Puck.Transpiler` and no world schema, so it stays as narrow as the forge package it sits beside. Packable as `ByteTerrace.Puck.GamingBricks.Transpiler`. |
| `Puck.World.Transpiler` | The `puck.world.def.v1` vocabulary on top of `Puck.Transpiler`: lowering a parsed document to the world JSON (`Lowering/`, including the field-name-to-dimension table the core's unit conversion is driven from), decompiling JSON back to `.puck` (`Decompiler/`, one-way by design—`let` and `template` do not survive), the multi-document composer (`Composition/`), the reference lint and semantic validator (`Validation/`), the world-aware language server (`Lsp/`), and the wasm addon inspector (`Addons/`). |
...
| `Puck.World.Server` | The authoritative world runtime, physically split out of `Puck.World` alongside `Puck.World.Schema`/`Puck.World.Protocol`: `WorldServer` (the fold, authority-operation serialization, the mutation journal, snapshot emission), `WorldGrants`/`WorldHandleTable`, `WorldPopulation`/`WorldBody` (the entity table and its per-tick advance), the compiler-derived adjacency overlap/contact field and FED3 federation codec (source-scoped escrow plus human/autonomous entity admission), `WorldOwnedWorlds` (the owned-world identity catalog and its cross-document durable-state door), the addon host seam (`IWorldAddonHost`/`WorldAddonReceipt`—the mounted guest runtime lives in `Puck.World.Addons`, which references this project rather than the reverse), the collision/contact/solid-field support a body advances against, and the true-deterministic-replay codec (`WorldReplayTape`/`WorldReplaySnapshot`/`WorldReplayVerdict`/`WorldReplayRefusal`/`WorldReplayCodecException`). Machine admission receives the runtime host's explicit `IMachineValidationCatalog`; `WorldMachineCatalog` lives in `Puck.World.Machines` with no process-global registration. Post-render validation still uses `WorldPostRenderExtensions` in `Puck.World.Client` over the deployment's shader manifests. References `Puck.World.Schema`, `Puck.World.Protocol`, `Puck.Attestation`, `Puck.Networking`, `Puck.Physics`, `Puck.SignedDistance`, `Puck.Audio` (`WorldMachineHost` boots every machine at `MachineAudioRate.SampleRate`), `Puck.Storage`, `Puck.Hosting`, `Puck.Shaders`; generic contact math and the body motion-program core live in Physics while Server retains pair selection, authority, grounding/walkability, obstruction reporting, and the per-phase execution of a compiled program over the body state it owns. Same presentation denials as `Puck.World.Schema`. |
| `Puck.World.Addons` | The mounted addon guest host, physically split out of `Puck.World.Server` so the authoritative server carries no WASM/Wasmtime surface of its own: `WorldAddonRuntime` (the `IWorldAddonHost` implementation—mounting, the three tick-boundary pump points, lifecycle verbs), `WorldAddonMutationDecoder` (the addon mutation seam's stage 6 decode), `WorldAddonWire` (the World-side ABI vocabulary mappings), `AddonMutateRefusal` (the `addon.mutate` door's cataloged refusal reasons), and `AddonSimulationPump` (the crossing from a guest's decoded cells to typed, vocabulary-validated submissions—`AddonActSubmission`/`AddonQuerySubmission`/`AddonAskSubmission`; whole-batch refusal; authority stays the consumer's). Dedicated exclusively to scripting guests; carries no emulator dependencies. References `Puck.World.Server` (the seam it implements), `Puck.Scripting` (the WASM guest ABI), `Puck.Maths`, `Puck.Assets`. Same presentation denials as `Puck.World.Schema`. |
| `Puck.World.Machines` | The engine-neutral screen-machine host, physically split out of `Puck.World.Server` and `Puck.World.Addons`: `WorldMachineHost` (the `IWorldMachineHost` implementation—booting, stepping, cable-linking, memory-peeking, two-phase transactional prepare/commit/finish lifecycle), `WorldMachineCatalog` (immutable host-selected engines and neutral content providers), and symbol resolution for cartridge variables. References `Puck.World.Server`, `Puck.Audio`, `Puck.Assets`, `Puck.Abstractions`, `Puck.Maths`; machine and content-provider contracts come from Abstractions. |
...
| `Puck.World.Silo` | The second substrate driving the moved `Puck.World.Server` host engine (`WorldInstanceHost`, boot-free): an Orleans silo (`--silo <puck.silo.def.v1 path>`) whose grains (`WorldGrain`, keyed by owner oid + world id) carry activation lifecycle only—game traffic never rides the Orleans wire, only the same `WorldPeerCall` the desktop uses. `WorldSiloHost` owns one boot-free `WorldInstanceHost`, an activation mailbox drained on the one tick thread `Puck.Launcher`'s headless terminal pumps, and per-row federation identity/checkpoint bookkeeping. `SiloCommandModule` carries `silo.status`/`.grains`/`.publish`/`.activate`/`.deactivate`/`.checkpoint`. References `Puck.World.Server`, `Puck.World.Protocol`, `Puck.World.Schema`, `Puck.World.Console`, `Puck.World.Client`, `Puck.World.Addons`, `Puck.World.Azure`, `Puck.Actors`, `Puck.Commands`, `Puck.Hosting`, `Puck.Launcher`, `Puck.Storage` (`Puck.Networking` only transitively, through `Puck.World.Server`). |
| `Puck.World.Browser` | The engine embedded in a browser tab: a `browser-wasm` publish of `Puck.World.Schema` + `Puck.State` behind a JSON-string `[JSExport]` surface (`Version`/`Parse`/`ParseFragment`/`ComposeTree`/`Canonicalize`/`Compile`/`Release`/`Rows`/`Rebind`/`Judge`/`ReadRow`/`WriteRow`/`Evaluate`/`BoardMask`/`StateHash`/`Cells`)—the studio's (`src/Puck.Dashboard`) native validator and rule tick, replacing its TypeScript twin, through the TypeScript facade at `src/Puck.Dashboard/src/portal/src/native/`. `Engine/` is pure C# (no JS-interop attribute), linked as source into `tests/Puck.World.Browser.Tests` since a browser-wasm exe cannot be referenced as an ordinary assembly; `Exports/` is the `[JSExport]` marshalling shim alone. Installs its own minimal `WorldExtensionVocabularyHook`/`WorldProbeVocabularyHook` wiring answering every predicate `null` (this build ships no emulator core, shader catalog, or probe kind of its own), which the validator routes to a `deferred` notice rather than a refusal or a silent pass—a document naming a `screens[].source.machine` engine (arcade cabinets in the shipped island) parses and validates, its unregistered engines named in `deferred[]` instead of crashing on an uninstalled hook or refusing outright. Same lane-profile discipline as every other exact-equality row: Schema's own closure plus `Puck.World.Schema` itself, no `Puck.World.Protocol`/`Puck.World.Server`/presentation/backend edge. |
...
| `Puck.Actors` | The Orleans actor tier: one `UserGrain` per user, keyed by Entra `oid`, owning the provisioning lifecycle with reminder-driven, verified retries. An ASP.NET minimal API co-hosts with the silo and exposes the internal `ensure-provisioned`/`provisioning-state` contract the `Puck.Azure.Functions` edge proxies to. References `Puck.Azure`, `Puck.Maths`, and `Puck.Storage`. |
