...
| `Puck.Actors` | The Orleans actor tier: one `UserGrain` per user, keyed by Entra `oid`, owning the provisioning lifecycle with reminder-driven, verified retries. An ASP.NET minimal API co-hosts with the silo and exposes the internal `ensure-provisioned`/`provisioning-state` contract the `Puck.Azure.Functions` edge proxies to. References `Puck.Maths`. |
| `Puck.Azure.Functions` | The public Azure Functions edge: self-onboarding (`SelfOnboard`/`SelfOnboardStatus`, proxying to `Puck.Actors`), key attestation, blob SAS generation and load-balanced blob endpoints, guest access, publishing/unpublishing and listing public files, feature checks, and health checks, plus a blob-created Event Grid trigger. References `Puck.Attestation` and `Puck.Maths`. |
