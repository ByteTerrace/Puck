...
| `Puck.State` | The state and rule engine as a standalone deterministic library, with no world, body, rendering, or presentation concept: the postfix expression vocabulary (`WorldValueExpression`/`WorldValueToken`) and its infix spelling (`WorldExpressionSyntax`), the compiled opcode enum and the allocation-free Int/Q48.16 evaluator (`WorldExpressionOp`/`WorldExpressionArithmetic`), the `puck.table.v1` lookup-table document and its canonicalizer, the `WorldStateTransform` union, the validated-identifier family (`WorldSafeName`/`WorldCellName` and the owned-world file-name mapping), and the reserved fact channels (`WorldRuleFacts`) a host answers. `Puck.World.Schema` consumes and extends it for the world document; every type keeps its `World` prefix until the charter's last phase. Packable as `ByteTerrace.Puck.State`. |
