...
```text
Composition roots        Puck.Launcher.Stub  Puck.Networking.Host  Puck.World
                         Puck.World.Silo
Validation               Puck.AdvancedGamingBrick.Post  Puck.GamingBricks.Post
                         Puck.HumbleGamingBrick.Post
Engine services          Puck.AdvancedGamingBrick  Puck.Audio
                         ...
                         Puck.Text  Puck.World.Addons  Puck.World.Client
                         Puck.World.Console  Puck.World.Forge
...
| `Puck.Shaders` | Shader sets as data: the `puck.shader.v1` manifest (stages, bindings, config schema, push-constant block and its sources), its loader and config binder, the shipped-set catalog, and `FullscreenPassNode`, which runs a graphics set as one pass over an inner render node. Also compiled shader-bytecode loading, format detection, and validation, and the shared HLSL-to-SPIR-V/DXIL build recipe. References `Puck.Abstractions`, `Puck.Assets`, `Puck.Hosting`. |
| `Puck.Networking` | The dialect-agnostic wire substrate: the socketless frame grammar (`FrameCodec`, `[u32 length][u8 kind][payload]`) and the bounded forward-only reader/writer pair (`WireReader`/`WireWriter`) every socket frames its bytes through, plus the named refusal vocabulary and the async stream framing built on them. Also carries the generic Hello/identity handshake grammar (`HandshakeWireFormat`), the challenge/proof authentication contract (`IAuthenticator` — a verified identity is what the proof itself derives, never a caller's assertion), the persistent authenticated request/response lane state machine (`PersistentRequestLane`/`ILaneProtocol`), and, in `Puck.Networking.Peers`, the symmetric peer substrate: `Peer`/`PeerLink` over the `IPeerTransport`/`IPeerListener`/`IPeerConnection` seam (streams plus a datagram slot), `QuicPeerTransport` (`System.Net.Quic`, TLS 1.3 with a certificate on both sides), a handshake that binds the offered identity to the key the transport proved (`ChannelUnbound` otherwise), and `Puck.Attestation`-signed messages verified against the identity proved at handshake; a refused message names its `PeerRefusal` and leaves the link open. No server role. Carries no document or protocol vocabulary of its own; depends only on `Puck.Maths` and `Puck.Attestation`. |
...
| `Puck.Physics` | Deterministic fixed-point simulation kernels: an exact pairwise gravity oracle, reusable Barnes–Hut monopoles, adaptive dual-tree FMM with M2M/M2L/L2L passes, shared compound body-collider vocabulary, dynamic and analytic-static contact geometry, the `IContactField` seam a grounded body resolves against with both providers behind it (`FixedStaticContactSolver` relaxes against a collider set; `FixedFieldContactSolver` measures against a scalar field through `Puck.Maths`' seams), and a temporal-substep sequential-impulse rigid solver with persistent manifolds. It owns no world schema, authority, walkability policy, or presentation seam and depends only on `Puck.Maths`. |
...
| `Puck.Networking.Host` | A thin console demonstration of `Puck.Networking.Peers` over `QuicPeerTransport`: `--listen <port>`/`--dial <host:port>`/`--key <file>` at startup, then stdin verbs (`dial`, `send`, `peers`, `quit`) and stdout lines (`peer.id`, `listening`, `link.up`, `link.down`, `recv`, `refused`, `handshake.refused`) for every event. Same binary on every side; no role flag. |
