# APU and sound channels

The Humble Gaming Brick (HGB)'s audio processing unit (APU) generates two pulse
channels, a programmable wave channel, and a noise channel. Their registers,
frame sequencer, and output mixing are part of emulated state; host resampling
is performed after the core boundary.
...
- **Wave RAM access corruption**: On physical DMG hardware, accessing Wave RAM
  while Channel 3 is active can trigger bus conflicts. Puck models this
  behavior in `ApuComponent.cs`; the relevant Post and focused tests establish
  the cases covered by the repository, rather than a blanket claim about every
  hardware revision.
...
- **`NR50` (`0xFF24`): channel volume and Vin**:
