...
**The creative loop.** The owner-ratified charter (2026-08-06) is four worlds, no others: `play` — the overworld hub, the game's first main city, an MMO plaza local multiplayer shares — and three instanced "dungeons" reached from it, `dive`, `kart`, `jump`, each behind a wall-mounted picture-frame portal standing in the plaza. Walking to a frame swaps the world underneath with no restart, never a loading menu. Design is feel-first: a dungeon earns its place by how it feels to move through before any scoring or progression system is layered on top. The bootable-cabinet arcade room this section described before 2026-08-06 — an intro cartridge, a win condition, a fourth-wall break into a room of machines — is superseded; boot loads directly into `play`, and later a diegetic moment is meant to hand you the editor, which stays always-on for developers and agents. From inside `play` you will sculpt a creature, animate it, bake it into a cartridge, and place it in a dungeon. The sculpt workbench, the timeline, the IK rig, and the cartridge forge all exist; none is yet hosted in the running game, and no plan of record sequences bringing them over.
