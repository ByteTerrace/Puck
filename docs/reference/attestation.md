# Offline attestation and key-bound identities

Puck.Attestation carries signed claims that can be verified without contacting the
issuer. An **attestation** says “this identity made this claim” and carries
...
This reference defines the verification model and contracts. The
...
- *Key-bound identities:* an identity includes the domain root’s SHA-256
  ...
  display name.
- *One attestation and one verification path:* claims and key bindings use the same
...
A binding and a claim use the same attestation. Under the mandatory base profile,
...
```mermaid
graph TB
    subgraph Attestation ["✉️ Signed attestation"]
...
var received = AttestationProfile.Base.DecodeAttestation(
    codec: codec,
    wire: wire);
...
| `TrustList` / `TrustListEntry` / `AttestationTrustMode` | Define which keys or roots the receiver trusts, their reach, and their age and replay policies. |
| `AttestationProfile` / `AttestationExtensions` / `AttestationResourceLimits` | Select the mandatory base profile, optional sealed payload support, and fixed resource ceilings. |
...
Claims and key bindings use one attestation. The signed context includes the
...
algorithm from the pinned identity and checks that the attestation agrees; message
...
Attestation performs no path discovery and fetches no missing certificates or
bindings.
...
Sealed attestation encrypts a payload for a recipient by using an ephemeral ECDH
...
confidentiality but not sender identity. Sign an attestation around the sealed
...
[Signed attestation](#signed-attestation) above: the byte layout, the canonicality
...
The mandatory profile is **`attestation-v1-base`**, and this implementation
supports all of it:
...
| Extension | Adds |
|---|---|
| `sealed-attestation-v1` | payload kind 3 and the sealing scheme in §14 |

A verifier selects its profile out of band. **No attestation, algorithm field,
...
not weaken §6 or let an attestation choose an algorithm other than the one
...
| Item | Maximum |
|---|---:|
| complete encoded attestation | 65,536 bytes |
...
The complete-attestation ceiling MUST be checked before parsing. The remaining
...
the **attestation**, the **signed portion**, and the **payload**.

```cddl
attestation = [ signedPortion: bstr, signature: bstr ]
...
payloadKind 1 (opaque) is caller-defined bytes. The attestation codec and verifier
...
`purpose = "key-binding"` and `payloadKind = 2`. One attestation means one verify
path.
...
are both valid over the same message, so two distinct attestations can carry one
...
**Purpose** separates signature *uses*. `key-binding` is reserved: an attestation
...
### 11. Verifying one attestation

Given an attestation's wire bytes, a pinned id, that key's SPKI bytes, and the
...
**Step 3's right-hand side is normative, and reading it as the attestation's own
...
Inputs: the attestation carrying the claim, its chain (0 or 2 bindings), a trust list, `now`,
...
### 14. Sealed attestation

The same attestation with `payloadKind = 3`: the payload uses authenticated
...
Because none of it is observable from a signed attestation alone,
`tests/Puck.Attestation.Tests` pins the derivation against an independently minted
BindingAttestation known-answer attestation. Its test-only independent implementation
...
- **A mutated-but-parseable demonstration.** Wire bytes that differ from an
  honest attestation, still decode, decode to the **same model**, and MUST be
...
attestation's. Puck's routine suite includes both the deliberately split in-memory
