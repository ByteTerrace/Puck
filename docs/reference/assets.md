...
oracle, and it refuses the modes it does not read: BC6H's two-region modes,
which its encoder never writes. BC7's partitioned modes fit the
`Bc7Codec.PartitionCandidates` partitions whose subsets vary least, ranked by
exact integer variance with the lower partition number first among equals;
modes 0 to 3, which hold no alpha, are tried only for a block whose alpha is
255 throughout. A tie in decoded error keeps the earlier mode in the order 6,
5, 7, 3, 1, 2, 0.
