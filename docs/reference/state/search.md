...
When a judge rule or score reads `$tick`, the job also folds the simulation and
engine tick pair into its cache keys and restarts on either change. A row with
a value-over-time trait has the same effect. Jobs that read neither time source
keep their progress across ticks, so ordinary searches do not restart merely
because the host advanced its clock.
