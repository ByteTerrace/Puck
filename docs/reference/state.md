...
Puck.State evaluates rules over named rows of data. Use it to build a card game,
a turn-based resolver, or another deterministic simulation with its own host.
The library provides the state vocabulary, rule compiler, evaluator, and search
runtime. Your host decides how mutations become authoritative, how time advances,
and how state is persisted. Puck.World.Schema extends this vocabulary for worlds.

A **row** gives related values a name, a kind, and an addressing shape. A
**cell** holds one value within that row. A **rule** reads those cells, tests a
condition called its **gate**, and applies effects when the gate holds. Numeric
expressions use integers or Q48.16 fixed-point values from [Deterministic
numerics](maths.md); rows also support booleans and text.

Start with [Evaluate a small rule](#evaluate-a-small-rule). Use the reference
sections to look up [rows and cells](#rows-and-cells),
[expressions](#expressions), [evaluation](#evaluation),
[frames](#the-store-and-the-frame), or [host extensions](#how-a-document-project-extends-it).
For world-specific fields, follow the [world schema](../../src/Puck.World.Schema/README.md).
...
The catalog resolves names before evaluation; compiled handles belong to that
catalog. The frame holds the candidate values separately from the authored rows,
so this evaluation leaves the row's original value at two. Loading a frame from
the row store supplies its starting values. With no gate, the rule always fires;
with no explicit mode, it uses Level and fires on every evaluation.
...
frame](#the-store-and-the-frame); it refuses cell removal and generator draws,
...
The following sections describe the shared contracts a document host can use.
...
`LatticeTopology` defines a discrete space (`grid`, `ring`, `hex`, `box`, `graph`, `tiling`
...
`ValueExpression` and its `ValueToken` postfix vocabulary,
...
`Puck.Maths`: `pair`/`pairX`/`pairY` and the pairing operations (`pairSwap`,
...
`OperandFact` (`StateCellOperand`, `BindingOperand`,
...
`RuleEvaluator` runs a compiled family over an `IRuleHost`—
...
`StateStore` is where a read finds a cell's
stored value beneath the rows' structure—`RowStore` over a section's own
cells (`IRuleReader.Store`), or `StateFrame`, the framed cells of a section
...
every framed row carries a version—a counter
...
The `StateTransform` union describes `transfer`, `setRay`,
`shuffle`, `sortZone`, `sortKeyed`, `writeSet`, `boardCombine`, `arrange`,
`push`, `clearEnclosed`, and `observe` operations. `ZoneSelector` and `SortKey`
...
`RuleFacts` defines the state-neutral `$`-prefixed channels
...
- [State test suite](../../tests/Puck.State.Tests/README.md) — scope and run instructions.
