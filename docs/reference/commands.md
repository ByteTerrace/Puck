...
Start with [Quick start](#quick-start) to route one key press. Then read
[Values](#values), [Maps](#maps), and [Bindings](#bindings) for device input, or
[Who can dispatch a command](#who-can-dispatch-a-command) for text sessions,
authority, ordering, and refusal behavior. The [core types](#core-types) table
provides a map of the API.
...
Keep two questions separate: *which simulation slot changes?* and *who is
acting through it?* `CommandContext.Slot` answers the
...
This example follows one key press into a handler. Use a console project
referencing Puck.Commands; the final simulation-interface example also needs
Puck.Hosting. Define one command per action, whatever drives it:
...
supplying its registry before accepting authored bindings. Bindability validation limits which commands a profile can name. Handlers
still enforce the principal’s authority for the operation.
...
The [command test suite](../../tests/Puck.Commands.Tests/README.md) owns the
coverage map and run instructions. The [project map](../project-map.md) and
[project declaration](../../src/Puck.Commands/Puck.Commands.csproj) own dependency
information; [release guidance](../development/ci.md) owns package publication.
...
- [State and rules](state.md) — evaluating rules after input dispatch.
- [Deterministic numerics](maths.md) — representing simulation values.
