...
| Uniform grid / spatial hash instance cull | build the grid, not the BVH — but measure-gate it | M | measure `PUCK_TIMING=1` beam-ms vs. views-ms on the town overview; promote only if beam-ms dominates and grows with instance count | review-03 (Leg 2) |
| Two-level BVH / TLAS-BLAS instance acceleration | recommended against for ≤1024 dynamic analytic instances | L | — | review-03 (Leg 2) |
| Third pyramid level (64×64 pre-beam, DIST coarse-to-fine schedule) | defer to measurement | M–L | uniform grid (Leg 2) landed first; only if beam-ms still dominates after Legs 1+3a+Leg 2 | review-03 (Leg 3b) |
...
| Forward-mode analytic gradient dual (per-op micro-code, hit-only specialization) | adopt — and make it the design of the planned gradient accumulator | L | keep as a strictly parallel channel to the distance accumulator; hit-only second `mapCore` specialization to protect march occupancy | review-07 |
