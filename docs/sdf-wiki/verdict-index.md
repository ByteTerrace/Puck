...
| MTSDF-alpha as the marchable glyph field source (pre-baked, loader path) | adopt-now — sample the true-distance **alpha**, never the median (C2) | S–M | pre-baked MTSDF atlas (`Puck.Text` loader already speaks the JSON) | review-text-A |
| Extruded-slab proxy + box-fallback combine for band-limited atlases (C3) | adopt-now — **required**, not optional (atlas fields saturate outside ±pxrange/2) | rides Tier-1 | true-channel extrude + conservative underestimate handoff | review-text-A |
| Engrave/emboss glyphs as CSG (Subtraction / blend) | adopt-now — the near-novel Puck move; correctness spec = C1/C2/C3 | M | analytic normals (`ce36f80`); the Tier-1 glyph op | review-text-A |
| Label composition: emboss/engrave, NEVER coplanar (coincident-surface speckle, observed live 2026-07-09) | adopt-now — posture for every label build | S | the Tier-1 glyph op | review-text-A |
| Contrast patterns for floating text (backing plate; onion-shell outline in a contrasting material) | adopt-now with the Tier-2 action bar | S | Tier-1 op + material choice | review-text-A |
| `GlyphDecal` — material-level atlas text on a host surface (dense reading text; median-of-3 LEGITIMATE here — 2D coverage is its designed job) | gated-on the UI build needing dense text | M | the `ScreenSlab` material-sampling precedent | review-text-A |
| Runtime coverage→chamfer SDF generator | gated-on ÷1.0824 fold OR exact-EDT swap (C1: chamfer is Lipschitz-1.0824, overestimates) | S–M | fold the 1.0824 factor or swap Felzenszwalb-Huttenlocher EDT; no-toolchain fallback only | review-text-A |
| Analytic-dual perspective AA for world text (not `fwidth`) | adopt-now — exact distance-per-screen-pixel from `mapGradCore`, `fwidth` is a parity hazard | S | rides shipped Tier-0 coverage AA + forward-mode dual | review-text-A |
| Slug (per-pixel winding coverage from outlines) | reject — architectural (coverage, not distance); patent public-domain 2026-03-17, so **not** a licensing reject | — | record as the now-license-free flat-text quality ceiling | review-text-A |
| Pathfinder 3 / Vello / Loop-Blinn (coverage/framebuffer rasterizers) | reject — architectural (coverage into a framebuffer, nothing marchable) | — | — | review-text-A |
| Neural glyph fields at runtime | reject — non-deterministic cross-backend NN eval; defensible only as an offline baker | — | offline-baker-feeding-atlas path only | review-text-A |
| Enrichment transform/param/frame-channel effects (shake/wave/pulse/outline/dilate) | adopt-now — cheap class on the post-grid-cull budget | M | the determinism fix below; per-instance transform/color channels on glyph instances | review-text-B |
| Determinism fix: `TimeSpan.TotalSeconds` → produced-frame/content-tick | adopt-now — small and mechanical; effect math already RNG/history-free | S | — | review-text-B |
| Markup: BBCode front-end compiling to the control-char stream + span side-table | adopt-now — keep the robust prior-art stack parser; span table for console/JSON | S | — | review-text-B |
| Deterministic typewriter/reveal (tick-driven appearance/persistent/disappearance) | adopt-now — reveal = pure function of content-tick (`floor((tick−start)·charsPerTick)`) | S–M | the determinism fix | review-text-B |
| Glyph morphing (`lerp(dA,dB,t)` between glyph fields) | reject/defer — naive SDF interp violates SDF properties (ghosting/pinching/phantom zeros) AND per-frame-rebuild class | L | — | review-text-B |
| Reduced-motion (run-doc field + `text.motion off|reduced|full` verb) | adopt-now — WCAG 2.2.2/2.3.1/2.3.3; damps motion, keeps static enrichment | S | env→verb doctrine | review-text-B |
| Delight doctrine — restraint (sparse, semantic, event-driven; motion opt-in) | adopt-now (posture) — DELIGHT ≠ MOTION; shapes every future text brief | S | — | review-text-B |
...
Each row above is a one-line ruling; the full reasoning, cost/benefit derivation, determinism analysis, and implementation seam for every technique lives on its topic page — [gradients-and-normals.md](gradients-and-normals.md), [shading-ao-shadows.md](shading-ao-shadows.md), [materials-and-primitives.md](materials-and-primitives.md), [tape-pruning-and-inclusion.md](tape-pruning-and-inclusion.md), [hierarchical-and-instance-acceleration.md](hierarchical-and-instance-acceleration.md), [march-loop-scheduling.md](march-loop-scheduling.md), [lod-and-bounds.md](lod-and-bounds.md), [antialiasing-and-filtering.md](antialiasing-and-filtering.md), [lipschitz-and-field-correctness.md](lipschitz-and-field-correctness.md), and [text-and-glyphs.md](text-and-glyphs.md) — grouped by cluster rather than by source review. For the techniques ruled out entirely (DD-family smins, persistent-threads work-stealing, the two-level BVH, Tier-2 transparency accumulation, the full 27-op interval interpreter, and the baked/discretized/temporal families triaged out before Phase 3), see `negative-results-and-rejections.md`. For the ranked priority shortlist across the whole survey — what to build first given everything in this index — see [../sdf-sota-survey.md](../sdf-sota-survey.md).
