...
| [sdf-bench-notes.md](sdf-bench-notes.md) | The `sdf.bench` perf-bench measurement chronicle (per-primitive, per-op, instance-sweep, and carve-ladder GPU cost tables), driven from inside the `sdf` fullscreen debug mode. Carries the beam-slope verdict AND its 2026-07-09 resolution: the O(instances) cost was the cone march's per-sample enumeration, felled by the mask-first uniform-grid instance cull (sweep beam+mask 187.8→6.6 ms @4096); plus the measured destruction budget and the snapped-camera instrument fix. |
| [sdf-backlog.md](sdf-backlog.md) | The ONE living ledger of open SDF work — perf levers (reranked for the views-bound reality), quality items, the authoring-surface gap, Post coverage gaps, live defects (the horizon-notch writeup of record), and cleanup seams. Distilled 2026-07-09 from the retired accumulator-plan / vm-briefing / vm-evolution-plan docs (full texts in git history). |
