...
| [examples/](examples/) | Twenty valid `puck.run.v1` run documents plus one checked-in negative (`world-single-bad-material.json`), all exercised by the Post `run-document` stage. The `world`-graph documents also run live (`--run <path>`, host backend) through the shared world renderer, not only as parse corpus. |
...
| Doc | Scope |
|---|---|
| [overworld-demo-plan.md](overworld-demo-plan.md) | The demo's plan of record — the overworld (room + bootable console stands + staged layout transitions), its seams, and the next steps. Covers multiplayer console mode + proximity takeover, battery saves, and `--rom` fourth-wall boots. **The Demo is GREENFIELD** — verify by running it, not by gating; don't promote its features into Post unasked. |
