...
backend's requirements before choosing a device. Live shader-pipeline editing
compiles with the same `dxc`; the
[shader compiler guide](reference/shaders.md#one-off-shaders) explains
how it is resolved.
