...
### R12 — RETIRED by R18 (2026-07-20)

R12 required every new document section to be a **nullable** property with
`[JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingNull)]` and a `??`
coalesce at every read, so that a world authoring no such section carried no key
and the shipped worlds stayed byte-identical.

**That is the one thing R18 rule 3 says is fine.** The mechanism bought nothing
but the byte check it was built to satisfy, and it cost the repo two competing
...
## Greenfield refactor (2026-07-20)

The owner's directive of 2026-07-20 — *"hunt down the locations where we've made
poor decisions because of concerns about breaking existing clients or violating
goldens; ruthless refactor until we reach our normal greenfield based quality
level"* — landed as a sequence of units on this branch. The units that touch the
world document are recorded here; the rest are self-describing commits.
