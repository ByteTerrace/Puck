...
Puck represents an SDF scene as a short program that the GPU interprets per
pixel. Four instruction families operate on one running accumulator: union
can add a candidate without disturbing earlier geometry, while intersection
can discard it. Field scopes isolate operations that need their own accumulator;
instances and materials travel in the same stream. A Lipschitz step clamp keeps
marching safe, and the ISA admission rule keeps the instruction set correct and
small.

## An SDF program is content, not a mesh
...
uses the same field semantics on both GPU backends and can be re-evaluated on
the CPU for physics queries (see [Queries and determinism](queries-and-determinism.md)).
Backend presentation differences are covered by parity evidence. The whole
engine is an interpreter and a compiler for this one little language.
...
(see [Bricks and baking](bricks-and-baking.md)). A shape reads the current transformed point, computes its own
...
## Field scopes isolate composition
...
free. The shading pass ([Lighting and shading](lighting-and-shading.md)) takes the
winning material and lights it.

## Instances place one program many times
...
- **Static** instances are placed once and don't move—a forest of rocks, a
  field of carves. They bin into the spatial grid ([SDF frame rendering](frame-rendering.md))
...
## The ISA admission rule keeps the instruction set small
...
- The accumulator rule, intersection annihilation, and the `Xor`
  exemption as working constraints: [the rendering skill](../../../../.claude/skills/rendering/SKILL.md)
...
- The ISA admission rule and the partners a new instruction touches:
  "Changing the instruction set" in [the rendering skill](../../../../.claude/skills/rendering/SKILL.md),
  with every C#↔HLSL coupling in its [sync pairs](../../../../.claude/skills/rendering/references/sync-pairs.md).
- Capacity growth and the composition probe: "Capacity and emission" in the
  same skill.
