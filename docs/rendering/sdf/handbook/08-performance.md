...
Frame time is distributed across the fixed render passes, and its shape reveals
the bottleneck. A scene can be *views-bound*, *beam-bound*, or *pace-bound*;
each regime responds to a different measurement. Every VM operation also
contributes to interpreter occupancy, even when a program does not use that
operation. The benchmark method described here located a hidden cost in
destructible geometry by measuring slopes and matched toggles.

The historical measurements below use the reference GPU—an RTX 4070 with
Vulkan, 1280×800, native render scale, and default switches. They describe the
shape and order of magnitude of the workload; another GPU can shift the
absolute values. The useful comparisons are the *ratios* between passes and
the *slopes* under load.
...
require more primary samples. See [SDF frame rendering](03-the-frame.md).
