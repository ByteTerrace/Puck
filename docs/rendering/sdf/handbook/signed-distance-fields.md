...
cases—more on that in [SDF program model](program-model.md)), so it can feed
...
[SDF frame rendering](frame-rendering.md) and the acceleration references), not a
...
The next chapter, [SDF program model](program-model.md), is
...
instruction set correct and small. [Authoring SDF scenes](authoring-scenes.md) then turns
...
- [SDF performance](performance.md)—how to measure per-pass GPU cost in
  a running world, for readers who want to see "cheap coarse passes let the
  fine march skip regions" in numbers of their own.
