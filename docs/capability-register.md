...
| Save round-trip | SHIPPED | A canonical writer (stable order, invariant numbers, LF, one trailing newline); `world.save` folds live session dimensions into their document homes without mutating live state, observed idempotent by the ouroboros round-trip (an observation, not an acceptance criterion). | — | `WorldSessionCapture.cs:23-55` |
