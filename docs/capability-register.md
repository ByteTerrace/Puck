...
| Binding document | SHIPPED | `puck.bindings.v8`: modifiers with press/release hysteresis, and chord rows that resolve to a page or a command. Chord order is significant; chord depth is unbounded by data. | 23 tracked button edges/pad | `BindingProfileDocument.cs:21`; `HeldOrderTracker.cs` |
