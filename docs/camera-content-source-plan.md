...
> **Note (2026-07-02):** the demo `--validate-camera` / `--validate-genlock` gates, `CameraValidationNode.cs`, and
> `ViewportCompositorNode` referenced below were **retired into the `Puck.Post` battery** (C9 camera-share, A5 genlock)
> when Puck.Demo's superseded conformance gates were removed — see `docs/demo-post-rework-plan.md`. The demo-only
> hardware bring-up gates `--validate-camera-live` / `--validate-camera-gpu` remain. The design content below is
