...
- **Erin Catto** — [box3d](https://github.com/erincatto/box3d) (MIT), the design reference for the rigid-dynamics re-derivation in `tests/Puck.Physics.Tests`: substep soft constraints, sequential impulses with warm starting, persistent manifolds, speculative activation, the once-per-step restitution pass, and the coupled tangent-mass friction solve with its world-space warm-start reprojection (applied per contact point here, where box3d's own manifold can group several).
