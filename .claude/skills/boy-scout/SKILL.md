...
tier three. Flags are in [the CLI reference](../../../docs/reference/cli.md#puck-scansource-sweep).
The same classification is a build gate: `CommentSmells.json` records each
file's smell count, and a count may only fall (SMELL001–SMELL004). A smell
your change adds fails the build; fix the comment rather than raising the
ledger, and run `puck comment-smells` after removing one.
...
- **A formatting sweep is its own change, never incidental.** `puck format`
  (C# and `.puck` alike) rewrites the whole tree it is given, `.` by default.
  For touched files, pass `--file-list <json>`, a JSON array of paths relative
  to the working directory, and no root; every phase stays within that
  selection, including whitespace and standalone-app formatting. Use `--check`
  to inspect drift and land broad baseline sweeps separately. CI applies the
  same selection discipline to PRs.
