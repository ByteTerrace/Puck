...
3. **Rule on every applicable surface:**
   ...
   | A checked-in `.puck` source paired with its compiled `.world.json`/`.cartridge.json`, or a companion `.md` beside either | Editing one half obliges regenerating or verifying the other (`puck compile`/`puck decompile`). Both pairs are machine-gated byte-for-byte (`CartridgeRoundTripTests`, `ShippedWorldsParityTests`), so regenerate the document with `puck compile` in the same change. A companion `.md` that quotes a `.puck` value (for example a `let` constant) goes stale the same way a README does. |
