...
3. **Rule on every applicable surface:**
   ...
   | A checked-in `.puck` source paired with its compiled `.world.json`/`.cartridge.json`, or a companion `.md` beside either | Editing one half obliges regenerating or verifying the other (`puck compile`/`puck decompile`). Both pairs are machine-gated byte-for-byte (`CartridgeRoundTripTests`, `ShippedWorldsParityTests`). World outputs listed in `build/WorldAssets.targets` are ignored and generated by the CLI build; commit their `.puck` parents, never force-add the JSON. Cartridge outputs remain tracked. Regenerate and verify the document in the same change. A companion `.md` that quotes a `.puck` value (for example a `let` constant) goes stale the same way a README does. |
