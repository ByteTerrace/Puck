...
| PUCK037/038/039 | Error | Control flow, a call-form gate, or a compound-assignment operator in a vocabulary whose rule shape can't carry it — `if` lowers to the world vocabulary's own conditional effect, so only `repeat`/`break` still fire PUCK037 there. | Restructure to the vocabulary's admitted shape (see `grammar.md`'s cross-vocabulary table), or move the logic to a vocabulary that does carry it. |
