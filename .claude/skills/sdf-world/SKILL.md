...
- **The soft-shadow march is GRID-CULLED (default ON; `sdf.shadowcull on|off`,
  ...
  `SDF_SHADOW_MASK_WORDS = ceil(SDF_MAX_INSTANCES/32)` = 1024 words, covering
...
- **Exact AO uses a complete live-instance mask.** At the same uniform seam,
  ...
  fast AO retains its camera-tile approximation. The two 1024-word masks consume
  8 KiB shared memory per workgroup. This favors avatar fidelity; dense scenes
