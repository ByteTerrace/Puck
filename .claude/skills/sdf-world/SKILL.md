...
Primary traversal and hit shading are separate dispatches. `sdf-world-primary.comp`
includes the views entry point with `SDF_PRIMARY_PASS`; ordinary views variants
use `SDF_PRIMARY_READ`. The primary kernel keeps the full ISA. Both use the same
indirect bbox, active-pixel test, view extent, and child mask. Primary writes every
active record, including misses; shade reads only active records after the
compute barrier in `RecordPrimary`. Cadence-skipped frames run neither dispatch.
`SdfWorldKernels.Primary`, its reload entry, and its ISA report must accompany
every kernel set. `primary` and `views` have separate timing marks; nine timestamp
slots cover frame start plus all eight pass closes, including skipped frames.
...
is 48: twelve words per full-extent pixel per viewport. Rows carry depth/terminal
radius/threshold/material bits; four hit lanes; then frame-slot bits/seam weight/
other-material bits/flags. Flags hold march steps in bits 0–7, primary evaluation
count in 8–15, and hit acceptance in bit 16. These widths cover MaxSteps 128 plus
its one exhaustion re-evaluation; revise packing if the budget outgrows them.
Depth and attributes are unquantized. Allocation is width × height × viewport
capacity × 48 bytes and is shared under the cross-frame barrier, like the source
textures. Check memory as well as frame time. Shader A/B can define
`SDF_MONOLITHIC_VIEWS` and pair it with an ISA-report-preserving no-op primary
kernel; do not charge an unused primary traversal to the reference. Validate
...
by the sky pre-pass.** Eight passes per frame: `sdf-frame-upload.comp` (2026-09-03:
...
119→1.0 ms @1024 scattered carves) → `sdf-cull-args` → primary → views → composite.
...
(`SdfWorldEngine.PassLabels`; `TimingCapacity` 9 covers all marks).
...
The cull-args reduction closes its own mark. Gated by
...
- **Independent primary tracing requires hard-union root composition.**
  ...
  `SdfWorldEngine.PartBoundFloatCount` / `SdfPartBoundFloatCount` are six floats
  per instance per viewport, appended after the four tile planes in `m_tileBuffer`.
  ...
  The 48-byte primary record's last word packs selected-march steps in bits 0..7,
