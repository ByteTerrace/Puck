...
| `DynamicTransformByteLength` = 48 B/slot | `sdfDynamicTransforms` (3×float4: position [.w = soft-shadow participation], quaternion, `Lanes` [x = damage, y = thrust, z/w free]) | dynamic transforms + per-instance render lanes (`Puck.SignedDistance.DynamicTransform.Lanes`) — `SDF_OP_TRANSFORM_DYNAMIC` reads the third row into `currentLanes`, the tape state `SDF_OP_LANE_ERODE` and shade-volumes.hlsli's thrust read consume; a chain riding no dynamic slot reads lane 0 |
| `SdfOp.LaneErode` (id 34) / `SdfProgramBuilder.LaneErode(lane, from, to, noiseScale, reach)` — a POINT op ordered immediately before the `SdfOp.ShapeBlend` it targets; Data0 = (lane index 0..3 — `SdfLaneIndex`, from, to, noiseScale), Data1.x = the target shape's HOST-BAKED reach (its bound radius) | `SDF_OP_LANE_ERODE` (34u), gated `#ifndef SDF_STRIP_HEAVY` — reads `currentLanes[laneIndex]`, `t = saturate((lane-from)/(to-from))` (reversed from>to runs the fold in reverse); `t >= 1` sets a cheap early-out (`SDF_OP_SHAPE` skips its own evaluation, no field cost) via `laneErodeSkipShape`; otherwise `sdfValueNoise3(localPosition*noiseScale, SDF_LANE_ERODE_SEED)` ragged-fronts `t` and `laneErodeAmount = raggedT*reach` adds to the immediately-following shape's distance-scaled candidate, consumed and cleared at that `SDF_OP_SHAPE` | per-shape lane-driven erosion — the diegetic "the body IS the health bar" carrier. Its noise term folds into `chainDisplaceWarpProduct` via `NoiseDisplaceStepFactor(amplitude: reach*SdfProgramBuilder.LaneErodeRaggedAmount, frequency: noiseScale, octaves: 1, gain: 1, lacunarity: 1)` (reach-independent, like Displace/NoiseDisplace/GaussianPush), so an authoring door emitting it takes the SAME per-shape scope those warps take (`ownScope`/`wantsErode` in `CreationStampEmitter.EmitShapeChain`/`WorldStampPool.EmitShape`). RENDER-ONLY: `SdfFieldEvaluator` does not interpret it (no dynamic-transform table in its signature to read a lane from). Classified HEAVY in `SdfViewsKernelVariants.FirstHeavyTouch`. Exposed to `puck.creation.v1` as `ShapeDocument.Erode` (`ShapeErodeDocument`). The winning candidate's `currentLanes` also ride `SdfHit.lanes` (tracked beside `material` by `sdfComposeCandidate`/`sdfComposeDualCandidate`, saved/restored across `PUSH_FIELD`/`POP_FIELD`, zero for a rigid leaf or a static chain), so `renderView` hands `hitLanes.x` to shade-wear.hlsli's `applyWear` without a second field evaluation |
...
| `SdfMaterial(Albedo, Emissive, Specular, Roughness, Sheen, Metal, Coat, Wear, Wrap, Soften, Bounce, Eye)` — packed as EIGHT uvec4 words (`MaterialVectorsPerEntry`): `m0` = albedo.rgb + emissive, `m1` = specular + roughness + sheen + metal, `m2` = coat + wear.seed + wear.scale + wear.static, `m3` = wear.chips + wear.scratches + wear.dust + wear.contact, `m4` = wrap + soften + 2 reserved, `m5` = bounce.rgb + 1 reserved, `m6` = eye.iris.rgb + eye.irisRadius, `m7` = eye.pupilRadius + eye.limbus + eye.depth + eye.fibers; `AddMaterial`/the packed `SdfProgram` constructor refuse Roughness/Sheen/Metal/Coat/Wrap/Soften/Wear's chips/scratches/dust/contact/static outside [0, 1], Wear's scale negative, Wear's seed non-finite, `Bounce`'s components negative, or an authored `Eye` (`SdfEyePaint.IrisRadius` > 0) whose pupilRadius/limbus/irisRadius are not ascending (`RequireValidEye`) | `SdfMaterialData`/`SdfMaterialWear`/`sdfMaterialLoad` (the one decode point, `SDF_MATERIAL_VECTORS_PER_ENTRY` = 8u) + `sdfGgxDistribution`/`sdfMaterialShade` in `sdf-vm.hlsli`; `applyWear` in `shade-wear.hlsli`; `sdfWrapDiffuse`/`applySoften`/`sdfEyeShading`/`sdfEyeCatchlights` in `shade-portrait.hlsli` (included after `worldStudioReflection`, since the eye catchlight reuses its per-softbox falloff) — call sites live in `sdf-world.hlsli`'s `renderView`, right before `sdfMaterialShade` (detail-resolve, wear, then eye paint, then soften, in that order, ahead of the AO/light loop so wrap/soften/eye see the resolved material before it) | GGX specular, replacing the retired Blinn-Phong lobe: a roughness FLOOR keeps a finite light's lobe from collapsing to a delta function — `alpha = sqrt(roughness^2 + SdfRoughnessFloorSquared)` (0.018), `a2 = alpha^2`, `distribution = a2/(pi*d^2)` (`d = nh^2*(a2-1)+1`), Smith-Schlick geometry `k = (roughness+1)^2/8`, Schlick fresnel `f0 + (1-f0)*(1-VdotH)^5`, `f0 = lerp(specular, albedo, metal)`; diffuse scales by `(1-metal)`. The whole GGX/coat term is skipped when `specular == 0 && metal == 0`, so a matte material pays nothing and is byte-identical to before. Coat is a SECOND GGX lobe at fixed roughness `SdfCoatRoughness` (0.25), sharing the primary lobe's geometry term, `* coat * SdfCoatScale` (0.04), divided by `4*nDotV` only (no nDotL). `SdfMaterial.DefaultRoughness` (0.20204833) is the roughness whose GGX alpha equals the Walter et al. (2007) equivalent of the retired Blinn-Phong exponent 32 — KEEP IN SYNC with `SdfRoughnessFloorSquared`; presentation-only, inside the renderer's existing per-backend specular noise. Sheen is unchanged: a fresnel edge lift, `sheen * (1 - N.-rayDir)^SdfSheenFresnelExponent`, multiplied into the material's own already-lit color. A document still spelling `shininess` is an unmapped member and is refused by name. `Wrap` widens every directional/point Lambert term through `sdfWrapDiffuse(ndotl, wrap) = max((ndotl+wrap)/(1+wrap), 0)` (0 reduces to plain `max(ndotl,0)` exactly) and relaxes ambient occlusion toward 1 by `wrap*.35`. `Soften` blends the LIT normal (AO/light loop only — the geometric/debug normal is untouched) toward a wide-stencil (0.05 creation-unit epsilon) tetrahedron field-gradient probe. `Bounce` adds `albedo*bounce*(1-max(n·key,0))*ao` after `sdfMaterialShade`. `Eye` (`SdfEyePaint`: `Iris`, `IrisRadius`, `PupilRadius`, `Limbus`, `Depth`, `Fibers`) paints an iris/pupil/limbus/fiber pattern into the albedo before the light loop and returns a sharp corneal catchlight (added beside the studio reflection); `IrisRadius` <= 0 is the off state. No shape-local frame reaches shading, so the iris plane stands perpendicular to the per-pixel view direction (a billboard — it does not turn with the character's own head); refraction (IOR 1.336) bends the local ray, and Pupil/Limbus/Iris radii compare against that ray's local xy magnitude, not a world-unit length, so their scale ties to `Depth`. Lid occlusion is out of scope. `render.grounding` (`strength`, `radius`) is carried on the SAME environment lane table appended past the studio-reflection horizon (rows 53 control, 54..57 up to 8 centres) — see the environment row below |
...
| `render.grounding` (`WorldRenderGrounding`: `strength?`, `radius?`) — appended past `render.environment`'s horizon rows (`RowCount` 53→58): `GroundingControlRow`=53 (x strength, y radius, z live centre count), `GroundingCentresRow`=54..57 (2 centres/row, xy/zw = each centre's world x/z, up to `MaxGroundingCentres`=8). `strength`/`radius` are written ONCE per revision like `render.environment` (absent leaves the lane table's already-zero baseline untouched — writing a default unconditionally would desync an absent world from `SdfEnvironment.Default()`'s pinned zero); the live centres are NOT — `WorldRenderCycleTrack.ApplyGrounding` overwrites them every `Resolve` call from the caller's `liveBodyPositions`, the SAME "resolved after the blend, never cached with statics/keys" posture `ApplyAnchors` uses for a point light's dynamic slot. `WorldFramePresenter` gathers up to 8 positions by scanning `WorldBodiesLimits.DetailedRenderBand` (not the full population ceiling) for `m_client.IsActive` | `SdfEnvGroundingControl`/`SdfEnvGroundingCentres` (offset from `SdfEnvBase`=40) + `worldGroundingStrength`/`worldGroundingRadius`/`worldGroundingCentreCount`/`worldGroundingCentre` in `sdf-world.hlsli`; the darkening block sits at `renderView`'s material-shade site, after the point-light specular loop and before the curvature stylization | a contact-darkening blob under each live body: on any upward-facing hit (`normal.y > 0.7`, no material-id test) the shaded color is multiplied by `1 - saturate(strength * max_i(exp(-((surfacePoint.xz - centre_i)/radius)^2)))`. Zero live centres or an unauthored strength contributes exactly 0 |
...
| `SdfOp.FlareY` (id 30) / `SdfProgramBuilder.FlareY(amount, bulge, top, span)` — a POINT op; Data0 = (amount, bulge, top, 1/span — the last HOST-BAKED), Data1.x = the conservative size correction `1/max(s)` over t ∈ [0, 1] (HOST-BAKED, `SdfProgram.FlareExtrema` — always ≤ 1 since s(0) = 1 is always a max candidate); Data1.yzw reserved | `SDF_OP_FLARE_Y` (30u) in `mapCore`, gated `#ifndef SDF_STRIP_HEAVY` — `t = saturate((top − y) * invSpan)`, `s = max(1 + amount·t + bulge·sin(π·t), SDF_FLARE_MIN_SCALE)`, `p.xz /= s`, `distanceScale *= data1.x`; the analytic-normal dual case in `mapGradCore` carries the exact Jacobian (`diag(1/s, 1, 1/s)` plus the rank-1 `ds/dy` shear, zeroed on the clamp plateau or where the floor is active — KEEP the two cases IN SYNC) | radial flare warp — scales a shape's local XZ cross-section by a profile along local Y, the study's `shinRadii`/`curvedSection` silhouette (a limb flaring toward the hip, tapering to the ankle, with a mid-span bulge). s(t) floors at `SdfProgramBuilder.FlareMinScale` (0.05) so an admitted amount/bulge combination that drives it non-positive still warps finitely. NOT 1-Lipschitz: the y-varying scale shears space (moving along y rescales x/z), so `AnalyzeLipschitz` folds a reach-dependent conservative operator-norm bound (`SdfProgram.FlareOperatorNorm` — triangle-inequality bound `max(1/s, 1) + max(s)·rho·|ds/dy|/s²` over the FLARED reach `max(s)·rho` (the shear scales with the evaluated point's own radial distance, and the flared surface — and every emitter's widened instance bound — reaches max(s) times the un-flared chain reach rho; the bare rho understates it by max(s) wherever amount > 0), times the SAME 1/maxS distanceScale factor, since that factor bounds the residual after the size correction, exactly as `LogSphere`'s `exp(w/2)` sits alongside its own `shellScale` distanceScale factor) into `chainFlares`, folded at chain-close against the FINAL chain reach like `CellJitter`'s boundary factor — reach-DEPENDENT, unlike `LogSphere`/`Displace`/`NoiseDisplace`'s reach-independent products. amount == 0 and bulge == 0 is an exact identity (byte-identical). RENDER-ONLY, like `BendX`/`BendY`/`BendZ`/`TwistY`: `Puck.SignedDistance.Queries.SdfFieldEvaluator` does not interpret it (left out of `IsSupportedOp`'s allow-list), so a program carrying it is unreachable for deterministic field contact — the constructor throws, naming the op. Exposed to `puck.creation.v1` as `ShapeFlareDocument` (`amount` ∈ [-0.9, 4], `bulge` ∈ [-1, 1], `span` finite and > 0 or refused by name, `top` optional = 0), admitted on every primitive, composing with `twist`/`bend` in that documented order (twist, then bend, then flare) on both emission paths (`WorldStampPool.EmitShape`'s prefix, `CreationStampEmitter.EmitShapeChain`'s `BuildTransformChain`); `top`/`span` are creation-unit lengths — a chain that already carries the placement scale as its own `Scale` op (the static stamper's chain, the pool's domain-bearing branch) passes them through UNSCALED, every other pool chain bakes the placement scale in by hand (baking it on a chain that carries the op scales them twice); `amount`/`bulge` are dimensionless and pass through unscaled everywhere. A flared shape takes its OWN field scope on both paths, like an eccentric primitive, so `FlareOperatorNorm` clamps its own candidate at the pop and the global stepScale stays 1; and every cull bound multiplies the primitive's reach by `ShapeFlareDocument.ReachFactor` = max(s) (`RenderReach`, `ShapeStampBound`, the pool's per-shape sphere; the probe by `ProbeReachFactor`, the factor at `MaxAmount`/`MaxBulge`), since the flared surface reaches that far past the un-flared one. Laws: `tests/Puck.SignedDistance.Tests/FlareLawTests.cs` (operator-norm mirror, exact endpoint extrema, zero-flare identity, a finite-difference grid proof the corrected field stays conservative) + `tests/Puck.World.Tests/ShapeFlareLawTests.cs` (admission, span/amount/bulge/top refusals, units on both emission paths); no Post stage exists (quarantine) |
| `SdfOp.ShearX` (id 31) / `SdfProgramBuilder.ShearX(linear, quadratic)` — a POINT op; Data0 = (linear, quadratic, 0, 0): `x += linear·y + quadratic·y²`, y and z untouched (exact, never a small-angle rotation) | `SDF_OP_SHEAR_X` (31u) in `mapCore` and `mapGradCore` (exact Jacobian `[[1, linear + 2·quadratic·y, 0], [0, 1, 0], [0, 0, 1]]`) | reach-dependent Lipschitz factor like Bend/Flare: `ShearOperatorNorm` feeds `TwistOperatorNorm`'s formula the effective rate `|linear| + 2·|quadratic|·reach` (both are a rank-1 perturbation of the identity driven by a coordinate outside the axis it moves). Render-only (`SdfFieldEvaluator` refuses it by name). Exposed as `ShapeDocument.Shear` (`[linear, quadratic, 0]` — the third lane is reserved and must be 0; |each| ≤ `ShapeDocument.MaxShear`) |
| `SdfOp.GaussianPush` (id 32) + `SdfOp.GaussianPushData` (id 33) / `SdfProgramBuilder.GaussianPush(center, radii, push)` — a POINT op packed as a head + tail pair: head Data0.xyz = center, Data1.xyz = radii (host-clamped ≥ `GaussianPushMinRadius`); tail Data0.xyz = push, its own case an inert no-op: `p −= push · exp(−|(p − center)/radii|²)` | `SDF_OP_GAUSSIAN_PUSH`/`SDF_OP_GAUSSIAN_PUSH_DATA` (32u/33u) in `mapCore` and `mapGradCore` (Jacobian `I − push ⊗ ∇g`), gated `#ifndef SDF_STRIP_HEAVY`; the head reads the tail's data0 one instruction ahead in the word stream | `SdfProgram.Validation.RequireGaussianPushPairs` refuses a head with no adjacent tail and an orphan tail; reach-independent Lipschitz factor `1 + |push|·sqrt(2/e)/min(radii)` (`GaussianPushLipschitz`, folded per chain like Displace). Render-only. Exposed as `ShapeDocument.Bumps` (`ShapeBumpDocument {center, radii, push}`, ≤ `MaxBumps`); both warps take the same per-shape field scope FlareY takes on both emission paths and grow the cull bound by `ShearBumpExtra` |
...
| `Puck.SignedDistance.SdfVolume` (`Kind`, `Position`/`Rotation`/`HalfExtent` — the volume's own frame relative to `DynamicSlot` when non-negative, else world space directly — `DynamicSlot`, `Axis`/`Width`/`Speed`/`Seed`/`Steps`, `CoreColor`/`SheathColor`/`CellColor`, `Intensity`/`Extinction`/`Thrust`) + `SdfProgramBuilder.MaxVolumes` (= 8) → `Puck.SdfVm.SdfFrame.Volumes` → `SdfWorldEngine.PackVolumes` packs each into 7 float4 rows (position.xyz+dynamicSlot, rotation.xyzw, halfExtent.xyz+axis, width/speed/seed/steps, coreColor.rgb+intensity, sheathColor.rgb+extinction, cellColor.rgb+thrustFlag) into its OWN structured buffer — never `sdfScreenLights` — cleared to zero before each frame's live entries are written, so an unauthored/trailing slot packs an inert zero-extent box rather than needing a separate live-count push word; `SdfWorldEngine.MaxVolumes` reads `SdfProgramBuilder.MaxVolumes` rather than hand-syncing a second literal | `sdfVolumes` (binding 48, register t43 — APPENDED LAST in the views binding list after the frame instance grid t42, declared unconditionally under `SDF_SCREEN_SOURCES` in `sdf-world.hlsli` beside `sdfScreenLights`) decoded by `shade-volumes.hlsli`'s `sdfLoadVolume`/`SdfVolumeData`; `sdfVolumeLocalPoint`/`sdfVolumeLocalDirection` undo the riding `sdfDynamicTransforms` slot (when `dynamicSlot >= 0`) then the volume's own `Position`/`Rotation`, exactly like a dynamic shape instance's `rigidBasePosition`; `sdfVolumeSlabInterval` is the local-frame AABB slab test; `sdfIntegrateVolume` ray-marches `Steps` (clamped [8, 64]) samples of core/sheath/turbulent-cell emission (`sdfLatticeNoise3`, a seeded wrapper over `sdfValueNoise3`) and Beer-Lambert extinction, depth-clipped to the surface hit distance (or the far distance on a miss); `shadeVolumes` composites every volume whose slab intersects the ray in camera order (farthest first, nearest last) into `color`, called once at the end of `renderView` before tonemap. THRUST reads `sdfDynamicTransforms[(3u * slot) + 2u].y` (`DynamicTransform.Lanes.y`) when `Thrust` is set and `DynamicSlot >= 0` | the engine's first PARTICIPATING MEDIUM — a bounded emissive volume ray-marched AFTER the opaque surface color is final, never a distance-field shape and never part of `map()`; a pixel whose ray misses every volume's slab pays only the slab test. Ported from `docs/art/moth-concept-pack-2026-09-09/moth-shadertoy.glsl`'s `jetInterval`/`integrateJet`/`compositeJets` technique (the taper/turbulence/extinction profile), generalized from that file's two hand-placed jets to an authored table. `SdfFrame.Volumes` is produced by `puck.creation.v1`'s `volumes[]` (`Puck.World.Authoring.VolumeDocument`, kind `plume`, validated by `CreationCanonicalizer.ValidateVolumes` — a known kind, a `parent` naming a declared shape, a positive box, steps in [`MinSteps`, `MaxSteps`], hex colours, ≤ `MaxVolumes`): `VolumeDocument.ToVolume` is the one door both paths use — `WorldStampPool.PackTransforms` packs each live registration's volumes onto `WorldStampPool.Volumes` riding the parent shape's slot (`rootSlot + 1 + shapeIndex`) or the root slot, `WorldPlacementStamper.EmitStatic` bakes a static placement's into world space with slot −1 (`WorldSceneEmitter.StaticVolumes`), and `WorldFramePresenter.Dress` concatenates static then pool onto the frame up to the ceiling. `CreationFrame.ToEngine` flips only a root-riding volume (a parented one is shape-local and its shape's flip already carries it). Pinned by `VolumeLawTests` |
...
- **The shadow/AO de-scale is GRADIENT-SCALED, on top of the program stepScale
  ...
  `stepScale` alone); moth-shadertoy.glsl's `surfaceGradient`/`shadow`/
