...
contradicts the grid or whose trailing padding bits address cells outside it.
...
`NoHeightSentinel` and would erase the cells it authored), a grid whose far
edge leaves signed Q48.16, a grid spanning more cells than a 32-bit cell index
addresses, and a bake above `DefaultMaxCellCount` unless the caller supplies a
different explicit budget. The baker transfers its newly allocated layers into
