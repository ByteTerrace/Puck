...
Euclidean clamp-to-box test and is the tighter of the two. The artifact's
...
contradicts the grid. A blocked cell blocks at every Y (no height in
`WorldQueryBlockerInput`); the heightfield blocks at or below its
authored ground; both answer `LineOfSight`. `SdfFieldEvaluator` (GRAVITY ARC Wave 1) wraps a LIVE
