...
| `src/Puck.World.Machines` | The engine-neutral screen-machine host | `WorldMachineHost` (the `IWorldMachineHost` implementation — boot, per-tick stepping, cable-linking, memory peek/poke, the two-phase prepare/commit/finish lifecycle, cartridge symbol resolution), `WorldScreenMachineEngines` (the live engine + `ICartridgeCompiler` catalog), `WorldMachineExtensionLoader`. References neither `Puck.HumbleGamingBrick*` nor `Puck.AdvancedGamingBrick*` |
