...
|---|---|---|
| `src/Puck.State` | The state and rule engine beneath the document, with no world or presentation concept | `ValueExpression`/`ValueToken`/`ExpressionSpelling`, `ExpressionOp`/`ExpressionArithmetic`, `TableDocument`/`TableCanonicalizer`/`TableRow`, the `StateTransform` union, `SafeName`/`CellName`/`WorldOwnedWorldFileName`, `CellKind`, `RuleFacts` — every type keeping its `World` prefix until the charter's last phase; consumers reach them through a project-wide `Using` |
