...
`puck.world.definition.v1` (`WorldDefinition.cs`) describes a world. An identity is not
...
restated here.** `puck schema` derives `puck.world.definition.v1.schema.json` (root),
...
the same run writes the dashboard portal's `worldDefinition.generated.ts`
from the bundled schema (`WorldSchema.ToTypeScript`), so a schema change is
regenerated by `puck schema` and never by hand or through npm. The same run
writes `src/Puck.World.Schema/WorldModelShape.generated.cs`, the model shape
the engine's model walks read (`WorldModelShape`).
...
`compile`/`decompile`/`lint`/`format`/`lsp` CLI verbs and `PUCKnnn` diagnostics —
...
`WorldDisclosureTier` (`src/Puck.World.Schema/WorldAdmission.cs`) is the whole vocabulary:
`frames` (no document), `presentation` (a `puck.world.projection.v1`
document), `replica` (the `puck.world.definition.v1` document verbatim, hash-identical
...
journaled, or an authority. Its `observations` (composed for this recipient)
arrive as plain `state.world` rows of the disclosed cells; a disclosed vector row
names its space (`WorldObservedRow.Space`) and the projection's `spaces` carries
exactly the spaces those rows name, so it hydrates as a vector row. A projection is FLAT:
because it discloses no `state` section, `Compose` answers every
`state.<row>[.<key>]` document identifier or spatial value from the composing
...
framing above, the top-level `puck.world.definition.v1` member and section-grant
...
| [documents-state.md](documents-state.md) | The `state` section: rows/cells, kinds, envelopes, the `advance`/`dynamics`/`cycle` traits, keyed-row eviction, reserved `$` names, `state.lattices` + the `lattice` trait, authored randomness (source/site/moment), `search`, `navigation`, the tabletop primitive, discrete-board patterns, records, pools, and retained turns. |
| [documents-render.md](documents-render.md) | `render` defaults, `dynamics` (the personality table), `curves`, `views.pipelines`, kit producer `flock`, crowd-scale policies, render validation and stamp capacity, the `rigid`/`carry`/`tether` facets. |
...
| [documents-composition.md](documents-composition.md) | Boot-authored topology/timing members (`References`, `Gravity`, `Portals`, `Simulation`, `Adjacencies`, `Metadata`); the districts; the worked `puck.world.json` walkthrough (debugRoom, chessBoard, drinkMe/eatMe); the sculpting library; `basis`/`imports` composition and merge rules; the validator; serialization conventions. |
...
all optional — every one carries `[JsonIgnore(Condition =
...
ABSENT behavior. Declaration (= canonical-write) order is the contract a
...
- `WorldBodiesLimits` (`Puck.World.Schema`): `CapacityCeiling = 4096`,
  ...
  `WorldClient.EntityCapacity` (see
  [SKILL.md](../SKILL.md)'s "Boundaries worth knowing" section). The client reserves full
...
- `WorldPlacementPolicy`: `MaxShapesPerStamp = 367`,
  ...
  `MaxStampRegistrations x MaxShapesPerStamp` against
  `Puck.SignedDistance.SdfProgramBuilder.MaxInstances`. `MaxShapesPerStamp`
  is sized so the shipped overworld's whole COMPOSED boot probe (the
  presenter's four emitters: the scene — stamp pool, avatar catalog, static
  placements, screens — plus the SDF-document, adjacency-band, and field
  reservations) stays at least 4096 instances under that cap; the scene
  emitter alone under-counts by the adjacency and field reservations.
  `WorldRenderEnvelopeLawTests.ShippedWorldBootProbeInstancesFitTheEngineCeilingWithHeadroom`
  enforces the floor and prints the current figures on its
  `world.budget probe:` output line.
...
- Refusal taxonomy (outside this directory): `src/Puck.State/RuleRefusal.cs`
  declares `RefusalKind` (protocol-fault vs verdict) and the
  `[Refusal(door, condition, kind)]` attribute; `src/Puck.World/RefusalCatalog.cs`
  builds the catalog `world.refusals` prints.
