...
| Brick pool; `BrickBakeRequestHeaderFloat4Count`; the brick push length | binding 46 (t41 in views, t4 in beam); `sdf-brick-bake.comp.hlsl`, `sdf-brick-upload.comp.hlsl` | `SdfBrickPoolLayout` = 8 bricks × 128³. Brick pushes are 16 bytes. |
| `GpuRegion`'s `Copy*` constants and `CopyPipeline`; `RecordCopy`, `StageOwed`; `SdfWorldEngine.RecordFrameUpload`, `FrameUploadRunTableWords` | `src/Puck.Shaders/Assets/Shaders/Residency/region-copy.comp.hlsl`: `copySource` binding 0 (t0), `copyDestination` binding 1 (u1), `RegionCopyPush`, `[numthreads(64, 1, 1)]` | Registers equal bindings (`GpuRegisterNumbering.Binding`). The push is `(count, runCount, offset, tableBase)` in uints. The staging buffer is a run-table reserve (`MaxCopyRuns` × 2 words, which the SDF engine's `MaxUploadRunsPerTable` equals) then the block; two or more runs stage `(block offset, prefix)` pairs at its front. |
