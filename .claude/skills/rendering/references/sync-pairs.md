...
| `SdfProgram.ShapeDetailFlag`, `ShapeNoSecondaryFlag` | *Generated:* `SDF_SHAPE_DETAIL_FLAG`, `SDF_SHAPE_NO_SECONDARY_FLAG`; by hand: `SDF_SHAPE_TYPE_MASK` and `sdfShapeEnabled` in `sdf-vm.hlsli` | The type mask 0x3FFFFFFF clears both flags and has no C# constant. Rigid fast paths test the original header, then mask the flags before dispatch. Instance header `.z` bit 0 is `NoDetailShapesFlag` (by hand). |
| `SdfProgram.BoundModeNone` / `BoundModeStatic` / `BoundModeDynamic`; `SegmentRigidPlanFlag`; `ShadowTransparentInstanceFlag`, `SegmentEndMask`; parked bound | *Generated:* `SDF_BOUND_*`, `SDF_SEGMENT_RIGID_PLAN`, `SDF_INSTANCE_SHADOW_TRANSPARENT_BIT`, `SDF_INSTANCE_SEGMENT_END_MASK`; by hand: `SDF_SEGMENT_BOUND_MASK` | A parked instance packs radius −1. |
| `SdfProgram.RigidLeafIdentityRotationFlag`, `RigidLeafFoldedFlag`, `RigidLeafMaxFoldRun`; folded rigid leaves | *Generated:* `SDF_RIGID_LEAF_IDENTITY_ROTATION`, `SDF_RIGID_LEAF_FOLDED`, `SDF_RIGID_LEAF_MAX_FOLD_RUN`; by hand: `SDF_RIGID_LEAF_SHAPE_MASK`, `sdfRigidFoldPoint` / `sdfRigidFoldGradient` in both rigid walks | A fold run packs an extension slot (pre-run pose, quaternion, run length) and a negative bound. |
