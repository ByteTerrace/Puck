...
| `GpuRegion`'s `Copy*` constants (`CopyHeaderWords`, `MaxCopyRuns`, `CopyWorkgroupSize`) and `CopyPipeline`; `RecordCopy`, `StageOwed` | `src/Puck.Shaders/Assets/Shaders/Residency/region-copy.comp.hlsl`: `copySource` binding 0 (t0), `copyDestination` binding 1 (u1), `RegionCopyHeaderWords`, `[numthreads(64, 1, 1)]` | Registers equal bindings (`GpuRegisterNumbering.Binding`); no push constants. The staging buffer is a four-uint header `(count, runCount, blockBase, destinationBase)`, then `runCount` `(block offset, first thread)` pairs in a reserve of `MaxCopyRuns` pairs, then the block at `blockBase`. Thread `i` copies `destination[destinationBase + word] = source[blockBase + word]`. |
