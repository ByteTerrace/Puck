...
interface and generated declarations, and SPIR-V and DXIL per stage for each of
`ShaderPackageVariant.Names` (`default`, then `low`, `medium`, `high`, compiled
with `QualityTiers.Define` set through `ShaderCompiler.StepsOf`); the pass entry
records the interface hash, and a load reads the variant a `views.graphs` row's
`tier` names (`LoadSource`'s `tier`). A build holds
