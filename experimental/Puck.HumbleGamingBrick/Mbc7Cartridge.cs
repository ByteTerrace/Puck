...
/// captures a reading); with no tilt source in the machine every capture reads the centered value <c>0x81D0</c> — a
/// fixed, <b>deterministic</b> choice. The EEPROM's command state machine (EWEN/EWDS/READ/WRITE/ERASE/ERAL/WRAL) is
/// implemented bit-serially over the CS/CLK/DI/DO pins with the chip's clear-then-OR write semantics and the
...
public sealed class Mbc7Cartridge : CartridgeBase {
    // The latched accelerometer reading for a motionless cartridge; the hardware centers near this, not at 0x8000.
    private const int AccelerometerCenter = 0x81D0;
    ...
    public override void WriteRam(ushort address, byte value) {
        ...
        switch ((address >> 4) & 0x0F) {
            case 0x00: // erase: reset the latches and arm the capture step
                ...
                break;
            case 0x01: // latch: capture a reading, only after an erase; no tilt source means the centered value
                if ((value == 0xAA) && m_latchErased) {
                    m_latchErased = false;
                    m_xLatch = AccelerometerCenter;
                    m_yLatch = AccelerometerCenter;
                }
