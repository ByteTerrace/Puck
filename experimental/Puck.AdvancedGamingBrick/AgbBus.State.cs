...
public sealed partial class AgbBus : IAgbSnapshotable {
    /// <inheritdoc/>
    // The bus-owned memories (on-board EWRAM, on-chip IWRAM, the I/O register backing) and every bus-level latch: the
    // open-bus value, the BIOS read latch and its in-BIOS/code-fetch flags, the DMA active/stall flags, the halt
    // flag, the APU sync clock, POSTFLG and KEYCNT, the WAITCNT-derived wait-state table (persisted directly so no
    // re-derive is needed), and the whole game-pak prefetch FIFO with its clock state. The BIOS image itself is
    // immutable and identified by the snapshot's identity stamp, so it is not serialized.
