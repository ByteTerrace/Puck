...
/// <summary>
/// The full serial communication subsystem (SIO / RCNT / JOY-bus), ported from ARES's register model
/// (gba/cpu/{io.cpp,cpu.hpp}) and the GBATEK hardware spec. SIOCNT, RCNT and the JOY registers are decomposed into
/// their hardware fields; writing SIOCNT's start bit begins a transfer in the selected mode (Normal 8/32-bit,
/// Multiplayer, UART) over the attached <see cref="IGbaLink"/>, and RCNT bit 15 hands the lines to General-purpose
/// (GPIO) or JOY-bus mode. With the default lone-console transport (<see cref="NullGbaLink"/>) a started transfer is
/// left PENDING — no fabricated completion and no serial IRQ — exactly as ARES does, so cable-less games (Pokémon
/// Emerald's boot SIO probe) proceed via their own timeouts instead of acting on a phantom partner.
/// </summary>
public sealed class GbaSerialController : IGbaSerialController {
    private readonly GbaScheduler m_scheduler;
    private readonly IGbaInterruptController m_interrupts;
    private readonly GbaScheduler.Event m_transferEvent;

    private IGbaLink m_link = NullGbaLink.Instance;
    private IGbaLinkNode m_node = NullGbaLink.Instance;

    // SIOCNT (0x128) fields — ARES Serial struct.
    ...
    // RCNT (0x134) / JOY-bus fields — ARES Joybus struct.
    ...
    public GbaSerialController(GbaScheduler scheduler, IGbaInterruptController interrupts) {
        ArgumentNullException.ThrowIfNull(scheduler);
        ArgumentNullException.ThrowIfNull(interrupts);

        m_scheduler = scheduler;
        m_interrupts = interrupts;
        m_transferEvent = new GbaScheduler.Event { Callback = _ => CompleteTransfer() };
    }

    /// <inheritdoc/>
    public void Connect(IGbaLink link) {
    ...
    private void WriteSioControl(ushort value) {
        ...
        // state machine (Pokémon Emerald's boot) stalls forever. BeginTransfer no-ops if one is already in flight.
        if (m_startBit) {
            BeginTransfer();
        }
        else {
    ...
    private void BeginTransfer() {
        ...
        switch (m_sioMode) {
            ...
            case 1:
                ...
                // (ARES has no transfer engine at all and never completes — a documented simplification, not the
                // hardware behavior; Pokémon Emerald's boot link-probe needs the real completion + idle-high data.)
    ...
    private void WriteJoyControl(ushort value) {
        // ARES io.cpp:389-392: the status bits are write-one-to-clear; bit 6 is the reset-IRQ enable.
        if ((value & 0x0001u) != 0u) {
            m_joyReset = false;
        }
