...
/// <see cref="IAgbSnapshotable"/> bytes into one flat, deterministic image, and <see cref="Restore"/> replays it —
...
public sealed class AdvancedGamingBrickMachine {
    ...
    private readonly AgbStateWriter m_stateWriter = new();

    private IAgbSnapshotable[]? m_snapshotables;
    ...
    /// aliases nothing live. Restore it into this machine to rewind. A component's own <see cref="IAgbSnapshotable"/>
    ...
    /// <exception cref="NotSupportedException">A subsystem does not implement <see cref="IAgbSnapshotable"/> (an
    ...
    public AgbMachineSnapshot Snapshot() {
        ...
        foreach (var component in order) {
            component.SaveState(writer: m_stateWriter);
        }

        return new AgbMachineSnapshot(identity: m_identity, takenAt: m_scheduler.Now, data: m_stateWriter.ToArray());
    }
    ...
    /// <exception cref="NotSupportedException">A subsystem does not implement <see cref="IAgbSnapshotable"/>.</exception>
    ...
    private IAgbSnapshotable[] EnsureSnapshotables() {
    ...
    private static IAgbSnapshotable AsSnapshotable(object component) =>
        (component as IAgbSnapshotable)
            ?? throw new NotSupportedException(
                message: $"Component '{component.GetType().Name}' does not implement {nameof(IAgbSnapshotable)}; whole-machine snapshot is unavailable for this composition."
            );
}
