...
public sealed class AgbBus : IAgbBus {
    ...
    private uint ReadRegion(uint address, int width, BusAccessType access) {
        ...
        // 8-bit save bus fetches the byte at the ALIGNED address and repeats it (proven by the hardware memory suite;
    ...
    // (the last ReadRegion result, i.e. the prefetched opcode), selected to the accessed halfword — the hardware's
    // memory-data-register shifted by `8*(addr&3)`.
    ...
    private ushort ReadIoHalf(uint offset) {
        ...
        if ((offset >= 0xB0u) && (offset < 0xE0u)) {
            ...
            // read back as open bus (DMA SAD/DAD have no read case → open-bus; the unlatched CNT_L address regs also
            // return open bus on hardware). CNT_H is offset 0xA within the 12-byte channel.
