...
/// </para>
/// <para>
/// PLACEMENT: the planetoid sits at <see cref="PlanetCenter"/>, a FAR <see cref="FixedPosition"/> cell (distinct from
...
/// <see cref="FixedPosition.Local"/> feeds <see cref="SdfFieldEvaluator.TryDistance"/> (see its own remarks) — the
/// walker's <see cref="FixedPosition"/> stays in THIS SAME cell for the whole proof (a planetoid of radius
/// <see cref="PlanetRadius"/> never approaches a cell boundary), so cell bookkeeping never needs to cross a seam
/// mid-walk; the far placement alone already proves the cell arithmetic (<see cref="FixedPosition.Normalize"/>,
/// <see cref="FixedPosition.Delta"/>, <see cref="FixedPosition.ToRenderRelative"/>) holds far from the world origin,
...
public static class GravityScenario {
    ...
    public static FixedPosition PlanetCenter => new(cellX: PlanetCellX, cellY: PlanetCellY, cellZ: PlanetCellZ, local: FixedVector3.Zero);
    ...
    /// a caller places it in the world by choosing which <see cref="FixedPosition"/> cell to evaluate/render it in).</returns>
    ...
    public static (FixedPosition Position, FixedVector3 Up) StartPose(float longitudeDegrees) {
    ...
    public static double LongitudeDegrees(FixedPosition position) {
