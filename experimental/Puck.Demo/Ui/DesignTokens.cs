...
/// push-constant float slot (the style the overlay shaders' push-constant floats already use) or a
/// <see cref="Vector3"/>/<see cref="Vector4"/> accessor for CPU-side math.
/// </summary>
...
internal readonly record struct RgbaColor(float R, float G, float B, float A) {
    /// <summary>The color as an opaque <see cref="Vector3"/> (<see cref="A"/> dropped).</summary>
    public Vector3 Rgb => new(x: R, y: G, z: B);

    /// <summary>The color as a <see cref="Vector4"/>.</summary>
...
internal readonly record struct BloomHue(RgbaColor Ring, RgbaColor Halo);

/// <summary>A CSS-style cubic-bezier easing curve's four control-point components.</summary>
internal readonly record struct CubicBezier(float X1, float Y1, float X2, float Y2);

/// <summary>
/// The canonical Puck UI design tokens — docs/ui-design-tokens.md, "Instrument + grafts", FINAL — transcribed to C#
/// constants. This is the single source every 2D overlay surface (the console panel, the binding bar, and later the
/// hub/tracker/plaques) reads instead of hand-picked literals, so the whole UI shares one 4px grid, one radius/type
/// scale, and one 12-role palette. Every nested class mirrors one numbered section of the spec 1:1; the doc
/// comments condense the WHY only where the spec calls out a non-obvious rule (the MSDF type floor, the two-tier
/// bloom elevation, the 0.84 scrim floor) — read the spec for full rationale and the rendered reference capture.
/// </summary>
internal static class DesignTokens {
    ...
    /// <summary>
    /// Section 4 — the 12 semantic color roles: near-neutral graphite surfaces, hairline outlines, and ONE electric
    ...
    internal static class Elevation {
        ...
        // Bloom hue table (derived from the 12 roles — no new hues). Neutral (held chips with no semantic hue)
        // blooms quieter BY DESIGN: pressed metal, not a signal.
    ...
    /// </summary>
    internal static class Diegetic {
