...
public sealed class OverworldWorld {
    ...
    public readonly record struct FieldWalkerSnapshot(bool Active, FixedPosition Position, FixedVector3 Velocity, FixedVector3 Up, FixedQ4816 FacingAngle, bool Grounded);
    ...
    private readonly FixedPosition m_hiddenPosition;
    ...
    public OverworldWorld(OverworldRoom room, PlatformerTuning tuning, float tickSeconds, uint seed, long spawnCellX = 0L, long spawnCellY = 0L, long spawnCellZ = 0L, bool startLoaded = false, bool spawnAtConsoles = false, int startCartType = -1, IReadOnlyList<int>? perConsoleStartCartType = null) {
        ...
        m_hiddenPosition = new FixedPosition(
            cellX: spawnCellX,
            cellY: spawnCellY,
            cellZ: spawnCellZ,
    ...
    public FixedPosition SpawnAnchor => new(cellX: m_spawnCellX, cellY: m_spawnCellY, cellZ: m_spawnCellZ, local: FixedVector3.Zero);
    ...
    public FixedPosition RoomPositionForFraction(Vector2 fraction) {
        var minX = (double)m_collision.MinX;
        var maxX = (double)m_collision.MaxX;
        var minZ = (double)m_collision.MinZ;
        var maxZ = (double)m_collision.MaxZ;
    ...
    public int SpawnRtsUnit(FixedQ4816 x, FixedQ4816 z) {
        ...
        if ((m_rtsQuery is { } query)) {
            if (query.Overlap(center: FixedPosition.FromLocal(local: new FixedVector3(X: x, Y: groundY, Z: z)), radius: RtsUnitSpawnRadiusRaw)) {
                return -1;
            }

            if (query.TryGroundHeight(position: FixedPosition.FromLocal(local: new FixedVector3(X: x, Y: groundY, Z: z)), probeUp: RtsGroundProbeRaw, probeDown: RtsGroundProbeRaw, groundY: out var sampled)) {
                groundY = sampled;
            }
        }
    ...
    private void AdvanceRtsUnits() {
        ...
        for (var slot = 0; (slot < MaxRtsUnits); slot++) {
            ...
            if ((m_rtsQuery is { } query) && query.TryGroundHeight(position: FixedPosition.FromLocal(local: new FixedVector3(X: x, Y: unit.Y, Z: z)), probeUp: RtsGroundProbeRaw, probeDown: RtsGroundProbeRaw, groundY: out var sampled)) {
                y = sampled;
            }
    ...
    public void SpawnFieldWalker(FixedPosition position, FixedVector3 up) {
    ...
    public DynamicTransform FieldWalkerTransform(FixedPosition renderOrigin, float alpha) {
        if (m_fieldWalker is not { } body) {
    ...
    public IReadOnlyList<DynamicTransform> DynamicTransforms(FixedPosition renderOrigin, float alpha) {
        for (var slot = 0; (slot < MaxPlayers); slot++) {
            var player = m_slots[slot];
