...
internal sealed class DiegeticUiDirector : ISdfFrameSource {
    ...
    // plate) is the r.2 (Radius2) component sized against HeightBindBar — the section-2 token ratios, so every
    // chip surface reads as the SAME rounding.
    ...
    // Bound/Pressed/Accent — the section-5 isHeld/isAccentTier/isDisabled precedence, implemented by the Emit
    // loop's own isHeld/isAccentTier/isDisabled):
    ...
    public void Emit(SdfProgramBuilder builder) {
        ...
        for (var index = 0; (index < chipCount); index++) {
            ...
            // The FOUR chip tiers, in the token contract's EXACT precedence (docs/ui-design-tokens.md
            // section 5, isHeld/isAccentTier/isDisabled): HELD (the physical button
