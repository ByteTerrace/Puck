...
| `Forge/` | **ROM forging — Arc 8's port source.** SDF-art → `.gbc` (`RomForge`, `SceneForge`), the SM83 emitter, the camera / avatar / world-lens cartridges. `Forge/Framework/` is the shared SM83 game framework (kernel, WRAM map, saves, PRNG, input, text, OAM, link, sound); `Forge/Tune/` is the audio-document jukebox; `Forge/Bake/` is the SDF→brick bake pipeline. The eight hand-authored games that lived here were deleted (OQ-4, 2026-07-31) — their designs are in [SM83 game designs](../../docs/sm83-game-designs.md). `Puck.Forge` (born on the audio arc) already lifts the tune chain + framework closure; the Demo keeps its copies as the byte-compare oracle until it dies. |
...
**OQ-4 is closed** (owner ruling, 2026-07-31): none of the eight games port — all
47 files were deleted and their designs recorded in
[SM83 game designs](../../docs/sm83-game-designs.md). `SceneForge.cs` and
`Flagship*.cs` (≈ 1 446 lines) were grouped with them but are not games; their
...
- Skills to load before touching an area: `rom-forge` (the ROM forge — Arc 8),
  ...
  `gaming-bricks` (the emulators), `post-batteries`.
