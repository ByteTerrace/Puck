...
pipeline serves every consumer: the live easel preview in creator mode, the `--forge-bake*` tool modes, and
`AvatarForge` (12 views = 4 facings × 3 poses; a creation document's timeline frames become the walk poses).
...
cartridge (see the [Framework README](../Framework/README.md)'s linker section); the retired games' baked title
screens installed through exactly this round trip.
...
| `--forge-bake-calibration <dir>` | DMG-classic bakes of SDF stand-ins for the pipeline's hand-pixelled reference art (paddle / ball / net) vs the hand 2bpp bytes: per-tile shade match % on stderr + a side-by-side PNG (hand \| baked, ×8). A calibration REPORT, never a failing gate — at landing: paddle 100%, ball 67%, net 81%, overall 89.7% |
