...
/// authors SDF stand-ins for three hand-pixelled reference tiles (a solid paddle bar, a ball dot, a dashed court
/// net — kept here as the pipeline's fixed measuring stick, no longer sourced from a game), bakes
...
internal static class BakeCalibration {
    ...
    // The reference art's 4-colour palette. The comparison maps these through their own luma onto the DMG ramp, so the
    // exact hues matter only in that they separate the four indices.
    private static HgbImage.Rgb[] ReferencePalette => [
    ...
    private static List<CalibrationSubject> BuildSubjects() {
        return [
            new CalibrationSubject(HandColours: ReferencePalette, HandTiles: StackTiles(count: 3, tile: PaddleTileArt()), Height: 24, Name: "paddle", Program: PaddleScene(), Width: 8),
            new CalibrationSubject(HandColours: ReferencePalette, HandTiles: BallTileArt(), Height: 8, Name: "ball", Program: BallScene(), Width: 8),
            new CalibrationSubject(HandColours: ReferencePalette, HandTiles: NetTileArt(), Height: 8, Name: "net", Program: NetScene(), Width: 8),
        ];
    }
    ...
    // The hand side: each 2bpp index maps through its reference palette colour's luma onto the DMG ramp — the honest
