...
subroutines. Its live consumer is the Tune jukebox under `Forge/Tune/`. Eight
hand-authored games proved the framework out and were retired once verified —
their designs, and the patterns they established, are recorded in
[SM83 game designs](../../../../docs/sm83-game-designs.md).

> **In-game reflection** (the [experience contract](../../../../docs/overworld-demo-plan.md#experience-contract)):
> the cartridges this framework powers are reached
> in-session — walk up to a cabinet, Cycle to the cart, boot it — with no restart. The
> `--forge-*` CLIs are the headless CI/proof twin that builds
...
| `LinkProtocolModule.cs` | deterministic role negotiation and validated full-duplex block exchange; no live consumer since the critter-trading cart was retired |
...
bit-for-bit (the retired games' batteries proved this across machines). This is the engine's determinism
doctrine scaled down into the cartridge: randomness is a deterministic function of input.
...
`framework.Sound.EmitEffect(emitter: e, effectId: SoundTables.EffectDeal)`. The model wiring is a title chirp +
loop start on the START edge, a cursor tick, a lock thud, a resolve sweep, a game-over run, and the win jingle.
Everything the driver plays is deterministic work-RAM state driven by inputs — replay-identical, no
wall clock. The HOST side (real speakers) lives in `src/Puck.Demo/Audio/`: each booted cabinet opens its own OS
output stream fed from the emulator's `IAudioSink`, output-only by contract; every game forge writes an
asserted `<out>.audio.wav` listen-proof beside the cartridge.

## How a new game verifies

No gate — a **self-verify battery** that runs inside the game's own forge tool before any bytes hit disk:
...
2. Read work RAM through `FrameworkMemoryMap` plus a game-owned protocol constants class — the game's `GameRam`
   addresses and state ids, shared between the emitter and the verifier so the two cannot drift.
