...
internal static class RomForge {
    ...
    // The default spread floor: proven for the framework games, whose capture windows always overlap at least
    ...
    /// <summary>The <c>--forge-bake-calibration</c> tool mode: bake SDF stand-ins for the pipeline's hand-pixelled
    /// reference tiles at their native sizes and report the per-tile pixel match — a calibration REPORT, never a gate
    /// (see <see cref="Bake.BakeCalibration"/>) — forwarded so the composition root keeps one forge dispatcher.</summary>
