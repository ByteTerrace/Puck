;; autopilot-ghost.wat — the demo's WASM "autopilot ghost" addon (puck.addon.v1).
;;
;; It drives its exclusive roster slot to the center cabinet and boots it, using ONLY the fixed-point
;; local position the host writes into the snapshot each tick — no geometry access, no runtime state
;; beyond a single one-shot latch. Every value is FixedQ4816 raw i64 bits (One = 0x1_0000 = 65536);
;; no floating point ever crosses the ABI. The math is dead reckoning: sub + clamp only (no mul, div,
;; sqrt, or normalize), so per-axis magnitude is bounded to 1 and the sim clamps overall move anyway.
;;
;; Room geometry the ghost hardcodes (a pure function of the room formula, not read at runtime):
;;   cabinets sit along the -Z wall at z = BoundsMin.Y + 1.4 = -6.6; the center cabinet at x ~= 0.
;;   Target (TX = 0, TZ = -6.6). -6.6 * 65536 = -432537.6 -> -432538 (nearest, matching the Post walker).
;;
...
;; A command record (24 bytes): padId u16@0, phase u8@2, reserved0 u8@3, reserved1 u32@4,
;;   valueX i64@8, valueY i64@16. PadMove = 0 (Axis2D), PadNorth = 4 (Digital). Phase: Started=0, Active=1.
(module
  (memory (export "memory") 1)

  ;; The one-shot interact latch: 0 until the ghost has fired its single PadNorth Started edge, then 1
  ;; forever (a fresh Store on `addon enable` resets it to 0 — a clean, deterministic restart).
  ...
  ;; The per-tick body: read the local position, emit a clamp-walk PadMove toward (0, -6.6), and — once,
  ;; when within the ~1.8-unit-per-axis interact range — a single PadNorth Started edge, then latch.
  (func (export "puck_on_tick") (result i32)
    ...
    ;; Remaining vector toward the target (TX = 0, TZ = -432538).
    (local.set $remX (i64.sub (i64.const 0) (local.get $posX)))
    ...
    ;; The addon speaks the intent's OWN frame: valueX -> strafe (world X), valueY -> forward (world Z),
    ;; written straight into PlayerIntent.Move with NO Y negation on the host side.
    ...
    ;; Within the interact range on BOTH axes and not yet booted? Fire ONE PadNorth Started edge.
    ;; Range ~= 1.8 world units = 1.8 * 65536 = 117964.8 -> 117965 (per-axis, matching the sim's proximity).
    (if
      (i32.and
        (i32.and
      ...
      (then
        ;; Record 1 @ 88: PadNorth (id 4, Digital), phase Started (0). valueX = One (pressed), valueY = 0.
