...
internal sealed class WorldGlyphStage : IPostStage {
    ...
    internal static SdfProgram BuildGlyphScene(FontAtlas atlas) {
        ...
        var slabMaterial = builder.AddMaterial(material: new SdfMaterial(Albedo: new Vector3(x: 0.52f, y: 0.48f, z: 0.42f), Specular: 0.3f, Shininess: 24f));
        var embossMaterial = builder.AddMaterial(material: new SdfMaterial(Albedo: new Vector3(x: 0.90f, y: 0.55f, z: 0.20f), Emissive: 0.25f));
        var floatMaterial = builder.AddMaterial(material: new SdfMaterial(Albedo: new Vector3(x: 0.92f, y: 0.72f, z: 0.18f), Emissive: 0.35f));
        var smoothMaterial = builder.AddMaterial(material: new SdfMaterial(Albedo: new Vector3(x: 0.28f, y: 0.66f, z: 0.90f), Emissive: 0.20f));
        var slabFrontZ = 0.30f;

        _ = builder
            .Plane(normal: Vector3.UnitY, offset: 0f, material: ground)
            // The backing slab: front face at z = slabFrontZ.
            .Translate(offset: new Vector3(x: 0f, y: 1.35f, z: 0f))
            .Box(halfExtents: new Vector3(x: 2.2f, y: 1.15f, z: slabFrontZ), round: 0.03f, material: slabMaterial)
            .ResetPoint();
        ...
        _ = builder.Text(atlas: atlas, text: "PUCK", origin: new Vector3(x: -1.40f, y: 1.72f, z: slabFrontZ), right: Vector3.UnitX, up: Vector3.UnitY, worldEmHeight: 0.58f, material: embossMaterial, blend: SdfBlendOp.Union, extrudeHalfDepth: 0.10f);

        // ENGRAVE: carved into the same face lower down; depth 0.20 cuts a clean channel below the surface.
        _ = builder.Text(atlas: atlas, text: "PUCK", origin: new Vector3(x: -1.40f, y: 0.72f, z: slabFrontZ), right: Vector3.UnitX, up: Vector3.UnitY, worldEmHeight: 0.58f, material: slabMaterial, blend: SdfBlendOp.Subtraction, extrudeHalfDepth: 0.20f);

        // FLOAT: free-standing, a LARGER scale, above the slab.
        _ = builder.Text(atlas: atlas, text: "PUCK", origin: new Vector3(x: -2.35f, y: 2.85f, z: 0f), right: Vector3.UnitX, up: Vector3.UnitY, worldEmHeight: 1.05f, material: floatMaterial, blend: SdfBlendOp.Union, extrudeHalfDepth: 0.15f);
        ...
        _ = builder
            .Translate(offset: new Vector3(x: -2.85f, y: 1.25f, z: 1.40f))
            .Sphere(radius: 0.55f, material: smoothMaterial)
            .ResetPoint()
            .Text(atlas: atlas, text: "P", origin: new Vector3(x: -3.25f, y: 0.92f, z: 1.65f), right: Vector3.UnitX, up: Vector3.UnitY, worldEmHeight: 0.85f, material: smoothMaterial, blend: SdfBlendOp.SmoothUnion, extrudeHalfDepth: 0.30f, smooth: 0.28f);

        return builder.Build();
    }
    internal static SdfFrame BuildGlyphFrame(SdfProgram program, uint width, uint height) {
        var camera = CameraSnapshot.LookAt(
            position: new Vector3(x: 0.4f, y: 2.15f, z: 7.6f),
            target: new Vector3(x: -0.4f, y: 1.35f, z: 0f),
    ...
    private static PostStageOutcome RunCore(PostContext context) {
        ...
        var diffPath = Path.Combine(path1: context.ArtifactsDirectory, path2: "world-glyph-diff.png");

        PngEncoder.Write(height: (int)WorldHeight, path: Path.Combine(path1: context.ArtifactsDirectory, path2: "world-glyph-vulkan.png"), rgba: vulkanPixels, width: (int)WorldWidth);
        PngEncoder.Write(height: (int)WorldHeight, path: Path.Combine(path1: context.ArtifactsDirectory, path2: "world-glyph-directx.png"), rgba: directXPixels, width: (int)WorldWidth);
        PngEncoder.Write(height: (int)WorldHeight, path: Path.Combine(path1: context.ArtifactsDirectory, path2: "world-glyph-noatlas.png"), rgba: vulkanNoAtlasPixels, width: (int)WorldWidth);
    ...
    private static double ChangedPercent(byte[] a, byte[] b, int width, int height) {
        ...
        for (var index = 0; (index < total); index++) {
            var offset = (index * 4);
            var delta = Math.Max(val1: Math.Abs(value: (a[offset] - b[offset])), val2: Math.Max(val1: Math.Abs(value: (a[(offset + 1)] - b[(offset + 1)])), val2: Math.Abs(value: (a[(offset + 2)] - b[(offset + 2)]))));
