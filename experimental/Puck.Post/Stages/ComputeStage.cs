    ...
    public PostStageOutcome Run(PostContext context) {
        ...
        try {
            ...
            if (mismatch is not null) {
                return PostStageOutcome.Fail(detail: $"the compute dispatch did not produce the expected UV gradient: {mismatch}");
            }

            return PostStageOutcome.Pass(detail: $"{RenderSize}x{RenderSize} | dispatched gradient.comp into a storage image; every pixel matches the CPU oracle (R=x/w, G=y/h, B=0.5) within 1 LSB");
        } finally {
