...
// Puck.AdvancedGamingBrick.Post — the Game Boy Advance machine's power-on self-test and the primary way the machine is
...
// vectors and a synthetic cartridge; Tier B needs the reference corpus (PUCK_GBA_TESTROMS), user ROMs (PUCK_GBA_GAMES),
// and/or a real replacement BIOS (PUCK_GBA_BIOS), and its stages skip when those are absent. The accuracy-frontier
// diagnostic modes (mGBA/ARES co-sim, single-ROM inspectors) live in Diagnostics and run before the battery when their
...
var testRomRoot = ResolveRoot(args: args, flag: "--roms", variable: "PUCK_GBA_TESTROMS");
var gamesRoot = ResolveRoot(args: args, flag: "--games", variable: "PUCK_GBA_GAMES");
...
// Loads the open-source replacement BIOS from PUCK_GBA_BIOS when present and correctly sized, so the BIOS-dependent
// stages (BIOS IRQ dispatch) run; otherwise a zeroed 16 KiB stub, on which those stages skip cleanly.
static ReadOnlyMemory<byte> LoadBios() {
    ...
    if (!string.IsNullOrEmpty(value: biosPath) && File.Exists(path: biosPath)) {
        ...
        if (bytes.Length == ReplacementBios.ImageSize) {
            Console.WriteLine(value: $"== BIOS: loaded replacement BIOS from {biosPath} ==");
