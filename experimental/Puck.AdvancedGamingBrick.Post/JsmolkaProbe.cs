...
/// <summary>
/// Runs a jsmolka gba-tests ROM and reads its verdict from the CPU register file. These ROMs accumulate the first failing
/// test number in <c>r12</c> (0 = every test passed) before spinning in a vsync/idle loop; with no display driver the ROM
/// hangs in that wait, but <c>r12</c> already holds the result. The probe steps the machine until execution settles,
/// guards against a settled PC in unmapped memory (a crash, not a pass), then reads <c>r12</c>.
/// </summary>
internal static class JsmolkaProbe {
