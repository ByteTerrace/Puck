...
internal enum RenderFloorSource {
    /// <summary>Under the jsmolka gba-tests corpus root (<c>--roms</c> / <c>PUCK_GBA_TESTROMS</c>).</summary>
    Corpus,
    /// <summary>Under the commercial-ROM directory (<c>--games</c> / <c>PUCK_GBA_GAMES</c>).</summary>
    Games,
}
...
/// frontier is worked. Floors sourced from the corpus (jsmolka screen demos) or the commercial-ROM directory skip
...
internal sealed class RenderHashStage : IPostStage {
    ...
    public PostStageOutcome Run(PostContext context) {
        ...
        if (ran == 0) {
            return PostStageOutcome.Skip(detail: "no render-hash floor ROMs present (set PUCK_GBA_TESTROMS / PUCK_GBA_GAMES)");
        }
