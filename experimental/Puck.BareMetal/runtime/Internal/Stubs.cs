...
namespace Internal.Runtime.CompilerHelpers {
    ...
    internal partial class ThrowHelpers {
        private static void ThrowIndexOutOfRangeException() => Environment.FailFast(message: null);
        private static void ThrowDivideByZeroException() => Environment.FailFast(message: null);
        private static void ThrowPlatformNotSupportedException() => Environment.FailFast(message: null);
    }
    ...
    internal static unsafe partial class StartupCodeHelpers {
        ...
        private static void RhpFallbackFailFast() => Environment.FailFast(message: null);
        [RuntimeExport("RhpNewFast")]
        private static void* RhpNewFast(MethodTable* pMT) {
            MethodTable** obj = AllocObject(size: pMT->_uBaseSize);
        ...
        private static void* RhpNewArray(MethodTable* pMT, int numElements) {
            ...
            MethodTable** obj = AllocObject(size: (pMT->_uBaseSize + ((uint)numElements * pMT->_usComponentSize)));
        ...
        public static void StelemRef(Array array, nint index, object obj) {
            ref object element = ref Unsafe.As<ArrayElement[]>(value: array)[index].Value;
            MethodTable* elementType = array.m_pMethodTable->_relatedType;

            if ((obj is not null) && (elementType != obj.m_pMethodTable))
                Environment.FailFast(message: null); // array covariance violation
        ...
        private static MethodTable** AllocObject(uint size) {
            ...
            if (Object.EfiSystemTable->BootServices->AllocatePool(2 /* EfiLoaderData */, (nint)size, (void**)&result) != 0)
                result = null;
#else
            result = PuckAllocZeroed(size: size);
#endif
            if (result is null)
                Environment.FailFast(message: null);
            return result;
