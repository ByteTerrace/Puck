...
internal static unsafe class Program {
    ...
    private static int Main() {
        ...
        int[] a = pool.Rent(minimumLength: 100);
        ...
        int[] b = pool.Rent(minimumLength: 100);
        ...
        int[] c = pool.Rent(minimumLength: 100);
        ...
        int[] big = pool.Rent(minimumLength: 5000);
        ...
        for (int i = 0; (i < 8); i++)
            if (pool.Rent(minimumLength: 100).Length != capacity)
                return 8;

        // Distinct element types get distinct pools (distinct Shared instantiations).
        byte[] bytes = ArrayPool<byte>.Shared.Rent(minimumLength: 64);
