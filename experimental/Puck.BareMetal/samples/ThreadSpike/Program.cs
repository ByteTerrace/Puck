...
internal static unsafe class Program {
    ...
    private static int Main() {
        ...
        for (int i = 0; (i < ThreadCount); i++) {
            ...
            handles[i] = CreateThread(dwCreationFlags: 0, dwStackSize: 0, lpParameter: (void*)(nint)i, lpStartAddress: &ThreadProc, lpThreadAttributes: default, lpThreadId: &threadId);
            if (handles[i] == 0)
                return 10;
        }

        s_go = 1; // release all threads at once
        WaitForMultipleObjects(bWaitAll: 1, dwMilliseconds: INFINITE, lpHandles: handles, nCount: ThreadCount);

        for (int i = 0; (i < ThreadCount); i++)
            CloseHandle(hObject: handles[i]);
