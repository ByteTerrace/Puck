...
internal static unsafe class Program
{
    ...
    private static void Log(string message)
    {
        ...
        buffer[length] = (byte)'\n';
        int written;
        WriteFile(GetStdHandle(STD_OUTPUT_HANDLE), buffer, length + 1, &written, default);
    }
    ...
    {
        Log("Puck.BareMetal VulkanWindow: starting");

        // The scope disposes everything (in reverse) when Main returns.
        using DisposalScope scope = new DisposalScope();

        nint window = Win32.CreateMainWindow(1280, 720);
        if (window == 0)
        {
            Log("FAILED: could not create window");
            return 1;
        }
        scope.Track(new WindowResource(window));
        Log("window created");

        if (!Vk.Initialize(window) || !Vk.InitializeRendering(1280, 720))
        {
            Log("FAILED: Vulkan bring-up did not complete");
            return 2;
        }
        scope.Track(new VulkanResource());

        Log("rendering; window should be cleared to color");
        ...
        Log("loop ended; disposing scope (Vulkan teardown, then window)");
        return 0;
    }
}
