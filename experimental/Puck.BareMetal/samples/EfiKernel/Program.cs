...
internal static class Program
{
    ...
    {
        Serial.WriteLine("[managed] self-hosted managed code is running.");
        ...
        Serial.Write("[managed] 8 KB heap buffer zeroed + round-trip: ");
        Serial.WriteLine(ok ? "OK" : "FAIL");
        ...
        for (int i = 0; i < numbers.Length; i++)
            sum += numbers[i];
        Serial.Write("[managed] sum(0..99) = ");
        Serial.WriteUInt(sum);
        Serial.WriteLine(" (expected 4950)");
        ...
        for (int i = 0; i < 2000; i++)
            if (new object() != null)
                allocated++;
        Serial.Write("[managed] allocated objects: ");
        Serial.WriteUInt(allocated);
        Serial.WriteLine("");
        ...
        ArrayPool<byte>.Shared.Return(ArrayPool<byte>.Shared.Rent(4096)); // warm up (one-time setup)
        ...
        Serial.Write("[managed] 10000 pooled 4KB rent/return grew the heap by ");
        Serial.WriteUInt(pooledGrowth);
        ...
        Serial.Write("[managed] 100 raw 4KB allocations grew the heap by ");
        Serial.WriteUInt(rawGrowth);
        Serial.WriteLine(" bytes (no GC -> never reclaimed)");

        Serial.WriteLine("[managed] Puck.BareMetal is now an operating system with pooled memory.");
        return 0;
    }
}
