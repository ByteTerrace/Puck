...
internal sealed class ServiceList : IEnumerable<IService> {
    ...
    public IEnumerator<IService> GetEnumerator() => new Enumerator(list: this);
...
internal static unsafe class Program {
    ...
    private static int Main() {
        ...
        ServiceList list = new ServiceList(capacity: 3);

        list.Add(item: new ServiceA());
        list.Add(item: new ServiceB());
        list.Add(item: new ServiceC());

        if (SumIds(services: list) != ((1 + 2) + 4)) // distinct bits: any missed/duplicated element changes it
            return 1;

        // Iterate a second time to confirm a fresh enumerator restarts correctly.
        if (SumIds(services: list) != 7)
            return 2;
