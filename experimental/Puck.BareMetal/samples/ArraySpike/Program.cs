...
internal static unsafe class Program {
    ...
    private static int Main() {
        ...
        if (SumViaEnumerable(services: arr) != ((1 + 2) + 4)) // foreach over IEnumerable<IService>
            return 1;

        if (SumViaEnumerable(services: arr) != 7) // a fresh enumerator restarts correctly
            return 2;

        if (SumViaReadOnlyList(services: arr) != 7) // Count + indexer through IReadOnlyList<IService>
            return 3;
