...
public sealed class SdfDistanceGridLawTests {
    ...
    public void CastsAgreeOnHitOrMissAndABandBoundMarchIsBitIdentical() {
            ...
            foreach (var direction in Directions) {
                foreach (var radius in new[] { 0.0, 0.25 }) {
                    ...
                    if (expected.Confidence == WorldQueryConfidence.Exact) {
                        ...
                        var clearance = (Exact(
                            evaluator: exact,
                            position: actual.Point
                        ) - fixedRadius);

                        Assert.True(
                            condition: (clearance <= FixedQ4816.FromDouble(value: 0.001)),
                            userMessage: $"the banded hit at {actual.Point.Local} has clearance {clearance}"
                        );
                    }
                }
            }
        ...
        Assert.True(
            condition: (casts > 2_000),
            userMessage: $"only {casts} casts ran"
        );
        ...
        foreach (var x in new[] { -6.2, -2.3, 0.4, 3.1, 7.7 }) {
            foreach (var z in new[] { -6.6, -1.3, 0.2, 2.9, 6.1 }) {
                foreach (var radius in new[] { 0.0, 0.2 }) {
                    ...
                    Assert.Equal(
                        expected: exact.TryGroundHeight(
                            groundY: out var expectedGround,
                            position: origin,
                            probeDown: maxDistance,
                            probeUp: FixedQ4816.Zero
                        ),
                        actual: banded.TryGroundHeight(
                            groundY: out var actualGround,
                            position: origin,
                            probeDown: maxDistance,
                            probeUp: FixedQ4816.Zero
                        )
                    ...
                    Assert.Equal(
                        expected: expectedGround.Value,
                        actual: actualGround.Value
                    );
                }
            }
        }
    ...
    public void InsideTheBandTheBandedSurfaceIsTheExactOne() {
        ...
        foreach (var position in Lattice()) {
            ...
            if (field < banded.Band) {
                inBand++;
                Assert.Equal(
                    expected: exactDistance.Value,
                    actual: bandedDistance.Value
                );
                Assert.Equal(
                    actual: bandedMaterial,
                    expected: exactMaterial
                );
            } else {
                beyond++;
                Assert.True(
                    condition: (bandedDistance <= exactDistance),
                    userMessage: $"beyond the band at {position.Local}: banded {bandedDistance} exceeds exact {exactDistance}"
                );
                Assert.True(
                    condition: (bandedDistance >= banded.Band),
                    userMessage: $"beyond the band at {position.Local}: banded {bandedDistance} fell under the band {banded.Band}"
                );
            }
            ...
            Assert.Equal(
                expected: exactGradient,
                actual: banded.TryFieldGradient(
            ...
            foreach (var radius in new[] { 0.0, 0.2, 0.4, 1.0 }) {
                ...
                Assert.Equal(
                    expected: exact.Overlap(
                        center: position,
                        radius: fixedRadius
                    ),
                    actual: banded.Overlap(
                        center: position,
                        radius: fixedRadius
                    )
                );
        ...
        Assert.True(
            condition: (inBand > 1_000),
            userMessage: $"only {inBand} lattice points fell inside the band"
        );
        Assert.True(
            condition: (beyond > 1_000),
            userMessage: $"only {beyond} lattice points fell beyond the band"
        );
    }
    [Fact]
    public void TheBakeIsAPureFunctionOfTheProgram() {
        ...
        Assert.Equal(
            expected: gridA.Origin,
            actual: gridB.Origin
        );
        Assert.Equal(
            expected: gridA.Slack.Value,
            actual: gridB.Slack.Value
        );
        Assert.Equal(
            expected: gridA.CornerCount,
            actual: gridB.CornerCount
        );
        ...
        for (var x = 0; (x < 24); x++) {
            for (var y = 0; (y < 12); y++) {
                for (var z = 0; (z < 24); z++) {
                    Assert.True(condition: gridA.TryCornerDistance(
                        distance: out _,
                        material: out _,
                        x: x,
                        y: y,
                    ...
                    Assert.True(condition: gridB.TryCornerDistance(
                        distance: out _,
                        material: out _,
                        x: (23 - x),
                        y: (11 - y),
            ...
            for (var y = 0; (y < 12); y++) {
                for (var z = 0; (z < 24); z++) {
                    ...
                    Assert.True(condition: exactA.TryDistance(
                        ...
                        position: FixedPosition.FromLocal(local: gridA.CornerPosition(
                            x: x,
                            y: y,
                            z: z
                        ))
        ...
        Assert.Equal(
            expected: ((24L * 12L) * 24L),
            actual: gridA.BakedCornerCount
        );
    }
    [Fact]
    public void TheBoundNeverExceedsTheExactFieldAndCoversEveryFiniteInstance() {
        ...
        Assert.True(
            condition: (bounded > 10_000),
            userMessage: $"the lattice touched only {bounded} bounded points"
        );
        ...
        Assert.True(condition: (((double)grid.CornerPosition(
            x: (grid.CornerCountX - 1),
            y: 0,
            z: 0
        ).X) >= (7.4 + Padding)));
        Assert.True(condition: (((double)grid.CornerPosition(
            x: 0,
            y: 0,
            z: (grid.CornerCountZ - 1)
        ).Z) >= (7.5 + Padding)));
        Assert.Equal(
            expected: CellSize,
            actual: ((double)grid.CellSize)
        );
    }
}
