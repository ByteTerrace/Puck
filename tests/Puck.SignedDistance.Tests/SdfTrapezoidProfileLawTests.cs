...
public sealed class SdfTrapezoidProfileLawTests {
    ...
    public void ANonFiniteRoundingIsRefusedByEveryProfileBuilder(float rounding) {
        ...
        Assert.Throws<ArgumentOutOfRangeException>(testCode: () => builder.Ellipse(
            1f,
            .5f,
            SdfLift.Extrude,
            1f,
        ...
        Assert.Throws<ArgumentOutOfRangeException>(testCode: () => builder.RegularPolygon(
            6,
            1f,
            SdfLift.Extrude,
            1f,
        ...
        Assert.Throws<ArgumentOutOfRangeException>(testCode: () => builder.Trapezoid(
            1f,
            .5f,
            1f,
            SdfLift.Extrude,
        ...
        Assert.Throws<ArgumentOutOfRangeException>(testCode: () => builder.RoundedRectangle(
            1f,
            .5f,
            .1f,
            SdfLift.Extrude,
    ...
    public void ARevolvedPrismAtAPositiveOffsetIsARing() {
        ...
        Assert.True(condition: (onRing < FixedQ4816.Zero));
        ...
        Assert.True(condition: (hole > FixedQ4816.Zero));
    }
    [Fact]
    public void ARevolvedPrismIsASolidOfRevolutionAboutY() {
        ...
        foreach (var (x, z) in new[] { (0.9d, 0d), (0d, 0.9d), (-0.9d, 0d), (0.6364d, 0.6364d) }) {
            ...
            Assert.True(
                ...
                userMessage: $"({x}, 0, {z}) read {((double)inside)} outside the disc"
            );
        }
        ...
        foreach (var (x, z) in new[] { (1.2d, 0d), (0d, 1.2d), (0.8485d, 0.8485d) }) {
            ...
            Assert.True(
                ...
                userMessage: $"({x}, 0, {z}) read {((double)outside)} inside the disc"
            );
        }
        ...
        Assert.True(condition: (above > FixedQ4816.Zero));
    }
    [Fact]
    ...
    public void AnUnroundedShapeLeavesTheRoundingLaneAtZero() {
        ...
        foreach (var instruction in builder.Build().Instructions.Where(predicate: instruction => (instruction.Op == SdfOp.ShapeBlend))) {
            Assert.Equal(
                0f,
                instruction.Data1.W
            );
    ...
    public void InvalidPrismTaperIsRefusedAtBothDoors(float taper) {
        ...
        Assert.Throws<ArgumentOutOfRangeException>(testCode: () => SdfSolidGeometry.AppendScaledPrimitive(
    ...
    public void PrismTaperAndExtrusionHaveTheAuthoredSurface(float taper) {
        ...
        Assert.True(condition: (SdfSolidGeometry.Reach(
            SdfSolidPrimitive.Prism,
            new Vector3(
                x: 2f,
                y: 1f,
    ...
    public void ProfileUsesTheExistingRendererInstruction(SdfPrismProfileKind kind, SdfShapeType expected) {
        ...
        Assert.Contains(
            ...
            filter: instruction => (instruction.Shape == ((uint)expected))
        );
    }
    [Fact]
    public void RoundedProfileControlsCornersWithoutBulgingTheCaps() {
        ...
        Assert.True(condition: (corner > FixedQ4816.Zero));
        Assert.True(condition: evaluator.TryDistance(
            Position(
                x: .9,
                y: 0,
        ...
        Assert.True(condition: (face < FixedQ4816.Zero));
    }
    [Fact]
    public void RoundingFilletsThePrismRimWithoutMovingItsFaces() {
        ...
        foreach (var probe in new[] { Position(
            x: 0,
            y: 0,
            z: 1
        ), Position(
            ...
            Assert.True(condition: sharp.TryDistance(
                distance: out var sharpFace,
                material: out _,
                position: probe
            ));
            Assert.True(condition: rounded.TryDistance(
                distance: out var roundedFace,
                material: out _,
                position: probe
            ));
            Assert.InRange(
        ...
        Assert.True(
            (sharpRim < FixedQ4816.Zero),
            userMessage: $"the sharp rim read {((double)sharpRim)}"
        );
        Assert.True(
            (roundedRim > FixedQ4816.Zero),
            userMessage: $"the rounded rim read {((double)roundedRim)}"
        );
    }
    [Fact]
    public void RoundingIsRefusedPastTheShapeItFits() {
        ...
        Assert.Contains(
            actualString: refusal,
            expectedSubstring: "edge-rounding"
        );
    }
    [Fact]
    public void RoundingNeverGrowsAThinProfilePastItsAuthoredWidth() {
        ...
        Assert.Equal(
            .1f,
            SdfProgramBuilder.TrapezoidRoundingCeiling(
                bottomHalfWidth: .1f,
                halfHeight: 1f,
                topHalfWidth: .1f
            ),
        ...
        Assert.True(condition: rounded.TryDistance(
            ...
            out var roundedOutside,
            out _
        ));
        Assert.InRange(
            actual: ((double)sharpOutside),
            high: .21,
            low: .19
        );
        Assert.True(
            (roundedOutside > FixedQ4816.Zero),
            userMessage: $"the rounded prism grew past its width: {((double)roundedOutside)}"
        );
        ...
        Assert.True(
            (sharpRim < FixedQ4816.Zero),
            userMessage: $"sharp rim {((double)sharpRim)}, rounded rim {((double)roundedRim)}, rounded outside {((double)roundedOutside)}"
        );
        Assert.True(
            (roundedRim > FixedQ4816.Zero),
            userMessage: $"sharp rim {((double)sharpRim)}, rounded rim {((double)roundedRim)}"
        );
    }
    [Fact]
