...
/// </summary>
internal static class IndependentCarriageImplementation {
    ...
    internal static byte[] SignKeyBinding(
        ...
        return SignEnvelope(header: header, payloadKind: KeyBindingPayloadKind, payload: payload, signingKey: signingKey);
    }

    internal static byte[] SignClaim(IndependentHeader header, ulong payloadKind, ReadOnlySpan<byte> payload, ECDsa signingKey) =>
        SignEnvelope(header: header, payloadKind: payloadKind, payload: payload, signingKey: signingKey);
    ...
    internal static byte[] VerifyChain(
        ...
        var rootToIssuing = DecodeEnvelope(wire: rootToIssuingWire);

        VerifyEnvelope(
            envelope: rootToIssuing,
        ...
        var issuingToSubject = DecodeEnvelope(wire: issuingToSubjectWire);

        VerifyEnvelope(
            envelope: issuingToSubject,
        ...
        var claim = DecodeEnvelope(wire: claimWire);
    ...
    private static byte[] SignEnvelope(IndependentHeader header, ulong payloadKind, ReadOnlySpan<byte> payload, ECDsa signingKey) {
    ...
    private static IndependentEnvelope DecodeEnvelope(ReadOnlySpan<byte> wire) {
        ...
        return new IndependentEnvelope(
            Header: header,
    ...
    private static void VerifyEnvelope(
        ...
        if (
            !string.Equals(envelope.Header.Domain, pinnedId.Domain, StringComparison.Ordinal) ||
            !string.Equals(envelope.Header.Subject, pinnedId.Subject, StringComparison.Ordinal) ||
            !string.Equals(envelope.Header.Algorithm, pinnedId.Algorithm, StringComparison.Ordinal) ||
            !string.Equals(envelope.Header.Purpose, expectedPurpose, StringComparison.Ordinal) ||
            (envelope.PayloadKind != expectedPayloadKind) ||
            (now < envelope.Header.NotBefore) ||
            (now > envelope.Header.NotAfter) ||
            !string.Equals(Fingerprint(bytes: pinnedSubjectPublicKeyInfo), pinnedId.KeyHash, StringComparison.Ordinal)
        ) {
            throw new CryptographicException(message: "An independent envelope policy check failed.");
        }
        ...
        if (!verifier.VerifyData(
            data: envelope.SignedPortion,
            signature: envelope.Signature,
            hashAlgorithm: HashAlgorithmName.SHA256,
            signatureFormat: DSASignatureFormat.IeeeP1363FixedFieldConcatenation
        )) {
            throw new CryptographicException(message: "An independent carriage signature check failed.");
        }
    }
    ...
    private static void RequireSelfCertifying(IndependentBinding binding) {
        if (!string.Equals(Fingerprint(bytes: binding.SubjectPublicKeyInfo), binding.TargetId.KeyHash, StringComparison.Ordinal)) {
            throw new CryptographicException(message: "An independent carriage binding is not self-certifying.");
        }
    }
...
internal sealed record IndependentEnvelope(
