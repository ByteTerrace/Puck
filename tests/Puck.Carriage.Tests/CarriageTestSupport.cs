...
internal static class CarriageTestSupport {
    ...
    internal static (SignedCarriageEnvelope RootToIssuing, SignedCarriageEnvelope IssuingToSubject) BuildChain(ICarriageCodec codec, DomainKeys keys, long notBefore, long notAfter) {
    ...
    internal static SignedCarriageEnvelope SignTestClaim(
    ...
    internal static void AssertAccepted(CarriageVerifyResult result) =>
        Assert.True(condition: result.Verified, userMessage: $"unexpectedly refused: {result.RefusalReason}");

    internal static void AssertRefused(CarriageVerifyResult result, string reasonMustContain) {
    ...
    /// </summary>
    internal static byte[] BuildHandWrittenEnvelope(int domainWidth = 32, ulong payloadKind = (ulong)CarriagePayloadKind.Opaque) {
    ...
    internal static byte[] BuildIndefiniteLengthEnvelope(byte[] wire) {
