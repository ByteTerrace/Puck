...
public sealed class CoreTests {
    ...
    public void HappyPath_Verifies() {
        ...
        var (rootToIssuing, issuingToSubject) = BuildChain(codec: codec, keys: keys, notBefore: BindingNotBefore, notAfter: BindingNotAfter);
    ...
    public void AlgorithmConfusion_DeclaredAlgorithmDiffersFromPinnedKey_IsRefused() {
        ...
        var chain = new[] { rootToIssuing, issuingToSubject };
        var trust = BuildTrustList(keys: keys, defaultMaximumAge: TimeSpan.FromHours(hours: 24));
        var confusedClaim = SignTestClaim(codec: codec, keys: keys, purpose: "test.claim", notBefore: (Epoch - 60), notAfter: (Epoch + 3_600), audience: "world:home", sequence: null, text: "algorithm-confused claim", declaredAlgorithm: CarriageAlgorithms.EcdhP256HkdfSha256Aes256Gcm);

        var result = CarriageVerifier.VerifyChain(codec: codec, claim: confusedClaim, chain: chain, trustList: trust, now: Now, expectedPurpose: "test.claim", expectedAudience: "world:home");

        AssertRefused(result: result, reasonMustContain: "algorithm confusion");
    }

    [Fact]
    public void PurposeReplay_KeyBindingEnvelopePresentedAsClaim_IsRefused() {
        var codec = new CborCarriageCodec();
        var keys = MintDomainKeys(subject: "user:alice");
        var (rootToIssuing, issuingToSubject) = BuildChain(codec: codec, keys: keys, notBefore: BindingNotBefore, notAfter: BindingNotAfter);
        var chain = new[] { rootToIssuing, issuingToSubject };
        ...
        var result = CarriageVerifier.VerifyChain(codec: codec, claim: issuingToSubject, chain: chain, trustList: trust, now: Now, expectedPurpose: "test.claim", expectedAudience: "world:home");

        AssertRefused(result: result, reasonMustContain: "purpose");
    }
    ...
    public void CrossDomain_ClaimAgainstForeignTrustList_IsRefused() {
        ...
        var result = CarriageVerifier.VerifyChain(codec: codec, claim: domainBClaim, chain: chainB, trustList: trustA, now: Now, expectedPurpose: "test.claim", expectedAudience: "world:home");

        AssertRefused(result: result, reasonMustContain: "trusted vouching root");
    }
    ...
    public void CrossDomain_ClaimAgainstOwnTrustList_Accepts() {
        ...
        var result = CarriageVerifier.VerifyChain(codec: codec, claim: domainBClaim, chain: chainB, trustList: trustB, now: Now, expectedPurpose: "test.claim", expectedAudience: "world:home");
    ...
    public void ExpiredWindow_ClaimPastNotAfter_IsRefused() {
        ...
        var result = CarriageVerifier.VerifyChain(codec: codec, claim: expiredClaim, chain: chain, trustList: trust, now: Now, expectedPurpose: "test.claim", expectedAudience: "world:home");
    ...
    public void ExpiredWindow_ClaimWithinOwnWindow_Accepts() {
        ...
        var result = CarriageVerifier.VerifyChain(codec: codec, claim: freshClaim, chain: chain, trustList: trust, now: Now, expectedPurpose: "test.claim", expectedAudience: "world:home");
    ...
    public void TighterOfTwo_IssuerWindowGovernsWhenTighter_AcceptsWithinIt() {
        ...
        var result = CarriageVerifier.VerifyChain(codec: codec, claim: tightIssuerClaim, chain: chain, trustList: trustGenerous, now: Now, expectedPurpose: "test.claim", expectedAudience: "world:home");
    ...
    public void TighterOfTwo_IssuerWindowGovernsWhenTighter_RefusesPastIssuersOwnWindow() {
        ...
        var result = CarriageVerifier.VerifyChain(codec: codec, claim: tightIssuerClaim, chain: chain, trustList: trustGenerous, now: laterNow, expectedPurpose: "test.claim", expectedAudience: "world:home");

        AssertRefused(result: result, reasonMustContain: "issuer");
    }
    ...
    public void TighterOfTwo_VerifierCeilingGovernsWhenTighter_AcceptsWithinIt() {
        ...
        var result = CarriageVerifier.VerifyChain(codec: codec, claim: tightVerifierClaim, chain: chain, trustList: trustTight, now: Now, expectedPurpose: "test.claim", expectedAudience: "world:home");
    ...
    public void TighterOfTwo_VerifierCeilingGovernsWhenTighter_RefusesPastVerifiersHour() {
        ...
        var result = CarriageVerifier.VerifyChain(codec: codec, claim: tightVerifierClaim, chain: chain, trustList: trustTight, now: muchLaterNow, expectedPurpose: "test.claim", expectedAudience: "world:home");

        AssertRefused(result: result, reasonMustContain: "verifier");
    }
    ...
    public void SeparatedLifetimePolicy_RootBindingCeilingGovernsIndependentlyOfClaimCeiling() {
        ...
        var result = CarriageVerifier.VerifyChain(codec: codec, claim: tightVerifierClaim, chain: chain, trustList: rootAgedTrust, now: muchLaterNow, expectedPurpose: "test.claim", expectedAudience: "world:home");

        AssertRefused(result: result, reasonMustContain: "root-vouches-issuing");
    }
    ...
    public void MissingChain_ClaimWithNoBindings_IsRefused() {
        ...
        var result = CarriageVerifier.VerifyChain(codec: codec, claim: happyClaim, chain: null, trustList: trust, now: Now, expectedPurpose: "test.claim", expectedAudience: "world:home");

        AssertRefused(result: result, reasonMustContain: "missing chain");
    }
    ...
    public void BrokenChain_IssuingVouchesSubjectBindingAbsent_IsRefused() {
        var codec = new CborCarriageCodec();
        var keys = MintDomainKeys(subject: "user:alice");
        var (rootToIssuing, _) = BuildChain(codec: codec, keys: keys, notBefore: BindingNotBefore, notAfter: BindingNotAfter);
        var trust = BuildTrustList(keys: keys, defaultMaximumAge: TimeSpan.FromHours(hours: 24));
        var happyClaim = SignTestClaim(codec: codec, keys: keys, purpose: "test.claim", notBefore: (Epoch - 60), notAfter: (Epoch + 3_600), audience: "world:home", sequence: null, text: "hello from alice");
        var brokenChain = new[] { rootToIssuing };

        var result = CarriageVerifier.VerifyChain(codec: codec, claim: happyClaim, chain: brokenChain, trustList: trust, now: Now, expectedPurpose: "test.claim", expectedAudience: "world:home");
    ...
    public void AudienceMismatch_ExpectedAudienceMatches_Accepts() {
        ...
        var result = CarriageVerifier.VerifyChain(codec: codec, claim: marketClaim, chain: chain, trustList: trust, now: Now, expectedPurpose: "test.claim", expectedAudience: "world:market");
    ...
    public void AudienceMismatch_ExpectedAudienceDiffers_IsRefused() {
        ...
        var result = CarriageVerifier.VerifyChain(codec: codec, claim: marketClaim, chain: chain, trustList: trust, now: Now, expectedPurpose: "test.claim", expectedAudience: "world:elsewhere");

        AssertRefused(result: result, reasonMustContain: "audience mismatch");
    }
    ...
    public void BearerSequence_FirstUse_AcceptsAndAdvancesMark() {
        ...
        var result = store.Commit(result: CarriageVerifier.VerifyChain(codec: codec, claim: bearer10, chain: chain, trustList: trust, now: Now, expectedPurpose: "test.bearer", expectedAudience: null));
    ...
    public void BearerSequence_EqualSequenceReplay_IsRefused() {
        ...
        _ = store.Commit(result: CarriageVerifier.VerifyChain(codec: codec, claim: bearer10, chain: chain, trustList: trust, now: Now, expectedPurpose: "test.bearer", expectedAudience: null));
        var result = store.Commit(result: CarriageVerifier.VerifyChain(codec: codec, claim: bearerEqual, chain: chain, trustList: trust, now: Now, expectedPurpose: "test.bearer", expectedAudience: null));

        AssertRefused(result: result, reasonMustContain: "replay");
    }
    ...
    public void BearerSequence_LowerSequenceReplay_IsRefused() {
        ...
        _ = store.Commit(result: CarriageVerifier.VerifyChain(codec: codec, claim: bearer10, chain: chain, trustList: trust, now: Now, expectedPurpose: "test.bearer", expectedAudience: null));
        var result = store.Commit(result: CarriageVerifier.VerifyChain(codec: codec, claim: bearerLower, chain: chain, trustList: trust, now: Now, expectedPurpose: "test.bearer", expectedAudience: null));
    ...
    public void BearerSequence_HigherSequenceAdvancesMark_Accepts() {
        ...
        _ = store.Commit(result: CarriageVerifier.VerifyChain(codec: codec, claim: bearer10, chain: chain, trustList: trust, now: Now, expectedPurpose: "test.bearer", expectedAudience: null));
        var result = store.Commit(result: CarriageVerifier.VerifyChain(codec: codec, claim: bearer11, chain: chain, trustList: trust, now: Now, expectedPurpose: "test.bearer", expectedAudience: null));
