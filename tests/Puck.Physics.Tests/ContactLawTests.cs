...
public sealed class ContactLawTests {
    ...
    public void CapsuleWaistAgainstAThinSlabIsCaughtWhileBothEndSpheresStayClear() {
        ...
        Assert.True(
            condition: (world.LastStepCandidateCount > 0),
            userMessage: "the segment scan must find the waist contact"
        );
        Assert.True(
    ...
    public void FixedSpeculativeActivationCatchesAFirstAppearanceApproachWithoutTunnelling() {
        ...
        Assert.Equal(
            expected: 1,
            actual: world.LastStepCandidateCount
        );
        ...
        MeasurementAssert.Near(
            actual: world.Pose.Center.Y,
            expected: 0.1d,
            tolerance: 0.001d,
            subject: "height after the caught step"
        );
    }
    [Fact]
    public void KeyingASlotByTheBodyFeatureAloneCollapsesTheCornerAndLosesTheFloor() {
        ...
        Assert.Equal(
            expected: 1,
            actual: world.Slots.ActiveCount
        );
        Assert.True(
    ...
    public void NoConstraintAndNoImpulseAppearBeyondThePredictedBound() {
        ...
        Assert.Equal(
            expected: 0L,
            actual: world.Solver.LastStepMaximumImpulseRaw
        );
        Assert.True(
    ...
    public void RotatingBoxSettlesOnThePlaneWithoutJitter() {
        ...
        for (var step = 0; (step < 120); ++step) {
            world.Advance();
            lowest = FixedQ4816.Min(
                x: lowest,
                y: world.Pose.Center.Y
            );
            highest = FixedQ4816.Max(
                x: highest,
                y: world.Pose.Center.Y
            );
        }
        ...
        MeasurementAssert.Near(
            actual: settled,
            expected: 0.25d,
            subject: "resting height of the box",
            tolerance: 0.002d
        );
    ...
    public void SphereRestingInACornerKeepsTwoPersistentContactsWithDistinctNormals() {
        ...
        var wall = FindSlot(
            sourceId: SpikeFixtures.WallSourceId,
            world: world
        );

        // Distinct normals, both carrying a real accumulated impulse: the corner is two constraints, not one.
        Assert.True(
            condition: (floor.Normal.Y > FixedQ4816.FromDouble(value: 0.99d)),
            userMessage: "the floor contact's normal must be the floor's"
        );
        Assert.True(
            condition: (wall.Normal.X > FixedQ4816.FromDouble(value: 0.99d)),
            userMessage: "the wall contact's normal must be the wall's"
        );
        Assert.True(
            condition: (floor.NormalImpulseRaw > 0L),
            userMessage: "the floor contact must carry an accumulated impulse"
        );
        Assert.True(
            condition: (wall.NormalImpulseRaw > 0L),
            userMessage: "the wall contact must carry an accumulated impulse"
        );

        // The sphere rests exactly one radius from both surfaces, to within the soft constraint's own slop.
        MeasurementAssert.Near(
            actual: world.Pose.Center.X,
            expected: 0.5d,
            tolerance: 0.002d,
            subject: "resting distance from the wall"
        );
        MeasurementAssert.Near(
            actual: world.Pose.Center.Y,
            expected: 0.5d,
            tolerance: 0.002d,
            subject: "resting distance from the floor"
        );
    }
    [Fact]
