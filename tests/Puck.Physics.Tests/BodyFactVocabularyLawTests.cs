...
public sealed class BodyFactVocabularyLawTests {
    ...
    public void EveryFactSpellsADistinctLowerCaseToken() {
        ...
        foreach (var fact in Enum.GetValues<ActionFact>()) {
            ...
            Assert.Equal(
                actual: token,
                expected: token.ToLowerInvariant()
            );
            Assert.True(
                condition: tokens.Add(item: token),
                userMessage: $"{fact} reuses the token '{token}'"
            );
        }
    }
    [Fact]
    public void EveryPublishableFactCarriesADistinctBit_AndAllIsExactlyTheirUnion() {
        ...
        foreach (var fact in BodyFactVocabulary.Publishable) {
            ...
            Assert.True(
                condition: seen.Add(item: bit),
                userMessage: $"{fact} shares a bit with an earlier fact"
            );
    ...
    public void TheEchoJoinsSetBitsInBitOrder_AndReadsNoneWhenEmpty() {
        Assert.Equal(
            actual: BodyFactVocabulary.Describe(facts: BodyFacts.None),
            expected: "none"
        );
        Assert.Equal(
            actual: BodyFactVocabulary.Describe(facts: BodyFacts.Grounded),
            expected: "grounded"
        );
