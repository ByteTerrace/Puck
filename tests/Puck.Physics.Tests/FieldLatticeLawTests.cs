...
public sealed class FieldLatticeLawTests {
    private static FieldLatticeInput Fields(
        ...
        Lattice: new FieldLatticeTopology(
            ...
            CellSize: (cellSize ?? FixedQ4816.One),
            Width: width,
            Depth: depth,
            Layers: layers,
            StepEveryTicks: 1
        ),
        Fields: [new FieldDescriptorInput(
                Name: "heat",
                Initial: FixedQ4816.Zero,
                Minimum: FixedQ4816.Zero,
                Maximum: FixedQ4816.FromInteger(value: 10),
                HeightScale: (heightScale ?? FixedQ4816.Zero),
                IsMedium: false,
                Color: null
            )],
        Reactions: (reactions ?? []),
    ...
    private static StateHandle Row(string name, CellKind kind = CellKind.Fixed) {
        var catalog = StateCatalog.Compile(section: new StateSection(Rows: [new StateRow(
    ...
    private static long SumBits(FieldLattice lattice, int cells) {
        ...
        for (var cell = 0; (cell < cells); cell++) {
            sum = unchecked((sum + (lattice.Value(
                cell: cell,
                field: 0
            ).Value * 31L)));
        }
    ...
    public void ABodyAboveTheDerivedCouplingCeilingDoesNotCouple() {
        ...
        var lattice = new FieldLattice(input: Fields(
            ...
            reactions: [new FieldReactionInput.Emit(
                    Tag: hot,
                    Field: 0,
                    Amount: new FieldScalarInput(
                        Literal: FixedQ4816.FromInteger(value: 4),
                        State: default
                    )
                )]
        ));
    ...
    public void ABodyOutsideTheLatticeOrOverAZeroValueCellHasNoMediumSurface() {
        var zeroValued = new FieldLattice(input: MediumFields(
            initial: FixedQ4816.Zero,
            heightScale: FixedQ4816.FromInteger(value: 5)
        ));
        var outside = new FieldLattice(input: MediumFields(
            initial: FixedQ4816.One,
            heightScale: FixedQ4816.FromInteger(value: 5)
        ));
    ...
    public void AMediumFieldAdmitsAPointAboveItsSingleVoxelLayer_WhereTheFreeSurfaceReachesThatHigh() {
        ...
        var lattice = new FieldLattice(input: MediumFields(
            initial: FixedQ4816.One,
            heightScale: FixedQ4816.FromInteger(value: 5)
        ));
        var submerged = new FixedVector3(
            X: FixedQ4816.FromDouble(value: 1.5),
            Y: FixedQ4816.FromInteger(value: 3),
            Z: FixedQ4816.FromDouble(value: 1.5)
        );
        var aboveSurface = new FixedVector3(
            X: FixedQ4816.FromDouble(value: 1.5),
            Y: FixedQ4816.FromInteger(value: 6),
            Z: FixedQ4816.FromDouble(value: 1.5)
        );

        Assert.True(condition: lattice.IsInsideMedium(
            field: 0,
            position: in submerged
        ));
        Assert.False(condition: lattice.IsInsideMedium(
            field: 0,
            position: in aboveSurface
        ));
    }
    [Fact]
    public void AMediumFieldsSurfaceIsOriginYPlusValueTimesHeightScaleAtTheCoupledCell() {
        ...
        Assert.Equal(
            expected: position.X,
            actual: surface.Value.Point.X
        );
        Assert.Equal(
            expected: position.Z,
            actual: surface.Value.Point.Z
        );
    }
    [Fact]
    public void ANoiseFillIsBitIdenticalAcrossConstructionsAndMovesWithTheWorldSeed() {
        var fill = new FieldFillInput.Noise(
            Field: 0,
            Value: FixedQ4816.One,
            Frequency: 8,
            Threshold: FixedQ4816.FromDouble(value: 0.4),
            Octaves: 3,
            Seed: 7u
        );
        var a = new FieldLattice(
            input: FilledFields(fill: fill),
            worldSeed: 5UL
        );
        var b = new FieldLattice(
            input: FilledFields(fill: fill),
            worldSeed: 5UL
        );
        var rerolled = new FieldLattice(
            input: FilledFields(fill: fill),
            worldSeed: 6UL
        );
        var filled = 0;
    ...
    public void ARowReferencedReactionScalarModulatesChemistryOnlyWhileTheRowIsNonzero() {
        ...
        var input = FilledFields(
            fill: new FieldFillInput.Rect(
                0,
                FixedQ4816.One,
                FixedQ4816.Zero,
                FixedQ4816.Zero,
                FixedQ4816.FromInteger(value: 32),
                FixedQ4816.FromInteger(value: 32)
            ),
            width: 4,
            depth: 4
        ...
        StepOnce();
        Assert.Equal(
            expected: FixedQ4816.FromDouble(value: 0.75).Value,
            actual: lattice.Value(
                cell: 0,
                field: 0
            ).Value
    ...
    public void AScatterFillWritesDiscsAndNothingOutsideThem() {
        var fill = new FieldFillInput.Scatter(
            Field: 0,
            Value: FixedQ4816.One,
            Spacing: 8,
            Radius: 2,
    ...
    public void ASegmentEntirelyBelowTheFreeSurfaceProvesInsideTheMedium_WhereOneCrossingTheSurfaceRefuses() {
        ...
        var to = new FixedVector3(
            X: FixedQ4816.FromDouble(value: 1.6),
            Y: FixedQ4816.FromInteger(value: 3),
            Z: FixedQ4816.FromDouble(value: 1.5)
        );
        var crossing = new FixedVector3(
            X: FixedQ4816.FromDouble(value: 1.5),
            Y: FixedQ4816.FromInteger(value: 6),
            Z: FixedQ4816.FromDouble(value: 1.5)
        );

        Assert.True(condition: lattice.IsSegmentInsideMedium(
            field: 0,
            from: in from,
            to: in to,
            clearance: FixedQ4816.FromDouble(value: 0.1),
            maximumSubdivisions: 8
        ));
        Assert.False(condition: lattice.IsSegmentInsideMedium(
            field: 0,
            from: in from,
            to: in crossing,
            clearance: FixedQ4816.FromDouble(value: 0.1),
    ...
    public void BudgetCostUsesCompilerPassClassesInsteadOfTreatingEveryNodeAsOneCellPass() {
        ...
        var lattice = new FieldLattice(input: Fields(
            width: 2,
            reactions: [
            new FieldReactionInput.Diffuse(
                    Field: 0,
                    Rate: new FieldScalarInput(
                        Literal: FixedQ4816.FromDouble(value: 0.5),
                        State: default
                    )
                ),
        ...
        Assert.Contains(
            ...
            expectedSubstring: "3 node(s) every 1 tick(s)"
        );
        Assert.Contains(
            actualString: cost,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "2 cell(s) x 3 pass(es) = 6 cell visit(s)"
        );
        Assert.Contains(
            actualString: cost,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "bodies 3/8 active/capacity x 1 pass(es) = 8 slot visit(s)"
        );
    }
    [Fact]
    public void CompatibleReactionReplacementPreservesCellsAndExecutesTheNewPlanInDocumentOrder() {
        ...
        var replacement = Fields(reactions: [
            new FieldReactionInput.Transform(
                ...
                Then: [new FieldWriteInput(
                        ...
                        Op: FieldWriteOp.Add,
                        Value: new FieldScalarInput(
                            Literal: FixedQ4816.FromInteger(value: 2),
                            State: default
                        )
                    )]
            ),
            new FieldReactionInput.Decay(
                Field: 0,
                Rate: new FieldScalarInput(
                    Literal: FixedQ4816.FromDouble(value: 0.5),
                    State: default
        ...
        Assert.True(
            condition: lattice.CanInstallInput(
                input: replacement,
        ...
        Assert.Equal(
            expected: preservedRaw,
            actual: lattice.Capture().Raw[0][0]
        );
        Assert.Equal(
            expected: preservedRevision,
            actual: lattice.Revision
        );
        Assert.Same(
            expected: replacement,
            actual: lattice.Input
        );
        ...
        Assert.Equal(
            expected: FixedQ4816.FromDouble(value: 1.5),
            actual: lattice.Value(
                cell: 0,
                field: 0
            )
    ...
    public void DiffusionUsesTheCompiledFieldOrdinalAndSnapshotsBeforeWriting() {
        var lattice = new FieldLattice(input: Fields(
            width: 3,
            reactions: [new FieldReactionInput.Diffuse(
                    Field: 0,
                    Rate: new FieldScalarInput(
                        Literal: FixedQ4816.One,
                        State: default
    ...
    public void DrawFill_PreservesAuthoredPaintOrder() {
        var input = new FieldLatticeInput(
            ...
            Reactions: [],
            Paint: [
                new FieldFillInput.Rect(
                    0,
                    FixedQ4816.FromInteger(value: 9),
                    FixedQ4816.Zero,
                    FixedQ4816.Zero,
                    FixedQ4816.One,
                    FixedQ4816.One
                ),
                new FieldFillInput.DrawMarker(Field: 0),
                new FieldFillInput.Rect(
                    0,
                    FixedQ4816.FromInteger(value: 7),
                    FixedQ4816.One,
                    FixedQ4816.Zero,
        ...
        Assert.Equal(
            expected: FixedQ4816.FromInteger(value: 1),
            actual: lattice.Value(
                cell: 0,
                field: 0
            )
        ...
        Assert.Equal(
            expected: FixedQ4816.FromInteger(value: 7),
            actual: lattice.Value(
                cell: 1,
                field: 0
            )
    ...
    public void ExposureWritesTheCompiledStateHandle() {
        ...
        lattice.Step(
            ...
            bodyCount: 1,
            host: new LambdaHost(
                bodyPosition: static _ => new FixedVector3(
                    X: FixedQ4816.FromDouble(value: 0.5),
    ...
    public void LiveFieldEnvelopeChangesRefuseInsteadOfMigratingCells() {
        ...
        var incompatible = Fields() with {
            Fields = [new FieldDescriptorInput(
                ...
                Initial: FixedQ4816.Zero,
                Minimum: FixedQ4816.Zero,
                Maximum: FixedQ4816.FromInteger(value: 20),
                HeightScale: FixedQ4816.Zero,
                IsMedium: false,
                Color: null
            )],
        ...
        Assert.False(condition: lattice.CanInstallInput(
            input: incompatible,
            reason: out var reason
        ));
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "restart the host"
        );
        Assert.Same(
            expected: liveInput,
            actual: lattice.Input
    ...
    public void MultipleWritesToOneCell_DeliverOneFinalDelta() {
        ...
        Assert.Equal(
            expected: FixedQ4816.FromInteger(value: 3).Value,
            actual: delta.Raw
        );
    }
    [Fact]
    public void PrimerFullTake_DoesNotConsumeSharedIncrementalDeltas() {
        ...
        var primer = lattice.TakeDeltas(
            full: true,
            isFull: out var primerFull
        );
        var incremental = lattice.TakeDeltas(
            full: false,
            isFull: out var incrementalFull
        );
    ...
    public void ReadBackNamesTheCompiledExecutionAndDependencyPlan() {
        ...
        Assert.Contains(
            actualString: readBack,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "plan nodes=2"
        );
        Assert.Contains(
            actualString: readBack,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "order=[0:transform,1:decay]"
        );
        Assert.Contains(
            actualString: readBack,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "dependencies=[0>1]"
        );
    }
    [Fact]
