...
public sealed class FixedSurfaceQueryTests {
    ...
    public void DirectedVariantExcludesCandidatesOutsideTheAssistCone() {
        ...
        var perpendicular = FixedStaticCollider.Sphere(
            center: V(
                x: 0d,
                y: 1d,
                z: 0d
            ),
            radius: Q(value: 0.001d)
        );
    ...
    public void DirectedVariantRanksAngularDeviationBeforeDistanceSoTheNearerCandidateCanLoseOnAngle() {
        ...
        var near = FixedStaticCollider.Sphere(
            center: V(
                x: 1d,
                y: 0.6d,
                z: 0d
        ...
        var far = FixedStaticCollider.Sphere(
            center: V(
                x: 2d,
                y: 0.05d,
                z: 0d
        ...
        Assert.Equal(
            expected: 0,
            actual: swappedCandidate.ColliderIndex
        );
        Assert.Equal(
            expected: candidate.Point,
            actual: swappedCandidate.Point
        );
    }
    [Fact]
    ...
    public void NormalsAreUnitLengthAndPushOutwardAcrossEveryColliderKind() {
        FixedStaticCollider[] colliders = [
            FixedStaticCollider.AxisAlignedBox(
                center: FixedVector3.Zero,
                halfExtents: V(
                    x: 1d,
                    y: 1d,
    ...
    public void ResultIsDeterministicAndTieBreaksByAscendingIndexAcrossOneThousandShuffledRebuilds() {
        ...
        for (var trial = 0; (trial < 1000); trial++) {
            ...
            Assert.True(
                condition: found,
                userMessage: $"trial {trial} lost the result entirely"
            );
            ...
            Assert.Equal(
                expected: expectedIndexThisTrial,
                actual: candidate.ColliderIndex
            );
            Assert.Equal(
                expected: FixedSurfaceColliderSource.Static,
            ...
            Assert.Equal(
                expected: shuffled[expectedIndexThisTrial],
                actual: shuffled[candidate.ColliderIndex]
            );
        }
    }
    [Fact]
    ...
    public void StaticSpanBeatsDynamicSpanAtEqualDistanceAndIndex() {
        var probe = FixedVector3.Zero;
        var staticSphere = FixedStaticCollider.Sphere(
            center: V(
                x: 5d,
                y: 0d,
                z: 0d
        ...
        var dynamicSphere = FixedStaticCollider.Sphere(
            center: V(
                x: 0d,
                y: 5d,
                z: 0d
