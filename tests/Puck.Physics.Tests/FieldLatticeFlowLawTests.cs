...
public sealed class FieldLatticeFlowLawTests {
    ...
    private static FieldLatticeInput FlowFields(
        ...
        var fields = new List<FieldDescriptorInput> {
            ...
            Maximum: FixedQ4816.FromInteger(value: 100),
            HeightScale: (waterHeightScale ?? FixedQ4816.One),
            IsMedium: false,
            Color: "#3B7BD6"
        ),
        };
        ...
        if (includeGround) {
            fields.Add(item: new FieldDescriptorInput(
                Name: "ground",
                Initial: FixedQ4816.Zero,
                Minimum: FixedQ4816.Zero,
                Maximum: FixedQ4816.FromInteger(value: 100),
                HeightScale: (groundHeightScale ?? FixedQ4816.One),
                IsMedium: false,
                Color: "#808080"
            ));
            over = [1];
        ...
        return new FieldLatticeInput(
            ...
            Fields: fields,
            Reactions: [new FieldReactionInput.Flow(
                    Field: 0,
                    Rate: new FieldScalarInput(
                        Literal: (rate ?? FixedQ4816.One),
                        State: default
                    ),
                    Over: (((IReadOnlyList<int>?)over) ?? []),
                    SpillRow: spillRow
                )],
            Paint: []
        );
    }
    private static FieldLattice.Checkpoint IntCheckpoint(params int[][] fieldsRaw) => new(Raw: [.. fieldsRaw.Select(selector: static values => values.Select(selector: static v => FixedQ4816.FromInteger(value: v).Value).ToArray())]);
    ...
    private static void StepLattice(FieldLattice lattice) => lattice.Step(
        tick: 1,
        bodyCount: 0,
        host: new NoopHost()
    );
    ...
    public void ARampTerrainMovesMassStrictlyTowardTheLowEnd() {
        ...
        var lattice = new FieldLattice(input: FlowFields(
            width: Width,
            depth: 1,
            waterHeightScale: FixedQ4816.Zero
        ));

        lattice.Restore(checkpoint: IntCheckpoint(
            [8, 0, 0, 0],
            [6, 4, 2, 0]
        ));
        ...
        for (var step = 0; (step < 3); step++) {
            ...
            Assert.True(
                condition: (next >= position),
                userMessage: $"step {step}: weighted position fell from {position} to {next}."
            );
            position = next;
        }
    ...
    public void AnEdgeCellSpillsExactlyItsShareIntoSpillRowAndTheWholeSystemStillConserves() {
        ...
        Assert.Equal(
            expected: FixedQ4816.FromInteger(value: 10).Value,
            actual: (SumRaw(
                cellCount: 2,
                field: 0,
                lattice: lattice
    ...
    public void FlatTerrainLevelsAWaterSpikeWithTheGlobalMaxMinGapNeverIncreasing() {
        ...
        for (var step = 0; (step < 12); step++) {
            ...
            Assert.True(
                condition: (nextGap <= gap),
                userMessage: $"step {step}: gap grew from {gap} to {nextGap}."
            );
            Assert.Equal(
                expected: FixedQ4816.FromInteger(value: 10).Value,
                actual: next.Sum()
    ...
    public void TwoIdenticallyConstructedLatticesStayBitIdenticalAcrossSteps() {
        ...
        var a = new FieldLattice(input: FlowFields(
            width: Width,
            depth: Depth
        ));
        var b = new FieldLattice(input: FlowFields(
            width: Width,
            depth: Depth
        ));
    ...
    private sealed class SpillHost(Action<StateHandle, FixedQ4816> onAddScalar) : IFieldLatticeHost {
        public void AddScalar(StateHandle row, FixedQ4816 amount, ulong tick) => onAddScalar(
            row,
            amount
        );
        public FixedVector3? BodyPosition(int body) => null;
