...
public sealed class GravitySolverTests {
    private static readonly GravityParameters Parameters = new(
    ...
    private static double RelativeRootMeanSquareError(ReadOnlySpan<FixedVector3> expected, ReadOnlySpan<FixedVector3> actual) {
        ...
        for (var index = 0; (index < expected.Length); index++) {
            Accumulate(
                componentExpected: expected[index].X,
                componentActual: actual[index].X,
                squaredError: ref squaredError,
                squaredSignal: ref squaredSignal
            );
            Accumulate(
                componentExpected: expected[index].Y,
                componentActual: actual[index].Y,
                squaredError: ref squaredError,
                squaredSignal: ref squaredSignal
            );
            Accumulate(
                componentExpected: expected[index].Z,
                componentActual: actual[index].Z,
                squaredError: ref squaredError,
                squaredSignal: ref squaredSignal
            );
        }
    ...
    public void AdaptiveFmmDefersAnAncestorExpansionWhoseGradientCannotBeCombined() {
        GravityBody[] bodies = [
            ...
            new(
                ...
                Mass: Scalar(value: 700_000_000d)
            ),
            new(
                Position: Vector(
                    x: 4d,
                ...
                Mass: Scalar(value: 20_000_000_000d)
            ),
        ];
        ...
        Assert.True(
            condition: (RelativeRootMeanSquareError(
                actual: fmm,
                expected: exact
            ) < 0.2d),
    ...
    public void AdaptiveFmmLargePopulationUsesCellTranslationsInsteadOfTargetTreeWalks() {
        ...
        var barnesHutStatistics = new FastMonopoleGravitySolver(options: new FastMonopoleOptions(
            ...
            accelerations: barnesHutAccelerations,
            bodies: bodies,
            parameters: Parameters
        );
        var fmmStatistics = new AdaptiveFmmGravitySolver(options: new AdaptiveFmmOptions(
            ...
            accelerations: fmmAccelerations,
            bodies: bodies,
            parameters: Parameters
        );
        ...
        Assert.DoesNotContain(
            collection: fmmAccelerations,
            filter: static acceleration => (acceleration == FixedVector3.Zero)
        );
    }
    [Fact]
    public void AdaptiveFmmOpensCellPairsWhoseGradientLeavesTheGrid() {
        ...
        var relativeRootMeanSquareError = RelativeRootMeanSquareError(
            actual: fmm,
            expected: exact
        );
    ...
    public void AdaptiveFmmRunsDeterministicTranslationAndDownwardPasses() {
        ...
        var firstStatistics = solver.ComputeAccelerations(
            accelerations: first,
            bodies: bodies,
            parameters: Parameters
        );
        var secondStatistics = solver.ComputeAccelerations(
        ...
        Assert.Equal(
            actual: secondStatistics,
            expected: firstStatistics
        );
        ...
        Assert.Equal(
            ...
            actual: firstStatistics.LocalExpansionEvaluations
        );
        Assert.Equal(
            expected: 0L,
            actual: firstStatistics.ApproximatedNodeEvaluations
        );
    }
    [Fact]
    public void AdaptiveFmmTracksPairwiseAndBarnesHutWithinMeasuredErrorEnvelope() {
        ...
        var barnesHutError = RelativeRootMeanSquareError(
            actual: barnesHut,
            expected: exact
        );
        var fmmError = RelativeRootMeanSquareError(
            actual: fmm,
            expected: exact
        );
    ...
    public void AdaptiveFmmZeroOpeningAngleIsTheBitExactPairwiseOracle() {
        ...
        var fmm = new FixedVector3[bodies.Length];
        var exactStatistics = new PairwiseGravitySolver().ComputeAccelerations(
            accelerations: pairwise,
            bodies: bodies,
            parameters: Parameters
        );
        var fmmStatistics = new AdaptiveFmmGravitySolver(options: new AdaptiveFmmOptions(
        ...
        Assert.Equal(
            actual: fmmStatistics,
            expected: exactStatistics
        );
    }
    [Fact]
    public void FactoryCreatesTheRequestedKindWithForwardedOptions() {
        ...
        Assert.Same(
            expected: monopoleOptions,
            actual: monopole.Options
        );
        Assert.Same(
            expected: fmmOptions,
            actual: fmm.Options
    ...
    public void InvalidPhysicalDomainsRefuseByName() {
        ...
        Assert.Equal(
            expected: "bodies",
            actual: massException.ParamName
        );
        Assert.Equal(
            expected: "parameters",
            actual: softeningException.ParamName
        );
    }
    [Fact]
    public void PairwiseEqualMassPairProducesExactOpposites() {
        ...
        Assert.Equal(
            ...
            actual: accelerations[0].Y
        );
        Assert.Equal(
            expected: FixedQ4816.Zero,
            actual: accelerations[0].Z
        );
        Assert.Equal(
            expected: GravityKernelForTests.Negate(vector: accelerations[0]),
            actual: accelerations[1]
        );
        Assert.Equal(
            expected: 2L,
            actual: statistics.ExactSourceEvaluations
        );
        Assert.Equal(
            expected: 0L,
            actual: statistics.ApproximatedNodeEvaluations
        );
    }
    [Fact]
    ...
    public void ZeroOpeningAngleIsTheBitExactPairwiseOracle() {
        ...
        var exactStatistics = exactSolver.ComputeAccelerations(
            accelerations: pairwise,
            bodies: bodies,
            parameters: Parameters
        );
        var selectedStatistics = selectedSolver.ComputeAccelerations(
            accelerations: monopole,
            bodies: bodies,
            parameters: Parameters
        );
