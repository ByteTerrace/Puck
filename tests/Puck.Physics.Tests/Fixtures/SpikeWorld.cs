...
internal sealed class SpikeWorld {
    ...
    internal SpikeWorld(FixedRigidSolverOptions options, FixedRigidBody body, BodyPose pose, SpikeShape shape, FixedQ4816 reach, params ISpikeSurface[] surfaces) {
    ...
    private FixedQ4816 ActivationBound() {
        if (m_options.Activation == FixedSpeculativeActivation.CurrentOnly) {
            return m_options.ContactMargin;
        }

        if (m_options.Activation == FixedSpeculativeActivation.NearestRounded) {
        ...
        return (m_options.ContactMargin + FixedRigidArithmetic.CeilingProductSum(
            left: FixedRigidArithmetic.CeilingMagnitude(value: Body.LinearVelocity),
            right: m_stepSecondsCeiling,
            addend: spin
        ));
    }
...
internal static class SpikeBodies {
    ...
    /// <returns>The body.</returns>
    /// <exception cref="InvalidOperationException">A mass-property kernel refused the authored solid.</exception>
    internal static FixedRigidBody Box(FixedVector3 halfExtents, FixedQ4816 density, FixedRigidScales scales) {
    ...
    internal static FixedRigidBody CapsuleAlongX(FixedQ4816 radius, FixedQ4816 centerDistance, FixedQ4816 density, FixedRigidScales scales) {
