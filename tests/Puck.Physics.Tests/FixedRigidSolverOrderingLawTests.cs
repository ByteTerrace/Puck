...
public sealed class OrderingLawTests {
    ...
    private static SpikeWorld Build(int fixtureIndex) =>
        fixtureIndex switch {
            0 => SpikeFixtures.Corner(options: SpikeFixtures.CornerOptions(
            rateHz: 60,
            substepCount: 4
        )),
            1 => SpikeFixtures.CapsuleWaist(
            options: new() { RateHz = 60, SubstepCount = 4, },
            mode: Geometry.CapsuleWitnessMode.SegmentScan,
            surface: out _
        ),
            2 => SpikeFixtures.RotatingBox(options: new() { RateHz = 60, SubstepCount = 4, }),
            3 => SpikeFixtures.HighSpeedApproach(
            options: new() { RateHz = 60, SubstepCount = 1, },
            height: 1d,
            downwardSpeed: 400d
        ),
            4 => SpikeFixtures.DeepOverlap(options: new() { RateHz = 60, SubstepCount = 4, }),
            _ => SpikeFixtures.BoxInCorner(options: SpikeFixtures.BoxInCornerOptions(
            rateHz: 60,
            substepCount: 4
        )),
        };
    ...
    private static ulong RunPermuted(int key, bool canonicalOrder, int steps) {
        ...
        world.Permutation = candidates => Permute(
            key: key,
            source: candidates
        );
    ...
    public void ShuffledCandidateOrderYieldsBitIdenticalState() {
        var reference = RunPermuted(
            key: PermutationKeys[0],
            canonicalOrder: true,
            steps: 240
        );

        for (var index = 1; (index < PermutationKeys.Length); ++index) {
            var permuted = RunPermuted(
                key: PermutationKeys[index],
                canonicalOrder: true,
                steps: 240
            );
    ...
    public void TwoRunsFromIdenticalStateAgreeBitForBitOverAThousandSteps(int fixtureIndex) {
        ...
        Assert.Equal(
            expected: 0,
            actual: first.Solver.RefusalCount
        );
        Assert.Equal(
            expected: 0,
            actual: second.Solver.RefusalCount
        );
    }
    [Fact]
    ...
    public void WithoutTheCanonicalSortShuffledCandidateOrderDiverges() {
        ...
        Assert.True(
            condition: diverged,
            userMessage: "removing the canonical sort must let candidate order reach the result"
        );
    }
}
