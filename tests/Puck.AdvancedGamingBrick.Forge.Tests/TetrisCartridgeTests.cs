...
public sealed class TetrisCartridgeTests {
    ...
    public void ACompletedRowClearsAndCountsTowardTheLineTotal() {
        ...
        foreach (var column in new[] { 0, 1, 2, 7, 8, 9 }) {
            machine.Write(
                address: ((ushort)((result.Arrays["field"] + (17 * 10)) + ((uint)column))),
                value: 1
            );
        }
        ...
        Assert.Equal(
            expected: 1,
            actual: machine.Read(address: ((ushort)result.Variables["lines"]))
        );
    ...
    public void AFinishedRunIsMeasuredAgainstTheBestAndKeptWhenItWins() {
        ...
        for (var row = 0; (row < 6); ++row) {
            for (var column = 0; (column < 9); ++column) {
                machine.Write(
                    address: ((ushort)(result.Arrays["field"] + ((uint)((row * 10) + column)))),
                    value: 1
                );
            }
        }
        ...
        Assert.Equal(
            expected: 9,
            actual: machine.Read(address: ((ushort)result.Variables["ph"]))
        );
        Assert.Equal(
            expected: scored,
            actual: Digits(
                machine: machine,
                address: result.Arrays["best"]
            )
        );
    }
    [Fact]
    public void AGameSoundsOnThreeVoicesAndTheTitleOnThreeOfItsOwn() {
        ...
        Assert.Equal(
            expected: 0x07,
            actual: machine.Read(address: SoundStatus) & 0x07
        );
    ...
    public void APieceComesToRestOnTheFloorAndAnotherFollowsIt() {
        ...
        Assert.Equal(
            expected: 0,
            actual: machine.Read(address: ((ushort)result.Variables["over"]))
        );
    }
    [Fact]
    ...
    public void TheCartridgeOpensOnItsTitleAndWalksThroughTheModeMenuIntoAGame() {
        ...
        Assert.Contains(
            expected: machine.Read(address: phase),
        ...
        Assert.Equal(
            expected: 18,
            actual: machine.Read(address: phase)
        );
        ...
        Assert.Equal(
            expected: 1,
            actual: machine.Read(address: ((ushort)result.Variables["gtype"]))
        );
        Assert.Equal(
            expected: 0,
            actual: machine.Read(address: phase)
    ...
    public void WinningTheSecondModeLaunchesTheRocketAndReturnsToTheTitle() {
        ...
        machine.Write(
            address: ((ushort)result.Variables["lines"]),
            value: 25
        );
        machine.RunFrames(
            buttons: JoypadButtons.None,
