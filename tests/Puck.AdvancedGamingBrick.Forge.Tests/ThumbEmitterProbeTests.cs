...
public sealed class ThumbEmitterProbeTests {
    ...
    private static AgbVerifyMachineDriver RunProbe(Action<ThumbEmitter> emit, int steps = 256) {
        ...
        var driver = new AgbVerifyMachineDriver(
            label: "probe",
            rom: rom
        );
    ...
    public void AluRegisterOperations() {
        ...
        Assert.Equal(
            expected: 0x30u,
            actual: driver.ReadRegister(index: 2)
        );
        Assert.Equal(
            expected: 0xFCu,
            actual: driver.ReadRegister(index: 3)
        );
        Assert.Equal(
            expected: 0xCCu,
            actual: driver.ReadRegister(index: 4)
        );
        Assert.Equal(
            expected: 420u,
            actual: driver.ReadRegister(index: 5)
        );
        Assert.Equal(
            expected: 0xFFFFFF0Fu,
            actual: driver.ReadRegister(index: 6)
        );
        Assert.Equal(
            expected: unchecked((uint)(-5)),
            actual: driver.ReadRegister(index: 7)
        );
    }
    [Fact]
    public void CallReturnAndNestedCalls() {
        ...
        Assert.Equal(
            expected: 111u,
            actual: driver.ReadRegister(index: 0)
        );
    }
    [Fact]
    public void ConditionalAndBackwardBranches() {
        using var driver = RunProbe(emit: static emitter => {
            ...
            emitter.CompareImmediate(
                register: LowRegister.R3,
                value: 0
            );
            emitter.Branch(
                condition: ThumbCondition.SignedGreaterOrEqual,
                label: signedSkip
            );
            emitter.MoveImmediate(
                destination: LowRegister.R4,
    ...
    public void HiRegisterOperations() {
        using var driver = RunProbe(emit: static emitter => {
            ...
            emitter.MoveHigh(
                destination: CoreRegister.R8,
                source: CoreRegister.R0
            );
            emitter.AddHigh(
                destination: CoreRegister.R8,
                source: CoreRegister.R0
            ...
            emitter.MoveHigh(
                destination: CoreRegister.R1,
                source: CoreRegister.R8
            );
            emitter.MoveHigh(
                destination: CoreRegister.R2,
                source: CoreRegister.Sp
            );
        });

        Assert.Equal(
            expected: 10u,
            actual: driver.ReadRegister(index: 8)
        );
        Assert.Equal(
            expected: 10u,
            actual: driver.ReadRegister(index: 1)
        );
        Assert.Equal(
            expected: 0x03007F00u,
            actual: driver.ReadRegister(index: 2)
        );
    }
    [Fact]
    ...
    public void LoadStoreImmediateOffsets() {
        ...
        Assert.Equal(
            expected: 0x11223344u,
            actual: driver.ReadWord(address: ScratchBase)
        );
        Assert.Equal(
            expected: ((byte)0x44),
            actual: driver.ReadByte(address: (ScratchBase + 8u))
        );
        Assert.Equal(
            expected: ((ushort)0x3344),
            actual: driver.ReadHalf(address: (ScratchBase + 12u))
        );
    }
    [Fact]
    public void LoadStoreRegisterOffsetsAndSignExtension() {
        using var driver = RunProbe(emit: static emitter => {
            ...
            emitter.StoreHalfRegister(
                baseRegister: LowRegister.R0,
                offsetRegister: LowRegister.R1,
                source: LowRegister.R2
            );
            emitter.LoadByteRegister(
                baseRegister: LowRegister.R0,
                destination: LowRegister.R3,
                offsetRegister: LowRegister.R1
            );
            emitter.LoadSignedByteRegister(
                baseRegister: LowRegister.R0,
                destination: LowRegister.R4,
                offsetRegister: LowRegister.R1
            );
            emitter.LoadHalfRegister(
                baseRegister: LowRegister.R0,
                destination: LowRegister.R5,
                offsetRegister: LowRegister.R1
            );
            ...
            emitter.StoreWordRegister(
                baseRegister: LowRegister.R0,
                offsetRegister: LowRegister.R1,
                source: LowRegister.R2
            );
            emitter.LoadWordRegister(
                baseRegister: LowRegister.R0,
                destination: LowRegister.R7,
                offsetRegister: LowRegister.R1
            );
        ...
        Assert.Equal(
            expected: 0xFFFF80FEu,
            actual: driver.ReadRegister(index: 6)
        );
        Assert.Equal(
            expected: 0x000080FEu,
            actual: driver.ReadRegister(index: 7)
        );
        Assert.Equal(
            expected: ((byte)0xFE),
            actual: driver.ReadByte(address: (ScratchBase + 0x21u))
        );
    }
    [Fact]
    public void MoveAddSubtractCompareImmediates() {
        using var driver = RunProbe(emit: static emitter => {
            ...
            emitter.SubtractImmediate3(
                destination: LowRegister.R4,
                source: LowRegister.R0,
                value: 7
            );
            emitter.AddImmediate3(
                destination: LowRegister.R6,
                source: LowRegister.R1,
                value: 4
            );
    ...
    public void ShiftImmediateAndRegisterShifts() {
        using var driver = RunProbe(emit: static emitter => {
            ...
            emitter.Alu(
                destination: LowRegister.R5,
                op: ThumbAlu.LogicalLeft,
                source: LowRegister.R4
            );
            emitter.MoveImmediate(
                destination: LowRegister.R6,
                value: 0x10
        ...
        Assert.Equal(
            expected: 0x08000000u,
            actual: driver.ReadRegister(index: 2)
        );
        Assert.Equal(
            expected: 0xF8000000u,
            actual: driver.ReadRegister(index: 3)
        );
        Assert.Equal(
            expected: 0x800u,
    ...
    public void StackOperations() {
        using var driver = RunProbe(emit: static emitter => {
            ...
            emitter.Push(
                includeLinkRegister: false,
                registers: LowRegisterMask.R0
            );
            emitter.Pop(
                includeProgramCounter: false,
                registers: LowRegisterMask.R1
            );
            ...
            emitter.StoreSpRelative(
                byteOffset: 4,
                source: LowRegister.R2
            );
            emitter.LoadSpRelative(
                byteOffset: 4,
                destination: LowRegister.R3
            );
            emitter.AddToStackPointer(byteOffset: 8);
            emitter.MoveHigh(
                destination: CoreRegister.R4,
                source: CoreRegister.Sp
            );
        });
