...
public sealed class TetrisSideBySide {
    ...
    private static string Beside(VerifyMachineDriver left, VerifyMachineDriver right) {
        ...
        for (var y = 0; (y < Height); y += 8) {
            page.Append(value: Row(
                machine: left,
                y: y
            )).Append(value: "  |  ").AppendLine(value: Row(
                machine: right,
                y: y
            ));
        }
    ...
    public void RenderBothForComparison() {
        ...
        var reference = (Path.IsPathRooted(path: named)
            ? named
            : Path.Combine(
                path1: Checkout(),
                path2: named
        ...
        using var ours = new VerifyMachineDriver(
            rom: authored.Rom,
            label: "authored"
        );
        using var theirs = new VerifyMachineDriver(
            rom: File.ReadAllBytes(path: reference),
            label: "reference"
        );
        ...
        var destination = (Environment.GetEnvironmentVariable(variable: "PUCK_TETRIS_COMPARE_OUT")
            ...
            path2: "tetris-side-by-side.txt"
        ));

        File.WriteAllText(
            path: destination,
            contents: page.ToString()
        );
    }
}
