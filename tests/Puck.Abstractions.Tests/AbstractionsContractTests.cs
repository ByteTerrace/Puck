...
public sealed class AbstractionsContractTests {
    ...
    public void NormalizedInputAndCameraRejectNonFiniteValues() {
        _ = Assert.Throws<ArgumentOutOfRangeException>(testCode: () => new MachinePadState(
            default,
            new Vector2(
                x: float.NaN,
                y: 0
        ...
        _ = Assert.Throws<ArgumentException>(testCode: () => CameraSnapshot.LookAt(
            new Vector3(
                x: float.NaN,
                y: 0,
                z: 0
        ...
        _ = Assert.Throws<ArgumentOutOfRangeException>(testCode: () => CameraSnapshot.LookAt(
            Vector3.Zero,
            -Vector3.UnitZ,
            1f,
            1,
    ...
    public void PushConstantsRequireDefinedStagesAndWordAlignment() {
        _ = new GpuPushConstantBinding(
            data: new byte[4],
            offset: 0,
            stageFlags: GpuShaderStage.Compute
        );

        _ = Assert.Throws<ArgumentOutOfRangeException>(testCode: () => new GpuPushConstantBinding(
            data: new byte[4],
            offset: 2,
            stageFlags: GpuShaderStage.Compute
        ));
        ...
        _ = Assert.Throws<ArgumentOutOfRangeException>(testCode: () => new GpuPushConstantBinding(
            data: new byte[4],
            offset: 0,
            stageFlags: ((GpuShaderStage)0x8000_0000u)
        ));
    }
    [Fact]
    public void ShaderValidationWalksContainerStructure() {
        ...
        BinaryPrimitives.WriteUInt32LittleEndian(
            destination: spirV.AsSpan(start: 4),
            value: 0x00010000
        );
        BinaryPrimitives.WriteUInt32LittleEndian(
            destination: spirV.AsSpan(start: 12),
            value: 1
        );
        BinaryPrimitives.WriteUInt32LittleEndian(
            destination: spirV.AsSpan(start: 20),
            value: 0x00010000
        );
        ...
        BinaryPrimitives.WriteUInt32LittleEndian(
            destination: truncatedInstruction.AsSpan(start: 20),
            value: 0x00020000
        );
        ...
        BinaryPrimitives.WriteUInt32LittleEndian(
            destination: dxbc.AsSpan(start: 24),
            value: ((uint)dxbc.Length)
        );
        BinaryPrimitives.WriteUInt32LittleEndian(
            destination: dxbc.AsSpan(start: 28),
            value: 1
        );
        BinaryPrimitives.WriteUInt32LittleEndian(
            destination: dxbc.AsSpan(start: 32),
            value: 36
        );
        BinaryPrimitives.WriteUInt32LittleEndian(
            destination: dxbc.AsSpan(start: 36),
            value: 0x4c495844
        );
        ShaderBytecode.ValidateFormat(bytecode: dxbc);

        BinaryPrimitives.WriteUInt32LittleEndian(
            destination: dxbc.AsSpan(start: 40),
            value: 1
        );
