...
public sealed class GpuWorkCountingLawTests {
    ...
    private static Dictionary<string, (Action<Rig> Act, (WorkKind Kind, long Amount)[] Expected)> Cases() {
        ...
        return new(comparer: StringComparer.Ordinal) {
            ...
            ["IGpuBindings.CreatePool"] = (rig => rig.Services.Bindings.CreatePool(sizes: new GpuDescriptorPoolSizes(MaxSets: 1, StorageBufferCount: 1, StorageImageCount: 0), name: default), [(GpuWork.DescriptorPoolsCreated, 1L)]),
