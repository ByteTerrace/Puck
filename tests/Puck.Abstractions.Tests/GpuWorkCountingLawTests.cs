...
public sealed class GpuWorkCountingLawTests {
    ...
    private static Dictionary<string, (Action<Rig> Act, (WorkKind Kind, long Amount)[] Expected)> Cases() {
        ...
        return new(comparer: StringComparer.Ordinal) {
            ...
            ["IGpuPipelineFactory.Create(compute)"] = (rig => rig.Services.PipelineFactory.Create(computeShaderModule: null!, description: null!, name: default).Dispose(), [(GpuWork.PipelinesCreated, 1L)]),
            ["IGpuBindings.AllocateSet"] = (rig => rig.Services.Bindings.AllocateSet(descriptorSetLayoutHandle: 3, name: default, poolHandle: 2), [(GpuWork.DescriptorSetsCreated, 1L)]),
            ["IGpuBindings.CanAdmit"] = (rig => _ = rig.Services.Bindings.CanAdmit(owner: "candidate", pools: [], refusal: out _), none),
            ["IGpuBindings.CreatePool"] = (rig => rig.Services.Bindings.CreatePool(sizes: new GpuDescriptorPoolSizes(CombinedImageSamplerCount: 0, MaxSets: 1, StorageBufferCount: 1, StorageImageCount: 0), name: default), [(GpuWork.DescriptorPoolsCreated, 1L)]),
            ...
            ["IGpuStorageBuffer.Write"] = (rig => rig.Services.BufferFactory.CreateHostVisible(name: default, sizeBytes: 64UL, usage: GpuBufferUsage.Storage).Write<float>(data: [1f, 2f, 3f]), [(GpuWork.BuffersCreated, 1L), (GpuWork.HostVisibleUploadBytes, 12L)]),
            ["IGpuStorageBuffer.Write(offset)"] = (rig => rig.Services.BufferFactory.CreateHostVisible(name: default, sizeBytes: 64UL, usage: GpuBufferUsage.Storage | GpuBufferUsage.Indirect).Write<uint>(data: [1u, 2u], destinationOffsetBytes: 8UL), [(GpuWork.BuffersCreated, 1L), (GpuWork.HostVisibleUploadBytes, 8L)]),
            ["IGpuBufferFactory.CreateHostVisible"] = (rig => rig.Services.BufferFactory.CreateHostVisible(name: default, sizeBytes: 64UL, usage: GpuBufferUsage.Storage), [(GpuWork.BuffersCreated, 1L)]),
            ["IGpuBufferFactory.CreateHostVisibleDeviceLocal"] = (rig => rig.Services.BufferFactory.CreateHostVisibleDeviceLocal(name: default, sizeBytes: 64UL, usage: GpuBufferUsage.Storage), [(GpuWork.BuffersCreated, 1L)]),
