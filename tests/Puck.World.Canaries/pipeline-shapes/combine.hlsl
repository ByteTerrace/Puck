// The second compute stage: mixed = (2 * field.r, word at byte 0 / 256, word at byte 4 / 256, 1) = (3/4, 3/8, 1/8, 1).
// Reading byte 4 is what proves the table is bound as a byte-address buffer.
#include "combine.interface.hlsli"

[numthreads(8, 8, 1)]
void main(uint3 id : SV_DispatchThreadID) {
    if (any(id.xy >= passGroup.extent)) {
        return;
    }
    float2 uv = ((float2(id.xy) + 0.5) / float2(passGroup.extent));
    float value = field.SampleLevel(fieldSampler, uv, 0.0).r;

    mixed[id.xy] = float4((2.0 * value), (float(words.Load(0)) / 256.0), (float(words.Load(4)) / 256.0), 1.0);
}
