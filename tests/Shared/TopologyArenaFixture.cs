...
/// <summary>The section the arena-facing topology, pattern, and generator law suites store: a token domain, an
/// ordered pile over it, an attribute row keyed by the same tokens, a lattice board, a draw site, and a four-member
/// family of slots.</summary>
internal static class TopologyArenaFixture {
    ...
    public static (StateCatalog Catalog, StateArena Arena) Build() => Build(section: Section());
    /// <summary>Builds a catalog and an arena at the time origin over <paramref name="section"/>, with default
    /// options.</summary>
    /// <param name="section">The section the arena stores.</param>
    /// <returns>The catalog and its arena.</returns>
    public static (StateCatalog Catalog, StateArena Arena) Build(StateSection section) {
