...
public sealed class ShaderProgramLawTests {
    ...
    public void Build_RefusesABackwardJump() {
        ...
        Assert.Contains(
            ...
            expectedSubstring: "forward jumps only"
        );
    }
    [Fact]
    ...
    public void DirectionalGradientFixture_IsExpressedWithGenericStackOperations() {
        ...
        Assert.Equal(
            actual: operations[^1],
            expected: ShaderOp.Halt
        );
        Assert.Contains(
            collection: operations,
            expected: ShaderOp.Swizzle
        );
        Assert.Contains(
            collection: operations,
            expected: ShaderOp.Lerp
        );
        Assert.DoesNotContain(
            collection: operations,
            expected: ShaderOp.Jump
        );
    }
    [Fact]
    public void Fbm2_StaysWithinTheUnitInterval() {
        var program = ShaderExpressionCompiler.Compile(root: ShaderMath.Fbm2(
            octaves: 4,
            position: ShaderExpression.Input(input: ShaderInput.Coordinate)
        ));
        ...
        for (var step = 0; (step < 512); step++) {
            ...
            var value = ShaderInterpreter.Evaluate(
                context: in context,
                parameters: [],
                program: program
            ).X;
    ...
    public void FromWords_CopiesTheValidatedInput() {
        ...
        Assert.Equal(
            actual: program.Words.Span[ShaderIsa.HeaderWordCount],
            expected: first
        );
    }
    [Fact]
    public void FromWords_RejectsAnUndeclaredOperation() {
        ...
        Assert.Contains(
            actualString: exception.Message,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: nameof(ShaderOp)
        );
    }
    [Fact]
    public void Hash3_AgreesWithThePinnedPcgLattice() {
        var expected = ShaderIsa.Pcg3d(
            x: 7u,
            y: 9u,
            z: 1337u
        );
        ...
        Assert.Equal(
            actual: ShaderInterpreter.Evaluate(
                context: in context,
                parameters: [],
                program: program
            ).X,
