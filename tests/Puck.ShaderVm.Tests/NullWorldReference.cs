...
public static class NullWorldReference {
    ...
    private static float Cylinder(Vector3 point, float radius, float halfHeight) {
        ...
        var outsideX = MathF.Max(
            x: radial,
            y: 0f
        );
        var outsideY = MathF.Max(
            x: axial,
            y: 0f
        );

        return (MathF.Sqrt(x: ((outsideX * outsideX) + (outsideY * outsideY))) + MathF.Min(
            x: MathF.Max(
                x: radial,
                y: axial
            ),
    ...
    private static (float Distance, int Material) Pillars(Vector3 point) {
        var folded = new Vector3(
            x: MathF.Abs(x: point.X),
            y: point.Y,
            z: MathF.Abs(x: point.Z)
        );
        var plinth = (Distance: Cylinder(
            halfHeight: 0.3f,
            point: (folded - new Vector3(
                x: 6f,
                y: 0.3f,
                z: 6f
            )),
            radius: 0.8f
        ), Material: NullWorldScene.PillarTrim);
        var shaft = (Distance: Cylinder(
            halfHeight: 2.9f,
            point: (folded - new Vector3(
                x: 6f,
                y: 3.3f,
            ...
            radius: 0.45f
        ), Material: NullWorldScene.PillarStone);
        var capital = (Distance: Cylinder(
            halfHeight: 0.3f,
            point: (folded - new Vector3(
                x: 6f,
                y: 6.35f,
        ...
        var lamp = (Distance: ((folded - new Vector3(
            x: 6f,
            y: 7f,
            z: 6f
        )).Length() - 0.45f), Material: NullWorldScene.PillarLamp);
        var column = SmoothUnion(
            candidate: shaft,
            current: plinth,
            radius: 0.25f
        );
    ...
    private static (float Distance, int Material) Pip(Vector3 point) {
        ...
        var eye = (Distance: ((mirrored - new Vector3(
            x: 0.2f,
            y: 1.1f,
            z: 0.42f
        )).Length() - 0.14f), Material: NullWorldScene.PipEye);
        var pupil = (Distance: ((mirrored - new Vector3(
            x: 0.2f,
            y: 1.1f,
            z: 0.54f
        )).Length() - 0.055f), Material: NullWorldScene.PipPupil);
    ...
    private static (float Distance, int Material) Planetoid(Vector3 point, Vector3 position, float scale) {
        ...
        var folded = new Vector3(
            x: ((MathF.Cos(x: angle) * radius) - 1.32f),
            y: local.Y,
            z: (MathF.Sin(x: angle) * radius)
        );
        var outcrop = (Distance: (folded.Length() - 0.3f), Material: NullWorldScene.PlanetoidRock);
        var blended = SmoothUnion(
            candidate: outcrop,
            current: core,
            radius: 0.3f
        );
    ...
    private static (float Distance, int Material) Quadrant((float Distance, int Material) candidate, Vector3 point, bool keepNegativeX, bool keepPositiveZ) {
        var alongX = (keepNegativeX
            ? -point.X
            : point.X
        );
        var alongZ = (keepPositiveZ
            ? point.Z
            : -point.Z
        );
    ...
    private static (float Distance, int Material) SmoothUnion((float Distance, int Material) current, (float Distance, int Material) candidate, float radius) {
        var weight = Math.Clamp(
            max: 1f,
            min: 0f,
            value: (0.5f + ((0.5f * (current.Distance - candidate.Distance)) / radius))
        );
    ...
    public static (float Distance, int Material) Sample(Vector3 point) {
        ...
        world = Union(
            current: world,
            candidate: GroundNorthEast(point: point)
        );
        world = Union(
            current: world,
            candidate: GroundSouthEast(point: point)
        );
        world = Union(
            current: world,
            candidate: GroundSouthWest(point: point)
        );
        world = Union(
            current: world,
            candidate: Pillars(point: point)
        );
        world = Union(
            current: world,
            candidate: Planetoids(point: point)
        );
        world = Union(
            current: world,
            candidate: Pip(point: point)
        );
