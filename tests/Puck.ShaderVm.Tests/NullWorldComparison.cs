...
public sealed class NullWorldComparison {
    ...
    private static double Nanoseconds(Func<Vector3, float> evaluate, Vector3[] points) {
        ...
        for (var repetition = 0; (repetition < 5); repetition++) {
            ...
            best = Math.Min(
                val1: best,
                val2: (stopwatch.Elapsed.TotalNanoseconds / points.Length)
            );
        }
    ...
    public void Measures() {
        ...
        foreach (var point in points) {
            ...
            var actual = ShaderInterpreter.Evaluate(
                context: in context,
                parameters: [],
                program: program
            );
        ...
        var bespoke = Nanoseconds(
            evaluate: point => NullWorldReference.Sample(point: point).Distance,
            points: points
        );
        ...
        _ = report.AppendLine(value: $"  mean relative delta             {((relativeSum / Math.Max(
            val1: compared,
            val2: 1
        ))):E3}");
        ...
        Assert.Equal(
            actual: materialMismatches,
            expected: 0
        );
        Assert.True(
            condition: (worstDistance < 1e-4),
            userMessage: $"worst distance delta {worstDistance:E3} at ({worstAt.X}, {worstAt.Y}, {worstAt.Z})"
        );
    }
}
