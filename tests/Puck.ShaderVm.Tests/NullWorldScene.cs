...
public static class NullWorldScene {
    ...
    private static Expression Up => Expression.Constant(
        x: 0f,
        y: 1f,
        z: 0f
    );
    ...
    private static Expression GroundNorthEast(Expression point) {
        var folded = WallpaperFoldP1(
            cell: Expression.Constant(
                x: 1f,
                y: 1f
            ),
        ...
        var surface = Sample(
            ...
            material: (Paper + WallpaperMaterial(
                cellIndex: cellIndex,
                stride: 1f
            ))
        );
    ...
    private static Expression GroundSouthWest(Expression point) {
        ...
        var surface = Sample(
            ...
            material: (BlueDeep + WallpaperMaterial(
                cellIndex: cellIndex,
                stride: 1f
            ))
        );
    ...
    private static Expression Planetoid(Expression point, Expression position, float scale) {
        ...
        var core = Sample(
            ...
            material: PlanetoidCrust
        );
        var folded = RepeatPolarY(
            count: 6f,
            point: local,
            sector: out _
        );
        ...
        return Sample(
            distance: (Distance(sample: blended) * scale),
            material: Material(sample: blended)
        );
    }
