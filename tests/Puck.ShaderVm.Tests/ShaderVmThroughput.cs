...
public sealed class ShaderVmThroughput {
    ...
    private static ShaderProgram Chain(ShaderOp op, int count, int octaves = 4) {
        ...
        var operand = ((op == ShaderOp.Fbm2)
            ? checked((uint)octaves)
            : ((op == ShaderOp.Swizzle)
                ? ShaderIsa.PackSwizzle(
                    w: 0,
                    x: 1,
                    y: 2,
                    z: 3
        ...
        for (var step = 0; (step < count); step++) {
            ...
            total += (Arity(op: op) switch {
                1 => ShaderExpression.Unary(
                op: op,
                operand: operand,
                value: input
            ),
                2 => ShaderExpression.Binary(
                left: input,
                op: op,
                right: right
            ),
                _ => ShaderExpression.Ternary(
                a: input,
                b: ShaderExpression.Constant(value: 0.5f),
                c: right,
                op: op
            ),
            });
    ...
    private static ShaderExpression CloudsOnly(ShaderExpression direction) {
        ...
        var point = (direction.Swizzle(
            w: 2,
            x: 0,
            y: 2,
            z: 0
        ) / shape.Y);
        var warp = ShaderMath.Fbm2(
            octaves: 4,
            position: ShaderMath.Seeded(
                position: point,
                seed: 0x9E3779B9u
            )
        );
        var density = ShaderMath.Fbm2(
            octaves: 4,
            position: ShaderMath.Seeded(
                position: (point + warp),
                seed: 4242u
    ...
    private static ShaderProgram Layered(int layers) {
        ...
        for (var layer = 0; (layer < layers); layer++) {
            var point = ((direction.Swizzle(
                ...
                y: 2,
                z: 0
            ) * (1f + layer)) + ShaderExpression.Constant(value: (layer * 7.3f)));
            var density = ShaderMath.Fbm2(
                octaves: 4,
                position: ShaderMath.Seeded(
                    position: point,
                    seed: (4242u + ((uint)layer))
                )
            );
            var mask = ShaderMath.SmoothStep(
                edge0: 0.55f,
                edge1: 0.85f,
                value: density
            );

            color = ShaderMath.Lerp(
                amount: mask,
                from: color,
                to: ShaderExpression.Parameter(index: SkyParameters.Clouds)
            );
        }
    ...
    private static double Nanoseconds(ShaderProgram program) {
        ...
        for (var repetition = 0; (repetition < 7); repetition++) {
            ...
            best = Math.Min(
                val1: best,
                val2: (stopwatch.Elapsed.TotalNanoseconds / Samples)
            );
        }
    ...
    private static ShaderExpression StarsOnly(ShaderExpression direction) {
        ...
        var cell = ShaderMath.Floor(value: (direction.Swizzle(
            w: 1,
            x: 0,
            y: 1,
            z: 0
        ) * stars.X));
        var hash = ShaderMath.Hash3(value: ShaderMath.Seeded(
            position: cell,
            seed: 1337u
        ));
        ...
        return ShaderMath.Select(
            condition: ShaderMath.Step(
                edge: unit.X,
                value: 0.08f
            ),
            whenFalse: ShaderExpression.Constant(value: 0f),
            whenTrue: ((second.X * flicker) * ShaderMath.Sqrt(value: ShaderMath.Max(
                left: second.Y,
                right: 1e-6f
            )))
        );
    ...
    private static ShaderProgram Synthetic(string kind) {
        ...
        return (kind switch {
            ...
            value: ShaderMath.Saturate(value: ShaderMath.Dot3(
                left: direction,
                right: ShaderExpression.Parameter(index: SkyParameters.Sun)
            ))
        ))),
    ...
    public void Measures() {
        ...
        Measure(
            label: "sky (full)",
            program: SkyProgram.Compile(),
            report: report
        );
        Measure(
            label: "gradient only",
            program: Synthetic(kind: "gradient"),
            report: report
        );
        Measure(
            label: "gradient + sun disc",
            program: Synthetic(kind: "disc"),
            report: report
        );
        Measure(
            label: "stars only",
            program: Synthetic(kind: "stars"),
            report: report
        );
        Measure(
            label: "clouds only",
            program: Synthetic(kind: "clouds"),
            report: report
        );
        ...
        var baseline = Nanoseconds(program: Chain(
            count: 64,
            op: ShaderOp.Add
        ));
        ...
        foreach (var op in ((ShaderOp[])[ShaderOp.Multiply, ShaderOp.Swizzle, ShaderOp.Dot3, ShaderOp.Lerp, ShaderOp.Select, ShaderOp.SquareRoot, ShaderOp.Sine, ShaderOp.Exponential, ShaderOp.Power, ShaderOp.Hash3, ShaderOp.ValueNoise2])) {
            var cost = ((Nanoseconds(program: Chain(
                count: 64,
                op: op
            )) - baseline) / 64d);
