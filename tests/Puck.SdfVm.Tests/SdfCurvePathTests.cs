...
public sealed class SdfCurvePathTests {
    ...
    private static CompiledCurvatureSpline SpiralWithShortTail() {
        ...
        for (var i = 0; (i < bigKnotCount); i++) {
            knots.Add(item: CircleKnot(elevation: 0f, knotIndex: i, radius: bigRadius, signedCurvature: 0f, turnRadians: bigTurn));
        }

        var lastAngle = ((bigKnotCount - 1) * bigTurn);
        var lastPosition = new Vector2(x: (bigRadius * MathF.Cos(x: lastAngle)), y: (bigRadius * MathF.Sin(x: lastAngle)));
        var lastYaw = (lastAngle + (MathF.PI / 2f));
        var forward = new Vector2(x: MathF.Cos(x: lastYaw), y: MathF.Sin(x: lastYaw));
        var perpendicular = new Vector2(x: -MathF.Sin(x: lastYaw), y: MathF.Cos(x: lastYaw));
        var tailPosition = ((lastPosition + (0.15f * forward)) + (0.2f * perpendicular));

        knots.Add(item: Knot(curvature: 0f, elevation: 0f, tangentYaw: (lastYaw + (MathF.PI / 2f)), x: tailPosition.X, z: tailPosition.Y));
    ...
    private static CompiledCurvatureSpline OpenCurve() {
        ...
        return CurvatureSpline.Compile(
            closed: false,
            knots: [
                CircleKnot(elevation: 0f, knotIndex: 0, radius: radius, signedCurvature: curvature, turnRadians: turn),
                CircleKnot(elevation: 1f, knotIndex: 1, radius: radius, signedCurvature: curvature, turnRadians: turn),
                CircleKnot(elevation: -0.5f, knotIndex: 2, radius: radius, signedCurvature: curvature, turnRadians: turn),
            ]
        );
    }
    private static CompiledCurvatureSpline ClosedCurve() {
        ...
        return CurvatureSpline.Compile(
            ...
            knots: [
                CircleKnot(elevation: 0f, knotIndex: 0, radius: radius, signedCurvature: curvature, turnRadians: turn),
                CircleKnot(elevation: 0.5f, knotIndex: 1, radius: radius, signedCurvature: curvature, turnRadians: turn),
                CircleKnot(elevation: 0f, knotIndex: 2, radius: radius, signedCurvature: curvature, turnRadians: turn),
            ]
        );
    }
    ...
    private static void AssertAgreesAlongTheWholeArc(CompiledCurvatureSpline compiled, float[] arcLengthsToSample) {
        ...
        foreach (var arcLength in arcLengthsToSample) {
            ...
            var yawDelta = MathF.Abs(x: MathF.IEEERemainder(x: (floatYaw - expectedYaw), y: (2f * MathF.PI)));
    ...
    public void FloatTwinAgreesWithFixedEvaluateAlongAnOpenCurve() {
        ...
        AssertAgreesAlongTheWholeArc(arcLengthsToSample: [.. samples], compiled: compiled);
    }
    [Fact]
    ...
    public void OpenCurveClampsPastEitherEnd() {
        ...
        Assert.Equal(actual: belowZero, expected: atZero);
        Assert.Equal(actual: pastEnd, expected: atEnd);
    }
    [Fact]
    ...
    public void PathOpSeedsTheEyeAtTheSampledPositionFacingTheCurveTangent() {
        ...
        var expectedForward = new Vector3(x: -MathF.Sin(x: expectedYaw), y: 0f, z: -MathF.Cos(x: expectedYaw));
        var actualForward = Vector3.Transform(value: -Vector3.UnitZ, rotation: Quaternion.CreateFromYawPitchRoll(pitch: 0f, roll: 0f, yaw: expectedYaw));
    ...
    public void PathOpComposesWithOrbitPivotingTheSampledPoint() {
        ...
        Assert.True(condition: ((pose.Eye - (sampledPosition + OrbitRig.Offset(distance: 4f, pitch: 0f, yaw: 0f))).Length() < 1e-4f));
    }
    [Fact]
