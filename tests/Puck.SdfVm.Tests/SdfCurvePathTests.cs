...
public sealed class SdfCurvePathTests {
    private static void AssertAgreesAlongTheWholeArc(CompiledCurvatureSpline compiled, float[] arcLengthsToSample) {
        ...
        Assert.Equal(
            expected: compiled.Closed,
            actual: twin.Closed
        );
        Assert.Equal(
            expected: ((float)((double)compiled.TotalLength)),
            actual: twin.TotalLength,
            precision: 4
        );
        ...
        foreach (var arcLength in arcLengthsToSample) {
            ...
            Assert.True(
                condition: ((floatPosition - expectedPosition).Length() < 5e-3f),
                userMessage: $"arcLength={arcLength}: float={floatPosition} fixed={expectedPosition}"
            );

            var expectedYaw = ExpectedYaw(fixedTangent: fixedSample.Tangent);
            var yawDelta = MathF.Abs(x: MathF.IEEERemainder(
                x: (floatYaw - expectedYaw),
                y: (2f * MathF.PI)
            ));

            Assert.True(
                condition: (yawDelta < 1e-2f),
                userMessage: $"arcLength={arcLength}: floatYaw={floatYaw} expectedYaw={expectedYaw}"
            );
        }
    }
    ...
    private static CurvatureSplineKnot CircleKnot(float radius, float elevation, int knotIndex, float turnRadians, float signedCurvature) {
        ...
        return Knot(
            x: (radius * MathF.Cos(x: angle)),
            z: (radius * MathF.Sin(x: angle)),
            elevation: elevation,
    ...
    private static SdfCameraProgramSet PathAtFraction(SdfCurvePath curve, float fraction) => new(Programs: [
        new SdfCameraProgram(
            Name: "dolly",
            Operations: [
                new SdfCameraOp.Path(
                    Curve: curve,
                    Fraction: SdfCameraScalar.FromLiteral(value: fraction)
                ),
    ...
    private static SdfCameraProgramSet PathProgram(SdfCurvePath curve, params SdfCameraOp[] trailingOps) => new(Programs: [
        new SdfCameraProgram(
            ...
            Operations: [
                new SdfCameraOp.Path(
                    Curve: curve,
                    Fraction: SdfCameraScalar.FromLiteral(value: 0.5f)
                ),
                .. trailingOps,
                new SdfCameraOp.Fov(FieldOfViewRadians: SdfCameraScalar.FromLiteral(value: OrbitRig.DefaultFieldOfViewRadians)),
            ]
    ...
    private static CompiledCurvatureSpline SpiralWithShortTail() {
        ...
        var lastPosition = new Vector2(
            x: (bigRadius * MathF.Cos(x: lastAngle)),
            y: (bigRadius * MathF.Sin(x: lastAngle))
        );
        var lastYaw = (lastAngle + (MathF.PI / 2f));
        var forward = new Vector2(
            x: MathF.Cos(x: lastYaw),
            y: MathF.Sin(x: lastYaw)
        );
        var perpendicular = new Vector2(
            x: -MathF.Sin(x: lastYaw),
            y: MathF.Cos(x: lastYaw)
        );
        ...
        knots.Add(item: Knot(
            curvature: 0f,
            elevation: 0f,
            tangentYaw: (lastYaw + (MathF.PI / 2f)),
            x: tailPosition.X,
            z: tailPosition.Y
        ));
    ...
    public void ClosedCurveWrapsRatherThanClamping() {
        ...
        Assert.True(condition: ((justPastEnd - nearStart).Length() < 1e-3f));
    }
    [Fact]
    public void FloatTwinAgreesWithFixedEvaluateAlongAClosedCurveIncludingTheWrap() {
        ...
        for (var index = 0; (index <= 40); index++) {
            samples.Add(item: ((totalLength * index) / 40f));
        }

        // Past-the-end and negative arc lengths both wrap — the fixed and float twins must agree there too.
        samples.Add(item: (-totalLength * 0.25f));
        samples.Add(item: (totalLength * 1.25f));
        samples.Add(item: totalLength);

        AssertAgreesAlongTheWholeArc(
            arcLengthsToSample: [.. samples],
            compiled: compiled
        );
    }
    [Fact]
    ...
    public void FloatTwinTracksPositionAcrossAMassiveAccumulatedStationDownToAShortFinalSegment() {
        ...
        Assert.True(
            condition: (compiled.TotalLengthRaw > (1L << 56)),
            userMessage: $"fixture no longer accumulates past 2^24 arc units (TotalLengthRaw={compiled.TotalLengthRaw}); re-engineer SpiralWithShortTail."
        );

        var tailSegment = compiled.GetSegment(index: (compiled.SegmentCount - 1));

        Assert.True(
            condition: (tailSegment.LengthRaw < (16L << 32)),
            userMessage: $"the fixture's own final segment is no longer short (LengthRaw={tailSegment.LengthRaw}); re-engineer SpiralWithShortTail."
        );
        ...
        var slack = ((MathF.Abs(x: twin.TotalLength) * 2e-6f) + 32f);
        ...
        void AssertNear(long arcRaw, string label) {
            ...
            Assert.True(
                condition: ((floatPosition - expectedPosition).Length() < slack),
                userMessage: $"{label} (arc raw {arcRaw}): float={floatPosition} fixed={expectedPosition} slack={slack}"
            );
        }

        for (var segmentIndex = 0; (segmentIndex < compiled.SegmentCount); segmentIndex += 4) {
            AssertNear(
                arcRaw: compiled.GetSegment(index: segmentIndex).StationRaw,
                label: $"segment {segmentIndex}'s own station"
            );
        }

        AssertNear(
            arcRaw: tailSegment.StationRaw,
            label: "the short tail's own start"
        );
        AssertNear(
            arcRaw: compiled.TotalLengthRaw,
            label: "the curve's own end (inside the short tail)"
        );
    }
    [Fact]
    ...
    public void PathOpComposesWithOrbitPivotingTheSampledPoint() {
        ...
        Assert.True(condition: ((pose.Eye - (sampledPosition + OrbitRig.Offset(
            distance: 4f,
            pitch: 0f,
            yaw: 0f
        ))).Length() < 1e-4f));
    ...
    public void PathOpFractionClampsOnAnOpenCurveControlWrapsOnAClosedOne() {
        ...
        var openAtEnd = new SdfCameraProgramRig(
            programs: PathAtFraction(
                curve: openTwin,
                fraction: 1f
            ),
            scalarCount: 0,
        ...
        var openPastEnd = new SdfCameraProgramRig(
            programs: PathAtFraction(
                curve: openTwin,
                fraction: 1.5f
            ),
        ...
        Assert.Equal(
            expected: openAtEnd.Eye,
            actual: openPastEnd.Eye
        );

        var closedNearStart = new SdfCameraProgramRig(
            programs: PathAtFraction(
                curve: closedTwin,
                fraction: (1f / closedTwin.TotalLength)
            ),
            scalarCount: 0,
            subjectCount: 0
        )
        ...
        var closedJustPastEnd = new SdfCameraProgramRig(
            programs: PathAtFraction(
                curve: closedTwin,
                fraction: (1f + (1f / closedTwin.TotalLength))
            ),
            scalarCount: 0,
            subjectCount: 0
        )
        ...
        Assert.True(condition: ((closedNearStart.Eye - closedJustPastEnd.Eye).Length() < 1e-3f));
        ...
        Assert.True(condition: (openAtEnd.Eye != new SdfCurvePath(compiled: OpenCurve()).Sample(arcLength: 0f).Position));
    }
    [Fact]
    public void PathOpSeedsTheEyeAtTheSampledPositionFacingTheCurveTangent() {
        ...
        var twin = new SdfCurvePath(compiled: compiled);
        var rig = new SdfCameraProgramRig(
            programs: PathProgram(curve: twin),
            scalarCount: 0,
            subjectCount: 0
        );
        var reference = new SdfAnchor(
        ...
        var expectedForward = new Vector3(
            x: -MathF.Sin(x: expectedYaw),
            y: 0f,
            z: -MathF.Cos(x: expectedYaw)
        );
        var actualForward = Vector3.Transform(
            value: -Vector3.UnitZ,
        ...
        Assert.True(condition: ((actualForward - expectedForward).Length() < 1e-5f));
        ...
        Assert.True(condition: (((pose.Target - pose.Eye) - (expectedForward * SdfCameraProgramEvaluator.DefaultFocusDistance)).Length() < 1e-4f));
    }
}
