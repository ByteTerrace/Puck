...
public sealed class SdfCameraProgramLawTests {
    ...
    private static readonly SdfCameraDynamics ChaseDynamics = new(
        Damping: 1f,
        Frequency: 0.9549f,
        Response: 1f
    );
    private static readonly Vector3 ChaseAimOffset = new(
        x: 0f,
        y: 1f,
        z: 0f
    );
    private static readonly Vector3 ChasePivotOffset = Vector3.Zero;
    ...
    private static (Vector3 Eye, Vector3 Target, float FovRadians) ResolveClosedChase(in SdfAnchor anchor, float liveYaw, float livePitch) {
        var pivot = (anchor.Position + ChasePivotOffset);
    ...
    public void Blend_InterpolatesEyeTargetFovAndDynamics() {
        var aDynamics = new SdfCameraDynamics(
            Damping: 1f,
            Frequency: 2f,
            Response: 0f
        );
        var bDynamics = new SdfCameraDynamics(
            Damping: 1f,
            Frequency: 6f,
            Response: 0f
        );
                ...
                new SdfCameraProgram(
                    ...
                    Operations: [
                        new SdfCameraOp.Blend(
                            ProgramA: 1,
                            ProgramB: 2,
                            Weight: SdfCameraScalar.FromSlot(
                                fallback: 0f,
                                slot: 0
                            )
                        ),
        ...
        Assert.Equal(
            ...
            actual: blended.Target
        );
        Assert.Equal(
            expected: 2f,
            actual: blended.FovRadians
        );
        Assert.Equal(
            expected: new SdfCameraDynamics(
                Damping: 1f,
                Frequency: 4f,
                Response: 0f
            ),
            actual: blended.Dynamics
        );
    ...
    public void Blend_WithAnIndexOutsideTheSet_ResolvesTheReferenceFraming() {
        ...
        Assert.Equal(
            expected: OrbitRig.DefaultFieldOfViewRadians,
            actual: pose.FovRadians
        );
    }
    ...
    public void BoomFollower_SeedsThenLagsOnlyTheBoom() {
        ...
        var live = new SdfCameraDynamics(
            Damping: 1f,
            Frequency: 0.9549297f,
            Response: 0f
        );
        var eye = new Vector3(
            x: 0f,
        ...
        var movedEye = new Vector3(
            x: 0f,
            y: 0f,
            z: 30f
        );
        var movedTarget = new Vector3(
            x: 5f,
            y: 0f,
            z: 0f
        );
    ...
    public void ClampPitch_BoundsTheSummedOrbitPitch() {
        ...
        Assert.Equal(
            actual: clamped.ResolvePose(
                anchor: in anchor,
                clock: in clock
            ).Eye,
            expected: (ChasePivotOffset + OrbitRig.Offset(
                distance: ChaseDistance,
                pitch: 0.5f,
                yaw: ChaseYaw
            ))
        ...
        var unclamped = new SdfCameraProgramRig(
            programs: ChaseProgram(),
            scalarCount: 0,
            subjectCount: 0
        ) {
            Look = new SdfCameraLook(
    ...
    public void OrbitWithoutAppliesLook_IgnoresTheLookSample() {
        ...
        var still = rig.ResolvePose(
            anchor: in anchor,
            clock: in clock
        );

        rig.Look = new SdfCameraLook(
            Pitch: 0.9f,
            Yaw: 2.1f
        );
        ...
        var interactive = new SdfCameraProgramRig(
            programs: ChaseProgram(),
            scalarCount: 0,
            subjectCount: 0
        ) {
        ...
        Assert.NotEqual(
            expected: still.Eye,
            actual: interactive.ResolvePose(
                anchor: in anchor,
                clock: in clock
            ).Eye
        );
    ...
    public void ProgramWithoutALookAt_AimsAlongTheSubjectsForward() {
        ...
        Assert.Equal(
            expected: (-Vector3.UnitZ * SdfCameraProgramEvaluator.DefaultFocusDistance),
            actual: pose.Target
        );
    }
    ...
    public void ScalarSlots_ReadTheFrameBuffer() {
        var rig = new SdfCameraProgramRig(
            programs: new SdfCameraProgramSet(Programs: [
                new SdfCameraProgram(
                    Name: "bound",
                    Operations: [new SdfCameraOp.Fov(FieldOfViewRadians: SdfCameraScalar.FromSlot(
                            fallback: 0.1f,
                            slot: 0
                        ))]
                ),
    ...
    public void SubjectSlots_ReseatTheEyeAndTheAim() {
        ...
        var pose = rig.ResolvePose(
            anchor: new SdfAnchor(
                Position: new Vector3(value: 999f),
                Orientation: Quaternion.Identity
            ),
            clock: new SdfCameraClock(
                AuthoritativeTick: 0UL,
    ...
    public void TranscribedChaseProgram_FramesIdenticallyToTheClosedChaseRig() {
        ...
        var clock = new SdfCameraClock(
            AuthoritativeTick: 17UL,
            PresentationSeconds: 0.5f
        );
        ...
        foreach (var anchor in Anchors()) {
            foreach (var liveYaw in new[] { 0f, 0.25f, -1.75f, 3.0f }) {
                foreach (var livePitch in new[] { 0f, -0.35f, 0.4f, 0.7854f }) {
                    ...
                    var expected = ResolveClosedChase(
                        anchor: in anchor,
                        livePitch: livePitch,
                        liveYaw: liveYaw
                    );
                    var actual = rig.ResolvePose(
                        anchor: in anchor,
                        clock: in clock
                    );

                    Assert.Equal(
                        expected: expected.Eye,
                        actual: actual.Eye
                    );
                    Assert.Equal(
                    ...
                    Assert.Equal(
                        expected: expected.FovRadians,
                        actual: actual.FovRadians
                    );
                    Assert.Equal(
                        expected: ChaseDynamics,
