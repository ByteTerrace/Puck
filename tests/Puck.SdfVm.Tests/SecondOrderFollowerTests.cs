...
public sealed class SecondOrderFollowerTests {
    ...
    public void StepAgreesWithFixedEvaluateOverAWalk(float zeta, float initialError, float initialVelocity) {
        ...
        var response = SecondOrderResponse.Create(frequencyHz: frequencyHz, dampingRatio: zeta, initialResponse: 0f);
        var fixedDynamics = SecondOrderDynamics.Create(
            frequencyHz: FixedQ4816.FromDouble(frequencyHz),
            dampingRatio: FixedQ4816.FromDouble(zeta),
        ...
        for (var step = 1; (step <= 480); ++step) {
            ...
            var fixedSample = fixedDynamics.Evaluate(
                initialValue: FixedQ4816.FromDouble(target + initialError),
                initialVelocity: FixedQ4816.FromDouble(initialVelocity),
                target: FixedQ4816.FromDouble(target),
                elapsedTicks: elapsedTicks,
                ticksPerSecond: TicksPerSecond
            );

            Assert.True(
                MathF.Abs(position - ((float)fixedSample.Value.Value / 65536f)) < 2e-3f,
                $"step {step}: float={position} fixed={((float)fixedSample.Value.Value / 65536f)}"
            );
        }
    }
    ...
    public void CriticalDampingFromRestNeverOvershoots() {
        ...
        for (var i = 0; (i < (240 * 10)); ++i) {
            var value = follower.Step(response: response, deltaSeconds: StepSeconds, target: target);

            maxX = MathF.Max(maxX, value.X);
        }

        Assert.True(maxX <= (10f + 1e-4f));
        Assert.True(MathF.Abs(maxX - 10f) < 1e-3f);
    }

    [Fact]
    public void LightDampingFromRestOvershoots() {
        var response = SecondOrderResponse.Create(frequencyHz: 2f, dampingRatio: 0.2f, initialResponse: 0f);
        var follower = new SecondOrderFollower3();
        var target = new Vector3(x: 10f, y: 0f, z: 0f);
        var maxX = float.MinValue;
        ...
        Assert.True(maxX > 10.05f);
    }

    [Fact]
    public void OneLargeStepAgreesWithTenSmallStepsWithinATolerance() {
        var response = SecondOrderResponse.Create(frequencyHz: 1f, dampingRatio: 0.7f, initialResponse: 0f);
        ...
        var coarseValue = coarse.Step(response: response, deltaSeconds: 0.1f, target: target);
        ...
        for (var i = 0; (i < 10); ++i) {
            fineValue = fine.Step(response: response, deltaSeconds: 0.01f, target: target);
        }

        Assert.True((coarseValue - fineValue).Length() < 1e-3f);
    }

    [Fact]
    public void ALargeStepLandsNearTheTargetWithoutOvershootAtCriticalDamping() {
        var response = SecondOrderResponse.Create(frequencyHz: 1f, dampingRatio: 1f, initialResponse: 0f);
        var follower = new SecondOrderFollower3();
        ...
        Assert.True(MathF.Abs(value.X - 20f) < 1e-3f);
        Assert.True(value.X <= (20f + 1e-4f));
    }
    ...
    public void CriticalDampingMatchesTheOldExponentialCameraSmootherAtTheMigratedFrequency(float rate) {
        ...
        var response = SecondOrderResponse.Create(frequencyHz: frequencyHz, dampingRatio: 1f, initialResponse: 1f);
        var follower = new SecondOrderFollower3();
        ...
        foreach (var checkpoint in new[] { 0.25f, 0.5f, 1f, 2f }) {
            while (elapsed < (checkpoint - 1e-6f)) {
                follower.Step(response: response, deltaSeconds: StepSeconds, target: target);
                elapsed += StepSeconds;
            }

            var expected = (30f - (20f * MathF.Exp(-rate * checkpoint)));

            Assert.True(
                MathF.Abs(follower.Value.X - expected) < (0.05f * 20f),
                $"rate={rate} t={checkpoint}: follower={follower.Value.X} expected={expected}"
            );
        }
    }
