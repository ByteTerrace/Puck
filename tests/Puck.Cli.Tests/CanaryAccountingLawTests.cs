...
public sealed class CanaryAccountingLawTests {
    ...
    public void ANarratedVerbsAcceptedClaimDefaultsToStderrNotStdout() {
        var commands = new[] { new CanaryCommandClaim(
            Verb: "world.generate",
            Occurrence: 1,
    ...
    public void ASpaceLedAnswerContinuesOverFurtherSpaceLedLinesWithNoGap() {
        ...
        var lines = new[] {
            Line(
            text: "[world.rule.trace r: 2/2 captured, complete]",
            sequence: 1
        ),
            Line(
            text: "[world.rule.trace r tick=1 gate=closed: …]",
            sequence: 2
        ),
            Line(
            text: "[world.rule.trace r tick=2 gate=open: …]",
            sequence: 3
        ),
        };
        var events = CanaryCommand.ResponseEvents(
    ...
    public void AWorldStateRowHeaderAlwaysOpensFreshEvenRightAfterAnotherRowsCells() {
        ...
        var lines = new[] {
            Line(
            text: "[world.state.row 'a' kind=Int value=1 domain=slot]",
            sequence: 1
        ),
            Line(
            text: "[world.state.cell 'a'.'$value' value=1]",
            sequence: 2
        ),
            Line(
            text: "[world.state.row 'b' kind=Int value=2 domain=slot]",
            sequence: 3
        ),
            Line(
            text: "[world.state.cell 'b'.'$value' value=2]",
            sequence: 4
        ),
        };
        var events = CanaryCommand.ResponseEvents(
    ...
    public void AWorldStateSingleCellAnswerNeverMergesIntoADifferentRowsSingleCellAnswer() {
        ...
        var lines = new[] {
            ...
            Line(
            text: "[world.state.cell 'y'.'k' value=2]",
            sequence: 2
        ),
        };
        var events = CanaryCommand.ResponseEvents(
    ...
    public void AWorldSymmetryTwoArgumentCallsTwoLinesStayOneEventOnTheSharedNode() {
        var lines = new[] {
            Line(
            text: "[world.symmetry node=5 ring=2 antipode=18 …]",
            sequence: 1
        ),
            Line(
            text: "[world.symmetry node=5 other=17 reflect=16 …]",
            sequence: 2
        ),
        };
        var events = CanaryCommand.ResponseEvents(
    ...
    public void TwoSeparateSpaceLedCallsOfAGenericVerbWithNoGapBetweenThemStillMergeAsOneEvent() {
        ...
        var lines = new[] {
            Line(
            text: "[world.rule.trace r: armed for 1 evaluation(s) …]",
            sequence: 1
        ),
            Line(
            text: "[world.rule.trace r: 1/1 captured, complete]",
            sequence: 2
        ),
        };
        var events = CanaryCommand.ResponseEvents(
