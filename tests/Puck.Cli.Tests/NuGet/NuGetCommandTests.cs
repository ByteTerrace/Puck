...
public sealed class NuGetCommandTests {
    ...
    public void CatalogFindsNestedOptInsAndIgnoresBuildOutputs() {
        var root = Path.Combine(
            ...
            path2: $"puck-package-catalog-{Guid.NewGuid():N}"
        );
    ...
    public void PackRejectsUnknownOptionsBeforeProducingArtifacts() {
        Assert.Equal(
            expected: 2,
            actual: PuckRootCommand.Invoke(args: ["nuget", "pack", "--unknown"])
        );
        Assert.Equal(
            expected: 2,
            actual: PuckRootCommand.Invoke(args: ["nuget", "pack", "--no-build", "extra", "positional"])
        );
    }
    [Fact]
    public async Task ReleaseSelectionAndProvenanceAsync() {
        var directory = Path.Combine(
            path1: Path.GetTempPath(),
            path2: $"puck-nuget-release-{Guid.NewGuid():N}"
        );
        ...
        Task FailsSyncAsync(Action action, string message) => FailsAsync(
        ...
        try {
            ...
            Fixture(
                dependencies: $"<dependency id=\"{Basis}\" version=\"[{Version}, )\" /><dependency id=\"External.Library\" version=\"9.0.0\" />",
                id: Consumer,
                input: input
            );
            Task<JsonObject> BatchAsync(string name, string selection = "all") => PrepareAsync(
                input,
                Path.Combine(
                    path1: directory,
                    path2: name
            ...
            Check(
                condition: (ids.Length == 3),
                message: "all must include every package."
            );
            Check(
                condition: (Array.IndexOf(
                    array: ids,
                    value: Basis
                ) < Array.IndexOf(
                    array: ids,
                ...
                message: "Dependencies must precede consumers."
            );
            Check(
                condition: (lookups == 0),
                message: "A complete batch must not query NuGet.org."
            );
            Verify(
                Path.Combine(
                    path1: directory,
            ...
            Check(
                condition: (((string?)all["commit"]) == commit),
                message: "Manifest must retain source commit."
            );
            foreach (var package in all["packages"]!.AsArray()) {
                Check(
                    condition: (package!["files"]!.AsArray().Count == 2),
            ...
            var one = await BatchAsync(
                name: "one",
                selection: Independent
            );

            Check(
                condition: ((one["packages"]!.AsArray().Count == 1) && (((string?)one["packages"]![0]!["id"]) == Independent)),
                message: "Single selection leaked a package."
            );
            Check(
                ...
                message: "Single selection copied extra packages."
            );
            Verify(
                Path.Combine(
                    path1: directory,
            ...
            var pair = await BatchAsync(
                name: "pair",
                selection: $" {Basis}, {Consumer} "
            );

            Check(
                condition: (pair["packages"]!.AsArray().Count == 2),
                message: "Multiple selection must preserve the requested set."
            );
            await FailsAsync(
                action: () => BatchAsync(
                    name: "missing",
                ...
                message: "requires omitted package"
            );
            Check(
                condition: !Directory.Exists(path: Path.Combine(
                    path1: directory,
                    path2: "missing"
                )),
                message: "Missing dependency must fail before copying artifacts."
            );
            ...
            available = [Version];
            var partial = await BatchAsync(
                name: "partial",
                selection: Consumer
            );

            Check(
                condition: ((partial["packages"]!.AsArray().Count == 1) && (((string?)partial["publishedDependencies"]![0]!["id"]) == Basis)),
                message: "Published dependency must be recorded without being copied."
            );
            ...
            await FailsAsync(
                action: () => BatchAsync(
                    name: "duplicate",
                    selection: $"{Basis},{Basis.ToLowerInvariant()}"
                ),
                message: "Duplicate selection"
            );
            await FailsAsync(
                action: () => BatchAsync("all"),
            ...
            await FailsAsync(
                ...
                message: "expected shared version"
            );
            foreach (var (name, dependency, error) in new[] {
                ...
                Fixture(
                    dependencies: $"<dependency id=\"{dependency}\" version=\"1.0.0\" />",
                    id: Consumer,
                    input: bad
                );
                await FailsAsync(
            ...
            Fixture(
                dependencies: $"<dependency id=\"{Consumer}\" version=\"{Version}\" />",
                id: Basis,
                input: cycle
            );
            Fixture(
                dependencies: $"<dependency id=\"{Basis}\" version=\"{Version}\" />",
                id: Consumer,
                input: cycle
            );
            await FailsAsync(
            ...
            foreach (var candidate in new[] { "01.2.3", "1.2", "1.2.3+build", "1.2.3-RC.1", "1.2.3;echo", "v1.2.3" }) {
                await FailsSyncAsync(
                    ...
                    message: "Invalid shared version"
                );
            }
            await FailsSyncAsync(
                action: () => ValidateVersion(version: "1.2.3-01"),
                message: "leading zeroes"
            );
            await FailsSyncAsync(
                action: () => Verify(
                    Path.Combine(
                ...
                message: "does not match this run"
            );
            var pushes = PushArguments(
                all,
                Path.Combine(
                    path1: directory,
                    path2: "all"
                )
            ).Concat(second: PushArguments(
                all,
                Path.Combine(
                    path1: directory,
                    path2: "all"
            ...
            Check(
                condition: (pushes.Length == 12),
                message: "Both attempts must push all packages and symbols."
            );
            foreach (var push in pushes) {
                Check(
                    condition: push.Contains(value: "--skip-duplicate"),
                    message: "Retry must tolerate existing versions."
                );
                Check(
                    condition: (push[2].EndsWith(
                        comparisonType: StringComparison.Ordinal,
                        value: ".nupkg"
                    ) == push.Contains(value: "--no-symbols")),
                    message: "Explicit symbols must not be disabled."
                );
            }
            var artifact = Path.Combine(
                path1: directory,
                path2: "all",
                path3: ((string)all["packages"]![0]!["files"]![0]!["name"]!)
            );
