...
public sealed class OpenAiEmbeddingGeneratorTests {
    ...
    public async Task RequestShapeAndIdentityBearerTokenMatchSpecificationAsync() {
        ...
        Assert.Equal(actual: receivedHeader, expected: "Bearer test-identity-token");
    ...
    private sealed class MockTokenCredential : TokenCredential {
        ...
        public override AccessToken GetToken(TokenRequestContext requestContext, CancellationToken cancellationToken) =>
            new(accessToken: m_token, expiresOn: DateTimeOffset.UtcNow.AddHours(hours: 1));
        public override ValueTask<AccessToken> GetTokenAsync(TokenRequestContext requestContext, CancellationToken cancellationToken) =>
            ValueTask.FromResult(result: new AccessToken(accessToken: m_token, expiresOn: DateTimeOffset.UtcNow.AddHours(hours: 1)));
    }
