...
public sealed class FormatSubmissionTests {
    ...
    public async Task ACommitRaceDoesNotDispatchChecksForTheWrongHead() {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(testCode: () => new FormatSubmission(
            client: client,
    ...
    public async Task AFormattingCommitUsesAnExpectedHeadAndDispatchesTheSharedVerificationGraphOnce() {
        ...
        Assert.Equal(
            expected: GitHubHandler.Head,
            actual: ((string?)input["expectedHeadOid"])
        );
        Assert.Equal(
            expected: "feature",
            actual: ((string?)input["branch"]!["refName"])
        );
        Assert.Single(collection: input["fileChanges"]!["additions"]!.AsArray());
        Assert.Equal(
            expected: 2,
            actual: handler.Dispatches.Count
        );
        Assert.Single(
            collection: handler.Dispatches,
            predicate: item => item.Path.Contains(
        ...
        Assert.Equal(
            expected: "false",
            actual: ((string?)handler.Dispatches.Single(predicate: item => item.Path.Contains(
                comparisonType: StringComparison.Ordinal,
                value: "azure.yml"
            )).Body["inputs"]!["deploy"])
        );
        Assert.Equal(
            expected: 2,
            actual: m_reports.Count
        );
        Assert.StartsWith(
            expectedStartString: "Applied 1 formatted file(s) to PR #7",
            actualString: m_reports[0]
        );
        Assert.StartsWith(
            expectedStartString: "Dispatched formatting",
            actualString: m_reports[1]
        );
    }
    [InlineData("stale")]
    [InlineData("fork")]
    ...
    public async Task UntrustedArtifactsCannotExpandTheWriteScope(string scenario) {
        ...
        await Assert.ThrowsAsync<InvalidDataException>(testCode: () => new FormatSubmission(
            client: client,
            report: m_reports.Add
        ).RunAsync(
            graphUrl: "https://api.invalid/graphql",
    ...
    private sealed class GitHubHandler : HttpMessageHandler {
        ...
        private HttpResponseMessage Artifact() {
            ...
            var files = ((m_scenario == "clean")
                ? new JsonArray()
                : new JsonArray(change)
            );
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken) {
            ...
            if (request.Method == HttpMethod.Post) {
                ...
                if (path == "/graphql") {
                    ...
                    return Json(value: ((m_scenario == "race")
                        ? new JsonObject { ["errors"] = new JsonArray("head advanced") }
                        : new JsonObject { ["data"] = new JsonObject { ["createCommitOnBranch"] = new JsonObject { ["commit"] = new JsonObject { ["oid"] = NewHead } } } }));
                }
            ...
            return relative switch {
                ...
                new JsonObject { ["filename"] = "src/One.cs", ["status"] = "modified" },
