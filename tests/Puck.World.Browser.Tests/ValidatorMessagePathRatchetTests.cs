...
/// the flagship island's own refusal (a path on every message) and one deliberately minimal admission-shape defect
/// (no path on its one message). A future pass that adds a path token to a message the validator emits today should
/// move one of these two counts, which is the point: this test fails, by design, the moment that sweep touches
/// either fixture's own message, so the sweep's own progress is visible rather than silently absorbed.</summary>
public sealed class ValidatorMessagePathRatchetTests {
    ...
    private static IReadOnlyList<BrowserErrorPath> ValidateAndSplit(byte[] utf8Json) {
    ...
    [Fact]
    public void Composed_puck_world_refusal_carries_a_path_on_every_message() {
        ...
        var split = ValidateAndSplit(utf8Json: Encoding.UTF8.GetBytes(s: tree!.ToJsonString()));
    ...
    public void Nameless_addon_refusal_carries_no_path() {
        ...
        var split = ValidateAndSplit(utf8Json: Encoding.UTF8.GetBytes(s: Json));
