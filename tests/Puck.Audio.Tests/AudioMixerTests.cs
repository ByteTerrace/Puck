...
public sealed class WorldAudioMixerTests {
    ...
    private static void BuildScriptedSnapshot(AudioSnapshot snapshot, int step, int steps, ref ulong sequence) {
        ...
        snapshot.Reset(listener: new AudioListener(
            Position: new FixedVector3(
                X: QRaw(raw: (cos.Value * 2)),
                Y: FixedQ4816.Zero,
                Z: QRaw(raw: (sin.Value * 2))
            ),
        ...
        snapshot.TryAddEmitter(emitter: new AudioEmitter(
            ...
            Kind: AudioEmitterKind.Point,
            Position: new FixedVector3(
                X: QRaw(raw: (-655360L + ((step * 40960L) / 15))),
                Y: FixedQ4816.Zero,
                Z: FixedQ4816.Zero
            ),
            MinRadius: QRaw(raw: 32768),
        ...
        snapshot.TryAddEmitter(emitter: new AudioEmitter(
            ...
            GainQ16: 45875,
            Channel: AudioChannel.Mix,
            Source: AudioSourceKey.Synth(patchId: "bed")
        ));
        ...
        snapshot.TryAddEmitter(emitter: new AudioEmitter(
            ...
            MinRadius: QRaw(raw: 32768),
            MaxRadius: Q(units: 10),
            Curve: AudioAttenuationCurve.Smoothstep,
            FadeFrames: 0,
            GainQ16: UnityQ16,
            Channel: AudioChannel.Mix,
            Source: AudioSourceKey.Synth(patchId: "chirp")
        ));
    ...
    private static short[] RenderCullScenario(bool withEmitter, long distance, IAudioBlockSource source) {
        ...
        for (var i = 0; (i < 3); i++) {
            ...
            mixer.MixBlock(
                snapshot: snapshot,
                stereoInterleaved: output.AsSpan(
                    length: (Frames * 2),
                    start: ((i * Frames) * 2)
                )
            );
    ...
    public void HardLeftEmitterLandsItsEnergyInTheLeftChannel() {
        ...
        Assert.True(condition: (left > (10 * Math.Max(
            val1: right,
            val2: 1
        ))));
    }
    ...
    public void HardRightEmitterLandsItsEnergyInTheRightChannel() {
        ...
        Assert.True(condition: (right > (10 * Math.Max(
            val1: left,
            val2: 1
        ))));
    }
    ...
    public void ScriptedTimelineReproducesItsPcmHashAcrossTwoFreshRuns() {
        static string RunTimeline() {
            ...
            mixer.SetSource(
                ...
                source: new SeededPatternSource(seed: 0xC0FFEEUL)
            );
    ...
    public void SoftClipBoundariesAreExactAndSymmetric() {
        Assert.Equal(
            expected: 24575,
            actual: AudioMixer.SoftClip(sample: 24575)
        );
        Assert.Equal(
            expected: 24576,
            actual: AudioMixer.SoftClip(sample: 24576)
        );
        Assert.Equal(
            expected: 32767,
            actual: AudioMixer.SoftClip(sample: 49151)
        );
        Assert.Equal(
            expected: -32767,
            actual: AudioMixer.SoftClip(sample: -49151)
