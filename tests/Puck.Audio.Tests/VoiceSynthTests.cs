...
public sealed class WorldVoiceSynthTests {
    ...
    private static short[] Render(VoiceSynth synth, int totalFrames) {
        ...
        for (var offset = 0; (offset < totalFrames); offset += AudioMixer.MaxBlockFrames) {
            var frames = Math.Min(
                val1: AudioMixer.MaxBlockFrames,
                val2: (totalFrames - offset)
            );

            synth.Render(
                destination: output.AsSpan(
                    length: frames,
                    start: offset
                ),
                frames: frames
    ...
    private static (List<int> Slots, short[] Render) RunBabbleUtterance(int syllableCount, long cadenceTicks, ulong identitySeed, ulong utteranceOrdinal, ulong baseTick) {
        ...
        return (Slots: slots, Render: Render(
            synth: synth,
            totalFrames: 4800
        ));
    }
    ...
    public void BabbleUtteranceFiresOneDistinctSeededTriggerPerSyllableBitIdenticalAcrossTwoFreshPairings() {
        var (slotsFirst, renderFirst) = RunBabbleUtterance(
            baseTick: 1_000UL,
            cadenceTicks: 600,
            identitySeed: 0x1234UL,
            syllableCount: 6,
            utteranceOrdinal: 3UL
        );
        var (slotsSecond, renderSecond) = RunBabbleUtterance(
            baseTick: 1_000UL,
            cadenceTicks: 600,
            identitySeed: 0x1234UL,
            syllableCount: 6,
        ...
        Assert.Equal(
            expected: 6,
            actual: slotsFirst.Distinct().Count()
        );
    ...
    public void BabbleUtteranceNeverCollapsesToASingleSustainedToneForMultipleSyllables(int syllableCount) {
        var (slots, _) = RunBabbleUtterance(
            baseTick: 0UL,
            cadenceTicks: 500,
            identitySeed: 7UL,
            syllableCount: syllableCount,
            utteranceOrdinal: 1UL
        );

        Assert.Equal(
            expected: syllableCount,
            actual: slots.Count
        );
        Assert.NotEqual(
            expected: 1,
    ...
    public void BedFadeFramesBoundsThePresenceSlewOnAFullPresenceBed() {
        short[] BedBlock(int fadeFrames) {
            ...
            snapshot.Reset(listener: new AudioListener(
                Position: default,
                Yaw: FixedComplex.MultiplicativeIdentity
            ));
            snapshot.TryAddEmitter(emitter: new AudioEmitter(
                Id: 1,
                Kind: AudioEmitterKind.Bed,
                Position: default,
                MinRadius: FixedQ4816.FromRawBits(value: (2 * 65536)),
                MaxRadius: FixedQ4816.FromRawBits(value: (5 * 65536)),
        ...
        for (var n = 0; (n < AudioMixer.FramesPerSimStep); n++) {
            fadedPeak = Math.Max(
                val1: fadedPeak,
                val2: Math.Abs(value: ((int)faded[(2 * n)]))
            );
            instantPeak = Math.Max(
                val1: instantPeak,
                val2: Math.Abs(value: ((int)instant[(2 * n)]))
            );
        }
    ...
    public void FortyTriggersStealTheQuietestVoiceAndPinAtCapacity() {
        ...
        Assert.Equal(
            expected: VoiceSynth.VoiceCount,
            actual: synth.ActiveVoiceCount
        );
    }
    [Fact]
    public void LowPassFilterMeasurablyDarkensSeededNoise() {
        ...
        var openRender = Render(
            synth: open,
            totalFrames: 9600
        );
        var darkRender = Render(
            synth: dark,
            totalFrames: 9600
        );
    ...
    public void SeededNoiseReproducesBitwiseFromTheSameSeed() {
        ...
        Assert.True(condition: Render(
            synth: first,
            totalFrames: 4800
        ).AsSpan().SequenceEqual(other: Render(
            synth: second,
            totalFrames: 4800
        )));
    }
    ...
    public void SeededVoiceReproducesBitwiseAcrossTwoFreshSynths() {
        ...
        var renderFirst = Render(
            synth: first,
            totalFrames: 12_000
        );
        var renderSecond = Render(
            synth: second,
            totalFrames: 12_000
        );
    ...
    public void TriggerFiresOnceWhenTheSameSnapshotIsMixedTwiceUnderHold() {
        ...
        snapshot.TryAddEmitter(emitter: new AudioEmitter(
            ...
            Position: new FixedVector3(
                X: FixedQ4816.FromRawBits(value: 65536),
                Y: FixedQ4816.Zero,
                Z: FixedQ4816.Zero
            ),
            MinRadius: FixedQ4816.FromRawBits(value: (2 * 65536)),
            MaxRadius: FixedQ4816.FromRawBits(value: (4 * 65536)),
            Curve: AudioAttenuationCurve.Smoothstep,
            FadeFrames: 0,
            GainQ16: UnityQ16,
            Channel: AudioChannel.Mix,
            Source: AudioSourceKey.Synth(patchId: "hum")
        ));
        snapshot.TryAddTrigger(trigger: new SynthTrigger(
            EmitterId: 1,
            GainQ16: UnityQ16,
            PatchId: "hum",
            Seed: 1UL,
        ...
        Assert.Equal(
            expected: 1,
            actual: mixer.Synth.ActiveVoiceCount
        );
    }
    [Fact]
