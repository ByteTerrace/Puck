...
public sealed class ArenaSearchLawTests {
    ...
    public void ACheckpointOfAnotherShapeIsRefusedWhole() {
        ...
        foreach (var bent in new[] {
            ...
            (job with { Root = (job.Root with { Shape = -1 }) }),
            (job with { Root = (job.Root with { Token = -1 }) }),
            (job with { Root = (job.Root with { Target = -1 }) }),
            (job with { Root = (job.Root with { Seats = new long[1] }) }),
            (job with { Root = null! }),
