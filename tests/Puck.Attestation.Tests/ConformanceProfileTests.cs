...
public sealed class ConformanceProfileTests {
    ...
    public void AttestationCeiling_IsEnforcedBeforeCborParsing() {
        ...
        var exception = Assert.Throws<FormatException>(testCode: () => _ = AttestationProfile.Base.DecodeAttestation(
            codec: cbor,
            wire: new byte[(AttestationResourceLimits.AttestationBytes + 1)]
        ));

        Assert.Contains(
            expectedSubstring: "permits at most",
    ...
    public void ComposedProfileNames_UseTheNormativeExtensionSpelling() {
        ...
        Assert.Equal(
            expected: "attestation-v1-base+sealed-attestation-v1",
            actual: profile.Name
        );
    }
    [InlineData(0)]
    ...
    public void PayloadAboveTheExplicitCeiling_IsRefusedDeterministically() {
        var cbor = new CborAttestationCodec();
        var keys = MintDomainKeys(subject: "user:profile");
        var oversizedPayloadClaim = AttestationSigner.SignClaim(
            codec: cbor,
            domain: keys.Domain,
            ...
            audience: "world:profile",
            sequence: null,
            claimBytes: new byte[(AttestationResourceLimits.PayloadBytes + 1)]
        );

        var exception = Assert.Throws<FormatException>(testCode: () => _ = AttestationProfile.Base.DecodeAttestation(
            codec: cbor,
            wire: cbor.EncodeAttestation(attestation: oversizedPayloadClaim)
        ));

        Assert.Contains(
            expectedSubstring: "payload",
            actualString: exception.Message,
            comparisonType: StringComparison.OrdinalIgnoreCase
        );
    }
    ...
    public void SealedRecipientTextCeiling_IsCheckedOnTheAuthenticatedPayloadDecode() {
        ...
        var oversizedRecipient = sealedPayload with {
            RecipientId = sealedPayload.RecipientId with {
                Subject = new string(
            c: 's',
            count: (AttestationResourceLimits.TextStringUtf8Bytes + 1)
        ),
            },
        };
    ...
    public void SignatureOutsideTheExactCeiling_IsRefusedBeforeSignatureVerification() {
        ...
        var result = AttestationProfile.Base.VerifyChain(
            codec: cbor,
            claim: (claim with { Signature = oversizedSignature }),
            chain: null,
            trustList: trust,
            now: Now,
            expectedPurpose: "profile.test",
    ...
    public void TextCeilings_AreMeasuredInUtf8BytesNotCharacters() {
        var cbor = new CborAttestationCodec();
        var keys = MintDomainKeys(subject: "user:profile");
        var oversizedPurposeClaim = AttestationSigner.SignClaim(
            codec: cbor,
            domain: keys.Domain,
            subject: keys.Subject,
            signerKey: keys.SubjectSigningKey,
        ...
        var exception = Assert.Throws<FormatException>(testCode: () => _ = AttestationProfile.Base.DecodeAttestation(
            codec: cbor,
            wire: cbor.EncodeAttestation(attestation: oversizedPurposeClaim)
        ));

        Assert.Contains(
            expectedSubstring: "UTF-8 bytes",
            actualString: exception.Message,
            comparisonType: StringComparison.OrdinalIgnoreCase
        );
    }
