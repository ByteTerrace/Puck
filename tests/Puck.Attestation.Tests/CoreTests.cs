...
public sealed class CoreTests {
    ...
    public void AlgorithmConfusion_DeclaredAlgorithmDiffersFromPinnedKey_IsRefused() {
        ...
        var chain = new[] { rootToIssuing, issuingToSubject };
        var trust = BuildTrustList(keys: keys, defaultMaximumAge: TimeSpan.FromHours(hours: 24));
        var confusedClaim = SignTestClaim(codec: codec, keys: keys, purpose: "test.claim", notBefore: (Epoch - 60), notAfter: (Epoch + 3_600), audience: "world:home", sequence: null, text: "algorithm-confused claim", declaredAlgorithm: AttestationAlgorithms.EcdhP256HkdfSha256Aes256Gcm);
