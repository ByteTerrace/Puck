...
internal static class IndependentAttestationImplementation {
    ...
    internal static byte[] Seal(
        ...
        if (!string.Equals(
            a: Fingerprint(bytes: recipientSubjectPublicKeyInfo),
            b: recipientId.KeyHash,
        ...
        recipient.ImportSubjectPublicKeyInfo(
            bytesRead: out var recipientBytesRead,
            source: recipientSubjectPublicKeyInfo
        );
        RequireEntireSubjectPublicKeyInfo(
            bytesRead: recipientBytesRead,
            encodedLength: recipientSubjectPublicKeyInfo.Length
        );
        ...
        try {
            ...
            using (var aes = new AesGcm(
                ...
                aes.Encrypt(
                    ...
                    tag: tag,
                    associatedData: BindAssociatedData(
    ...
    internal static byte[] SignKeyBinding(
        ...
        var payload = EncodeKeyBinding(
            targetId: targetId,
            targetSubjectPublicKeyInfo: targetSubjectPublicKeyInfo
        );
        ...
        return SignAttestation(
            header: header,
            payload: payload,
            payloadKind: KeyBindingPayloadKind,
            signingKey: signingKey
        );
    }
    ...
    internal static byte[] Unseal(ReadOnlySpan<byte> sealedPayloadBytes, ECDiffieHellman recipientPrivateKey, ReadOnlySpan<byte> headerBytes) {
        ...
        if (!string.Equals(
            a: Fingerprint(bytes: recipientPrivateKey.ExportSubjectPublicKeyInfo()),
            b: payload.RecipientId.KeyHash,
            comparisonType: StringComparison.Ordinal
        )) {
        ...
        ephemeral.ImportSubjectPublicKeyInfo(
            source: payload.EphemeralSubjectPublicKeyInfo,
            bytesRead: out var ephemeralBytesRead
        );
        RequireEntireSubjectPublicKeyInfo(
            bytesRead: ephemeralBytesRead,
            encodedLength: payload.EphemeralSubjectPublicKeyInfo.Length
        );
        ...
        try {
            ...
            using var aes = new AesGcm(
                key: key,
                tagSizeInBytes: payload.Tag.Length
            );
    ...
    internal static byte[] VerifyChain(
        ...
        if (
            (issuing.TargetId.Subject is not null) ||
            !string.Equals(
            a: issuing.TargetId.Domain,
            b: trustedRootId.Domain,
            comparisonType: StringComparison.Ordinal
        )
        ) {
        ...
        if (
            (subject.TargetId.Subject is null) ||
            !string.Equals(
            a: subject.TargetId.Domain,
            b: trustedRootId.Domain,
            comparisonType: StringComparison.Ordinal
        )
        ) {
        ...
        VerifyAttestation(
            ...
            expectedPayloadKind: ((claim.Header.Purpose == KeyBindingPurpose)
            ? KeyBindingPayloadKind
            : claim.PayloadKind),
            now: now
        );

        if (
            (claim.PayloadKind != OpaquePayloadKind) &&
            (claim.PayloadKind != SealedPayloadKind)
        ) {
    ...
    private static byte[] BindAssociatedData(ReadOnlySpan<byte> headerBytes, ReadOnlySpan<byte> recipientContext) {
        ...
        BinaryPrimitives.WriteUInt64BigEndian(
            ...
            value: checked((ulong)headerBytes.Length)
        );
        offset += sizeof(ulong);
    ...
    private static IndependentSealedPayload DecodeSealedPayload(ReadOnlySpan<byte> bytes) {
        ...
        return new IndependentSealedPayload(
            Ciphertext: ciphertext,
            EphemeralSubjectPublicKeyInfo: ephemeral,
            Nonce: nonce,
            RecipientId: id,
            Tag: tag
        );
    ...
    private static CborReader NewReader(ReadOnlySpan<byte> bytes) => new(
        data: bytes.ToArray(),
    ...
    private static void RequireSelfCertifying(IndependentBinding binding) {
        if (!string.Equals(
            a: Fingerprint(bytes: binding.SubjectPublicKeyInfo),
            b: binding.TargetId.KeyHash,
            comparisonType: StringComparison.Ordinal
        )) {
    ...
    private static void VerifyAttestation(
        ...
        if (
            !string.Equals(
            a: attestation.Header.Domain,
            b: pinnedId.Domain,
            comparisonType: StringComparison.Ordinal
        ) ||
            !string.Equals(
            a: attestation.Header.Subject,
            b: pinnedId.Subject,
            comparisonType: StringComparison.Ordinal
        ) ||
            !string.Equals(
            a: attestation.Header.Algorithm,
            b: pinnedId.Algorithm,
            comparisonType: StringComparison.Ordinal
        ) ||
            !string.Equals(
            a: attestation.Header.Purpose,
            b: expectedPurpose,
            comparisonType: StringComparison.Ordinal
        ) ||
            ...
            !string.Equals(
            a: Fingerprint(bytes: pinnedSubjectPublicKeyInfo),
            b: pinnedId.KeyHash,
            comparisonType: StringComparison.Ordinal
        )
        ) {
        ...
        verifier.ImportSubjectPublicKeyInfo(
            bytesRead: out var verifierBytesRead,
            source: pinnedSubjectPublicKeyInfo
        );
        RequireEntireSubjectPublicKeyInfo(
            bytesRead: verifierBytesRead,
            encodedLength: pinnedSubjectPublicKeyInfo.Length
        );
