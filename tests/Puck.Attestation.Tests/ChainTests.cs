...
public sealed class ChainTests {
    ...
    public void BindingAgeCompatibility_OmittedDefaultInheritsAuthoredCeiling() {
        ...
        Assert.Equal(
            expected: trust.DefaultMaximumAge,
            actual: trust.DefaultRootBindingMaximumAge
        );
        Assert.Equal(
            expected: trust.DefaultMaximumAge,
            actual: trust.DefaultSubjectBindingMaximumAge
        );
    }
    [Fact]
    public void BindingTargetSpki_TrailingBytesAreRefusedAtTheFollowingSignatureHop() {
        ...
        AssertRefused(
            reasonMustContain: "issuing-vouches-subject binding signature",
            result: result
        );
    }
    [Fact]
    ...
    public void DepthOneInDisguise_RootVouchesForItselfAsTheIssuingKey_IsRefused() {
        ...
        var rootVouchesItself = AttestationSigner.SignKeyBinding(
            codec: codec,
            domain: keys.Domain,
            signerKey: keys.RootKey,
            signerAlgorithm: AttestationAlgorithms.EcdsaP256Sha256,
        ...
        var rootVouchesSubject = AttestationSigner.SignKeyBinding(
            codec: codec,
            domain: keys.Domain,
            signerKey: keys.RootKey,
            signerAlgorithm: AttestationAlgorithms.EcdsaP256Sha256,
    ...
    public void DepthThree_SubjectKeyVouchesForAFurtherKey_IsRefused() {
        ...
        var subjectToDelegate = AttestationSigner.SignKeyBinding(
            codec: codec,
            domain: keys.Domain,
        ...
        var delegateClaim = AttestationSigner.SignClaim(
            codec: codec,
            domain: keys.Domain,
            subject: "user:frank",
            ...
            audience: "world:home",
            sequence: null,
            claimBytes: System.Text.Encoding.UTF8.GetBytes(s: "claim signed one hop too deep")
        );
    ...
    public void PayloadKindSeparation_ClaimDeclaringAnUndefinedKind_IsRefusedByDefault() {
        var (codec, _, chain, trust, claim) = BuildFixture();
        var unknownKind = SignedAttestation.Reencode(
            codec: codec,
            header: claim.Header,
            payloadKind: ((AttestationPayloadKind)99),
    ...
    public void PayloadKindSeparation_ClaimDeclaringKeyBindingKind_IsRefused() {
        var (codec, _, chain, trust, claim) = BuildFixture();
        var kindConfused = SignedAttestation.Reencode(
            codec: codec,
            header: claim.Header,
    ...
    public void PinnedDomain_ClaimNamingADomainNoEntryPins_IsRefused() {
        ...
        var foreignDomainClaim = AttestationSigner.SignClaim(
            codec: codec,
            ...
            audience: "world:home",
            sequence: null,
            claimBytes: System.Text.Encoding.UTF8.GetBytes(s: "frank's key, somebody else's domain")
        );
    ...
    public void TrustListShape_DirectlySigningEntryPinsARootRatherThanASubjectKey_Throws() {
        ...
        Assert.Contains(
            expectedSubstring: "must pin a SUBJECT key",
    ...
    public void TrustListShape_EntryPinningASealingKey_Throws() {
        ...
        Assert.Contains(
            expectedSubstring: "not an attestation SIGNING algorithm",
            actualString: exception.Message,
            comparisonType: StringComparison.OrdinalIgnoreCase
        );
    }
    ...
    public void TrustListShape_KeyBytesDoNotHashToPinnedId_Throws() {
        ...
        Assert.Contains(
            expectedSubstring: "not self-certifying",
            actualString: exception.Message,
            comparisonType: StringComparison.OrdinalIgnoreCase
        );
    }
    ...
    public void TrustListShape_SameDomainPinnedTwiceInOneMode_Throws() {
        ...
        Assert.Contains(
            expectedSubstring: "twice in the same mode",
            actualString: exception.Message,
            comparisonType: StringComparison.OrdinalIgnoreCase
        );
    }
    ...
    public void TrustListShape_SelfConsistentVouchingEntry_Constructs() {
        ...
        var exception = Record.Exception(testCode: () => _ = BuildTrustList(
            keys: keys,
            defaultMaximumAge: null
        ));
    ...
    public void TrustListShape_TwoDifferentKeysPinnedForOneSubjectInOneMode_Throws() {
        ...
        Assert.Contains(
            expectedSubstring: "lookup returns the first match",
            actualString: exception.Message,
            comparisonType: StringComparison.OrdinalIgnoreCase
        );
    }
    ...
    public void TrustListShape_VouchingEntryPinsAnIssuingKeyRatherThanARoot_Throws() {
        ...
        Assert.Contains(
            expectedSubstring: "must pin a root id",
            actualString: exception.Message,
            comparisonType: StringComparison.OrdinalIgnoreCase
        );
    }
