...
public sealed class ChainTests {
    ...
    public void SlotReach_NarrowerEntryYieldsNarrowerReachForTheSameClaim() {
        ...
        var result = AttestationVerifier.VerifyChain(codec: codec, claim: claim, chain: chain, trustList: narrowTrust, now: Now, expectedPurpose: "test.claim", expectedAudience: "world:home");
    ...
    public void SlotReach_DenyByDefault_EmptyReachVerifiesButReachesNothing() {
        ...
        var result = AttestationVerifier.VerifyChain(codec: codec, claim: claim, chain: chain, trustList: emptyReachTrust, now: Now, expectedPurpose: "test.claim", expectedAudience: "world:home");
    ...
    public void PayloadKindSeparation_ClaimDeclaringKeyBindingKind_IsRefused() {
        var (codec, _, chain, trust, claim) = BuildFixture();
        var kindConfused = SignedAttestation.Reencode(codec: codec, header: claim.Header, payloadKind: AttestationPayloadKind.KeyBinding, payloadBytes: claim.PayloadBytes, signature: claim.Signature);
    ...
    public void DepthThree_SubjectKeyVouchesForAFurtherKey_IsRefused() {
        ...
        var delegateId = KeyId.ForSubject(domain: keys.Domain, subject: "user:frank", subjectPublicKeyInfo: delegateSpki, algorithm: AttestationAlgorithms.EcdsaP256Sha256);
        var subjectToDelegate = AttestationSigner.SignKeyBinding(codec: codec, domain: keys.Domain, signerKey: keys.SubjectSigningKey, signerAlgorithm: AttestationAlgorithms.EcdsaP256Sha256, targetId: delegateId, targetSubjectPublicKeyInfo: delegateSpki, notBefore: (Epoch - 30), notAfter: (Epoch + (86_400L * 30)));
        var delegateClaim = AttestationSigner.SignClaim(codec: codec, domain: keys.Domain, subject: "user:frank", signerKey: delegateKey, signerAlgorithm: AttestationAlgorithms.EcdsaP256Sha256, purpose: "test.claim", notBefore: (Epoch - 60), notAfter: (Epoch + 3_600), audience: "world:home", sequence: null, claimBytes: System.Text.Encoding.UTF8.GetBytes(s: "claim signed one hop too deep"));
    ...
    public void DepthOneInDisguise_RootVouchesForItselfAsTheIssuingKey_IsRefused() {
        ...
        var rootVouchesItself = AttestationSigner.SignKeyBinding(codec: codec, domain: keys.Domain, signerKey: keys.RootKey, signerAlgorithm: AttestationAlgorithms.EcdsaP256Sha256, targetId: rootAsIssuingId, targetSubjectPublicKeyInfo: keys.RootSpki, notBefore: (Epoch - 30), notAfter: (Epoch + (86_400L * 30)));
        var rootVouchesSubject = AttestationSigner.SignKeyBinding(codec: codec, domain: keys.Domain, signerKey: keys.RootKey, signerAlgorithm: AttestationAlgorithms.EcdsaP256Sha256, targetId: keys.SubjectSigningId, targetSubjectPublicKeyInfo: keys.SubjectSigningSpki, notBefore: (Epoch - 30), notAfter: (Epoch + (86_400L * 30)));
    ...
    public void DepthZero_DirectlyPinnedSubjectKeyAdmitsClaimWithNoBindings() {
        ...
        var directTrust = new TrustList(
            entries: [new TrustListEntry(PinnedId: keys.SubjectSigningId, PublicKeySubjectPublicKeyInfo: keys.SubjectSigningSpki, Mode: AttestationTrustMode.SignsDirectly, Reach: friendReach, MaximumAge: null)],
            defaultMaximumAge: TimeSpan.FromHours(hours: 24)
        );

        var result = AttestationVerifier.VerifyChain(codec: codec, claim: claim, chain: null, trustList: directTrust, now: Now, expectedPurpose: "test.claim", expectedAudience: "world:home");
    ...
    public void PinnedDomain_ClaimNamingADomainNoEntryPins_IsRefused() {
        ...
        var foreignDomainClaim = AttestationSigner.SignClaim(codec: codec, domain: foreignDomainKeys.Domain, subject: keys.Subject, signerKey: keys.SubjectSigningKey, signerAlgorithm: AttestationAlgorithms.EcdsaP256Sha256, purpose: "test.claim", notBefore: (Epoch - 60), notAfter: (Epoch + 3_600), audience: "world:home", sequence: null, claimBytes: System.Text.Encoding.UTF8.GetBytes(s: "frank's key, somebody else's domain"));
    ...
    public void TrustListShape_KeyBytesDoNotHashToPinnedId_Throws() {
        ...
        var exception = Assert.Throws<ArgumentException>(testCode: () => _ = new TrustList(
            entries: [new TrustListEntry(PinnedId: keys.RootId, PublicKeySubjectPublicKeyInfo: keys.SubjectSigningSpki, Mode: AttestationTrustMode.Vouches, Reach: friendReach, MaximumAge: null)],
    ...
    public void TrustListShape_VouchingEntryPinsAnIssuingKeyRatherThanARoot_Throws() {
        ...
        var exception = Assert.Throws<ArgumentException>(testCode: () => _ = new TrustList(
            entries: [new TrustListEntry(PinnedId: keys.IssuingId, PublicKeySubjectPublicKeyInfo: keys.IssuingSpki, Mode: AttestationTrustMode.Vouches, Reach: friendReach, MaximumAge: null)],
            defaultMaximumAge: null
        ));
    ...
    public void TrustListShape_DirectlySigningEntryPinsARootRatherThanASubjectKey_Throws() {
        ...
        var exception = Assert.Throws<ArgumentException>(testCode: () => _ = new TrustList(
            entries: [new TrustListEntry(PinnedId: keys.RootId, PublicKeySubjectPublicKeyInfo: keys.RootSpki, Mode: AttestationTrustMode.SignsDirectly, Reach: friendReach, MaximumAge: null)],
            defaultMaximumAge: null
        ));
    ...
    public void TrustListShape_EntryPinningASealingKey_Throws() {
        ...
        var exception = Assert.Throws<ArgumentException>(testCode: () => _ = new TrustList(
            entries: [new TrustListEntry(PinnedId: keys.SubjectSealingId, PublicKeySubjectPublicKeyInfo: keys.SubjectSealingSpki, Mode: AttestationTrustMode.SignsDirectly, Reach: friendReach, MaximumAge: null)],
            defaultMaximumAge: null
        ));
    ...
    public void TrustListShape_SameDomainPinnedTwiceInOneMode_Throws() {
        var (_, keys, _, _, _) = BuildFixture();
        var friendReach = new HashSet<string>(comparer: StringComparer.Ordinal) { "slot:friend" };
        var entry = new TrustListEntry(PinnedId: keys.RootId, PublicKeySubjectPublicKeyInfo: keys.RootSpki, Mode: AttestationTrustMode.Vouches, Reach: friendReach, MaximumAge: null);
    ...
    public void TrustListShape_TwoDifferentKeysPinnedForOneSubjectInOneMode_Throws() {
        ...
        var rotatedId = KeyId.ForSubject(domain: keys.Domain, subject: keys.Subject, subjectPublicKeyInfo: rotatedSpki, algorithm: AttestationAlgorithms.EcdsaP256Sha256);

        var exception = Assert.Throws<ArgumentException>(testCode: () => _ = new TrustList(
            entries: [
                new TrustListEntry(PinnedId: keys.SubjectSigningId, PublicKeySubjectPublicKeyInfo: keys.SubjectSigningSpki, Mode: AttestationTrustMode.SignsDirectly, Reach: friendReach, MaximumAge: null),
                new TrustListEntry(PinnedId: rotatedId, PublicKeySubjectPublicKeyInfo: rotatedSpki, Mode: AttestationTrustMode.SignsDirectly, Reach: friendReach, MaximumAge: null),
            ],
            defaultMaximumAge: null
        ));
    ...
    public void TrustListShape_SameTwoKeysInDifferentSlots_Constructs() {
        ...
        var exception = Record.Exception(testCode: () => _ = new TrustList(
            entries: [
                new TrustListEntry(PinnedId: keys.SubjectSigningId, PublicKeySubjectPublicKeyInfo: keys.SubjectSigningSpki, Mode: AttestationTrustMode.SignsDirectly, Reach: friendReach, MaximumAge: null),
                new TrustListEntry(PinnedId: KeyId.ForSubject(domain: keys.Domain, subject: "user:other", subjectPublicKeyInfo: rotatedSpki, algorithm: AttestationAlgorithms.EcdsaP256Sha256), PublicKeySubjectPublicKeyInfo: rotatedSpki, Mode: AttestationTrustMode.SignsDirectly, Reach: friendReach, MaximumAge: null),
            ],
            defaultMaximumAge: null
        ));
