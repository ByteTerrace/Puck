...
public sealed class AttestationKeysLawTests {
    ...
    public void ImportPkcs8PrivateKey_Garbage_ThrowsCryptographicException() {
        ...
        _ = Assert.Throws<CryptographicException>(testCode: () => AttestationKeys.ImportPkcs8PrivateKey(
    ...
    public void ImportPkcs8PrivateKey_P256Key_RoundTripsThePublicKey() {
        ...
        using var imported = AttestationKeys.ImportPkcs8PrivateKey(
            algorithm: AttestationAlgorithms.EcdsaP256Sha256,
            pkcs8: pkcs8
        );

        Assert.Equal(
            expected: original.ExportSubjectPublicKeyInfo(),
            actual: imported.ExportSubjectPublicKeyInfo()
        );
    ...
    public void ImportPkcs8PrivateKey_P384Key_IsRefused() {
        ...
        var exception = Assert.Throws<ArgumentException>(testCode: () => AttestationKeys.ImportPkcs8PrivateKey(
            algorithm: AttestationAlgorithms.EcdsaP256Sha256,
            pkcs8: pkcs8
        ));
    ...
    public void ImportPkcs8PrivateKey_SealingOrUnknownAlgorithm_IsRefusedBeforeTheBytesAreRead() {
        ...
        var sealing = Assert.Throws<ArgumentException>(testCode: () => AttestationKeys.ImportPkcs8PrivateKey(
            algorithm: AttestationAlgorithms.EcdhP256HkdfSha256Aes256Gcm,
            pkcs8: garbage
        ));
        var unknown = Assert.Throws<ArgumentException>(testCode: () => AttestationKeys.ImportPkcs8PrivateKey(
        ...
        Assert.Equal(
            expected: "algorithm",
            actual: sealing.ParamName
        );
        Assert.Equal(
            expected: "algorithm",
            actual: unknown.ParamName
        );
    }
    [Fact]
