...
public sealed class DurableReplayTransactionTests {
    ...
    private static ReplayCommitRequirement VerifySequencedClaim() {
        ...
        Assert.True(condition: result.TryGetReplayCommit(
            requirement: out var requirement,
            slot: "slot:wallet"
        ));
    ...
    public void Commit_PersistsTheEffectAndReplayMarkAcrossAClosedAndReopenedDatabase() {
        ...
        Assert.True(condition: database.TryCommit(
            requirement: requirement,
            effectKey: "wallet:user:durable",
            delta: 7
        ));

        Assert.Equal(
            expected: 7L,
            actual: database.ReadEffect(effectKey: "wallet:user:durable")
        );
        Assert.Equal(
            expected: requirement.Sequence,
            actual: database.ReadSequence(requirement: requirement)
        );
    ...
    public async Task ConcurrentReceivers_CommitOneEffectAndOneReplayAdvanceExactlyOnce() {
        ...
        var tasks = Enumerable.Range(
            ...
            .Select(selector: _ => Task.Run(function: () => {
                ...
                return database.TryCommit(
                    requirement: requirement,
                    effectKey: "wallet:user:contended",
                    delta: 5
                );
            }))
            .ToArray();
        ...
        Assert.Equal(
            expected: 1,
            actual: tasks.Count(predicate: task => task.Result)
        );
        Assert.Equal(
            expected: 5L,
            actual: database.ReadEffect(effectKey: "wallet:user:contended")
        );
        Assert.Equal(
            expected: requirement.Sequence,
            actual: database.ReadSequence(requirement: requirement)
    ...
    public void FailureBeforeCommit_RollsBackBothTheEffectAndReplayMark() {
        ...
        Assert.Equal(
            expected: 11L,
            actual: database.ReadEffect(effectKey: "wallet:user:rollback")
        );
    }
}
...
internal sealed class DurableReplayDatabase : IDisposable {
    ...
    internal bool TryCommit(ReplayCommitRequirement requirement, string effectKey, long delta, bool failBeforeCommit = false) {
        ...
        if (
            (current is not null) &&
            (requirement.Sequence <= current.Value)
        ) {
            return false;
        }
        ...
        using (var effect = connection.CreateCommand()) {
            ...
            effect.CommandText = """
                ...
                """;
            _ = effect.Parameters.AddWithValue(
                parameterName: "$effect_key",
                value: effectKey
            );
            _ = effect.Parameters.AddWithValue(
        ...
        using (var replay = connection.CreateCommand()) {
            ...
            replay.CommandText = """
                ...
                """;
            _ = replay.Parameters.AddWithValue(
            ...
            _ = replay.Parameters.AddWithValue(
                parameterName: "$epoch_start",
                value: requirement.EpochStartUnixSeconds
            );
            _ = replay.Parameters.AddWithValue(
                parameterName: "$retain_through",
                value: requirement.RetainThroughUnixSeconds
            );
            _ = replay.Parameters.AddWithValue(
                parameterName: "$sequence",
                value: EncodeSequence(value: requirement.Sequence)
            );
    ...
    private static ulong? ReadSequence(SqliteConnection connection, SqliteTransaction? transaction, ReplayCommitRequirement requirement) {
        ...
        return ((value is byte[] bytes)
            ? BinaryPrimitives.ReadUInt64BigEndian(source: bytes)
            : null
        );
    }
