...
public sealed class ClockAndSequenceTests {
    ...
    public void FiniteReplayHorizon_DerivesTheEpochAndRetentionDeadlineFromSignedNotBefore() {
        ...
        Assert.Equal(
            expected: expectedEpochStart,
            actual: commit.EpochStartUnixSeconds
        );
        Assert.Equal(
            expected: ((expectedEpochStart + (2 * horizonSeconds)) - 1),
            actual: commit.RetainThroughUnixSeconds
        );
        Assert.Equal(
            expected: 7UL,
    ...
    public void SequenceAtomicity_OneBearerClaimPresentedByFourReceiversAtOnce_AcceptsExactlyOnce() {
        ...
        Assert.Equal(
            expected: 1,
            actual: contendedResults.Count(predicate: result => result.Verified)
        );
    }
    [Fact]
    public void SequenceAtomicity_SplitCompareAndAdvanceControl_AcceptsEveryPresentation() {
        ...
        var bearerOnce = SignTestClaim(
            ...
            audience: null,
            sequence: 42UL,
            text: "one bearer claim, four receivers"
        );
