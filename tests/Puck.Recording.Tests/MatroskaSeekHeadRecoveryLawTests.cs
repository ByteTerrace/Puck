...
public sealed class MatroskaSeekHeadRecoveryLawTests {
    ...
    private static int SegmentPayloadOffset(ReadOnlySpan<byte> bytes) {
        ...
        Assert.True(
            condition: (segmentOffset >= 0),
            userMessage: "the muxer did not write a Segment element"
        );
    ...
    public void LiveFileReservesVoidAndCleanStopPublishesSeekHeadAtTheSameOffset() {
        ...
        using var muxer = new MatroskaMuxer(
            output: output,
            webmDocType: true
        );
        ...
        Assert.Equal(
            expected: 0xEC,
            actual: live[reservationOffset]
        );
        Assert.False(
            condition: live.AsSpan(start: reservationOffset).StartsWith(value: SeekHeadId),
            userMessage: "a live file must not advertise unresolved Seek entries"
        );
        ...
        Assert.True(
            condition: completed.AsSpan(start: reservationOffset).StartsWith(value: SeekHeadId),
            userMessage: "clean Stop must replace the live Void reservation with SeekHead at the identical offset"
        );
    }
}
