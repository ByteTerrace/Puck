...
public sealed class StreamDrainTests {
    ...
    public async Task CancellationStopsAnAlwaysBufferedReaderEvenIfItIgnoresTheToken() {
        ...
        using var stream = new BufferedSource(cancellation: cancelled);
        ...
        Assert.True(condition: stream.CanRead);
    }
    [Fact]
    public async Task DrainsThroughEofWithoutTakingStreamOwnership() {
        using var stream = new MemoryStream(buffer: new byte[1025]);
    ...
    private sealed class BufferedSource(CancellationTokenSource cancellation) : MemoryStream {
        ...
        public override ValueTask<int> ReadAsync(Memory<byte> buffer, CancellationToken cancellationToken = default) {
            if (++Reads > 3) { throw new InvalidOperationException(message: "Drain ignored its deadline while data remained buffered."); }
            if (Reads == 3) { cancellation.Cancel(); }
            buffer.Span[0] = 1;
            return ValueTask.FromResult(result: 1);
        }
    }
}
