...
public sealed class FixedStepPumpTests {
    ...
    private static (FixedStepPump Pump, RecordingSimulation Simulation) NewPump() {
        ...
        var inputRouter = new InputRouter(
            ...
            principalResolver: new SingleSlotPrincipal()
        );
    ...
    public void Advance_ConstantStepTicks_MatchesLegacyTickAndElapsedTicksFormula() {
        ...
        pump.Advance(
            deltaTicks: StepTicks240Hz,
            maxFrameTicks: 10_000UL,
            stepTicks: StepTicks240Hz
        );
        pump.Advance(
            deltaTicks: (StepTicks240Hz / 2UL),
            maxFrameTicks: 10_000UL,
            stepTicks: StepTicks240Hz
        );
        pump.Advance(
            deltaTicks: ((StepTicks240Hz * 2UL) + (StepTicks240Hz / 2UL)),
            maxFrameTicks: 10_000UL,
            stepTicks: StepTicks240Hz
        );
        ...
        for (var index = 0; (index < simulation.Steps.Count); index++) {
            ...
            Assert.Equal(
                expected: expectedElapsedTicks,
                actual: step.ElapsedTicks
            );
            Assert.Equal(
                expected: StepTicks240Hz,
                actual: step.StepTicks
            );
        }

        Assert.Equal(
            expected: (4UL * StepTicks240Hz),
    ...
    public void Advance_StepTicksChangesBetweenCalls_TickStaysMonotonicAndElapsedTicksStaysCorrect() {
        ...
        Assert.Equal(
            actual: legacyBuggyFirstTick,
            expected: 1UL
        ); // sanity: the bug is real for these inputs

        // Second call: rate changes to 120 Hz mid-life. 2 steps' worth of wall time is due.
        pump.Advance(
            deltaTicks: (StepTicks120Hz * 2UL),
            maxFrameTicks: 10_000UL,
            stepTicks: StepTicks120Hz
        );
        ...
        Assert.Equal(
            expected: 3UL,
            actual: fourthStep.Tick
        );
        Assert.Equal(
            expected: 4UL,
            actual: fifthStep.Tick
        );
        Assert.NotEqual(
            expected: legacyBuggyFirstTick,
            actual: fourthStep.Tick
        );
        ...
        Assert.Equal(
            expected: (previousElapsedTicksBeforeRateChange + StepTicks120Hz),
            actual: fourthStep.ElapsedTicks
        );
        Assert.Equal(
            expected: (previousElapsedTicksBeforeRateChange + (2UL * StepTicks120Hz)),
            actual: fifthStep.ElapsedTicks
        );
        Assert.Equal(
            expected: StepTicks120Hz,
            actual: fourthStep.StepTicks
        );
        Assert.Equal(
            expected: StepTicks120Hz,
            actual: fifthStep.StepTicks
        );
        ...
        foreach (var step in simulation.Steps) {
            Assert.True(
                condition: distinctTicks.Add(item: step.Tick),
                userMessage: $"tick {step.Tick} was assigned to more than one step"
            );
        }
    }
}
