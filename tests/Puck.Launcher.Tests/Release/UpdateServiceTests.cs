...
public sealed class UpdateServiceTests : IDisposable {
    ...
    private void PublishManifest(ReleaseManifest signedManifest, string channel) {
        ...
        File.WriteAllBytes(
            ...
            bytes: ReleaseChainFixture.ToWireBytes(manifest: signedManifest)
        );
    }
    ...
    public async Task CheckAsync_ReportsAvailable_ForAVerifiedNewerManifestInsideRollout() {
        ...
        var service = new UpdateService(
            ...
            stager: new ContentAddressedUpdateStager(
                cache: new ContentAddressedStore(root: Path.Combine(
                    path1: m_root.RootPath,
                    path2: "objects"
                )),
                cacheRoot: m_root.RootPath,
        ...
        Assert.Equal(
            expected: UpdateCheckOutcome.Available,
            actual: result.Outcome
        );
        Assert.NotNull(@object: result.Manifest);
        Assert.Equal(
            expected: "1.0.1",
            actual: result.Manifest!.Version
        );
    }
    [Fact]
    public async Task CheckAsync_ReportsOutsideRollout_WhenBucketExcludesThisInstall() {
        ...
        Assert.Equal(
            expected: UpdateCheckOutcome.OutsideRollout,
            actual: result.Outcome
        );
    }
    [Fact]
    public async Task CheckAsync_ReportsUpToDate_WhenNoManifestPublished() {
        ...
        Assert.Equal(
            expected: UpdateCheckOutcome.UpToDate,
            actual: result.Outcome
        );
    }
