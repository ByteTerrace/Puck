...
public sealed class ReleaseManifestTests {
    ...
    public void Canonicalize_RoundTrips_ByteIdentically() {
        ...
        var reparsed = System.Text.Json.JsonSerializer.Deserialize<ReleaseManifest>(
    ...
    public void Normalize_IsIdempotent() {
        ...
        Assert.Equal(
            expected: ReleaseCanonicalizer.Canonicalize(document: once).Bytes,
            actual: ReleaseCanonicalizer.Canonicalize(document: twice).Bytes
        );
    }
    [Fact]
    public void Normalize_SortsPayloadsAndDedupesRevoked() {
        ...
        Assert.Equal(
            expected: "linux-x64",
            actual: normalized.Payloads[0].Rid
        );
        Assert.Equal(
            expected: "win-x64",
            actual: normalized.Payloads[1].Rid
        );
        Assert.Equal(
            expected: "a.dll",
            actual: normalized.Payloads[1].Files[0].Path
        );
        Assert.Equal(
            expected: "b.dll",
            actual: normalized.Payloads[1].Files[1].Path
        );
    ...
    public void ReleaseTrustAnchor_BuildsValidatingTrustListEntry() {
        ...
        var entry = anchor.ToTrustListEntry(
            maximumAge: null,
            reach: new HashSet<string>(comparer: StringComparer.Ordinal) { "release" }
        );

        entry.Validate();

        Assert.Equal(
            expected: AttestationTrustMode.Vouches,
            actual: entry.Mode
        );
    ...
    public void Validate_RefusesEmptyPayloads() {
        ...
        Assert.Contains(
            collection: errors,
            filter: error => (error.Path == "payloads")
        );
    }
    [Fact]
    ...
    public void Validate_RefusesRolloutPercentOutOfRange() {
        ...
        Assert.Contains(
            collection: errors,
            filter: error => (error.Path == "rollout.percent")
        );
    }
    [Fact]
    public void Validate_RefusesUnknownExtensionMember() {
        ...
        Assert.Contains(
            collection: errors,
            filter: error => (error.Path == "extensions.app")
        );
    }
}
