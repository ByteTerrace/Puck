...
public sealed class FileLengthAnalyzerTests {
    ...
    private static string Ledger(params (string Path, int Lines)[] recorded) {
        ...
        for (var index = 0; (index < recorded.Length); index++) {
            builder.Append(value: ((index == 0)
                ? " "
                : ", ")).Append(value: '"').Append(value: recorded[index].Path).Append(value: "\": ").Append(value: recorded[index].Lines);
        }
    ...
    private static AnalysisResult Run(string fileName, int lines, string? ledgerJson) {
        var compilation = Harness.Compile(
            ...
            sources: new SourceFile(
                Name: fileName,
                Text: SourceOfLines(lines: lines)
            )
        );
        ...
        return Harness.Analyze(
            ...
            analyzer: new FileLengthAnalyzer(),
    ...
    public void ALedgerEntryAtOrUnderTheCeilingIsOffSchema() {
        var result = Run(
            fileName: "Long.cs",
            lines: 5,
            ledgerJson: Ledger(("Long.cs", Ceiling))
        );

        var diagnostic = result.Single(id: "LEN004");

        Assert.Contains(
            expectedSubstring: "must be an integer above the ceiling",
    ...
    public void AMissingLedgerReportsLen004() {
        ...
        Assert.Contains(
            expectedSubstring: "No FileLengths.json",
            actualString: diagnostic.GetMessage(),
            comparisonType: StringComparison.Ordinal
        );
    }
    ...
    public void ARecordedFileThatGrewReportsLen002() {
        var result = Run(
            fileName: "Long.cs",
            lines: 131,
            ledgerJson: Ledger(("Long.cs", 130))
        );

        var diagnostic = result.Single(id: "LEN002");

        Assert.Contains(
            expectedSubstring: "'Long.cs' is 131 lines, over the 130 lines",
            actualString: diagnostic.GetMessage(),
            comparisonType: StringComparison.Ordinal
        );
    }
    ...
    public void ARecordedFileThatShrankToTheCeilingReportsLen003() {
        ...
        Assert.Contains(
            expectedSubstring: "still records it at 130",
            actualString: diagnostic.GetMessage(),
            comparisonType: StringComparison.Ordinal
        );
    }
    ...
    public void AnUnrecordedFileOverTheCeilingReportsLen001() {
        ...
        Assert.Contains(
            expectedSubstring: "'Long.cs' is 101 lines, over the 100-line ceiling",
            actualString: diagnostic.GetMessage(),
            comparisonType: StringComparison.Ordinal
        );
    }
