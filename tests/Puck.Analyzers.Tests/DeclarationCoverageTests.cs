...
public sealed class DeclarationCoverageTests {
    ...
    private static void AssertBrandWasRefusedAsUnrecordable(string source, string placement) {
        ...
        Assert.Contains(
            expectedSubstring: placement,
            actualString: result.Single(id: "VER007").GetMessage()
        );
    }
    ...
    public void BrandOnALambdaIsRefused() {
        ...
        AssertBrandWasRefusedAsUnrecordable(
            placement: "a lambda",
            source: source
        );
    }
    ...
    public void BrandWithAnEmptyIdIsIgnoredRatherThanTracked() {
        var result = Harness.Run(
            source: Sources.BrandedMethod(attribute: "[VerifiedCode(\"\")]"),
            manifestJson: Manifest.Empty
        );
