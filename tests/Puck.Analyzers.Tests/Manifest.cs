...
internal static class Manifest {
    ...
    public static string? RecordedHash(string json, string id) {
        var entries = json.IndexOf(value: "\"entries\"", comparisonType: StringComparison.Ordinal);

        if (entries < 0) {
            return null;
        }

        var key = json.IndexOf(value: ("\"" + id + "\""), startIndex: entries, comparisonType: StringComparison.Ordinal);

        if (key < 0) {
            return null;
        }

        var marker = json.IndexOf(value: "\"sha256\"", startIndex: key, comparisonType: StringComparison.Ordinal);

        if (marker < 0) {
            return null;
        }

        var open = json.IndexOf(value: '"', startIndex: json.IndexOf(value: ':', startIndex: marker));
        var close = json.IndexOf(value: '"', startIndex: (open + 1));

        return json.Substring(startIndex: (open + 1), length: (close - open - 1));
    }
