...
public sealed class ManifestIntegrityTests {
    private static readonly string ShadowManifestPath = Path.Combine(
    ...
    public void AbsentManifestIsRefusedRatherThanReadAsAnEmptyLedger() {
        ...
        Assert.Contains(
            expectedSubstring: "AdditionalFile",
            actualString: result.Single(id: "VER006").GetMessage()
        );
    }
    [Fact]
    ...
    public void EntryNamingAnAlgorithmNothingImplementsIsRefused() {
        ...
        Assert.Contains(
            expectedSubstring: "not-a-real-algorithm",
            actualString: result.Single(id: "VER006").GetMessage()
        );
    }
    [Fact]
    public void MalformedManifestIsRefusedRatherThanReadAsAnEmptyLedger() {
        var result = Harness.Run(
            ...
            manifestJson: Manifest.Malformed
        );
    ...
    public void ManifestDiagnosticIsReportedOnTheLedgerFileItself() {
        ...
        Assert.Equal(
            expected: Harness.ManifestPath,
            actual: result.Single(id: "VER006").Location.GetLineSpan().Path
        );
    }
    [Fact]
    ...
    public void OneOffSchemaEntryIsReportedWithoutDiscardingEveryOtherEntrysSweep() {
        ...
        Assert.Equal(
            expected: new[] { "VER002", "VER006" },
            actual: result.Ids
        );
        Assert.Contains(
            expectedSubstring: "missing-laws",
            actualString: result.Single(id: "VER006").GetMessage()
        );
        Assert.Contains(
            expectedSubstring: "deleted-brand",
            actualString: result.Single(id: "VER002").GetMessage()
        );
    }
    [Fact]
    ...
    public void RepeatedEntryKeyIsRefusedRatherThanLettingTheLastCopyDecide() {
        ...
        Assert.Contains(
            expectedSubstring: "Duplicate object member 'target'",
            actualString: result.Single(id: "VER006").GetMessage()
        );
    }
    [Fact]
    public void RepeatedRootMemberIsRefusedRatherThanLettingTheLastCopyReplaceTheLedger() {
        ...
        Assert.Contains(
            expectedSubstring: "Duplicate object member 'entries'",
            actualString: result.Single(id: "VER006").GetMessage()
        );
    }
    [Fact]
    ...
    public void UnreadableManifestIsRefusedRatherThanReadAsAnEmptyLedger() {
        ...
        Assert.Contains(
            expectedSubstring: "could not be read",
            actualString: result.Single(id: "VER006").GetMessage()
        );
    }
}
