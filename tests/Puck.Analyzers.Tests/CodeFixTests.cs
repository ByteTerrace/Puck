...
public sealed class CodeFixTests {
    private static readonly string StaleHash = new(
        c: '0',
        count: 64
    );
    ...
    private static async Task<(ImmutableArray<Diagnostic> Diagnostics, ImmutableArray<CodeAction> Actions)> DiagnoseAndOfferAsync(FixSubject subject, CancellationToken cancellationToken) {
        var diagnostics = await FixHarness.DiagnoseAsync(
            solution: subject.Solution,
            projectId: subject.Solution.ProjectIds[0],
            cancellationToken: cancellationToken
        );
        var mismatches = diagnostics.Where(predicate: diagnostic => string.Equals(
            a: diagnostic.Id,
            b: "VER001",
        ...
        return (Diagnostics: diagnostics, Actions: await FixHarness.ActionsAsync(
            solution: subject.Solution,
    ...
    private static async Task<string> RepairAsync(FixSubject subject, CancellationToken cancellationToken) {
        var (_, actions) = await DiagnoseAndOfferAsync(
            cancellationToken: cancellationToken,
            subject: subject
        );
        var changed = await FixHarness.ApplyAsync(
            action: Assert.Single(collection: actions),
            cancellationToken: cancellationToken
        );

        return (await FixHarness.ManifestTextAsync(
            solution: changed!,
    ...
    public async Task FixAllWritesEveryDriftedBrandsRecomputedHash() {
        ...
        var after = (await FixHarness.ManifestTextAsync(
            solution: changed!,
            manifestId: subject.ManifestId,
            cancellationToken: cancellationToken
        )).ToString();
        ...
        var expected = mismatches
            .OrderBy(
            keySelector: diagnostic => diagnostic.Properties["VerifiedCodeId"],
        ...
        Assert.Equal(
            expected: expected,
            actual: new[] { Manifest.RecordedHash(
                id: "alpha",
                json: after
    ...
    public async Task FixChangesNothingButTheRecordedHash() {
        ...
        Assert.Equal(
            expected: before.Replace(
                comparisonType: StringComparison.Ordinal,
                newValue: LetterLeadingDrift.Hash,
                oldValue: StaleHash
            ),
            actual: after
        );
    ...
    public async Task FixEditsTheLedgersOwnEntryNotAnUnrelatedObjectSharingItsKey() {
        ...
        Assert.Contains(
            actualString: text,
            expectedSubstring: decoyHash
        );
        Assert.Equal(
            expected: LetterLeadingDrift.Hash,
            actual: Manifest.RecordedHash(
                id: Sources.TargetId,
                json: text
            )
        );
    ...
    public void FixIsOfferedForFingerprintDriftAlone() =>
        Assert.Equal(
            expected: new[] { "VER001" },
            actual: new VerifiedCodeCodeFixProvider().FixableDiagnosticIds.ToArray()
        );
    [Fact]
    public async Task FixIsOfferedWhenTheBrandIdContainsAnApostrophe() {
        var cancellationToken = TestContext.Current.CancellationToken;
        var source = Sources.BrandedMethod(
            attribute: "[VerifiedCode(\"target's\")]",
            body: "        return 2;"
        );
        var hash = Harness.Fingerprint(
            source: source,
        ...
        using var subject = FixHarness.Create(
            source: source,
            manifestJson: StaleLedger(id: "target's")
        );
    ...
    public async Task FixLeavesEveryHashNestedInsideTheEntryAlone() {
        ...
        Assert.Contains(
            actualString: text,
            expectedSubstring: nestedHash
        );
        Assert.Equal(
            expected: 1,
            actual: (text.Split(separator: LetterLeadingDrift.Hash).Length - 1)
        );
        Assert.Equal(
            expected: LetterLeadingDrift.Hash,
            actual: Manifest.RecordedHash(
    ...
    public async Task FixLeavesTheSamePhysicalLedgerStaleInEveryOtherProjectThatLinksIt() {
        ...
        var linkedManifestId = subject.AddLinkedProject(
        ...
        var edited = (await FixHarness.ManifestTextAsync(
            solution: changed!,
            manifestId: subject.ManifestId,
            cancellationToken: cancellationToken
        )).ToString();
        var linked = (await FixHarness.ManifestTextAsync(
            cancellationToken: cancellationToken,
            manifestId: linkedManifestId,
            solution: changed!
        )).ToString();
    ...
    public async Task FixPreservesLineFeedOnlyLineEndings() {
        using var subject = FixHarness.Create(
            source: LetterLeadingDrift.Source,
            manifestJson: StaleLedger().Replace(
                comparisonType: StringComparison.Ordinal,
                newValue: "\n",
                oldValue: "\r\n"
            )
    ...
    public async Task FixPreservesTheLedgersEncoding() {
        ...
        var text = await FixHarness.ManifestTextAsync(
            solution: changed!,
            manifestId: subject.ManifestId,
            cancellationToken: cancellationToken
        );
        ...
        Assert.Equal(
            expected: 3,
            actual: text.Encoding!.GetPreamble().Length
        );
    }
    [Fact]
    public async Task FixWritesADigitLeadingHashIntactAndLeavesTheEntryParseable() {
        ...
        using var subject = FixHarness.Create(
            source: DigitLeadingDrift.Source,
            manifestJson: StaleLedger()
        );
        ...
        Assert.Contains(
            actualString: text,
            expectedSubstring: "\"sha256\""
        );
        Assert.Equal(
            expected: DigitLeadingDrift.Hash,
            actual: Manifest.RecordedHash(
                id: Sources.TargetId,
                json: text
            )
        ...
        Assert.Equal(
            expected: StaleLedger().Replace(
                comparisonType: StringComparison.Ordinal,
                newValue: DigitLeadingDrift.Hash,
                oldValue: StaleHash
            ),
            actual: text
        );
    ...
    public async Task LedgerTheFixProducedSatisfiesTheAnalyzer() {
        ...
        var after = await FixHarness.DiagnoseAsync(
            solution: changed!,
            projectId: changed!.ProjectIds[0],
            cancellationToken: cancellationToken
        );
    ...
    public async Task NoFixIsOfferedForABrandTheLedgerDoesNotRecord() {
        ...
        using var subject = FixHarness.Create(
            ...
            manifestJson: Manifest.Empty
        );

        var (diagnostics, actions) = await DiagnoseAndOfferAsync(
            cancellationToken: cancellationToken,
            subject: subject
        );
        ...
        Assert.Contains(
            ...
            filter: diagnostic => string.Equals(
                a: diagnostic.Id,
                b: "VER001",
                comparisonType: StringComparison.Ordinal
            )
    ...
    public async Task NoFixIsOfferedForAnUnclaimedEntry() {
        ...
        using var subject = FixHarness.Create(
            source: Sources.Unbranded(),
            manifestJson: StaleLedger(id: "deleted-brand")
        );
        ...
        var actions = await FixHarness.ActionsAsync(
            solution: subject.Solution,
            documentId: subject.SourceId,
    ...
    public async Task NoFixIsOfferedForAnUnusableRecordedHashBecauseTheLedgerIsRefusedFirst() {
        ...
        Assert.Equal(
            expected: new[] { "VER006" },
            actual: diagnostics.Select(selector: diagnostic => diagnostic.Id).ToArray()
        );
    }
    [Fact]
    public async Task NoFixIsOfferedWhenTwoDocumentsAreNamedLikeTheLedger() {
        var cancellationToken = TestContext.Current.CancellationToken;
        var secondHash = new string(
            c: 'c',
            count: 64
        );
        ...
        var withSecond = FixHarness.AddSecondManifest(
            ...
            path: Path.Combine(
                ...
                path2: "second",
                path3: Harness.ManifestFileName
            )
        );

        var diagnostics = await FixHarness.DiagnoseAsync(
            solution: withSecond,
            projectId: withSecond.ProjectIds[0],
            cancellationToken: cancellationToken
        );
