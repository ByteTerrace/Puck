...
public sealed class ManifestOwnershipTests {
    ...
    public void BrandedDeclarationMovedToAnotherTypeNoLongerSatisfiesTheEntryItLeft() {
        ...
        Assert.Contains(
            expectedSubstring: "SomewhereElse",
            actualString: result.Single(id: "VER008").GetMessage()
        );
    }
    [Fact]
    public void CompilationWithoutTheMarkerAttributeStillSweepsTheEntriesItOwns() {
        ...
        var result = Harness.Analyze(
            compilation: Harness.CompileWithoutBrandAttribute(
    ...
    public void EntryRecordingNoSymbolIsRefused() {
        ...
        Assert.Contains(
            expectedSubstring: "documentation-comment id",
    ...
    public void EntryWhoseRecordedSymbolNamesAnAssemblyOtherThanItsOwnerIsRefused() {
        ...
        Assert.Contains(
            expectedSubstring: "Nonexistent.Assembly",
            actualString: result.Single(id: "VER006").GetMessage()
        );
    }
    [Fact]
    public void MovingABrandedTypeLeavesItsDeclaredDependenciesNamingNothing() {
        ...
        Assert.Contains(
            expectedSubstring: "InvertibleBitMix.FirstMultiplier",
            actualString: result.Single(id: "VER010").GetMessage()
        );
    }
    [Fact]
    public void SecondClaimantWithDifferentTokensIsReportedAsADuplicateClaimAsWellAsADrift() {
        ...
        Assert.Equal(
            expected: new[] { "VER001", "VER008", "VER009" },
            actual: result.Ids
        );
        Assert.Contains(
            expectedSubstring: "Beta.Target",
            actualString: result.Single(id: "VER001").GetMessage()
        );
    }
    [Fact]
    public void TwoDeclarationsClaimingOneEntryAreBothRefused() {
        ...
        // for Alpha.
        Assert.Equal(
            expected: new[] { "VER008", "VER009" },
            actual: result.Ids
        );
        Assert.Contains(
            expectedSubstring: "Beta.Target",
            actualString: result.Single(id: "VER009").GetMessage()
        );
        Assert.Contains(
            expectedSubstring: "Alpha.Target",
            actualString: result.Single(id: "VER009").GetMessage()
        );
    }
    [Fact]
