...
public sealed class ShapePanelLawTests {
    ...
    private static SdfInstruction[] FirstScope(SdfProgram program) {
        ...
        Assert.True(
            condition: (push >= 0),
            userMessage: "no field scope was emitted"
        );
        ...
        return instructions.Skip(count: push).Take(count: ((pop - push) + 1)).ToArray();
    }
    ...
    private static double MeasuredPlateFace(ShapeDocument shape, Vector3 axis) {
        ...
        return (Probe - DistanceAt(
            program: EmitStatic(shape: (shape with { Panel = null })),
            point: (axis * Probe)
        ));
    }
    ...
    private static double MeasuredRecessDepth(ShapeDocument shape) {
        ...
        return (face - (probe - DistanceAt(
            program: EmitStatic(shape: shape),
            z: probe
        )));
    }
    ...
    private static (SdfInstruction Plate, SdfInstruction Copy, bool FreshChain, SdfInstruction FaceTranslate, SdfInstruction? CopyScale) PanelPair(SdfInstruction[] scope) {
        var shapes = scope.Select(selector: (instruction, index) => (instruction, index)).Where(predicate: pair => (pair.instruction.Op == SdfOp.ShapeBlend)).ToArray();
        ...
        var between = scope.Skip(count: (shapes[0].index + 1)).Take(count: ((shapes[1].index - shapes[0].index) - 1)).ToArray();
        var reset = Array.FindLastIndex(
            array: between,
            match: instruction => (instruction.Op == SdfOp.ResetPoint)
        );
        var faceTranslate = Array.FindLastIndex(
            array: between,
            match: instruction => (instruction.Op == SdfOp.Translate)
        );

        Assert.True(
            condition: (faceTranslate > reset),
            userMessage: "no face translate follows the copy's own chain"
        );
        ...
        return (
            ...
            between.Skip(count: (faceTranslate + 1)).Where(predicate: instruction => (instruction.Op == SdfOp.Scale)).Cast<SdfInstruction?>().LastOrDefault()
        );
    }
    ...
    public void ANonFiniteDepthIsRefusedByName() =>
        AssertCanonicalizerRefusesNaming(
            document: Document(Shape(
                SdfSolidPrimitive.Box,
                PlateScale,
                (RecessedPanel with { Depth = float.NaN })
            )),
            needle: "depth"
        );
    [Fact]
    ...
    public void ANonFiniteOrNegativeInsetIsRefusedByName() {
        AssertCanonicalizerRefusesNaming(
            document: Document(Shape(
                SdfSolidPrimitive.Box,
                PlateScale,
                (RecessedPanel with { Inset = -0.01f })
            )),
            needle: "inset"
        );
        AssertCanonicalizerRefusesNaming(
            document: Document(Shape(
                SdfSolidPrimitive.Box,
                PlateScale,
                (RecessedPanel with { Inset = float.NaN })
            )),
            needle: "inset"
        );
    }
    ...
    public void APanelledShapeChargesTwoAgainstTheStampBudget() {
        var document = Document(Shape(
            SdfSolidPrimitive.Box,
            PlateScale,
            RecessedPanel
        ));
        var bare = Document(Shape(
            SdfSolidPrimitive.Box,
            PlateScale
        ));
        ...
        Assert.Equal(
            2,
            CreationStampEmitter.PerCopyInstanceCount(document: document)
        );
        Assert.Equal(
            1,
            CreationStampEmitter.PerCopyInstanceCount(document: bare)
        );
    }
    [Fact]
    public void APanelledShapeEmitsExactlyTwoShapesInItsOwnScopeInThePool() {
        foreach (var scale in new[] { UniformPlateScale, PlateScale }) {
            ...
            Assert.True(
                condition: freshChain,
                userMessage: "the pool's panel copy chained after the plate's own shape instead of from its own ResetPoint"
            );
            Assert.Equal(
                ((uint)SdfBlendOp.Subtraction),
                copy.Blend
            );
        }
    }
    ...
    public void APanelledShapeEmitsExactlyTwoShapesInOneScopeOnTheStaticPath() {
        var program = EmitStatic(shape: Shape(
            SdfSolidPrimitive.Box,
            PlateScale,
            RecessedPanel
        ));
        var shapeCount = program.Instructions.Count(predicate: instruction => (instruction.Op == SdfOp.ShapeBlend));
        var pushCount = program.Instructions.Count(predicate: instruction => (instruction.Op == SdfOp.PushField));
        var popCount = program.Instructions.Count(predicate: instruction => (instruction.Op == SdfOp.PopField));
    ...
    public void APanelledShapeUsesItsOwnMaterialForThePanelCandidateOnBothStaticForms() {
        var whole = EmitStatic(shape: Shape(
            SdfSolidPrimitive.Box,
            PlateScale,
            RecessedPanel
        ));
        var perShape = EmitShapeStamp(document: Document(Shape(
            SdfSolidPrimitive.Box,
            PlateScale,
            RecessedPanel
        )));
        ...
        Assert.Equal(
            2,
            whole.Instructions.Where(predicate: instruction => (instruction.Op == SdfOp.ShapeBlend)).Select(selector: instruction => instruction.Material).Distinct().Count()
        );
        ...
        var perShapeMaterials = perShape.Instructions.Where(predicate: instruction => (instruction.Op == SdfOp.ShapeBlend)).Select(selector: instruction => instruction.Material).ToArray();

        Assert.Equal(
            2,
            perShapeMaterials.Length
        );
        Assert.NotEqual(
            perShapeMaterials[0],
            perShapeMaterials[1]
    ...
    public void ARaisedPanelStandsExactlyDepthProudOfThePlateFaceWhateverTheInset() {
        ...
        foreach (var inset in new[] { 0f, 0.02f, 0.05f, 0.09f }) {
            var shape = Shape(
                SdfSolidPrimitive.Box,
                PlateScale,
                (RaisedPanel with { Inset = inset })
            );
            var face = MeasuredPlateFace(shape: shape);
            var raisedFace = (Probe - DistanceAt(
                program: EmitStatic(shape: shape),
                z: Probe
            ));
    ...
    public void ARaisedPanelledShapePacksAnInstanceRadiusCoveringTheRaise() {
        ...
        Assert.NotEmpty(collection: active);
        Assert.True(
            condition: active.Any(predicate: instance => (instance.Radius >= required)),
            userMessage: $"expected an instance covering reach+|depth| {required}, saw radii [{string.Join(
                separator: ", ",
                values: active.Select(selector: instance => instance.Radius)
            )}]"
        );
    }
    [Fact]
    public void ARecessFloorSitsExactlyDepthBelowThePlateFaceWhateverTheInset() {
        foreach (var inset in new[] { 0f, 0.02f, 0.05f, 0.09f }) {
            var shape = Shape(
                SdfSolidPrimitive.Box,
                PlateScale,
                (RecessedPanel with { Inset = inset })
            );
    ...
    public void ARecessedPanelComposesBySubtractionAndARaisedOneByUnion() {
        var recessed = EmitStatic(shape: Shape(
            SdfSolidPrimitive.Box,
            PlateScale,
            RecessedPanel
        ));
        var raised = EmitStatic(shape: Shape(
            SdfSolidPrimitive.Box,
            PlateScale,
            RaisedPanel
        ));
        var recessedBlends = recessed.Instructions.Where(predicate: instruction => (instruction.Op == SdfOp.ShapeBlend)).Select(selector: instruction => ((SdfBlendOp)instruction.Blend)).ToArray();
        var raisedBlends = raised.Instructions.Where(predicate: instruction => (instruction.Op == SdfOp.ShapeBlend)).Select(selector: instruction => ((SdfBlendOp)instruction.Blend)).ToArray();

        Assert.Equal(
            2,
            recessedBlends.Length
        );
        Assert.Contains(
            collection: recessedBlends,
            expected: SdfBlendOp.Subtraction
        );
        Assert.Equal(
            2,
            raisedBlends.Length
        ...
        Assert.Contains(
            collection: raisedBlends,
            expected: SdfBlendOp.Union
        );
    ...
    public void AZeroLengthFaceIsRefusedByNameWhileANonUnitOneIsNormalized() {
        ...
        var refusal = Assert.Throws<DocumentValidationException>(testCode: () => CreationCanonicalizer.Canonicalize(
            document: Document(Shape(
                SdfSolidPrimitive.Box,
                PlateScale,
                zero
        ...
        var normalizedControl = CreationCanonicalizer.Canonicalize(
            document: Document(Shape(
                SdfSolidPrimitive.Box,
                PlateScale,
                control
        ...
        Assert.Equal(
            Vector3.UnitZ,
            normalizedControl.Shapes![0].Panel!.Face!.Value
        );
    }
    [Fact]
    ...
    public void ContactCopiesAreIdenticalWithAndWithoutAPanel() {
        ...
        Assert.Single(collection: panelledCopies);
        Assert.Single(collection: bareCopies);
        Assert.Equal(
            bareCopies[0].Center,
            panelledCopies[0].Center
        );
        Assert.Equal(
            bareCopies[0].HalfExtents,
            panelledCopies[0].HalfExtents
        );
        Assert.Equal(
            bareCopies[0].UniformScale,
            panelledCopies[0].UniformScale
        );
        Assert.Equal(
            bareCopies[0].PlaneNormal,
            panelledCopies[0].PlaneNormal
        );
    }
    [Fact]
    ...
    public void HalfExtentReadsASphereOrEllipsoidsSupportOffAxis() {
        var diagonal = Vector3.Normalize(value: new Vector3(
            x: 1f,
            y: 1f,
            z: 0f
        ));
    ...
    public void HalfExtentRefusesAZeroOrNonFiniteAxisByName() {
        var zero = Assert.Throws<ArgumentOutOfRangeException>(testCode: () => SdfSolidGeometry.HalfExtent(
            SdfSolidPrimitive.Sphere,
            Vector3.One,
            SdfLift.Extrude,
            Vector3.Zero
        ));
        var nan = Assert.Throws<ArgumentOutOfRangeException>(testCode: () => SdfSolidGeometry.HalfExtent(
            SdfSolidPrimitive.Sphere,
            Vector3.One,
            SdfLift.Extrude,
            new Vector3(
    ...
    public void ReachWideningAddsOnlyAPositivePanelRaise() {
        var baseReach = SdfSolidGeometry.Reach(
            lift: SdfLift.Extrude,
            scale: PlateScale,
            type: SdfSolidPrimitive.Box
        );
    ...
    public void StaticAndDynamicPathsEmitTheSamePanelPairForTheSamePanelledDocument() {
        foreach (var scale in new[] { UniformPlateScale, PlateScale }) {
            foreach (var panel in new[] { RecessedPanel, RaisedPanel }) {
                var staticPair = PanelPair(scope: FirstScope(program: EmitStatic(shape: Shape(
                    SdfSolidPrimitive.Box,
                    scale,
                    panel
                ))));
                var poolPair = PanelPair(scope: FirstScope(program: EmitPool(
                    scale: scale,
                    panel: panel
                )));

                AssertSameShape(
                    actual: poolPair.Plate,
                    expected: staticPair.Plate
                );
                AssertSameShape(
                    actual: poolPair.Copy,
                    expected: staticPair.Copy
                );
                Assert.True(condition: staticPair.FreshChain);
                Assert.True(condition: poolPair.FreshChain);
                Assert.Equal(
                    staticPair.FaceTranslate.Data0,
                    poolPair.FaceTranslate.Data0
                );
                Assert.Equal(
                    staticPair.CopyScale.HasValue,
                    poolPair.CopyScale.HasValue
                );

                if (staticPair.CopyScale is { } copyScale) {
                    Assert.Equal(
                        copyScale.Data0,
                        poolPair.CopyScale!.Value.Data0
                    );
                }
            }
        }
    ...
    public void StaticPerShapeProbeDominatesAPanelledShapeAtTheWordLevel() {
        foreach (var scale in new[] { UniformPlateScale, PlateScale }) {
            ...
            WorldPlacementStamper.EmitProbe(
                ...
                reservedCount: 0,
                reservedShapeInstances: CreationStampEmitter.PerCopyInstanceCount(document: document)
            );
    ...
    public void StaticScopedProbeDominatesAWholeCreationOfPanelledShapesWithinTheStampBudget() {
        ...
        var shapes = Enumerable.Range(
            count: shapeCount,
            start: 0
        ).Select(selector: index => Shape(
            SdfSolidPrimitive.Box,
            UniformPlateScale,
            RecessedPanel,
            id: index
        ...
        foreach (var scale in new[] { UniformPlateScale, PlateScale }) {
            foreach (var panel in new[] { RecessedPanel, RaisedPanel }) {
                ...
                var authoredTwice = FirstScope(program: EmitPool(
                    scale: (scale * 2f),
                    panel: (panel with { Inset = (panel.Inset * 2f), Depth = (panel.Depth * 2f) })
                ));

                Assert.Equal(
                    authoredTwice.Length,
                    scaled.Length
                );

                for (var index = 0; (index < scaled.Length); index++) {
                    Assert.Equal(
                        authoredTwice[index].Op,
                        scaled[index].Op
                    );
                    Assert.Equal(
                        authoredTwice[index].Shape,
                        scaled[index].Shape
                    );
                    Assert.Equal(
                        authoredTwice[index].Blend,
                        scaled[index].Blend
                    );
                    Assert.Equal(
                        authoredTwice[index].Data0,
                        scaled[index].Data0
                    );
                    Assert.Equal(
                        authoredTwice[index].Data1,
                        scaled[index].Data1
                    );
                }
            }
    ...
    private static readonly ShapePanelDocument RecessedPanel = new(
        Inset: 0.05f,
        Depth: 0.1f,
        Material: 1
    );
    private static readonly ShapePanelDocument RaisedPanel = new(
        Inset: 0.05f,
        Depth: -0.1f,
        Material: 1
    );
