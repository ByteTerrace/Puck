# Advisory tick wall time

Wall time is advisory: it is machine- and load-dependent and is not part of the
byte-for-byte baseline. The work units in each `*.cost.json` are the exact record.

Machine: Microsoft Windows 10.0.26200; X64; 16 logical processor(s); .NET 10.0.12

| World | Ticks | Median tick (us) | Mean tick (us) |
|---|---|---|---|
| arena | 224 | 16.9 | 15.9 |
| backgammon | 630 | 1.0 | 10.0 |
| billiards | 510 | 23.8 | 27.9 |
| bowling | 600 | 0.6 | 0.6 |
| codenames | 162 | 24.1 | 108.1 |
| dominoes | 600 | 0.6 | 0.6 |
| freecell | 610 | 5.4 | 25.6 |
| hexlines | 600 | 98.8 | 124.2 |
| klondike | 680 | 10.1 | 43.8 |
| mancala | 590 | 3.9 | 7.2 |
| moth-courtyard | 600 | 10.0 | 16.7 |
| pipeline | 600 | 0.5 | 0.5 |
| poker | 610 | 3.1 | 8.1 |
| pong | 2941 | 35.4 | 41.3 |
| reversi | 580 | 140.7 | 156.2 |
| snake | 635 | 11.5 | 14.6 |
| solitaire | 600 | 0.6 | 0.6 |
| spider | 810 | 9.4 | 98.8 |
| stratego | 190 | 10.6 | 123.9 |
| tictactoe | 600 | 1.5 | 4.5 |
