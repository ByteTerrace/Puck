...
public sealed class CreationEffectorLawTests {
    ...
    private static readonly Vector3 Hip = new(
        x: 0f,
        y: 1f,
        z: 0f
    );
    private static readonly Vector3 Knee = new(
        x: 0f,
        y: 0.5f,
        z: 0f
    );
    private static readonly Vector3 Ankle = new(
        x: 0f,
        y: 0.1f,
        z: 0.1f
    );
    ...
    public void AReleasedSwingReturnsToTheAuthoredPoseWhileStanceKeepsContact() {
        var released = Pack(
            effectors: [Leg(plant: new CreationPlantDocument(
                    "clock",
                    new Vector2(
                        x: 0f,
                        y: 0.5f
        ...
        AssertNear(
            actual: released,
            expected: authored,
            what: "the released swing was still pulled to the floor"
        );
        Assert.True(condition: (MathF.Abs(x: (following.Y - 0.03f)) < 1e-3f));
        Assert.True(condition: (MathF.Abs(x: (stance.Y - 0.03f)) < 1e-3f));
    }
    ...
    public void ASurfaceEffectorBringsTheTipToTheProbedStandoff() {
        ...
        Assert.True(
            condition: (MathF.Abs(x: (solved.Y - 0.03f)) < 1e-3f),
            userMessage: $"the tip did not settle at the standoff; y={solved.Y}"
        );
        Assert.True(
            condition: (inert.Y < -0.01f),
            userMessage: $"the control rig's tip moved without an effector; y={inert.Y}"
        );
    }
    ...
    public void ASurfaceProbeLandsAtStandoffOffTheHitNormal() {
        ...
        Assert.True(
            condition: WorldEffectorSolver.TryProbeSurface(
                field: field,
                origin: new Vector3(
                    x: 0f,
                    y: 0.4f,
    ...
    public void AnAuthoredPartAnchorAgreesWithThePackedPose() {
        ...
        Assert.True(condition: pool.TryBodyPartAuthoredPose(
            bodyIndex: 0,
            client: client,
            partId: "foot",
        ...
        Assert.True(condition: pool.TryBodyPartAuthoredPose(
            ...
            client: client,
            partId: "spare",
            pose: out var inertAuthored
        ));
        Assert.True(condition: pool.TryBodyPartPose(
            bodyIndex: 0,
            partId: "spare",
            pose: out var inertPacked,
            transforms: transforms
        ));
        AssertNear(
    ...
    public void CyclicDescentConvergesOnAFourBoneChain() {
        ...
        Assert.True(
            condition: (MathF.Abs(x: (residual - (Vector3.Distance(
                value1: Vector3.Zero,
                value2: far
            ) - 1f))) < 1e-2f),
        ...
        static float Solve(Vector3 target, out Vector3 tip) {
            ...
            Span<Vector3> joints = [Vector3.Zero, new Vector3(
                    x: 0f,
                    y: 0.25f,
                    z: 0f
                ), new Vector3(
            ...
            for (var bone = 0; (bone < 4); bone++) {
                ...
                Assert.True(
                    condition: (MathF.Abs(x: (Vector3.Distance(
                        value1: joints[bone],
                        value2: end
                    ) - 0.25f)) < Tolerance),
    ...
    public void EffectorsChangeNeitherTheRenderProgramNorTheHashOfAnUnEffectedRig() {
        ...
        Assert.Equal(
            expected: EmitWords(creation: inert),
            actual: EmitWords(creation: solved)
        );
    ...
    public void InvalidSwingInfluenceIsRefused(float weight) {
        Assert.Contains(
            "swingWeight",
            Refusal(document: Rig(effectors: [Leg(plant: new CreationPlantDocument(
                    "clock",
                    new Vector2(
                        x: 0f,
                        y: 1f
    ...
    public void TheBendStaysInTheDriverPosedPlane() {
        ...
        var forwardKnee = new Vector3(
            x: 0f,
            y: 0.55f,
            z: 0.15f
        );
        var behindKnee = new Vector3(
            x: 0f,
            y: 0.55f,
            z: -0.15f
        );
        ...
        Assert.True(
            condition: (forwardSolved.Z > 0f),
            userMessage: $"a knee bent forward at rest solved backward; z={forwardSolved.Z}"
        );
        Assert.True(
            condition: (behindSolved.Z < 0f),
            userMessage: $"a knee bent backward at rest solved forward; z={behindSolved.Z}"
        );
    }
    ...
    public void TheCanonicalizerRefusesEachMalformedEffectorByName() {
        ...
        Assert.Contains(
            actualString: Refuse(effector: Leg(plant: new CreationPlantDocument(
                Driver: "phantom",
                Window: new Vector2(
                    x: 0f,
                    y: 1f
        ...
        Assert.Contains(
            actualString: Refusal(document: Rig(effectors: [.. Enumerable.Range(
                    count: (CreationDocument.MaxEffectors + 1),
                    start: 0
                ).Select(selector: index => Leg(plant: null) with { Name = $"e{index}" })])),
            comparisonType: StringComparison.Ordinal,
    ...
    public void TheRigReadBackReportsAPlantedTargetThatDoesNotMoveWithTheBody() {
        ...
        Assert.True(
            condition: (driver.Phase > 0f),
            userMessage: $"the driver's phase was not reported advancing; phase={driver.Phase}"
        );
        ...
        Assert.True(
            condition: (driver.Weight > 0.99f),
            userMessage: $"the ungated driver's weight was not reported settled; weight={driver.Weight}"
        );
        Assert.True(
            condition: (planted.After.Speed > WorldGaitDrivers.MovingSpeed),
            userMessage: $"the eased speed the moving gate reads was not reported; speed={planted.After.Speed}"
        );
        ...
        Assert.True(
            condition: after.Planted,
            userMessage: "the effector inside its plant window did not report planted"
        );
        Assert.Equal(
            expected: 1f,
            actual: after.Weight
        ...
        Assert.Equal(
            expected: CreationEffectorDocument.MinChainBones,
            actual: after.Bones
        );
        Assert.True(
            condition: (Vector3.Distance(
                value1: before.Target!.Value,
                value2: after.Target!.Value
            ) < Tolerance),
        ...
        Assert.False(
            condition: freeAfter.Planted,
            userMessage: "an effector with no plant reported planted"
        );
        Assert.True(
            condition: (Vector3.Distance(
                value1: freeBefore.Target!.Value,
                value2: freeAfter.Target!.Value
            ) > (0.5f * free.Travel)),
