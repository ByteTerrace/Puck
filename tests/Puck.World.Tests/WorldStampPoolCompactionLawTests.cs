using System.Numerics;

using Puck.SignedDistance;
using Puck.World.Authoring;
using Puck.World.Client;
using Xunit;

namespace Puck.World.Tests;

/// <summary>Unused render capacity must not widen a live program's masks or move surviving transform addresses.</summary>
public sealed class WorldStampPoolCompactionLawTests {
    private const int SlotBase = 23;

    private static WorldPrototype Creation(params ShapeDocument[] shapes) {
        var canonical = CreationCanonicalizer.Canonicalize(
            document: new CreationDocument(
                Schema: CreationDocument.CurrentSchema,
                Name: "compact",
                Palette: [new(
                        "#AA7755",
                        null,
                        null,
                        null
                    )],
                Shapes: shapes,
                Frames: null
            ),
            source: "compact"
        );

        return new WorldPrototype(
            Id: "compact",
            Document: canonical.Document,
            HashRaw: canonical.Hash
        );
    }
    private static SdfProgram Emit(WorldStampPool pool) {
        var builder = new SdfProgramBuilder();

        pool.Emit(
            builder: builder,
            definition: Fixtures.BuildGradientUpDocument(gradientUp: false),
            probeWorstCase: false,
            maxPlacementScale: 1f,
            slotBase: SlotBase
        );
        return builder.Build(buildInstanceGrid: false);
    }
    private static void Reconcile(WorldStampPool pool, WorldPrototype creation, params int[] bodies) => pool.Reconcile(
        placements: [],
        creations: [creation],
        dynamics: [],
        bodyStamps: bodies.Select(selector: body => new WorldStampPool.BodyStamp(
            BodyIndex: body,
            Creation: creation,
            Scale: 1f,
            Motion: WorldLookMotion.Default
        )).ToArray()
    );
    private static ShapeDocument Shape(int id, int? group = null, SdfBlendOp blend = SdfBlendOp.Union) => new(
        Id: id,
        Name: $"part-{id}",
        Type: SdfSolidPrimitive.Box,
        Position: new Vector3(
            x: (id * 0.1f),
            y: 0f,
            z: 0f
        ),
        Rotation: Quaternion.Identity,
        Scale: new Vector3(value: 0.1f),
        Material: 0,
        Blend: blend,
        Smooth: 0f,
        Group: group
    );

    [Fact]
    public void AFrameMovingAUnionMemberIsIncludedInTheBound() {
        var pool = new WorldStampPool();
        var first = Shape(
            id: 0,
            group: 7
        );
        var second = Shape(
            id: 1,
            group: 7
        );
        var creation = Creation(
            first,
            second
        );
        var canonical = CreationCanonicalizer.Canonicalize(
            document: creation.Document with {
                Frames = [new FrameDocument(
                    Name: "move",
                    Transforms: [new FrameTransformDocument(
                            Id: second.Id,
                            Position: new Vector3(
                                x: 2f,
                                y: 0f,
                                z: 0f
                            ),
                            Rotation: Quaternion.Identity,
                            Scale: second.Scale
                        )]
                )],
            },
            source: "compact"
        );

        creation = new WorldPrototype(
            Id: creation.Id,
            Document: canonical.Document,
            HashRaw: canonical.Hash
        );
        Reconcile(
            pool,
            creation,
            0
        );
        var group = Assert.Single(collection: Emit(pool: pool).Instances);

        Assert.Equal(
            (SlotBase + 1),
            group.Slot
        );
        Assert.True(condition: (group.Radius >= 2.1f));
    }
    [Fact]
    public void ARemoteCutterDoesNotWidenTheGroupsSolidBound() {
        var pool = new WorldStampPool();
        var solid = Shape(
            id: 0,
            group: 7
        ) with { Position = new Vector3(
            x: 0f,
            y: 8f,
            z: 0f
        ) };
        var cutter = Shape(
            blend: SdfBlendOp.Subtraction,
            group: 7,
            id: 1
        ) with {
            Position = new Vector3(
            x: 10f,
            y: 0f,
            z: 0f
        ),
            Scale = new Vector3(value: 2f),
        };

        Reconcile(
            pool,
            Creation(
                solid,
                cutter
            ),
            0
        );
        var group = Assert.Single(collection: Emit(pool: pool).Instances);

        Assert.Equal(
            (SlotBase + 1),
            group.Slot
        );
        Assert.InRange(
            group.Radius,
            0.1f,
            0.3f
        );
    }
    [InlineData(false, 2)]
    [InlineData(true, 1)]
    [Theory]
    public void AnInhabitedAnimatedPlacementRendersOnlyThroughItsBody(bool inhabited, int expectedInstances) {
        var initial = Creation(Shape(0));
        var creation = new WorldPrototype(
            initial.Id,
            initial.Document with {
            Frames = [new FrameDocument(
                    Name: "pose",
                    Transforms: [new FrameTransformDocument(
                            Id: 0,
                            Position: Vector3.Zero,
                            Rotation: Quaternion.Identity,
                            Scale: new Vector3(value: .1f)
                        )]
                )],
        }
        );
        var row = new WorldPlacement(
            Id: "station",
            PrototypeId: creation.Id,
            Position: Vector3.Zero,
            YawDegrees: 0f,
            Scale: 1f,
            Inhabit: (inhabited
            ? new WorldPlacementInhabit(
                    Kit: "display",
                    Look: null,
                    Source: Puck.World.Protocol.IntentSource.Idle
                )
            : null)
        );
        var pool = new WorldStampPool();
        var bodies = new[] { new WorldStampPool.BodyStamp(
            0,
            creation,
            1f,
            WorldLookMotion.Default
        ) };

        pool.Reconcile(
            bodyStamps: bodies,
            creations: [creation],
            dynamics: [],
            placements: [row]
        );
        Assert.Equal(
            expectedInstances,
            Emit(pool: pool).Instances.Count
        );

        // Changing the row's ownership also retires an already-registered placement copy.
        pool.Reconcile(
            [row with { Inhabit = new WorldPlacementInhabit(
                    "display",
                    null,
                    Puck.World.Protocol.IntentSource.Idle
                ) }],
            [creation],
            [],
            bodies
        );
        Assert.Single(collection: Emit(pool: pool).Instances);
        pool.Reconcile(
            [row with { Inhabit = new WorldPlacementInhabit(
                    "display",
                    null,
                    Puck.World.Protocol.IntentSource.Idle
                ) }],
            [creation],
            [],
            []
        );
        Assert.Empty(collection: Emit(pool: pool).Instances);
    }
    [Fact]
    public void EmptyPoolEmitsNoGeometry() {
        var program = Emit(pool: new WorldStampPool());

        Assert.Empty(collection: program.Instances);
        Assert.DoesNotContain(
            collection: program.Instructions,
            filter: instruction => (instruction.Op == SdfOp.ShapeBlend)
        );
    }
    [Fact]
    public void GroupsAndNewShapesUseAuthorSlotsWithoutPlaceholderInstances() {
        var pool = new WorldStampPool();
        var shapes = new[] { Shape(
            id: 0,
            group: 7
        ), Shape(id: 1), Shape(
            blend: SdfBlendOp.Subtraction,
            group: 7,
            id: 2
        ) };

        Reconcile(
            pool,
            Creation(shapes),
            0
        );
        var initial = Emit(pool: pool);

        Assert.Equal(
            2,
            initial.Instances.Count
        );
        Assert.Contains(
            collection: initial.Instances,
            filter: item => (item.Slot == (SlotBase + 1))
        );
        Assert.Contains(
            collection: initial.Instances,
            filter: item => (item.Slot == (SlotBase + 2))
        );

        Reconcile(
            pool,
            Creation([.. shapes, Shape(id: 3)]),
            0
        );
        var grown = Emit(pool: pool);

        Assert.Equal(
            3,
            grown.Instances.Count
        );
        Assert.All(
            grown.Instances,
            item => Assert.True(condition: item.Active)
        );
        Assert.Contains(
            collection: grown.Instances,
            filter: item => (item.Slot == (SlotBase + 4))
        );
        Assert.Equal(
            1,
            grown.InstanceMaskWordCount
        );
    }
    [InlineData(false)]
    [InlineData(true)]
    [Theory]
    public void OnlyMembersSharingTheSameRigidMotionUseATightBound(bool inheritMotion) {
        var pool = new WorldStampPool();
        var first = Shape(
            id: 0,
            group: 7
        ) with {
            Swings = [new ShapeSwingDocument(
                Amplitude: 0.7f,
                Axis: Vector3.UnitZ,
                Driver: "sway",
                Pivot: Vector3.Zero
            )],
        };
        var second = Shape(
            id: 1,
            group: 7
        ) with { Parent = (inheritMotion
            ? first.Name!.Value
            : null) };
        var template = Creation(Shape(id: 0));
        var canonical = CreationCanonicalizer.Canonicalize(
            document: template.Document with {
                Shapes = [first, second],
                Drivers = [new CreationDriverDocument(
                    Name: "sway",
                    Signal: CreationDriverDocument.SignalTime,
                    Cadence: 1f
                )],
            },
            source: "compact"
        );

        Reconcile(
            pool,
            new WorldPrototype(
                Id: template.Id,
                Document: canonical.Document,
                HashRaw: canonical.Hash
            ),
            0
        );
        var group = Assert.Single(collection: Emit(pool: pool).Instances);

        Assert.Equal(
            (inheritMotion
            ? (SlotBase + 1)
            : SlotBase),
            group.Slot
        );
    }
    [Fact]
    public void RemovingAnEarlierBodyKeepsTheSurvivorsTransformAddress() {
        var pool = new WorldStampPool();
        var creation = Creation(Shape(id: 0));

        Reconcile(
            pool,
            creation,
            0,
            1
        );
        var before = Emit(pool: pool);

        Assert.Equal(
            2,
            before.Instances.Count
        );
        var survivorSlot = before.Instances[1].Slot;

        Assert.Equal(
            ((SlotBase + WorldStampPool.SlotsPerPlacement) + 1),
            survivorSlot
        );

        Reconcile(
            pool,
            creation,
            1
        );
        var after = Emit(pool: pool);
        var instance = Assert.Single(collection: after.Instances);

        Assert.True(condition: instance.Active);
        Assert.Equal(
            survivorSlot,
            instance.Slot
        );
        Assert.Equal(
            1,
            after.InstanceMaskWordCount
        );

        Reconcile(
            pool,
            creation,
            1,
            2
        );
        var reused = Emit(pool: pool);

        Assert.Equal(
            2,
            reused.Instances.Count
        );
        Assert.Contains(
            collection: reused.Instances,
            filter: item => (item.Slot == survivorSlot)
        );
        Assert.Contains(
            collection: reused.Instances,
            filter: item => (item.Slot == (SlotBase + 1))
        );
    }
}
