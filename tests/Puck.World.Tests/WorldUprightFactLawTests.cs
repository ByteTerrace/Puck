...
public sealed class WorldUprightFactLawTests {
    ...
    private static WorldDefinition RigidBodyDocument(string? bodyRefUnderTest = null) {
        ...
        return source with {
            ...
            StateRaw = new WorldStateSection(World: [
                new WorldStateRow(
                Name: CellName.Parse(candidate: UprightRow),
                Kind: CellKind.Fixed,
                Cells: [new StateCell(
                ...
                new WorldStateRow(
                Name: CellName.Parse(candidate: AbsentRow),
                Kind: CellKind.Fixed,
                Cells: [new StateCell(
                        Key: WorldStateRow.SlotKey,
                        Value: 0L
            ...
            Rules = [
                new WorldRule(
                Name: CellName.Parse(candidate: "upright-mirror"),
                Effects: [new ActionEffect.SetState(
                        State: UprightRow,
                        FromState: uprightBody
                    )]
            ),
                new WorldRule(
                ...
                Effects: [new ActionEffect.SetState(
                        State: AbsentRow,
                        FromState: $"{WorldRuleFacts.UprightPrefix}body:3"
                    )]
            ),
            ],
        };
    ...
    public void TheUprightChannelRefusesAMissingBodyReference_ControlAWellFormedOneValidates() {
        ...
        Assert.Contains(
            expectedSubstring: nameof(WorldRuleRefusal.SpatialChannelMalformed),
            actualString: reason,
