...
/// position/yaw) — the ratified in-flight rule docs/architecture/worlds.md states: "drop and re-derive what the engine can
...
public sealed class TransferAbortDynamicStateLawTests {
    ...
    public void DetachThenRestore_DynamicState_RoundTripsExactly_AndNeverReinstatesPark() {
        ...
        )); // "forward" role channel (Fixtures' one declared channel at ordinal 0)
        ...
        Assert.NotEqual(
            ...
            actual: capturedState.VerticalVelocity
        );
        Assert.True(
            condition: (capturedState.ChannelTimerTicks[1] > 0),
            userMessage: "the in-flight timed press must have a live remaining-ticks countdown to capture"
        );
        Assert.Equal(
            expected: FixedQ4816.One,
            actual: capturedState.ChannelTimerValues[1]
        );
        ...
        Assert.Equal(
            expected: capturedState.OverlayVelocity,
            actual: restoredState.OverlayVelocity
        );
        Assert.Equal(
            expected: capturedState.OverlayRemainingTicks,
            actual: restoredState.OverlayRemainingTicks
        );
        Assert.Equal(
            expected: capturedState.ChannelTimerTicks[1],
            actual: restoredState.ChannelTimerTicks[1]
        );
        Assert.Equal(
            expected: capturedState.ChannelTimerValues[1],
            actual: restoredState.ChannelTimerValues[1]
        );
