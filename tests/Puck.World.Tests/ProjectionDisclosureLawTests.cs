...
public sealed class ProjectionDisclosureLawTests {
    ...
    public void PresentationProjection_CarriesNoRedactedMember() {
        ...
        CollectMemberNames(
            element: JsonDocument.Parse(json: json).RootElement,
            names: found
        );
    ...
    public void PresentationProjection_RoundTripsAndHydrates() {
        ...
        Assert.NotNull(@object: projection);
        Assert.True(
            condition: WorldProjection.TryDeserialize(
                utf8Json: WorldProjection.Serialize(projection: projection!),
                projection: out var decoded,
        ...
        Assert.Equal(
            expected: WorldDisclosureTier.Presentation,
            actual: decoded!.Provenance.Tier
        );
        Assert.Equal(
            expected: "127.0.0.1:5000",
            actual: decoded.Provenance.Authority
        );
        Assert.Equal(
            expected: 3,
            actual: decoded.Provenance.Revision
        );

        Assert.True(
            condition: WorldProjection.TryToDefinition(
                definition: out var hydrated,
                projection: decoded,
                reason: out var hydrationReason
            ),
            userMessage: hydrationReason
        );
        Assert.NotNull(@object: hydrated);
        Assert.Equal(
            expected: definition.Kits.Count,
            actual: hydrated!.Kits.Count
        );
        Assert.Equal(
            expected: definition.Kits[0].Name,
            actual: hydrated.Kits[0].Name
        );
        Assert.Equal(
            expected: definition.DefaultSeatKit,
            actual: hydrated.DefaultSeatKit
        );
        Assert.Equal(
            expected: definition.Placements.Count,
            actual: hydrated.Placements.Count
        );
        Assert.Equal(
            expected: definition.SimulationRateHz,
            actual: hydrated.SimulationRateHz
        );
        ...
        Assert.Equal(
            expected: "Play",
            actual: hydrated.Metadata!.Title
        );
        Assert.Equal(
            expected: "The overworld hub.",
            actual: hydrated.Metadata.Description
        );
    ...
    public void ProjectionParse_RefusesForeignAndUnreservedMembers() {
        ...
        Assert.False(condition: WorldProjection.TryDeserialize(
            utf8Json: Encoding.UTF8.GetBytes(s: json.Replace(
                comparisonType: StringComparison.Ordinal,
                newValue: "\"$schema\"",
                oldValue: "\"schema\""
            )),
            projection: out _,
            reason: out var schemaReason
        ));
        Assert.Contains(
            actualString: schemaReason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "puck.world.projection.v1"
        );

        var withUnknown = json.Insert(
        ...
        Assert.False(condition: WorldProjection.TryDeserialize(
            utf8Json: Encoding.UTF8.GetBytes(s: withUnknown),
            projection: out _,
            reason: out var unknownReason
        ));
        Assert.Contains(
            actualString: unknownReason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "rules"
        );

        var withReserved = json.Insert(
            startIndex: (json.IndexOf(
                comparisonType: StringComparison.Ordinal,
                value: '{'
            ) + 1),
        ...
        Assert.True(
            condition: WorldProjection.TryDeserialize(
                utf8Json: Encoding.UTF8.GetBytes(s: withReserved),
                projection: out var reserved,
                reason: out var reservedReason
            ),
            userMessage: reservedReason
        );
    ...
    public void UnauthoredAdmissionEntry_ResolvesToPresentation() {
        ...
        Assert.Equal(
            expected: WorldDisclosureTier.Replica,
            actual: (entry with { Disclosure = WorldDisclosureTier.Replica }).Tier
        );

        Assert.Null(value: WorldAdmissionDoor.TryAdmitArrival(
            entries: [entry],
            sourceAuthority: "127.0.0.1:9",
        ...
        Assert.Null(value: WorldAdmissionDoor.TryAdmitArrival(
            entries: [entry with { Disclosure = WorldDisclosureTier.Replica }],
            sourceAuthority: "127.0.0.1:9",
            verdict: out var replica
        ));
        Assert.Equal(
            expected: WorldDisclosureTier.Replica,
            actual: replica!.Tier
