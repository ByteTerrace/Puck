...
public sealed class CreationSkeletonLawTests {
    ...
    private static readonly Vector3 Shoulder = new(
        x: 0.5f,
        y: 1.25f,
        z: 0f
    );
    private static readonly Vector3 Elbow = new(
        x: 0.5f,
        y: 0.75f,
        z: 0f
    );
    ...
    public void AChildRidesItsParentsSwingPivotsIncluded() {
        ...
        WorldGaitDrivers.ComposeDelta(
            ...
            phases: phases,
            rotation: out var parentRotation,
            shape: upper,
            translation: out var parentTranslation,
            weights: weights
        );
        WorldGaitDrivers.ComposeDelta(
            drivers: document.Drivers,
            phases: phases,
            rotation: out var childRotation,
            shape: carried,
            translation: out var childTranslation,
            weights: weights
        );
        WorldGaitDrivers.Chain(
            parentRotation: parentRotation,
            parentTranslation: parentTranslation,
            rotation: ref childRotation,
            translation: ref childTranslation
        );
        ...
        WorldGaitDrivers.Apply(
            deltaRotation: childRotation,
            deltaTranslation: childTranslation,
        ...
        WorldGaitDrivers.ComposeDelta(
            drivers: document.Drivers,
            phases: phases,
            rotation: out var looseRotation,
            shape: loose,
            translation: out var looseTranslation,
            weights: weights
        );
        WorldGaitDrivers.Apply(
            deltaRotation: looseRotation,
            deltaTranslation: looseTranslation,
            position: ref control,
            rotation: ref controlRotation
        );
        AssertNear(
            actual: control,
            expected: Elbow
    ...
    public void AChildsOwnSwingComposesUnderTheCarriedFrame() {
        ...
        WorldGaitDrivers.Apply(
            deltaRotation: childRotation,
            deltaTranslation: childTranslation,
            position: ref chained,
            rotation: ref chainedRotation
        );
        ...
        var angle = (2f * MathF.Acos(x: MathF.Min(
            x: 1f,
            y: MathF.Abs(x: chainedRotation.W)
        )));

        Assert.InRange(
            actual: angle,
            high: (MathF.PI + Tolerance),
            low: (MathF.PI - Tolerance)
        );
    }
    ...
    public void TheCanonicalizerRefusesAMissingSelfOrLaterParent() {
        ...
        Assert.Contains(
            expectedSubstring: "names no shape 'ghost'",
            actualString: Refusal(document: Rig(Part(
                id: 0,
                name: "fore",
                position: Elbow,
                parent: "ghost"
        ...
        Assert.Contains(
            expectedSubstring: "must be declared before",
            actualString: Refusal(document: Rig(Part(
                id: 0,
                name: "fore",
                position: Elbow,
