...
public sealed class CurveFollowLawTests {
    ...
    private static FixedVector3 ExpectedCurvePoint(float rate, float elapsedSeconds) => new(
        X: FixedQ4816.FromDouble(value: Math.Min(
            val1: (rate * elapsedSeconds),
            val2: 20f
        )),
        Y: FixedQ4816.Zero,
        Z: FixedQ4816.Zero
    ...
    public void CountCurveFollowers_ReportsTheSelectedBody_ButNotTheNoProducerControl() {
        ...
        Assert.Equal(
            expected: 1,
            actual: followFixture.Server.Population.CountCurveFollowers()
        );
        ...
        Assert.Equal(
            expected: 0,
            actual: controlFixture.Server.Population.CountCurveFollowers()
        );
    }
    [Fact]
    public void CurveFollowProducer_ArcAccumulatorSurvivesCheckpointRestoreUnderTheSameProducer() {
        ...
        Assert.True(
            condition: (capturedArcRaw > 0L),
            userMessage: "the follower must have travelled before the checkpoint, or this law proves nothing"
        );
        ...
        var arcRawAfterRestoreAndOneTick = CurrentCurveArcRaw(
            bodyIndex: 0,
            fixture: fixture
        );
        ...
        Assert.True(
            condition: (arcRawAfterRestoreAndOneTick >= capturedArcRaw),
            userMessage: $"restoring a checkpoint under the same producer must resume its travelled arc rather than restart it; captured {capturedArcRaw}, after restore + one tick {arcRawAfterRestoreAndOneTick}"
        );
    }
    [Fact]
    public void CurveFollowProducer_DrivesTheBodyOffSpawn_WhileANoProducerControlStaysPut() {
        ...
        var followDistance = PlanarDistance(
            from: followSpawn,
            to: followBody.FixedPosition
        );

        Assert.True(
            condition: (followDistance > 1f),
            userMessage: $"the curve-follow producer should have moved the body meaningfully off spawn after 2 s; moved {followDistance:0.###} units"
        );
        ...
        var controlDistance = PlanarDistance(
            from: controlSpawn,
            to: controlBody.FixedPosition
        );

        Assert.True(
            condition: (controlDistance < 1e-4f),
            userMessage: $"the control (no producer selected) must not move; moved {controlDistance:0.######} units"
        );
    }
    [Fact]
    public void CurveFollowProducer_OrbitScalarMeasurablyWidensTrackingDistance() {
        ...
        var zeroOrbitDistance = MeanTrackingDistance(
            document: WithLoopFollower(
                orbit: 0f,
                rate: Rate
            ),
            rate: Rate,
        ...
        var withOrbitDistance = MeanTrackingDistance(
            document: WithLoopFollower(
                orbit: Orbit,
                rate: Rate
            ),
        ...
        Assert.True(
            condition: (withOrbitDistance > (zeroOrbitDistance + 0.002f)),
            userMessage: $"a nonzero orbit scalar must measurably widen the mean tracking distance versus orbit = 0 — an inert (never-wired) orbit scalar would leave the two indistinguishable; orbit = 0 mean {zeroOrbitDistance:0.######}, orbit = {Orbit} mean {withOrbitDistance:0.######}"
        );
    }
    [Fact]
    ...
    public void CurveFollowProducer_SwitchingProducersRestartsTheArcAtZero_RatherThanResuming() {
        ...
        var arcOnPathBeforeSwitch = CurrentCurveArcRaw(
            bodyIndex: 0,
            fixture: fixture
        );

        Assert.True(
            condition: (arcOnPathBeforeSwitch > 0L),
            userMessage: "the follower must have travelled before switching, or this law proves nothing"
        );
        ...
        var arcOnLoopFirstTick = CurrentCurveArcRaw(
            bodyIndex: 0,
            fixture: fixture
        );

        Assert.True(
            condition: (arcOnLoopFirstTick < (arcOnPathBeforeSwitch / 4)),
            userMessage: $"selecting a different producer must restart its curve accumulator near zero rather than resume the prior producer's travelled arc; before switching {arcOnPathBeforeSwitch}, one tick after switching {arcOnLoopFirstTick}"
        );
        ...
        var arcOnPathAfterSwitchBack = CurrentCurveArcRaw(
            bodyIndex: 0,
            fixture: fixture
        );

        Assert.True(
            condition: (arcOnPathAfterSwitchBack < (arcOnPathBeforeSwitch / 4)),
            userMessage: $"switching back to the original producer must restart its arc at zero rather than resume where it was left off; before the first switch away {arcOnPathBeforeSwitch}, after switching back {arcOnPathAfterSwitchBack}"
        );
    }
    [Fact]
    public void CurveFollowProducer_TracksAGenuinelyCurvedClosedPathOnTrackAndAtTheAuthoredRate() {
        ...
        for (var tick = 0; (tick < WindowTicks); tick++) {
            ...
            var error = PlanarDistance(
                from: expected,
                to: body.FixedPosition
            );

            maxTrackingError = Math.Max(
                val1: maxTrackingError,
                val2: error
            );
        }
        ...
        Assert.True(
            condition: (maxTrackingError <= trackingTolerance),
            userMessage: $"the follower drifted {maxTrackingError:0.###} units from the travelling curve point on a genuinely curved closed path (tolerance {trackingTolerance:0.###})"
        );
    }
    [Fact]
    public void CurveFollowProducer_TracksTheCurvePointOnTrackAndAtTheAuthoredRate() {
        ...
        Assert.True(
            condition: (maxTrackingError <= trackingTolerance),
            userMessage: $"the follower drifted {maxTrackingError:0.###} units from the travelling curve point (tolerance {trackingTolerance}) — a body that never steers at the target (only marches at a fixed world axis) drifts unboundedly instead"
        );
        ...
        Assert.True(
            condition: (MathF.Abs(x: (observedRate - Rate)) <= (Rate * 0.02f)),
            userMessage: $"the follower's steady-state arc progress rate was {observedRate:0.######} u/s against an authored rate of {Rate} u/s (2% tolerance) — a body driving in a fixed world direction never converges to the curve's own rate at all"
        );
    }
}
