...
public sealed class GravityAuthoringLawTests {
    ...
    public void APlacementMayNotBeCountedByBothSourceSpellings() {
        var denied = WithGravity(gravity: PointGravity(
            attractors: [new WorldGravityAttractor(
                    Mass: 10f,
                    PlacementId: "ball"
                )],
            points: [new WorldGravityPoint(
                    PlacementId: "ball",
                    ReferenceRadius: 100f,
                    SurfaceGravity: 9.81f
                )]
        ));
        var control = WithGravity(gravity: PointGravity(points: [new WorldGravityPoint(
                PlacementId: "ball",
                ReferenceRadius: 100f,
                SurfaceGravity: 9.81f
            )]));
    ...
    public void AttachedArea_CheckpointRestoreContinuesBitIdenticallyOnTheNextSolve(bool zeroAcceleration) {
        ...
        fixture.Server.Population.EntryBody(index: 0)!.Pose(
            pitchRadians: 0f,
            rollRadians: 0f,
            x: 100f,
            y: 100f,
        ...
        Assert.Equal(
            ...
            actual: WorldReplaySnapshot.HashState(population: restored.Population)
        );
        ...
        for (var step = 0; (step < 2); step++) {
            ...
            Assert.Equal(
                expected: fixture.Server.Population.GravityAreaStatistics,
                actual: restored.Population.GravityAreaStatistics
            );
        }
    }
    [Fact]
    public void BodyOnlyGlobalSolve_ComposesWithMatchingLocalArea() {
        var definition = WithGravity(gravity: PointGravity(
            ...
            areas: [new WorldGravityArea(
                    ...
                    Acceleration: new WorldGravityAreaAcceleration.Directional(Value: new Vector3(
                        x: 0f,
                        y: -3f,
                        z: 0f
                    ))
        ...
        var field = CompileField(
            capacity: 2,
            definition: definition
        );

        field.Solve(targets: [
            new WorldGravityTarget(
                EntityIndex: 0,
                Position: new FixedVector3(
        ...
        Assert.True(condition: field.TryAcceleration(
            acceleration: out var left,
            entityIndex: 0
        ));
        ...
        Assert.Equal(
            expected: 2,
            actual: field.AreaStatistics.MatchCount
        );
    }
    [Fact]
    public void BoxArea_EvaluatesMembershipInThePlacementsYawLocalFrame() {
        ...
        var definition = source with {
            ...
            areas: [new WorldGravityArea(
                    ...
                    Priority: 0,
                    Mode: WorldGravityAreaMode.Replace,
                    Bounds: new WorldGravityAreaBounds.BoxBounds(HalfExtents: new Vector3(
                        x: 1f,
                        y: 1f,
                        z: 3f
                    )),
        ...
        Assert.False(condition: field.TryAcceleration(
            acceleration: out _,
            entityIndex: 1
        ));
    }
    ...
    public void LegacyMassSources_CompileUnchangedWhenPointsAreAbsent() {
        var definition = WithGravity(gravity: PointGravity(attractors: [new WorldGravityAttractor(
                Mass: 10f,
                PlacementId: "ball"
            )]));
    ...
    public void LocalAreaComposition_SaturatesInsteadOfWrapping_AndReplaceResetsTheFold() {
        ...
        var uniformCombine = CompileField(
            definition: WithGravity(gravity: PointGravity(
                gravitationalConstant: 0f,
        ...
        var replaceThenCombine = CompileField(
            definition: WithGravity(gravity: PointGravity(
                gravitationalConstant: 0f,
                areas: [
        ...
        foreach (var field in new[] { uniformCombine, replaceThenCombine }) {
            ...
            Assert.True(condition: field.TryAcceleration(
                acceleration: out var acceleration,
                entityIndex: 0
            ));
            Assert.Equal(
                expected: FixedQ4816.MaxValue,
    ...
    public void LocalAreaValidationRefusesNullRowsAndUnrepresentableLoweringByIndexedPath() {
        ...
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "gravity.areas[0] cannot lower"
        );
        ...
        Assert.Contains(
            expectedSubstring: "gravity.areas[0] is required",
    ...
    public void LocalArea_BoundaryIsInclusive_AndAreaOnlyOutsideBodyDoesNotParticipate() {
        ...
        Assert.True(condition: field.TryAcceleration(
            acceleration: out var boundary,
            entityIndex: 0
        ));
        Assert.Equal(
            expected: FixedQ4816.FromInteger(value: -2),
            actual: boundary.Y
        );
        Assert.False(condition: field.TryAcceleration(
            acceleration: out _,
        ...
        Assert.Equal(
            expected: 2,
            actual: field.AreaStatistics.TargetCount
        );
        Assert.Equal(
            expected: 2,
            actual: field.AreaStatistics.EvaluationCount
        );
        Assert.Equal(
            expected: 1,
            actual: field.AreaStatistics.MatchCount
    ...
    public void LocalArea_RidesTheExistingAuthoritativePlacementAttachmentPose() {
        ...
        Assert.Equal(
            expected: 0,
            actual: fixture.Server.Population.GravityAreaStatistics.ActiveAreaCount
        );
        ...
        Assert.Equal(
            expected: 1,
            actual: fixture.Server.Population.GravityAreaStatistics.MatchCount
        );
        Assert.Equal(
            expected: FixedQ4816.FromInteger(value: 100),
            actual: fixture.Server.Population.EntryBody(index: 0)!.FixedPosition.X
        );
        Assert.NotEqual(
            expected: FixedQ4816.FromInteger(value: 100),
            actual: fixture.Server.Population.EntryBody(index: 0)!.FixedPosition.Z
        );

        fixture.Server.Population.DeactivateSeat(
            slot: 0,
            tick: 100UL
        );
        fixture.Step();
    ...
    public void LocalAreas_FoldGlobalThenAscendingPriorityAndAuthoredTieOrder() {
        ...
        Assert.Equal(
            ...
            actual: acceleration.Y
        );
        Assert.Equal(
            expected: FixedQ4816.FromInteger(value: 3),
            actual: acceleration.Z
        );
        Assert.Equal(
            expected: [0, 1, 2],
            actual: field.Compiled.Areas.Select(selector: area => area.AuthoredIndex)
        );
    }
    [Fact]
    ...
    public void PointPreset_RequiresPositiveG_WhileUniformOnlyDoesNot() {
        ...
        Assert.Equal(
            expected: FixedQ4816.FromDouble(value: -9.81),
            actual: compiled.Uniform.Y
        );
    }
    [Fact]
    public void PointPreset_RoundTripsAsAuthoredQuantities() {
        ...
        Assert.Equal(
            expected: "ball",
            actual: point.PlacementId
        );
        Assert.Equal(
            expected: 9.81f,
            actual: point.SurfaceGravity
        );
        Assert.Equal(
            expected: 100f,
            actual: point.ReferenceRadius
        );
    }
    [Fact]
    public void PointPreset_UnrepresentableLoweringRefusesBeforeRuntimeCompilation() {
        var denied = WithGravity(gravity: PointGravity(points: [
            new WorldGravityPoint(
                PlacementId: "ball",
                ReferenceRadius: float.MaxValue,
                SurfaceGravity: float.MaxValue
            ),
        ]));
    ...
    public void PositiveGlobalConstant_SolvesBodyOnlySources_AndParticipatesAtZeroWithOneBody() {
        ...
        Assert.True(condition: oneBody.TryAcceleration(
            acceleration: out var loneAcceleration,
            entityIndex: 0
        ));
        Assert.Equal(
            expected: FixedVector3.Zero,
            actual: loneAcceleration
        );

        var twoBodies = CompileField(
            capacity: 2,
            definition: definition
        );
        ...
        Assert.True(condition: twoBodies.TryAcceleration(
            acceleration: out var left,
            entityIndex: 0
        ));
        Assert.True(condition: twoBodies.TryAcceleration(
            acceleration: out var right,
            entityIndex: 1
        ));
        Assert.True(condition: (left.X > FixedQ4816.Zero));
        Assert.Equal(
        ...
        Assert.Equal(
            expected: 2,
            actual: twoBodies.Statistics.BodyCount
        );
    }
    [Fact]
    public void ZeroReplaceSuppressesKitGravity_WhileAnOutsideAreaBodyKeepsFallbackGravity() {
        ...
        Assert.True(condition: outside.Server.ApplySession(request: new SessionRequest.Join(
            actor,
            actor.Index,
            null,
            WorldProtocol.WireProtocolKey
        )).Accepted);
        inside.Server.Population.EntryBody(index: 0)!.Pose(
            pitchRadians: 0f,
            rollRadians: 0f,
            x: 0f,
            y: 100f,
        ...
        outside.Server.Population.EntryBody(index: 0)!.Pose(
            pitchRadians: 0f,
            rollRadians: 0f,
            x: 0f,
            y: 100f,
        ...
        Assert.Equal(
            expected: FixedQ4816.FromInteger(value: 100),
            actual: inside.Server.Population.EntryBody(index: 0)!.FixedPosition.Y
        );
    ...
    public void ZeroReplace_CancellationAndRadialCentreRemainParticipatingZeroAnswers() {
        ...
        var radialCentre = CompileField(
            definition: WithGravity(gravity: PointGravity(
                gravitationalConstant: 0f,
                areas: [Area(
                        mode: WorldGravityAreaMode.Replace,
                        acceleration: new WorldGravityAreaAcceleration.Radial(Magnitude: 8f)
                    )]
            )),
            capacity: 1
        );
