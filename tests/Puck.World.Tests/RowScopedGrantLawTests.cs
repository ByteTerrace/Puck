...
public sealed class RowScopedGrantLawTests {
    ...
    public void RowScopedMutateAdmitsOnlyItsOwnRow_ForeignRowRefused_OwnRowAdmitted() {
        ...
        Laws.RefusalWithControl(
            ...
            deniedOutcome: () => UpsertAndObserveChange(
                fixture: fixture,
                row: HostRow
            ),
            controlOutcome: () => UpsertAndObserveChange(
                fixture: fixture,
                row: SlotRow
            )
        );
    ...
    public void RowScopedVerbMaskIsBoundedByItsOwnSection_ForeignKindRefused_OwnKindHeld() {
        ...
        Laws.RefusalWithControl(
            ...
            deniedOutcome: () => GrantAndObserveHeld(
                fixture: fixture,
                kinds: WorldMutationKindCatalog.KindsOf(section: WorldSection.Kits),
                subject: GrantSubject.Creation(id: HostRow)
            ),
            controlOutcome: () => GrantAndObserveHeld(
                fixture: fixture,
                kinds: WorldMutationKindCatalog.KindsOf(section: WorldSection.Creations),
                subject: GrantSubject.Creation(id: HostRow)
            )
        );
    }
    ...
    public void RowScopedVerbMaskNarrowsTheRow_RemoveMasked_RemoveAdmitted() {
        ...
        Assert.True(
            condition: UpsertAndObserveChange(
                fixture: fixture,
                row: SlotRow
            ),
            userMessage: "the upsert-only mask was expected to admit the slot row's own upsert"
        );

        Laws.RefusalWithControl(
            lawId: "authority.row-scoped-verb-mask-narrows-the-row",
            deniedOutcome: () => RemoveAndObserveChange(
                fixture: fixture,
                row: SlotRow
            ),
            controlOutcome: () => {
