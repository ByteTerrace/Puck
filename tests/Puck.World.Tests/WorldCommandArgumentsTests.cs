...
public sealed class WorldCommandArgumentsTests {
    ...
    public void ADeeperAddressStillFindsItsOptionalKeyword() {
        ...
        Assert.Equal(
            expected: "force|plain",
            actual: registry.Submit(line: "tail.deep row force").Output
        );
        Assert.Equal(
            expected: "w.json|force",
            actual: registry.Submit(line: "tail.deep row w.json force").Output
        );
        Assert.Equal(
            expected: "a tail|force",
            actual: registry.Submit(line: "tail.deep row a tail force").Output
        );
    }
    [Fact]
    public void AQuotedTailReachesTheHandlerWithItsSurroundingQuotesRemoved() {
        ...
        Assert.Equal(
            expected: "{world.tick} ticks",
            actual: registry.Submit(line: "tail.probe \"{world.tick} ticks\"").Output
        );
        // The interior spacing the raw line carries is exactly why this reads Text rather than WireArgs.Tail.
        Assert.Equal(
            expected: "a   b",
            actual: registry.Submit(line: "tail.probe \"a   b\"").Output
        );
        Assert.Equal(
            expected: string.Empty,
            actual: registry.Submit(line: "tail.probe \"\"").Output
        );
    }
    [Fact]
    public void ATailSeparatedByExoticWhitespaceIsFoundJustLikeASpaceSeparatedOne() {
        ...
        Assert.Equal(
            expected: "{world.tick}",
            actual: registry.Submit(line: "tail.probe\v{world.tick}").Output
        );
        Assert.Equal(
            expected: "{world.tick}",
            actual: registry.Submit(line: "tail.probe\f{world.tick}").Output
        );
        Assert.Equal(
            expected: "{world.tick}",
            actual: registry.Submit(line: "tail.probe\t{world.tick}").Output
        );
        Assert.Equal(
            expected: "{world.tick}",
            actual: registry.Submit(line: "tail.probe {world.tick}").Output
    ...
    public void ATailThatIsNotOneQuotedTokenIsReturnedVerbatim() {
        ...
        Assert.Equal(
            expected: "{\"kind\": \"row\"}",
            actual: registry.Submit(line: "tail.probe {\"kind\": \"row\"}").Output
        );
        Assert.Equal(
            expected: "plain tail",
            actual: registry.Submit(line: "tail.probe plain tail").Output
        );
    }
    [Fact]
    public void ATrailingPositionalTokenIsStrippedWhateverWhitespacePrecedesIt() {
        ...
        Assert.Equal(
            expected: "{\"id\":\"hp\"}",
            actual: registry.Submit(line: "tail.between {\"id\":\"hp\"}\v3").Output
        );
        Assert.Equal(
            expected: "{\"id\":\"hp\"}",
            actual: registry.Submit(line: "tail.between {\"id\":\"hp\"}\f3").Output
        );
        Assert.Equal(
            expected: "{\"id\":\"hp\"}",
            actual: registry.Submit(line: "tail.between {\"id\":\"hp\"}\t3").Output
        );
        Assert.Equal(
            expected: "{\"id\":\"hp\"}",
            actual: registry.Submit(line: "tail.between {\"id\":\"hp\"} 3").Output
        ...
        Assert.Equal(
            expected: "{\"id\":\"hp\"}",
            actual: registry.Submit(line: "tail.between {\"id\":\"hp\"}").Output
        );
    }
    [Fact]
    public void ATrailingTokenIsStrippedByTheSameSplitterThatCountedIt() {
        ...
        Assert.Equal(
            expected: "{a}",
            actual: registry.Submit(line: "tail.between \"{a}\" \"x y\"").Output
        );
        Assert.Equal(
            expected: "{a} {b}",
            actual: registry.Submit(line: "tail.between {a} {b} \"x y\"").Output
        );
        // A quoted LEADING token is one token to the parser too, so the address strip must not cut it either.
        Assert.Equal(
            expected: "the rest",
            actual: registry.Submit(line: "tail.after \"an address\" the rest").Output
        );
    }
    [Fact]
    public void AnAbsentTrailingKeywordLeavesTheWholeTailAlone() {
        ...
        Assert.Equal(
            expected: "w.json|plain",
            actual: registry.Submit(line: "tail.keyword w.json").Output
        );
        // A path whose own last token merely CONTAINS the keyword is not the keyword.
        Assert.Equal(
            expected: "my forced.json|plain",
            actual: registry.Submit(line: "tail.keyword my forced.json").Output
        );
        ...
        Assert.Equal(
            expected: "force|plain",
            actual: registry.Submit(line: "tail.keyword force").Output
        );
    }
    [Fact]
    public void AnAddressedTailStripsExactlyTheLeadingTokensItWasToldTo() {
        ...
        Assert.Equal(
            expected: "the rest",
            actual: registry.Submit(line: "tail.after two the rest").Output
        );
        Assert.Equal(
            expected: "quoted rest",
            actual: registry.Submit(line: "tail.after two \"quoted rest\"").Output
        );
        Assert.Equal(
            expected: "the rest",
            actual: registry.Submit(line: "tail.after\vtwo\vthe rest").Output
        );
        // Fewer tokens than the address names carries no tail at all.
        Assert.Equal(
            expected: string.Empty,
            actual: registry.Submit(line: "tail.after two").Output
        );
    }
    [Fact]
    public void AnOptionalTrailingKeywordIsRecognizedWhateverWhitespacePrecedesIt() {
        ...
        Assert.Equal(
            expected: "w.json|force",
            actual: registry.Submit(line: "tail.keyword w.json\vforce").Output
        );
        Assert.Equal(
            expected: "w.json|force",
            actual: registry.Submit(line: "tail.keyword w.json\fforce").Output
        );
        Assert.Equal(
            expected: "w.json|force",
            actual: registry.Submit(line: "tail.keyword w.json\tforce").Output
        );
        Assert.Equal(
            expected: "w.json|force",
            actual: registry.Submit(line: "tail.keyword w.json force").Output
    ...
    public void JsonTailPreservesQuotesAndInteriorWhitespace() {
        ...
        Assert.Equal(
            expected: "\"my   title\"",
            actual: registry.Submit(line: "tail.json /title \"my   title\"").Output
        );
        Assert.Equal(
            expected: "{\"constant\":42}",
            actual: registry.Submit(line: "tail.json /value {\"constant\":42}").Output
        );
    }
