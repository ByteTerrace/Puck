...
public sealed class WorldMarkerValidationLawTests {
    ...
    public void DuplicateIdRefusesByName() {
        ...
        Assert.Contains(
            ...
            expectedSubstring: "markers[1].id 'dup' is duplicated"
        );
    }
    [Fact]
    public void RingOnPointSourceRefusesByName() {
        var definition = Fixtures.BuildDocument() with {
            ...
            MarkersRaw = [
                ValidSpeakerRow() with {
                    Source = new WorldMarkerSource.Point(Position: new Puck.Assets.Documents.DocumentVector3(
                x: 0f,
                y: 0f,
                z: 0f
            )),
        ...
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "markers[0].ring.field 'radius' names a speakers-only field"
        );
    }
    [Fact]
    public void RingPolicyWithoutStyleRingRefusesByName() {
        ...
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "a ring policy without a style.ringColor/ringAlpha pair"
        );
    }
    [Fact]
    public void StyleRingWithoutRingPolicyRefusesByName() {
        ...
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "a style.ringColor/ringAlpha without a ring policy"
        );
    }
    [Fact]
    public void UnrecognizedRingFieldRefusesByName() {
        ...
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "markers[0].ring.field 'size' names no recognized field"
        );
    }
    [Fact]
    public void UnresolvedIconRefusesByName() {
        ...
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "markers[0].icon 'marker.missing' names no row in icons.icons"
        );
    }
    [Fact]
