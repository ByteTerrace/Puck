...
public sealed class ObserverDisclosureLawTests {
    ...
    public void LiveProjectionCadenceChangePreservesSkippedFullFieldImage() {
        ...
        Assert.True(condition: sampler.TryProject(
            projected: out _,
            snapshot: in primer
        ));
        ...
        Assert.False(condition: sampler.TryProject(
            projected: out _,
            snapshot: in full
        ));
        ...
        var latest = primer with {
            Tick = 3UL,
            FieldCells = new[] { new FieldCellDelta(
            Cell: 3,
            Field: 0,
            Raw: 31L
        ) },
        ...
        Assert.Equal(
            new[] { (3, 31L), (4, 40L) },
            projected.FieldCells.Span.ToArray().Select(selector: delta => (delta.Cell, delta.Raw)).ToArray()
        );
    }
    [Fact]
    public void ProjectionSamplerRefusesInvalidCadenceAndNonIncreasingTicks() {
        Assert.Throws<ArgumentOutOfRangeException>(testCode: () => new WorldProjectionSampler(updateSeconds: -0.01f));
        Assert.Throws<ArgumentOutOfRangeException>(testCode: () => new WorldProjectionSampler(updateSeconds: float.NaN));
        Assert.Throws<ArgumentOutOfRangeException>(testCode: () => new WorldProjectionSampler(updateSeconds: (WorldObserverDisclosure.MaximumUpdateSeconds + 0.01f)));
        ...
        Assert.Throws<ArgumentException>(testCode: () => sampler.TryProject(
            projected: out _,
            snapshot: in snapshot
        ));
    }
    ...
    public void RadiusPolicy_FiltersOneSink_WhileTheControlSinkSeesAll() {
        ...
        using var redactedLease = hub.Subscribe(
            sink: redacted,
            disclosure: new WorldSinkDisclosure(
                Policy: new WorldObserverDisclosure(
        ...
        Assert.Equal(
            expected: [0, 1],
            actual: redacted.LastIndices
        );
        Assert.Equal(
            expected: [0, 1, 2],
            actual: control.LastIndices
        );
        // The tick's own facts are untouched — redaction removes rows, it never rewrites the frame.
        Assert.Equal(
            expected: 12UL,
            actual: redacted.LastTick
        );
        Assert.Equal(
            expected: "boot",
            actual: redacted.LastAuthority
        );
    }
    [Fact]
    public void RemoteProjectionCadenceCoalescesFieldsAndRetainsDiscontinuities() {
        ...
        WorldSnapshot Snapshot(ulong tick, EntityContinuity continuity = default, long fieldRaw = 0L) => new(
            ...
            Entries: new[] { Entity(
                index: 7,
                x: tick
            ) with { Continuity = continuity, Generation = 3 } },
            FieldCells: ((fieldRaw == 0L)
            ? []
            : new[] { new FieldCellDelta(
                    Cell: 11,
                    Field: 2,
                    Raw: fieldRaw
                ) }),
        ...
        for (var tick = 2UL; (tick <= 8UL); tick++) {
            var continuity = ((tick == 3UL)
                ? EntityContinuity.Teleport
                : EntityContinuity.Continuous
            );
            var raw = ((tick == 4UL)
                ? 40L
                : ((tick == 6UL)
        ...
        Assert.Equal(
            expected: 9UL,
            actual: projected.Tick
        );
        Assert.Equal(
            expected: EntityContinuity.Teleport,
            actual: Assert.Single(collection: projected.Entries.ToArray()).Continuity
        );
        Assert.Equal(
            expected: 90L,
            actual: Assert.Single(collection: projected.FieldCells.ToArray()).Raw
        );
    }
    [Fact]
    public void SelfOnlyPolicy_DisclosesNothingToAnUnembodiedObserver() {
        ...
        using var unembodiedLease = hub.Subscribe(
            sink: unembodied,
            disclosure: new WorldSinkDisclosure(
                ObserverBodyIndex: -1,
                Policy: policy
        ...
        using var embodiedLease = hub.Subscribe(
            sink: embodied,
            disclosure: new WorldSinkDisclosure(
                ObserverBodyIndex: 1,
                Policy: policy
        ...
        var snapshot = new WorldSnapshot(
            ...
            Revision: 0,
            StepTicks: 1UL,
            Entries: new EntitySnapshot[] { Entity(
                index: 0,
                x: 0f
            ), Entity(
                index: 1,
    ...
    public void UnauthoredWorld_IsDiscloseAll() {
        ...
        Assert.Equal(
            expected: WorldObserverDisclosureMode.All,
            actual: population.ObserverDisclosure.Mode
        );
        Assert.Equal(
            expected: 0.03f,
            actual: population.ObserverDisclosure.UpdateSeconds
        );
        Assert.True(condition: new WorldSinkDisclosure(
            Policy: population.ObserverDisclosure,
            ObserverBodyIndex: -1
        ).IsFull);
    }
    [Fact]
    public void ValidatorRefusesAMisauthoredPolicyByName() {
        ...
        Assert.Contains(
            actualString: missingRadius,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "bodies.disclosure.radius is required"
        );
        ...
        Assert.Contains(
            actualString: strayRadius,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "bodies.disclosure.radius must be absent"
        );
