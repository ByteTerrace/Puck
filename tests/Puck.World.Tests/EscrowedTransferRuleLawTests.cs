...
public sealed class EscrowedTransferRuleLawTests {
    ...
    private static WorldDefinition BuildDocument() {
        ...
        var coins = HolderRow(
            name: "coins",
            nonNegative: true,
            balances: [0, 500, 500, 1_000]
        );
        var rows = new List<WorldStateRow> {
            goods, coins,
            Slot(
            name: "auctionListRequest",
            initial: -1
        ),
            Slot(
            name: "auctionActive",
            ...
            Slot(
            initial: 0,
            name: "auctionEscrowItem",
            nonNegative: true
        ),
            Slot(
            initial: 0,
            name: "auctionEscrowCoin",
            nonNegative: true
        ),
            Slot(
            initial: 0,
            name: "auctionCurrentBid",
            nonNegative: true
        ),
            Slot(
            name: "auctionCurrentBidder",
            initial: -1
        ),
            Slot(
            initial: 0,
            ...
            Slot(
            name: "auctionBidRequest1",
            initial: -1
        ),
            Slot(
            name: "auctionBidRequest2",
            initial: -1
        ),
            new(
            Name: CellName.Parse(candidate: "auctionBidHistory"),
            Kind: CellKind.Int,
        ...
        WorldRule BidRule(int seat, string request, string other) => new(
            ...
            Effects: [
                ...
                AddNegated(
                    state: "coins",
                    key: seat.ToString(provider: System.Globalization.CultureInfo.InvariantCulture),
                    fromState: request
                ),
                Copy(
                    state: "auctionEscrowCoin",
                    fromState: request
                ),
                Copy(
        ...
        var document = Fixtures.BuildDocumentAtRate(rateHz: Fixtures.RecordedTraceRateHz).WithWorldState(rows: rows) with {
            Rules = [
                ...
                Effects: [
                        AddNegated(
                        fromState: "auctionListRequest",
                        key: "0",
                        state: "goods"
                    ),
                        AddFrom(
                        state: "auctionEscrowItem",
                        fromState: "auctionListRequest"
                    ),
                        Set(
                        state: "auctionActive",
                ...
                BidRule(
                other: "2",
                request: "auctionBidRequest1",
                seat: 1
            ),
                BidRule(
                other: "1",
                request: "auctionBidRequest2",
                seat: 2
            ),
                new WorldRule(
                ...
                Effects: [
                        ...
                        State: "goods",
                        Key: $"{RuleFacts.CellKeyPrefix}auctionCurrentBidder:{WorldStateRow.SlotKey}",
                        FromState: "auctionEscrowItem"
                    ),
                        Set(
                        state: "auctionEscrowItem",
                        value: 0
                ...
                Effects: [
                        AddNegated(
                        fromState: "buyoutListRequest",
                        key: "0",
                        state: "goods"
                    ),
                        AddFrom(
                        state: "buyoutEscrowItem",
                        fromState: "buyoutListRequest"
                    ),
                        Set(
                        state: "buyoutActive",
    ...
    public void EnglishAuction_NoBidThenDeadline_ReturnsTheItemToTheSeller() {
        ...
        ); // returned — back to its starting count
        Assert.Equal(
            expected: 0L,
            actual: ReadSlot(
                definition: fixture.Server.Definition,
    ...
    public void EnglishAuction_OutbidThenDeadline_SettlesPayingSellerNetOfFeeAndCreditingTheWinner() {
        ...
        Assert.Equal(
            ...
            actual: ReadSlot(
                definition: fixture.Server.Definition,
                row: "auctionCurrentBidder"
            )
        );
        Assert.Equal(
            expected: 100L,
            actual: ReadHistoryTop(
                definition: fixture.Server.Definition,
                row: "auctionBidHistory"
            )
        );
        ...
        ); // the seller's own remaining unit (unlisted, for the buyout demo)
        Assert.Equal(
            expected: 135L,
            actual: Read(
                definition: fixture.Server.Definition,
