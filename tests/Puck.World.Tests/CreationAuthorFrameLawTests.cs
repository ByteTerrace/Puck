...
public sealed class CreationAuthorFrameLawTests {
    ...
    public void AuthoredPlusZFaceConvertsToTheEngineMinusZSideAndTheSolidFieldAgrees() {
        ...
        var faceRun = new TextRunDocument(
            ...
            Position: new Vector3(
                ...
                y: 1f,
                z: 6f
            ), // the sphere's authored +Z face (center + unit radius)
            Rotation: Quaternion.Identity,
            EmHeight: 0.3f,
            Depth: 0.02f,
        ...
        Assert.True(
            (engineRun.Position.Z < 0f),
            userMessage: $"an authored +Z face run converted to the engine +Z side; z={engineRun.Position.Z:0.###}"
        );
        ...
        Assert.True(condition: field!.Probe(
            distance: out var distance,
            gradient: out _,
            material: out _,
            position: in surfacePoint
        ));
        Assert.True(
            (FixedQ4816.Abs(value: distance) < SurfaceTolerance),
    ...
    public void CapsuleAuthoredScaleCompilesToItsUnitTableRadiusAndLength() {
        ...
        var equatorPoint = new FixedVector3(
            X: FixedQ4816.FromDouble(value: 0.5),
            Y: FixedQ4816.Zero,
            Z: FixedQ4816.Zero
        );
        var topPoint = new FixedVector3(
            X: FixedQ4816.Zero,
            Y: FixedQ4816.FromDouble(value: 1.0),
            Z: FixedQ4816.Zero
        );

        Assert.True(condition: field!.Probe(
            distance: out var equatorDistance,
            gradient: out _,
            material: out _,
            position: in equatorPoint
        ));
        Assert.True(
            (FixedQ4816.Abs(value: equatorDistance) < SurfaceTolerance),
            userMessage: $"scale.x/z=0.5 did not compile to radius 0.5; equator distance={((double)equatorDistance):0.####}"
        );

        Assert.True(condition: field.Probe(
            distance: out var topDistance,
            gradient: out _,
            material: out _,
            position: in topPoint
        ));
        Assert.True(
            (FixedQ4816.Abs(value: topDistance) < SurfaceTolerance),
    ...
    public void DomainLessShapeCanonicalizesWithNoDomainMember() {
        ...
        var omitted = CreationCanonicalizer.Canonicalize(
            document: document,
            source: "domain-less"
        );
        var explicitNull = CreationCanonicalizer.Canonicalize(
            document: (document with { Shapes = [shape with { Domain = null }] }),
            source: "domain-less"
        );
        ...
        Assert.DoesNotContain(
            ...
            actualString: Encoding.UTF8.GetString(bytes: omitted.Bytes),
            comparisonType: StringComparison.Ordinal
        );
        Assert.Equal(
            expected: omitted.Hash,
            actual: explicitNull.Hash
        );
        Assert.Equal(
            expected: omitted.Bytes,
            actual: explicitNull.Bytes
        );
    }
    ...
    public void SymmetryDomainOpMirrorsBothRenderAndTheCompiledSolidField() {
        ...
        var authoredPoint = FixedVector3.FromVector3(value: (enginePosition + new Vector3(
            x: 0f,
            y: Radius,
            z: 0f
        )));
        var reflectedPoint = new FixedVector3(
            X: -authoredPoint.X,
            Y: authoredPoint.Y,
            Z: authoredPoint.Z
        );
        ...
        Assert.True(condition: renderEvaluator.TryDistance(
            position: FixedPosition.FromLocal(local: authoredPoint),
            distance: out var authoredRenderDistance,
            material: out _
        ));
        Assert.True(
            (FixedQ4816.Abs(value: authoredRenderDistance) < SurfaceTolerance),
            userMessage: $"the authored point did not read as solid in the render program; distance={((double)authoredRenderDistance):0.####}"
        );
        Assert.True(condition: renderEvaluator.TryDistance(
            position: FixedPosition.FromLocal(local: reflectedPoint),
            distance: out var reflectedRenderDistance,
            material: out _
        ));
        Assert.True(
            (FixedQ4816.Abs(value: reflectedRenderDistance) < SurfaceTolerance),
        ...
        Assert.True(condition: field!.Probe(
            distance: out var authoredFieldDistance,
            gradient: out _,
            material: out _,
            position: in authoredPoint
        ));
        Assert.True(
            (FixedQ4816.Abs(value: authoredFieldDistance) < SurfaceTolerance),
        ...
        Assert.True(condition: field.Probe(
            distance: out var reflectedFieldDistance,
            gradient: out _,
            material: out _,
            position: in reflectedPoint
        ));
        Assert.True(
            (FixedQ4816.Abs(value: reflectedFieldDistance) < SurfaceTolerance),
