...
public sealed class WorldFrameIsometryLawTests {
    ...
    private static long RawAbs(FixedVector3 a, FixedVector3 b) =>
        Math.Max(RawAbs(a: a.X, b: b.X), Math.Max(RawAbs(a: a.Y, b: b.Y), RawAbs(a: a.Z, b: b.Z)));
    ...
    public void MapArrival_DifferentYawPair_MapsHeadingAndVelocityThroughTheSameIsometry() {
        ...
        var sourceFacing = FixedQuaternion.FromAxisAngle(axis: s_upAxis, angle: travelerYaw).Rotate(vector: s_forward);
        var expectedFacing = WorldFrameIsometry.MapVector(value: sourceFacing, source: in sourceFrame, destination: in destinationFrame);
        ...
        Assert.Equal(expected: (travelerYaw + WorldFrameIsometry.YawDelta(source: in sourceFrame, destination: in destinationFrame)), actual: mapped.YawRadians);
        ...
        var expectedVelocity = WorldFrameIsometry.MapVector(value: velocity, source: in sourceFrame, destination: in destinationFrame);
    ...
    public void YawDelta_SameYawPair_IsExactlyTheHalfTurn() {
        ...
        Assert.True(condition: (RawAbs(a: WorldFrameIsometry.YawDelta(source: in frame, destination: in counterpart), b: s_piRadians) <= YawErrorBudgetRaw));
    }
    ...
    public void YawDelta_OpposedPair_IsTheIdentityControl() {
        ...
        Assert.True(condition: (Math.Abs(WorldFrameIsometry.YawDelta(source: in frame, destination: in counterpart).Value) <= YawErrorBudgetRaw));
    ...
    public void MapArrival_ReciprocalRoundTrip_StaysWithinTheMeasuredErrorBudget() {
        ...
        foreach (var (offset, planarVelocity, verticalVelocity, configLabel) in travelerConfigs) {
            ...
            foreach (var (sourceYawDegrees, destinationYawDegrees, yawLabel) in yawPairsDegrees) {
                ...
                var velocityErrorRaw = Math.Max(RawAbs(a: backward.PlanarVelocity, b: planarVelocity), RawAbs(a: backward.VerticalVelocity, b: verticalVelocity));
                ...
                var expectedFacing = FixedQuaternion.FromAxisAngle(axis: s_upAxis, angle: travelerYaw).Rotate(vector: s_forward);
