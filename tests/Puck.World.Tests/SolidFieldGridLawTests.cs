...
public sealed class SolidFieldGridLawTests {
    ...
    public void TheBakeHashNamesTheBakeAndACollisionEditKeepsTheGrid() {
        ...
        Assert.NotEqual(
            expected: 0UL,
            actual: half.Census.SolidBakeHash
        );
        Assert.Equal(
            expected: half.Census.SolidBakeHash,
            actual: halfAgain.Census.SolidBakeHash
        );
        Assert.NotEqual(
            expected: half.Census.SolidBakeHash,
            actual: whole.Census.SolidBakeHash
        );
        Assert.Equal(
            expected: 0UL,
            actual: none.Census.SolidBakeHash
        );
        ...
        Assert.Same(
            expected: before,
            actual: fixture.Server.SolidField!.Grid
        );
        ...
        Assert.Equal(
            expected: 1.0,
            actual: ((double)fixture.Server.SolidField!.Grid!.CellSize)
        );
    ...
    public void TheCellSizeIsValidatedByName() {
        ...
        Assert.Contains(
            ...
            expectedSubstring: "collision.gridCellSize"
        );
        Assert.False(condition: WorldDefinitionValidator.TryValidateLocally(
            definition: tooLarge,
            reason: out reason
        ...
        Assert.True(
            condition: WorldDefinitionValidator.TryValidateLocally(
                definition: Shipped(gridCellSize: 0f),
                reason: out reason
            ),
            userMessage: reason
        );
    ...
    public void TheShippedWorldBakesAGridWhoseSurfaceIsExactInsideTheBand() {
        ...
        Assert.Equal(
            expected: 0.5,
            actual: ((double)gridded.Grid.CellSize)
        );
        // The walker's capsule radius at the scale row's ceiling of 1, plus the contact skin, plus the grid's slack.
        Assert.True(
            condition: (((double)gridded.ContactBand) > (0.35 + 0.02)),
            userMessage: $"band {gridded.ContactBand}"
        );
        ...
        foreach (var position in Lattice()) {
            Assert.True(condition: exact.Evaluator.TryDistance(
                distance: out var expectedDistance,
                material: out var expectedMaterial,
                position: position
            ));
            Assert.True(condition: gridded.Evaluator.TryDistance(
            ...
            if (expectedDistance < gridded.ContactBand) {
                inBand++;
                Assert.Equal(
                    expected: expectedDistance.Value,
                    actual: actualDistance.Value
                );
                Assert.Equal(
            ...
            Assert.Equal(
                expected: exact.Evaluator.TryFieldGradient(
                    gradient: out var expectedGradient,
                    position: position
                ),
                actual: gridded.Evaluator.TryFieldGradient(
            ...
            Assert.Equal(
                expected: exact.Query.Overlap(
                    center: position,
                    radius: radius
                ),
                actual: gridded.Query.Overlap(
                    center: position,
                    radius: radius
                )
            );
            ...
            foreach (var direction in Directions) {
                casts++;
                Assert.Equal(
                    expected: exact.Query.SphereCast(
                        dir: direction,
                        hit: out var expectedHit,
                        maxDist: maxDistance,
                        origin: position,
                        radius: radius
                    ),
                    actual: gridded.Query.SphereCast(
                        dir: direction,
                        hit: out var actualHit,
                        maxDist: maxDistance,
                        origin: position,
                        radius: radius
                    )
                ...
                Assert.Equal(
                    expected: expectedHit.Confidence,
                    actual: actualHit.Confidence
                );
            }
        }

        Assert.True(
            condition: (inBand > 100),
            userMessage: $"only {inBand} lattice points fell inside the band"
        );
        Assert.True(
            condition: (beyond > 100),
