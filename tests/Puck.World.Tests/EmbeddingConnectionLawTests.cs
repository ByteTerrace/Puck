...
public sealed class EmbeddingConnectionLawTests {
        ...
        return baseDoc with {
            ...
            StateRaw = new WorldStateSection(
                ...
                World: [
                    ...
                    Table(name: "results", kind: resKind, capacity: resCapacity, space: ((resKind == CellKind.Vector && includeSpace) ? spaceName : null)),
    ...
    private static WorldExtensionConfiguration Configuration(
        ...
        for (var i = 0; i < connectionCount; i++) {
            ...
            embeddings.Add(new WorldExtensionEmbeddingSettings(
                Name: $"conn{suffix}",
                Provider: "fixture",
    ...
    private static StateCell? FindCell(WorldServer server, string row, string key) =>
        server.Definition.State.FirstOrDefault(r => (r.Name.Value == row))
            ?.Cells?.FirstOrDefault(c => (c.Key.Value == key));
    ...
    public async Task Law2_RequestToVectorWithStatus3InOneBatch() {
        ...
        Assert.NotNull(resultCell.Vector);
        Assert.Equal(expected: 256, actual: resultCell.Vector.Dimensions);

        Assert.NotNull(statusCell);
        Assert.Equal(expected: (long)WorldExternalOperationStatus.Succeeded, actual: statusCell.Value);
        Assert.Equal(expected: 3L, actual: statusCell.Value);
    }
    ...
    private sealed class CountingEmbeddingSource(IWorldEmbeddingSource inner, StrongBox<int> counter) : IWorldEmbeddingSource {
        ...
        public async Task<IReadOnlyList<EmbeddingAnswer>> EmbedAsync(IReadOnlyList<string> texts, CancellationToken cancellationToken) {
            counter.Value++;
            return await inner.EmbedAsync(texts: texts, cancellationToken: cancellationToken);
        }
    ...
    public async Task Law3_CacheHitMakesZeroProviderCalls() {
        ...
        Assert.Equal(expected: 1, actual: callCount.Value);
        var result1 = FindCell(server: world.Server, row: "results", key: "req-1");
        Assert.NotNull(result1?.Vector);
        ...
        Assert.NotNull(result2?.Vector);
        Assert.Equal(expected: result1.Vector, actual: result2.Vector);
    }
    ...
    private sealed class RefusingEmbeddingSource(EmbeddingIdentity identity, StrongBox<int> callCounter) : IWorldEmbeddingSource {
        ...
        public Task<IReadOnlyList<EmbeddingAnswer>> EmbedAsync(IReadOnlyList<string> texts, CancellationToken cancellationToken) {
            callCounter.Value++;
            var answers = texts.Select(static _ => new EmbeddingAnswer(Vector: null, Refusal: "Moderation refusal")).ToList();
    ...
    public async Task Law5_RefusalSetsStatus4AndWaitsRetryTicks() {
                ...
                new WorldExtensionEmbeddingProviderType(
                    ...
                    Create: _ => new DelegatingConfiguredEmbeddingProvider(
                        bind: _ => new RefusingEmbeddingSource(
                            identity: new EmbeddingIdentity(Model: "puck-fixture", Revision: "1", Dimensions: 256),
        ...
        var statusCell = FindCell(server: world.Server, row: "status", key: "req-1");
        Assert.NotNull(statusCell);
        Assert.Equal(expected: 4L, actual: statusCell.Value);
        Assert.Null(FindCell(server: world.Server, row: "results", key: "req-1"));
        ...
        Assert.Equal(expected: 2, actual: callCount.Value);
    }
    ...
    public async Task Law7_ReplayReproducesVectorsWithoutProviderWithIdenticalStateHash() {
        ...
        var liveHash = WorldRuntimeStateHash.HashAuthoritative(server: liveWorld.Server, tick: 1);
        ...
        var replayedHash = WorldRuntimeStateHash.HashAuthoritative(server: replayedWorld.Server, tick: 1);

        Assert.Equal(expected: liveHash, actual: replayedHash);
    }
