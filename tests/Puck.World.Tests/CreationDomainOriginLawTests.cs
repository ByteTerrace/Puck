...
public sealed class CreationDomainOriginLawTests {
    ...
    private static readonly Vector3 BandShapePosition = new(
        x: 0f,
        y: 9.6f,
        z: 0f
    );
    private static readonly Vector3 BandSpacing = new(
        x: 0f,
        y: 2.1f,
        z: 0f
    );
    private static readonly Vector3 BandLimit = new(
        x: 0f,
        y: 1f,
        z: 0f
    );
    private static readonly Vector3 NearBand = new(
        x: 0f,
        y: 7.5f,
        z: 0f
    );
    private static readonly Vector3 CentreBand = new(
        x: 0f,
        y: 9.6f,
        z: 0f
    );
    private static readonly Vector3 FarBand = new(
        x: 0f,
        y: 11.7f,
        z: 0f
    );
    ...
    public void AFinitePolarOriginIsAccepted() =>
        AssertCanonicalizerAccepts(shape: SphereAt(
            domain: [new ShapeDomainOp.Polar(
                    Count: 4,
                    Origin: new Vector3(
                        x: 1f,
                        y: 2f,
    ...
    public void AFiniteRepeatOriginIsAccepted() =>
        AssertCanonicalizerAccepts(shape: SphereAt(
            domain: [new ShapeDomainOp.Repeat(
                    Spacing: Vector3.One,
                    Origin: new Vector3(
                        x: 1f,
                        y: 2f,
    ...
    public void ANullOriginFoldsAroundTheCreationRootAndCollapsesAnOffCentreLattice() {
        ...
        Assert.True(
            ...
            userMessage: "the far band should still resolve without an origin."
        );
        Assert.False(
            condition: IsInside(
                point: CentreBand,
            ...
            userMessage: "the origin-free fold collapses the centre band."
        );
        Assert.False(
            condition: IsInside(
                point: NearBand,
            ...
            userMessage: "the origin-free fold collapses the near band."
        );
    }
    ...
    public void APolarOriginWidensReachByTwiceItsDistanceWhileARepeatOriginDoesNotChangeIt() {
        ...
        Assert.Equal(
            actual: ShapeDomainOps.Reach(domain: [new ShapeDomainOp.Polar(
                    Count: 4,
                    Origin: pivot
                )]),
        ...
        var repeatWithOrigin = ShapeDomainOps.Reach(domain: [new ShapeDomainOp.Repeat(
                Limit: BandLimit,
                Origin: pivot,
                Spacing: BandSpacing
            )]);
        var repeatWithoutOrigin = ShapeDomainOps.Reach(domain: [new ShapeDomainOp.Repeat(
                Spacing: BandSpacing,
                Limit: BandLimit
            )]);

        Assert.Equal(
            actual: repeatWithOrigin,
            expected: repeatWithoutOrigin,
            tolerance: 1e-5f
        );
    }
    [Fact]
    ...
    public void AnOriginAtTheShapesOwnPositionFoldsTheLatticeAroundItInstead() {
        ...
        Assert.True(
            ...
            userMessage: "the far band did not resolve with the fold centred on the shape's own position."
        );
        Assert.True(
            condition: IsInside(
                point: CentreBand,
            ...
            userMessage: "the centre band did not resolve with the fold centred on the shape's own position."
        );
        Assert.True(
            condition: IsInside(
                point: NearBand,
            ...
            userMessage: "the near band did not resolve with the fold centred on the shape's own position."
        );
    }
    ...
    public void AnOriginBearingPolarSandwichesItsFoldBetweenMatchingTranslates() {
        ...
        Assert.True(
            condition: (polarIndex > 0),
            userMessage: "the program never emits its RepeatPolar instruction."
        );
        Assert.Equal(
            expected: SdfOp.Translate,
            actual: instructions[(polarIndex - 1)].Op
        );
        Assert.Equal(
            expected: SdfOp.Translate,
            actual: instructions[(polarIndex + 1)].Op
