...
public sealed class WorldFieldFlowLawTests {
    ...
    public void ValidatorRefusesASpillRowThatIsNotADeclaredScalarFixedRow() {
        var missing = Fixtures.WithLattice(
            definition: Fixtures.BuildDocument(),
            composite: FlowFields(
                width: 2,
                depth: 2,
                spillRow: "missing"
            )
        );
        var wrongShapeBase = Fixtures.WithLattice(
            definition: Fixtures.BuildDocument(),
            composite: FlowFields(
                width: 2,
                depth: 2,
        ...
        var wrongShape = wrongShapeBase with {
            StateRaw = wrongShapeBase.StateRaw! with {
                World = [.. (wrongShapeBase.StateRaw!.World ?? []), new WorldStateRow(
                Name: CellName.Parse(candidate: "keyed"),
                Kind: CellKind.Fixed,
                Capacity: 4
            )],
            },
    ...
    public void ValidatorRefusesAnUndeclaredOverFieldASelfReferencingOverEntryAndADuplicateOverEntry() {
        ...
        Assert.False(condition: WorldDefinitionValidator.TryValidate(
            definition: selfReferencing,
            neighbours: null,
            reason: out var selfReason
        ));
        Assert.Contains(
            actualString: selfReason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "itself transports"
        );
        ...
        Assert.Contains(
            ...
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "duplicated within over"
        );
    }
}
