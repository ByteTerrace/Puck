...
public sealed class WorldStampPoolBoundLawTests {
    ...
    public void ADilatedAndOnionedShapeBoundScalesWithTheBodyLook() {
        ...
        Assert.True(condition: (required > ((SdfSolidGeometry.Reach(
            type: shape.Type,
            scale: (shape.Scale * BodyScale),
            lift: SdfLift.Extrude
        ) + shape.Dilate.Value) + shape.Onion.Value)));
    }
    ...
    public void AParentedDomainShapeBoundRidesItsOwnSlotAndCoversEveryFoldImage() {
        ...
        var solidReach = SdfSolidGeometry.Reach(
            type: fold.Type,
            scale: fold.Scale
        );
        ...
        var creationReach = CreationStampEmitter.RenderReach(
            document: engine,
            scale: 1f,
            fontFor: null
        );
        ...
        for (var frame = 0; (frame < 120); frame++) {
            ...
            foreach (var image in images) {
                var center = (slot.Position + Vector3.Transform(
                    value: image,
                    rotation: slot.Orientation
                ));
                var fromSlot = Vector3.Distance(
                    value1: center,
                    value2: slot.Position
                );
                ...
                escapedRoot |= ((Vector3.Distance(
                    value1: center,
                    value2: root.Position
                ) + solidReach) > creationReach);
            }
        }

        Assert.True(
            condition: escapedRoot,
            userMessage: "no swing frame carried a fold image past the creation's static reach around the root, so this law cannot tell a travelling bound from a root-anchored one."
        );
    }
