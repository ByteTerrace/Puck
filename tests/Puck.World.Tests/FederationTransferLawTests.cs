...
public sealed partial class FederationTransferLawTests {
    ...
    private static async Task<WireFrameRead> RequireFrameAsync(Stream stream, CancellationToken ct) {
        ...
        return (read.Ok
            ? read
            : throw new Xunit.Sdk.XunitException(userMessage: $"federation peer answered no response frame ({read.Failure})")
        );
    }
    ...
    public void AutonomousTransfer_RefusesAnUnsupportedProducerBeforeCommit() {
        ...
        var request = Reservation(
            ...
            Members = [new WorldTransferReservationMember(
                ...
                PreferredSlot: 4,
                Identity: null,
                Source: IntentSource.Producer(name: "not-declared"),
                BodyColor: default,
                CatalogRig: 4,
        ...
        Assert.Contains(
            expectedSubstring: "declares no parameters for producer 'not-declared'",
            actualString: reservation.Reason,
    ...
    public void AutonomousTransfer_UsesEntityTableWithoutBecomingAHumanPeer() {
        ...
        Assert.Equal(
            expected: IntentSource.Idle,
            actual: fixture.Server.Population.EntryBody(index: bodyIndex)!.Source
        );
        Assert.Equal(
            ...
            actual: fixture.Server.Population.BodyColor(index: bodyIndex)
        );
        Assert.Equal(
            expected: ((byte)73),
            actual: fixture.Server.Population.CatalogRig(index: bodyIndex)
        );
        Assert.True(condition: fixture.Server.Population.TryCaptureTransferredEntity(
            index: bodyIndex,
    ...
    public void CommitStatus_DoesNotFabricateAnEarlierMissingTransferFromALaterCommit() {
        ...
        fixture.Server.AbortTransfer(
            sourceAuthority: SourceAuthority,
            transferId: missing.TransferId
        );
        Assert.Equal(
            WorldTransferStatus.Missing,
            fixture.Server.TransferStatus(
                sourceAuthority: SourceAuthority,
                transferId: missing.TransferId
            )
        );
    ...
    public void CommitStatus_SurvivesALostAcknowledgement_AndOnlyExactCommitReplays() {
        ...
        Assert.Equal(
            ...
            actual: fixture.Server.TransferStatus(
                sourceAuthority: request.SourceAuthority,
                transferId: request.TransferId
            )
        );
    ...
    public void CommitWire_PreservesTheSelectedMotionProgramAndTheCommitTimeProfile() {
        ...
        var expected = new WorldTransferCommitMember(
            ...
            Position: new FixedVector3(
                X: FixedQ4816.One,
                Y: FixedQ4816.FromInteger(value: 8),
                Z: -FixedQ4816.One
            ),
            ...
            Continuum: new WorldContinuumTrajectory(
                PreviousPosition: new FixedVector3(
                    X: FixedQ4816.Zero,
                    Y: FixedQ4816.FromInteger(value: 8),
                    Z: -FixedQ4816.One
                ),
        ...
        var encoded = WorldFederationCodec.EncodeCommit(
            members: [expected],
            sourceAuthority: "source/world",
            transferId: 7
        );
        ...
        Assert.Equal(
            expected: "free",
            actual: member.BodyMotionProgramName
        );
        Assert.Equal(
            expected: profile.Id,
            actual: member.Profile?.Id
        );
        Assert.Equal(
        ...
        Assert.Equal(
            ...
            actual: Assert.Single(collection: member.ActionContinuity.Channels).HeldValue
        );
        Assert.Equal(
            expected: FixedQ4816.One,
            actual: Assert.Single(collection: member.ActionContinuity.Registers).Value
        );
        Assert.Equal(
            expected: expected.Continuum,
            actual: member.Continuum
        );
    }
    [Fact]
    public void CommitWire_RefusesTrailingBytesAndAnOverCountedCohortByName() {
        ...
        Assert.False(condition: WorldFederationCodec.TryDecodeCommit(
            ...
            transferId: out _,
            members: out _,
            failure: out var overCounted
        ));
        Assert.Equal(
            expected: WireRefusal.CountOutOfRange,
            actual: overCounted.Refusal
        );
    }
    [Fact]
    public void Commit_RejectsAnInvalidContinuumBeforeLandingTheTraveler() {
        ...
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "invalid continuum"
        );
    ...
    public void CommittedRetryRequiresEveryActionContinuityFieldAndAcceptsIndependentEqualCopies() {
        ...
        var registers = new[] {
            new WorldTransferActionRegister(
            "uses",
            ActionStateKind.Counter,
            FixedQ4816.One,
            0
        ),
            new WorldTransferActionRegister(
            "cooldown",
            ActionStateKind.Timer,
            FixedQ4816.Zero,
            50400
        ),
        };
    ...
    public void EscrowContinuityCopiesAreDetachedAcrossCheckpointCaptureAndRestore() {
        using var fixture = Fixtures.FreshServer();
        var escrow = new WorldTransferEscrow(server: fixture.Server);
        var request = Reservation(
            border: "east",
            sourceAuthority: "machine-a/boot",
            transferId: 291
        ...
        var carried = Assert.Single(collection: checkpoint.Committed).Members[0].ActionContinuity!;
        ...
        carried = new WorldTransferActionContinuity(
            Channels: channels,
            Registers: registers
        );
        checkpoint.Committed[0].Members[0] = member with { ActionContinuity = carried };
        var restored = new WorldTransferEscrow(server: fixture.Server); restored.Restore(checkpoint: checkpoint);
        channels[0] = new(
            "jump",
            false,
            FixedQ4816.Zero
    ...
    public void FederatedIntentStream_HoldsDeviceStateAcrossFasterDestinationTicks_AndReleasesByLease() {
        ...
        var request = Reservation(
            border: "seam",
            sourceAuthority: "slow-player-world",
            transferId: 43
        ) with {
            PeerAdmission = true,
            Members = [new WorldTransferReservationMember(
        ...
        var heldForward = default(PlayerIntent).WithChannel(
            ordinal: 0,
            value: FixedQ4816.One
        );
        ...
        fixture.Server.PublishFederatedIntent(
            leaseId: 77,
        ...
        Assert.True(
            condition: ((body.FixedPosition - start).Length > FixedQ4816.FromDouble(value: 2.5)),
            userMessage: $"sparse held input only moved {((double)(body.FixedPosition - start).Length):0.###}"
        );
        Assert.True(
            condition: (body.PlanarSpeed > 3.5f),
            userMessage: $"held stream settled at only {body.PlanarSpeed:0.###}"
        );
    ...
    public async Task FederatedIntentStream_RejectsAuthorityRebindingAndUnknownTransferCredentials(bool rebindAuthority) {
        ...
        Assert.Equal(
            expected: ((byte)WorldFederationResponse.Ack),
            actual: (await RequireFrameAsync(
                ct: testToken,
                stream: stream
            )).Kind
        ...
        var submission = new IntentSubmission(
            Tick: 1,
            EntityIndex: 0,
            Intent: default(PlayerIntent).WithChannel(
                ordinal: 0,
                value: FixedQ4816.One
            ),
            Principal: Principal.Console
        );
        var carriedAuthority = (rebindAuthority
            ? "forged-world/source"
            : SourceAuthority
        );
        ...
        var body = WorldFederationCodec.EncodeIntent(
            ...
            sourceAuthority: carriedAuthority,
            submission: in submission
        );
        ...
        var expectedRefusal = (rebindAuthority
            ? WorldFederationRefusal.SourceAuthorityMismatch
            : WorldFederationRefusal.CredentialUnknown
        );
        ...
        Assert.StartsWith(
            expectedStartString: expectedRefusal.ToString(),
    ...
    public void FederatedNeutralDeviceState_DoesNotMaskAnAuthorityAcceptedTapeSegment() {
        ...
        var body = fixture.Server.Population.EntryBody(index: bodyIndex)!;
        var start = body.FixedPosition;
        var run = default(PlayerIntent).WithChannel(
            ordinal: 0,
            value: FixedQ4816.One
        );

        fixture.Server.ApplyCommand(command: new WorldCommand.EnqueueSegment(
            EntityIndex: bodyIndex,
            Intent: run,
            Principal: principal,
            Seconds: 1f
        ...
        Assert.True(
            condition: ((body.FixedPosition - start).Length > FixedQ4816.FromDouble(value: 0.75)),
            userMessage: $"the held neutral device image masked the accepted tape segment: delta={((double)(body.FixedPosition - start).Length):0.###}, speed={body.PlanarSpeed:0.###}"
        );
    }
    ...
    public async Task FederationDoor_AcceptsAnAuthenticatorWhoseChallengeAndProofAreNotThirtyTwoBytes() {
        ...
        Assert.Equal(
            expected: ((byte)WorldFederationResponse.Challenge),
            actual: challenge.Kind
        );
        Assert.Equal(
            expected: OddWidthAuthenticator.ChallengeWidth,
            actual: challenge.Body.Length
        );
        ...
        var accepted = await RequireFrameAsync(
            ct: testToken,
            stream: stream
        );
    ...
    public async Task FederationDoor_NeverAdmitsAVerifiedProofUnderAnyNamespaceOtherThanItsOwnVerifiedSubject() {
        ...
        var proof = oracleY.Sign(
            challenge: challenge.Body.Span,
        ...
        await WorldFederationCodec.WriteRequestAsync(
            ...
            body: WorldFederationCodec.EncodeTransferKey(
                sourceAuthority: "authority-x",
                transferId: 1
            ),
            ct: testToken
        );
        var refusedAsX = await RequireFrameAsync(
            ct: testToken,
            stream: stream
        );
        ...
        Assert.Equal(
            expected: ((byte)WorldFederationResponse.Refusal),
            actual: refusedAsX.Kind
        );
        Assert.StartsWith(
            expectedStartString: nameof(WorldFederationRefusal.SourceAuthorityMismatch),
            actualString: Encoding.UTF8.GetString(bytes: refusedAsX.Body.Span),
            comparisonType: StringComparison.Ordinal
        );
        ...
        await WorldFederationCodec.WriteRequestAsync(
            ...
            kind: WorldFederationRequest.Status,
            body: WorldFederationCodec.EncodeTransferKey(
                sourceAuthority: "authority-y",
                transferId: 1
            ),
            ct: testToken
        );
        var admittedAsY = await RequireFrameAsync(
            ct: testToken,
            stream: stream
        );

        Assert.Equal(
            expected: ((byte)WorldFederationResponse.Status),
            actual: admittedAsY.Kind
        );
    }
    ...
    public async Task FederationDoor_RefusesAReplayedProofAgainstAFreshChallenge() {
        ...
        using (var first = new PeerTestClient()) {
            ...
            await WorldFederationCodec.WriteRequestAsync(
                stream: stream,
                kind: WorldFederationRequest.Authenticate,
                body: WorldFederationCodec.EncodeAuthentication(proof: capturedProof),
                ct: testToken
            );
            Assert.Equal(
                expected: ((byte)WorldFederationResponse.Authenticated),
        ...
        using (var replay = new PeerTestClient()) {
            ...
            Assert.StartsWith(
                expectedStartString: nameof(WorldFederationRefusal.AuthenticationFailed),
                actualString: Encoding.UTF8.GetString(bytes: refusal.Body.Span),
                comparisonType: StringComparison.Ordinal
            );
        }
    ...
    public async Task FederationDoor_RejectsBadProof_AndAuthorityRebinding() {
        ...
        using (var attacker = new PeerTestClient()) {
            ...
            await WorldFederationCodec.WriteRequestAsync(
                stream: stream,
                kind: WorldFederationRequest.Authenticate,
                body: WorldFederationCodec.EncodeAuthentication(proof: new byte[64]),
                ct: testToken
            );
            var refusal = await RequireFrameAsync(
                ct: testToken,
        ...
        using (var authenticated = new PeerTestClient()) {
            await authenticated.ConnectAsync(
                address: endpoint.Address,
                port: endpoint.Port,
                cancellationToken: testToken
            );
            ...
            var status = await RequireFrameAsync(
                ct: testToken,
                stream: stream
            );
            ...
            Assert.Equal(
                expected: ((byte)WorldTransferStatus.Missing),
                actual: Assert.Single(collection: status.Body.ToArray())
            );
        ...
        Assert.Contains(
            collection: host.FederationRefusals,
            filter: row => ((row.Refusal == WorldFederationRefusal.AuthenticationFailed) && (row.Count == 1))
        );
        Assert.Contains(
            collection: host.FederationRefusals,
            filter: row => ((row.Refusal == WorldFederationRefusal.SourceAuthorityMismatch) && (row.Count == 1))
        );
    }
    [Fact]
    public void LostAcknowledgementOutcomes_AreSupersededPerMobilityIdentity() {
        ...
        var commit = new WorldTransferCommitMember(
            Profile: null,
            HasMappedArrival: false,
            BodyMotionProgramName: "grounded",
            Position: default,
        ...
        for (ulong crossing = 60; (crossing < 80); crossing++) {
            ...
            Assert.Equal(
                1,
                fixture.Server.TransferTableCounts.ActiveTransactions
            );
            if (crossing > 60) {
                Assert.Equal(
                    WorldTransferStatus.Missing,
                    fixture.Server.TransferStatus(
                        sourceAuthority: SourceAuthority,
                        transferId: (crossing - 1UL)
                    )
                );
            }

            mobility = mobility.Advance();
            Assert.True(condition: fixture.Server.Population.TryDetachSeatForTransfer(
                profile: out _,
                slot: 0
            ));
        ...
        Assert.Equal(
            0,
            fixture.Server.TransferTableCounts.MobilityCredentials
        );
    }
    [Fact]
    public void MappedCommit_RefusesAMotionProgramTheDestinationDoesNotDeclare() {
        ...
        var member = new WorldTransferCommitMember(
            Profile: null,
            HasMappedArrival: true,
            BodyMotionProgramName: "not-declared",
            Position: default,
            YawRadians: default,
            PlanarVelocity: default,
            VerticalVelocity: default
        ...
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "unavailable destination motion program 'not-declared'"
        );
    ...
    public void MobilityCredential_ChurnIsBounded_AndGenerationBound() {
        ...
        for (ulong crossing = 1; (crossing <= 100); crossing++) {
            ...
            Assert.False(condition: fixture.Server.TryTransferredPrincipal(
                mobility: in forgedFuture,
                principal: out _,
                sourceAuthority: SourceAuthority
            ));
            ...
            Assert.Equal(
                0,
                fixture.Server.TransferTableCounts.MobilityLeases
            );
            Assert.True(condition: fixture.Server.Population.TryDetachSeatForTransfer(
    ...
    public void MobilityEpochLease_AllowsOnlyOneReservationAndReleasesOnAbort() {
        ...
        Assert.True(
            condition: firstReply.Accepted,
            userMessage: firstReply.Reason
        );
        Assert.False(condition: competingReply.Accepted);
        Assert.Contains(
            "already leased",
            competingReply.Reason,
    ...
    /// response's default kind into the answer turns the first answer's kind red. Every deadline on both sides —
    /// the host's handshake and drain, the lane's per-attempt bound, the caller's answer deadline and the connect
    /// retry — runs on one <see cref="VirtualClock"/>, so none can elapse under load; the law advances exactly the one
    /// connect retry the refused proof schedules.</summary>
    [Fact]
    public async Task RemoteAuthority_ClosesTheSigningGateOnAnAuthenticatorThatVerifiesButCannotProve() {
        var testToken = TestContext.Current.CancellationToken;
        ...
        using var remote = new WorldRemoteAuthority(
            endpoint: host.ListenEndpoint!,
            network: network,
            placeholder: fixture.Server.Definition,
            security: verifyOnly,
            observerAuthority: "machine-b/boot"
        );
        var body = WorldFederationCodec.EncodeTransferKey(
            sourceAuthority: "machine-b/boot",
            transferId: 17
        );
        // AwaitAnswer blocks its caller for the answer, as the tick thread does, so it runs off the law's thread
        // while the law drives the clock.
        var pending = Task.Run(
            cancellationToken: testToken,
            function: () => remote.AwaitAnswer(
                body: body,
                kind: WorldFederationRequest.Status,
                sourceAuthority: "machine-b/boot"
            )
        ...
        await clock.ExpireAsync(
            ct: testToken,
            dueTime: WorldRemoteAuthority.ConnectRetryDelay,
            pending: pending
        );

        var first = await pending.WaitAsync(cancellationToken: testToken);

        Assert.False(condition: first.Ok);
        Assert.Equal(
            expected: WorldFederationResponse.Refusal,
            actual: first.Kind
        );
        Assert.Equal(
        ...
        Assert.Contains(
            expectedSubstring: "this run holds no federation signing identity",
            actualString: first.Failure.Detail,
            comparisonType: StringComparison.Ordinal
        );

        var second = remote.AwaitAnswer(
            body: body,
            kind: WorldFederationRequest.Status,
            sourceAuthority: "machine-b/boot"
        );
        ...
        Assert.Equal(
            expected: "this run holds no federation signing identity",
            actual: second.Failure.Detail
        );
    }
    [Fact]
    public void ReservationIdentity_IsSourceScoped_AndOnlyExactReplayIsIdempotent() {
        using var fixture = Fixtures.FreshServer();
        var sourceA = Reservation(
            border: "east",
            sourceAuthority: "machine-a/boot",
            transferId: 17
        );
        ...
        var otherSourceRequest = Reservation(
            border: "east",
            sourceAuthority: "machine-b/boot",
            transferId: 17
        ) with {
        ...
        Assert.Equal(
            expected: first.BodyIndices,
            actual: exactReplay.BodyIndices
        );
        Assert.False(condition: conflictingReplay.Accepted);
        Assert.Contains(
            expectedSubstring: "different reservation",
            actualString: conflictingReplay.Reason,
            comparisonType: StringComparison.Ordinal
        );
        Assert.True(condition: otherSource.Accepted);
        Assert.NotEqual(
            expected: first.BodyIndices[0],
            actual: otherSource.BodyIndices[0]
        );
        Assert.Equal(
            expected: WorldTransferStatus.Reserved,
            actual: fixture.Server.TransferStatus(
                sourceAuthority: sourceA.SourceAuthority,
                transferId: sourceA.TransferId
            )
        );
        Assert.Equal(
            expected: WorldTransferStatus.Reserved,
    ...
    public void ReservationRefusesAnOutOfCatalogTravelerRigByName() {
        ...
        Assert.Contains(
            expectedSubstring: "catalog rig 128 is outside 0..127",
            actualString: reply.Reason,
            comparisonType: StringComparison.Ordinal
        );
    }
    [InlineData(false)]
    ...
    public void ReservationWire_PreservesAnonymousAutonomousIntentSource(bool producer) {
        using var fixture = Fixtures.FreshServer();
        var source = (producer
            ? IntentSource.Producer(name: "roam")
            : IntentSource.Idle
        );
        ...
        Assert.Equal(
            expected: color,
            actual: member.BodyColor
        );
        Assert.Equal(
            expected: ((byte)73),
            actual: member.CatalogRig
        );
    ...
    public void SnapshotWire_PreservesOccupantRigIndependentlyOfAuthoritySlot() {
        ...
        Assert.True(
            condition: WorldFederationCodec.TryDecodeSnapshot(
                body: WorldFederationCodec.EncodeSnapshot(snapshot: in snapshot),
    ...
    public void SnapshotWire_RefusesATruncatedRecordByName() {
        ...
        Assert.True(
            condition: WorldFederationCodec.TryDecodeSnapshot(
                body: encoded,
                failure: out var control,
                snapshot: out var whole
            ),
            userMessage: control.ToString()
        );
        Assert.Equal(
            expected: WireRefusal.None,
            actual: control.Refusal
        );
        Assert.Single(collection: whole.Entries.ToArray());

        Assert.False(condition: WorldFederationCodec.TryDecodeSnapshot(
            body: encoded.AsSpan(
                start: 0,
    ...
    public void SnapshotWire_RefusesAnEntryCountPastThePopulationCeilingByName() {
        ...
        Assert.Contains(
            expectedSubstring: "snapshot entry count",
            actualString: failure.Detail,
            comparisonType: StringComparison.Ordinal
        );
    }
