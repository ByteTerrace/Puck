...
public sealed class SculptPatchLawTests {
    ...
    public void RemoveMemberOnKeyedRowIsAModificationNotARowRemoval() {
        ...
        Assert.Equal(
            expected: "looks.rows",
            actual: touched[0].Section
        );
        Assert.Equal(
            expected: "moth",
            actual: touched[0].KeyValue
        );
    ...
    public void RemoveRowDropsOnlyTheKeyedRow() {
        ...
        Assert.Equal(
            expected: "b",
            actual: kits[0]!["name"]!.GetValue<string>()
        );
    }
    ...
    public void RemoveRowOnAbsentKeyIsUnset() {
        ...
        Assert.Equal(
            expected: 2,
            actual: document["kits"]!.AsArray().Count
        );
    }
    ...
    public void SetMemberThroughSelectorDescendsByKeyNotIndex() {
        ...
        Assert.Equal(
            expected: "name",
            actual: results[0].KeyField
        );
        Assert.Equal(
            expected: "moth",
            actual: results[0].KeyValue
        );
        ...
        Assert.Equal(
            expected: "y",
            actual: moth["motion"]!["poses"]!["blink"]!.GetValue<string>()
        );
    ...
    public void TouchedRowsDeduplicatesAndPreservesFirstTouchedOrder() {
        ...
        var patch = new SculptPatch()
            ...
            keyField: "name",
            keyValue: "a",
            row: JsonNode.Parse(json: """{"name":"a","value":5}""")!
        )
            .SetMember(
            path: "render.sky",
            value: JsonNode.Parse(json: "{}")
    ...
    public void UpsertRowIntoAbsentSectionCreatesObjectIntermediates() {
        ...
        var results = new SculptPatch()
            .UpsertRow(
            sectionPath: "state.world",
            keyField: "name",
            keyValue: "a",
            row: JsonNode.Parse(json: """{"name":"a"}""")!
        )
            .UpsertRow(
            sectionPath: "state.world",
            keyField: "name",
            keyValue: "b",
            row: JsonNode.Parse(json: """{"name":"b"}""")!
        )
            .Apply(document: document);
        ...
        Assert.Equal(
            expected: "inserted",
            actual: results[1].Verdict
        );
        Assert.IsType<JsonObject>(@object: document["state"]);
        Assert.Equal(
            expected: 2,
            actual: document["state"]!["world"]!.AsArray().Count
        );
        Assert.Equal(
            expected: """{"state":{"world":[{"name":"a"},{"name":"b"}]}}""",
            actual: document.ToJsonString()
        );
    }
    ...
    public void UpsertRowReplacesInPlacePreservingOrderAndSiblings() {
        ...
        Assert.Equal(
            expected: 99,
            actual: kits[0]!["value"]!.GetValue<int>()
        );
        Assert.Equal(
            expected: "b",
            actual: kits[1]!["name"]!.GetValue<string>()
        );
        Assert.Equal(
            expected: 2,
            actual: kits[1]!["value"]!.GetValue<int>()
        );
    }
    ...
    public void UpsertRowWithNewKeyAppends() {
        ...
        Assert.Equal(
            expected: "c",
            actual: kits[2]!["name"]!.GetValue<string>()
        );
