...
public sealed class FieldScopedMutationLawTests {
    ...
    public void PopulationSpawnAndCensusQueuedTogetherBothLand() {
        ...
        var distribution = new WorldDistribution(
            Region: new WorldDistributionRegion.Disc(
                Radius: 9f,
                SampleCount: 16
            ),
            Fill: before.Distribution.Fill
        );
        IReadOnlyList<SeatActivationPolicy> activation = [.. Enumerable.Range(
                count: WorldBodiesLimits.LocalSeatCount,
                start: 0
            ).Select(selector: static slot => ((slot < 1)
        ...
        var disc = Assert.IsType<WorldDistributionRegion.Disc>(@object: after.Distribution.Region);
        ...
        Assert.Equal(
            activation,
            after.SeatActivation
        );
        Assert.Equal(
            0,
            after.NetworkPlayers
        );
    }
    [Fact]
    public void SeatRigAndSeatControlQueuedTogetherBothLand() {
        ...
        var rig = views.SeatRig with {
            Operations = [.. views.SeatRig.Operations.Select(selector: static op => ((op is WorldCameraProgramOp.Orbit orbit)
            ? (orbit with { Distance = 9f })
            : op))],
        };
