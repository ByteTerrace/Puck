...
public sealed class CreationStateRigLawTests {
    ...
    private static float Phase(WorldDefinition world, CreationDocument creation, int bodyIndex, ulong tick) {
        ...
        var address = new WorldEntityAddress(
            Authority: "a",
            Generation: 1,
            Index: bodyIndex
        );
    ...
    public void AStateGateTokenHoldsOnTheStoredTruthNotTheEasedSample() {
        ...
        Assert.True(condition: WorldGaitDrivers.GateHolds(
    ...
    public void TheWorldValidatorRefusesAGateTokenOrPoseNamingNoNumericRow_ControlDeclaredClean() {
        ...
        Assert.Contains(
            expectedSubstring: "poses['wink'] names no frame",
            actualString: Refusal(definition: WithPoses(
                world: world,
                look: look,
                poses: new Dictionary<string, string> { ["wink"] = $"state.{AirRow}.$body" }
            ))
        ...
        Assert.Contains(
            expectedSubstring: "must be a state.<row>[.<key>] reference",
            actualString: Refusal(definition: WithPoses(
                world: world,
                look: look,
                poses: new Dictionary<string, string> { ["wink"] = "always" }
            ))
        );
    }
}
