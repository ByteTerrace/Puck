...
public sealed class HudFrameElementValidationLawTests {
    ...
    private static WorldDefinition WithFrameSources(IReadOnlyList<WorldFrameSource> sources) => Fixtures.BuildDocument() with {
        ...
        Panels: [
                ...
                Elements: sources.Select(selector: (source, index) => FrameElement(
                    source: source,
                    id: $"feed-{index}"
                )).ToArray()
            ),
    ...
    public void AFrameElementRoundTripsThroughTheHudElementAccessor() {
        ...
        Assert.Equal(
            expected: WorldHudElementKind.Frame,
            actual: roundTripped.Kind
        );
        Assert.Equal(
            expected: WorldHudFrameFit.Contain,
            actual: roundTripped.Fit
        );
        Assert.True(condition: roundTripped.Mirror);
        Assert.Equal(
            expected: 12f,
            actual: roundTripped.Radius
        );
        Assert.Equal(
            expected: 0.75f,
    ...
    public void ANegativeFrameRadiusRefusesWhileAZeroRadiusPasses() {
        Laws.RefusalWithControl(
            ...
            deniedOutcome: () => WorldDefinitionValidator.TryValidateLocally(
                definition: WithPanelElement(element: FrameElement(
                    source: CameraSource(),
                    radius: -1f
                )),
                reason: out _
    ...
    public void ANinthDistinctFrameSourceRefusesWhileEightPass() {
        static WorldFrameSource[] Sources(int count) => Enumerable.Range(
            count: count,
            start: 0
        )
        ...
        Assert.Contains(
            ...
            expectedSubstring: "hud.TooManyFrameSources"
        );
    }
    [Fact]
