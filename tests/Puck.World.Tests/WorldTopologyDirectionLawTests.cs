...
public sealed class WorldTopologyDirectionLawTests {
    ...
    public void AnUnauthoredGridStillCompilesTheEightCompassNames() {
        var topology = TopologyCompilation.Find(new WorldStateSection(Lattices: [Grid()]), "grid")!;
    ...
    public void AFourConnectedGridReplacesTheDefaultCompassVocabularyWholesale() {
        ...
        var topology = TopologyCompilation.Find(new WorldStateSection(Lattices: [Grid() with { Directions = orthogonal }]), "grid")!;
    ...
    public void ARenamedBoxVocabularyStillDerivesACorrectClosedOppositeTable() {
        var renamed = new TopologyDirection[] { new("up", 0, 0, 1), new("down", 0, 0, -1) };
        var topology = TopologyCompilation.Find(new WorldStateSection(Lattices: [Box() with { Directions = renamed }]), "box")!;
    ...
    public void AnUnclosedOrDuplicateOrMisplacedDirectionListRefuses() {
        var missingOpposite = new TopologyDirection[] { new("east", 1, 0) };
        Assert.False(TopologyCompilation.TryValidate(Grid() with { Directions = missingOpposite }, out var noOppositeReason));
        Assert.Contains("opposite", noOppositeReason);

        var duplicateName = new TopologyDirection[] { new("east", 1, 0), new("east", -1, 0) };
        Assert.False(TopologyCompilation.TryValidate(Grid() with { Directions = duplicateName }, out var duplicateReason));
        Assert.Contains("distinct", duplicateReason);

        var zeroStep = new TopologyDirection[] { new("east", 1, 0), new("nowhere", 0, 0) };
        Assert.False(TopologyCompilation.TryValidate(Grid() with { Directions = zeroStep }, out var zeroReason));
        Assert.Contains("zero step", zeroReason);

        var layerOnAGrid = new TopologyDirection[] { new("up", 0, 0, 1), new("down", 0, 0, -1) };
        Assert.False(TopologyCompilation.TryValidate(Grid() with { Directions = layerOnAGrid }, out var layerReason));
        ...
        var closed = new TopologyDirection[] { new("east", 1, 0), new("west", -1, 0) };
        Assert.True(TopologyCompilation.TryValidate(Grid() with { Directions = closed }, out _));
    }
    ...
    public void ARuleCompilesAgainstAnAuthoredDirectionAndRefusesTheRetiredDefaultName() {
        ...
        var board = new WorldStateRow(CellName.Parse("board"), CellKind.Int, Domain: new StateDomain.CellsOf("grid"));
        var slot = new WorldStateRow(CellName.Parse("neighbour"), CellKind.Int, Cells: [new StateCell(WorldStateRow.SlotKey, 0L)]);
    ...
    public void ARingRefusesAYStepAndAWrappedAxisRefusesAStepAtOrPastItsExtent() {
        var ring = new LatticeTopology.Ring("ring", new DocumentVector3(0, 0, 0), 1, 5);
        Assert.False(TopologyCompilation.TryValidate(ring with { Directions = [new("skew", 1, 1)] }, out var ringReason));
        Assert.Contains("no second axis", ringReason);
        Assert.False(TopologyCompilation.TryValidate(ring with { Directions = [new("around", 5, 0), new("back", -5, 0)] }, out var wideReason));
        Assert.Contains("magnitude must be under the width", wideReason);
        // Control: a step under the ring's own width is admitted.
        Assert.True(TopologyCompilation.TryValidate(ring with { Directions = [new("step", 4, 0), new("back", -4, 0)] }, out _));

        var wrappedGrid = Grid() with { Wrap = TopologyWrap.Both };
        Assert.False(TopologyCompilation.TryValidate(wrappedGrid with { Directions = [new("far", 0, 4), new("near", 0, -4)] }, out var deepReason));
    ...
    public void APhysicalFieldRefusesAnAuthoredDirectionVocabulary() {
        var field = new WorldFieldTopology("field", new DocumentVector3(0, 0, 0), 1, 4, 4);
    ...
    public void AnAuthoredDirectionListRoundTripsThroughJson() {
        var orthogonal = new TopologyDirection[] { new("east", 1, 0), new("west", -1, 0) };
        ...
        var topology = TopologyCompilation.Find(restored.StateRaw, "grid")!;
        Assert.Equal(2, topology.DirectionCount);
        Assert.Equal(-1, topology.Direction("N"));
