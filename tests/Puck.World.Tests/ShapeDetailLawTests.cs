...
public sealed class ShapeDetailLawTests {
    ...
    private static readonly ShapePanelDocument SomePanel = new(
        Inset: 0.05f,
        Depth: 0.1f,
        Material: 1
    );
    private static readonly ShapeTrimDocument SomeTrim = new(
        Inset: 0.003f,
        Material: 1,
        Shape: "cutter",
        Width: 0.05f
    ...
    public void ContactCopiesAreIdenticalWithAndWithoutADetailSibling() {
        ...
        Assert.Equal(
            actual: withRivetCopies[0].Center,
            expected: bareCopies[0].Center
        );
        Assert.Equal(
            actual: withRivetCopies[0].HalfExtents,
            expected: bareCopies[0].HalfExtents
        );
    }
    [Fact]
    public void EmitStaticPacksTheDetailFlagOnTheShapeInstruction() {
        var detailed = EmitStatic(shape: Shape(
            SdfSolidPrimitive.Sphere,
            PlateScale,
            detail: true
        ));
