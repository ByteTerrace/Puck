...
public sealed class BindingBarAuthoringValidationLawTests {
    public static IEnumerable<object[]> Cases() {
        ...
        yield return ["layouts['only'].banks['resting'].anchor.inset", Policy(layout: layout with { Banks = Banks(resting: new WorldBindingBarBankPlacement(Pieces: [], Anchor: new WorldBindingBarAnchor(Edge: BindingBarEdge.Left, Inset: float.NaN))) }), Policy(layout: layout with { Banks = Banks(resting: new WorldBindingBarBankPlacement(Pieces: [], Anchor: new WorldBindingBarAnchor(Edge: BindingBarEdge.Left, Inset: 0f))) })];
        yield return ["layouts['only'].tables['cross'][0].badge needs exactly", Policy(layout: Tabled(layout, new WorldBindingBarSlotPlacement(Badge: [1f], Source: InputSources.Gamepad.DpadUp, X: 0f, Y: 0f))), Policy(layout: Tabled(layout, new WorldBindingBarSlotPlacement(Badge: [1f, 0f], Source: InputSources.Gamepad.DpadUp, X: 0f, Y: 0f)))];
        yield return ["layouts['only'].tables['cross'][0].badge [1.5, 0]", Policy(layout: Tabled(layout, new WorldBindingBarSlotPlacement(Badge: [1.5f, 0f], Source: InputSources.Gamepad.DpadUp, X: 0f, Y: 0f))), Policy(layout: Tabled(layout, new WorldBindingBarSlotPlacement(Badge: [1f, 0f], Source: InputSources.Gamepad.DpadUp, X: 0f, Y: 0f)))];
        ...
        yield return ["layouts['only'].tables['cross'][1].source 'gamepad.dpadUp' is duplicated", Policy(layout: Tabled(layout, new WorldBindingBarSlotPlacement(Source: InputSources.Gamepad.DpadUp, X: 0f, Y: 0f), new WorldBindingBarSlotPlacement(Source: InputSources.Gamepad.DpadUp, X: 1f, Y: 0f))), Policy(layout: Tabled(layout, new WorldBindingBarSlotPlacement(Source: InputSources.Gamepad.DpadUp, X: 0f, Y: 0f)))];
        ...
        yield return ["layouts['only'].banks['resting'].pieces[0].at needs finite", Policy(layout: Tabled(layout) with { Banks = Banks(resting: new WorldBindingBarBankPlacement(Pieces: [new WorldBindingBarPiece(Table: "cross", At: [1f])])) }), Policy(layout: Tabled(layout) with { Banks = Banks(resting: new WorldBindingBarBankPlacement(Pieces: [new WorldBindingBarPiece(Table: "cross", At: [1f, 2f])])) })];
        yield return ["layouts['only'].banks['resting'].pieces[0].badge needs", Policy(layout: Tabled(layout) with { Banks = Banks(resting: new WorldBindingBarBankPlacement(Pieces: [new WorldBindingBarPiece(Table: "cross", Badge: [2f, 0f])])) }), Policy(layout: Tabled(layout) with { Banks = Banks(resting: new WorldBindingBarBankPlacement(Pieces: [new WorldBindingBarPiece(Table: "cross", Badge: [1f, 0f])])) })];
        ...
        yield return ["banks declares 6 entries", Policy(layout: layout) with { Banks = [.. Enumerable.Range(count: 6, start: 0).Select(selector: static index => (OneBank[0] with { Id = $"bank{index}" }))] }, Policy(layout: layout)];
    ...
    private static WorldDefinition WithPolicy(WorldBindingBarAuthoring policy) => Fixtures.BuildDocument() with {
        StateRaw = new WorldStateSection(World: [
            new WorldStateRow(Name: CellName.Parse(candidate: "actionIcons"), Kind: CellKind.Text, Capacity: 8, Cells: [new StateCell(Key: CellName.Parse(candidate: "jump"), Text: "known.icon")]),
            new WorldStateRow(Name: CellName.Parse(candidate: "numericIcons"), Kind: CellKind.Int, Capacity: 8),
            new WorldStateRow(Name: CellName.Parse(candidate: "scalarIcons"), Kind: CellKind.Text, Cells: [new StateCell(Key: WorldStateRow.SlotKey, Text: "known.icon")]),
        ]),
        BindingOverlaysRaw = [
