...
public sealed class BindingBarAuthoringValidationLawTests {
    ...
    private static WorldDefinition WithPolicy(WorldBindingBarAuthoring policy) => Fixtures.BuildDocument() with {
        StateRaw = new WorldStateSection(World: [
            new WorldStateRow(
            Name: CellName.Parse(candidate: "actionIcons"),
            Kind: CellKind.Text,
            Capacity: 8,
            Cells: [new StateCell(
            ...
            new WorldStateRow(
            Name: CellName.Parse(candidate: "numericIcons"),
            Kind: CellKind.Int,
            Capacity: 8
        ),
            new WorldStateRow(
            Name: CellName.Parse(candidate: "scalarIcons"),
            Kind: CellKind.Text,
            Cells: [new StateCell(
    ...
    public static IEnumerable<object[]> Cases() {
        ...
        yield return ["visible.windowSeconds", Policy(layout: layout) with { Visible = new OverlayPredicate.Recently(
                Fact: OverlayFact.SeatInput,
                WindowSeconds: -1f
            ) }, Policy(layout: layout) with { Visible = new OverlayPredicate.Recently(
                Fact: OverlayFact.SeatInput,
                WindowSeconds: 3f
            ) }];
        ...
        yield return ["visible.predicates[0].windowSeconds", Policy(layout: layout) with { Visible = new OverlayPredicate.Any(Predicates: [new OverlayPredicate.Recently(
                    Fact: OverlayFact.SeatInput,
                    WindowSeconds: float.NaN
                )]) }, Policy(layout: layout) with { Visible = new OverlayPredicate.Any(Predicates: [new OverlayPredicate.Now(Fact: OverlayFact.WheelOpen)]) }];
        ...
        yield return ["layouts['only'].banks['resting'].anchor.inset", Policy(layout: layout with { Banks = Banks(resting: new WorldBindingBarBankPlacement(
                Pieces: [],
                Anchor: new WorldBindingBarAnchor(
                    Edge: BindingBarEdge.Left,
                    Inset: float.NaN
                )
            )) }), Policy(layout: layout with { Banks = Banks(resting: new WorldBindingBarBankPlacement(
                Pieces: [],
                Anchor: new WorldBindingBarAnchor(
                    Edge: BindingBarEdge.Left,
                    Inset: 0f
        ...
        yield return ["layouts['only'].tables['cross'][0].badge needs exactly", Policy(layout: Tabled(
                layout,
                new WorldBindingBarSlotPlacement(
                    Badge: [1f],
                    Source: InputSources.Gamepad.DpadUp,
                    X: 0f,
                    Y: 0f
                )
            )), Policy(layout: Tabled(
                layout,
                new WorldBindingBarSlotPlacement(
                    Badge: [1f, 0f],
                    Source: InputSources.Gamepad.DpadUp,
        ...
        yield return ["layouts['only'].tables['cross'][0].badge [1.5, 0]", Policy(layout: Tabled(
                layout,
                new WorldBindingBarSlotPlacement(
                    Badge: [1.5f, 0f],
                    Source: InputSources.Gamepad.DpadUp,
        ...
        yield return ["layouts['only'].tables['cross'][0].source 'mouse.button1' is not in", Policy(layout: Tabled(
                layout,
                new WorldBindingBarSlotPlacement(
                    Source: InputSources.Mouse.Button(number: 1),
                    X: 0f,
                    Y: 0f
                )
            )), Policy(layout: Tabled(
        ...
        yield return ["layouts['only'].tables['cross'][1].source 'gamepad.dpadUp' is duplicated", Policy(layout: Tabled(
                layout,
                new WorldBindingBarSlotPlacement(
                    Source: InputSources.Gamepad.DpadUp,
                    X: 0f,
        ...
        yield return ["layouts['only'].tables['cross'][0] needs finite x and y", Policy(layout: Tabled(
                layout,
                new WorldBindingBarSlotPlacement(
                    Source: InputSources.Gamepad.DpadUp,
                    X: float.NaN,
        ...
        yield return ["layouts['only'].banks['ghost'] names no bank", Policy(layout: layout with { Banks = new Dictionary<string, WorldBindingBarBankPlacement>(comparer: StringComparer.Ordinal) { ["ghost"] = new WorldBindingBarBankPlacement(Pieces: []) } }), Policy(layout: layout with { Banks = Banks(resting: new WorldBindingBarBankPlacement(Pieces: [])) })];
        yield return ["layouts['only'].banks['resting'].pieces is required", Policy(layout: layout with { Banks = Banks(resting: new WorldBindingBarBankPlacement(Pieces: null!)) }), Policy(layout: layout with { Banks = Banks(resting: new WorldBindingBarBankPlacement(Pieces: [])) })];
        yield return ["layouts['only'].banks['resting'].pieces[0].table 'ghost' names no entry", Policy(layout: Tabled(layout) with { Banks = Banks(resting: new WorldBindingBarBankPlacement(Pieces: [new WorldBindingBarPiece(Table: "ghost")])) }), Policy(layout: Tabled(layout) with { Banks = Banks(resting: new WorldBindingBarBankPlacement(Pieces: [new WorldBindingBarPiece(Table: "cross")])) })];
        yield return ["layouts['only'].banks['resting'].pieces[0].at needs finite", Policy(layout: Tabled(layout) with { Banks = Banks(resting: new WorldBindingBarBankPlacement(Pieces: [new WorldBindingBarPiece(
                    Table: "cross",
                    At: [1f]
                )])) }), Policy(layout: Tabled(layout) with { Banks = Banks(resting: new WorldBindingBarBankPlacement(Pieces: [new WorldBindingBarPiece(
                    Table: "cross",
                    At: [1f, 2f]
                )])) })];
        yield return ["layouts['only'].banks['resting'].pieces[0].badge needs", Policy(layout: Tabled(layout) with { Banks = Banks(resting: new WorldBindingBarBankPlacement(Pieces: [new WorldBindingBarPiece(
                    Table: "cross",
                    Badge: [2f, 0f]
                )])) }), Policy(layout: Tabled(layout) with { Banks = Banks(resting: new WorldBindingBarBankPlacement(Pieces: [new WorldBindingBarPiece(
                    Table: "cross",
        ...
        yield return ["banks declares 6 entries", Policy(layout: layout) with { Banks = [.. Enumerable.Range(
                    count: 6,
                    start: 0
                ).Select(selector: static index => (OneBank[0] with { Id = $"bank{index}" }))] }, Policy(layout: layout)];
    ...
    public void InvalidValueRefusesByNameBesidePassingControl(string field, WorldBindingBarAuthoring invalid, WorldBindingBarAuthoring control) {
        ...
        Assert.Contains(
            ...
            expectedSubstring: $"bindingOverlays[0].bindingBar.{field}"
        );
        Assert.True(
    ...
    public void OmittedLayoutOverridesResolveToTheirDefaults() {
        ...
        Assert.Equal(
            expected: WorldBindingBarLayout.DefaultButtonSize,
            actual: bare.ResolvedButtonSize
        );
        Assert.Equal(
            expected: WorldBindingBarLayout.DefaultModifierHalfRatio,
            actual: bare.ResolvedModifierHalfRatio
        );
        Assert.Equal(
            expected: WorldBindingBarLayout.DefaultHintBaseGapRatio,
            actual: bare.ResolvedHintBaseGapRatio
        );
        Assert.Same(
            expected: WorldBindingBarLayout.DefaultAnchor,
            actual: bare.ResolvedAnchor
        );
        ...
        Assert.Equal(
            expected: 0.5f,
            actual: authored.ResolvedButtonSize
        );
        Assert.Equal(
            expected: WorldBindingBarLayout.DefaultGlyphSizeRatio,
            actual: authored.ResolvedGlyphSizeRatio
        );
    }

    private static readonly WorldBindingBarBank[] OneBank = [new WorldBindingBarBank(
            Id: "resting",
            PageId: "base",
            Alpha: 1f
        )];
