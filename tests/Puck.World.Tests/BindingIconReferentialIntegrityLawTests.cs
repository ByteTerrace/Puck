...
public sealed class BindingIconReferentialIntegrityLawTests {
    ...
    private static void AssertDoorRefusesTypo(string door, Func<string, BindingProfileDocument> document) {
        ...
        Assert.Contains(
            ...
            expectedSubstring: door
        );
        Assert.Contains(
            actualString: deniedReason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: $"icon '{TypoIcon}' names no row in icons.icons"
        );
        Assert.True(
    ...
    private static WorldDefinition WithBindingDocument(BindingProfileDocument document) => Fixtures.BuildDocument() with {
        StateRaw = new WorldStateSection(World: [
            new WorldStateRow(
            Name: CellName.Parse(candidate: "presentation"),
            Kind: CellKind.Text,
            Capacity: 8,
            Cells: [new StateCell(
            ...
            new WorldStateRow(
            Name: CellName.Parse(candidate: "numericPresentation"),
            Kind: CellKind.Int,
            Capacity: 8
        ),
        ]),
        IconsRaw = new WorldIconographySection(IconsRaw: [new WorldIconRow(
            Name: KnownIcon,
            Glyph: new WorldIconGlyphRef(
                Font: WorldIconFontCatalog.JetBrainsMonoRegular,
                Glyph: "U+2191"
            )
        )]),
    ...
    public void WheelPresentationRowsRefuseUnknownWrongKindAndMissingSectorIdentity() {
        var unknown = WithBindingDocument(document: WheelPresentationDocument(
            iconRow: null,
            labelRow: "state.noSuch",
            sectorId: "jump"
        ));
        var numeric = WithBindingDocument(document: WheelPresentationDocument(
            iconRow: "state.numericPresentation",
            labelRow: null,
            sectorId: "jump"
        ));
        var missingId = WithBindingDocument(document: WheelPresentationDocument(
            iconRow: null,
            labelRow: "state.presentation",
            sectorId: null
        ));
        var admitted = WithBindingDocument(document: WheelPresentationDocument(
            iconRow: "state.presentation",
            labelRow: "state.presentation",
            sectorId: "jump"
        ));
        ...
        Assert.Contains(
            ...
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "wheels row 0.labelRow 'state.noSuch' names no declared state row"
        );
        Assert.False(condition: WorldDefinitionValidator.TryValidate(
            definition: numeric,
            neighbours: null,
            reason: out var numericReason
        ));
        Assert.Contains(
            actualString: numericReason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "wheels row 0.iconRow 'state.numericPresentation' names a Int row"
        );
        Assert.False(condition: WorldDefinitionValidator.TryValidate(
            definition: missingId,
            neighbours: null,
            reason: out var missingIdReason
        ));
        Assert.Contains(
            actualString: missingIdReason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "wheels row 0.rings[0].entries[0].id is required"
        );
        Assert.True(
            condition: WorldDefinitionValidator.TryValidate(
                definition: admitted,
