...
public sealed class ParkedGrantReleaseLawTests {
    ...
    public void AResumedGenerationRemintsItsAdmissionRowsThroughThePeerAdmittedEvent() {
        ...
        Assert.False(
            ...
            userMessage: "a re-admission of the parked generation must unpark the retained body — live and on a replay re-drive alike"
        );
        Assert.Contains(
    ...
    public void ExclusiveSubjectHeldByAPeerIsAcquirableImmediatelyAfterItDisconnects() {
        ...
        Assert.False(
            ...
            userMessage: "the peer's exclusive reservation must beat the rival's seeded Control/all wildcard"
        );
        ...
        Assert.False(
            ...
            userMessage: "an exclusive acquisition over a live peer's reserved subject must be refused"
        );
        ...
        Assert.True(
            ...
            userMessage: "the disconnected peer's reservation must not outlive its connection"
        );
        Assert.True(condition: Controls(
            fixture: fixture,
            principal: rival
    ...
    public void LocalSeatParkAndResumeKeepTheirGrantRows() {
        ...
        Assert.True(
            condition: fixture.Server.Population.IsSeatParked(slot: 1),
            userMessage: "a positive grace must park the seat"
        );
        Assert.True(
            condition: Holds(
                capability: WorldCapability.Control,
            ...
            userMessage: "a parked seat keeps every row it held — it is the one participant a re-join can resume onto"
        );
        Assert.True(condition: Holds(
            fixture: fixture,
            principal: seat,
        ...
        Assert.True(
            ...
            userMessage: "the seeded Drive row is what authorizes the resuming re-join, so losing it would break resume outright"
        );
    ...
    public void PeerDisconnectParksTheBodyItReleasedTheAuthorityOf() {
        ...
        Assert.True(
            condition: fixture.Server.Population.IsHumanOccupied(bodyIndex: PeerBodyIndex),
            userMessage: "a parked body stays human-occupied through its grace window"
        );
        Assert.Equal(
            actual: fixture.Server.Population.ParkedRemainingTicks(
                index: PeerBodyIndex,
    ...
    public void RateZeroWorldHoldsNoRowsForADisconnectedPeerGeneration() {
        using var fixture = Fixtures.FreshServer(definition: WithGrace(
            rateHz: 0,
            seconds: 3.0f
        ));
        ...
        Assert.True(condition: fixture.Server.Population.IsParked(index: PeerBodyIndex));
        Assert.Null(@object: fixture.Server.Population.ParkedRemainingTicks(
            index: PeerBodyIndex,
            tick: fixture.Server.NextInputTick
        ));
    }
