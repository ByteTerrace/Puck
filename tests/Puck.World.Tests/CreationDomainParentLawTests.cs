...
public sealed class CreationDomainParentLawTests {
    ...
    private static readonly Vector3 SwingPivot = new(
        x: 0f,
        y: 1f,
        z: 0f
    );
    ...
    internal static CreationDocument FollowRig() => Rig(
        ...
        parts: [new CreationPartDocument(
                Id: "fold",
                ShapeId: 1
            ), new CreationPartDocument(
                Id: "torso",
                ShapeId: 0
            )],
        torso: Torso(swings: [new ShapeSwingDocument(
                Amplitude: 1f,
                Axis: Vector3.UnitZ,
                Driver: "sway",
                Pivot: SwingPivot
    ...
    private static int AssertFoldChain(SdfInstruction[] program, float scale, Vector3 engineRestPosition) {
        ...
        Assert.True(
            condition: (transformIndex >= 0),
            userMessage: "the fold shape never rides its own dynamic-transform slot."
        );
        Assert.True(
            condition: ((transformIndex + 4) < program.Length),
            userMessage: "the fold shape's own slot is not followed by its scale, fold, and rest pose."
        );
        Assert.Equal(
            expected: SdfOp.Scale,
            actual: program[(transformIndex + 1)].Op
        );
        AssertClose(
            actual: new Vector3(
                x: program[(transformIndex + 1)].Data0.X,
                y: program[(transformIndex + 1)].Data0.Y,
                z: program[(transformIndex + 1)].Data0.Z
            ),
            expected: new Vector3(value: scale),
            what: "the Scale op after the fold's slot is not the placement scale"
        );
        Assert.Equal(
            expected: SdfOp.SymmetryPlane,
            actual: program[(transformIndex + 2)].Op
        );
        Assert.Equal(
            expected: SdfOp.Translate,
            actual: program[(transformIndex + 3)].Op
        );
        AssertClose(
            actual: new Vector3(
                x: program[(transformIndex + 3)].Data0.X,
                y: program[(transformIndex + 3)].Data0.Y,
                z: program[(transformIndex + 3)].Data0.Z
            ),
        ...
        Assert.Equal(
            expected: SdfOp.Rotate,
            actual: program[(transformIndex + 4)].Op
        );

        return transformIndex;
    }
    private static void AssertSameOrientation(Quaternion expected, Quaternion actual, string what) => Assert.True(
        condition: (MathF.Abs(x: Quaternion.Dot(
            quaternion1: expected,
            quaternion2: actual
        )) > (1f - Tolerance)),
        userMessage: $"{what}: expected {expected}, read {actual}."
    );
    ...
    public void DomainShapeFollowsTheParentDeltaInPlacementUnitsAtLookScaleTwo() {
        ...
        Assert.True(
            condition: (Vector3.Distance(
                value1: torso.Position,
                value2: Vector3.Zero
            ) > 0.1f),
            userMessage: $"the parent's swing never displaced the torso at scale {Scale} (torso at {torso.Position})."
        );
        AssertClose(
            actual: fold.Position,
            expected: torso.Position,
            what: $"the fold's slot position is not the parent's delta frame at scale {Scale} — the delta was not scaled"
        );
        AssertSameOrientation(
            actual: (fold.Orientation * Yaw180),
            expected: torso.Orientation,
            what: $"the fold's slot orientation is not the parent's delta frame at scale {Scale}"
        );

        var program = EmitProgram(
            creation: creation,
            scale: Scale
        ...
        Assert.True(
            condition: (shapeIndex > transformIndex),
            userMessage: "the fold's chain never reaches its primitive."
        );
        ...
        Assert.True(
            condition: (primitiveScaleIndex > (transformIndex + 1)),
            userMessage: "the fold's primitive carries no scale of its own after the chain's placement Scale op."
        );
        Assert.True(
    ...
    public void DomainShapeParentedToASwingingShapeRidesExactlyTheParentDelta() {
        ...
        AssertClose(
            ...
            what: "an unparented domain shape's slot is not the root position — the identity-delta contract does not hold"
        );
        AssertSameOrientation(
            actual: anchor.Orientation,
            expected: Quaternion.Identity,
            what: "an unparented domain shape's orientation is not the root orientation — the identity-delta contract does not hold"
        );
        ...
        Assert.True(
            condition: ((Vector3.Distance(
                value1: torso.Position,
                value2: Vector3.Zero
            ) > 0.05f) && (MathF.Abs(x: Quaternion.Dot(
                quaternion1: torso.Orientation,
                quaternion2: Yaw180
            )) < 0.99f)),
        ...
        AssertClose(
            actual: fold.Position,
            expected: torso.Position,
            what: "the fold's slot position is not the parent's delta frame"
        );
        AssertSameOrientation(
            actual: (fold.Orientation * Yaw180),
            expected: torso.Orientation,
            what: "the fold's slot orientation is not the parent's delta frame"
        );
    ...
    public void PackedAndAuthoredPartPosesAgreeOnADomainPart() {
        ...
        foreach (var part in new[] { "fold", "torso" }) {
            Assert.True(
                condition: pool.TryBodyPartPose(
                    bodyIndex: 0,
                    partId: part,
                    pose: out var packed,
                    transforms: transforms
                ),
                userMessage: $"part '{part}' resolves no packed pose."
            );
            Assert.True(
                condition: pool.TryBodyPartAuthoredPose(
                    bodyIndex: 0,
                    client: client,
                    partId: part,
                    pose: out var authored
                ),
                userMessage: $"part '{part}' resolves no authored pose."
            );

            AssertClose(
                actual: packed.Position,
                expected: authored.Position,
                what: $"part '{part}': the packed and authored part poses disagree on position"
            );
            AssertSameOrientation(
                actual: packed.Orientation,
                expected: authored.Orientation,
                what: $"part '{part}': the packed and authored part poses disagree on orientation"
            );
        }

        // The fold's part pose is its REST pose carried by the parent, not the bare delta frame the slot holds.
        Assert.True(condition: pool.TryBodyPartPose(
            bodyIndex: 0,
            partId: "fold",
            pose: out var foldPose,
            transforms: transforms
        ));
        Assert.True(
            condition: (Vector3.Distance(
                value1: foldPose.Position,
                value2: transforms[FoldSlot].Position
            ) > 0.5f),
