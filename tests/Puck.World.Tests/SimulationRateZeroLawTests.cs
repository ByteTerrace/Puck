...
public sealed class SimulationRateZeroLawTests {
    ...
    private static WorldDefinition BuildWithRateGraceAndPeerCapacity(int rateHz, float reconnectGraceSeconds) {
        var document = BuildWithRateAndGrace(
            rateHz: rateHz,
    ...
    public void ApplyPeerDisconnected_AtPositiveRate_BehaviorUnchanged() {
        var document = BuildWithRateGraceAndPeerCapacity(
            rateHz: 240,
        ...
        Assert.True(
            condition: population.TryAdmitRemotePeer(
            ...
            userMessage: $"peer admission was expected to succeed; refused: {refusal}"
        );
        ...
        Assert.Equal(
            expected: ((long?)720L),
            actual: population.ParkedRemainingTicks(
                index: admitted.BodyIndex,
                tick: 0UL
            )
        );
    ...
    public void ApplyPeerDisconnected_AtRateZero_ParksForever() {
        ...
        Assert.True(condition: population.IsParked(index: admitted.BodyIndex));
        Assert.Null(@object: population.ParkedRemainingTicks(
            index: admitted.BodyIndex,
            tick: 100UL
        ));
    ...
    public void CompiledDuration_AtPositiveRate_MatchesDurationTicks() {
        var compiled = WorldSimulationTickConversion.CompiledDuration(
            ratePerSecond: 240U,
            seconds: 3.0f
        );
        ...
        Assert.Equal(
            expected: checked((int)WorldSimulationTickConversion.DurationTicks(
                ratePerSecond: 240U,
                seconds: 3.0f
            )),
            actual: compiled.Ticks
        );
        Assert.Equal(
            expected: 720,
            actual: compiled.Ticks
    ...
    public void CompiledDuration_AtRateZero_PositiveSecondsIsNever_ZeroSecondsIsAuthoredDisabled() {
        var never = WorldSimulationTickConversion.CompiledDuration(
            ratePerSecond: 0U,
            seconds: 3.0f
        );
        var disabled = WorldSimulationTickConversion.CompiledDuration(
            ratePerSecond: 0U,
            seconds: 0f
        );
    ...
    public void DeactivateSeat_AtPositiveRate_BehaviorUnchanged() {
        ...
        Assert.True(
            condition: population.IsSeatParked(slot: 0),
            userMessage: "the grace window has not elapsed yet"
        );

        population.ReclaimExpiredParks(tick: 720UL);
        Assert.False(
            condition: population.IsActive(index: 0),
            userMessage: "the grace window elapsed; the body must be torn down"
        );
    ...
    public void DeactivateSeat_AtRateZero_ParksForever() {
        ...
        Assert.True(
            condition: population.IsActive(index: 0),
            userMessage: "a parked seat must stay Active (still IsHumanOccupied)"
        );
        Assert.True(
            condition: population.IsSeatParked(slot: 0),
            userMessage: "a rate-0 disconnect must park, never tear down immediately"
        );
        Assert.Null(@object: population.ParkedRemainingTicks(
            index: 0,
            tick: 100UL
    ...
    public void ParkedForever_RevivedAtPositiveRate_ReclaimsAndAdmitsReplacement() {
        var zeroRateDocument = BuildWithRateGraceAndPeerCapacity(
            rateHz: 0,
            reconnectGraceSeconds: 3.0f
        );
        ...
        Assert.True(
            condition: population.IsActive(index: admitted.BodyIndex),
            userMessage: "the revival sweep must not tear the entry down on the same tick it gets a fresh deadline"
        );
        Assert.True(condition: population.IsParked(index: admitted.BodyIndex));
        Assert.Equal(
            expected: ((long?)720L),
        ...
        Assert.True(
            condition: population.IsParked(index: admitted.BodyIndex),
            userMessage: "the re-derived grace window has not elapsed yet"
        );
        ...
        Assert.True(
            condition: population.TryAdmitRemotePeer(
                ...
                identityDomain: string.Empty,
                identitySubject: string.Empty,
                admitted: out var replacement,
                refusal: out var replacementRefusal
            ),
            userMessage: $"a replacement peer was expected to be admitted into the reclaimed slot; refused: {replacementRefusal}"
        );
        Assert.Equal(
            expected: admitted.BodyIndex,
            actual: replacement.BodyIndex
        );
    }
    [Fact]
    ...
    public void PopulationReconnectGraceTicks_AtRateZero_ReflectsNeverAndDisabled() {
        var neverDocument = BuildWithRateAndGrace(
            rateHz: 0,
            reconnectGraceSeconds: 3.0f
        );
        var disabledDocument = BuildWithRateAndGrace(
            rateHz: 0,
            reconnectGraceSeconds: 0f
        );
    ...
    public void PositiveRateDocument_CeilingSecondsOverflowsCompiledRange_Refuses() {
        ...
        Assert.Equal(
            expected: 240,
            actual: document.SimulationRateHz
        );
        ...
        Assert.False(
            ...
            userMessage: "an uncompilable ceilingSeconds was expected to refuse"
        );
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "ceilingSeconds"
        );
    }
    ...
    public void PositiveRateDocument_SubRepresentableLowerAfterSeconds_Refuses() {
        ...
        Assert.Equal(
            expected: 0,
            actual: document.InputHold.Compile(ratePerSecond: ((uint)document.SimulationRateHz)).LowerAfterTicks
        );
        ...
        Assert.False(
            ...
            userMessage: "a sub-representable positive lowerAfterSeconds was expected to refuse at 240 Hz"
        );
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "lowerAfterSeconds"
        );
    }
    ...
    public void RateZeroDocument_NonFiniteCeilingSeconds_Refuses() {
        ...
        Assert.False(
            ...
            userMessage: "a NaN ceilingSeconds was expected to refuse even at rate 0"
        );
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "finite"
        );
    }
    ...
    public void RateZeroDocument_SubRepresentableLowerAfterSeconds_Refuses() {
        ...
        Assert.False(
            ...
            userMessage: "a sub-representable positive lowerAfterSeconds was expected to refuse even at rate 0"
        );
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
    ...
    public void RateZeroDocument_WithOrdinaryInputHold_StillValidates() {
        ...
        Assert.True(
            ...
            userMessage: $"a rate-0 world authoring an ordinary inputHold section was expected to validate; refused: {reason}"
        );
    }
    [Fact]
    public void SecondsFromTicks_RefusesAtRateZero() {
        Assert.Throws<InvalidOperationException>(testCode: () => WorldSimulationTickConversion.SecondsFromTicks(
            ratePerSecond: 0U,
            ticks: 720
        ));
    ...
    public void SecondsFromTicks_WorksAtPositiveRate() {
        Assert.Equal(
            expected: 3.0f,
            actual: WorldSimulationTickConversion.SecondsFromTicks(
                ratePerSecond: 240U,
                ticks: 720
            )
        );
    ...
    public void SetInputHold_RawTicks_AtRateZero_RefusesThroughAdministrativeDrain_NoException() {
        ...
        Assert.True(
            ...
            userMessage: "a raw-ticks SetInputHold at rate 0 was expected to be refused (document left unchanged), not silently applied"
        );
    }
    ...
    public void UnauthoredSimulation_RunsAtTheShippedRate_NotResidentZero() {
        ...
        Assert.Equal(
            expected: WorldDefinition.UnauthoredSimulationRateHz,
            actual: unauthored.SimulationRateHz
        );
        Assert.NotEqual(
            expected: 0,
            actual: unauthored.SimulationRateHz
        ...
        Assert.True(
            condition: WorldDefinitionValidator.TryValidate(
                definition: unauthored,
                neighbours: null,
                reason: out var reason
            ),
            userMessage: $"an unauthored simulation section was expected to validate; refused: {reason}"
        );
    ...
    public void ValidatorAdmitsRateZero_RefusesNegativeRate() {
        ...
        Assert.True(
            condition: WorldDefinitionValidator.TryValidate(
                definition: zeroRate,
                neighbours: null,
                reason: out var zeroReason
            ),
            userMessage: $"rate 0 was expected to validate; refused: {zeroReason}"
        );
        Assert.False(
            condition: WorldDefinitionValidator.TryValidate(
                definition: negativeRate,
                neighbours: null,
                reason: out var negativeReason
            ),
            userMessage: "a negative rate was expected to refuse"
        );
        Assert.Contains(
            actualString: negativeReason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "rateHz"
        );
    }
    ...
    public void ValidatorAtPositiveRate_RefusesNonDivisorRate() {
        ...
        Assert.False(
            condition: WorldDefinitionValidator.TryValidate(
                definition: nonDivisor,
                neighbours: null,
                reason: out var reason
            ),
            userMessage: "241 Hz does not divide 50400 and was expected to refuse"
        );
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
