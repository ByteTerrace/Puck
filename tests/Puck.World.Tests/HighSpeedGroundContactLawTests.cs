...
public sealed class HighSpeedGroundContactLawTests {
    ...
    public void AdjacencyContinuationSweepsDestinationTerrainBeforeAnotherAuthorityStep(int distance) {
        ...
        var trajectory = new WorldContinuumTrajectory(
            PreviousPosition: new FixedVector3(
                X: FixedQ4816.Zero,
                Y: FixedQ4816.FromInteger(value: distance),
                Z: FixedQ4816.Zero
            ),
            SourceTick: 17,
        ...
        Assert.True(
            (body.FixedPosition.Y >= FixedQ4816.Zero),
    ...
    public void AnisotropicallyScaledFloorHasNoContactBeyondItsRenderedEdge() {
        ...
        var collider = FixedWorldCollider.Compile(
            collider: definition.Kits[0].Collider,
        ...
        Assert.True(
            (distance > FixedQ4816.FromDouble(value: 10.5)),
    ...
    public void DynamicDepenetrationCannotPushAStandingBodyThroughTheFloor() {
        ...
        Assert.True(
            (standing.Position.Y >= 0f),
            userMessage: $"dynamic contacts pushed the standing body through the floor; y={standing.Position.Y:0.###}"
        );
    }
    [InlineData(-40f)]
    [InlineData(-8f)]
    ...
    public void FieldContactExtractsAnEdgePenetrationTowardTheApproachedTop(float verticalVelocity) {
        ...
        Assert.True(
            (position.Y > FixedQ4816.Zero),
            userMessage: $"edge penetration extracted below the floor: {((double)position.Y):0.###}"
        );
        Assert.True(
            resolution.Grounded,
            userMessage: "edge penetration did not settle as an approached top contact"
        );
    }
    [InlineData(30U)]
    [InlineData(60U)]
    ...
    public void TerminalFallLandsOnThinAuthoredFloor(uint rateHz) {
        ...
        for (var phase = 0; (phase < 24); phase++) {
            AssertLanding(
                rateHz: rateHz,
                startY: (8f + ((terminalStep * phase) / 24f)),
                stepTicks: stepTicks,
                x: 0f,
                z: 0f
            );
        ...
        static void AssertLanding(uint rateHz, ulong stepTicks, float startY, float x, float z) {
            ...
            Assert.True(
                body.Grounded,
                userMessage: $"body was not grounded after a four-second fall at {rateHz} Hz from ({x:0.###}, {startY:0.###}, {z:0.###}); y={body.Position.Y:0.###}"
            );
            Assert.True(
                (body.Position.Y >= 0f),
                userMessage: $"body escaped the floor at {rateHz} Hz from ({x:0.###}, {startY:0.###}, {z:0.###}); y={body.Position.Y:0.###}"
            );
        }
    }
