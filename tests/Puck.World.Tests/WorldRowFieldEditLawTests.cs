...
public sealed class WorldRowFieldEditLawTests {
    ...
    public void Add_AppendsByDefaultAndInsertsAfterASelector() {
        ...
        Assert.False(
            condition: appendResult.IsError,
            userMessage: appendResult.Output
        );
        ...
        Assert.True(
            condition: (palette.Count == 3),
            userMessage: diag
        );
        Assert.Equal(
            expected: "#112233",
            actual: palette[2].Color
        );
        ...
        Assert.False(
            condition: afterResult.IsError,
            userMessage: afterResult.Output
        );
        ...
        Assert.True(
            condition: (shapes.Count == 3),
            userMessage: diag
        );
        Assert.Equal(
            expected: SdfSolidPrimitive.Sphere,
            actual: shapes[1].Type
        );
        Assert.Equal(
            expected: "torso",
            actual: shapes[0].Name?.Value
        );
        Assert.Equal(
            expected: "forearmL",
            actual: shapes[2].Name?.Value
        );
    }
    [Fact]
    public void BoundRotation_SurvivesEachStageOfTheComposePath() {
        ...
        var copy = System.Text.Json.JsonSerializer.Deserialize(
            utf8Json: bytes,
            jsonTypeInfo: WorldJsonContext.Default.WorldPrototype
        )!;

        Assert.Equal(
            expected: "state.armRot",
            actual: copy.Document.Shapes![1].Rotation.Reference
        );
        Assert.True(
        ...
        Assert.Equal(
            expected: "state.armRot",
            actual: canonical.Document.Shapes![1].Rotation.Reference
        );
        ...
        Assert.True(
            condition: WorldStateDocumentValues.TryRehydrate(
                definition: candidate,
                reason: out var rehydrateReason,
                refreshed: out var refreshed
            ),
            userMessage: rehydrateReason
        );
        Assert.Equal(
            expected: "state.armRot",
            actual: refreshed.Creations.Single().Document.Shapes![1].Rotation.Reference
        );
    }
    [Fact]
    ...
    public void LiteralSet_ByNameSelector_AppliesAndIsReadBack() {
        ...
        Assert.False(
            condition: setResult.IsError,
            userMessage: setResult.Output
        );
        ...
        Assert.False(
            condition: readResult.IsError,
            userMessage: readResult.Output
        );
        Assert.Contains(
            expectedSubstring: "0.03",
            actualString: readResult.Output,
            comparisonType: StringComparison.Ordinal
        );
    }
    [Fact]
    public void LiteralSet_DottedSelectorValue_ResolvesInsideTheBracket() {
        ...
        Assert.Contains(
            expectedSubstring: "no element of 'palette' has specular=0.5",
            actualString: badResult.Output,
            comparisonType: StringComparison.Ordinal
        );
        Assert.Contains(
        ...
        Assert.False(
            condition: goodResult.IsError,
            userMessage: goodResult.Output
        );
        ...
        Assert.Equal(
            expected: "#010203",
            actual: MothDocument(row: row).Palette![1].Color
        );
    }
    [Fact]
    public void LiteralSet_StateBindingNamingNoCell_RefusesByNameAndLeavesRowUnchanged() {
        ...
        Assert.Contains(
            actualString: diag,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "state.nowhere"
        );
        Assert.Contains(
            actualString: diag,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "must name a declared state cell"
        );
        Assert.Same(
            expected: before,
            actual: MothDocument(row: row)
        );
    }
    [Fact]
    public void LiteralSet_StateBindingString_OnACreationShape_RoundTripsAndStaysLive() {
        ...
        Assert.Equal(
            expected: "state.armLift",
            actual: shape.Position.Reference
        );
        Assert.Equal(
        ...
        Assert.Contains(
            expectedSubstring: "\"state.armLift\"",
            actualString: readResult.Output,
            comparisonType: StringComparison.Ordinal
        );
        ...
        Assert.Equal(
            expected: "state.armLift",
            actual: refreshed.Position.Reference
        );
        Assert.Equal(
            expected: new Vector3(
                x: 5f,
            ...
            actual: refreshed.Position.Value
        );
    }
    [Fact]
    public void LiteralSet_StateBindingString_OnAPlacement_RoundTrips() {
        ...
        Assert.Equal(
            expected: "state.armLift",
            actual: placement.Position.Reference
        );
        Assert.Equal(
            expected: new Vector3(
                x: 2f,
            ...
            actual: placement.Position.Value
        );
        ...
        Assert.False(
            condition: controlResult.IsError,
            userMessage: controlResult.Output
        );
    ...
    public void LiteralSet_StateBindingString_OnARotation_SurvivesCanonicalization() {
        ...
        Assert.Equal(
            expected: "state.armRot",
            actual: shape.Rotation.Reference
        );
        Assert.Equal(
            expected: Quaternion.Identity,
            actual: shape.Rotation.Value
        );
        ...
        Assert.False(
            condition: otherResult.IsError,
            userMessage: otherResult.Output
        );
    ...
    public void LiteralSet_ThenAddOrRemove_SameRowSameWindow_RefusesByNameThenAppliesNextWindow() {
        ...
        Assert.Contains(
            expectedSubstring: "row 'creations.moth' already has an edit buffered this tick",
            actualString: addCollided.Output,
            comparisonType: StringComparison.Ordinal
        );
        ...
        Assert.Contains(
            expectedSubstring: "row 'creations.moth' already has an edit buffered this tick",
            actualString: removeCollided.Output,
            comparisonType: StringComparison.Ordinal
        );
        ...
        Assert.False(
            condition: addResult.IsError,
            userMessage: addResult.Output
        );
        ...
        Assert.False(
            condition: removeResult.IsError,
            userMessage: removeResult.Output
        );
    ...
    public void LiteralSet_ThenStep_SameRowSameWindow_RefusesByNameThenAppliesNextWindow() {
        ...
        Assert.Contains(
            expectedSubstring: "row 'creations.moth' already has a step buffered this tick",
            actualString: collided.Output,
            comparisonType: StringComparison.Ordinal
        );
        ...
        Assert.False(
            condition: stepResult.IsError,
            userMessage: stepResult.Output
        );
    ...
    public void LiteralSet_UnknownSelector_RefusesByNameListingCandidates() {
        ...
        Assert.True(condition: badResult.IsError);
        Assert.Contains(
            expectedSubstring: "no element of 'shapes' has name=bogus",
            actualString: badResult.Output,
            comparisonType: StringComparison.Ordinal
        );
        Assert.Contains(
            expectedSubstring: "torso",
            actualString: badResult.Output,
            comparisonType: StringComparison.Ordinal
        );
        Assert.Contains(
            expectedSubstring: "forearmL",
            actualString: badResult.Output,
            comparisonType: StringComparison.Ordinal
        );
    ...
    public void LiteralSet_ValidationFailure_RefusesWithValidatorTextAndLeavesRowUnchanged() {
        ...
        Assert.Contains(
            actualString: rejectedMessage,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "edge-rounding radius"
        );
    ...
    public void Read_BareListField_ListsOneLinePerElement() {
        ...
        Assert.Equal(
            expected: "[world.row: creations moth.document.shapes: 2 element(s)]",
            actual: lines[0]
        );
        Assert.StartsWith(
            expectedStartString: "[world.row 0: \"torso\" {",
            actualString: lines[1],
            comparisonType: StringComparison.Ordinal
        );
        Assert.StartsWith(
            expectedStartString: "[world.row 1: \"forearmL\" {",
    ...
    public void Read_TrailingSelector_ReadsBackTheWholeElement() {
        ...
        Assert.Contains(
            expectedSubstring: "\"name\":\"forearmL\"",
            actualString: result.Output,
            comparisonType: StringComparison.Ordinal
        );
    }
    ...
    public void Read_WholeRow_OmitsTheHashAndIsAcceptedBackByTheWholeRowSet() {
        ...
        Assert.StartsWith(
            expectedStartString: "[world.row: creations moth = {",
            actualString: result.Output,
            comparisonType: StringComparison.Ordinal
        );
        ...
        Assert.False(
            condition: hashResult.IsError,
            userMessage: hashResult.Output
        );
        Assert.Contains(
            expectedSubstring: $"\"{MothPrototype(row: row).Hash}\"",
            actualString: hashResult.Output,
            comparisonType: StringComparison.Ordinal
        );

        var echoed = System.Text.Json.Nodes.JsonNode.Parse(json: result.Output[(result.Output.IndexOf(
            comparisonType: StringComparison.Ordinal,
            value: " = "
        ) + 3)..^1])!.AsObject();
        ...
        Assert.Null(@object: diag);
        Assert.True(condition: (MathF.Abs(x: (FindShape(
            name: "forearmL",
            row: row
        ).Rounding!.Value - 0.04f)) < 0.0001f));
        ...
        Assert.False(
            condition: staleResult.IsError,
            userMessage: staleResult.Output
        );
        ...
        Assert.Contains(
            actualString: diag,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "does not match the canonical sha256"
        );
        Assert.True(condition: (MathF.Abs(x: (FindShape(
            name: "forearmL",
            row: row
