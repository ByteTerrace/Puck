...
public sealed class AuthoredShapeAdmissionLawTests {
    ...
    private static SdfInstruction PrismShape(Vector3 scale, float rounding) {
        ...
        return builder.Build().Instructions.Single(predicate: instruction => (instruction.Op == SdfOp.ShapeBlend));
    }
    ...
    public void ARevolvedPrismIsRefusedOnABodyCollider() {
        ...
        Assert.False(condition: WorldDefinitionValidator.TryValidateLocally(
            definition: BodyCollider(lift: SdfLift.Revolve),
            reason: out var reason
        ));
        Assert.Contains(
            actualString: reason,
            expectedSubstring: "revolve"
        );
        Assert.True(
            WorldDefinitionValidator.TryValidateLocally(
                definition: BodyCollider(lift: SdfLift.Extrude),
                reason: out reason
            ),
            userMessage: reason
        );
    ...
    public void ARevolvedPrismReachCoversItsOffsetRing() {
        ...
        foreach (var scale in new[] { new Vector3(
            x: .4f,
            y: .25f,
            z: 2f
        ), new Vector3(
    ...
    public void ATriangularPrismAdmitsACapChamferWhereItRefusesRounding() {
        ...
        var inradius = SdfSolidGeometry.MaxChamfer(
            SdfSolidPrimitive.Prism,
            scale,
            taper: 0f
        );
        ...
        AssertCanonicalizerAccepts(shape: Shape(
            SdfSolidPrimitive.Prism,
            scale
        ) with { Taper = 0f, Chamfer = (inradius - .01f) });
        AssertCanonicalizerRefusesNaming(
            Shape(
                SdfSolidPrimitive.Prism,
                scale
    ...
    public void BoxChamferEmitsAChamferedRectangleShapeUnderThePlainBoxFillet() {
        ...
        Assert.True(condition: (instruction.Data1.W > 0f));
        Assert.Equal(
            (.3f - (instruction.Data1.W * (2f - MathF.Sqrt(x: 2f)))),
            instruction.Data0.Z,
            tolerance: 1e-6f
        );
    }
    ...
    public void ChamferOnAPolygonProfilePrismIsRefusedByName_TheLaneIsAlreadyTaken() {
        ...
        AssertCanonicalizerRefusesNaming(
            Shape(
                SdfSolidPrimitive.Prism,
                scale
            ) with { Profile = profile, Chamfer = .1f },
            "chamfer"
        );
    }
    [Fact]
    public void ChamferOnARevolvedPrismIsRefusedByName_ARevolveHasNoCapSeam() {
        ...
        AssertCanonicalizerRefusesNaming(
            Shape(
                SdfSolidPrimitive.Prism,
                scale
            ) with { Lift = SdfLift.Revolve, Chamfer = .1f },
            "chamfer"
        );
    }
    ...
    public void EveryProfileRevolvesAndEmitsAShape(SdfPrismProfileKind kind) {
        var shape = Shape(
            ...
            Lift = SdfLift.Revolve,
            Profile = ((kind == SdfPrismProfileKind.Trapezoid)
            ? null
            : new SdfPrismProfile(kind)),
        };
        ...
        var shapes = program.Instructions.Where(predicate: instruction => (instruction.Op == SdfOp.ShapeBlend)).ToArray();
    ...
    public void InvalidProfileControlsAreRefusedBeforeEmission() {
        foreach (var profile in new[] { new SdfPrismProfile(((SdfPrismProfileKind)99)), new(
            SdfPrismProfileKind.Polygon,
            Sides: 2
        ),
            new(
            ...
            Assert.Contains(
                ...
                filter: error => (error.Path == "shapes[0].profile")
            );
        }
    }
    [InlineData(SdfSolidPrimitive.Box)]
    ...
    public void PrismProfileSurvivesCanonicalizationAndBothContactProviders(float taper) {
        ...
        Assert.Equal(
            taper,
            canonical.Document.Shapes![0].Taper
        );
        AssertWorldValidates(shape);
        AssertWorldValidates(
            shape,
    ...
    public void RoundingInsetsTheShapeInsteadOfGrowingIt() {
        ...
        var sharp = PrismShape(
            rounding: 0f,
            scale: scale
        );
        var rounded = PrismShape(
            rounding: .1f,
            scale: scale
        );
        ...
        Assert.True(condition: ((rounded.Data0.X + rounded.Data1.W) <= (sharp.Data0.X + 1e-6f)));
        Assert.True(condition: ((rounded.Data0.Z + rounded.Data1.W) <= (sharp.Data0.Z + 1e-6f)));
