...
public sealed class AdmissionArrivalLawTests {
    ...
    private static WorldAdmissionEntry Arrivals(string domain, bool drive = true, bool control = true, ushort budget = 64) {
        var grants = new List<WorldAdmissionGrant> { new(
        ...
        if (control) {
            grants.Add(item: new WorldAdmissionGrant(
    ...
    private static bool ReserveArrival(IReadOnlyList<WorldAdmissionEntry> admission) {
        ...
        return fixture.Server.ReserveTransfer(request: Request(
            identity: null,
            transferId: 17
        )).Accepted;
    }
    ...
    public void ArrivalRows_AreInvisibleToTheAttestationClaimArm() {
        Assert.False(condition: WorldAdmissionDoor.TryMatchEntry(
            entries: [Arrivals(domain: SourceAuthority)],
            domain: SourceAuthority,
            subject: null,
            verdict: out _
        ));
    }
    ...
    public void Arrival_RequiresDriveOverTheAssignedBody_ControlDriveTemplateLands() {
        Laws.RefusalWithControl(
            ...
            deniedOutcome: static () => ReserveArrival(admission: [Arrivals(
                    domain: SourceAuthority,
                    drive: false
                )]),
            controlOutcome: static () => ReserveArrival(admission: [Arrivals(domain: SourceAuthority)])
    ...
    public void CommittedArrival_HoldsExactlyTheAuthoredTemplates() {
        using var fixture = Fixtures.FreshServer(definition: Document(admission: [Arrivals(
                domain: SourceAuthority,
                control: false
            )]));
        var slot = Land(
        ...
        Assert.DoesNotContain(
            collection: rows,
            filter: row => (row.Subject.Kind == GrantSubjectKind.All)
        );
    }
    ...
    public void CommittedArrival_IdentityNamesTheAuthenticatedAuthority_NotTheCarriedProfile() {
        ...
        var carried = WorldIdentity.Pinned(
            name: "forged-subject",
            moveSpeed: Puck.Maths.FixedQ4816.One,
            turnSpeed: Puck.Maths.FixedQ4816.One,
            defaults: document.PlayerDefaults
        );
        var slot = Land(
            fixture: fixture,
            identity: carried,
    ...
    public void NamedAuthority_OutranksTheWildcardRow_InEitherAuthoredOrder() {
        ...
        foreach (var entries in new[] { new[] { wildcard, narrow }, new[] { narrow, wildcard } }) {
            ...
            Assert.Equal(
                expected: ((ushort)7),
                actual: verdict!.Templates.Single(predicate: template => (template.Capability == WorldCapability.Drive)).Budget
            );
        }
    }
}
