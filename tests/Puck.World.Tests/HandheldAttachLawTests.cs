...
public sealed class HandheldAttachLawTests {
    ...
    private static readonly Vector3 HandOffset = new(
        x: 0.22f,
        y: 1.02f,
        z: 0.34f
    );
    ...
    private static WorldDefinition Document(bool includeRules) {
        ...
        var document = Fixtures.BuildDocument() with {
            ChannelsRaw = [.. Fixtures.BuildDocument().Channels, new WorldChannel(
                Name: JumpChannel,
            ...
            Policy: null,
            Rows: [standPlacement, handheldPlacement]
        ),
    ...
    public void APressedChannelInsideTheStandsRegionAttachesTheHandheldToTheBody_WalkingOutReturnsItToTheStand() {
        ...
        Assert.Equal(
            expected: 0L,
            actual: HandheldHeldCell(fixture: fixture)
        );
        ...
        Assert.Equal(
            expected: 0,
            actual: held.Attach!.BodyIndex
        );
        Assert.Equal(
            expected: HandOffset,
            actual: held.Attach!.LocalOffset.Value
        );
        ...
        Assert.True(
            condition: resolvedAttach,
            userMessage: attachReason
        );
        ...
        Assert.Equal(
            expected: (((double)bodyPosition.X) + HandOffset.X),
            actual: ((double)attachedPosition.X),
            precision: 3
        );
        Assert.Equal(
            expected: (((double)bodyPosition.Y) + HandOffset.Y),
            actual: ((double)attachedPosition.Y),
            precision: 3
        );
        ...
        Assert.Equal(
            expected: 0f,
            actual: returned.Position.Value.X
        );
        Assert.Equal(
            expected: 1.08f,
            actual: returned.Position.Value.Y
        );
    }
    [Fact]
...
public sealed class RealArcadeModuleLawTests {
    ...
    public void TheArcadeModuleShipsTheHandheldPairUnderItsImportAlias() {
        ...
        Assert.Contains(
            collection: pickup.Effects,
            filter: effect => ((effect is WorldEffect.UpsertPlacement upsert) && (upsert.Placement.Attach != null))
        );
        Assert.Contains(
            collection: release.Effects,
            filter: effect => ((effect is WorldEffect.UpsertPlacement upsert) && (upsert.Placement.Attach == null))
        );
