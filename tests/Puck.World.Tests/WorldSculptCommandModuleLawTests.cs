...
public sealed class WorldSculptCommandModuleLawTests {
    ...
    public void PatchFaultIsANamedRefusalThatSubmitsNothing() {
        ...
        Assert.Contains(
            expectedSubstring: $"[creation.sculpt: {FaultingSculpt.SculptName}: patch fault",
            actualString: result.Output,
            comparisonType: StringComparison.Ordinal
        );
        Assert.Contains(
            expectedSubstring: "unterminated selector",
            actualString: result.Output,
            comparisonType: StringComparison.Ordinal
        );
    ...
    public void SculptCollidesWithAPendingRowEditInTheSameWindow_LandsAfterTheDrain() {
        ...
        Assert.False(
            condition: edit.IsError,
            userMessage: edit.Output
        );
        ...
        Assert.True(
            condition: collided.IsError,
            userMessage: collided.Output
        );
        Assert.Contains(
            expectedSubstring: "row 'render' already has an edit buffered this tick",
        ...
        Assert.Equal(
            expected: 32f,
            actual: harness.Server.Definition.Render.FarDistance
        );
        ...
        Assert.False(
            condition: landed.IsError,
            userMessage: landed.Output
        );

        harness.Step();

        Assert.Equal(
            expected: FarDistanceSculpt.FarDistance,
            actual: harness.Server.Definition.Render.FarDistance
        );
    }
    ...
    public void UnknownSculptIsRefusedByName() {
        ...
        Assert.Contains(
            expectedSubstring: "unknown sculpt 'not-a-real-sculpt'",
            actualString: result.Output,
            comparisonType: StringComparison.Ordinal
        );
        Assert.Contains(
            expectedSubstring: FarDistanceSculpt.SculptName,
            actualString: result.Output,
            comparisonType: StringComparison.Ordinal
        );
        Assert.Empty(collection: harness.Submitted);
