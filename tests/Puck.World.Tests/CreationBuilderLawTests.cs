...
public sealed class CreationBuilderLawTests {
    ...
    public void ChainParentsEachBeadToThePrevious() {
        ...
        var names = builder.Chain(
            ...
            positionOf: static i => new Vector3(
                x: 0,
                y: i,
                z: 0
            ),
        ...
        Assert.Equal(
            actual: names,
            expected: ["bead0", "bead1", "bead2"]
        );
        Assert.Equal(
            expected: "root",
            actual: builder.Find(name: "bead0").Parent
        );
        Assert.Equal(
            expected: "bead0",
            actual: builder.Find(name: "bead1").Parent
        );
        Assert.Equal(
            expected: "bead1",
            actual: builder.Find(name: "bead2").Parent
        );
    }
    ...
    public void DuplicateIdIsRefused() {
        ...
        var exception = Assert.Throws<ArgumentException>(testCode: () => builder.Shape(
            id: 5,
            name: "b",
            type: SdfSolidPrimitive.Box,
            position: Vector3.Zero,
    ...
    public void EqualRotationsInternToTheSameName() {
        var builder = Builder();
        var first = builder.AxisAngleDegrees(
            axis: new Vector3(
                x: 0,
                y: 0,
                z: 1
        ...
        var second = builder.AxisAngleDegrees(
            axis: new Vector3(
                x: 0,
                y: 0,
                z: 1
        ...
        Assert.True(condition: builder.TryRotationName(
            name: out var name,
            value: first
        ));
        Assert.Equal(
        ...
        Assert.Single(
            collection: builder.NamedRotations,
            predicate: r => (r.Name == "z90")
        );
    }
    ...
    public void FixedIdsAreHonoredAndSkippedByTheCounter() {
        ...
        Assert.Equal(
            expected: 100,
            actual: builder.Find(name: "auto1").Id
        );
        Assert.Equal(
            expected: 7,
            actual: builder.Find(name: "pinned").Id
        );
        Assert.Equal(
            expected: 101,
            actual: builder.Find(name: "auto2").Id
        );
    }
    ...
    public void MultiplyRefusesAnUninternedOperand() {
        var builder = Builder();
        var foreign = Quaternion.CreateFromAxisAngle(
            axis: Vector3.UnitY,
            angle: 1f
        );

        Assert.Throws<ArgumentException>(testCode: () => builder.Multiply(
            a: foreign,
            b: builder.Identity
        ));
    }
    ...
    public void ParentDeclaredFirstBuildsCleanly() {
        ...
        Assert.Equal(
            expected: 2,
            actual: document.Shapes!.Count
        );
        Assert.Equal(
            expected: "root",
            actual: document.Shapes![1].Parent
        );
    }
    ...
    public void ShapeRefusesAnUninternedRotation() {
        ...
        Assert.Throws<ArgumentException>(testCode: () => builder.Shape(
            name: "a",
            type: SdfSolidPrimitive.Box,
            position: Vector3.Zero,
            rotation: foreign,
