...
public sealed class OverlayPresentationPredicateLawTests {
    ...
    private static readonly WorldHudRect UnitRect = new(
        Height: 1f,
        Width: 1f,
        X: 0f,
        Y: 0f
    ...
    public void ACameraRefusesAnAnchorBesideARankedListWhileEitherAlonePasses() {
        ...
        var ranked = new WorldCameraAnchorCandidate[] { new(
            Anchor: new WorldAnchor.RecentSpeaker(),
            When: new OverlayPredicate.Speaking(
                Subject: new OverlaySubject.RecentSpeaker(),
                WindowSeconds: 2f
            )
        ), new(Anchor: seat) };
        ...
        Laws.RefusalWithControl(
            ...
            deniedOutcome: () => Validates(definition: WithCameras(Camera(
                anchor: seat,
                anchors: ranked
            ))),
    ...
    public void AFrameElementRefusesANegativeFadeWhileZeroPasses() {
        Laws.RefusalWithControl(
            ...
            deniedOutcome: () => Validates(definition: WithPanel(element: FrameElement(
                source: CaptureSource(title: "a"),
                fadeSeconds: -1f
            ))),
            controlOutcome: () => Validates(definition: WithPanel(element: FrameElement(
                source: CaptureSource(title: "a"),
                fadeSeconds: 0f
            )))
        );
    ...
    public void AFrameElementRefusesSourceBesideSourcesWhileEitherAlonePasses() {
        var candidates = new WorldHudFrameCandidate[] { new(
            Source: CaptureSource(title: "a"),
            When: new OverlayPredicate.Now(Fact: OverlayFact.ConsoleOpen)
        ), new(Source: CaptureSource(title: "b")) };
    ...
    public void ANearPredicateRefusesANegativeDistanceWhileANonNegativeOnePasses() {
        Laws.RefusalWithControl(
            ...
            deniedOutcome: () => Validates(definition: WithPanel(visible: new OverlayPredicate.Near(
                Subject: new OverlaySubject.RecentSpeaker(),
                Distance: -0.5f
            ))),
            controlOutcome: () => Validates(definition: WithPanel(visible: new OverlayPredicate.Near(
                Subject: new OverlaySubject.RecentSpeaker(),
                Distance: 2f,
                Of: new OverlaySubject.AnySeat()
            )))
        );
    }
    [Fact]
    public void ARankedCameraSelectsTheFirstHoldingCandidateAndTheWorldFrameWhenNoneHolds() {
        ...
        var camera = Camera(anchors: [new WorldCameraAnchorCandidate(
                Anchor: new WorldAnchor.RecentSpeaker(),
                When: speaking
            ), new WorldCameraAnchorCandidate(
                Anchor: new WorldAnchor.Seat(),
                When: new OverlayPredicate.Now(Fact: OverlayFact.ConsoleOpen)
            )]);

        var winner = WorldSeatAnchors.SelectAnchor(
            camera: camera,
            candidateIndex: out var winnerIndex,
            evaluator: new FactEvaluator(holding: OverlayFact.WheelOpen),
            slot: 0
        );
        var none = WorldSeatAnchors.SelectAnchor(
            camera: camera,
            candidateIndex: out var noneIndex,
            evaluator: new FactEvaluator(holding: null),
            slot: 0
        );
        var bare = WorldSeatAnchors.SelectAnchor(
            camera: Camera(anchor: new WorldAnchor.Seat()),
            candidateIndex: out var bareIndex,
            evaluator: new FactEvaluator(holding: null),
            slot: 0
        );
    ...
    public void ASeatAnchorFollowsTheSeatsPerceivedBodyAndARecentSpeakerFollowsTheClock() {
        ...
        Assert.Equal(
            expected: 9,
            actual: WorldSeatAnchors.BodyOf(
                anchor: new WorldAnchor.Seat(),
                slot: 1,
        ...
        Assert.Equal(
            expected: 9,
            actual: WorldSeatAnchors.BodyOf(
                anchor: new WorldAnchor.Seat(Number: 2),
                slot: 0,
                perception: perception,
                speech: speech
            )
        ...
        Assert.Equal(
            expected: 0,
            actual: WorldSeatAnchors.BodyOf(
                anchor: new WorldAnchor.Seat(Number: 1),
                slot: 1,
                perception: perception,
                speech: speech
            )
        ...
        Assert.Equal(
            expected: -1,
            actual: WorldSeatAnchors.BodyOf(
                anchor: new WorldAnchor.Entity(Index: 3),
                slot: 0,
                perception: perception,
                speech: speech
            )
    ...
    public void ASeatRelativeCameraRegistersPerSeatWhileASharedCameraRegistersOnce() {
        var seatCamera = Camera(anchors: [new WorldCameraAnchorCandidate(
                Anchor: new WorldAnchor.RecentSpeaker(),
                When: new OverlayPredicate.Speaking(
                    Subject: new OverlaySubject.RecentSpeaker(),
                    WindowSeconds: 1f
                )
            ), new WorldCameraAnchorCandidate(Anchor: new WorldAnchor.Seat())]);
        ...
        Assert.NotEqual(
            expected: WorldSeatAnchors.RegistrationName(
                camera: seatCamera,
                seat: 1
            ),
            actual: WorldSeatAnchors.RegistrationName(
                camera: seatCamera,
                seat: 2
            )
        );
    ...
    public void ASpeakingPredicateRefusesANegativeWindowWhileANonNegativeOnePasses() {
        Laws.RefusalWithControl(
            ...
            deniedOutcome: () => Validates(definition: WithPanel(visible: new OverlayPredicate.Speaking(
                Subject: new OverlaySubject.Seat(),
                WindowSeconds: -1f
            ))),
            controlOutcome: () => Validates(definition: WithPanel(visible: new OverlayPredicate.Speaking(
                Subject: new OverlaySubject.Seat(),
                WindowSeconds: 1f,
                FadeSeconds: 0.5f
            )))
    ...
    public void AWinnerChangeHoldsBothKeysUntilTheFadeCompletes() {
        ...
        Assert.Equal(
            expected: (7, -1, 1f),
            actual: (fade.Current, fade.Outgoing, fade.Mix)
        );
        ...
        Assert.Equal(
            expected: (9, 7),
            actual: (fade.Current, fade.Outgoing)
        );
        Assert.Equal(
            expected: 0f,
            actual: fade.Mix
    ...
    public void AZeroFadeCutsAtTheWindowEnd() {
        ...
        Assert.Equal(
            expected: 1f,
            actual: OverlayRecency.Presence(
                completedTick: ((10UL + RateHz) - 1),
                fadeSeconds: 0f,
                lastHeldTick: clock.LastSpokeTick(bodyIndex: 0),
                rateHz: RateHz,
                windowSeconds: 1f
            )
        );
    ...
    public void SpeakingPresenceHoldsThroughTheWindowEasesAcrossTheFadeThenCuts() {
        ...
        Assert.Equal(
            expected: 2,
            actual: clock.RecentSpeakerBody
        );
        Assert.Equal(
            expected: 1f,
            actual: At(
        ...
        Assert.Equal(
            ...
            actual: OverlayRecency.Presence(
                completedTick: Spoke,
                fadeSeconds: Fade,
                lastHeldTick: clock.LastSpokeTick(bodyIndex: 3),
                rateHz: RateHz,
                windowSeconds: Window
            )
        );
    ...
    public void TheFirstHoldingCandidateWinsAndNoneHoldingIsNoWinner() {
        ...
        Assert.Equal(
            expected: 1,
            actual: OverlayRanking.FirstHolding(
    ...
    public void TheRecentSpeakerIsTheLatestStamp() {
        ...
        Assert.Equal(
            expected: 5UL,
            actual: clock.LastSpokeTick(bodyIndex: 4)
        );
        Assert.Equal(
            expected: 0UL,
            actual: clock.LastSpokeTick(bodyIndex: -1)
        );
    }
    ...
    private sealed class FactEvaluator(OverlayFact? holding) : IOverlayPredicateEvaluator {
        ...
        public bool EvaluateAnySeat(OverlayPredicate? predicate) => Evaluate(
            predicate: predicate,
            slot: 0
        );
    }
