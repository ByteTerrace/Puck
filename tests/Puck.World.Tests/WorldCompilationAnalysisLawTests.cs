...
public sealed class WorldCompilationAnalysisLawTests {
    ...
    public void LoaderReceiptReachesServerConstructionWithoutReplacingItsPrograms() {
        ...
        var server = new WorldServer(definition, new WorldPopulation(definition: definition), fixture.Server.Profiles,
        ...
        Assert.True(condition: WorldDefinitionValidator.TryCompleteAdmission(admission: admission, machines: catalog, neighbours: null, reason: out reason), userMessage: reason);
        Assert.False(condition: WorldDefinitionValidator.TryCompleteAdmission(admission: admission, machines: new WorldMachineCatalog(engines: []), neighbours: null, reason: out reason));
        Assert.Contains(actualString: reason, expectedSubstring: "different machine catalog");
    }
    [Fact]
    public void AdmissionCarriesTheExactProgramsIntoConstructionAndRefusesAnotherOwner() {
        ...
        Assert.True(condition: WorldDefinitionValidator.TryAdmitLocally(admission: out var admission, definition: definition,
            machines: catalog, reason: out var reason), userMessage: reason);
        Assert.NotNull(@object: admission);
        ...
        Assert.False(condition: machines.TryPrepare(admission: admission, candidate: another, current: definition,
            plan: out var plan, reason: out reason));
        Assert.Null(@object: plan);
        Assert.Contains(actualString: reason, expectedSubstring: "different definition or machine catalog");
        ...
        Assert.False(condition: otherMachines.TryPrepare(admission: admission, candidate: definition, current: null,
        ...
        var definition = Fixtures.BuildDocument() with {
            ...
            Rules = [
                new WorldRule(CellName.Parse(candidate: "observe"),
                    ...
                    Gate: new ActionPredicate.CompareState(State: "value", Comparison: ExpressionOp.Equal, Value: 0)),
