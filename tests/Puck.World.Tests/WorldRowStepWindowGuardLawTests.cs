...
public sealed class WorldRowStepWindowGuardLawTests {
    ...
    public void SameRow_SameWindow_SecondClaimCollides() {
        ...
        Assert.False(condition: guard.IsClaimed(window: 5UL, rowIdentity: "render"));
        ...
        Assert.True(condition: guard.IsClaimed(window: 5UL, rowIdentity: "render"));
    }
    [Fact]
    public void SameWindow_DifferentRows_DoNotCollide() {
        var guard = new WorldRowStepWindowGuard();

        Assert.False(condition: guard.IsClaimed(window: 5UL, rowIdentity: "creations.a"));
        ...
        Assert.False(condition: guard.IsClaimed(window: 5UL, rowIdentity: "creations.b"));
    }
    [Fact]
    public void SameRow_AcrossWindows_NeverCollides_HeldChordKeepsStepping() {
        ...
        for (var window = 1UL; (window <= 8UL); window++) {
            Assert.False(condition: guard.IsClaimed(window: window, rowIdentity: "render"));
            guard.Claim(rowIdentity: "render");
        }
    }
    [Fact]
    ...
    public void WindowAdvance_ClearsPriorClaims() {
        ...
        Assert.False(condition: guard.IsClaimed(window: 1UL, rowIdentity: "render"));
        guard.Claim(rowIdentity: "render");
        Assert.True(condition: guard.IsClaimed(window: 1UL, rowIdentity: "render"));
        // The next window starts empty.
        Assert.False(condition: guard.IsClaimed(window: 2UL, rowIdentity: "render"));
    }
}
