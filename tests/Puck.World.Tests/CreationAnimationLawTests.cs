...
public sealed class CreationAnimationLawTests {
    ...
    private static WorldSolidField BuildField(WorldPrototype creation) {
        ...
        Assert.True(
            condition: WorldSolidField.TryBuild(
    ...
    public void AFacetNamingNoDriverComposesNothing() {
        var shape = Limb(swings: [new ShapeSwingDocument(
                Amplitude: 1f,
                Axis: Vector3.UnitZ,
                Driver: "absent",
                Pivot: Vector3.Zero
        ...
        Assert.NotEqual(
            expected: named.Position.Value,
            actual: namedPosition
        );
    }
    ...
    public void AMovingGateReleasesWhenTheBodyStopsWhileAFactOnlyGateDoesNot() {
        ...
        Assert.True(
            (movingWeights[0] > 0.9f),
            userMessage: $"the moving-gated driver did not reach full weight while walking; weight={movingWeights[0]}"
        );
        Assert.True(
            (factWeights[0] > 0.9f),
            userMessage: $"the fact-gated control did not reach full weight while walking; weight={factWeights[0]}"
        );
        ...
        Assert.True(
            (factWeights[0] > 0.9f),
            userMessage: $"the fact-gated control released on a stop; weight={factWeights[0]}"
        );
        // A released driver stops charging its phase too, so the pose it returns from does not drift.
        Assert.Equal(
            expected: walkingPhase,
            actual: movingPhases[0]
        );
        ...
        Assert.True(
            (movingWeights[0] > 0.9f),
            userMessage: $"the moving-gated driver did not re-arm when the body walked again; weight={movingWeights[0]}"
        );
    }
    ...
    public void AlwaysGateHoldsRegardlessOfFacts() {
        ...
        Assert.True(
            (weights[0] > 0.9f),
            userMessage: $"an ungated driver did not ease to full weight; weight={weights[0]}"
        );
        Assert.True(
            (phases[0] > 0f),
            userMessage: "a time-driven ungated driver did not advance while the body stood still"
        );
        ...
        var gated = new[] { Stride(
            cadence: 1f,
            signal: CreationDriverDocument.SignalTime,
            when: [nameof(BodyFacts.InMedium)]
        ) };
        ...
        Assert.Equal(
            expected: 0f,
            actual: gatedPhases[0]
        );
        Assert.Equal(
            expected: 0f,
            actual: gatedWeights[0]
        );
    }
    ...
    public void AnAddressChangeReseedsPhaseAndWeight() {
        ...
        Assert.True(
            (phases[0] > 0f),
            userMessage: "the control never built a phase to reseed"
        );
        Assert.True(
            (weights[0] > 0f),
            userMessage: "the control never built a weight to reseed"
        );

        Step(
            at: (FirstBody with { Generation = 2 }),
            position: new Vector3(
                x: 1f,
                y: 0f,
                z: 0f
    ...
    public void AnUnanimatedCreationCarriesNoAnimationMembers() {
        ...
        Assert.DoesNotContain(
            ...
            expectedSubstring: "\"drivers\""
        );
        Assert.DoesNotContain(
            actualString: json,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "\"swings\""
        );
        Assert.DoesNotContain(
            actualString: json,
            comparisonType: StringComparison.Ordinal,
        ...
        var animated = CreationCanonicalizer.Canonicalize(
            document: Rig(
                drivers: [Stride()],
                shape: Limb(swings: [new ShapeSwingDocument(
                        Amplitude: 0.6f,
                        Axis: Vector3.UnitX,
                        Driver: "stride",
                        Pivot: Vector3.Zero
        ...
        Assert.Contains(
            actualString: Encoding.UTF8.GetString(bytes: animated.Bytes),
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "\"drivers\""
        );
        Assert.Contains(
    ...
    public void AnimatedFacetsChangeNeitherTheRenderProgramNorTheSolidField() {
        ...
        var animatedCanonical = CreationCanonicalizer.Canonicalize(
            document: animated,
            source: "rig"
        );
        var inertCanonical = CreationCanonicalizer.Canonicalize(
            document: inert,
            source: "rig"
        );
        ...
        Assert.NotEqual(
            expected: inertCanonical.Hash,
            actual: animatedCanonical.Hash
        );

        var animatedCreation = new WorldPrototype(
            Id: "rig",
            Document: animatedCanonical.Document,
            HashRaw: animatedCanonical.Hash
        );
        var inertCreation = new WorldPrototype(
            Id: "rig",
            Document: inertCanonical.Document,
            HashRaw: inertCanonical.Hash
        );

        Assert.Equal(
            expected: EmitBodyStampWords(creation: inertCreation),
            actual: EmitBodyStampWords(creation: animatedCreation)
        );
        ...
        var probe = new FixedVector3(
            ...
            Y: FixedQ4816.FromDouble(value: 0.9),
            Z: FixedQ4816.Zero
        );

        Assert.True(condition: BuildField(creation: animatedCreation).Probe(
            distance: out var animatedDistance,
            gradient: out _,
            material: out _,
            position: in probe
        ));
        Assert.True(condition: BuildField(creation: inertCreation).Probe(
            distance: out var inertDistance,
            gradient: out _,
            material: out _,
            position: in probe
        ));
        Assert.Equal(
            actual: animatedDistance,
            expected: inertDistance
        );
        Assert.True(
            (FixedQ4816.Abs(value: animatedDistance) < SurfaceTolerance),
    ...
    public void AuthoredSwingPivotAndAxisTakeTheAuthorFrameHalfTurn() {
        ...
        Assert.Equal(
            ...
            actual: engine.Swings![0].Axis.Value
        );
        Assert.Equal(
            expected: new Vector3(
                x: -pivot.X,
            ...
            actual: engine.Swings[0].Pivot.Value
        );
        Assert.Equal(
            expected: new Vector3(
                x: 0f,
            ...
            actual: engine.Slides![0].Axis.Value
        );
        ...
        Assert.NotEqual(
            expected: Vector3.UnitX,
            actual: engine.Swings[0].Axis.Value
        );
        // Amplitude, phase, and waveform ride a proper rotation unchanged.
        Assert.Equal(
            expected: 0.6f,
            actual: engine.Swings[0].Amplitude.Value
        );
        Assert.Equal(
            expected: CreationWave.Sine,
            actual: engine.Swings[0].Wave
        );
        // ToAuthor is the same half turn, so the round trip is the identity.
        Assert.Equal(
            expected: pivot,
            actual: CreationFrame.ToAuthor(document: CreationFrame.ToEngine(document: canonical.Document)).Shapes![0].Swings![0].Pivot.Value
        );
    }
    ...
    public void AuthoredTransitionTimesSeparateBodyAndMechanismResponse() {
        ...
        Assert.True(condition: ((fast.Open > 0.9f) && (slow.Open < 0.65f)));
        Assert.True(condition: (fast.Closed < slow.Closed));
        Assert.InRange(
            MathF.Abs(x: (slow.Open - smooth.Open)),
            0f,
            Tolerance
        );
        Assert.InRange(
            MathF.Abs(x: (slow.Closed - smooth.Closed)),
            0f,
            Tolerance
        );
        var immediate = Transition(
            enter: 0f,
            leave: 0f,
            rate: 30
        );
        ...
        Assert.Equal(
            actual: immediate.Open,
            expected: 1f
        );
        Assert.Equal(
            actual: immediate.Closed,
            expected: 0f
        );
        Assert.Equal(
            0f,
            WorldGaitDrivers.WeightBlend(
                deltaSeconds: 0f,
                seconds: 0f
            )
        );
        ...
        static (float Open, float Closed) Transition(int rate, float enter, float leave) {
            CreationDriverDocument[] drivers = [new(
                    BlendInSeconds: enter,
                    BlendOutSeconds: leave,
                    Cadence: 0f,
                    Name: "mechanism",
                    Signal: "time",
                    When: ["Airborne"]
            ...
            void Step(BodyFacts facts) => WorldGaitDrivers.Advance(
                drivers,
                phases,
                weights,
                (1f / rate),
            ...
            Step(facts: BodyFacts.Airborne);
            for (var i = 0; (i < (rate / 5)); i++) { Step(facts: BodyFacts.Airborne); }
            var open = weights[0];

            for (var i = 0; (i < (rate / 5)); i++) { Step(facts: BodyFacts.Grounded); }
    ...
    public void EveryActionFactHasASingleBodyFactsBit() {
        foreach (var fact in BodyFactVocabulary.Publishable) {
            Assert.True(
                condition: BodyFactVocabulary.TryResolve(
                    gate: out var gate,
                    name: fact.ToString()
                ),
            ...
            Assert.True(
                condition: uint.IsPow2(value: ((uint)gate)),
                userMessage: $"BodyFacts.{fact} is not a single bit."
            );
        }

        Assert.True(condition: BodyFactVocabulary.TryResolve(
            gate: out var holdingUnwalkable,
            name: nameof(BodyFacts.HoldingUnwalkable)
        ));
        Assert.Equal(
            actual: holdingUnwalkable,
            expected: BodyFacts.HoldingUnwalkable
        );
        // The controls: the ungated token resolves to no bit, and an unrecognized token resolves to nothing at all.
        Assert.True(condition: BodyFactVocabulary.TryResolve(
            gate: out var always,
            name: BodyFactVocabulary.Always
        ));
        Assert.Equal(
            actual: always,
            expected: BodyFacts.None
        );
        Assert.False(condition: BodyFactVocabulary.TryResolve(
            gate: out _,
            name: "Swimming"
        ));
        Assert.False(condition: BodyFactVocabulary.TryResolve(
            gate: out _,
            name: "Grounded, Airborne"
        ));
    }
    ...
    public void GateShapeRefusalsAndTheSingleTokenSpelling() {
        ...
        Assert.Contains(
            actualString: Encoding.UTF8.GetString(bytes: canonical.Bytes),
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "\"when\": ["
        );
    ...
    public void InstantaneousSignalsTrackRatherThanAccumulate() {
        var drivers = new[] {
            Stride(
            cadence: 1f,
            signal: CreationDriverDocument.SignalVerticalSpeed,
            when: [CreationDriverDocument.WhenAlways]
        ),
            new CreationDriverDocument(
            Cadence: 1f,
            Name: "turn",
            Signal: CreationDriverDocument.SignalTurnRate,
            When: [CreationDriverDocument.WhenAlways]
        ),
        };
        var phases = new float[CreationDocument.MaxDrivers];
        ...
        Assert.True(
            (MathF.Abs(x: (phases[0] - 2f)) < Tolerance),
            userMessage: $"1 m of rise over 0.5 s did not read as 2 m/s; phase={phases[0]}"
        );
        Assert.True(
            (MathF.Abs(x: (phases[1] - 0.5f)) < Tolerance),
            userMessage: $"0.25 rad of yaw over 0.5 s did not read as 0.5 rad/s; phase={phases[1]}"
        );
        ...
        Assert.True(
            (MathF.Abs(x: (phases[0] - 2f)) < Tolerance),
            userMessage: $"verticalSpeed accumulated instead of tracking; phase={phases[0]}"
        );
        Assert.True(
            (MathF.Abs(x: (phases[1] - 0.5f)) < Tolerance),
            userMessage: $"turnRate accumulated instead of tracking; phase={phases[1]}"
        );
        ...
        Assert.True(
            (MathF.Abs(x: (phases[0] + 2f)) < Tolerance),
            userMessage: $"a descent did not read as a negative vertical speed; phase={phases[0]}"
        );
        Assert.True(
            (MathF.Abs(x: (phases[1] + 0.5f)) < Tolerance),
            userMessage: $"a reversed turn did not read as a negative turn rate; phase={phases[1]}"
        );
    }
    ...
    public void InvalidTransitionTimesAreRefused(float seconds) {
        Assert.Contains(
            "blendInSeconds",
            Refusal(document: Rig(
                Limb(),
                [Stride() with { BlendInSeconds = seconds }]
            )),
        ...
        Assert.Contains(
            ...
            Refusal(document: Rig(
                Limb(),
                [Stride() with { BlendOutSeconds = seconds }]
            )),
            StringComparison.Ordinal
        );
    }
    ...
    public void LinearWaveIsTheIdentityAndSineIsNot() {
        Assert.Equal(
            expected: 12.5f,
            actual: CreationWave.Evaluate(
                argument: 12.5f,
                wave: CreationWave.Linear
            )
        );
        Assert.Equal(
            expected: MathF.Sin(x: 12.5f),
            actual: CreationWave.Evaluate(
                argument: 12.5f,
                wave: CreationWave.Sine
            )
        );
        Assert.Equal(
            expected: MathF.Sin(x: 12.5f),
    ...
    public void OneFrameChargesAtMostTheTravelCap() {
        var drivers = new[] { Stride(
            cadence: 1f,
            signal: CreationDriverDocument.SignalTravel,
            when: [CreationDriverDocument.WhenAlways]
        ) };
        var phases = new float[CreationDocument.MaxDrivers];
        var weights = new float[CreationDocument.MaxDrivers];
        ...
        Step(position: new Vector3(
            ...
            y: 0f,
            z: 0f
        )); // ease the weight above zero so the phase may advance
        ...
        Assert.True(
            ((phases[0] - before) <= (WorldGaitDrivers.MaxTravelPerFrame + Tolerance)),
        ...
        Assert.True(
            (MathF.Abs(x: ((phases[0] - capped) - 0.05f)) < Tolerance),
    ...
    public void PlanarTravelDriverChargesOnlyHorizontalTravelUnderItsGate() {
        ...
        Step(
            facts: BodyFacts.Grounded,
            position: Vector3.Zero
        ); // the seeding frame charges nothing
        ...
        Assert.True(
            (MathF.Abs(x: (afterPlanar - (0.1f * 8f))) < Tolerance),
            userMessage: $"0.1 m of planar travel at cadence 8 did not advance the phase by 0.8; phase={afterPlanar}"
        );
        Assert.True(
            (weights[0] > 0f),
            userMessage: "the weight did not ease in while the gate held"
        );
        ...
        Assert.True(
            (MathF.Abs(x: (phases[0] - afterPlanar)) < Tolerance),
        ...
        Assert.True(
            (weights[0] < gatedWeight),
            userMessage: "the weight did not ease back out once the gate stopped holding"
        );
        ...
        Assert.True(
            (phases[0] > atRest),
            userMessage: "restoring the gate did not resume the phase"
        );
    }
    ...
    public void SlideDisplacesAlongItsAxisAndLeavesRotationAlone() {
        var slide = new ShapeSlideDocument(
            Amplitude: 0.25f,
            Axis: Vector3.UnitY,
            Driver: "bob"
        );
        ...
        slide.Compose(
            offset: (slide.Amplitude * CreationWave.Evaluate(
                argument: (MathF.PI / 2f),
                wave: slide.Wave
            )),
            position: ref position
        );
    ...
    public void SwingTurnsTheShapeAboutItsPivotRightHanded() {
        ...
        var swing = new ShapeSwingDocument(
            Amplitude: 1f,
            Axis: Vector3.UnitZ,
            Driver: "stride",
            Pivot: pivot
        );
        var position = (pivot + new Vector3(
            x: 0f,
            y: -1f,
            z: 0f
        ));
        ...
        Assert.True(
            (Vector3.Distance(
                value1: position,
                value2: (pivot + Vector3.UnitX)
            ) < Tolerance),
            userMessage: $"a +90° swing about +Z did not land at pivot + (1,0,0); landed at {position}"
        );
        Assert.True(
            (Vector3.Distance(
                value1: Vector3.Transform(
                    rotation: rotation,
                    value: Vector3.UnitX
                ),
                value2: Vector3.UnitY
            ) < Tolerance),
            userMessage: "the composed orientation did not carry the same turn as the position"
        );
        ...
        var mirrored = (pivot + new Vector3(
            x: 0f,
            y: -1f,
            z: 0f
        ));
    ...
    public void TheCanonicalizerRefusesEachMalformedFacetByName() {
        ...
        Assert.Empty(collection: CreationCanonicalizer.Validate(document: Rig(
            ...
            shape: Limb(swings: [new ShapeSwingDocument(
                    ...
                    Driver: "stride",
                    Pivot: Vector3.Zero,
                    Wave: $"{CreationWave.CurvePrefix}sway"
                )])
        )));
        Assert.Contains(
            actualString: Refusal(document: Rig(
        ...
        Assert.Contains(
            actualString: Refusal(document: Rig(
                drivers: [Stride()],
                shape: Limb(slides: [new ShapeSlideDocument(
                        Amplitude: 40f,
                        Axis: Vector3.UnitY,
                        Driver: "stride"
                    )])
        ...
        Assert.Contains(
            actualString: Refusal(document: Rig(
                drivers: [.. Enumerable.Range(
                        count: (CreationDocument.MaxDrivers + 1),
                        start: 0
                    ).Select(selector: index => Stride() with { Name = $"d{index}" })],
                shape: Limb()
            )),
            comparisonType: StringComparison.Ordinal,
        ...
        Assert.Contains(
            actualString: Refusal(document: Rig(
                ...
                shape: Limb(swings: [.. Enumerable.Repeat(
                        ...
                        element: new ShapeSwingDocument(
                            Amplitude: 0.6f,
                            Axis: Vector3.UnitX,
                            Driver: "stride",
                            Pivot: Vector3.Zero
        ...
        Assert.Contains(
            actualString: Refusal(document: Rig(
                ...
                shape: Limb(slides: [.. Enumerable.Repeat(
                        ...
                        element: new ShapeSlideDocument(
                            Amplitude: 0.2f,
                            Axis: Vector3.UnitY,
                            Driver: "stride"
                        )
    ...
    private static readonly WorldEntityAddress FirstBody = new(
        Authority: "a",
        Generation: 1,
        Index: 0
    );
