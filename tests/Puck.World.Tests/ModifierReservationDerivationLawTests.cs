...
public sealed class ModifierReservationDerivationLawTests {
    ...
    public void BindingBarReservationScalesOneElementPerModifierPerSeat() {
        ...
        Assert.Equal(
            expected: capacity.Seats,
            actual: (plusOneElements - baseElements)
        );
    }
    ...
    public void OverlayCapacityCarriesTheSchemaModifierCeiling() =>
        Assert.Equal(
            expected: WorldBindingBarCapacity.MaxModifiers,
            actual: WorldOverlayCapacity.FromSchema().BindingBarMaxModifiers
        );
}
