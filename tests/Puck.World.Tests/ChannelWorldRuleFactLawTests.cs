...
public sealed class ChannelWorldRuleFactLawTests {
    ...
    public void ARuleGatedOnAChannelFactFiresOnlyAfterThatSeatsChannelReachesTheThreshold() {
        ...
        Assert.Equal(
            expected: 0L,
            actual: BeaconCell(fixture: fixture)
        );
    ...
    public void TheChannelFactRefusesABadSeatOrAnUndeclaredChannelName_ControlAWellFormedGateClean() {
        ...
        Assert.False(condition: WorldDefinitionValidator.TryValidateLocally(
            definition: badSeat,
            reason: out var seatReason
        ));
        Assert.Contains(
            expectedSubstring: nameof(WorldRuleRefusal.ChannelMalformed),
            actualString: seatReason,
            comparisonType: StringComparison.Ordinal
        );

        Assert.False(condition: WorldDefinitionValidator.TryValidateLocally(
            definition: badChannel,
            reason: out var channelReason
        ));
        Assert.Contains(
            expectedSubstring: nameof(WorldRuleRefusal.ChannelMalformed),
            actualString: channelReason,
            comparisonType: StringComparison.Ordinal
        );
