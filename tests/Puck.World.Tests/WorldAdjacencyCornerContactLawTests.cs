...
public sealed class WorldAdjacencyCornerContactLawTests {
    ...
    private static WorldDefinition SeamDocument(float floorCenterZ) {
        ...
        return source with {
            ...
            Adjacencies = [new WorldAdjacency(
                SafeName.Parse(candidate: "seam"),
                "beyond",
                "seam",
                SeamBoundary(outwardYaw: ((floorCenterZ < 0f)
            ? 0f
            : 180f))
            )],
        };
    ...
    private static SeamWalker SeamWalk(WorldDefinition local, bool neighbourHasField = true, bool attachSource = true) {
        ...
        Assert.True(
            WorldAdjacencyPolicy.TryDeriveOverlap(
                depth: out var depth,
                local: local,
                neighbour: neighbourDefinition,
                reason: out var depthReason
            ),
            userMessage: depthReason
        );
        Assert.True(
            WorldAdjacencyPolicy.TryReciprocalHysteresis(
                definition: local,
                depth: out var hysteresis,
                reason: out var hysteresisReason
            ),
        ...
        if (attachSource) {
            fixture.Server.Adjacencies = new SeamSource(
                neighbour: new SeamNeighbour(
                    counterpartFrame: neighbourFrame,
                    definition: neighbourDefinition,
                    hasField: neighbourHasField
                ),
    ...
    public void DerivedCornerProjectionSuppliesGroundWhenBothDirectWorldsEnd() {
        ...
        source = source with {
            PlacementRowsRaw = [source.Placements[0] with { Position = new Vector3(
                x: 100f,
                y: 0f,
                z: 100f
            ) }],
            ...
            Destinations = [
                ...
                SafeName.Parse(candidate: "unused-east"),
                "unused-east-ref",
                WorldDestinationDurability.Persisted,
                WorldDestinationScope.Global
            ),
                new WorldDestination(
                SafeName.Parse(candidate: "unused-south"),
                "unused-south-ref",
                WorldDestinationDurability.Persisted,
                WorldDestinationScope.Global
            ),
            ...
            Adjacencies = [
                ...
                new WorldAdjacency(
                SafeName.Parse(candidate: "south"),
                "unused-south",
                "north",
                Boundary(yaw: 0f)
            ),
        ...
        fixture.Server.Body(index: actor.Index)!.Pose(
            pitchRadians: 0f,
            rollRadians: 0f,
            x: 0f,
            y: 3f,
        ...
        Assert.True(
            body.Grounded,
            userMessage: $"the body fell through the derived corner projection; y={body.Position.Y:0.###}"
        );
        Assert.True(
            (body.Position.Y > 0f),
            userMessage: $"the projected corner ground never arrested the fall; y={body.Position.Y:0.###}"
        );
    }
    ...
    public void StarvedNeighbourDeliveryLeavesTheLocalAnswerUntouched() {
        ...
        using var starved = SeamWalk(
            local: local,
            neighbourHasField: false
        );
        using var bare = SeamWalk(
            local: local,
            attachSource: false
        );
        ...
        for (var tick = 0; (tick < 75); tick++) {
            ...
            Assert.Equal(
                expected: bare.Body.FixedPosition,
                actual: starved.Body.FixedPosition
            );
            Assert.Equal(
                expected: bare.Body.Grounded,
                actual: starved.Body.Grounded
            );
        }
    }
    ...
    public void WalkingOffTheNeighboursFarRimStillFalls() {
        ...
        while (
            (walk.Body.Position.Z < (rim + 2f)) &&
            (walk.Body.Position.Y > (restingY - 1f))
        ) {
            walk.Step(forward: -FixedQ4816.One);
        }

        Assert.True(
            (walk.Body.Position.Z > rim),
            userMessage: $"the body stopped before the neighbour's far rim at z={walk.Body.Position.Z:0.###}"
        );
        Assert.True(
            (walk.Body.Position.Y < (restingY - 1f)),
            userMessage: $"the body was still supported past the neighbour's own floor; y={walk.Body.Position.Y:0.###}"
        );
    }
    ...
    public void WalkingOffTheOwnOuterRimStillFalls() {
        ...
        Assert.True(
            (walk.Body.Position.Y < (restingY - 1f)),
            userMessage: $"the body was supported past its own outer rim; y={walk.Body.Position.Y:0.###} z={walk.Body.Position.Z:0.###}"
        );
    }
    ...
    public void WalkingTheSeamStripRetainsGroundPastTheOwnershipThreshold() {
        ...
        while (walk.Body.Position.Z < (handoff + 4f)) {
            ...
            Assert.True(
                (walk.Body.Position.Y >= (restingY - GroundTolerance)),
            ...
            Assert.True(
                ((walk.Body.Position.Z <= 0f) || walk.Body.Grounded),
                userMessage: $"the body was airborne over the neighbour's floor at z={walk.Body.Position.Z:0.####}"
            );
        }

        Assert.True(
            (walk.Body.Position.Z > handoff),
            userMessage: "the walk never reached the handoff plane"
        );
    }
    ...
    private sealed class SeamWalker(WorldFixture fixture, WorldBody body) : IDisposable {
        ...
        public float Settle() {
            ...
            Assert.True(
                body.Grounded,
                userMessage: "the body never settled on its own floor"
            );
    ...
    private sealed class SeamSource : IWorldAdjacencySource {
        ...
        public WorldEntityAddress LocalEntityAddress(int index) => new(
            Authority: "source",
            Generation: 0,
            Index: index
        );
    ...
    private sealed class SeamNeighbour : IWorldAdjacencyNeighbourContact {
        ...
        public bool TryGetSolidField(out WorldSolidField? field, out string reason) {
            field = m_field;
            reason = ((m_field is null)
                ? "the neighbour has delivered no compiled field"
                : string.Empty
            );
