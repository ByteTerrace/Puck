...
public sealed class ChannelFrameLawTests {
    private static readonly WorldChannel Turn = new(
        Name: "turn",
    ...
    private static WorldChannel Forward(ChannelFrame frame) => new(
        Name: "forward",
        Shape: ChannelShape.Bipolar,
        Role: ChannelRole.MoveAdvance,
        Frame: frame
    );
    private static WorldChannel Strafe(ChannelFrame frame) => new(
        Name: "strafe",
        Shape: ChannelShape.Bipolar,
    ...
    public void FacingSnap_AnglesTheAttitudeTowardTravel_LeavesTheHeading_UntilMovementStops() {
        ...
        Assert.True(
            condition: (MathF.Abs(x: attitudeYaw) > 1f),
            userMessage: $"a full strafe should angle the attitude toward its travel; attitude yaw {attitudeYaw}"
        );
        ...
        Assert.True(
            condition: (MathF.Abs(x: (((float)((double)body.FixedYaw)) - attitudeYaw)) < 1e-3f),
            userMessage: $"at rest the heading adopts the way the body faced; heading {((double)body.FixedYaw):0.###}, attitude {attitudeYaw:0.###}"
        );
        Assert.True(
            condition: (MathF.Abs(x: (AttitudeYaw(orientation: body.Orientation) - attitudeYaw)) < 1e-3f),
            userMessage: "at rest the attitude holds"
        );
    }
    [Fact]
    ...
    public void TableReadsThePairsFrame_WorldWhenUndeclared() {
        ...
        Assert.Equal(
            expected: ChannelFrame.Heading,
            actual: framed.MoveFrame
        );
        Assert.Equal(
            expected: ChannelFrame.Heading,
            actual: framed.Frame(ordinal: framed.RoleOrdinals.MoveStrafe)
        );
        Assert.Equal(
            expected: ChannelFrame.World,
            actual: framed.Frame(ordinal: framed.RoleOrdinals.Turn)
        );
        Assert.Equal(
            expected: ChannelFrame.World,
            actual: bare.MoveFrame
        );
        Assert.Equal(
            expected: ChannelFrame.World,
            actual: WorldChannelTable.Empty.MoveFrame
        );
    }
}
