...
public sealed class WorldWindowProjectionMathLawTests {
    ...
    public void MapPoint_AsymmetricPair_AgreesWithWorldPortalArrivalMathsPositionMapping() {
        ...
        var destinationOrigin = new Vector3(
            x: 6f,
            y: 1.5f,
            z: -3f
        );
        var source = BuildFaceGeometry(
            origin: sourceOrigin,
            yawDegrees: -90f
        );
        var destination = BuildFaceGeometry(
        ...
        var mappedFloat = WorldWindowProjectionMath.MapPoint(
        ...
        var mappedFromArrival = WorldFrameIsometry.MapPoint(
            point: ToFixed(value: eye),
        ...
        Assert.True(
            condition: (Vector3.Distance(
                value1: mappedFloat,
                value2: mappedFromArrival
            ) < Tolerance),
            userMessage: $"MapPoint={mappedFloat} vs ComputeArrival.Position={mappedFromArrival} (source yaw -90, destination yaw 90) — a reflection bug mirrors this by several world units, far past the {Tolerance} tolerance."
        );
        ...
        Assert.True(
            condition: (Vector3.Distance(
                value1: mappedFloat,
                value2: reflectedWrong
            ) > 1f),
            userMessage: $"MapPoint={mappedFloat} matches the KNOWN-WRONG Normal-only-flip reflection {reflectedWrong} — the regression this law exists to catch."
        );
    }
    [Fact]
    public void MapPoint_IdentityBorder_ReturnsTheSameWorldPoint() {
        ...
        var eye = new Vector3(
            ...
            y: 2f,
            z: 0f
        ); // offset (-3, 1, 5) from origin: u=5, v=1, n=3 (all nonzero).

        var mapped = WorldWindowProjectionMath.MapPoint(
            destination: destination,
            point: eye,
            source: source
        );
        ...
        Assert.True(
            ...
            userMessage: $"identity-border MapPoint={mapped} should reproduce the original eye {eye} exactly (deltaYaw collapses to 0 for this yaw pair) — a Normal-only flip instead mirrors it laterally."
        );
    }
}
