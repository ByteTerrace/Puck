...
public sealed class WorldWindowProjectionMathLawTests {
    ...
    public void MapPoint_AsymmetricPair_AgreesWithWorldPortalArrivalMathsPositionMapping() {
        ...
        var sourceOrigin = new Vector3(-4f, 1.5f, 2f);
        var destinationOrigin = new Vector3(6f, 1.5f, -3f);
        ...
        var eye = new Vector3(-4f, 2.6f, 7f);
        ...
        var u = Vector3.Dot(offset, source.Right);
        var v = Vector3.Dot(offset, source.Up);
        var n = Vector3.Dot(offset, source.Normal);
    ...
    public void MapPoint_IdentityBorder_ReturnsTheSameWorldPoint() {
        ...
        var origin = new Vector3(2f, 1f, -5f);
        var source = BuildFaceGeometry(origin: origin, yawDegrees: -90f);
        var destination = BuildFaceGeometry(origin: origin, yawDegrees: 90f);
        var eye = new Vector3(-1f, 2f, 0f); // offset (-3, 1, 5) from origin: u=5, v=1, n=3 (all nonzero).

        var mapped = WorldWindowProjectionMath.MapPoint(point: eye, source: source, destination: destination);
