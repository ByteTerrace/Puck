...
public sealed class BatchComposeLawTests {
    ...
    private static string[] Keys(WorldDefinition definition, string row) => [.. Row(
    ...
    public void ABatchInstallsTheDocumentItsMembersReachOneByOne() {
        ...
        Assert.Equal(
            expected: 8,
            actual: Slot(
                definition: batched.Server.Definition,
                row: "counter"
            )
        );
        Assert.Equal(
        ...
        Assert.Equal(
            expected: "beta",
            actual: ComposedGroup(definition: batched.Server.Definition)
        );
        Assert.Equal(
            expected: "beta",
            actual: ComposedGroup(definition: sequential.Server.Definition)
        );
        Assert.Equal(
            expected: 3,
            actual: Row(
        ...
        Assert.Equal(
            expected: sequential.DefinitionBytes(),
