...
public sealed class GranaryContentLawTests {
    ...
    private static WorldDefinition GranaryModule() {
        ...
        host.Remove(propertyName: "state");
        host.Remove(propertyName: "placements");
        host.Remove(propertyName: "prototypes");
        host.Remove(propertyName: "navigation");
    ...
    public void CoverageRowsAreKeyedAndBoundedToTheDealCapacity() {
        ...
        foreach (var name in new[] { "granaryNames", "granaryWaterCoverage", "granaryPowerCoverage" }) {
            var row = Assert.Single(
                ...
                predicate: state => (state.Name.Value == $"{Alias}{name}")
            );
        ...
        Assert.Equal(
            expected: 7,
            actual: definition!.Rules!.Count(predicate: rule => rule.Name.ToString().StartsWith(
    ...
    public void GranaryModuleDeclaresCoverageProvidersAndControls() {
        ...
        Assert.Contains(
            collection: stores.Spatial!,
            filter: static volume => (volume.Role == WorldPlacementSpatialRole.Occupation)
        );
        Assert.Contains(
            collection: stores.Spatial!,
            filter: static volume => (volume.Role == WorldPlacementSpatialRole.Clearance)
        );
        ...
        Assert.Equal(
            expected: "water",
            actual: Assert.Single(collection: irrigation.Spatial!).Channel
        );
        Assert.Equal(
            expected: "power",
        ...
        Assert.Contains(
            collection: tablePrototype.Document.TextRuns!,
            filter: static run => (run.Text == "SURVEY TABLE")
        );

        Assert.Contains(
            collection: definition.Rules!,
            filter: rule => (rule.Name.ToString() == $"{Alias}granary-water-coverage")
        );
        Assert.Contains(
            collection: definition.Rules!,
            filter: rule => (rule.Name.ToString() == $"{Alias}granary-power-coverage")
        );
        Assert.Contains(
            collection: definition.Rules!,
            filter: rule => (rule.Name.ToString() == $"{Alias}granary-survey-cycle")
        );
    }
    [Fact]
    public void GranaryModuleLoadsThroughTheShippedNexus() {
        ...
        Assert.Contains(
            collection: definition.Rules!,
            filter: rule => (rule.Name.ToString() == $"{Alias}granary-irrigation-stage")
        );
    }
    [Fact]
    public void IrrigationRequestGrowsTheProviderThroughAnAtomicWorldRule() {
        ...
        var uncovered = fixture.Server.Definition.Placements
            ...
            .FirstOrDefault(predicate: placement => {
                if (
                    ...
                    !fixture.Server.Definition.PlacementFrames.TryGetValue(
                    key: placement.Id,
                    value: out var targetFrame
                )
                ) {
                    return false;
                }
                ...
                return (
                    (MathF.Abs(x: delta.X) <= 6.5f) &&
                    (MathF.Abs(x: delta.Z) <= 6.5f)
                );
            });
        ...
        var stage = WorldDefinitionRows.FindStateRow(
            fixture.Server.Definition.State,
            $"{Alias}granaryIrrigationStage"
        )!;
        ...
        Assert.Equal(
            expected: 1f,
            actual: provider.Scale
        );
        var coverage = Assert.Single(
            collection: provider.Spatial!,
            predicate: static volume => (volume.Name == "irrigation-coverage")
        );

        Assert.Equal(
            expected: 6.5f,
            actual: coverage.Shape.HalfExtents.X
        );
        Assert.True(condition: (fixture.Server.Definition.SpatialQueryIndex.CountInfluences(
            channel: "water",
            placementId: uncovered!.Id
        ) > 0));
        ...
        stage = WorldDefinitionRows.FindStateRow(
            fixture.Server.Definition.State,
            $"{Alias}granaryIrrigationStage"
        )!;
        Assert.Equal(
        ...
        foreach (var coverageName in new[] { "granaryWaterCoverage", "granaryPowerCoverage" }) {
            var coverageRow = WorldDefinitionRows.FindStateRow(
                fixture.Server.Definition.State,
                $"{Alias}{coverageName}"
            )!;

            Assert.DoesNotContain(
                collection: (coverageRow.Cells ?? []),
                filter: cell => (cell.Key.Value == removedKey)
            );
        }
    }
}
