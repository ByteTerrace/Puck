...
/// cref="CompiledTopology.TryCellOf"/>/<see cref="CompiledTopology.TryOffset"/>) and the physical-to-
...
public sealed class TabletopBoardLawTests {
    ...
    private static WorldDefinition TabletopBridgeDocument(bool excludeOffFrameMover = true) {
        ...
        WorldStateRow BoardRow(string name, bool seeded) => new(
            CellName.Parse(candidate: name),
            CellKind.Int,
        ...
        WorldStateRow Keyed(string name, long initial) => new(
            CellName.Parse(candidate: name),
            CellKind.Int,
            Min: -1,
            Max: 3,
        ...
        WorldStateRow Slot(string name, long initial) => new(
            CellName.Parse(candidate: name),
            CellKind.Int,
            Min: -1,
            Max: 5,
        ...
        WorldRule[] rules = [
            ...
            Rule(
                ...
                Set(
                    "pieceCell",
                    key: "0",
                    fromState: "$board:cellOf:board:body:0"
                ),
                Set(
                    "board",
            ...
            Rule(
                "mover-detect",
                All(MoverDetectPredicates(
                    excludeOffFrameMover: excludeOffFrameMover,
                    quiescentEqualsOne: quiescentEqualsOne
                )),
                ActionTriggerMode.Edge,
                Set(
                    "justMoved",
    ...
    private static WorldDefinition TwoPieceTabletopDocument(bool splitRules) {
        ...
        WorldStateRow BoardRow(string name) => new(
            CellName.Parse(candidate: name),
            CellKind.Int,
            Cells: [.. Enumerable.Range(
                    count: 4,
        ...
        WorldStateRow Keyed(string name, long initial, int capacity) => new(
            CellName.Parse(candidate: name),
            CellKind.Int,
            Min: -1,
            Max: 3,
        ...
        ActionEffect[] deriveEffects = [
            ...
            Set(
                "pieceCell",
                key: "1",
                fromState: "$board:cellOf:board:body:1"
            ),
            Set(
                "board",
                key: "$cell:pieceCell:1",
                value: 7m
            ),
        ];
        ...
        return source with {
            ...
            StateRaw = new WorldStateSection(
            Lattices: [topology],
            World: [BoardRow(name: "board"), Keyed(
                    "pieceCell",
                    -1,
                    capacity: 2
                )]
    ...
    public void GridFrameRoundTripsPositionToCellAndRejectsOutOfBoundsAndWrongKind() {
        var grid = new LatticeTopology.Grid(
            "board",
            new DocumentVector3(
                x: 10f,
                y: 2f,
        ...
        for (var z = 0; (z < 4); z++) {
            for (var x = 0; (x < 4); x++) {
                var center = new FixedVector3(
                    X: FixedQ4816.FromDouble(value: (10d + ((x + 0.5) * 0.5))),
                    Y: FixedQ4816.FromDouble(value: 2d),
                    Z: FixedQ4816.FromDouble(value: (-5d + ((z + 0.5) * 0.5)))
                );

                Assert.True(condition: topology.TryCellOf(
                    cell: out var resolved,
                    position: center
                ));
                Assert.Equal(
                    actual: resolved,
        ...
        Assert.True(condition: topology.TryCellOf(
            position: new FixedVector3(
                X: FixedQ4816.FromDouble(value: 10.25d),
                Y: FixedQ4816.FromDouble(value: 502d),
                Z: FixedQ4816.FromDouble(value: -4.75d)
            ),
            cell: out var highCell
        ));
        Assert.Equal(
            actual: highCell,
            expected: 0
        ...
        Assert.True(condition: topology.TryCellOf(
            position: new FixedVector3(
                X: FixedQ4816.FromDouble(value: 10.25d),
                Y: FixedQ4816.FromDouble(value: -498d),
                Z: FixedQ4816.FromDouble(value: -4.75d)
            ),
            cell: out var lowCell
        ));
        Assert.Equal(
            actual: lowCell,
            expected: 0
        );
        var banded = TopologyCompilation.Find(
            new WorldStateSection(Lattices: [.. state.Lattices!.Select(selector: t => ((t.Name == "board")
            ? ((LatticeTopology.Grid)t) with { Band = 0.3f }
            : t))]),
            "board"
        )!;

        Assert.True(condition: banded.TryCellOf(
            position: new FixedVector3(
                X: FixedQ4816.FromDouble(value: 10.25d),
                Y: FixedQ4816.FromDouble(value: 2.2d),
                Z: FixedQ4816.FromDouble(value: -4.75d)
            ),
            cell: out var nearCell
        ));
        Assert.Equal(
            actual: nearCell,
            expected: 0
        );
        Assert.False(condition: banded.TryCellOf(
            position: new FixedVector3(
                X: FixedQ4816.FromDouble(value: 10.25d),
        ...
        Assert.False(condition: TopologyCompilation.TryValidate(
            ((LatticeTopology.Grid)state.Lattices![0]) with { Band = -1f },
            out var bandReason
        ));
        Assert.Contains(
            actualString: bandReason,
            expectedSubstring: "band"
        );

        // Below the origin corner: outside the frame on both axes.
        Assert.False(condition: topology.TryCellOf(
            position: new FixedVector3(
                X: FixedQ4816.FromDouble(value: 9d),
                Y: FixedQ4816.Zero,
                Z: FixedQ4816.FromDouble(value: -6d)
            ),
            cell: out _
        ));
        ...
        Assert.False(condition: topology.TryCellOf(
            position: new FixedVector3(
                X: FixedQ4816.FromDouble(value: 12.1d),
                Y: FixedQ4816.Zero,
                Z: FixedQ4816.FromDouble(value: -4.75d)
            ),
            cell: out _
        ...
        Assert.True(condition: topology.TryOffset(
            cell: 0,
            dx: 1,
            dz: 2,
            result: out var leap
        ...
        Assert.False(condition: topology.TryOffset(
            ...
            dz: 0,
            result: out _
        )); // off the west edge, no wrap declared
        Assert.False(condition: topology.TryOffset(
            cell: 3,
            dx: 1,
            dz: 0,
        ...
        var hexTopology = TopologyCompilation.Find(
            name: "hex",
            section: state
        )!;

        Assert.True(condition: hexTopology.TryCellOf(
            position: new FixedVector3(
                X: FixedQ4816.Zero,
                Y: FixedQ4816.Zero,
                Z: FixedQ4816.Zero
            ),
            cell: out var hexOrigin
        ));
        Assert.Equal(
            actual: hexOrigin,
            expected: 0
        );
        Assert.True(condition: hexTopology.TryOffset(
            cell: 0,
            dx: 1,
            dz: 0,
            result: out var hexEast
        ));
        Assert.Equal(
            actual: hexEast,
            expected: 1
        );
        Assert.False(condition: hexTopology.TryOffset(
            cell: 1,
            dx: 1,
            dz: 0,
            result: out _
        )); // off the radius-1 disk
    }
    [Fact]
    ...
    public void OneOffFrameSiblingNeverCostsTheOtherPieceItsDeriveWhateverTheRuleShape(bool splitRules) {
        ...
        Assert.Null(@object: fixture.Server.Body(index: 1)); // never joined — body:1 stays inactive, cellOf answers -1.
        ...
        Assert.Equal(
            actual: cell0,
            expected: 9
        ); // piece 1's off-frame write is refused alone — piece 0 still derives, in either shape.
    }
    ...
    public void PieceLeavingTheTopologyEntirelyNeverRegistersAsItsOwnMover(bool excludeOffFrameMover) {
        ...
        body.Pose(
            ...
            yawRadians: 0f,
            z: 0.5f
        ); // over cell 0, on the topology
        for (var tick = 0; (tick < 400); tick++) {
            fixture.Step();
        }

        WorldStateRow Row(string name) => WorldDefinitionRows.FindStateRow(
            fixture.Server.Definition.State,
            name
        )!;
        ...
        ); // settled, on the topology — the baseline both variants share.
        var verdictBefore = Cell(
            Row(name: "verdict"),
            WorldStateRow.SlotKey
        );
        var illegalCountBefore = Cell(
            Row(name: "illegalCount"),
            WorldStateRow.SlotKey
        );
        var lastLegalCell0Before = Cell(
            Row(name: "lastLegal"),
            "0"
        );
        ...
        body.Pose(
            ...
            yawRadians: 0f,
            z: 10f
        ); // still on the floor, off the 2x2 topology
        for (var tick = 0; (tick < 400); tick++) {
            fixture.Step();
        }
        ...
        if (excludeOffFrameMover) {
            ...
            ); // ruled legal — nothing checks a destination of "no cell".
            Assert.Equal(
                illegalCountBefore,
                Cell(
                    Row(name: "illegalCount"),
            ...
            ); // the piece is erased from the remembered legal position.
        }
    }
    [Fact]
    public void RestingBodyDerivesOccupancyAndAnIllegalDestinationLeavesLastLegalUntouched() {
        ...
        ); // still the seeded initial position — nothing moved yet.
        ...
        Set(
            ...
            key: "1",
            value: 9
        ); // the physical write piece-derive would have made, landing on cell 1.
        ...
        Set(
            "pieceCell",
            2
        ); // cell 2 is empty in previousBoard — a legal destination.
        ...
        ); // NOW lastLegal adopts the board this legal move left behind.
    }
