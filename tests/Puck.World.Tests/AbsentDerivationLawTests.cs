...
public sealed class AbsentDerivationLawTests {
    ...
    public void Capacity_Absent_DerivesFromLocalSeatsPlusNetworkPlayers() {
        ...
        Assert.Equal(
            expected: 5,
            actual: definition.Population.Capacity
        );
    }
    [Fact]
    public void Channels_Absent_ResolvesToNone() {
        ...
        Assert.Contains(
            expectedSubstring: "requires channel role 'MoveAdvance'",
    ...
    public void CreationWithANonIdentityQuaternion_RoundTripsThroughCanonicalizeWithUnchangedBytes() {
        var document = new CreationDocument(
            ...
            Palette: [new PaletteEntryDocument(
                    Color: "#CCCCCC",
                    Emissive: 0,
                    Specular: 0,
                    Roughness: 0
    ...
    public void DefaultSeatKit_Absent_DerivesFromTheSoleKit() {
              ...
              "kits": { "rows": [
                {
                  "name": "solo",
                  "bodyMotionProgram": "p",
                  "motion": { "speed": { "value": 4 }, "turn": { "rate": 2.5 }, "holds": [ { "name": "air", "bond": "Free", "hold": "Gravity", "gravity": { "rise": 28, "fall": 46 }, "envelope": { "sinkSpeed": 40 } } ], "shaping": [ { "along": {} } ] }
                }
              ] }
            }
            """);
        ...
        Assert.Equal(
            expected: "solo",
            actual: definition.DefaultSeatKit
        );
    }
    [Fact]
    ...
    public void LocalSeats_Absent_DerivesFromDeclaredSeatSpawnsRowCount() {
        ...
        Assert.Equal(
            expected: 2,
            actual: definition.Population.LocalSeats
        );
        Assert.Equal(
            expected: 2,
            actual: definition.Population.Capacity
    ...
    public void NullSeatWorld_DeserializesAndValidates() {
              ...
              "kits": { "rows": [
                {
                  ...
                  "motion": {
                    ...
                    "turn": { "rate": 2.5 },
                    "holds": [
                      { "name": "air", "bond": "Free", "hold": "Gravity", "gravity": { "rise": 28, "fall": 46 }, "envelope": { "sinkSpeed": 40 } }
                    ],
                    "shaping": [ { "along": {} } ]
                  },
        ...
        Assert.Equal(
            expected: SeatActivationPolicy.Eager,
            actual: Assert.Single(collection: definition.Population.SeatActivation)
        );
        Assert.Equal(
            expected: WorldSpawnPointDefaults.ImplicitOriginId,
            actual: Assert.Single(collection: definition.Population.SeatSpawns)
        );
    }
    [Fact]
    ...
    public void Views_Absent_RequiredOnlyWhenPopulationImpliesABody() {
        ...
        Assert.Contains(
            expectedSubstring: "views is required",
            actualString: exception.Message,
            comparisonType: StringComparison.Ordinal
        );
    }
