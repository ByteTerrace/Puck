...
public sealed class DeliveredDocumentIdentifierLawTests {
    ...
    private static WorldDefinition Delivered(WorldDefinition live, WorldDisclosureTier tier) {
        Assert.True(
            condition: WorldFederationCodec.TryDecodeDocument(
    ...
    private static string DeliveredGroup(WorldDefinition live, WorldDisclosureTier tier) {
        ...
        var composed = WorldBindingComposer.Compose(Delivered(
            live: live,
            tier: tier
        ).BindingOverlays[0].Document);
    ...
    private static WorldDefinition Live(string groupName) {
        ...
        Assert.Equal(
            expected: GroupReference,
            actual: live.BindingOverlays[0].Document.Chords[0].Group.Reference
        );
        Assert.Equal(
            expected: groupName,
            actual: live.BindingOverlays[0].Document.Chords[0].Group.Value
        );
    ...
    public void DeliveredDefinitionResolvesItsIdentifier_ControlOneValueDifferent(WorldDisclosureTier tier) {
        Assert.Equal(
            ...
            actual: DeliveredGroup(
                live: Live(groupName: "actionGroup"),
                tier: tier
            )
        );
        Assert.Equal(
            expected: "renamedActionGroup",
            actual: DeliveredGroup(
                live: Live(groupName: "renamedActionGroup"),
                tier: tier
            )
        );
    }
    ...
    public void PeerProjectionNamingAStateCell_Refuses_ControlLiteralClean() {
        ...
        Laws.RefusalWithControl(
            ...
            deniedOutcome: () => WorldFederationCodec.TryDecodeDocument(
    ...
    public void PresentationEgressCarriesNoStateReference() {
        ...
        Assert.DoesNotContain(
            actualString: json,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: GroupReference
        );
        Assert.Contains(
            actualString: json,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "\"actionGroup\""
        );
    }
}
