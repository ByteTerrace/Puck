...
/// pair can be refused, per docs/architecture/worlds.md's "Adjacency and crossing" section: <see cref="WorldDefinitionValidator"/> at BOOT (arrival/counterpart
...
public sealed class PortalArrivalValidationLawTests {
    ...
    private static WorldDefinition BuildTwoFaceDocument(bool secondFaceCarriesPortal) {
        ...
        var behavior = creation.Document.Behavior! with {
            Faces = [.. creation.Document.Behavior!.Faces!, new CreationFaceDocument(
                DefaultSource: null,
                Name: "back",
                ShapeId: null
            )],
        ...
        var canonical = CreationCanonicalizer.Canonicalize(
            document: (creation.Document with { Behavior = behavior }),
            source: "door"
        );
        var twoFaced = new WorldPrototype(
            Id: creation.Id,
            Document: canonical.Document,
            HashRaw: canonical.Hash
        );
        ...
        var placement = new WorldPlacement(
            Id: DoorPlacementId,
            PrototypeId: twoFaced.Id,
            Position: Vector3.Zero,
            YawDegrees: 0f,
            Scale: 1f,
            FaceSources: [
                new WorldPlacementFace(
                    Face: DoorFace,
                    Source: new WorldScreenSource.None(),
                    Portal: portal
                ),
                new WorldPlacementFace(
                    Face: "back",
                    Source: new WorldScreenSource.None(),
                    Portal: (secondFaceCarriesPortal
            ? portal
            : null)
                ),
            ]
    ...
    private static bool TryValidate(WorldPortalArrival arrival, string? counterpart) {
        var bytes = WorldDefinitionSerialization.Serialize(definition: BuildPortalDocument(
            arrival: arrival,
            counterpart: counterpart
        ));
    ...
    public void Counterpart_WithoutMappedArrival_RefusesByName() {
        var exception = Assert.Throws<InvalidDataException>(testCode: () => WorldDefinitionSerialization.Deserialize(utf8Json: WorldDefinitionSerialization.Serialize(definition: BuildPortalDocument(
            arrival: WorldPortalArrival.Spawn,
            counterpart: $"{DoorPlacementId}/{DoorFace}"
        ))));

        Assert.Contains(
            expectedSubstring: "counterpart",
            actualString: exception.Message,
            comparisonType: StringComparison.Ordinal
        );
        Assert.Contains(
            expectedSubstring: "arrival is not 'mapped'",
            actualString: exception.Message,
            comparisonType: StringComparison.Ordinal
        );
    }
    ...
    public void Counterpart_WithoutMappedArrival_RefusesByName_ControlParsesClean() {
        Laws.RefusalWithControl(
            ...
            deniedOutcome: static () => TryValidate(
                arrival: WorldPortalArrival.Spawn,
                counterpart: $"{DoorPlacementId}/{DoorFace}"
            ),
            controlOutcome: static () => TryValidate(
                arrival: WorldPortalArrival.Spawn,
                counterpart: null
            )
        );
    ...
    public void MalformedCounterpart_RefusesByName_ControlParsesClean() {
        Laws.RefusalWithControl(
            ...
            deniedOutcome: static () => TryValidate(
                ...
                counterpart: "not-a-placement-slash-face"
            ),
            controlOutcome: static () => TryValidate(
                arrival: WorldPortalArrival.Mapped,
                counterpart: $"{DoorPlacementId}/{DoorFace}"
    ...
    public void MappedArrival_WithoutCounterpart_RefusesByName() {
        ...
        Assert.Contains(
            expectedSubstring: "arrival 'mapped' requires",
            actualString: exception.Message,
            comparisonType: StringComparison.Ordinal
        );
    }
    ...
    public void SecondPortalFaceOnOnePlacement_ControlWithOnePortalFaceValidates() {
        Assert.True(
            condition: WorldDefinitionValidator.TryValidate(
                definition: BuildTwoFaceDocument(secondFaceCarriesPortal: false),
    ...
    public void SecondPortalFaceOnOnePlacement_RefusesByName() {
        ...
        Assert.False(
            condition: WorldDefinitionValidator.TryValidate(
                definition: BuildTwoFaceDocument(secondFaceCarriesPortal: true),
                reason: out var reason,
                neighbours: null
            ),
            userMessage: "a second portal face was expected to refuse"
        );
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "second portal face"
        );
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: DoorPlacementId
        );
    }
    ...
    public void TryParse_MalformedShapes_Refuse(string? counterpart) {
        Assert.False(condition: WorldPortalCounterpart.TryParse(
            counterpart: counterpart,
            face: out _,
            placementId: out _
        ));
    ...
    public void TryParse_WellFormed_SplitsAtFirstSlash() {
        Assert.True(condition: WorldPortalCounterpart.TryParse(
            counterpart: "placement-a/face/with/slashes",
            face: out var face,
    ...
    public void TryResolve_MissingFace_RefusesNamingIt() {
        ...
        var resolved = WorldPortalCounterpart.TryResolve(
            counterpart: $"{DoorPlacementId}/no-such-face",
        ...
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "no-such-face"
        );
    }
    [Fact]
    public void TryResolve_MissingPlacement_RefusesNamingIt() {
        ...
        var resolved = WorldPortalCounterpart.TryResolve(
            counterpart: $"no-such-placement/{DoorFace}",
            definition: definition,
            face: out var face,
            placement: out var placement,
            reason: out var reason
        ...
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "no-such-placement"
        );
    }
    [Fact]
    ...
    public void WellFormedMappedPortal_ValidatesClean() {
        ...
        _ = WorldDefinitionSerialization.Deserialize(utf8Json: WorldDefinitionSerialization.Serialize(definition: BuildPortalDocument(
            arrival: WorldPortalArrival.Mapped,
            counterpart: $"{DoorPlacementId}/{DoorFace}"
        )));
    }
