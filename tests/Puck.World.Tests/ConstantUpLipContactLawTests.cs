...
public sealed class ConstantUpLipContactLawTests {
        ...
        return source with {
            ...
            KitRowsRaw = source.Kits.Select(selector: kit => kit with {
                Motion = kit.Motion with {
                    ...
                    Holds = [
                        kit.Motion.Holds![0] with { Envelope = new WorldHoldEnvelope(SinkSpeed: 40f), Gravity = new WorldHoldGravity(
                Fall: 46f,
                Rise: 28f
            ) },
                    ],
    ...
    public void GroundedBodyUnderConstantUp_StaysYawOnlyAcrossTheLip_AndFalls() {
        ...
        Assert.True(
            condition: body.Grounded,
            userMessage: $"the body did not settle grounded on the platform top; y={body.Position.Y:0.###}"
        );
        ...
        for (var tick = SettleTicks; (tick < TotalTicks); tick++) {
            ...
            body.SubmitIntent(intent: ((releasedAt < 0)
                ? default(PlayerIntent).WithChannel(
            ...
            if (airborneAt < 0) {
                ...
                Assert.True(
                    condition: ((tick - releasedAt) <= AirborneWithinTicks),
                    userMessage: $"the body stayed grounded {AirborneWithinTicks} ticks past the lip — pinned at ({body.Position.X:0.##}, {body.Position.Y:0.##}, {body.Position.Z:0.##}) instead of falling"
                );
            } else {
                Assert.False(
                    condition: body.Grounded,
                    userMessage: $"tick {tick}: the body re-grounded mid-fall at ({body.Position.X:0.##}, {body.Position.Y:0.##}, {body.Position.Z:0.##}) — a vertical face may push but never grounds"
                );
                Assert.True(
                    condition: (body.FixedPosition.Y < previousY),
                    userMessage: $"tick {tick}: y={body.Position.Y:0.###} did not descend from {((double)previousY):0.###} while falling"
                );
            }
        ...
        Assert.True(
            condition: (releasedAt >= 0),
            userMessage: $"the walk never reached the release point z<={((double)ReleaseZ):0.##}; final z={body.Position.Z:0.###}"
        );
        Assert.True(
            condition: (airborneAt >= 0),
            userMessage: "the body never left the ground state after the lip"
        );
        Assert.True(
            condition: (body.Position.Y < -5f),
            userMessage: $"the body fell only to y={body.Position.Y:0.###} in {(TotalTicks - airborneAt)} airborne ticks"
        );
    }
}
