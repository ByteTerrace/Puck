...
public sealed class TetherLawTests {
    ...
    private static void AssertOnWallFace(FixedVector3? anchor) {
        ...
        Assert.Equal(
            ...
            actual: anchor.Value.Y
        );
        Assert.Equal(
            expected: FixedQ4816.Zero,
            actual: anchor.Value.Z
        );
    }
    private static PlayerIntent Attach() => default(PlayerIntent).WithChannel(
        ordinal: AttachOrdinal,
    ...
    private static PlayerIntent Detach() => default(PlayerIntent).WithChannel(
    ...
    public void AttachChannelNamingAnUndeclaredChannel_RefusesValidation_WhereADeclaredNameIsAdmitted() {
        var denied = BuildTetherDocument(
            attachChannelName: "no-such-channel",
            declareAttachChannel: false
        );
        ...
        Assert.Contains(
            ...
            expectedSubstring: "attachChannel"
        );
    ...
    public void Detach_ScalesSurvivingVelocityByReleaseVelocityScale_WhereAUnitScalePreservesIt() {
        var seeded = new FixedVector3(
            X: FixedQ4816.FromInteger(value: 6L),
            Y: FixedQ4816.Zero,
            Z: FixedQ4816.FromInteger(value: 2L)
        );
        ...
        scaledBody.SetTetherToWorldPoint(
            anchor: new FixedVector3(
                X: FixedQ4816.Zero,
                Y: FixedQ4816.Zero,
                Z: FixedQ4816.FromInteger(value: -5L)
            ),
        ...
        Assert.Equal(
            expected: FixedQ4816.FromInteger(value: 3L),
            actual: scaledSurvivor.PlanarVelocity.X
        );
        Assert.Equal(
            expected: FixedQ4816.FromInteger(value: 1L),
            actual: scaledSurvivor.PlanarVelocity.Z
        );
        Assert.Equal(
            expected: FixedQ4816.FromInteger(value: -2L),
            actual: scaledSurvivor.VerticalVelocity
        );
        ...
        Assert.Equal(
            expected: seeded,
            actual: unitSurvivor.PlanarVelocity
        );
        Assert.Equal(
            expected: seededVertical,
            actual: unitSurvivor.VerticalVelocity
        );
    }
    [Fact]
    ...
    public void HeldReelIn_ShrinksTheRopeThroughTheTickPathAndClampsAtMinLength() {
        ...
        Assert.Equal(
            ...
            actual: body.TetherMinLength
        );
        Assert.True(
            condition: (attachedLength!.Value > FixedQ4816.One),
            userMessage: $"expected an initial rope longer than minLength to give the reel something to shrink; got {attachedLength}"
        );
    ...
    public void LiveTetherToTetherKitSwap_ClearsALiveAttach_WhereAKitSwapKeepingTheSameFacetDoesNot() {
        ...
        var changedDocument = BuildTetherDocument(
            includeWall: false,
            modeState: "tethered"
        );
        ...
        Assert.True(condition: changedBody.TryDescribeActionState(
            kind: out _,
            lifetime: out _,
            name: "tethered",
            playerWritable: out _,
            timerTicks: out _,
            value: out var clearedMode
        ));
        Assert.Equal(
            expected: FixedQ4816.Zero,
            actual: clearedMode
    ...
    public void ModeStateNamingATimerSlot_RefusesValidation() {
        var denied = BuildTetherDocument(
            modeState: "tethered",
            modeStateRowKind: ActionStateKind.Timer
        );
    ...
    public void PlacementGripWithoutSolid_RefusesValidation_WhereGripPairedWithSolidIsAdmitted() {
        ...
        var ungripped = document with {
            ...
            PlacementsRaw = (document.PlacementsRaw! with {
                ...
                Scale: 1f,
                Solid: null,
                Grip: new WorldPlacementGrip(Holdable: true)
            ),
                ],
            }),
        };
    ...
    public void SetTetherToBody_DragsTheTetheredBodyThroughTheRealPopulationPipeline() {
        ...
        for (var tick = 0; (tick < 240); tick++) {
            ...
            anchorBody.SubmitIntent(intent: default(PlayerIntent).WithChannel(
                ordinal: ForwardOrdinal,
                value: FixedQ4816.One
            ));
    ...
    public void TetherCheckpointRestore_PreservesRopeAnchorAndReelFractionAndContinuesBitIdentically() {
        ...
        Assert.Equal(
            expected: uninterruptedBody.CaptureIntegrationResidue(),
            actual: restoredBody.CaptureIntegrationResidue()
        );
        ...
        for (var step = 0; (step < 3); step++) {
            ...
            Assert.Equal(
                expected: uninterruptedBody.TetherLength,
                actual: restoredBody.TetherLength
            );
            Assert.Equal(
                expected: WorldReplaySnapshot.HashState(population: fixture.Server.Population),
                actual: WorldReplaySnapshot.HashState(population: restored.Server.Population)
            );
            Assert.Equal(
                expected: uninterruptedBody.CaptureIntegrationResidue(),
                actual: restoredBody.CaptureIntegrationResidue()
    ...
    public void TetherModeStateFlipsThroughTheOrdinaryAttachAndDetachChannelEdges() {
        ...
        Assert.True(condition: body.TryDescribeActionState(
            ...
            playerWritable: out _,
            timerTicks: out _,
            value: out var initial
        ));
        Assert.Equal(
            actual: kind,
            expected: ActionStateKind.Counter
        );
        Assert.Equal(
            expected: FixedQ4816.Zero,
            actual: initial
