...
public sealed class IdentityFactsLawTests(ITestOutputHelper output) {
    ...
    public void EffectWritesLaneAndPersistedRowTogether_LaneZeroesWhenSeatLeaves() {
        ...
        Assert.Equal(
            expected: identity.FactsDefinition.State,
            actual: persisted.Name
        );
        Assert.Equal(
            expected: identity.FactsDefinition.Capacity,
            actual: persisted.Capacity
        );
        Assert.Equal(
            expected: 1L,
        ...
        Assert.Equal(
            expected: 1L,
            actual: PersistedFact(
                directory: fixture.Server.Profiles.FilePath,
                identity: IdentityName,
                fact: Fact
            )
        );
        ...
        var other = Assert.IsType<WorldIdentity>(@object: fixture.Server.Profiles.Create(
            name: SafeName.Parse(candidate: OtherIdentityName),
            colorHex: "#336699",
            reason: out _
        ));

        Assert.True(condition: fixture.Server.ApplySession(request: new SessionRequest.SetIdentity(
            Principal: Seat,
            Slot: 0,
    ...
    public void SeatDrivingUnderNoIdentityIsRefusedNotMinted() {
        ...
        Assert.Equal(
            expected: WorldRuleEffectRefusal.IdentityUnbound,
    ...
    public void UnchangedWriteCostsAnOrdinaryWrite_ChangedWriteIsBoundedByThePersistedDocument() {
        ...
        Assert.Equal(
            actual: steadySamples.Median,
            expected: steadyControlSamples.Median
        );
        Assert.True(
            condition: (beyond <= (32L * documentBytes)),
            userMessage: $"a changing fact allocated {beyond:N0} bytes beyond an ordinary changing write, against a {documentBytes:N0}-byte persisted document"
        );
        Assert.Equal(
            expected: ((long)(moving.Server.NextInputTick - 1UL)),
            actual: Lane(
                body: 0,
                fact: "ticks",
                fixture: moving
    ...
    private static void Join(WorldFixture fixture, string? identity) {
        var reply = fixture.Server.ApplySession(request: new SessionRequest.Join(
        ...
        Assert.Equal(
            expected: identity,
            actual: fixture.Server.Body(index: 0)?.Profile?.Id
        );
    }
    ...
    private static WorldStateRow LaneRow() => new(
        Name: CellName.Parse(candidate: WorldIdentityFactLane.RowName),
        Kind: CellKind.Int,
        Capacity: LaneCapacity
    );
    private static WorldStateRow IntSlot(string name) => new(
    ...
    private static IReadOnlyList<StateCell> LaneCells(WorldFixture fixture) => (WorldDefinitionRows.FindStateRow(
    ...
    private static long Lane(WorldFixture fixture, int body, string fact) {
        var cell = StateRows.FindCell(
            cells: LaneCells(fixture: fixture),
