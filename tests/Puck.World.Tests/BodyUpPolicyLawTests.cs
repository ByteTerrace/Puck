...
public sealed class BodyUpPolicyLawTests {
    ...
    public void Ambient_FollowsTiltedSolvedGravityWithoutSurfaceFollowing() {
        using var fixture = Fixtures.FreshServer(definition: WithFlankGravity(definition: Fixtures.BuildGradientUpDocument(gradientUp: false)));
        ...
        AssertNotYawOnly(
            body: body,
            context: "ambient frame under tilted solved gravity"
        );
    }
    [Fact]
    public void LiveRebuildFromSurfaceFollowingToAmbient_ReseatsUpOnTheNextStep() {
        ...
        Assert.True(
            ...
            userMessage: "the surface-following control never grounded on the ball's flank"
        );
        AssertNotYawOnly(
            body: body,
            context: "surface-following control"
        );
        ...
        AssertYawOnly(
            body: body,
            context: "first Ambient step after the live rebuild"
        );
    }
}
