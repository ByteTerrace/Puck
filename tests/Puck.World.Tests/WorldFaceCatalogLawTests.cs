...
public sealed class WorldFaceCatalogLawTests {
    ...
    private static Vector3 ShapeOffset { get; } = new(
        x: 0.75f,
        y: 0.25f,
        z: -0.5f
    );
    private static Vector3 ShapeScale { get; } = new(
        x: 3f,
        y: 2f,
        z: 0.15f
    );
    // A rotation about the face's own RIGHT axis — pitch, the axis a world-up-pinned frame cannot represent.
    private static Quaternion Pitched { get; } = Quaternion.CreateFromAxisAngle(
        axis: Vector3.UnitX,
        angle: (MathF.PI / 6f)
    );
    ...
    private static WorldPrototype BuildDoorCreation(SdfSolidPrimitive faceShape = SdfSolidPrimitive.Box, bool faceNamesShape = true, bool animated = false, bool pitched = false) {
        ...
        var canonical = CreationCanonicalizer.Canonicalize(
            document: CreationFrame.ToAuthor(document: document),
            source: "door"
        );
    ...
    private static WorldDefinition BuildDoorDocument(
        ...
        var creation = BuildDoorCreation(
            animated: animated,
            faceNamesShape: faceNamesShape,
            faceShape: faceShape,
            pitched: pitched
        );
        var portal = new WorldPlacementPortal(
        ...
        var placement = new WorldPlacement(
            ...
            FaceSources: [new WorldPlacementFace(
                    Face: DoorFace,
                    Source: (source ?? new WorldScreenSource.None()),
                    Portal: (carriesPortal
            ? portal
            : null)
                )],
            Attach: attach
    ...
    private static WorldFaceRow DoorRow(WorldDefinition definition) {
        Assert.True(condition: WorldFaceCatalog.For(definition: definition).TryFind(
            placementId: DoorPlacementId,
            faceName: DoorFace,
            out var row
        ));
    ...
    private static bool Validates(WorldDefinition definition) => WorldDefinitionValidator.TryValidate(
    ...
    public void AFaceWithNoApertureStillDrawsWithoutAPortal() {
        ...
        Assert.True(condition: Validates(definition: BuildDoorDocument(
            faceShape: SdfSolidPrimitive.Sphere,
            carriesPortal: false
        )));
        Assert.Null(@object: DoorRow(definition: BuildDoorDocument(
            faceShape: SdfSolidPrimitive.Sphere,
            carriesPortal: false
        )).Aperture);
    }
    ...
    public void APitchedFaceIsNotYawOnly_AndALevelOneIs() {
        ...
        Assert.True(condition: DoorRow(definition: BuildDoorDocument(
            yawDegrees: 25f,
            carriesPortal: false
        )).Frame.IsYawOnly);
        Assert.False(condition: DoorRow(definition: BuildDoorDocument(
            yawDegrees: 25f,
            carriesPortal: false,
            pitched: true
        )).Frame.IsYawOnly);
    ...
    public void APortalOnAPitchedFace_RefusesByName() {
        Assert.False(condition: WorldDefinitionValidator.TryValidate(
            definition: BuildDoorDocument(pitched: true),
        ...
        Assert.Contains(
            ...
            expectedSubstring: "pitch or roll"
        );
    }
    [Fact]
    public void APortalOnAPitchedFace_RefusesByName_YawOnlyControlValidates() {
        Laws.RefusalWithControl(
            ...
            deniedOutcome: static () => Validates(definition: BuildDoorDocument(
                yawDegrees: 25f,
                pitched: true
            )),
    ...
    public void APortalOnAShapeKindWithNoApertureMapping_RefusesByName(SdfSolidPrimitive faceShape) {
        ...
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "aperture"
        );
        Assert.Contains(
            expectedSubstring: faceShape.ToString(),
            actualString: reason,
            comparisonType: StringComparison.Ordinal
        );
    }
    ...
    public void APortalOnAnAttachedPlacement_RefusesByName() {
        var attached = BuildDoorDocument(attach: new WorldPlacementAttach(
            BodyIndex: 0,
        ...
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "ATTACHES"
        );
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "portal"
        );
    }
    [Fact]
    public void APortalOnAnAttachedPlacement_RefusesByName_ControlValidates() {
        Laws.RefusalWithControl(
            ...
            deniedOutcome: static () => Validates(definition: BuildDoorDocument(attach: new WorldPlacementAttach(
                BodyIndex: 0,
                LocalOffset: Vector3.Zero
            ))),
            controlOutcome: static () => Validates(definition: BuildDoorDocument())
    ...
    public void ARestingWorldDerivesNoCrossingFloor() {
        ...
        Assert.Equal(
            ...
            actual: WorldFacePortalPolicy.CrossingFloor(definition: definition)
        );
    }
    [Fact]
    ...
    public void MoreLiveFacesThanReservedSlots_RefusesByName() {
        Assert.False(condition: WorldDefinitionValidator.TryValidate(
            definition: BuildBudgetDocument(
                liveFaces: 3,
                reservedSlots: 2
            ),
            reason: out var reason,
        ...
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "derivedFaceScreens"
        );
    }
    [Fact]
    public void MoreLiveFacesThanReservedSlots_RefusesByName_AtBudgetControlBoots() {
        Laws.RefusalWithControl(
            ...
            deniedOutcome: static () => Validates(definition: BuildBudgetDocument(
                liveFaces: 3,
                reservedSlots: 2
            )),
            controlOutcome: static () => Validates(definition: BuildBudgetDocument(
                liveFaces: 2,
                reservedSlots: 2
            ))
        );
    ...
    public void RotatingThePlacementMovesTheWholeFrame() {
        ...
        Assert.NotEqual(
            expected: level.Origin,
            actual: turned.Origin
        );
        Assert.NotEqual(
            expected: level.Normal,
            actual: turned.Normal
        );
        Assert.NotEqual(
            expected: level.Right,
            actual: turned.Right
        );
        Assert.Equal(
            expected: level.Up,
        ...
        Assert.Equal(
            expected: level.HalfWidth,
            actual: turned.HalfWidth
        );
        ...
        Assert.True(
            condition: (Math.Abs(value: (turn - (Math.PI / 6.0))) < 0.001),
            userMessage: $"the frame heading turned by {turn} radians, not pi/6"
        );
    }
    [Fact]
    public void SlotExhaustionDarkensTheNewestClaimant_ButNeverRemovesItsGeometry() {
        ...
        var catalog = WorldFaceCatalog.For(definition: BuildBudgetDocument(
            liveFaces: 3,
            reservedSlots: 2
        ));
        ...
        Assert.Equal(
            expected: -1,
            actual: starved.ScreenIndex
        );
        Assert.NotEqual(
            expected: default,
            actual: starved.Frame
        );
        Assert.Equal(
            expected: SdfSolidPrimitive.Box,
            actual: starved.Aperture?.Primitive
        );
        Assert.Contains(
            collection: catalog.Notices,
            filter: notice => (notice.Contains(
                value: starved.PlacementId,
                comparisonType: StringComparison.Ordinal
            ) && notice.Contains(
                comparisonType: StringComparison.Ordinal,
                value: "DARKENED"
        ...
        Assert.Equal(
            expected: WorldPlacementPolicy.DerivedFaceBase,
            actual: catalog.Rows[0].ScreenIndex
        );
        Assert.Equal(
            expected: (WorldPlacementPolicy.DerivedFaceBase + 1),
            actual: catalog.Rows[1].ScreenIndex
        );
    }
    // ---- The trigger policy ----
    ...
    public void TheBandIsNeverThinnerThanOneStepOfDeclaredTravel() {
        ...
        Assert.True(
            condition: (band.Depth >= floor),
            userMessage: "the band is thinner than the derived crossing floor"
        );
        Assert.True(
            condition: (band.Depth >= row.Frame.HalfDepth),
            userMessage: "the band is thinner than the door itself"
        );
        Assert.True(
            condition: (floor > FixedQ4816.Zero),
            userMessage: "a stepping world must derive a positive crossing floor"
        );
    }
    [Fact]
    public void TheDerivationIsReusedForOneDefinitionInstance() {
        ...
        Assert.Same(
            expected: WorldFaceCatalog.For(definition: definition),
            actual: WorldFaceCatalog.For(definition: definition)
        );
    }
    [Fact]
    ...
    public void TheFrameIsSizedFromTheAuthoredScale_PlacementScaleIncluded() {
        ...
        Assert.Equal(
            expected: FixedQ4816.FromDouble(value: ShapeScale.X),
            actual: single.Frame.HalfWidth
        );
        Assert.Equal(
            expected: FixedQ4816.FromDouble(value: ShapeScale.Y),
            actual: single.Frame.HalfHeight
        );
        Assert.Equal(
            expected: FixedQ4816.FromDouble(value: ShapeScale.Z),
            actual: single.Frame.HalfDepth
        );
        Assert.Equal(
            expected: FixedQ4816.FromDouble(value: (ShapeScale.X * 2.0)),
            actual: doubled.Frame.HalfWidth
        );
        Assert.Equal(
            expected: FixedQ4816.FromDouble(value: (ShapeScale.Y * 2.0)),
            actual: doubled.Frame.HalfHeight
        );
    }
    // ---- The frame ----
    ...
    public void TheFrameSitsOnTheNamedShape_NotOnThePlacementRoot() {
        ...
        var root = FixedVector3.FromVector3(value: new Vector3(
            x: 4f,
            y: 1.5f,
            z: -7f
        ));
        ...
        Assert.NotEqual(
            expected: root,
            actual: row.Frame.Origin
        );
        Assert.Equal(
            expected: (root + FixedVector3.FromVector3(value: ShapeOffset)),
            actual: row.Frame.Origin
        );
    }
    [Fact]
    ...
    public void TheTriadIsOrthonormal_SoTheWalkedSlabAndTheDrawnScreenAreOnePlane() {
        ...
        foreach (var pitched in new[] { false, true }) {
            var frame = DoorRow(definition: BuildDoorDocument(
                yawDegrees: 25f,
                pitched: pitched
            )).Frame;
            var rendered = FixedVector3.Cross(
                left: frame.Right,
                right: frame.Up
            ).Normalize();
            ...
            Assert.True(
                condition: (FixedQ4816.Abs(value: (rendered.X - frame.Normal.X)) <= tolerance),
                userMessage: $"pitched={pitched}: rendered normal x {((double)rendered.X)} != frame normal x {((double)frame.Normal.X)}"
            );
            Assert.True(
                condition: (FixedQ4816.Abs(value: (rendered.Y - frame.Normal.Y)) <= tolerance),
                userMessage: $"pitched={pitched}: rendered normal y {((double)rendered.Y)} != frame normal y {((double)frame.Normal.Y)}"
            );
            Assert.True(
                condition: (FixedQ4816.Abs(value: (rendered.Z - frame.Normal.Z)) <= tolerance),
                userMessage: $"pitched={pitched}: rendered normal z {((double)rendered.Z)} != frame normal z {((double)frame.Normal.Z)}"
            );
            Assert.True(
                condition: (FixedQ4816.Abs(value: FixedVector3.Dot(
                    left: frame.Right,
                    right: frame.Up
                )) <= tolerance),
                userMessage: $"pitched={pitched}: right and up are not perpendicular"
            );
        }
    }
}
