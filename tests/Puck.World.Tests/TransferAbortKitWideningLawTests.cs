...
public sealed class TransferAbortKitWideningLawTests {
    ...
    private static WorldDefinition BuildDriveKitDocument() {
        ...
        var kit = new WorldKit(
            ...
            ActionsRaw: new Dictionary<string, ActionSpec> {
                ...
                OnPress: null,
                OnRelease: null,
                OnFact: [new ActionFactTrigger(
                        Fact: ActionFact.Falling,
        ...
        return Fixtures.BuildDocument() with {
            ...
            World: Fixtures.BuildDocument().State,
    ...
    private static WorldDefinition BuildMediumKitDocument() {
        ...
        var kit = new WorldKit(
            ...
            Motion: new WorldMotion(
                ...
                Shaping: [
                    new WorldShaping(
                        When: new WorldPredicate.Recently(
                            Fact: ActionFact.Rising,
                            WindowSeconds: 1f
                        ),
                        Along: new WorldShapingAlong(
                            Engage: 9f,
    ...
    public void DetachThenRestore_DriveKitBody_EveryNewlyCapturedFieldRoundTripsExactly() {
        ...
        body.Pose(
            ...
            pitchRadians: FixedQ4816.FromDouble(value: 0.2),
        ...
        var pressOutcome = body.PressChannel(
            ordinal: TimedPressOrdinal,
            value: FixedQ4816.One,
            holdSeconds: 5f,
            authoredMaximum: FixedQ4816.FromInteger(value: 60)
        );

        Assert.Equal(
            expected: PressHoldCapKind.None,
            actual: pressOutcome.CapKind
        );

        body.PressChannel(
            ordinal: UntimedPressOrdinal,
            value: FixedQ4816.One
        );
        body.EnqueueRun(
        ...
        // all.
        Assert.Equal(
            expected: "drive-ground-alt",
            actual: capturedState.BodyMotionProgramName
        );
        Assert.Equal(
            expected: IntentSource.Idle,
            actual: capturedState.Source
        );
        Assert.NotEqual(
            expected: FixedQ4816.Zero,
            actual: capturedState.DrivePitch
        );
        Assert.True(
            condition: capturedState.PreviousChannelBit[ForwardOrdinal],
            userMessage: "the forward channel was held above threshold on the last Advance before capture"
        );
        Assert.True(
            condition: capturedState.PendingDefaultChannelPress[UntimedPressOrdinal],
            userMessage: "the untimed tap must still be pending (not yet materialized) at capture"
        );
        Assert.Equal(
            expected: FixedQ4816.One,
            actual: capturedState.PendingDefaultChannelValue[UntimedPressOrdinal]
        );
        Assert.True(
            condition: (capturedState.ChannelTimerTicks[TimedPressOrdinal] > 0),
            userMessage: "the timed press must have a live remaining-ticks countdown"
        );
        Assert.NotEqual(
            expected: 0L,
            actual: capturedState.PlanarRampRemainder | capturedState.DriveLongRemainder | capturedState.DriveLatRemainder | capturedState.DriveResidualRemainder
        );
        Assert.True(
            condition: (capturedState.LaneFactHeld[SurgeOrdinal] != 0UL),
            userMessage: "the surge action's OnFact edge (Falling) must be latched held by now"
        );
        ...
        Assert.True(condition: population.RestoreDetachedSeat(
            slot: actor.Index,
            profile: profile,
            position: capturedPosition,
            yawRadians: capturedYaw,
            dynamicState: capturedState
        ));

        var restoredBody = fixture.Server.Body(index: actor.Index)!;

        Assert.Equal(
            expected: capturedPosition,
            actual: restoredBody.FixedPosition
        );
        Assert.Equal(
            expected: capturedYaw,
            actual: restoredBody.FixedYaw
        );
        Assert.Equal(
            expected: capturedOrientation,
            actual: restoredBody.FixedOrientation
        );

        var restoredState = restoredBody.CaptureTransferState();

        Assert.Equal(
            expected: capturedState.PlanarVelocity,
            actual: restoredState.PlanarVelocity
        );
        Assert.Equal(
            expected: capturedState.VerticalVelocity,
            actual: restoredState.VerticalVelocity
        );
        Assert.Equal(
            expected: capturedState.Orientation,
            actual: restoredState.Orientation
        );
        Assert.Equal(
            expected: capturedState.DrivePitch,
            actual: restoredState.DrivePitch
        );
        Assert.Equal(
            expected: capturedState.BodyMotionProgramName,
            actual: restoredState.BodyMotionProgramName
        );
        Assert.Equal(
            expected: capturedState.Source,
            actual: restoredState.Source
        );
        Assert.Equal(
            expected: capturedState.PreviousChannelBit,
            actual: restoredState.PreviousChannelBit
        );
        Assert.Equal(
            expected: capturedState.PendingDefaultChannelPress,
            actual: restoredState.PendingDefaultChannelPress
        );
        Assert.Equal(
            expected: capturedState.PendingDefaultChannelValue,
            actual: restoredState.PendingDefaultChannelValue
        );
        Assert.Equal(
            expected: capturedState.ChannelTimerTicks[TimedPressOrdinal],
            actual: restoredState.ChannelTimerTicks[TimedPressOrdinal]
        );
        Assert.Equal(
            expected: capturedState.ChannelTimerValues[TimedPressOrdinal],
            actual: restoredState.ChannelTimerValues[TimedPressOrdinal]
        );
        Assert.Equal(
            expected: capturedState.PlanarRampRemainder,
            actual: restoredState.PlanarRampRemainder
        );
        Assert.Equal(
            expected: capturedState.DriveLongRemainder,
            actual: restoredState.DriveLongRemainder
        );
        Assert.Equal(
            expected: capturedState.DriveLatRemainder,
            actual: restoredState.DriveLatRemainder
        );
        Assert.Equal(
            expected: capturedState.DriveResidualRemainder,
            actual: restoredState.DriveResidualRemainder
        );
        Assert.Equal(
            expected: capturedState.LaneLatch,
            actual: restoredState.LaneLatch
        );
        Assert.Equal(
            expected: capturedState.LaneFactHeld,
        ...
        Assert.Equal(
            ...
            actual: restoredState.ActionStateDirty
        );
        Assert.Equal(
            expected: capturedState.ActionStateDirtyKind,
            actual: restoredState.ActionStateDirtyKind
        );
        Assert.Equal(
            expected: capturedState.ActionStateDirtyOperand,
            actual: restoredState.ActionStateDirtyOperand
        );
        Assert.Equal(
            expected: capturedState.TapeIntents,
            actual: restoredState.TapeIntents
        );
        Assert.Equal(
            expected: capturedState.TapeRemainingTicks,
            actual: restoredState.TapeRemainingTicks
        );
    ...
    public void DetachThenRestore_GroundedDynamicsKitBody_PlanarFollowerStateRoundTripsExactly() {
        ...
        Assert.True(
            condition: ((capturedState.PlanarFollowerPositionRawX | capturedState.PlanarFollowerPositionRawY | capturedState.PlanarFollowerPositionRawZ) != 0L),
            userMessage: "the driven planar follower must have moved off rest before capture"
        );
        Assert.NotEqual(
            expected: FixedVector3.Zero,
            actual: capturedState.PlanarFollowerPreviousTarget
        );
        ...
        Assert.Equal(
            expected: capturedState.PlanarFollowerPositionRawX,
            actual: restoredState.PlanarFollowerPositionRawX
        );
        Assert.Equal(
            expected: capturedState.PlanarFollowerPositionRawY,
            actual: restoredState.PlanarFollowerPositionRawY
        );
        Assert.Equal(
            expected: capturedState.PlanarFollowerPositionRawZ,
            actual: restoredState.PlanarFollowerPositionRawZ
        );
        Assert.Equal(
            expected: capturedState.PlanarFollowerVelocityRawX,
            actual: restoredState.PlanarFollowerVelocityRawX
        );
        Assert.Equal(
            expected: capturedState.PlanarFollowerVelocityRawY,
            actual: restoredState.PlanarFollowerVelocityRawY
        );
        Assert.Equal(
            expected: capturedState.PlanarFollowerVelocityRawZ,
            actual: restoredState.PlanarFollowerVelocityRawZ
        );
        Assert.Equal(
            expected: capturedState.PlanarFollowerPreviousTarget,
            actual: restoredState.PlanarFollowerPreviousTarget
        );
    }
    [Fact]
    public void DetachThenRestore_MediumDynamicsKitBody_VerticalFollowerStateRoundTripsExactly() {
        ...
        Assert.NotEqual(
            expected: 0L,
            actual: capturedState.VerticalFollowerPositionRaw
        );
        Assert.NotEqual(
            expected: FixedQ4816.Zero,
            actual: capturedState.VerticalFollowerPreviousTarget
        );
        ...
        Assert.Equal(
            expected: capturedState.VerticalFollowerPositionRaw,
            actual: restoredState.VerticalFollowerPositionRaw
        );
        Assert.Equal(
            expected: capturedState.VerticalFollowerVelocityRaw,
            actual: restoredState.VerticalFollowerVelocityRaw
        );
        Assert.Equal(
            expected: capturedState.VerticalFollowerPreviousTarget,
            actual: restoredState.VerticalFollowerPreviousTarget
        );
    }
    [Fact]
    public void DetachThenRestore_MediumKitBody_EveryNewlyCapturedFieldRoundTripsExactly() {
        ...
        Assert.NotEqual(
            expected: 0L,
            actual: capturedState.MediumThrustRampRemainder
        );
        ...
        Assert.True(
            condition: (capturedState.MotionRecency[0] > 0UL),
            userMessage: "the Recently(Rising) response row's clock must have refreshed while Rising held"
        );
        ...
        Assert.Equal(
            expected: capturedState.MediumThrustRampRemainder,
            actual: restoredState.MediumThrustRampRemainder
        );
        Assert.Equal(
            expected: capturedState.MotionRecency,
            actual: restoredState.MotionRecency
        );
        Assert.Equal(
            expected: capturedState.LaneLatch,
            actual: restoredState.LaneLatch
    ...
    private static DynamicsRow Settle => new(
        Damping: 1f,
        Frequency: 2f,
        Name: "settle",
        Response: 0f
