...
public sealed class WorldSeatModeLawTests {
    ...
    private static WorldSeatModeFamily Family(string defaultState = OffState) => new(
        Name: FamilyName,
        States: [new WorldSeatModeState(Name: OffState), new WorldSeatModeState(
                Name: OnState,
                Target: "camera"
            )],
    ...
    public void CameraTargetingState_WithoutAnAuthoredCameraRig_RefusesByName() {
        ...
        Assert.Contains(
            ...
            expectedSubstring: "views.cameraRig is not authored"
        );
        Assert.True(
    ...
    public void CameraTargetingState_WithoutAnInhabitedCameraBody_RefusesByName() {
        ...
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: WorldSeatModeState.CameraPlacementIdPrefix
        );
        Assert.True(
            condition: WorldDefinitionValidator.TryValidateLocally(
                definition: control,
    ...
    public void FamilyName_CollidingWithABuiltInFamily_RefusesByName() {
        ...
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "collides with a built-in context family"
        );
        Assert.True(
            condition: WorldDefinitionValidator.TryValidateLocally(
                definition: control,
    ...
    public void SetContextState_FlipsPublishedStateAndDerivedGroup() {
        ...
        Assert.Equal(
            expected: RestingGroup,
            actual: bindings.PageView(slot: 0).Group
        );

        bindings.SetContextState(
            family: FamilyName,
            slot: 0,
            state: OnState
        );
    ...
    public void SetContextState_TakesAnyStateTextForAKnownFamily_ValidationIsTheCallersJob() {
        ...
        bindings.SetContextState(
            family: FamilyName,
            slot: 0,
            state: "not-a-declared-state"
        );

        Assert.Equal(
            expected: "not-a-declared-state",
            actual: bindings.ModeState(
                family: FamilyName,
                slot: 0
            )
    ...
    public void SetContextState_UndeclaredFamily_IsIgnored() {
        ...
        Assert.Null(@object: bindings.ModeState(
            family: "no-such-family",
            slot: 0
        ));
        Assert.Null(@object: bindings.TryResolveMode(
            family: "no-such-family",
            slot: 0
        ));
    ...
    public void TryResolveCameraMode_ResolvesTheCameraTargetingFamilyAndState() {
        ...
        Assert.Equal(
            expected: FamilyName,
            actual: resolved!.Value.Family.Name
        );
        Assert.Equal(
            expected: OnState,
            actual: resolved.Value.State.Name
        );
    }
    [Fact]
    ...
    public void TryResolveMode_ReturnsTheDeclaredFamilyAndStates() {
        ...
        var resolved = bindings.TryResolveMode(
            family: FamilyName,
            slot: 0
        );
        ...
        Assert.Equal(
            expected: 2,
            actual: resolved!.States.Count
        );
        Assert.Contains(
            collection: resolved.States,
            filter: state => ((state.Name == OffState) && (state.Target is null))
        );
        Assert.Contains(
            collection: resolved.States,
            filter: state => ((state.Name == OnState) && (state.Target == "camera"))
        );
    }
    [Fact]
    public void TryResolveMode_StatesList_IsWhatACallerValidatesATokenAgainst() {
        var bindings = new WorldSeatBindings(definition: DocumentWithFamily(family: Family()));
        var family = bindings.TryResolveMode(
            family: FamilyName,
            slot: 0
        )!;

        Assert.DoesNotContain(
            collection: family.States,
            filter: state => (state.Name == "not-a-declared-state")
        );
        // Control: the declared states ARE present, by name.
        Assert.Contains(
            collection: family.States,
            filter: state => (state.Name == OffState)
        );
        Assert.Contains(
            collection: family.States,
            filter: state => (state.Name == OnState)
        );
    }
    [Fact]
    public void UnknownDefaultState_RefusesByName() {
        ...
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "names no state"
        );
        Assert.True(
            condition: WorldDefinitionValidator.TryValidateLocally(
                definition: control,
    ...
    public void UnknownTarget_RefusesByName() {
        ...
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "is not admitted"
        );
        Assert.True(
            condition: WorldDefinitionValidator.TryValidateLocally(
                definition: control,
