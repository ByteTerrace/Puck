...
public sealed class WorldFieldLatticeLawTests {
    ...
    public void PopulationRebuildInstallsReactionOnlyEditsWithoutResettingLiveCells() {
        ...
        var original = Fixtures.WithLattice(
            definition: Fixtures.BuildDocument(),
            composite: originalFields
        );
        var replacement = Fixtures.WithLattice(
            composite: replacementFields,
            definition: original
        );
    ...
    public void PopulationRebuildRejectsAnIncompatibleLatticeBeforeChangingDerivedState() {
        ...
        var incompatible = Fixtures.WithLattice(
            definition: original,
            composite: Fields(width: 2)
        );
        var population = new WorldPopulation(definition: original);
        var lattice = Assert.IsType<FieldLattice>(@object: population.Fields);
    ...
    public void UndoRefusesAnInjectedIncompatibleBaseBeforeChangingDefinitionProgramCellsOrSolids() {
        ...
        var incompatibleBase = Fixtures.WithLattice(
        ...
        Assert.Contains(
            expectedSubstring: "restored field runtime is incompatible",
            actualString: observed.Message,
            comparisonType: StringComparison.Ordinal
        );
        Assert.Equal(
            expected: definitionBefore,
            actual: fixture.DefinitionBytes()
        );
        Assert.Same(
            expected: lattice,
            actual: fixture.Server.Population.Fields
        );
        Assert.Same(
            expected: inputBefore,
            actual: lattice.Input
        );
        Assert.Equal(
            expected: revisionBefore,
            actual: lattice.Revision
        );
        Assert.Equal(
            expected: cellsBefore,
            actual: lattice.Capture().Raw.Select(selector: static field => field.ToArray()).ToArray()
        );
        Assert.Same(
    ...
    public void ValidatorRefusesHeightGeometryThatCannotFitOneRenderBrick() {
        var tooWide = Fixtures.WithLattice(
            definition: Fixtures.BuildDocument(),
            composite: Fields(
                width: (WorldFieldCapacity.MaxSurfaceCells + 1),
                heightScale: 1f
            )
        );
        var tooTall = Fixtures.WithLattice(
            definition: Fixtures.BuildDocument(),
            composite: Fields(
                layers: 2,
                heightScale: 64f
        ...
        Assert.Contains(
            ...
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "render brick"
        );
        Assert.False(condition: WorldDefinitionValidator.TryValidate(
            definition: tooTall,
            neighbours: null,
            reason: out var tallReason
        ));
        Assert.Contains(
            actualString: tallReason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "across 2 layers"
        );
    }
    [Fact]
    public void ValidatorRefusesValuesThatCollapseOrChangeMeaningAtTheFixedPointBoundary() {
        var tinyCell = Fixtures.WithLattice(
            definition: Fixtures.BuildDocument(),
            composite: Fields(cellSize: 0.000001f)
        );
        var invalidComparison = Fixtures.WithLattice(
            definition: Fixtures.BuildDocument(),
        ...
        Assert.False(condition: WorldDefinitionValidator.TryValidate(
            definition: tinyCell,
            neighbours: null,
            reason: out var cellReason
        ));
        Assert.Contains(
            actualString: cellReason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "quantize to a positive Q48.16"
        );
        Assert.False(condition: WorldDefinitionValidator.TryValidate(
            definition: invalidComparison,
            neighbours: null,
            reason: out var comparisonReason
        ));
        Assert.Contains(
            actualString: comparisonReason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "comparison"
        );
    }
}
