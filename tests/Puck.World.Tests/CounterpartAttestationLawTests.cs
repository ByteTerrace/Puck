...
public sealed class CounterpartAttestationLawTests {
    private static (WorldDefinition Source, WorldDefinition Left, WorldDefinition Right, WorldDefinition Corner) OwnerCorner(Guid owner, string world) {
        var boundary = static (float yaw) => new WorldAdjacencyBoundary(
            Center: Vector3.Zero,
        ...
        var cornerReference = new WorldReference(
            Name: SafeName.Parse(candidate: "corner-ref"),
            Owner: owner,
    ...
    private static bool TryVerifySigned(
        ...
        return WorldCounterpartAttestationProtocol.TryVerify(
            entries: entries,
            codec: codec,
            claim: claim,
            chain: [],
            now: now,
            attestation: out verified,
            subject: out _,
            reason: out reason
        );
    }
    ...
    public void AnUnsignedOrForeignClaim_NeverBecomesAnAttestation() {
        ...
        Assert.True(
            condition: TryVerifySigned(
                ...
                entries: [trust],
                key: trusted,
                reason: out var trustedReason,
                subject: "counterpart",
                verified: out _
            ),
            userMessage: trustedReason
        );
        ...
        Assert.False(condition: TryVerifySigned(
            ...
            entries: [],
            key: trusted,
            reason: out var noTrust,
            subject: "counterpart",
            verified: out _
        ));
        Assert.Contains(
            actualString: noTrust,
    ...
    public void ApiResolverRefusesAClaimFromAnUnadmittedRoot() {
        ...
        Assert.True(
            condition: WorldCounterpartAttestation.TryCompose(
                ...
                definition: corner,
                document: $"owner/{owner:D}/{WorldName}",
    ...
    public void ApiResolverRefusesAClaimWhoseSubjectIsNotTheReferenceOwner() {
        ...
        Assert.Contains(
            expectedSubstring: "does not name the reference's owner",
            actualString: outcome.Reason,
            comparisonType: StringComparison.Ordinal
        );
    }
    ...
    public void AttestedEdgeBoundaryRoundTripsToTheSameCompiledFrame() {
        var definition = Fixtures.BuildDocument() with {
            ...
            Destinations = [
                ...
                new WorldDestination(
                SafeName.Parse(candidate: "north"),
                "north-ref",
                WorldDestinationDurability.Persisted,
                WorldDestinationScope.Global
            ),
            ...
            Adjacencies = [
                ...
                new WorldAdjacency(
                SafeName.Parse(candidate: "north-edge"),
                "north",
                "south-edge",
                new WorldAdjacencyBoundary(
                    Center: new Vector3(
        ...
        var roundTripped = JsonSerializer.Deserialize(
            utf8Json: payload,
            jsonTypeInfo: WorldJsonContext.Default.WorldCounterpartAttestation
        );
        ...
        Assert.Equal(
            attestation.Edges.Count,
            roundTripped.Edges.Count
        );

        for (var index = 0; (index < attestation.Edges.Count); index++) {
            Assert.Equal(
                attestation.Edges[index].Boundary.CompileFrame(),
                roundTripped.Edges[index].Boundary.CompileFrame()
            );
        }
    }
    ...
    public void OwnerNamedNeighbourProvesItsCornerThroughTheApiResolver() {
        ...
        Assert.Contains(
            expected: $"owner/{owner:D}/{WorldName}",
            collection: handler.RequestedPaths
        );
    }
    [Fact]
    public void SignedAttestation_SettlesReciprocity_AndRefusesATamperedExtent() {
        ...
        Assert.True(
            ...
            userMessage: verifyReason
        );
        Assert.True(
            condition: WorldDefinitionValidator.TryValidate(
                definition: west,
                reason: out var accepted,
                neighbours: new StubResolver(attestation: verified!)
            ),
            userMessage: accepted
        );
        ...
        Assert.False(condition: WorldDefinitionValidator.TryValidate(
            ...
            neighbours: new StubResolver(attestation: tampered)
        ));
        Assert.Contains(
        ...
        Assert.False(condition: WorldDefinitionValidator.TryValidate(
            definition: west,
            reason: out var broken,
            neighbours: new StubResolver(attestation: nonReciprocal)
        ));
        Assert.Contains(
            actualString: broken,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "is not reciprocal"
        );
    }
    ...
    private sealed class FakeCounterpartHandler : HttpMessageHandler {
        ...
        protected override HttpResponseMessage Send(HttpRequestMessage request, CancellationToken cancellationToken) {
            ...
            var found = m_responses.Keys.FirstOrDefault(predicate: candidate => candidate.StartsWith(
                value: $"owner/{segments[2]}/",
                comparisonType: StringComparison.Ordinal
            ));
