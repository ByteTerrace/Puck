...
public sealed class EngagementTranslationLawTests {
    ...
    public void AnalogTranslationClampsToDestinationElementDomain() {
        ...
        var positive = fixture.Server.Engagement.Translate(
            intent: default(PlayerIntent)
                .WithChannel(
                ordinal: 0,
                value: FixedQ4816.FromInteger(value: 2)
            )
                .WithChannel(
                ordinal: 1,
                value: FixedQ4816.FromInteger(value: 2)
        ...
        Assert.Equal(
            expected: 1f,
            actual: positive.LeftTrigger
        );
        Assert.Equal(
            expected: 1f,
            actual: positive.LeftStick.X
        );
        Assert.Equal(
            expected: 1f,
            actual: positive.RightTrigger
        );
        Assert.Equal(
            expected: 0f,
            actual: negative.LeftTrigger
        );
        Assert.Equal(
            expected: -1f,
            actual: negative.LeftStick.X
        );
        Assert.Equal(
            expected: 0f,
            actual: negative.RightTrigger
        );
    }
    ...
    public void UnnamedKitReadsTheBakedMovementRoleDefault() {
        ...
        Assert.Equal(
            expected: 1f,
            actual: pad.LeftStick.X
        );
        Assert.Equal(
            expected: 1f,
            actual: pad.LeftStick.Y
        );
        Assert.Equal(
            expected: 0f,
            actual: pad.LeftTrigger
        );
        Assert.Equal(
            expected: 0f,
            actual: pad.RightTrigger
        );
    }
}
