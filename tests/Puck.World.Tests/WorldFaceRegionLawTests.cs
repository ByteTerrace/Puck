...
public sealed class WorldFaceRegionLawTests {
    ...
    public void ACrossingFiresOnTheEdgeIn_AndOnceOnly() {
        ...
        Assert.True(condition: Observe(occupancy: occupancy, crossing: inside));
        Assert.False(condition: Observe(occupancy: occupancy, crossing: inside));
    }
    ...
    public void LeavingTheBandReArmsTheDoor() {
        ...
        Assert.True(condition: Observe(occupancy: occupancy, crossing: inside));
        Assert.False(condition: Observe(occupancy: occupancy, crossing: outside));
        Assert.True(condition: Observe(occupancy: occupancy, crossing: inside));
    }
    ...
    public void ATunnellingCrossingFiresWithoutLatching() {
        ...
        Assert.True(condition: Observe(occupancy: occupancy, crossing: through));
        Assert.False(condition: occupancy.IsInside(placementId: Placement, faceName: Face, seat: 0));
        Assert.True(condition: Observe(occupancy: occupancy, crossing: through));
    }
    ...
    public void ArrivalSeedsOccupancy_SoAMappedPairCannotBounceTheTravelerBack() {
        ...
        arrived.SeedInside(placementId: Placement, faceName: Face, seat: 0);

        Assert.False(condition: Observe(occupancy: arrived, crossing: standing), userMessage: "a seeded arrival must not fire the door it arrived at");
        Assert.True(condition: Observe(occupancy: walkedIn, crossing: standing), userMessage: "the control body, which walked in, must still fire");
    }
    ...
    public void SeedingOneSeatDoesNotSilenceAnother() {
        ...
        occupancy.SeedInside(placementId: Placement, faceName: Face, seat: 0);
    ...
    public void ForgettingASeatReArmsIt() {
        ...
        Assert.True(condition: Observe(occupancy: occupancy, crossing: standing));

        occupancy.Forget(placementId: Placement, faceName: Face, seat: 0);
    ...
    public void TheEarliestCrossingWins_RegardlessOfWhichFaceWasScannedFirst() {
        var early = Claim(placement: "zeta", face: "screen", parameter: 0.25);
        var late = Claim(placement: "alpha", face: "screen", parameter: 0.75);
    ...
    public void EqualParameters_TieBreakOnStablePlacementThenFaceIdentity() {
        var alphaFirst = Claim(placement: "alpha", face: "b", parameter: 0.5);
        var alphaSecond = Claim(placement: "alpha", face: "c", parameter: 0.5);
        var zeta = Claim(placement: "zeta", face: "a", parameter: 0.5);
    ...
    public void TheOrderingIsTotalAndAntisymmetric() {
        var claims = new[] {
            Claim(placement: "b", face: "y", parameter: 0.5),
            Claim(placement: "a", face: "z", parameter: 0.5),
            Claim(placement: "a", face: "y", parameter: 0.5),
            Claim(placement: "c", face: "x", parameter: 0.1),
        };
