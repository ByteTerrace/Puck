...
public sealed class BodyDynamicsLawTests {
    ...
    public void CriticalDamping_RisesMonotonicallyToMoveSpeed_ThenDecaysMonotonicallyOnRelease() {
        using var fixture = Fixtures.FreshServer(definition: WithKitDynamics(
            damping: 1f,
            dynamicsRow: "settle"
        ));
        ...
        for (var tick = 0; (tick < 60); tick++) {
            ...
            Assert.True(
                condition: (speed >= (previous - 1e-4f)),
                userMessage: $"tick {tick}: speed dropped from {previous:0.#####} to {speed:0.#####} under critical damping"
            );
            Assert.True(
                condition: (speed <= (moveSpeed + 1e-3f)),
                userMessage: $"tick {tick}: speed {speed:0.#####} exceeds the move speed {moveSpeed:0.#####}"
            );
            previous = speed;
        }

        Assert.True(
            condition: (MathF.Abs(x: (previous - moveSpeed)) < (moveSpeed * 0.01f)),
            userMessage: $"after 2 s the speed {previous:0.#####} should sit within 1% of the move speed {moveSpeed:0.#####}"
        );
        ...
        for (var tick = 0; (tick < 60); tick++) {
            ...
            Assert.True(
                condition: (speed <= (previous + 1e-4f)),
                userMessage: $"tick {tick}: speed rose from {previous:0.#####} to {speed:0.#####} while releasing under critical damping"
            );
            previous = speed;
        }

        Assert.True(
            condition: (previous < 1e-3f),
            userMessage: $"after 2 s of release the speed should have decayed below one LSB; read {previous:0.#####}"
        );
    }
    [Fact]
    public void IdenticalIntentReplays_ProduceIdenticalHashTraces_WhileADifferentSimulationRateDiverges() {
        Action<WorldBody, int> holdForward = static (body, _) => body.SubmitIntent(intent: default(PlayerIntent).WithChannel(
            ordinal: ForwardOrdinal,
            value: FixedQ4816.One
        ));
        var document = WithKitDynamics(
            damping: 1f,
            dynamicsRow: "settle"
        );
    ...
    public void LightDamping_OvershootsThePartialTarget_WhereCriticalDampingNeverDoes() {
        ...
        using var lightFixture = Fixtures.FreshServer(definition: WithKitDynamics(
            damping: 0.25f,
            dynamicsRow: "loose"
        ));
        ...
        for (var tick = 0; (tick < 30); tick++) {
            lightBody.SubmitIntent(intent: default(PlayerIntent).WithChannel(
                ordinal: ForwardOrdinal,
                value: partial
            ));
            lightFixture.Step();
        ...
        Assert.True(
            condition: lightOvershot,
            userMessage: $"light damping should overshoot the partial target {targetSpeed:0.#####} at some tick"
        );

        using var criticalFixture = Fixtures.FreshServer(definition: WithKitDynamics(
            damping: 1f,
            dynamicsRow: "settle"
        ));
        ...
        for (var tick = 0; (tick < 30); tick++) {
            criticalBody.SubmitIntent(intent: default(PlayerIntent).WithChannel(
                ordinal: ForwardOrdinal,
                value: partial
            ));
        ...
        Assert.False(
            condition: criticalOvershot,
            userMessage: "critical damping should never overshoot the partial target"
        );
    }
}
