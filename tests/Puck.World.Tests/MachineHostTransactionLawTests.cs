...
public sealed class MachineHostTransactionLawTests {
    ...
    public void TryResolveSymbol_ResolvesNamedVariablesOnCompiledCartridge() {
        ...
        Assert.True(
            condition: (declared.Length >= 2),
            userMessage: "the cartridge declares too few variables to tell two addresses apart"
        );
        ...
        var addresses = new Dictionary<string, int>(comparer: StringComparer.Ordinal);

        foreach (var variable in declared) {
            Assert.True(
                condition: fixture.Server.Machines.TryResolveSymbol(
                    "cabinet",
                    variable.Name,
                    out var address
                ),
            ...
            Assert.True(
                condition: (address > 0),
                userMessage: $"'{variable.Name}' resolved to {address}"
            );
        ...
        Assert.Equal(
            ...
            actual: addresses.Values.Distinct().Count()
        );

        // A name the cartridge does not declare resolves to nothing.
        Assert.False(condition: fixture.Server.Machines.TryResolveSymbol(
            address: out _,
            instance: "cabinet",
            symbol: "nonexistent_variable"
        ));

        // Fast boot skips firmware presentation, but the cartridge still runs its own initialization.
        _ = Fixtures.StepUntil(
            fixture,
            ceiling: 120,
            settled: () => declared.All(predicate: variable =>
            ((fixture.Server.Machines.Inspect(
                "cabinet",
                new(
                    "bus",
                    checked((ulong)addresses[variable.Name]),
                    1
                )
            ).Status == Puck.Abstractions.Machines.MachineAccessStatus.Available) &&
            (fixture.Server.Machines.Inspect(
                "cabinet",
                new(
                    "bus",
        ...
        foreach (var variable in declared) {
            ...
            Assert.Equal(
                expected: variable.Initial,
                actual: ((byte)fixture.Server.Machines.Inspect(
                    "cabinet",
                    new(
                        "bus",
                        checked((ulong)addresses[variable.Name]),
    ...
    public void TwoPhasePrepareAndCommit_AppliesCandidateDefinition() {
        ...
        var candidateDef = WithMachineScreen(
            contentPath: path,
            engine: CgbEngine,
            options: "cgb fast"
        );
        ...
        Assert.NotNull(value: host.InstanceState(name: "cabinet"));
        var state = host.InstanceState(name: "cabinet");
