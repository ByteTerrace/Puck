...
public sealed class WorldBodyFactChannelLawTests {
    ...
    private static WorldDefinition FactDocument(string? airborneChannel = null) {
        ...
        return source with {
            ...
            Uniform: new Puck.Assets.Documents.DocumentVector3(
                x: 0f,
                y: -46f,
                z: 0f
            )
            ...
            KitRowsRaw = source.Kits.Select(selector: kit => kit with {
                Motion = kit.Motion with {
                    Holds = [kit.Motion.Holds![0] with { Envelope = new WorldHoldEnvelope(SinkSpeed: 40f), Gravity = new WorldHoldGravity(
                Fall: 46f,
                Rise: 28f
            ) }],
                },
            ...
            Rules = [
                ...
                Effects: [new ActionEffect.SetState(
                        State: AirborneRow,
                        FromState: (airborneChannel ?? $"{WorldRuleFacts.FactPrefix}body:0:Airborne")
                    )]
            ),
                new WorldRule(
                Name: CellName.Parse(candidate: "grounded-mirror"),
                Effects: [new ActionEffect.SetState(
                        State: GroundedRow,
                        FromState: $"{WorldRuleFacts.FactPrefix}body:0:Grounded"
                    )]
            ),
                new WorldRule(
                Name: CellName.Parse(candidate: "absent-mirror"),
                Effects: [new ActionEffect.SetState(
                        State: AbsentRow,
                        FromState: $"{WorldRuleFacts.FactPrefix}body:3:Grounded"
                    )]
            ),
            ],
        };
    ...
    public void TheFactChannelRefusesANameThatIsNoBodyFact_ControlAWellFormedOneValidates() {
        ...
        Assert.Contains(
            expectedSubstring: nameof(WorldRuleRefusal.BodyFactMalformed),
            actualString: reason,
