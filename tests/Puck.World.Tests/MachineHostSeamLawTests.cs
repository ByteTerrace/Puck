...
/// <c>Puck.World.Addons.Machines</c>), and <c>Puck.World.Server</c>'s own compiled assembly carries neither the
...
public sealed class MachineHostSeamLawTests {
    ...
    public void ServerAssemblyReferencesNoEmulatorCoreOrRendererProject() {
        ...
        foreach (var denied in DeniedAssemblyNames) {
            Assert.False(condition: referenced.Contains(denied), userMessage: $"Puck.World.Server's compiled assembly references '{denied}' — the screen-machine host and its shipped engines belong in Puck.World.Addons now, so a browser or silo build of Server must not carry this edge.");
        }
    }
    [Fact]
