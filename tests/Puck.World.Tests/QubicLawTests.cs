...
public sealed class QubicLawTests {
    ...
    private static WorldStateRow Seed(WorldStateRow row, params long[] values) => row with {
        Cells = [.. values.Select((value, index) => new StateCell(
            CellName.Parse(values.Length == 1 ? WorldStateRow.SlotKey : index.ToString()),
            CellValue.Int(value)
        ))],
    };
    ...
    private long Winner(long[] board) {
        var source = Fixtures.BuildDocument() with {
            StateRaw = new WorldStateSection(
                World: [.. Garden.State.Where(row => row.Name.Value.StartsWith("ttt", StringComparison.Ordinal)).Select(row => row.Name.Value switch {
                    "tttBoard" => Seed(row, board),
                    "tttBoardVersion" => Seed(row, 1),
                    "tttMoveCount" => Seed(row, board.Count(value => value != 0)),
                    _ => row,
                })],
                Lattices: [Garden.StateRaw!.Lattices!.Single(topology => topology.Name == "tttCube")]
            ),
            Rules = [Garden.Rules!.Single(rule => rule.Name.Value == "ttt-check-win")],
        };
    ...
    public void EveryLineWinsAndEveryThreeMarkNearMissDoesNot(int player) {
        ...
        foreach (var line in lines) {
            ...
            Assert.Equal(player, Winner(board));
            foreach (var missing in line) {
                board[missing] = 0;
                Assert.Equal(0, Winner(board));
