...
public sealed class WorldCameraProgramValidationLawTests {
    ...
    private static void Refuses(WorldCameraProgram denied, WorldCameraProgram control, string expected) {
        Assert.False(condition: WorldDefinitionValidator.TryValidateLocally(
            definition: DocumentWithCamera(rig: denied),
        ...
        Assert.True(
            condition: WorldDefinitionValidator.TryValidateLocally(
                definition: DocumentWithCamera(rig: control),
    ...
    public void ABlendCycleIsRefusedByNameWithItsTrail() {
        ...
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "cycles back to a program already being blended"
        );
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "-> 'right-rig' -> 'left-rig'"
        );
        ...
        var control = (denied with {
            CamerasRaw = [
                denied.Cameras[0],
                (denied.Cameras[1] with { Rig = Program(
                "right-rig",
                Fov()
            ) }),
            ],
    ...
    public void ACameraRigWithAnOffsetOpIsRefusedByName() {
        ...
        var family = new WorldSeatModeFamily(
            Name: "camera",
            States: [new WorldSeatModeState(Name: "seat"), new WorldSeatModeState(
                    Name: "free",
                    Target: WorldSeatModeState.CameraTarget
                )],
            DefaultState: "seat"
        );
        var freeCam = Program(
        ...
        var denied = (control with {
            ViewsRaw = (baseline.Views with {
                CameraRig = Program(
            "seatFreeCam",
            new WorldCameraProgramOp.Offset(Value: new DocumentVector3(
                x: 0f,
                y: 0f,
                z: 3f
            )),
        ...
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "views.cameraRig must author no 'orbit', 'offset', or 'path' op"
        );
        Assert.True(
            condition: WorldDefinitionValidator.TryValidateLocally(
                definition: control,
    ...
    public void AClampPitchAfterTheOrbitItGovernsIsRefusedByName() => Refuses(
        control: Program(
            "probe-rig",
            new WorldCameraProgramOp.ClampPitch(
                MaxPitch: 1f,
                MinPitch: -1f
            ),
    ...
    public void ADuplicatedProgramNameIsRefusedByName() {
        ...
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "'shared-rig' is duplicated across the document's camera programs"
        );
    ...
    public void AProgramRoundTripsThroughTheDocumentWire() {
        var document = DocumentWithCamera(rig: Program(
            "probe-rig",
            new WorldCameraProgramOp.Anchor(Subject: new WorldCameraSubject.WorldPoint(Point: new DocumentVector3(
                x: 1f,
                y: 2f,
                z: 3f
            ))),
            ...
            new WorldCameraProgramOp.Orbit(
                ...
                PivotOffset: new DocumentVector3(
                    x: 0f,
                    y: 1f,
                    z: 0f
                )
        ...
        Assert.Equal(
            ...
            actual: ((WorldCameraSubject.WorldPoint)rig.AnchorOp!.Subject).Point.Value
        );
    }
    [Fact]
    public void AProgramWithNoRenderedFieldOfViewIsRefusedByName() => Refuses(
        ...
        denied: Program(
            "probe-rig",
            new WorldCameraProgramOp.Offset(Value: new DocumentVector3(value: Vector3.Zero))
        ),
    ...
    public void ASeatRigWithoutAnOrbitOpIsRefusedByName() {
        ...
        Assert.Contains(
            actualString: reason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "views.seatRig must contain an 'orbit' op"
        );
        Assert.True(
            condition: WorldDefinitionValidator.TryValidateLocally(
    ...
    public void ASubjectNamingAnUndeclaredPlacementIsRefusedByName() => Refuses(
        ...
        denied: Program(
            "probe-rig",
            new WorldCameraProgramOp.Anchor(Subject: new WorldCameraSubject.Placement(PlacementId: "no-such-placement")),
            Fov()
        ),
    ...
    public void MoreOperationsThanTheCeilingAreRefusedByName() {
        ...
        Refuses(
            control: Program(
                "probe-rig",
                [.. operations.Take(count: WorldCameraProgram.MaxOperations)]
            ),
            denied: Program(
                "probe-rig",
                [.. operations]
