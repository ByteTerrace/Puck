...
public sealed class MancalaModuleLawTests {
    private static readonly WorldDefinition Module = AuthoredGameFixtures.Program(module: "mancala");
    ...
    public void Capture_Rule_SweepsLandingSeedAndOppositePit() {
        ...
        var board = ReadBoard(fixture: fixture);
        ...
        Assert.Equal(
            0L,
            board[1]
        ); // Landing pit was captured -> 0!
        Assert.Equal(
            0L,
            board[11]
        ); // Opposite pit was captured -> 0!
        Assert.Equal(
            6L,
            board[6]
        ); // P0 Store got landing (1) + opposite (5) = 6 seeds!
    }
    [Fact]
    ...
    public void LargeWrapAround_SowsMoreThan13Seeds() {
        ...
        Assert.Equal(
            1L,
            board[5]
        ); // 15 seeds sown: 1 full lap + 2 extra, pit 5 gets 1 back!
        Assert.Equal(
            2L,
            board[6]
        ); // Store 6 (dist 1) gets 2
        Assert.Equal(
            6L,
            board[7]
        ); // Pit 7 (dist 2) had 4 + 2 = 6
        Assert.Equal(
            0L,
            board[13]
        ); // Opponent store 13 skipped -> remains 0
    }
    [Fact]
    ...
    public void OpponentStore_IsSkipped_ByBothPlayers() {
        ...
        Assert.Equal(
            0L,
            board[13]
        ); // Opponent Store 13 MUST be skipped (remains 0)!
        Assert.Equal(
            5L,
            board[0]
        ); // 8th seed wrapped around store 13 into pit 0!
    }
    [Fact]
