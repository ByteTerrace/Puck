...
public sealed class WorldIconRowValidationLawTests {
    ...
    public void NonAsciiLabelRefusesWhileAsciiPasses() {
        var denied = WithIconRow(row: new WorldIconRow(
            Name: "icon",
            Label: "©"
        ));
        var admitted = WithIconRow(row: new WorldIconRow(
            Name: "icon",
            Label: "OK"
        ));
        ...
        Assert.Contains(
            ...
            expectedSubstring: "icons.icons[0].label"
        );
        Assert.Contains(
            actualString: deniedReason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "printable-ASCII"
        );
        Assert.True(
    ...
    public void NonMonoGlyphFontRefusesWhileMonoPasses() {
        var denied = WithIconRow(row: new WorldIconRow(
            ...
            Glyph: new WorldIconGlyphRef(
                Font: WorldIconFontCatalog.InterRegular,
                Glyph: "U+2191"
            )
        ));
        var admitted = WithIconRow(row: new WorldIconRow(
        ...
        Assert.Contains(
            actualString: deniedReason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "icons.icons[0].glyph.font"
        );
        Assert.Contains(
            actualString: deniedReason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: WorldIconFontCatalog.JetBrainsMonoRegular
        );
        Assert.True(
            condition: WorldDefinitionValidator.TryValidate(
                definition: admitted,
