...
public sealed class DocumentBasisLawTests {
    ...
    public void KeyedCellMerge_RefinesByKey_AndAControlStillReplacesWholesale() {
        ...
        Assert.Equal(
            expected: 9L,
            actual: StateRows.FindCell(cells: refinedRow.Cells, key: CellName.Parse(candidate: "a"))!.Value
        );
        Assert.Equal(
            expected: 2L,
            actual: StateRows.FindCell(cells: refinedRow.Cells, key: CellName.Parse(candidate: "b"))!.Value
        );
        ...
        Assert.Equal(
            expected: 9L,
            actual: StateRows.FindCell(cells: replacedRow.Cells, key: CellName.Parse(candidate: "a"))!.Value
        );
    }
}
