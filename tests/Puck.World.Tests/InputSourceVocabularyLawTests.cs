...
public sealed class InputSourceVocabularyLawTests {
    private static WorldDefinition WithBadgeSource(string source) => Fixtures.BuildDocument() with {
        ...
        BadgesRaw: [new WorldIconBadgeRow(
                Source: source,
                Icon: BadgeIcon
            )],
        IconsRaw: [new WorldIconRow(
                Name: BadgeIcon,
                Label: "MB"
            )]
    ),
    ...
    public void BadgeRowAdmitsAnyDeclaredSourceAndRefusesANonSource() {
        ...
        Assert.Contains(
            ...
            expectedSubstring: "icons.badges[0].source"
        );
        Assert.Contains(
            actualString: deniedReason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "is not a declared input source id"
        );
    }
    ...
    public void CatalogAcceptsOnlyDeclaredSourceIds(string candidate, bool known) =>
        Assert.Equal(
            expected: known,
            actual: InputSourceVocabulary.IsKnownSourceId(sourceId: candidate)
        );
    ...
    public void DeviceEnumSlotNameRefusesWhileSourceIdPasses() {
        ...
        Assert.Contains(
            actualString: deniedReason,
            comparisonType: StringComparison.Ordinal,
            expectedSubstring: "bindingOverlays[0].bindingBar.slotSet[0]"
        );
        Assert.Contains(
            actualString: deniedReason,
            comparisonType: StringComparison.Ordinal,
    ...
    public void EveryDeclaredButtonFlagResolvesToAKnownSourceId() {
        ...
        foreach (var (flag, source) in GamepadButtonCatalog.Sources) {
            Assert.True(
                condition: InputSourceVocabulary.IsKnownSourceId(sourceId: source),
                userMessage: $"{flag} -> {source}"
            );
            Assert.Equal(
                expected: source,
                actual: GamepadButtonCatalog.SourceOf(button: flag)
            );
        }
    }
    ...
    public void FamilyCatalogAcceptsOnlyExactDeclaredNames(string candidate, bool known) =>
        Assert.Equal(
            expected: known,
            actual: GamepadFamilyCatalog.IsKnownName(name: candidate)
        );
