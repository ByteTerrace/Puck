...
public sealed class BindingBarAuthoredLayoutLawTests {
    ...
    public void APlacedControlRidesItsPlateAndFrameAndAnUnplacedOneIsHidden() {
        var plates = new Dictionary<string, BindingPlatePlacement>(comparer: StringComparer.Ordinal) {
            [InputSources.Gamepad.DpadUp] = new BindingPlatePlacement(
        ...
        Compose(
            ...
            frame: 3,
            plates: plates,
            slotSet: [InputSources.Gamepad.DpadUp, InputSources.Gamepad.Guide]
        );

        Assert.True(condition: destination[0].Visible);
        Assert.Equal(
            expected: (-5f, 1f),
            actual: (destination[0].PitchX, destination[0].PitchY)
        );
        Assert.Equal(
            expected: (0f, 1f),
            actual: (destination[0].BadgeX, destination[0].BadgeY)
        );
        Assert.Equal(
            expected: 3,
            actual: destination[0].Frame
        );
    ...
    public void APlateHangsFromItsEdgeWithItsPitchZeroEdgeOnTheInsetLine() {
        ...
        var bottom = BindingBarLayout.BarAnchor(
            aspect: 1.6f,
        ...
        AssertPoint(
            expectedX: 0.5f,
            expectedY: 0.6f,
            actual: BindingBarLayout.PlateCenter(
                anchor: bottom,
                buttonSize: 0.1f,
                edge: BindingBarEdge.Bottom,
                pitchX: -3f,
        ...
        var left = BindingBarLayout.BarAnchor(
            aspect: 1.6f,
        ...
        var right = BindingBarLayout.BarAnchor(
            aspect: 1.6f,
            edge: BindingBarEdge.Right,
            inset: 0.04f
        );
    ...
    public void AnUnauthoredBadgeIsTheUpRightCornerAndAnAuthoredPairIsItself() {
        Assert.Equal(
            ...
            actual: new WorldBindingBarSlotPlacement(
    ...
    public void CompilingMergesPiecesWithALaterPieceWinningASourceAndNormalizesEachFrame() {
        ...
        var layout = new WorldBindingBarLayout(
            ...
            Banks: new Dictionary<string, WorldBindingBarBankPlacement>(comparer: StringComparer.Ordinal) {
                ...
                ["lt"] = new WorldBindingBarBankPlacement(Pieces: [new WorldBindingBarPiece(
                    Table: "cross",
                    At: [-3f, 1f]
                )]),
                ...
                Pieces: [new WorldBindingBarPiece(
                        Table: "column",
                        At: [2f, 0f]
                    )]
            ),
        ...
        Assert.Equal(
            expected: 8f,
            actual: bottom.Across
        );
        Assert.Equal(
            expected: BindingBarEdge.Left,
            actual: left.Edge
        );
        Assert.Equal(
            expected: 0f,
        ...
        Assert.Equal(
            ...
            actual: resting.Plates[InputSources.Gamepad.DpadUp].Position
        );
        Assert.Equal(
            expected: new Vector2(
                x: -1f,
            ...
            actual: resting.Plates[InputSources.Gamepad.DpadUp].Badge
        );
        Assert.Equal(
            expected: new Vector2(
                x: 4f,
            ...
            actual: resting.Plates[InputSources.Gamepad.DpadDown].Position
        );
        ...
        Assert.Equal(
            ...
            actual: compiled.Banks["lt"].Plates[InputSources.Gamepad.DpadUp].Position
        );
        ...
        Assert.Equal(
            ...
            actual: side.Plates[InputSources.Gamepad.DpadUp].Position
        );
        Assert.Equal(
            expected: new Vector2(
                x: 0f,
            ...
            actual: side.Plates[InputSources.Gamepad.DpadDown].Position
        );
    ...
    public void TheButtonSizeShrinksUntilEveryFrameFitsTheRegion() {
        ...
        Assert.Equal(
            expected: (1f / 12f),
            actual: CompiledBindingBarLayout.FitButtonSize(
                aspect: 1.6f,
                buttonSize: 0.1f,
                frames: frames
            ),
        ...
        Assert.Equal(
            expected: 0.1f,
            actual: CompiledBindingBarLayout.Empty.FitButtonSize(
                aspect: 1.6f,
                buttonSize: 0.1f
            )
        );
    ...
    public void TheLayoutCellSelectsANamedLayoutThenTheDefaultNameThenNothing() {
        ...
        Assert.Same(
            expected: linear,
            actual: authoring.LayoutNamed(name: "linear")
        );
        Assert.Same(
            expected: crossbar,
            actual: authoring.LayoutNamed(name: "no-such")
        );
        Assert.Same(
            expected: crossbar,
            actual: authoring.LayoutNamed(name: null)
        );
        Assert.Same(
            expected: WorldBindingBarLayout.Default,
            actual: (authoring with { Layout = null }).LayoutNamed(name: "no-such")
        );
