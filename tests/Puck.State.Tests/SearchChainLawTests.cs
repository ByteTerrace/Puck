...
public sealed class SearchChainLawTests {
    ...
    private static (SearchRuntime Runtime, SearchPlan Plan) ChainFixture(int maxHops) {
        ...
        var plan = new SearchPlan(
            ...
            Shapes: [new SearchShapePlan(
                    ...
                    Displace: false,
                    Directions: [topology.Direction(token: "E")],
                    PairWithIndex: -1,
                    MaxHops: maxHops
                )],
            Legal: "legal"
    ...
    public void AChainOfThreeHopsEnumeratesExactlyTheLegalSequences() {
        ...
        Assert.Equal(
            expected: 8,
            actual: plan.Shapes[0].CandidateCount(cellCount: plan.CellCount)
        );
        ...
        Assert.Contains(
            collection: landed!,
            filter: write => (write is SearchWrite.Cell { Row: "legal", Key: "a", Value: 0b1010100L })
        );
        Assert.Contains(
            collection: landed!,
            filter: write => (write is SearchWrite.Cell { Row: "legal", Key: "b", Value: 0L })
        );
        Assert.Contains(
            collection: landed!,
            filter: write => (write is SearchWrite.Cell { Row: "legal", Key: "c", Value: 0L })
        );
        Assert.Contains(
            collection: landed!,
            filter: write => (write is SearchWrite.Cell { Row: "legal", Key: "d", Value: 0L })
        );
        Assert.Contains(
            collection: landed!,
            filter: write => (write is SearchWrite.Cell { Row: "legal", Key: "e", Value: 0L })
        );
    }
    [Fact]
    public void MaxHopsBoundsTheChainsEnumeration() {
        ...
        Assert.Contains(
            collection: landed!,
            filter: write => (write is SearchWrite.Cell { Row: "legal", Key: "a", Value: 0b0010100L })
        );
    }
    ...
    public void MaxNMaximizesTheMoversOwnSeatWhereNegatingTheReplyWouldChooseDifferently() {
        ...
        var judge = new[] {
            ...
            rule: new Rule(
                ...
                Effects: [
                    new ActionEffect.SetState(
                        State: "isRoot",
                        Expression: ValueExpression.Parse(text: "(turn == 0) & ((piece[p] == 1) | (piece[p] == 2))")
                    ),
                    new ActionEffect.SetState(
                        State: "isReply",
                        Expression: ValueExpression.Parse(text: "(turn == 1) & (((branch == 1) & (piece[p] == 3)) | ((branch == 2) & (piece[p] == 4)))")
                    ),
                    new ActionEffect.SetState(
                        State: "accepted",
                        Expression: ValueExpression.Parse(text: "isRoot | isReply")
                    ),
                    new ActionEffect.SetState(
                        State: "verdict",
                        Expression: ValueExpression.Parse(text: "accepted")
                    ),
                    new ActionEffect.SetState(
                        State: "turn",
                        Expression: ValueExpression.Parse(text: "turn + accepted")
                    ),
                    new ActionEffect.SetState(
                        State: "branch",
                        Expression: ValueExpression.Parse(text: "isRoot ? piece[p] : branch")
                    ),
                    new ActionEffect.SetState(
                        State: "scores",
                        Key: "0",
                        Expression: ValueExpression.Parse(text: "(piece[p] == 3) ? 10 : ((piece[p] == 4) ? 8 : 0)")
                    ),
                    new ActionEffect.SetState(
                        State: "scores",
                        Key: "1",
                        Expression: ValueExpression.Parse(text: "(piece[p] == 3) ? 9 : ((piece[p] == 4) ? 1 : 0)")
                    ),
                    new ActionEffect.SetState(
                        State: "scores",
                        Key: "2",
        ...
        Assert.Contains(
            collection: landed!,
            filter: write => (write is SearchWrite.Cell { Row: "best", Key: "to", Value: 1L })
        );
        Assert.Contains(
            collection: landed!,
            filter: write => (write is SearchWrite.Cell { Row: "best", Key: "score", Value: 10L })
        );
