...
public sealed class BindingChainScheduleLawTests {
    ...
    private static ActionEffect.AddState Add(string state, decimal value) => new(
        State: state,
        Value: value
    );
        ...
        Rule[] rules = [
            new Rule(
                ...
                Effects: [new ActionEffect.SetState(
                        State: "total",
                        Expression: Expr(text: "$bind:doubled")
                    )],
                Bindings: [
                    new RuleBinding(
                        Name: Name(value: "first"),
                        Kind: CellKind.Int,
                        Expression: Expr(text: "src")
                    ),
                    new RuleBinding(
                        Name: Name(value: "doubled"),
                        Kind: CellKind.Int,
                        Expression: Expr(text: "$bind:first * 2")
                    ),
                ]
            ),
        ];
