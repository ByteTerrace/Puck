...
public sealed class HexTopologyLawTests {
    ...
    private static CompiledTopology Compile() => TopologyCompilation.Compile(
        topology: new LatticeTopology.Hex(
            Name: "disk",
    ...
    public void CellsAreHexagonalIndicesAndNeighboursAreTheSixDirections() {
        ...
        Assert.Equal(
            TopologyCompilation.HexDirectionNames,
            Enumerable.Range(
                0,
        ...
        for (var cell = 0; (cell < CellCount); cell++) {
            ...
            for (var direction = 0; (direction < 6); direction++) {
                Assert.Equal(
                    IndexOf(coordinate: (coordinate + HexagonalCoordinate.Direction(direction: direction))),
                    topology.Neighbour(
                        cell: cell,
                        direction: direction
                    )
            ...
            if (cell > 0) {
                Assert.Contains(
                    ...
                    Enumerable.Range(
                        count: 6,
                        start: 0
                    ).Select(selector: direction => topology.Neighbour(
                        cell: (cell - 1),
                        direction: direction
                    ))
                );
    ...
    public void CentresRoundTripThroughPositionToCell() {
        ...
        for (var cell = 0; (cell < CellCount); cell++) {
            ...
            var expectedZ = (3.0 + (((0.5 * coordinate.R) * Math.Sqrt(d: 3.0)) / 2.0));
            ...
            Assert.True(
                condition: (topology.TryCellOf(
                    cell: out var back,
                    position: in centre
                ) && (back == cell)),
                userMessage: $"centre of {cell} resolved to {back}"
            );
            ...
            for (var direction = 0; (direction < 6); direction++) {
                ...
                Assert.True(
                    ...
                    userMessage: $"{cell} toward {next} resolved to {near}"
                );
            }
        }

        var outside = new FixedVector3(
            X: FixedQ4816.FromDouble(value: (1.0 + (0.5 * (Radius + 1)))),
        ...
        for (var cell = 0; (cell < CellCount); cell++) {
            ...
            for (var dq = -2; (dq <= 2); dq++) {
                for (var dr = -2; (dr <= 2); dr++) {
                    var expected = IndexOf(coordinate: (coordinate + new HexagonalCoordinate(
                        Q: dq,
                        R: dr
                    )));
                    var landed = topology.TryOffset(
                        cell: cell,
                        dx: dq,
                        dz: dr,
    ...
    public void SymmetryElementsPermuteCellsAndPreserveAdjacency() {
        ...
        for (var element = 0; (element < topology.ElementCount); element++) {
            ...
            for (var cell = 0; (cell < CellCount); cell++) {
                var image = topology.Image(
                    cell: cell,
                    element: element
                );

                Assert.True(condition: seen.Add(item: image));
                for (var direction = 0; (direction < 6); direction++) {
                    var next = topology.Neighbour(
                        cell: cell,
                        direction: direction
                    ...
                    Assert.Contains(
                        ...
                        Enumerable.Range(
                            count: 6,
                            start: 0
                        ).Select(selector: d => topology.Neighbour(
                            cell: image,
                            direction: d
                        ))
                    );
