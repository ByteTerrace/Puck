...
public sealed class RowVersionSchedulingLawTests {
    ...
    private static (FrameHost Host, CompiledRule[] Rules) Build(bool scheduling) {
        ...
        Rule[] rules = [
            ...
            new Rule(
                ...
                Bindings: [new RuleBinding(
                        Name: Name(value: "doubled"),
                        Kind: CellKind.Int,
                        Expression: Expr(text: "baseVal * 2")
                    )]
            ),
            R(
                name: "flash",
        ...
        return (host, RuleCompiler.CompileAll(
            context: context,
            rules: rules
        ));
    }
    ...
    public void SchedulingProducesIdenticalLatchesEffectsAndFramesAsFullEvaluation() {
        ...
        for (var tick = 1UL; (tick <= 12UL); tick++) {
            ...
            var wroteScheduled = scheduled.Evaluator.Evaluate(
            ...
            Assert.Equal(
                actual: wroteScheduled,
                expected: wroteFull
            );
            Assert.Equal(
                expected: full.Frame.Values.ToArray(),
                actual: scheduled.Frame.Values.ToArray()
            );

            var scheduledHash = new Fnv1aHash();
            var fullHash = new Fnv1aHash();

            scheduledLatch.AppendStateHash(
                compiled: scheduledRules,
                hash: ref scheduledHash
            );
            fullLatch.AppendStateHash(
                compiled: fullRules,
                hash: ref fullHash
            );
            Assert.Equal(
                expected: fullHash.Value,
                actual: scheduledHash.Value
            );
            Assert.Equal(
                expected: full.Refusals,
                actual: scheduled.Refusals
            );
        }
        ...
        Assert.Equal(
            expected: 5L,
            actual: (full.Frame.TryStored(
                row: full.Frame.Find(name: "score")!,
                key: StateRow.SlotKey,
                value: out var score,
                text: out _
            )
