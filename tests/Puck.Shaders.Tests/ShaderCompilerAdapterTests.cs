...
public sealed class ShaderCompilerAdapterTests {
    ...
    public void Adapter_uses_authored_workgroup_dimensions() {
        ...
        Assert.Contains(
            "local_size_x = 16, local_size_y = 4, local_size_z = 2",
            adapted.Text
        );
        var floatOutput = ShadertoyShaderAdapter.Adapt(
            "void mainImage(out vec4 c, in vec2 p) { c = vec4(1); }",
            outputFormat: Puck.Abstractions.Gpu.GpuPixelFormat.R32G32B32A32Float
        );
    ...
    public void Comments_do_not_create_channel_bindings() {
        ...
        Assert.Empty(collection: adapted.Channels);
    ...
    public void Config_fields_use_canonical_ordinal_order_for_vector_packing() {
        ...
        Assert.Contains(
            "layout(offset = 112) float aScalar;",
            adapted.Text
        );
        Assert.Contains(
            "layout(offset = 116) float puck_mVector_0;",
            adapted.Text
        );
        Assert.Contains(
            "layout(offset = 120) float puck_mVector_1;",
            adapted.Text
        );
        Assert.Contains(
            "layout(offset = 124) float puck_mVector_2;",
            adapted.Text
        );
        Assert.Contains(
            "layout(offset = 128) float puck_zVector_0;",
            adapted.Text
        );
        Assert.Contains(
            "layout(offset = 132) float puck_zVector_1;",
            adapted.Text
        );
        Assert.True(condition: (adapted.Text.IndexOf(
            comparisonType: StringComparison.Ordinal,
            value: "float aScalar"
        ) < adapted.Text.IndexOf(
            comparisonType: StringComparison.Ordinal,
            value: "float puck_mVector_0"
        )));
        Assert.True(condition: (adapted.Text.IndexOf(
            comparisonType: StringComparison.Ordinal,
            value: "float puck_mVector_0"
    ...
    public void Config_fields_use_shared_hlsl_offsets_and_glsl_types() {
        ...
        Assert.Contains(
            "layout(offset = 112) uint count;",
            adapted.Text
        );
        Assert.Contains(
            "layout(offset = 116) float puck_gain_0;",
            adapted.Text
        );
        Assert.Contains(
            "layout(offset = 120) float puck_gain_1;",
            adapted.Text
        );
        Assert.Contains(
            "#define gain vec2(puck.puck_gain_0, puck.puck_gain_1)",
            adapted.Text
        );
    }
    [Fact]
    public void Explicit_channel_map_refuses_an_undeclared_channel() {
        var ex = Assert.Throws<InvalidDataException>(testCode: () =>
    ...
    public void Frame_constants_keep_the_cross_backend_wire_size() {
        Assert.Equal(
            ShaderFrameConstants.SizeBytes,
            Marshal.SizeOf<ShaderFrameConstants>()
        );
    ...
    public void Shadertoy_channels_are_bound_by_descriptor_without_blanket_rejection() {
        ...
        var adapted = ShadertoyShaderAdapter.Adapt(
            Source,
            new Dictionary<string, uint> { ["iChannel0"] = 7 }
        );
        ...
        Assert.Contains(
            "mainImage(puckColor, fragCoord)",
            adapted.Text
        );
        Assert.Single(collection: adapted.Channels);
        Assert.Equal(
            7u,
            adapted.Channels[0].Binding
        );
    }
}
