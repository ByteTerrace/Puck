...
// structured buffer of a 4-byte and of a 16-byte element, a raw buffer, and a structured and a raw buffer it writes; the
// pushed index offsets every element the dispatch touches.
...
void CSMain(uint3 id : SV_DispatchThreadID) {
    ...
    uint value = (narrow[element] + raw.Load(element * 4u) + frameGroup.tick + asuint(wideValue.x + wideValue.w));
