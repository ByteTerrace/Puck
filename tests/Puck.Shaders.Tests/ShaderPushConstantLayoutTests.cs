...
public sealed class ShaderPushConstantLayoutTests {
    ...
    private static IReadOnlyList<uint> ReadSpirvMemberOffsets(string path, string structName) {
        ...
        for (var index = 0; (index < words.Length); index++) {
            words[index] = BinaryPrimitives.ReadUInt32LittleEndian(source: bytes.AsSpan(
                length: 4,
                start: (index * 4)
            ));
        }
        ...
        for (var cursor = 5; (cursor < words.Length);) {
            ...
            if (
                ...
                (opcode == SpirvOpMemberDecorate) &&
                (wordCount >= 5) &&
                (words[(cursor + 3)] == SpirvDecorationOffset) &&
                (structId == words[(cursor + 1)])
            ) {
    ...
    public void A_vector_is_bumped_to_the_next_row_only_when_it_would_straddle_one() {
        Assert.Equal(
            expected: [0u, 4u],
            actual: ShaderPushConstantLayout.ComputeOffsets(
    ...
    public void Computed_layout_matches_the_offsets_dxc_assigned_in_the_compiled_spirv() {
        ...
        var spirvOffsets = ReadSpirvMemberOffsets(
            path: manifest.BytecodePath(
        ...
        Assert.Equal(
            expected: layout.Slots.Count,
            actual: spirvOffsets.Count
        );

        for (var index = 0; (index < layout.Slots.Count); index++) {
            Assert.Equal(
                expected: spirvOffsets[index],
                actual: layout.Slots[index].Offset
            );
        }
    }
    [Fact]
    ...
    public void Every_slot_source_resolves_as_declared() {
        ...
        Assert.Equal(
            expected: ShaderPushConstantSourceKind.Config,
            actual: slots[0].Kind
        );
        Assert.Equal(
            expected: "a",
            actual: slots[0].ConfigField
        );
        Assert.Equal(
            expected: ShaderPushConstantSourceKind.Tick,
            actual: slots[4].Kind
        );
        Assert.Equal(
            expected: 24u,
            actual: slots[4].QuantizeHzLiteral
        );
        Assert.Equal(
            expected: ShaderPushConstantSourceKind.Resolution,
            actual: slots[6].Kind
        );
        Assert.Equal(
            expected: ShaderPushConstantSourceKind.Tick,
            actual: slots[10].Kind
        );
        Assert.Equal(
            expected: "rate",
            actual: slots[10].QuantizeHzConfigField
        );
        Assert.Equal(
            expected: ShaderPushConstantSourceKind.Frame,
            actual: slots[11].Kind
        );
    }
}
