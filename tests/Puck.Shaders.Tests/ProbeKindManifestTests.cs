...
public sealed class ProbeKindManifestTests {
    ...
    public void Absent_config_binds_every_default() {
        ...
        Assert.Equal(
            expected: 0.5f,
            actual: BitConverter.UInt32BitsToSingle(value: values["threshold"].ComponentBits(index: 0))
        );
        Assert.Equal(
            expected: 0.02f,
            actual: BitConverter.UInt32BitsToSingle(value: values["minCoverage"].ComponentBits(index: 0))
        );
    }
    [Fact]
    public void Authored_config_overrides_and_refuses_by_field() {
        ...
        Assert.True(condition: manifest.TryBindConfig(
            config: Parse(json: """{ "threshold": 0.6, "minCoverage": 0.05 }"""),
            values: out var values,
            reason: out _
        ));
        Assert.Equal(
        ...
        Assert.False(condition: manifest.TryBindConfig(
            config: Parse(json: """{ "threshold": 1.5 }"""),
            values: out _,
            reason: out var reason
        ));
        Assert.Equal(
            actual: reason,
            expected: "'threshold' must be a number in [0, 1]."
        );
        Assert.False(condition: manifest.TryBindConfig(
            config: Parse(json: """{ "gain": 1 }"""),
            values: out _,
            reason: out reason
        ));
        Assert.Equal(
            actual: reason,
            expected: "'gain' is not a config field of 'ir-blob'."
        );
    }
    [Fact]
    public void Catalog_finds_shipped_kinds_and_refuses_a_duplicate_id() {
        ...
        Assert.Equal(
            expected: "ir-blob",
            actual: catalog.Load(id: "ir-blob").Name
        );
        Assert.Throws<KeyNotFoundException>(testCode: () => catalog.Load(id: "no-such-kind"));
        Assert.Empty(collection: ProbeKindCatalog.Scan(rootDirectory: Path.Combine(
            path1: ProbesDirectory,
    ...
    public void Config_schema_emits_types_ranges_and_defaults() {
        ...
        Assert.Equal(
            expected: "number",
            actual: properties["threshold"]!["type"]!.GetValue<string>()
        );
        Assert.Equal(
            expected: 0d,
            actual: properties["threshold"]!["minimum"]!.GetValue<double>()
        );
        Assert.Equal(
            expected: 1d,
            actual: properties["threshold"]!["maximum"]!.GetValue<double>()
        );
        Assert.Equal(
            expected: 0.5,
            actual: properties["threshold"]!["default"]!.GetValue<double>()
        );
    }
    [Fact]
    public void Constants_block_packs_bound_config_in_declaration_order() {
        ...
        Assert.Equal(
            expected: ProbeKindManifest.ConstantsBlockAlignment,
            actual: block.Length
        );
        Assert.Equal(
            expected: 0.75f,
            actual: BitConverter.ToSingle(
                startIndex: 0,
                value: block
            )
        );
        ...
        Assert.All(
            collection: block[8..],
            action: static padding => Assert.Equal(
                actual: padding,
                expected: ((byte)0)
            )
        );
    ...
    public void Ir_blob_manifest_loads_with_its_channels_and_kernel_source() {
        ...
        Assert.Equal(
            expected: ProbeKindClass.Kernel,
            actual: manifest.Class
        );
        Assert.Equal(
            expected: 0,
            actual: manifest.TriggerSocket
        );
        Assert.Single(collection: manifest.Inputs);
        Assert.Equal(
            expected: "lit",
            actual: manifest.Inputs[0].Name
        );
        Assert.Equal(
            expected: ProbeSocketClass.Frame,
            actual: manifest.Inputs[0].Class
        );
        Assert.False(condition: manifest.Inputs[0].Optional);
        Assert.Null(@object: manifest.Output);
        Assert.Equal(
            expected: ["x", "y", "coverage", "luminance"],
            actual: manifest.Channels.Select(selector: channel => channel.Name).ToArray()
        );
        Assert.Equal(
            expected: Path.GetDirectoryName(path: IrBlobManifestPath),
        ...
        Assert.Equal(
            expected: "ir-blob.hlsl",
            actual: kernel.Source
        );
        Assert.Equal(
            expected: "accumulate",
            actual: kernel.Accumulate
        );
        Assert.Equal(
            expected: "finalize",
            actual: kernel.Finalize
        );
        Assert.True(condition: File.Exists(path: Path.Combine(
            path1: manifest.Directory,
            path2: kernel.Source
        )));
    }
    [Fact]
