...
public sealed class GeneratedNameReversalLawTests {
    ...
    private static readonly IReadOnlyList<Site> Sites = [
        ...
        new(Calls: 4, File: Views, Mints: "a session screen's and a camera seat's view name, a screen source's instance name from its producer and settings digest, and the synthesized root graph's own versions and passes, which exist only at run time"),
