...
namespace Puck.Dynamics.Spike.Tests.Geometry;
...
internal interface ISpikeSurface {
    ...
    void Generate(BodyPose pose, SpikeShape shape, FixedQ4816 activationBound, List<ContactCandidate> output);
}
...
internal sealed class HalfSpaceSurface : ISpikeSurface {
    ...
    /// <inheritdoc/>
    public void Generate(BodyPose pose, SpikeShape shape, FixedQ4816 activationBound, List<ContactCandidate> output) {
    ...
    private void Emit(int featureId, FixedVector3 offset, FixedQ4816 separation, FixedQ4816 radius, FixedQ4816 activationBound, List<ContactCandidate> output) {
        if (separation > activationBound) {
            return;
        }

        output.Add(item: new ContactCandidate(
            SourceId: m_sourceId,
            FeatureId: featureId,
...
/// <summary>
/// A surface described by a standalone <see cref="Puck.SdfVm.SdfProgram"/>, read through the fixed-point evaluator. It
...
internal sealed class FieldSurface : ISpikeSurface {
    ...
    public void Generate(BodyPose pose, SpikeShape shape, FixedQ4816 activationBound, List<ContactCandidate> output) {
        ...
        var (witness, distance) = FindWitness(start: start, end: end, startDistance: startDistance, endDistance: endDistance);
    ...
    private (FixedVector3 Witness, FixedQ4816 Distance) FindWitness(FixedVector3 start, FixedVector3 end, FixedQ4816 startDistance, FixedQ4816 endDistance) {
        ...
        for (var refinement = 0; (refinement < m_refinementSteps); ++refinement) {
            ...
            var lowMidDistance = Distance(point: FixedVector3.Lerp(from: start, to: end, amount: lowMid));
            var highMidDistance = Distance(point: FixedVector3.Lerp(from: start, to: end, amount: highMid));
        ...
        return (FixedVector3.Lerp(from: start, to: end, amount: mid), bestDistance);
    }

    private void EmitAt(BodyPose pose, FixedVector3 witness, FixedQ4816 distance, FixedQ4816 radius, int featureId, FixedQ4816 activationBound, List<ContactCandidate> output) {
