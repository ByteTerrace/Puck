...
public sealed class ContactLawTests {
    ...
    public void SphereRestingInACornerKeepsTwoPersistentContactsWithDistinctNormals() {
        ...
        var floor = FindSlot(world: world, sourceId: SpikeFixtures.FloorSourceId);
        var wall = FindSlot(world: world, sourceId: SpikeFixtures.WallSourceId);
    ...
    public void KeyingASlotByTheBodyFeatureAloneCollapsesTheCornerAndLosesTheFloor() {
        ...
        Assert.True(
            ...
            userMessage: $"the aliased corner must lose the floor contact and let the sphere fall; it rested at y={SpikeReport.Format(value: world.Pose.Center.Y)}"
        );
    }
    ...
    public void CapsuleWaistAgainstAThinSlabIsCaughtWhileBothEndSpheresStayClear() {
        ...
        Assert.True(
            ...
            userMessage: $"both end spheres must stay clear of the slab; the nearer reported {SpikeReport.Format(value: surface.LastEndpointSeparation)}"
        );
        Assert.True(
            condition: (world.Pose.Center.Y > FixedQ4816.FromDouble(value: 0.3d)),
            userMessage: $"the capsule must rest on the slab; it reached y={SpikeReport.Format(value: world.Pose.Center.Y)}"
        );
    }
    ...
    public void SamplingOnlyTheCapsuleEndpointsTunnelsThroughTheSlab() {
        ...
        Assert.True(
            ...
            userMessage: $"a fixed endpoint recipe must miss the waist and let the capsule fall; it reached y={SpikeReport.Format(value: world.Pose.Center.Y)}"
        );
    }
    ...
    public void RotatingBoxSettlesOnThePlaneWithoutJitter() {
        ...
        Assert.Equal(expected: 0, actual: world.Solver.RefusalCount);
        AssertNear(actual: settled, expected: 0.25d, tolerance: 0.002d, subject: "resting height of the box");
        ...
        Assert.True(
            ...
            userMessage: $"the settled box moved between {SpikeReport.Format(value: lowest)} and {SpikeReport.Format(value: highest)}"
        );
        Assert.True(
            condition: (FixedQ4816.Abs(value: world.Body.AngularVelocity.Z) < FixedQ4816.FromDouble(value: 0.01d)),
            userMessage: $"the box's spin must be absorbed; it retained {SpikeReport.Format(value: world.Body.AngularVelocity.Z)}"
        );
        Assert.True(
    ...
    [Fact]
    public void SpeculativeActivationCatchesAFirstAppearanceApproachWithoutTunnelling() {
        ...
        Assert.True(
            ...
            userMessage: $"the swept activation bound was only {SpikeReport.Format(value: world.LastStepActivationBound)}"
        );
    ...
    public void RemovingThePredictedBoundTunnelsThroughTheSurface() {
        var world = SpikeFixtures.HighSpeedApproach(
            options: new() { RateHz = 60, SubstepCount = 1, Activation = SpeculativeActivation.CurrentOnly, },
            height: 1d,
            downwardSpeed: 400d
        );
        ...
        Assert.True(
            ...
            userMessage: $"activation from the current separation alone must tunnel; the body reached y={SpikeReport.Format(value: world.Pose.Center.Y)}"
        );
    }
    ...
    public void NoConstraintAndNoImpulseAppearBeyondThePredictedBound() {
        ...
        Assert.True(
            ...
            userMessage: $"a slow distant body's bound must stay near the authored margin; it was {SpikeReport.Format(value: world.LastStepActivationBound)}"
        );
    }
    ...
    public void AnInjectedDeepOverlapIsRecoveredAlongTheAuthoredEscapeDirection() {
        ...
        Assert.Equal(expected: 0L, actual: firstStepImpulse);
        Assert.True(
            condition: (world.Pose.Center.Y > FixedQ4816.FromDouble(value: 0.55d)),
            userMessage: $"the sphere must be extracted to the side it entered from; it reached y={SpikeReport.Format(value: world.Pose.Center.Y)}"
        );
    }

    [Fact]
    public void WithoutTheRecoveryPathTheOrdinaryCorrectionDrivesTheBodyOutTheWrongSide() {
        var world = SpikeFixtures.DeepOverlap(options: new() { RateHz = 60, SubstepCount = 4, DeepRecovery = false, });

        world.Advance(count: 60);

        Assert.True(
            condition: (world.Pose.Center.Y < FixedQ4816.FromDouble(value: -0.55d)),
            userMessage: $"the nearest-surface normal points through the slab, so the correction clamp must expel the body downward; it reached y={SpikeReport.Format(value: world.Pose.Center.Y)}"
        );
    }

    private static ManifoldSlot FindSlot(SpikeWorld world, int sourceId) {
    ...
    private static void AssertNear(FixedQ4816 actual, double expected, double tolerance, string subject) {
        ...
        Assert.True(condition: (difference <= tolerance), userMessage: $"{subject}: expected {expected}, measured {SpikeReport.Format(value: actual)}");
    }
}
