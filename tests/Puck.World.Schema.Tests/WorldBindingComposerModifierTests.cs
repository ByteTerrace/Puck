...
public sealed class WorldBindingComposerModifierTests {
    ...
    public void ALaterModifierSharingASourceAbsorbsTheEarlierOneAndRewritesEveryChordThatHeldIt() {
        ...
        Assert.Equal(
            expected: "actionWheel",
            actual: modifier.Id
        );
        Assert.Equal(
            expected: ["gamepad.leftShoulder", "keyboard.tab"],
            actual: modifier.Sources
        );
        ...
        Assert.Equal(
            ...
            actual: Assert.Single(
                collection: composed.Chords,
                predicate: static row => ((row.Group == "play") && (row.Chord is { Count: 1 }))
            ).Page!.Id
        );
        Assert.Equal(
            expected: ["actionWheel"],
            actual: Assert.Single(
                collection: composed.Chords,
                predicate: static row => (row.Page?.Id == "editor-wheel")
            ).Chord
        );
    ...
    public void ALaterModifierUnderTheSameIdOverridesInPlaceWithoutRenaming() {
        ...
        Assert.Equal(
            ...
            actual: Assert.Single(
                collection: composed.Chords,
                predicate: static row => (row.Page?.Id == "play-wheel")
            ).Chord
        );
    }
    [Fact]
    public void AbsorbingBothHalvesOfAnExistingChordRefusesByName() {
        ...
        var overlay = new BindingProfileDocument(
            ...
            Modifiers: [new BindingModifierDefinition(
                    Id: "triggers",
                    Sources: ["gamepad.leftTrigger", "gamepad.rightTrigger"]
                )],
            Chords: []
        );

        var exception = Assert.Throws<ArgumentException>(testCode: () => WorldBindingComposer.Compose(
            baseDocument,
            overlay
        ));

        Assert.Contains(
            expectedSubstring: "\"triggers\"",
