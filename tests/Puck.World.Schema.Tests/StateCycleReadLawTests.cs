...
public sealed class StateCycleReadLawTests {
    ...
    public void Envelope_ClampsTheComputedValue_NeverTheStoredPhase() {
        var definition = BuildDefinition(SlotRow(
            name: "spin",
            kind: CellKind.Int,
            value: 0L,
    ...
    public void FixedOutputs_ReadTheRotorTurnsAndComponents() {
        var turns = BuildDefinition(SlotRow(
            ...
            kind: CellKind.Fixed,
            value: 0L,
            cycle: new StateCycle(Output: CycleOutput.Turns)
        ));
        ...
        var sin = BuildDefinition(SlotRow(
            name: "s",
            kind: CellKind.Fixed,
            value: (3L << FixedQ4816.FractionBitCount),
            cycle: new StateCycle(
        ...
        for (var tick = 0UL; (tick < 90UL); ++tick) {
            Assert.Equal(
                expected: ((((long)(tick % 30UL)) << FixedQ4816.FractionBitCount) / 30L),
                actual: Read(
                    definition: turns,
                    row: "t",
                    tick: tick
            ...
            Assert.Equal(
                expected: CyclicRotation.Rotor(step: (CyclicRotation.Step(
                    plane: 3,
                    tick: ((long)tick)
                ) + 3)).Imaginary.Value,
                actual: Read(
    ...
    public void KeyedCells_TurnIndependently_AndAPlainCellStaysStored() {
        ...
        for (var tick = 0UL; (tick < 70UL); ++tick) {
            ...
            Assert.Equal(
                expected: ((int)(((long)CyclicRotation.Step(
                    plane: 1,
                    tick: ((long)(tick / 2UL))
                )) + 10L).FloorModulo(modulus: 30L)),
                actual: Read(
                    definition: definition,
                    key: "b",
                    row: "dials",
    ...
    public void LatticeOutputs_CarryThePhaseNodeAroundItsRing() {
        ...
        var x = BuildDefinition(SlotRow(
            name: "x",
            kind: CellKind.Fixed,
            value: (5L << FixedQ4816.FractionBitCount),
            cycle: new StateCycle(
                Output: CycleOutput.ProjectionX,
                Power: 7
            )
        ));
        var y = BuildDefinition(SlotRow(
            name: "y",
            kind: CellKind.Fixed,
            value: (5L << FixedQ4816.FractionBitCount),
            cycle: new StateCycle(
                Output: CycleOutput.ProjectionY,
                Power: 7
            )
        ));

        for (var tick = 0UL; (tick < 60UL); ++tick) {
            var expected = SymmetryLattice.Cycle(
                node: 5,
                steps: CyclicRotation.Step(
                    plane: 1,
                    tick: ((long)tick)
                )
            );
            ...
            Assert.Equal(
                expected: SymmetryLattice.Project(node: expected).Y.Value,
                actual: Read(
                    definition: y,
                    row: "y",
                    tick: tick
            ...
            Assert.Equal(
                expected: SymmetryLattice.Ring(node: 5),
                actual: SymmetryLattice.Ring(node: ((int)Read(
                    definition: node,
                    row: "n",
                    tick: tick
                )))
        ...
        var walk = BuildDefinition(SlotRow(
            name: "n",
            kind: CellKind.Int,
            value: 17L,
        ...
        Assert.Equal(
            expected: SymmetryLattice.Cycle(node: 17),
            actual: Read(
                definition: walk,
                row: "n",
                tick: 1UL
    ...
    public void SettledPhase_OnAFixedRow_RidesTheRowsEncoding() {
        ...
        Assert.Equal(
            actual: settledRaw & ((1L << FixedQ4816.FractionBitCount) - 1L),
            expected: 0L
        );
        Assert.Equal(
            expected: liveValue,
            actual: Read(
                definition: BuildDefinition(SlotRow(
                    name: "x",
                    kind: CellKind.Fixed,
                    value: settledRaw,
    ...
    public void SettledPhase_PreservesTheCurrentValueAndNextTransition() {
        foreach (var output in new[] { CycleOutput.Step, CycleOutput.Node }) {
            ...
            var live = BuildDefinition(SlotRow(
                name: "r",
                kind: CellKind.Int,
                value: 9L,
                cycle: cycle
        ...
        foreach (var (plane, power) in new[] { (0, 1), (1, 7), (2, 11), (3, 13) }) {
            foreach (var phase in new long[] { 0L, 7L, -1L, 61L }) {
                var definition = BuildDefinition(SlotRow(
                    ...
                    kind: CellKind.Int,
                    value: phase,
                    cycle: new StateCycle(Power: power)
                ));

                foreach (var tick in new ulong[] { 0UL, 1UL, 29UL, 30UL, 31UL, 1000UL, 123456789UL }) {
                    var expected = ((int)(((long)CyclicRotation.Step(
                        plane: plane,
                        tick: ((long)(tick % 30UL))
                    )) + phase).FloorModulo(modulus: 30L));
    ...
    public void Validator_RefusesTheShapesTheReadSideCannotHonour() {
        ...
        Assert.Contains(
            expectedSubstring: "does not suit",
        ...
        Assert.Contains(
            ...
            actualString: Refusal(row: SlotRow(
                ...
                kind: CellKind.Fixed,
                value: 0L,
                cycle: new StateCycle(Output: CycleOutput.Step)
            ))
        );
        Assert.Contains(
            expectedSubstring: "only int/fixed cells turn",
            actualString: Refusal(row: SlotRow(
                name: "r",
                kind: CellKind.Text,
                value: 0L,
        ...
        Assert.Contains(
            expectedSubstring: ".cycle.power 30 is outside",
            actualString: Refusal(row: SlotRow(
                name: "r",
                kind: CellKind.Int,
                value: 0L,
                cycle: new StateCycle(Power: 30)
            ))
        );
        Assert.Contains(
            ...
            actualString: Refusal(row: SlotRow(
                ...
                kind: CellKind.Int,
                value: 0L,
                cycle: new StateCycle(Power: 0)
            ))
        );
        Assert.Contains(
            expectedSubstring: ".cycle.ticksPerStep 0",
            actualString: Refusal(row: SlotRow(
                name: "r",
                kind: CellKind.Int,
                value: 0L,
                cycle: new StateCycle(TicksPerStep: 0)
            ))
        );
        Assert.Contains(
        ...
        Assert.Contains(
            expectedSubstring: "is not a defined CycleOutput",
            actualString: Refusal(row: SlotRow(
                name: "r",
                kind: CellKind.Fixed,
                value: 0L,
                cycle: new StateCycle(Output: unchecked((CycleOutput)byte.MaxValue))
            ))
        );
        Assert.Contains(
        ...
        Assert.Contains(
            expectedSubstring: "declares both advance and cycle",
            actualString: Refusal(row: SlotRow(
                name: "r",
                kind: CellKind.Int,
                value: 0L,
        ...
        Assert.Contains(
            expectedSubstring: "declares cycle beside advance or dynamics",
            actualString: Refusal(row: new WorldStateRow(
                Name: CellName.Parse(candidate: "r"),
                Kind: CellKind.Int,
                Cells: [new StateCell(
        ...
        Assert.Equal(
            ...
            actual: Refusal(row: SlotRow(
                ...
                kind: CellKind.Fixed,
                value: 0L,
                cycle: new StateCycle(Output: CycleOutput.ProjectionY)
            ))
        );
    }
}
