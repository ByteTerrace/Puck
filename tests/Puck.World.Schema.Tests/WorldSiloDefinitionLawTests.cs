...
public sealed class WorldSiloDefinitionLawTests : IDisposable {
    ...
    public void DuplicateWorldId_Refuses() {
        ...
        Assert.Contains(
            actualString: reason,
            expectedSubstring: "declared more than once"
        );
    }
    ...
    public void InstalledClusteringVocabularyIsCaseSensitive(string kind, bool accepted) {
        var definition = MakeValid() with { Clustering = new(Kind: kind) };

        Assert.Equal(
            accepted,
            WorldSiloDefinitionValidator.TryValidate(
                clusteringKinds: ["Localhost"],
    ...
    public void PinnedCountExceedsBudget_Refuses() {
        ...
        Assert.False(condition: WorldSiloDefinitionValidator.TryValidate(
            definition: overPinned,
            reason: out var reason
        ));
        Assert.Contains(
            actualString: reason,
            expectedSubstring: "exceed the declared doors.budget"
        );
    }
    ...
    public void ProviderSettingsRoundTripWithoutSchemaInterpretation() {
        var source = MakeValid() with {
            Store = new WorldSiloExtension(
            "test.store",
            JsonDocument.Parse("""{"region":"test","options":{"count":3}}""").RootElement.Clone()
        ),
        ...
        File.WriteAllBytes(
            path,
            WorldSiloDefinitionSerialization.Serialize(definition: source)
        );
        Assert.True(
            condition: WorldSiloDefinitionSerialization.TryLoadFile(
                definition: out var loaded,
        ...
        Assert.Equal(
            3,
            loaded.Store.Settings.GetProperty(propertyName: "options").GetProperty(propertyName: "count").GetInt32()
        );
        Assert.Equal(
            "test.retirement",
            loaded.Lifecycle!.Observer!.Type
        );
    }
    [Fact]
    public void SerializeThenLoadFile_RoundTrips() {
        ...
        Assert.True(condition: WorldSiloDefinitionSerialization.TryLoadFile(
            definition: out var loaded,
            path: path,
            reason: out var reason
        ));
        ...
        Assert.Equal(
            expected: definition.Worlds.Count,
            actual: loaded!.Worlds.Count
        );
        Assert.Equal(
            expected: definition.Doors.Budget,
            actual: loaded.Doors.Budget
        );
        Assert.Equal(
            expected: definition.Store.Type,
            actual: loaded.Store.Type
        );
        Assert.Equal(
            expected: definition.Clustering.Kind,
            actual: loaded.Clustering.Kind
        );
    }
    [Fact]
    public void SharedKeyFile_Refuses() {
        ...
        Assert.Contains(
            actualString: reason,
            expectedSubstring: "share the key file"
        );
    }
    [Fact]
    public void StoreWithNonObjectSettings_Refuses() {
        ...
        Assert.Contains(
            actualString: reason,
            expectedSubstring: "store requires"
        );
    }
    [Fact]
    public void ValidDocument_Validates() {
        Assert.True(condition: WorldSiloDefinitionValidator.TryValidate(
