...
public sealed class WorldSchemaCompletenessLawTests {
    ...
    private static bool DeclaresArrayType(JsonObject node) {
        if (
            (node["type"] is JsonValue single) &&
            single.TryGetValue<string>(value: out var singleName)
        ) {
        ...
        if (node["type"] is JsonArray many) {
            foreach (var entry in many) {
                if (
                    (entry is JsonValue entryValue) &&
                    entryValue.TryGetValue<string>(value: out var entryName) &&
                    string.Equals(
                    a: entryName,
                    b: "array",
                    comparisonType: StringComparison.Ordinal
    ...
    private static JsonObject ResolveDef(JsonNode node) {
        ...
        if (obj["anyOf"] is JsonArray arms) {
            foreach (var arm in arms) {
                if (
                    (arm is JsonObject armObject) &&
                    armObject.ContainsKey(propertyName: "$ref")
                ) {
    ...
    private static void Walk(JsonNode? node, string pointer, HashSet<JsonNode> visited, Action<JsonObject, string> visit) {
        if (node is JsonObject obj) {
            if (
                obj.ContainsKey(propertyName: "$ref") ||
                !visited.Add(item: obj)
            ) {
                return;
            }
            ...
            if (
                (pointer != "#") &&
                obj.ContainsKey(propertyName: "$id")
            ) {
                return;
            }
    ...
    public void EveryDefTitleEqualsItsKey() {
        ...
        foreach (var (name, defNode) in defs) {
            if (
                ((defNode as JsonObject)?["title"] is not JsonValue titleValue) ||
    ...
    public void EveryGameFragmentValidatesAgainstTheBundle(string path) =>
        AssertValidatesAgainstBundle(
            instance: ParseFragment(path: path),
            label: path
        );
    ...
    public void RootIsSelfIdentifying() {
        ...
        Assert.Equal(
            expected: WorldDefinition.SchemaVersion,
            actual: ((string?)identity["schemaVersion"])
        );
        ...
        Assert.Equal(
            expected: WorldDefinition.SchemaVersion,
            actual: ((string?)Bundle["properties"]?["schema"]?["const"])
        );
    }
    [Fact]
    public void SiloRootIsSelfIdentifying() {
        ...
        Assert.Equal(
            expected: WorldSiloDefinition.SchemaVersion,
            actual: ((string?)identity["schemaVersion"])
        );
        Assert.Equal(
            expected: WorldSiloDefinition.SchemaVersion,
            actual: ((string?)silo["properties"]?["schema"]?["const"])
        );
    }
    [Fact]
    ...
    public void TheAllowlistNamesNoAlreadyTypedLeaf() {
        ...
        Assert.True(
            condition: (typed.Count == 0),
            userMessage: $"already typed — drop from the allowlist:\n{string.Join(
                separator: "\n",
                values: typed
            )}"
        );
    ...
    public void UnconstrainedDefinitionsStayExplicit() {
        ...
        Assert.Equal(
            actual: gaps,
            expected: ["ChannelRef", "ClosedBitset256", "CommandValue", "WorldLatticeScalar", "WorldScreenResolution"]
        );
    }
}
