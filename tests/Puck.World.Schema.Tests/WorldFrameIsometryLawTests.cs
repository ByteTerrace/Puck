...
public sealed class WorldFrameIsometryLawTests {
    ...
    private static readonly FixedQ4816 PiRadians = FixedQ4816.FromDouble(value: Math.PI);
    ...
    private static WorldFaceFrame BuildFrame(FixedVector3 origin, double yawDegrees) {
        var rotation = FixedQuaternion.FromAxisAngle(
            axis: UpAxis,
            angle: DegreesToRadians(degrees: yawDegrees)
        );

        return new WorldFaceFrame(
            Origin: origin,
            Right: rotation.Rotate(vector: new FixedVector3(
                X: FixedQ4816.One,
                Y: FixedQ4816.Zero,
                Z: FixedQ4816.Zero
            )).Normalize(),
            Up: UpAxis,
            Normal: rotation.Rotate(vector: Forward).Normalize(),
    ...
    public void MapArrival_DifferentYawPair_MapsHeadingAndVelocityThroughTheSameIsometry() {
        ...
        var sourceFrame = BuildFrame(
            origin: FixedVector3.Zero,
            yawDegrees: 30.0
        );
        var destinationFrame = BuildFrame(
            origin: new FixedVector3(
                X: FixedQ4816.FromInteger(value: 5),
                Y: FixedQ4816.Zero,
                Z: FixedQ4816.FromInteger(value: 5)
            ),
            yawDegrees: 120.0
        );

        var travelerYaw = DegreesToRadians(degrees: 15.0);
        var travelerPlanarVelocity = new FixedVector3(
        ...
        var mapped = WorldFrameIsometry.MapArrival(
            travelerPosition: new FixedVector3(
        ...
        var sourceFacing = FixedQuaternion.FromAxisAngle(
            angle: travelerYaw,
            axis: UpAxis
        ).Rotate(vector: Forward);
        var expectedFacing = WorldFrameIsometry.MapVector(
            destination: in destinationFrame,
            source: in sourceFrame,
            value: sourceFacing
        );
        var actualFacing = FixedQuaternion.FromAxisAngle(
            axis: UpAxis,
            angle: mapped.YawRadians
        ).Rotate(vector: Forward);
        ...
        Assert.Equal(
            expected: (travelerYaw + WorldFrameIsometry.YawDelta(
                destination: in destinationFrame,
                source: in sourceFrame
            )),
            actual: mapped.YawRadians
        );
        ...
        var expectedVelocity = WorldFrameIsometry.MapVector(
            destination: in destinationFrame,
            source: in sourceFrame,
            value: velocity
        );
        ...
        Assert.Equal(
            expected: expectedVelocity.X,
            actual: mapped.PlanarVelocity.X
        );
        Assert.Equal(
            expected: expectedVelocity.Z,
            actual: mapped.PlanarVelocity.Z
        );
        Assert.Equal(
            expected: expectedVelocity.Y,
            actual: mapped.VerticalVelocity
        );
        Assert.Equal(
            expected: FixedQ4816.Zero,
            actual: mapped.PlanarVelocity.Y
        );
    }
    [InlineData(90.0, 0.0)]
    [InlineData(0.0, 90.0)]
    ...
    public void MapArrival_OffCentreCrossing_LandsAtItsCounterpartPoint(double sourceYawDegrees, double destinationYawDegrees) {
        ...
        var sourceFrame = BuildFrame(
            origin: new FixedVector3(
                X: FixedQ4816.FromInteger(value: -10),
                Y: FixedQ4816.FromDouble(value: 1.5),
                Z: FixedQ4816.FromInteger(value: 6)
            ),
            yawDegrees: sourceYawDegrees
        );
        var destinationFrame = BuildFrame(
            origin: new FixedVector3(
                X: FixedQ4816.Zero,
                Y: FixedQ4816.FromDouble(value: 1.5),
                Z: FixedQ4816.FromInteger(value: -8)
            ),
            yawDegrees: destinationYawDegrees
        );

        var seamU = FixedQ4816.FromDouble(value: 1.25);
        var seamV = FixedQ4816.FromDouble(value: 0.4);
        var crossingPoint = sourceFrame.PointAt(
            u: seamU,
            v: seamV
        );
        ...
        var counterpart = destinationFrame.PointAt(
            u: -seamU,
            v: seamV
        );
        ...
        Assert.True(condition: (RawAbs(
            a: mapped.Position,
            b: destinationFrame.PointAt(
                u: seamU,
                v: seamV
            )
        ) > SeamCorrespondenceBudgetRaw));
    ...
    public void MapArrival_ReciprocalRoundTrip_StaysWithinTheMeasuredErrorBudget() {
        ...
        var travelerConfigs = new (FixedVector3 Offset, FixedVector3 PlanarVelocity, FixedQ4816 VerticalVelocity, string Label)[] {
            (new FixedVector3(
            X: FixedQ4816.FromDouble(value: 2.4),
            Y: FixedQ4816.FromDouble(value: 1.8),
            Z: FixedQ4816.FromDouble(value: -2.9)
        ),
                new FixedVector3(
            X: FixedQ4816.FromDouble(value: 12.0),
            Y: FixedQ4816.Zero,
            Z: FixedQ4816.FromDouble(value: -9.0)
        ),
                FixedQ4816.FromDouble(value: -4.5), "moderate"),
            (new FixedVector3(
            X: FixedQ4816.FromDouble(value: 1.4),
            Y: FixedQ4816.FromDouble(value: 1.9),
            Z: FixedQ4816.FromDouble(value: 1.9)
        ),
                new FixedVector3(
            X: FixedQ4816.FromDouble(value: 28.0),
            Y: FixedQ4816.Zero,
            Z: FixedQ4816.FromDouble(value: -19.0)
        ),
            ...
            Y: FixedQ4816.Zero,
            Z: FixedQ4816.FromDouble(value: 0.3)
        ),
                FixedQ4816.FromDouble(value: 0.1), "small everything"),
        };

        var sourceOrigin = new FixedVector3(
            X: FixedQ4816.FromDouble(value: 12.0),
            Y: FixedQ4816.Zero,
            Z: FixedQ4816.FromDouble(value: -18.0)
        );
        var destinationOrigin = new FixedVector3(
            X: FixedQ4816.FromDouble(value: -40.0),
            Y: FixedQ4816.FromDouble(value: 2.0),
            Z: FixedQ4816.FromDouble(value: 55.0)
        );
        var travelerYaw = DegreesToRadians(degrees: 15.0);

        foreach (var (offset, planarVelocity, verticalVelocity, configLabel) in travelerConfigs) {
            var travelerPosition = new FixedVector3(
                X: (sourceOrigin.X + offset.X),
                Y: (sourceOrigin.Y + offset.Y),
                Z: (sourceOrigin.Z + offset.Z)
            );
            ...
            foreach (var (sourceYawDegrees, destinationYawDegrees, yawLabel) in yawPairsDegrees) {
                ...
                var positionErrorRaw = RawAbs(
                    a: backward.Position,
                    b: travelerPosition
                );
                var velocityErrorRaw = Math.Max(
                    val1: RawAbs(
                        a: backward.PlanarVelocity,
                        b: planarVelocity
                    ),
                    val2: RawAbs(
                        a: backward.VerticalVelocity,
                        b: verticalVelocity
                    )
                );
                ...
                var expectedFacing = FixedQuaternion.FromAxisAngle(
                    angle: travelerYaw,
                    axis: UpAxis
                ).Rotate(vector: Forward);
                var actualFacing = FixedQuaternion.FromAxisAngle(
                    axis: UpAxis,
                    angle: backward.YawRadians
                ).Rotate(vector: Forward);
                var facingErrorRaw = RawAbs(
                    a: actualFacing,
                    b: expectedFacing
                );

                Assert.True(
                    condition: (positionErrorRaw <= PositionErrorBudgetRaw),
                    userMessage: $"{caseLabel}: position round-trip error {positionErrorRaw} raw exceeds the {PositionErrorBudgetRaw}-raw budget"
                );
                Assert.True(
                    condition: (velocityErrorRaw <= VelocityErrorBudgetRaw),
                    userMessage: $"{caseLabel}: velocity round-trip error {velocityErrorRaw} raw exceeds the {VelocityErrorBudgetRaw}-raw budget"
                );
                Assert.True(
                    condition: (facingErrorRaw <= SeamCorrespondenceBudgetRaw),
                    userMessage: $"{caseLabel}: facing round-trip error {facingErrorRaw} raw exceeds the {SeamCorrespondenceBudgetRaw}-raw budget"
                );
            }
        }
    }
    ...
    public void YawDelta_OpposedPair_IsTheIdentityControl() {
        ...
        var mapped = WorldFrameIsometry.MapArrival(
            ...
            travelerPlanarVelocity: new FixedVector3(
                X: FixedQ4816.One,
                Y: FixedQ4816.Zero,
                Z: -FixedQ4816.FromInteger(value: 2)
            ),
            travelerVerticalVelocity: FixedQ4816.Zero,
            source: in frame,
        ...
        Assert.True(condition: (Math.Abs(value: WorldFrameIsometry.YawDelta(
            destination: in counterpart,
            source: in frame
        ).Value) <= YawErrorBudgetRaw));
        Assert.True(condition: (RawAbs(
            a: mapped.Position,
            b: offset
        ) <= SeamCorrespondenceBudgetRaw));
    ...
    public void YawDelta_SameYawPair_IsExactlyTheHalfTurn() {
        ...
        var counterpart = BuildFrame(
            origin: new FixedVector3(
                X: FixedQ4816.FromInteger(value: 9),
                Y: FixedQ4816.Zero,
                Z: FixedQ4816.FromInteger(value: -3)
            ),
            yawDegrees: 45.0
