...
public sealed class WorldFieldProgramLawTests {
    ...
    public void AFieldHandleFromAnotherProgramIsRejectedEvenWhenItsOrdinalFits() {
        ...
        var first = WorldFieldProgram.Compile(
            document: fields,
            state: catalog
        );
        var second = WorldFieldProgram.Compile(
            document: fields,
            state: catalog
        );
    ...
    public void Compile_AdditiveTransformDeclaresItsTargetAsAReadAndWrite() {
        ...
        var transform = Assert.IsType<WorldFieldNode.Transform>(@object: Assert.Single(collection: WorldFieldProgram.Compile(
            document: fields,
        ...
        Assert.Equal(
            expected: [0],
            actual: transform.FieldReads.Select(selector: static handle => handle.Ordinal)
        );
        Assert.Equal(
            expected: [0],
    ...
    public void Compile_EmissionDeclaresItsTargetAsAReadAndWrite() {
        var state = BuildState(reactions: [new WorldReaction.Emit(
                Amount: 1f,
                Field: "heat",
                Tag: "burning"
            )]);
        ...
        var emit = Assert.IsType<WorldFieldNode.Emit>(@object: Assert.Single(collection: WorldFieldProgram.Compile(
            document: fields,
            state: StateCatalog.Compile(section: state)
        ).Nodes));
        ...
        Assert.Equal(
            expected: [0],
            actual: emit.FieldReads.Select(selector: static handle => handle.Ordinal)
        );
        Assert.Equal(
            expected: [0],
            actual: emit.FieldWrites.Select(selector: static handle => handle.Ordinal)
        );
    }
    [Fact]
    public void Compile_LowersOrderedReactionsToTypedNodesAndCanonicalDependencies() {
        ...
        Assert.Equal(
            expected: 4,
            actual: program.Nodes.Count
        );
        Assert.Equal(
            expected: 2,
            actual: program.CellNodeCount
        );
        Assert.Equal(
            expected: 3,
            actual: program.CellPassCount
        );
        Assert.Equal(
            expected: 2,
            actual: program.BodyPassCount
        );
        Assert.Equal(
            expected: 12,
            actual: program.CellCount
        );
        Assert.Equal(
            expected: 6,
            actual: program.Dependencies.Count
        );
        Assert.Equal(
        ...
        Assert.Equal(
            expected: 0,
            actual: diffuse.Handle.Ordinal
        );
        Assert.Equal(
            expected: 0,
            actual: diffuse.Field.Ordinal
        );
        Assert.True(condition: diffuse.Rate.IsState);
        Assert.Equal(
            expected: "season",
            actual: catalog[diffuse.Rate.State].Name
        );
        Assert.Same(
            expected: catalog,
            actual: program.StateCatalog
        );
        ...
        Assert.Equal(
            expected: "season",
            actual: catalog[transform.StateReads[0]].Name
        );

        var emit = Assert.IsType<WorldFieldNode.Emit>(@object: program.Nodes[2]);

        Assert.Equal(
            expected: WorldFieldWorkKind.Bodies,
            actual: emit.Work
        );
        Assert.Equal(
            expected: [0],
        ...
        Assert.Equal(
            expected: "burning",
            actual: catalog[emit.Tag].Name
        );
        Assert.Equal(
            expected: ["season", "burning"],
            actual: emit.StateReads.Select(selector: handle => catalog[handle].Name)
        );
        ...
        Assert.Equal(
            expected: "exposed",
            actual: catalog[expose.Row].Name
        );
    }
    [Fact]
    public void Compile_NodeCollectionsAreImmutableAfterDependencyAnalysis() {
        ...
        var alteredWrites = transform.FieldWrites.SetItem(
            index: 0,
            item: default
        );
        ...
        Assert.Equal(
            expected: originalWrite,
            actual: transform.FieldWrites[0]
        );
        Assert.Equal(
            expected: 6,
            actual: program.Dependencies.Count
    ...
    public void Compile_QuantizesLiteralsOnceAtTheCompilerBoundary() {
        var state = BuildState(reactions: [new WorldReaction.Decay(
                Field: "heat",
                Rate: 0.125f
            )]);
        var fields = Assert.IsType<WorldFieldsSection>(@object: WorldFieldsSection.Compile(state: state));
    ...
    public void Compile_StateWriteToReadCreatesAnEdgeBetweenOtherwiseIndependentFields() {
        ...
        Assert.Equal(
            expected: 0,
            actual: dependency.Before.Ordinal
        );
        Assert.Equal(
            expected: 1,
            actual: dependency.After.Ordinal
        );
    }
    [Fact]
    ...
    public void WorldDefinition_FieldProgram_RecompilesWhenAWithExpressionReplacesState() {
        ...
        Assert.Equal(
            expected: 4,
            actual: originalReactions.Count
        );
        Assert.Equal(
            expected: 4,
            actual: originalProgram.Nodes.Count
        );
        ...
        Assert.Equal(
            expected: "cold",
            actual: Assert.Single(collection: replacementFields.Fields).Name
        );
        ...
        Assert.Equal(
            expected: "cold",
            actual: Assert.Single(collection: replacementProgram.Fields).Name
        );
        Assert.IsType<WorldFieldNode.Decay>(@object: replacementProgram.Nodes[0]);
        Assert.NotSame(
            actual: replacementProgram,
            expected: originalProgram
        );
    ...
    public void WorldDefinition_InPlaceReactionArrayMutationCannotLeaveAStaleProgram() {
        WorldReaction[] reactions = [new WorldReaction.Decay(
                Field: "heat",
                Rate: 0.25f
            )];
        ...
        reactions[0] = new WorldReaction.Diffuse(
            Field: "heat",
            Rate: 0.5f
        );
    ...
    public void WorldDefinition_PaintAndColorOnlyChangesPreserveProgramButEnvelopeChangesReplaceIt() {
        ...
        Assert.NotSame(
            expected: originalFields,
            actual: cosmetic.Fields
        );
        Assert.Same(
            expected: originalProgram,
            actual: cosmetic.FieldProgram
        );
        Assert.Equal(
            expected: "heat",
            actual: cosmetic.FieldProgram![originalHandle].Name
        );
    ...
    public void WorldDefinition_ValueOnlyStateUpdatePreservesCompiledFieldsCatalogProgramAndHandles() {
        ...
        Assert.Same(
            expected: program,
            actual: updated.FieldProgram
        );
        Assert.Equal(
            expected: "season",
            actual: updated.StateCatalog[season].Name
        );
        ...
        Assert.Same(
            expected: fields,
            actual: unrelatedSectionEdit.Fields
        );
        Assert.Same(
            expected: catalog,
            actual: unrelatedSectionEdit.StateCatalog
        );
        Assert.Same(
            expected: program,
            actual: unrelatedSectionEdit.FieldProgram
        );
        ...
        Assert.Same(
            expected: fields,
            actual: chainedEdit.Fields
        );
        Assert.Same(
            expected: catalog,
            actual: chainedEdit.StateCatalog
        );
        Assert.Same(
            expected: program,
            actual: chainedEdit.FieldProgram
        );
    }
}
