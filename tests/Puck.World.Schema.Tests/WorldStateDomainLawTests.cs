...
public sealed class WorldStateDomainLawTests {
    private static WorldDefinition Document(params WorldStateRow[] rows) => new(
        ...
        StateRaw: new WorldStateSection(World: rows, Lattices: [
            new LatticeTopology.Grid(Name: "board", Origin: new Puck.Assets.Documents.DocumentVector3(0, 0, 0), CellSize: 1, Width: 2, Depth: 2),
        ])
    );
    ...
    public void UnauthoredRow_InfersSlotOrKeys_FromCellsAlone() {
        ...
        Assert.Same(expected: StateDomain.Slot.Instance, actual: emptySlot.EffectiveDomain);
        Assert.Same(expected: StateDomain.Keys.Instance, actual: byCapacity.EffectiveDomain);
    }

    [Fact]
    public void KeysOf_OrderedGivesZoneSemanticsAndUnorderedGivesAttributeSemantics() {
        var zone = new WorldStateRow(CellName.Parse("pile"), CellKind.Bool, Domain: new StateDomain.KeysOf(CellName.Parse("cards"), Ordered: true), Capacity: 1, Cells: [new(CellName.Parse("t1"), 1)]);
        var attribute = new WorldStateRow(CellName.Parse("rank"), CellKind.Int, Domain: new StateDomain.KeysOf(CellName.Parse("cards")), Cells: [new(CellName.Parse("t1"), 7)]);
    ...
    public void KeysOf_RefusesAnEmptyDomainAndABoardDomain() {
        ...
        var board = new WorldStateRow(CellName.Parse("board"), CellKind.Int, Domain: new StateDomain.CellsOf(Topology: "board"));
        var attributeOverBoard = new WorldStateRow(CellName.Parse("owner"), CellKind.Int, Domain: new StateDomain.KeysOf(CellName.Parse("board")));
    ...
    public void CellsOf_AddressesATopologyAndAdmitsAnUnwrittenEmptyFill() {
        var board = new WorldStateRow(CellName.Parse("occupancy"), CellKind.Int, Domain: new StateDomain.CellsOf(Topology: "board", Empty: -1), Cells: [new(CellName.Parse("0"), 3)]);
    ...
    public void Ring_KeepsCapacitySlotsAndAnEmptyFillOffTheDomainNotTheRow() {
        var ring = new WorldStateRow(CellName.Parse("taps"), CellKind.Int, Domain: new StateDomain.Ring(Capacity: 3, Empty: -1), HistoryCursor: 1, Cells: [new(CellName.Parse("0"), 9)]);
    ...
    public void EveryCase_RoundTripsThroughTheDollarTypeDiscriminatedWire() {
        var slot = new WorldStateRow(CellName.Parse("slot"), CellKind.Int, Domain: new StateDomain.Slot(), Cells: [new(WorldStateRow.SlotKey, 1)]);
        var keys = new WorldStateRow(CellName.Parse("keys"), CellKind.Int, Domain: new StateDomain.Keys(), Capacity: 2, Cells: [new(CellName.Parse("a"), 1)]);
        var keysOf = new WorldStateRow(CellName.Parse("keysOf"), CellKind.Int, Domain: new StateDomain.KeysOf(CellName.Parse("keys"), Ordered: false), Cells: [new(CellName.Parse("a"), 1)]);
        var cellsOf = new WorldStateRow(CellName.Parse("cellsOf"), CellKind.Int, Domain: new StateDomain.CellsOf(Topology: "board", Empty: 2));
        var ring = new WorldStateRow(CellName.Parse("ring"), CellKind.Int, Domain: new StateDomain.Ring(Capacity: 4, Empty: 7));

        var definition = Document(slot, keys, keysOf, cellsOf, ring);
        Assert.Equal(expected: string.Empty, actual: Validate(definition));

        Assert.IsType<StateDomain.Slot>(@object: RoundTrip(definition, "slot").Domain);
        Assert.IsType<StateDomain.Keys>(@object: RoundTrip(definition, "keys").Domain);
    ...
    public void UnauthoredPhaseRow_InfersKeys_NeverSlot() {
        var phase = new WorldStateRow(CellName.Parse("turns"), CellKind.Int, Phase: new StatePhase(Sequence: 1));
    ...
    public void CellCeiling_HonoursAnAuthoredCapacityUpToTheOneBoundAndDefaultsTheRoomOtherwise() {
        ...
        var linked = new WorldStateRow(CellName.Parse("linked"), CellKind.Int, Domain: new StateDomain.KeysOf(CellName.Parse("domain")), Capacity: 200);
        var unauthored = new WorldStateRow(CellName.Parse("room"), CellKind.Int, Cells: [new StateCell(CellName.Parse("a"), 0L)]);
        var oversized = new WorldStateRow(CellName.Parse("big"), CellKind.Int, Capacity: StateCapacity.MaxCellsPerRow + 1);

        Assert.Equal(expected: 200, actual: ordinary.CellCeiling);
        Assert.Equal(expected: 200, actual: linked.CellCeiling);
        Assert.Equal(expected: StateCapacity.DefaultCellRoom, actual: unauthored.CellCeiling);
        Assert.Equal(expected: StateCapacity.MaxCellsPerRow, actual: oversized.CellCeiling);
    }
}
