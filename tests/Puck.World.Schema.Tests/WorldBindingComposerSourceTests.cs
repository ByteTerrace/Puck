...
public sealed class WorldBindingComposerSourceTests {
    ...
    public void ALaterLayersNarrowerRowReplacesOnlyTheSourceItNamesLeavingTheRestOfTheEarlierEntryInPlace() {
        ...
        Assert.Equal(
            expected: ["gamepad.buttonSouth"],
            actual: Assert.Single(
                collection: entries,
                predicate: static entry => (entry.Command == "overlay.jump")
            ).Sources
        );
        Assert.Equal(
            expected: ["keyboard.space"],
            actual: Assert.Single(
                collection: entries,
                predicate: static entry => (entry.Command == "base.jump")
            ).Sources
        );
    }
    [Fact]
    public void ALaterLayersRowClaimingEveryOneOfTheEarlierEntrysSourcesReplacesItWhollyAndStaysCombined() {
        ...
        Assert.Equal(
            expected: ["gamepad.buttonSouth", "keyboard.space"],
            actual: entry.Sources
        );
    }
}
