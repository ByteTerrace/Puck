...
public sealed class HudBindingTargetFacetTests {
    ...
    public void KeyedTokenNamedTargetWithoutTheDollarSign_ParsesAsAnOrdinaryKeyWithTargetUnset() {
        Assert.True(condition: HudBindingVocabulary.TryParse(
            binding: out var parsed,
            token: "state.hp.target"
        ));
        Assert.Equal(
            actual: parsed.Kind,
            expected: HudBindingKind.StateNamed
        );
        Assert.Equal(
            actual: parsed.StateName,
            expected: "hp"
        );
        Assert.Equal(
            actual: parsed.StateCellKey,
            expected: "target"
        );
    ...
    public void TargetFacetWithAnEmptyRow_Refuses() {
        Assert.False(condition: HudBindingVocabulary.TryParse(
            binding: out _,
            token: "state.$target"
        ));
    }
