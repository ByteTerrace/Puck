...
public sealed class WorldFaceRegionLawTests {
    ...
    private static WorldFaceCrossingClaim Claim(string placement, string face, double parameter) =>
        new(
            ...
            FaceName: face,
            Parameter: FixedQ4816.FromDouble(value: parameter)
        );
    ...
    public void ACrossingFiresOnTheEdgeIn_AndOnceOnly() {
        ...
        Assert.True(condition: Observe(
            crossing: inside,
            occupancy: occupancy
        ));
        Assert.False(condition: Observe(
            crossing: inside,
            occupancy: occupancy
        ));
    }
    ...
    public void ATunnellingCrossingFiresWithoutLatching() {
        ...
        Assert.False(condition: occupancy.IsInside(
            faceName: Face,
            placementId: Placement,
            seat: 0
        ));
    ...
    public void ArrivalSeedsOccupancy_SoAMappedPairCannotBounceTheTravelerBack() {
        ...
        Assert.False(
            ...
            userMessage: "a seeded arrival must not fire the door it arrived at"
        );
        Assert.True(
            condition: Observe(
                crossing: standing,
            ...
            userMessage: "the control body, which walked in, must still fire"
        );
    }
    [Fact]
    public void BackToFrontTunnelling_MustNotFire() {
        ...
        Assert.False(
            condition: behindToFront.Crossed,
            userMessage: "a sweep from behind the face, through the band, and out the front is not a front-entry crossing"
        );
        Assert.Equal(
            expected: FixedQ4816.Zero,
            actual: behindToFront.Parameter
        );

        // The forward direction over the identical band is the control that proves the gate is direction-aware, not
        // just broken.
        var frontToBehind = Sweep(
            from: At(
                x: 0,
                y: 0,
                z: 2
        ...
        Assert.True(
            condition: frontToBehind.Crossed,
            userMessage: "the forward direction over the same band must still fire"
        );
    }
    [Fact]
    ...
    public void SeedingOneSeatDoesNotSilenceAnother() {
        ...
        occupancy.SeedInside(
            faceName: Face,
            placementId: Placement,
            seat: 0
        );

        Assert.False(condition: occupancy.Observe(
            placementId: Placement,
            faceName: Face,
            seat: 0,
            inside: standing.Inside,
            crossed: standing.Crossed
        ));
        Assert.True(condition: occupancy.Observe(
            placementId: Placement,
            faceName: Face,
            seat: 1,
            inside: standing.Inside,
    ...
    public void TheCrossingParameterIsTheEarliestMeetingPoint() {
        ...
        Assert.True(
            condition: (FixedQ4816.Abs(value: (crossing.Parameter - expected)) <= FixedQ4816.FromDouble(value: 0.001)),
            userMessage: $"parameter {((double)crossing.Parameter)} is not the band entry {((double)expected)}"
        );
    }
    [Fact]
    ...
    public void TheOrderingIsTotalAndAntisymmetric() {
        ...
        foreach (var left in claims) {
            ...
            foreach (var right in claims) {
                ...
                Assert.True(
                    condition: (left.Outranks(other: right) != right.Outranks(other: left)),
                    userMessage: $"{left} vs {right} is not antisymmetric"
                );
            }
        }
    }
    ...
    public void TheSeamIsReadAtTheCrossingParameter_NotAtEitherEndpoint() {
        ...
        Assert.True(condition: crossing.Crossed);
        Assert.True(
            condition: (FixedQ4816.Abs(value: (crossing.SeamU - expectedU)) <= tolerance),
            userMessage: $"seam u {((double)crossing.SeamU)} != {((double)expectedU)}"
        );
        Assert.True(
            condition: (FixedQ4816.Abs(value: (crossing.SeamV - expectedV)) <= tolerance),
            userMessage: $"seam v {((double)crossing.SeamV)} != {((double)expectedV)}"
        );
        Assert.NotEqual(
            expected: crossing.SeamU,
            actual: FixedQ4816.FromDouble(value: -1.0)
        );
        Assert.NotEqual(
            expected: crossing.SeamU,
            actual: FixedQ4816.One
        );
    }
    ...
    private static WorldFaceAperture Aperture { get; } = new WorldFaceAperture.Box(
        Frame: Frame,
        Depth: FixedQ4816.FromDouble(value: Depth)
    );
}
