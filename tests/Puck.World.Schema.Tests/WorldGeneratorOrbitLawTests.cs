...
public sealed class WorldGeneratorOrbitLawTests {
    ...
    private static StateGenerator RingSource(int ring, GeneratorMode mode = GeneratorMode.WithReplacement) => new(
        Source: GeneratorSource.SymmetryOrbit,
        Mode: mode,
        Ring: ring
    );
    private static StateGenerator NodeSource(int node, IReadOnlyList<int>? word, GeneratorMode mode = GeneratorMode.WithReplacement) => new(
        Source: GeneratorSource.SymmetryOrbit,
        Mode: mode,
        Node: node,
        Word: word
    ...
    private static string Validate(StateGenerator generator, CellKind kind = CellKind.Int, long? max = null, IReadOnlyList<ClosedBitset256>? masks = null) {
    ...
    public void ARingSource_DrawsEveryNodeOfTheRingOncePerPass() {
        var source = RingSource(ring: 2, mode: GeneratorMode.WithoutReplacement);
        var cursor = 0L;
        IReadOnlyList<ClosedBitset256>? masks = null;
        var drawn = new List<long>();

        Assert.True(condition: GeneratorEngine.TryResolveOrbit(generator: source, nodes: out var nodes, reason: out var orbitReason), userMessage: orbitReason);
    ...
    public void ANodeSource_DrawsTheOrbitUnderItsWord_OrTheNodesRingWithoutOne() {
        ...
        var source = NodeSource(node: 5, word: letters, mode: GeneratorMode.RestartOnExhaustion);

        Assert.True(condition: GeneratorEngine.TryResolveOrbit(generator: source, nodes: out var nodes, reason: out var reason), userMessage: reason);
        ...
        Assert.True(condition: GeneratorEngine.TryResolveOrbit(generator: NodeSource(node: 17, word: null), nodes: out var ringNodes, reason: out _));
    ...
    public void Validator_RefusesEveryShapeThatNamesNoSingleOrbit() {
        ...
        Assert.Contains(expectedSubstring: "neither 'ring' nor 'node'", actualString: Validate(generator: new StateGenerator(Source: GeneratorSource.SymmetryOrbit)));
        Assert.Contains(expectedSubstring: "both 'ring' and 'node'", actualString: Validate(generator: new StateGenerator(Source: GeneratorSource.SymmetryOrbit, Ring: 1, Node: 4)));
        Assert.Contains(expectedSubstring: "'word' beside 'ring'", actualString: Validate(generator: new StateGenerator(Source: GeneratorSource.SymmetryOrbit, Ring: 1, Word: [0])));
        ...
        Assert.Equal(expected: string.Empty, actual: Validate(generator: RingSource(ring: 0, mode: GeneratorMode.WithoutReplacement), masks: [new(Word0: 0b101UL)]));
        Assert.Contains(expectedSubstring: "marks an entry past the 30", actualString: Validate(generator: RingSource(ring: 0, mode: GeneratorMode.WithoutReplacement), masks: [new(Word0: (1UL << 30))]));
        Assert.Contains(expectedSubstring: "never exhausts", actualString: Validate(generator: RingSource(ring: 0), masks: [new(Word0: 1UL)]));
        Assert.Contains(expectedSubstring: "exactly one", actualString: Validate(generator: RingSource(ring: 0, mode: GeneratorMode.WithoutReplacement), masks: [new(Word0: 1UL), new(Word0: 2UL)]));
    }
    [Fact]
    public void ALatticeFill_NeedsEnoughUndrawnUnits() {
        var source = RingSource(ring: 3, mode: GeneratorMode.WithoutReplacement);

        Assert.True(condition: GeneratorEngine.TryCheckBatchCapacity(generator: source, masks: null, sampleCount: 30L, reason: out _));
        Assert.False(condition: GeneratorEngine.TryCheckBatchCapacity(generator: source, masks: null, sampleCount: 31L, reason: out var reason));
        Assert.Contains(expectedSubstring: "can supply only 30", actualString: reason);
        Assert.False(condition: GeneratorEngine.TryCheckBatchCapacity(generator: source, masks: [new(Word0: 0b111UL)], sampleCount: 28L, reason: out _));
        Assert.True(condition: GeneratorEngine.TryCheckBatchCapacity(generator: source, masks: [new(Word0: 0b111UL)], sampleCount: 27L, reason: out _));
        Assert.True(condition: GeneratorEngine.TryCheckBatchCapacity(generator: RingSource(ring: 3, mode: GeneratorMode.RestartOnExhaustion), masks: null, sampleCount: 90L, reason: out _));
    }
}
