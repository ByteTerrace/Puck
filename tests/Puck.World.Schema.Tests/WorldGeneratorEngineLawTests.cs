...
public sealed class WorldGeneratorEngineLawTests {
    ...
    private static readonly StateGenerator Stream = new(Source: GeneratorSource.StreamDraw);
    private static readonly StateGenerator Uniform = new(
        Source: GeneratorSource.UniformRange,
        RangeMin: -17,
        RangeMax: 29
    );
    private static readonly StateGenerator Weighted = new(
    ...
    private static readonly StateGenerator TwoTokenMarkov = new(
    ...
    private static long NumericOracle(StateGenerator generator, string site, long cursor) {
        var rng = Pcg32XshRr.Create(
            state: GeneratorEngine.ComputeSeedState(
    ...
    private static bool TryFire(
        ...
        generator: generator,
        targetKind: ((generator.Source == GeneratorSource.Markov)
        ? CellKind.Text
        : CellKind.Int),
    ...
    public void MarkovCursorCountsSamplesRatherThanEmissions() {
        ...
        foreach (var expected in walked) {
            ...
            Assert.Equal(
                expected: 2L,
                actual: seeked.Samples
            );
        }
    }
    [Fact]
    public void NumericSourcesSeekToTheSameSampleAsWalkingEveryPriorCursor() {
        foreach (var generator in ((StateGenerator[])[Stream, Uniform, Weighted])) {
    ...
    public void PersistedCursorAndMaskReplayTheNextWithoutReplacementDraw() {
        ...
        Assert.Equal(
            expected: 3,
            actual: drawn.Distinct(comparer: StringComparer.Ordinal).Count()
        );
    ...
    public void ReorderingSitesDoesNotRepointTheirStreams() {
        ...
        var baseline = firstOrder.ToDictionary(
            ...
            elementSelector: site => NumericSequence(
                generator: Uniform,
                site: site,
                count: 24
            ),
        ...
        foreach (var site in secondOrder) {
            Assert.Equal(
                expected: baseline[site],
                actual: NumericSequence(
                    generator: Uniform,
                    site: site,
                    count: 24
                )
    ...
    public void RestartOnExhaustionStartsANewDeterministicPass() {
        ...
        Assert.Equal(
            expected: restarted.Text,
            actual: replayed.Text
        );
        Assert.Equal(
            expected: restarted.Numeric,
            actual: replayed.Numeric
        );
        Assert.Equal(
            expected: restarted.Samples,
            actual: replayed.Samples
        );
        Assert.Equal(
            expected: restarted.Masks,
            actual: replayed.Masks
        );
        Assert.NotNull(@object: restarted.Masks);
        Assert.Equal(
            expected: 1,
            actual: restarted.Masks![0].Count
        );
    }
    [Fact]
    public void SharingASourceSharesNoPositionBetweenSites() {
        var baseline = NumericSequence(
            generator: Stream,
            site: "state.left",
            count: 32
        );
        ...
        var afterOtherSiteAdvanced = NumericSequence(
            generator: Stream,
            site: "state.left",
            count: 32
        );
        ...
        Assert.Equal(
            actual: afterOtherSiteAdvanced,
            expected: baseline
        );
        Assert.False(
            condition: baseline.SequenceEqual(second: NumericSequence(
                generator: Stream,
                site: "state.right",
                count: 32
            )),
    ...
    public void SourcesDeclareTheFixedAdvanceCostCursorSeekingDependsOn() {
        Assert.Equal(
            expected: 2UL,
            actual: GeneratorEngine.AdvancesPerSample(source: GeneratorSource.Markov)
        );
        Assert.Equal(
            expected: 1UL,
            actual: GeneratorEngine.AdvancesPerSample(source: GeneratorSource.UniformRange)
        );
        Assert.Equal(
            expected: 2UL,
            actual: GeneratorEngine.AdvancesPerSample(source: GeneratorSource.WeightedNumeric)
        );
        Assert.Equal(
            expected: 1UL,
            actual: GeneratorEngine.AdvancesPerSample(source: GeneratorSource.StreamDraw)
        );
    }
    [Fact]
    public void TimingControlsTheMomentWithoutPerturbingTheSiteSequence() {
        var sourceName = Name(value: "shared");
        var sources = ((IReadOnlyList<GeneratorRow>)[new GeneratorRow(
                Generator: Weighted,
                Name: sourceName
            )]);
        ...
        foreach (var timing in Enum.GetValues<DrawTiming>()) {
            ...
            Assert.True(
                condition: GeneratorEngine.TryResolveSource(
                    draw: draw,
    ...
    public void WorldSeedAndInstanceIdentityEachIsolateTheWholeSiteSequence() {
        ...
        var otherWorldSeed = NumericSequence(
            generator: Stream,
            site: "state.roll",
            count: 32,
            worldSeed: (WorldSeed + 1UL)
        );
        var otherInstance = NumericSequence(
            generator: Stream,
            site: "state.roll",
            count: 32,
            instance: "instance-beta"
        );

        Assert.False(
            condition: baseline.SequenceEqual(second: otherWorldSeed),
            userMessage: "changing the world-seed rung left the 32-sample sequence unchanged"
        );
        Assert.False(
            condition: baseline.SequenceEqual(second: otherInstance),
            userMessage: "changing the instance rung left the 32-sample sequence unchanged"
        );
        Assert.False(
            condition: otherWorldSeed.SequenceEqual(second: otherInstance),
            userMessage: "the independently changed seed and instance rungs produced the same 32-sample sequence"
        );
    }
}
