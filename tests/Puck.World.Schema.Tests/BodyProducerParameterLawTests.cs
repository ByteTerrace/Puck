...
public sealed class BodyProducerParameterLawTests {
    ...
    public void ASensingProgramCompilesWithTheApproachScalarsAuthored() {
        var producer = Compile(
            program: SensingProgram(),
            scalars: CompleteApproachScalars()
        );

        Assert.Equal(
            expected: 1f,
            actual: ((double)producer.Scalar(parameter: BodyProducerParameter.StandoffRadius))
        );
    }
    [Fact]
    public void ASensingProgramMayOmitTheWholeRoamShape_ButNotOnlyPartOfIt() {
        ...
        var producer = Compile(
            program: SensingProgram(),
            scalars: approachOnly
        );

        Assert.False(condition: producer.RoamActive);

        approachOnly["forward"] = 0.5f;
        var exception = Assert.Throws<BodyMotionProgramException>(testCode: () => Compile(
            program: SensingProgram(),
            scalars: approachOnly
        ));
    ...
    public void TheCompleteRoamSetCompiles() {
        ...
        Assert.Equal(
            expected: 0.5f,
            actual: ((double)producer.Scalar(parameter: BodyProducerParameter.Forward))
        );
    }
}
