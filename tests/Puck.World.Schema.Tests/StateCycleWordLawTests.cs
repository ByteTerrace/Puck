...
public sealed class StateCycleWordLawTests {
    ...
    public void AWord_RoundTripsThroughSerialization_AndCompares_LetterForLetter() {
        ...
        Assert.Contains(
            expectedSubstring: $"\"word\":[{string.Join(
                separator: ',',
                values: letters
            )}]",
            actualString: json.Replace(
        ...
        Assert.DoesNotContain(
            actualString: json,
            expectedSubstring: "plane"
        );

        var restored = WorldDefinitionRows.FindStateRow(
            rows: WorldDefinitionSerialization.Deserialize(utf8Json: WorldDefinitionSerialization.Serialize(definition: definition)).State,
            name: "dial"
        )!;

        Assert.Equal(
            expected: row.Cycle,
            actual: restored.Cycle
        );
        Assert.Equal(
            expected: row.Cycle!.GetHashCode(),
            actual: restored.Cycle!.GetHashCode()
        );
        Assert.Equal(
            expected: new StateCycle(
                Word: [.. letters],
                Power: 5,
                TicksPerStep: 3
            ),
        ...
        Assert.NotEqual(
            expected: new StateCycle(
                Word: [letters[1], letters[0], letters[2]],
                Power: 5,
                TicksPerStep: 3
            ),
            actual: row.Cycle
        ...
        var plain = System.Text.Encoding.UTF8.GetString(bytes: WorldDefinitionSerialization.Serialize(definition: BuildDefinition(SlotRow(
            name: "spin",
            kind: CellKind.Int,
            value: 0L,
            cycle: new StateCycle(Power: 7)
        ))));

        Assert.DoesNotContain(
            actualString: plain,
            expectedSubstring: "\"word\""
        );
        Assert.Contains(
            actualString: plain,
            expectedSubstring: "\"power\": 7"
        );
    }
    [Fact]
    public void AWordsOrder_IsTheLoopsPeriod_OnEveryRotationOutput() {
        foreach (var order in new[] { 12, 18 }) {
            ...
            foreach (var power in new[] { 1, 5, -1 }) {
                var step = BuildDefinition(SlotRow(
                    name: "s",
                    kind: CellKind.Int,
                    value: 3L,
                    cycle: new StateCycle(
                ...
                var turns = BuildDefinition(SlotRow(
                    name: "t",
                    kind: CellKind.Fixed,
                    value: 0L,
                    cycle: new StateCycle(
                ...
                Assert.Equal(
                    expected: order,
                    actual: new StateCycle(Word: letters).Order
                );
                Assert.Equal(
                    expected: string.Empty,
                    actual: Validate(definition: step)
                );
                ...
                for (var tick = 0UL; (tick < ((ulong)((3 * order) * 2))); tick++) {
                    ...
                    Assert.Equal(
                        expected: ((((long)(index - 3).FloorModulo(modulus: order)) << FixedQ4816.FractionBitCount) / order),
                        actual: Read(
                            definition: turns,
                            row: "t",
                            tick: tick
    ...
    public void LatticeOutputs_WalkTheWordsOrbit_AndRingMovesWhenTheOrbitCrossesRings() {
        ...
        var word = SymmetryWord.Create(mirrors: letters);
        var seed = 5;
        var node = BuildDefinition(SlotRow(
            name: "n",
            kind: CellKind.Int,
            value: seed,
            cycle: new StateCycle(
                Word: letters,
                Output: CycleOutput.Node
            )
        ));
        var ring = BuildDefinition(SlotRow(
            name: "r",
            kind: CellKind.Int,
            value: seed,
            cycle: new StateCycle(
                Word: letters,
                Output: CycleOutput.Ring
            )
        ));
        var x = BuildDefinition(SlotRow(
            name: "x",
            kind: CellKind.Fixed,
            value: (((long)seed) << FixedQ4816.FractionBitCount),
            cycle: new StateCycle(
                Word: letters,
                Output: CycleOutput.ProjectionX
            )
        ));
        var ringsVisited = new HashSet<long>();

        for (var tick = 0UL; (tick < 36UL); tick++) {
            var expected = word.Apply(
                node: seed,
                steps: ((long)tick)
            );
            ...
            Assert.Equal(
                expected: SymmetryLattice.Ring(node: expected),
                actual: Read(
                    definition: ring,
                    row: "r",
                    tick: tick
            ...
            Assert.Equal(
                expected: SymmetryLattice.Project(node: expected).X.Value,
                actual: Read(
                    definition: x,
                    row: "x",
                    tick: tick
        ...
        Assert.Equal(
            ...
            actual: Read(
                definition: node,
                row: "n",
                tick: ((ulong)word.OrbitLength(node: seed))
            )
        );
        ...
        var coxeterRing = BuildDefinition(SlotRow(
            name: "r",
            kind: CellKind.Int,
            value: seed,
            cycle: new StateCycle(
        ...
        for (var tick = 0UL; (tick < 60UL); tick++) {
            Assert.Equal(
                expected: SymmetryLattice.Ring(node: seed),
                actual: Read(
                    definition: coxeterRing,
                    row: "r",
                    tick: tick
                )
            );
        ...
        for (var step = 0; (step < word.OrbitLength(node: seed)); step++) {
            seedOrbitCrossesRings |= (SymmetryLattice.Ring(node: word.Apply(
                node: seed,
                steps: step
            )) != SymmetryLattice.Ring(node: seed));
        }

        Assert.Equal(
            expected: seedOrbitCrossesRings,
            actual: (ringsVisited.Count > 1)
        );
    }
    [Fact]
    ...
    public void Validator_RefusesAWordThatLoopsNothing_AndAnIdentityPower() {
        static string Refusal(StateCycle cycle) => Validate(definition: BuildDefinition(SlotRow(
            cycle: cycle,
            kind: CellKind.Int,
            name: "r",
            value: 0L
        ...
        Assert.Contains(
            expectedSubstring: "moves no node",
            actualString: Refusal(cycle: new StateCycle(Word: [3, 3]))
        );
        Assert.Contains(
            expectedSubstring: "word holds 0 letters",
            actualString: Refusal(cycle: new StateCycle(Word: []))
        );
        Assert.Contains(
            expectedSubstring: "word holds 9 letters",
            actualString: Refusal(cycle: new StateCycle(Word: [0, 1, 2, 3, 4, 5, 6, 7, 0]))
        );
        Assert.Contains(
            expectedSubstring: "word[1] 240 is not a symmetry-lattice node",
            actualString: Refusal(cycle: new StateCycle(Word: [0, 240]))
        );
        Assert.Contains(
            expectedSubstring: "word[0] -1 is not a symmetry-lattice node",
            actualString: Refusal(cycle: new StateCycle(Word: [-1]))
        );
        Assert.Contains(
            expectedSubstring: ".cycle.power 0 is the identity",
            actualString: Refusal(cycle: new StateCycle(
                Word: [0, 2],
                Power: 0
            ))
        );
        Assert.Contains(
            expectedSubstring: ".cycle.power 3 is outside the generator's order 3",
            actualString: Refusal(cycle: new StateCycle(
                Word: [0, 2],
                Power: 3
            ))
        ...
        Assert.Equal(
            expected: string.Empty,
            actual: Refusal(cycle: new StateCycle(
                Word: [0, 2],
                Power: -2
            ))
        );
        Assert.Equal(
            expected: string.Empty,
            actual: Refusal(cycle: new StateCycle(Power: 29))
        );
        Assert.Equal(
            expected: string.Empty,
            actual: Refusal(cycle: new StateCycle(Power: -13))
        );
    }
}
