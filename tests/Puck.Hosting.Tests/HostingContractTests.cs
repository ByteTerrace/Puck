...
public sealed class HostingContractTests {
    ...
    public void CapabilityRevocationCascadesThroughIrrevocableSubgrant() {
        ...
        var parent = new HostContext(
            capabilities: new Dictionary<Type, object>(),
            heldGrants: parentGrants
        );
        var childGrants = new HeldCapabilityGrants();
        var childTakeBack = childGrants.Grant<ITestCapability>(
            grantor: parent,
            revocable: false
        );
        var child = new HostContext(
            capabilities: new Dictionary<Type, object>(),
    ...
    public void ExternalClocksAcceptZeroTimestampAndUnregister() {
        ...
        Assert.True(condition: clock.TryRead(
            arrivalTimestamp: out var timestamp,
        ...
        Assert.True(condition: registry.PacerClock.TryRead(
            arrivalTimestamp: out timestamp,
            frameVersion: out version
        ));
        source.Dispose();
        Assert.Empty(collection: registry.SourceIds);
        _ = Assert.Throws<ObjectDisposedException>(testCode: () => source.Publish(
            arrivalTimestamp: 12,
            frameVersion: 2
        ));
    }
    ...
    public void FrameCaptureControllerReadsTheExactGpuSurfaceAndResetsSessionIndex() {
        ...
        Assert.Equal(
            expected: 0L,
            actual: firstSink.LastFrame.FrameIndex
        );
        Assert.Equal(
            expected: 42UL,
            actual: firstSink.LastFrame.TimestampTicks
        );
        Assert.Equal(
            expected: surface,
            actual: readback.LastSurface
        );
        Assert.Same(
            expected: firstSink,
            actual: controller.Disarm()
