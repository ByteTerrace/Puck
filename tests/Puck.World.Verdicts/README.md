...
| `sources/every-kind-is-given.puck` | A `given` line writes a Fixed, a Bool and a Text row as an authored cell of that kind would, and an expectation over a row that is not Int still boots and passes: the verdict row records only what its gate read of an Int row. |
