...
public sealed class LinkQuerySeamCodecLawTests {
    ...
    public void GrantAllows_RoundTrips() {
        ...
        Assert.Equal(
            ...
            expected: WorldCapability.Drive
        );
        Assert.Equal(
            actual: decoded.Subject,
            expected: subject
        );
    }
    [Fact]
    public void GrantHandleMint_RoundTrips() {
        ...
        Assert.Equal(
            actual: decoded.Capability,
            expected: WorldCapability.Observe
        );
        Assert.Equal(
            actual: decoded.Index,
            expected: 3
        );
    }
    [Fact]
