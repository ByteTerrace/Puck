...
public sealed class OutputTests {
    ...
    public void Xbox_zero_duration_stops_every_motor() {
        ...
        var start = new RumbleEffect(
            DurationMilliseconds: 1000u,
            HighFrequency: 0.5f,
            LowFrequency: 1f
        );
        var stop = new RumbleEffect(
        ...
        Assert.Equal(
            actual: stopped.LowFrequency,
            expected: 0f
        );
        Assert.Equal(
            actual: stopped.HighFrequency,
            expected: 0f
        );
        Assert.Equal(
            actual: stopped.LeftTrigger,
            expected: 0f
        );
        Assert.Equal(
            actual: stopped.RightTrigger,
            expected: 0f
        );
    }
}
