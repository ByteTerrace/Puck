...
public sealed class WindowCommandInputTests {
    ...
    public void HeldDigitalStateReassertsInFirstDownOrderWithoutRepeatsOrSameFrameDuplicates() {
        ...
        var second = InputSignal.Press(
            source: "mouse.button17",
            deviceId: device
        );
        var first = InputSignal.Press(
            source: "keyboard.a",
            deviceId: device
        );
        ...
        Assert.True(condition: state.TryReassert(
            captureTick: 10UL,
            frameKey: 2UL,
            index: 0,
            signal: out var reassertedSecond
        ));
        Assert.False(condition: state.TryReassert(
            captureTick: 10UL,
            frameKey: 2UL,
            index: 1,
            signal: out _
        ...
        Assert.Equal(
            expected: "mouse.button17",
            actual: reassertedSecond.Source
        );
        Assert.Equal(
            expected: CommandPhase.Active,
            actual: reassertedSecond.Phase
        );
        ...
        Assert.True(condition: state.TryReassert(
            ...
            frameKey: 3UL,
            index: 0,
            signal: out reassertedSecond
        ));
        Assert.True(condition: state.TryReassert(
            captureTick: 11UL,
            frameKey: 3UL,
            index: 1,
            signal: out var reassertedFirst
        ));
        Assert.Equal(
            expected: "mouse.button17",
            actual: reassertedSecond.Source
        ...
        Assert.Equal(
            expected: "keyboard.a",
            actual: reassertedFirst.Source
        );

        var releaseSecond = InputSignal.Release(
    ...
    public void MouseButtonVocabularyIsOpenEndedButCanonical() {
        Assert.Equal(
            expected: InputSources.Mouse.LeftButton,
            actual: InputSources.Mouse.Button(number: 1)
        );
        Assert.Equal(
            expected: "mouse.button17",
            actual: InputSources.Mouse.Button(number: 17)
        );
        Assert.True(condition: InputSourceVocabulary.TryResolveDeclaredKind(
            kind: out var kind,
            sourceId: "mouse.button17"
        ));
        Assert.Equal(
            actual: kind,
        ...
        Assert.False(condition: InputSourceVocabulary.TryResolveDeclaredKind(
            kind: out _,
            sourceId: "mouse.button017"
        ));
        Assert.False(condition: InputSourceVocabulary.TryResolveDeclaredKind(
            kind: out _,
            sourceId: "mouse.button0"
        ));
    ...
    public void MouseProjectionPreservesArbitraryButtonAndTwoAxisMotion() {
        var buttonEvent = WindowInputEvent.PointerButton(
            button: 16,
            phase: CommandPhase.Started
        );
        var button = WindowInputMapper.ToInputSignal(inputEvent: in buttonEvent);
        var motionEvent = WindowInputEvent.PointerDelta(delta: new Vector2(
            x: 3f,
            y: -2f
        ));
        var motion = WindowInputMapper.ToInputSignal(inputEvent: in motionEvent);
        var wheelEvent = WindowInputEvent.PointerWheel(notches: new Vector2(
            x: -1f,
            y: 0.5f
        ));
        ...
        Assert.Equal(
            expected: InputSources.Mouse.Motion,
            actual: motion.Source
        );
        Assert.Equal(
            ...
            actual: motion.Value.AsAxis2D
        );
        Assert.True(condition: motion.Transient);
        Assert.Equal(
            expected: InputSources.Mouse.Wheel,
            actual: wheel.Source
        );
        Assert.Equal(
            expected: new Vector2(
    ...
    public void NumberRowAndNumpadProfilesRemainDistinctBindableControls() {
        ...
        Assert.True(condition: InputSourceVocabulary.TryResolveDeclaredKind(
            sourceId: row.Source,
            kind: out var rowKind
        ));
        Assert.True(condition: InputSourceVocabulary.TryResolveDeclaredKind(
            sourceId: pad.Source,
            kind: out var padKind
        ));
        Assert.Equal(
            actual: rowKind,
            expected: CommandValueKind.Digital
