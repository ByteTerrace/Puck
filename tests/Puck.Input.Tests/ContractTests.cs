...
public sealed class ContractTests {
    ...
    public void Normalization_rejects_non_finite_and_invalid_ranges() {
        _ = Assert.Throws<ArgumentOutOfRangeException>(testCode: () => GamepadNormalization.ApplyRadialDeadzone(
            stick: Vector2.One,
            deadzone: float.NaN
        ));
        _ = Assert.Throws<ArgumentOutOfRangeException>(testCode: () => GamepadNormalization.ApplyRadialDeadzone(
        ...
        _ = Assert.Throws<ArgumentOutOfRangeException>(testCode: () => GamepadNormalization.NormalizeTrigger(
            range: 1f,
        ...
        _ = Assert.Throws<ArgumentException>(testCode: () => GamepadNormalization.ReadVector3Int16(
            offset: 0,
            scale: 1f,
            source: [0, 1, 2, 3, 4]
        ));
    ...
    public void NormalizeTrigger_clamps_out_of_range_raw_values() {
        Assert.Equal(
            expected: 0f,
            actual: GamepadNormalization.NormalizeTrigger(
                range: 100f,
                raw: -10f,
                threshold: 10f
            )
    ...
    public void Source_factories_reject_identifiers_outside_the_declared_vocabulary() {
        Assert.Equal(
            expected: "keyboard.a",
            actual: InputSources.Keyboard.Letter(letter: 'A')
        );
        Assert.Equal(
            expected: "keyboard.f12",
            actual: InputSources.Keyboard.Function(number: 12)
        );
        Assert.Equal(
            expected: "keyboard.1",
            actual: InputSources.Keyboard.Digit(number: 1)
        );
        Assert.Equal(
            expected: "keyboard.numpad1",
            actual: InputSources.Keyboard.NumpadDigit(number: 1)
        );
