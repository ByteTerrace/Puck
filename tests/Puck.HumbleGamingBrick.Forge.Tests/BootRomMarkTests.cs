...
public sealed class BootRomMarkTests {
    ...
    public void AForgedCartridgeIsRunByTheHouseImageAndRefusedByTheEraOne() {
        var forged = new HgbCartridgeCompiler().Compile(document: GamingBricks.Forge.CartridgeDocuments.Create(
            target: "cgb",
            title: "MARK"
        )).Rom;
        ...
        Assert.True(
            condition: HandsOff(
                mark: BootRomMark.House,
                rom: forged
            ),
            userMessage: "the house image wedged on a forged cartridge"
        );
        Assert.False(
            condition: HandsOff(
            ...
            userMessage: "the era image handed off to a forged cartridge"
        );
    }
    [Fact]
    public void AnEraCartridgeIsRunByTheEraImageAndRefusedByTheHouseOne() {
        ...
        Assert.True(
            ...
            userMessage: "the era image wedged on an era cartridge"
        );
        Assert.False(
            condition: HandsOff(
                mark: BootRomMark.House,
            ...
            userMessage: "the house image handed off to an era cartridge"
        );
    }
    [Fact]
    public void CompatibleIsTheDefaultAndUnknownPolicyIsRefused() {
        Assert.Equal(
            expected: BootRomBuilder.Build(
                mark: BootRomMark.Compatible,
                model: ConsoleModel.Mgb
            ),
            actual: BootRomBuilder.Build(model: ConsoleModel.Mgb)
        );
        Assert.Throws<ArgumentOutOfRangeException>(testCode: () => BootRomBuilder.Build(
            mark: ((BootRomMark)255),
            model: ConsoleModel.Mgb
        ));
    }
    ...
    public void CompatibleRunsEraHouseAndArbitraryLogos(ConsoleModel model) {
        ...
        Assert.True(condition: HandsOff(
            mark: BootRomMark.Compatible,
            model: model,
            rom: rom
        ));
    ...
    public void CompatibleStillRefusesCorruptedHeaderChecksum(ConsoleModel model) {
        ...
        rom[0x014D] ^= 0x01;
        Assert.False(condition: HandsOff(
            mark: BootRomMark.Compatible,
            model: model,
            rom: rom
        ));
    ...
    public void TheHouseMarkIsAWholeHeaderBitmapDistinctFromTheEraOne() {
        Assert.Equal(
            expected: CartridgeHeader.Logo.Length,
            actual: CartridgeHeader.HouseLogo.Length
        );
