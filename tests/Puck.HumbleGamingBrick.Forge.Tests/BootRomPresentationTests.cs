...
public sealed class BootRomPresentationTests {
    ...
    private static bool ContainsWordmark(ReadOnlySpan<uint> pixels) {
        ...
        for (var top = 0; (top < 137); ++top) {
            ...
            for (var row = 0; (matches && (row < Wordmark.Length)); ++row) {
                for (var column = 0; (column < 32); ++column) {
                    if ((pixels[((((top + row) * Width) + Left) + column)] != background) != (Wordmark[row][column] == '1')) {
                        matches = false;
                        break;
                    }
                }
    ...
    public void CompatibleFirmwareDrawsItsWordmarkAndProducesItsChime(ConsoleModel model) {
        ...
        using var core = new HumbleGamingBrickCore(
            configuration: new MachineConfiguration(
                model: model,
                cartridgeRom: rom,
                bootRom: BootRomBuilder.Build(model: model)
            ),
            dmgSpeed: true
        );
        ...
        for (var frame = 0; ((frame < 40) && ((bus.ReadByte(address: MemoryMap.BootRomDisable) & 1) == 0)); ++frame) {
            ...
            if (
                ...
                foundAudio |= samples.AsSpan(
                    length: count,
                    start: 0
                ).ContainsAnyExcept(value: samples[0]);
            }
            VerifyMachineSettle.SettleOutOfOamDma(
                machine: core.Instance.Machine,
                cpu: core.Instance.GetRequiredService<ICpu>(),
                label: "boot presentation"
            );
            seenScrolls.Add(item: bus.ReadByte(address: 0xFF42));
        }

        Assert.True(
            condition: foundWordmark,
            userMessage: "The native framebuffer never contained the complete PUCK wordmark."
        );
        Assert.True(
            condition: (seenScrolls.Count > 20),
            userMessage: "The native scroll did not advance across its animation."
        );
        Assert.Equal(
            expected: !model.IsSuperGameBoy(),
            actual: foundAudio
        );
        Assert.True(
            condition: ((bus.ReadByte(address: MemoryMap.BootRomDisable) & 1) != 0),
