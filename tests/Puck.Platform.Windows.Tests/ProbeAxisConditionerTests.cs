...
public sealed class ProbeAxisConditionerTests {
    ...
    public void A_stale_measurement_expires_to_neutral_exactly_once_then_stops_changing() {
        ...
        var live = MakeMeasurement(
            captureTimestamp: 0L,
            value: 0.50
        );
        var fresh = conditioner.Step(
            channel: 0,
            nowTimestamp: 0L,
            reading: live
        );
        ...
        var stale = MakeMeasurement(
            captureTimestamp: 0L,
            value: 0.50
        );
        var firstExpiry = conditioner.Step(
            reading: stale,
            channel: 0,
            nowTimestamp: (SymmetricPolicy.MaxAgeTicks + 1L)
        );

        Assert.True(condition: firstExpiry.Expired);
        Assert.True(condition: firstExpiry.Changed);
        Assert.Equal(
            actual: firstExpiry.Value,
            expected: FixedQ4816.Zero
        );
        Assert.Equal(
            actual: firstExpiry.Confidence,
            expected: FixedQ4816.Zero
        );

        var repeatedExpiry = conditioner.Step(
            reading: stale,
            channel: 0,
            nowTimestamp: (SymmetricPolicy.MaxAgeTicks + 2L)
        );

        Assert.True(condition: repeatedExpiry.Expired);
        Assert.False(condition: repeatedExpiry.Changed);

        var recovered = MakeMeasurement(
            value: 0.50,
            captureTimestamp: (SymmetricPolicy.MaxAgeTicks + 2L)
        );
        var reactivated = conditioner.Step(
            reading: recovered,
            channel: 0,
            nowTimestamp: (SymmetricPolicy.MaxAgeTicks + 2L)
        );
    ...
    public void A_value_at_or_inside_the_deadband_yields_neutral(double raw) {
        var conditioner = new ProbeAxisConditioner(policy: SymmetricPolicy);
        var sample = conditioner.Step(
            reading: MakeMeasurement(
                captureTimestamp: 0L,
                value: raw
            ),
            channel: 0,
    ...
    public void Leaving_the_deadband_requires_the_magnitude_to_exceed_deadband_plus_hysteresis_on_both_sides() {
        ...
        Assert.Equal(
            actual: positiveAtBoundary.Step(
                reading: MakeMeasurement(
                    captureTimestamp: 0L,
                    value: 0.10
                ),
        ...
        Assert.Equal(
            actual: negativeAtBoundary.Step(
                reading: MakeMeasurement(
                    captureTimestamp: 0L,
                    value: -0.10
                ),
    ...
    public void Nonzero_smoothing_lags_toward_the_gated_value_instead_of_jumping_to_it() {
        ...
        Assert.True(condition: (second.Value > Quantize(
            raw: 0.20,
            bits: policy.QuantizeBits
        )));
        Assert.True(condition: (second.Value < Quantize(
            raw: 0.60,
            bits: policy.QuantizeBits
    ...
    public void One_bit_quantization_exposes_exactly_negative_one_zero_and_one() {
        ...
        for (var raw = -1.0; (raw <= 1.0); raw += 0.05) {
            ...
            _ = observed.Add(item: conditioner.Step(
                reading: MakeMeasurement(
                    captureTimestamp: 0L,
                    value: raw
                ),
        ...
        Assert.Equal(
            expected: [FixedQ4816.NegativeOne, FixedQ4816.Zero, FixedQ4816.One],
            actual: observed.OrderBy(keySelector: static value => value).ToArray()
        );
    }
    [InlineData(2)]
    [InlineData(4)]
    ...
    public void Quantization_exposes_two_to_the_bits_plus_one_values_with_exact_endpoints(int bits) {
        ...
        Assert.Contains(
            expected: FixedQ4816.NegativeOne,
            collection: observed
        );
        Assert.Contains(
            expected: FixedQ4816.Zero,
    ...
    public void Quantization_is_idempotent_on_an_already_quantized_value() {
        ...
        var requantized = second.Step(
            reading: MakeMeasurement(
                captureTimestamp: 0L,
                value: ((double)quantized)
            ),
            channel: 0,
            nowTimestamp: 0L
        ).Value;
    ...
    public void Quantization_is_symmetric_about_the_neutral() {
        ...
        var positiveValue = positive.Step(
            reading: MakeMeasurement(
                captureTimestamp: 0L,
                value: 0.37
            ),
        ...
        var negativeValue = negative.Step(
            reading: MakeMeasurement(
                captureTimestamp: 0L,
                value: -0.37
            ),
    ...
    public void Step_rejects_a_channel_ordinal_outside_the_measurement() {
        var conditioner = new ProbeAxisConditioner(policy: SymmetricPolicy);
        var reading = MakeMeasurement(
            captureTimestamp: 0L,
            value: 0.0
        );

        _ = Assert.Throws<ArgumentOutOfRangeException>(testCode: () => conditioner.Step(
            channel: -1,
            nowTimestamp: 0L,
            reading: reading
        ));
