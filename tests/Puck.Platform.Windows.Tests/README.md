...
`Win32PassthroughWindowTests` posts a passthrough source's pointer events to a
hidden Puck window, which reads them back at their client points, and holds the
virtual-key table to a round trip for every key.
