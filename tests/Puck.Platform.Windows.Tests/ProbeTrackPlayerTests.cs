...
public sealed class ProbeTrackPlayerTests {
    ...
    public void Capture_timestamp_is_the_sample_own_declared_time_not_the_advance_call_time() {
        ...
        var document = new ProbeTrackDocument(
            ...
            Channels: 1,
            Samples: [
                new ProbeTrackSample(
                    T: 0.0,
                    C: [0.0]
                ),
                new ProbeTrackSample(
                    T: GridSeconds(index: 1),
                    C: [1.0]
                ),
            ]
        );
        var player = new ProbeTrackPlayer(
            document: document,
            ring: ring
        );
        ...
        Assert.Equal(
            actual: second.CaptureTimestamp,
            expected: (Origin + secondSampleOffsetTicks)
        );
        Assert.Equal(
            actual: second.CompletionTimestamp,
            expected: lateArrival
        );
    }
    [Fact]
    public void Irregular_sample_times_set_the_playback_cadence_and_the_loop_length() {
        ...
        Assert.Equal(
            expected: 1,
            actual: ((int)Math.Round(a: ((double)second[0])))
        );
        Assert.Equal(
            expected: (Origin + Seconds(seconds: 0.5)),
            actual: second.CaptureTimestamp
        );
        ...
        Assert.Equal(
            expected: 2,
            actual: ((int)Math.Round(a: ((double)third[0])))
        );
        ...
        Assert.Equal(
            expected: 0,
            actual: ((int)Math.Round(a: ((double)looped[0])))
        );
        Assert.Equal(
            expected: ((Origin + Seconds(seconds: 0.6)) + PeriodTicks),
            actual: looped.CaptureTimestamp
        );
    }
    [Fact]
    public void Publishes_each_sample_exactly_once_per_period_and_loops() {
        ...
        Assert.Equal(
            actual: observedSlots,
            expected: [0, 1, 2, 0, 1, 2, 0, 1, 2]
        );
        Assert.Equal(
            actual: observedVersions,
            expected: [1L, 2L, 3L, 4L, 5L, 6L, 7L, 8L, 9L]
        );
    }
    [Fact]
    public void Rejects_a_document_that_fails_a_shape_check() {
        ...
        _ = Assert.Throws<InvalidDataException>(testCode: () => new ProbeTrackPlayer(
            document: new ProbeTrackDocument(
                Channels: 2,
                Samples: [new ProbeTrackSample(
                        T: 0.0,
                        C: [1.0]
                    )]
            ),
