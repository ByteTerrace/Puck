...
public sealed class TextContractTests {
    ...
    public void ArtifactWriterRoundTripsGeneratedAtlas() {
        var fontBytes = File.ReadAllBytes(path: Path.Combine(
    ...
    public void AtlasRetainsGlyphIdOnlyRowsForFutureShapingResults() {
        ...
        Assert.False(condition: atlas.TryGetGlyph(
            glyph: out _,
            unicode: -1
        ));
    ...
    public void CatalogPackerRemapsLogicalFontsIntoOneTexture() {
        ...
        Assert.Equal(
            2,
            packed.ImageData.Width
        );
        Assert.Equal(
            new byte[] { 1, 2, 3, 4, 5, 6, 7, 8 },
            packed.ImageData.RgbaPixels
        );
        Assert.True(condition: packed.Resolve(name: "second").TryGetGlyph(
            glyph: out var glyph,
            unicode: 'B'
        ));
        Assert.Equal(
            1f,
            glyph.AtlasBounds!.Value.Left
        );
        Assert.True(condition: packed.Resolve(name: "first").TryGetGlyphById(
            glyph: out _,
            glyphId: 7
        ));
        Assert.Same(
            packed.Resolve(name: null),
            packed.Resolve(name: "first")
        );
    }
    [Fact]
    ...
    public void ImageDataRequiresExactPackedRgbaLength() {
        _ = new FontAtlasImageData(
            new byte[16],
            height: 2,
            width: 2
        );

        _ = Assert.Throws<ArgumentException>(testCode: () => new FontAtlasImageData(
            new byte[4],
            height: 2,
            width: 2
        ));
    ...
    public void InProcessGeneratorBuildsCompositeGlyphsDeterministically() {
        ...
        Assert.True(condition: first.TryGetGlyph(
            glyph: out var glyph,
            unicode: 'é'
        ));
        Assert.NotNull(value: glyph.AtlasBounds);
        ...
        Assert.Contains(
            collection: first.ImageData.RgbaPixels.ToArray(),
            filter: static value => (value > 128)
        );
    }
    [Fact]
    public void InProcessGeneratorBuildsRequestedMtsdfGlyphs() {
        ...
        Assert.Contains(
            collection: atlas.ImageData.RgbaPixels.ToArray(),
            filter: static value => (value > 128)
        );
        Assert.Contains(
            collection: atlas.ImageData.RgbaPixels.ToArray(),
    ...
    public void InProcessGeneratorReportsMissingTablesForMalformedCffContainer() {
        ...
        Assert.Contains(
            expectedSubstring: "missing its required 'head' table",
    ...
    public void LayoutCenterAlignmentAccountsForNegativeLeftBearing() {
        ...
        Assert.Equal(
            -1f,
            layout.Placements[0].PlaneBounds.Left
        );
        Assert.Equal(
            -0.5f,
            layout.Placements[1].PlaneBounds.Left
        );
        Assert.Equal(
            2f,
            layout.Width
    ...
    public void LayoutCenterAlignmentCentersShorterLines() {
        ...
        Assert.Equal(
            -0.5f,
            layout.Placements[2].PlaneBounds.Left
        );
        Assert.Equal(
            -0.5f,
            layout.Placements[2].BaselineOrigin.X
        );
    }
    [Fact]
    public void LayoutLineHeightScaleStepsBaselines() {
        ...
        Assert.Equal(
            -2f,
            layout.Placements[1].BaselineOrigin.Y
        );
    }
    [Fact]
    public void LayoutOptionsDefaultMatchesPlainOverload() {
        var atlas = LayoutAtlas();
        var plain = new TextLayout().Layout(
            atlas: atlas,
            text: "AB\nA",
            scale: 2f
        );
        var optioned = new TextLayout().Layout(
            atlas: atlas,
    ...
    public void LayoutRejectsNonFiniteDimensions() {
        ...
        _ = Assert.Throws<ArgumentOutOfRangeException>(testCode: () => layout.Layout(
            atlas,
            "A",
            scale: float.NaN
        ));
        ...
        _ = Assert.Throws<ArgumentOutOfRangeException>(testCode: () => layout.Layout(
            atlas,
            "A",
            maxLineWidth: float.NaN
        ));
        _ = Assert.Throws<ArgumentOutOfRangeException>(testCode: () => layout.Layout(
            atlas,
            "A",
            maxLineWidth: float.PositiveInfinity
        ));
    }
    [Fact]
    ...
    public void LayoutRightAlignmentMeetsWidestLineRightEdge() {
        ...
        Assert.Equal(
            0f,
            layout.Placements[1].PlaneBounds.Right
        );
        Assert.Equal(
            0f,
            layout.Placements[2].PlaneBounds.Right
        );
    }
    [Fact]
    public void LayoutTrackingAddsPerGlyphAdvance() {
        ...
        Assert.Equal(
            1.5f,
            layout.Placements[1].BaselineOrigin.X
        );
    }
    [Fact]
    ...
    public void MismatchedEndTagDoesNotPopTheCurrentEffect() {
        var enriched = BbCodeTextMarkup.EnrichRunes(markup: "[wave]a[color=#ff0000]b[/wave]c[/color]d").ToArray();
    ...
    public void PrebakedCacheKeepsResolvedImagePathInItsIdentity() {
        ...
        var firstAtlasPath = Path.Combine(
            path1: basePath,
            path2: "first",
            path3: "atlas.json"
        );
        var secondAtlasPath = Path.Combine(
            path1: basePath,
            path2: "second",
            path3: "atlas.json"
        );
        var firstImagePath = Path.ChangeExtension(
            extension: ".png",
            path: firstAtlasPath
        );
        var secondImagePath = Path.ChangeExtension(
            extension: ".png",
            path: secondAtlasPath
        );
    ...
    public void RuntimeCacheTreatsCollectionFaceAsPartOfFontIdentity() {
        var basePath = Path.GetFullPath(path: Path.Combine(
            path1: Path.GetTempPath(),
            path2: "puck-text-face-cache-contract"
        ));
        ...
        var first = resolver.Resolve(
            fontPath: "family.ttc",
            generationOptions: new FontAtlasGenerationOptions { FaceIndex = 0 },
            basePath: basePath
        );
        var second = resolver.Resolve(
            fontPath: "family.ttc",
            generationOptions: new FontAtlasGenerationOptions { FaceIndex = 1 },
            basePath: basePath
        );
        var firstAgain = resolver.Resolve(
            fontPath: "family.ttc",
            generationOptions: new FontAtlasGenerationOptions { FaceIndex = 0 },
            basePath: basePath
        );
