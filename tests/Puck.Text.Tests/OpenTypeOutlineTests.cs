...
public sealed class OpenTypeOutlineTests {
    ...
    public void CollectionFaceIndexRefusesOutOfRangeAndStandaloneSelections() {
        ...
        var collectionError = Assert.Throws<ArgumentException>(testCode: () => Generate(
            bytes: collection,
            faceIndex: 1
        ));
        var standaloneError = Assert.Throws<ArgumentException>(testCode: () => Generate(
            bytes: SyntheticTrueTypeFont.Build(),
            faceIndex: 1
        ));

        Assert.Contains(
            expectedSubstring: "face index 1 is out of range",
            actualString: collectionError.Message,
            comparisonType: StringComparison.Ordinal
        );
        Assert.Contains(
            expectedSubstring: "not a collection",
            actualString: standaloneError.Message,
            comparisonType: StringComparison.Ordinal
        );
    }
    [Fact]
    public void CollectionFaceIndexSelectsTheRequestedFace() {
        ...
        Assert.True(condition: first.TryGetGlyph(
            glyph: out var firstGlyph,
            unicode: 'A'
        ));
        Assert.True(condition: second.TryGetGlyph(
            glyph: out var secondGlyph,
            unicode: 'A'
        ));
        Assert.Equal(
            1f,
...
internal static class SyntheticCffFont {
    ...
    private static void AppendIndex(List<byte> output, bool cff2, params byte[][] objects) {
        if (cff2) {
            AppendUInt32(
                output: output,
                value: checked((uint)objects.Length)
            );
        } else {
            AppendUInt16(
                output: output,
                value: checked((ushort)objects.Length)
            );
        }
    ...
    private static byte[] AssembleSfnt(bool cff2, IReadOnlyList<(string Tag, byte[] Bytes)> tables) {
        ...
        AppendUInt16(
            output: output,
            value: checked((ushort)tables.Count)
        );
        AppendUInt16(
            output: output,
            value: 0
        ...
        foreach (var (tag, bytes) in tables) {
            ...
            AppendUInt32(
                output: output,
                value: checked((uint)(dataOffset + data.Count))
            );
            AppendUInt32(
                output: output,
    ...
    private static byte[] BuildCff1(byte[] charString, int subroutineDepth) {
        ...
        if (subroutineDepth == 0) {
            ...
            for (var index = 0; (index < subroutineDepth); index++) {
                ...
                programs[index] = ((index == (subroutineDepth - 1))
                    ? [11]
                    : [next, 29, next, 29, 11]
                );
            }
        ...
        AppendIndex(
            output: localSubroutines,
            cff2: false,
            localSubroutine
        );
        AppendDictInteger(
            output: privateDictionary,
            value: 6
        );
        ...
        AppendDictInteger(
            output: topDictionary,
            value: charStringsOffset
        );
        topDictionary.Add(item: 17);
        AppendDictInteger(
            output: topDictionary,
            value: privateDictionary.Count
        );
        AppendDictInteger(
            output: topDictionary,
            value: privateOffset
    ...
    private static byte[] BuildCff2(byte[] charString) {
        ...
        AppendDictInteger(
            output: output,
            value: VariationStoreOffset
        );
        output.Add(item: 24);
        AppendDictInteger(
            output: output,
            value: FontDictionariesOffset
        );
        output.Add(item: 12);
        output.Add(item: 36);
        AppendIndex(
    ...
    public static byte[] Build(bool cff2, int subroutineDepth = 0) {
        ...
        var cff = (cff2
            ? BuildCff2(charString: charString)
            : BuildCff1(
                charString: charString,
                subroutineDepth: subroutineDepth
            )
        );
        ...
        BinaryPrimitives.WriteUInt16BigEndian(
            destination: hmtx.AsSpan(start: 0),
            value: 1000
        );
        BinaryPrimitives.WriteUInt16BigEndian(
            destination: hmtx.AsSpan(start: 4),
            value: 1000
        );
        var maxp = new byte[6];
...
internal static class SyntheticOpenTypeCollection {
    private static void RebaseTableOffsets(Span<byte> face, int faceOffset) {
        var tableCount = BinaryPrimitives.ReadUInt16BigEndian(source: face.Slice(
            length: 2,
            start: 4
        ));
        ...
        for (var index = 0; (index < tableCount); index++) {
            ...
            var oldOffset = BinaryPrimitives.ReadUInt32BigEndian(source: face.Slice(
                length: 4,
                start: offsetPosition
            ));

            BinaryPrimitives.WriteUInt32BigEndian(
                destination: face.Slice(
                    length: 4,
                    start: offsetPosition
                ),
    ...
    public static byte[] Build(params byte[][] faces) {
        ...
        BinaryPrimitives.WriteUInt32BigEndian(
            destination: output.AsSpan(start: 4),
            value: 0x00010000
        );
        BinaryPrimitives.WriteUInt32BigEndian(
            destination: output.AsSpan(start: 8),
            value: checked((uint)faces.Length)
        );

        for (var index = 0; (index < faces.Length); index++) {
            BinaryPrimitives.WriteUInt32BigEndian(
                destination: output.AsSpan(start: checked((12 + (index * 4)))),
                value: checked((uint)offsets[index])
            );
            faces[index].CopyTo(
                array: output,
                index: offsets[index]
            ...
            RebaseTableOffsets(
                face: output.AsSpan(
                    start: offsets[index],
                    length: faces[index].Length
                ),
                faceOffset: offsets[index]
            );
        }
