...
public sealed class MtsdfContractTests {
    ...
    private static float DecodeBilinearMedian(FontAtlas atlas, Vector2 texel) {
        var r = DecodeBilinear(
            atlas: atlas,
            channel: 0,
            texel: texel
        );
        var g = DecodeBilinear(
            atlas: atlas,
            channel: 1,
            texel: texel
        );
        var b = DecodeBilinear(
            atlas: atlas,
            channel: 2,
            texel: texel
        );
    ...
    private static float PolylineSignedDistance(Vector2 point, IReadOnlyList<Vector2> polyline) {
        ...
        for (var index = 0; (index < polyline.Count); index++) {
            ...
            best = MathF.Min(
                x: best,
                y: (point - (start + (t * direction))).Length()
            );
    ...
    public void AlphaMatchesAnalyticSquareDistance() {
        ...
        Assert.True(
            condition: (worst <= 0.16f),
            userMessage: $"Worst alpha error {worst} texels exceeds the tolerance."
        );
    }
    [Fact]
    public void AlphaMatchesBruteForceDistanceOnCurvedGlyph() {
        ...
        for (var y = ((int)cell.Top); (y < ((int)cell.Bottom)); y++) {
            for (var x = ((int)cell.Left); (x < ((int)cell.Right)); x++) {
                ...
                var analytic = (PolylineSignedDistance(
                    point: em,
                    polyline: polyline
                ) * FontPixelSize);
        ...
        Assert.True(
            condition: (worst <= 0.2f),
            userMessage: $"Worst curved-glyph alpha error {worst} texels exceeds the tolerance."
        );
    }
    [Fact]
    public void GenerationIsDeterministic() {
        ...
        Assert.Equal(
            first.ImageData!.RgbaPixels,
            second.ImageData!.RgbaPixels
        );
    }
    ...
    public void MedianKeepsDilatedSquareCornerSharpWhereAlphaRounds() {
        ...
        for (var stepY = 0; (stepY <= 20); stepY++) {
            for (var stepX = 0; (stepX <= 20); stepX++) {
                var outwardTexels = new Vector2(
                    x: (stepX / 6f),
                    y: (stepY / 6f)
                );
                var sharpDistance = -MathF.Max(
                    x: outwardTexels.X,
                    y: outwardTexels.Y
                );
                ...
                if ((DecodeBilinearMedian(
                    atlas: atlas,
                    texel: texel
                ) > -DilationTexels) != sharpInsideDilation) {
                    medianErrors++;
                }
        ...
        Assert.True(
            condition: (sampled > 300),
            userMessage: $"Only {sampled} samples survived the boundary margin."
        );
        Assert.True(
            condition: (medianErrors <= 2),
            userMessage: $"Median misclassified {medianErrors} of {sampled} dilated-corner samples against the sharp contour."
        );
        Assert.True(
            condition: (alphaErrors >= 8),
            userMessage: $"Alpha misclassified only {alphaErrors} samples — the rounding crescent should be visible, so this test is no longer discriminating."
        );
    }
    [Fact]
    public void UnicodeAliasesShareOneRasterCellWithoutLosingEitherMapping() {
        ...
        Assert.True(condition: aliases.TryGetGlyph(
            glyph: out var a,
            unicode: 'A'
        ));
        Assert.True(condition: aliases.TryGetGlyph(
        ...
        Assert.Equal(
            single.ImageData!.RgbaPixels,
            aliases.ImageData!.RgbaPixels
        );
    }
}
...
internal static class SyntheticTrueTypeFont {
    ...
    private static byte[] Assemble(IReadOnlyList<(string Tag, byte[] Bytes)> tables) {
        ...
        foreach (var (tag, bytes) in tables) {
            ...
            AppendUInt32(
                output: output,
                value: ((uint)(dataOffset + data.Count))
            );
            AppendUInt32(
                output: output,
                value: ((uint)bytes.Length)
            );
    ...
    private static byte[] BuildCmapFormat4() {
        ...
        AppendUInt16(
            output: subtable,
            value: unchecked((ushort)(1 - 'A'))
        );
        AppendUInt16(
            output: subtable,
            value: unchecked((ushort)(1 - 'D'))
        );
        AppendUInt16(
            output: subtable,
            value: 1
    ...
    private static byte[] BuildGpos(params byte[][] pairPositions) {
        ...
        AppendUInt16(
            output: output,
            value: checked((ushort)lookupListOffset)
        );
        AppendUInt16(
            output: output,
            value: 0
        ...
        AppendUInt16(
            output: output,
            value: checked((ushort)pairPositions.Length)
        );

        for (var index = 0; (index < pairPositions.Length); index++) {
            AppendUInt16(
                output: output,
                value: checked((ushort)index)
            );
        }
        ...
        foreach (var pairPosition in pairPositions) {
            AppendUInt16(
                output: output,
                value: checked((ushort)lookupOffset)
            );
    ...
    private static byte[] BuildLegacyKern(params (ushort Coverage, short Value)[] subtables) {
        ...
        AppendUInt16(
            output: output,
            value: checked((ushort)subtables.Length)
        );
    ...
    private static byte[] BuildPairPosFormat1(short adjustment, bool maximumCoverageRange = false) {
        ...
        AppendUInt16(
            output: output,
            value: unchecked((ushort)adjustment)
        );
    ...
    private static byte[] SimpleGlyph(IReadOnlyList<(short X, short Y, bool OnCurve)> points) {
        ...
        AppendUInt16(
            output: output,
            value: unchecked((ushort)minX)
        );
        AppendUInt16(
            output: output,
            value: unchecked((ushort)minY)
        );
        AppendUInt16(
            output: output,
            value: unchecked((ushort)maxX)
        );
        AppendUInt16(
            output: output,
            value: unchecked((ushort)maxY)
        );
        AppendUInt16(
            output: output,
            value: ((ushort)(points.Count - 1))
        );
        AppendUInt16(
            output: output,
            value: 0
        ...
        foreach (var point in points) {
            output.Add(item: ((byte)(point.OnCurve
                ? 0x01
                : 0x00)));
        }
        ...
        foreach (var point in points) {
            AppendUInt16(
                output: output,
                value: unchecked((ushort)(point.X - previousX))
            );
            previousX = point.X;
        }
        ...
        foreach (var point in points) {
            AppendUInt16(
                output: output,
                value: unchecked((ushort)(point.Y - previousY))
            );
            previousY = point.Y;
        }
    ...
    public static byte[] Build(SyntheticKerning kerning = SyntheticKerning.None, ushort advanceWidth = 1000) {
        ...
        BinaryPrimitives.WriteUInt16BigEndian(
            destination: head.AsSpan(start: 18),
            value: 1000
        );
        BinaryPrimitives.WriteInt16BigEndian(
            destination: head.AsSpan(start: 50),
            value: 0
        );
        ...
        BinaryPrimitives.WriteInt16BigEndian(
            destination: hhea.AsSpan(start: 4),
            value: 800
        );
        BinaryPrimitives.WriteInt16BigEndian(
            destination: hhea.AsSpan(start: 6),
            value: -200
        );
        BinaryPrimitives.WriteUInt16BigEndian(
            destination: hhea.AsSpan(start: 34),
            value: 4
        );
        ...
        BinaryPrimitives.WriteUInt16BigEndian(
            destination: maxp.AsSpan(start: 4),
            value: 4
        );
