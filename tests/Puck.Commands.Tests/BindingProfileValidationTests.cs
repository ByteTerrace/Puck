...
public sealed class BindingProfileValidationTests {
    ...
    public void EntryIdsMustBeNonEmptyAndUniqueWithinTheirEffectivePage() {
        _ = Assert.Throws<ArgumentException>(testCode: static () => BindingProfile.Compile(document: DocumentWithEntries(new BindingPageEntryDefinition(
            Sources: ["key.a"],
            Command: "action",
            Id: string.Empty
        ))));
    ...
    public void TextPayloadPreservesOuterWhitespaceAndTreatsSeparatorsAsArguments() {
        ...
        Assert.Equal(
            expected: Text,
            actual: Assert.Single(collection: new PagedInputBindings(profile: profile).Resolve(
                slot: 0,
                source: "key.a"
            )!).Text
        );
    ...
    public void TextPayloadsMustBeNonblankSingleLineAndBounded() {
        foreach (var text in new[] {
            ...
            new string(
            c: 'a',
            count: (BindingProfile.MaxTextPayloadLength + 1)
        ),
        }) {
