...
public sealed class WireArgsTests {
    ...
    public void AFloatRunTooShortForItsRangeFailsBeforeItAllocates() {
        ...
        Assert.Equal(
            expected: "False|0",
            actual: registry.Submit(line: "wire.floats 1 2").Output
        );
        Assert.Equal(
            expected: "True|3",
            actual: registry.Submit(line: "wire.floats 1 2 3").Output
        );

        // A present-but-unparsable token still yields the full-length array, zeroed from the token that failed.
        Assert.Equal(
            expected: "False|3",
            actual: registry.Submit(line: "wire.floats 1 two 3").Output
        );
    }
    [Fact]
    public void AnOutOfRangeIndexIsAMissRatherThanAThrow() {
        ...
        Assert.Equal(
            expected: "True|False|False|False|0|0",
            actual: registry.Submit(line: "wire.optional YES").Output
        );
        Assert.Equal(
            expected: "False|False|False|False|0|0",
            actual: registry.Submit(line: "wire.optional").Output
        );
    }
    [Fact]
    public void EveryWideningParseAnswersTheSameWayForAnAbsentToken() {
        ...
        Assert.Equal(
            expected: "False|False|False|0|0|0",
            actual: registry.Submit(line: "wire.optional.wide").Output
        );
        Assert.Equal(
            expected: "True|True|True|12|12|12",
            actual: registry.Submit(line: "wire.optional.wide x x x x x x x x x 12").Output
        );
    }
    [Fact]
    public void NumericTokensParseInvariantlyAndFailAsIndividualTokens() {
        ...
        Assert.Equal(
            expected: "7|1.5",
            actual: registry.Submit(line: "wire.numbers 7 1.5").Output
        );
        Assert.Equal(
            expected: "-|1.5",
            actual: registry.Submit(line: "wire.numbers seven 1.5").Output
        );
        Assert.Equal(
            expected: "7|-",
            actual: registry.Submit(line: "wire.numbers 7 NaN").Output
        );
    }
    [Fact]
    public void SpanModeAndArrayModeSeeTheSameTokens() {
        ...
        Assert.Equal(
            expected: "3|a b c|c",
            actual: registry.Submit(line: "wire.shape a b c").Output
        );
        Assert.Equal(
            expected: "3|a b c|c",
            actual: registry.Submit(line: "wire.shape a \"b\" c").Output
    ...
    public void TailJoinsTokensWithSingleSpacesCollapsingWhitespaceRuns() {
        ...
        Assert.Equal(
            expected: "a b c",
            actual: registry.Submit(line: "wire.tail a\t\t b   c").Output
        );
    }
    [Fact]
    public void TailRentsTheHeapForATailWiderThanItsStackScratch() {
        ...
        Assert.Equal(
            expected: joined,
            actual: registry.Submit(line: $"wire.tail {joined}").Output
        );
    }
    [Fact]
    public void TailStartingPastTheLastTokenIsEmpty() {
        ...
        Assert.Equal(
            ...
            actual: registry.Submit(line: "wire.tail.from5 a b").Output
        );
        Assert.Equal(
            expected: string.Empty,
            actual: registry.Submit(line: "wire.tail").Output
        );
    }
        ...
        public IEnumerable<CommandDefinition> GetCommands() {
            ...
            yield return CommandDefinition.WithWireArgs(
                ...
                handler: static (_, args) => new CommandResult(Output: string.Join(
                    ...
                    args.TryInt(
                        index: 9,
                        value: out var absentInteger
                    ),
                    args.TryFloat(
                        index: 9,
                        value: out var absentReal
                    ),
                    absentInteger,
                    absentReal
                )),
            ...
            yield return CommandDefinition.WithWireArgs(
                ...
                handler: static (_, args) => new CommandResult(Output: string.Join(
                    ...
                    args.TryLong(
                        index: 9,
                        value: out var absentLong
                    ),
                    args.TryULong(
                        index: 9,
                        value: out var absentUnsigned
                    ),
                    args.TryUnsignedDigits(
                        index: 9,
                        value: out var absentDigits
                    ),
                    absentLong,
                    absentUnsigned,
                    absentDigits
