...
public sealed class InputRouterMomentaryReleaseTests {
    ...
    public void ATapSurvivesTheChordsOwnReleaseOnTheSameDestinationThenAFocusLoss() {
        ...
        var interleaved = Assert.Single(collection: router.SnapshotForTick(
            tick: 2UL,
            windowEndTick: ulong.MaxValue
        ).Lanes).Entries.ToArray();

        Assert.Equal(
            actual: interleaved.Length,
            expected: 3
        );
        Assert.Equal(
            actual: interleaved[1].Phase,
            expected: CommandPhase.Started
        );
        Assert.True(condition: interleaved[1].Dispatch);
        Assert.Equal(
            actual: interleaved[2].Phase,
    ...
    public void ATapsScheduledReleaseLeavesALiveChordHoldOnTheSameCommandIntact() {
        ...
        var scheduled = Assert.Single(collection: router.SnapshotForTick(
            tick: 3UL,
            windowEndTick: ulong.MaxValue
        ).Lanes).Entries.ToArray();
