...
public sealed class BindingSessionPlanReservationTests {
    ...
    public void AMemberDifferingFromADeclaredModifierOnlyByCaseIsNotAPhantomControl() {
        ...
        Assert.Equal(
            ...
            actual: Assert.Single(collection: reserved!)
        );
    }
    [Fact]
    ...
    public void ReservedSourcesAreExactlyTheCompiledProfilesModifierSources() {
        foreach (var (label, document) in Documents()) {
            ...
            Assert.True(
                ...
                userMessage: $"{label}: compiled [{string.Join(
                    separator: ", ",
                    values: compiled.Order(comparer: StringComparer.Ordinal)
                )}] != reserved [{string.Join(
                    separator: ", ",
                    values: reserved.Order(comparer: StringComparer.Ordinal)
                )}]"
            );
        }
    }
