...
public sealed class TextCommandSessionTests {
    ...
    public void AHeldSessionRotatesWhileAnotherDrainsFifoPerSessionPreserved() {
        ...
        Assert.Equal(
            actual: heldResults,
            expected: ["probe first", "probe second"]
        );
    }
    [Fact]
    public void OneSeatsSimulationBarrierDoesNotBlockAnotherSeatsReadyText() {
        ...
        Assert.Equal(
            actual: seatOneResults,
            expected: ["simulate"]
        );
        Assert.Equal(
            actual: seatTwoResults,
            expected: ["probe"]
        );
    ...
    public void ReleasedFocusDispatchesOnlyFocusExemptCommandsAndIgnoresKeyRepeat() {
        ...
        router.Capture(signal: InputSignal.Press(
            source: "keyboard.backtick",
            deviceId: keyboard
        ));
        var opening = router.SnapshotForTick(
            tick: 1UL,
            windowEndTick: ulong.MaxValue
        );
        ...
        router.CaptureFocusExempt(signal: InputSignal.Press(
            source: "keyboard.backtick",
            deviceId: keyboard
        ));
        ...
        router.CaptureFocusExempt(signal: InputSignal.Release(
            source: "keyboard.backtick",
            deviceId: keyboard
        ));
        ...
        var closing = router.SnapshotForTick(
            tick: 4UL,
            windowEndTick: ulong.MaxValue
        );
        ...
        Assert.Equal(
            actual: dispatched,
            expected: ["gameplay.action", "terminal.console", "terminal.console"]
        );
    }
    [Fact]
    public void RestrictedSessionsAuthorizeAtDispatchAndPreserveEachCaller() {
        ...
        using var bob = source.CreateSession(
            ...
            authorize: command => (command.Name == "probe")
        );

        alice.Enqueue(line: "probe");
        bob.Enqueue(line: "probe");
        ...
        Assert.Equal(
            ...
            seen.Select(selector: value => value.Principal)
        );
        alice.Enqueue(line: "probe");
        permitted = false;
        source.Collect();
    ...
    private sealed class FocusModule(List<string> dispatched) : ICommandModule {
        ...
        public IEnumerable<CommandDefinition> GetCommands() {
            yield return Definition(
                inputScope: CommandInputScope.Focused,
