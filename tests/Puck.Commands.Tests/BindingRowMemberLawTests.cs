...
public sealed class BindingRowMemberLawTests {
    ...
    private static bool Fired(InputRouter router, ulong tick) =>
        router.SnapshotForTick(
            tick: tick,
            windowEndTick: ulong.MaxValue
        ).Lanes.Any(predicate: lane => lane.Entries.Any(predicate: e => (e.Dispatch && (e.Phase == CommandPhase.Started))));
    ...
    public void AMemberListedInBothListsIsRefused() {
        ...
        Assert.Contains(
            expectedSubstring: "more than once",
            actualString: refused.Message
        );
    }
    [Fact]
    public void ChordRowFiresOnlyInItsPressOrder() {
        var profile = Compile(rows: [new BindingChordDefinition(
                Group: "g",
                Chord: ["key.a", "key.b"],
                Command: new BindingCommandDefinition(Command: ActionCommand)
            )]);
    ...
    public void HeldRowFiresInEitherPressOrder() {
        var profile = Compile(rows: [new BindingChordDefinition(
                Group: "g",
                Held: ["mouse.button1", "mouse.button2"],
                Command: new BindingCommandDefinition(Command: ActionCommand)
            )]);
    ...
    public void RawSourceMembersResolveToDeclaredModifiersOwningThemOrBecomeImplicitOnes() {
        ...
        Assert.Equal(
            expected: 2,
            actual: profile.Modifiers.Count
        );
        Assert.Equal(
            expected: "wheel",
            actual: profile.Modifiers[0].Id
        );
        Assert.Equal(
            expected: "mouse.button1",
            actual: profile.Modifiers[1].Id
        );
        Assert.Equal(
            expected: ["mouse.button1"],
            actual: profile.Modifiers[1].Sources
        );
    }
    [Fact]
    public void RowsOverTheSameMembersAreRefusedUnlessBothAreOrderedPaths() {
        ...
        Assert.Contains(
            expectedSubstring: "not chord-only",
            actualString: refused.Message
        );
