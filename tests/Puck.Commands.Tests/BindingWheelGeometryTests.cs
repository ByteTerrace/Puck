...
public sealed class BindingWheelGeometryTests {
    ...
    public void ScalingAVectorByAPowerOfTwoNeverMovesItsSector() {
        var style = new BindingWheelStyleDefinition(RotationDegrees: 11f);
    ...
    // The previous MathF.Atan2 reading, kept here as the oracle the reproducible implementation must still agree
    // with everywhere except the boundary ties it exists to pin down.
    private static int ReferenceSector(Vector2 vector, int sectorCount, BindingWheelStyleDefinition style) {
        ...
        return (((int)((relative + (span * 0.5f)) / span)) % sectorCount);
    }
    private static BindingWheelStyleDefinition[] Styles() {
        return [
            new BindingWheelStyleDefinition(),
            new BindingWheelStyleDefinition(RotationDegrees: 17.5f),
            new BindingWheelStyleDefinition(RotationDegrees: 90f),
            new BindingWheelStyleDefinition(RotationDegrees: -37.25f),
            new BindingWheelStyleDefinition(Clockwise: false),
            new BindingWheelStyleDefinition(Clockwise: false, RotationDegrees: 143f),
        ];
    }
    // The screen-space vector whose ROTATION-RELATIVE clockwise angle is `relative`: sector zero sits at twelve
    // o'clock (0, -1) and the angle advances the way the style says it does.
    private static Vector2 VectorFor(double relative, BindingWheelStyleDefinition style) {
