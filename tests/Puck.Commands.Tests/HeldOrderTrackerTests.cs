...
public sealed class HeldOrderTrackerTests {
    ...
    public void HeldOrderRecoversPressOrder() {
        var tracker = new HeldOrderTracker(
            modifierCount: 3,
            pressThreshold: 0.5f,
    ...
    public void MismatchedPerModifierThresholdListsAreRefused() {
        _ = Assert.Throws<ArgumentException>(testCode: static () => new HeldOrderTracker(
            pressThresholds: [0.5f, 0.5f],
            releaseThresholds: [0.25f]
        ));
    }
    [Fact]
    ...
    public void SetLatchesAndReleasesAcrossTheHysteresisBand() {
        ...
        Assert.True(condition: tracker.Set(
            index: 0,
            value: 0.6f
        ));   // crosses press → latched
        Assert.Equal(
            expected: 1,
            actual: tracker.Count
        );

        Assert.False(condition: tracker.Set(
            index: 0,
            value: 0.4f
        ));  // inside the band → holds, no change
        Assert.Equal(
            expected: 1,
            actual: tracker.Count
        );
    ...
    public void SignalOverloadReleasesOnACompletedEdgeWhateverValueItCarries() {
        ...
        Assert.True(condition: tracker.Set(
            index: 0,
            signal: InputSignal.Press(source: "trigger")
        ));
        Assert.Equal(
            expected: 1,
            actual: tracker.Count
        ...
        Assert.True(condition: tracker.Set(
            index: 0,
            signal: InputSignal.Release(source: "trigger")
        ));
        Assert.Equal(
            expected: 0,
            actual: tracker.Count
