...
public sealed class BindingVocabularyCheckTests {
    ...
    public void ADeclaredModifiersOwnSourcesAreCheckedAgainstTheControlCatalog() {
        ...
        var document = new BindingProfileDocument(
            ...
            Modifiers: [
                new BindingModifierDefinition(
                    Id: "aim",
                    Sources: ["gamepad.buttonSouht"]
                ),
                new BindingModifierDefinition(
                    Id: "type",
                    Sources: ["keyboard.text"]
                ),
                new BindingModifierDefinition(
                    Id: "look",
        ...
        Assert.Contains(
            collection: errors,
            filter: static error => (error == "modifier \"aim\" declares unknown control \"gamepad.buttonSouht\"")
        );
        Assert.Contains(
            collection: errors,
            filter: static error => (error == "modifier \"type\" declares unaddressable control \"keyboard.text\"")
        );
    ...
    public void ANullPageEntryIsRefusedNotThrown() {
        ...
        Assert.Contains(
            expectedSubstring: "entry 0 is null",
            actualString: Assert.Single(collection: errors)
        );
    }
    [Fact]
    public void APlainSourceTheCatalogCannotResolveIsRefusedByName() {
        ...
        Assert.Contains(
            expectedSubstring: "binds unknown control \"gamepad.buttonSouht\"",
            actualString: Assert.Single(collection: errors)
        );
    }
    [Fact]
    ...
    public void ARowMemberNamingNoDeclaredModifierAndNoKnownControlIsRefused() {
        ...
        Assert.Contains(
            expectedSubstring: "names \"key.shfit\", which is neither a declared modifier nor a known control",
            actualString: Assert.Single(collection: errors)
        );
    }
    [Fact]
    public void AValueLessActivatorDispatchesDigitalForKindValidation() {
        ...
        Assert.Contains(
            expectedSubstring: "sends digital",
            actualString: Assert.Single(collection: errors)
        );
    }
    [Fact]
    public void AWheelSectorsTextRequiresAWireArgsCommand() {
        ...
        Assert.Contains(
            expectedSubstring: "binds text arguments to \"action\", which accepts no wire arguments",
            actualString: Assert.Single(collection: errors)
        );
    ...
    public void AnUnaddressableSourceIsRefusedOnceRatherThanTwice() {
        ...
        Assert.Contains(
            expectedSubstring: "unaddressable control \"keyboard.text\"",
            actualString: Assert.Single(collection: errors)
        );
    }
    [Fact]
    ...
    public void ArgumentBindingRequiresAWireArgsCommandAndAPressCapableSource() {
        ...
        Assert.Contains(
            collection: errors,
            filter: static error => error.Contains(
                comparisonType: StringComparison.Ordinal,
                value: "accepts no wire arguments"
            )
        );
        Assert.Contains(
            collection: errors,
