...
public sealed class CommandModalityTests {
    ...
    private static CommandRegistry Registry(List<(string Name, int Slot)> activations, params Spec[] specs) {
        return new CommandRegistry(modules: [new ProbeModule(
    ...
    public void AModalityMayRetainGameplayWhileAddingAMenuOverlay() {
        ...
        Assert.True(condition: router.IsMapActive(
            map: Foot,
            slot: 0
        ));
        Assert.True(condition: router.IsMapActive(
        ...
        Assert.False(condition: router.IsMapActive(
            map: Foot,
            slot: 0
        ));
        Assert.True(condition: router.IsMapActive(
    ...
    public void AnAlreadyBuiltSnapshotRetainsItsResolvedModality() {
        ...
        var router = Router(
            registry: registry,
            bindings: new SameSourceBindings(
                "tank.fire",
                "plan.place"
            )
        );
    ...
    public void HeldDigitalChannelRecoversAfterItsMapBecomesActiveAgain() {
        ...
        var started = router.SnapshotForTick(
            tick: 1UL,
            windowEndTick: ulong.MaxValue
        );
        ...
        var closed = router.SnapshotForTick(
            tick: 2UL,
            windowEndTick: ulong.MaxValue
        );
        ...
        var reopened = router.SnapshotForTick(
            tick: 3UL,
            windowEndTick: ulong.MaxValue
        );
    ...
    public void RemovingAMapCancelsATappedChannelBeforeItsDeferredRelease() {
        ...
        var transitioned = router.SnapshotForTick(
            tick: 2UL,
            windowEndTick: ulong.MaxValue
        );
    ...
    public void RemovingAMapCancelsOnlyThatSlotsAffectedHolds() {
        ...
        Assert.Equal(
            actual: phases,
            expected: [(0, CommandPhase.Canceled), (1, CommandPhase.Active)]
        );
        Assert.False(condition: router.IsCommandHeld(
            command: "tank.drive",
            slot: 0
    ...
    public void ReplacingOneSlotsModalityDoesNotChangeAnotherSlot() {
        ...
        var router = new InputRouter(
            ...
            principalResolver: new SeatPrincipals(),
            slotResolver: device => ((device == menuDevice)
            ? 0
            : 1)
        );
        ...
        Assert.Equal(
            actual: activations,
            expected: [("menu.accept", 0), ("foot.jump", 1)]
        );
        Assert.True(condition: router.IsMapActive(
            map: CommandMaps.Global,
            slot: 0
        ));
        Assert.True(condition: router.IsMapActive(
            map: Menu,
    ...
    public void SubmittedTextRemainsOutsidePlayerModality() {
        ...
        Assert.Equal(
            ...
            actual: registry.Submit(line: "plan.text")
        );
        var snapshot = router.SnapshotForTick(
            tick: 1UL,
            windowEndTick: ulong.MaxValue
    ...
    public void ThreeSlotsResolveTankFootAndPlanIndependentlyFromTheSameSource() {
        ...
        var router = new InputRouter(
            ...
            principalResolver: new SeatPrincipals(),
            slotResolver: device => ((device == tankDevice)
            ? 0
            : ((device == footDevice)
                ? 1
                : 2))
    ...
    public void UnknownMapIsRefusedWithoutChangingTheSlot() {
        ...
        _ = Assert.Throws<ArgumentException>(testCode: () => router.SetActiveMaps(
            maps: new[] { "typo" },
            slot: 0
        ));
        Assert.True(condition: router.IsMapActive(
    ...
    private sealed class SameSourceBindings(params string[] commands) : IInputBindings {
        ...
        public IReadOnlyList<CommandBinding>? Resolve(int slot, string source) => ((source == "button.action")
            ? m_bindings
            : null
        );
    }
    private sealed class ChannelBindings(string command) : IInputBindings {
        ...
        public IReadOnlyList<CommandBinding>? Resolve(int slot, string source) => ((source == "axis.drive")
            ? m_bindings
            : null
        );
    }
