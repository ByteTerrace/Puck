...
public sealed class PagedInputBindingsTests {
    ...
    public void AChordCompletingPressIsStillJudgedByTheActivePagesTappedActivator() {
        ...
        Assert.DoesNotContain(
            collection: bindings.DrainChordEdges(slot: 0).ToArray(),
            filter: static edge => (edge.Command == ActionCommand)
        );
    }
    [Fact]
    ...
    public void AModeFlipTouchesNeitherTheLatchesNorTheChordTracker() {
        ...
        Assert.Equal(
            expected: "edit-left",
            actual: bindings.ViewFor(slot: 0).PageId
        );
    ...
    public void AModifierReleaseThatSatisfiesAShorterRowStillFiresThatRowsPressEdge() {
        ...
        Assert.Contains(
            collection: edges,
            filter: static edge => ((edge.Command == LongChordCommand) && (edge.Phase == CommandPhase.Completed))
        );
        Assert.Contains(
            collection: edges,
            filter: static edge => ((edge.Command == ShortChordCommand) && (edge.Phase == CommandPhase.Started))
        );
    }
    [Fact]
    public void AWithheldCompletionArmsNothingAndEmitsOnlyTheBreak() {
        ...
        Assert.Equal(
            actual: pressed.Command,
            expected: ShortChordCommand
        );
        Assert.Equal(
            actual: pressed.Phase,
    ...
    public void AuthoredTextReachesAnActivatorPressButNotItsCarriedStateOrRelease() {
        ...
        var opened = Assert.Single(collection: Assert.Single(collection: router.SnapshotForTick(
            tick: 2UL,
            windowEndTick: ulong.MaxValue
        ).Lanes).Entries);
        ...
        Assert.Equal(
            expected: $"{ActionCommand} first second",
            actual: opened.Text
        );

        var carried = Assert.Single(collection: Assert.Single(collection: router.SnapshotForTick(
            tick: 3UL,
            windowEndTick: ulong.MaxValue
        ).Lanes).Entries);
        ...
        var closed = Assert.Single(
            Assert.Single(collection: router.SnapshotForTick(
                tick: 4UL,
                windowEndTick: ulong.MaxValue
            ).Lanes).Entries,
    ...
    public void ChannelCommandChordRecoversAsActiveWhileAnEdgeCommandChordDoesNot() {
        ...
        Assert.Empty(collection: actionRouter.SnapshotForTick(
            tick: 1UL,
            windowEndTick: ulong.MaxValue
        ).Lanes);
        ...
        var recovered = Assert.Single(collection: Assert.Single(collection: channelRouter.SnapshotForTick(
            tick: 1UL,
            windowEndTick: ulong.MaxValue
        ).Lanes).Entries);
        ...
        Assert.Equal(
            ...
            actual: recovered.Phase
        );
        Assert.True(condition: recovered.Dispatch);
        Assert.True(condition: channelRouter.IsCommandHeld(
    ...
    public void CompiledProfileDefensivelyOwnsAuthoredCollections() {
        ...
        Assert.Equal(
            expected: 2,
            actual: bindings.ViewFor(slot: 0).Buttons.Count
        );
        Assert.Equal(
            expected: ["key.action"],
            actual: bindings.ViewFor(slot: 0).Buttons[0].Sources
        );
    ...
    public void CompiledWheelDefensivelyOwnsNestedThresholdCollections() {
        ...
        Assert.Equal(
            expected: 0.5f,
            actual: Assert.Single(collection: wheel.Style.Excursion!.Thresholds)
        );
        Assert.Equal(
            expected: 0.25f,
            actual: Assert.Single(collection: wheel.Excursion!.ThresholdsSquared)
        );
    }
    [Fact]
    public void DigitalReassertionRecoversAChannelWithoutFiringCommandsOrActivators() {
        ...
        var recovered = Assert.Single(collection: Assert.Single(collection: router.SnapshotForTick(
            tick: 2UL,
            windowEndTick: ulong.MaxValue
        ).Lanes).Entries);

        Assert.True(condition: registry.TryGetId(
            id: out var channelId,
            name: ChannelCommand
        ));
        Assert.Equal(
            expected: channelId,
            actual: recovered.CommandId
        );
        Assert.Equal(
            expected: CommandPhase.Active,
            actual: recovered.Phase
        ...
        Assert.Equal(
            expected: 1f,
            actual: recovered.Value.AsAxis1D
        );
        Assert.True(condition: router.IsCommandHeld(
            command: ChannelCommand,
            slot: 0
        ));
        ...
        Assert.False(condition: router.IsCommandHeld(
            command: ChannelCommand,
            slot: 0
        ));
    }
    ...
    public void HeldCommandAuthoredAsAPressRowAndAReleaseRowDispatchesItsRelease() {
        ...
        var pressed = Assert.Single(collection: router.SnapshotForTick(
            tick: 1UL,
            windowEndTick: ulong.MaxValue
        ).Lanes).Entries;
        ...
        Assert.Contains(
            collection: pressed,
            filter: e => (e.Dispatch && (e.Phase == CommandPhase.Started))
        );

        router.Capture(signal: InputSignal.Release(source: "mouse.button1"));
        var lanes = router.SnapshotForTick(
            tick: 2UL,
            windowEndTick: ulong.MaxValue
        ).Lanes;
        ...
        Assert.Contains(
            collection: lanes[0].Entries,
            filter: e => (e.Dispatch && (e.Phase == CommandPhase.Completed))
        );
    }
    [Fact]
    public void HoldsSourceGoesFalseOnceAnAnalogChordSourceHasReturnedToRest() {
        ...
        Assert.True(condition: bindings.HoldsSource(
            slot: 0,
            source: "pad.leftTrigger"
        ));
        ...
        Assert.False(condition: bindings.HoldsSource(
            slot: 0,
            source: "pad.leftTrigger"
        ));
    }
    ...
    public void MultiSourceModifierStaysHeldWhileOneOfItsSourcesRemainsDown() {
        var profile = BindingProfile.Compile(document: new BindingProfileDocument(
            ...
            Modifiers: [new BindingModifierDefinition(
                    Id: "wheel",
                    Sources: ["gamepad.leftShoulder", "keyboard.tab"]
                )],
            Chords: [new BindingChordDefinition(
                    Group: "play",
                    Chord: [],
        ...
        Assert.Equal(
            expected: [0],
            actual: tracker.HeldOrder.ToArray()
        );
        ...
        Assert.Equal(
            expected: 0,
            actual: tracker.HeldOrder.Length
        );
    }
    [Fact]
    public void MultiSourceRowPressesAndReleasesFromEitherControlIndependently() {
        var bindings = Bindings(entries: [
            new BindingPageEntryDefinition(
                Sources: ["gamepad.buttonSouth", "keyboard.space"],
                Command: ActionCommand
            ),
            new BindingPageEntryDefinition(
                Sources: ["gamepad.buttonSouth", "keyboard.space"],
        ...
        var padPress = Assert.Single(collection: router.SnapshotForTick(
            tick: 1UL,
            windowEndTick: ulong.MaxValue
        ).Lanes).Entries;
        ...
        var padRelease = router.SnapshotForTick(
            tick: 2UL,
            windowEndTick: ulong.MaxValue
        ).Lanes;
        ...
        Assert.Contains(
            collection: padRelease[0].Entries,
            filter: e => (e.Dispatch && (e.Phase == CommandPhase.Completed))
        );
        ...
        var keyPress = Assert.Single(collection: router.SnapshotForTick(
            tick: 3UL,
            windowEndTick: ulong.MaxValue
        ).Lanes).Entries;
        ...
        var keyRelease = router.SnapshotForTick(
            tick: 4UL,
            windowEndTick: ulong.MaxValue
        ).Lanes;
        ...
        Assert.Contains(
            collection: keyRelease[0].Entries,
            filter: e => (e.Dispatch && (e.Phase == CommandPhase.Completed))
        );
    }
    [Fact]
    public void OrderedModifierReassertionRestoresThePageButDoesNotFireItsCommandChord() {
        ...
        Assert.DoesNotContain(
            collection: snapshot.Lanes.SelectMany(selector: static lane => lane.Entries),
            filter: entry => (entry.CommandId == actionId)
        );
    }
    [Fact]
    public void ReleaseAfterReloadCannotFireANewReleaseOnlyCommand() {
        var initial = Bindings(entries: [new BindingPageEntryDefinition(
                Sources: ["key.hold"],
                Command: ActionCommand
            )]);
        var router = Router(
    ...
    public void SuppressedTransientChannelSampleNeverBecomesCarriedState() {
        ...
        router.Capture(signal: InputSignal.Axis(
            source: "mouse.motion",
        ...
        var suppressed = Assert.Single(collection: Assert.Single(collection: router.SnapshotForTick(
            tick: 1UL,
            windowEndTick: ulong.MaxValue
        ).Lanes).Entries);
    ...
    public void TappedActivatorFiresNowAndReleasesOnTheNextTick() {
        ...
        router.Capture(signal: InputSignal.Press(source: "key.b"));
        var pulse = Assert.Single(collection: Assert.Single(collection: router.SnapshotForTick(
            tick: 2UL,
            windowEndTick: ulong.MaxValue
        ).Lanes).Entries);
        ...
        Assert.Equal(
            expected: CommandOrigin.Binding,
            actual: pulse.Origin
        );
        ...
        var release = Assert.Single(collection: Assert.Single(collection: router.SnapshotForTick(
            tick: 3UL,
            windowEndTick: ulong.MaxValue
        ).Lanes).Entries);
        ...
        Assert.Equal(
            expected: pulse.Source,
            actual: release.Source
        );
    ...
    public void ToggleChannelFlipsOnPressAndIgnoresPhysicalRelease() {
        ...
        var started = Assert.Single(collection: Assert.Single(collection: router.SnapshotForTick(
            tick: 1UL,
            windowEndTick: ulong.MaxValue
        ).Lanes).Entries);
        ...
        Assert.Equal(
            expected: CommandPhase.Started,
            actual: started.Phase
        );
        Assert.Equal(
            expected: 1f,
            actual: started.Value.AsAxis1D
        );
        Assert.True(condition: router.IsCommandHeld(
            command: ChannelCommand,
            slot: 0
    ...
    public void TransientAxisChannelReleasesOnTheFollowingTickInsteadOfBecomingHeld() {
        ...
        var active = Assert.Single(collection: Assert.Single(collection: router.SnapshotForTick(
            tick: 1UL,
            windowEndTick: ulong.MaxValue
        ).Lanes).Entries);
        ...
        var released = Assert.Single(collection: Assert.Single(collection: router.SnapshotForTick(
            tick: 2UL,
            windowEndTick: ulong.MaxValue
        ).Lanes).Entries);
        ...
        Assert.Equal(
            expected: CommandPhase.Completed,
            actual: released.Phase
        );
        Assert.Equal(
            expected: 0f,
            actual: released.Value.AsAxis1D
        );
        Assert.True(condition: released.Dispatch);
        Assert.Empty(collection: router.SnapshotForTick(
            tick: 3UL,
    ...
    public void WheelForAnswersFromTheLoadedProfileAlone() {
        ...
        Assert.Equal(
            expected: "tools",
            actual: bindings.WheelFor(slot: 3)?.Id
        );
        ...
        Assert.Equal(
            expected: "belt",
            actual: bindings.WheelFor(slot: 0)?.Id
        );
    }
    [Fact]
