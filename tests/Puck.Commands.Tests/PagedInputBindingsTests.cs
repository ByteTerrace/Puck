...
public sealed class PagedInputBindingsTests {
    ...
    public void TappedActivatorFiresNowAndReleasesOnTheNextTick() {
        ...
        // A synthesized activator has no physical control to own its downstream held contribution, so the compiled
        // edge carries the destination's deterministic synthetic owner (BindingSourceIdentity) instead of nothing.
        Assert.Equal(
            actual: pulse.Source,
            expected: $"$binding:{ActionCommand}"
        );
