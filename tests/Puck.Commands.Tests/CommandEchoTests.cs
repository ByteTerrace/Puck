...
public sealed class CommandEchoTests {
    ...
    public void AFieldValueCarryingADelimiterCannotEndTheTokenSegmentOrEnvelope() {
        ...
        Assert.Equal(
            actual: line,
            expected: "[world.update: cacheRoot=\"C:\\\\my games\\\\cache\" members=\"[seat1|seat2]\"]"
        );
    ...
    public void ALineBreakIsEscapedRatherThanCarriedSoAnEchoIsAlwaysOneLine(string value, string expected) {
        ...
        Assert.DoesNotContain(
            actualString: CommandEcho.Open(verb: "world.note").Field(
                key: "message",
                value: value
            ).Close(),
            comparisonType: StringComparison.Ordinal,
    ...
    public void ASegmentFollowedByAFieldNeverTrails() {
        ...
        Assert.Equal(
            actual: line,
            expected: "[world.contributions: slot 'a' state=empty | tick=3]"
        );
    }
    [Fact]
    public void ASplicedTagDecodesThroughTheSameOnePass() {
        var tagged = CommandEcho.SpliceTag(
            prefix: "instance:",
            text: CommandEcho.Open(verb: "world.inhabitants").Field(
                key: "bodyIndex",
                value: 0
            ).Close(),
            value: "my world"
    ...
    public void ASplicedTagSurvivesTheConsoleResubmit(string value) {
        ...
        Assert.Equal(
            actual: CommandEcho.Unescape(value: token["instance:".Length..]),
            expected: value
        );
    }
    [Fact]
    ...
    public void AValueIsQuotedExactlyWhenItCarriesAReservedCharacter(string value, string expected) {
        Assert.Equal(
            actual: CommandEcho.Quote(value: value),
            expected: expected
        );
    }
    [Fact]
    public void AWholeEchoLineDecodesInOnePassThatASplitCannotDo() {
        ...
        Assert.Equal(
            actual: tokens,
            expected: ["[world.update:", "path=C:\\my games", "members=[seat1|seat2]", "|", "listing]"]
        );
    }
    [Fact]
    public void AnInteriorQuoteRidesAsAUnicodeEscapeRatherThanABackslashPair() {
        ...
        Assert.Equal(
            actual: CommandEcho.Quote(value: "say \"hi\""),
            expected: "\"say \\u0022hi\\u0022\""
        );
        Assert.Equal(
            actual: CommandEcho.Unquote(token: CommandEcho.Quote(value: "say \"hi\"")),
            expected: "say \"hi\""
        );
        ...
        Assert.Equal(
            actual: CommandEcho.Quote(value: "C:\\games\\"),
            expected: "\"C:\\\\games\\\\\""
        );
    }
    [Theory]
    ...
    public void EveryControlCharacterAndUnicodeSeparatorIsEscapedSoAnEchoIsStillOneLine(string value, string expected) {
        ...
        Assert.Equal(
            actual: CommandEcho.Unquote(token: CommandEcho.Quote(value: value)),
            expected: value
        );
    }
    [Fact]
    public void HeadAndTextAreDeliberatelyUnquoted() {
        ...
        Assert.Equal(
            actual: line,
            expected: "[world.contributions: slot 'a' state=empty | listing]"
        );
    }
    [Fact]
    public void HeadThenFieldsFormsOneSegmentDistinctFromAPureFieldSegment() {
        ...
        Assert.Equal(
            actual: line,
            expected: "[demo.groups: kindCount=1 | group id=1 status=Active]"
        );
    }
    [Fact]
    public void NoContentClosesToAnEmptyBody() {
        Assert.Equal(
            actual: CommandEcho.Open(verb: "world.groups").Close(),
            expected: "[world.groups:]"
        );
    }
    [Fact]
    public void OpenFieldCloseProducesTheBracketedLine() {
        ...
        Assert.Equal(
            actual: line,
            expected: "[world.status: kits=1 dirty=false]"
        );
    }
    [Theory]
    ...
    public void ReadingPastTheEndOfALineAnswersFalseRatherThanLooping() {
        ...
        Assert.True(condition: CommandEcho.TryReadToken(
            index: ref index,
            line: "  only  ",
            token: out var token
        ));
        ...
        Assert.False(condition: CommandEcho.TryReadToken(
            index: ref index,
            line: "  only  ",
            token: out var trailing
        ));
        Assert.Equal(
            actual: trailing,
    ...
    public void SegmentBetweenHeadFieldGroupsInsertsThePipeOnlyWhenSomethingFollows() {
        ...
        Assert.Equal(
            actual: line,
            expected: "[world.groups: kind name=party | group id=alpha kind=party]"
        );
    }
    [Theory]
    ...
    public void UnescapeInvertsQuoteForAValueWhoseQuotesAnotherSplitterRemoved(string value) {
        var quoted = CommandEcho.Quote(value: value);
        var stripped = (quoted.StartsWith(
            comparisonType: StringComparison.Ordinal,
            value: "\""
        )
            ? quoted[1..^1]
