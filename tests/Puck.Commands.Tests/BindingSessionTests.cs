...
public sealed class BindingSessionTests {
    ...
    private static (IReadOnlyList<BindingSessionEvent> Events, IReadOnlyList<BindingSessionCapture> Captures) RunProtocol() {
        var session = new BindingSession(plan: new BindingSessionPlan(
            Steps: [
                new BindingSessionStep(
                    Command: "jump",
                    SuggestedSource: "pad.south"
                ),
                new BindingSessionStep(
                    Command: "interact",
                    SuggestedSource: "pad.east"
                ),
                new BindingSessionStep(
                    Command: "target",
                    SuggestedSource: "axis.leftTrigger"
                ),
            ],
            RequiredPresses: 3,
        ...
        events.Add(item: session.Advance(signal: Axis(
        ...
        Assert.Equal(
            expected: BindingSessionStatus.Completed,
    ...
    public void AChannelStepIsWrittenBackAsAChannelEntryNotAsItsInternalCommandName() {
        ...
        Assert.Equal(
            ...
            expected: new ChannelRef.Name(Value: "movement")
        );
        Assert.Null(@object: entry.Command);
        Assert.Equal(
            expected: "key.new",
            actual: entry.Sources![0]
        );
        _ = BindingProfile.Compile(document: rewritten);
    }
    [Fact]
    ...
    public void ACompletedSessionIsInertAndSaysWhichStepItStoppedOn() {
        ...
        Assert.Equal(
            expected: BindingSessionEventKind.None,
            actual: ignored.Kind
        );
        // The step index is the one the session actually stands on — never a hard-coded zero that reads as a rewind.
        Assert.Equal(
            expected: 1,
            actual: ignored.StepIndex
        );
        Assert.Equal(
            expected: "pad.south",
            actual: Assert.Single(collection: session.Captures).Source
        );
    }
    [Fact]
    ...
    public void APlanWithNoReservedSourcesRefusesNothing() {
        ...
        Assert.Equal(
            expected: BindingSessionEventKind.SessionCompleted,
            actual: session.Advance(signal: InputSignal.Press(source: "pad.south")).Kind
        );
    }
    [Fact]
    public void ASinglePressPlanConfirmsOnTheVeryFirstPress() {
        ...
        Assert.Equal(
            expected: BindingSessionEventKind.SessionCompleted,
            actual: confirmed.Kind
        );
        Assert.Equal(
            expected: 0,
            actual: confirmed.StepIndex
        );
        Assert.Equal(
            expected: BindingSessionStatus.Completed,
            actual: session.Status
    ...
    public void AbandonStopsTheMachineAndKeepsTheCapturesItAlreadyConfirmed() {
        ...
        Assert.False(condition: session.Abandon());
        Assert.Equal(
            expected: BindingSessionStatus.Abandoned,
            actual: session.Status
        );
        Assert.Null(@object: session.PendingSource);
        Assert.Equal(
            expected: BindingSessionEventKind.None,
            actual: session.Advance(signal: InputSignal.Press(source: "pad.east")).Kind
        );
        Assert.Equal(
            expected: "jump",
            actual: Assert.Single(collection: session.Result.Captures).Command
        );
    }
    [Fact]
    public void PlanFromPageAlsoReservesARawChordMemberThatBecomesAnImplicitModifier() {
        ...
        Assert.Contains(
            ...
            expected: "pad.leftShoulder"
        );
        Assert.Contains(
            collection: plan.ReservedSources!,
            expected: "pad.rightShoulder"
        );

        var session = new BindingSession(plan: plan);

        Assert.Equal(
            expected: BindingSessionEventKind.ReservedRejected,
            actual: session.Advance(signal: InputSignal.Press(source: "pad.leftShoulder")).Kind
        );
    }
    [Fact]
    ...
    public void PlanFromPageSkipsActivatorRowsAndReservesModifierSources() {
        ...
        Assert.Equal(
            expected: "pad.south",
            actual: step.SuggestedSource
        );
        Assert.Equal(
            expected: 2,
            actual: plan.RequiredPresses
        );
        Assert.Equal(
            expected: "pad.leftTrigger",
            actual: Assert.Single(collection: plan.ReservedSources!)
        );
    }
    [Fact]
    ...
    public void ResultRewritesDisplacesAppendsAndStillCompiles() {
        ...
        Assert.Equal(
            expected: "menu",
            actual: Assert.Single(collection: displaced).Command
        );
        Assert.Equal(
            expected: "pad.east",
            actual: Assert.Single(
                entries,
                predicate: static entry => (entry.Command == "jump")
            ).Sources![0]
        );
        Assert.Equal(
            expected: "pad.north",
            actual: Assert.Single(
                entries,
                predicate: static entry => (entry.Command == "keep")
            ).Sources![0]
        );
        Assert.Equal(
            expected: "pad.west",
            actual: Assert.Single(
                entries,
                predicate: static entry => (entry.Command == "new-action")
            ).Sources![0]
        );
        _ = BindingProfile.Compile(document: rewritten);
    }
    ...
    public void SessionProtocolIsDeterministicAcrossSuggestionsMismatchesConflictsAndAnalogHysteresis() {
        ...
        Assert.Equal(
            actual: second.Captures,
            expected: first.Captures
        );
        Assert.Contains(
            collection: first.Events,
            filter: static item => (item.Kind == BindingSessionEventKind.ReservedRejected)
        );
        Assert.Contains(
            collection: first.Events,
            filter: static item => (item.Kind == BindingSessionEventKind.Mismatch)
        );
        Assert.Contains(
            collection: first.Events,
            filter: static item => (item.Kind == BindingSessionEventKind.ConflictRejected)
        );
        Assert.Contains(
            collection: first.Events,
            filter: static item => (item.Kind == BindingSessionEventKind.ConfirmationProgress)
        );
        Assert.Equal(
            expected: BindingSessionEventKind.SessionCompleted,
            actual: first.Events[^1].Kind
        );
