...
public sealed class BindingProfileCompilationLawTests {
    ...
    public void ADeepPageInheritanceChainCompilesWithoutExhaustingTheStack() {
        ...
        Assert.Equal(
            expected: Depth,
            actual: profile.RowCount
        );
        Assert.Equal(
            expected: "page0",
            actual: profile.RestingPageIdOf(group: "g")
        );
    }
    [Fact]
    public void ADocumentWithNoChordRowsCompilesToTheAnonymousEmptyGroup() {
        foreach (var chords in new IReadOnlyList<BindingChordDefinition>?[] { null, [] }) {
            ...
            Assert.Equal(
                expected: BindingProfile.EmptyGroup,
                actual: Assert.Single(collection: profile.Groups)
            );
            Assert.Equal(
                expected: BindingProfile.EmptyGroup,
                actual: profile.RestingPageIdOf(group: BindingProfile.EmptyGroup)
            );
    ...
    public void AGroupWhoseOnlyRowIsACommandHasNoRestingPageAndIsRefused() {
        ...
        Assert.Contains(
            actualString: memberless.Message,
            expectedSubstring: "resting row must be a page"
        );
    }
    [Fact]
    ...
    public void AHoldChannelChordRowDispatchesItsReleaseWithoutAuthoringHoldRelease() {
        ...
        Assert.Equal(
            expected: 1,
            actual: pressed.Length
        );
        Assert.Equal(
            expected: CommandPhase.Started,
            actual: pressed[0].Phase
        );
    ...
    public void ANullGroupIsRefusedByRowRatherThanCrashingTheCompiler() {
        ...
        Assert.Contains(
            actualString: chordRow.Message,
            expectedSubstring: "Chord row 0"
        );
        ...
        Assert.Contains(
            actualString: contextRow.Message,
            expectedSubstring: "Contexts row 0"
        );
        ...
        Assert.Contains(
            actualString: wheelRow.Message,
            expectedSubstring: "Wheel \"w\""
        );
    }
    [Fact]
    public void ARowMemberResolvesToADeclaredModifierIdRegardlessOfCase() {
        ...
        Assert.Equal(
            expected: "Wheel",
            actual: Assert.Single(collection: profile.Modifiers).Id
        );
    }
    [Fact]
    public void AWheelSectorsAuthoredTextIsSubmittedAsTheCommandsLine() {
        ...
        Assert.True(condition: router.Activate(
            slot: 0,
            activation: sectors[0].Activation
        ));
        Assert.True(condition: router.Activate(
            slot: 0,
            activation: sectors[1].Activation
        ));
        ...
        Assert.Equal(
            actual: entries[0].Text,
            expected: $"{TextCommand} north"
        );
        Assert.All(
            collection: entries,
    ...
    public void AnActivatorEntryKeysNoSourceButStillAppearsInButtons() {
        ...
        Assert.Equal(
            expected: "activator[key.a,key.b]",
            actual: Assert.Single(collection: view.Buttons).Source
        );
    ...
    public void EverySourceOfAMultiSourceEntryFindsItsButton() {
        ...
        var view = new PagedInputBindings(profile: Compile(rows: [
            Resting(
                new BindingPageEntryDefinition(
                    Sources: ["gamepad.buttonSouth", "key.space"],
                    Command: "test.jump"
                ),
                new BindingPageEntryDefinition(
                    Sources: ["gamepad.buttonEast"],
                    Command: "test.cancel"
                )
            ),
        ])).ViewFor(slot: 0);
    ...
    public void PageInheritanceOverlaysSourceBySourceAndByActivatorIdentity() {
        ...
        Assert.Equal(
            actual: child.ButtonsBySource["key.a"].Command,
            expected: "parent.trio"
        );
        Assert.Equal(
            actual: child.ButtonsBySource["key.c"].Command,
            expected: "parent.trio"
        );
        Assert.Equal(
            actual: child.ButtonsBySource["key.b"].Command,
            expected: "child.one"
        );
        Assert.Equal(
            actual: child.ButtonsBySource["key.d"].Command,
            expected: "parent.solo"
        );
    ...
    public void PageInheritanceRefusesCyclesAndCrossGroupParents() {
        ...
        Assert.Contains(
            actualString: crossGroup.Message,
            expectedSubstring: "same group"
        );
    }
    [Fact]
    public void TwoModifierIdsDifferingOnlyByCaseAreRefused() {
        ...
        Assert.Contains(
            actualString: refused.Message,
            expectedSubstring: "Modifier 1 re-declares id"
        );
    }
