...
public sealed class InputRouterHardeningTests {
    ...
    public void ADisposedRouterRefusesEveryIngressDoor() {
        ...
        _ = Assert.Throws<ObjectDisposedException>(testCode: () => router.SnapshotForTick(
            tick: 1UL,
    ...
    public void AFocusExemptReleaseReturnsAFlippedPageToRest() {
        ...
        var pressed = Assert.Single(collection: Assert.Single(collection: router.SnapshotForTick(
            tick: 3UL,
            windowEndTick: ulong.MaxValue
        ).Lanes).Entries);
    ...
    public void AMapTransitionThatKeepsTheMapActiveStillCompletesATappedActivator() {
        ...
        var completion = Assert.Single(collection: Assert.Single(collection: router.SnapshotForTick(
            tick: 2UL,
            windowEndTick: ulong.MaxValue
        ).Lanes).Entries);
    ...
    public void HeldSeedingEmitsOneSlotsHoldsInCommandIdOrder() {
        ...
        var reasserted = Assert.Single(collection: router.SnapshotForTick(
            tick: 2UL,
            windowEndTick: ulong.MaxValue
        ).Lanes).Entries.ToArray();

        Assert.Equal(
            actual: reasserted.Length,
            expected: 2
        );
        Assert.Equal(
            actual: reasserted[0].CommandId,
            expected: alphaId
        );
        Assert.Equal(
            actual: reasserted[1].CommandId,
            expected: betaId
        );
    }
    [Fact]
    public void SnapshotForTickRefusesATickBehindTheOneItLastProduced() {
        ...
        _ = Assert.Throws<ArgumentOutOfRangeException>(testCode: () => router.SnapshotForTick(
            tick: 9UL,
            windowEndTick: 12UL
        ));
    }
    ...
    public void StrandedReleasesEmitInCommandIdOrder() {
        ...
        var stranded = Assert.Single(collection: router.SnapshotForTick(
            tick: 2UL,
            windowEndTick: ulong.MaxValue
        ).Lanes)
            .Entries
    ...
    public void SynthesizedEdgesLandExactlyOneTickAfterTheirCauseDuringACatchUp() {
        ...
        var router = new InputRouter(
            ...
            bindings: new SourceBindings(bindings: new Dictionary<string, CommandBinding[]>(comparer: StringComparer.Ordinal) {
                ...
                ["pad.trigger"] = [new CommandBinding(
                    Command: ChannelCommand,
                    ChannelScale: 1f
                )],
            }),
        ...
        var first = Assert.Single(collection: router.SnapshotForTick(
            tick: 1UL,
            windowEndTick: 11UL
        ).Lanes).Entries;
        ...
        Assert.Contains(
            ...
            filter: static entry => ((entry.Phase == CommandPhase.Started) && entry.Dispatch)
        );
        ...
        var second = Assert.Single(collection: router.SnapshotForTick(
            tick: 2UL,
            windowEndTick: 12UL
        ).Lanes).Entries.ToArray();
        ...
        Assert.Contains(
            collection: second,
            filter: static entry => ((entry.Phase == CommandPhase.Completed) && (entry.Value.AsAxis1D == 0f))
        );
        Assert.Contains(
            collection: second,
            filter: static entry => (entry.Phase == CommandPhase.Canceled)
        );
        Assert.Empty(collection: router.SnapshotForTick(
            tick: 3UL,
            windowEndTick: 13UL
    ...
    public void TheCaptureQueueDropsItsOldestBeyondTheCap() {
        ...
        Assert.Equal(
            actual: router.DroppedCaptureCount,
            expected: ((long)Overflow)
        );
        ...
        Assert.Equal(
            actual: bindings.Resolved.Count,
            expected: InputRouter.MaxCapturedSignals
        );
        Assert.Equal(
            actual: bindings.Resolved[0],
            expected: $"key.{Overflow}"
        );
    }
    [Fact]
    public void TheInjectionQueueDropsItsOldestBeyondTheCap() {
        ...
        Assert.Equal(
            actual: router.DroppedInjectionCount,
            expected: ((long)(Total - InputRouter.MaxCapturedInjections))
        );
        Assert.Equal(
            actual: router.DroppedCaptureCount,
            expected: 0L
        ...
        Assert.Equal(
            actual: Assert.Single(collection: router.SnapshotForTick(
                tick: 1UL,
                windowEndTick: ulong.MaxValue
            ).Lanes).Entries.Count,
