...
public sealed class InputRouterTests {
    ...
    private static void Inject(CommandRegistry registry, string text) =>
        Assert.Equal(
            ...
            actual: registry.Submit(line: $"{Command} {text}")
        );
    ...
    public void AuthoredLaneSignalBypassesTheSlotResolverAndLeavesNoPresence() {
        ...
        Assert.False(condition: router.TryGetLastInputTick(
            slot: 2,
            tick: out _
        ));
    }
    ...
    public void BothCaptureStreamsRetainFutureDatedItemsUntilTheirWindow() {
        ...
        var entries = Assert.Single(collection: router.SnapshotForTick(
            tick: 2UL,
            windowEndTick: 41UL
        ).Lanes).Entries;
        ...
        Assert.Collection(
            ...
            static entry => {
                ...
                Assert.Equal(
                    expected: $"{Command} future-injection",
                    actual: entry.Text
                );
            }
        );
    ...
    public void BoundDispatchCarriesTheBindingRowsTextAndNeverTheSignalsTypedPayload() {
        ...
        var press = Assert.Single(collection: Assert.Single(collection: authored.SnapshotForTick(
            tick: 1UL,
            windowEndTick: ulong.MaxValue
        ).Lanes).Entries);
        ...
        overtyped.Capture(signal: InputSignal.Typed(
            source: "keyboard.text",
            text: "n"
        ));

        Assert.Equal(
            actual: Assert.Single(collection: Assert.Single(collection: overtyped.SnapshotForTick(
                tick: 1UL,
                windowEndTick: ulong.MaxValue
            ).Lanes).Entries).Text,
            expected: $"{Command} 8"
        ...
        typed.Capture(signal: InputSignal.Typed(
            source: "keyboard.text",
            text: "n"
        ));

        var character = Assert.Single(collection: Assert.Single(collection: typed.SnapshotForTick(
            tick: 1UL,
            windowEndTick: ulong.MaxValue
        ).Lanes).Entries);
    ...
    public void DeviceDisconnectCancelsAndDropsTheHoldItCarried() {
        ...
        var cancellation = Assert.Single(collection: Assert.Single(collection: router.SnapshotForTick(
            tick: 2UL,
            windowEndTick: ulong.MaxValue
        ).Lanes).Entries);

        Assert.Equal(
            expected: CommandPhase.Canceled,
            actual: cancellation.Phase
        );
        Assert.Equal(
            expected: CommandOrigin.Binding,
            actual: cancellation.Origin
        );
    }
    [Fact]
    public void DigitalReassertionCountsAsLiveInputWithoutDispatchingANewCommandEdge() {
        ...
        Assert.True(condition: router.TryGetLastInputTick(
            slot: 0,
            tick: out var lastInput
        ));
        Assert.Equal(
            actual: lastInput,
            expected: 7UL
    ...
    public void DisconnectRepairsAStaleDeviceAnnotationWhenAnotherControlSustainsTheHold() {
        ...
        var held = Assert.Single(collection: Assert.Single(collection: router.SnapshotForTick(
            tick: 4UL,
            windowEndTick: ulong.MaxValue
        ).Lanes).Entries);
    ...
    public void FocusLossCancelsHeldCommandsDropsQueuedPhysicalSignalsAndResetsBindings() {
        ...
        var injection = Assert.Single(
            collection: entries,
            predicate: static entry => (entry.Text == $"{Command} survives-focus-loss")
        );
        ...
        Assert.Equal(
            expected: CommandOrigin.Text,
            actual: injection.Origin
        );
        Assert.DoesNotContain(
            collection: entries,
            filter: static entry => (entry.Source == "key.up")
        );
        Assert.Empty(collection: router.SnapshotForTick(
            tick: 3UL,
            windowEndTick: ulong.MaxValue
    ...
    public void KeyRepeatRefreshesLastInputWithoutDispatchingASecondPress() {
        ...
        var repeated = router.SnapshotForTick(
            tick: 20UL,
            windowEndTick: ulong.MaxValue
        );
        ...
        Assert.DoesNotContain(
            collection: Assert.Single(collection: repeated.Lanes).Entries,
            filter: static entry => entry.Dispatch
        );
    }
    [Fact]
    public void RecycledHeldStateDoesNotRetainItsPreviousControl() {
        ...
        var press = Assert.Single(
            collection: entries,
            predicate: static entry => entry.Dispatch
        );
    ...
    public void RemovingAMapDeterministicallyCancelsAnExistingHold() {
        ...
        var press = router.SnapshotForTick(
            tick: 1UL,
            windowEndTick: ulong.MaxValue
        );
        ...
        var release = router.SnapshotForTick(
            tick: 2UL,
            windowEndTick: ulong.MaxValue
        );
        ...
        Assert.Equal(
            actual: phases,
            expected: [CommandPhase.Started, CommandPhase.Canceled]
        );
        Assert.False(condition: router.IsCommandHeld(
            command: "mapped.hold",
            slot: 0
    ...
    public void ResolvedBindingCacheDoesNotOwnRetiredListIdentities() {
        ...
        var retired = ResolveAndRetireList(
            bindings: bindings,
            router: router
        );
    ...
    public void RestBandAndPostureSamplesDoNotCountAsPlayerActivity() {
        ...
        router.Capture(signal: new InputSignal(
            ...
            DeviceId: default,
            Value: CommandValue.Axis(value: new System.Numerics.Vector3(
                x: 0f,
                y: 1f,
                z: 0f
            )),
    ...
    public void ScopedBindingReloadCancelsOnlyTheAffectedSlot() {
        ...
        var router = new InputRouter(
            ...
            bindings: bindings,
            principalResolver: new ConsolePrincipal(),
            slotResolver: device => ((device == firstDevice)
            ? 0
            : 1)
        );
        ...
        Assert.Equal(
            expected: CommandPhase.Canceled,
            actual: Assert.Single(collection: snapshot.Lanes[0].Entries).Phase
        );
        Assert.Equal(
            expected: CommandPhase.Active,
            actual: Assert.Single(collection: snapshot.Lanes[1].Entries).Phase
        );
    }
    [Fact]
    public void SignalAndInjectionStreamsMergeByCaptureSequence() {
        ...
        Assert.Collection(
            ...
            static entry => {
                ...
                Assert.Equal(
                    expected: $"{Command} between-edges",
                    actual: entry.Text
                );
            },
            static entry => {
    ...
    public void SnapshotIdentityIsStructuralAndExcludesLocalDeviceAnnotations() {
        ...
        var firstSnapshot = first.SnapshotForTick(
            tick: 1UL,
            windowEndTick: ulong.MaxValue
        );
        var secondSnapshot = second.SnapshotForTick(
            tick: 1UL,
            windowEndTick: ulong.MaxValue
        );

        Assert.Equal(
            actual: secondSnapshot,
            expected: firstSnapshot
        );
        Assert.Equal(
            expected: firstSnapshot.GetHashCode(),
            actual: secondSnapshot.GetHashCode()
        );
    }
    [Fact]
    public void TwoControlsOnOneCommandPressOnFirstDownAndReleaseOnLastUp() {
        ...
        var press = Assert.Single(collection: Assert.Single(collection: router.SnapshotForTick(
            tick: 1UL,
            windowEndTick: ulong.MaxValue
        ).Lanes).Entries);
        ...
        Assert.All(
            collection: Assert.Single(collection: router.SnapshotForTick(
                tick: 2UL,
                windowEndTick: ulong.MaxValue
            ).Lanes).Entries,
            action: static entry => Assert.False(condition: entry.Dispatch)
        );
        Assert.True(condition: router.IsCommandHeld(
            command: Command,
            slot: 0
