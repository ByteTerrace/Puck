...
internal static class GuardScaleTieDisciplineClaims {
        ...
        foreach (var (frequencyHz, damping) in createSamples) {
            ...
            foreach (var (stepTicks, ticksPerSecond) in stepSamples) {
                ...
                if (dynamics.Branch == SecondOrderDynamicsBranch.Underdamped) {
                    ...
                    var p1Mismatch = CheckExpNegativeReduction(numerator: p1Numerator, denominator: stepDenominator, guard: guard, label: $"ExpNegative(p1) at f={frequencyHz} zeta={damping} stepTicks={stepTicks} ticksPerSecond={ticksPerSecond}");

                    if (p1Mismatch is not null) { return p1Mismatch; }

                    var p2Mismatch = CheckExpNegativeReduction(numerator: p2Numerator, denominator: stepDenominator, guard: guard, label: $"ExpNegative(p2) at f={frequencyHz} zeta={damping} stepTicks={stepTicks} ticksPerSecond={ticksPerSecond}");
    ...
    private static string? CheckExpNegativeReduction(BigInteger numerator, BigInteger denominator, int guard, string label) {
        ...
        return CompareGuardRounding(numerator: numerator, denominator: reducedDenominator, guard: guard, label: label);
    }
    ...
    private static string? CheckSinCosReduction(BigInteger numerator, BigInteger denominator, int guard, string label) {
        ...
        return CompareGuardRounding(numerator: reducedNumerator, denominator: reducedDenominator, guard: guard, label: label);
    }
