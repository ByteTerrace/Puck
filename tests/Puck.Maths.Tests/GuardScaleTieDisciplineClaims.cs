...
internal static class GuardScaleTieDisciplineClaims {
    ...
    private static string? CheckExpNegativeReduction(BigInteger numerator, BigInteger denominator, int guard, string label) {
        if (
            numerator.IsZero ||
            (numerator > (SecondOrderExactMath.ExpUnderflowExponent * denominator))
        ) {
            return null;
        }
        ...
        foreach (var (frequencyHz, damping) in createSamples) {
            ...
            foreach (var (stepTicks, ticksPerSecond) in stepSamples) {
                ...
                if (dynamics.Branch == SecondOrderDynamicsBranch.Underdamped) {
                    ...
                    var p1Mismatch = CheckExpNegativeReduction(
                        denominator: stepDenominator,
                        guard: Guard,
                        label: $"ExpNegative(p1) at f={frequencyHz} zeta={damping} stepTicks={stepTicks} ticksPerSecond={ticksPerSecond}",
                        numerator: p1Numerator
                    );

                    if (p1Mismatch is not null) { return p1Mismatch; }

                    var p2Mismatch = CheckExpNegativeReduction(
                        denominator: stepDenominator,
                        guard: Guard,
                        label: $"ExpNegative(p2) at f={frequencyHz} zeta={damping} stepTicks={stepTicks} ticksPerSecond={ticksPerSecond}",
                        numerator: p2Numerator
                    );
    ...
    public static string? RoundToGuardScaleTiesVsHalfUpSurface() {
        ...
        for (var truncated = 0; (truncated <= 7); ++truncated) {
            ...
            var expectedNew = (isEven
                ? truncated
                : (truncated + 1)
            );

            if (newRaw != expectedNew) {
                return $"RoundRational({numerator}, {denominator}, {Guard}) = {newRaw}, expected {expectedNew} (ties-to-even, truncated={truncated} is {(isEven
                    ? "even"
                    : "odd")})";
            }
