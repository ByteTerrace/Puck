...
internal static class MoveTowardAndEmitterClaims {
    ...
    public static string? MoveTowardCarrierExtremesSurface() {
        foreach (var (currentRaw, targetRaw) in CarrierExtremeVectorSegments) {
            var current = VectorRaw(
                x: currentRaw.X,
                y: currentRaw.Y,
                z: currentRaw.Z
            );
            var target = VectorRaw(
                x: targetRaw.X,
                y: targetRaw.Y,
                z: targetRaw.Z
            );
            ...
            foreach (var maxDeltaRaw in CarrierExtremeSteps) {
                ...
                var (landing, signX, signY, signZ) = Oracles.MoveTowardVerdict(
                    currentX: currentRaw.X,
                    currentY: currentRaw.Y,
                    currentZ: currentRaw.Z,
                    maxDeltaRaw: maxDeltaRaw,
                    targetX: targetRaw.X,
                    targetY: targetRaw.Y,
                    targetZ: targetRaw.Z
                );
                var actual = FixedVector3.MoveToward(
                    current: current,
                    maxDelta: maxDelta,
                    target: target
                );
                ...
                if (
                    !AxisMovedTowardOracleSign(
                    before: currentRaw.X,
                    after: actual.X.Value,
                    expectedSign: signX
                ) ||
                    !AxisMovedTowardOracleSign(
                    before: currentRaw.Y,
                    after: actual.Y.Value,
                    expectedSign: signY
                ) ||
                    !AxisMovedTowardOracleSign(
                    before: currentRaw.Z,
                    after: actual.Z.Value,
                    expectedSign: signZ
                )
                ) {
    ...
    public static string? MoveTowardSurface() {
        ...
        try {
            _ = FixedVector3.MoveToward(
                ...
                maxDelta: FixedQ4816.FromRawBits(value: -1)
            );
        ...
        foreach (var (currentX, currentY, currentZ, targetX, targetY, targetZ) in Segments) {
            ...
            if (current != FixedVector3.MoveToward(
                current: current,
                maxDelta: separation,
                target: current
            )) {
            ...
            var held = FixedVector3.MoveToward(
                current: current,
                target: target,
                maxDelta: FixedQ4816.Zero
            );
            ...
            foreach (var reach in new[] { separation, (separation + FixedQ4816.One), FixedQ4816.FromInteger(value: 1000), }) {
                ...
                var landed = FixedVector3.MoveToward(
                    current: current,
                    maxDelta: reach,
                    target: target
                );
            ...
            foreach (var (numerator, denominator) in StepFractions) {
                ...
                if (
                    (step <= FixedQ4816.Zero) ||
                    (step >= separation)
                ) { continue; }

                var moved = FixedVector3.MoveToward(
                    current: current,
                    maxDelta: step,
                    target: target
                );
    ...
    public static string? RustPortEmitterSurface() {
        ...
        if (!string.Equals(
            a: generated,
            b: FixedQ4816RustPort.EmitGenerated(),
        ...
        if (!string.Equals(
            a: vectors,
            b: FixedQ4816RustPort.EmitVectors(),
            comparisonType: StringComparison.Ordinal
        )) {
    ...
    public static string? ScalarMoveTowardCarrierExtremesSurface() {
        foreach (var (currentRaw, targetRaw) in CarrierExtremeScalarSegments) {
            ...
            foreach (var maxDeltaRaw in CarrierExtremeSteps) {
                var maxDelta = FixedQ4816.FromRawBits(value: maxDeltaRaw);
                var expectedRaw = Oracles.MoveTowardRaw(
                    currentRaw: currentRaw,
                    maxDeltaRaw: maxDeltaRaw,
                    targetRaw: targetRaw
                );
                var actual = FixedQ4816.MoveToward(
                    current: current,
                    maxDelta: maxDelta,
                    target: target
                );
    ...
    public static string? ScalarMoveTowardSurface() {
        try {
            _ = FixedQ4816.MoveToward(
                current: FixedQ4816.Zero,
                target: FixedQ4816.One,
                maxDelta: FixedQ4816.FromRawBits(value: -1)
            );
        ...
        foreach (var (currentRaw, targetRaw) in ScalarSegments) {
            ...
            if (current != FixedQ4816.MoveToward(
                current: current,
                maxDelta: separation,
                target: current
            )) {
            ...
            var held = FixedQ4816.MoveToward(
                current: current,
                target: target,
                maxDelta: FixedQ4816.Zero
            );
            ...
            foreach (var reach in new[] { separation, (separation + FixedQ4816.One), FixedQ4816.FromInteger(value: 1000) }) {
                ...
                var landed = FixedQ4816.MoveToward(
                    current: current,
                    maxDelta: reach,
                    target: target
                );
            ...
            foreach (var (numerator, denominator) in StepFractions) {
                ...
                var moved = FixedQ4816.MoveToward(
                    current: current,
                    maxDelta: step,
                    target: target
                );
