...
public sealed class VerifiedCodeManifestTests {
    ...
    public void EveryCitedLawIdResolves() {
        var manifestPath = Path.GetFullPath(path: Path.Combine(
            path1: TestPaths.ProjectDirectory,
            path2: "..",
            path3: "..",
            path4: "VerifiedCode.json"
        ));
        ...
        Assert.True(
            ...
            userMessage: $"{unresolved.Count} VerifiedCode.json law citation(s) resolve to no LawDeclarations.All entry: {string.Join(
                separator: ", ",
