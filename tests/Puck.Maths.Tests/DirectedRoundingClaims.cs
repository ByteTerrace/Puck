...
internal static class DirectedRoundingClaims {
    private static string? Bound(long x, long y, BigInteger z, bool subjectOk, long result, string label) {
        ...
        if (
            (result > 0L) &&
            (((((BigInteger)result) - BigInteger.One) * (result - 1L)) >= radicand)
        ) {
    ...
    private static string? Verdict(bool subjectOk, long result, BigInteger numerator, BigInteger denominator, string operands) {
        var exact = Oracles.CeilingRational(
    ...
    public static string? CeilingMagnitudeIsLeastUpperBound(long[] left, long[] right) {
        ...
        var planarOk = FixedDirectedRounding.TryCeilingMagnitude(
            result: out var planar,
            x: x,
            y: y
        );
        var spatialOk = FixedDirectedRounding.TryCeilingMagnitude(
            result: out var spatial,
            x: x,
            y: y,
            z: z
        );
    ...
    public static string? CeilingProductSumIsLeastUpperBound(long[] left, long[] right) {
        ...
        var (numerator, denominator) = Scaled(
            shift: (fractionBitsOut - commonScale),
            value: (productTerm + addendTerm)
        );
        ...
        if (!withinBudget) {
            return (!subjectOk
                ? ((result == 0L)
                    ? null
                    : $"{operands}: refused outside the 127-bit per-term budget but left {result} behind")
    ...
    public static string? CeilingSquareRootIsLeastUpperBound(long[] left, long[] right) {
        ...
        var subjectOk = FixedDirectedRounding.TryCeilingSquareRoot(
    ...
    public static string? NegativeOperandsRefuse() {
        if (
            FixedDirectedRounding.TryCeilingSquareRoot(
            fractionBitCount: 16,
            result: out var root,
            value: -1L
        ) ||
        ...
        if (
            ...
            fractionBitsOut: 0,
            numerator: 1L,
            result: out var negativeDivisor
        ) ||
            (negativeDivisor != 0L)
        ) {
