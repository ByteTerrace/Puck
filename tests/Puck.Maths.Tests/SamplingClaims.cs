...
internal static class SamplingClaims {
    ...
    public static string? CertifiedLowDiscrepancyBoundTeethAndGapSurface() {
        ...
        foreach (var certifiedCase in cases) {
            ...
            Assert.Equal(
                expected: certifiedCase.Certificate,
                actual: sequence.Certificate
            );
        ...
        foreach (var pointCount in new[] { 1024, 4096, 16384 }) {
            var (goldenNumerator, goldenDenominator) = ExactStarDiscrepancy(
                pointCount: pointCount,
                sequence: golden
            );
        ...
        for (var i = 0UL; (i < 4096UL); ++i) {
            Assert.Equal(
                expected: golden.Point(index: i),
                actual: goldenAgain.Point(index: i)
            );
        }
        ...
        Assert.Equal(
            expected: 0x9E3779B9U,
            actual: golden.Point(index: 1UL).Value
        );
    ...
    public static string? FieldNoisePeriodicityAndDistributionSurface() {
        ...
        for (var probe = 0; (probe < 32); ++probe) {
            ...
            var unitTranslated = (position + new FixedVector3(
                X: FixedQ4816.One,
                Y: FixedQ4816.Zero,
                Z: FixedQ4816.Zero
            ));
            ...
            if (FieldNoise.Sample(
                position: position,
                seed: unchecked((probeSeed + FormerSeedCombineConstant))
            ) == FieldNoise.Sample(
                position: unitTranslated,
                seed: probeSeed
            )) {
        ...
        var seamRaw = (1L << 62);
        var seamLeft = FieldNoise.Sample(
            seed: 42UL,
            position: new FixedVector3(
                X: FixedQ4816.FromRawBits(value: (seamRaw - 1L)),
                Y: FixedQ4816.FromRawBits(value: 17_123L),
                Z: FixedQ4816.FromRawBits(value: -9_321L)
            ),
            octaves: 5
        );
        var seamRight = FieldNoise.Sample(
            seed: 42UL,
            position: new FixedVector3(
                X: FixedQ4816.FromRawBits(value: seamRaw),
                Y: FixedQ4816.FromRawBits(value: 17_123L),
                Z: FixedQ4816.FromRawBits(value: -9_321L)
            ),
            octaves: 5
        ...
        for (var sample = 0; (sample < SampleCount); ++sample) {
            ...
            var value = ((double)FieldNoise.Sample(
                position: position,
                seed: 42UL
            ).Value);
        ...
        var standardDeviation = (Math.Sqrt(d: ((sumOfSquares / SampleCount) - Math.Pow(
            x: (sum / SampleCount),
            y: 2
        ))) / 65536.0);
        ...
        ) { return $"noise standard deviation {standardDeviation:F5} outside [0.15, 0.45]"; }
    ...
    public static string? GaussianMomentsCdfTailSurface() {
        ...
        if (
            (tail < (tailLow - StatisticalMargin)) ||
            (tail > (tailHigh + StatisticalMargin))
        ) {
    ...
    public static string? PcgTranscribedReferenceAndDecorrelationSurface() {
        ...
        foreach (var (state, increment) in pairs) {
            ...
            var subject = Pcg32XshRr.FromRawBits(
                increment: oddIncrement,
        ...
        foreach (var (firstStreamId, secondStreamId) in new (ulong, ulong)[] { (0UL, 1UL), (1UL, 2UL), (7UL, 8UL), (100UL, 101UL), (0UL, 2UL) }) {
            var firstStream = Pcg32XshRr.Create(
                state: 99UL,
                stream: firstStreamId
            );
            var secondStream = Pcg32XshRr.Create(
                state: 99UL,
                stream: secondStreamId
            );
