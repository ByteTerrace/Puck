...
internal static class DctClaims {
    ...
    private static FixedQ4816[] DirectDct(ReadOnlySpan<FixedQ4816> values) {
        ...
        for (var k = 0; (k < n); ++k) {
            ...
            for (var index = 0; (index < n); ++index) {
                var phase = unchecked(((((ulong)((2 * index) + 1)) * ((ulong)k)) << (62 - BitOperations.Log2(value: ((uint)n)))));
    ...
    private static FixedQ4816[] Sequence(int length, ulong stream) {
        var rng = Pcg32XshRr.Create(
            state: 0x4443_5432_2D46_4958UL,
            stream: stream
        );
        var values = new FixedQ4816[length];

        for (var i = 0; (i < length); ++i) {
            var raw = (((long)rng.NextUInt32(
                maximum: ((uint)((2 * AmplitudeRaw) + 1)),
                minimum: 0U
            )) - AmplitudeRaw);
    ...
    public static string? ConstantAndImpulseExact() {
        foreach (var length in DefaultLengths) {
            ...
            for (var k = 0; (k < length); ++k) {
                var phase = unchecked(((0UL - ((ulong)k)) << (62 - BitOperations.Log2(value: ((uint)length)))));
    ...
    public static string? LengthRefusals() {
        ...
        return (Refuses(
            action: () => FixedCosineTransformPlan.Create(length: 0),
            type: typeof(ArgumentOutOfRangeException),
            parameterName: "length",
            what: "FixedCosineTransformPlan.Create(0)"
        ) ??
               (Refuses(
            action: () => FixedCosineTransformPlan.Create(length: -8),
            type: typeof(ArgumentOutOfRangeException),
            parameterName: "length",
            what: "FixedCosineTransformPlan.Create(-8)"
        ) ??
               (Refuses(
            action: () => FixedCosineTransformPlan.Create(length: 12),
            type: typeof(ArgumentOutOfRangeException),
            parameterName: "length",
            what: "FixedCosineTransformPlan.Create(12) (not a power of two)"
        ) ??
               (Refuses(
            action: () => FixedCosineTransform.Forward(
                plan: plan,
                scratch: new FixedComplex[8],
                values: new FixedQ4816[4]
            ),
            type: typeof(ArgumentException),
            parameterName: "values",
            what: "Forward with a mis-sized values span"
        ) ??
               (Refuses(
            action: () => FixedCosineTransform.Forward(
                plan: plan,
                scratch: new FixedComplex[4],
                values: new FixedQ4816[8]
            ),
            type: typeof(ArgumentException),
            parameterName: "scratch",
            what: "Forward with a mis-sized scratch span"
        ) ??
               (Refuses(
            action: () => FixedCosineTransform.Inverse(
                plan: plan,
                scratch: new FixedComplex[8],
                values: new FixedQ4816[16]
            ),
            type: typeof(ArgumentException),
            parameterName: "values",
            what: "Inverse with a mis-sized values span"
        ) ??
               Refuses(
            action: () => FixedCosineTransform.Inverse(
                plan: plan,
                scratch: new FixedComplex[16],
                values: new FixedQ4816[8]
            ),
            type: typeof(ArgumentException),
            parameterName: "scratch",
            what: "Inverse with a mis-sized scratch span"
        )))))));
    }
