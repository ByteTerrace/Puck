...
internal static class SymmetricSolveClaims {
    ...
    public static string? ApplyRefusalAndSymmetry() {
        ...
        if (
            !FixedSymmetricSolve.TryApplySymmetric2(
            a: 0L,
            b: 1L,
            d: 0L,
    ...
    public static string? DivideMagnitudeRoundedFullWidthAgrees() {
        var minimumOk = FusedArithmetic.TryDivideMagnitudeRounded(
            ...
            fractionBitCount: int.MinValue,
            quotient: out var minimumQuotient
        );
        var negativeOneOk = FusedArithmetic.TryDivideMagnitudeRounded(
            numeratorMagnitude: UInt128.One,
            denominatorMagnitude: UInt128.One,
            fractionBitCount: -1,
        ...
        if (
            negativeOneOk ||
            (negativeOneQuotient != UInt128.Zero)
        ) {
        ...
        if (FusedArithmetic.TryDivideMagnitudeRounded(
            denominatorMagnitude: smallDenominator,
            fractionBitCount: 129,
            numeratorMagnitude: smallNumerator,
            quotient: out var wrapped
        )) {
        ...
        if (!FusedArithmetic.TryDivideMagnitudeRounded(
            denominatorMagnitude: smallDenominator,
            fractionBitCount: 127,
            numeratorMagnitude: smallNumerator,
            quotient: out var margin
        )) {
            return "TryDivideMagnitudeRounded refused at fractionBitCount 127, one bit inside its own documented margin";
        }

        var expectedMargin = Oracles.RoundRationalTiesToEven(
            denominator: ((BigInteger)smallDenominator),
            numerator: (((BigInteger)smallNumerator) << 127)
        );
        ...
        if (!FusedArithmetic.TryDivideMagnitudeRounded(
            denominatorMagnitude: overflowDenominator,
            fractionBitCount: 1,
            numeratorMagnitude: overflowNumerator,
            quotient: out var overflowQuotient
        )) {
        ...
        var expectedOverflow = Oracles.RoundRationalTiesToEven(
            denominator: ((BigInteger)overflowDenominator),
            numerator: (((BigInteger)overflowNumerator) << 1)
        );
    ...
    public static string? InvertLargeMagnitudeEnvelopeRefuses() {
        ...
        if (
            FixedSymmetricSolve.TryInvertSymmetric2(
            a: Huge,
            b: 0L,
            d: Huge,
            invA: out var ia2,
        ...
        var solveOk = FixedSymmetricSolve.TrySolveSymmetric3(
            a: Huge,
            b: 0L,
            c: 0L,
            d: Huge,
    ...
    public static string? LosslessBoundaryIsExact() {
        ...
        if (
            !FixedSymmetricSolve.TrySolveSymmetric2(
            a: A,
            b: B,
            d: D,
            outputFractionShift: OutputFractionShift,
    ...
    public static string? SingularMatricesRefuse() {
        ...
        // 3x3 all zero.
        if (
            FixedSymmetricSolve.TrySolveSymmetric3(
            a: 0L,
            b: 0L,
            c: 0L,
            d: 0L,
