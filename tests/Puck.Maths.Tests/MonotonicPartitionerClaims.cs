...
internal static class MonotonicPartitionerClaims {
    ...
    private static int NextJump(int owner, int remainingRank) {
        ...
        return ((jumpAtBucketCount > MonotonicPartitioner.MaxBucketCount)
            ? 0
            : jumpAtBucketCount
        );
    }
    ...
    public static string? BucketCountOutOfRangeRefusesSurface() {
        Assert.Equal(
            ...
            actual: Assert.Throws<ArgumentOutOfRangeException>(testCode: () => MonotonicPartitioner.GetBucketId(
                bucketCount: 0,
                value: ((ushort)0)
            )).ParamName
        );
        Assert.Equal(
            expected: "bucketCount",
            actual: Assert.Throws<ArgumentOutOfRangeException>(testCode: () => MonotonicPartitioner.GetBucketId(
                bucketCount: (MonotonicPartitioner.MaxBucketCount + 1),
                value: ((ushort)0)
            )).ParamName
        );
        Assert.Equal(
        ...
        Assert.Equal(
            expected: "bucketCount",
            actual: Assert.Throws<ArgumentOutOfRangeException>(testCode: () => MonotonicPartitioner.GetMetrics(
                bucketCount: 0,
                value: ((ushort)0)
            )).ParamName
        );
    ...
    public static string? GuidRoutesThroughTrailingEntropyProtocolSurface() {
        ...
        for (var sample = 0; (sample < 64); ++sample) {
            ...
            foreach (var bucketCount in ((ReadOnlySpan<int>)[1, 64, 353, 1024,])) {
                ...
                Assert.True(
                    condition: (
                        MonotonicPartitioner.GetBucketIdDangerous(
                        bucketCount: bucketCount,
                        value: guid
                    ) ==
                        MonotonicPartitioner.GetBucketIdDangerous(
            ...
            Assert.Equal(
                expected: MonotonicPartitioner.GetMetrics(
                    bucketCount: 353,
                    value: expectedHash
                ),
                actual: MonotonicPartitioner.GetMetrics(
                    bucketCount: 353,
                    value: guid
                )
            );
    ...
    public static string? MetricsMatchReferenceChainWalkSurface() {
        ...
        foreach (var bucketCount in ((ReadOnlySpan<int>)[1, 100, 1024,])) {
            for (var value = 0; (value < MaxValueCount); ++value) {
                var metrics = MonotonicPartitioner.GetMetrics(
                    bucketCount: bucketCount,
                    value: ((ushort)value)
                );
                ...
                var expectedVelocity = ((0 == migrationDistance)
                    ? 0.0f
    ...
    public static string? RoutingIsDeterministicMonotonicAndUniformSurface() {
        ...
        for (var bucketCount = 1; (bucketCount <= MaxBucketCount); ++bucketCount) {
            for (var value = 0; (value < MaxValueCount); ++value) {
                ...
                var routed = MonotonicPartitioner.GetBucketIdDangerous(
                    bucketCount: bucketCount,
                    value: ((ushort)value)
                );
