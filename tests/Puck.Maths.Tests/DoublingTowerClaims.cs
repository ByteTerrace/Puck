...
internal static class DoublingTowerClaims {
    private static BFloor1 BComplex(BigInteger real, BigInteger imaginary) =>
        new(
            Left: new BigIntegerRing(Value: real),
            Right: new BigIntegerRing(Value: imaginary)
        );
    ...
    private static string? CheckFloor1(FixedComplex a, FixedComplex b, int step) {
        ...
        if (!MatchesComplex(
            ...
            expected: (a + b)
        )) { return $"floor1.add mismatch at step {step}"; }
        if (!MatchesComplex(
            actual: Floor1.Subtract(
                left: left,
                right: right
            ),
            expected: (a - b)
        )) { return $"floor1.sub mismatch at step {step}"; }
        if (!MatchesComplex(
            actual: Floor1.Multiply(
                left: left,
                right: right
            ),
            expected: (a * b)
        )) { return $"floor1.mul mismatch at step {step}"; }
        if (!MatchesComplex(
            actual: Floor1.Negate(value: left),
            expected: -a
        )) { return $"floor1.neg mismatch at step {step}"; }
        if (!MatchesComplex(
            actual: Floor1.Conjugate(value: left),
            expected: a.Conjugate()
        )) { return $"floor1.conj mismatch at step {step}"; }

        if (a.TryMagnitudeSquared(squaredMagnitude: out var expectedNorm)) {
    ...
    private static string? CheckFloor2(FixedQuaternion a, FixedQuaternion b, int step) {
        ...
        if (!MatchesQuaternion(
            actual: Floor2.Add(
                left: left,
                right: right
            ),
            expected: (a + b)
        )) { return $"floor2.add mismatch at step {step}"; }
        if (!MatchesQuaternion(
            actual: Floor2.Subtract(
                left: left,
                right: right
            ),
            expected: (a - b)
        )) { return $"floor2.sub mismatch at step {step}"; }
        if (!MatchesQuaternion(
            actual: Floor2.Multiply(
                left: left,
                right: right
            ),
            expected: (a * b)
        )) { return $"floor2.mul mismatch at step {step}"; }
        if (!MatchesQuaternion(
            actual: Floor2.Negate(value: left),
            expected: -a
        )) { return $"floor2.neg mismatch at step {step}"; }
        if (!MatchesQuaternion(
            actual: Floor2.Conjugate(value: left),
            expected: a.Conjugate()
        )) { return $"floor2.conj mismatch at step {step}"; }

        if (a.TryLengthSquared(squaredLength: out var expectedNorm)) {
    ...
    private static string? CheckFloor3Norm(long[] leaves, int step) {
        ...
        if (
            (norm.Left.Right.Value.Value != 0L) ||
            (norm.Right.Left.Value.Value != 0L) ||
            (norm.Right.Right.Value.Value != 0L)
        ) {
    ...
    private static string? CheckQuadraticLinearOps(QuadraticAlgebra<FixedQ4816> algebra, FixedQ4816 leftU, FixedQ4816 leftV, FixedQ4816 rightU, FixedQ4816 rightV, int step) {
        var left = new QuadraticAlgebra<FixedQ4816>.Element(
            U: leftU,
            V: leftV
        );
        var right = new QuadraticAlgebra<FixedQ4816>.Element(
            U: rightU,
            V: rightV
        );
        var sum = algebra.Add(
    ...
    private static long DeterministicBoundedRaw(int step, int lane, long bound) {
        var range = ((2 * bound) + 1);
        var raw = DeterministicFullRangeRaw(
            lane: lane,
            step: step
        );
    ...
    private static FixedQuaternion FromFloor2(Floor2 value) =>
        new(
            X: value.Left.Right.Value,
            Y: value.Right.Left.Value,
            Z: value.Right.Right.Value,
            W: value.Left.Left.Value
        );
    ...
    private static bool MatchesQuaternion(Floor2 actual, FixedQuaternion expected) {
        ...
        return (
            (q.X.Value == expected.X.Value) &&
            (q.Y.Value == expected.Y.Value) &&
            (q.Z.Value == expected.Z.Value) &&
            (q.W.Value == expected.W.Value)
        );
    }
    ...
    private static Floor1 ToFloor1(FixedComplex value) =>
        new(
            Left: new FixedScalarRing(Value: value.Real),
            Right: new FixedScalarRing(Value: value.Imaginary)
        );
    private static Floor2 ToFloor2(FixedQuaternion value) =>
        new(
            Left: new Floor1(
                Left: new FixedScalarRing(Value: value.W),
                Right: new FixedScalarRing(Value: value.X)
            ),
            Right: new Floor1(
                Left: new FixedScalarRing(Value: value.Y),
                Right: new FixedScalarRing(Value: value.Z)
            )
        );
    private static Floor3 ToFloor3(long[] o) =>
        new(
            Left: new Floor2(
                Left: new Floor1(
                    Left: FixedLeaf(raw: o[0]),
                    Right: FixedLeaf(raw: o[1])
                ),
                Right: new Floor1(
                    Left: FixedLeaf(raw: o[2]),
                    Right: FixedLeaf(raw: o[3])
                )
            ),
            Right: new Floor2(
                Left: new Floor1(
                    Left: FixedLeaf(raw: o[4]),
                    Right: FixedLeaf(raw: o[5])
                ),
                Right: new Floor1(
                    Left: FixedLeaf(raw: o[6]),
                    Right: FixedLeaf(raw: o[7])
                )
            )
        );
    ...
    public static string? DoublingFloor1MatchesFixedComplexSurface() {
        for (var step = 0; (step < 3000); ++step) {
            var a = ComplexFromRaw(
                real: DeterministicFullRangeRaw(
                    lane: 0,
                    step: step
                ),
                imaginary: DeterministicFullRangeRaw(
                    lane: 1,
                    step: step
                )
            );
            var b = ComplexFromRaw(
                real: DeterministicFullRangeRaw(
                    lane: 2,
                    step: step
                ),
    ...
    public static string? DoublingFloor2CommutatorWitnessSurface() {
        ...
        var commutator = BFloor2Components(value: BFloor2.Commutator(
            left: BQuaternion(c: i),
            right: BQuaternion(c: j)
        ));
    ...
    public static string? DoublingFloor2MatchesFixedQuaternionSurface() {
        for (var step = 0; (step < 3000); ++step) {
            var a = QuaternionFromRaw(
                w: DeterministicFullRangeRaw(
                    lane: 0,
                    step: step
                ),
                x: DeterministicFullRangeRaw(
                    lane: 1,
                    step: step
                ),
                y: DeterministicFullRangeRaw(
                    lane: 2,
                    step: step
                ),
                z: DeterministicFullRangeRaw(
                    lane: 3,
                    step: step
                )
            );
        ...
        for (var step = 0; (step < 800); ++step) {
            ...
            var product = BFloor2Components(value: BFloor2.Multiply(
                left: BQuaternion(c: left),
                right: BQuaternion(c: right)
            ));
            var reference = HamiltonReference(
                l: left,
                r: right
            );
    ...
    public static string? QuadraticTwinLinearOpsFullRangeSurface() {
        ...
        for (var step = 0; (step < 2000); ++step) {
            var error = CheckQuadraticLinearOps(
                algebra: algebra,
                leftU: FixedQ4816.FromRawBits(value: DeterministicFullRangeRaw(
                    lane: 0,
                    step: step
                )),
                leftV: FixedQ4816.FromRawBits(value: DeterministicFullRangeRaw(
                    lane: 1,
                    step: step
                )),
                rightU: FixedQ4816.FromRawBits(value: DeterministicFullRangeRaw(
                    lane: 2,
                    step: step
                )),
                rightV: FixedQ4816.FromRawBits(value: DeterministicFullRangeRaw(
                    lane: 3,
                    step: step
                )),
                step: step
    ...
    public static string? RealQuadraticTwinLaneSurface() {
        foreach (var k in new long[] { 1L, 2L, 3L, 5L }) {
            ...
            var alg = QuadraticAlgebra<BigInteger>.Create(
                p: kBig,
                q: BigInteger.One
            );
            ...
            RealQuadratic ToSurd(QuadraticAlgebra<BigInteger>.Element e) =>
                RealQuadratic.Create(
                    rationalNumerator: ((2 * e.U) + (e.V * kBig)),
                    surdNumerator: e.V,
                    radicand: radicand,
                    denominator: 2
                );
            ...
            for (var step = 0; (step < 300); ++step) {
                var eA = new QuadraticAlgebra<BigInteger>.Element(
                    U: DeterministicSmallInteger(
                        bound: 1000,
                        lane: 0,
                        step: step
                    ),
                    V: DeterministicSmallInteger(
                        bound: 1000,
                        lane: 1,
                        step: step
                    )
                );
                var eB = new QuadraticAlgebra<BigInteger>.Element(
                    U: DeterministicSmallInteger(
                        bound: 1000,
                        lane: 2,
                        step: step
                ...
                var sA = ToSurd(e: eA);
                var sB = ToSurd(e: eB);
                var conjA = RealQuadratic.Create(
                    rationalNumerator: sA.RationalNumerator,
                    surdNumerator: -sA.SurdNumerator,
                    radicand: sA.Radicand,
                    denominator: sA.Denominator
                );

                if (ToSurd(e: alg.Add(
                    left: eA,
                    right: eB
                )) != (sA + sB)) { return $"k={k}: Add disagrees with RealQuadratic at step {step}"; }
                if (ToSurd(e: alg.Subtract(
                    left: eA,
                    right: eB
                )) != (sA - sB)) { return $"k={k}: Subtract disagrees with RealQuadratic at step {step}"; }
                if (ToSurd(e: alg.Multiply(
                    left: eA,
                    right: eB
                )) != (sA * sB)) { return $"k={k}: Multiply disagrees with RealQuadratic at step {step}"; }
