...
internal static class DoublingTowerClaims {
    ...
    private static BigInteger DeterministicSmallInteger(int step, int lane, int bound) {
        var range = ((2 * bound) + 1);
        var raw = DeterministicFullRangeRaw(lane: lane, step: step);
    ...
    public static string? QuadraticTwinLinearOpsFullRangeSurface() {
        ...
        for (var step = 0; (step < 2000); ++step) {
            var error = CheckQuadraticLinearOps(
                algebra: algebra,
                leftU: FixedQ4816.FromRawBits(value: DeterministicFullRangeRaw(lane: 0, step: step)),
                leftV: FixedQ4816.FromRawBits(value: DeterministicFullRangeRaw(lane: 1, step: step)),
                rightU: FixedQ4816.FromRawBits(value: DeterministicFullRangeRaw(lane: 2, step: step)),
                rightV: FixedQ4816.FromRawBits(value: DeterministicFullRangeRaw(lane: 3, step: step)),
                step: step
            );
    ...
    public static string? DoublingFloor1MatchesFixedComplexSurface() {
        for (var step = 0; (step < 3000); ++step) {
            var a = ComplexFromRaw(real: DeterministicFullRangeRaw(lane: 0, step: step), imaginary: DeterministicFullRangeRaw(lane: 1, step: step));
            var b = ComplexFromRaw(real: DeterministicFullRangeRaw(lane: 2, step: step), imaginary: DeterministicFullRangeRaw(lane: 3, step: step));
            ...
            foreach (var leftImaginary in FullRangeEdgeRaws) {
                foreach (var rightReal in FullRangeEdgeRaws) {
                    foreach (var rightImaginary in FullRangeEdgeRaws) {
                        var a = ComplexFromRaw(imaginary: leftImaginary, real: leftReal);
                        var b = ComplexFromRaw(imaginary: rightImaginary, real: rightReal);
    ...
    public static string? DoublingFloor2MatchesFixedQuaternionSurface() {
        for (var step = 0; (step < 3000); ++step) {
            var a = QuaternionFromRaw(
                w: DeterministicFullRangeRaw(lane: 0, step: step), x: DeterministicFullRangeRaw(lane: 1, step: step),
                y: DeterministicFullRangeRaw(lane: 2, step: step), z: DeterministicFullRangeRaw(lane: 3, step: step));
            var b = QuaternionFromRaw(
                w: DeterministicFullRangeRaw(lane: 4, step: step), x: DeterministicFullRangeRaw(lane: 5, step: step),
                y: DeterministicFullRangeRaw(lane: 6, step: step), z: DeterministicFullRangeRaw(lane: 7, step: step));
        ...
        for (var step = 0; (step < 800); ++step) {
            BigInteger[] left = [
                DeterministicSmallInteger(bound: 1000, lane: 0, step: step), DeterministicSmallInteger(bound: 1000, lane: 1, step: step),
                DeterministicSmallInteger(bound: 1000, lane: 2, step: step), DeterministicSmallInteger(bound: 1000, lane: 3, step: step),
            ];
            BigInteger[] right = [
                DeterministicSmallInteger(bound: 1000, lane: 4, step: step), DeterministicSmallInteger(bound: 1000, lane: 5, step: step),
                DeterministicSmallInteger(bound: 1000, lane: 6, step: step), DeterministicSmallInteger(bound: 1000, lane: 7, step: step),
            ];
    ...
    public static string? DoublingFloor3OctonionNormVsOracleSurface() {
        for (var step = 0; (step < 1500); ++step) {
            ...
            for (var lane = 0; (lane < 8); ++lane) { leaves[lane] = DeterministicFullRangeRaw(lane: lane, step: step); }
        ...
        for (var step = 0; (step < 1500); ++step) {
            ...
            for (var lane = 0; (lane < 8); ++lane) { leaves[lane] = DeterministicBoundedRaw(bound: NarrowBound, lane: (100 + lane), step: step); }
