...
internal static class GeometryClaims {
    ...
    private static Oracles.Enclosure DivideEnclosureByRawFactor(Oracles.Enclosure enclosure, long divisorRaw) {
        ...
        return new(
            Low: FloorDivide(
                numerator: (enclosure.Low << FixedQ4816.FractionBitCount),
                positiveDenominator: divisor
            ),
            High: CeilingDivide(
                numerator: (enclosure.High << FixedQ4816.FractionBitCount),
                positiveDenominator: divisor
            )
        );
    }
    ...
    private static BigInteger MaxAbsCross3(long ix, long iy, long iz, long tx, long ty, long tz) {
        ...
        return BigInteger.Max(
            ...
            right: BigInteger.Max(
                left: BigInteger.Abs(value: ((biz * btx) - (bix * btz))),
                right: BigInteger.Abs(value: ((bix * bty) - (biy * btx)))
            )
        );
    }
    ...
    private static long[] NormalizeOracle(long[] values) {
        ...
        for (var i = 0; (i < values.Length); ++i) {
            ...
            if (
                (doubledNumeratorSquared > midpointSquared) ||
                ((doubledNumeratorSquared == midpointSquared) && ((low & 1L) != 0L))
            ) { ++low; }

            result[i] = ((values[i] < 0L)
                ? -low
                : low
            );
        }
    ...
    private static long RoundProductSumOracle(BigInteger sum) {
        ...
        var truncated = BigInteger.DivRem(
            dividend: magnitude,
            divisor: (BigInteger.One << FixedQ4816.FractionBitCount),
            remainder: out var remainder
        );
        ...
        if (
            (remainder > half) ||
            ((remainder == half) && !truncated.IsEven)
        ) { ++truncated; }
    ...
    private static long RoundRatioQ16(BigInteger numerator, BigInteger denominator) {
        ...
        var quotient = BigInteger.DivRem(
            dividend: (BigInteger.Abs(value: numerator) << FixedQ4816.FractionBitCount),
            divisor: denominator,
            remainder: out var remainder
        );
        ...
        if (
            ...
            ((remainder == distanceToNext) && !quotient.IsEven)
        ) { ++quotient; }
    ...
    private static Oracles.Enclosure ScaleByRawFactor(Oracles.Enclosure enclosure, long multiplierRaw) {
        ...
        return new(
            High: -((-hiProduct) >> FixedQ4816.FractionBitCount),
            Low: (loProduct >> FixedQ4816.FractionBitCount)
        );
    }
    ...
    private static FixedVector3 Vector3Of(long[] lanes) => new(
        X: Raw(value: lanes[0]),
        Y: Raw(value: lanes[1]),
        Z: Raw(value: lanes[2])
    );
            ...
            foreach (var ai in ComplexEdges10) {
                foreach (var br in ComplexEdges10) {
                    foreach (var bi in ComplexEdges10) {
                        ...
                        if (
                            (product.Real.Value != expectedProductReal) ||
                            (product.Imaginary.Value != expectedProductImaginary)
                        ) {
                        ...
                        var expectedQuotientReal = RoundRatioQ16(
                            denominator: denominator,
                            numerator: ((((BigInteger)ar) * br) + (((BigInteger)ai) * bi))
                        );
                        var expectedQuotientImaginary = RoundRatioQ16(
                            denominator: denominator,
                        ...
                        if (
                            (quotient.Real.Value != expectedQuotientReal) ||
                            (quotient.Imaginary.Value != expectedQuotientImaginary)
                        ) {
    ...
    public static string? ComplexFromToAndScaleSafetySurface() {
            ...
            foreach (var fy in edges) {
                foreach (var tx in edges) {
                    foreach (var ty in edges) {
                        ...
                        if (
                            (from == FixedVector2.Zero) ||
                            (to == FixedVector2.Zero)
                        ) { continue; }
                        ...
                        var fromDirection = new FixedVector2(
                            X: (from.X / fromLength),
                            Y: (from.Y / fromLength)
                        );
                        var toDirection = new FixedVector2(
                            X: (to.X / toLength),
                            Y: (to.Y / toLength)
                        );
        ...
        var epsilonAxis = new FixedVector2(
        ...
        if (new FixedComplex(
            Real: FixedQ4816.Epsilon,
            Imaginary: FixedQ4816.Zero
        ).Normalize() != FixedComplex.MultiplicativeIdentity) {
            return "an epsilon-component complex did not normalize to the multiplicative identity";
        }

        var extreme = new FixedComplex(
    ...
    public static string? DualDerivativeSurface() {
        ...
        foreach (var xRaw in DualChainRuleXRaws) {
            ...
            var f = ((FixedDual.Sqrt(value: x) * sinD) + FixedDual.Divide(
                left: (x * x),
                right: (x + FixedDual.Constant(value: FixedQ4816.One))
            ));
            ...
            var term1Low = FloorDivide(
                numerator: (enclosure.Sin.Low << FixedQ4816.FractionBitCount),
                positiveDenominator: doubledRoot
            );
            var term1High = CeilingDivide(
                numerator: (enclosure.Sin.High << FixedQ4816.FractionBitCount),
                positiveDenominator: doubledRoot
            );

            // term2 = idealRoot*cos(x): the cosine enclosure scaled by the exact root.
            var term2 = ScaleByRawFactor(
                enclosure: enclosure.Cos,
                multiplierRaw: ((long)idealRoot)
            );
            ...
            var term3Low = FloorDivide(
                numerator: numeratorScaled,
                positiveDenominator: denominatorSquared
            );
            var term3High = CeilingDivide(
                numerator: numeratorScaled,
                positiveDenominator: denominatorSquared
            );
            ...
            var tolerance = ((DivideToleranceByRawFactor(
                divisorRawMagnitude: ((long)doubledRoot),
                toleranceUnits: kernelUlp
            )
                + ScaleToleranceByRawFactor(
                factorRawMagnitude: ((long)idealRoot),
                toleranceUnits: kernelUlp
            ))
                + GuardUlpUnits(
                denominator: 1,
    ...
    public static string? NormalizeFullWidthOracleSurface() {
        ...
        var allMinimum = new FixedQuaternion(
        ...
        if (
            (normalizedAllMinimum.X != expectedAllMinimum) ||
            (normalizedAllMinimum.Y != expectedAllMinimum) ||
            (normalizedAllMinimum.Z != expectedAllMinimum) ||
            (normalizedAllMinimum.W != expectedAllMinimum)
        ) {
    ...
    public static string? QuaternionAlgebraicSanitySurface() {
        foreach (var (axis, angleRaw) in AxisAngleLadder) {
            var q = FixedQuaternion.FromAxisAngle(
                axis: Vector3Of(lanes: axis),
            ...
            var lengthProbe = q.Rotate(vector: new FixedVector3(
                X: FixedQ4816.FromInteger(value: 3L),
                Y: FixedQ4816.FromInteger(value: 4L),
                Z: FixedQ4816.Zero
            ));
        ...
        if (FixedQuaternion.FromTo(
            from: new FixedVector3(
                X: FixedQ4816.One,
                Y: FixedQ4816.Zero,
                Z: FixedQ4816.Zero
            ),
            to: FixedVector3.Zero
        ) != FixedQuaternion.Identity) {
    ...
    public static string? QuaternionExpLogSurface() {
        ...
        foreach (var bivector in ExpLadder) {
            ...
            if (Oracles.WithinEnvelope(
                ...
                enclosure: enclosure.Cos,
                toleranceUnits: kernelUlp
            ) is { } wDetail) { return wDetail; }

            var lanes = new[] { value.X.Value, value.Y.Value, value.Z.Value };

            for (var lane = 0; (lane < 3); ++lane) {
                var laneTolerance = (ScaleToleranceByRawFactor(
                    toleranceUnits: kernelUlp,
                    factorRawMagnitude: idealAxis[lane]
                ) + GuardUlpUnits(
                    denominator: 1,
                    numerator: 1
                ));
                var expected = ScaleByRawFactor(
                ...
                if (Oracles.WithinEnvelope(
                    ...
                    enclosure: expected,
                    toleranceUnits: laneTolerance
                ) is { } laneDetail) { return laneDetail; }
            }
        }
        ...
        foreach (var quaternion in LogLadder) {
            ...
            var thetaEnclosure = Oracles.EncloseAtan2(
                guardBitCount: Oracles.GuardBitCount,
                xRaw: qw,
            ...
            for (var lane = 0; (lane < 3); ++lane) {
                var laneTolerance = ((amplification * (kernelUlp + GuardUlpUnits(
                    denominator: 2,
                    numerator: 1
                ))) + GuardUlpUnits(
                    denominator: 2,
        ...
        if (
            ...
            (Math.Abs(value: (enormousLength.Value - FixedQ4816.One.Value)) > 8L)
        ) {
        ...
        foreach (var from in Vector3EdgeTriples) {
            foreach (var to in Vector3EdgeTriples) {
                ...
                var cross = MaxAbsCross3(
                    ...
                    iy: image.Y.Value,
                    iz: image.Z.Value,
                    tx: toDirection.X.Value,
                    ty: toDirection.Y.Value,
                    tz: toDirection.Z.Value
                );
    ...
    public static string? QuaternionHamiltonProductDotInverseSurface() {
        foreach (var left in QuaternionEdgeQuads) {
            foreach (var right in QuaternionEdgeQuads) {
                ...
                if (
                    (product.X.Value != expectedProduct[0]) ||
                    (product.Y.Value != expectedProduct[1]) ||
                    (product.Z.Value != expectedProduct[2]) ||
                    (product.W.Value != expectedProduct[3])
                ) {
                ...
                if (FixedQuaternion.Dot(
                    ...
                    return $"left=[{left[0]},{left[1]},{left[2]},{left[3]}] right=[{right[0]},{right[1]},{right[2]},{right[3]}]: Dot is {FixedQuaternion.Dot(
                        left: l,
                        right: r
            ...
            var expectedInverse = new long[] {
                ...
                denominator: squaredNorm
            ),
                RoundRatioQ16(
                numerator: -(((BigInteger)left[1]) << FixedQ4816.FractionBitCount),
                denominator: squaredNorm
            ),
                RoundRatioQ16(
                numerator: -(((BigInteger)left[2]) << FixedQ4816.FractionBitCount),
                denominator: squaredNorm
            ),
                RoundRatioQ16(
                numerator: (((BigInteger)left[3]) << FixedQ4816.FractionBitCount),
                denominator: squaredNorm
            ),
            };
            ...
            if (
                (inverse.X.Value != expectedInverse[0]) ||
                (inverse.Y.Value != expectedInverse[1]) ||
                (inverse.Z.Value != expectedInverse[2]) ||
                (inverse.W.Value != expectedInverse[3])
            ) {
    ...
    public static string? QuaternionRotateScheduleTranscriptionSurface() {
        ...
        foreach (var left in QuaternionEdgeQuads) {
            ...
            foreach (var right in Vector3ModerateEdgeTriples) {
                ...
                var sandwich = HamiltonProductOracle(
                    left: HamiltonProductOracle(
                        left: q,
                        right: vectorAsQuaternion
                    ),
                    right: conjugateQ
                );
    ...
    public static string? QuaternionSlerpSurface() {
        ...
        if (FixedQuaternion.Dot(
            ...
            right: quarter
        ).Value > NlerpThreshold) { return "the quarter-turn pair does not reach the sine-ratio branch"; }
        if (FixedQuaternion.Dot(
            left: FixedQuaternion.Identity,
            right: nearby
        ).Value <= NlerpThreshold) { return "the nearly-parallel pair does not reach the normalized linear blend branch"; }
        ...
        var thetaEnclosure = WidenEnclosure(
        ...
        var normalizeSlop = GuardUlpUnits(
            denominator: 1,
            numerator: 2
        );
        ...
        foreach (var amountRaw in SlerpLadderAmounts) {
            ...
            var angleArgEnclosure = WidenEnclosure(
                enclosure: ScaleByRawFactor(
                    enclosure: thetaEnclosure,
                    multiplierRaw: amountRaw
                ),
                toleranceUnits: halfUlp
            );
            ...
            var representativeGap = ((angleArgEnclosure.High - angleArgEnclosure.Low) + GuardUlpUnits(
                denominator: 1,
                numerator: 1
            ));

            var (sinBase, cosBase) = Oracles.EncloseSinCos(
                guardBitCount: Oracles.GuardBitCount,
                raw: representativeRaw
            );
            var sinEnclosure = WidenEnclosure(
                enclosure: sinBase,
                toleranceUnits: (kernelUlp + representativeGap)
            );
            var cosEnclosure = WidenEnclosure(
                enclosure: cosBase,
                toleranceUnits: (kernelUlp + representativeGap)
            );
            ...
            var toWeightEnclosure = WidenEnclosure(
                enclosure: DivideEnclosureByRawFactor(
                    divisorRaw: sinThetaRaw,
                    enclosure: sinEnclosure
                ),
                toleranceUnits: halfUlp
            );
            ...
            var fromWeightEnclosure = WidenEnclosure(
                enclosure: SubtractEnclosures(
                    left: cosEnclosure,
                    right: ScaleByRawFactor(
                        enclosure: toWeightEnclosure,
                        multiplierRaw: dotRaw
                    )
                ),
                toleranceUnits: halfUlp
            );
            ...
            var zPreEnclosure = WidenEnclosure(
                enclosure: ScaleByRawFactor(
                    enclosure: toWeightEnclosure,
                    multiplierRaw: 46341L
                ),
                toleranceUnits: halfUlp
            );
            ...
            var wPreEnclosure = WidenEnclosure(
                enclosure: AddEnclosures(
                    left: fromWeightEnclosure,
                    right: ScaleByRawFactor(
                        enclosure: toWeightEnclosure,
                        multiplierRaw: 46341L
                    )
            ...
            var crossEnclosure = SubtractEnclosures(
                left: ScaleByRawFactor(
                    enclosure: wPreEnclosure,
                    multiplierRaw: value.Z.Value
                ),
                right: ScaleByRawFactor(
                    enclosure: zPreEnclosure,
                    multiplierRaw: value.W.Value
                )
            );

            if (
                ((crossEnclosure.Low - normalizeSlop) > BigInteger.Zero) ||
                ((crossEnclosure.High + normalizeSlop) < BigInteger.Zero)
            ) {
        ...
        var atZero = FixedQuaternion.Slerp(
            from: FixedQuaternion.Identity,
            to: quarter,
    ...
    public static string? RigidTransformRoundTripSurface() {
        ...
        for (var i = 0; (i < RigidLadderRotations.Length); ++i) {
            ...
            transforms[i] = FixedRigidTransform.FromRotationTranslation(
                rotation: rotation,
            ...
            var expBack = FixedRigidTransform.Exp(
                dual: screwDual,
                real: screwReal
            );
            var realDiff = Math.Max(
                val1: Math.Max(
                    val1: Math.Abs(value: (expBack.Value.Real.X.Value - transforms[i].Value.Real.X.Value)),
                    val2: Math.Abs(value: (expBack.Value.Real.Y.Value - transforms[i].Value.Real.Y.Value))
                ),
                val2: Math.Max(
                    val1: Math.Abs(value: (expBack.Value.Real.Z.Value - transforms[i].Value.Real.Z.Value)),
                    val2: Math.Abs(value: (expBack.Value.Real.W.Value - transforms[i].Value.Real.W.Value))
                )
            );
            var dualDiff = Math.Max(
                val1: Math.Max(
                    val1: Math.Abs(value: (expBack.Value.Dual.X.Value - transforms[i].Value.Dual.X.Value)),
                    val2: Math.Abs(value: (expBack.Value.Dual.Y.Value - transforms[i].Value.Dual.Y.Value))
                ),
                val2: Math.Max(
                    val1: Math.Abs(value: (expBack.Value.Dual.Z.Value - transforms[i].Value.Dual.Z.Value)),
                    val2: Math.Abs(value: (expBack.Value.Dual.W.Value - transforms[i].Value.Dual.W.Value))
                )
            );
        ...
        for (var i = 0; (i < transforms.Length); ++i) {
            for (var j = 0; (j < transforms.Length); ++j) {
                ...
                var diff = Math.Max(
                    val1: Math.Abs(value: (composed.X.Value - sequential.X.Value)),
                    val2: Math.Max(
                        val1: Math.Abs(value: (composed.Y.Value - sequential.Y.Value)),
                        val2: Math.Abs(value: (composed.Z.Value - sequential.Z.Value))
                    )
                );
        ...
        if (FixedRigidTransform.Exp(
            dual: pureDual,
            real: pureReal
        ) != pureTranslation) { return "pure translation did not round-trip through Exp(Log(...)) bit-for-bit"; }

        // ScLerp endpoints return the operands.
        var atZero = FixedRigidTransform.ScLerp(
            from: transforms[0],
            to: transforms[1],
            amount: FixedQ4816.Zero
        ).TransformPoint(point: probe);
        var atOne = FixedRigidTransform.ScLerp(
            from: transforms[0],
            to: transforms[1],
    ...
    public static string? Vector2FullWidthOracleAndIdentitiesSurface() {
        var tinyVector = new FixedVector2(
            X: FixedQ4816.Epsilon,
        ...
        var fullRangeVector = new FixedVector2(
            X: fullRangeComponent,
            Y: FixedQ4816.Zero
        );

        if (
            !fullRangeVector.TryLength(length: out var fullRangeLength) ||
            (fullRangeLength != fullRangeComponent)
        ) { return "a full-range single-axis vector did not report its exact axis-aligned length"; }
            ...
            foreach (var ay in LongEdges9) {
                foreach (var bx in LongEdges9) {
                    foreach (var by in LongEdges9) {
                        var a = new FixedVector2(
                            X: Raw(value: ax),
                            Y: Raw(value: ay)
                        );
                        var b = new FixedVector2(
                            X: Raw(value: bx),
                            Y: Raw(value: by)
                        );
                        ...
                        if (FixedVector2.Wedge(
                            left: a,
                            right: a
                        ).Value != 0L) { return $"Wedge(a,a) at ({ax},{ay}) is not zero"; }
                        if (FixedVector2.Wedge(
                            left: a,
                            right: b
                        ).Value != -FixedVector2.Wedge(
                            left: b,
                            right: a
                        ).Value) { return $"Wedge is not antisymmetric at ({ax},{ay}),({bx},{by})"; }
                        if (FixedVector2.Dot(
                            left: a,
                            right: b
                            ...
                            left: b,
                            right: a
                        ).Value) { return $"Dot is not symmetric at ({ax},{ay}),({bx},{by})"; }
                    }
                }
            }
        }
    ...
    public static string? Vector3DotCrossOracleSurface() {
        foreach (var a in Vector3EdgeTriples) {
            foreach (var b in Vector3EdgeTriples) {
                ...
                if (cross != -FixedVector3.Cross(
                    left: bv,
                    right: av
                )) { return $"a=[{a[0]},{a[1]},{a[2]}] b=[{b[0]},{b[1]},{b[2]}]: Cross is not antisymmetric"; }
            }
        }

        var lengthAtMax = new FixedVector3(
            X: FixedQ4816.MaxValue,
            Y: FixedQ4816.Zero,
            Z: FixedQ4816.Zero
        );
        var lengthAboveMax = new FixedVector3(
            X: FixedQ4816.MaxValue,
            Y: FixedQ4816.FromRawBits(value: (1L << 32)),
            Z: FixedQ4816.Zero
        );
        var squaredLengthFits = new FixedVector3(
            X: FixedQ4816.FromRawBits(value: (1L << 39)),
            Y: FixedQ4816.Zero,
            Z: FixedQ4816.Zero
        );
        var squaredLengthOverflows = new FixedVector3(
            X: FixedQ4816.FromRawBits(value: (1L << 40)),
            Y: FixedQ4816.Zero,
            Z: FixedQ4816.Zero
        );

        if (
            !lengthAtMax.TryLength(length: out var exactMaxLength) ||
            (exactMaxLength != FixedQ4816.MaxValue)
        ) { return "an axis-aligned vector at exactly MaxValue did not report its exact length"; }
        if (
            lengthAboveMax.TryLength(length: out _) ||
            (lengthAboveMax.Length != FixedQ4816.MaxValue)
        ) { return "a vector whose length exceeds MaxValue did not refuse TryLength and saturate Length"; }
        ...
        if (
            squaredLengthOverflows.TryLengthSquared(squaredLength: out _) ||
            (squaredLengthOverflows.LengthSquared != FixedQ4816.MaxValue)
        ) { return "a vector whose squared length overflows did not refuse TryLengthSquared and saturate LengthSquared"; }
