...
internal static class QuasicrystalClaims {
    ...
    private static string? FormRootMappedIntoFundamentalDomain(long a, long b, long c, ModularTransform transform) {
        ...
        var (realLow, realHigh) = BracketRawQ16(
            rationalNumerator: -b,
            surdNumerator: 0,
            radicand: BigInteger.One,
            denominator: (2 * a)
        );
        var (imaginaryLow, imaginaryHigh) = BracketRawQ16(
            denominator: (2 * a),
            radicand: (((4 * ((BigInteger)a)) * c) - (((BigInteger)b) * b)),
            rationalNumerator: 0,
            surdNumerator: 1
        );
    ...
    private static string? GaussDefinitenessDecision(long a, long b, long c, ref int admitted, ref int refused) {
        ...
        try {
            reduction = ModularTransform.GaussReduce(
                a: a,
                b: b,
                c: c
            );
            ...
            return (definite
                ? $"a positive-definite form was refused at ({a},{b},{c}): exact discriminant {discriminant}"
                : null
            );
            ...
            return (definite
                ? null
                : $"an indefinite form was admitted at ({a},{b},{c}): exact discriminant {discriminant}"
            );
        }
    ...
    private static bool WithinBracket(
        ...
        return (
            (actualRaw >= (lower - toleranceRawTicks)) &&
            (actualRaw <= (upper + toleranceRawTicks))
        );
    }
    ...
    public static string? ChainSingleTermMatchesMetallicAndNewPeriodsWalk() {
        for (var n = 1; (n <= 6); ++n) {
            ...
            for (var step = 0; (step < chainWord.Length); ++step) {
                chainWord[step] = chain.StartsLongTile(
                    a: chainPoint.A,
                    b: chainPoint.B
                );
                chainPoint = chain.Next(
                    a: chainPoint.A,
                    b: chainPoint.B
                );
                metallicWord[step] = MetallicQuasicrystal.StartsLongTile(
                    a: metallicPoint.A,
                    b: metallicPoint.B,
                    n: n
                );
                metallicPoint = MetallicQuasicrystal.Next(
                    a: metallicPoint.A,
                    b: metallicPoint.B,
                    n: n
                );
        ...
        foreach (var testCase in newPeriods) {
            ...
            if (!chain.Contains(
                a: 0L,
                b: 0L
            )) { return $"the chain origin is not a member at d={testCase.D}"; }
            ...
            for (var step = 0; (step < walkWord.Length); ++step) {
                ...
                if (!chain.Contains(
                    a: next.A,
                    b: next.B
                )) { return $"the chain walk left the acceptance window at d={testCase.D} step={step}"; }
                if (chain.Previous(
                    a: next.A,
                    b: next.B
                ) != point) { return $"Previous does not invert Next at d={testCase.D} step={step}"; }
                if (isLong
                    ? (((next.A - point.A) != 1L) || ((next.B - point.B) != 0L))
                    : (((next.A - point.A) != 0L) || ((next.B - point.B) != 1L))) {
            ...
            if (
                (point.A <= Box) ||
                (point.B <= Box)
            ) { return $"the chain walk did not cover the coordinate box at d={testCase.D}"; }
            ...
            if (
                (chain.Inflation != inflation) ||
                (chain.InflationFactor != inflation.InflationFactor())
            ) {
    ...
    public static string? ContinuedFractionPeriodsAndFullWidthRegressions() {
        ...
        foreach (var testCase in cases) {
            ...
            for (var repeat = 0; (repeat < 24); ++repeat) {
                var term = ((repeat < start)
                    ? terms[repeat]
                    : terms[(start + ((repeat - start) % length))]
                );
        ...
        var smallWritten = ContinuedFraction.Expand(
            d: 3L,
            p: 0L,
            periodLength: out var smallLength,
            periodStart: out var smallStart,
            q: 1L,
            r: 1L,
            terms: smallEquivalent
        );
        var wideWritten = ContinuedFraction.Expand(
            d: 3L,
            p: 0L,
            periodLength: out var wideLength,
            periodStart: out var wideStart,
            q: long.MaxValue,
            r: long.MaxValue,
            terms: wideEquivalent
        );

        if (
            (smallWritten != wideWritten) ||
            (smallStart != wideStart) ||
            (smallLength != wideLength) ||
            !smallEquivalent.AsSpan(
            length: smallWritten,
            start: 0
        ).SequenceEqual(other: wideEquivalent.AsSpan(
            length: wideWritten,
            start: 0
        ))
        ) {
        ...
        smallWritten = ContinuedFraction.Expand(
            d: 3L,
            p: 0L,
            periodLength: out smallLength,
            periodStart: out smallStart,
            q: 5L,
            r: 6L,
            terms: smallEquivalent
        );
        wideWritten = ContinuedFraction.Expand(
            d: 3L,
            p: 0L,
            periodLength: out wideLength,
            periodStart: out wideStart,
            q: (5L * Scale),
            r: (6L * Scale),
            terms: wideEquivalent
        );
    ...
    public static string? GaussReductionEntersFundamentalDomain() {
        ...
        for (var a = 1L; (a <= 24L); ++a) {
            for (var b = -24L; (b <= 24L); ++b) {
                for (var c = 1L; (c <= 24L); ++c) {
                    ...
                    var reduction = ModularTransform.GaussReduce(
                        a: a,
                        b: b,
                        c: c
                    );
                    ...
                    if (ModularFormAction(
                        a: a,
                        b: b,
                        c: c,
                        g: reduction.Transform.Inverse
                    ...
                    if (ModularTransform.GaussReduce(
                        a: reduction.A,
                        b: reduction.B,
                        c: reduction.C
                    ).Transform != ModularTransform.Identity) {
                        return $"the reduction is not idempotent at ({a},{b},{c})";
                    }
                    if (FormRootMappedIntoFundamentalDomain(
                        a: a,
                        b: b,
                        c: c,
        ...
        return ((reductions > 0)
            ? null
            : "no positive-definite form was reduced -- the sweep is vacuous"
        );
    }
    ...
    public static string? GeneralQuasicrystalIsSturmianAndTileLengthConsistent() {
        var nonCanonicalSilver = RealQuadratic.Create(
            denominator: 2,
            radicand: 8,
            rationalNumerator: 2,
            surdNumerator: 1
        );
        var canonicalSilver = RealQuadratic.Create(
            denominator: 1,
            radicand: 2,
            rationalNumerator: 1,
            surdNumerator: 1
        ...
        if (
            (nonCanonicalSilver != canonicalSilver) ||
        ...
        foreach (var testCase in cases) {
            ...
            var index = QuadraticQuasicrystal.Compile(
                d: testCase.D,
                p: testCase.P,
                q: testCase.Q,
                r: testCase.R
            ...
            for (var position = 0; (position < 2048); ++position) {
                if (
                    (index.TileAt(index: position) != word[position]) ||
                    (index.CountLongTiles(exclusiveEnd: position) != streamedLongCount)
                ) {
            ...
            if (remoteAdvance != (index.TileAt(index: remoteIndex)
                ? BigInteger.One
                : BigInteger.Zero)) {
            ...
            for (var k = 1; (k <= 24); ++k) {
                if (WordComplexity(
                    k: k,
                    word: word
                ) != (k + 1)) { return $"the word is not Sturmian at d={testCase.D} k={k}"; }
            }
            ...
            var lambda = RealQuadratic.Create(
                denominator: 2,
                radicand: ((trace * trace) - (4 * determinant)),
                rationalNumerator: trace,
                surdNumerator: 1
            );
            ...
            var actualLength = QuadraticQuasicrystal.LongTileLength(
                d: testCase.D,
                p: testCase.P,
                q: testCase.Q,
                r: testCase.R
            ...
            if (!WithinBracket(
                actual: actualLength,
                rationalNumerator: exactTileLength.RationalNumerator,
                surdNumerator: exactTileLength.SurdNumerator,
                radicand: exactTileLength.Radicand,
                denominator: exactTileLength.Denominator,
                toleranceRawTicks: 32L
            )) {
        ...
        for (var i = 0; (i < goldenReference.Length); ++i) {
            goldenReference[i] = MetallicQuasicrystal.StartsLongTile(
                a: goldenWalk.A,
                b: goldenWalk.B,
                n: 1
            );
            goldenWalk = MetallicQuasicrystal.Next(
                a: goldenWalk.A,
                b: goldenWalk.B,
                n: 1
            );
        ...
        if (WordComplexity(
            k: 10,
            word: periodicProbe
        ) == 11) { return "the WordComplexity oracle has no teeth"; }
    ...
    public static string? MetallicRandomAccessMatchesStreamedWord() {
        for (var n = 1; (n <= 6); ++n) {
            if (!MetallicQuasicrystal.Contains(
                a: 0L,
                b: 0L,
                n: n
            )) { return $"the metallic origin is not a member at n={n}"; }
            ...
            for (var step = 0; (step < walkWord.Length); ++step) {
                ...
                var isLong = MetallicQuasicrystal.StartsLongTile(
                    a: point.A,
                    b: point.B,
                    n: n
                );
                var next = MetallicQuasicrystal.Next(
                    a: point.A,
                    b: point.B,
                    n: n
                );
                ...
                if (!MetallicQuasicrystal.Contains(
                    ...
                    b: next.B,
                    n: n
                )) { return $"the metallic walk left the set at n={n} step={step}"; }
                if (MetallicQuasicrystal.Previous(
                    a: next.A,
                    b: next.B,
                    n: n
                ) != point) { return $"Previous does not invert Next at n={n} step={step}"; }
                ...
                if (MetallicQuasicrystal.Position(
                    a: next.A,
                    b: next.B,
                    n: n
                ) <= MetallicQuasicrystal.Position(
                    a: point.A,
                    b: point.B,
                    n: n
                )) {
                ...
                if (isLong) {
                    ...
                    if (
                        (step > 0) &&
                        !previousWasLong
                    ) { return $"the forbidden factor SS appears at n={n} step={step}"; }
            ...
            if (
                (point.A <= 70L) ||
                (point.B <= 70L)
            ) { return $"the metallic walk did not cover the coordinate box at n={n}"; }
        ...
        if (MetallicQuasicrystal.Contains(
            ...
            b: 0L,
            n: 1
        )) { return "long.MinValue wrapped into metallic membership"; }

        _ = Assert.Throws<OverflowException>(testCode: () => MetallicQuasicrystal.Next(
            a: long.MaxValue,
            b: long.MaxValue,
            n: 1
        ));

        var positionRefusal = Assert.Throws<ArgumentOutOfRangeException>(testCode: () => MetallicQuasicrystal.Position(
            a: long.MaxValue,
            b: 0L,
            n: 1
        ));
        ...
        Assert.Equal(
            expected: "value",
            actual: positionRefusal.ParamName
        );
        Assert.Equal(
            expected: FixedQ4816.FromInteger(value: int.MaxValue),
            actual: MetallicQuasicrystal.InflationFactor(n: int.MaxValue)
        );

        var hugeIndexPrefix = new bool[16];

        MetallicQuasicrystal.Word(
            n: int.MaxValue,
            tiles: hugeIndexPrefix
        );

        if (Array.IndexOf(
            array: hugeIndexPrefix,
            value: false
        ) >= 0) { return "the large-index metallic prefix is not all long"; }
    ...
    public static string? MetallicReproducesGoldenSilverAndIsAFixedPoint() {
        ...
        MetallicQuasicrystal.Word(
            n: 1,
            tiles: metallicGolden
        );
        MetallicQuasicrystal.Word(
            n: 2,
            tiles: metallicSilver
        );
        ...
        for (var i = 0; (i < goldenFromOrigin.Length); ++i) {
            goldenFromOrigin[i] = MetallicQuasicrystal.StartsLongTile(
                a: goldenWalk.A,
                b: goldenWalk.B,
                n: 1
            );
        ...
        for (var i = 0; (i < silverFromOrigin.Length); ++i) {
            silverFromOrigin[i] = MetallicQuasicrystal.StartsLongTile(
                a: silverWalk.A,
                b: silverWalk.B,
                n: 2
            );
            silverWalk = MetallicQuasicrystal.Next(
                a: silverWalk.A,
                b: silverWalk.B,
                n: 2
            );
    ...
    public static string? ModularTransformClassesAndCuspAction() {
        ...
        if (ModularTransform.Create(
            a: 2L,
            b: 1L,
            c: 1L,
            d: 1L
        ).Classify() != ModularClass.Hyperbolic) { return "[2,1,1,1] is not hyperbolic"; }
        ...
        for (var index = 0; (index < words.Length); ++index) {
            ...
            for (var trial = 0; (trial < 6); ++trial) {
                ...
                var composed = (word * other).Apply(
                    denominator: q,
                    numerator: p
                );

                var (innerP, innerQ) = other.Apply(
                    denominator: q,
                    numerator: p
                );

                if (composed != word.Apply(
                    denominator: innerQ,
                    numerator: innerP
                )) {
                ...
                var (sP, sQ) = ModularTransform.S.Apply(
                    denominator: q,
                    numerator: p
                );

                if (ModularTransform.S.Apply(
                    denominator: sQ,
                    numerator: sP
                ) != ModularCuspOracle(
                    g: ModularTransform.Identity,
                    p: p,
                    q: q
                )) {
    ...
    public static string? QuadraticInflationInvariantsAndPolynomialTails() {
        ...
        foreach (var testCase in cases) {
            ...
            if (
                (inflation.PeriodLength != testCase.Period) ||
                (inflation.Determinant != testCase.Det) ||
                (inflation.Discriminant != testCase.Disc)
            ) {
                return $"the inflation invariants are wrong at d={testCase.D}";
            }
            if (inflation.Determinant != (((inflation.PeriodLength & 1) == 0)
                ? 1L
                : -1L)) {
                return $"the determinant sign disagrees with the period parity at d={testCase.D}";
            }
            if (
                (inflation.GeodesicClass != ModularClass.Hyperbolic) ||
                (inflation.Axis.Classify() != ModularClass.Hyperbolic)
            ) {
            ...
            var lambda = RealQuadratic.Create(
                rationalNumerator: inflation.Trace,
                surdNumerator: 1,
                radicand: inflation.Discriminant,
                denominator: 2
            );
            ...
            if (!WithinBracket(
                actual: inflation.InflationFactor(),
                rationalNumerator: inflation.Trace,
                surdNumerator: 1,
                radicand: inflation.Discriminant,
                denominator: 2,
                toleranceRawTicks: 8L
            )) {
        ...
        if (
            (goldenTail.Slope != RealQuadratic.Create(
            denominator: 2,
            radicand: 5,
            rationalNumerator: 1,
            surdNumerator: 1
        )) ||
            (goldenTail.Offset != RealQuadratic.Create(
            denominator: 10,
            radicand: 5,
            rationalNumerator: -5,
            surdNumerator: -3
        ...
        if (
            (goldenAsymptotics.Count != 16) ||
            (goldenAsymptotics[0] != goldenTail.Offset)
        ) {
        ...
        if (
            !wideTail.VerifyIntervalCertificate() ||
            (wideTail.CertifiedInterval(tailIndex: wideTail.IntervalCertificate.Cutoff).Lower.Sign <= 0)
        ) {
            return "the wide r>>p^2 contraction certificate is wrong";
        }

        _ = Assert.Throws<ArgumentOutOfRangeException>(testCode: () => PolynomialContinuedFractionTail.Analyze(
            constant: -2,
            linear: 1,
            numeratorConstant: 0,
            numeratorLinear: 0,
    ...
    public static string? QuadraticQuasicrystalWidthAndPeriodRegressions() {
        var perfectSquareRefusal = Assert.Throws<ArgumentOutOfRangeException>(testCode: () => QuadraticQuasicrystal.Word(
            d: 4L,
            p: 0L,
            q: 1L,
            r: 1L,
        ...
        Assert.Equal(
            expected: "d",
            actual: perfectSquareRefusal.ParamName
        );
        ...
        QuadraticQuasicrystal.Word(
            ...
            q: 1L,
            r: 1L,
            tiles: longPeriodWord
        ); // period length 217

        var longPeriodIndex = QuadraticQuasicrystal.Compile(
            d: 9949L,
            p: 0L,
            q: 1L,
            r: 1L
        ...
        for (var k = 1; (k <= 12); ++k) {
            if (WordComplexity(
                k: k,
                word: longPeriodWord
            ) != (k + 1)) { return $"the long-period word is not Sturmian at k={k}"; }
        }

        _ = Assert.Throws<OverflowException>(testCode: () => QuadraticInflation.FromQuadraticIrrational(
            d: 9949L,
            p: 0L,
            q: 1L,
            r: 1L
        ...
        if (Array.IndexOf(
            array: largePrefix,
            value: false
        ) >= 0) { return "the large-quotient prefix is wrong"; }

        var largeIndex = QuadraticQuasicrystal.Compile(
            d: LargeDiscriminant,
            p: LargeQuotient,
            q: 1L,
            r: 2L
        );
        var largeActualLength = QuadraticQuasicrystal.LongTileLength(
            d: LargeDiscriminant,
            p: LargeQuotient,
            q: 1L,
            r: 2L
        ...
        if (!WithinBracket(
            actual: largeActualLength,
            rationalNumerator: largeExactTileLength.RationalNumerator,
            surdNumerator: largeExactTileLength.SurdNumerator,
            radicand: largeExactTileLength.Radicand,
            denominator: largeExactTileLength.Denominator,
            toleranceRawTicks: (1L << 20)
        )) {
            return "the large-quotient tile length lost precision";
        }

        var overflowTiles = Enumerable.Repeat(
            count: 50_000,
            element: true
        ).ToArray();
