...
internal static class Laws {
    ...
    public static void BinaryMatchesOracle(string lawId, Domain domain, Tier tier, BinaryElemOp subject, BinaryElemOp oracle) {
        ...
        foreach (var (u1, v1, u2, v2) in Domains.Quads(
            domain: domain,
            index: index,
            tier: tier
        )) {
            ...
            if (actual != expected) {
                Fail(
                    detail: $"operands=({u1},{v1},{u2},{v2}) subject=({actual.U},{actual.V}) oracle=({expected.U},{expected.V})",
                    domain: domain,
                    index: index,
                    lawId: lawId
                );
    ...
    public static void ConjugateSymmetry(string lawId, Domain domain, Tier tier, BinaryElemOp mul, UnaryElemOp conj) {
        ...
        foreach (var (u1, v1, u2, v2) in Domains.Quads(
            ...
            if (conjugateOfProduct != productOfConjugates) {
                Fail(
                    detail: $"operands=({u1},{v1},{u2},{v2}) conj(ab)=({conjugateOfProduct.U},{conjugateOfProduct.V}) conj(a)conj(b)=({productOfConjugates.U},{productOfConjugates.V})",
                    domain: domain,
                    index: index,
                    lawId: lawId
                );
    ...
    public static void DivergenceCanary(string lawId, Domain domain, Tier tier, int width, VectorBinaryOp fused, VectorBinaryOp perProduct, int minimumDivergences) {
        ...
        foreach (var (left, right) in Domains.Vectors(
            domain: domain,
            index: index,
            tier: tier,
            width: width
        ...
        if (divergences < minimumDivergences) {
            Fail(
                detail: $"fused and per-product agreed too often: {divergences} divergence(s) over {cases} case(s), floor {minimumDivergences}",
                domain: domain,
                index: index,
                lawId: lawId
            );
    ...
    public static void IdentityElement(string lawId, Domain domain, Tier tier, BinaryElemOp op, long identityU, long identityV) {
        ...
        foreach (var (u, v) in Domains.Pairs(
            ...
            if (
                ...
                Fail(
                    detail: $"operands=({u},{v}) left=({left.U},{left.V}) right=({right.U},{right.V})",
                    domain: domain,
                    index: index,
                    lawId: lawId
                );
    ...
    public static void MobiusMatchesOracle(string lawId, Domain domain, Tier tier, UnaryElemOp subject, ScalarBinaryOp oracleNumerator) {
        ...
        foreach (var (n, d) in Domains.Pairs(
            domain: domain,
            index: index,
            tier: tier
        )) {
            var (numerator, denominator) = subject(
                n,
                d
            );
            var expected = oracleNumerator(
                n,
                d
            );
            ...
            if (
                ...
                Fail(
                    detail: $"operands=({n},{d}) step=({numerator}:{denominator}) expectedNumerator={expected}",
                    domain: domain,
                    index: index,
                    lawId: lawId
                );
    ...
    public static void NormMultiplicativity(string lawId, Domain domain, Tier tier, BinaryElemOp mul, ScalarElemOp norm, ScalarBinaryOp combineNorms) {
        ...
        foreach (var (u1, v1, u2, v2) in Domains.Quads(
            ...
            var normOfProduct = norm(
                product.U,
                product.V
            );
            var productOfNorms = combineNorms(
                norm(
                    u1,
                    v1
                ),
            ...
            if (normOfProduct != productOfNorms) {
                Fail(
                    detail: $"operands=({u1},{v1},{u2},{v2}) N(ab)={normOfProduct} N(a)*N(b)={productOfNorms}",
                    domain: domain,
                    index: index,
                    lawId: lawId
                );
    ...
    public static void PureBinary(string lawId, Domain domain, Tier tier, BinaryElemOp op) {
        ...
        foreach (var (u1, v1, u2, v2) in Domains.Quads(
            ...
            if (op(
                ...
                Fail(
                    detail: $"operands=({u1},{v1},{u2},{v2}) repeated call differed",
                    domain: domain,
                    index: index,
                    lawId: lawId
                );
    ...
    public static void PureScalarBinary(string lawId, Domain domain, Tier tier, ScalarBinaryOp op) {
        ...
        foreach (var (a, b) in Domains.Pairs(
            domain: domain,
            index: index,
            tier: tier
        )) {
            if (op(
                ...
                Fail(
                    detail: $"operands=({a},{b}) repeated call differed",
                    domain: domain,
                    index: index,
                    lawId: lawId
                );
    ...
    public static void RoundTrip(string lawId, Domain domain, Tier tier, UnaryElemOp forward, UnaryElemOp inverse) {
        ...
        foreach (var (u, v) in Domains.Pairs(
            ...
            if (restored != (u, v)) {
                Fail(
                    detail: $"operands=({u},{v}) round-trip=({restored.U},{restored.V})",
                    domain: domain,
                    index: index,
                    lawId: lawId
                );
    ...
    public static void ScalarBinaryMatchesOracle(string lawId, Domain domain, Tier tier, ScalarBinaryOp subject, ScalarBinaryOp oracle) {
        ...
        foreach (var (a, b) in Domains.Pairs(
            ...
            if (actual != expected) {
                Fail(
                    detail: $"operands=({a},{b}) subject={actual} oracle={expected}",
                    domain: domain,
                    index: index,
                    lawId: lawId
                );
    ...
    public static void ScalarTwin(string lawId, Domain domain, Tier tier, ScalarElemOp first, ScalarElemOp second, ScalarElemOp? witness) {
        ...
        foreach (var (u, v) in Domains.Pairs(
            ...
            if (a != b) {
                Fail(
                    detail: $"operands=({u},{v}) first={a} second={b}",
                    domain: domain,
                    index: index,
                    lawId: lawId
                );
            ...
            if (witness is not null) {
                ...
                if (a != c) {
                    Fail(
                        detail: $"operands=({u},{v}) twin={a} witness={c}",
                        domain: domain,
                        index: index,
                        lawId: lawId
                    );
        ...
        foreach (var (left, right) in Domains.Vectors(
            ...
            if (claim(
                ...
                Fail(
                    ...
                    domain: domain,
                    index: index,
                    detail: $"left=[{Render(values: left)}] right=[{Render(values: right)}] {detail}"
                );
            }
        }
    }
    ...
    public static void TwinBinary(string lawId, Domain domain, Tier tier, BinaryElemOp first, BinaryElemOp second, BinaryElemOp? witness) {
        ...
        foreach (var (u1, v1, u2, v2) in Domains.Quads(
            ...
            if (a != b) {
                Fail(
                    detail: $"operands=({u1},{v1},{u2},{v2}) first=({a.U},{a.V}) second=({b.U},{b.V})",
                    domain: domain,
                    index: index,
                    lawId: lawId
                );
            ...
            if (witness is not null) {
                ...
                if (a != c) {
                    Fail(
                        detail: $"operands=({u1},{v1},{u2},{v2}) twin=({a.U},{a.V}) witness=({c.U},{c.V})",
                        domain: domain,
                        index: index,
                        lawId: lawId
                    );
    ...
    public static void TwinPower(string lawId, Domain domain, Tier tier, PowerOp first, PowerOp second, PowerOp? witness) {
        ...
        foreach (var (p, q) in Domains.Pairs(
            domain: domain,
            index: index,
            tier: tier
        )) {
            foreach (var exponent in ExponentLadder) {
                ...
                if (a != b) {
                    Fail(
                        detail: $"relation=({p},{q}) exponent={exponent} first=({a.U},{a.V}) second=({b.U},{b.V})",
                        domain: domain,
                        index: index,
                        lawId: lawId
                    );
                ...
                if (witness is not null) {
                    ...
                    if (a != c) {
                        Fail(
                            detail: $"relation=({p},{q}) exponent={exponent} twin=({a.U},{a.V}) witness=({c.U},{c.V})",
                            domain: domain,
                            index: index,
                            lawId: lawId
                        );
        ...
        foreach (var (left, right) in Domains.Vectors(
            ...
            if (lane >= 0) {
                Fail(
                    ...
                    domain: domain,
                    index: index,
                    detail: $"left=[{Render(values: left)}] right=[{Render(values: right)}] lane={lane} subject={actual[lane]} oracle={expected[lane]}"
                );
            }
        }
    }
    ...
    public static void VectorTernaryTwin(string lawId, Domain domain, Tier tier, int width, VectorTernaryOp first, VectorTernaryOp second, VectorTernaryOp? witness) {
        ...
        var z = ((witness is null)
            ? []
            : new long[width]
        );

        foreach (var (a, b, c) in Domains.VectorTriples(
            domain: domain,
            index: index,
            tier: tier,
            width: width
            ...
            if (lane >= 0) {
                Fail(
                    ...
                    domain: domain,
                    index: index,
                    detail: $"a=[{Render(values: a)}] b=[{Render(values: b)}] c=[{Render(values: c)}] lane={lane} first={x[lane]} second={y[lane]}"
                );
            }
            ...
            if (witness is not null) {
                ...
                var witnessLane = FirstDifference(
                    left: x,
                    right: z
                );
                ...
                if (witnessLane >= 0) {
                    Fail(
                        ...
                        domain: domain,
                        index: index,
                        detail: $"a=[{Render(values: a)}] b=[{Render(values: b)}] c=[{Render(values: c)}] lane={witnessLane} twin={x[witnessLane]} witness={z[witnessLane]}"
                    );
                }
            }
        }
        ...
        foreach (var (left, right) in Domains.Vectors(
            ...
            if (lane >= 0) {
                Fail(
                    ...
                    domain: domain,
                    index: index,
                    detail: $"left=[{Render(values: left)}] right=[{Render(values: right)}] lane={lane} first={a[lane]} second={b[lane]}"
                );
            }
            ...
            if (witness is not null) {
                ...
                if (witnessLane >= 0) {
                    Fail(
                        ...
                        domain: domain,
                        index: index,
                        detail: $"left=[{Render(values: left)}] right=[{Render(values: right)}] lane={witnessLane} twin={a[witnessLane]} witness={c[witnessLane]}"
                    );
                }
            }
        }
