...
internal static class GeometricAlgebraClaims {
    private static readonly GeometricAlgebra ComplexAlgebra = GeometricAlgebra.Create(
        degenerateCount: 0,
        negativeCount: 1,
        positiveCount: 0
    );
    private static readonly GeometricAlgebra SplitAlgebra = GeometricAlgebra.Create(
        degenerateCount: 0,
        negativeCount: 0,
        positiveCount: 1
    );
    private static readonly GeometricAlgebra DualAlgebra = GeometricAlgebra.Create(
        degenerateCount: 1,
        negativeCount: 0,
        positiveCount: 0
    );
    private static readonly GeometricAlgebra QuaternionAlgebra = GeometricAlgebra.Create(
        degenerateCount: 0,
        negativeCount: 0,
        positiveCount: 3
    );
    private static readonly GeometricAlgebra MotorAlgebra = GeometricAlgebra.Create(
        degenerateCount: 1,
        negativeCount: 0,
        positiveCount: 3
    );
    ...
    private static FixedVector3 ApplySandwich(Multivector motor, FixedVector3 point) {
        ...
        var moved = MotorAlgebra.SandwichTransform(
            motor: motor,
            vector: embedded
        );
    ...
    private static Multivector Motor(FixedQuaternion rotation, FixedVector3 translation) =>
        MotorAlgebra.GeometricProduct(
            left: Translator(translation: translation),
            right: QuaternionToEven(value: rotation)
        );
    ...
    private static (long X, long Y, long Z, long W) QuaternionHamiltonOracle(FixedQuaternion left, FixedQuaternion right) {
        ...
        return (
            X: Oracles.RoundDyadic(
            exact: ((((lw * rx) + (lx * rw)) + (ly * rz)) - (lz * ry)),
            shift: 16
        ),
            Y: Oracles.RoundDyadic(
            exact: ((((lw * ry) - (lx * rz)) + (ly * rw)) + (lz * rx)),
            shift: 16
        ),
            Z: Oracles.RoundDyadic(
            exact: ((((lw * rz) + (lx * ry)) - (ly * rx)) + (lz * rw)),
            shift: 16
        ),
            W: Oracles.RoundDyadic(
            exact: ((((lw * rw) - (lx * rx)) - (ly * ry)) - (lz * rz)),
            shift: 16
        )
        );
    }
    ...
    private static long RawError(FixedVector3 actual, FixedVector3 expected) =>
        Math.Max(
            ...
            val2: Math.Max(
                val1: Math.Abs(value: (actual.Y.Value - expected.Y.Value)),
                val2: Math.Abs(value: (actual.Z.Value - expected.Z.Value))
            )
        );
    ...
    private static string? SignatureDistinction(GeometricAlgebra left, GeometricAlgebra right) {
        if (left.GeneratorCount != right.GeneratorCount) {
            var narrow = ((left.GeneratorCount < right.GeneratorCount)
                ? left
                : right
            );
            var wide = ((left.GeneratorCount < right.GeneratorCount)
                ? right
                : left
            );
        ...
        for (var generator = 0; (generator < left.GeneratorCount); ++generator) {
            ...
            if (left.GeometricProduct(
                left: probe,
                right: probe
            )[0].Value == right.GeometricProduct(
                left: probe,
                right: probe
            )[0].Value) {
    ...
    private static (long Scalar, long Other) VersorResidual(GeometricAlgebra algebra, Multivector value) {
        var product = algebra.GeometricProduct(
            left: value,
            right: algebra.Reverse(value: value)
        );
        ...
        for (var lane = 1; (lane < algebra.BladeCount); ++lane) {
            other = Math.Max(
                val1: other,
                val2: Math.Abs(value: product[lane].Value)
            );
        }
    ...
    public static string? CliffordDescriptorIdentitySurface() {
        ...
        var descriptors = signatures
            .Select(selector: signature => GeometricAlgebra.Create(
            degenerateCount: signature.Degenerate,
            negativeCount: signature.Negative,
            positiveCount: signature.Positive
        ))
            .ToArray();
        ...
        for (var index = 0; (index < signatures.Count); ++index) {
            ...
            var second = GeometricAlgebra.Create(
                degenerateCount: signature.Degenerate,
                negativeCount: signature.Negative,
                positiveCount: signature.Positive
            );
    ...
    public static string? CliffordExponentialDomainSurface() {
        var accepted = new (string Name, GeometricAlgebra Algebra, Multivector Bivector)[] {
            ("circular (2,0,0)", GeometricAlgebra.Create(
            degenerateCount: 0,
            negativeCount: 0,
            positiveCount: 2
        ), Blades((0b0011, 32768L))),
            ("circular (2,0,0) unit", GeometricAlgebra.Create(
            degenerateCount: 0,
            negativeCount: 0,
            positiveCount: 2
        ), Blades((0b0011, 65536L))),
            ("circular (0,2,0)", GeometricAlgebra.Create(
            degenerateCount: 0,
            negativeCount: 2,
            positiveCount: 0
        ), Blades((0b0011, 49152L))),
            ("hyperbolic (1,1,0)", GeometricAlgebra.Create(
            degenerateCount: 0,
            negativeCount: 1,
            positiveCount: 1
        ), Blades((0b0011, 32768L))),
            ("hyperbolic (1,1,0) unit", GeometricAlgebra.Create(
            degenerateCount: 0,
            negativeCount: 1,
            positiveCount: 1
        ), Blades((0b0011, 65536L))),
            ("degenerate (1,0,1)", GeometricAlgebra.Create(
            degenerateCount: 1,
            negativeCount: 0,
            positiveCount: 1
        ), Blades((0b0011, 98304L))),
            ("rotor (3,0,0)", GeometricAlgebra.Create(
            degenerateCount: 0,
            negativeCount: 0,
            positiveCount: 3
        ), Blades(
            ...
            ("zero (3,0,0)", GeometricAlgebra.Create(
            degenerateCount: 0,
            negativeCount: 0,
            positiveCount: 3
        ), Blades()),
            ("translator (3,0,1)", GeometricAlgebra.Create(
            degenerateCount: 1,
            negativeCount: 0,
            positiveCount: 3
        ), Blades(
            ...
            ("screw generator (3,0,1)", GeometricAlgebra.Create(
            degenerateCount: 1,
            negativeCount: 0,
            positiveCount: 3
        ), Blades((0b0011, 32768L))),
            ("zero scalar algebra", GeometricAlgebra.Create(
            degenerateCount: 0,
            negativeCount: 0,
            positiveCount: 0
        ), Blades()),
        ...
        foreach (var (name, algebra, bivector) in accepted) {
            ...
            var (scalarResidual, otherResidual) = VersorResidual(
                algebra: algebra,
                value: value
            );

            if (
                (scalarResidual > VersorToleranceRaw) ||
                (otherResidual > VersorToleranceRaw)
            ) {
        ...
        var cl300 = GeometricAlgebra.Create(
            degenerateCount: 0,
            negativeCount: 0,
            positiveCount: 3
        );
        var cl400 = GeometricAlgebra.Create(
            degenerateCount: 0,
            negativeCount: 0,
            positiveCount: 4
        );
        var refused = new (string Name, GeometricAlgebra Algebra, Multivector Input)[] {
            ...
            ("grade two plus a scalar", cl300, Blades(
            (0b0000, 65536L),
            (0b0011, 32768L)
        )),
            ("grade two plus a vector", cl300, Blades(
            (0b0001, 1L),
            (0b0011, 32768L)
        )),
            ("non-simple e12+e34", cl400, Blades(
            (0b0011, 65536L),
            (0b1100, 65536L)
        )),
            ("non-simple e13+e24", cl400, Blades(
            (0b0101, 65536L),
            (0b1010, 65536L)
        )),
            ("non-simple, unequal weights", cl400, Blades(
            (0b0011, 32768L),
            (0b1100, 131072L)
        )),
        };
        ...
        // to one raw.
        var nonSimple = Blades(
            (0b0011, 65536L),
            (0b1100, 65536L)
        );
        var square = cl400.GeometricProduct(
            left: nonSimple,
            right: nonSimple
        );
        ...
        var branchResidual = VersorResidual(
            algebra: cl400,
            value: branchValue
        );
        ...
        var trueResidual = VersorResidual(
            algebra: cl400,
            value: trueExponential
        );

        if (
            (trueResidual.Scalar > VersorToleranceRaw) ||
            (trueResidual.Other > VersorToleranceRaw)
        ) {
    ...
    public static string? GeometricMotorRigidTransformSurface(long[] left, long[] right) {
        ...
        var translation = new FixedVector3(
            X: ExactFixed(raw: left[4]),
            Y: ExactFixed(raw: left[5]),
            Z: ExactFixed(raw: left[6])
        );
        var point = new FixedVector3(
            X: ExactFixed(raw: left[7]),
            Y: ExactFixed(raw: left[8]),
            Z: ExactFixed(raw: left[9])
        );
        ...
        var transformed = ApplySandwich(
            motor: motor,
            point: point
        );
        var expected = FixedRigidTransform.FromRotationTranslation(
            rotation: rotation,
            translation: translation
        ...
        var rotationB = BoundedRotation(
            x: right[0],
            y: right[1],
            z: right[2],
            w: right[3]
        );
        var translationB = new FixedVector3(
            X: ExactFixed(raw: right[4]),
            Y: ExactFixed(raw: right[5]),
            Z: ExactFixed(raw: right[6])
        );
        var motorB = Motor(
            rotation: rotationB,
            translation: translationB
        );
        var composedMotor = MotorAlgebra.GeometricProduct(
            left: motor,
            right: motorB
        );
        var composedTransformed = ApplySandwich(
            motor: composedMotor,
            point: point
        );
        ...
        var composeError = RawError(
            actual: composedTransformed,
            expected: composedExpected
        );
    ...
    public static string? GeometricMultivectorDecompositionSurface(long[] left, long[] right) {
        ...
        for (var grade = 0; (grade <= MotorAlgebra.GeneratorCount); ++grade) {
            var projected = MotorAlgebra.GradeProjection(
                grade: grade,
                value: mv
            );
    ...
    public static void GeometricQuaternionEvenFirst(ReadOnlySpan<long> left, ReadOnlySpan<long> right, Span<long> result) {
        ...
        var product = QuaternionAlgebra.GeometricProduct(
            left: QuaternionToEven(value: a),
            right: QuaternionToEven(value: b)
        );
        ...
        var oracle = QuaternionHamiltonOracle(
            left: a,
            right: b
        );

        if (
            (recovered.X.Value != oracle.X) ||
            (recovered.Y.Value != oracle.Y) ||
            (recovered.Z.Value != oracle.Z) ||
            (recovered.W.Value != oracle.W)
        ) {
            throw new InvalidOperationException(message: ((string)$"the even (3,0,0) embedding of {a} and {b} recovered ({recovered.X.Value},{recovered.Y.Value},{recovered.Z.Value},{recovered.W.Value}) but the independent exact Hamilton-product oracle gives ({oracle.X},{oracle.Y},{oracle.Z},{oracle.W})"));
        }
    ...
    public static string? GeometricReverseSurface(long[] left, long[] right) {
        ...
        var reverseOfProduct = MotorAlgebra.Reverse(value: MotorAlgebra.GeometricProduct(
            left: x,
            right: y
        ));
        var productOfReverses = MotorAlgebra.GeometricProduct(
            left: MotorAlgebra.Reverse(value: y),
            right: MotorAlgebra.Reverse(value: x)
        );
    ...
    public static string? MonogenicExactSurface(long[] left, long[] right) {
        ...
        var quad = QuadraticAlgebra<BigInteger>.Create(
            p: p,
            q: q
        );
        ...
        if (
            (modulusReadback[0] != modulus2[0]) ||
            (modulusReadback[1] != modulus2[1])
        ) {
        ...
        var qa2 = new QuadraticAlgebra<BigInteger>.Element(
            U: a2[0],
            V: a2[1]
        );
        var qb2 = new QuadraticAlgebra<BigInteger>.Element(
            U: b2[0],
            V: b2[1]
        );

        var monoSum = mono2.Add(
            left: elementA2,
            right: elementB2
        );
        var monoDiff = mono2.Subtract(
            left: elementA2,
            right: elementB2
        );
        ...
        var monoProduct2 = mono2.Multiply(
            left: elementA2,
            right: elementB2
        );
        var quadProduct = quad.Multiply(
            left: qa2,
            right: qb2
        );
        var refProduct2 = MonogenicReference<BigInteger>.Multiply(
            left: a2,
            modulus: modulus2,
            right: b2
        );
        ...
        if (
            (monoProduct2[0] != quadProduct.U) ||
            (monoProduct2[1] != quadProduct.V)
        ) {
            return "degree-2 Multiply disagrees with QuadraticAlgebra<BigInteger>";
        }

        if (
            (monoProduct2[0] != refProduct2[0]) ||
            (monoProduct2[1] != refProduct2[1])
        ) {
        ...
        if (monoNorm2 != MonogenicReference<BigInteger>.Norm(
            modulus: modulus2,
            value: a2
        )) { return "degree-2 Norm disagrees with the from-definition reference"; }
        ...
        if (monoTrace2 != MonogenicReference<BigInteger>.Trace(
            modulus: modulus2,
            value: a2
        )) { return "degree-2 Trace disagrees with the from-definition reference"; }
        ...
        var refPower2 = MonogenicReference<BigInteger>.CompanionPower(
            exponent: Exponent,
            modulus: modulus2
        );

        if (
            (monoPower2[0] != quadPower.U) ||
            (monoPower2[1] != quadPower.V)
        ) {
            return "degree-2 CompanionPower disagrees with QuadraticAlgebra<BigInteger>";
        }

        if (
            (monoPower2[0] != refPower2[0]) ||
            (monoPower2[1] != refPower2[1])
        ) {
        ...
        if (
            (window2[0] != a2[0]) ||
            (window2[1] != a2[1])
        ) { return "FromWindow did not read back its coordinates at degree 2"; }

        var monoStep2 = mono2.ProjectiveStep(window: window2);
        var quadStep = quad.MobiusStep(pair: new QuadraticAlgebra<BigInteger>.Projective(
            Numerator: a2[0],
            Denominator: a2[1]
        ));

        if (
            (monoStep2[0] != quadStep.Numerator) ||
            (monoStep2[1] != quadStep.Denominator)
        ) {
            return "degree-2 ProjectiveStep disagrees with QuadraticAlgebra<BigInteger>.MobiusStep";
        }

        var one2 = mono2.One;
        var oneTimesA = mono2.Multiply(
            left: one2,
            right: elementA2
        );
        ...
        if (
            (oneTimesA[0] != a2[0]) ||
            (oneTimesA[1] != a2[1])
        ) { return "One is not a left multiplicative identity at degree 2"; }

        var zero2 = mono2.Zero;
        var zeroPlusA = mono2.Add(
            left: zero2,
            right: elementA2
        );
        ...
        if (
            (zeroPlusA[0] != a2[0]) ||
            (zeroPlusA[1] != a2[1])
        ) { return "Zero is not an additive identity at degree 2"; }

        var root2 = mono2.Root;
        var rootSquared = mono2.Multiply(
            left: root2,
            right: root2
        );
        var expectedRootSquared = mono2.Add(
            left: mono2.Multiply(
                left: mono2.FromCoordinates(coordinates: ((BigInteger[])[p, BigInteger.Zero])),
                right: root2
            ),
        ...
        if (
            (rootSquared[0] != expectedRootSquared[0]) ||
            (rootSquared[1] != expectedRootSquared[1])
        ) {
        ...
        var monoProduct3 = mono3.Multiply(
            left: elementA3,
            right: elementB3
        );
        var refProduct3 = MonogenicReference<BigInteger>.Multiply(
            left: a3,
            modulus: modulus3,
            right: b3
        );
        ...
        if (monoNorm3 != MonogenicReference<BigInteger>.Norm(
            modulus: modulus3,
            value: a3
        )) {
        ...
        if (monoTrace3 != MonogenicReference<BigInteger>.Trace(
            modulus: modulus3,
            value: a3
        )) {
    ...
    public static void MonogenicFusedMultiply(ReadOnlySpan<long> left, ReadOnlySpan<long> right, Span<long> result) {
        ...
        var product = FusionAlgebra.Multiply(
            left: FusionAlgebra.FromCoordinates(coordinates: a),
            right: FusionAlgebra.FromCoordinates(coordinates: b)
        );
    ...
    public static string? MonogenicIdentitySurface() {
        ...
        foreach (var other in differentTails) {
            ...
            if (algebra.Equals(other: otherAlgebra)) {
                return $"the descriptor of tail [{string.Join(
                    separator: ",",
                    values: tail
                )}] compares equal to the one of tail [{string.Join(
                    separator: ",",
                    values: other
                )}]";
            }
        ...
        if (!algebra.Add(
            left: algebra.One,
            right: algebra.Zero
        ).Equals(other: algebra.One)) { return "Add(One, Zero) is not equal to One"; }
        if (!(algebra.Add(
            left: algebra.One,
            right: algebra.Zero
        ) == algebra.One)) { return "operator == disagrees with Equals for Add(One, Zero) and One"; }
        ...
        if (!set.Contains(item: algebra.Add(
            left: algebra.One,
            right: algebra.Zero
        ))) { return "a set holding One does not recognize Add(One, Zero) as the same value"; }
        ...
        var fixedProduct = fixedAlgebra.Multiply(
            left: fixedElement,
            right: fixedAlgebra.One
        );
    ...
    public static void MonogenicPerProductMultiply(ReadOnlySpan<long> left, ReadOnlySpan<long> right, Span<long> result) {
        ...
        var product = MonogenicReference<FixedQ4816>.Multiply(
            left: a,
            modulus: FusionModulus,
            right: b
        );
    ...
    public static string? MonogenicPlasticRatioSurface() {
        ...
        foreach (var (aRaw, bRaw) in samples) {
            ...
            var normOfProduct = mono.Norm(value: mono.Multiply(
                left: a,
                right: b
            ));
            ...
            if (normOfProduct != productOfNorms) {
                return $"Norm is not multiplicative for [{string.Join(
                    separator: ",",
                    values: aRaw
                )}] and [{string.Join(
                    separator: ",",
            ...
            var traceOfSum = mono.Trace(value: mono.Add(
                left: a,
                right: b
            ));
            ...
            if (traceOfSum != sumOfTraces) {
                return $"Trace is not additive for [{string.Join(
                    separator: ",",
                    values: aRaw
                )}] and [{string.Join(
                    separator: ",",
