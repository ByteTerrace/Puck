...
internal static class FixedContributionFoldClaims {
    ...
    private static long Evaluate(
        ...
        FixedContributionFold.Evaluate(
            ...
            poolDeltaRaw: poolDeltaRaw,
            outsidePoolDeltaRaw: outsidePoolDeltaRaw,
            poolRadius: ((poolRadiusRaw is { } radius)
            ? FixedQ4816.FromRawBits(value: radius)
            : null),
            minimum: FixedQ4816.FromRawBits(value: minimumRaw),
            maximum: FixedQ4816.FromRawBits(value: maximumRaw),
            threshold: ((thresholdRaw is { } threshold)
            ? FixedQ4816.FromRawBits(value: threshold)
            : null),
    ...
    private static long PoolClampPerAdd(long baselineRaw, ReadOnlySpan<long> contributions, long radiusRaw) {
        ...
        foreach (var contribution in contributions) {
            accumulator = Math.Clamp(
                max: maximum,
                min: minimum,
                value: (accumulator + contribution)
            );
        }
    ...
    public static string? BinaryFlipBoundAndSharpness() {
        for (var threshold = 1L; (threshold <= OneRaw); ++threshold) {
            ...
            var sharpBaseline = (((threshold - 1L) <= (OneRaw - threshold))
                ? 0L
                : OneRaw
            );
            var sharpDelta = ((sharpBaseline == 0L)
                ? long.MaxValue
                : long.MinValue
            );
    ...
    public static string? DiscriminatingExamples() {
        ...
        var perAdd = PoolClampPerAdd(
            baselineRaw: 0L,
            contributions: cancelAcrossBoundary,
            radiusRaw: HalfRaw
        );
        ...
        var strictGreaterAlternative = StrictGreaterQuantize(
            maximumRaw: OneRaw,
            minimumRaw: 0L,
            raw: HalfRaw,
            thresholdRaw: HalfRaw
    ...
    public static string? FormulaSample(long[] left, long[] right) {
        ...
        var radius = (((right[2] & 1L) == 0L)
            ? null
            : (long?)NonNegative(raw: right[1])
        );
        var threshold = (((right[2] & 2L) == 0L)
            ? null
            : (long?)FoldIntoRange(
                raw: right[3],
                minimum: minimum,
    ...
    public static string? OverflowBoundaryExact() {
        ...
        if (
            (maximumSafeSum != ((BigInteger.One << 63) - one)) ||
            (positiveMargin != (one - BigInteger.One))
        ) {
        ...
        if (
            (negativeBoundarySum != new BigInteger(value: long.MinValue)) ||
            ((-firstUnderflowingCount * one) >= long.MinValue)
        ) {
        ...
        if (
            (widestNegative < int128Minimum) ||
            (widestPositive > int128Maximum)
        ) {
    ...
    public static string? RawSumEveryPermutation() {
        ...
        var expectedRawSum = contributions.Aggregate(
            func: static (sum, contribution) => checked((sum + contribution)),
            seed: 0L
        );
        ...
        foreach (var permutation in Permutations(values: contributions)) {
            var sum = permutation.Aggregate(
                func: static (total, contribution) => checked((total + contribution)),
                seed: 0L
            );
            var actual = Evaluate(
                baselineRaw: 0L,
            ...
            if (
                ...
                return $"permutation [{string.Join(
                    separator: ',',
                    values: permutation
                )}] produced sum={sum}, result={actual}, clamped={clamped}; expected sum={expectedRawSum}, result={expected}, clamped={expectedClamped}";
            }
        }
