...
public sealed class ModularTransformAndHexCoordinateTests {
    ...
    public void HexagonalCoordinateArithmeticDoesNotWrap() {
        var maximum = new HexagonalCoordinate(
            Q: int.MaxValue,
            R: int.MaxValue
        );
        var minimum = new HexagonalCoordinate(
            Q: int.MinValue,
            R: int.MinValue
        );
        ...
        Assert.Throws<OverflowException>(testCode: () => (new HexagonalCoordinate(
            Q: 50_000,
            R: 0
        ...
        Assert.Equal(
            ...
            actual: (maximum - new HexagonalCoordinate(
                Q: (int.MaxValue - 1),
                R: (int.MaxValue - 1)
            ))
        );
    ...
    public void HexagonalCoordinateRoundRefusesOutOfRangeCell() {
        Assert.Throws<OverflowException>(testCode: () => HexagonalCoordinate.Round(
    ...
    public void HexagonalCoordinateScalarQueriesDoNotWrap() {
        var far = new HexagonalCoordinate(
            Q: int.MaxValue,
            R: 0
        );
        var near = new HexagonalCoordinate(
            Q: int.MinValue,
            R: 0
        );

        Assert.Throws<OverflowException>(testCode: () => HexagonalCoordinate.Distance(
            left: far,
            right: near
        ));
    ...
    public void ModularTransformCuspNormalizesSignedMinimumMagnitude() {
        Assert.Equal(
            expected: (1L, 1L),
            actual: ModularTransform.Identity.Apply(
    ...
    public void ModularTransformCuspRefusesUnrepresentableReducedResult() {
        Assert.Throws<OverflowException>(testCode: () => ModularTransform.T.Apply(
            denominator: 1L,
    ...
    public void ModularTransformDefaultIsIdentity() {
        ...
        Assert.Equal(
            expected: ModularTransform.Identity,
            actual: value
        );
        Assert.Equal(
            expected: 1L,
        ...
        Assert.Equal(
            expected: ModularTransform.Identity.GetHashCode(),
            actual: value.GetHashCode()
        );
        Assert.Equal(
            expected: (7L, 5L),
            actual: value.Apply(
