...
internal static class OracleOwnershipClaims {
    internal static string? ForeignElementsAreRejectedUniformly() {
        var first = PresentedAlgebra<BigInteger, IntegerMaterial>.Create(presentation: Presentations.FreeMonoid<BigInteger, IntegerMaterial>(
            letterCount: 1,
            material: default,
            windowDegree: 2
        ));
        var second = PresentedAlgebra<BigInteger, IntegerMaterial>.Create(presentation: Presentations.FreeMonoid<BigInteger, IntegerMaterial>(
            letterCount: 1,
            material: default,
            windowDegree: 2
        ));
        ...
        var checks = new List<(string Name, string Parameter, Action Invoke)> {
            ("PresentedAlgebra.Add(left)", "left", () => _ = first.Add(
            left: foreign,
            right: first.Zero
        )),
            ("PresentedAlgebra.Add(right)", "right", () => _ = first.Add(
            left: first.Zero,
            right: foreign
        )),
            ("PresentedAlgebra.AreEqual", "left", () => _ = first.AreEqual(
            left: foreign,
            right: first.Zero
        )),
            ("PresentedAlgebra.AreEqual(right)", "right", () => _ = first.AreEqual(
            left: first.Zero,
            right: foreign
        )),
            ("PresentedAlgebra.Multiply", "left", () => _ = first.Multiply(
            left: foreign,
            right: first.Identity
        )),
            ("PresentedAlgebra.Multiply(right)", "right", () => _ = first.Multiply(
            left: first.Identity,
            right: foreign
        )),
            ("PresentedAlgebra.MultiplyInto", "left", () => _ = first.MultiplyInto(
            left: foreign,
            right: first.Identity,
            keys: keys,
            coefficients: coefficients
        )),
            ("PresentedAlgebra.MultiplyInto(right)", "right", () => _ = first.MultiplyInto(
            left: first.Identity,
            right: foreign,
            keys: keys,
            coefficients: coefficients
            ...
            ("PresentedAlgebra.Power at exponent zero", "value", () => _ = first.Power(
            exponent: 0UL,
            value: foreign
        )),
            ("PresentedAlgebra.PowerSequential at exponent zero", "value", () => _ = first.PowerSequential(
            exponent: 0UL,
            value: foreign
        )),
            ("PresentedAlgebra.Subtract(left)", "left", () => _ = first.Subtract(
            left: foreign,
            right: first.Zero
        )),
            ("PresentedAlgebra.Subtract", "right", () => _ = first.Subtract(
            left: first.Zero,
            right: foreign
        )),
            ("PresentedAlgebra.TruncatedSum at bound zero", "value", () => _ = first.TruncatedSum(
            bound: 0,
            value: foreign
        )),
            ("PresentedAlgebra.TrySumOverAllLengths", "value", () => _ = first.TrySumOverAllLengths(
            obstruction: out _,
            total: out _,
            value: foreign
        )),
            ("PresentedAlgebra.Residual", "value", () => _ = first.Residual(
            symbol: 0,
            value: foreign,
            twist: ResidualTwist.Counit
        )),
            ("PresentedAlgebra.TryCompileClosure", "seed", () => _ = first.TryCompileClosure(
            closure: out _,
            obstruction: out _,
            seed: foreign,
            shiftSymbol: -1,
            ...
            ("PresentedAlgebra.Pair(covector)", "covector", () => _ = first.Pair(
            covector: foreign,
            value: first.Identity
        )),
            ("PresentedAlgebra.Pair", "value", () => _ = first.Pair(
            covector: first.Identity,
            value: foreign
        )),
            ("PresentedAlgebra.Behavior", "initial", () => _ = first.Behavior(
            initial: foreign,
            value: first.Identity,
            readout: first.Identity
        )),
            ("PresentedAlgebra.Behavior(value)", "value", () => _ = first.Behavior(
            initial: first.Identity,
            value: foreign,
            readout: first.Identity
        )),
            ("PresentedAlgebra.Behavior(readout)", "readout", () => _ = first.Behavior(
            initial: first.Identity,
            value: first.Identity,
            readout: foreign
        )),
        ...
        var fieldFirst = PresentedAlgebra<ulong, PrimeFieldMaterial>.Create(presentation: Presentations.FreeMonoid<ulong, PrimeFieldMaterial>(
            letterCount: 1,
            material: fieldMaterial,
            windowDegree: 1
        ));
        var fieldSecond = PresentedAlgebra<ulong, PrimeFieldMaterial>.Create(presentation: Presentations.FreeMonoid<ulong, PrimeFieldMaterial>(
            letterCount: 1,
            material: fieldMaterial,
            windowDegree: 1
        ));
        ...
        checks.Add(item: ("PresentedAlgebra.TrySolve(divisor)", "divisor", () => _ = fieldFirst.TrySolve(
            divisor: fieldForeign,
            target: fieldFirst.Identity,
            quotient: out _,
            obstruction: out _
        )));
        checks.Add(item: ("PresentedAlgebra.TrySolve(target)", "target", () => _ = fieldFirst.TrySolve(
            divisor: fieldFirst.Identity,
            target: fieldForeign,
            quotient: out _,
            obstruction: out _
        )));
        checks.Add(item: ("PresentedAlgebra.TryResolvent", "value", () => _ = fieldFirst.TryResolvent(
            obstruction: out _,
            resolvent: out _,
            value: fieldForeign
        )));
        checks.Add(item: ("GraphZeta.TryCreate", "value", () => _ = GraphZeta<ulong, PrimeFieldMaterial>.TryCreate(
            algebra: fieldFirst,
            degreeBound: 1,
            obstruction: out _,
            order: 1,
        ...
        checks.Add(item: ("ConvergentTransfer.Entry", "value", () => _ = transferFirst.Entry(
            column: 0,
            row: 0,
            value: transferForeign
        )));

        var patternFirst = TokenPattern<bool, BooleanMaterial>.Create(
            letterCount: 1,
            material: default,
            window: 2
        );
        var patternSecond = TokenPattern<bool, BooleanMaterial>.Create(
            letterCount: 1,
            material: default,
            window: 2
        );
        ...
        checks.Add(item: ("TokenPattern.Concatenate", "left", () => _ = patternFirst.Concatenate(
            left: patternForeign,
            right: patternFirst.EmptyWord
        )));
        checks.Add(item: ("TokenPattern.Concatenate(right)", "right", () => _ = patternFirst.Concatenate(
            left: patternFirst.EmptyWord,
            right: patternForeign
        )));
        checks.Add(item: ("TokenPattern.Derivative", "value", () => _ = patternFirst.Derivative(
            letter: 0,
            value: patternForeign
        )));
        checks.Add(item: ("TokenPattern.Intersect(left)", "left", () => _ = patternFirst.Intersect(
            left: patternForeign,
            right: patternFirst.EmptyWord
        )));
        checks.Add(item: ("TokenPattern.Intersect(right)", "right", () => _ = patternFirst.Intersect(
            left: patternFirst.EmptyWord,
            right: patternForeign
        )));
        checks.Add(item: ("TokenPattern.Scale", "value", () => _ = patternFirst.Scale(
            value: patternForeign,
            weight: true
        )));
        checks.Add(item: ("TokenPattern.TryIterate", "value", () => _ = patternFirst.TryIterate(
            iterated: out _,
            obstruction: out _,
            value: patternForeign
        )));
        checks.Add(item: ("TokenPattern.TryWeigh", "value", () => _ = patternFirst.TryWeigh(
            letters: [0],
            value: patternForeign,
            weight: out _
        )));
        checks.Add(item: ("TokenPattern.Union", "left", () => _ = patternFirst.Union(
            left: patternForeign,
            right: patternFirst.EmptyWord
        )));
        checks.Add(item: ("TokenPattern.Union(right)", "right", () => _ = patternFirst.Union(
            left: patternFirst.EmptyWord,
            right: patternForeign
        )));
        checks.Add(item: ("PatternComplement.Complement", "value", () => _ = patternFirst.Complement(value: patternForeign)));
        checks.Add(item: ("PatternMatcher.TryCompile", "value", () => _ = PatternMatcher<bool, BooleanMaterial>.TryCompile(
            matcher: out _,
            obstruction: out _,
        ...
        var cliffordFirst = PresentedAlgebra<BigInteger, IntegerMaterial>.Create(presentation: Presentations.Clifford<BigInteger, IntegerMaterial>(
            degenerateCount: 0,
            material: default,
            negativeCount: 0,
            positiveCount: 1
        ));
        var cliffordSecond = PresentedAlgebra<BigInteger, IntegerMaterial>.Create(presentation: Presentations.Clifford<BigInteger, IntegerMaterial>(
            degenerateCount: 0,
            material: default,
            negativeCount: 0,
            positiveCount: 1
        ...
        checks.Add(item: ("GradedComplement.OuterProduct(left)", "left", () => _ = complement.OuterProduct(
            left: bladeForeign,
            right: cliffordFirst.Identity
        )));
        checks.Add(item: ("GradedComplement.OuterProduct(right)", "right", () => _ = complement.OuterProduct(
            left: cliffordFirst.Identity,
            right: bladeForeign
        )));
        checks.Add(item: ("GradedComplement.RegressiveProduct", "left", () => _ = complement.RegressiveProduct(
            left: bladeForeign,
            right: cliffordFirst.Identity
        )));
        checks.Add(item: ("GradedComplement.RegressiveProduct(right)", "right", () => _ = complement.RegressiveProduct(
            left: cliffordFirst.Identity,
            right: bladeForeign
        )));
        ...
        checks.Add(item: ("PresentedGroup.TryInvert", "value", () => _ = group!.TryInvert(
            inverse: out _,
            obstruction: out _,
            value: bladeForeign
        )));
        ...
        checks.Add(item: ("PresentedFunctor.TryCreate(images)", "images", () => _ = PresentedFunctor<BigInteger, IntegerMaterial>.TryCreate(
            functor: out _,
            images: [foreign],
            obstruction: out _,
            source: first,
        ...
        checks.Add(item: ("PresentedMachine.Create(initial)", "initial", () => _ = PresentedMachine<BigInteger, IntegerMaterial>.Create(
            algebra: first,
            initial: foreign,
            steps: [first.Identity],
            readout: first.Identity
        )));
        checks.Add(item: ("PresentedMachine.Create(steps)", "steps", () => _ = PresentedMachine<BigInteger, IntegerMaterial>.Create(
            algebra: first,
            initial: first.Identity,
            steps: [foreign],
            readout: first.Identity
        )));
        checks.Add(item: ("PresentedMachine.Create(readout)", "readout", () => _ = PresentedMachine<BigInteger, IntegerMaterial>.Create(
            algebra: first,
            initial: first.Identity,
            steps: [first.Identity],
            readout: foreign
        ...
        int[] dimensions = [0];
        var calculusFirst = ExteriorCalculus<BigInteger, IntegerMaterial>.Create(
            dimensions: dimensions,
            incidences: [],
            material: default
        );
        var calculusSecond = ExteriorCalculus<BigInteger, IntegerMaterial>.Create(
            dimensions: dimensions,
            incidences: [],
            material: default
        );
        ...
        checks.Add(item: ("ExteriorCalculus.Pair", "cochain", () => _ = calculusFirst.Pair(
            cochain: cochainForeign,
            chain: calculusFirst.Chain(values: [BigInteger.One])
        )));
        checks.Add(item: ("ExteriorCalculus.Pair(chain)", "chain", () => _ = calculusFirst.Pair(
            cochain: calculusFirst.Cochain(values: [BigInteger.One]),
            chain: chainForeign
        )));
        ...
        if (
            ...
            (BigInteger.Zero != transferFirst.Entry(
            column: 0,
            row: 0,
            value: default
        )) ||
            (tropicalTransfer.Algebra.Presentation.Material.Zero != tropicalTransfer.Entry(
            column: 0,
            row: 0,
            value: default
        )) ||
            (0 != patternFirst.Intersect(
            left: default,
            right: default
        ).SupportCount) ||
            !patternFirst.Algebra.AreEqual(
            left: patternFirst.Complement(value: default),
            right: patternFirst.Complement(value: patternFirst.Algebra.Zero)
        ) ||
            group!.TryInvert(
            inverse: out _,
            obstruction: out _,
            ...
            !functorTarget.AreEqual(
            left: functor!.Map(value: default),
            right: functorTarget.Zero
        )
        ) {
    ...
    internal static string? FunctorRequiresOneMaterial() {
        ...
        var unwindowedSource = PresentedAlgebra<ulong, PrimeFieldMaterial>.Create(presentation: Presentations.FreeMonoid<ulong, PrimeFieldMaterial>(
            letterCount: 1,
            material: three
        ));
        var unwindowedForeignTarget = PresentedAlgebra<ulong, PrimeFieldMaterial>.Create(presentation: Presentations.FreeMonoid<ulong, PrimeFieldMaterial>(
            letterCount: 1,
            material: five
        ));

        if (RefusedParameter(action: () => _ = PresentedFunctor<ulong, PrimeFieldMaterial>.TryCreate(
        ...
        var source = PresentedAlgebra<ulong, PrimeFieldMaterial>.Create(presentation: Presentations.FreeMonoid<ulong, PrimeFieldMaterial>(
            letterCount: 1,
            material: three,
            windowDegree: 2
        ));
        var target = PresentedAlgebra<ulong, PrimeFieldMaterial>.Create(presentation: Presentations.FreeMonoid<ulong, PrimeFieldMaterial>(
            letterCount: 1,
            material: PrimeFieldMaterial.Create(modulus: 3),
            windowDegree: 2
        ));
        ...
        var product = source.Multiply(
            left: sum,
            right: generator
        );

        if (!target.AreEqual(
            left: functor!.Map(value: zero),
            right: target.Zero
        )) {
        ...
        if (!target.AreEqual(
            ...
            right: target.Add(
                left: functor.Map(value: sum),
                right: functor.Map(value: generator)
            )
        )) {
    ...
    internal static string? PresentationOwnsAdmittedMemory() {
        ...
        var compiledProduct = algebra.Multiply(
            left: generator,
            right: generator
        );

        if (
            (BigInteger.One != generator.Coefficients[0]) ||
            !algebra.AreEqual(
            left: compiledProduct,
            right: generator
        ) ||
            ...
            (BigInteger.One != compiled.Charge(
            leftKey: 1L,
            rightKey: 1L
        ))
        ) {
        ...
        if (
            !PresentedFunctor<BigInteger, IntegerMaterial>.TryCreate(
            source: algebra,
            target: target,
            images: [target.Generator(symbol: 0)],
            functor: out var functor,
            ...
            !target.AreEqual(
            left: functor!.Map(value: compiledProduct),
            right: target.Generator(symbol: 0)
        )
        ) {
        ...
        if (
            (field?.GetValue(obj: presentation) is not Generator[] admitted) ||
    ...
    private static bool ContainsElement(Type type) {
        ...
        if (
            type.IsGenericType &&
            (type.GetGenericTypeDefinition() == typeof(PresentedAlgebra<,>.Element))
        ) {
            return true;
        }
        ...
        return (
            type.IsGenericType &&
            type.GetGenericArguments().Any(predicate: ContainsElement)
        );
    }
    private static string? PublicElementConsumerInventory() {
        ...
        var actual = typeof(PresentedAlgebra<,>).Assembly
            ...
            .SelectMany(selector: static type => type.GetMethods(bindingAttr: BindingFlags.Public | BindingFlags.Instance | BindingFlags.Static | BindingFlags.DeclaredOnly)
                ...
                .Select(selector: entry => $"{type.Name}.{entry.Method.Name}:{string.Join(
            separator: ',',
            values: entry.Parameters.Select(selector: static parameter => parameter.Name)
        )}"))
            .Distinct()
            .Order()
            .ToArray();
        ...
        return (actual.SequenceEqual(second: expected.Order().ToArray())
            ? null
            : $"the public Element-consumer inventory changed: expected [{string.Join(
                separator: "; ",
                values: expected.Order()
            )}], actual [{string.Join(
                separator: "; ",
                values: actual
            )}]"
    ...
    private static PresentedAlgebra<ulong, PrimeFieldMaterial>.Element Scale(
        ...
        for (var index = 0; (index < scaled.Length); ++index) {
            scaled[index] = algebra.Presentation.Material.Multiply(
                left: scalar,
    ...
    private static string? TensorPairingOwnership() {
        ...
        var left = PresentedAlgebra<ulong, PrimeFieldMaterial>.Create(presentation: Presentations.FreeMonoid<ulong, PrimeFieldMaterial>(
            letterCount: 1,
            material: material,
            windowDegree: 1
        ));
        var right = PresentedAlgebra<ulong, PrimeFieldMaterial>.Create(presentation: Presentations.FreeMonoid<ulong, PrimeFieldMaterial>(
            letterCount: 1,
            material: material,
            windowDegree: 1
        ));
        var tensor = PresentedAlgebra<ulong, PrimeFieldMaterial>.Create(presentation: Presentations.Tensor(
            left: left.Presentation,
            right: right.Presentation
        ));
        var paired = tensor.PairUp(
            left: left.Generator(symbol: 0),
            right: right.Generator(symbol: 0),
            rightKeyCount: right.Presentation.NormalFormCount
        );

        if (
            (1 != paired.SupportCount) ||
            (3L != paired.Keys[0]) ||
            (1UL != paired.Coefficients[0])
        ) {
        ...
        var wrongShape = PresentedAlgebra<ulong, PrimeFieldMaterial>.Create(presentation: Presentations.FreeMonoid<ulong, PrimeFieldMaterial>(
            letterCount: 2,
            material: material,
            windowDegree: 1
        ));

        if (RefusedParameter(action: () => _ = tensor.PairUp(
            left: wrongShape.Identity,
        ...
        if (RefusedParameter(action: () => _ = tensor.PairUp(
            left: left.Identity,
            right: wrongShape.Identity,
            rightKeyCount: right.Presentation.NormalFormCount
        )) is not "right") {
            return "tensor pairing accepted a right factor with the wrong owned coordinate width";
        }

        var wrongMaterial = PresentedAlgebra<ulong, PrimeFieldMaterial>.Create(presentation: Presentations.FreeMonoid<ulong, PrimeFieldMaterial>(
            letterCount: 1,
