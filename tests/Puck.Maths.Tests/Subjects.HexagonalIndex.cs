...
internal static partial class Subjects {
    ...
    public static string? HexagonalIndexPerimeter() {
        ...
        foreach (var (index, q, r, radius) in Oracles.HexIndexPerimeter(maximumRadius: 32)) {
            ...
            Assert.Equal(
                expected: new HexagonalCoordinate(
                    Q: ((int)q),
                    R: ((int)r)
                ),
                actual: subject.ToCoordinate()
            );
            Assert.Equal(
                expected: ((long)index),
                actual: HexagonalIndex.FromCoordinate(coordinate: new(
                    Q: ((int)q),
                    R: ((int)r)
                )).Value
            );
            Assert.Equal(
                expected: radius,
                actual: subject.Radius
            );
            Assert.True(
                condition: cells.Add(item: subject.ToCoordinate()),
                userMessage: $"Repeated cell at index {index}."
            );
    ...
    public static string? HexagonalIndexContinuity(long[] left, long[] right) {
        foreach (var value in new[] { HexIndexRaw(raw: left[0]), HexIndexRaw(raw: right[0]) }) {
            ...
            var radius = Oracles.HexIndexRadius(
                q: q,
                r: r
            );
    ...
    private static void CheckHexIndexSuccessor(long value) {
        ...
        var distance = Oracles.HexIndexRadius(
            q: (((BigInteger)b.Q) - a.Q),
            r: (((BigInteger)b.R) - a.R)
        );

        Assert.True(
            condition: distance.IsOne,
            userMessage: $"Indices {value} and {(value + 1)}: ({a.Q}, {a.R}) → ({b.Q}, {b.R}), distance {distance}."
        );
    }

    public static string? HexagonalIndexGeometry(long[] left, long[] right) {
        CheckHexIndexGeometry(
            value: HexIndexRaw(raw: left[0]),
            other: HexIndexRaw(raw: right[0]),
            turns: ((int)left[1])
        );
        return null;
    }
    ...
    private static void CheckHexIndexGeometry(long value, long other, int turns) {
        ...
        CheckHexIndexSuccessor(value: value);
        Assert.Equal(
            expected: ((int)Oracles.HexIndexRadius(
                q: q,
                r: r
            )),
            actual: subject.Radius
        ...
        Assert.Equal(
            expected: ((long)Oracles.HexIndexRadius(
                q: (q - bq),
                r: (r - br)
            )),
            actual: HexagonalIndex.Distance(
                left: subject,
        ...
        ExpectHexIndex(
            action: () => subject.Rotate(turns: turns),
            q: rotatedQ,
            r: rotatedR
        );
        ExpectHexIndex(
            action: subject.Conjugate,
            q: (q - r),
            r: -r
        );
        Assert.Equal(
            expected: subject,
            actual: subject.Rotate(turns: 6)
        );
        Assert.Equal(
            expected: subject,
            actual: subject.Conjugate().Conjugate()
        );
        Assert.Equal(
            expected: subject.Rotate(turns: -count),
            actual: subject.Conjugate().Rotate(turns: count).Conjugate()
        );

        for (var direction = 0; (direction < 6); ++direction) {
            var (dq, dr) = Oracles.HexIndexDirection(direction: direction);
            ExpectHexIndex(
                action: () => subject.Neighbor(direction: direction),
                q: (q + dq),
                r: (r + dr)
            );
        }
    ...
    public static string? HexagonalIndexArithmetic(long[] left, long[] right) {
        ...
        CheckHexIndexArithmetic(
            a: a,
            b: b
        );

        // Small coordinates admit nontrivial products and every intermediate in the distributive identity.
        var smallA = HexagonalIndex.FromCoordinate(coordinate: new(
            Q: ((int)(left[0] % 129)),
            R: ((int)(left[1] % 129))
        ));
        var smallB = HexagonalIndex.FromCoordinate(coordinate: new(
            Q: ((int)(right[0] % 129)),
            R: ((int)(right[1] % 129))
        ));
        ...
        Assert.Equal(
            actual: (smallA + (-smallA)),
            expected: zero
        );
        Assert.Equal(
            actual: (smallB + smallA),
            expected: (smallA + smallB)
        );
        Assert.Equal(
            actual: (smallB * smallA),
            expected: (smallA * smallB)
        ...
        Assert.Equal(
            actual: ((smallA * smallB) + smallA),
            expected: (smallA * (smallB + one))
        );
        Assert.Equal(
            expected: (smallA + smallB).Rotate(turns: 1),
            actual: (smallA.Rotate(turns: 1) + smallB.Rotate(turns: 1))
        );
        return null;
    }
    ...
    private static void CheckHexIndexArithmetic(HexagonalIndex a, HexagonalIndex b) {
        ...
        var (pq, pr) = Oracles.HexIndexProduct(
            aq: aq,
            ar: ar,
            bq: bq,
            br: br
    ...
    public static string? HexagonalIndexBoundaries() {
        ...
        Assert.Equal(
            actual: ((3 * radius) * (radius + 1)),
            expected: ((BigInteger)maximumValue)
        );
        Assert.True(condition: (((3 * (radius + 1)) * (radius + 2)) > long.MaxValue));
        Assert.True(condition: (maximumValue <= long.MaxValue));
        Assert.Equal(
            expected: 0L,
            actual: default(HexagonalIndex).Value
        );
        Assert.Equal(
            expected: default,
            actual: new HexagonalIndex(value: 0).ToCoordinate()
        );
        Assert.Equal(
            expected: default,
            actual: default(HexagonalIndex).Rotate(turns: int.MinValue)
        );
        Assert.Equal(
            expected: default,
            actual: default(HexagonalIndex).Conjugate()
        );
        Assert.Equal(
            expected: maximumValue,
            actual: new HexagonalIndex(value: maximumValue).Value
        );
        var example = HexagonalIndex.FromCoordinate(coordinate: new(
            Q: 2,
            R: 1
        ));
        ...
        Assert.Equal(
            ...
            actual: HexagonalIndex.Distance(
                left: example,
                right: example.Neighbor(direction: 1)
            )
        );
        Assert.Throws<ArgumentOutOfRangeException>(
            paramName: "value",
            testCode: () => new HexagonalIndex(value: -1)
        );
        Assert.Throws<ArgumentOutOfRangeException>(
            paramName: "value",
            testCode: () => new HexagonalIndex(value: long.MinValue)
        );
        Assert.Throws<ArgumentOutOfRangeException>(
            paramName: "value",
            testCode: () => new HexagonalIndex(value: (maximumValue + 1))
        );
        Assert.Throws<ArgumentOutOfRangeException>(
            paramName: "value",
            testCode: () => new HexagonalIndex(value: long.MaxValue)
        );
        Assert.Throws<OverflowException>(testCode: () => HexagonalIndex.FromCoordinate(coordinate: new(
            Q: (maximumRadius + 1),
            R: 0
        )));
        Assert.Throws<OverflowException>(testCode: () => HexagonalIndex.FromCoordinate(coordinate: new(
        ...
        foreach (var ring in HexIndexBoundaryRadii) {
            ...
            Assert.Equal(
                ...
                actual: new HexagonalIndex(value: start).ToCoordinate()
            );
            Assert.Equal(
                expected: new HexagonalCoordinate(
                    Q: 0,
                ...
                actual: new HexagonalIndex(value: end).ToCoordinate()
            );
        ...
        var outer = HexagonalIndex.FromCoordinate(coordinate: new(
            Q: maximumRadius,
            R: 0
        ));

        Assert.Equal(
            expected: (2L * maximumRadius),
            actual: HexagonalIndex.Distance(
                left: outer,
                right: -outer
            )
        ...
        foreach (var turns in HexIndexBoundaryTurns) {
            ...
            ExpectHexIndex(
                action: () => default(HexagonalIndex).Neighbor(direction: turns),
                q,
                r
            );
        }
