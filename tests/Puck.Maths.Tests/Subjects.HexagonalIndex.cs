...
internal static partial class Subjects {
    ...
    public static string? HexagonalIndexBoundaries() {
        ...
        Assert.Equal(expected: 8L, actual: example.Value);
        Assert.Equal(expected: 10L, actual: example.Rotate(turns: 1).Value);
        Assert.Equal(expected: 21L, actual: example.Neighbor(direction: 1).Value);
        ...
        foreach (var ring in HexIndexBoundaryRadii) {
            ...
            for (var side = 0; side < 6; ++side) {
                var corner = start + ((long)side * ring);
                CheckHexIndexGeometry(value: corner, other: maximumValue, turns: -1);
                CheckHexIndexGeometry(value: corner + ring - 1, other: 0, turns: 1);
                if (ring > 1) { CheckHexIndexGeometry(value: corner + 1, other: 0, turns: 2); }
            }
        }
