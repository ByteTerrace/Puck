...
internal static partial class Oracles {
    ...
    public static (int Kind, long Cycles) CostSum(long left, long right) => CostResult(exact: (((BigInteger)left) + right));
    public static (int Kind, long Cycles) CostProduct(long left, long right) => CostResult(exact: (((BigInteger)left) * right));
    ...
    public static long CostBudget(long frequency, long numerator, long denominator, int rate) =>
        ((rate == 0)
            ? 0L
            : (long)((((BigInteger)frequency) * numerator) / (((BigInteger)denominator) * rate))
        );
    ...
    public static BigInteger CostReferenceTicks(long cycles, long frequency) {
        var quotient = BigInteger.DivRem(
            dividend: (((BigInteger)cycles) * 50_400),
            divisor: frequency,
    ...
    public static (Int128 Numerator, long Denominator) CostStepFraction(long cycles, long frequency, int rate) =>
        ((rate == 0)
            ? (Int128.Zero, 1L)
            : (((Int128)(((BigInteger)cycles) * rate)), frequency)
        );
}
