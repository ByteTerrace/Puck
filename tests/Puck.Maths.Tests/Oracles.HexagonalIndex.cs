...
internal static partial class Oracles {
    ...
    public static (BigInteger Q, BigInteger R) HexIndexDirection(int direction) {
        ...
        return HexIndexUnits[((index < 0)
            ? (index + 6)
            : index)];
    }
    public static BigInteger HexIndexRadius(BigInteger q, BigInteger r) =>
        (((BigInteger.Abs(value: q) + BigInteger.Abs(value: r)) + BigInteger.Abs(value: (q - r))) / 2);
    public static (BigInteger Q, BigInteger R) HexIndexCoordinate(BigInteger index) {
        ...
        while (low < high) {
            ...
            if ((1 + ((3 * middle) * (middle + 1))) > index) { high = middle; } else { low = (middle + 1); }
        }
    ...
    public static BigInteger HexIndexValue(BigInteger q, BigInteger r) {
        ...
        for (var side = 0; (side < 6); ++side) {
            ...
            if (
                (position >= 0) &&
                (position < radius) &&
                (q == ((radius * a.Q) + (position * dq))) &&
                (r == ((radius * a.R) + (position * dr)))
            ) {
        ...
        throw new InvalidOperationException(message: $"No perimeter edge contains ({q}, {r}).");
    }
