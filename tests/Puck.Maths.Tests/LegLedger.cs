...
internal static class LegLedger {
    ...
    private static void AppendKindCounts(StringBuilder builder, IReadOnlyList<LegRow> rows) {
        ...
        foreach (var group in legs
            ...
            .OrderBy(keySelector: static group => KindRank(
            kind: group.Key.KindToken,
            flavor: group.Key.FlavorToken
        ))
            .ThenBy(
            keySelector: static group => ((group.Key.KindToken + ":") + group.Key.FlavorToken),
            ...
            _ = builder.Append(value: $"| {kind} | {((flavor.Length > 0)
                ? flavor
    ...
    private static void AppendOwedMarkers(StringBuilder builder, IReadOnlyList<LegRow> rows) {
        ...
        foreach (var row in rows) {
            foreach (var leg in row.Legs.Where(predicate: static leg => leg.Citation.StartsWith(
                comparisonType: StringComparison.Ordinal,
                value: OwedMarker
            ))) {
                _ = builder.Append(value: $"| {Cell(text: row.Statement)} | {Cell(text: row.Surface)} | {leg.KindToken}{((leg.FlavorToken.Length > 0)
                    ? (":" + leg.FlavorToken)
                    : "")} | {Cell(text: leg.Subject)} | {Cell(text: leg.Citation[OwedMarker.Length..].Trim())} |\n");
            }
        }
    ...
    private static void AppendOwedRegister(StringBuilder builder, IReadOnlyList<LegRow> rows) {
        ...
        foreach (var row in rows.Where(predicate: static row => (row.Legs.Any(predicate: static leg => (leg.Kind == LegKind.SharedSubstrate)) && !row.Legs.Any(predicate: static leg => leg.IsIndependent)))) {
            foreach (var leg in row.Legs.Where(predicate: static leg => (leg.Kind == LegKind.SharedSubstrate))) {
                _ = builder.Append(value: $"| {Cell(text: row.Statement)} | {Cell(text: row.Surface)} | {leg.FlavorToken} | {((leg.Flavor == ShareFlavor.FusedSubstrate)
                    ? "(B)"
                    : "(A)/(C)")} | {Cell(text: leg.Shared)} |\n");
            }
        }
    ...
    private static void AppendRows(StringBuilder builder, string heading, IReadOnlyList<LegRow> rows) {
        ...
        foreach (var row in rows) {
            foreach (var leg in row.Legs) {
                _ = builder.Append(value: $"| {Cell(text: row.Statement)} | {Cell(text: row.Surface)} | {leg.KindToken} | {((leg.FlavorToken.Length > 0)
                    ? leg.FlavorToken
                    : "—")} | {Cell(text: leg.Subject)} | {Cell(text: ((leg.Absolute.Length > 0)
                    ? $"{leg.Against} [absolute: {leg.Absolute}]"
                    : leg.Against))} | {Cell(text: leg.Shared)} | {Cell(text: leg.Citation)} |\n");
            }
        }
    ...
    private static void AppendSurfaceCounts(StringBuilder builder, IReadOnlyList<LegRow> rows) {
        ...
        foreach (var surface in rows.Select(selector: static row => row.Surface).Distinct(comparer: StringComparer.Ordinal).OrderBy(
            keySelector: static surface => surface,
            comparer: StringComparer.Ordinal
        )) {
    ...
    private static string ReferenceToken(string absolute) {
        var separator = absolute.IndexOf(
            comparisonType: StringComparison.Ordinal,
            value: " — "
        );
        var plain = absolute.IndexOf(
            comparisonType: StringComparison.Ordinal,
            value: " - "
        );
    ...
    public static IReadOnlyList<string> DeclarationViolations(IReadOnlyList<LegRow> rows) {
        ...
        foreach (var row in rows) {
            ...
            for (var i = 0; (i < row.Legs.Count); ++i) {
                var leg = row.Legs[i];
                var where = $"{row.Statement} leg {(i + 1)} ({leg.KindToken}{((leg.FlavorToken.Length > 0)
                    ? (":" + leg.FlavorToken)
                    : "")})";
                ...
                if (
                    leg.IsAgreement &&
                    string.IsNullOrWhiteSpace(value: leg.Against)
                ) {
                    violations.Add(item: $"{where}: an agreement leg names nothing to stand against.");
                }

                if (
                    (leg.Kind == LegKind.SharedSubstrate) &&
                    string.IsNullOrWhiteSpace(value: leg.Shared)
                ) {
                    violations.Add(item: $"{where}: a shared-substrate leg must name what the two sides share.");
                }

                if (
                    (leg.Flavor is ShareFlavor.DelegationTwin or ShareFlavor.SharedExactKernel) &&
                    string.IsNullOrWhiteSpace(value: leg.Citation)
                ) {
                ...
                if (
                    (leg.Kind == LegKind.InTreeIndependent) &&
                    string.IsNullOrWhiteSpace(value: leg.Citation)
                ) {
                ...
                if (
                    leg.IsRelativeCanary &&
                    string.IsNullOrWhiteSpace(value: leg.Absolute)
                ) {
                ...
                if (
                    leg.IsDocGap &&
                    string.IsNullOrWhiteSpace(value: leg.Documented)
                ) {
    ...
    public static IReadOnlyList<LegRow> LawRows() =>
        [.. LawRegistry.All
            .OrderBy(
                keySelector: static lawCase => lawCase.Id,
                comparer: StringComparer.Ordinal
            )
            .Select(selector: static lawCase => new LegRow(
                Surface: $"law: {lawCase.Tier}",
                Statement: lawCase.Id,
                Legs: lawCase.Legs
            ))];
    ...
    public static string Render(IReadOnlyList<LegRow> lawRows) {
        ...
        AppendDocGapRegister(
            builder: builder,
            rows: all
        );
        AppendTranscriptionRegister(
            builder: builder,
            rows: all
        );
        AppendCanaryRegister(
            builder: builder,
            rows: all
        );
        AppendStructuralRegister(
            builder: builder,
            rows: all
        );
    ...
    public static string? ShapeViolation(LawCase lawCase, int observed) {
        if (
            ...
            observed: observed,
            shape: LawShape.OracleAgreement
        )) &&
        ...
        if (
            LawShapes.Contains(
            observed: observed,
            shape: LawShape.Witnessed
        ) &&
            !lawCase.Legs.Any(predicate: static leg => leg.IsIndependent)
        ) {
        ...
        if (
            LawShapes.Contains(
            observed: observed,
            shape: LawShape.Divergence
        ) &&
            !lawCase.Legs.Any(predicate: static leg => leg.IsRelativeCanary)
        ) {
