...
internal static class FixedSaturateClaims {
    ...
    private static string? NarrowsTheSeams() {
        ...
        var saturatedHigh = FixedSaturate.Add(
            left: FixedQ4816.MaxValue,
            right: FixedQ4816.Epsilon
        ).Value;
        var saturatedLow = FixedSaturate.Add(
            left: FixedQ4816.MinValue,
    ...
    public static string? ClampsToTheExtremes(long[] left, long[] right) {
        for (var lane = 0; (lane < left.Length); ++lane) {
            var sum = FixedSaturate.Add(
                left: FixedQ4816.FromRawBits(value: left[lane]),
                right: FixedQ4816.FromRawBits(value: right[lane])
            ).Value;
