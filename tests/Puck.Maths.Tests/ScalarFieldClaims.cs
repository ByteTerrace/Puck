...
internal static class ScalarFieldClaims {
    ...
    private static int JacobiRecurse(BigInteger value, BigInteger modulus) {
        ...
        var twoFactorSign = (((1 == (twoExponent & 1)) && ((3 == modulusMod8) || (5 == modulusMod8)))
            ? -1
            : 1
        );
        var reciprocitySign = (((((int)(oddPart % 4)) == 3) && (((int)(modulus % 4)) == 3))
            ? -1
            : 1
        );
        var nextValue = (modulus % oddPart);

        return ((twoFactorSign * reciprocitySign) * JacobiRecurse(
            modulus: oddPart,
            value: nextValue
        ));
    }
    ...
    public static string? BinaryIntegerWideCarrierSurface() {
        ...
        for (var trial = 0; (trial < 30_000); ++trial) {
            ...
            var expectedPair = BitwisePairReference(
                other: high,
                value: low
            );
        ...
        var expectedDeepPower = ((UInt128)BigInteger.ModPow(
            value: ((BigInteger)deepBase),
            exponent: deepExponent,
            modulus: (BigInteger.One << 128)
        ));
    ...
    public static string? BinaryPolynomialCrc32PublishedVectorSurface() {
        ...
        foreach (var character in "123456789") {
            ...
            for (var bit = 7; (0 <= bit); --bit) {
                var term = ((1 == ((reflectedByte >>> bit) & 1))
                    ? monomial
    ...
    public static string? ConversionSeamsDoNotDependOnTheHost() {
        ...
        var surd = RealQuadratic.Create(
            denominator: 3,
            radicand: 2,
            rationalNumerator: -1234567890,
            surdNumerator: 1
        );
    ...
    public static string? JacobiSymbolFixedWidthVsExactDescentSurface() {
        ...
        var multiplicativityRng = Pcg32XshRr.Create(
            state: 0x9F1D2B6A5C3E7081UL,
            stream: 11UL
        );
        ...
        var bigRng = Pcg32XshRr.Create(
            state: 0x2E8B1F4C9A5D3067UL,
            stream: 13UL
        );
        ...
        for (var trial = 0; (trial < 20_000); ++trial) {
            ...
            var ja = NumberTheoryFunctions.JacobiSymbol(
                denominator: modulus,
                numerator: a
            );
            var jb = NumberTheoryFunctions.JacobiSymbol(
                denominator: modulus,
                numerator: b
            );
            var product = ((((a * b) % modulus) + modulus) % modulus);
            var jab = NumberTheoryFunctions.JacobiSymbol(
                denominator: modulus,
                numerator: product
            );
            ...
            var jShiftedDown = NumberTheoryFunctions.JacobiSymbol(
                denominator: modulus,
                numerator: (a - modulus)
            );
            var jShiftedUp = NumberTheoryFunctions.JacobiSymbol(
                denominator: modulus,
                numerator: (a + modulus)
            );
        ...
        var crossRng = Pcg32XshRr.Create(
            state: 0x7C6A3E1B8F2D5049UL,
            stream: 17UL
        );
        ...
        for (var trial = 0; (trial < 30_000); ++trial) {
            ...
            var expected = JacobiSymbolReferenceDescent(
                denominator: modulusRaw,
                numerator: valueRaw
            );
            ...
            var bigResult = NumberTheoryFunctions.JacobiSymbol(
                denominator: modulusRaw,
                numerator: valueRaw
            );
            ...
            if (
                (uintResult != expected) ||
                (ulongResult != expected) ||
                (uint128Result != expected) ||
                (bigResult != expected)
            ) {
        ...
        var wideRng = Pcg32XshRr.Create(
            state: 0x35E0A9C4712F68D3UL,
            stream: 19UL
        );
        ...
        for (var trial = 0; (trial < 20_000); ++trial) {
            ...
            var expected64 = JacobiSymbolReferenceDescent(
                denominator: modulus64,
                numerator: value64
            );
            ...
            var expected128 = JacobiSymbolReferenceDescent(
                denominator: ((BigInteger)modulus128),
                numerator: ((BigInteger)value128)
            );
        ...
        foreach (var (value, modulus) in boundaryVectors) {
            ...
            var actualBig = NumberTheoryFunctions.JacobiSymbol(
                denominator: modulus,
                numerator: value
            );
        ...
        foreach (var denominator in new BigInteger[] { 0, -1, -7, 4, -4 }) {
            var refusal = Assert.Throws<ArgumentOutOfRangeException>(testCode: () => _ = NumberTheoryFunctions.JacobiSymbol(
