...
internal static class LatticeClaims {
    ...
    private static Oracles.Enclosure MultiplyByEnclosure(BigInteger scalar, Oracles.Enclosure positive) {
        var low = ((scalar.Sign >= 0)
            ? (scalar * positive.Low)
            : (scalar * positive.High)
        );
        var high = ((scalar.Sign >= 0)
            ? (scalar * positive.High)
            : (scalar * positive.Low)
        );

        return Oracles.Rescale(
            value: new Oracles.Enclosure(
                High: high,
                Low: low
            ),
            fromBitCount: (16 + GeometryScaleBitCount),
    ...
    private static Oracles.Enclosure SurdEnclosure(BigInteger rationalNumerator, BigInteger surdNumerator, BigInteger radicand, BigInteger denominator) {
        ...
        return new Oracles.Enclosure(
            Low: (lowNumerator / divisor),
            High: (((highNumerator + divisor) - BigInteger.One) / divisor)
        );
    }
    ...
    private static bool WithinRelativeBand(BigInteger actualNumerator, BigInteger actualDenominator, Oracles.Enclosure expected, int toleranceDenominator) {
        ...
        return (
            (scaledActual >= lowBound) &&
            (scaledActual <= highBound)
        );
    }
    ...
    public static string? HexagonalCoordinateAlgebraicStructureSurface() {
        for (var direction = 0; (direction < HexagonalCoordinate.NeighborCount); ++direction) {
            Assert.Equal(
                expected: 1,
                actual: HexagonalCoordinate.Direction(direction: direction).Length
            );

            for (var other = (direction + 1); (other < HexagonalCoordinate.NeighborCount); ++other) {
                Assert.NotEqual(
                    expected: HexagonalCoordinate.Direction(direction: direction),
                    actual: HexagonalCoordinate.Direction(direction: other)
                );
            }
        }

        var omega = new HexagonalCoordinate(
            Q: 0,
            R: 1
        );

        Assert.Equal(
            expected: HexagonalCoordinate.AdditiveIdentity,
            actual: (((omega * omega) + omega) + HexagonalCoordinate.MultiplicativeIdentity)
        );

        for (var q = -24; (q <= 24); ++q) {
            for (var r = -24; (r <= 24); ++r) {
                var hex = new HexagonalCoordinate(
                    Q: q,
                    R: r
                );
                ...
                Assert.Equal(
                    expected: hex.Length,
                    actual: rotatedLeft.Length
                );
                Assert.Equal(
                    expected: hex.Norm,
                    actual: rotatedLeft.Norm
                );
                Assert.Equal(
                    expected: hex,
                    actual: rotatedLeft.RotatedRight()
                );
                ...
                var scaled = new HexagonalCoordinate(
                    Q: (q % 6),
                    R: (r % 6)
                );

                Assert.Equal(
                    actual: ((hex * omega) * scaled),
                    expected: (hex * (omega * scaled))
                );
            }
        }
            ...
            for (var stepR = -80; (stepR <= 80); ++stepR) {
                ...
                for (var a = -WindowRadius; (a <= WindowRadius); ++a) {
                    for (var b = -WindowRadius; (b <= WindowRadius); ++b) {
                        var candidate = HexagonalNearestCellMetric(
                            candidateQ: a,
                            candidateR: b,
                            qRaw: qRaw,
                            rRaw: rRaw,
                ...
                var rounded = HexagonalCoordinate.Round(
                    q: FixedQ4816.FromRawBits(value: qRaw),
                    r: FixedQ4816.FromRawBits(value: rRaw)
                );
                var roundedDistance = HexagonalNearestCellMetric(
                    candidateQ: rounded.Q,
                    candidateR: rounded.R,
                    qRaw: qRaw,
                    rRaw: rRaw,
                    unit: Unit
                );
    ...
    public static string? HilbertCurveExhaustiveBijectionSurface() {
        for (var order = 1; (order <= 9); ++order) {
            ...
            for (var distance = 0UL; (distance < cells); ++distance) {
                ...
                Assert.False(
                    condition: seen[cell],
                    userMessage: $"order {order}: distance {distance} decodes to cell ({point.X},{point.Y}), already visited — Decode is not a bijection"
                );
        ...
        for (var order = 10; (order <= 31); ++order) {
            foreach (var x in HilbertOrderPatterns(order: order)) {
                foreach (var y in HilbertOrderPatterns(order: order)) {
                    var distance = HilbertCurve.Encode(
                        order: order,
                        x: x,
                        y: y
                    );
    ...
    public static string? HilbertCurveRefusesOutsideItsDomain() {
        (string Name, Action Build)[] refusals = [
            ("an order of zero", static () => _ = HilbertCurve.Encode(
                order: 0,
                x: 0,
                y: 0
            )),
            ("a negative order", static () => _ = HilbertCurve.Encode(
                order: -1,
                x: 0,
                y: 0
            )),
            ("an order past thirty-one, where the shift count masks", static () => _ = HilbertCurve.Decode(
                distance: 12345,
                order: 32
            )),
            ("an order of sixty-four, which masks to zero", static () => _ = HilbertCurve.Decode(
                distance: 1,
                order: 64
            )),
            ("an x coordinate at the grid side", static () => _ = HilbertCurve.Encode(
                order: 1,
                x: 2,
                y: 0
            )),
            ("a y coordinate at the grid side", static () => _ = HilbertCurve.Encode(
                order: 1,
                x: 0,
                y: 2
            )),
            ("a coordinate far past the grid side", static () => _ = HilbertCurve.Encode(
                order: 4,
                x: uint.MaxValue,
                y: 0
            )),
            ("a distance at the curve length", static () => _ = HilbertCurve.Decode(
                distance: 4,
                order: 1
            )),
            ("a distance far past the curve length", static () => _ = HilbertCurve.Decode(
        ...
        _ = HilbertCurve.Encode(
            order: 1,
            x: 1,
            y: 1
        );
        _ = HilbertCurve.Decode(
            distance: 3,
            order: 1
        );
        _ = HilbertCurve.Encode(
    ...
    public static string? RayCycleFactorsAreNotWritableByConsumers() {
        ...
        Assert.True(
            condition: (count > 0),
            userMessage: "the ray-cycle factor surface reports nothing to protect"
        );
        ...
        foreach (var member in typeof(SymmetryLattice).GetProperties()) {
            ...
            if (
                type.IsGenericType &&
                ((type.GetGenericTypeDefinition() == typeof(ImmutableArray<>)) || (type.GetGenericTypeDefinition() == typeof(ReadOnlyMemory<>)) || (type.GetGenericTypeDefinition() == typeof(Memory<>)))
            ) {
    ...
    public static string? SymmetryLatticeExactStructureSurface() {
        Assert.Equal(
            actual: SymmetryLattice.RingSize,
            expected: CyclicRotation.Period
        );
        Assert.Equal(
            actual: SymmetryLattice.RayCount,
            expected: 120
        );
        Assert.Equal(
            actual: SymmetryLattice.RayCycleOrder,
            expected: 15
        );
        ...
        Assert.Equal(
            expected: (1UL << rayCycleOrder) | 1UL,
            actual: rayFactorProduct.Bits
        );

        foreach (var invalidNode in new[] { -1, SymmetryLattice.NodeCount, 268_435_456 }) {
            Assert.Equal(
                expected: "node",
                actual: Assert.Throws<ArgumentOutOfRangeException>(testCode: () => SymmetryLattice.Reflect(
                    mirror: 0,
                    node: invalidNode
                )).ParamName
            );
            ...
            Assert.Equal(
                expected: "node",
                actual: Assert.Throws<ArgumentOutOfRangeException>(testCode: () => SymmetryLattice.Cycle(node: invalidNode)).ParamName
            );
            Assert.Equal(
                expected: "node",
                actual: Assert.Throws<ArgumentOutOfRangeException>(testCode: () => SymmetryLattice.Cycle(
                    node: invalidNode,
                    steps: 3L
                )).ParamName
            );
            Assert.Equal(
                expected: "node",
                actual: Assert.Throws<ArgumentOutOfRangeException>(testCode: () => SymmetryLattice.Ring(node: invalidNode)).ParamName
            );
            Assert.Equal(
                expected: "node",
                actual: Assert.Throws<ArgumentOutOfRangeException>(testCode: () => SymmetryLattice.Project(node: invalidNode)).ParamName
            );
            Assert.Equal(
                expected: "node",
                actual: Assert.Throws<ArgumentOutOfRangeException>(testCode: () => SymmetryLattice.Antipode(node: invalidNode)).ParamName
            );
            Assert.Equal(
                expected: "node",
                actual: Assert.Throws<ArgumentOutOfRangeException>(testCode: () => SymmetryLattice.CanonicalRay(node: invalidNode)).ParamName
            );
            Assert.Equal(
                expected: "first",
                actual: Assert.Throws<ArgumentOutOfRangeException>(testCode: () => SymmetryLattice.AreOrthogonal(
                    first: invalidNode,
                    second: 0
                )).ParamName
            );
        ...
        for (var node = 0; (node < SymmetryLattice.NodeCount); ++node) {
            Assert.Equal(
                expected: SymmetryLattice.Ring(node: node),
                actual: SymmetryLattice.Ring(node: SymmetryLattice.Cycle(node: node))
            );
            ...
            Assert.Equal(
                expected: node,
                actual: SymmetryLattice.Cycle(
                    node: node,
                    steps: 0L
                )
            );
            Assert.Equal(
                expected: SymmetryLattice.Cycle(node: node),
                actual: SymmetryLattice.Cycle(
                    node: node,
                    steps: 1L
                )
            ...
            Assert.Equal(
                ...
                actual: SymmetryLattice.Cycle(
                    node: node,
                    steps: SymmetryLattice.RingSize
                )
            );
            Assert.Equal(
                expected: node,
                actual: SymmetryLattice.Cycle(
                    node: node,
                    steps: (-7L * SymmetryLattice.RingSize)
                )
            );
            Assert.Equal(
                expected: node,
                actual: SymmetryLattice.Cycle(
                    node: SymmetryLattice.Cycle(
                        node: node,
                        steps: -4L
                    ),
                    steps: 4L
            ...
            for (var count = 1; (count <= SymmetryLattice.RingSize); ++count) {
                ...
                Assert.Equal(
                    ...
                    actual: SymmetryLattice.Cycle(
                        node: node,
                        steps: (count + (2L * SymmetryLattice.RingSize))
                    )
                );
            }
            ...
            for (var mirror = 0; (mirror < SymmetryLattice.NodeCount); ++mirror) {
                Assert.Equal(
                    expected: node,
                    actual: SymmetryLattice.Reflect(
                ...
                Assert.Equal(
                    expected: (SymmetryLattice.Reflect(
                        mirror: mirror,
                        node: node
                    ) == node),
                    actual: SymmetryLattice.AreOrthogonal(
                        first: node,
                        second: mirror
                    )
                );
            ...
            Assert.Equal(
                expected: opposite,
                actual: SymmetryLattice.Antipode(node: node)
            );
            Assert.Equal(
                expected: SymmetryLattice.CanonicalRay(node: opposite),
                actual: SymmetryLattice.CanonicalRay(node: node)
            );
        }

        for (var ring = 0; (ring < SymmetryLattice.RingCount); ++ring) {
            Assert.Equal(
                expected: SymmetryLattice.RingSize,
                actual: ringSizes[ring]
            );
        }
        ...
        for (var cursor = SymmetryLattice.Cycle(node: 0); (cursor != 0); cursor = SymmetryLattice.Cycle(node: cursor)) {
            ...
            Assert.True(
                condition: (cycleOrbit <= SymmetryLattice.RingSize),
                userMessage: "the cycle from node 0 did not close within RingSize steps"
            );
        }
        ...
        Assert.Equal(
            actual: reachedCount,
            expected: SymmetryLattice.NodeCount
        );
        ...
        var cosPi30 = Oracles.EncloseSinCos(
            raw: PiFractionRaw(
                denominator: 30,
                numerator: 1
            ),
        ...
        var cos7Pi30 = Oracles.EncloseSinCos(
            raw: PiFractionRaw(
                denominator: 30,
                numerator: 7
            ),
        ...
        var cos2Pi15 = Oracles.EncloseSinCos(
            raw: PiFractionRaw(
                denominator: 30,
                numerator: 4
            ),
        ...
        var cosPi5 = Oracles.EncloseSinCos(
            raw: PiFractionRaw(
                denominator: 30,
                numerator: 6
            ),
        ...
        var golden = SurdEnclosure(
            rationalNumerator: BigInteger.One,
            surdNumerator: BigInteger.One,
            radicand: 5,
            denominator: 2
        );
        var goldenSquared = MultiplyPositive(
            left: golden,
            right: golden
        );
        var identity = new Oracles.Enclosure(
            Low: (BigInteger.One << GeometryScaleBitCount),
            High: (BigInteger.One << GeometryScaleBitCount)
        );
        ...
        Array.Fill(
            array: ringMinimumSquaredRadius,
            value: (BigInteger.One << 200)
        );
        Array.Fill(
            array: ringMaximumSquaredRadius,
        ...
        for (var node = 0; (node < SymmetryLattice.NodeCount); ++node) {
            ...
            var predictedX = AddEnclosures(
                left: MultiplyByEnclosure(
                    positive: turn.Cos,
                    scalar: x
                ),
                right: MultiplyByEnclosure(
                    positive: turn.Sin,
                    scalar: y
                )
            );
            var predictedY = SubtractEnclosures(
                left: MultiplyByEnclosure(
                    positive: turn.Cos,
                    scalar: y
                ),
            ...
            Assert.True(
                condition: ((actualX >= (predictedX.Low - turnToleranceScaled)) && (actualX <= (predictedX.High + turnToleranceScaled))),
                userMessage: $"SYMMETRY LATTICE CYCLE STEP AT NODE {node} DID NOT TURN TWELVE DEGREES (real part)"
            );
            Assert.True(
                condition: ((actualY >= (predictedY.Low - turnToleranceScaled)) && (actualY <= (predictedY.High + turnToleranceScaled))),
                userMessage: $"SYMMETRY LATTICE CYCLE STEP AT NODE {node} DID NOT TURN TWELVE DEGREES (imaginary part)"
            );
        }
        ...
        for (var ring = 0; (ring < SymmetryLattice.RingCount); ++ring) {
            ...
            Assert.True(
                condition: ((maximumRadius - minimumRadius) <= ConcentricityToleranceRawTicks),
                userMessage: $"ring {ring} is not concentric within {ConcentricityToleranceRawTicks} raw ticks (floor radii {minimumRadius} to {maximumRadius})"
            );
        }
        ...
        for (var i = 1; (i < SymmetryLattice.RingCount); ++i) {
            ...
            while (
                (j >= 0) &&
                (ringMinimumSquaredRadius[ringOrder[j]] > ringMinimumSquaredRadius[key])
            ) {
        ...
        for (var i = 0; (i < SymmetryLattice.RingCount); ++i) {
            ...
            var expectedRatioSquared = MultiplyPositive(
                left: massSpectrum[i],
                right: massSpectrum[i]
            );

            Assert.True(
                condition: WithinRelativeBand(
                    actualNumerator: ringMinimumSquaredRadius[ring],
                    actualDenominator: baseSquaredRadius,
                    expected: expectedRatioSquared,
                    toleranceDenominator: RelativeToleranceDenominator
                ),
        ...
        for (var inner = 0; (inner < SymmetryLattice.RingCount); ++inner) {
            for (var outer = (inner + 1); (outer < SymmetryLattice.RingCount); ++outer) {
                if (WithinRelativeBand(
                    actualNumerator: ringMinimumSquaredRadius[ringOrder[outer]],
                    actualDenominator: ringMinimumSquaredRadius[ringOrder[inner]],
                    expected: goldenSquared,
                    toleranceDenominator: RelativeToleranceDenominator
                )) {
                    ++goldenPairs;
                }
        ...
        Assert.Equal(
            actual: goldenPairs,
            expected: (SymmetryLattice.RingCount / 2)
        );
    ...
    public static string? SymmetryWordAndPairingSurface() {
        ...
        Assert.Equal(
            ...
            expected: SymmetryLattice.Dimension
        );
        Assert.Equal(
            expected: "mirrors",
            actual: Assert.Throws<ArgumentOutOfRangeException>(testCode: () => SymmetryWord.Create(mirrors: ReadOnlySpan<int>.Empty)).ParamName
        );
        Assert.Equal(
            expected: "mirrors",
            actual: Assert.Throws<ArgumentOutOfRangeException>(testCode: () => SymmetryWord.Create(mirrors: new int[(maximumLength + 1)])).ParamName
        );
        Assert.Equal(
            expected: "mirrors",
            actual: Assert.Throws<ArgumentOutOfRangeException>(testCode: () => SymmetryWord.Create(mirrors: [0, nodeCount])).ParamName
        );
        Assert.Equal(
            expected: "mirrors",
            actual: Assert.Throws<ArgumentOutOfRangeException>(testCode: () => SymmetryWord.Create(mirrors: [-1])).ParamName
        );
        Assert.Equal(
            expected: "order",
            actual: Assert.Throws<ArgumentOutOfRangeException>(testCode: () => CyclicRotation.Rotor(
                order: 0,
                step: 0L
            )).ParamName
        );
        Assert.Equal(
            expected: "ring",
            actual: Assert.Throws<ArgumentOutOfRangeException>(testCode: () => SymmetryLattice.RingNode(
                position: 0L,
        ...
        foreach (var invalidNode in new[] { -1, nodeCount, 268_435_456 }) {
            Assert.Equal(
                expected: "node",
                actual: Assert.Throws<ArgumentOutOfRangeException>(testCode: () => SymmetryWord.Coxeter.Apply(node: invalidNode)).ParamName
            );
            Assert.Equal(
                expected: "node",
                actual: Assert.Throws<ArgumentOutOfRangeException>(testCode: () => SymmetryWord.Coxeter.Apply(
                    node: invalidNode,
                    steps: 2L
                )).ParamName
            );
            Assert.Equal(
                expected: "node",
                actual: Assert.Throws<ArgumentOutOfRangeException>(testCode: () => SymmetryWord.Coxeter.OrbitLength(node: invalidNode)).ParamName
            );
            Assert.Equal(
                expected: "first",
                actual: Assert.Throws<ArgumentOutOfRangeException>(testCode: () => SymmetryLattice.InnerProduct(
                    first: invalidNode,
                    second: 0
                )).ParamName
            );
        ...
        Assert.Equal(
            expected: new[] { 7, 6, 5, 4, 3, 2, 1, 0 },
            actual: coxeter.Mirrors.ToArray()
        );
        Assert.Equal(
        ...
        for (var node = 0; (node < nodeCount); ++node) {
            Assert.Equal(
                expected: SymmetryLattice.Cycle(node: node),
                actual: coxeter.Apply(node: node)
            );
            Assert.Equal(
                expected: SymmetryLattice.RingSize,
                actual: coxeter.OrbitLength(node: node)
            );

            for (var steps = -40L; (steps <= 70L); ++steps) {
                Assert.Equal(
                    expected: SymmetryLattice.Cycle(
                        node: node,
                        steps: steps
                    ),
                    actual: coxeter.Apply(
                        node: node,
                        steps: steps
                    )
                );
        ...
        foreach (var letters in words) {
            ...
            for (var node = 0; (node < nodeCount); ++node) {
                ...
                Assert.False(
                    condition: seen[image],
                    userMessage: $"word [{string.Join(
                        separator: ',',
                        values: letters
                    )}] carries two nodes to {image}"
                );
                seen[image] = true;
                ...
                Assert.Equal(
                    expected: orbitLength,
                    actual: word.OrbitLength(node: node)
                );
                Assert.Equal(
                    expected: 0,
                    actual: (word.Order % orbitLength)
                );
            ...
            Assert.Equal(
                expected: measuredOrder,
                actual: word.Order
            );
            Assert.Equal(
                expected: (word.Order == 1),
                actual: word.IsIdentity
            );
            Assert.Contains(
                expected: word.Order,
                collection: WeylE8ElementOrders
            );
        }
        ...
        Assert.Equal(
            expected: 2,
            actual: SymmetryWord.Create(mirrors: [0, 1]).Order
        );
        Assert.Equal(
            expected: 3,
            actual: SymmetryWord.Create(mirrors: [0, 2]).Order
        );
        ...
        foreach (var pair in new int[][] { [0, 1], [0, 2], [5, 17], [40, 12], [101, 239] }) {
            ...
            Assert.Equal(
                expected: system.BondMatrix[1],
                actual: SymmetryWord.Create(mirrors: pair).Order
            );
        }
        ...
        for (var node = 0; (node < nodeCount); ++node) {
            ...
            Assert.Equal(
                expected: 2,
                actual: SymmetryLattice.InnerProduct(
                    first: node,
                    second: node
                )
            );
            Assert.Equal(
                ...
                actual: SymmetryLattice.InnerProduct(
                    first: node,
                    second: SymmetryLattice.Antipode(node: node)
                )
            );

            for (var other = 0; (other < nodeCount); ++other) {
                var product = SymmetryLattice.InnerProduct(
                    first: node,
                    second: other
                );
                ...
                Assert.Equal(
                    ...
                    actual: SymmetryLattice.InnerProduct(
                        ...
                        second: SymmetryLattice.Reflect(
                            mirror: 0,
                            node: other
                        )
                    )
        ...
        for (var ring = 0; (ring < SymmetryLattice.RingCount); ++ring) {
            ...
            for (var node = 0; (node < nodeCount); ++node) {
                if (
                    (lowest < 0) &&
                    (SymmetryLattice.Ring(node: node) == ring)
                ) { lowest = node; }
            }

            Assert.Equal(
                expected: lowest,
                actual: SymmetryLattice.RingNode(
                    position: 0L,
                    ring: ring
                )
            );
            Assert.Equal(
                expected: lowest,
                actual: SymmetryLattice.RingNode(
                    position: SymmetryLattice.RingSize,
                    ring: ring
                )
            );
            Assert.Equal(
                expected: lowest,
                actual: SymmetryLattice.RingNode(
                    position: (-4L * SymmetryLattice.RingSize),
                    ring: ring
                )
            );

            for (var position = 0; (position < SymmetryLattice.RingSize); ++position) {
                var node = SymmetryLattice.RingNode(
                    position: position,
                    ring: ring
                );
                ...
                Assert.Equal(
                    expected: ring,
                    actual: SymmetryLattice.Ring(node: node)
                );
                ...
                Assert.Equal(
                    expected: node,
                    actual: SymmetryLattice.RingNode(
                        position: (position - SymmetryLattice.RingSize),
                        ring: ring
                    )
                );
            }
        ...
        for (var step = -60L; (step <= 90L); ++step) {
            ...
            Assert.Equal(
                expected: baked.Real.Value,
                actual: CyclicRotation.Rotor(
                    order: CyclicRotation.Period,
                    step: step
                ).Real.Value
            );
            Assert.Equal(
                expected: baked.Imaginary.Value,
                actual: CyclicRotation.Rotor(
                    order: CyclicRotation.Period,
                    step: step
                ).Imaginary.Value
            ...
            Assert.Equal(
                expected: identity.Imaginary.Value,
                actual: CyclicRotation.Rotor(
                    order: 1,
                    step: step
                ).Imaginary.Value
        ...
        foreach (var order in new[] { 1, 2, 3, 5, 6, 10, 15, 30 }) {
            ...
            for (var step = -(2L * order); (step <= (3L * order)); ++step) {
                ...
                var actual = CyclicRotation.Rotor(
                    order: order,
                    step: step
                );

                Assert.Equal(
                    expected: expected.Real.Value,
                    actual: actual.Real.Value
                );
                Assert.Equal(
                    expected: expected.Imaginary.Value,
                    actual: actual.Imaginary.Value
                );
            }
        }

        foreach (var order in new[] { 4, 7, 8, 9, 12, 14, 18, 20, 24 }) {
            var first = CyclicRotation.Rotor(
                order: order,
                step: 0L
            );
            ...
            Assert.Equal(
                expected: identity.Real.Value,
                actual: first.Real.Value
            );
            Assert.Equal(
                expected: identity.Imaginary.Value,
                actual: first.Imaginary.Value
            );

            for (var step = 0L; (step < order); ++step) {
                var rotor = CyclicRotation.Rotor(
                    order: order,
                    step: step
                );
                var wrapped = CyclicRotation.Rotor(
                    order: order,
                    step: (step + (5L * order))
                );

                Assert.Equal(
                    expected: rotor.Real.Value,
                    actual: wrapped.Real.Value
                );
                Assert.Equal(
                    expected: rotor.Imaginary.Value,
                    actual: wrapped.Imaginary.Value
                );
            }
        }
