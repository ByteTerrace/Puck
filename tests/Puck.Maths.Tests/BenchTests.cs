...
public sealed class BenchTests {
    ...
    private static List<double> MeasureRatios(int samples) {
        ...
        for (var sample = 0; (sample < samples); ++sample) {
            var hand = Bench.BestNsPerOp(
                ops: Iterations,
                runs: 9,
                loop: () => BenchLoops.ComplexHand(
                    iterations: Iterations,
                    rotation: rotation,
                    seed: seed
                )
            );
            var generic = Bench.BestNsPerOp(
                ops: Iterations,
                runs: 9,
                loop: () => BenchLoops.ComplexGeneric(
                    algebra: algebra,
                    iterations: Iterations,
                    seed: elementSeed,
                    step: elementStep
    ...
    public void ComplexMultiplyRatioWithinBaseline() {
        ...
        if (
            (machine is not null) &&
            ((calibration > (machine.CalibrationNs * 2.0)) || (calibration < (machine.CalibrationNs * 0.5)))
        ) {
            BenchState.Record(
                id: RatioBenchId,
                median: Bench.Median(values: MeasureRatios(samples: RatioSamples)),
                baselineMedian: machine.CalibrationNs,
                band: 0.0,
        ...
        if (entry is null) {
            ...
            if (machine is null) {
                ...
                model.Machines.Sort(comparison: static (left, right) => string.CompareOrdinal(
                    strA: left.Fingerprint,
                    strB: right.Fingerprint
                ));
            }
            ...
            machine.Benches.Sort(comparison: static (left, right) => string.CompareOrdinal(
                strA: left.Id,
                strB: right.Id
            ));
            ...
            _ = ArtifactJson.WriteIfChanged(
                path: path,
                content: ArtifactJson.Serialize(value: model)
            );
            BenchState.Record(
                band: 0.0,
        ...
        var madBand = ((entry.Runs.Count >= MinimumRunsForMadTrust)
            ? (3.0 * entry.Mad)
            : 0.0
        );
        var band = Math.Max(
            val1: (0.05 * entry.Median),
            val2: madBand
        );

        if (Breach(
            value: median,
            baseline: entry.Median,
            band: band
        )) {
            ...
            BenchState.Record(
                ...
                baselineMedian: entry.Median,
                band: band,
                status: "breach-first-informational"
            );
