...
internal static class ReedSolomonClaims {
                ...
                foreach (var count in CheckCounts) {
                    ...
                    foreach (var length in MessageLengths) {
                        ...
                        for (var index = 0; (index < count); ++index) {
                            var root = Oracles.BinaryFieldRepeatedProduct(
                                degree: degree,
                                exponent: (first + index),
                                reductionTail: tail,
                                value: rootBase
                            );
                            var expected = Oracles.BinaryFieldPolynomialValue(
                        ...
                        for (var index = 0; (index < count); ++index) {
                            ...
                            var expected = Oracles.BinaryFieldPolynomialValue(
                                coefficients: damagedCoefficients,
                                degree: degree,
                                point: root,
                                reductionTail: tail
                            );
            ...
            foreach (var first in FirstRootExponents) {
                foreach (var count in CheckCounts) {
                    ...
                    for (var index = 0; (index < count); ++index) {
                        ...
                        var value = Oracles.BinaryFieldPolynomialValue(
                            coefficients: coefficients,
                            degree: degree,
                            point: root,
                            reductionTail: tail
                    ...
                    var beyond = Oracles.BinaryFieldRepeatedProduct(
                        degree: degree,
                        exponent: (first + count),
                        reductionTail: tail,
                        value: rootBase
                    );

                    if (!roots.Contains(item: beyond)) {
                        var beyondValue = Oracles.BinaryFieldPolynomialValue(
                            coefficients: coefficients,
                            degree: degree,
                            point: beyond,
                            reductionTail: tail
    ...
    public static string? PublishedRemainderSurface() {
        ...
        var neighbour = BinaryField<byte>.Create(
            degree: 8,
            reductionTail: 0x1B
        );
        ...
        ReedSolomon.ComputeCheckSymbols(
            checkSymbols: neighbourProduced,
            field: neighbour,
            generator: neighbourGenerator,
            message: PublishedMessage
        );
    ...
    public static string? SurfaceRefusalsAndWideCarrierSurface() {
        ...
        var refusal = (Refuses(
            action: () => ReedSolomon.BuildGenerator(
                field: field,
                firstRootExponent: 0,
            ...
            what: "BuildGenerator with a one-coefficient generator"
        ) ??
                       (Refuses(
            action: () => ReedSolomon.BuildGenerator(
                field: field,
            ...
            type: typeof(ArgumentOutOfRangeException),
            parameterName: "generator",
            what: "BuildGenerator with an empty generator"
        ) ??
                       (Refuses(
            action: () => ReedSolomon.BuildGenerator(
                field: field,
                firstRootExponent: -1,
                generator: new byte[3],
                rootBase: ((byte)2)
            ),
            type: typeof(ArgumentOutOfRangeException),
            parameterName: "firstRootExponent",
            what: "BuildGenerator with a negative first root exponent"
        ) ??
                       (Refuses(
            action: () => ReedSolomon.ComputeCheckSymbols(
                checkSymbols: [],
                field: field,
                generator: new byte[1],
                message: []
            ...
            type: typeof(ArgumentOutOfRangeException),
            parameterName: "generator",
            what: "ComputeCheckSymbols with a one-coefficient generator"
        ) ??
                       (Refuses(
            action: () => ReedSolomon.ComputeCheckSymbols(
                checkSymbols: new byte[9],
                field: field,
                generator: generator,
                message: []
            ),
            type: typeof(ArgumentOutOfRangeException),
            parameterName: "checkSymbols",
            what: "ComputeCheckSymbols with a check span shorter than the generator's degree"
        ) ??
                       (Refuses(
            action: () => ReedSolomon.ComputeCheckSymbols(
                checkSymbols: new byte[2],
                field: field,
                generator: new byte[] { 2, 1, 1 },
                message: []
            ),
            type: typeof(ArgumentException),
            parameterName: "generator",
            what: "ComputeCheckSymbols with a non-monic generator"
        ) ??
                       (Refuses(
            action: () => ReedSolomon.ComputeSyndromes(
                codeword: [],
                field: field,
                firstRootExponent: -1,
                rootBase: ((byte)2),
                syndromes: new byte[1]
            ),
            type: typeof(ArgumentOutOfRangeException),
            parameterName: "firstRootExponent",
            what: "ComputeSyndromes with a negative first root exponent"
        ) ??
                       (Refuses(
            action: () => ReedSolomon.BuildGenerator(
                field: default,
            ...
            parameterName: null,
            what: "BuildGenerator on a default-initialized field"
        ) ??
                       (Refuses(
            action: () => ReedSolomon.ComputeCheckSymbols(
                checkSymbols: new byte[10],
                field: default,
                generator: generator,
                message: []
            ),
            type: typeof(InvalidOperationException),
            parameterName: null,
            what: "ComputeCheckSymbols on a default-initialized field"
        ) ??
                       (Refuses(
            action: () => ReedSolomon.ComputeSyndromes(
                codeword: [],
                ...
                firstRootExponent: 0,
                rootBase: ((byte)2),
                syndromes: new byte[2]
            ),
            type: typeof(InvalidOperationException),
            parameterName: null,
            what: "ComputeSyndromes on a default-initialized field"
        ) ??
                       Refuses(
            action: () => ReedSolomon.ComputeSyndromes(
                codeword: [],
            ...
            type: typeof(InvalidOperationException),
            parameterName: null,
            what: "ComputeSyndromes on a default-initialized field with no syndromes requested"
        )))))))))));
        ...
        var wideField = BinaryField<ushort>.Create(
            degree: WideDegree,
            reductionTail: WideTail
        );
        ...
        ReedSolomon.BuildGenerator(
            field: wideField,
            firstRootExponent: 0,
            generator: wideGenerator,
            rootBase: ((ushort)2)
        );
        ReedSolomon.ComputeCheckSymbols(
            checkSymbols: wideCheck,
            field: wideField,
            generator: wideGenerator,
            message: wideMessage
        );
        ...
        ReedSolomon.ComputeSyndromes(
            codeword: wideCodeword,
            field: wideField,
            firstRootExponent: 0,
            rootBase: ((ushort)2),
            syndromes: wideSyndromes
        );
