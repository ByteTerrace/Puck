...
internal static class PrimalityScaleClaims {
    ...
    internal static string? FactorizationFullWidthSurface() {
        ...
        for (var offset = 0U; (200_000U > offset); ++offset) {
            var failure = NarrowFactorizationHolds(
                destination: destination,
                trialPrimes: trialPrimes,
                value: ((uint.MaxValue - 200_000U) + offset)
            );
    ...
    internal static string? JacobiSymbolSurface() {
        ...
        for (var modulus = 1U; (401U >= modulus); modulus += 2U) {
            ...
            for (var numerator = 0U; (400U >= numerator); ++numerator) {
                ...
                var expected = Oracles.JacobiSymbolReciprocity(
                    denominator: wideModulus,
                ...
                var defined = JacobiSymbolByFactorAndEuler(
                    numerator: wideNumerator,
                    oddModulus: modulus
                );
                ...
                var arbitrary = NumberTheoryFunctions.JacobiSymbol(
                    denominator: wideModulus,
                    numerator: wideNumerator
                );
    ...
    internal static string? MontgomeryChainsSurface() {
        ...
        for (var index = 0; (index < fields.Length); ++index) {
            var failure = MontgomeryChainAtModulus(
                field: fields[index],
                state: ref state
            );
        ...
        for (var trial = 0; (trial < 20_000); ++trial) {
            ...
            if (field.Pow(
                ...
                return $"Pow({value}, {exponent}) mod {modulus} = {field.Pow(
                    exponent: exponent,
                    value: value
                )} where BigInteger.ModPow says {expected}";
            }
        }
        ...
        foreach (var witness in witnesses) {
            for (var value = 0UL; (20_000UL > value); ++value) {
                if (PrimeField64.IsStrongProbablePrime(
                    value: value,
                    witness: witness
                ) != Oracles.ModularStrongProbablePrime(
                    value: value,
                    witness: witness
                )) {
                    return $"IsStrongProbablePrime({value}, {witness}) = {PrimeField64.IsStrongProbablePrime(
                        value: value,
                        witness: witness
                    )} where the plain-residue round says {Oracles.ModularStrongProbablePrime(
                        value: value,
                        witness: witness
                    )}";
                }
    ...
    private static string? BailliePopulationSurface() {
        ...
        if (!foundBaseTwo.SequenceEqual(second: BaseTwoPseudoprimes)) {
            return $"the enumerated base-two strong pseudoprimes below 10^5 are {string.Join(
                separator: ",",
                values: foundBaseTwo
            )} and OEIS A001262 says {string.Join(
                separator: ",",
                values: BaseTwoPseudoprimes
            )}";
        }
        if (!foundLucas.SequenceEqual(second: LucasPseudoprimes)) {
            return $"the enumerated strong Lucas pseudoprimes below 10^5 are {string.Join(
                separator: ",",
                values: foundLucas
            )} and OEIS A217255 says {string.Join(
                separator: ",",
                values: LucasPseudoprimes
            )}";
        }
        if (foundBaseTwo.Intersect(second: foundLucas).Any()) {
            return $"the two pseudoprime populations below 10^5 are not disjoint: {string.Join(
                separator: ",",
                values: foundBaseTwo.Intersect(second: foundLucas)
            )}";
        }
        ...
        while (true) {
            ...
            if (
                (fibonacci.Current > (ulong.MaxValue - fibonacci.Previous)) ||
                (lucas.Current > (ulong.MaxValue - lucas.Previous))
            ) {
                break;
            }
        ...
        foreach (var term in ladder) {
            for (var delta = -2L; (2L >= delta); ++delta) {
                if (
                    ((0L > delta) && (term < ((ulong)(-delta)))) ||
                    ((0L < delta) && (term > (ulong.MaxValue - ((ulong)delta))))
                ) {
                    continue;
                }
    ...
    private static string? JacobiWideSurface() {
        ...
        for (var trial = 0; (trial < 20_000); ++trial) {
            ...
            if (
                (leftSymbol != (left + oddLeft).JacobiSymbol(modulus: oddLeft)) ||
        ...
        foreach (var modulus in boundaryModuli) {
            foreach (var value in boundaryValues) {
                var expected = Oracles.JacobiSymbolReciprocity(
                    numerator: new BigInteger(value: value),
        ...
        if (
            (1 != uint.MaxValue.JacobiSymbol(modulus: 1U)) ||
        ...
        for (var trial = 0; (trial < 20_000); ++trial) {
            ...
            if (narrowValue.JacobiSymbol(modulus: narrowModulus) != Oracles.JacobiSymbolReciprocity(
                denominator: narrowModulus,
                numerator: narrowValue
            )) {
                return $"the uint symbol ({narrowValue}/{narrowModulus}) disagrees with the reciprocity descent";
            }
            if (wideValue.JacobiSymbol(modulus: wideModulus) != Oracles.JacobiSymbolReciprocity(
                denominator: wideModulus,
                numerator: wideValue
            )) {
                return $"the ulong symbol ({wideValue}/{wideModulus}) disagrees with the reciprocity descent";
            }
            if (hugeValue.JacobiSymbol(modulus: hugeModulus) != Oracles.JacobiSymbolReciprocity(
                denominator: ((BigInteger)hugeModulus),
                numerator: ((BigInteger)hugeValue)
            )) {
                return $"the UInt128 symbol ({hugeValue}/{hugeModulus}) disagrees with the reciprocity descent";
            }
            if (NumberTheoryFunctions.JacobiSymbol(
                denominator: ((BigInteger)hugeModulus),
                numerator: ((BigInteger)hugeValue)
            ) != Oracles.JacobiSymbolReciprocity(
                denominator: ((BigInteger)hugeModulus),
                numerator: ((BigInteger)hugeValue)
            )) {
    ...
    private static int LegendreByEuler(BigInteger numerator, ulong prime) {
        ...
        return (BigInteger.ModPow(
            value: residue,
            exponent: ((widePrime - BigInteger.One) / 2),
            modulus: widePrime
        ).IsOne
            ? 1
            : -1
    ...
    private static string? MontgomeryChainAtModulus(PrimeField64 field, ref ulong state) {
        ...
        foreach (var residue in residues) {
            ...
            if (0UL != residue) {
                ...
                var expected = ((ulong)BigInteger.ModPow(
                    value: wideResidue,
                    exponent: new BigInteger(value: (modulus - 2UL)),
                    modulus: wideModulus
                ));
    ...
    private static string? MontgomeryPrimalitySurface(ref ulong state) {
        ...
        var window = SieveWindow(
            length: WindowLength,
            start: WindowStart,
            trialPrimes: windowPrimes
        );
    ...
    private static string? NarrowFactorizationHolds(uint value, Span<uint> destination, ReadOnlySpan<ulong> trialPrimes) {
        ...
        if (!destination[..count].SequenceEqual(other: enumerated)) {
            return $"Factorize and EnumeratePrimeFactors disagree at {value}: [{string.Join(
                separator: ",",
                values: destination[..count].ToArray()
            )}] against [{string.Join(
                separator: ",",
                values: enumerated
            )}]";
        ...
        if (2U > value) {
            return ((0 == count)
                ? null
                : $"{value} is below two and reported {count} factor(s)"
            );
        }
    ...
    private static bool[] SieveWindow(ulong start, int length, ReadOnlySpan<ulong> trialPrimes) {
        ...
        foreach (var prime in trialPrimes) {
            ...
            var first = ((start <= square)
                ? square
                : ((((start + prime) - 1UL) / prime) * prime)
            );
    ...
    private static string? WideFactorizationHolds(ulong value) {
        ...
        if (2UL > value) {
            return ((0 == factors.Length)
                ? null
                : $"{value} is below two and reported {factors.Length} factor(s)"
            );
        }
    ...
    private static string? WideFactorizationSurface(ref ulong state) {
        ...
        for (var trial = 0; (trial < 2_000); ++trial) {
            ...
            if (!expected.SequenceEqual(second: actual)) {
                return $"the semiprime {left} x {right} factored as [{string.Join(
                    separator: ",",
                    values: actual
                )}]";
            }
        ...
        (ulong Value, ulong[] Factors)[] vectors = [
            ...
            ((1UL << 63), [.. Enumerable.Repeat(
                    count: 63,
                    element: 2UL
                ),]),
            (10_000_000_000_000_000_000UL, [.. Enumerable.Repeat(
                    count: 19,
                    element: 2UL
                ), .. Enumerable.Repeat(
                    count: 19,
        ...
        foreach (var (value, factors) in vectors) {
            ...
            if (!factors.SequenceEqual(second: actual)) {
                return $"{value} factored as [{string.Join(
                    separator: ",",
                    values: actual
                )}] rather than [{string.Join(
                    separator: ",",
                    values: factors
                )}]";
            }
