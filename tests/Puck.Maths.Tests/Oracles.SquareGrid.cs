...
internal static partial class Oracles {
    ...
    public static BigInteger SquareRadius(BigInteger x, BigInteger y) => BigInteger.Max(
        left: BigInteger.Abs(value: x),
        right: BigInteger.Abs(value: y)
    );
    public static (BigInteger X, BigInteger Y) SquareCoordinateAt(BigInteger index) {
        ...
        while (low < high) {
            ...
            if (BigInteger.Pow(
                exponent: 2,
                value: ((2 * middle) + 1)
            ) > index) { high = middle; } else { low = (middle + 1); }
        }
        var offset = (index - BigInteger.Pow(
            exponent: 2,
            value: ((2 * low) - 1)
        ));
    ...
    public static BigInteger SquareIndexAt(BigInteger x, BigInteger y) {
        ...
        for (var edge = 0; (edge < 4); ++edge) {
            ...
            var position = (direction.X.IsZero
                ? ((y - (radius * corner.Y)) / direction.Y)
                : ((x - (radius * corner.X)) / direction.X)
            );

            if (
                (position > 0) &&
                (position <= (2 * radius)) &&
                (x == ((radius * corner.X) + (position * direction.X))) &&
                (y == ((radius * corner.Y) + (position * direction.Y)))
            ) {
                return (((BigInteger.Pow(
                    exponent: 2,
                    value: ((2 * radius) - 1)
                ) + ((2 * edge) * radius)) + position) - 1);
            }
        }
        throw new InvalidOperationException(message: "No square perimeter edge contains the coordinate.");
    }
