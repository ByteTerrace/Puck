...
| 2 | Oracles | `Oracles.cs` | Shared-nothing `BigInteger` reference arithmetic (dyadic round/wrap, quadratic product/norm, Möbius step). Never calls a subject kernel. |
| 3 | Laws | `laws/*.json`, `LawDeclarations.cs`, `LawRegistry.cs`, `Laws.cs`, `Subjects.cs`, `*Claims.cs` | Declarations are **data**: id, tier, covered members and leg prose live in `laws/<family>.json`, one file per family (several files may feed one family — the loader reads every `*.json` and keys by id). `LawRegistry.cs` holds only a `Case(id, run)` binding per law, `Laws.cs` the combinators, and the claim bodies sit in `Subjects.cs` and the `*Claims.cs` files. A declaration without a binding, or a binding without a declaration, fails `LawDeclarationTests` by name. |
