...
internal static partial class Subjects {
    public static string? TrigonometryTwiddles() {
        for (var length = 1; (length <= 1024); length *= 2) {
            ...
            for (var kind = 0; (kind < 2); ++kind) {
                var forward = ((kind == 0)
                    ? fourier.ForwardTwiddles
                    : cosine.ForwardTwiddles
                );
                var inverse = ((kind == 0)
                    ? fourier.InverseTwiddles
                    : cosine.InverseTwiddles
                );
                ...
                for (var k = 0; (k < forward.Length); ++k) {
                    ...
                    var divisor = new BigInteger(value: ((kind == 0)
                        ? length
                        : (4L * length)));
                    var phase = unchecked((ulong)((long)((-(new BigInteger(value: k) << 64)) / divisor)));
                    var expected = Oracles.EncloseSinCosTurns(
                        fractionalTurns: phase,
    ...
    public static string? TrigonometryConstants() {
        ...
        var reciprocalLow = Oracles.RoundRationalTiesToEven(
            numerator: numerator,
            denominator: (2 * pi.High)
        );
        var reciprocalHigh = Oracles.RoundRationalTiesToEven(
            numerator: numerator,
            denominator: (2 * pi.Low)
        );

        if (
            (reciprocalLow != reciprocalHigh) ||
            (reciprocalLow != ((BigInteger)FixedQ4816.SinCosInvTwoPiQ96))
        ) { return "Q96 reciprocal is not independently rounded 1/(2*pi)"; }

        for (var i = 0; (i <= 64); ++i) {
            var sine = Oracles.EncloseSinCosTurns(
                fractionalTurns: (((ulong)i) << 56),
                guardBitCount: 80
            ).Sin;
            var low = Oracles.RoundRationalTiesToEven(
                numerator: sine.Low,
                denominator: (BigInteger.One << 36)
            );
            ...
            if (
                (low != high) ||
                (low != FixedQ4816.SinCosTableQ60[i])
            ) { return $"quarter-wave entry {i} disagrees with series [{low}, {high}]"; }
        }
        ...
        foreach (var (actual, denominator) in ((ReadOnlySpan<(long, int)>)[
            ...
            if (actual != Oracles.RoundRationalTiesToEven(
                numerator: (BigInteger.One << 60),
                denominator: denominator
            )) { return $"residual coefficient 1/{denominator} is not correctly rounded"; }
        }
        var circleLow = Oracles.RoundRationalTiesToEven(
            numerator: (2 * pi.Low),
            denominator: (BigInteger.One << 324)
        );
        var circleHigh = Oracles.RoundRationalTiesToEven(
            numerator: (2 * pi.High),
            denominator: (BigInteger.One << 324)
        );

        return (((circleLow == circleHigh) && (circleLow == FixedQ4816.SinCosTwoPiQ60))
            ? null
            : "Q60 circle constant is not independently rounded"
        );
    }
    public static string? TrigonometryTurns(long[] left, long[] right) =>
        CheckTrigonometryTurns(
            phase: unchecked((ulong)left[0]),
            checkCore: false
        );
    ...
    private static string? CheckTrigonometryTurns(ulong phase, bool checkCore) {
        ...
        if (
            (mirror.Sin.Value != -actual.Sin.Value) ||
            (mirror.Cos != actual.Cos)
        ) { return $"turn symmetry at {phase}"; }

        if (checkCore) {
            var core = FixedQ4816.SinCosCore(fractionalTurns: unchecked((long)phase));
            var reference = Oracles.EncloseSinCosTurns(
                fractionalTurns: phase,
                guardBitCount: 80
            );
            var tolerance = ((6 * (BigInteger.One << 96)) / BigInteger.Pow(
                exponent: 15,
                value: 10
            ));
            ...
            foreach (var (value, bounds) in ((ReadOnlySpan<(long, Oracles.Enclosure)>)[(core.SinQ60, reference.Sin), (core.CosQ60, reference.Cos)])) {
                ...
                if (
                    (scaled < (bounds.Low - tolerance)) ||
                    (scaled > (bounds.High + tolerance))
                ) { return $"Gaussian Q60 core envelope at phase {phase}"; }
            }
        }
        return null;
    }
    ...
    public static string? TrigonometrySeams() {
        ...
        foreach (var raw in witnesses) {
            ...
            var reference = Oracles.EncloseSinCosScaled(
                fractionBitCount: 16,
                guardBitCount: Oracles.GuardBitCount,
                raw: unsigned
            );
