    ...
    private static class SignedFixedPointSubject<T, TAdapter>
        ...
        private static string? IntegralPartsAt(long raw) {
            ...
            if (
                (fraction.Sign < 0) ||
                (fraction >= OneRaw)
            ) { return $"the fractional part of {raw} left the unit interval"; }
            if ((ceiling - floor) != (fraction.IsZero
                ? BigInteger.Zero
                : OneRaw)) { return $"the ceiling is not the floor plus one whole unit at {raw}"; }
            if (truncated != ((raw < 0L)
                ? ceiling
                : floor)) { return $"the truncation of {raw} does not round toward zero"; }
            if (
                (rounded != floor) &&
                (rounded != ceiling)
            ) { return $"the rounding of {raw} landed off the integer grid"; }
        ...
        private static string? PredicatesAt(long raw) {
            ...
            var wholeUnits = Oracles.FloorQuotient(
                denominator: OneRaw,
                numerator: exact
            );
            ...
            if (
                T.IsEvenInteger(value: value) &&
                T.IsOddInteger(value: value)
        ...
        private static string? WholeNumberSeam(ReadOnlySpan<long> integerLadder, ReadOnlySpan<long> refusedIntegers) {
            ...
            foreach (var value in refusedIntegers) {
                if (!Throws<ArgumentOutOfRangeException>(
                    action: () => _ = TAdapter.FromInteger(value: value),
                    paramName: "value"
                )) {
        ...
        public static string? CheckedOpsRefuse(long[] left, long[] right) {
            ...
            if (CheckedOperator(
                checkedCall: () => checked((a + b)),
                exact: (exactA + exactB),
                name: "addition",
                rawA: rawA,
                rawB: rawB,
                uncheckedCall: () => (a + b)
            ) is { } addition) { return addition; }
            if (CheckedOperator(
                checkedCall: () => checked((a - b)),
                exact: (exactA - exactB),
                name: "subtraction",
                ...
                rawB: rawB,
                uncheckedCall: () => (a - b)
            ) is { } subtraction) { return subtraction; }
            if (CheckedOperator(
                checkedCall: () => checked(-a),
                exact: -exactA,
                name: "negation",
                rawA: rawA,
                rawB: rawB,
                uncheckedCall: () => (-a)
            ) is { } negation) { return negation; }
            if (CheckedOperator(
                name: "increment",
                rawA: rawA,
                rawB: rawB,
                exact: (exactA + OneRaw),
                checkedCall: () => { var value = a; return checked(++value); },
                uncheckedCall: () => { var value = a; return ++value; }
            ) is { } increment) { return increment; }
            if (CheckedOperator(
                name: "decrement",
                rawA: rawA,
                rawB: rawB,
                ...
                checkedCall: () => { var value = a; return checked(--value); },
                uncheckedCall: () => { var value = a; return --value; }
            ) is { } decrement) { return decrement; }
            if (CheckedOperator(
                name: "multiplication",
                rawA: rawA,
                rawB: rawB,
                exact: Oracles.RoundRationalTiesToEven(
                    denominator: OneRaw,
                    numerator: (exactA * exactB)
                ),
                checkedCall: () => checked((a * b)),
                uncheckedCall: () => (a * b)
            ) is { } multiplication) { return multiplication; }
            if (CheckedOperator(
                name: "division",
                rawA: rawA,
                rawB: rawB,
                exact: Oracles.RoundRationalTiesToEven(
                    denominator: exactB,
                    numerator: (exactA * OneRaw)
                ),
                checkedCall: () => checked((a / b)),
                uncheckedCall: () => (a / b)
            ) is { } division) { return division; }
        ...
        public static string? GridAndConstruction(
            ...
            (WholeNumberSeam(
                integerLadder: integerLadder,
                refusedIntegers: refusedIntegers
            ) ??
            (DoubleSeam(doubleLadder: doubleLadder) ??
            (includeDoubleProjection
                ? DoubleProjection(rawLadder: rawLadder)
                : null)))));
        ...
        public static string? LerpEndpointsAndOracle(long[] left, long[] right) {
            ...
            if (TAdapter.Raw(value: TAdapter.Lerp(
                amount: amount,
                from: from,
                to: to
            )) != expected) { return $"the interpolation of ({fromRaw}, {toRaw}) at {amountRaw} is wrong"; }
            if (TAdapter.Lerp(
                from: from,
                to: to,
                amount: T.Zero
            ) != from) { return $"the zero endpoint of ({fromRaw}, {toRaw}) is not exact"; }
            if (TAdapter.Lerp(
                from: from,
                to: to,
                amount: T.One
            ) != to) { return $"the unit endpoint of ({fromRaw}, {toRaw}) is not exact"; }
            if (TAdapter.Lerp(
                amount: amount,
                from: from,
                to: from
            ) != from) { return $"the degenerate segment at {fromRaw} moved under the amount {amountRaw}"; }
        ...
        public static string? MagnitudeSelectionExact(long[] left, long[] right) {
            ...
            var expectedMaximum = (((magnitudeA > magnitudeB) || ((magnitudeA == magnitudeB) && (exactA >= exactB)))
                ? a
                : b
            );
            var expectedMinimum = (((magnitudeA < magnitudeB) || ((magnitudeA == magnitudeB) && (exactA <= exactB)))
                ? a
                : b
            );
            ...
            if (T.MaxMagnitude(
                x: a,
                y: b
            ) != expectedMaximum) { return $"the greater magnitude of ({rawA}, {rawB}) is wrong"; }
            if (T.MinMagnitude(
                x: a,
                y: b
            ) != expectedMinimum) { return $"the lesser magnitude of ({rawA}, {rawB}) is wrong"; }
            if (T.MaxMagnitudeNumber(
                x: a,
                y: b
            ) != T.MaxMagnitude(
                x: a,
                y: b
            )) { return $"MaxMagnitudeNumber diverges from MaxMagnitude at ({rawA}, {rawB})"; }
            if (T.MinMagnitudeNumber(
                x: a,
                y: b
            ) != T.MinMagnitude(
                x: a,
                y: b
            )) { return $"MinMagnitudeNumber diverges from MinMagnitude at ({rawA}, {rawB})"; }
            if (unchecked((TAdapter.Raw(value: T.MaxMagnitude(
                x: a,
                y: b
            )) + TAdapter.Raw(value: T.MinMagnitude(
                x: a,
                y: b
            )))) != unchecked((rawA + rawB))) { return $"the two selections do not partition the pair ({rawA}, {rawB})"; }
            ...
            foreach (var sign in ((ReadOnlySpan<long>)[rawB, -1L, 0L, 1L])) {
                ...
                if (
                    ...
                    if (!Throws<OverflowException>(action: () => _ = T.CopySign(
                        sign: target,
                        value: a
                    ))) { return $"the positive magnitude of MinValue was answered at sign {sign}"; }

                    continue;
                }

                var expected = ((sign < 0L)
                    ? -magnitudeA
                    : magnitudeA
                );

                if (TAdapter.Raw(value: T.CopySign(
                    sign: target,
                    value: a
                )) != Oracles.WrapToRaw(value: expected)) { return $"the sign transplant of {rawA} onto {sign} is wrong"; }
            }
        ...
        public static string? ModulusExact(long[] left, long[] right) {
            ...
            var exactA = new BigInteger(value: rawA);
            var exactB = new BigInteger(value: rawB);
            var expected = (exactA - (BigInteger.Divide(
                dividend: exactA,
                divisor: exactB
            ) * exactB));
        ...
        public static string? OrderExact(long[] left, long[] right) {
            ...
            ) { return $"the two selections disagree at the equal pair {rawA}"; }

            var bound = new BigInteger(value: left[1]);
            var lower = BigInteger.Min(
                left: exactB,
                right: bound
            );
            var upper = BigInteger.Max(
                left: exactB,
                right: bound
            );
            var clamped = T.Clamp(
                value: a,
                min: TAdapter.FromRaw(value: ((long)lower)),
                max: TAdapter.FromRaw(value: ((long)upper))
            );

            if (TAdapter.Raw(value: clamped) != BigInteger.Min(
                left: BigInteger.Max(
                    left: exactA,
                    right: lower
                ),
                right: upper
            )) { return $"the clamp of {rawA} into [{lower}, {upper}] is wrong"; }
            if (T.Clamp(
                max: a,
                min: a,
                value: a
            ...
            if (!Throws<ArgumentException>(
                action: () => _ = ((IComparable)a).CompareTo(obj: "not a fixed-point value"),
                paramName: "obj"
            )) { return "the boxed comparison accepted a foreign type"; }
            if (!Throws<ArgumentException>(action: () => _ = T.Clamp(
                value: a,
                min: T.One,
                max: T.Zero
        ...
        public static string? ParseAll(string text, IFormatProvider provider, long expected, NumberStyles parseStyle, bool includeStyledOverloads) {
            if (TAdapter.Raw(value: T.Parse(
                provider: provider,
                s: text
            )) != expected) { return $"the string parse of '{text}' did not return {expected}"; }
            if (TAdapter.Raw(value: T.Parse(
                s: text.AsSpan(),
                provider: provider
            )) != expected) { return $"the span parse of '{text}' did not return {expected}"; }
            if (
                !T.TryParse(
                provider: provider,
                ...
                (TAdapter.Raw(value: fromString) != expected)
            ...
            if (
                ...
                (TAdapter.Raw(value: fromSpan) != expected)
            ...
            if (TAdapter.Raw(value: T.Parse(
                ...
                s: text,
                style: parseStyle
            )) != expected) { return $"the styled string parse of '{text}' did not return {expected}"; }
            if (TAdapter.Raw(value: T.Parse(
                s: text.AsSpan(),
                style: parseStyle,
                provider: provider
            )) != expected) { return $"the styled span parse of '{text}' did not return {expected}"; }
            if (
                !T.TryParse(
                provider: provider,
                ...
                (TAdapter.Raw(value: styledString) != expected)
            ...
            if (
                ...
                (TAdapter.Raw(value: styledSpan) != expected)
        ...
        public static string? TextRoundTrip(long[] left, int formatBufferLength, NumberStyles parseStyle, bool includeStyledOverloads) {
            ...
            var reference = Oracles.ExactDyadicDecimalSigned(
                value: new BigInteger(value: raw),
                shift: TAdapter.FractionBitCount
            );
            ...
            if (
                ...
                !destination[..written].SequenceEqual(other: rendered)
            ) { return $"the span format did not fill an exact destination at raw {raw}"; }
