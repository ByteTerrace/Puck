...
internal static class DigitalNetClaims {
    ...
    private static bool IsPlaneNet(ReadOnlySpan<uint> points, int order, Span<bool> occupancy) {
        ...
        for (var horizontal = 0; (horizontal <= order); ++horizontal) {
            ...
            for (var index = 0; (index < count); ++index) {
                var key = (((int)HighBits(
                    value: points[(2 * index)],
                    count: horizontal
                )) << vertical) | ((int)HighBits(
                    value: points[((2 * index) + 1)],
                    count: vertical
                ));
    ...
    public static string? ConeTableBuildPurityAndQuantizedCoverageSurface() {
        ...
        ConeDirectionTable.Build(
            capHalfAngleRadians: CapHalfAngle,
            destination: first
        );
        ConeDirectionTable.Build(
            capHalfAngleRadians: CapHalfAngle,
        ...
        foreach (var seed in ShuffleSeeds) {
            ...
            for (var index = 0; (index < indices.Length); ++index) {
                indices[index] = DigitalNetSampler.ShuffleIndex(
                    index: ((uint)index),
                    salt: seed
                );
            }
            ...
            for (var index = 0; (index < indices.Length); ++index) {
                var azimuth = ((int)HighBits(
                    value: points[(2 * index)],
                    count: order
                ));
                var radius = ((int)HighBits(
                    value: points[((2 * index) + 1)],
                    count: order
                ));
    ...
    public static string? NetPropertyThroughOrderFourteenSurface() {
        ...
        for (var order = 1; (order <= MaximumOrder); ++order) {
            ...
            if (!IsPlaneNet(
                points: points.AsSpan(
                    length: (2 * count),
                    start: 0
                ),
                order: order,
                occupancy: occupancy.AsSpan(
                    length: count,
                    start: 0
                )
            )) {
    ...
    public static string? ShiftedAndShuffledBlocksAreNetsSurface() {
        ...
        foreach (var seed in ShuffleSeeds) {
            for (var order = 1; (order <= Order); ++order) {
                ...
                for (var index = 0; (index < count); ++index) {
                    shuffled[index] = DigitalNetSampler.ShuffleIndex(
                        index: ((uint)index),
                        salt: seed
                    );
                }
