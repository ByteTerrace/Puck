...
public sealed class CoverageManifestTests {
    ...
    public void ManifestRatchetHolds() {
        ...
        Assert.SkipWhen(
            condition: (committed is null),
            reason: "Bootstrapping: the assembly ledger generates the manifest at run end; commit it, then this gate enforces."
        );
        ...
        foreach (var lawCase in LawRegistry.All) {
            foreach (var reference in lawCase.Members) {
                Assert.True(
                    condition: MemberSurface.Resolve(reference: reference).Any(),
                    userMessage: $"CoverRef {reference.Type.Name}.{reference.Name} (case {lawCase.Id}) resolved to no public member."
                );
            }
        }
        ...
        foreach (var entry in committed!.Members.Where(predicate: entry => (entry.State == "waived"))) {
            Assert.False(
                ...
                userMessage: $"Waived member {entry.Id} has no reason."
            );
        }
        ...
        Assert.True(
            condition: (newMembers.Count == 0),
            userMessage: $"{newMembers.Count} unclassified public member(s) — no manifest state, no covering law, no waiver: {string.Join(
                separator: ", ",
                values: newMembers.Take(count: 20)
            )}"
        );
        Assert.True(
            condition: (regressions.Count == 0),
            userMessage: $"{regressions.Count} member(s) moved covered→uncovered: {string.Join(
                separator: ", ",
