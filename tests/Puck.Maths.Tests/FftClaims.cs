...
internal static class FftClaims {
    ...
    private static string? ConvolutionVsOracleBoundCore(int[] lengths, long bound, ulong saltBase) {
        foreach (var length in lengths) {
            ...
            var a = Sequence(
                amplitude: ConvolutionAmplitudeRaw,
                length: length,
                stream: (saltBase + ((ulong)length))
            );
            var b = Sequence(
                amplitude: ConvolutionAmplitudeRaw,
                length: length,
                stream: ((saltBase + 500_000UL) + ((ulong)length))
            );
            ...
            var expected = Oracles.CyclicConvolutionComplexRaw(
                left: leftRaw,
                right: rightRaw
            );
    ...
    private static FixedComplex[] DirectDft(ReadOnlySpan<FixedComplex> values) {
        ...
        var shift = (64 - BitOperations.Log2(value: ((uint)n)));
    ...
    private static FixedComplex[] Sequence(int length, ulong stream, long amplitude) {
        var rng = Pcg32XshRr.Create(
            state: 0x4658_5254_2D46_4654UL,
            stream: stream
        );
        var values = new FixedComplex[length];

        for (var i = 0; (i < length); ++i) {
            var realRaw = (((long)rng.NextUInt32(
                maximum: ((uint)((2 * amplitude) + 1)),
                minimum: 0U
            )) - amplitude);
            var imaginaryRaw = (((long)rng.NextUInt32(
                maximum: ((uint)((2 * amplitude) + 1)),
                minimum: 0U
            )) - amplitude);

            values[i] = new(
                Real: FixedQ4816.FromRawBits(value: realRaw),
                Imaginary: FixedQ4816.FromRawBits(value: imaginaryRaw)
            );
        }
    ...
    public static string? ConvolutionAliasingContract() {
        ...
        var refusal = Refuses(
            action: () => FixedFourierTransform.Convolve(
    ...
    public static string? ConvolutionVsOracleBound() =>
        ConvolutionVsOracleBoundCore(
            bound: ConvolutionBoundDefault,
    ...
    public static string? ImpulseDcNyquistExact() {
        foreach (var length in new[] { 2, 4, 8, 16, 32, 64, 128, 256 }) {
            ...
            var expectedDc = new FixedComplex(
                Real: (dcValue * FixedQ4816.FromInteger(value: length)),
                Imaginary: FixedQ4816.Zero
            );
            ...
            var topBin = (length / 2);
            var expectedNyquist = new FixedComplex(
                Real: (nyquistValue * FixedQ4816.FromInteger(value: length)),
                Imaginary: FixedQ4816.Zero
            );
    ...
    public static string? LengthRefusals() {
        ...
        return (Refuses(
            action: () => FixedFourierTransformPlan.Create(length: 0),
            type: typeof(ArgumentOutOfRangeException),
            parameterName: "length",
            what: "FixedFourierTransformPlan.Create(0)"
        ) ??
               (Refuses(
            action: () => FixedFourierTransformPlan.Create(length: -8),
            type: typeof(ArgumentOutOfRangeException),
            parameterName: "length",
            what: "FixedFourierTransformPlan.Create(-8)"
        ) ??
               (Refuses(
            action: () => FixedFourierTransformPlan.Create(length: 5),
            type: typeof(ArgumentOutOfRangeException),
            parameterName: "length",
            what: "FixedFourierTransformPlan.Create(5) (not a power of two)"
        ) ??
               (Refuses(
            action: () => FixedFourierTransform.Forward(
                plan: plan,
                values: new FixedComplex[4]
            ),
            type: typeof(ArgumentException),
            ...
            what: "Forward with a mis-sized span"
               ...
               (Refuses(
            action: () => FixedFourierTransform.Inverse(
                plan: plan,
                values: new FixedComplex[16]
            ),
            type: typeof(ArgumentException),
            parameterName: "values",
            what: "Inverse with a mis-sized span"
               ...
               (Refuses(
            action: () => FixedFourierTransform.ForwardReal(
                destination: new FixedComplex[8],
                plan: plan,
                real: new FixedQ4816[4]
            ),
            type: typeof(ArgumentException),
            parameterName: "real",
            what: "ForwardReal with a mis-sized real span"
        ) ??
               (Refuses(
            action: () => FixedFourierTransform.ForwardReal(
                destination: new FixedComplex[4],
                plan: plan,
                real: new FixedQ4816[8]
            ),
            type: typeof(ArgumentException),
            parameterName: "destination",
            what: "ForwardReal with a mis-sized destination span"
        ) ??
               (Refuses(
            action: () => FixedFourierTransform.InverseReal(
                destination: new FixedQ4816[8],
                plan: plan,
                spectrum: new FixedComplex[4]
            ),
            type: typeof(ArgumentException),
            parameterName: "spectrum",
            what: "InverseReal with a mis-sized spectrum span"
        ) ??
               (Refuses(
            action: () => FixedFourierTransform.InverseReal(
                destination: new FixedQ4816[4],
                plan: plan,
                spectrum: new FixedComplex[8]
            ),
            type: typeof(ArgumentException),
            parameterName: "destination",
            what: "InverseReal with a mis-sized destination span"
        ) ??
               (Refuses(
            action: () => FixedFourierTransform.Convolve(
                destination: new FixedComplex[8],
                left: new FixedComplex[4],
                plan: plan,
                right: new FixedComplex[8]
            ),
            type: typeof(ArgumentException),
            parameterName: "left",
            ...
            action: () => FixedFourierTransform.Convolve(
                destination: new FixedComplex[8],
                left: new FixedComplex[8],
                plan: plan,
                right: new FixedComplex[4]
            ),
            type: typeof(ArgumentException),
            parameterName: "right",
            ...
            what: "Convolve with a mis-sized destination span"
               ...
               (Refuses(
            action: () => FixedFourierTransform.PointwiseMultiply(
                destination: new FixedComplex[8],
                left: new FixedComplex[8],
                right: new FixedComplex[4]
            ),
    ...
    public static string? PointwiseMultiplyIsElementwiseProduct() {
        foreach (var length in DefaultLengths) {
            var a = Sequence(
                length: length,
                stream: (12_000UL + ((ulong)length))
            );
            var b = Sequence(
                length: length,
                stream: (12_500UL + ((ulong)length))
            );
            var destination = new FixedComplex[length];

            FixedFourierTransform.PointwiseMultiply(
                destination: destination,
                left: a,
                right: b
            );
    ...
    public static string? RealWrappersAreFaithfulEmbeddings() {
        foreach (var length in DefaultLengths) {
            var plan = FixedFourierTransformPlan.Create(length: length);
            var complexInput = Sequence(
                length: length,
                stream: (9_000UL + ((ulong)length))
            );
            ...
            FixedFourierTransform.ForwardReal(
                destination: viaReal,
                plan: plan,
                real: real
            );
            ...
            FixedFourierTransform.InverseReal(
                destination: extractedReal,
                plan: plan,
                spectrum: spectrumForReal
            );
            FixedFourierTransform.Inverse(
                plan: plan,
                values: spectrumForComplex
            );
