...
internal static class BessMalformedCorpus {
    ...
    public static IEnumerable<(string Label, byte[] Bytes)> Build(byte[] goodFile) {
        ...
        yield return ("oversized work-RAM destination", WithOversizedBufferEntry(
            goodFile: goodFile,
            tableOffset: 0x00
        ));
        yield return ("oversized video-RAM destination", WithOversizedBufferEntry(
            goodFile: goodFile,
            tableOffset: 0x08
        ));
        yield return ("oversized palette destination", WithOversizedBufferEntry(
            goodFile: goodFile,
            tableOffset: 0x28
        ));
    ...
    private static byte[] WithIncompatibleMajorVersion(byte[] goodFile) {
        ...
        BinaryPrimitives.WriteUInt16LittleEndian(
            destination: malformed.AsSpan(start: coreDataOffset),
            value: 2
        );
