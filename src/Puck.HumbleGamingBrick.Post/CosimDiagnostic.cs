...
internal static class CosimDiagnostic {
    ...
    private static int Compare(string sameboyPath, string puckPath) {
        ...
        foreach (var group in groups) {
            ...
            if (divergence is null) {
                Console.WriteLine(value: $"  cosim {group.Name}: no divergence in {Math.Min(
                    val1: sameboyGroup.Count,
                    val2: puckGroup.Count
                )} events ({sameboyGroup.Count} sameboy / {puckGroup.Count} puck).");
    ...
    private static int Run(string romPath, string sameboyExe, string bootDir, string? modelArg, int frames, string kindArg, string outDir) {
        ...
        if (string.Equals(
            ...
            bootPath = Path.Combine(
                path1: outDir,
                path2: $"puck_boot_{model}.bin"
            );
        ...
        var sameboyTracePath = Path.Combine(
            path1: outDir,
            path2: "sameboy.cosim.bin"
        );
        var puckTracePath = Path.Combine(
            path1: outDir,
            path2: "puck.cosim.bin"
        );
