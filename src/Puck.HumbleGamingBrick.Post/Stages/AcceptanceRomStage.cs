...
/// <summary>
/// Tier-B stage: run a directory of acceptance ROMs and pass only when every eligible (ROM, model) reports the
/// Fibonacci pass signature over serial. The acceptance suite is large and precise (timer/DIV, PPU timing, interrupts,
...
internal sealed class AcceptanceRomStage : IPostStage {
    ...
    /// <summary>Initializes a new instance of the <see cref="AcceptanceRomStage"/> class.</summary>
    /// <param name="group">The group name (also the stage-name suffix).</param>
    /// <param name="relativeDirectory">The directory under the corpus's on-disk <c>mooneye-test-suite/acceptance/</c> (empty for the root).</param>
    /// <param name="recurse">Whether to descend into sub-directories.</param>
    public AcceptanceRomStage(string group, string relativeDirectory, bool recurse) {
    ...
    public PostStageOutcome Run(PostContext context) {
        ...
        foreach (var romCase in cases) {
            try {
                var (result, detail) = AcceptanceRomProbe.Run(romCase: romCase);
