...
internal sealed class BundledFirmwareStage : IPostStage<PostContext> {
    ...
    private static string? VerifyOverridesAndRefusals() {
        ...
        using var custom = new HumbleGamingBrickCore(
            model: Model,
            cartridgeRom: rom,
            bootRom: external
        );
        ...
        if (
            (custom.PeekByte(address: 0) != 0x18) ||
            (custom.PeekByte(address: MemoryMap.BootRomDisable) != 0xFE) ||
            (custom.Instance.GetRequiredService<ICpu>().ProgramCounter != 0) ||
            (custom.PeekByte(address: ResultAddress) != 0)
        ) {
            return "an external boot image was ignored, silently replaced, or skipped";
        }
        using var ordinary = new HumbleGamingBrickCore(
            model: Model,
            cartridgeRom: rom
        );
        ...
        foreach (var mode in Enum.GetValues<MachineBootMode>()) {
            if (RefusesArgument(
                action: () => HgbFirmware.CreateConfiguration(
                    bootMode: mode,
                    bootRom: new byte[(BootRomBuilder.MonochromeLength - 1)],
                    cartridgeRom: rom,
                    model: Model
                ),
                parameter: "bootRom"
        ...
        return (RefusesArgument(
            action: () => HgbFirmware.GetImage(model: ((ConsoleModel)255)),
            parameter: "model"
        ) ??
            (RefusesArgument(
            action: () => HgbFirmware.CreateConfiguration(
                model: Model,
                cartridgeRom: rom,
                bootMode: ((MachineBootMode)255)
            ),
            parameter: "bootMode"
        ) ??
            (RefusesArgument(
            action: () => new MachineConfiguration(
                model: Model,
                cartridgeRom: rom,
            ...
            (RefusesArgument(
            action: () => new GamingBrickEngine().Create(
                options: "dmg cold fast",
                contentBytes: rom
            ),
            parameter: "options"
        ) ??
    ...
    private static string? VerifyQueuedStartup() {
        ...
        using var cold = ((MachineHost)engine.Create(
            options: null,
            contentBytes: rom
        ));
        using var fast = ((MachineHost)engine.Create(
            options: "dmg fast",
            contentBytes: rom
        ));

        if (
            (cold.Options != "dmg cold") ||
            (fast.Options != "dmg fast") ||
            (cold.PeekByte(address: MemoryMap.BootRomDisable) != 0xFE) ||
            (fast.PeekByte(address: MemoryMap.BootRomDisable) != 0xFF)
        ) {
        ...
        return (((cold.PeekByte(address: MemoryMap.BootRomDisable) == 0xFF) && (cold.PeekByte(address: ResultAddress) == ResultValue))
            ? null
            : "the default queued host never handed execution from bundled firmware to the cartridge"
        );
    }
    private static string? VerifyRevision(ConsoleModel model) {
        ...
        var rom = CreateCartridge(model: model);
        using var cold = new HumbleGamingBrickCore(
            model: model,
            cartridgeRom: rom,
            dmgSpeed: true
        );
        using var fast = new HumbleGamingBrickCore(
            model: model,
            cartridgeRom: rom,
            dmgSpeed: true,
            bootMode: MachineBootMode.Fast
        );
        using var seeded = MachineFactory.Create(configuration: new MachineConfiguration(
            model: model,
            cartridgeRom: rom
        ));
        ...
        if (
            (cold.Instance.Configuration.BootMode != MachineBootMode.Cold) ||
            (cold.PeekByte(address: MemoryMap.BootRomDisable) != 0xFE) ||
            (cold.Instance.GetRequiredService<ICpu>().ProgramCounter != 0) ||
            (cold.PeekByte(address: 0) != generated[0])
        ) {
            return "the convenience constructor did not begin in bundled native firmware";
        }
        if (
            (fast.Instance.Configuration.BootMode != MachineBootMode.Fast) ||
            (fast.PeekByte(address: MemoryMap.BootRomDisable) != 0xFF) ||
            (fast.Instance.GetRequiredService<ICpu>().ProgramCounter != 0x0100) ||
            (fast.PeekByte(address: 0) != rom[0])
        ) {
            return "fast startup left a boot overlay or did not begin at the cartridge entry";
        }
        if (
            (coldIdentity != fastSnapshot.Identity) ||
            (coldIdentity == seeded.Machine.Snapshot().Identity) ||
        ...
        if (
            ((cold.PeekByte(address: MemoryMap.BootRomDisable) & 1) != 0) ||
        ...
        if (
            ...
            length: count,
            start: 0
        ).ContainsAnyExcept(value: samples[0]) == model.IsSuperGameBoy())
        ) {
        ...
        if (
            (fast.PeekByte(address: MemoryMap.BootRomDisable) != 0xFE) ||
        ...
        if (
            !expected.Data.SequenceEqual(other: fast.Instance.Machine.Snapshot().Data) ||
        ...
        if (
            (cold.PeekByte(address: MemoryMap.BootRomDisable) != 0xFF) ||
            (cold.PeekByte(address: ResultAddress) != ResultValue)
        ) {
    ...
    public PostStageOutcome Run(PostContext context) {
        ...
        return PostStageOutcome.Pass(detail: (((((string)$"{models.Length} bundled images match their native generator and return private copies; every revision cold-boots a valid cartridge, fast-starts at its seeded entry with the same firmware identity, ") +
