...
/// <summary>
/// Tier-A stage: gates the BESS importer's own validate-then-apply safety contract against the shared
/// <see cref="BessMalformedCorpus"/> — always run, not diagnostic-only. Every case exercises
/// <see cref="BessImporter.Import"/>'s own behavior, with no dependency on an external reference emulator or corpus,
/// so it needs no skip path and runs anywhere. Three case sets, all against dedicated color-capable (CGB) probe
/// machines:
/// <list type="bullet">
...
/// import is a true no-op, not a caught exception over already-mutated state. Destination-capacity and CPU/register
/// mutation happen on the same call, so a partial apply before the throw would otherwise leave the probe silently
/// corrupted. Includes the optional <c>MBC </c> block's own shape (a length-not-divisible-by-3 fragment, and a
/// record whose address falls outside <c>0x0000-0x7FFF</c>/<c>0xA000-0xBFFF</c>), an incompatible <c>CORE</c> major
/// version, a duplicate <c>CORE</c> block, and a known block (<c>MBC </c>) appearing before the required
/// <c>CORE</c> block — all validated in the same pure parse pass, before the CORE/register/buffer state above it is
/// ever applied.</item>
...
/// imported span.</item>
/// <item><see cref="BessMalformedCorpus.BuildExtendedCoreCase"/>: a <c>CORE</c> block padded with extra tail
