...
/// <summary>
/// Tier-B stage: run a group of conformance reference ROMs and pass only when every one of them reports success through its
...
internal sealed class ConformanceRomStage : IPostStage {
    ...
    /// <param name="subPath">The path under the corpus's on-disk <c>blargg/</c> directory holding the group's ROMs.</param>
    /// <param name="model">The console model the group's ROMs run on.</param>
    public ConformanceRomStage(string group, string subPath, ConsoleModel model) {
    ...
    public PostStageOutcome Run(PostContext context) {
        ...
        foreach (var romCase in cases) {
            try {
                var (result, detail) = ConformanceRomProbe.Run(romCase: romCase);

                if (result == ConformanceRomResult.Pass) {
                    ++passed;
                } else {
