...
internal sealed class LinkChurnStage : IPostStage {
    ...
    public PostStageOutcome Run(PostContext context) {
        ...
        return PostStageOutcome.Pass(detail: $"{TransferCount} transfers each way (dmg master ↔ cgb slave), connect phase pinned, replay-identical and churn-identical (severed transfer-idle at budget step {churnStep}, {reference.MasterState.Size}+{reference.SlaveState.Size} state bytes)");
    }
            ...
            try {
                for (var step = 0; (step < StepCount); ++step) {
                    probes.Add(item: new BoundaryProbe(
                        ...
                        Completed: masterTally.Completions
                    ));
    ...
    private static string? Judge(ChurnScenarioResult result) {
        if (CompletionFault(
            completion: result.MasterCompletion,
            side: "master"
        ) is { } masterFault) {
            return masterFault;
        }
        ...
        if (CompletionFault(
            completion: result.SlaveCompletion,
            side: "slave"
        ) is { } slaveFault) {
            return slaveFault;
        }
        ...
        if (
            (result.MasterTraffic.Completions != TransferCount) ||
            (result.SlaveTraffic.Completions != TransferCount)
        ) {
    ...
    private static string? Difference(ChurnScenarioResult expected, ChurnScenarioResult actual, string leg) {
        ...
        if (!expected.MasterState.ContentEquals(other: actual.MasterState)) {
            return $"the {leg} master final state diverged — {HashDivergenceProbe.DescribeDivergence(
                a: expected.MasterState,
                b: actual.MasterState
            )}";
        }

        if (!expected.SlaveState.ContentEquals(other: actual.SlaveState)) {
            return $"the {leg} slave final state diverged — {HashDivergenceProbe.DescribeDivergence(
                a: expected.SlaveState,
                b: actual.SlaveState
            )}";
        }
