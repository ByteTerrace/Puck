...
internal sealed class RumbleDeviceStage : IPostStage {
    ...
    public PostStageOutcome Run(PostContext context) {
        var rumbleRom = SyntheticRom.Create(
            cartridgeType: 0x1E,
            ramSize: 0x02
        ...
        rumbleCartridge.WriteControl(
            address: 0x4000,
            value: 0x0F
        ); // bit 3 set + bank bits 0-2 = 7 (masked to 7 max)
        ...
        var plainRom = SyntheticRom.Create(
            cartridgeType: 0x1B,
            ramSize: 0x02
        );
