...
internal sealed class FirmwareReconfigureStage : IPostStage<PostContext> {
    ...
    private static void CheckBootBoundary(ConsoleModel source, ConsoleModel target, string sourceToken, string targetToken) {
        ...
        for (var observation = 0; (observation < 2); ++observation) {
            Require(
                condition: (core.PeekByte(address: MemoryMap.BootRomDisable) == 0xFE),
                detail: $"{source}: core left firmware before the boundary check"
            );
            Require(
                condition: (host.PeekByte(address: MemoryMap.BootRomDisable) == 0xFE),
                detail: $"{source}: host left firmware before the boundary check"
            );
            var coreBefore = core.Instance.Machine.Snapshot();
            var hostBefore = ObserveHost(host: host);

            Require(
                condition: core.Reconfigure(
                    options: sourceToken,
                    reason: out var coreReason
                ),
                detail: $"{source}: unchanged core model during boot was refused: {coreReason}"
            );
            Require(
                condition: host.TryReconfigure(
                    options: sourceToken,
                    reason: out var hostReason
                ),
                detail: $"{source}: unchanged host model during boot was refused: {hostReason}"
            );
            RequireSameSnapshot(
                expected: coreBefore,
                actual: core.Instance.Machine.Snapshot(),
                detail: $"{source}: unchanged core model altered native boot"
            );
            RequireSameSnapshot(
                expected: hostBefore.Snapshot,
                actual: ObserveHost(host: host).Snapshot,
                detail: $"{source}: unchanged host model altered native boot"
            );
            RequireCoreRefusal(
                core: core,
                options: targetToken,
                reasonContains: "boot ROM is still running"
            );
            RequireHostRefusal(
        ...
        Require(
            condition: ((core.PeekByte(address: MemoryMap.BootRomDisable) == 0xFF) && (host.PeekByte(address: MemoryMap.BootRomDisable) == 0xFF)),
        ...
        Require(
            condition: core.Reconfigure(
                options: targetToken,
                reason: out var acceptedCore
            ),
            detail: $"{source}->{target}: post-handoff core change failed: {acceptedCore}"
        );
        Require(
            condition: host.TryReconfigure(
                options: targetToken,
                reason: out var acceptedHost
            ),
            detail: $"{source}->{target}: post-handoff host change failed: {acceptedHost}"
        );
        Require(
            condition: ((core.Instance.Machine.Model == target) && (core.Instance.GetRequiredService<ICpu>().ProgramCounter == pc) &&
        ...
        RequireHostImage(
            host: host,
            image: HgbFirmware.GetImage(model: source),
            model: target,
            options: $"{targetToken} dmgspeed cold"
        );
    ...
    private static void CheckCoreSelections(MachineBootMode mode) {
        using var core = new HumbleGamingBrickCore(
            ...
            cartridgeRom: CreateCartridge(model: ConsoleModel.DmgC),
            bootMode: mode
        );
        var other = ((mode == MachineBootMode.Fast)
            ? "cold"
            : "fast"
        );
        ...
        RequireCoreRefusal(
            core: core,
            options: $"dmg {other}",
            reasonContains: "construction-fixed"
        );
        RequireCoreRefusal(
            core: core,
            options: "dmg bios=puck",
            reasonContains: "construction-fixed"
        );
        RequireCoreRefusal(
            core: core,
            options: "dmg bios=does-not-exist.bin",
            reasonContains: "construction-fixed"
        );
        ...
        Require(
            condition: core.Reconfigure(
                options: $"dmg {mode.ToString().ToLowerInvariant()}",
                reason: out var reason
            ),
            detail: $"repeating core startup mode was refused: {reason}"
        );
        RequireSameSnapshot(
            expected: before,
            actual: core.Instance.Machine.Snapshot(),
            detail: "an unchanged core selection mutated the machine"
        );
    ...
    private static void CheckExternalHost(string directory) {
        var path = Path.Combine(
            path1: directory,
            path2: "external firmware with spaces.bin"
        );
        ...
        using var host = ((MachineHost)new GamingBrickEngine().Create(
            options: $"cgb dmgspeed fast bios=\"{path}\"",
            contentBytes: CreateCartridge(model: ConsoleModel.CgbE)
        ));
        ...
        File.WriteAllBytes(
            bytes: [0],
            path: path
        ); // Any reload would now fail the image-length guard.
        Require(
            condition: host.TryReconfigure(
                options: "dmg",
                reason: out var changedReason
            ),
            detail: $"bare hardware change reread or lost external firmware: {changedReason}"
        );
        RequireHostImage(
            host: host,
            image: image,
            model: ConsoleModel.DmgC,
            options: $"dmg dmgspeed fast bios={path}"
        );
        Require(
            condition: host.TryReconfigure(
                options: $"cgb fast bios=\"{path}\"",
                reason: out var repeatedReason
            ),
            detail: $"unchanged explicit image selection failed: {repeatedReason}"
        );
        RequireHostImage(
            host: host,
            image: image,
        ...
        Require(
            condition: host.TryReconfigure(
                options: $"dmgb fast bios=\"{path}\"",
                reason: out var deletedReason
            ),
            detail: $"live host forwarding reread a deleted image path: {deletedReason}"
        );
        RequireHostImage(
            host: host,
            image: image,
            model: ConsoleModel.DmgB,
            options: $"dmgb dmgspeed fast bios={path}"
        );
        ...
        RequireHostRefusal(
            host: host,
            options: "dmgb bios=puck",
            reasonContains: "construction-fixed"
        );
        RequireHostRefusal(
            host: host,
            options: $"dmgb bios={path}.absent",
            reasonContains: "construction-fixed"
        );
        RequireHostRefusal(
            host: host,
            options: "dmgb cold fast",
            reasonContains: "Choose cold or fast"
        );
        RequireHostRefusal(
            host: host,
            options: "dmgb bios=",
            reasonContains: "bios= requires"
        );
        RequireHostImage(
            host: host,
            image: image,
    ...
    private static (MachineSnapshot Snapshot, byte[] Firmware, ConsoleModel Model) ObserveHost(MachineHost host) {
        ...
        Require(
            condition: (core is not null),
            detail: "the host had no core to observe"
        );
        try {
            Require(
    ...
    private static void RequireCoreExecution(HumbleGamingBrickCore core) {
        ...
        Require(
            condition: ((core.PeekByte(address: MemoryMap.BootRomDisable) == 0xFF) && (core.PeekByte(address: CounterAddress) != CounterSeed)),
    ...
    private static void RequireCoreRefusal(HumbleGamingBrickCore core, string options, string reasonContains) {
        ...
        RequireSameSnapshot(
            expected: before,
            actual: core.Instance.Machine.Snapshot(),
            detail: $"core refusal of '{options}' changed machine state"
        );
    }
    private static void RequireHostExecution(MachineHost host) {
        ...
        Require(
            condition: ((host.PeekByte(address: MemoryMap.BootRomDisable) == 0xFF) && (host.PeekByte(address: CounterAddress) != CounterSeed) && (host.Worker.QueueFault is null)),
    ...
    private static void RequireHostImage(MachineHost host, byte[] image, ConsoleModel model, string options) {
        ...
        Require(
            condition: ((host.Options == options) && (observed.Model == model) && observed.Firmware.AsSpan().SequenceEqual(other: image)),
    ...
    private static void RequireHostRefusal(MachineHost host, string options, string reasonContains, bool checkExecution = true) {
        ...
        Require(
            condition: (host.Options == beforeOptions),
            detail: $"host refusal of '{options}' changed canonical options"
        );
        RequireSameSnapshot(
            expected: before.Snapshot,
            actual: ObserveHost(host: host).Snapshot,
            detail: $"host refusal of '{options}' changed machine state"
        );
        if (checkExecution) {
            RequireHostExecution(host: host);
        }
    ...
    private static void RequireSameSnapshot(MachineSnapshot expected, MachineSnapshot actual, string detail) =>
        Require(
            condition: ((expected.Identity == actual.Identity) && expected.Data.SequenceEqual(other: actual.Data)),
            detail: detail
        );
    ...
    public PostStageOutcome Run(PostContext context) {
        var directory = Path.Combine(
            ...
            path2: $"firmware-reconfigure-{Guid.NewGuid():N}"
        );
        ...
        try {
            ...
            CheckBootBoundary(
                source: ConsoleModel.DmgC,
                sourceToken: "dmg",
                target: ConsoleModel.CgbE,
                targetToken: "cgb"
            );
            CheckBootBoundary(
                source: ConsoleModel.CgbE,
                sourceToken: "cgb",
                target: ConsoleModel.DmgC,
                targetToken: "dmg"
            );
            return PostStageOutcome.Pass(detail: "fast external-image hosts retain exact firmware bytes and canonical paths after the source file is changed and deleted; hardware-only and repeated unchanged selections reach the worker without rereading the path; mode/image refusals preserve full state and subsequent execution; both DMG-to-Color and Color-to-DMG changes refuse during native boot, unchanged hardware remains allowed, and both directions succeed after cartridge handoff");
