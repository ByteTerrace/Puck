...
public static class NumberTheoryFunctions {
    ...
    public static void SegmentedPrimeSieve(ulong low, ulong high, Action<ulong> onPrime) {
        ...
        ArgumentOutOfRangeException.ThrowIfGreaterThan(value: high, other: MaximumSieveBound);
        ...
        var windowLow = ((0UL == (low & 1UL)) ? (low + 1UL) : low);
    ...
    public static BigInteger HenselLiftRoot(ReadOnlySpan<BigInteger> coefficients, BigInteger root, BigInteger baseModulus, int targetPower) {
        ...
        ArgumentOutOfRangeException.ThrowIfLessThan(value: baseModulus, other: (BigInteger.One + BigInteger.One));
        ArgumentOutOfRangeException.ThrowIfLessThan(value: targetPower, other: 1);

        if (!Evaluate(coefficients: coefficients, point: root).IsZeroModulo(modulus: baseModulus)) {
            throw new ArgumentException(message: "The supplied value is not a root of the polynomial modulo the base modulus.", paramName: nameof(root));
        }

        var derivativeAtRoot = EvaluateDerivative(coefficients: coefficients, point: root).FloorModulo(modulus: baseModulus);

        if (BigInteger.GreatestCommonDivisor(left: derivativeAtRoot, right: baseModulus) != BigInteger.One) {
            throw new ArgumentException(message: "The derivative is not a unit modulo the base modulus, so the unique derivative-unit lift does not apply.", paramName: nameof(root));
        }

        var inverseDerivative = BigIntegerFunctions.ModularInverse(value: derivativeAtRoot, modulus: baseModulus);
        ...
        for (var power = 1; (power < targetPower); ++power) {
            ...
            var deficit = Evaluate(coefficients: coefficients, point: lifted).FloorModulo(modulus: nextModulus);
