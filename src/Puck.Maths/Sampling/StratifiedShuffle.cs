...
/// </para>
/// </remarks>
public static class StratifiedShuffle {
    ...
    public static uint Permute(uint index, uint seed) {
        unchecked {
            ...
            value *= InvertibleBitMix.FirstMultiplier;
            value ^= (value << SecondShift);
            value *= InvertibleBitMix.SecondMultiplier;
    ...
    public static uint Unpermute(uint index, uint seed) {
        unchecked {
            ...
            value ^= (value << ThirdShift);
            value *= InvertibleBitMix.SecondMultiplierInverse;
            value ^= (value << SecondShift) ^ (value << (2 * SecondShift)) ^ (value << (3 * SecondShift)) ^ (value << (4 * SecondShift));
            value *= InvertibleBitMix.FirstMultiplierInverse;
