...
public sealed record CostModelProfile {
    ...
    public static CostModelProfile Portable { get; } = new(
        cyclesPerSecond: 3_000_000_000L,
        name: "puck.cost.portable-profile.v1",
        subsystemShareDenominator: 2L,
        subsystemShareNumerator: 1L
    );
    ...
    public CostModelProfile(string name, long cyclesPerSecond, long subsystemShareNumerator, long subsystemShareDenominator) {
        ...
        ArgumentOutOfRangeException.ThrowIfGreaterThan(
            subsystemShareNumerator,
            subsystemShareDenominator
        );
        Name = name;
    ...
    private void ValidateRate(int rateHz) {
        ...
        if (
            (rateHz > 0) &&
            ((EngineTicksPerSecond % rateHz) != 0)
        ) {
            throw new ArgumentException(
                message: "A positive simulation rate must divide the engine time base exactly.",
                paramName: nameof(rateHz)
            );
        }
    }
    ...
    public bool Admits(CostBound bound, int rateHz) {
        ValidateRate(rateHz: rateHz);
        return (
            (rateHz > 0) &&
            bound.IsKnown &&
            (bound.Cycles <= AuthoredBudgetCycles(rateHz: rateHz))
        );
    }
    ...
    public long AuthoredBudgetCycles(int rateHz) {
        ...
        return ((rateHz == 0)
            ? 0L
            : (long)((((Int128)CyclesPerSecond) * SubsystemShareNumerator) / (((Int128)SubsystemShareDenominator) * rateHz))
        );
    }
    ...
    public (Int128 Numerator, long Denominator) ReferenceStepFraction(long cycles, int rateHz) {
        ...
        return ((rateHz == 0)
            ? (Int128.Zero, 1L)
            : ((((Int128)cycles) * rateHz), CyclesPerSecond)
        );
    }
