...
public sealed class QuadraticOstrowskiSystem {
    ...
    public BigInteger Evaluate(IReadOnlyList<BigInteger> digits) {
        ...
        if (digits.Count == 0) { throw new ArgumentException(message: "an Ostrowski representation cannot be empty", paramName: nameof(digits)); }
    ...
    public bool IsCanonical(IReadOnlyList<BigInteger> digits) {
        ...
        if ((leastFirst[0] < 0) || (leastFirst[0] >= PartialQuotient(index: 1))) { return false; }
...
public sealed class OstrowskiPellChannelCertificate {
    ...
    public OstrowskiDigitAutomaton CompileAutomaton() =>
        OstrowskiDigitAutomaton.FromPeriodicPattern(block: RepeatedBlock, prefix: PrefixDigits, suffix: SuffixDigits);
    ...
    public IReadOnlyList<BigInteger> Word(int repeatCount) {
        ...
        var result = new List<BigInteger>(
            capacity: ((PrefixDigits.Count + (repeatCount * RepeatedBlock.Count)) + SuffixDigits.Count)
        );
...
public sealed class OstrowskiDigitAutomaton {
    ...
    public int Transition(int state, BigInteger digit) {
        ...
        return m_transitions.GetValueOrDefault(defaultValue: DeadState, key: (state, digit));
    }
    ...
    internal static OstrowskiDigitAutomaton FromPeriodicPattern(
        ...
        if (block.Count == 0) { throw new ArgumentException(message: "the repeated block cannot be empty", paramName: nameof(block)); }
        ...
        foreach (var digit in prefix) {
            ...
            AddTransition(digit: digit, state: cursor, target: target, transitions: symbolTransitions);
            cursor = target;
        }
        ...
        AddEpsilon(state: loopState, target: suffixStart, transitions: epsilonTransitions);
        cursor = suffixStart;
        ...
        var startSet = EpsilonClosure(epsilonTransitions: epsilonTransitions, states: [0]);
        ...
        while (pending.Count > 0) {
            ...
            foreach (var digit in alphabet) {
                ...
                foreach (var nfaState in set) {
                    if (symbolTransitions.TryGetValue(key: (nfaState, digit), value: out var targets)) {
                        moved.UnionWith(other: targets);
                    }
                }
                var closure = EpsilonClosure(epsilonTransitions: epsilonTransitions, states: moved);
                var key = SetKey(states: closure);

                if (!dfaIndex.TryGetValue(key: key, value: out var targetState)) {
        ...
        if (!dfaIndex.TryGetValue(key: deadKey, value: out var deadState)) {
        ...
        var accepting = dfaSets.Select(selector: set => (Array.BinarySearch(array: set, value: acceptingNfaState) >= 0)).ToArray();

        return new OstrowskiDigitAutomaton(accepting: accepting, alphabet: alphabet, deadState: deadState, startState: 0, transitions: dfaTransitions);

        int AddDfaSet(int[] set) {
            var key = SetKey(states: set);

            if (dfaIndex.TryGetValue(key: key, value: out var existing)) { return existing; }
    ...
    internal static OstrowskiDigitAutomaton FromLiteral(IReadOnlyList<BigInteger> word) {
        ...
        return new OstrowskiDigitAutomaton(accepting: accepting, alphabet: alphabet, deadState: deadState, startState: 0, transitions: transitions);
    }
    ...
    private static void AddTransition(
        ...
        BigInteger digit,
        int target) {
        if (!transitions.TryGetValue(key: (state, digit), value: out var targets)) {
            targets = [];
    ...
    private static void AddEpsilon(Dictionary<int, HashSet<int>> transitions, int state, int target) {
        if (!transitions.TryGetValue(key: state, value: out var targets)) {
            targets = [];
    ...
    private static int[] EpsilonClosure(
        ...
        while (pending.Count > 0) {
            ...
            if (!epsilonTransitions.TryGetValue(key: state, value: out var targets)) { continue; }
    ...
    private static string SetKey(IEnumerable<int> states) => string.Join(separator: ',', values: states);
}
...
public sealed class OstrowskiOutputAutomaton {
    ...
    public BigInteger Output(BigInteger value) {
        ...
        foreach (var digit in digits) {
            if (!m_transitions.TryGetValue(key: (state, digit), value: out state)) { return 0; }
        }
    ...
    public static OstrowskiOutputAutomaton Build(
        ...
        return new OstrowskiOutputAutomaton(alphabet: alphabet, outputs: outputs, system: system, transitions: transitions);
...
public static class OstrowskiPellChannel {
    ...
    public static OstrowskiPellChannelCertificate Build(
        ...
        while (true) {
            ...
            if (usable && TryInfer(block: out var block, prefix: out var prefix, suffix: out var suffix, words: words)) {
    ...
    private static bool TryInfer(
        ...
        for (var prefixLength = 0; (prefixLength <= words[0].Length); ++prefixLength) {
            ...
            if (!words[1].SequenceEqual(other: Compose(block: candidateBlock, prefix: candidatePrefix, repetitions: 1, suffix: candidateSuffix)) ||
                !words[2].SequenceEqual(other: Compose(block: candidateBlock, prefix: candidatePrefix, repetitions: 2, suffix: candidateSuffix)) ||
                !words[3].SequenceEqual(other: Compose(block: candidateBlock, prefix: candidatePrefix, repetitions: 3, suffix: candidateSuffix))) {
                continue;
            }
