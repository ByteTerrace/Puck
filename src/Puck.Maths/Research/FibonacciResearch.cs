...
public readonly record struct GoldenInteger(BigInteger A, BigInteger B) {
    ...
    public GoldenInteger Advance(BigInteger exponent) {
        ...
        var (current, next) = FibonacciResearch.FibonacciPair(index: exponent);
...
public static class FibonacciResearch {
    ...
    public static QuadraticSurd GoldenRatio { get; } = QuadraticSurd.Create(denominator: 2, radicand: 5, rationalNumerator: 1, surdNumerator: 1);

    /// <summary>Gets the conjugate <c>1−τ = (1 − √5) / 2</c>.</summary>
    public static QuadraticSurd GoldenConjugate { get; } = QuadraticSurd.Create(denominator: 2, radicand: 5, rationalNumerator: 1, surdNumerator: -1);
    ...
    public static (BigInteger Current, BigInteger Next) FibonacciPair(BigInteger index) {
        if (index.Sign < 0) { throw new ArgumentOutOfRangeException(paramName: nameof(index)); }
        return FibonacciPairCore(index: index);
    }
    ...
    public static (BigInteger Current, BigInteger Next) FibonacciPairModulo(
        ...
        if (index.Sign < 0) { throw new ArgumentOutOfRangeException(paramName: nameof(index)); }
        if (modulus.Sign <= 0) { throw new ArgumentOutOfRangeException(paramName: nameof(modulus)); }
        return FibonacciPairModuloCore(index: index, modulus: modulus);
    }
    ...
    public static BigInteger TwoPowerRankOfApparition(int exponent) {
        ...
        return ((exponent == 2) ? new BigInteger(value: 6) : (3 * (BigInteger.One << (exponent - 2))));
    }
    ...
    public static GoldenProjectiveOrbit ClassifyTwoPowerProjectiveOrbit(
        ...
        if (value.A.IsEven && value.B.IsEven) {
            throw new ArgumentException(message: "the projective pair must be primitive at two", paramName: nameof(value));
        }

        var residue = value.Norm.FloorModulo(modulus: ((BigInteger)8));
        ...
        throw new InvalidOperationException(message: "a primitive golden integer unexpectedly had even norm");
    }
    ...
    public static GoldenInteger AdvanceModulo(
        ...
        if (exponent.Sign < 0) { throw new ArgumentOutOfRangeException(paramName: nameof(exponent)); }
        if (modulus.Sign <= 0) { throw new ArgumentOutOfRangeException(paramName: nameof(modulus)); }
        if (exponent.IsZero) {
            return new GoldenInteger(
                A: value.A.FloorModulo(modulus: modulus),
                B: value.B.FloorModulo(modulus: modulus)
            );
        }

        var (current, next) = FibonacciPairModulo(index: exponent, modulus: modulus);
        var previous = (next - current).FloorModulo(modulus: modulus);

        return new GoldenInteger(
            A: ((previous * value.A) + (current * value.B)).FloorModulo(modulus: modulus),
            B: ((current * value.A) + (next * value.B)).FloorModulo(modulus: modulus)
        );
    }
    ...
    private static (BigInteger Current, BigInteger Next) FibonacciPairCore(BigInteger index) {
        ...
        var (halfCurrent, halfNext) = FibonacciPairCore(index: (index >> 1));
    ...
    private static (BigInteger Current, BigInteger Next) FibonacciPairModuloCore(
        ...
        var (halfCurrent, halfNext) = FibonacciPairModuloCore(index: (index >> 1), modulus: modulus);
        var current = (halfCurrent * ((2 * halfNext) - halfCurrent)).FloorModulo(modulus: modulus);
        var next = ((halfCurrent * halfCurrent) + (halfNext * halfNext)).FloorModulo(modulus: modulus);

        return (index.IsEven
            ? (current, next)
            : (next, (current + next).FloorModulo(modulus: modulus)));
    }
}
...
public readonly record struct FibonacciSymmetricMinimum(
    ...
    public bool IsBelowThreeCellGap =>
        (DeterminantNormalizedExcess < QuadraticSurd.Rational(denominator: 3, numerator: 8));
    ...
    public bool Verify() {
        ...
        var expected = FibonacciSymmetricMinimum.Find(period: Period);
    ...
    public static FibonacciSymmetricMinimum Find(BigInteger period) {
        ...
        return scaleIndex switch {
            ...
            _ => FromFibonacciPair(period: period, scaleIndex: scaleIndex)
        };
    }
    ...
    private static FibonacciSymmetricMinimum FromFibonacciPair(BigInteger period, int scaleIndex) {
        ...
        return new FibonacciSymmetricMinimum(Kappa: kappa, Lambda: lambda, Period: period, ScaleIndex: scaleIndex);
    }
}
...
public readonly record struct FibonacciRulerLetter(bool BaseLetter, int Color) {
    ...
    public int SymbolIndex(int rulerDepth) {
        ...
        if ((Color < 0) || (Color > rulerDepth)) {
            throw new InvalidOperationException(message: "the ruler color is outside the requested alphabet");
        }
...
public sealed class FibonacciRulerWordIndex {
    ...
    public BigInteger FalsePrefixCount(BigInteger exclusiveEnd) =>
        (exclusiveEnd - TruePrefixCount(exclusiveEnd: exclusiveEnd));

    /// <summary>Returns the base Fibonacci letter at a non-negative index.</summary>
    public bool BaseLetterAt(BigInteger index) {
        if (index.Sign < 0) { throw new ArgumentOutOfRangeException(paramName: nameof(index)); }
        return (TruePrefixCount(exclusiveEnd: (index + BigInteger.One)) != TruePrefixCount(exclusiveEnd: index));
    }
    ...
    public FibonacciRulerLetter LetterAt(BigInteger index) {
        if (index.Sign < 0) { throw new ArgumentOutOfRangeException(paramName: nameof(index)); }
        var trueBefore = TruePrefixCount(exclusiveEnd: index);
        var baseLetter = (TruePrefixCount(exclusiveEnd: (index + BigInteger.One)) != trueBefore);
        var occurrenceRank = (baseLetter ? trueBefore : (index - trueBefore));

        return new FibonacciRulerLetter(BaseLetter: baseLetter, Color: ColorOfOccurrence(occurrenceRank: occurrenceRank, rulerDepth: RulerDepth));
    }
    ...
    public FibonacciFactorCounts FactorCounts(BigInteger start, BigInteger length) {
        ...
        var trueCount = (TruePrefixCount(exclusiveEnd: end) - TruePrefixCount(exclusiveEnd: start));

        return new FibonacciFactorCounts(FalseCount: (length - trueCount), TrueCount: trueCount);
    }
    ...
    public bool IsRulerRich(BigInteger start, BigInteger length) {
        var counts = FactorCounts(length: length, start: start);
    ...
    public static int ColorOfOccurrence(BigInteger occurrenceRank, int rulerDepth) {
        if (occurrenceRank.Sign < 0) { throw new ArgumentOutOfRangeException(paramName: nameof(occurrenceRank)); }
        ArgumentOutOfRangeException.ThrowIfNegative(rulerDepth);
