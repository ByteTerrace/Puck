...
public readonly record struct GoldenInteger(BigInteger A, BigInteger B) {
    ...
    public GoldenInteger Advance(BigInteger exponent) {
        if (exponent.Sign < 0) {
            throw new ArgumentOutOfRangeException(
                nameof(exponent),
                "the forward exponent must be non-negative"
            );
        }
    ...
    public GoldenInteger Next() => new(
        A: B,
        B: (A + B)
    );
    /// <summary>Multiplies by <c>τ⁻¹ = τ−1</c>.</summary>
    public GoldenInteger Previous() => new(
        A: (B - A),
        B: A
    );
}
...
public static class FibonacciResearch {
    ...
    private static (BigInteger Current, BigInteger Next) FibonacciPairModuloCore(
        ...
        var (halfCurrent, halfNext) = FibonacciPairModuloCore(
            index: (index >> 1),
            modulus: modulus
        );
    ...
    public static GoldenInteger AdvanceModulo(
        ...
        var (current, next) = FibonacciPairModulo(
            index: exponent,
            modulus: modulus
        );
    ...
    public static GoldenProjectiveOrbit ClassifyTwoPowerProjectiveOrbit(
        ...
        if (exponent < 3) {
            throw new ArgumentOutOfRangeException(
                nameof(exponent),
                "the two-orbit classification requires exponent at least three"
            );
        }
        if (
            value.A.IsEven &&
            ...
            throw new ArgumentException(
                message: "the projective pair must be primitive at two",
        ...
        if (
            (residue == BigInteger.One) ||
            (residue == 7)
        ) { return GoldenProjectiveOrbit.UnitNorm; }
        if (
            (residue == 3) ||
            (residue == 5)
        ) { return GoldenProjectiveOrbit.FiveNorm; }
    ...
    public static (BigInteger Current, BigInteger Next) FibonacciPairModulo(
        ...
        if (index.Sign < 0) { throw new ArgumentOutOfRangeException(paramName: nameof(index)); }
        if (modulus.Sign <= 0) { throw new ArgumentOutOfRangeException(paramName: nameof(modulus)); }
        return FibonacciPairModuloCore(
            index: index,
            modulus: modulus
        );
    }
    ...
    public static BigInteger TwoPowerRankOfApparition(int exponent) {
        if (exponent < 2) {
            throw new ArgumentOutOfRangeException(
                nameof(exponent),
                "the closed form requires exponent at least two"
            );
        }
        return ((exponent == 2)
            ? new BigInteger(value: 6)
            : (3 * (BigInteger.One << (exponent - 2)))
        );
    }
}
...
public readonly record struct FibonacciSymmetricMinimum(
    ...
    private static FibonacciSymmetricMinimum FromFibonacciPair(BigInteger period, int scaleIndex) {
        ...
        return new FibonacciSymmetricMinimum(
            Kappa: kappa,
            Lambda: lambda,
            Period: period,
    ...
    public static FibonacciSymmetricMinimum Find(BigInteger period) {
        ...
        return scaleIndex switch {
            -1 => new FibonacciSymmetricMinimum(
            period,
            scaleIndex,
            Lambda: 1,
            Kappa: 0
        ),
            0 => new FibonacciSymmetricMinimum(
            period,
            scaleIndex,
            Lambda: 0,
            Kappa: 1
    ...
    public bool Verify() {
        ...
        return (
            (ScaleIndex == expected.ScaleIndex) &&
...
public sealed class FibonacciRulerWordIndex {
    ...
    public FibonacciFactorCounts FactorCounts(BigInteger start, BigInteger length) {
        ...
        return new FibonacciFactorCounts(
    ...
    public bool IsRulerRich(BigInteger start, BigInteger length) {
        var counts = FactorCounts(
            length: length,
            start: start
        );

        return (
            (counts.FalseCount >= ColoringPeriod) &&
            (counts.TrueCount >= ColoringPeriod)
        );
    }
    ...
    public FibonacciRulerLetter LetterAt(BigInteger index) {
        ...
        var occurrenceRank = (baseLetter
            ? trueBefore
            : (index - trueBefore)
        );

        return new FibonacciRulerLetter(
            BaseLetter: baseLetter,
            Color: ColorOfOccurrence(
