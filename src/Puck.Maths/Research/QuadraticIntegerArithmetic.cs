...
public static class QuadraticIntegerArithmetic {
    ...
    public static bool IsUnit(this QuadraticAlgebra<BigInteger> algebra, Element value) =>
        BigInteger.Abs(value: FastNorm(algebra: algebra, value: value)).IsOne;
    ...
    public static Element FundamentalUnit(this QuadraticAlgebra<BigInteger> algebra) {
        ...
        return new Element(U: ((solution.X - (algebra.P * solution.Y)) / 2), V: solution.Y);
    }
    ...
    public static Element CanonicalAssociate(this QuadraticAlgebra<BigInteger> algebra, Element value) {
        ...
        return CanonicalAssociate(algebra: algebra, value: value, units: new UnitBasis(algebra: algebra));
    }
    ...
    public static bool IsPrimeElement(this QuadraticAlgebra<BigInteger> algebra, Element value) {
        var norm = BigInteger.Abs(value: FastNorm(algebra: algebra, value: value));
        ...
        if (((root * root) == norm) && BigIntegerFunctions.IsPrime(value: root) && (QuadraticSplitting.Inert == algebra.SplittingCharacter(rationalPrime: root))) {
            ...
            var inert = CanonicalAssociate(algebra: algebra, value: new Element(U: root, V: BigInteger.Zero), units: units);

            return (CanonicalAssociate(algebra: algebra, value: value, units: units) == inert);
        }
    ...
    public static bool TryFactorize(this QuadraticAlgebra<BigInteger> algebra, Element value, out QuadraticFactorization factorization, out QuadraticFactorizationObstruction obstruction) {
        ...
        var norm = FastNorm(algebra: algebra, value: value);
        ...
        foreach (var rationalPrime in BigIntegerFunctions.EnumeratePrimeFactors(value: magnitude).Distinct()) {
            ...
            if (QuadraticSplitting.Inert == splitting) {
                var prime = CanonicalAssociate(algebra: algebra, value: new Element(U: rationalPrime, V: BigInteger.Zero), units: units);
            ...
            var primeA = CanonicalAssociate(algebra: algebra, value: generator.Value, units: units);
            var primeB = CanonicalAssociate(algebra: algebra, value: algebra.Conjugate(value: generator.Value), units: units);
    ...
    private readonly struct UnitBasis {
        internal UnitBasis(QuadraticAlgebra<BigInteger> algebra) {
            ...
            if (Discriminant.Sign < 0) {
                ...
                FundamentalInverse = InverseUnit(algebra: algebra, unit: Fundamental);
            }
        }
    ...
    private static Element CanonicalAssociate(QuadraticAlgebra<BigInteger> algebra, Element value, in UnitBasis units) {
        ...
        if (units.ImaginaryGroup is not null) {
            ...
            foreach (var unit in units.ImaginaryGroup) {
                var candidate = FastMultiply(algebra: algebra, left: unit, right: value);
        ...
        if (0 > Embedding1Sign(units: units, value: current)) { current = algebra.Negate(value: current); }

        while (0 <= Embedding1Compare(units: units, left: current, right: units.Fundamental)) {
            current = FastMultiply(algebra: algebra, left: current, right: units.FundamentalInverse);
        }

        while (0 > Embedding1CompareToOne(units: units, value: current)) {
            current = FastMultiply(algebra: algebra, left: current, right: units.Fundamental);
        }
    ...
    private static bool TryDivideExact(QuadraticAlgebra<BigInteger> algebra, Element dividend, Element divisor, out Element quotient) {
        if (TryExtractCoefficients(algebra: algebra, p: out var p, q: out var q)
            && TryExtract(value: dividend.U, result: out var a) && TryExtract(value: dividend.V, result: out var b)
            && TryExtract(value: divisor.U, result: out var c) && TryExtract(value: divisor.V, result: out var e)) {
            ...
            if (((productU % norm) == Int128.Zero) && ((productV % norm) == Int128.Zero)) {
                quotient = new Element(U: ((BigInteger)(productU / norm)), V: ((BigInteger)(productV / norm)));
        ...
        if ((product.U % wideNorm).IsZero && (product.V % wideNorm).IsZero) {
            quotient = new Element(U: (product.U / wideNorm), V: (product.V / wideNorm));
    ...
    private static Element? FindNormElement(QuadraticAlgebra<BigInteger> algebra, BigInteger magnitude) {
        ...
        if (discriminant.Sign < 0) {
            ...
            for (var b = -bBound; (b <= bBound); ++b) {
                ...
                foreach (var signedRoot in ((root.IsZero) ? (BigInteger[])[root] : (BigInteger[])[root, -root])) {
                    ...
                    if (FastNorm(algebra: algebra, value: candidate) == magnitude) { return candidate; }
                }
            }
        ...
        var solution = QuadraticNormEquation.SolveForPrimeNorm(discriminant: discriminant, rationalPrime: magnitude);

        if (solution is null) { return null; }

        return new Element(U: ((solution.Value.X - (p * solution.Value.Y)) / 2), V: solution.Value.Y);
    }
    ...
    private static List<Element> ImaginaryUnits(QuadraticAlgebra<BigInteger> algebra) {
        ...
        for (var b = -bBound; (b <= bBound); ++b) {
            ...
            foreach (var signedRoot in ((root.IsZero) ? (BigInteger[])[root] : (BigInteger[])[root, -root])) {
                ...
                if (FastNorm(algebra: algebra, value: candidate).IsOne && (!units.Contains(item: candidate))) { units.Add(item: candidate); }
            }
        }
    ...
    private static int Embedding1Sign(in UnitBasis units, Element value) =>
        SurdSign(rational: ((2 * value.U) + (units.P * value.V)), surd: value.V, radicand: units.Discriminant);
    ...
    private static int Embedding1CompareToOne(in UnitBasis units, Element value) =>
        SurdSign(rational: (((2 * value.U) + (units.P * value.V)) - 2), surd: value.V, radicand: units.Discriminant);
    ...
    private static int Embedding1Compare(in UnitBasis units, Element left, Element right) =>
        SurdSign(rational: ((2 * (left.U - right.U)) + (units.P * (left.V - right.V))), surd: (left.V - right.V), radicand: units.Discriminant);
    ...
    private static BigInteger FastNorm(QuadraticAlgebra<BigInteger> algebra, Element value) {
        if (TryExtractCoefficients(algebra: algebra, p: out var p, q: out var q)
            && TryExtract(value: value.U, result: out var u) && TryExtract(value: value.V, result: out var v)) {
    ...
    private static Element FastMultiply(QuadraticAlgebra<BigInteger> algebra, Element left, Element right) {
        if (TryExtractCoefficients(algebra: algebra, p: out var p, q: out var q)
            && TryExtract(value: left.U, result: out var u1) && TryExtract(value: left.V, result: out var v1)
            && TryExtract(value: right.U, result: out var u2) && TryExtract(value: right.V, result: out var v2)) {
    ...
    private static bool TryExtractCoefficients(QuadraticAlgebra<BigInteger> algebra, out long p, out long q) {
        var fitsP = TryExtract(value: algebra.P, result: out p);
        var fitsQ = TryExtract(value: algebra.Q, result: out q);
    ...
    private static bool TryExtract(BigInteger value, out long result) {
        if ((value >= FastTierLowerBound) && (value <= FastTierUpperBound)) {
            result = ((long)value);
