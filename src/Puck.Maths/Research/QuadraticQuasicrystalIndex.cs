...
public sealed class QuadraticQuasicrystalIndex {
    ...
    private readonly List<SubstitutionLevel> levels = [new(LongCount: BigInteger.One, LongLength: BigInteger.One, ShortCount: BigInteger.Zero, ShortLength: BigInteger.One)];
    private readonly object levelLock = new();

    internal QuadraticQuasicrystalIndex(long p, long q, long d, long r) {
        period = PeriodicBlock(d: d, p: p, q: q, r: r);
        if (period.Any(predicate: term => (term <= 0))) {
    ...
    private LocatedTile Locate(BigInteger index) {
        ...
        return new LocatedTile(LetterIsLong: letterIsLong, LongBefore: longBefore);
    }
    private int EnsureDepth(BigInteger index) {
        while (levels[^1].LongLength <= index) {
            var previous = levels[^1];

            levels.Add(item: new SubstitutionLevel(
    ...
    private ImageSelection SelectPeriodImage(
        ...
        if (factorIndex == period.Length) {
            ...
            return new ImageSelection(LetterIsLong: seedIsLong, LongBefore: BigInteger.Zero, Offset: index, ShortBefore: BigInteger.Zero);
        }
        ...
        if (!inner.LetterIsLong) {
            return new ImageSelection(LetterIsLong: true, LongBefore: longBefore, Offset: inner.Offset, ShortBefore: shortBefore);
        }

        var longRunWeight = (factor * longWeight);

        if (inner.Offset < longRunWeight) {
            var longOffset = BigInteger.DivRem(dividend: inner.Offset, divisor: longWeight, remainder: out var remainder);

            return new ImageSelection(LetterIsLong: true, LongBefore: (longBefore + longOffset), Offset: remainder, ShortBefore: shortBefore);
        }

        return new ImageSelection(LetterIsLong: false, LongBefore: (longBefore + factor), Offset: (inner.Offset - longRunWeight), ShortBefore: shortBefore);
    }
    private static long[] PeriodicBlock(long p, long q, long d, long r) {
        ...
        while (true) {
            try {
                ...
                return terms.Slice(length: periodLength, start: periodStart).ToArray();
