...
internal static class QuadraticNormEquation {
    ...
    internal static QuadraticNormSolution FundamentalUnit(BigInteger discriminant) {
        ...
        var b = ((root.IsEven == discriminant.IsEven) ? root : (root - BigInteger.One));
        ...
        return Walk(discriminant: discriminant, root: root, startP: b, startQ: new BigInteger(value: 2))!.Value;
    }
    ...
    internal static QuadraticNormSolution? SolveForPrimeNorm(BigInteger discriminant, BigInteger rationalPrime) {
        var root = Validate(discriminant: discriminant);
        var coefficient = IdealCoefficient(discriminant: discriminant, rationalPrime: rationalPrime);
        ...
        return Walk(discriminant: discriminant, root: root, startP: coefficient.Value, startQ: (2 * rationalPrime));
    }
    ...
    private static BigInteger? IdealCoefficient(BigInteger discriminant, BigInteger rationalPrime) {
        ...
        if (!BigIntegerFunctions.TrySquareRootModuloOddPrime(value: discriminant, oddPrime: rationalPrime, root: out var residue)) { return null; }

        return ((residue.IsEven == discriminant.IsEven) ? residue : (residue + rationalPrime));
    }
    ...
    private static QuadraticNormSolution? Walk(BigInteger discriminant, BigInteger root, BigInteger startP, BigInteger startQ) {
        ...
        while (true) {
            ...
            if (anchored) {
                if ((recurrence.StateP == cycleP) && (recurrence.StateQ == cycleQ)) { return null; }
            } else if (IsReduced(root: root, stateP: recurrence.StateP, stateQ: recurrence.StateQ)) {
                anchored = true;
