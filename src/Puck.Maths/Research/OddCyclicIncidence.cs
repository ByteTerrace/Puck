...
public sealed class OddCyclicIncidence {
    ...
    public OddCyclicIncidence(
        ...
        if (columns.Length != checked((rayOrbitCount * letterCount))) {
            throw new ArgumentException(message: "The polynomial table length does not match rayOrbitCount * letterCount.", paramName: nameof(columns));
        }
        if (factors.IsEmpty) { throw new ArgumentException(message: "At least one factor is required.", paramName: nameof(factors)); }
        ...
        for (var index = 0; (index < Columns.Length); ++index) {
            if ((Columns[index] & ~coefficientMask) != 0UL) {
                throw new ArgumentException(message: $"Polynomial column {index} has coefficients outside the cycle.", paramName: nameof(columns));
            }
        }
        ...
        for (var letter = 0; (letter < letterCount); ++letter) {
            ...
            for (var rayOrbit = 0; (rayOrbit < rayOrbitCount); ++rayOrbit) {
                if ((BitOperations.PopCount(value: PolynomialAt(letter: letter, rayOrbit: rayOrbit)) & 1) != 0) {
    ...
    /// </summary>
    public int IrreducibleWordLengthCeiling => Math.Min(val1: LetterCount, val2: checked((SyndromeRank + 1)));
    ...
    public OddCyclicWordAnalysis Analyze(ReadOnlySpan<int> selectedLetters, bool verifyExpandedMatrix = false) {
        ...
        if (verifyExpandedMatrix) {
            ...
            if (expandedNullity.Value != nullity) {
                throw new InvalidOperationException(
                    message: $"CRT nullity {nullity} disagrees with expanded binary nullity {expandedNullity.Value}."
                );
            }
        }
        ...
        return new OddCyclicWordAnalysis(
            ...
            isSyndromeCircuit: (hasEvenIncidence && (components[Array.FindIndex(array: FactorStorage, match: factor => (factor.Bits == 3UL))].Rank == (wordLength - 1))),
            nullity: nullity,
    ...
    private void ValidateFactors() {
        ...
        for (var first = 0; (first < FactorStorage.Length); ++first) {
            ...
            if (!factor.IsIrreducible()) { throw new ArgumentException(message: $"Factor {factor.Bits:x} is reducible.", paramName: "factors"); }
            ...
            for (var second = (first + 1); (second < FactorStorage.Length); ++second) {
                if (!factor.GreatestCommonDivisor(other: FactorStorage[second]).IsOne) {
                    throw new ArgumentException(message: "The proposed factors are not pairwise coprime.", paramName: "factors");
                }
            }
        }
        ...
        if (product != expected) { throw new ArgumentException(message: "The proposed factors do not multiply to t^n + 1.", paramName: "factors"); }
        if (!FactorStorage.Any(predicate: factor => (factor.Bits == 3UL))) {
            throw new ArgumentException(message: "The factorization omits t + 1.", paramName: "factors");
        }
    }
    private void ValidateSelection(ReadOnlySpan<int> selectedLetters) {
        if (selectedLetters.IsEmpty) { throw new ArgumentException(message: "At least one letter must be selected.", paramName: nameof(selectedLetters)); }
        ...
        foreach (var letter in selectedLetters) {
            ValidateLetter(letter: letter);
            if (seen[letter]) { throw new ArgumentException(message: $"Letter {letter} is selected more than once.", paramName: nameof(selectedLetters)); }
