    ...
    public bool Verify() {
        ...
        foreach (var item in m_classes) {
            if (
                (item.Gap <= 0) ||
                (item.Residue < 0) ||
                (item.Residue >= item.Gap) ||
                ((Period % item.Gap) != 0) ||
                !distinct.Add(item: item)
            ) {
                return false;
            }
            leastPeriod = LeastCommonMultiple(
                left: leastPeriod,
                right: item.Gap
            );
        }
...
public static class ConstantGapCoveringResearch {
    ...
    public static ConstantGapCovering CanonicalRuler(int symbolCount) {
        ...
        if (symbolCount == 1) {
            return new ConstantGapCovering(
                1,
                [new ConstantGapClass(
                        Gap: 1,
                        Residue: 0
                    )]
            );
        ...
        for (var symbol = 0; (symbol < (symbolCount - 1)); ++symbol) {
            ...
            classes[symbol] = new ConstantGapClass(
                Gap: gap,
                Residue: ((gap / 2) - 1)
            );
        }
        classes[^1] = new ConstantGapClass(
            Gap: period,
            Residue: (period - 1)
        );
        var result = new ConstantGapCovering(
            classes: classes,
            period: period
        );
    ...
    public static IReadOnlyList<ConstantGapPeriodWitness> PeriodSpectrum(
        ...
        for (var period = 1; (period <= inclusiveMaximumPeriod); ++period) {
            if (TryFind(
                covering: out var covering,
                leastPeriod: period,
                symbolCount: symbolCount
            )) {
                result.Add(item: new ConstantGapPeriodWitness(
                    Covering: covering!,
                    Period: period
                ));
            }
    ...
    public static bool TryFind(
        ...
        if (!DensityCanComplete(
            currentLcm: 1,
            remaining: leastPeriod,
            slots: symbolCount
        )) {
            covering = null;
            return false;
        ...
        covering = new ConstantGapCovering(
            classes: [.. chosen],
            period: leastPeriod
        );
        if (!covering.Verify()) {
        ...
        bool Search(BigInteger covered, int slots, int currentLcm) {
            ...
            var first = FirstUnset(
                covered: covered,
                period: leastPeriod
            );

            foreach (var gap in divisors) {
                var item = new ConstantGapClass(
                    Gap: gap,
                    Residue: (first % gap)
                );
                ...
                var nextLcm = LeastCommonMultiple(
                    left: currentLcm,
                    right: gap
                );
        ...
        bool DensityCanComplete(int slots, int remaining, int currentLcm) {
            ...
            if (
                (remaining < slots) ||
                (remaining > (slots * leastPeriod))
            ) {
                return false;
            }
            var state = new DensityState(
                Lcm: currentLcm,
                Remaining: remaining,
                Slots: slots
            );
