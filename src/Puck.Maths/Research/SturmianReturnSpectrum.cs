...
public static class SturmianReturnSpectrumResearch {
    ...
    private static CongruenceState Advance(
        ...
        return new CongruenceState(
            state.PCurrent,
            AddMultiplyMod(
                current: state.PCurrent,
                digit: digit,
                modulus: leftColoringPeriod,
                previous: state.PPrevious
            ),
            state.QCurrent,
            AddMultiplyMod(
                current: state.QCurrent,
                digit: digit,
                modulus: rightColoringPeriod,
                previous: state.QPrevious
            ),
            Mod(
                modulus: period.Count,
                value: (state.NextPhase + 1)
            )
        );
    }
        ...
        for (var m = 0; (m < nextDigit); ++m) {
            ...
            for (var ell = 0; (ell <= maximumTotal); ++ell) {
                for (var k = 0; (k <= (maximumTotal - ell)); ++k) {
                    if ((ell + k) == 0) { continue; }
                    var vector = new SturmianReturnVector(
                        Ell: ell,
                        K: k,
                        M: m
                    );
    ...
    private static IReadOnlyList<PhaseData> BuildPhaseData(
        ...
        for (var nextPhase = 0; (nextPhase < period.Count); ++nextPhase) {
            var delta = PeriodicTail(
                period,
                nextPhase
            );
            var currentPhase = Mod(
                modulus: period.Count,
                value: (nextPhase - 1)
            );
            var reversed = PeriodicTail(
                period,
                currentPhase,
                step: -1
            );
            ...
            result[nextPhase] = new PhaseData(
                ColoredCandidates: colored,
                Delta: delta,
                Uncolored: uncolored,
                X: x
            );
    ...
    private static bool CongruenceHoldsUnchecked(
        ...
        return (
            (first % leftColoringPeriod).IsZero &&
    ...
    private static SturmianReturnSpectrum EvaluateMatrices(
        ...
        if (matrices.Count == 0) {
            throw new ArgumentException(
                message: "a congruence component cannot be empty",
                paramName: nameof(matrices)
            );
        }
    ...
    private static IReadOnlyList<SturmianCongruencePhase> EventualMatrixCycle(
        ...
        for (var n = 1; ; ++n) {
            var digit = ((n <= prefix.Count)
                ...
                : period[Mod(
                    modulus: period.Count,
                    value: ((n - prefix.Count) - 1)
                )]
            );
            var pN = AddMultiplyMod(
                current: pNm1,
                digit: digit,
                modulus: leftColoringPeriod,
                previous: pNm2
            );
            var qN = AddMultiplyMod(
                current: qNm1,
                digit: digit,
                modulus: rightColoringPeriod,
                previous: qNm2
            ...
            if (n > prefix.Count) {
                var nextPhase = Mod(
                    modulus: period.Count,
                    value: (n - prefix.Count)
                );
                var key = new CongruenceState(
                    NextPhase: nextPhase,
                    PCurrent: pN,
                    PPrevious: pNm1,
                    QCurrent: qN,
                ...
                if (seen.TryGetValue(
                    key: key,
                    value: out var cycleStart
                )) {
                    return phases.GetRange(
                        count: (phases.Count - cycleStart),
                        index: cycleStart
                    );
                }
                seen.Add(
            ...
            if (phases.Count > 10_000_000) {
                throw new InvalidOperationException(message: "congruence-state cycle exceeded the safety limit");
            }
        }
    }
    ...
    private static void ValidateDirective(
        ...
        if (
            (period.Count == 0) ||
            period.Any(predicate: value => (value <= 0))
        ) {
    ...
    private static int WeightedReturnSearchBound(
        ...
        var minimum = ((firstWeight < secondWeight)
            ? firstWeight
            : secondWeight
        );
        var maximum = ((firstWeight > secondWeight)
            ? firstWeight
            : secondWeight
        );
    ...
    public static SturmianComponentMinimum ComponentMinimum(
        ...
        ValidateColoringPeriods(
            left: leftColoringPeriod,
            right: rightColoringPeriod
        );
                    ...
                    for (var c = 0; (c < rightColoringPeriod); ++c) {
                        for (var d = 0; (d < rightColoringPeriod); ++d) {
                            ...
                            histogram.TryGetValue(
                                key: spectrum.ColoredLimsup,
                                value: out var multiplicity
                            );
        ...
        return new SturmianComponentMinimum(
            cycleCount,
            (minimum ?? throw new InvalidOperationException(message: "no determinant-compatible congruence component exists")),
            representative,
            histogram
        );
    }
                    ...
                    for (var d = 0; (d < rightColoringPeriod); ++d) {
                        ...
                        do {
                            ...
                            if (phaseValue is null) {
                                throw new InvalidOperationException(message: "no admissible Fibonacci return vector was found");
                            }
                            if (
                                (cycleMaximum is null) ||
                        ...
                        ++cycleCount;
                        var value = (cycleMaximum ?? throw new InvalidOperationException(message: "empty Fibonacci congruence component"));
    ...
    public static bool TryFindPrefix(
        ...
        var maximumDigit = LeastCommonMultiple(
            left: leftColoringPeriod,
            right: rightColoringPeriod
        );
        ...
        while (queue.Count != 0) {
            ...
            if (state == target) {
                ...
                if (!check.Phases.Any(predicate: item => (item.Matrix == aligned))) {
                    throw new InvalidOperationException(message: "the reconstructed finite prefix missed its congruence component");
                }
                prefix = result;
                return true;
            }
    ...
    private readonly record struct IntegerMatrix2(
        ...
        public static IntegerMatrix2 Identity => new(
            A: 1,
            B: 0,
            C: 0,
            D: 1
        ...
        public static IntegerMatrix2 Digit(int value) => new(
            A: value,
            B: 1,
            C: 1,
            D: 0
