...
public readonly struct GeometricAlgebra : IEquatable<GeometricAlgebra> {
    ...
    private static void ThrowExponentialOperandNotBivector(string paramName, int blade) =>
        throw new ArgumentException(
            message: ((string)$"The exponential is defined here only for a bivector, but blade {blade} is of grade {BitOperations.PopCount(value: ((uint)blade))} and nonzero; every lane of a grade other than two must be zero."),
            paramName: paramName
        );
    ...
    private static void ThrowExponentialSquareNotScalar(string paramName, int blade) =>
        throw new ArgumentException(
            message: ((string)$"The exponential's closed form is valid only where the bivector squares to a scalar, but blade {blade} of the square is nonzero; the general multivector exponential is not implemented."),
            paramName: paramName
        );
    ...
    private void ValidateOperand(in Multivector value, string paramName) {
        for (var blade = BladeCount; (blade < Multivector.BladeCapacity); ++blade) {
            if (value[blade].Value != 0L) {
                ThrowOperandOutsideSignature(
                    blade: blade,
                    paramName: paramName
                );
            }
    ...
    public static GeometricAlgebra Create(int positiveCount, int negativeCount, int degenerateCount) {
        ...
        if (positiveCount > 4) {
            ThrowSignatureTooLarge(
                value: positiveCount,
                paramName: nameof(positiveCount)
            );
        }
        if (negativeCount > (4 - positiveCount)) {
            ThrowSignatureTooLarge(
                value: negativeCount,
                paramName: nameof(negativeCount)
            );
        }
        if (degenerateCount > ((4 - positiveCount) - negativeCount)) {
            ThrowSignatureTooLarge(
                value: degenerateCount,
                paramName: nameof(degenerateCount)
            );
        }
    ...
    public Multivector Exponential(Multivector bivector) {
        ...
        for (var blade = 0; (blade < dimension); ++blade) {
            if (
                (BitOperations.PopCount(value: ((uint)blade)) != 2) &&
                (bivector[blade].Value != 0L)
            ) {
                ThrowExponentialOperandNotBivector(
                    paramName: nameof(bivector),
                    blade: blade
                );
            }
        ...
        var square = GeometricProductCore(
            left: bivector,
            right: bivector
        );
        ...
        for (var blade = 1; (blade < dimension); ++blade) {
            if (square[blade].Value != 0L) {
                ThrowExponentialSquareNotScalar(
                    paramName: nameof(bivector),
                    blade: blade
                );
            }
    ...
    public bool IsEven(Multivector value) {
        ...
        for (var i = 0; (i < dimension); ++i) {
            if (
                ((BitOperations.PopCount(value: ((uint)i)) & 1) != 0) &&
                (value[i].Value != 0L)
            ) {
                return false;
            }
...
public struct Multivector : IEquatable<Multivector> {
    ...
    public static Multivector FromCoefficients(ReadOnlySpan<FixedQ4816> coefficients) {
        if (coefficients.Length > BladeCapacity) {
            throw new ArgumentException(
                message: $"A multivector holds at most {BladeCapacity} blade coefficients.",
