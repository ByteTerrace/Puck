...
public readonly struct MonogenicAlgebra<TScalar> : IEquatable<MonogenicAlgebra<TScalar>>
    ...
    private readonly TScalar[] m_modulus;
    ...
    private readonly bool m_integerTail;
    ...
    private readonly bool m_delegatesQuadratic;
    // The degree-2 twin, built when _delegatesQuadratic is set and left default otherwise.
    private readonly QuadraticAlgebra<TScalar> m_quadratic;
    ...
    private MonogenicAlgebra(TScalar[] modulus) {
        m_modulus = modulus;
        m_integerTail = (IsHouseCarrier && (modulus.Length >= 2) && HasIntegerTail(modulus: modulus));
        m_delegatesQuadratic = ((2 == modulus.Length) && HasNegatableTail(modulus: modulus));
        m_quadratic = (m_delegatesQuadratic
            ? QuadraticAlgebra<TScalar>.Create(
                p: -modulus[1],
                q: -modulus[0]
            )
            : default
    ...
    public readonly struct Element : IEquatable<Element> {
        ...
        private readonly TScalar[] m_coordinates;
        ...
        private TScalar[] GetCoordinates() =>
            (m_coordinates ?? ThrowUninitialized());
        /// <summary>Throws the default-element diagnosis.</summary>
        /// <returns>Never returns.</returns>
        /// <exception cref="InvalidOperationException">Always.</exception>
        [MethodImpl(MethodImplOptions.NoInlining)]
        private static TScalar[] ThrowUninitialized() =>
            throw new InvalidOperationException(message: "The monogenic element is default-initialized and belongs to no algebra; obtain it from a MonogenicAlgebra<TScalar> descriptor.");
        ...
        public override int GetHashCode() =>
            VectorHash(vector: m_coordinates);
    ...
    public readonly struct Projective : IEquatable<Projective> {
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        private static TScalar[] ThrowUninitialized() =>
            throw new InvalidOperationException(message: "The monogenic projective window is default-initialized and belongs to no algebra; obtain it from MonogenicAlgebra<TScalar>.FromWindow.");
    ...
    private static TScalar BerkowitzDeterminant(TScalar[] matrix, int order) {
        ...
        for (var stage = 2; (stage <= order); ++stage) {
            ...
            polynomial = MultiplyToeplitz(
                column: toeplitzColumn,
                vector: polynomial
            );
        }
    ...
    private static TScalar CofactorDeterminant(TScalar[] matrix, int order) {
        ...
        for (var column = 0; (column < order); ++column) {
            ...
            var cofactor = (matrix[column] * CofactorDeterminant(
                matrix: minor,
                order: (order - 1)
            ));
    ...
    private static Int128 Int128CofactorDeterminant(ReadOnlySpan<Int128> matrix, int order) {
        ...
        for (var column = 0; (column < order); ++column) {
            ...
            var cofactor = unchecked((matrix[column] * Int128CofactorDeterminant(
                matrix: minor.Slice(
    ...
    private static sbyte LimbBerkowitzDeterminant(ReadOnlySpan<ulong> matrix, ReadOnlySpan<sbyte> matrixSigns, int order, int width, Span<ulong> result) {
        ...
        var pool = RentLimbCells(
            cellCount: cellCount,
            rented: out var rented,
            width: width
        );

        try {
            var polynomial = pool.Slice(
                length: ((order + 1) * width),
                start: 0
            );
            var toeplitz = pool.Slice(
                length: ((order + 1) * width),
                start: ((order + 1) * width)
            );
            var vector = pool.Slice(
                length: (order * width),
                start: ((2 * (order + 1)) * width)
            );
            var nextVector = pool.Slice(
                length: (order * width),
                start: (((2 * (order + 1)) + order) * width)
            );
            var product = pool.Slice(
                length: width,
                start: (((2 * (order + 1)) + (2 * order)) * width)
            );
            var accumulator = pool.Slice(
                length: width,
                start: ((((2 * (order + 1)) + (2 * order)) + 1) * width)
            );
            var toeplitzScratch = pool.Slice(
                length: ((order + 1) * width),
                start: ((((2 * (order + 1)) + (2 * order)) + 2) * width)
            );
            var polynomialSigns = (((order + 1) <= LimbStackColumnLimit)
                ? stackalloc sbyte[LimbStackColumnLimit]
                : new sbyte[(order + 1)]
            );
            var toeplitzSigns = (((order + 1) <= LimbStackColumnLimit)
                ? stackalloc sbyte[LimbStackColumnLimit]
                : new sbyte[(order + 1)]
            );
            var vectorSigns = ((order <= LimbStackColumnLimit)
                ? stackalloc sbyte[LimbStackColumnLimit]
                : new sbyte[order]
            );
            var nextVectorSigns = ((order <= LimbStackColumnLimit)
                ? stackalloc sbyte[LimbStackColumnLimit]
                : new sbyte[order]
            );

            polynomialSigns = polynomialSigns.Slice(
                length: (order + 1),
                start: 0
            );
            toeplitzSigns = toeplitzSigns.Slice(
                length: (order + 1),
                start: 0
            );
            vectorSigns = vectorSigns.Slice(
                length: order,
                start: 0
            );
            nextVectorSigns = nextVectorSigns.Slice(
                length: order,
                start: 0
            );
            ...
            polynomialSigns[0] = LimbBig.SetFromInt64(
                magnitude: polynomial.Slice(
                    length: width,
                    start: 0
                ),
                value: 1L
            ...
            MatrixCopy(
                ...
                index: 0,
                width: width,
                destination: polynomial.Slice(
                    length: width,
                    start: width
                ),
                destinationSign: out polynomialSigns[1]
            );
            ...
            for (var stage = 2; (stage <= order); ++stage) {
                ...
                for (var index = 0; (index < toeplitzLength); ++index) {
                    toeplitz.Slice(
                        length: width,
                        start: (index * width)
                    ).Clear();
                ...
                toeplitzSigns[0] = LimbBig.SetFromInt64(
                    magnitude: toeplitz.Slice(
                        length: width,
                        start: 0
                    ),
                    value: 1L
                ...
                MatrixCopy(
                    matrix: matrix,
                    matrixSigns: matrixSigns,
                    index: ((submatrixOrder * order) + submatrixOrder),
                    width: width,
                    destination: toeplitz.Slice(
                        length: width,
                        start: width
                    ),
                    destinationSign: out toeplitzSigns[1]
                );
                ...
                for (var row = 0; (row < submatrixOrder); ++row) {
                    MatrixCopy(
                        matrix: matrix,
                        matrixSigns: matrixSigns,
                        index: ((row * order) + submatrixOrder),
                        width: width,
                        destination: vector.Slice(
                            length: width,
                            start: (row * width)
                        ),
                        destinationSign: out vectorSigns[row]
                    );
                }
                ...
                for (var power = 1; (power <= submatrixOrder); ++power) {
                    ...
                    for (var column = 0; (column < submatrixOrder); ++column) {
                        var factorSign = LimbBig.MultiplyFull(
                            destination: product,
                            left: matrix.Slice(
                                length: width,
                                start: (((submatrixOrder * order) + column) * width)
                            ),
                            leftSign: matrixSigns[((submatrixOrder * order) + column)],
                            right: vector.Slice(
                                length: width,
                                start: (column * width)
                            ),
                            rightSign: vectorSigns[column]
                        );

                        entrySign = LimbBig.AddInto(
                            addend: product,
                            addendSign: factorSign,
                            destination: accumulator,
                            destinationSign: entrySign
                        );
                    }

                    accumulator.CopyTo(destination: toeplitz.Slice(
                        length: width,
                        start: ((power + 1) * width)
                    ));
                        ...
                        for (var row = 0; (row < submatrixOrder); ++row) {
                            ...
                            for (var column = 0; (column < submatrixOrder); ++column) {
                                var factorSign = LimbBig.MultiplyFull(
                                    ...
                                    left: matrix.Slice(
                                        length: width,
                                        start: (((row * order) + column) * width)
                                    ),
                                    leftSign: matrixSigns[((row * order) + column)],
                                    right: vector.Slice(
                                        length: width,
                                        start: (column * width)
                                    ),
                                ...
                                rowSign = LimbBig.AddInto(
                                    addend: product,
                                    addendSign: factorSign,
                                    destination: accumulator,
                                    destinationSign: rowSign
                                );
                            }

                            accumulator.CopyTo(destination: nextVector.Slice(
                                length: width,
                                start: (row * width)
                            ));
                ...
                polynomialLength = LimbToeplitzMultiply(
                    column: toeplitz,
                    columnLength: toeplitzLength,
                    columnSigns: toeplitzSigns,
                    product: product,
                    resultScratch: toeplitzScratch,
                    vector: polynomial,
                    vectorLength: polynomialLength,
                    vectorSigns: polynomialSigns,
                    width: width
                );
            }
            ...
            return ((0 == (order & 1))
                ? polynomialSigns[order]
                : (sbyte)(-polynomialSigns[order])
            );
        } finally {
    ...
    private void LimbColumnByRoot(ReadOnlySpan<ulong> source, ReadOnlySpan<sbyte> sourceSigns, Span<ulong> destination, Span<sbyte> destinationSigns, int width, Span<ulong> productScratch) {
        ...
        for (var k = (n - 1); (k >= 1); --k) {
            ...
            source.Slice(
                length: width,
                start: ((k - 1) * width)
            ).CopyTo(destination: cell);
            LimbBig.ShiftLeft(
                bits: FixedQ4816.FractionBitCount,
                magnitude: cell
            );

            var mRaw = Unsafe.BitCast<TScalar, FixedQ4816>(source: m_modulus[k]).Value;
            var productSign = LimbBig.MultiplyByInt64(
                destination: productScratch,
                multiplier: mRaw,
                source: top,
                sourceSign: topSign
            );

            destinationSigns[k] = LimbBig.AddInto(
                destination: cell,
                destinationSign: sourceSigns[(k - 1)],
                addend: productScratch,
                addendSign: ((sbyte)(-productSign))
            );
        }

        var lowCell = destination.Slice(
            length: width,
            start: 0
        );
        var m0 = Unsafe.BitCast<TScalar, FixedQ4816>(source: m_modulus[0]).Value;

        destinationSigns[0] = ((sbyte)(-LimbBig.MultiplyByInt64(
            destination: lowCell,
            multiplier: m0,
            source: top,
            sourceSign: topSign
    ...
    private static int LimbToeplitzMultiply(Span<ulong> column, ReadOnlySpan<sbyte> columnSigns, int columnLength, Span<ulong> vector, Span<sbyte> vectorSigns, int vectorLength, int width, Span<ulong> product, Span<ulong> resultScratch) {
        var resultMagnitudes = resultScratch.Slice(
            length: (columnLength * width),
            start: 0
        );
        var resultSigns = ((columnLength <= LimbStackColumnLimit)
            ? stackalloc sbyte[LimbStackColumnLimit]
            : new sbyte[columnLength]
        );

        resultSigns = resultSigns.Slice(
            length: columnLength,
            start: 0
        );
        ...
        for (var row = 0; (row < columnLength); ++row) {
            ...
            for (var index = 0; (index < vectorLength); ++index) {
                ...
                if (
                    (0 <= diagonal) &&
                    (diagonal < columnLength)
                ) {
                    var factorSign = LimbBig.MultiplyFull(
                        destination: product,
                        left: column.Slice(
                            length: width,
                            start: (diagonal * width)
                        ),
                        leftSign: columnSigns[diagonal],
                        right: vector.Slice(
                            length: width,
                            start: (index * width)
                        ),
                        rightSign: vectorSigns[index]
                    );

                    rowSign = LimbBig.AddInto(
                        destination: resultMagnitudes.Slice(
                            length: width,
                            start: (row * width)
                        ),
                        destinationSign: rowSign,
        ...
        resultMagnitudes.CopyTo(destination: vector.Slice(
            length: (columnLength * width),
            start: 0
        ));
        resultSigns.CopyTo(destination: vectorSigns.Slice(
            length: columnLength,
            start: 0
        ));
    ...
    private Element MultiplyIntegerCascade(Element left, Element right) {
        ...
        var columns = cells.Slice(
            length: total,
            start: 0
        );
        var tailInteger = cells.Slice(
            length: n,
            start: total
        );
        ...
        for (var j = 0; (j < n); ++j) {
            tailInteger[j] = (Unsafe.BitCast<TScalar, FixedQ4816>(source: m_modulus[j]).Value >> FixedQ4816.FractionBitCount);
        }
    ...
    private Element MultiplyLimb(Element left, Element right) {
        ...
        var pool = RentLimbCells(
            cellCount: cellCount,
            rented: out rented,
            stackBuffer: stackalloc ulong[LimbStackCellLimit],
            width: width
        );
        var productScratch = pool.Slice(
            length: width,
            start: (total * width)
        );
        var signs = ((total <= LimbStackColumnLimit)
            ? stackalloc sbyte[LimbStackColumnLimit]
            : new sbyte[total]
        );
        ...
        for (var i = 0; (i < n); ++i) {
            ...
            for (var j = 0; (j < n); ++j) {
                ...
                var productSign = LimbBig.SetFromInt128(
                    magnitude: productScratch,
                    value: product
                );
                var target = pool.Slice(
                    length: width,
                    start: ((i + j) * width)
                );

                signs[(i + j)] = LimbBig.AddInto(
                    destination: target,
                    destinationSign: signs[(i + j)],
                    addend: productScratch,
                    addendSign: productSign
                );
            }
        }
        ...
        for (var d = (total - 1); (d >= n); --d) {
            ...
            for (var j = 0; (j < n); ++j) {
                var mRaw = Unsafe.BitCast<TScalar, FixedQ4816>(source: m_modulus[j]).Value;
                var productSign = LimbBig.MultiplyByInt64(
                    destination: productScratch,
                    multiplier: mRaw,
                    source: carry,
                    sourceSign: carrySign
                );

                LimbBig.ShiftRightExact(
                    bits: FixedQ4816.FractionBitCount,
                    magnitude: productScratch
                );
                ...
                signs[((d - n) + j)] = LimbBig.AddInto(
                    destination: target,
                    destinationSign: signs[((d - n) + j)],
                    addend: productScratch,
                    addendSign: ((sbyte)(-productSign))
                );
            }
    ...
    private static TScalar[] MultiplyToeplitz(TScalar[] column, TScalar[] vector) {
        ...
        for (var row = 0; (row < result.Length); ++row) {
            ...
            for (var index = 0; (index < vector.Length); ++index) {
                ...
                if (
                    (0 <= diagonal) &&
                    (diagonal < column.Length)
                ) { accumulated = (accumulated + (column[diagonal] * vector[index])); }
            }
    ...
    private TScalar NormIntegerInt128(Element value) {
        ...
        for (var index = 0; (index < n); ++index) {
            tailInteger[index] = (Unsafe.BitCast<TScalar, FixedQ4816>(source: m_modulus[index]).Value >> FixedQ4816.FractionBitCount);
        ...
        var determinant = Int128CofactorDeterminant(
            matrix: matrix,
            order: n
        );
        var shift = (FixedQ4816.FractionBitCount * (n - 1));

        return Unsafe.BitCast<FixedQ4816, TScalar>(source: FixedQ4816.FromRawBits(value: FixedQ4816.RoundProduct(
    ...
    private TScalar NormLimb(Element value) {
        ...
        var matrix = pool.Slice(
            length: ((n * n) * width),
            start: 0
        );
        var columnBlock = pool.Slice(
            length: (n * width),
            start: ((n * n) * width)
        );
        var nextBlock = pool.Slice(
            length: (n * width),
            start: (((n * n) + n) * width)
        );
        var productScratch = pool.Slice(
        ...
        var matrixSigns = (((n * n) <= LimbStackCellLimit)
            ? stackalloc sbyte[LimbStackCellLimit]
            : new sbyte[(n * n)]
        );
        var columnSigns = ((n <= LimbStackColumnLimit)
            ? stackalloc sbyte[LimbStackColumnLimit]
            : new sbyte[n]
        );
        var nextSigns = ((n <= LimbStackColumnLimit)
            ? stackalloc sbyte[LimbStackColumnLimit]
            : new sbyte[n]
        );

        matrixSigns = matrixSigns.Slice(
            length: (n * n),
            start: 0
        );
        columnSigns = columnSigns.Slice(
            length: n,
            start: 0
        );
        nextSigns = nextSigns.Slice(
            length: n,
            start: 0
        );

        for (var index = 0; (index < n); ++index) {
            columnSigns[index] = LimbBig.SetFromInt64(
                magnitude: columnBlock.Slice(
                    length: width,
                    start: (index * width)
                ),
                value: Unsafe.BitCast<TScalar, FixedQ4816>(source: value[index]).Value
            );
        }
        ...
        for (var columnIndex = 0; (columnIndex < n); ++columnIndex) {
            for (var rowIndex = 0; (rowIndex < n); ++rowIndex) {
                columnBlock.Slice(
                    length: width,
                    start: (rowIndex * width)
                ).CopyTo(destination: matrix.Slice(
                    length: width,
                    start: (((rowIndex * n) + columnIndex) * width)
                ));
            ...
            if (columnIndex < (n - 1)) {
                LimbColumnByRoot(
                    ...
                    destinationSigns: nextSigns,
                    productScratch: productScratch,
                    source: columnBlock,
                    sourceSigns: columnSigns,
                    width: width
                );
        ...
        var determinantSign = LimbBerkowitzDeterminant(
            matrix: matrix,
            matrixSigns: matrixSigns,
            order: n,
            result: productScratch,
            width: width
        );
    ...
    private Projective ProjectiveStepFused(Projective window) {
        ...
        for (var index = 0; (index < n); ++index) {
            var mRaw = Unsafe.BitCast<TScalar, FixedQ4816>(source: m_modulus[((n - 1) - index)]).Value;
    ...
    private void ReduceInPlace(TScalar[] wide) {
        ...
        for (var degree = (wide.Length - 1); (degree >= n); --degree) {
            ...
            for (var j = 0; (j < n); ++j) { wide[((degree - n) + j)] = (wide[((degree - n) + j)] - (carry * m_modulus[j])); }
    ...
    [MethodImpl(MethodImplOptions.NoInlining)]
    private static TScalar[] ThrowUninitialized() =>
        throw new InvalidOperationException(message: "The monogenic algebra is default-initialized and names no modulus; construct it with MonogenicAlgebra<TScalar>.Create before using it.");
    ...
    private TScalar TraceLimb(Element value) {
        ...
        for (var i = 0; (i < n); ++i) {
            ...
            LimbBig.CopyMagnitude(
                destination: productScratch,
                source: columnBlock.Slice(
                    length: width,
                    start: (i * width)
                )
            );
            LimbBig.ShiftLeft(
                bits: (FixedQ4816.FractionBitCount * ((n - 1) - i)),
                magnitude: productScratch
            );

            accumulatorSign = LimbBig.AddInto(
                destination: accumulator,
                destinationSign: accumulatorSign,
                addend: productScratch,
                addendSign: columnSigns[i]
            );
        ...
        var raw = LimbBig.RoundAtShift(
            ...
            shift: (FixedQ4816.FractionBitCount * (n - 1)),
            sign: accumulatorSign
        );
    ...
    private static int VectorHash(TScalar[]? vector) {
        ...
        if (vector is not null) {
            ...
            for (var index = 0; (index < vector.Length); ++index) {
                ...
                hash.Add(value: unchecked((uint)((item is null)
                    ? 0
                    : EqualityComparer<TScalar>.Default.GetHashCode(obj: item))));
            }
        }
    ...
    private static bool VectorsEqual(TScalar[]? left, TScalar[]? right) {
        ...
        for (var index = 0; (index < left.Length); ++index) {
            if (!EqualityComparer<TScalar>.Default.Equals(
                x: left[index],
                y: right[index]
            )) { return false; }
        }
    ...
    public TScalar CharacteristicDiscriminant() {
        ...
        for (var index = 1; (index <= n); ++index) { modulusHigh[index] = m_modulus[(n - index)]; }

        var derivativeHigh = new TScalar[n];

        for (var index = 0; (index < n); ++index) {
            // Coefficient of x^(n-1-index) in f' is (n-index)·[coefficient of x^(n-index) in f].
            derivativeHigh[index] = IntegerScale(
                value: modulusHigh[index],
                count: (n - index)
            );
        }

        var sylvester = SylvesterMatrix(
            highFirstDerivative: derivativeHigh,
            highFirstModulus: modulusHigh
        );
        var resultant = Determinant(
            matrix: sylvester,
            order: ((2 * n) - 1)
        );
    ...
    public Element CompanionPower(ulong exponent) {
        ...
        if (m_delegatesQuadratic) {
            var twinPower = m_quadratic.CompanionPower(exponent: exponent);
    ...
    public static MonogenicAlgebra<TScalar> Create(ReadOnlySpan<TScalar> monicModulus) {
        if (monicModulus.IsEmpty) {
            throw new ArgumentException(
                message: "A monogenic algebra needs a monic modulus of degree at least one.",
                paramName: nameof(monicModulus)
            );
        }
    ...
    public Element FromCoordinates(ReadOnlySpan<TScalar> coordinates) {
        ...
        if (coordinates.Length != n) {
            throw new ArgumentException(
                message: $"An element of a degree-{n} algebra needs exactly {n} coordinates.",
    ...
    public Projective FromWindow(ReadOnlySpan<TScalar> window) {
        ...
        if (window.Length != n) {
            throw new ArgumentException(
                message: $"A window over a degree-{n} algebra needs exactly {n} coordinates.",
    ...
    public Element Multiply(Element left, Element right) {
        ...
        if (m_delegatesQuadratic) {
            var product = m_quadratic.Multiply(
                left: new QuadraticAlgebra<TScalar>.Element(
                    U: left[0],
                    V: left[1]
                ),
                right: new QuadraticAlgebra<TScalar>.Element(
                    U: right[0],
                    V: right[1]
                )
            );
        ...
        if (UsesFusedLanes) {
            return (m_integerTail
                ? MultiplyIntegerCascade(
                    left: left,
                    right: right
                )
                : MultiplyLimb(
    ...
    public Element MultiplyByRoot(Element value) {
        ...
        result[0] = (TScalar.AdditiveIdentity - (top * m_modulus[0]));

        for (var index = 1; (index < n); ++index) { result[index] = (value[(index - 1)] - (top * m_modulus[index])); }
    ...
    public TScalar Norm(Element value) {
        ...
        if (m_delegatesQuadratic) {
            return m_quadratic.Norm(value: new QuadraticAlgebra<TScalar>.Element(
                U: value[0],
                V: value[1]
            ));
        }
    ...
    public Projective ProjectiveStep(Projective window) {
        ...
        if (m_delegatesQuadratic) {
            var next = m_quadratic.MobiusStep(pair: new QuadraticAlgebra<TScalar>.Projective(
                Numerator: window[0],
        ...
        for (var index = 0; (index < n); ++index) { head = (head - (m_modulus[((n - 1) - index)] * window[index])); }
    ...
    public TScalar Trace(Element value) {
        ...
        if (m_delegatesQuadratic) {
            return m_quadratic.Trace(value: new QuadraticAlgebra<TScalar>.Element(
                U: value[0],
                V: value[1]
            ));
        }
