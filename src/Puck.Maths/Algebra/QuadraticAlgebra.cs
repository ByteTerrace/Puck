...
public readonly record struct QuadraticAlgebra<TScalar>
    ...
    private readonly bool m_mIntegerCoefficients;
    ...
    private readonly bool m_mPIsAdditiveIdentity;
    private readonly bool m_mQIsAdditiveIdentity;
    ...
    public QuadraticAlgebra(TScalar p, TScalar q) {
        ...
        m_mIntegerCoefficients = HasIntegerCoefficients(
            p: p,
            q: q
        );
        ...
        m_mPIsAdditiveIdentity = EqualityComparer<TScalar>.Default.Equals(
            x: p,
            y: TScalar.AdditiveIdentity
        );
        m_mQIsAdditiveIdentity = EqualityComparer<TScalar>.Default.Equals(
            x: q,
            y: TScalar.AdditiveIdentity
        );
    }
    ...
    private static bool HasIntegerCoefficients(TScalar p, TScalar q) {
        ...
        return (
            FixedQ4816.IsExactInteger(raw: pRaw) &&
            FixedQ4816.IsExactInteger(raw: qRaw)
        );
    }
    ...
    public bool Equals(QuadraticAlgebra<TScalar> other) =>
        (EqualityComparer<TScalar>.Default.Equals(
            x: P,
            y: other.P
        ) && EqualityComparer<TScalar>.Default.Equals(
            x: Q,
    ...
    public Projective MobiusStep(Projective pair) {
        ...
        return new(
            ...
            Numerator: (m_mPIsAdditiveIdentity
            ? (m_mQIsAdditiveIdentity
                ? TScalar.AdditiveIdentity
                : (Q * pair.Denominator))
            : (m_mQIsAdditiveIdentity
                ? (P * pair.Numerator)
                : ((P * pair.Numerator) + (Q * pair.Denominator)))),
            Denominator: pair.Numerator
        );
    }
    ...
    public Element Multiply(Element left, Element right) {
        ...
        if (typeof(TScalar) == typeof(FixedQ4816)) {
            return (m_mIntegerCoefficients
                ? MultiplyFusedInteger(
                    left: left,
                    right: right
                )
                : MultiplyFusedFractional(
                    left: left,
                    right: right
                )
            );
        ...
        return new(
            U: (m_mQIsAdditiveIdentity
            ? scalarProduct
            : (scalarProduct + (Q * rootProduct))),
            V: (m_mPIsAdditiveIdentity
            ? cross
            : (cross + (P * rootProduct)))
        );
    }
    ...
    public TScalar Norm(Element value) {
        ...
        if (!m_mPIsAdditiveIdentity) { norm = (norm + ((P * value.U) * value.V)); }
        if (!m_mQIsAdditiveIdentity) { norm = (norm - ((Q * value.V) * value.V)); }
