...
public readonly record struct HexagonalIndex
    ...
    public static long Distance(HexagonalIndex left, HexagonalIndex right) {
        ...
        return Math.Max(
            val1: Math.Max(
                val1: Math.Abs(value: q),
                val2: Math.Abs(value: r)
            ),
            val2: Math.Abs(value: (q - r))
        );
    }
    ...
    public static HexagonalIndex FromCoordinate(HexagonalCoordinate coordinate) {
        ...
        if (radius > MaxRadius) { throw new OverflowException(message: "The cell lies outside the complete-ring index domain."); }
    ...
    public HexagonalIndex Translate(HexagonalCoordinate displacement) {
        if (
            ...
            if (
                ...
                if ((radius + amount) > MaxRadius) { throw new OverflowException(message: "The translated cell lies outside the complete-ring index domain."); }
                return new(value: (Value + (amount * (((6 * radius) + (3 * amount)) - 1))));
            }
            return FromCoordinate(coordinate: (Decode(
                offset: offset,
                radius: radius
            ) + displacement));
        }
