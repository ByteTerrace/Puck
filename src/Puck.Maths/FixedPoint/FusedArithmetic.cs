...
public static class FusedArithmetic {
    ...
    // (remainder << fractionBitCount) / denominator and its remainder, for remainder < denominator and a fraction count
    // in [1, 64]: the dividend's high word is then below the divisor, so the x64 128-by-64 divide cannot fault; other
    // hosts take UInt128's own division to the identical quotient and remainder.
...
#pragma warning restore SYSLIB5004
        ...
        return (
            ...
            Remainder: ((ulong)(dividend - (quotient * denominator)))
        );
    }
    ...
    private static long DivideProductSumCore((bool Negative, UInt128 Magnitude) numerator, UInt128 denominatorMagnitude, bool resultNegative) {
        ...
        // The sixteen fraction bits are exactly (remainder << 16) / denominator with its remainder — one division.
        // When the denominator fits a machine word so does the remainder (it is below the denominator), and the
        // shifted dividend's high word (remainder >> 48) is below the divisor, so the hardware 128-by-64 lane applies.
        if (denominatorMagnitude <= ulong.MaxValue) {
            ...
            quotient |= fractionQuotient;
            remainder = fractionRemainder;
        } else {
    ...
    public static bool TryDivideMagnitudeRounded(UInt128 numeratorMagnitude, UInt128 denominatorMagnitude, int fractionBitCount, out UInt128 quotient) {
        ...
        if (
            (fractionBitCount > 0) &&
            (fractionBitCount <= 64) &&
            (denominatorMagnitude <= ulong.MaxValue)
        ) {
            // The fraction bits are exactly (remainder << fractionBitCount) / denominator with its remainder — one
            // division on a dividend whose high word is below the divisor, so the hardware 128-by-64 lane applies.
            var (fractionQuotient, fractionRemainder) = DivideShifted(
                denominator: ((ulong)denominatorMagnitude),
                fractionBitCount: fractionBitCount,
                remainder: ((ulong)remainder)
            );

            result |= fractionQuotient;
            remainder = fractionRemainder;
        } else {
            ...
            // iteration). Doubling unconditionally first can wrap `remainder` past UInt128's own ceiling when it is
            // already in the top half of its range, corrupting the quotient silently.
