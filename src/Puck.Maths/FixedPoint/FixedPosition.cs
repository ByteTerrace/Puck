...
/// </summary>
public readonly record struct FixedPosition
    : IAdditionOperators<FixedPosition, FixedVector3, FixedPosition>,
      ISubtractionOperators<FixedPosition, FixedPosition, FixedVector3> {
    ...
    public static FixedPosition Zero => default;
    ...
    public FixedPosition(long cellX, long cellY, long cellZ, FixedVector3 local) {
        if (!TryCreate(
            cellX: cellX,
            cellY: cellY,
            cellZ: cellZ,
    ...
    private FixedPosition(long cellX, long cellY, long cellZ, FixedVector3 local, CanonicalTag _) {
        CellX = cellX;
        CellY = cellY;
        CellZ = cellZ;
        Local = local;
    ...
    public static FixedPosition operator +(FixedPosition coord, FixedVector3 delta) {
        if (!coord.TryTranslate(
            delta: delta,
            result: out var result
        )) {
    ...
    public static FixedVector3 operator -(FixedPosition coord, FixedPosition origin) =>
        coord.Delta(origin: origin);

    private static FixedPosition CreateCanonical(long cellX, long cellY, long cellZ, long localX, long localY, long localZ) =>
        new(
            cellX: cellX,
            cellY: cellY,
            cellZ: cellZ,
    ...
    /// <summary>Prints the three cell indices with the invariant culture, then the local offset.</summary>
    ...
    /// <remarks>Hand-written because the compiler-synthesized body renders the <see cref="long"/> cell indices through the ambient culture — the one culture read the library's no-current-culture promise would otherwise carry. <see cref="Local"/> renders through <see cref="FixedVector3"/>'s own bounded, invariant formatter.</remarks>
    private bool PrintMembers(StringBuilder builder) {
        builder.Append(value: "CellX = ");
        builder.Append(value: CellX.ToString(provider: CultureInfo.InvariantCulture));
        builder.Append(value: ", CellY = ");
        builder.Append(value: CellY.ToString(provider: CultureInfo.InvariantCulture));
        builder.Append(value: ", CellZ = ");
        builder.Append(value: CellZ.ToString(provider: CultureInfo.InvariantCulture));
        builder.Append(value: ", Local = ");
        builder.Append(value: Local.ToString());
    ...
    private static bool TryDeltaComponent(long cell, long localRaw, long originCell, long originLocalRaw, out long deltaRaw) {
        ...
        if (
            (wideDelta < long.MinValue) ||
            (wideDelta > long.MaxValue)
        ) {
            deltaRaw = default;
    ...
    private static bool TryNormalizeComponent(long cell, Int128 localRaw, out long normalizedCell, out long normalizedLocalRaw) {
        ...
        if (
            (wideCell < long.MinValue) ||
            (wideCell > long.MaxValue)
        ) {
            normalizedCell = default;
            normalizedLocalRaw = default;
    ...
    /// <exception cref="OverflowException">The displacement is outside the signed Q48.16 range.</exception>
    public FixedVector3 Delta(FixedPosition origin) {
        if (!TryDelta(
            delta: out var delta,
            origin: origin
        )) {
    ...
    public static FixedPosition FromLocal(FixedVector3 local) =>
        new(
            cellX: 0L,
            cellY: 0L,
            cellZ: 0L,
    ...
    public FixedPosition Normalize() => this;
    ...
    /// <exception cref="OverflowException">The displacement is outside the signed Q48.16 range.</exception>
    public Vector3 ToRenderRelative(FixedPosition origin) =>
    ...
    public static bool TryCreate(long cellX, long cellY, long cellZ, FixedVector3 local, out FixedPosition result) =>
        TryCreateFromRaw(
            cellX: cellX,
            cellY: cellY,
            cellZ: cellZ,
    ...
    public bool TryDelta(FixedPosition origin, out FixedVector3 delta) {
        if (
            !TryDeltaComponent(
            cell: CellX,
            localRaw: Local.X.Value,
    ...
    /// <exception cref="OverflowException">Canonicalization would move a cell index outside the signed 64-bit range.</exception>
    public FixedPosition WithLocal(FixedVector3 local) =>
        new(
            cellX: CellX,
            cellY: CellY,
            cellZ: CellZ,
