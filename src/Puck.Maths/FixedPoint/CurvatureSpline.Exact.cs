...
internal static class CurvatureSplineExactMath {
    ...
    private static readonly Rational GuardWidth = new(
        BigInteger.One,
        (BigInteger.One << GuardBits)
    );
    ...
    private static readonly Rational RootNudge = new(
        BigInteger.One,
        (BigInteger.One << 200)
    );
    private static readonly Rational RationalZero = new(
        BigInteger.Zero,
        BigInteger.One
    );
    private static readonly Rational RationalTwo = Rational.Two;
    private static readonly Rational OneHalf = new(
        BigInteger.One,
        (2 * BigInteger.One)
    );
    private static readonly Rational ThreeHalves = new(
        (3 * BigInteger.One),
        (2 * BigInteger.One)
    );
    private static readonly Rational NineHalves = new(
        (9 * BigInteger.One),
        (2 * BigInteger.One)
    );
    private static readonly Rational TwentySevenEighths = new(
        (27 * BigInteger.One),
        (8 * BigInteger.One)
    );
    private static readonly Rational TwoThirds = new(
        (2 * BigInteger.One),
        (3 * BigInteger.One)
    );
    private static readonly Rational OneThird = new(
        BigInteger.One,
        (3 * BigInteger.One)
    );
    private static readonly Rational RationalThree = new(
        (3 * BigInteger.One),
        BigInteger.One
    );
    private static readonly Rational RationalFour = new(
        (4 * BigInteger.One),
        BigInteger.One
    );
    ...
    internal static CurvatureSplineSegment CompileSegment(CurvatureSplineKnot start, CurvatureSplineKnot end, int segmentIndex) {
        ...
        if (SignOf(value: (chordLengthSquared - Square(value: ExactQ16(value: CurvatureSpline.MinChordLength)))) < 0) {
            throw new CurvatureSplineException(
                detail: $"the planar chord is shorter than {CurvatureSpline.MinChordLength}.",
                refusal: CurvatureSplineRefusal.ZeroLengthChord,
                segmentIndex: segmentIndex
            );
        }

        var (sin0, cos0) = SecondOrderExactMath.SinCosExact(
            numerator: start.TangentYaw.Value,
            denominator: (1L << FixedQ4816.FractionBitCount)
        );
        var (sin1, cos1) = SecondOrderExactMath.SinCosExact(
            numerator: end.TangentYaw.Value,
            denominator: (1L << FixedQ4816.FractionBitCount)
        );
        var t0X = new Rational(
            cos0,
            (BigInteger.One << SecondOrderExactMath.GuardFractionBitCount)
        );
        var t0Z = new Rational(
            sin0,
            (BigInteger.One << SecondOrderExactMath.GuardFractionBitCount)
        );
        var t1X = new Rational(
            cos1,
            (BigInteger.One << SecondOrderExactMath.GuardFractionBitCount)
        );
        var t1Z = new Rational(
            sin1,
            (BigInteger.One << SecondOrderExactMath.GuardFractionBitCount)
        );
        ...
        var searchHigh = (((Abs(value: chordX) + Abs(value: chordZ)) * new Rational(
            ((BigInteger)CurvatureSpline.MaxTangentChordRatio),
            BigInteger.One
        )) + Rational.One);
        ...
        if (!FixedPointRounding.TryRoundRational(
            denominator: lengthRaw,
            fractionBitCount: CurvatureSpline.CoefficientFractionBitCount,
            numerator: (y1Raw - y0Raw),
            result: out var gradeRaw
        )) {
            throw new CurvatureSplineException(
                detail: "the elevation grade does not fit the Q32 coefficient carrier.",
                refusal: CurvatureSplineRefusal.CarrierOverflow,
                segmentIndex: segmentIndex
            );
        }
        ...
        return new() {
            D0X = RoundQ32(
            detail: "the derivative control point D0.x",
            segmentIndex: segmentIndex,
            value: d0X
        ),
            D0Z = RoundQ32(
            detail: "the derivative control point D0.z",
            segmentIndex: segmentIndex,
            value: d0Z
        ),
            D1X = RoundQ32(
            detail: "the derivative control point D1.x",
            segmentIndex: segmentIndex,
            value: d1X
        ),
            D1Z = RoundQ32(
            detail: "the derivative control point D1.z",
            segmentIndex: segmentIndex,
            value: d1Z
        ),
            D2X = RoundQ32(
            detail: "the derivative control point D2.x",
            segmentIndex: segmentIndex,
            value: d2X
        ),
            D2Z = RoundQ32(
            detail: "the derivative control point D2.z",
            segmentIndex: segmentIndex,
            value: d2Z
        ),
            E0X = RoundQ32(
            detail: "the second-derivative control point E0.x",
            segmentIndex: segmentIndex,
            value: e0X
        ),
            E0Z = RoundQ32(
            detail: "the second-derivative control point E0.z",
            segmentIndex: segmentIndex,
            value: e0Z
        ),
            E1X = RoundQ32(
            detail: "the second-derivative control point E1.x",
            segmentIndex: segmentIndex,
            value: e1X
        ),
            E1Z = RoundQ32(
            detail: "the second-derivative control point E1.z",
            segmentIndex: segmentIndex,
            value: e1Z
        ),
            ArcTable = table,
            ...
            LengthRaw = lengthRaw,
            P0X = RoundQ32(
            detail: "the shared knot P0.x",
            segmentIndex: segmentIndex,
            value: p0X
        ),
            P0Z = RoundQ32(
            detail: "the shared knot P0.z",
            segmentIndex: segmentIndex,
            value: p0Z
        ),
            P1X = RoundQ32(
            detail: "the derived control point P1.x",
            segmentIndex: segmentIndex,
            value: p1X
        ),
            P1Z = RoundQ32(
            detail: "the derived control point P1.z",
            segmentIndex: segmentIndex,
            value: p1Z
        ),
            P2X = RoundQ32(
            detail: "the derived control point P2.x",
            segmentIndex: segmentIndex,
            value: p2X
        ),
            P2Z = RoundQ32(
            detail: "the derived control point P2.z",
            segmentIndex: segmentIndex,
            value: p2Z
        ),
            P3X = RoundQ32(
            detail: "the shared knot P3.x",
            segmentIndex: segmentIndex,
            value: p3X
        ),
            P3Z = RoundQ32(
            detail: "the shared knot P3.z",
            segmentIndex: segmentIndex,
            value: p3Z
        ),
            StationRaw = 0L, // filled in by the caller once every segment's LengthRaw is known.
            Tangent0LengthRaw = RoundQ32(
            detail: "the derived tangent length l0",
            segmentIndex: segmentIndex,
            value: tangent0
        ),
            Tangent1LengthRaw = RoundQ32(
            detail: "the derived tangent length l1",
            segmentIndex: segmentIndex,
            value: tangent1
        ),
            Y0Raw = y0Raw,
    ...
    private static (Rational L0, Rational L1) AdmitOrRefuse(Rational l0, Rational l1, Rational chordLengthSquared, int segmentIndex) {
        if (!IsAdmissible(
            chordLengthSquared: chordLengthSquared,
            l0: l0,
            l1: l1
        )) {
            throw new CurvatureSplineException(
                detail: "the unique tangent-length solution falls outside the authoring bounds.",
                refusal: CurvatureSplineRefusal.CurvatureUnreachable,
                segmentIndex: segmentIndex
            );
        }
    ...
    private static long[] BuildArcTable(Rational d0X, Rational d0Z, Rational d1X, Rational d1Z, Rational d2X, Rational d2Z, int segmentIndex) {
        ...
        var (table, speeds) = BuildArcTableAt(
            coarseSpeeds: null,
            d0X: d0X,
            d0Z: d0Z,
            d1X: d1X,
        ...
        while (true) {
            ...
            if (refinedCount > MaxSubintervalCount) {
                throw new CurvatureSplineException(
                    detail: $"the arc-length table's estimated error did not fall under its scaled bound within {MaxSubintervalCount} panels.",
                    refusal: CurvatureSplineRefusal.ArcLengthErrorUnbounded,
                    segmentIndex: segmentIndex
                );
            }

            var (refinedTable, refinedSpeeds) = BuildArcTableAt(
                coarseSpeeds: speeds,
                d0X: d0X,
                d0Z: d0Z,
                d1X: d1X,
            ...
            var quadratureError = MaxQuadratureError(
                coarse: table,
                fine: refinedTable
            );
            var linearError = MaxLinearizationError(
                speeds: refinedSpeeds,
                subintervalCount: refinedCount
            );
            var boundRaw = ScaledErrorBoundRaw(lengthRaw: refinedTable[^1]);
            var errorBound = new Rational(
                boundRaw,
                (BigInteger.One << CurvatureSpline.CoefficientFractionBitCount)
            );

            if (
                (quadratureError <= boundRaw) &&
                (SignOf(value: (linearError - errorBound)) <= 0)
            ) {
                return refinedTable;
            }
    ...
    private static (long[] Table, Rational[] Speeds) BuildArcTableAt(int subintervalCount, Rational[]? coarseSpeeds, Rational d0X, Rational d0Z, Rational d1X, Rational d1Z, Rational d2X, Rational d2Z, int segmentIndex) {
        ...
        for (var j = 0; (j <= (2 * subintervalCount)); ++j) {
            ...
            var t = new Rational(
                (BigInteger.One * j),
                (BigInteger.One * (2 * subintervalCount))
            );
        ...
        var panelTimesSix = new Rational(
            ((subintervalCount * 6) * BigInteger.One),
            BigInteger.One
        );
        ...
        for (var j = 0; (j < subintervalCount); ++j) {
            ...
            table[(j + 1)] = RoundQ32(
                detail: $"the arc-length table entry at t = {(j + 1)}/{subintervalCount}",
                segmentIndex: segmentIndex,
                value: cumulative
            );
        }
        ...
        for (var j = 1; (j < table.Length); ++j) {
            if (table[j] <= table[(j - 1)]) {
                throw new CurvatureSplineException(
                    detail: "the arc-length table failed to advance strictly — the segment's speed underflowed the Q32 raw carrier.",
                    refusal: CurvatureSplineRefusal.InteriorCusp,
                    segmentIndex: segmentIndex
                );
            }
        }
    ...
    private static (AdmissibilityVerdict Verdict, Rational Lo, Rational Hi) CertifyAdmissibility(Rational[] quartic, Rational lo, Rational hi, Rational s0, Rational kappa0, Rational w, Rational chordLengthSquared) {
        for (var attempt = 0; ; ++attempt) {
            ...
            var verdict = ClassifyAdmissibility(
                chordLengthSquared: chordLengthSquared,
                l0Hi: hi,
                l0Lo: lo,
                l1Hi: l1Hi,
            ...
            if (
                (verdict != AdmissibilityVerdict.Uncertain) ||
                (attempt >= CertificationRefinementBudget)
            ) {
    ...
    private static void CheckInteriorCusp(Rational a0X, Rational a0Z, Rational a1X, Rational a1Z, Rational a2X, Rational a2Z, int segmentIndex) {
        ...
        if (!IsZeroPolynomial(coefficients: derivative)) {
            foreach (var (lo, hi) in IsolateRoots(
        ...
        if (SignOf(value: (minimum - floorSquared)) < 0) {
            throw new CurvatureSplineException(
                detail: "the segment's speed |B'(t)| dips below the speed floor somewhere on [0, 1].",
                refusal: CurvatureSplineRefusal.InteriorCusp,
                segmentIndex: segmentIndex
            );
        }
    ...
    private static AdmissibilityVerdict ClassifyAdmissibility(Rational l0Lo, Rational l0Hi, Rational l1Lo, Rational l1Hi, Rational chordLengthSquared) {
        ...
        var capSquared = (chordLengthSquared * new Rational(
            (((BigInteger)CurvatureSpline.MaxTangentChordRatio) * CurvatureSpline.MaxTangentChordRatio),
            BigInteger.One
        ));

        var (l0SqLo, l0SqHi) = SquareRange(
            hi: l0Hi,
            lo: l0Lo
        );
        var (l1SqLo, l1SqHi) = SquareRange(
            hi: l1Hi,
            lo: l1Lo
        );
    ...
    private static Rational[] DerivativeOf(Rational[] coefficients) {
        ...
        for (var i = 1; (i <= degree); ++i) {
            result[(i - 1)] = (coefficients[i] * new Rational(
                (i * BigInteger.One),
                BigInteger.One
            ));
        }
    ...
    private static Rational ExactQ16(FixedQ4816 value) =>
        new(
            value.Value,
            (BigInteger.One << FixedQ4816.FractionBitCount)
        );
    ...
    private static List<(Rational Lo, Rational Hi)> IsolateRoots(Rational[] polynomial, Rational lo, Rational hi) {
        ...
        var boundedHi = NudgeAwayFromRoot(
            leading: sequence[0],
            x: hi,
            negative: false
        );
        ...
        while (stack.Count > 0) {
            ...
            if (count == 1) {
                ...
                while (!IntervalNarrowerThanGuard(
                    hi: narrowHi,
                    lo: narrowLo
                )) {
                    var mid = NudgeAwayFromRoot(
                        leading: sequence[0],
                        x: ((narrowLo + narrowHi) * OneHalf),
                        negative: false
                    );
                    var leftCount = (SignVariationsAt(
                        sequence: sequence,
                        x: narrowLo
                    ) - SignVariationsAt(
                        sequence: sequence,
    ...
    private static (Rational Lo, Rational Hi) L1Range(Rational lo, Rational hi, Rational s0, Rational kappa0, Rational w) {
        ...
        return ((SignOf(value: (atHi - atLo)) < 0)
            ? (atHi, atLo)
            : (atLo, atHi)
        );
    }
    ...
    private static Rational MaxLinearizationError(Rational[] speeds, int subintervalCount) {
        var deltaT = new Rational(
            BigInteger.One,
            (BigInteger.One * subintervalCount)
        );
    ...
    private static (Rational Lo, Rational Hi) RefineBracket(Rational[] polynomial, Rational lo, Rational hi) {
        var signAtLo = SignOf(value: Evaluate(
        ...
        var signAtMid = SignOf(value: Evaluate(
            coefficients: polynomial,
            x: mid
        ));

        return ((signAtMid == signAtLo)
            ? (mid, hi)
            : (lo, mid)
        );
    }
    ...
    private static long RoundQ32(Rational value, int segmentIndex, string detail) {
        if (!FixedPointRounding.TryRoundRational(
            ...
            throw new CurvatureSplineException(
                detail: $"{detail} does not fit the Q32 coefficient carrier.",
                refusal: CurvatureSplineRefusal.CarrierOverflow,
                segmentIndex: segmentIndex
            );
        }
    ...
    private static long ScaledErrorBoundRaw(long lengthRaw) {
        ...
        return ((scaled > CurvatureSpline.ArcLengthMinimumErrorBoundRaw)
            ? scaled
            : CurvatureSpline.ArcLengthMinimumErrorBoundRaw
        );
    }
    ...
    private static (Rational L0, Rational L1) SolveTangentLengths(Rational chordLengthSquared, Rational kappa0, Rational kappa1, Rational s0, Rational s1, int segmentIndex, Rational searchHigh, Rational w) {
        ...
        if (!wZero) {
            if (
                ...
                var brackets = IsolateRoots(
                    polynomial: quartic,
                    lo: ExactQ16(value: CurvatureSpline.MinTangentLength),
                    hi: searchHigh
                );
                ...
                foreach (var (isolatedLo, isolatedHi) in brackets) {
                    var (verdict, lo, hi) = CertifyAdmissibility(
                        chordLengthSquared: chordLengthSquared,
                        hi: isolatedHi,
                        kappa0: kappa0,
                        lo: isolatedLo,
                    ...
                    if (verdict == AdmissibilityVerdict.Uncertain) {
                        throw new CurvatureSplineException(
                            detail: $"a tangent-length root lies within {CertificationRefinementBudget} extra bisections of an authoring bound; refusing rather than choosing across the boundary.",
                            refusal: CurvatureSplineRefusal.CurvatureUnreachable,
                            segmentIndex: segmentIndex
                        );
                    }

                    var candidate = Recompute(
                        hi: hi,
                        lo: lo
                    );
                    ...
                    for (var attempt = 0; ; ++attempt) {
                        ...
                        if (
                            (SignOf(value: (candidate.FLo - best.Value.FHi)) >= 0) ||
                            (attempt >= CertificationRefinementBudget)
                        ) {
                            break;
                        }
                        ...
                        if (SignOf(value: (bestWidth - candidateWidth)) >= 0) {
                            var (refinedLo, refinedHi) = RefineBracket(
                                polynomial: quartic,
                                lo: best.Value.Lo,
                                hi: best.Value.Hi
                            );
                ...
                if (best is null) {
                    throw new CurvatureSplineException(
                        detail: "no admissible tangent-length pair solves the curvature system within the authoring bounds.",
                        refusal: CurvatureSplineRefusal.CurvatureUnreachable,
                        segmentIndex: segmentIndex
                    );
                }
        ...
        if (!kappa0Zero) {
            ...
            if (SignOf(value: product) <= 0) {
                throw new CurvatureSplineException(
                    detail: "w = 0 and κ0 ≠ 0, but s0·κ0 is not strictly positive, so no real tangent length solves the start curvature.",
                    refusal: CurvatureSplineRefusal.TangentCurvatureInconsistent,
                    segmentIndex: segmentIndex
                );
            }
            ...
            throw new CurvatureSplineException(
                detail: "w = 0 and κ0 = 0, but s0 ≠ 0 — the start tangent and chord geometry cannot meet at zero curvature.",
                refusal: CurvatureSplineRefusal.TangentCurvatureInconsistent,
                segmentIndex: segmentIndex
            );
        } else {
        ...
        if (!kappa1Zero) {
            ...
            if (SignOf(value: product) >= 0) {
                throw new CurvatureSplineException(
                    detail: "w = 0 and κ1 ≠ 0, but s1·κ1 is not strictly negative, so no real tangent length solves the end curvature.",
                    refusal: CurvatureSplineRefusal.TangentCurvatureInconsistent,
                    segmentIndex: segmentIndex
                );
            }
            ...
            throw new CurvatureSplineException(
                detail: "w = 0 and κ1 = 0, but s1 ≠ 0 — the end tangent and chord geometry cannot meet at zero curvature.",
                refusal: CurvatureSplineRefusal.TangentCurvatureInconsistent,
                segmentIndex: segmentIndex
            );
        } else {
    ...
    private static (Rational Lo, Rational Hi) SquareRange(Rational lo, Rational hi) {
        ...
        return (RationalZero, ((SignOf(value: (loSquared - hiSquared)) > 0)
            ? loSquared
            : hiSquared));
    }
