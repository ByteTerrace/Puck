...
Q60 → Q16 to nearest with ties to even on the magnitude and re-signs, as its
turn reduction and radian product also do, so the sine is exactly odd and the
cosine exactly even in the angle. All are internal narrowings inside a kernel
that is already correctly rounded to within an ULP; they are not a second
public rounding discipline.
...
| `SinCos` / `Sin` / `Cos` | Turn-domain reduction of the angle's magnitude by the single Q64 constant `round(2⁶⁴/2π)` — the two's-complement wrap of the 128-bit product *is* the exact mod-one-turn — re-signed, then seven- and eight-term odd and even Taylor polynomials at Q60, narrowed to nearest on the magnitude and clamped to `±1`; `Sin(−θ)` is exactly `−Sin(θ)` and `Cos(−θ)` exactly `Cos(θ)`. Maximum observed 0.51 ULP within a few turns, and around 2 ULP at extreme magnitudes. |
...
`QuadraticSurd`. Every representable `(f, ζ)` pair already resolves under the
