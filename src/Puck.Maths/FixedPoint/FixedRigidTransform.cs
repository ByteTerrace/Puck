...
/// composition chains. Precision of the representation and of composition scales with translation magnitude (about
/// 2⁻¹⁵ relative — the Q16 unit-quaternion norm quantization): sub-millimeter at ten world units.
/// <see cref="ScLerp"/>'s screw path amplifies that envelope by roughly one over the sine of the relative half-angle
/// and carries its own measured band at the member.
/// </summary>
...
public readonly record struct FixedRigidTransform(FixedDual<FixedQuaternion> Value)
    ...
    /// <param name="left">The second transform.</param>
    /// <param name="right">The first transform.</param>
    /// <returns>The normalized composition; <paramref name="right"/> is applied first. A composition whose real
    ...
    public static FixedRigidTransform FromRotationTranslation(FixedQuaternion rotation, FixedVector3 translation) {
        ...
        const int HalvedFractionBitCount = (FixedQ4816.FractionBitCount + 1);

        var (qx, qy, qz, qw) = (rotation.X.Value, rotation.Y.Value, rotation.Z.Value, rotation.W.Value);
        var (tx, ty, tz) = (translation.X.Value, translation.Y.Value, translation.Z.Value);
...
#pragma warning restore SYSLIB5004
    ...
    /// and <see cref="TryFromDualQuaternion"/> reports, answered here under the family's normalize-to-identity
    /// convention (<see cref="FixedQuaternion.Normalize"/>).</returns>
