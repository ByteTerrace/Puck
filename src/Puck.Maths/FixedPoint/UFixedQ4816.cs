...
public readonly partial record struct UFixedQ4816(ulong Value)
    ...
    public static UFixedQ4816 operator *(UFixedQ4816 x, UFixedQ4816 y) {
        ...
        return new UFixedQ4816(Value: FixedPointRounding.RoundHalfToEven(
            remainder: remainder,
            threshold: RawHalf,
            truncated: truncated
        ));
    ...
    public static UFixedQ4816 operator /(UFixedQ4816 x, UFixedQ4816 y) {
        ...
        // The remainder ties against its own complement, not against a fixed half: the divisor is the unit here.
        var remainderComplement = (y.Value - remainder);

        return new UFixedQ4816(Value: FixedPointRounding.RoundHalfToEven(
            remainder: remainder,
            threshold: remainderComplement,
            truncated: truncated
        ));
    }
    ...
    /// <param name="shiftAmount">The number of bit positions to shift left. The count acts modulo sixty-four, following the raw <see cref="ulong"/> storage: an amount of sixty-four returns the operand unchanged, and a negative amount shifts by its masked residue.</param>
    ...
    /// <param name="value">The value to shift.</param>
    /// <param name="shiftAmount">The number of bit positions to shift right. The count acts modulo sixty-four, following the raw <see cref="ulong"/> storage: an amount of sixty-four returns the operand unchanged, and a negative amount shifts by its masked residue.</param>
    ...
    /// <exception cref="DivideByZeroException"><paramref name="y"/> is zero. Unchecked names the range check, never the divisor.</exception>
    /// <remarks>
...
#pragma warning restore SYSLIB5004
    ...
    // even when rounding would land it on zero, and above-MaxValue text refuses even when rounding would land it
    // back inside, where the styled surface rounds first at both ends and answers Zero or MaxValue instead.
    ...
    private static bool TryParseCore(ReadOnlySpan<char> s, IFormatProvider? provider, out UFixedQ4816 result) {
        ...
        if (FixedPointParseStatus.Success != FixedPointText.Parse(
            s: s,
            style: NumberStyles.AllowLeadingWhite | NumberStyles.AllowTrailingWhite | NumberStyles.AllowLeadingSign | NumberStyles.AllowDecimalPoint,
            provider: provider,
            fractionBitCount: FractionBitCount,
