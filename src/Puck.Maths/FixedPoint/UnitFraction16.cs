...
public readonly record struct UnitFraction16(ushort Value)
    : IComparable,
      IComparable<UnitFraction16>,
      IComparisonOperators<UnitFraction16, UnitFraction16, bool>,
      IAdditionOperators<UnitFraction16, UnitFraction16, UnitFraction16>,
      ISubtractionOperators<UnitFraction16, UnitFraction16, UnitFraction16>,
      IMultiplyOperators<UnitFraction16, UnitFraction16, UnitFraction16>,
      IDivisionOperators<UnitFraction16, UnitFraction16, UnitFraction16>,
      IModulusOperators<UnitFraction16, UnitFraction16, UnitFraction16>,
      IBitwiseOperators<UnitFraction16, UnitFraction16, UnitFraction16>,
      IShiftOperators<UnitFraction16, int, UnitFraction16>,
      IMinMaxValue<UnitFraction16>,
      IAdditiveIdentity<UnitFraction16, UnitFraction16>,
      ISpanFormattable,
      ISpanParsable<UnitFraction16> {
    ...
    /// <summary>Converts a <see cref="UnitFraction16"/> to its exact <see cref="double"/> value.</summary>
    /// <param name="value">The value to convert.</param>
    /// <returns>The real value of <paramref name="value"/>, a number in <c>[0, 1)</c>, as a <see cref="double"/>.</returns>
    public static explicit operator double(UnitFraction16 value) =>
    ...
    public static UnitFraction16 operator ~(UnitFraction16 value) =>
    ...
    /// <returns>The sum <c><paramref name="x"/> + <paramref name="y"/></c>. Use <see cref="AddSaturating(UnitFraction16, UnitFraction16)"/> to clamp instead of wrap.</returns>
    public static UnitFraction16 operator +(UnitFraction16 x, UnitFraction16 y) =>
    ...
    /// <returns>The difference <c><paramref name="x"/> − <paramref name="y"/></c>. Use <see cref="SubtractSaturating(UnitFraction16, UnitFraction16)"/> to clamp instead of wrap.</returns>
    public static UnitFraction16 operator -(UnitFraction16 x, UnitFraction16 y) =>
        new(Value: unchecked((ushort)(x.Value - y.Value)));
    ...
    public static UnitFraction16 operator *(UnitFraction16 x, UnitFraction16 y) {
        ...
        return new(Value: ((ushort)FixedPointRounding.RoundHalfToEven(
            remainder: remainder,
            threshold: RawHalf,
            truncated: truncated
        )));
    ...
    public static UnitFraction16 operator /(UnitFraction16 x, UnitFraction16 y) {
        ...
        // The doubled remainder ties against the divisor itself, so the threshold is the divisor, not a fixed half.
        var twiceRemainder = (remainder << 1);
        var rounded = FixedPointRounding.RoundHalfToEven(
            remainder: twiceRemainder,
            threshold: ((uint)y.Value),
            truncated: quotient
        );

        // The saturation stays outside the correction: a quotient that reaches one clamps AFTER rounding.
    ...
    /// <returns><paramref name="value"/> with its raw bits shifted left, wrapping on overflow. The sixteen-bit storage promotes to a thirty-two-bit word before the shift, so <paramref name="shiftAmount"/> acts MODULO THIRTY-TWO — the mask is 31, not 15: an amount of thirty-two returns <paramref name="value"/> unchanged, and a negative amount shifts by its masked residue.</returns>
    public static UnitFraction16 operator <<(UnitFraction16 value, int shiftAmount) =>
    ...
    /// <returns><paramref name="value"/> with its raw bits shifted right. Because the storage is unsigned, this is equivalent to the unsigned shift <c>&gt;&gt;&gt;</c>. The sixteen-bit storage promotes to a thirty-two-bit word before the shift, so <paramref name="shiftAmount"/> acts MODULO THIRTY-TWO — the mask is 31, not 15: an amount of thirty-two returns <paramref name="value"/> unchanged, and a negative amount shifts by its masked residue.</returns>
    public static UnitFraction16 operator >>(UnitFraction16 value, int shiftAmount) =>
    ...
    /// <param name="value">The value to shift.</param>
    /// <param name="shiftAmount">The number of bit positions to shift right.</param>
    /// <returns><paramref name="value"/> with its raw bits shifted right. The sixteen-bit storage promotes to a thirty-two-bit word before the shift, so <paramref name="shiftAmount"/> acts MODULO THIRTY-TWO — the mask is 31, not 15: an amount of thirty-two returns <paramref name="value"/> unchanged, and a negative amount shifts by its masked residue.</returns>
    public static UnitFraction16 operator >>>(UnitFraction16 value, int shiftAmount) =>
        new(Value: ((ushort)(value.Value >>> shiftAmount)));
    ...
    public static bool operator <(UnitFraction16 x, UnitFraction16 y) =>
        (x.Value < y.Value);
    ...
    /// <summary>Converts a <see cref="double"/> to a <see cref="UnitFraction16"/>, rounding to nearest with ties to even.</summary>
    /// <param name="value">The value to convert.</param>
    /// <returns>The nearest representable <see cref="UnitFraction16"/>, clamped to <c>[0, <see cref="MaxValue"/>]</c>. Inputs at or above one, as well as negative and not-a-number inputs, clamp into range.</returns>
    [MethodImpl(methodImplOptions: MethodImplOptions.AggressiveInlining)]
    ...
    /// <summary>Constructs a <see cref="UnitFraction16"/> directly from a raw storage bit pattern.</summary>
    /// <param name="value">The pre-scaled raw value to wrap, interpreted as the real number <c><paramref name="value"/> / 2¹⁶</c>.</param>
    /// <returns>A <see cref="UnitFraction16"/> whose <see cref="Value"/> equals <paramref name="value"/>.</returns>
    [MethodImpl(methodImplOptions: MethodImplOptions.AggressiveInlining)]
    public static UnitFraction16 FromRawBits(ushort value) =>
        new(Value: value);
    ...
    /// <summary>Compares this instance with a boxed <see cref="UnitFraction16"/> and indicates their relative order.</summary>
    ...
    /// <exception cref="ArgumentException"><paramref name="obj"/> is neither <see langword="null"/> nor a <see cref="UnitFraction16"/>.</exception>
    public int CompareTo(object? obj) {
        if (obj is null) { return 1; }
        if (obj is UnitFraction16 other) { return CompareTo(other: other); }

        throw new ArgumentException(
            message: $"Object must be of type {nameof(UnitFraction16)}.",
            paramName: nameof(obj)
        );
    }
    /// <summary>Compares this instance with another <see cref="UnitFraction16"/> and indicates their relative order.</summary>
    ...
    [MethodImpl(methodImplOptions: MethodImplOptions.AggressiveInlining)]
    public int CompareTo(UnitFraction16 other) =>
        Value.CompareTo(value: other.Value);
    /// <summary>Parses a character span into a <see cref="UnitFraction16"/>.</summary>
    ...
    /// <exception cref="FormatException"><paramref name="s"/> is not a valid, in-range <see cref="UnitFraction16"/> literal.</exception>
    public static UnitFraction16 Parse(ReadOnlySpan<char> s, IFormatProvider? provider) {
        if (!TryParseCore(
            s: s,
            provider: provider,
            result: out var result
        )) {
            throw new FormatException(message: $"The input span was not in a valid {nameof(UnitFraction16)} format.");
        }

        return result;
    }
    /// <summary>Parses a string into a <see cref="UnitFraction16"/>.</summary>
    ...
    /// <exception cref="FormatException"><paramref name="s"/> is not a valid, in-range <see cref="UnitFraction16"/> literal.</exception>
    public static UnitFraction16 Parse(string s, IFormatProvider? provider) {
    ...
    /// <summary>Tries to parse a character span into a <see cref="UnitFraction16"/>.</summary>
    ...
    public static bool TryParse(ReadOnlySpan<char> s, IFormatProvider? provider, out UnitFraction16 result) =>
        TryParseCore(
        s: s,
        provider: provider,
        result: out result
    );
    /// <summary>Tries to parse a string into a <see cref="UnitFraction16"/>.</summary>
    ...
    public static bool TryParse(string? s, IFormatProvider? provider, out UnitFraction16 result) {
        if (s is null) {
            result = default;
    ...
    private static bool TryParseCore(ReadOnlySpan<char> s, IFormatProvider? provider, out UnitFraction16 result) {
        result = default;
