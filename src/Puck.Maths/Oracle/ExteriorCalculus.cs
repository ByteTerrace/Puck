...
public sealed class ExteriorCalculus<TValue, TOps>
    ...
    private ExteriorCalculus(
        ...
        m_counit = poset.Algebra.FromSupport(
            keys: [counitKey],
            coefficients: [poset.Algebra.Presentation.Material.One]
        );
    ...
    public PresentedAlgebra<TValue, TOps>.Element Boundary(in PresentedAlgebra<TValue, TOps>.Element chain) {
        Poset.Algebra.RequireOwned(
            value: chain,
            paramName: nameof(chain)
        );

        return Poset.Algebra.Multiply(
            left: Incidence,
            right: chain
        );
    }
    ...
    public void BoundaryMatrix(int degree, Span<TValue> entries) {
        ...
        if (entries.Length != (rows.Length * columns.Length)) {
            throw new ArgumentException(
                message: "The destination is not exactly the size of this graded boundary matrix.",
                paramName: nameof(entries)
            );
        }

        for (var column = 0; (column < columns.Length); ++column) {
            var image = Boundary(chain: Poset.Algebra.FromSupport(
                keys: [m_chainKey[columns[column]]],
                coefficients: [Poset.Algebra.Presentation.Material.One]
            ));
    ...
    public PresentedAlgebra<TValue, TOps>.Element Chain(ReadOnlySpan<TValue> values) {
        ...
        return Poset.Algebra.FromSupport(
            keys: m_chainKey.AsSpan(
                start: 0,
    ...
    public static ExteriorCalculus<TValue, TOps> Create(ReadOnlySpan<int> dimensions, ReadOnlySpan<(int Face, int Coface, int Sign)> incidences, TOps material) {
        ...
        ArgumentOutOfRangeException.ThrowIfGreaterThan(
            value: cellCount,
            other: MaximumCellCount,
            paramName: nameof(dimensions)
        );

        if (material is not ISignedMaterial<TValue, TOps> signed) {
            throw new ArgumentException(
                message: "An incidence number is a sign, which an unsigned material cannot express.",
        ...
        foreach (var (face, coface, sign) in incidences) {
            if (
                ...
                throw new ArgumentException(
                    message: "An incidence names a cell outside this complex.",
                    paramName: nameof(incidences)
                );
            }

            if (dimensions[coface] != (dimensions[face] + 1)) {
                throw new ArgumentException(
                    message: "An incidence relates a face to a coface of one higher dimension, and this pair does not step by one.",
                    paramName: nameof(incidences)
                );
            }
            ...
            if (
                ...
                throw new ArgumentException(
                    message: "An orientation sign is one or minus one.",
                    paramName: nameof(incidences)
                );
            }
            ...
            if (declared[((face * cellCount) + coface)]) {
                throw new ArgumentException(
                    message: "An incidence pair is declared twice, and the second number would silently add to the first.",
                    paramName: nameof(incidences)
                );
            }
        ...
        var poset = IncidenceAlgebra<TValue, TOps>.Create(
            elementCount: (cellCount + 2),
            relations: CollectionsMarshal.AsSpan(list: relations),
            material: material
        );
        ...
        for (var cell = 0; (cell < cellCount); ++cell) {
            _ = poset.TryKey(
                lower: cell,
                upper: top,
                key: out chainKey[cell]
            );
            _ = poset.TryKey(
                lower: bottom,
                upper: cell,
                key: out cochainKey[cell]
            );
        }

        _ = poset.TryKey(
            key: out var counitKey,
            lower: bottom,
            upper: top
        );
        ...
        return new(
            ...
            counitKey: counitKey,
            incidence: poset.Algebra.FromSupport(
    ...
    public bool TryEulerCharacteristic(out TValue characteristic, out SumClosureObstruction obstruction) {
        ...
        characteristic = material.Add(
            left: material.One,
            right: mobius[m_counitKey]
        );
