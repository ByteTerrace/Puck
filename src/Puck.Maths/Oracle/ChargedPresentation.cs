...
public readonly struct RewriteRule<TValue> {
    ...
    public static ReadOnlyMemory<int> PackReplacement(params int[][] terms) {
        ...
        foreach (var term in terms) {
            ...
            term.CopyTo(array: packed, index: offset);
            offset += term.Length;
        }
...
public sealed class ChargedPresentation<TValue, TOps>
    ...
    private ChargedPresentation(
        ...
        Lane = ClassifyLane(rules: rules, generatorCharges: generatorCharges);
        Material = material;
        ...
        m_spliceCharge = [(((declared < 0) || declaredTable) ? material.One : rules[declared].Charges[0])];
        m_spliceFold = [0L];
        m_spliceScale = 0;
        m_spliceStride = 0;
        m_spliceUnit = 0L;

        BuildRuleIndex(rules: rules, generatorCount: generators.Length, bucketStart: out m_ruleBucketStart, bucket: out m_ruleBucket, emptyPatternRules: out m_emptyPatternRules);

        var identity = new KeyedCombination();
        var identityScratch = new Combination();
        var discovered = Discover(boundedness: boundedness, status: out var discoveryStatus);
        ...
        if (discovered is null) {
            ...
            if (TryRewriteToKeys(word: [], charge: material.One, stepLimit: RewriteStepLimit, result: identity, combination: identityScratch, stepsTaken: out _)) {
        ...
        for (var index = 0; (index < NormalFormCount); ++index) {
            discovered[index].CopyTo(array: m_normalFormSymbols, index: m_normalFormStart[index]);
        }

        BuildTrie(words: discovered, child: out var trieChild, key: out var trieKey);
        ...
        if (!TryRewriteToKeysDetailed(
            ...
            BasisStatus = StatusForFailure(
                ...
                amountReached: ((RewriteFailure.StepLimit == identityFailure) ? identitySteps : identityAmount)
            );
            HasCompiledNormalFormBasis = false;
            NormalFormCount = 0;
            m_normalFormStart = [];
    ...
    public static ChargedPresentation<TValue, TOps> Create(
        ...
        for (var index = 0; (index < admittedGenerators.Length); ++index) {
            ...
            if (generator.Symbol != index) {
                throw new ArgumentException(message: "A generator's symbol must equal its index in the generator list.", paramName: nameof(generators));
            }

            ArgumentOutOfRangeException.ThrowIfNegative(value: generator.Degree, paramName: nameof(generators));

            foreach (var colour in generator.Inputs) { colourCount = Math.Max(val1: colourCount, val2: AdmitColour(colour: colour, symbol: index, boundary: "input", paramName: nameof(generators))); }
            foreach (var colour in generator.Outputs) { colourCount = Math.Max(val1: colourCount, val2: AdmitColour(colour: colour, symbol: index, boundary: "output", paramName: nameof(generators))); }
        }

        if ((0 != admittedGeneratorCharges.Length) && (admittedGeneratorCharges.Length != admittedGenerators.Length)) {
            throw new ArgumentOutOfRangeException(paramName: nameof(generatorCharges), message: "The generator charges must be empty or carry one entry per generator.");
        }

        ValidateCanonicalCharges(rules: admittedRules, generatorCharges: admittedGeneratorCharges, material: material);
        ValidateRules(rules: admittedRules, generatorCount: admittedGenerators.Length, material: material);
    ...
    public ReadOnlySpan<int> NormalFormWord(long key) {
        if ((key < 0L) || (key >= NormalFormCount)) {
            throw new ArgumentOutOfRangeException(paramName: nameof(key), message: "The key names no normal form of this presentation.");
        }

        var index = ((int)key);

        return m_normalFormSymbols.AsSpan(start: m_normalFormStart[index], length: (m_normalFormStart[(index + 1)] - m_normalFormStart[index]));
    }
    ...
    internal bool TryKeyOf(ReadOnlySpan<int> word, out long key) {
        if (!HasCompiledNormalFormBasis) {
            return TryPackWord(word: word, key: out key);
        }
    ...
    internal bool TryWordOf(long key, Span<int> word, out int length) {
        if (!HasCompiledNormalFormBasis) { return TryUnpackWord(key: key, word: word, length: out length); }

        length = 0;

        if ((key < 0L) || (key >= NormalFormCount)) { return false; }
        ...
        m_normalFormSymbols.AsSpan(start: start, length: symbols).CopyTo(destination: word);
        length = symbols;
    ...
    private bool TryRewriteDetailed(
        ...
        if (material.IsZero(value: charge) || ExceedsWindow(word: word)) { return true; }

        Span<int> rewritten = stackalloc int[MaximumWordLength];

        combination.Merge(word: word, charge: charge, material: material);
        ...
        while (true) {
            ...
            while (--index >= 0) {
                if (TryFindRedex(word: combination.WordAt(index: index), position: out position, ruleIndex: out ruleIndex)) { break; }
            }
            ...
            var source = combination.TakeAt(index: index, charge: out var sourceCharge);
            ...
            for (var term = 0; (term < rule.TermCount); ++term) {
                ...
                var termCharge = material.Multiply(left: sourceCharge, right: rule.Charges[term]);

                if (!material.IsZero(value: termCharge)) {
                    var product = rewritten.Slice(start: 0, length: grown);

                    source.AsSpan(start: 0, length: position).CopyTo(destination: product);
                    replacement.Slice(start: offset, length: length).CopyTo(destination: product.Slice(start: position, length: length));
                    ...
                    if (!ExceedsWindow(word: product)) {
                        combination.Merge(word: product, charge: termCharge, material: material);
                    }
                }
    ...
    private bool TryRewriteToKeysDetailed(
        ...
        for (var index = 0; (index < combination.Count); ++index) {
            ...
            if (!TryKeyOf(word: normal, key: out var key)) {
            ...
            result.Merge(key: key, charge: combination.ChargeAt(index: index), material: Material);
        }
    ...
    private static void BuildRuleIndex(RewriteRule<TValue>[] rules, int generatorCount, out int[] bucketStart, out int[] bucket, out int[] emptyPatternRules) {
        ...
        for (var index = 0; (index < rules.Length); ++index) {
            ...
            if ((RuleKind.Reassociate == rule.Kind) || (0 == rule.Pattern.Length)) { continue; }
    ...
    private static int CompareWordArrays(int[] left, int[] right) =>
        CompareWords(left: left, right: right);
    ...
    private static void ValidateCanonicalCharges(
        ...
        for (var index = 0; (index < generatorCharges.Length); ++index) {
            ...
            if (!comparer.Equals(x: generatorCharges[index], y: canonical)) {
                throw new ArgumentOutOfRangeException(
                    paramName: nameof(generatorCharges),
        ...
        for (var ruleIndex = 0; (ruleIndex < rules.Length); ++ruleIndex) {
            ...
            for (var term = 0; (term < charges.Length); ++term) {
                ...
                if (!comparer.Equals(x: charges[term], y: canonical)) {
                    throw new ArgumentException(
    ...
    private static void ValidateRules(ReadOnlySpan<RewriteRule<TValue>> rules, int generatorCount, TOps material) {
        for (var index = 0; (index < rules.Length); ++index) {
            ...
            foreach (var symbol in rule.Pattern) {
                if ((symbol < 0) || (symbol >= generatorCount)) {
                    throw new ArgumentException(message: "A rule pattern references a symbol outside the generator range.", paramName: nameof(rules));
                }
            }
            ...
            if (RuleKind.Reassociate == rule.Kind) {
                ...
                if (1 == rule.TermCount) {
                    if (!EqualityComparer<TValue>.Default.Equals(x: rule.Charges[0], y: material.One)) {
                        throw new ArgumentException(message: "A uniform re-association charge is applied once per bracket splice, and the unit spelled as the empty product splices one time fewer, so a charge that is not the material's one makes two spellings of one element differ.", paramName: nameof(rules));
                    }
                ...
                if (generatorCount > MaximumReassociationGenerators) {
                    throw new ArgumentException(message: $"A re-association 3-cochain is one charge per ordered generator triple, which is declared for at most {MaximumReassociationGenerators} generators.", paramName: nameof(rules));
                }

                if (rule.TermCount != ((generatorCount * generatorCount) * generatorCount)) {
                    throw new ArgumentException(message: "A re-association rule carries either one charge, the uniform per-splice charge, or one per ordered generator triple, the associator 3-cochain.", paramName: nameof(rules));
                }
            ...
            for (var term = 0; (term < rule.TermCount); ++term) {
                if (offset >= replacement.Length) {
                    throw new ArgumentException(message: "A rule's packed replacement carries fewer terms than it has charges.", paramName: nameof(rules));
                }

                var length = replacement[offset++];

                if ((length < 0) || ((offset + length) > replacement.Length)) {
                    throw new ArgumentException(message: "A rule's packed replacement is malformed.", paramName: nameof(rules));
                }

                if ((0 == rule.Pattern.Length) && (0 == length)) {
                    throw new ArgumentException(message: "An empty-pattern rule may not rewrite the unit word to itself.", paramName: nameof(rules));
                }

                for (var symbol = 0; (symbol < length); ++symbol) {
                    var value = replacement[(offset + symbol)];

                    if ((value < 0) || (value >= generatorCount)) {
                        throw new ArgumentException(message: "A rule replacement references a symbol outside the generator range.", paramName: nameof(rules));
                    }
                }
            ...
            if (offset != replacement.Length) {
                throw new ArgumentException(message: "A rule's packed replacement carries more terms than it has charges.", paramName: nameof(rules));
            }
        }
    }
    ...
    private List<int[]>? Discover(NormalFormBoundedness boundedness, out NormalFormBasisStatus status) {
        ...
        bool Admit(int[] candidate) {
            ...
            while (low < high) {
                ...
                var order = CompareWordArrays(left: ordered[middle], right: candidate);
            ...
            ordered.Insert(index: low, item: candidate);
        ...
        bool AdmitNormalizationOf(ReadOnlySpan<int> word) {
            if (!TryRewriteDetailed(
                ...
                failureStatus = StatusForFailure(
                    ...
                    stage: NormalFormBasisStage.Discovery,
                    failure: failure,
                    amountReached: ((RewriteFailure.StepLimit == failure) ? stepsTaken : amountReached)
                );
        ...
        for (var cursor = 0; (cursor < frontier.Count); ++cursor) {
            ...
            for (var symbol = 0; (symbol < GeneratorCount); ++symbol) {
                var candidate = extended.Slice(start: 0, length: (word.Length + 1));
    ...
    private long[] BuildSpliceFold() {
        ...
        if (count != GeneratorCount) {
            throw new ArgumentException(message: "A declared re-association 3-cochain indexes generator triples, which name normal forms only when every generator is its own normal form.", paramName: "rules");
        }

        for (var key = 0; (key < count); ++key) {
            if ((1 != (m_normalFormStart[(key + 1)] - m_normalFormStart[key])) || (key != m_normalFormSymbols[m_normalFormStart[key]])) {
                throw new ArgumentException(message: "A declared re-association 3-cochain indexes generator triples, which name normal forms only when every generator is its own normal form.", paramName: "rules");
            }
        }

        if (1 != m_identityKeys.Length) {
            throw new ArgumentException(message: "A declared re-association 3-cochain reads the key a bracket's contents reach, so the unit must be a single normal form.", paramName: "rules");
        }
        ...
        for (var cell = 0; (cell < fold.Length); ++cell) {
            if (1L != (m_cellStart[(cell + 1)] - m_cellStart[cell])) {
                throw new ArgumentException(message: "A bracket carries a charge only where every product of two normal forms is a single normal form.", paramName: "rules");
            }
            ...
            for (var middle = 0; (middle < count); ++middle) {
                ...
                for (var right = 0; (right < count); ++right) {
                    if (fold[((pair * count) + right)] != fold[((left * count) + ((int)fold[((middle * count) + right)]))]) {
                        throw new ArgumentException(message: "Re-association carries no charge here: the two bracketings of a triple reach different normal forms, so no charge relates them.", paramName: "rules");
                    }
                }
            }
        }
        ...
        for (var first = 0; (first < count); ++first) {
            for (var second = 0; (second < count); ++second) {
                if (!comparer.Equals(x: m_spliceCharge[((((unit * count) + first) * count) + second)], y: one)
                    || !comparer.Equals(x: m_spliceCharge[((((first * count) + unit) * count) + second)], y: one)
                    || !comparer.Equals(x: m_spliceCharge[((((first * count) + second) * count) + unit)], y: one)) {
                    throw new ArgumentException(message: "A declared re-association 3-cochain charges the material's one wherever the unit appears in the triple: the unit spelled as the empty product carries no letter for a splice to charge with, so any other charge makes two spellings of one element differ.", paramName: "rules");
                }
            }
        }
    ...
    private bool TryBuildCells(
        ...
        for (var left = 0; (left < count); ++left) {
            ...
            for (var right = 0; (right < count); ++right) {
                ...
                rightWord.CopyTo(destination: graft.Slice(start: leftWord.Length, length: rightWord.Length));

                if (!TryRewriteToKeysDetailed(
                    word: graft.Slice(start: 0, length: total),
                    charge: material.One,
                    stepLimit: RewriteStepLimit,
                    result: cell,
                    combination: combination,
        ...
        foreach (var terms in perTarget) { maximumCellTerms = Math.Max(val1: maximumCellTerms, val2: terms); }
    ...
    private static void RequireIndexableTriples(bool declaredTable) {
        if (declaredTable) {
            throw new ArgumentException(message: "A declared re-association 3-cochain is read through the compiled cells, which this presentation has no finite basis to build.", paramName: "rules");
        }
    }
    ...
    private bool TryFindRedex(ReadOnlySpan<int> word, out int position, out int ruleIndex) {
        ...
        for (var start = 0; (start < word.Length); ++start) {
            ...
            for (var slot = m_ruleBucketStart[symbol]; (slot < last); ++slot) {
                ...
                if (((start + pattern.Length) <= word.Length) && word.Slice(start: start, length: pattern.Length).SequenceEqual(other: pattern)) {
                    position = start;
                    ruleIndex = index;
    ...
    internal sealed class Combination {
        ...
        internal void Merge(ReadOnlySpan<int> word, TValue charge, TOps material) {
            ...
            while (low < high) {
                var middle = ((low + high) >> 1);
                var order = CompareWords(left: m_words[middle], right: word);

                if (0 == order) {
                    var combined = material.Add(left: m_charges[middle], right: charge);
            ...
            m_charges.Insert(index: low, item: charge);
            m_words.Insert(index: low, item: word.ToArray());
        }
    ...
    internal sealed class KeyedCombination {
        ...
        internal void Merge(long key, TValue charge, TOps material) {
            ...
            m_charges.Insert(index: low, item: charge);
            m_keys.Insert(index: low, item: key);
        }
    }
}
