...
| `CertifiedLowDiscrepancy` | `readonly record struct` | A Kronecker sequence `{n·α}` for a quadratic irrational `α = (p + q·√d)/r`, carrying an EXACT equidistribution certificate: `Certificate` is the max continued-fraction partial quotient `K` (via `ContinuedFraction`) — the badly-approximable bound, `K = 1` for the golden ratio (Hurwitz-optimal), `K = n` for the `n`-th metallic mean. `DiscrepancyBound(N)` is the closed-form `O(K·log N / N)` star-discrepancy guarantee that follows from `K` alone; `Point` is the `[0, 1)` value (the one seam). Reframes "well-distributed" as a bounded-partial-quotient certificate instead of a measured statistic. |
