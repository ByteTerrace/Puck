...
internal static class PrimeKernels {
    ...
    private static ulong FindFactorCycle(ulong addend, ref int budget, in ScaledResidueRing64 ring) {
        ...
        var anchor = 0UL;
        var backtrack = 0UL;
        var y = Advance(addend: addend, budget: ref budget, ring: in ring, value: ring.Encode(value: 2UL));

        do {
            anchor = y;

            for (var i = 0UL; (i < range); ++i) { y = Advance(addend: addend, budget: ref budget, ring: in ring, value: y); }
            ...
            do {
                ...
                for (var i = 0UL; (i < limit); ++i) {
                    y = Advance(addend: addend, budget: ref budget, ring: in ring, value: y);
                    product = ring.Multiply(
                        left: product,
                        right: ring.Subtract(
                            left: ((anchor > y) ? anchor : y),
                            right: ((anchor > y) ? y : anchor)
                        )
                    );
                }
        ...
        if (divisor == modulus) {
            do {
                backtrack = Advance(addend: addend, budget: ref budget, ring: in ring, value: backtrack);
                divisor = ring.Subtract(
                    left: ((anchor > backtrack) ? anchor : backtrack),
                    right: ((anchor > backtrack) ? backtrack : anchor)
    ...
    private static ulong Advance(ulong addend, ref int budget, in ScaledResidueRing64 ring, ulong value) {
        ...
        return ring.Add(left: ring.Multiply(left: value, right: value), right: addend);
    }
    ...
    internal static ulong FindFactor(ulong value) {
        ...
        while (true) {
            var divisor = FindFactorCycle(addend: addend, budget: ref budget, ring: in ring);
