...
public readonly record struct BinaryPolynomial :
    ...
    public static BinaryPolynomial[] FactorOddCycle(int cycleOrder) {
        ...
        if (!remaining.IsOne) {
            if (!remaining.IsIrreducible()) {
                throw new InvalidOperationException(message: "The binary factorization did not finish with an irreducible factor.");
            }
    ...
    /// means <c>t</c> divides the polynomial, so it is reducible; otherwise the polynomial defines a quotient ring that
    /// is a field exactly when it is irreducible, so the decision is delegated to <see cref="BinaryField{T}"/> rather
    /// than derived a second time here.
    /// </remarks>
    public bool IsIrreducible() {
        if (1 > Degree) { return false; }
        if (1 == Degree) { return true; }
        if (0UL == (Bits & 1UL)) { return false; }
    ...
    /// <remarks>
    /// <para>
    /// Primitivity is strictly stronger than irreducibility: it additionally says that <c>t</c> has order
    /// <c>2^degree - 1</c> rather than some proper divisor of it, which is what makes the polynomial usable as the
    /// characteristic polynomial of a maximal-period linear recurrence — the shift register behind a digital net's
    /// direction numbers, a maximal-length sequence, or a full-period CRC scrambler.
    /// </para>
    /// <para>
    /// A zero constant term is refused first, because <c>t</c> then divides the polynomial and the root is zero in the
    /// quotient rather than a generator of anything. Above degree one <see cref="IsIrreducible"/> already refuses those,
    /// but <c>t</c> itself is irreducible, so it is the one polynomial that rule has to answer on its own.
    /// </para>
    /// <para>
    /// The rest of the decision is the standard one: reject if reducible, then check that <c>t</c> raised to
    /// <c>(2^degree - 1) / p</c> is not one for every prime <c>p</c> dividing the group order. It therefore needs that
    /// order factored, and the shipped factorization is trial division, which is why the degree is bounded at
    /// <see cref="MaximumPrimitiveDegree"/> rather than at the carrier's width.
    /// </para>
    /// </remarks>
    /// <exception cref="NotSupportedException">The degree exceeds <see cref="MaximumPrimitiveDegree"/>.</exception>
    public bool IsPrimitive() {
        var degree = Degree;

        if (degree > MaximumPrimitiveDegree) {
            throw new NotSupportedException(message: $"A primitivity decision above degree {MaximumPrimitiveDegree} would need 2^degree-1 factored by something stronger than trial division.");
        }

        // A primitive polynomial has a non-zero constant term. Above degree one IsIrreducible refuses one without it
        // anyway, but t is irreducible and would otherwise build a quotient whose modulus t divides — which is not a
        // field at all, and which the field's own validation refuses rather than answering.
        if (0UL == (Bits & 1UL)) { return false; }

        if (!IsIrreducible()) { return false; }

        var field = BinaryField<ulong>.FromModulus(modulus: this);
        var groupOrder = ((1UL << degree) - 1UL);
        // The root, reduced: at degree one the modulus is t+1, so t itself is congruent to the identity and the
        // trivial group is generated by it.
        var root = field.Reduce(value: 2UL);

        Span<ulong> primeDivisors = stackalloc ulong[MaximumDistinctPrimeDivisorCount];
