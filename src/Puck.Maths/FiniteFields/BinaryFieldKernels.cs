...
internal static class BinaryFieldKernels {
    ...
    /// tuning value and never a correctness bound: every rung produces the same bytes at every length, so moving it can
    ...
    /// on the same terms.
    /// </remarks>
    private const int WideAffineMatrixAmortizationVectors = 4;
    ...
    /// alone because the result is the same at every width. The byte count and the loop cursor are pointer-width, so a
    ...
    internal static T Inverse<T>(T value, int degree, T tail) where T : IBinaryInteger<T>, IUnsignedNumber<T> {
        if (T.Zero == value) { throw new DivideByZeroException(message: "Zero has no multiplicative inverse."); }
    ...
    /// 128-bit nor the 512-bit form, so each rung asks its own instruction-set leaf. The 512-bit rungs are deliberately
    /// not gated on the wide vector types' availability: those stay functional everywhere, emulated in narrower chunks
    ...
    /// second time. Only the packing is subtle, and it is spelled out at <see cref="MatrixBit(int, int)"/>.
    /// </remarks>
    ...
    /// <summary>Places one bit of an eight-by-eight scaling matrix into the packed qword the affine transform reads.</summary>
    /// <param name="row">The output bit the row governs.</param>
    /// <param name="column">The input bit the column consumes.</param>
    /// <returns>The qword with exactly that matrix bit set.</returns>
    /// <remarks>
    /// The transform reads the row governing output bit <paramref name="row"/> from byte <c>7 - row</c> of the matrix
    /// qword, so the rows are packed from the top of the qword downwards. Writing them in the obvious order instead
    /// produces a byte-reversed matrix that still runs and still looks plausible, which is why the convention has one
    /// spelling rather than one per matrix builder.
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static ulong MatrixBit(int row, int column) =>
        (1UL << (((7 - row) * 8) + column));
    ...
    /// second time; the row packing is <see cref="MatrixBit(int, int)"/>'s.
    /// </remarks>
