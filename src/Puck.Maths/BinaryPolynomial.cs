...
public readonly record struct BinaryPolynomial :
    ...
    /// <returns><see langword="true"/> when the polynomial is irreducible and its root generates the whole multiplicative group of the quotient field; otherwise <see langword="false"/>.</returns>
    /// <remarks>
    /// <para>
    ...
    /// </para>
    /// <para>
    /// The decision is the standard one: reject if reducible, then check that <c>t</c> raised to
    ...
    public bool IsPrimitive() {
        ...
        if (degree > MaximumPrimitiveDegree) {
            throw new NotSupportedException($"A primitivity decision above degree {MaximumPrimitiveDegree} would need 2^degree-1 factored by something stronger than trial division.");
        }
        ...
        // A prime order reports no proper factorization at all, so it supplies itself as its own only prime divisor.
        if ((0 == divisorCount) && (1UL < groupOrder)) { primeDivisors[divisorCount++] = groupOrder; }
