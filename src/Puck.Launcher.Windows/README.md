...
Puck.Launcher.Windows registers the Windows GPU host used by windowed
composition roots such as `Puck.World`.
...
method, then `AddBackendSwitcher(...)`. Each registration stays in the project
that owns its dependencies.
