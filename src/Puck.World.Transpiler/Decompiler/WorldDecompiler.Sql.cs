...
public static partial class WorldDecompiler {
    ...
    private static bool CanDecompileEffect(
        ...
        switch (type) {
            ...
            case "addState": {
                    ...
                    if (eff["expression"]?.ToString() is string expr && expr.Contains('$') && !expr.Contains("$each")) {
                        return false;
                    }
                    return true;
                }
    ...
    private static bool CanDecompilePredicate(
        ...
        switch (type) {
            ...
            case "compareValue":
                ...
                var left = pred["left"]?.ToString() ?? "";
                var right = pred["right"]?.ToString() ?? "";
    ...
    private static string DecompilePredicate(
        ...
        switch (type) {
            ...
            case "compareValue": {
                    var left = gate["left"]?.ToString() ?? "";
                    var right = gate["right"]?.ToString() ?? "";
                    var cmp = gate["comparison"]?.ToString() switch {
                        "Equal" => "=",
                        "NotEqual" => "<>",
                        "Less" => "<",
