...
public static class WorldSourceDiagnostics {
    ...
    public static DiagnosticBag Diagnose(
        ...
        if ((compilation.Worlds.Count > 0) && !diagnostics.HasErrors) {
            foreach (var output in compilation.Worlds) {
                if (sourcePath is not null) {
                    WorldSemanticValidator.ValidateComposedWorld(output.Json, output.SourceMap, diagnostics, sourcePath, machines: machines, catalogFingerprint: catalogFingerprint);
                    PuckLinter.LintReferences(catalogFingerprint: catalogFingerprint, diagnostics: diagnostics, document: output.Json, machines: machines, sourceMap: output.SourceMap, sourcePath: sourcePath);
