...
internal sealed class PuckSchemaHover {
    ...
    internal PuckSchemaHover() {
        ...
        foreach (var section in m_schema.Sections) {
            Index(cursor: new Cursor(
                Node: section.Node,
                Root: section.Node
            ));
        }
        Index(cursor: new Cursor(
            Node: m_schema.Common,
        ...
        for (var index = 1; (index < path.Count); ++index) {
            ...
            switch (node) {
                case BlockNode block:
                    if (
                        (index > 0) &&
                        (path[(index - 1)] is TemplateNode)
                    ) {
                        continue;
                    }
                    current = block.Identifier switch {
                        "prototype" when (owner == "prototypes") => Item(cursor: current),
                        "shape" => Item(cursor: Property(
                        cursor: current,
                        name: "shapes"
                    )),
                        "material" => Item(cursor: Property(
                        cursor: current,
                        name: "materials"
                    )),
                        "addon" => Item(cursor: Property(
                        cursor: current,
                        name: "addons"
                    )),
                        "placement" => Item(cursor: Property(
                        cursor: current,
                        name: "rows"
                    )),
                        _ => Property(
                    ...
                    if (
                        (block.Identifier == "shape") &&
                        (block.Target == word) &&
                        (offset < (block.Statements.FirstOrDefault()?.Offset ?? (block.Offset + block.Length)))
                    ) {
                        return Card(
                            Property(
                                cursor: current,
                    ...
                    if (
                        ...
                        (offset < (block.Offset + block.Identifier.Length))
                    ) {
                        return Card(
                            cursor: current,
                            enumValue: null,
                ...
                case PropertyNode property:
                    ...
                    if (
                        (word == property.Name) &&
                        (offset < property.Value.Offset)
                    ) {
                        return Card(
                            current,
                            $"{fieldOwner}.{property.Name}",
                            null
                        );
                    }
                    break;
                ...
                case CallExpressionNode call:
                    ...
                    if (
                        (word == call.Name) &&
                        (offset < (call.Offset + call.Name.Length)) &&
                        (PuckHoverInfo.Builtin(word: call.Name) is null)
                    ) {
                        return Card(
                            cursor: current,
                            enumValue: null,
    ...
    private string? Card(Cursor? cursor, string name, string? enumValue) {
        ...
        if (enumValue is not null) {
            if (
                (enumeration is null) ||
                !enumeration.Any(predicate: value => (value?.ToString() == enumValue))
            ) {
                return null;
            }
    ...
    private void Index(Cursor cursor) {
        if (cursor.Node is JsonObject obj) {
            ...
            if (obj["properties"]?["$type"]?["const"]?.ToString() is { } name) {
                ...
                if (!candidates.Any(predicate: candidate => JsonNode.DeepEquals(
                    node1: candidate.Node,
                    node2: obj
                ))) {
    ...
    private Cursor? Operation(Cursor? context, string name) {
        ...
        return ((m_operations.TryGetValue(
            key: name,
            value: out var candidates
        ) && (candidates.Count == 1))
            ? candidates[0]
    ...
    private Cursor? Property(Cursor? cursor, string name) {
        foreach (var variant in Variants(cursor)) {
            if (
                (variant.Node["properties"] is JsonObject properties) &&
                (properties[name] is { } value)
            ) {
                return new Cursor(
                    Node: value,
                    Root: variant.Root
    ...
    private Cursor? Resolve(Cursor cursor, string reference) {
        ...
        if (parts.Length == 2) {
            foreach (var segment in parts[1].Split('/').Skip(count: 1)) {
                ...
                node = ((node is JsonObject obj)
                    ? obj[decoded]
                    : (((node is JsonArray array) && int.TryParse(
        ...
        return (((node is null) || (root is null))
            ? null
            : new Cursor(
                Node: node,
                Root: root
    ...
    private IEnumerable<Cursor> Variants(Cursor? cursor, int depth = 0) {
        if (
            (cursor is not { Node: JsonObject obj } value) ||
            (depth > 32)
        ) {
            yield break;
        }
