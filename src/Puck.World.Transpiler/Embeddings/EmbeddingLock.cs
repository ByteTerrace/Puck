...
public sealed class EmbeddingLock {
    ...
    public bool IsSpaceStale(StateSpace space) {
        ...
        if (!Spaces.TryGetValue(key: space.Name.Value, value: out var lockSpace)) {
            return true;
        }

        var lockedIdentity = new StateSpace(Name: space.Name, Model: lockSpace.Model, Revision: lockSpace.Revision, Dimensions: lockSpace.Dimensions);

        return !space.HasSameIdentity(other: lockedIdentity);
    }
