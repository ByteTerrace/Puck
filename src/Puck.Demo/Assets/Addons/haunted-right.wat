;; haunted-right.wat — the east-wing ghost of the haunted arcade (puck.addon.v1).
;;
;; The autopilot ghost's sibling: identical dead-reckoning brain, different haunt. It drives its
;; exclusive roster slot to the RIGHT cabinet and boots it. With three consoles the stands sit 5 units
;; apart along the -Z wall (spacing 5.0, firstX = -5): right at x = +5, z = -6.6. Everything else —
...
(module
  ...
  ;; The per-tick body: clamp-walk toward the right cabinet (+5, -6.6), then one PadNorth edge.
  (func (export "puck_on_tick") (result i32)
    ...
    ;; Fire ONE PadNorth Started edge — the nearest console to the body is then the right stand.
    (if
      (i32.and
        (i32.and
