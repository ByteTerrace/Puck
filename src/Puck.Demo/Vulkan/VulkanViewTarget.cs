...
/// <summary>
/// An offscreen color render target a viewport renders into: a sampleable device-local image plus the
/// render pass, framebuffer, image view, and a command buffer to record one pass into it. The compositor
/// samples <see cref="ImageViewHandle"/> to place the view on the swapchain — the "viewport becomes a
/// texture" that split-screen and the jumbotron build on.
/// </summary>
/// <remarks>
/// Sized to match the swapchain, so it is recreated on resize. It records, but does not submit: the
/// renderer batches every view target's command buffer into one queue submit (see
/// <see cref="VulkanQueueSubmitter"/>). The offscreen render pass's final layout is shader-read-only with
/// a fragment-visibility dependency (<see cref="VulkanRenderPassRequests.Sampled"/>), so the image is
/// ready to sample once that submit completes.
/// </remarks>
internal sealed class VulkanViewTarget : IDisposable {
    private const uint ImageUsageColorAttachment = 0x00000010;
    private const uint ImageUsageSampled = 0x00000004;
    ...
    public VulkanViewTarget(
        ...
        var image = offscreenImageApi.CreateColorImage(request: new VulkanOffscreenImageCreateRequest(
            ...
            UsageFlags: ImageUsageColorAttachment | ImageUsageSampled,
            Width: width
        ));
    ...
    /// <summary>The command buffer the offscreen pass is recorded into.</summary>
    public nint CommandBufferHandle => m_commandResources.CommandBufferHandles[0];

    /// <summary>The offscreen framebuffer wrapping the color image.</summary>
    public nint FramebufferHandle => m_framebufferHandle;

    /// <summary>The render-target height in pixels.</summary>
    public uint Height { get; }

    /// <summary>The color image's view, for sampling the rendered viewport.</summary>
    public nint ImageViewHandle => m_imageViewHandle;

    /// <summary>The offscreen render pass (shader-read final layout), to build the viewport's pipeline against.</summary>
