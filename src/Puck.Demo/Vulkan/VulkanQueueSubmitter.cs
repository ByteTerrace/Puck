...
/// <summary>Submits offscreen command buffers to the graphics queue and waits for them — the demo's
/// simple "render the views, then composite" sync point. Batching every viewport's command buffer into
/// one submit + one <c>vkQueueWaitIdle</c> keeps the per-frame stall to a single GPU round-trip. A later
/// phase can replace the wait with a semaphore to pipeline the offscreen passes with presentation.</summary>
internal sealed unsafe class VulkanQueueSubmitter {
    ...
    /// <summary>Submits the given command buffers in one batch and blocks until the queue is idle.</summary>
