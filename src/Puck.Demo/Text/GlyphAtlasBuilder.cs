...
/// the same inputs always yield a byte-identical atlas. The higher-fidelity marchable source is a pre-baked
/// <c>msdf-atlas-gen</c> MTSDF atlas (true-distance by construction); this runtime path is the no-toolchain fallback.
/// </remarks>
internal static class GlyphAtlasBuilder {
    ...
    private static FontAtlas BuildCoverage(int fontPixelSize, int padding) {
        ...
        var ascentPixels = (int)Math.Ceiling(((double)family.GetCellAscent(style: FontStyle.Regular) / emHeight) * fontPixelSize);
        var descentPixels = (int)Math.Ceiling(((double)family.GetCellDescent(style: FontStyle.Regular) / emHeight) * fontPixelSize);
        var lineHeightPixels = (int)Math.Ceiling(((double)family.GetLineSpacing(style: FontStyle.Regular) / emHeight) * fontPixelSize);
        ...
        var unpaddedCellHeight = Math.Max(lineHeightPixels, fontPixelSize);
    ...
    private static int MeasureCellWidth(Font font, StringFormat format, int fontPixelSize) {
        ...
        for (var code = FirstChar; (code <= LastChar); code++) {
            var measured = (int)Math.Ceiling(graphics.MeasureString(text: ((char)code).ToString(), font: font, width: int.MaxValue, format: format).Width);
