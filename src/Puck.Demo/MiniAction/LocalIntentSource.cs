...
/// simulation never knows which it is. In <c>--mini-action</c> mode this is the SOLE gamepad drainer (the global gamepad
...
public sealed class LocalIntentSource : IPlayerIntentSource {
    ...
    private readonly PlayerIntent[] m_frameIntents = new PlayerIntent[MiniActionWorld.MaxPlayers];
    ...
    public LocalIntentSource(GamepadManager manager, ControllerPlayerRegistry registry, MiniActionWorld world) {
    ...
    public void BeginFrame(ulong firstTick) {
        ...
        foreach (var drain in m_drainBuffer) {
            ...
            if ((slot < 0) || (slot >= MiniActionWorld.MaxPlayers)) {
                continue;
            }
    ...
    public PlayerIntent[] CollectTick(ulong tick, IReadOnlyList<Guid> players) {
        ...
        for (var slot = 0; (slot < MiniActionWorld.MaxPlayers); slot++) {
            ...
            row[slot] = (firstOfFrame ? intent : (intent with { JumpPressed = false, JumpReleased = false }));
        }
