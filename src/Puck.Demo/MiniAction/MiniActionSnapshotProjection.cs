...
/// consumes — the single place that maps the engine's command lanes onto MiniAction's move/jump intent. The live
/// router path and the replay/determinism path both project through here, so a recorded snapshot reproduces the
/// exact intent the live capture produced.
/// </summary>
internal static class MiniActionSnapshotProjection {
    ...
    /// <returns>One intent per slot, in slot order (length <see cref="MiniActionWorld.MaxPlayers"/>).</returns>
    public static PlayerIntent[] ToIntents(in CommandSnapshot snapshot, ushort moveId, bool hasMove, ushort jumpId, bool hasJump) {
        ...
        for (var slot = 0; (slot < MiniActionWorld.MaxPlayers); slot++) {
            if (snapshot.TryGetLane(slot: slot, out var lane)) {
                row[slot] = FromLane(lane: lane, moveId: moveId, hasMove: hasMove, jumpId: jumpId, hasJump: hasJump);
            }
        }
    ...
    public static PlayerIntent FromLane(CommandLane lane, ushort moveId, bool hasMove, ushort jumpId, bool hasJump) {
