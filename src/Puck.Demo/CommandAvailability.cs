...
/// <summary>
/// The console verbs' shared availability-guard idiom — every stateful <c>*CommandModule</c> wraps its editing
/// handlers so a verb typed before its target exists (the overworld root not yet the active root, or the target's
/// own mode not yet entered) reports a friendly narration instead of throwing. Two independent gates compose:
/// <list type="bullet">
/// <item><b>Host gate</b> (always checked): the target getter returns <see langword="null"/> when the composition
/// point isn't ready — the overworld root isn't the active root, or (for a nested target) the parent scene hasn't
...
/// optional second predicate reports whether the target's OWN mode is entered — <c>CreatorScene.Active</c>,
/// <c>TrackerScene.Active</c>. <c>WorldScene</c> and <see cref="Creator.ICompanionHost"/> have no such concept (the
/// world sculptor and the companion roster are live for the whole run once the root resolves), so they pass
/// <see langword="null"/> and only the host gate applies — this mirrors each target's REAL state machine rather than
/// forcing a uniform two-tier check where only one tier exists.</item>
/// </list>
/// The message shape is picked from the clearest of the pre-unification wordings: "[[verb]: unavailable — the
/// overworld is not the active root]" for the host gate, "[[verb]: enter [mode] mode first (console: [mode])]" for
/// the active gate.
/// </summary>
internal static class CommandAvailability {
    ...
    /// <typeparam name="T">The target type (a scene, or a small host adapter like <see cref="Creator.ICompanionHost"/>).</typeparam>
    ...
    /// separate on/off mode (e.g. <c>WorldScene</c>, the companion roster).</param>
