...
internal sealed class CameraGpuValidationNode : IRenderNode {
    ...
    // read it back (BGRA bytes). Mirrors the synthetic-camera zero-copy import (whose parity is now owned by Puck.Post's
    // CameraShareStage); only the format differs.
