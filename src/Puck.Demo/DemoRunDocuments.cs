...
internal static class DemoRunDocuments {
    ...
    public static PuckRunDocument Synthesize(DemoFlags flags) {
        ...
        var offscreen = (flags.Validate || flags.ValidateExport || flags.ValidateCompute || flags.ValidateMiniAction || flags.ValidateReverseShare || flags.ValidateIndirect || flags.ValidateResample || flags.ValidateViewports || flags.ValidatePixelate || flags.ValidateCapture || flags.ValidateWorld || flags.ValidateWorldChild);
        ...
        if (flags.ValidateCapture) {
            return Gate(host: host, gate: "capture");
        }
