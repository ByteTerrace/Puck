...
/// <summary>
/// A complete Overworld session recording: the engine's per-tick <see cref="CommandSnapshot"/> input stream — a
/// general <see cref="SnapshotRecording"/>, the cross-machine unit a peer transmits and the recorder stores — PLUS
/// the overworld's own per-tick roster events (joins/leaves), the one game-specific channel the engine recorder does
...
public sealed class OverworldRecording {
    private const uint Magic = 0x4443_5241u; // "ARCD"
    ...
    /// <exception cref="ArgumentNullException">Any argument is <see langword="null"/>.</exception>
    /// <exception cref="InvalidDataException">The roster channel is not an overworld recording or is an unsupported version.</exception>
    public static OverworldRecording Read(Stream stream, CommandRegistry registry) {
        ArgumentNullException.ThrowIfNull(argument: registry);
        ArgumentNullException.ThrowIfNull(argument: stream);
        ...
        if ((reader.ReadUInt32() != Magic) || (reader.ReadUInt32() != Version)) {
            throw new InvalidDataException(message: "Not an overworld recording, or an unsupported version.");
        }
        ...
        return new OverworldRecording { Input = input, RosterEvents = roster.DrainToImmutable(), };
    }
}
