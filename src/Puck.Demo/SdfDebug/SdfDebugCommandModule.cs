...
internal sealed class SdfDebugCommandModule(IRenderNode rootNode) : ICommandModule {
    ...
    public IEnumerable<CommandDefinition> GetCommands() {
        ...
        yield return WithArgs(
            ...
            handler: WithSceneArgs(handler: static (scene, args) => {
                ...
                scene.SetOffset2(offset: new Vector3(x: xyz[0], y: xyz[1], z: xyz[2]));
        ...
        yield return WithArgs(
            ...
            handler: WithSceneArgs(handler: static (scene, args) => {
                if ((args.Length > 0) && string.Equals(a: args[0], b: "off", comparisonType: StringComparison.OrdinalIgnoreCase)) {
        ...
        yield return Plain(
            description: "Prints the debug subject's facts: shape, op stack, scope state, program word/instance count, the baked Lipschitz step scale, and per-pass GPU ms (arm with gpu.timing on).",
            handler: _ => new CommandResult(DescribeInfo()),
            name: "sdf.info"
        );
        yield return WithArgs(
            description: "The SDF perf-bench (needs the sdf mode active + gpu.timing on): sdf.bench shapes | ops | carves | rigs [n] | storm | instances <shape> <n> | sweep [shape] | warm <n> | frames <n> | abort. rigs = n heterogeneous 24-bone walking avatars (120 VM instructions each). Runs async across frames — progress + a fixed-width table print to stdout.",
        ...
        yield return WithArgs(
            description: "Inspect or settle the active debug scene's carve-bake planner: sdf.bake status (bins, slots, states, and pool occupancy) | sdf.bake now (settles the current pool immediately instead of waiting for the quiet window).",
            handler: (context, args) => new CommandResult(HandleBake(args: args)),
            name: "sdf.bake"
        );
    }

    // These verbs inspect or settle the active debug scene's carve-bake planner. Global enablement remains the
    // separate sdf.carve-bake feature switch.
    private string HandleBake(string[] args) {
        if (Mode is not { } mode) {
            return "[sdf.bake: unavailable — the overworld is not the active root]";
        }

        if (args.Length == 0) {
            return $"[sdf.bake: status | now — carve-bake {(SdfCarveBakePlanner.Enabled ? "on" : "off")} (sdf.carve-bake to flip)]";
        }

        switch (args[0].ToLowerInvariant()) {
            case "status":
                return $"[sdf.bake status] {mode.CarveBake.DescribeStatus()}";
            case "now":
                return $"[sdf.bake now] {mode.CarveBake.SettleNow()}";
            default:
                return $"[sdf.bake: '{args[0]}' — status | now]";
        }
    }
    ...
    private string HandleCam(string[] args) {
        ...
        for (var index = 0; (index < args.Length); index++) {
            switch (args[index].ToLowerInvariant()) {
                ...
                case "target":
                    ...
                    target = new Vector3(x: xyz[0], y: xyz[1], z: xyz[2]);
                    index += 3;
                    break;
                default:
        ...
        return $"[sdf.cam pitch={((pitch is { } pv) ? pv.ToString(format: "0.###", provider: CultureInfo.InvariantCulture) : "·")} yaw={((yaw is { } yv) ? yv.ToString(format: "0.###", provider: CultureInfo.InvariantCulture) : "·")} dist={((distance is { } dv) ? dv.ToString(format: "0.###", provider: CultureInfo.InvariantCulture) : "·")} target={((target is { } tv) ? $"{tv.X:0.##},{tv.Y:0.##},{tv.Z:0.##}" : "·")}]";
    }
    private string HandleBench(string[] args) {
        ...
        if (args.Length == 0) {
            return $"[sdf.bench: shapes | ops | carves | rigs [n] | storm | instances <shape> <n> | sweep [shape] | warm <n> | frames <n> | abort — warm={bench.WarmFrames} samples={bench.SampleFrames}]";
        }
        ...
        switch (args[0].ToLowerInvariant()) {
            ...
            case "rigs": {
                    var count = (((args.Length > 1) && TryParseInt(text: args[1], value: out var parsed)) ? parsed : 128);

                    return ((count < 1)
                        ? "[sdf.bench rigs: usage — sdf.bench rigs [1..128]]"
                        : (RequireBenchMode(mode: mode) ?? bench.StartRigs(count: count)));
                }
            case "storm":
            ...
            default:
                return $"[sdf.bench: '{args[0]}' — shapes | ops | carves | rigs [n] | storm | instances <shape> <n> | sweep [shape] | warm <n> | frames <n> | abort]";
        }
    }
    ...
    private static string HandleCarve(SdfDebugScene scene, string[] args) {
        ...
        var carve = new SdfCarve(Center: new Vector3(x: xyz[0], y: xyz[1], z: xyz[2]), Radius: radius, Smooth: smooth, SmoothK: smoothK);
    ...
    private static SdfDebugOp? BuildOp(string[] args) {
        ...
        Vector3 V3(int start, Vector3 fallback) => new(x: F(fallback: fallback.X, index: start), y: F(fallback: fallback.Y, index: (start + 1)), z: F(fallback: fallback.Z, index: (start + 2)));

        switch (args[0].ToLowerInvariant()) {
            case "twist": return Op(kind: SdfDebugOpKind.Twist, a: F(fallback: 1f, index: 0));
            case "bend-x" or "bendx": return Op(kind: SdfDebugOpKind.BendX, a: F(fallback: 1f, index: 0));
            case "bend-y" or "bendy": return Op(kind: SdfDebugOpKind.BendY, a: F(fallback: 1f, index: 0));
            case "bend-z" or "bendz": return Op(kind: SdfDebugOpKind.BendZ, a: F(fallback: 1f, index: 0));
            case "scale": return Op(kind: SdfDebugOpKind.Scale, v0: ((rest.Length == 1) ? new Vector3(value: F(fallback: 1f, index: 0)) : V3(fallback: Vector3.One, start: 0)));
            case "elongate": return Op(kind: SdfDebugOpKind.Elongate, v0: V3(0, new Vector3(x: 0.3f, y: 0f, z: 0f)));
            case "repeat": return Op(kind: SdfDebugOpKind.Repeat, v0: V3(0, new Vector3(x: 2.5f, y: 2.5f, z: 2.5f)));
            case "repeat-limited" or "repeatlimited": return Op(kind: SdfDebugOpKind.RepeatLimited, v0: V3(0, new Vector3(x: 2.5f, y: 2.5f, z: 2.5f)), v1: V3(3, new Vector3(x: 1f, y: 1f, z: 1f)));
            case "polar": return BuildPolar(rest: rest);
            case "symmetry": return BuildSymmetry(rest: rest);
            case "logsphere": return Op(kind: SdfDebugOpKind.LogSphere, a: F(fallback: 2f, index: 0), b: F(fallback: 0f, index: 1));
            case "celljitter": return BuildCellJitter(rest: rest);
            case "domainwarp": return Op(kind: SdfDebugOpKind.DomainWarp, v0: V3(0, new Vector3(x: 2f, y: 2f, z: 2f)), a: F(fallback: 0.2f, index: 3));
            case "onion": return Op(kind: SdfDebugOpKind.Onion, a: F(fallback: 0.05f, index: 0));
            case "dilate": return Op(kind: SdfDebugOpKind.Dilate, a: F(fallback: 0.1f, index: 0));
            case "displace": return Op(kind: SdfDebugOpKind.Displace, v0: V3(0, new Vector3(x: 6f, y: 6f, z: 6f)), a: F(fallback: 0.08f, index: 3));
            default: return null;
        }
    }
    private static SdfDebugOp? BuildPolar(string[] rest) {
        var count = (((rest.Length > 0) && TryParseInt(text: rest[0], value: out var parsed)) ? Math.Max(val1: 1, val2: parsed) : 6);
    ...
    private static SdfDebugOp? BuildSymmetry(string[] rest) {
        ...
        if ((rest.Length >= 3) && TryParseFloat(text: rest[0], value: out var nx) && TryParseFloat(text: rest[1], value: out var ny) && TryParseFloat(text: rest[2], value: out var nz)) {
            var normal = new Vector3(x: nx, y: ny, z: nz);
    ...
    private static SdfDebugOp? BuildCellJitter(string[] rest) {
        var spacing = (((rest.Length > 0) && TryParseFloat(text: rest[0], value: out var s)) ? MathF.Max(x: 0.001f, y: MathF.Abs(x: s)) : 2f);
        ...
        if ((MathF.Abs(x: jitter) * 0.5f) >= (0.5f * spacing)) {
            return null;
        }

        return new SdfDebugOp(Kind: SdfDebugOpKind.CellJitter, V0: new Vector3(value: spacing), V1: Vector3.Zero, A: jitter, B: tumble, I0: seed, I1: 0, Flag: false);
    }
    ...
    private string DescribeInfo() {
        ...
        var cadence = DescribeCadence();

        return string.Create(provider: CultureInfo.InvariantCulture, handler: $"[sdf.info active={mode.Active} {Summary(scene: scene)} slice={DescribeSlice(scene: scene)} | words={words} instances={instances} bound={bound} stepScale={lipschitz} | {ListOps(scene: scene)} | timing: {timing} | cadence: {cadence}]");
    }

    // The cadence gate's per-span diagnostics (STEP 1 instrumentation, perf plan Phase 6.1 follow-up): which spans
    // differed from the previous decided frame, so a never-skipping gate names its driver instead of being guessed at.
    // "off" covers both an unbuilt engine and a disarmed gate (frame.EnableCadenceGate false — see feature.set sdf.cadence-gate).
    // When every span matched (changed=(none)) but the frame still rendered, one of the two conditions NOT covered by
    // any hashed span forced it — named via "blocked=" so a matching-signature-yet-never-skips case (a declared
    // ScreenSlab or an in-flight carve bake) is never mistaken for a signature bug. src=N reports the raw bound-screen
    // bitmask ALONGSIDE (informational only — it no longer gates the decision; see DecideCadenceSkip's root-cause note).
    private string DescribeCadence() {
        if ((m_host is null) || !m_host.TryReadSdfCadenceDiagnostics(diagnostics: out var diagnostics) || !diagnostics.GateEnabled) {
            return "gate=off";
        }

        var total = (diagnostics.SkippedFrameCount + diagnostics.RenderedFrameCount);
        var blocked = DescribeBlockers(diagnostics: diagnostics);
        var suffix = ((blocked.Length == 0) ? "" : $" blocked={blocked}");

        return $"gate=on skipped={diagnostics.SkippedFrameCount}/{total} changed={DescribeChangedSpans(spans: diagnostics.ChangedSpans)}{suffix} src={(diagnostics.ScreenSourceBound ? "bound" : "none")}";
    }

    private static string DescribeBlockers(SdfCadenceDiagnostics diagnostics) {
        ...
        if (diagnostics.ProgramDeclaresScreenSlab) { names.Add(item: "screen-slab"); }
        if (diagnostics.BrickBaking) { names.Add(item: "brick-bake"); }

        return string.Join(separator: ",", values: names);
    }

    private static string DescribeChangedSpans(SdfCadenceSpan spans) {
        if (spans == SdfCadenceSpan.None) {
            return "(none)";
        }

        var names = new List<string>(capacity: 6);

        if ((spans & SdfCadenceSpan.Revisions) != 0) { names.Add(item: "revisions"); }
        if ((spans & SdfCadenceSpan.Push) != 0) { names.Add(item: "push"); }
        if ((spans & SdfCadenceSpan.Viewports) != 0) { names.Add(item: "viewports"); }
        if ((spans & SdfCadenceSpan.Dynamics) != 0) { names.Add(item: "dynamics"); }
        if ((spans & SdfCadenceSpan.ScreenSurfaces) != 0) { names.Add(item: "screen-surfaces"); }
        if ((spans & SdfCadenceSpan.ScreenLights) != 0) { names.Add(item: "screen-lights"); }

        return string.Join(separator: ",", values: names);
    }

    // The previous frame's per-pass GPU ms, one term per SdfWorldEngine.PassTimingLabels entry (so a future pass shows
    // here with no edit), plus the whole-frame total. "off" when timing is not live (gpu.timing on arms it).
    private string DescribeTiming() {
        ...
        if ((m_host is null) || !m_host.TryReadSdfPassTimings(passMilliseconds: passMilliseconds, passCount: out var passCount, frame: out var frame)) {
            return "off (gpu.timing on to time)";
        }
