    ...
    private static readonly ExhibitEntry[] Exhibits = [
        new ExhibitEntry(
            ...
            Plaque: [
                ...
                "stepScale, the D1 keystone) exists: an unclamped march tunnels straight through the blade.",
        ...
        new ExhibitEntry(
            ...
            Plaque: [
                "A LogSphere shellRatio-2 fold — the worst historical cross-backend parity in the suite. The log-polar",
                "map makes the field DISCONTINUOUS at each shell boundary, so a ±1-LSB UV delta lands in a different",
                "shell on each backend. Camera stays OUTSIDE the fold (the settled world-log-sphere Post envelope).",
            ]
        ),
        new ExhibitEntry(
            ...
            Plaque: [
                ...
                "boundary. The documented real-but-unfixed seam (docs/sdf-backlog.md). Look along the cell walls.",
            ]
        ),
        new ExhibitEntry(
            ...
            Target: new Vector3(0f, -1.0f, -6f), Yaw: 0.0f, Pitch: 0.08f, Distance: 4.0f,
            Plaque: [
                ...
                "grazing horizon — MaxSteps exhaustion and the occluder framing are both REFUTED. Fix deferred; the open",
                "question is whether it is worth fixing at all (docs/sdf-backlog.md §Live defects). Drive with a low sdf.cam pitch.",
            ]
        ),
        new ExhibitEntry(
            ...
            Target: new Vector3(0.4f, 0f, 0f), Yaw: 0.7f, Pitch: 0.5f, Distance: 6.5f,
            Plaque: [
        ...
        new ExhibitEntry(
            ...
            Name: "wallpaper-p4g",
            Title: "The P4G wallpaper defect",
            Target: new Vector3(0f, -1.0f, 0f), Yaw: 0.5f, Pitch: 0.9f, Distance: 7.0f,
            Plaque: [
                "SDF_WPG_P4G renders as p4 — the glide-mirror classes do not survive the wallpaper turn cocycle, so the",
                "tiling drops its mirror symmetry. Marked KNOWN DEFECT in sdf-vm.hlsli; recovery is a redesign of the",
                "cocycle, not a tweak — DON'T FIX CASUALLY (docs/sdf-backlog.md §Live defects). An asymmetric tile reveals it.",
            ]
        ),
        new ExhibitEntry(
        ...
        new ExhibitEntry(
            ...
            Target: new Vector3(0f, -0.8f, 0f), Yaw: 0.4f, Pitch: 0.18f, Distance: 2.6f,
            Plaque: [
                "An aggressive LogSphere (shellRatio ~2.8, twist) with the camera DOWN INSIDE the fold near a floor — a",
                "run-document the schema validator PASSES, yet the Droste field's shell discontinuities defeat beam tile",
                "classification: tile-granular black holes + heavy ground moire. The settled world-log-sphere Post scene",
                "(shellRatio 2.0, camera OUTSIDE) is the safe envelope. Open question: should the run-doc validator warn on",
                "camera-in-fold or aggressive shell ratios? PAIR WITH debug.view.overshoot AND debug.view.termination —",
                "this exhibit is WHY those views exist.",
            ]
        ),
    ];
