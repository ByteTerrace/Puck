...
public static class SdfSolidGeometry {
    ...
    private static SdfProgramBuilder AppendProfile(SdfProgramBuilder chain, Vector3 scale, SdfPrismProfile profile,
        ...
        if (!profile.IsValid()) { throw new ArgumentOutOfRangeException(paramName: nameof(profile)); }
        ...
        var scaledRounding = (rounding / MathF.Max(
            x: polygonScale.X,
            y: MathF.Max(
                x: polygonScale.Y,
                y: polygonScale.Z
        ...
        return profile.Kind switch {
            ...
            scale.X,
            scale.Y,
            (profile.CornerRadius * MathF.Min(
                x: scale.X,
                y: scale.Y
            )),
            lift,
            ...
            SdfPrismProfileKind.Polygon => chain.Scale(scale: polygonScale).RegularPolygon(
            profile.Sides,
            1f,
            lift,
            (revolve
            ? (revolveOffset / polygonScale.X)
            : 1f),
            material,
            blend,
            smooth,
            ...
            : chain.Scale(scale: scale).Ellipse(
                1f,
                1f,
                SdfLift.Extrude,
                1f,
            ...
            SdfPrismProfileKind.ChamferedRectangle => chain.ChamferedRectangle(
            scale.X,
            scale.Y,
            (profile.CornerRadius * MathF.Min(
                x: scale.X,
            ...
            SdfPrismProfileKind.Convex => chain.Scale(scale: polygonScale).ConvexPolygon(
            profile.Vertices!,
            ((profile.CornerRadius * SdfProgramBuilder.ConvexPolygonInradius(vertices: profile.Vertices!)) + scaledRounding),
            lift,
            (revolve
            ? (revolveOffset / polygonScale.X)
            : 1f),
            ...
            _ => throw new ArgumentOutOfRangeException(paramName: nameof(profile)),
        };
    }
    ...
    private static void RequireTaper(float taper) {
        ...
        if (!float.IsFinite(f: taper)) { throw new ArgumentOutOfRangeException(paramName: nameof(taper)); }
    }
    ...
    public static SdfProgramBuilder AppendPrimitive(SdfProgramBuilder chain, SdfSolidPrimitive type, int material, SdfBlendOp blend = SdfBlendOp.Union, float smooth = 0f, float taper = 0.5f, SdfPrismProfile? profile = null, SdfLift lift = SdfLift.Extrude, float rounding = 0f, float chamfer = 0f, bool detail = false, float exponent = SdfProgramBuilder.MinSuperellipsoidExponent) {
        ...
        if (
            (profile is not null) &&
            ((type != SdfSolidPrimitive.Prism) || !profile.IsValid())
        ) { throw new ArgumentOutOfRangeException(paramName: nameof(profile)); }

        if (
            (type == SdfSolidPrimitive.Prism) &&
            (profile is { Kind: not SdfPrismProfileKind.Trapezoid })
        ) {
        ...
        return type switch {
            ...
            SdfSolidPrimitive.Ellipsoid => chain.Superellipsoid(
            blend: blend,
            detail: detail,
    ...
    public static SdfProgramBuilder AppendScaledPrimitive(SdfProgramBuilder chain, SdfSolidPrimitive type, Vector3 scale,
        ...
        return type switch {
            ...
            // A non-uniformly scaled Sphere or Ellipsoid bakes its radii into the exact exponent-2 superellipsoid gauge:
            // 1-Lipschitz at any eccentricity, so it needs no step clamp and no field scope.
            SdfSolidPrimitive.Sphere => chain.Superellipsoid(
            blend: blend,
            detail: detail,
            exponent: SdfProgramBuilder.MinSuperellipsoidExponent,
            material: material,
    ...
    public static SdfSolidBounds GetLocalBounds(SdfSolidPrimitive type) {
        return type switch {
            ...
            SdfSolidPrimitive.Prism => new(
            Center: Vector3.Zero,
            HalfExtents: new Vector3(value: 1.0001f)
        ),
    ...
    public static float HalfExtent(SdfSolidPrimitive type, Vector3 scale, SdfLift lift, Vector3 axis) {
        ...
        var canonical = type switch {
            SdfSolidPrimitive.Cylinder => new Vector3(
            x: CylinderRadius,
            y: CylinderHalfHeight,
            z: CylinderRadius
        ),
            SdfSolidPrimitive.Cone => new Vector3(
            x: ConeRadius,
            y: ConeHalfHeight,
            z: ConeRadius
        ),
            SdfSolidPrimitive.Capsule => new Vector3(
            x: CapsuleRadius,
            y: (CapsuleEndpoint.Y + CapsuleRadius),
            z: CapsuleRadius
        ),
            ...
            SdfSolidPrimitive.Torus => new Vector3(
            x: (TorusMajor + TorusMinor),
            y: TorusMinor,
            z: (TorusMajor + TorusMinor)
        ),
        ...
        switch (type) {
            ...
            case SdfSolidPrimitive.Prism: {
                    ...
                    var inradius = (profile?.Kind switch {
                        SdfPrismProfileKind.RoundedRectangle => MathF.Min(
                        x: radial,
                        y: axial
                    ),
                        SdfPrismProfileKind.Ellipse => MathF.Max(
                        x: 0f,
                        y: (MathF.Min(
                            x: radial,
                            y: axial
                        ...
                        topHalfWidth: (radial * Math.Clamp(
                            max: 1f,
                            min: 0f,
                            value: taper
                        )),
        ...
        switch (type) {
            case SdfSolidPrimitive.Prism: {
                    inradius = (profile?.Kind switch {
                        ...
                        SdfPrismProfileKind.Polygon => (MathF.Min(
                        x: radial,
                        y: axial
                    ) * MathF.Cos(x: (MathF.PI / Math.Clamp(
                        value: (profile?.Sides ?? 3),
                        min: 3,
                        max: 32
                    )))),
                        SdfPrismProfileKind.Ellipse => MathF.Max(
                        ...
                        chamfer: SdfProgramBuilder.ClampChamfer(
                            chamfer: (profile!.CornerRadius * MathF.Min(
                                x: radial,
                                y: axial
                            )),
                            halfWidth: radial,
    ...
    public static float Reach(SdfSolidPrimitive type, Vector3 scale, SdfLift lift = SdfLift.Extrude) {
        ...
        var reach = type switch {
            SdfSolidPrimitive.Prism => (MathF.Sqrt(x: 3f) * 1.0001f),
    ...
    public static bool TryValidateScaledPrimitive(SdfSolidPrimitive type, Vector3 scale, out string refusal, float taper = 0.5f, SdfPrismProfile? profile = null, SdfLift lift = SdfLift.Extrude, float rounding = 0f, float chamfer = 0f, float exponent = SdfProgramBuilder.MinSuperellipsoidExponent) {
        ...
        if (
            (type == SdfSolidPrimitive.Prism) &&
            (!float.IsFinite(f: taper) || (taper < 0f) || (taper > 1f))
        ) {
        ...
        if (
            (type == SdfSolidPrimitive.Sweep) &&
            !IsUniform(scale: effectiveScale)
        ) {
        ...
        if (
            ...
            (!float.IsFinite(f: exponent) || (exponent < SdfProgramBuilder.MinSuperellipsoidExponent) || (exponent > SdfProgramBuilder.MaxSuperellipsoidExponent))
        ) {
        ...
        if (
            (profile is { Kind: SdfPrismProfileKind.ChamferedRectangle }) &&
            (lift == SdfLift.Extrude)
        ) {
            ...
            var profileChamfer = (profile.CornerRadius * MathF.Min(
                x: effectiveScale.X,
                y: effectiveScale.Y
            ));
        ...
        var slant = new Vector2(
            x: (((type == SdfSolidPrimitive.Prism)
            ? (1f - taper)
            : ConeRadius) * effectiveScale.X),
