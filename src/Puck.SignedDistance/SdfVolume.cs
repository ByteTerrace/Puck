...
public readonly record struct SdfVolume(SdfVolumeKind Kind, Vector3 Position, Quaternion Rotation, Vector3 HalfExtent,
    ...
    public void Validate(int dynamicTransformCount) {
        ...
        if (
            ...
            (Steps < MinSteps) ||
            (Steps > MaxSteps) ||
            !NonNegative(v: Intensity) ||
            !NonNegative(v: Extinction) ||
            !NonNegative(v: PulseAmplitude) ||
            (PulseAmplitude > 1f) ||
            !NonNegative(v: PulseFrequency) ||
            ((IntensityLane is { } lane) && (((uint)lane) > 3u)) ||
            (Ramp is not { Count: >= 1 and <= 4 }) ||
            !NonNegative(v: Coverage) ||
            (Coverage > 1f) ||
            ...
            throw new ArgumentOutOfRangeException(
                nameof(SdfVolume),
                "Invalid bounded volume controls, frame, lane, or ramp."
            );
        }
        var previous = -1f;
        ...
        foreach (var stop in Ramp) {
            if (
                ...
                throw new ArgumentOutOfRangeException(
                    nameof(Ramp),
                    "Density stops must increase in [0, 1] with finite non-negative colors."
                );
            }
