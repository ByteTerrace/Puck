...
public readonly record struct SdfInstanceGridInput(Vector3 Center, float Radius, bool Binnable, bool FrameBinnable);
...
public static class SdfInstanceGrid {
    ...
    private static int CellOf(Vector3 point, Vector3 origin, float invCellSize, (int X, int Y, int Z) dimensions) {
        var x = Math.Clamp(
            ((int)MathF.Floor(x: ((point.X - origin.X) * invCellSize))),
            0,
            (dimensions.X - 1)
        );
        var y = Math.Clamp(
            ((int)MathF.Floor(x: ((point.Y - origin.Y) * invCellSize))),
            0,
            (dimensions.Y - 1)
        );
        var z = Math.Clamp(
            ((int)MathF.Floor(x: ((point.Z - origin.Z) * invCellSize))),
            0,
            (dimensions.Z - 1)
        );
    ...
    private static (int X, int Y, int Z) DimensionsFor(Vector3 extent, float cellSize) {
        return (
            Math.Max(
            val1: 1,
            val2: ((int)MathF.Ceiling(x: (extent.X / cellSize)))
        ),
            Math.Max(
            val1: 1,
            val2: ((int)MathF.Ceiling(x: (extent.Y / cellSize)))
        ),
            Math.Max(
            val1: 1,
            val2: ((int)MathF.Ceiling(x: (extent.Z / cellSize)))
        )
        );
    }
    ...
    public static int WordCapacity(int maxInstances) {
        var count = Math.Max(
            val1: 0,
            val2: maxInstances
        );
    ...
    public sealed class Workspace {
        ...
        public Workspace(int maxInstances) {
            m_maxInstances = Math.Max(
                val1: 0,
                val2: maxInstances
            );
            var instanceScratchLength = Math.Max(
                val1: 1,
                val2: m_maxInstances
            );
            var cellScratchLength = Math.Max(
                val1: 1,
                val2: (CellCapacityFactor * m_maxInstances)
            );
        ...
        public ReadOnlySpan<uint> Build(ReadOnlySpan<SdfInstanceGridInput> instances, bool enabled) {
            if (instances.Length > m_maxInstances) {
                throw new ArgumentException(
                    message: $"The frame grid has {instances.Length} instances; its workspace was sized for {m_maxInstances}.",
            ...
            var invCellSize = (1.0f / cellSize);
            var cellStart = m_cellStart.AsSpan(
                length: (cellCount + 1),
                start: 0
            );
