...
namespace Puck.SignedDistance.Queries.Debug;
...
public sealed record WorldQueryDriftHistogram(
    ...
    public double BakedAgreementRate => ((BakedComparisons == 0)
        ? 1.0
        : (((double)BakedAgreements) / BakedComparisons)
    );
    ...
    public double GpuSignAgreementRate => ((GpuComparisons == 0)
        ? 1.0
        : (((double)GpuAgreements) / GpuComparisons)
    );
}
/// <summary>
        ...
        for (var index = 0; (index < points.Count); index++) {
            ...
            if (withinShell) {
                ...
                if (evaluatorInside == gpuInside) {
                    ...
                    gpuDisagreements.Add(item: new WorldQueryDriftSample(
                        Channel: "gpu",
                        EvaluatorDistance: distance,
                        Position: point
                    ));
                }
            }
            ...
            if (
                ...
                var evaluatorHasGround = evaluator.TryGroundHeight(
                    groundY: out var evaluatorGround,
                    position: point,
                    probeDown: groundProbeDown,
                    probeUp: groundProbeUp
                );
                var bakedHasGround = baked.TryGroundHeight(
                    groundY: out var bakedGround,
                    position: point,
                    probeDown: groundProbeDown,
                    probeUp: groundProbeUp
                );
                ...
                if (agree) {
                    ...
                    bakedDisagreements.Add(item: new WorldQueryDriftSample(
                        Channel: "baked",
                        EvaluatorDistance: distance,
                        Position: point
                    ));
