...
/// <summary>The lifecycle state of one brick pool slot. A slot is <see cref="Empty"/> until a
...
/// <summary>A brick slot's current state plus the monotonic per-slot bake serial: a re-request
...
/// <param name="InverseLambda">The stored-value scale <c>1/λ</c> (λ = √3 for the carve-union field);
