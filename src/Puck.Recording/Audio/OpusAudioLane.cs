...
public sealed class OpusAudioLane : IDisposable {
    ...
    public OpusAudioLane(int bitrateBitsPerSecond, IReadOnlyList<AudioLaneSource> sources) {
        ...
        foreach (var binding in sources) {
            ...
            var state = new SourceState {
                ...
                inputSampleRate: binding.Source.SampleRate,
                inputChannels: binding.Source.Channels
            ),
    ...
    private void DrainSources(List<SourceState> states) {
        foreach (var state in states) {
            var read = state.Source.Read(
                firstSampleTimestampNanoseconds: out var timestamp,
                interleaved: m_readScratch
            );
            ...
            state.Resampler.Resample(
                input: m_readScratch.AsSpan(
                    length: read,
                    start: 0
                ),
                output: state.Ring
    ...
    private void PumpIsolated(SourceState state, IAudioPacketSink sink) {
        ...
        _ = state.Ring.Dequeue(destination: m_dequeueScratch.AsSpan(
            length: available,
            start: 0
        ));
        state.Encoder.Append(
            firstSampleTimestampNanoseconds: state.EpochNanoseconds,
            interleavedStereo: m_dequeueScratch.AsSpan(
                length: available,
                start: 0
            ),
            sink: sink
    ...
    private void PumpMix(IAudioPacketSink sink) {
        ...
        m_mixEncoder.Append(
            ...
            interleavedStereo: m_mixAccumulator.AsSpan(
                length: available,
                start: 0
            ),
            sink: sink
