...
public sealed class MatroskaMuxer : IDisposable {
    ...
    private static byte[] BuildSeekEntry(uint target, long position) {
        ...
        BinaryPrimitives.WriteUInt64BigEndian(
            destination: encodedPosition,
            value: ((ulong)position)
        );
        EbmlWriter.WriteBinary(
            id: MatroskaIds.SeekId,
            stream: entryContent,
            value: targetId
        );
        EbmlWriter.WriteBinary(
            id: MatroskaIds.SeekPosition,
            stream: entryContent,
    ...
    private static byte[] BuildSeekHead(long infoPosition, long tracksPosition, long cuesPosition) {
        ...
        for (var index = 0; (index < SeekTargets.Length); index++) {
            var entry = BuildSeekEntry(
                target: SeekTargets[index],
                position: Math.Max(
                    val1: 0L,
                    val2: positions[index]
                )
            );
        ...
        EbmlWriter.WriteMasterHeader(
            stream: seekHead,
            id: MatroskaIds.SeekHead,
            contentSize: seekHeadContent.Length
        );
    ...
    private void FlushCluster() {
        ...
        var content = m_clusterBuffer.GetBuffer().AsSpan(
            start: 0,
            length: ((int)m_clusterBuffer.Length)
        );
        var clusterPosition = (m_output.Position - m_segmentDataStart);

        EbmlWriter.WriteMasterHeader(
            stream: m_output,
            id: MatroskaIds.Cluster,
            contentSize: content.Length
        );
    ...
    private void OpenCluster(long baseMilliseconds) {
        ...
        EbmlWriter.WriteUInt(
            id: MatroskaIds.Timestamp,
            stream: m_clusterBuffer,
            value: ((ulong)baseMilliseconds)
        );
    ...
    private void PatchSeekHead() {
        ...
        var completed = BuildSeekHead(
            cuesPosition: m_cuesPosition,
            infoPosition: m_infoPosition,
            tracksPosition: m_tracksPosition
        );
    ...
    private void PatchSegmentSize(long segmentEnd) {
        ...
        EbmlWriter.WriteSizeFixed(
            length: SegmentSizeWidth,
            size: (segmentEnd - m_segmentDataStart),
            stream: m_output
        );
    }
    private void ReserveSeekHead() {
        ...
        var completedShape = BuildSeekHead(
            cuesPosition: 0L,
            infoPosition: 0L,
            tracksPosition: 0L
        );
    ...
    private void WriteCues() {
        ...
        foreach (var cue in m_cues) {
            ...
            EbmlWriter.WriteUInt(
                stream: positions,
                id: MatroskaIds.CueTrack,
                value: ((ulong)cue.Track)
            );
            EbmlWriter.WriteUInt(
                stream: positions,
                id: MatroskaIds.CueClusterPosition,
                value: ((ulong)cue.ClusterPosition)
            );
            ...
            EbmlWriter.WriteUInt(
                stream: point,
                id: MatroskaIds.CueTime,
                value: ((ulong)cue.TimeMilliseconds)
            );
            EbmlWriter.WriteMasterHeader(
                stream: point,
                id: MatroskaIds.CueTrackPositions,
                contentSize: positionsContent.Length
            );
            ...
            EbmlWriter.WriteMasterHeader(
                stream: content,
                id: MatroskaIds.CuePoint,
                contentSize: pointContent.Length
            );
        ...
        EbmlWriter.WriteMasterHeader(
            stream: m_output,
            id: MatroskaIds.Cues,
            contentSize: cuesContent.Length
        );
    ...
    private void WriteEbmlHeader() {
        ...
        EbmlWriter.WriteUInt(
            id: MatroskaIds.EbmlVersion,
            stream: content,
            value: 1
        );
        EbmlWriter.WriteUInt(
            id: MatroskaIds.EbmlReadVersion,
            stream: content,
            value: 1
        );
        EbmlWriter.WriteUInt(
            id: MatroskaIds.EbmlMaxIdLength,
            stream: content,
            value: 4
        );
        EbmlWriter.WriteUInt(
            id: MatroskaIds.EbmlMaxSizeLength,
            stream: content,
            value: 8
        );
        EbmlWriter.WriteAsciiString(
            id: MatroskaIds.DocType,
            stream: content,
            value: (m_docTypeWebm
            ? "webm"
            : "matroska")
        ...
        EbmlWriter.WriteUInt(
            id: MatroskaIds.DocTypeVersion,
            stream: content,
            value: 4
        );
        EbmlWriter.WriteUInt(
            id: MatroskaIds.DocTypeReadVersion,
            stream: content,
            value: 2
        );
    ...
    private static void WriteInfo(Stream stream, out long durationOffset) {
        ...
        EbmlWriter.WriteUInt(
            id: MatroskaIds.TimestampScale,
            stream: content,
            value: NanosecondsPerMillisecond
        );
        EbmlWriter.WriteAsciiString(
            id: MatroskaIds.MuxingApp,
            stream: content,
            value: "puck"
        );
        EbmlWriter.WriteAsciiString(
            id: MatroskaIds.WritingApp,
            stream: content,
            value: "puck"
        );
        ...
        EbmlWriter.WriteId(
            id: MatroskaIds.Duration,
            stream: content
        );
        EbmlWriter.WriteSize(
            size: 8,
            stream: content
        );
        ...
        EbmlWriter.WriteMasterHeader(
            stream: stream,
            id: MatroskaIds.Info,
            contentSize: infoContent.Length
        );
    ...
    private static void WriteTrackEntry(Stream stream, TrackDeclaration track) {
        ...
        EbmlWriter.WriteUInt(
            stream: content,
            id: MatroskaIds.TrackNumber,
            value: ((ulong)track.Number)
        );
        EbmlWriter.WriteUInt(
            stream: content,
            id: MatroskaIds.TrackUid,
            value: track.Uid
        );
        EbmlWriter.WriteUInt(
            stream: content,
            id: MatroskaIds.TrackType,
            value: track.Type
        );
        EbmlWriter.WriteUInt(
            id: MatroskaIds.FlagLacing,
            stream: content,
            value: 0
        );
        EbmlWriter.WriteAsciiString(
            stream: content,
            id: MatroskaIds.CodecId,
            value: track.CodecId
        );
        ...
        if (!track.CodecPrivate.IsEmpty) {
            EbmlWriter.WriteBinary(
                stream: content,
                id: MatroskaIds.CodecPrivate,
                value: track.CodecPrivate.Span
            );
        }
        ...
        if (track.Type == MatroskaIds.TrackTypeVideo) {
            ...
            EbmlWriter.WriteUInt(
                stream: video,
                id: MatroskaIds.PixelWidth,
                value: ((ulong)track.Width)
            );
            EbmlWriter.WriteUInt(
                stream: video,
                id: MatroskaIds.PixelHeight,
                value: ((ulong)track.Height)
            );
            ...
            using (var colour = new MemoryStream()) {
                EbmlWriter.WriteUInt(
                    id: MatroskaIds.ColourRange,
                    stream: colour,
                    value: MatroskaIds.ColourRangeLimited
                );
                EbmlWriter.WriteUInt(
                    id: MatroskaIds.ColourMatrixCoefficients,
                    stream: colour,
                    value: MatroskaIds.ColourBt709
                );
                EbmlWriter.WriteUInt(
                    id: MatroskaIds.ColourTransferCharacteristics,
                    stream: colour,
                    value: MatroskaIds.ColourBt709
                );
                EbmlWriter.WriteUInt(
                    id: MatroskaIds.ColourPrimaries,
                    stream: colour,
                    value: MatroskaIds.ColourBt709
                );
                ...
                EbmlWriter.WriteMasterHeader(
                    stream: video,
                    id: MatroskaIds.Colour,
                    contentSize: colourContent.Length
                );
            ...
            EbmlWriter.WriteMasterHeader(
                stream: content,
                id: MatroskaIds.Video,
                contentSize: videoContent.Length
            );
            content.Write(buffer: videoContent);
        } else {
            using var audio = new MemoryStream();

            EbmlWriter.WriteDouble(
                stream: audio,
                id: MatroskaIds.SamplingFrequency,
                value: track.SamplingFrequency
            );
            EbmlWriter.WriteUInt(
                stream: audio,
                id: MatroskaIds.Channels,
                value: ((ulong)track.ChannelCount)
            );
            ...
            EbmlWriter.WriteMasterHeader(
                stream: content,
                id: MatroskaIds.Audio,
                contentSize: audioContent.Length
            );
            ...
            if (track.CodecDelayNanoseconds > 0L) {
                EbmlWriter.WriteUInt(
                    stream: content,
                    id: MatroskaIds.CodecDelay,
                    value: ((ulong)track.CodecDelayNanoseconds)
                );
            }
            ...
            if (track.SeekPreRollNanoseconds > 0L) {
                EbmlWriter.WriteUInt(
                    stream: content,
                    id: MatroskaIds.SeekPreRoll,
                    value: ((ulong)track.SeekPreRollNanoseconds)
                );
            }
        }
    ...
    public void Start() {
        ...
        WriteInfo(
            durationOffset: out m_durationOffset,
            stream: m_output
        );
    ...
    public void WriteBlock(int trackNumber, ReadOnlySpan<byte> data, long timestampNanoseconds, bool isKeyframe) {
        ...
        if (
            isCuePoint &&
            (m_pendingCue is null)
        ) {
            m_pendingCue = new PendingCue(
                TimeMilliseconds: blockMilliseconds,
                Track: trackNumber
            );
        }
        ...
        BinaryPrimitives.WriteInt16BigEndian(
            destination: prefix[1..3],
            value: ((short)relative)
        );
