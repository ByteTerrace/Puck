...
/// when the ladder or a device declines. Arm the host's frame-capture controller with the returned sink; call
...
public sealed class RecordingSession : ICaptureSink, IAudioPacketSink {
    ...
    private static OpusAudioLane? ResolveAudioLane(RecordingSessionOptions options, List<string> notes) {
        if (
            (options.AudioSourceFactory is null) ||
            (options.Document.Audio is not { Count: > 0 } rows)
        ) {
            return null;
        }
        ...
        foreach (var row in rows) {
            var source = ((row.Kind == RecordingAudioKind.Loopback)
                ...
                : options.AudioSourceFactory.CreateMicrophone(
                    deviceId: row.Device,
                    reason: out reason
                )
            );
            ...
            bindings.Add(item: new AudioLaneSource(
                Gain: row.Gain,
                Isolated: (row.Track == RecordingAudioTrackMode.Isolated),
                Source: source
            ));
        }
        ...
        return ((bindings.Count == 0)
            ? null
            : new OpusAudioLane(
                bitrateBitsPerSecond: AudioBitrateBitsPerSecond,
                sources: bindings
            )
        );
    }
    private static string ResolveOutputPath(RecordingSessionOptions options, bool hasVideo, bool webm) {
        ...
        return Path.Combine(
            path1: options.OutputDirectory,
            path2: (options.FileNamePrefix + extension)
        );
    }
    private static IVideoEncoder? ResolveVideoEncoder(RecordingSessionOptions options, List<string> notes) {
        if (
            (options.VideoEncoderFactory is null) ||
            (options.Document.Video is not { } video)
        ) {
            return null;
        }
    ...
    private static bool TryOpenOutput(
        ...
        if (
            (options.Document.Output is not null) &&
            string.IsNullOrWhiteSpace(value: options.Document.Output)
        ) {
        ...
        try {
            ...
            reason = $"output path '{displayPath.Replace(
                comparisonType: StringComparison.Ordinal,
                newValue: "\\0",
                oldValue: "\0"
            )}' is unusable: {exception.Message.ReplaceLineEndings(replacementText: " ")}";
    ...
    private void WriteVideoBlock(RecordedPacket packet) {
        ...
        if (m_stripAv1Td) {
            var stripped = Av1TemporalDelimiterFilter.Strip(
                destination: ref m_av1Scratch,
                temporalUnit: data
            );

            if (stripped >= 0) {
                data = m_av1Scratch.AsSpan(
                    length: stripped,
                    start: 0
                );
            }
        ...
        m_muxer.WriteBlock(
            ...
            trackNumber: m_videoTrackNumber
        );
    }
    ...
    public void Consume(in CaptureFrame frame) {
        ...
        if (
            (slot is null) ||
            (pixels.Length > slot.Pixels.Length)
        ) {
    ...
    public static bool TryCreate(RecordingSessionOptions options, out RecordingSession? session, out string reason) {
        ...
        var videoEncoder = ResolveVideoEncoder(
            notes: notes,
            options: options
        );
        var audioLane = ResolveAudioLane(
            notes: notes,
            options: options
        );

        if (
            (videoEncoder is null) &&
            (audioLane is null)
        ) {
            reason = $"nothing to record: {string.Join(
                separator: "; ",
                values: notes
            )}";
        ...
        var docTypeWebm = ((videoEncoder is not null) && string.Equals(
            a: videoEncoder.CodecId,
            b: "V_AV1",
            comparisonType: StringComparison.Ordinal
        ));
        ...
        var muxer = new MatroskaMuxer(
            output: output,
            webmDocType: docTypeWebm
        );
    ...
    private readonly Lock m_muxSync = new();
    ...
    private RecordingSession(
        ...
        m_stripAv1Td = ((videoEncoder is not null) && string.Equals(
            a: videoEncoder.CodecId,
            b: "V_AV1",
            comparisonType: StringComparison.Ordinal
        ));
