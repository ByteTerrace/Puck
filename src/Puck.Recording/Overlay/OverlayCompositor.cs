...
/// timecode) straight onto the copied CPU frame after frame capture, so they exist in the recording and never in
...
public sealed class OverlayCompositor {
    ...
    private static void DrawRect(Span<byte> pixels, SurfaceFormat format, int width, int height, OverlayRow row) {
        ...
        var rectWidth = Math.Max(
            val1: 0,
            val2: ((int)MathF.Round(x: (row.Width * width)))
        );
        var rectHeight = Math.Max(
            val1: 0,
            val2: ((int)MathF.Round(x: (row.Height * height)))
        );
        var originX = AnchorX(
            normalized: row.X,
            frame: width,
        ...
        if (
            (row.OutlineColor is { } outlineText) &&
            Rgba32.TryParse(
            color: out var outline,
            value: outlineText
        )
        ) {
    ...
    private static string FormatTimecode(long nanoseconds) {
        var totalMilliseconds = Math.Max(
            val1: 0L,
            val2: (nanoseconds / NanosecondsPerMillisecond)
        );
            ...
            switch (row.Kind) {
                ...
                case OverlayKind.Timecode:
                    DrawText(
                        ...
                        height: height,
                        row: row,
                        text: FormatTimecode(nanoseconds: ((row.Clock == OverlayClock.Sim)
                        ? simTimeNanoseconds
