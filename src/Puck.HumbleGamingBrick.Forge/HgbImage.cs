...
public static class HgbImage {
    ...
    public static byte[] BoxReduce(byte[] rgba, int width, int height, int factor, out int outWidth, out int outHeight) {
        ...
        if (
            ...
            throw new ArgumentException(
                message: $"The reduce factor {factor} must be >= 1 and divide the {width}x{height} source exactly.",
                paramName: nameof(factor)
            );
        }
    ...
    public static byte[] DecodeTile2bpp(ReadOnlySpan<byte> tileBytes) {
        if (tileBytes.Length != 16) {
            throw new ArgumentException(
                message: "A 2bpp tile is 16 bytes.",
                paramName: nameof(tileBytes)
            );
        }
    ...
    public static byte[] EncodePalette(Rgb[] palette) {
        ...
        if (palette.Length != 4) {
            throw new ArgumentException(
                message: "A brick palette is 4 colours.",
                paramName: nameof(palette)
            );
        }
    ...
    public static byte[] EncodeTile2bpp(ReadOnlySpan<byte> tileIndices) {
        if (tileIndices.Length != 64) {
            throw new ArgumentException(
                message: "A tile is 8x8 = 64 indices.",
                paramName: nameof(tileIndices)
            );
        }
    ...
    public static Rgb[] MedianCutPalette(IReadOnlyList<Rgb> pixels, int count, IReadOnlyList<Rgb>? seed = null) {
        ...
        if (remaining > 0) {
            ...
            while (boxes.Count < remaining) {
                ...
                source.Sort(comparison: (left, right) => ChannelValue(
                    channel: channel,
                    colour: left
                ).CompareTo(value: ChannelValue(
                    channel: channel,
                    colour: right
                )));
                ...
                boxes[bestBox] = source.GetRange(
                    count: mid,
                    index: 0
                );
                boxes.Add(item: source.GetRange(
                    index: mid,
                    count: (source.Count - mid)
                ));
            }
        ...
        while (palette.Count < count) {
            palette.Add(item: ((palette.Count > 0)
                ? palette[^1]
                : new Rgb(
                    B: 0,
                    G: 0,
    ...
    public static void SliceTilesDeduplicated(byte[] indices, int width, int height, out byte[] tileData, out byte[] tileIds, out int tileCount) {
        ...
        if (
            ...
            throw new ArgumentException(
                message: $"The {width}x{height} image must tile into 8x8 blocks.",
