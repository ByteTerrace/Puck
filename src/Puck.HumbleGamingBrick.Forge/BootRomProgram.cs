...
internal static class BootRomProgram {
    ...
    private static void EmitHandoffRegisters(Sm83Emitter emitter, BootRomLayout layout) {
        ...
        emitter.LoadImmediate(
            pair: Reg16.Bc,
            value: ((ushort)((handoff.B << 8) | handoff.C))
        );
        emitter.LoadImmediate(
            pair: Reg16.De,
            value: ((ushort)((handoff.D << 8) | handoff.E))
        );
        emitter.LoadImmediate(
            pair: Reg16.Hl,
            value: ((ushort)((handoff.H << 8) | handoff.L))
        );
    }
    ...
    private static void EmitHeaderVerification(Sm83Emitter emitter, int logo, bool compatible) {
        ...
        if (!compatible) {
            emitter.LoadImmediate(
                pair: Reg16.Hl,
                value: CartridgeHeader.LogoOffset
            );
            emitter.LoadImmediateAddressOf(
                label: logo,
                pair: Reg16.De
            ...
            emitter.LoadImmediate(
                destination: Reg8.C,
                value: ((byte)CartridgeHeader.Logo.Length)
            );
    ...
    private static void EmitTables(Sm83Emitter emitter, BootRomLayout layout, int logo, int mark, BootRomMark bitmap, int registerTable, BootRomColorLabels labels) {
        ...
        emitter.EmitData(value: ((bitmap == BootRomMark.Compatible)
            ? WordmarkBitmap
            : MarkBitmap));
        ...
        if (
            layout.VerifiesHeader &&
            (bitmap != BootRomMark.Compatible)
        ) {
    ...
    private static void EmitVideoSetup(Sm83Emitter emitter, BootRomLayout layout, int mark, bool compatible) {
        ...
        emitter.LoadImmediate(
            destination: Reg8.C,
            value: ((byte)(compatible
            ? WordmarkBitmap.Length
            : MarkBitmap.Length))
        );
        ...
        emitter.LoadImmediate(
            ...
            value: (compatible
            ? MarkTileMapEntry
            : (ushort)(MarkTileMapEntry + 1))
        );
        emitter.LoadAImmediate(value: 0x01);
        for (var tile = 0; (tile < (compatible
            ? 4
            : 1)); ++tile) {
            emitter.StoreAToHlIncrement();
            if (
