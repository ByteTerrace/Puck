...
/// forge's "verify by running" discipline, mirroring <c>Puck.HumbleGamingBrick.Forge.Tune.TuneVerify</c>'s identical shape.
/// </summary>
public static class ArcadeQuestVerify {
    /// <summary>Runs the whole battery.</summary>
    /// <param name="rom">The ROM image.</param>
    public static void Run(byte[] rom) {
        ArgumentNullException.ThrowIfNull(rom);

        using var driver = new VerifyMachineDriver(label: "arcade-quest", rom: rom);
    ...
    private static void Assert(bool condition, string message) =>
        VerifyMachineDriver.Assert(condition: condition, label: "arcade-quest", message: message);
}
