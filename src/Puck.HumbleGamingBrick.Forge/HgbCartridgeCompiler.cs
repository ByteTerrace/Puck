    ...
    public CartridgeCompilation Compile(CartridgeDocument document) {
        ...
        if (document.Target != Target) {
            throw new ArgumentException(
                message: "This compiler requires target cgb.",
        ...
        if (next > ArrayLimitAddress) {
            throw new ArgumentException(
                message: $"Arrays need {(next - ArrayBaseAddress)} bytes; work RAM holds {(ArrayLimitAddress - ArrayBaseAddress)}.",
                paramName: nameof(document)
            );
        }
        ...
        var data = new RomDataBuilder(text: new TextModule(
            bg: background,
            emitter: emitter,
            fontTileBase: 0
        ));
        var input = new InputModule(emitter: emitter);
        var backgroundPalettes = data.Add(
            name: "bg-palettes",
            bytes: CartridgeGraphics.PaletteBank(bank: document.Palettes.Background)
        );
        var objectPalettes = data.Add(
            name: "obj-palettes",
            bytes: CartridgeGraphics.PaletteBank(bank: document.Palettes.Object)
        );
        ...
        var tilePlacement = Place(
            bytes: tileBytes,
            name: "tiles"
        );
        var mapPlacement = Place(
            name: "map",
            bytes: document.Map.Select(selector: static tile => ((byte)tile)).ToArray()
        );
        // Video bank one holds one attribute byte per cell; the low three bits select the background palette.
        var attributePlacement = Place(
            name: "bg-attributes",
            bytes: (document.MapPalettes ?? new int[1024]).Select(selector: static entry => ((byte)(entry & 0x07))).ToArray()
        );
        var fadesToBlack = data.Add(
            name: "fade-black",
            bytes: FadeBanks(
                document: document,
                toWhite: false
        ...
        var windowMap = ((document.Window is null)
            ? default
            : Place(
                name: "window-map",
                bytes: document.Window.Map.Select(selector: static tile => ((byte)tile)).ToArray()
            )
        );
        var windowAttributes = ((document.Window is null)
            ? default
            : Place(
                name: "window-attributes",
                bytes: (document.Window.MapPalettes ?? new int[1024]).Select(selector: static entry => ((byte)(entry & 0x07))).ToArray()
            )
        );
        var trampoline = data.Add(
            name: "dma",
        ...
        var screens = document.Screens.ToDictionary(
            ...
            elementSelector: screen => (
                ...
                bytes: screen.Tiles.Select(selector: static tile => ((byte)tile)).ToArray()
            ),
                Shades: ((screen.Palettes is { } palettes)
            ? data.Add(
                    name: $"screen-{screen.Name}-palettes",
                    bytes: palettes.Select(selector: static shade => ((byte)(shade & 0x07))).ToArray()
                )
            : (RomTable?)null),
                screen.Width,
        ...
        var arrayData = data.Add(
            name: "arrays",
            bytes: ((arrayInitial.Length == 0)
            ? [0]
            : arrayInitial)
        );
        ...
        var sounds = document.Sounds.ToDictionary(
            ...
            ? (IsMusic: true, Parts: music.Select(selector: part => (
                    Table: data.Add(
                    name: $"music-{sound.Name}-{part.Voice}",
                    bytes: AudioDocumentCompiler.CompileMusicVoice(
                        document: AudioCanonicalizer.Normalize(document: part.Part),
                        voice: part.Voice
                    )
                ),
                    Voice: Voice(name: part.Voice),
                    Pattern: Waveform(
                    levels: part.Waveform,
                    name: $"wave-{sound.Name}-{part.Voice}"
                ))).ToArray())
            : (IsMusic: false, Parts: new[] { (
                    Table: data.Add(
                    name: $"effect-{sound.Name}",
                    bytes: AudioDocumentCompiler.CompileEffect(
                        effect: sound.Effect!,
                        frames: sound.Frames!.Value
                    )
                ),
                    Voice: Voice(name: (sound.Effect!.Voice ?? AudioEffectDocument.VoicePulse1)),
                    Pattern: Waveform(
                    levels: sound.Waveform,
                    name: $"wave-{sound.Name}"
                )) })),
            comparer: StringComparer.Ordinal
        );
        ...
        RomTable? Waveform(int[]? levels, string name) => ((levels is null)
            ...
            : data.Add(
                name: name,
                bytes: Enumerable.Range(
                    count: 16,
                    start: 0
                ).Select(selector: index => ((byte)((levels[(index * 2)] << 4) | levels[((index * 2) + 1)]))).ToArray()
            )
        );
        var audio = ((document.Sounds.Length == 0)
            ? null
            : new ApuSoundDriver()
        );
        var save = ((document.Save is null)
            ? null
            : new SaveModule(
                emitter: emitter,
                defaults: data.Add(
                ...
                version: ((byte)document.Save.Version)
            )
        );
        ...
        var spec = new FrameworkBootSpec(
            BgPalettes: backgroundPalettes,
            ObjPalettes: objectPalettes,
            Tiles: new RomTable(
                Address: tilePlacement.Address,
                Length: tileBytes.Length
            ),
            TileByteCount: tileBytes.Length,
            InitialMap: new RomTable(
                Address: mapPlacement.Address,
                Length: 0x400
            ),
            Lcdc: BaseControl(document: document),
            InitialState: 0,
            BgAttributes: new RomTable(
                Address: attributePlacement.Address,
                Length: 0x400
            ),
            ...
            RestoreFixedBank: ((banked.Banks.Count == 0)
            ? null
            : static emitter => Sm83BankedData.EmitSelect(
                    bank: 1,
                    emitter: emitter
                )),
        ...
        FrameworkKernel.EmitPrologue(
            emitter: emitter,
            bootLabel: boot,
            rasterRearm: ((document.Raster.Length == 0)
            ? null
            : EmitRasterRearm)
        );
        emitter.MarkLabel(label: boot);
        FrameworkKernel.EmitBootPrologue(
            dmaTrampoline: trampoline,
            emitter: emitter,
            spec: spec
        );
        Initialize();
        ...
        try {
            ...
            statHandlerAddress: ((document.Raster.Length == 0)
                ? (ushort)0
                : emitter.LabelAddress(
                        baseAddress: Hw.EntryAddress,
                        label: rasterHandler
                    )));
        } catch (Exception overrun) when ((overrun is ArgumentException or ArgumentOutOfRangeException)) {
            throw new CartridgeCapacityException(
                message: $"The document does not fit the Color machine's cartridge: {overrun.Message}",
                innerException: overrun
            );
        }

        return new CartridgeCompilation(
            Rom: rom,
        ...
        Sm83BankedData.Placement Place(string name, byte[] bytes) {
            if ((data.BytesUsed + bytes.Length) <= FrameworkCartridge.BankSize) {
                return new Sm83BankedData.Placement(
                    Bank: 1,
                    Address: data.Add(
                        bytes: bytes,
                        name: name
        ...
        static Action<Sm83Emitter>? Select(Sm83BankedData.Placement placement) =>
            ((placement.Bank == 1)
                ? null
                : emitter => Sm83BankedData.EmitSelect(
                    emitter: emitter,
                    bank: placement.Bank
                )
            );
        ...
        void InitializeWindow() {
            ...
            FrameworkKernel.EmitBlockCopy(
                emitter: emitter,
                sourceAddress: windowMap.Address,
                destinationAddress: Hw.VramWindowMap,
                byteCount: 0x400
            );
            ...
            FrameworkKernel.EmitBlockCopy(
                emitter: emitter,
                sourceAddress: windowAttributes.Address,
                destinationAddress: Hw.VramWindowMap,
                byteCount: 0x400
            );
            emitter.XorA();
            emitter.StoreAToHighPage(port: Hw.PortVramBank);
            Sm83BankedData.EmitSelect(
                bank: 1,
                emitter: emitter
            );
        }
        static byte BaseControl(CartridgeDocument document) =>
            ((byte)(0x93 | (document.TallSprites
                ? 0x04
                : 0x00) | ((document.Window is null)
                ? 0x00
                : 0x60)));
        ...
        void Initialize() {
            ...
            if (arrayInitial.Length != 0) {
                FrameworkKernel.EmitBlockCopy(
                    emitter: emitter,
                    sourceAddress: arrayData.Address,
                    destinationAddress: ArrayBaseAddress,
                    byteCount: ((ushort)arrayInitial.Length)
                );
            }
        }
        void Frame() {
            ...
            for (var i = 0; (i < document.Sprites.Length); ++i) {
                ...
                Load(expression: sprite.Visible); emitter.ArithmeticImmediate(
                    op: AluOp.Compare,
                    value: 0
                );
                emitter.JumpAbsolute(
                ...
                Load(expression: sprite.X); emitter.ArithmeticImmediate(
                    op: AluOp.Compare,
                    value: 160
                ); emitter.JumpAbsolute(
                    condition: Condition.NoCarry,
                ...
                emitter.ArithmeticImmediate(
                    op: AluOp.Add,
                    value: 8
                ); emitter.StoreAToAddress(address: ((ushort)(0xC101 + (i * 4))));
                Load(expression: sprite.Y); emitter.ArithmeticImmediate(
                    op: AluOp.Compare,
                    value: 144
                ); emitter.JumpAbsolute(
                    condition: Condition.NoCarry,
                ...
                emitter.ArithmeticImmediate(
                    op: AluOp.Add,
                    value: 16
                ); emitter.StoreAToAddress(address: ((ushort)(0xC100 + (i * 4))));
        ...
        void Gate(ActionPredicate? predicate, int fail) {
            switch (predicate) {
                ...
                default:
                    Compare(
                        compare: ((ActionPredicate.CompareValue)predicate),
                        fail: fail
                    );
        ...
        void Compare(ActionPredicate.CompareValue compare, int fail) {
            if (
                WideOperand(expression: compare.Left) ||
                WideOperand(expression: compare.Right)
            ) {
            ...
            foreach (var statement in statements) {
                switch (statement.Kind) {
                    ...
                    case "repeat": {
                            ...
                            emitter.ArithmeticImmediate(
                                op: AluOp.Compare,
                                value: ((byte)statement.Count!.Value)
                            );
                            emitter.JumpAbsolute(
                                condition: Condition.NoCarry,
                                label: exit
                    ...
                    case "play": {
                            ...
                            foreach (var part in sound.Parts) {
                                ...
                                if (part.Pattern is { } pattern) {
                                    ApuSoundDriver.EmitWavePatternLoad(
                                        emitter: emitter,
                                        pattern: pattern
                                    );
                                }
                                ...
                                if (sound.IsMusic) {
                                    ApuSoundDriver.EmitMusicStart(
                                        emitter: emitter,
                                        stream: part.Table,
                                        voice: part.Voice
                                    );
                                } else {
                                    ApuSoundDriver.EmitEffectStart(
                                        emitter: emitter,
                                        stream: part.Table,
                                        voice: part.Voice
                                    );
                    ...
                    case "fade": {
                            ...
                            var table = ((statement.Toward == "white")
                                ? fadesToWhite
                                : fadesToBlack
                            );
                            ...
                            emitter.ArithmeticImmediate(
                                op: AluOp.Compare,
                                value: (CartridgeLimits.FadeSteps + 1)
                            );
                            ...
                            emitter.MarkLabel(label: placed);
                            FrameworkKernel.EmitPaletteRepublish(
                                byteCount: backgroundBytes,
                                dataPort: Hw.PortBgPaletteData,
                                emitter: emitter,
                                indexPort: Hw.PortBgPaletteIndex
                            );
                            // The object table follows the background one, stepped by its own stride.
                            emitter.LoadImmediate(
                                pair: Reg16.Hl,
                                value: ((ushort)(table.Address + ((CartridgeLimits.FadeSteps + 1) * backgroundBytes)))
                            );
                            emitter.LoadImmediate(
                                pair: Reg16.De,
                                value: ((ushort)objectBytes)
                            );
                            Load(expression: statement.Amount!);
                            emitter.ArithmeticImmediate(
                                op: AluOp.Compare,
                            ...
                            FrameworkKernel.EmitPaletteRepublish(
                                byteCount: objectBytes,
                                dataPort: Hw.PortObjPaletteData,
                                emitter: emitter,
                                indexPort: Hw.PortObjPaletteIndex
                            );
                            break;
                        }
                    case "clock": {
                    ...
                    case "save": {
                            ...
                            foreach (var (address, length) in persisted) {
                                CopyRun(
                                    destination: ((ushort)(FrameworkMemoryMap.SaveMirror + offset)),
                                    length: length,
                                    source: ((ushort)address)
                                );
                                offset += length;
                            }
                    ...
                    case "load": {
                            ...
                            foreach (var (address, length) in persisted) {
                                CopyRun(
                                    destination: ((ushort)address),
                                    length: length,
                                    source: ((ushort)(FrameworkMemoryMap.SaveMirror + offset))
                                );
                                offset += length;
                            }
                    ...
                    case "blit": {
                            ...
                            background.EmitPaintRect(
                                sourceAddress: screen.Table.Address,
                                row: CartridgeExpressions.Whole(expression: statement.Row)!.Value,
                                column: CartridgeExpressions.Whole(expression: statement.Column)!.Value,
                                width: screen.Width,
        ...
        void Act(CartridgeStatement action) {
            ...
            if (
                WideTarget(target: action.Target) ||
                WideOperand(expression: action.Value)
            ) {
                LoadWide(
                ...
                if (action.Operation is ExpressionOp.Add) {
                    LoadWide(
                        expression: CartridgeExpressions.Of(state: action.Target!.State),
                        pair: Reg16.Hl
                    );
                    emitter.AddToHl(pair: Reg16.De);
                    StoreWide(
        ...
        void EmitAttributes(CartridgeSprite sprite, int index) {
            if (
                (sprite.Palette is null) &&
                (sprite.FlipX is null) &&
                (sprite.FlipY is null) &&
                (sprite.BehindBackground is null)
            ) {
                return;
            }
        ...
        static byte[] FadeBanks(CartridgeDocument document, bool toWhite) => [
            .. CartridgeGraphics.FadeTable(
                bank: document.Palettes.Background,
                steps: CartridgeLimits.FadeSteps,
                toWhite: toWhite
            ),
            .. CartridgeGraphics.FadeTable(
                bank: document.Palettes.Object,
                steps: CartridgeLimits.FadeSteps,
                toWhite: toWhite
            ),
        ];
        ...
        void EmitRasterHandler() {
            ...
            emitter.ArithmeticImmediate(
                op: AluOp.Compare,
                value: ((byte)document.Raster.Length)
            );
            emitter.JumpRelative(
                condition: Condition.NoCarry,
                label: parked
        ...
        bool WideTarget(CartridgeTarget? target) => ((target is { Key: null }) && widths.TryGetValue(
            key: target.State,
            value: out var width
        ) && (width == 2));
        ...
        void LoadWide(ExpressionProgram expression, Reg16 pair, bool guard = false) {
            ...
            if (
                (Bare(expression: expression) is { } name) &&
                variables.TryGetValue(
                key: name,
                value: out var variableAddress
            )
            ) {
                var address = ((guard && (name == document.Scene))
                    ? SceneAddress
        ...
        void Binary(ExpressionOp operation) {
            ...
            switch (operation) {
                ...
                case ExpressionOp.Maximum: {
                        ...
                        emitter.JumpRelative(
                            condition: ((operation == ExpressionOp.Minimum)
                            ? Condition.Carry
                            : Condition.NoCarry),
                            label: keep
                        );
        ...
        void Address(CartridgeTarget target) {
            ...
            emitter.LoadImmediate(
                pair: Reg16.Hl,
                value: ((ushort)variables[target.State])
            );
        }
