...
public sealed record PbakBundle(PbakBackground? Background, IReadOnlyList<PbakSpriteSet> Sprites) {
    ...
    private static byte[] Expect(List<(string FourCc, byte[] Payload)> chunks, ref int index, string fourCc) {
        if (
            (index >= chunks.Count) ||
            (chunks[index].FourCc != fourCc)
        ) {
            var found = ((index < chunks.Count)
                ? $"'{chunks[index].FourCc}'"
                : "the end of the blob"
            );
    ...
    private static PbakBackground ParseBackground(List<(string FourCc, byte[] Payload)> chunks, ref int index) {
        var tiles = ParseTiles(payload: Expect(
            chunks: chunks,
            fourCc: "TILE",
            index: ref index
        ));
        var map = ParseMap(payload: Expect(
            chunks: chunks,
            fourCc: "MAPX",
            index: ref index
        ));
        var attributes = TryTake(
            chunks: chunks,
            fourCc: "ATTR",
            index: ref index
        );
        ...
        var registers = ParseRegisters(payload: TryTake(
            chunks: chunks,
            fourCc: "DMGP",
            index: ref index
        ));

        return new PbakBackground(
    ...
    private static PbakSpriteSet ParseSpriteSet(List<(string FourCc, byte[] Payload)> chunks, ref int index) {
        ...
        var frames = ParseFrames(payload: Expect(
            chunks: chunks,
            fourCc: "META",
            index: ref index
        ));
        var animation = TryTake(
            chunks: chunks,
            fourCc: "ANIM",
            index: ref index
        );

        return new PbakSpriteSet(
    ...
    public static PbakBundle Parse(byte[] blob) {
        ...
        var index = 0;
        var background = (IsBackgroundStart(
            chunks: chunks,
            index: index
        )
            ? ParseBackground(
        ...
        while (index < chunks.Count) {
            sprites.Add(item: ParseSpriteSet(
                chunks: chunks,
                index: ref index
            ));
        }
