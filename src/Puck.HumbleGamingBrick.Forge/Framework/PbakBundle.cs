...
public readonly record struct PbakDmgRegisters(byte Bgp, byte Obp0, byte Obp1);
...
public sealed record PbakBackground(byte[] Tiles2bpp, byte[] TileMap, byte[]? AttributeMap, byte[] PaletteData, PbakDmgRegisters? Registers) {
...
public sealed record PbakMetaspriteFrame(byte[] Rows) {
...
public sealed record PbakSpriteSet(byte[] Tiles2bpp, byte[] PaletteData, PbakDmgRegisters? Registers, IReadOnlyList<PbakMetaspriteFrame> Frames, byte[]? AnimationPayload) {
    /// <summary>How many 8×8 tiles the section carries.</summary>
    public int TileCount => (Tiles2bpp.Length / 16);
    /// <summary>How many 4-colour palettes the section carries.</summary>
    public int PaletteCount => (PaletteData.Length / 8);
...
public sealed record PbakBundle(PbakBackground? Background, IReadOnlyList<PbakSpriteSet> Sprites) {
    ...
    private static PbakBackground ParseBackground(List<(string FourCc, byte[] Payload)> chunks, ref int index) {
        var tiles = ParseTiles(payload: Expect(chunks: chunks, fourCc: "TILE", index: ref index));
        var map = ParseMap(payload: Expect(chunks: chunks, fourCc: "MAPX", index: ref index));
        var attributes = TryTake(chunks: chunks, fourCc: "ATTR", index: ref index);
        ...
        var palettes = ParsePalettes(payload: Expect(chunks: chunks, fourCc: "PALB", index: ref index), fourCc: "PALB");
        var registers = ParseRegisters(payload: TryTake(chunks: chunks, fourCc: "DMGP", index: ref index));

        return new PbakBackground(AttributeMap: attributes, PaletteData: palettes, Registers: registers, TileMap: map, Tiles2bpp: tiles);
    }
    // TILE, PALO, [DMGP], META, [ANIM].
    private static PbakSpriteSet ParseSpriteSet(List<(string FourCc, byte[] Payload)> chunks, ref int index) {
        var tiles = ParseTiles(payload: Expect(chunks: chunks, fourCc: "TILE", index: ref index));
        var palettes = ParsePalettes(payload: Expect(chunks: chunks, fourCc: "PALO", index: ref index), fourCc: "PALO");
        var registers = ParseRegisters(payload: TryTake(chunks: chunks, fourCc: "DMGP", index: ref index));
        var frames = ParseFrames(payload: Expect(chunks: chunks, fourCc: "META", index: ref index));
        var animation = TryTake(chunks: chunks, fourCc: "ANIM", index: ref index);

        return new PbakSpriteSet(AnimationPayload: animation, Frames: frames, PaletteData: palettes, Registers: registers, Tiles2bpp: tiles);
    }
