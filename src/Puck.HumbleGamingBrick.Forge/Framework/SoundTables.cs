...
public static class SoundTables {
    ...
    public static SoundEffect[] BuildEffectCatalog() => [
        ...
        new SoundEffect(Id: EffectShuffle, Name: "shuffle", Voice: SoundVoice.Noise, Stream: BuildStream(
            NoiseStep(envelope: 0x51, frames: 2, polynomial: 0x46),
            NoiseStep(envelope: 0x41, frames: 2, polynomial: 0x52),
            NoiseStep(envelope: 0x51, frames: 2, polynomial: 0x46),
            NoiseStep(envelope: 0x41, frames: 2, polynomial: 0x52),
    ...
    public static byte[] BuildMusicLoop() {
        ...
        foreach (var note in bars) {
            stream.AddRange(collection: MusicNote(duty: DutyQuarter, envelope: 0x63, frames: 8, millihertz: note));
        }
