...
public readonly record struct SoundEffect(byte Id, string Name, SoundVoice Voice, byte[] Stream);
/// <summary>
...
public static class SoundTables {
    ...
    public static byte[] BuildMusicLoop() {
        ...
        foreach (var note in bars) {
            stream.AddRange(collection: MusicNote(
                duty: DutyQuarter,
                envelope: 0x63,
                frames: 8,
                millihertz: note
    ...
    public static void DefineIn(GameManifest manifest, byte[]? musicLoop = null) {
        ...
        foreach (var effect in BuildEffectCatalog()) {
            manifest.DefineTable(
                name: EffectTableName(name: effect.Name),
                bytes: effect.Stream
            );
        }

        manifest.DefineTable(
            name: MusicLoopTableName,
            bytes: (musicLoop ?? BuildMusicLoop())
        );
    }
