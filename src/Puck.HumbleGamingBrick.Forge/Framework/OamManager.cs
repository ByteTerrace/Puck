...
/// </summary>
public sealed class OamManager {
    ...
    public void EmitDrawMetasprite(int baseSlot, int spriteCount) {
        for (var index = 0; (index < spriteCount); index++) {
            ...
            m_emitter.StoreAToAddress(address: SpriteAddress(
                byteIndex: 0,
                slot: slot
            ));
    ...
    public int Reserve(int count) {
        if (
            (count < 1) ||
            ((m_reservedSlots + count) > SlotCount)
        ) {
            throw new ArgumentOutOfRangeException(
                paramName: nameof(count),
                message: $"Reserving {count} sprite slots exceeds the {SlotCount}-slot table ({m_reservedSlots} already reserved)."
            );
        }
    ...
    public static ushort SpriteAddress(int slot, int byteIndex) {
        if (
            ...
            (slot >= SlotCount) ||
            (byteIndex < 0) ||
            (byteIndex >= BytesPerSlot)
        ) {
            throw new ArgumentOutOfRangeException(
                paramName: nameof(slot),
                message: $"Sprite byte ({slot}, {byteIndex}) is outside the shadow OAM."
            );
        }
