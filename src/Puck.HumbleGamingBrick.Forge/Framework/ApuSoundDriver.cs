...
public sealed class ApuSoundDriver : ISoundDriver {
    ...
    public void EmitEffect(Sm83Emitter emitter, byte effectId) {
        ...
        if (!m_effects.TryGetValue(
            key: effectId,
            value: out var effect
        )) {
            throw new ArgumentException(
                message: $"Effect id {effectId} is not in the sound catalog (was the driver bound?).",
                paramName: nameof(effectId)
            );
        }
