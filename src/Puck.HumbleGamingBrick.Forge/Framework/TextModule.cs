...
public sealed class TextModule {
    ...
    private static byte[] EncodeGlyph(string[] rows) {
        ...
        for (var row = 0; (row < 8); row++) {
            ...
            for (var column = 0; (column < 8); column++) {
                indices[((row * 8) + column)] = ((byte)(((column < line.Length) && (line[column] == '#'))
                    ? 3
                    : 0));
            }
        }
    ...
    private static int GlyphIndexOf(char character) =>
        character switch {
            ...
            message: $"The framework font has no glyph for '{character}' (supported: space, 0-9, A-Z, '>', '-', '.').",
    ...
    public static byte[] BuildFontTiles() {
        ...
        for (var glyph = 0; (glyph < GlyphCount); glyph++) {
            EncodeGlyph(rows: Glyphs[glyph]).CopyTo(
                array: tiles,
                index: (glyph * 16)
            );
        }
    ...
    public void EmitPrintBcdDirect(ushort bcdAddress, int byteCount, int row, int column) =>
        EmitPrintBcd(
            bcdAddress: bcdAddress,
            byteCount: byteCount,
            column: column,
            label: m_printBcdDirectLabel,
            row: row
        );
    ...
    /// <returns>The tile id.</returns>
    public byte TileFor(char character) => TileFor(
        character: character,
        fontTileBase: m_fontTileBase
    );
}
