...
public sealed class BgModule {
    ...
    public void EmitFullPaint(RomTable map, byte lcdc) {
        if (map.Length != 0x400) {
            throw new ArgumentException(
                message: "A full-screen map is 32×32 = 1024 bytes.",
    ...
    public void EmitLibrary() {
        ...
        m_emitter.ArithmeticImmediate(
            op: AluOp.Compare,
            value: FrameworkMemoryMap.VramQueueCapacity
        );
        ...
        m_emitter.ArithmeticImmediate(
            op: AluOp.Add,
            value: ((byte)(FrameworkMemoryMap.VramQueue & 0xFF))
        );
        m_emitter.Load(
            destination: Reg8.L,
            source: Reg8.A
        );                                   // Entries never cross the page.
        m_emitter.LoadImmediate(
            destination: Reg8.H,
            value: ((byte)(FrameworkMemoryMap.VramQueue >> 8))
        );
        m_emitter.Load(
    ...
    public void EmitPaintRect(ushort sourceAddress, int row, int column, int width, int height) {
        if (
            ...
            throw new ArgumentOutOfRangeException(
                ...
                message: $"A {width}×{height} paint rectangle is out of the map's range."
            );
        }
