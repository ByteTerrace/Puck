...
public sealed record ScreenText(int Row, int Column, string Text);
...
public readonly record struct InputScriptStep(byte Buttons, byte Frames);
...
public sealed record ScoreTableEntry(string Initials, int Score);
...
public readonly record struct RomRecords(RomTable Table, int Stride, int Count);
...
public sealed record LinkedScreen(RomTable Map, RomTable? Attributes);
/// <summary>
...
/// </summary>
public sealed class LinkedManifest {
