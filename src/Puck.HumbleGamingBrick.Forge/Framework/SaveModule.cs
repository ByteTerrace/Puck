...
/// </summary>
public sealed class SaveModule {
    ...
    public SaveModule(Sm83Emitter emitter, RomTable defaults, byte version) {
        ...
        if (
            (defaults.Length < 1) ||
            (defaults.Length > FrameworkMemoryMap.SaveMirrorCapacity)
        ) {
            throw new ArgumentException(
                message: $"The save payload is {defaults.Length} bytes; the mirror holds 1..{FrameworkMemoryMap.SaveMirrorCapacity}.",
                paramName: nameof(defaults)
            );
        }
    ...
    private void EmitChecksumSubroutine() {
        ...
        m_emitter.LoadImmediate(
            destination: Reg8.D,
            value: ((byte)m_defaults.Length)
        );
    ...
    private void EmitLoadSubroutine() {
        ...
        FrameworkKernel.EmitBlockCopy(
            byteCount: ((ushort)payloadCount),
            destinationAddress: FrameworkMemoryMap.SaveMirror,
            emitter: m_emitter,
            sourceAddress: PayloadBase
        );
        ...
        FrameworkKernel.EmitBlockCopy(
            emitter: m_emitter,
            sourceAddress: m_defaults.Address,
            destinationAddress: FrameworkMemoryMap.SaveMirror,
            byteCount: ((ushort)payloadCount)
        );
    ...
    private void EmitStoreSubroutine() {
        ...
        FrameworkKernel.EmitBlockCopy(
            byteCount: ((ushort)payloadCount),
            destinationAddress: PayloadBase,
            emitter: m_emitter,
            sourceAddress: FrameworkMemoryMap.SaveMirror
        );
