    ...
    public static byte[] Build(string title, byte[] routine, byte[] data, IReadOnlyList<byte[]>? banks = null, bool clock = false, ushort statHandlerAddress = 0, byte[]? logo = null) {
        ...
        if (routine.Length > MaxRoutineBytes) {
            throw new ArgumentException(
                message: $"The game routine is {routine.Length} bytes, over the {MaxRoutineBytes}-byte code window (0x{Hw.EntryAddress:X4}..0x{RomDataBuilder.BaseAddress:X4}).",
        ...
        if (
            (routine.Length < 3) ||
            (routine[0] != OpcodeJumpAbsolute)
        ) {
            throw new ArgumentException(
                message: "The routine must open with the 3-byte 'jp boot' prologue so the VBlank handler lands at 0x0153.",
                paramName: nameof(routine)
            );
        }
        ...
        for (var index = 0; (index < extra.Count); ++index) {
            if (
                (extra[index] is null) ||
                (extra[index].Length > BankSize)
            ) {
                throw new ArgumentException(
                    message: $"Bank {(index + 2)} is {(extra[index]?.Length ?? 0)} bytes, over the {BankSize}-byte window.",
                    paramName: nameof(banks)
                );
            }
        }
        ...
        if (bankCount > MaxBankCount) {
            throw new ArgumentException(
                message: $"{extra.Count} extra banks exceed the {MaxBankCount}-bank ceiling.",
                paramName: nameof(banks)
            );
        }
        ...
        WriteInterruptVectors(
            rom: rom,
            statHandlerAddress: statHandlerAddress
        );
        WriteHeader(
            bankCount: bankCount,
            clock: clock,
        ...
        routine.CopyTo(
            array: rom,
            index: Hw.EntryAddress
        );
        data.CopyTo(
            array: rom,
            index: RomDataBuilder.BaseAddress
        );
        for (var index = 0; (index < extra.Count); ++index) {
            extra[index].CopyTo(
                array: rom,
                index: ((index + 2) * BankSize)
            );
        }
