...
public enum Reg8 : byte { B = 0, C = 1, D = 2, E = 3, H = 4, L = 5, Memory = 6, A = 7 }
...
public enum Reg16 : byte { Bc = 0, De = 1, Hl = 2, Sp = 3 }
...
public enum StackPair : byte { Bc = 0, De = 1, Hl = 2, Af = 3 }
...
public enum Condition : byte { NotZero = 0, Zero = 1, NoCarry = 2, Carry = 3 }
/// <summary>
...
public sealed class Sm83Emitter {
    ...
    public void Call(int label) => EmitAbsolute(
        label: label,
        opcode: 0xCD
    );
    /// <summary>call cc, label — conditional absolute call.</summary>
    public void Call(Condition condition, int label) => EmitAbsolute(
        label: label,
        opcode: ((byte)(0xC4 + (((byte)condition) * 8)))
    );
    ...
    public void Call(ushort address) => EmitImmediate16(
        opcode: 0xCD,
        value: address
    );
    ...
    public void JumpAbsolute(int label) => EmitAbsolute(
        label: label,
        opcode: 0xC3
    );
    /// <summary>jp cc, label — absolute jump taken when <paramref name="condition"/> holds.</summary>
    public void JumpAbsolute(Condition condition, int label) => EmitAbsolute(
        label: label,
        opcode: ((byte)(0xC2 + (((byte)condition) * 8)))
    );
    ...
    public void JumpAbsolute(ushort address) => EmitImmediate16(
        opcode: 0xC3,
        value: address
    );
    ...
    /// <returns>The label's bus address.</returns>
    ...
    public ushort LabelAddress(int label, ushort baseAddress) =>
        ((ushort)(baseAddress + m_labels.Resolve(
            kind: "A vector",
            label: label
        )));
    ...
    public void LoadAFromAddress(ushort address) => EmitImmediate16(
        opcode: 0xFA,
        value: address
    );
    ...
    public void LoadAImmediate(byte value) => LoadImmediate(
        destination: Reg8.A,
        value: value
    );
    ...
    public void LoadImmediate(Reg16 pair, ushort value) => EmitImmediate16(
        opcode: ((byte)(0x01 + (((byte)pair) * 16))),
        value: value
    );
    ...
    public void LoadImmediateAddressOf(Reg16 pair, int label) => EmitAbsolute(
        label: label,
        opcode: ((byte)(0x01 + (((byte)pair) * 16)))
    );
    /// <summary>ld sp, nn — set the stack pointer (a named <see cref="LoadImmediate(Reg16, ushort)"/> for readability).</summary>
    public void LoadStackPointer(ushort value) => LoadImmediate(
        pair: Reg16.Sp,
        value: value
    );
    ...
    public void ResetBit(int bit, Reg8 register) => EmitBitOp(
        baseOpcode: 0x80,
        bit: bit,
        register: register
    );
    ...
    public void RestartVector(int vector) {
        if (
            ...
            throw new ArgumentOutOfRangeException(
                paramName: nameof(vector),
                message: "A restart vector is one of 0x00, 0x08, 0x10, 0x18, 0x20, 0x28, 0x30, 0x38."
            );
        }
    ...
    public void SetBit(int bit, Reg8 register) => EmitBitOp(
        baseOpcode: 0xC0,
        bit: bit,
        register: register
    );
    ...
    public void StoreAToAddress(ushort address) => EmitImmediate16(
        opcode: 0xEA,
        value: address
    );
    ...
    public void StoreStackPointerToAddress(ushort address) => EmitImmediate16(
        opcode: 0x08,
        value: address
    );
    ...
    public void TestBit(int bit, Reg8 register) => EmitBitOp(
        baseOpcode: 0x40,
        bit: bit,
        register: register
    );
