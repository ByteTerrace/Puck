...
public static class CartridgeLanguageServices {
    ...
    public static JsonArray? Completions(DocumentNode document) {
        ...
        Add(
            insertText: "template ${1:name}(${2:parameters}) {\n    $0\n}",
            label: "template"
        );
        Add(
        ...
        Add(
            insertText: "repeat ${1:count} as ${2:index} {\n    $0\n}",
            label: "repeat"
        );
        Add(
    ...
    public static void Validate(JsonObject document, SourceMap sourceMap, DiagnosticBag diagnostics, SourceSpan fallback) {
        foreach (var error in CartridgeDocuments.Validate(utf8: CanonicalJsonDocument.Serialize(node: document))) {
            var pointer = error.Path.TrimStart(
                '$',
                '.'
            ).Replace(
            ...
            var span = (sourceMap.TryGetSpan(
                jsonPointer: pointer,
                span: out var located
            )
                ? located
                : fallback
            );
