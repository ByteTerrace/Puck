...
public static class CartridgeDocumentEmitter {
    ...
    private static readonly Dictionary<string, string> Operations = new(comparer: StringComparer.Ordinal) {
    ...
    private static readonly HashSet<string> OperandArguments = new(comparer: StringComparer.Ordinal) {
    ...
    private static readonly HashSet<string> CallActions = new(comparer: StringComparer.Ordinal) {
    ...
    private static JsonNode? Composite(string discriminator, IReadOnlyList<PredicateNode> operands, DocumentScope scope) {
        ...
        return ((predicates.Count == 1)
            ? predicates[0]!.DeepClone()
            : new JsonObject { ["$type"] = discriminator, ["predicates"] = predicates }
        );
    }
    private static JsonNode? LowerAction(StatementNode statement, DocumentScope scope) {
        switch (statement) {
            ...
            case AddCellStatementNode add:
                return LowerAssignment(
                    target: add.Target,
                    operation: nameof(ExpressionOp.Add),
                    rhs: add.Rhs,
                    span: add.Span,
                    scope: scope
                );
            ...
            case CompoundAssignStatementNode compound: {
                    if (!Operations.TryGetValue(
                        ...
                        Refuse(
                            scope: scope,
                            span: compound.Span,
                            message: $"'{compound.Operator}=' is not a cartridge operation"
                        );
            ...
            case IfStatementNode branch: {
                    ...
                    result["then"] = LowerActions(
                        statements: branch.Then,
                        scope: scope
                    );

                    if (branch.Else is not null) {
                        result["else"] = LowerActions(
            ...
            case RepeatStatementNode loop: {
                    var count = DocumentLowering.LowerValue(
                        expr: loop.Count,
                        scope: scope
                    );
                    ...
                    if (
                        ...
                        node: count,
                        number: out var iterations
                    ) ||
                        (iterations != Math.Truncate(d: iterations)) ||
                        (iterations < 1) ||
                        (iterations > 255)
                    ) {
                        Refuse(
                            scope: scope,
                            span: loop.Span,
                            message: "a repeat count is a whole number in 1..255 known at compile time"
                        );
            ...
            default:
                Refuse(
                    scope: scope,
                    span: statement.Span,
                    message: $"{Spell(statement: statement)} is not a cartridge rule step"
                );
    ...
    private static JsonNode? LowerAssignment(RowRefNode target, string? operation, RhsNode rhs, SourceSpan span, DocumentScope scope) {
        var destination = CartridgeOperand.FromRowRef(
            reason: out var targetReason,
            rowRef: target,
            scope: scope
        );
        ...
        if (rhs is not RhsOperandNode operand) {
            Refuse(
                message: "a cartridge value is a literal, variable or array element",
                scope: scope,
                span: span
            );
    ...
    private static void LowerBlock(BlockNode block, JsonObject target, DocumentScope scope) {
        ...
        var name = DocumentLowering.ResolveBlockName(
            block: block,
            scope: scope
        );

        var section = Pluralize(identifier: block.Identifier);

        scope.CurrentPointer = ((name is null)
            ? $"{pointer}/{block.Identifier}"
            : $"{pointer}/{section}/{((target[section] as JsonArray)?.Count ?? 0)}"
        );
        scope.SourceMap?.Register(
            jsonPointer: scope.CurrentPointer,
            span: block.Span
        );
        ...
        Append(
            target: target,
            section: Pluralize(identifier: block.Identifier),
            item: obj
        );
    }
    private static JsonNode? LowerCallAction(CallExpressionNode call, DocumentScope scope) {
        if (!CallActions.Contains(item: call.Name)) {
            Refuse(
                scope: scope,
                span: call.Span,
                message: $"'{call.Name}' is not a cartridge rule step"
            );
        ...
        foreach (var argument in call.Arguments) {
            ...
            if (OperandArguments.Contains(item: key)) {
                ...
                result[key] = DocumentLowering.LowerValue(
                    expr: argument.Value,
                    scope: scope,
                    fieldKey: key
                );
        ...
        switch (predicate) {
            ...
            case NotPredicateNode not: {
                    ...
                    return ((inner is null)
                        ? null
                        : new JsonObject { ["$type"] = "not", ["predicate"] = inner }
                    );
                }
            ...
            case ComparisonPredicateNode comparison: {
                    if (!ExpressionComparisons.TryParseSymbol(
                        ...
                        Refuse(
                            ...
                            message: $"'{comparison.Comparator}' is not a cartridge comparison"
                        );
                    ...
                    if (
                        ...
                        Refuse(
                            scope: scope,
                            span: comparison.Span,
                            message: (leftReason ?? rightReason)!
                        );
            ...
            case CallPredicateNode { Call.Name: "key" } key: {
                    if (key.Call.Arguments.Count != 2) {
                        Refuse(
                            scope: scope,
                            span: key.Span,
                            message: "a key condition reads 'key(<button>, <held|pressed|released>)'"
                        );
            ...
            default:
                Refuse(
                    scope: scope,
                    span: predicate.Span,
                    message: "a cartridge gate composes comparisons and key tests through and, or and not"
                );
    ...
    private static string Pluralize(string identifier) => (identifier.EndsWith(
    ...
    private static void ProcessStatement(StatementNode statement, JsonObject target, DocumentScope scope) {
        ...
        switch (statement) {
            ...
            case PropertyNode property: {
                    var lowered = DocumentLowering.LowerValue(
                        expr: property.Value,
                        scope: scope,
                    ...
                    scope.SourceMap?.Register(
                        jsonPointer: $"{scope.CurrentPointer}/{property.Name}",
                        span: property.Span
                    );
            ...
            case RuleBlockNode rule:
                scope.SourceMap?.Register(
                    jsonPointer: $"{scope.CurrentPointer}/rules/{((target["rules"] as JsonArray)?.Count ?? 0)}",
                    span: rule.Span
                );
                Append(
                    target: target,
            ...
            default:
                Refuse(
                    scope: scope,
                    span: statement.Span,
                    message: $"{Spell(statement: statement)} has no place in a cartridge document"
                );
    ...
    public static CompilationResult<JsonObject> LowerWithDiagnostics(DocumentNode document, DiagnosticBag? diagnostics = null, SourceMap? sourceMap = null, CancellationToken cancellationToken = default) {
        ...
        return new CompilationResult<JsonObject>(
            ((JsonObject)DocumentLowering.Canonicalize(node: root)!),
            diagnostics
        );
    }
}
