...
internal sealed class WorldMutationCommandModule(WorldServer server, IServerLink link, WorldDefinitionSource definitionSource, WorldRenderSettings renderSettings, WorldScreenBinder screenBinder, Client.WorldAudioDirector audioDirector, PresentPacingControl pacing) : ICommandModule {
    ...
    public IEnumerable<CommandDefinition> GetCommands() {
        yield return Simulation(
            ...
            handler: (context, args) => {
                ...
                return Submit(mutation: new WorldMutation.SetDefaultSeatKit(Principal: context.ActingPrincipal(), Name: args[0].ToString()));
            }
        );
        yield return Simulation(
            ...
            handler: (context, args) => {
                ...
                if ((local < 1) || (local > WorldPopulation.LocalSeatCount)) {
                ...
                for (var slot = 0; (slot < seatActivation.Length); slot++) {
                    seatActivation[slot] = ((slot < local) ? SeatActivationPolicy.Eager : SeatActivationPolicy.OnDemand);
                }
                ...
                return Submit(mutation: new WorldMutation.SetPopulationDefaults(Principal: context.ActingPrincipal(), Population: (server.Definition.Population with { SeatActivation = seatActivation, NetworkPlayers = network })));
            }
        );
        yield return CommandDefinition.WithWireArgs(
            ...
            handler: (_, args) => {
                if (args.Count != 1) {
                    return Usage(verb: "world.placement.get", form: "<id>");
                }

                var id = args[0].ToString();

                if (WorldDefinitionRows.FindPlacement(placements: server.Definition.Placements, id: id) is not { } placement) {
                    return CommandResult.Error(output: $"[world.placement.get: no placement row named '{id}']");
                }

                return new CommandResult(Output: JsonSerializer.Serialize(value: placement, jsonTypeInfo: WorldJsonContext.Default.WorldPlacement));
            }
        );
        yield return Simulation(
            ...
            handler: (context, args) => {
                ...
                if (!WorldGrantCommandModule.TryParseGrant(args: args, exclusiveAllowed: true, verb: "world.grant.set", grant: out var grant, error: out var error, channels: server.Population.Channels, targets: server.Population.TargetRegisters, verbsAllowed: true)) {
                    return error;
                }

                return Submit(mutation: new WorldMutation.UpsertGrant(Principal: context.ActingPrincipal(), Row: grant));
            }
        );
        yield return Simulation(
            ...
            handler: (context, args) => {
                if (!WorldGrantCommandModule.TryParseGrant(args: args, exclusiveAllowed: false, verb: "world.grant.remove", grant: out var grant, error: out var error)) {
                    return error;
                }

                return Submit(mutation: new WorldMutation.RemoveGrant(Principal: context.ActingPrincipal(), Target: grant));
            }
        );
        yield return Simulation(
            name: "world.reset",
            ...
            handler: (context, _) => {
                link.SubmitRebuild(request: new WorldRebuildRequest(Kind: WorldRebuildKind.Reset, Definition: null, PathHint: null, Force: false), principal: context.ActingPrincipal());
        ...
        yield return Simulation(
            ...
            handler: (context, args) => {
                if (!TryParseLoadArgs(raw: WorldCommandArguments.Raw(context: context, args: args), path: out var path, force: out var force)) {
                    return Usage(verb: "world.load", form: "<path> [force]");
                }
                ...
                link.SubmitRebuild(request: new WorldRebuildRequest(Kind: WorldRebuildKind.Load, Definition: loaded!, PathHint: fullPath, Force: force, ContentHash: contentHash), principal: context.ActingPrincipal());
        ...
        yield return Simulation(
            ...
            handler: (context, _) => {
                ...
                link.SubmitRebuild(request: new WorldRebuildRequest(Kind: WorldRebuildKind.Reload, Definition: loaded!, PathHint: path, Force: false, ContentHash: contentHash), principal: context.ActingPrincipal());
        ...
        yield return Simulation(
            ...
            handler: (context, args) => {
                ...
                link.SubmitUndo(count: count, principal: context.ActingPrincipal());
        ...
        yield return CommandDefinition.WithWireArgs(
            ...
            handler: (context, args) => {
                var target = ((args.Count >= 1) ? args.Tail(start: 0) : definitionSource.SourcePath);
                ...
                // leak durably.
                if (!server.Grants.AllowsAllSections(principal: context.ActingPrincipal(), capability: WorldCapability.Mutate, deniedSection: out var savedSection, denial: out var saveVerdict)) {
                ...
                try {
                    ...
                    var snapshot = WorldSessionCapture.Capture(definition: server.Definition, render: renderSettings, population: server.Population, binder: screenBinder, audio: audioDirector, pacing: pacing, tick: (server.NextInputTick - 1UL));
        ...
        yield return CommandDefinition.WithWireArgs(
            ...
            handler: (_, _) => {
                ...
                var audioCurve = (string.Equals(a: definition.Audio.DefaultCurve, b: WorldAudioDefaults.CurveLinear, comparisonType: StringComparison.Ordinal)
    ...
    private static bool TryParseLoadArgs(string raw, out string path, out bool force) {
        ...
        if ((lastSpace >= 0) && string.Equals(a: trimmed[(lastSpace + 1)..], b: "force", comparisonType: StringComparison.OrdinalIgnoreCase)) {
            force = true;
