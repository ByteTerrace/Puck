...
internal sealed partial class WorldProbes {
    ...
    // Resolves and deep-validates one declared control binding row's (probe, channel) reference and its control
    // name against the runtime vocabulary — WorldDefinitionValidator.ProbeControlNames already proved the string
    // itself is one of the thirteen recognized names, so a miss here can only mean the two lists drifted.
    private ControlState BuildControl(int probeIndex, WorldProbeBinding.Control control, string path) {
    ...
    // Writes every declared control binding's conditioned value onto the physical device's control surface, at most
    // once every ControlWriteFrameDivisor host frames and only on change — mirroring ApplyCameraControls' own
    // best-effort, device-authoritative contract. A reopened device reapplies the world's AUTHORED control state
    // first (WorldScreenBinder.ApplyCameraControls), so a sense-driven write here only ever moves the device off
    // that baseline; it never competes with it for which one "wins" on open.
    private void ServiceControls(ulong frameKey) {
        ...
        foreach (var control in m_controlBindings) {
            var probe = m_probes[control.ProbeIndex];
            ...
            if (
                !m_screens.TryGetCameraAttachment(sensor: sensor, attachment: out var attachment) ||
                (attachment.Controls is not { } surface)
            ) {
                continue;
            }

            var spec = probe.Manifest.Channels[control.Channel];
