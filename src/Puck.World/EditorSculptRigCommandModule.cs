...
internal sealed class EditorSculptRigCommandModule(WorldEditorSession session, WorldWorkbench workbench) : ICommandModule {
    ...
    public IEnumerable<CommandDefinition> GetCommands() {
        ...
        yield return CommandDefinition.WithWireArgs(
            ...
            handler: (context, args) => FrameStepHandler(context: context, args: in args, direction: 1, verb: FrameNextCommand)
        );
        yield return CommandDefinition.WithWireArgs(
            ...
            handler: (context, args) => FrameStepHandler(context: context, args: in args, direction: -1, verb: FramePrevCommand)
        );
        yield return CommandDefinition.WithWireArgs(
    ...
    private CommandResult FrameHandler(CommandContext context, WireArgs args) {
        ...
        var (slot, model, error) = EditorSculptCommandModule.ResolveBench(context: context, args: in args, at: 1, verb: "editor.sculpt.frame", session: m_session, workbench: m_workbench);
    ...
    private CommandResult FrameStepHandler(CommandContext context, in WireArgs args, int direction, string verb) {
    ...
    private CommandResult ChainHandler(CommandContext context, WireArgs args) {
        ...
        var (slot, model, error) = EditorSculptCommandModule.ResolveBench(context: context, args: WireArgs.Empty, at: 0, verb: "editor.sculpt.chain", session: m_session, workbench: m_workbench);
    ...
    private CommandResult ChainDefineHandler(CommandContext context, WireArgs args) {
        var (slot, model, error) = EditorSculptCommandModule.ResolveBench(context: context, args: in args, at: 0, verb: ChainDefineCommand, session: m_session, workbench: m_workbench);
    ...
    private CommandResult ChainKindHandler(CommandContext context, WireArgs args) {
        ...
        var hasTarget = (hasKind && (args.Count >= 2) && !SeatToken(token: args[1]));
    ...
    private CommandResult ChainRemoveHandler(CommandContext context, WireArgs args) {
        var hasTarget = ((args.Count >= 1) && !SeatToken(token: args[0]));
    ...
    private CommandResult ChainPointHandler(CommandContext context, in WireArgs args, string verb, bool isGoal) {
        ...
        if (!applied) {
            return Error(text: $"[{verb}: no chain '{idOrName}']");
        }
