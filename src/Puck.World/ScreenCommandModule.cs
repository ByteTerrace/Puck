...
internal sealed class ScreenCommandModule(WorldScreenBinder binder, WorldServer server, IServerLink link) : ICommandModule {
    ...
    private bool AllowsControl(WorldPrincipal principal, int index) =>
        m_server.Grants.Allows(principal: principal, capability: WorldCapability.Control, subject: GrantSubject.Screen(index: index));
    ...
    private CommandResult InsertHandler(CommandContext context, WireArgs args) {
        ...
        if (!AllowsControl(principal: principal, index: index)) {
            return Denied(principal: principal, verb: "screen.insert", index: index);
        }
        ...
        m_link.SubmitScreenOp(op: new WorldScreenOp.Insert(Index: index, ContentPath: contentPath, EngineId: engineId, Options: options), principal: principal);
    ...
    private CommandResult SourceHandler(CommandContext context, WireArgs args) {
        ...
        if (!AllowsControl(principal: principal, index: index)) {
            return Denied(principal: principal, verb: "screen.source", index: index);
        }

        if (args.Is(index: 1, value: "camera")) {
            return SourceCamera(index: index, principal: principal, args: in args);
        }
        if (args.Is(index: 1, value: "capture")) {
            return SourceCapture(index: index, principal: principal, args: in args);
        }
        if (args.Is(index: 1, value: "desktop")) {
            return SourceDesktop(index: index, principal: principal, args: in args);
        }
        if (args.Is(index: 1, value: "qr")) {
            return SourceQr(index: index, principal: principal, args: in args);
        }
        if (args.Is(index: 1, value: "view")) {
            return SourceView(index: index, principal: principal, args: in args);
        }
    ...
    private CommandResult SourceCamera(int index, WorldPrincipal principal, in WireArgs args) {
        ...
        EjectMachineFirst(index: index, principal: principal);

        var (ok, message) = m_binder.TryCamera(index: index);

        return (ok
            ? Success(args: in args, message: $"[screen.source: {message}]")
    ...
    private CommandResult SourceDesktop(int index, WorldPrincipal principal, in WireArgs args) {
        ...
        if ((args.Count == 3) && !args.TryInt(index: 2, value: out monitorIndex)) {
    ...
    private CommandResult SourceView(int index, WorldPrincipal principal, in WireArgs args) {
        ...
        var (ok, message) = m_binder.TryView(index: index, cameraName: args[2].ToString());
    ...
    private CommandResult SourceQr(int index, WorldPrincipal principal, in WireArgs args) {
        ...
        if (args.Count == 2) {
            return (m_binder.TryReadQr(index: index, authoring: out var authoring)
        ...
        if (args.Count == 5) {
            if (!args.TryInt(index: 4, value: out var quietZone)) {
        ...
        var (ok, message) = m_binder.TryQr(
            ...
            ecLevel: ((args.Count >= 4) ? args[3].ToString() : null),
    ...
    private CommandResult EjectHandler(CommandContext context, WireArgs args) {
        ...
        if (!AllowsControl(principal: principal, index: index)) {
            return Denied(principal: principal, verb: "screen.eject", index: index);
        }
    ...
    private CommandResult SelectHandler(CommandContext context, WireArgs args) {
        ...
        if (!AllowsControl(principal: principal, index: index)) {
            return Denied(principal: principal, verb: "screen.select", index: index);
        }
        ...
        if ((target < 0) || (target >= magazine.Entries.Count)) {
        ...
        m_link.SubmitScreenOp(op: new WorldScreenOp.Select(Index: index, Entry: target), principal: principal);

        if (magazine.Entries[target] is WorldScreenSource.Machine) {
            return CommandResult.None;
        }

        var (ok, message) = m_binder.ApplyNonMachineSource(index: index, source: magazine.Entries[target]);

        return new CommandResult(Output: $"[screen.select: {index} entry {target}/{magazine.Entries.Count} {(ok ? message : $"selected (presentation apply failed: {message})")}]") {
            IsError = !ok,
        };
    }
    private CommandResult OptionsHandler(CommandContext context, WireArgs args) {
        ...
        if (!AllowsControl(principal: principal, index: index)) {
            return Denied(principal: principal, verb: "screen.options", index: index);
        }
        ...
        m_link.SubmitScreenOp(op: new WorldScreenOp.SetOptions(Index: index, Options: optionsBuilder.ToString()), principal: principal);
    ...
    private CommandResult LinkHandler(CommandContext context, WireArgs args) {
        ...
        for (var token = 1; (token < args.Count); token++) {
            ...
            if (!AllowsControl(principal: principal, index: member)) {
                return Denied(principal: principal, verb: "screen.link", index: member);
            }

            members.Add(item: member);
        }

        m_link.SubmitScreenOp(op: new WorldScreenOp.Link(Name: name, Members: members), principal: principal);
    ...
    private CommandResult UnlinkHandler(CommandContext context, WireArgs args) {
        ...
        if (m_binder.TryReadLinkMembers(name: name, members: out var members)) {
            foreach (var member in members) {
                if (!AllowsControl(principal: principal, index: member)) {
                    return Denied(principal: principal, verb: "screen.unlink", index: member);
                }
            }
        }

        m_link.SubmitScreenOp(op: new WorldScreenOp.Unlink(Name: name), principal: principal);
    ...
    private CommandResult StateHandler(CommandContext context, WireArgs args) {
        ...
        var engagedText = ((engaged.Count > 0)
            ? string.Join(separator: "+", values: engaged.Select(selector: static entry => (entry.Capture ? $"p{entry.Display}" : $"p{entry.Display}(mirror)")))
            : "none");
