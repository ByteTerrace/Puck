...
internal sealed class WorldAddonCommandModule(WorldAddonRuntime runtime, WorldReplayTape tape, WorldServer server, IServerLink link) : ICommandModule {
    ...
    public IEnumerable<CommandDefinition> GetCommands() {
        yield return CommandDefinition.WithWireArgs(
            ...
            handler: (context, args) => Mount(context: context, args: args),
        ...
        yield return CommandDefinition.WithWireArgs(
            ...
            handler: (context, args) => Unmount(context: context, args: args),
            routing: CommandRouting.Simulation
        );
        yield return CommandDefinition.WithWireArgs(
            bindability: CommandBindability.Unbindable,
            ...
            handler: (context, args) => Reload(context: context, args: args),
            routing: CommandRouting.Simulation
        );
        yield return CommandDefinition.WithWireArgs(
            bindability: CommandBindability.Unbindable,
            ...
            handler: (context, args) => SetEnabled(context: context, args: args, enabled: true),
            routing: CommandRouting.Simulation
        );
        yield return CommandDefinition.WithWireArgs(
            bindability: CommandBindability.Unbindable,
            ...
            handler: (context, args) => SetEnabled(context: context, args: args, enabled: false),
            routing: CommandRouting.Simulation
        );
        yield return CommandDefinition.WithWireArgs(
            bindability: CommandBindability.Unbindable,
    ...
    private CommandResult Mount(CommandContext context, WireArgs args) {
        if ((args.Count < 4) || (((args.Count - 4) % 2) != 0)) {
            return Usage(verb: "world.addon.mount", form: "<name> <modulePath> <hash> <fuel> [<capability> <subject>]...");
        }

        if (!ulong.TryParse(s: args[3].ToString(), style: NumberStyles.Integer, provider: CultureInfo.InvariantCulture, result: out var fuel)) {
        ...
        for (var index = 4; (index < args.Count); index += 2) {
            if (!WorldGrantCommandModule.TryParseCapability(token: args[index], capability: out var capability)) {
                return CommandResult.Error(output: $"[world.addon.mount: unrecognized capability '{args[index]}']");
            }

            if (!WorldGrantCommandModule.TryParseSubject(token: args[(index + 1)], subject: out var subject)) {
        ...
        link.SubmitAddonLifecycle(
            lifecycle: new WorldAddonLifecycle.Mount(Name: args[0].ToString(), ModulePath: args[1].ToString(), Hash: args[2].ToString(), Fuel: fuel, Requests: requests),
    ...
    private CommandResult Unmount(CommandContext context, WireArgs args) {
        if (args.Count != 1) {
            return Usage(verb: "world.addon.unmount", form: "<name>");
        }

        link.SubmitAddonLifecycle(lifecycle: new WorldAddonLifecycle.Unmount(Name: args[0].ToString()), principal: context.ActingPrincipal());

        return CommandResult.None;
    }
    private CommandResult Reload(CommandContext context, WireArgs args) {
        if (args.Count != 1) {
            return Usage(verb: "world.addon.reload", form: "<name>");
        }

        if (CheckAuthority(context: context, verb: "world.addon.reload") is { } denial) {
            return denial;
        }

        var name = args[0].ToString();

        if (RefuseIfArmed(verb: "world.addon.reload", name: name) is { } refusal) {
            return refusal;
        }
    ...
    private CommandResult SetEnabled(CommandContext context, WireArgs args, bool enabled) {
        var verb = (enabled ? "world.addon.enable" : "world.addon.disable");

        if (args.Count != 1) {
            return Usage(verb: verb, form: "<name>");
        }

        if (CheckAuthority(context: context, verb: verb) is { } denial) {
            return denial;
        }

        var name = args[0].ToString();

        if (RefuseIfArmed(verb: verb, name: name) is { } refusal) {
            return refusal;
        }

        return new CommandResult(Output: $"[{verb}: {runtime.SetEnabled(name: name, enabled: enabled)}]");
    }
    ...
    private CommandResult? CheckAuthority(CommandContext context, string verb) {
        ...
        if (server.Grants.Allows(principal: actingPrincipal, capability: WorldCapability.Mutate, subject: GrantSubject.Section(section: WorldSection.Addons)) is { IsAllowed: false } verdict) {
    ...
    private CommandResult Describe(WireArgs args) {
        if (args.Count > 0) {
            return Usage(verb: "world.addons", form: "");
        }
    ...
    private static CommandResult Usage(string verb, string form) {
        return CommandResult.Error(output: (string.IsNullOrEmpty(value: form) ? $"[{verb}: expected no arguments]" : $"[{verb}: expected {form}]"));
    }
}
