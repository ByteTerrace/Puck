...
internal sealed class WorldWheelFeed : IWorldWheelConsumer {
    ...
    /// <param name="clock">The engine-tick capture clock the selection-grace window is counted on. Pass the
    /// process's one <see cref="IInputClock"/> so the window shares the base every input timestamp already uses;
    /// omitted, the feed starts a private monotonic clock of its own.</param>
    /// <exception cref="ArgumentNullException">An argument is <see langword="null"/>.</exception>
    public WorldWheelFeed(WorldPointer pointer, PlayerRoster roster, WorldSeatBindings bindings, WorldCursorFeed cursor, WorldSeatViewports viewports, WheelStore store, Func<InputRouter> router, IInputClock? clock = null) {
        ...
        m_bindings = bindings;
        m_clock = (clock ?? InputClock.Start());
        m_cursor = cursor;
        m_pointer = pointer;
        m_roster = roster;
        m_router = router;
