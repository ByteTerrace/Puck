...
internal sealed class EditorSelectionCommandModule(WorldEditorSession session, WorldEditorTargeting targeting, WorldEditorDrag drag, WorldClient client, IServerLink link) : ICommandModule {
    ...
    private CommandResult DeleteHandler(CommandContext context, WireArgs args) {
        ...
        switch (selection.Section) {
            case WorldSection.Screens:
                m_link.SubmitWorldMutation(mutation: new WorldMutation.RemoveScreen(
                    Principal: principal,
            ...
            case WorldSection.Cameras:
                m_link.SubmitWorldMutation(mutation: new WorldMutation.RemoveCamera(
                    Principal: principal,
                    Name: selection.Id
                ));
            ...
            case WorldSection.Placements:
                m_link.SubmitWorldMutation(mutation: new WorldMutation.RemovePlacement(
                    Principal: principal,
                    Id: selection.Id
                ));
            ...
            case WorldSection.Speakers:
                m_link.SubmitWorldMutation(mutation: new WorldMutation.RemoveSpeaker(
                    Principal: principal,
                    Name: selection.Id
                ));
            ...
            case WorldSection.Spawns: {
                    ...
                    m_link.SubmitWorldMutation(mutation: new WorldMutation.SetSpawns(
                        Principal: principal,
                        Spawns: spawns
                    ));
        ...
        return EditorSculptCommandModule.Echo(
            slot: slot,
            verb: DeleteCommand,
            detail: $"{selection.Describe()} — remove submitted"
        );
    }
    ...
    private CommandResult DirectiveSelectHandler(CommandContext context, in WireArgs args) {
        ...
        if (args.Is(
            ...
            return EditorSculptCommandModule.Echo(
                slot: slot,
                verb: "editor.select",
                detail: (m_targeting.Deselect(slot: slot)
                ? "cleared"
                : "nothing selected")
            );
        }
    ...
    private CommandResult DragHandler(CommandContext context, WireArgs args) {
        ...
        return EditorSculptCommandModule.Echo(
            slot: slot,
            verb: "editor.drag",
            detail: (m_drag.Describe(slot: slot) ?? "moved")
        );
    }
    private CommandResult GrabHandler(CommandContext context, WireArgs args) {
        ...
        return EditorSculptCommandModule.Echo(
            slot: slot,
            verb: GrabCommand,
            detail: $"dragging {selection.Describe()} — sticks move it, grab again commits, {CancelCommand} aborts"
        );
    }
    ...
    private CommandResult PickHandler(CommandContext context, WireArgs args) {
        ...
        if (!m_targeting.TryPick(
            selection: out var selection,
            slot: slot
        )) {
            return EditorSculptCommandModule.Echo(
                detail: "nothing under the crosshair",
                slot: slot,
                verb: PickCommand
            );
        }
        ...
        return EditorSculptCommandModule.Echo(
            slot: slot,
            verb: PickCommand,
            detail: $"selected {DescribeSelection(
                selection: in selection,
                slot: slot
            )}"
        );
    ...
    private CommandResult PlaceHandler(CommandContext context, WireArgs args) {
        ...
        return PlaceCreation(
            ...
            slot: slot,
            args: args,
            focus: m_session.Focus(slot: slot)
        );
    }
    ...
    private CommandResult SnapHandler(CommandContext context, WireArgs args) {
        ...
        return EditorSculptCommandModule.Echo(
            ...
            detail: string.Create(
                provider: CultureInfo.InvariantCulture,
                handler: $"{(snap.Enabled
            ? "on"
            : "off")} pitch={snap.Pitch.X:0.##}"
            )
        );
    }
        ...
        switch (selection.Section) {
                ...
                foreach (var screen in definition.Screens) {
                    if (screen.Index == selection.Index) {
                        m_link.SubmitWorldMutation(mutation: new WorldMutation.UpsertScreen(
                            Principal: principal,
                            Screen: (screen with { Origin = target })
                        ));
            ...
            case WorldSection.Cameras:
                foreach (var camera in definition.Cameras) {
                    ...
                    var movedMotion = WorldCameraRigCompiler.Move(
                        motion: camera.Rig.Motion,
                        value: value,
                        relative: false
                    );
                    ...
                    switch (speaker) {
                        case WorldSpeaker.Fixed fixedSpeaker:
                            m_link.SubmitWorldMutation(mutation: new WorldMutation.UpsertSpeaker(
                                Principal: principal,
                                Speaker: (fixedSpeaker with { Position = target })
                            ));
                        ...
                        case WorldSpeaker.Bed bed:
                            m_link.SubmitWorldMutation(mutation: new WorldMutation.UpsertSpeaker(
                                Principal: principal,
                                Speaker: (bed with { Center = target })
                            ));
                        ...
                        case WorldSpeaker.Anchored anchored:
                            m_link.SubmitWorldMutation(mutation: new WorldMutation.UpsertSpeaker(
                                Principal: principal,
                                Speaker: (anchored with { Offset = target })
                            ));
