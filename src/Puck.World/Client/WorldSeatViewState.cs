...
internal sealed class WorldSeatViewState {
    ...
    public void Nudge(Vector2 input, float yawScale, float pitchScale, WorldSeatLook preference, WorldSeatViewControl control) {
        ...
        lock (m_gate) {
            m_yaw = Wrap(m_yaw - ((input.X * yawScale) * (preference.InvertYaw ? -1f : 1f)));
            m_pitch = Math.Clamp(
                value: m_pitch - ((input.Y * pitchScale) * (preference.InvertPitch ? -1f : 1f)),
    ...
    public void Reclamp(WorldSeatViewControl control) {
        ArgumentNullException.ThrowIfNull(argument: control);
        lock (m_gate) {
            m_pitch = Math.Clamp(value: m_pitch, min: control.MinPitch, max: control.MaxPitch);
        }
    }
    ...
    public float LogicalYaw(WorldViewDefaults views, Quaternion bodyOrientation) {
        ...
        lock (m_gate) {
            return Wrap(authoredYaw + m_yaw + bodyYaw);
        }
    }

    public ISdfCameraRig ResolveChase(WorldViewDefaults views, Quaternion bodyOrientation) {
        if (!ReferenceEquals(objA: m_authoredRig, objB: views.SeatRig)) {
        ...
        float pitch;
        lock (m_gate) {
            m_pitch = Math.Clamp(value: m_pitch, min: views.SeatControl.MinPitch, max: views.SeatControl.MaxPitch);
            yaw = m_yaw;
            pitch = m_pitch;
        }
    ...
    public void Smooth(float rate, bool enabled, float deltaSeconds, ref Vector3 eye, ref Vector3 target) {
        lock (m_gate) {
            WorldSeatCameraResolver.Smooth(state: m_smoothing, smoothRate: rate, isPlainChase: enabled,
