...
/// <see cref="ISdfSceneEmitter"/>/<see cref="ISdfFrameDresser"/> split <see cref="Client.WorldFrameSource"/> and
/// <see cref="Client.WorldSceneEmitter"/> already establish, collapsed into one type here because a session
...
/// <see cref="Client.WorldSceneEmitter"/> calls for the boot world's own decoration placements — and
...
/// into) — <see cref="WorldScreenBinder"/> still narrates the depth-1 policy by name when it detects that shape, so
