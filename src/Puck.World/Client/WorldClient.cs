...
internal sealed class WorldClient : IClientSink, ISdfAnchorSource {
    ...
    private PlayerIntent ComposeMoveFrame(int slot, int bodyIndex, WorldAuthorityEndpoint endpoint, WorldDefinition definition, PlayerIntent intent) {
        ...
        var rawForwardValue = roles.Read(
            intent: in intent,
            role: ChannelRole.MoveForward
        );
        var rawStrafeValue = roles.Read(
            intent: in intent,
            role: ChannelRole.MoveStrafe
        );

        if (
            ((rawForwardValue == FixedQ4816.Zero) && (rawStrafeValue == FixedQ4816.Zero)) ||
        ...
        var yaw = (m_roster.Seat(slot: slot)?.View.LogicalYaw(
        ...
        if (kit.Motion is WorldMotionModel.Swim) {
            ...
            if (MathF.Abs(x: elevation) >= 1e-6f) {
                var rawUp = ((float)((double)roles.Read(
                    intent: in intent,
                ...
                intent = roles.Write(
                    ...
                    value: FixedQ4816.Clamp(
                        value: FixedQ4816.FromDouble(value: composedUp),
        ...
        intent = roles.Write(
            ...
            role: ChannelRole.MoveForward,
            value: FixedQ4816.Clamp(
                value: FixedQ4816.FromDouble(value: moveForward),
                minimum: negativeOne,
                maximum: FixedQ4816.One
            )
        );
        ...
        return roles.Write(
            ...
            role: ChannelRole.MoveStrafe,
            value: FixedQ4816.Clamp(
                value: FixedQ4816.FromDouble(value: moveStrafe),
                minimum: negativeOne,
                maximum: FixedQ4816.One
            )
        );
