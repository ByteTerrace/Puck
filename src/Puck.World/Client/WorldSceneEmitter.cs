...
internal sealed class WorldSceneEmitter : ISdfSceneEmitter {
    ...
    public WorldSceneEmitter(WorldClient client, WorldRenderSettings settings, WorldEditorTargeting targeting, WorldEditorDrag drag, WorldWorkbench workbench, WorldStampPool animator, WorldAudioDirector audio, WorldPerceptionAnchor anchor) {
        ...
        m_placementReservation = (WorldPlacementStamper.StaticStampInstances(creations: definition.Creations, placements: definition.Placements) + m_authoringHeadroomPlacements);
        // The boot placements + speakers are the shimmer baseline — the first delivery pulses only what it changed.
        m_shimmer.Observe(placements: definition.Placements, speakers: definition.Speakers, now: 0d);
    }
    ...
    public void PackDynamicTransforms(Span<DynamicTransform> slots, in SdfEmitContext context) {
        ...
        m_animator.PackTransforms(transforms: slots, client: m_client, slotBase: (context.SlotBase + WorldAvatarCatalog.DynamicTransformCapacity));
    }
    ...
    private void Compose(SdfProgramBuilder builder, IReadOnlyList<WorldScreen> screens, IReadOnlyList<WorldScreen> derivedFaces, IReadOnlyList<WorldPlacement> placements, IReadOnlyList<WorldCreation> creations, bool probeWorstCase, bool placementProbe, WorldEditorTargeting? highlight, float maxPlacementScale, double shimmerNow, int slotBase) {
        ...
        if (placementProbe || probeWorstCase) {
            var candidateInstances = WorldPlacementStamper.StaticStampInstances(creations: creations, placements: placements);

            WorldPlacementStamper.EmitProbe(builder: builder, reservedCount: Math.Max(val1: candidateInstances, val2: m_placementReservation));
        } else {
        ...
        m_animator.Emit(builder: builder, probeWorstCase: probeWorstCase, maxPlacementScale: maxPlacementScale, slotBase: (slotBase + WorldAvatarCatalog.DynamicTransformCapacity));
    ...
    private WorldStampPool.BodyStamp? ResolveStampCreation(int index, WorldDefinition definition) {
        ...
        if (m_client.PlacementId(index: index) is { } placementId) {
            ...
            return ((WorldDefinitionRows.FindCreation(creations: definition.Creations, id: creationId) is { } creation)
        ...
        if (look.Source is WorldLookSource.Creation crowdLook) {
            return ((WorldDefinitionRows.FindCreation(creations: definition.Creations, id: crowdLook.CreationId) is { } creation)
