...
internal sealed class WorldSceneEmitter : ISdfSceneEmitter {
    ...
    public WorldSceneEmitter(WorldClient client, WorldRenderSettings settings, WorldEditorTargeting targeting, WorldEditorDrag drag, WorldWorkbench workbench, WorldStampPool animator, WorldAudioDirector audio, WorldPerceptionAnchor anchor, WorldContinuum continuum, WorldTextCatalog text) {
    ...
    private bool TryTravelingRoute(int index, out WorldAuthorityRoute route) {
        if (
            (((uint)index) < WorldPopulation.LocalSeatCount) &&
    ...
    public void PackDynamicTransforms(Span<DynamicTransform> slots, in SdfEmitContext context) {
        ...
        for (var index = 0; (index < WorldAvatarCatalog.Capacity); index++) {
            ...
            var castsSoftShadow = ((index < WorldPopulation.LocalSeatCount) || WithinCrowd(
                position: position,
            ...
            if (
                ...
                if (
                    (index < WorldPopulation.LocalSeatCount) &&
