...
internal sealed class WorldStampPool {
    ...
    private static void EmitOne(SdfProgramBuilder builder, Registration? live, bool probeWorstCase, int rootSlot, float maxPlacementScale, PackedFontAtlasCatalog? textCatalog) {
        ...
        for (var index = 0; (index < WorldPlacementPolicy.MaxAnimatedStampShapes); index++) {
            ...
            _ = builder.BeginInstanceDynamic(
                slot: slot,
                boundOffset: Vector3.Zero,
                boundRadius: (InstanceRadiusUnitScale * MaxComponent(scale: scale)),
                active: active
            );
            EmitShape(
                bend: (placed?.Bend ?? 0f),
    ...
    private static Dictionary<int, FrameTransformDocument>? FramePoses(Registration live, int frameCursor) {
        if (
            (frameCursor <= 0) ||
            (live.Creation.Document.Frames is not { Count: > 0 } frames) ||
    ...
    /// them.</param>
    public void Emit(SdfProgramBuilder builder, bool probeWorstCase, float maxPlacementScale, int slotBase, PackedFontAtlasCatalog? textCatalog = null) {
        for (var index = 0; (index < m_pool.Length); index++) {
    ...
    public bool TryBodyPartAuthoredPose(int bodyIndex, string partId, WorldClient client, out SdfAnchor pose) {
        if (
            ...
            (live.Creation.Document.Shapes is not { } shapes) ||
