...
internal sealed class WorldStateCommandModule(WorldServer server, IServerLink link) : ICommandModule {
    ...
    private WorldStateRow? FindRow(string name) => WorldDefinitionRows.FindStateRow(
    ...
    private CommandResult HandleCellSet(CommandContext context, WireArgs args) {
        ...
        if (
            (FindRow(name: rowName) is { Kind: CellKind.Text }) &&
            (args.Count >= 3)
        ) {
    ...
    public IEnumerable<CommandDefinition> GetCommands() {
        yield return CommandDefinition.WithWireArgs(
            ...
            handler: (context, args) => HandleCellSet(
                args: args,
                context: context
            ),
