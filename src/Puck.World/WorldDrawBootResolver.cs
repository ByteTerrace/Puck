...
internal static class WorldDrawBootResolver {
    ...
    public static bool TryResolve(WorldDefinition definition, string instanceIdentity, out WorldDefinition resolved, out string reason) {
        ...
        if (population.CapacityDraw is { } capacityDraw) {
            if (!TryDrawSite(definition: definition, worldSeed: worldSeed, instanceIdentity: instanceIdentity, site: WorldDrawSites.PopulationCapacity, draw: capacityDraw, targetKind: CellKind.Int, fired: out var fired, reason: out reason)) {
                return false;
            }

            var drawn = fired.Numeric!.Value;

            if ((drawn < int.MinValue) || (drawn > int.MaxValue)) {
                reason = $"{WorldDrawSites.PopulationCapacity} drew {drawn}, which does not fit population.capacity's int32 storage";

                return false;
            }

            Narrate(site: WorldDrawSites.PopulationCapacity, instanceIdentity: instanceIdentity, settled: drawn.ToString(provider: System.Globalization.CultureInfo.InvariantCulture));
        ...
        if (host.BackendDraw is { } backendDraw) {
            if (!TryDrawSite(definition: definition, worldSeed: worldSeed, instanceIdentity: instanceIdentity, site: WorldDrawSites.HostBackend, draw: backendDraw, targetKind: CellKind.Text, fired: out var fired, reason: out reason)) {
                return false;
            }
            ...
            Narrate(site: WorldDrawSites.HostBackend, instanceIdentity: instanceIdentity, settled: WorldHostTokens.BackendToken(backend: backend));
        ...
        foreach (var row in definition.State) {
            ...
            if ((row.Draw is not { } draw) || (row.Cells is { Count: > 0 })) {
                state.Add(item: row);

                continue;
            }

            if (!TryDrawSite(definition: definition, worldSeed: worldSeed, instanceIdentity: instanceIdentity, site: WorldDrawSites.StateRow(rowName: row.Name), draw: draw, targetKind: row.Kind, fired: out var fired, reason: out reason, cursor: row.DrawCursor, decks: row.DrawDecks)) {
                return false;
            }
    ...
    private static bool TryDrawSite(WorldDefinition definition, ulong worldSeed, string instanceIdentity, string site, WorldDraw draw, CellKind targetKind, out WorldGeneratorEngine.FireResult fired, out string reason, long cursor = 0L, IReadOnlyList<long>? decks = null) {
        ...
        if (!WorldGeneratorEngine.TryFire(
                ...
                seedState: WorldGeneratorEngine.ComputeSeedState(worldSeed: worldSeed, instanceIdentity: instanceIdentity, site: site),
