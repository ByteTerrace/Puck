...
internal sealed class WorldHudCommandModule(WorldServer server, IHudBindingResolver bindings, PlayerRoster roster) : ICommandModule {
    ...
    public IEnumerable<CommandDefinition> GetCommands() {
        ...
        yield return CommandDefinition.WithWireArgs(
            ...
            handler: (context, args) => {
                var template = WorldCommandArguments.Raw(context: context, args: in args);

                return ((template.Length == 0) ? Usage(verb: "world.hud.template", form: "<template text...>") : ResolveDynamicTemplate(template: template));
            },
            routing: CommandRouting.Immediate
        );
    }
    ...
    private CommandResult ResolveDynamicTemplate(string template) {
        if (!HudTemplate.TryParse(template: template, segments: out var segments, error: out var parseError)) {
        ...
        foreach (var segment in segments) {
            ...
            if (!HudBindingVocabulary.TryParse(token: segment.Text, binding: out var parsed)) {
            ...
            if (!TryFindStateRow(name: parsed.StateName!, row: out var row)) {
    ...
    private string ResolveSegments(IReadOnlyList<HudTemplateSegment> segments) {
        ...
        foreach (var segment in segments) {
            ...
            if (bindings.TryResolve(binding: segment.Text, fraction: out _, text: out var value)) {
    ...
    private bool TryFindStateRow(string name, [NotNullWhen(true)] out WorldStateRow? row) {
        row = WorldDefinitionRows.FindStateRow(rows: server.Definition.State, name: name);
    ...
    private CommandResult DescribeHudHandler(WireArgs args) {
        ...
        if ((args.Count == 1) && TryParseSeatFilter(token: args[0].ToString(), seat: out var seat)) {
    ...
    private static bool TryParseSeatFilter(string token, out int seat) {
        ...
        if (!token.StartsWith(value: SeatFilterPrefix, comparisonType: StringComparison.OrdinalIgnoreCase)) {
            return false;
        }

        return (int.TryParse(s: token.AsSpan(start: SeatFilterPrefix.Length), style: System.Globalization.NumberStyles.Integer, provider: System.Globalization.CultureInfo.InvariantCulture, result: out seat)
            && (seat >= 1) && (seat <= PlayerRoster.MaxSlots));
    }
    private CommandResult DescribeSeatHud(int seat) {
        ...
        foreach (var element in panel.Elements) {
            lines.Add(item: DescribeElement(panelId: panel.Id, element: element));
        }
    ...
    private string DescribeHud() {
        ...
        var lines = new List<string>(capacity: (1 + section.Panels.Count)) {
            string.Create(
                ...
                handler: $"[world.hud: enabled {(section.Defaults.Enabled ? "true" : "false")} cursorHoverRadius={cursor.HoverRadius:0.##} cursorSizePx={cursor.SizePx:0.##} cursorRole={cursor.Role.ToString().ToLowerInvariant()} panels {section.Panels.Count}/{WorldHudCapacity.MaxWorldPanels}]"
            ),
        };
    ...
    private string DescribeElement(string panelId, WorldHudElement element) {
        ...
        } else if ((element.Binding is { Length: > 0 } binding) && bindings.TryResolve(binding: binding, fraction: out var fraction, text: out var text)) {
    ...
    private string ResolveTemplateEcho(string template) {
        return (HudTemplate.TryParse(template: template, segments: out var segments, error: out _) ? ResolveSegments(segments: segments) : template);
    }
