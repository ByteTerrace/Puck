...
internal sealed class WorldBindingCommandModule(PlayerRoster roster, WorldSeatBindings seatBindings, IServerLink link, Func<InputRouter> router, Func<CommandRegistry> registry, IInputClock clock, WorldDefinition definition, WorldOwnedWorlds ownedWorlds) : ICommandModule {
    ...
    private CommandResult BindHandler(CommandContext context, WireArgs args) {
        if (
            ...
            if (
                ...
                args: in args,
                index: 3,
                prefix: ScaleTokenPrefix
            ) || IsToken(
                args: in args,
                index: 3,
                prefix: ValueTokenPrefix
            )) &&
                (IsToken(
                args: in args,
                index: 4,
        ...
        if (destination.StartsWith(
            ...
            value: ChannelDestinationPrefix
        )) {
        ...
        if (args.Count == 4) {
            ...
            var isScale = token.StartsWith(
                comparisonType: StringComparison.OrdinalIgnoreCase,
                value: ScaleTokenPrefix
            );
            var isValue = token.StartsWith(
                comparisonType: StringComparison.OrdinalIgnoreCase,
                value: ValueTokenPrefix
            );
            ...
            if (isScale) {
                ...
                if (!m_registry().TryGetMetadata(
                    metadata: out var destinationMetadata,
                    name: command
                )) {
        ...
        var destinationLabel = ((channel is not null)
            ? ChannelLabel(channel: channel)
            : FormatCommandDestination(
                command: command!,
                value: constantValue
            )
        );
        ...
        WorldAffordances.Validate(
            document: probe,
            channels: m_seatBindings.Channels(slot: slot),
            seatModes: m_seatBindings.SeatModes(slot: slot),
        ...
        if (!m_seatBindings.TryValidateSessionRebind(
        ...
        m_seatBindings.SetSessionRebind(
            rebinds: rebind,
            slot: slot
        );
    ...
    private CommandResult BindingsHandler(CommandContext context, WireArgs args) {
        ...
        foreach (var family in derivation.Families) {
            ...
            _ = ((family.Group is { } matchedGroup)
                ? builder.Append(value: matchedGroup).Append(value: (family.Wins
                    ? " (wins)"
                    : " (shadowed)"))
        ...
        foreach (var row in (document.Chords ?? [])) {
            ...
            foreach (var entry in entries) {
                ...
                var destination = ((entry.Channel is { } channel)
                    ? ChannelLabel(channel: channel)
                    : FormatCommandDestination(
                ...
                if (seen.Add(item: $"{trigger}\0{destination}")) {
                    _ = builder.Append(value: (any
                        ? " | "
                        : " ")).Append(value: trigger).Append(value: "→").Append(value: destination);
    ...
    private string DescribeAffordances() {
        ...
        foreach (var channel in m_channels) {
            _ = builder.Append(value: ((channelCount > 0)
                ...
                .Append(value: "\",\"consumer\":\"").Append(value: ((channel.Role is { } role)
    ...
    private void RefreshSeatsBoundTo(string profileId) {
        for (var slot = 0; (slot < PlayerRoster.MaxSlots); slot++) {
            if (
                (m_roster.ProfileAt(slot: slot) is { } profile) &&
                string.Equals(
                a: profile.Id,
                b: profileId,
                comparisonType: StringComparison.Ordinal
    ...
    private CommandResult SaveHandler(CommandContext context, WireArgs args) {
        ...
        var merged = WorldBindingComposer.Compose(
            profile.Bindings,
            session
        );
    ...
    private CommandResult SignalHandler(CommandContext context, WireArgs args) {
        ...
        var describedSample = ((args.Count == 3)
            ...
            : args[1].ToString().ToLowerInvariant()
        );
