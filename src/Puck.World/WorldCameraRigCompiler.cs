...
internal static class WorldCameraRigCompiler {
    ...
    public static ISdfCameraRig Compile(WorldCameraRig rig, Vector3 referenceOffset = default, float spread = 0f) {
        ...
        return new ComposedWorldCameraRig(rig: rig, referenceOffset: referenceOffset, spread: spread);
    }
    ...
    public static Vector3 AuthoredPosition(WorldCameraMotion motion) => motion switch {
        ...
        WorldCameraMotion.Orbit value => (value.PivotOffset + OrbitRig.Offset(yaw: value.Yaw, pitch: value.Pitch, distance: value.Distance)),
    ...
    public static WorldCameraMotion Move(WorldCameraMotion motion, Vector3 value, bool relative) {
        ...
        return motion switch {
            ...
            _ => throw new ArgumentOutOfRangeException(paramName: nameof(motion), actualValue: motion, message: "unknown camera motion kind."),
        };
    }
    ...
    private sealed class ComposedWorldCameraRig(WorldCameraRig rig, Vector3 referenceOffset, float spread) : ISdfCameraRig {
        public (Vector3 Eye, Vector3 Target, float FovRadians) Resolve(in SdfAnchor anchor, in SdfCameraClock clock) {
            var referencePosition = (anchor.Position + Vector3.Transform(value: referenceOffset, rotation: anchor.Orientation));
            var eye = rig.Motion switch {
                WorldCameraMotion.Follow follow => ResolveFollow(referencePosition: referencePosition, orientation: anchor.Orientation, follow: follow, spread: spread),
                WorldCameraMotion.Orbit orbit => ResolveOrbit(referencePosition: referencePosition, orbit: orbit),
                WorldCameraMotion.Static value => ResolvePosition(referencePosition: referencePosition, orientation: anchor.Orientation, value: value.Position, worldAxes: value.WorldAxes),
                WorldCameraMotion.Track track => ResolvePosition(referencePosition: referencePosition, orientation: anchor.Orientation, value: EvaluateTrack(track: track, clock: in clock), worldAxes: track.WorldAxes),
                var motion => throw new ArgumentOutOfRangeException(paramName: nameof(rig), actualValue: motion, message: $"unknown camera motion kind '{(motion?.GetType().Name ?? "<null>")}'."),
            };
            var target = rig.Aim switch {
                WorldCameraAim.Anchor value => ResolveOffset(referencePosition: referencePosition, orientation: anchor.Orientation, value: value.Offset, worldAxes: value.WorldAxes),
                WorldCameraAim.Forward value => (eye + (Vector3.Transform(value: -Vector3.UnitZ, rotation: anchor.Orientation) * MathF.Max(x: value.FocusDistance, y: 0.01f))),
                WorldCameraAim.WorldPoint value => value.Target,
                var aim => throw new ArgumentOutOfRangeException(paramName: nameof(rig), actualValue: aim, message: $"unknown camera aim kind '{(aim?.GetType().Name ?? "<null>")}'."),
            };
        ...
        private static Vector3 ResolveFollow(Vector3 referencePosition, Quaternion orientation, WorldCameraMotion.Follow follow, float spread) {
            var offset = (follow.Offset * (1f + (follow.SpreadPullback * MathF.Max(x: spread, y: 0f))));

            return ResolveOffset(referencePosition: referencePosition, orientation: orientation, value: offset, worldAxes: follow.WorldAxes);
        }
        private static Vector3 ResolveOrbit(Vector3 referencePosition, WorldCameraMotion.Orbit orbit) {
            ...
            return (pivot + OrbitRig.Offset(yaw: orbit.Yaw, pitch: orbit.Pitch, distance: orbit.Distance));
        }
        private static Vector3 ResolvePosition(Vector3 referencePosition, Quaternion orientation, Vector3 value, bool worldAxes) =>
            (worldAxes ? value : (referencePosition + Vector3.Transform(value: value, rotation: orientation)));
        private static Vector3 ResolveOffset(Vector3 referencePosition, Quaternion orientation, Vector3 value, bool worldAxes) =>
            (referencePosition + (worldAxes ? value : Vector3.Transform(value: value, rotation: orientation)));
        private static Vector3 EvaluateTrack(WorldCameraMotion.Track track, in SdfCameraClock clock) {
            ...
            var elapsed = Math.Max(val1: 0.0, val2: (clockTick - track.Playback.StartTick));
            ...
            switch (track.Playback.LoopMode) {
                case WorldCameraTrackLoopMode.Once:
                    localTick = Math.Min(val1: localTick, val2: last.Tick);
                    break;
                ...
                case WorldCameraTrackLoopMode.PingPong:
                    ...
                    localTick = (first.Tick + ((phase <= duration) ? phase : ((duration * 2.0) - phase)));
                    break;
                default:
                    throw new ArgumentOutOfRangeException(paramName: nameof(track), actualValue: track.Playback.LoopMode, message: "unknown camera track loop mode.");
            }
            ...
            for (var index = 1; (index < keyframes.Count); index++) {
                ...
                return Vector3.Lerp(value1: left.Position, value2: right.Position, amount: amount);
            }
