...
internal sealed class WorldRowCommandModule(WorldServer server, IServerLink link) : ICommandModule {
    ...
    private static IReadOnlyDictionary<string, RowSection> BuildSections(WorldServer server) => new Dictionary<string, RowSection>(comparer: StringComparer.Ordinal) {
        ...
        ["inputHold"] = new RowSection(Upsert: Upsert(info: WorldJsonContext.Default.WorldInputHoldAuthoring, toMutation: (principal, authoring) => new WorldMutation.SetInputHold(Principal: principal, Settings: authoring.Compile(ratePerSecond: (uint)server.Definition.SimulationRateHz))), Remove: null),
        ...
        ["views.seatControl"] = new RowSection(Upsert: Upsert(info: WorldJsonContext.Default.WorldSeatViewControl, toMutation: (principal, control) => new WorldMutation.SetViewDefaults(Principal: principal, Views: (server.Definition.Views with { SeatControl = control }))), Remove: null),
