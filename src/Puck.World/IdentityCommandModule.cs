...
internal sealed class IdentityCommandModule(WorldOwnedWorlds worlds, PlayerRoster roster, WorldServer server) : ICommandModule {
    ...
    public IEnumerable<CommandDefinition> GetCommands() {
        ...
        yield return CommandDefinition.WithWireArgs(
            ...
            handler: (context, args) => Deliver(context: context, args: args),
    ...
    private CommandResult Create(CommandContext context, WireArgs args) {
        ...
        var color = ((args.Count == 2) ? args[1].ToString() : m_worlds.Defaults.NeutralColor);

        if (!WorldSafeName.TryParse(candidate: id, name: out var safeId, reason: out var nameReason)) {
            return CommandResult.Error(output: $"[identity.create: refused — '{id}' {nameReason}]");
        }

        var identity = m_worlds.Create(name: safeId, colorHex: color, reason: out var reason);
    ...
    private CommandResult Show(CommandContext context, WireArgs args) {
        if (!TryPlayer(args: in args, optionalAt: 0, player: out var player, identity: out var identity, error: out var error)) {
    ...
    private CommandResult SetMotion(CommandContext context, WireArgs args) {
        ...
        } else if (string.Equals(a: key, b: "turn-speed", comparisonType: StringComparison.OrdinalIgnoreCase)) {
    ...
    private CommandResult SetHud(CommandContext context, WireArgs args) {
        ...
        if (!TryPlayer(args: in args, optionalAt: 1, player: out var player, identity: out var identity, error: out var error)) {
            return CommandResult.Error(output: error);
        }
        ...
        var raw = RawPanelJson(context: context, args: in args);

        if (!WorldJsonPayload.TryParse(json: raw, info: WorldJsonContext.Default.WorldHudPanel, value: out var panel, error: out var parseError)) {
    ...
    private static string RawPanelJson(CommandContext context, in WireArgs args) {
        ...
        return ((lastSeparator < 0) ? trimmed : trimmed[..lastSeparator].TrimEnd());
    }
    ...
    private static string DescribeHudSummary(WorldIdentity identity) =>
        ((identity.Hud is { } panel)
            ? string.Create(provider: CultureInfo.InvariantCulture, handler: $"{panel.Id}:{panel.Layer.ToString().ToLowerInvariant()}:{panel.Style.ToString().ToLowerInvariant()}:elements={panel.Elements.Count}/{WorldHudCapacity.MaxElementsPerSeatPanel}")
            : "none");
    ...
    private CommandResult Deliver(CommandContext context, WireArgs args) {
        ...
        if (string.Equals(a: opToken, b: "set", comparisonType: StringComparison.OrdinalIgnoreCase)) {
            kind = WorldDocumentWriteKind.Set;
        } else if (string.Equals(a: opToken, b: "add", comparisonType: StringComparison.OrdinalIgnoreCase)) {
        ...
        var text = DeliverTextTail(context: context, args: in args);
        ...
        return new CommandResult(Output: $"[identity.deliver: source=world:{sourceId} owner=world:{ownerId} row={rowName} op={opToken.ToLowerInvariant()} verdict={(receipt.Accepted ? "accepted" : "refused")} reason={receipt.Reason}]");
    }
    ...
    private static string DeliverTextTail(CommandContext context, in WireArgs args) =>
        WorldCommandArguments.RawAfter(context: context, args: in args, tokens: 5);
    ...
    private CommandResult DescribeIdentityState(WireArgs args) {
        ...
        if (!identity.TryReadState(name: rowName, row: out var row) || (row is null)) {
        ...
        var value = ((row.Kind == CellKind.Text) ? $"'{cell.Text}'" : cell.Value.ToString(provider: CultureInfo.InvariantCulture));

        return new CommandResult(Output: $"[identity.state: {id}.{rowName} kind={row.Kind.ToString().ToLowerInvariant()} value={value}]");
    }
    private string Describe() => $"[identity.list: {string.Join(separator: ", ", values: m_worlds.All.Select(selector: identity => $"{identity.Id}:{identity.Name}:{identity.ColorHex}"))} root={m_worlds.FilePath}]";
    private string DescribeWriteback() {
        ...
        var operand = ((submission.Text is { } text) ? $"'{text}'" : submission.Value.ToString(provider: CultureInfo.InvariantCulture));

        return $"[identity.writebacks: source=world:{submission.SourceDocumentId} owner=world:{submission.OwnerDocumentId} tick={submission.Tick} slot={submission.Slot} operation={submission.Kind.ToString().ToLowerInvariant()} value={operand} verdict={(receipt.Accepted ? "accepted" : "refused")} reason={receipt.Reason}]";
    }
    private static bool TryBool(ReadOnlySpan<char> token, out bool value) {
        value = (token.Equals(other: "on", comparisonType: StringComparison.OrdinalIgnoreCase) || token.Equals(other: "true", comparisonType: StringComparison.OrdinalIgnoreCase));
        return (value || token.Equals(other: "off", comparisonType: StringComparison.OrdinalIgnoreCase) || token.Equals(other: "false", comparisonType: StringComparison.OrdinalIgnoreCase));
    }
}
