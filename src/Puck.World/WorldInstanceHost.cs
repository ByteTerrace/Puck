...
internal sealed class WorldInstanceHost : IDisposable, IWorldTransferForwarder {
    ...
    private void ApplyTransfer(in PendingTransfer transfer) {
        ...
        if (!TryResolveTransferAuthority(
        ...
        // The wire-facing federation identity is this process's own authenticated namespace — the value
        ...
        var sourceAuthority = Boot.Server.AuthorityIdentity;
    ...
    private void FinalizeCommittedTransfer(in PendingTransfer transfer, TransferAuthority targetAuthority, string targetName, bool spawned, List<LandedMember> landed, int memberCount) {
        ...
        foreach (var member in landed) {
            ...
            for (var followedSlot = 0; (followedSlot < WorldSeatBindings.SeatCount); followedSlot++) {
                ...
                if (
                    ...
                    (location.EntityIndex != member.SourceSlot)
                ) {
                    continue;
                }
                ...
                if (targetAuthority.Remote is { } remoteTarget) {
                    ...
                    if (!m_remoteAuthorities.TryGetValue(
                        ...
                        routeAuthority = new WorldRemoteAuthority(
                            ...
                            security: m_authenticator,
                            observerAuthority: Boot.Server.AuthorityIdentity,
                            ...
                            routeChanged: route => {
                                ...
                                if (
                                    ...
                                    _ = m_seatRouter.CompareExchangeEntity(
                                        slot: trackedSlot,
                                        expected: expected,
                                        entity: route.Entity,
                                        current: out _
        ...
        try {
            targetAuthority.Acknowledge(
                sourceAuthority: Boot.Server.AuthorityIdentity,
                transferId: transfer.TransferId
            );
        } catch (Exception exception) when ((exception is IOException or System.Net.Sockets.SocketException or OperationCanceledException)) {
    ...
    private bool LeaveRosterSeat(int rosterSlot, WorldPrincipal actingPrincipal) {
        ...
        if (
            location.Endpoint.Identity.StartsWith(
            comparisonType: StringComparison.Ordinal,
            value: "$traveler/"
        ) &&
            m_remoteAuthorities.Remove(
            key: location.Endpoint.Identity,
    ...
    private static void SeedArrivalOccupancy(WorldInstance instance, int seat) {
        if (
            (seat < 0) ||
            (seat >= WorldPopulation.LocalSeatCount) ||
        ...
        if (
            ...
            if (
                ...
                try {
                    ...
                    authority = new TransferAuthority(
                        Local: null,
                        Remote: remote
                    );
                    spawned = false;
    ...
    public void PrepareBootSeatIntents(bool stepsBoot, ulong tick, ulong stepTicks) {
        ...
        m_client.SubmitAuthorityIntents(
            endpoint: EndpointFor(instance: boot),
            tick: tick
        );
    }
    ...
    /// <param name="authenticator">The process-scoped credentials used for authenticated remote authorities.</param>
