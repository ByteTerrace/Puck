...
internal sealed class WorldPointer {
    ...
    internal void SetWheelConsumerRegistered(bool registered) {
        if (Interlocked.Exchange(
            location1: ref m_wheelConsumerDeclared,
            value: 1
        ) != 0) {
        ...
        Volatile.Write(
            location: ref m_wheelConsumerRegistered,
            value: (registered
            ? 1
            : 0)
        );
    ...
    public bool HasPosition(int slot) {
        return (
            InRange(slot: slot) &&
            (Volatile.Read(location: ref m_hasPosition[slot]) != 0)
        );
    }
    ...
    public Vector2 Position(int slot) {
        return (InRange(slot: slot)
            ? new Vector2(
                x: Volatile.Read(location: ref m_positionX[slot]),
                y: Volatile.Read(location: ref m_positionY[slot])
            )
            : Vector2.Zero
        );
    }
    ...
    public int SystemReleaseCount(int slot) {
        return (InRange(slot: slot)
            ? Volatile.Read(location: ref m_systemReleaseCount[slot])
            : 0
        );
    }
    ...
    public Vector2 TakeMotion(int slot) {
        return (InRange(slot: slot)
            ? new Vector2(
                x: Interlocked.Exchange(
                    location1: ref m_motionX[slot],
                    value: 0f
                ),
                y: Interlocked.Exchange(
                    location1: ref m_motionY[slot],
                    value: 0f
                )
            )
            : Vector2.Zero
    ...
    public float TakeWheel(int slot) {
        if (Volatile.Read(location: ref m_wheelConsumerRegistered) == 0) {
            throw new InvalidOperationException(message: $"WorldPointer.TakeWheel: no {nameof(IWorldWheelConsumer)} is registered, so WorldPointerSink is draining the wheel accumulator itself on arrival — this call would read zero forever. Implement {nameof(IWorldWheelConsumer)} to register as the real reader instead of calling TakeWheel directly.");
        }
