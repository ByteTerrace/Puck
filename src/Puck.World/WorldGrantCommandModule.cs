...
internal sealed class WorldGrantCommandModule(WorldServer server, IServerLink link) : ICommandModule {
    ...
    public IEnumerable<CommandDefinition> GetCommands() {
        ...
        yield return CommandDefinition.WithWireArgs(
            ...
            handler: (context, args) => Handle(actor: context.ActingPrincipal(), args: args, exclusiveAllowed: false, revoke: true),
            routing: CommandRouting.Simulation
        );
        yield return CommandDefinition.WithWireArgs(
            bindability: CommandBindability.Unbindable,
            ...
            handler: (_, args) => {
                ...
                return new CommandResult(Output: (server.Grants.Describe(filter: filter) + DescribeDocumentRows(definition: server.Definition, filter: filter)));
            }
        );
        yield return CommandDefinition.WithWireArgs(
            bindability: CommandBindability.Unbindable,
            ...
            handler: (_, args) => {
                ...
                var verdict = (((query.Capability == WorldCapability.Drive) && (query.Subject.Kind == GrantSubjectKind.Body) && server.Grants.TryGetDriveGate(bodyIndex: query.Subject.Value, gateRow: out var gateRow))
                    ? new GrantVerdict(Rule: GrantRule.DriveGated, GateRow: gateRow)
                    : server.Grants.Allows(principal: query.Principal, capability: query.Capability, subject: query.Subject));
                ...
                var output = $"[world.why: {query.Principal.Describe()} {query.Capability.ToString().ToLowerInvariant()} {query.Subject.Describe()} = {(verdict.IsAllowed ? "allowed" : "denied")} ({verdict.Describe()}) — {detail}]";

                if (verdict.IsAllowed && (query.Capability == WorldCapability.Drive)) {
                    var rawHold = server.Grants.HoldCeiling(principal: query.Principal, subject: query.Subject);
                    var seconds = (double)FixedQ4816.FromRawBits(value: rawHold);

                    output += string.Create(provider: CultureInfo.InvariantCulture, handler: $" hold:{seconds:0.###}s engine-backstop:{WorldBody.MaxActionHoldSeconds:0.###}s");
                }
                ...
                var decidingSubject = ((verdict.Rule == GrantRule.WildcardHold) ? GrantSubject.All : query.Subject);

                if (query.KindMask is { } queriedKinds) {
                    var hasMask = server.Grants.TryGetKindMask(principal: query.Principal, capability: query.Capability, subject: decidingSubject, out var deciding);
                    ...
                    foreach (var entry in WorldMutationKindCatalog.All()) {
                        ...
                        // NOTE: no `_ = ` discard here — the enclosing handler's own first parameter is literally
                        // named `_` (CommandContext), so `_ =` inside this lambda body would assign THROUGH IT
                        // rather than discard, and fail to compile against StringBuilder.
                        ...
                        coverage.Append(value: ((coverage.Length == 0) ? "" : ", ")).Append(value: entry.Type.Name).Append(value: ':').Append(value: ((!hasMask || deciding.Contains(ordinal: entry.Ordinal)) ? "admitted" : "denied-by-mask"));
                    }

                    output += $" verbs: {(hasMask ? $"mask on {query.Capability.ToString().ToLowerInvariant()} {decidingSubject.Describe()} admits {deciding.Describe()}; " : "(deciding row carries no mask — every kind admitted) ")}{coverage}";
                }

                if (query.WriteMask is { } queriedWrites) {
                    var hasWrites = server.Grants.TryGetWriteMask(principal: query.Principal, capability: query.Capability, subject: decidingSubject, out var decidingWrites);
                    var coverage = new StringBuilder();
                    ...
                    foreach (var kind in Enum.GetValues<WorldDocumentWriteKind>()) {
                        ...
                        coverage.Append(value: ((coverage.Length == 0) ? "" : ", ")).Append(value: kind.ToString()).Append(value: ':').Append(value: ((hasWrites && decidingWrites.Contains(kind: kind)) ? "admitted" : "denied-by-mask"));
                    }
                    ...
                    // already admitted.
                    output += $" writes: {(hasWrites ? $"mask admits {decidingWrites.Describe()}; " : "(deciding row carries no write mask — the cross-document channel admits nothing) ")}{coverage}";
                }
    ...
    private static string DescribeDocumentRows(WorldDefinition definition, WorldPrincipal? filter) {
        ...
        foreach (var grant in definition.Grants) {
            if ((grant.Principal.Kind != PrincipalKind.Document) || ((filter is { } only) && (grant.Principal != only))) {
                continue;
            }

            _ = builder
                .Append(value: ((builder.Length == 0) ? " [world.grants.document: " : " | "))
        ...
        return ((builder.Length == 0) ? string.Empty : builder.Append(value: ']').ToString());
    }
    ...
    private CommandResult Handle(WorldPrincipal actor, in WireArgs args, bool exclusiveAllowed, bool revoke) {
        ...
        if (!TryParseGrant(args: args, exclusiveAllowed: exclusiveAllowed, verb: verb, grant: out var grant, error: out var error, channels: server.Population.Channels, targets: server.Population.TargetRegisters, verbsAllowed: exclusiveAllowed)) {
            return error;
        }
        ...
        if (revoke) {
            link.SubmitRevoke(grant: grant, actor: actor);
        } else {
            link.SubmitGrant(grant: grant, actor: actor);
        }
    ...
    internal static bool TryParseGrant(in WireArgs args, bool exclusiveAllowed, string verb, out WorldGrant grant, out CommandResult error, WorldChannelTable? channels = null, WorldTargetRegisterTable? targets = null, bool verbsAllowed = false) {
        ...
        var maximum = ((3 + (exclusiveAllowed ? 7 : 0)) + (verbsAllowed ? 2 : 0));
        ...
        if ((args.Count < 3) || (args.Count > maximum)) {
            var form = (exclusiveAllowed
                ...
                : (verbsAllowed ? "<principal> <capability> <subject> [verbs:<name,...>] [writes:<name,...>]" : "<principal> <capability> <subject>"));

            error = Usage(verb: verb, form: form);
        ...
        for (var index = 3; (index < args.Count); index++) {
            ...
            if (!exclusive && exclusiveAllowed && args.Is(index: index, value: "exclusive")) {
                exclusive = true;

                continue;
            }

            if (verbsAllowed && token.StartsWith(value: "verbs:", comparisonType: StringComparison.OrdinalIgnoreCase) && (token.Length > 6)) {
                ...
                foreach (var name in token[6..].ToString().Split(separator: ',', options: StringSplitOptions.None)) {
                    if (!TryParseMutationKindName(name: name, ordinal: out var ordinal)) {
            ...
            if (verbsAllowed && token.StartsWith(value: "writes:", comparisonType: StringComparison.OrdinalIgnoreCase) && (token.Length > 7)) {
                ...
                foreach (var name in token[7..].ToString().Split(separator: ',', options: StringSplitOptions.None)) {
                    if (!Enum.TryParse<WorldDocumentWriteKind>(value: name, ignoreCase: true, result: out var writeKind) || !Enum.IsDefined(value: writeKind)) {
            ...
            if (exclusiveAllowed && token.StartsWith(value: "budget:", comparisonType: StringComparison.OrdinalIgnoreCase)) {
                ...
                if ((budget is not null) ||
                    !ushort.TryParse(s: token[7..], style: NumberStyles.Integer, provider: CultureInfo.InvariantCulture, result: out var parsedBudget) ||
                    (parsedBudget == 0)) {
            ...
            if (exclusiveAllowed && token.StartsWith(value: "events:", comparisonType: StringComparison.OrdinalIgnoreCase)) {
                // The identical budget:0 discipline: 0 is not a spelling for "no event budget" — omit the token.
                if ((eventBudget is not null) ||
                    !ushort.TryParse(s: token[7..], style: NumberStyles.Integer, provider: CultureInfo.InvariantCulture, result: out var parsedEventBudget) ||
                    (parsedEventBudget == 0)) {
            ...
            if (exclusiveAllowed && token.StartsWith(value: "channels:", comparisonType: StringComparison.OrdinalIgnoreCase) && (token.Length > 9)) {
                ...
                foreach (var name in token[9..].ToString().Split(separator: ',', options: StringSplitOptions.None)) {
            ...
            if (exclusiveAllowed && token.StartsWith(value: "ceiling:", comparisonType: StringComparison.OrdinalIgnoreCase) && (token.Length > 8)) {
                ...
                if (!double.TryParse(s: token[8..], style: NumberStyles.Float, provider: CultureInfo.InvariantCulture, result: out var parsedCeiling) ||
                    !double.IsFinite(d: parsedCeiling) ||
                    (parsedCeiling < 0.0) || (parsedCeiling > 1.0)) {
            ...
            if (exclusiveAllowed && token.StartsWith(value: "registers:", comparisonType: StringComparison.OrdinalIgnoreCase) && (token.Length > 10)) {
                ...
                foreach (var name in token[10..].ToString().Split(separator: ',', options: StringSplitOptions.None)) {
                    if (!targets.TryGetIndex(name: name, index: out var targetIndex)) {
            ...
            if (exclusiveAllowed && token.StartsWith(value: "hold:", comparisonType: StringComparison.OrdinalIgnoreCase) && (token.Length > 5)) {
                if ((holdCeiling is not null) ||
                    !double.TryParse(s: token[5..], style: NumberStyles.Float, provider: CultureInfo.InvariantCulture, result: out var parsedHold) ||
                    !double.IsFinite(d: parsedHold) || (parsedHold < 0.0) || (parsedHold > WorldBody.MaxActionHoldSeconds)) {
            ...
            var expected = (exclusiveAllowed
                ...
                : (verbsAllowed ? "'verbs:<name,...>' and/or 'writes:<name,...>', each at most once" : "no trailing tokens"));
        ...
        if ((ceiling is not null) && (namedChannels is null)) {
        ...
        var reachedBits = ((ceiling is null) ? (namedChannels?.Bits ?? 0UL) : 0UL) | (namedRegisters?.Bits ?? 0UL);
        var reach = (((ceiling is null) && (reachedBits != 0UL)) ? new ChannelReachMask(Bits: reachedBits) : (ChannelReachMask?)null);
        var consent = (((ceiling is not null) && (namedChannels is { } consented)) ? new ChannelConsentMask(Bits: consented.Bits) : (ChannelConsentMask?)null);

        grant = new WorldGrant(Principal: principal, Capability: capability, Subject: subject, Exclusive: exclusive, Budget: budget, Reach: reach, Consent: consent, Ceiling: ceiling, KindMask: kindMask, EventBudget: eventBudget, HoldCeiling: holdCeiling, WriteMask: writeMask);
    ...
    internal static bool TryParsePrincipal(ReadOnlySpan<char> token, out WorldPrincipal principal) {
        return WorldPrincipal.TryParse(token: token, principal: out principal);
    }

    // internal (not private): WorldAddonCommandModule.Mount reuses this exact token grammar for its trailing
    // <capability> <subject> manifest pairs, so "drive body:0" means the identical thing on both verbs — widened to
    // the minimum needed (same-assembly internal, not public) rather than duplicating the parse.
    internal static bool TryParseCapability(ReadOnlySpan<char> token, out WorldCapability capability) {
        ...
        if (token.Equals(other: "observe", comparisonType: StringComparison.OrdinalIgnoreCase)) {
    ...
    /// <returns><see langword="true"/> when the token parsed.</returns>
    internal static bool TryParseSubject(ReadOnlySpan<char> token, out GrantSubject subject) => GrantSubject.TryParse(token: token, subject: out subject);
}
