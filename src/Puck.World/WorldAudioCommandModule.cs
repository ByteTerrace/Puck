...
internal sealed class WorldAudioCommandModule(WorldServer server, IServerLink link, WorldAudioDirector director, Audio.WorldAudioRenderService device) : ICommandModule {
    ...
    private CommandResult VolumeHandler(CommandContext context, WireArgs args) {
        ...
        if ((args.Count != 1) ||
            ...
            !float.IsFinite(f: volume) || (volume < 0f) || (volume > Puck.Forge.Authoring.CreationSoundDocument.MaxLevel)) {
        ...
        link.SubmitSessionLever(
            lever: new WorldSessionLever(Section: WorldSection.Audio, Kind: WorldLeverKind.MasterVolume, A: volume),
            principal: context.ActingPrincipal()
        );

        return new CommandResult(Output: string.Create(
            provider: CultureInfo.InvariantCulture,
            handler: $"[world.volume: {director.EffectiveMasterVolume:0.###} ({(director.MasterVolumeLeverEngaged ? "session lever; world.save folds into audio.masterGain" : "document audio.masterGain — lever refused")})]"
        ));
    }
