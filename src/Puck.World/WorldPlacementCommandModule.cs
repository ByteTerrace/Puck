...
internal sealed class WorldPlacementCommandModule(WorldServer server, WorldPopulation population, WorldScreenBinder binder, WorldInstanceHost instances, WorldSessionResolver resolver) : ICommandModule {
    ...
    public IEnumerable<CommandDefinition> GetCommands() {
        yield return CommandDefinition.WithWireArgs(
            ...
            handler: (_, args) => {
                if (!TryResolveInstance(args: in args, verb: "world.inhabitants", instance: out var instance, error: out var tokenError)) {
                    return tokenError!.Value;
                }

                return new CommandResult(Output: WithInstanceTag(text: DescribeInhabitants(instance: instance), instance: instance));
            }
        );
        yield return CommandDefinition.WithWireArgs(
            bindability: CommandBindability.Unbindable,
            ...
            handler: (_, args) => {
                ...
                if (args[0].StartsWith(value: InstanceTokenPrefix, comparisonType: StringComparison.OrdinalIgnoreCase)) {
        ...
        yield return CommandDefinition.WithWireArgs(
            ...
            handler: (_, args) => {
                if (!TryResolveInstance(args: in args, verb: "world.attachments", instance: out var instance, error: out var tokenError)) {
                    return tokenError!.Value;
                }

                return new CommandResult(Output: WithInstanceTag(text: DescribeAttachments(instance: instance), instance: instance));
            }
        );
        yield return CommandDefinition.WithWireArgs(
            bindability: CommandBindability.Unbindable,
            ...
            handler: (_, args) => {
                if (!TryResolveInstance(args: in args, verb: "world.portals", instance: out var instance, error: out var tokenError)) {
                    return tokenError!.Value;
                }

                return new CommandResult(Output: WithInstanceTag(text: DescribePortals(instance: instance), instance: instance));
            }
        );
        yield return CommandDefinition.WithWireArgs(
            bindability: CommandBindability.Unbindable,
            ...
            handler: (_, args) => {
                if (!TryResolveInstance(args: in args, verb: "world.destinations", instance: out var instance, error: out var tokenError)) {
                    return tokenError!.Value;
                }
    ...
    private bool TryResolveInstance(in WireArgs args, string verb, out WorldInstance? instance, out CommandResult? error) {
        ...
        if (!instances.TryGet(name: name, instance: out var resolved) || (resolved is null)) {
            instance = null;
    ...
    private string DescribeInhabitants(WorldInstance? instance) {
        ...
        for (var index = 0; (index < pop.Capacity); index++) {
            if ((pop.InhabitantPlacementId(index: index) is not { } placementId) || (pop.EntryBody(index: index) is not { } body)) {
                continue;
            }

            any = true;

            var placement = WorldDefinitionRows.FindPlacement(placements: placements, id: placementId);
    ...
    private string DescribeFaces() {
        ...
        foreach (var row in catalog.Rows) {
            ...
            var placement = WorldDefinitionRows.FindPlacement(placements: definition.Placements, id: row.PlacementId);
            var slot = (row.SlotStarved ? "darkened (band full)"
                : ((row.ScreenIndex < 0) ? "unlit (source shows nothing)"
                : row.ScreenIndex.ToString(provider: CultureInfo.InvariantCulture)));
            var handle = ((row.ScreenIndex >= 0) ? binder.CurrentHandle(index: row.ScreenIndex) : nint.Zero);
            var session = ((placement is null) ? string.Empty : DescribeSession(index: row.ScreenIndex, placement: placement, faceName: row.FaceName));

            _ = builder.Append(value: $" {row.PlacementId}/{row.FaceName}[screen={slot} handle={((handle != 0) ? "bound" : "no-signal")}{session}]");
        }
    ...
    private string DescribeSession(int index, WorldPlacement placement, string faceName) {
        if (binder.TryDescribeSession(index: index, description: out var session)) {
            var cameraText = (session.EffectiveCamera ?? session.RequestedCamera ?? "(default)");
            var leaseText = (session.InstanceGone ? "instance-retired (holding last image)" : "held");
            var projectionText = ProjectionText(projection: session.Projection, width: session.RenderWidth, height: session.RenderHeight, rendersEveryFrame: session.RendersEveryFrame);
        ...
        foreach (var faceOverride in (placement.FaceSources ?? [])) {
            if (string.Equals(a: faceOverride.Face, b: faceName, comparisonType: StringComparison.Ordinal) &&
                (faceOverride.Source is WorldScreenSource.Session declared)) {
                var projectionText = ProjectionText(projection: declared.Projection, width: (declared.Resolution?.Width ?? 0), height: (declared.Resolution?.Height ?? 0), rendersEveryFrame: (declared.Projection == WorldScreenProjection.Window));
    ...
    private static string ProjectionText(WorldScreenProjection projection, int width, int height, bool rendersEveryFrame) =>
        $" projection={projection.ToString().ToLowerInvariant()} cost={width}x{height}{(rendersEveryFrame ? "/frame" : "")}";
    ...
    private string DescribeAttachments(WorldInstance? instance) {
        ...
        foreach (var placement in definition.Placements) {
            ...
            if (WorldPlacementAttachment.TryResolve(attach: attach, population: pop, position: out var position, yawRadians: out var yaw, reason: out var reason)) {
    ...
    private string DescribePortals(WorldInstance? instance) {
        ...
        foreach (var placement in definition.Placements) {
            foreach (var face in (placement.FaceSources ?? [])) {
                ...
                var destination = WorldDefinitionRows.FindDestination(destinations: definition.Destinations, name: portal.Destination);
    ...
    private static string DescribeOccupancy(WorldInstance? instance, string placementId, string faceName) {
        ...
        for (var seat = 0; (seat < WorldPopulation.LocalSeatCount); seat++) {
            if (instance.PortalOccupancy.IsInside(placementId: placementId, faceName: faceName, seat: seat)) {
                seats.Add(item: (seat + 1).ToString(provider: CultureInfo.InvariantCulture));
            }
        }

        return ((seats.Count == 0) ? "none" : string.Join(separator: ",", values: seats));
    }
    private string DescribeDestinations(WorldInstance? instance) {
        ...
        foreach (var destination in (definition.Destinations ?? [])) {
            ...
            var reference = WorldDefinitionRows.FindReference(references: definition.References, name: destination.Reference);
            var documentPath = (reference?.Document ?? "?");
            var consumers = DescribeDestinationConsumers(definition: definition, name: destination.Name.Value);
            ...
            var active = DescribeActiveGenerations(destinationName: destination.Name.Value, durability: destination.Durability, referencedDocument: WorldInstanceHost.ResolveReferenceDocument(source: sourceInstance, documentPath: documentPath));
    ...
    private static string DescribeDestinationConsumers(WorldDefinition definition, string name) {
        ...
        foreach (var placement in definition.Placements) {
            foreach (var face in (placement.FaceSources ?? [])) {
                if ((face.Portal is { } portal) && string.Equals(a: portal.Destination, b: name, comparisonType: StringComparison.Ordinal)) {
                    consumers.Add(item: $"{placement.Id}/{face.Face}");
                }
            }
        }

        return ((consumers.Count > 0) ? string.Join(separator: ",", values: consumers) : "none");
    }
    ...
    private string DescribeActiveGenerations(string destinationName, WorldDestinationDurability durability, string referencedDocument) {
        var rows = resolver.DescribeActive(destinationName: destinationName, durability: durability, referencedDocument: referencedDocument);
        ...
        foreach (var row in rows) {
            var occupancy = (instances.TryGet(name: row.InstanceName, instance: out var instance) ? instance!.Server.Population.ActiveCount().ToString() : "?");
