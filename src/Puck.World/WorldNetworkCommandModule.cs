...
internal sealed class WorldNetworkCommandModule(WorldTcpHost tcpHost, WorldServer server) : ICommandModule {
    ...
    private string DescribeProjection(in WireArgs arguments) {
        var first = ((arguments.Count > 0) ? arguments[0].ToString() : string.Empty);

        if (string.Equals(a: first, b: "peer", comparisonType: StringComparison.Ordinal)) {
            var authority = ((arguments.Count > 1) ? arguments[1].ToString() : string.Empty);

            if (string.IsNullOrWhiteSpace(value: authority)) {
                return "[world.projection refused: 'peer' needs an authority namespace]";
            }

            var refusal = WorldAdmissionDoor.TryAdmitArrival(entries: server.Definition.Admission, sourceAuthority: authority, verdict: out var verdict);
        ...
        if (string.IsNullOrWhiteSpace(value: first)) {
            return $"[world.projection: {string.Join(separator: " | ", values: Enum.GetValues<WorldDisclosureTier>().Select(selector: DescribeTier))}]";
        }

        return (Enum.TryParse<WorldDisclosureTier>(value: first, ignoreCase: true, result: out var requested) && Enum.IsDefined(value: requested)
            ? $"[world.projection: {DescribeTier(tier: requested)}]"
            : $"[world.projection refused: '{first}' names no disclosure tier — one of {string.Join(separator: ", ", values: Enum.GetNames<WorldDisclosureTier>())}]");
    }
    ...
    private string DescribeTier(WorldDisclosureTier tier) {
        ...
        var projection = WorldProjection.Compose(definition: definition, tier: tier, authority: server.AuthorityIdentity, revision: server.Population.Revision)!;
        var bytes = WorldProjection.Serialize(projection: projection);

        return $"presentation: {bytes.Length} bytes, {WorldProjectionDocument.SchemaVersion} carrying {string.Join(separator: ",", values: ProjectionSections(projection: projection))}";
    }
    ...
    private string Describe() {
        ...
        var rows = string.Join(separator: " | ", values: connections.Select(selector: c => $"conn:{c.ConnectionId} peer:{c.PeerIndex}:{c.Generation} identity:{c.IdentityDomain}/{c.IdentitySubject} tier:{c.Tier} @ {c.RemoteEndpoint}"));
