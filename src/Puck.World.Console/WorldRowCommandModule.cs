...
public sealed class WorldRowCommandModule(IWorldConsoleAuthority authority, IServerLink link, WorldDeferredVerbEchoes echoes, WorldRowStepWindowGuard? stepGuard = null) : ICommandModule {
    ...
    private static IReadOnlyDictionary<string, RowSection> BuildSections() => new Dictionary<string, RowSection>(comparer: StringComparer.Ordinal) {
        ...
        ["views.studies"] = new RowSection(
        RowType: typeof(WorldViewStudy),
        Upsert: Upsert(
            info: WorldJsonContext.Default.WorldViewStudy,
            toMutation: static (principal, study) => new WorldMutation.UpsertViewStudy(
                Study: study,
                Principal: principal
            )
        ),
        Remove: RemoveByName(remove: static (principal, name) => new WorldMutation.RemoveViewStudy(
            Name: name,
            Principal: principal
        )),
        Read: ReadRowByKey(
            info: WorldJsonContext.Default.WorldViewStudy,
            keyOf: static row => row.Name,
            select: static server => server.Definition.Views.Studies
        )
    ),
