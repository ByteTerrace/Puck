        ...
        session.HoldWhile(hold: () => {
            ...
            if (engine.IsReady) {
                Console.Error.WriteLine(value: string.Create(
                    provider: CultureInfo.InvariantCulture,
                    handler: $"[world.wait: the engine is ready at tick {gate.Tick}]"
                ));
            } else if (Stopwatch.GetTimestamp() >= deadline) {
                Console.Error.WriteLine(value: string.Create(
                    provider: CultureInfo.InvariantCulture,
                    handler: $"[world.wait: released — the engine was not ready after {seconds} seconds: {(engine.NotReadyReason ?? "no reason given")}]"
                ));
            } else {
                return true;
            }
