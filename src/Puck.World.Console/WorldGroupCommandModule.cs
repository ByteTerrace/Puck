...
/// (<c>groups.kinds</c>) is authored through the general <c>world.row.set</c>/<c>.remove groups.kinds ...</c>;
/// <c>world.group.form</c>/<c>.join</c>/<c>.leave</c>/<c>.kick</c> work the live roster (a runtime group is added by
/// <c>form</c> and wiped by the next whole-document rebuild — see <see cref="WorldGroup"/>'s own remarks);
/// <c>world.ownership.offer</c>/<c>.accept</c>/<c>.reclaim</c> work the escrow/transfer lane over an already-declared
/// <see cref="WorldOwnership"/> row (see <see cref="WorldMutation.OfferOwnership"/>/
/// <see cref="WorldMutation.SettleOwnership"/>); <c>world.groups</c> is the read-back for all of it. Every mutating
...
public sealed class WorldGroupCommandModule(IWorldConsoleAuthority authority, IServerLink link) : ICommandModule {
    ...
    private static string Describe(WorldDefinition definition, string? filter) {
        ...
        if (
            (groupsSection.Kinds.Count == 0) &&
            (groupsSection.Groups.Count == 0) &&
            (groupsSection.Ownership.Count == 0)
        ) {
    ...
    public IEnumerable<CommandDefinition> GetCommands() {
        ...
        yield return CommandDefinition.WithWireArgs(
            ...
            handler: (context, args) => Handle(
                context: context,
                args: args,
                verb: "world.group.join",
                build: static (groupId, member, actor) => new WorldMutation.JoinGroup(
                    GroupId: groupId,
                    Member: member,
                    Principal: actor
                )
        ...
        yield return CommandDefinition.WithWireArgs(
            ...
            handler: (context, args) => Handle(
                context: context,
                args: args,
                verb: "world.group.leave",
                build: static (groupId, member, actor) => new WorldMutation.LeaveGroup(
                    GroupId: groupId,
                    Member: member,
                    Principal: actor
                )
        ...
        yield return CommandDefinition.WithWireArgs(
            ...
            handler: (context, args) => Handle(
                context: context,
                args: args,
                verb: "world.group.kick",
                build: static (groupId, member, actor) => new WorldMutation.KickMember(
                    GroupId: groupId,
                    Member: member,
                    Principal: actor
                )
        ...
        yield return CommandDefinition.WithWireArgs(
            ...
            handler: (context, args) => Settle(
                args: args,
                context: context,
                reclaim: false,
                verb: "world.ownership.accept"
            ),
            routing: CommandRouting.Simulation
        );
        yield return CommandDefinition.WithWireArgs(
