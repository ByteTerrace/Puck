...
internal static class WorldRowFieldPath {
    ...
    private static bool SelectorMatches(JsonNode node, string wanted) => (node.GetValueKind() switch {
        ...
        a: node.GetValue<string>(),
        b: wanted,
        comparisonType: StringComparison.Ordinal
    ),
        JsonValueKind.Number => (decimal.TryParse(
        s: wanted,
        provider: CultureInfo.InvariantCulture,
        style: NumberStyles.Number,
        result: out var wantedNumber
    ) &&
            node.AsValue().TryGetValue<decimal>(value: out var actualNumber) &&
            (wantedNumber == actualNumber)),
        JsonValueKind.True or JsonValueKind.False => (bool.TryParse(
        result: out var wantedBool,
        value: wanted
    ) && (node.GetValue<bool>() == wantedBool)),
        _ => string.Equals(
        a: node.ToJsonString(),
        b: wanted,
        comparisonType: StringComparison.Ordinal
    ...
    public static bool TryParse(string path, out Segment[] segments, out string? error) {
        ...
        for (var i = 0; (i < tokens.Count); i++) {
            ...
            result[i] = new Segment(
                Index: null,
                Name: name,
                SelectorField: content[..equals],
                SelectorValue: content[(equals + 1)..]
            );
        }
    ...
    public static bool TryParseSelector(string text, string containerName, out Segment segment, out string? error) {
        ...
        segment = new Segment(
            Index: null,
            Name: containerName,
            SelectorField: text[..equals],
            SelectorValue: text[(equals + 1)..]
        );
        error = null;
    ...
    public static bool TryResolveArrayElement(JsonArray array, Segment segment, string path, out int index, out string? error) {
        ...
        if (matches.Count == 0) {
            error = ((candidates.Count == 0)
                ...
                : $"'{path}': no element of '{segment.Name}' has {field}={wanted} — candidates: {string.Join(
                    separator: ", ",
                    values: candidates
                )}"
            );
        } else {
            error = $"'{path}': {field}={wanted} is ambiguous — {matches.Count} elements of '{segment.Name}' match (indices {string.Join(
                separator: ", ",
                values: matches
            )})";
        }
