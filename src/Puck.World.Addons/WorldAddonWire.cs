...
internal static class WorldAddonWire {
    ...
    public static ulong CapabilityBit(WorldCapability capability) {
        return capability switch {
            ...
            message: "unmapped capability — the wire mapping must be extended deliberately, never defaulted"
        ),
        };
    }
    ...
    public static AddonVerdict FromRule(GrantRule rule) {
        return rule switch {
            ...
            _ => throw new ArgumentOutOfRangeException(
            paramName: nameof(rule),
            actualValue: rule,
            message: "unmapped grant rule — the wire mapping must be extended deliberately, never defaulted"
        ),
        };
    }
...
internal sealed class WorldAddonChannelResolver(WorldChannelTable channels) : IAddonChannelResolver {
    ...
    private static AddonChannelValueShape WireShape(ChannelShape shape) {
        return shape switch {
            ...
            actualValue: shape,
            message: "unmapped channel shape — the wire mapping must be extended deliberately, never defaulted"
        ),
        };
    }
