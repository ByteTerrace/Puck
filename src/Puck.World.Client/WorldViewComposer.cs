...
/// </summary>
public sealed class WorldViewComposer {
    ...
    public void Compose(int joinedCount, WorldViewDefaults views,
        ...
        if (m_transition is { } transition) {
            var eased = transition.Sample(
                complete: out var complete,
                elapsedSeconds: (elapsedSeconds - m_transitionStart)
            );

            CopyBindings(
                source: eased.Bindings,
                into: m_currentBindings
            );
            TransitionProgress = Math.Clamp(
                max: 1f,
                min: 0f,
                value: ((elapsedSeconds - m_transitionStart) / m_transitionSeconds)
            );
