...
public sealed class WorldEditorDrag {
    ...
    private void ApplySnap(Channel channel) {
        var snapped = GridSnap.Apply(
            ...
            config: in channel.Snap,
            candidateLocalHalfExtents: Vector3.Zero,
            previousSnapped: channel.Snapped
        );
    ...
    private static void BumpPast(string id, string prefix, ref int next) {
        if (
            ...
            int.TryParse(
            s: id.AsSpan(start: prefix.Length),
    ...
    private bool DeliveredExpected(Channel channel) {
        ...
        if (channel.ExpectedSpeaker is { } expectedSpeaker) {
            foreach (var speaker in definition.Speakers) {
                if (speaker.Equals(other: expectedSpeaker)) {
                    return true;
                }
            }
    ...
    // The WithPosition analog for the two draggable speaker kinds (anchored rows never enter the channel).
    private static WorldSpeaker MovedSpeaker(WorldSpeaker speaker, Vector3 position) => speaker switch {
        WorldSpeaker.Bed bed => (bed with { Center = position }),
        WorldSpeaker.Fixed fixedSpeaker => (fixedSpeaker with { Position = position }),
        _ => speaker,
    };
    ...
    private int SlotOrFirst(int slot) => ((((uint)slot) < ((uint)m_channels.Length))
        ? slot
        : 0
    );
    ...
    private static string Subject(Channel channel) => ((channel.Placement is { } placement)
        ...
        : ((channel.Speaker is { } speaker)
            ? $"speaker '{speaker.Name}'"
            : $"screen {channel.Screen!.Index}"
    ));
    ...
    private bool TryFitComposed(WorldDefinition candidate, out string reason) {
        return m_envelope.TryFit(
            candidate: (CandidateComposer?.Invoke(arg: candidate) ?? candidate),
            reason: out reason
        );
    }
    ...
    public string? Cancel(int slot) {
        ...
        return (wasGhost
            ? $"ghost {subject} discarded"
            : $"{subject} back at its document pose"
        );
    }
    ...
    public IReadOnlyList<WorldPlacement> ComposePlacements(IReadOnlyList<WorldPlacement> live) {
        ...
        foreach (var channel in m_channels) {
            if (
                (!channel.Active && !channel.Frozen) ||
                (channel.Placement is not { } pending)
            ) {
                continue;
            }
    ...
    public IReadOnlyList<WorldSpeaker> ComposeSpeakers(IReadOnlyList<WorldSpeaker> live) {
        List<WorldSpeaker>? rows = null;

        foreach (var channel in m_channels) {
            if (
                (!channel.Active && !channel.Frozen) ||
                (channel.Speaker is not { } pending)
            ) {
                continue;
            }
            ...
            var moved = MovedSpeaker(
                position: channel.Snapped,
                speaker: pending
            );
    ...
    public string? Describe(int slot) {
        ...
        return string.Create(
            provider: CultureInfo.InvariantCulture,
            handler: $"{(channel.Frozen
            ? "frozen "
            : string.Empty)}{(channel.IsGhost
            ? "ghost "
            : string.Empty)}{subject} at ({channel.Snapped.X:0.00}, {channel.Snapped.Y:0.00}, {channel.Snapped.Z:0.00})"
        );
    }
    ...
    public string NextFreePlacementId() {
        ...
        if (
            m_idWatermarks.TryGetValue(
            key: Prefix,
            value: out var issued
        ) &&
            (issued >= next)
        ) {
            next = (issued + 1);
    ...
    public Vector3? PendingPosition(int slot) =>
        (IsDragging(slot: slot)
            ? m_channels[slot].Snapped
            : null
        );
    ...
    public string? Release(int slot, WorldPrincipal principal) {
        ...
        if (channel.Speaker is { } speaker) {
            ...
            channel.ExpectedSpeaker = moved;
            subject = $"speaker '{speaker.Name}'";
        } else if (channel.Placement is { } placement) {
            var moved = (placement with { Position = channel.Snapped });

            m_link.SubmitWorldMutation(mutation: new WorldMutation.UpsertPlacement(
                Placement: moved,
                Principal: principal
            ));
            ...
            case WorldSection.Speakers:
                ...
                foreach (var speaker in definition.Speakers) {
                    ...
                    Begin(
                        ...
                        placement: null,
                        speaker: speaker,
                        origin: ((speaker is WorldSpeaker.Bed bed)
                        ? bed.Center
                        : ((WorldSpeaker.Fixed)speaker).Position),
                        isGhost: false
                    );
                ...
                error = $"no speaker '{selection.Id}' to grab";
    ...
    public bool TrySpawnPlacementGhost(int slot, WorldPlacement placement, out string error) {
        ...
        if (!TryFitComposed(
            candidate: (definition with { PlacementsRaw = rows }),
