...
public readonly record struct EditorPickTarget(WorldSection Section, string Id, int Index, Vector3 Focus);
/// <summary>
...
/// </summary>
public sealed class WorldEditorPicker {
    ...
    public const int MaxPickReach = 256;
    ...
    private void Build() {
        ...
        foreach (var screen in definition.Screens) {
            ...
            _ = builder
                ...
                .ResetPoint();
            targets.Add(item: new EditorPickTarget(
        ...
        foreach (var placement in definition.Placements) {
            ...
            var radius = MathF.Max(
                x: (Puck.Forge.Authoring.CreationGeometry.Reach(document: creation.Document) * (placement.Scale * 0.5f)),
                y: 0.35f
            );
            ...
            AddProxy(
                builder: builder,
                targets: targets,
                target: new EditorPickTarget(
                    Section: WorldSection.Placements,
                    Id: placement.Id,
                    Index: -1,
                    Focus: placement.Position
                ),
                center: (placement.Position + new Vector3(
                    x: 0f,
                    y: radius,
                    z: 0f
                )),
        ...
        foreach (var spawn in definition.SpawnPoints) {
            AddProxy(
                ...
                targets: targets,
                target: new EditorPickTarget(
                    Section: WorldSection.Spawns,
                    Id: spawn.Id,
                    Index: -1,
                    Focus: spawn.Position
                ),
                center: (spawn.Position + new Vector3(
                    x: 0f,
                    y: SpawnProxyRadius,
                    z: 0f
                )),
                radius: SpawnProxyRadius
            );
        }
        ...
        foreach (var camera in definition.Cameras) {
            ...
            if (camera.Anchor is null) {
                ...
                AddProxy(
                    ...
                    targets: targets,
                    target: new EditorPickTarget(
                        Section: WorldSection.Cameras,
                        Id: camera.Name,
                        Index: -1,
                        Focus: eye
                    ),
                    center: eye,
                    radius: CameraProxyRadius
                );
            }
        }
            ...
            switch (speaker) {
                case WorldSpeaker.Fixed fixedSpeaker:
                    AddProxy(
                        ...
                        targets: targets,
                        target: new EditorPickTarget(
                            Section: WorldSection.Speakers,
                            Id: speaker.Name,
                            Index: -1,
                            Focus: fixedSpeaker.Position
                        ),
                        center: fixedSpeaker.Position,
                        radius: SpeakerProxyRadius
                    );
    ...
    public bool TryPick(Vector3 eye, Vector3 direction, out EditorPickTarget target, float? maxDistance = null) {
        ...
        if (
            (m_evaluator is not { } evaluator) ||
            (direction.LengthSquared() < 1e-8f)
        ) {
            return false;
        }

        var reach = ((maxDistance is { } authored)
            ? FixedQ4816.FromDouble(value: authored)
            : MaxPickDistance
        );

        if (!evaluator.Raycast(
            origin: FixedPosition.FromLocal(local: FixedVector3.FromVector3(value: eye)),
