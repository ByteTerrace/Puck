...
/// the row lookup (<see cref="StateRows.FindDynamics"/>) plus the derivation
...
internal static class WorldDynamicsResponse {
    ...
    public static bool TryResolveResponse(IReadOnlyList<DynamicsRow>? rows, string? name, out SecondOrderResponse response) {
        if (
