...
/// ACTIVE binding group (the <see cref="WorldContextFamilies.Editor"/> context state, mapped to a group by the document's own <c>contexts</c> row
...
public sealed class WorldEditorSession {
    ...
    private static readonly Vector3 OrbitPivotLift = new(
        x: 0f,
        y: 1f,
        z: 0f
    );
    ...
    private void AdvanceDrag(int slot, Seat seat, float deltaSeconds) {
        ...
        m_drag.Advance(
            ...
            move: new Vector2(
                x: Response(value: move.X),
                y: Response(value: move.Y)
            ),
            vertical: vertical,
            speed: seat.Speed,
    ...
    private static void AdvanceOrbit(Seat seat, Vector3 pivot, float deltaSeconds) {
        ...
        seat.Rig.Eye = (pivot + OrbitRig.Offset(
            distance: seat.OrbitDistance,
            pitch: seat.OrbitPitch,
            yaw: seat.OrbitYaw
        ));
    ...
    private void AdvanceWorkbench(int slot, Seat seat, float deltaSeconds) {
        ...
        forward = ((forward.LengthSquared() > 1e-8f)
            ? Vector3.Normalize(value: forward)
            : Vector3.UnitZ
        );
    ...
    private void Deactivate(int slot, Seat seat) {
        m_bindings.SetContextState(
            family: WorldContextFamilies.Editor,
            slot: slot,
            state: WorldContextFamilies.EditorNone
        );
    ...
    public EditorModeOutcome Enter(int slot) {
        ...
        // The group flip is a precondition, checked before anything else mutates: a document whose contexts map no
        // group to the editing state makes the mode unenterable — the seat would edit with its play page.
        if (m_bindings.ContextGroup(
            family: WorldContextFamilies.Editor,
            slot: slot,
    ...
    public ISdfCameraRig ResolveRig(int slot, ISdfCameraRig chase, in SdfAnchor anchor, float time, float deltaSeconds) {
        ...
        if (m_drag.IsDragging(slot: slot)) {
            ...
            var pivot = (OrbitPivotSource?.Invoke(arg: slot) ?? (anchor.Position + OrbitPivotLift));
    ...
    public float StepOrbitDistance(int slot, bool zoomIn) {
        ...
        return SetOrbitDistance(
            distance: (zoomIn
            ? (seat.OrbitDistance / OrbitZoomStepFactor)
            : (seat.OrbitDistance * OrbitZoomStepFactor)),
            slot: slot
        );
    }
