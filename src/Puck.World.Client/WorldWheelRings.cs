...
/// <see cref="BindingWheelDefinition.HubLabelKey"/> is the one reserved key.
/// <para>The rings are rebuilt only when the wheel, the mirror, the mirror's installed document or its registered
/// bindings (<see cref="WorldStateMirror.Generation"/>) change, or a
...
public sealed class WorldWheelRings(Func<string?, OverlayResolvedGlyph> resolveIcon) {
    ...
    /// <summary>Gets the hub's label as of the last <see cref="Resolve"/>: the label row's <see cref="BindingWheelDefinition.HubLabelKey"/>
    ...
    private void Build(BindingWheelView wheel, WorldStateMirror? mirror) {
        ...
        m_generation = (mirror?.Generation ?? 0);
