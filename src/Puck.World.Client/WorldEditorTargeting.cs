...
public readonly record struct EditorSelection(WorldSection Section, string Id, int Index) {
...
public sealed class WorldEditorTargeting {
    ...
    private int GatherCandidates(int slot) {
        ...
        m_sortScratch.AsSpan(
            length: count,
            start: 0
        ).Sort(comparison: static (a, b) => a.DistanceSquared.CompareTo(value: b.DistanceSquared));
                ...
                foreach (var camera in definition.Cameras) {
                    if (string.Equals(
                        ...
                        return (camera.Anchor switch {
                            ...
                            WorldAnchor.EntityPart part when WorldEntityPartResolver.TryAuthoredPose(
                            client: m_client,
                            stamps: m_stamps,
                            ...
                            pose: out var pose
                                ...
                                (pose.Position + Vector3.Transform(
                            value: motionPosition,
                            rotation: pose.Orientation
                        )),
                            WorldAnchor.Placement placement => (WorldAnchorGeometry.StaticPlacementPosition(
                ...
                foreach (var speaker in definition.Speakers) {
                    if (string.Equals(
                        ...
                        return (speaker switch {
                            ...
                            WorldSpeaker.Anchored { Anchor: WorldAnchor.EntityPart part } anchored when WorldEntityPartResolver.TryAuthoredPose(
                            client: m_client,
                            stamps: m_stamps,
                            entityIndex: part.Index,
                            partId: part.PartId,
                            pose: out var pose
                                ...
                                (pose.Position + Vector3.Transform(
                            value: anchored.Offset,
                            rotation: pose.Orientation
                        )),
                            WorldSpeaker.Anchored { Anchor: WorldAnchor.Placement placementAnchor } anchored => AnchoredPlacementPosition(
                            definition: definition,
                            placementId: placementAnchor.PlacementId,
    ...
    public (EditorSelection Selection, float Distance, int Count)? Cycle(int slot, int direction) {
        ...
        if (Selected(slot: slot) is { } current) {
            for (var rank = 0; (rank < span.Length); rank++) {
                if (Matches(
                    target: in targets[span[rank].Target],
    ...
    public bool Deselect(int slot) {
        if (
            (((uint)slot) >= ((uint)m_selected.Length)) ||
            (m_selected[slot] is null)
        ) {
            return false;
        }
    ...
    public bool IsPlacementSelected(string id) {
        foreach (var selection in m_selected) {
            if (
                (selection is { Section: WorldSection.Placements } selected) &&
                string.Equals(
                a: selected.Id,
                b: id,
                comparisonType: StringComparison.Ordinal
    ...
    public EditorSelection? Selected(int slot) {
        if (
            (((uint)slot) >= ((uint)m_selected.Length)) ||
            (m_selected[slot] is not { } selection)
        ) {
            return null;
        }
    ...
    public Vector3? SelectionPosition(int slot) {
        return ((Selected(slot: slot) is { } selection)
            ? ResolvePosition(selection: in selection)
            : null
        );
    }
    ...
    public bool TryPick(int slot, out EditorSelection selection) {
        ...
        if (!m_picker.TryPick(
            eye: m_session.Eye(slot: slot),
            direction: m_session.Facing(slot: slot),
