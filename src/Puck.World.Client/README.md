...
submitted intents. Poses arrive in per-tick `WorldSnapshot`s; intents,
commands, and session requests leave over `IServerLink`. The client never
writes an authoritative pose. Presentation may use floating point, while
the server owns deterministic state and evaluates submitted intents.
The project references `Puck.World.Protocol` for the wire
