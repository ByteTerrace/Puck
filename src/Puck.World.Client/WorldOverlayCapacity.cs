...
/// <see cref="OverlayCapacity"/>: every host that composes the overlay package (<see cref="Puck.Overlays.OverlayPackage"/>)
/// over a world calls <see cref="FromSchema"/> rather than restating a number, so the seat count and the HUD ceilings
/// cross from <c>Puck.World.Schema</c> to <c>Puck.Overlays</c> as constructor data.</summary>
