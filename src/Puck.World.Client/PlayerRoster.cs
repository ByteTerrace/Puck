...
/// <see cref="PlayerRoster.JoinPending(int, ParticipantOrigin, Principal)"/>).</summary>
...
/// </para>
/// </remarks>
public sealed partial class PlayerRoster : IInputSlotResolver, IPrincipalResolver {
    ...
    /// today).
    /// <c>WorldServer.Body(int)</c> answers <see langword="null"/> for any negative index, so a
    ...
    // The session wire seat occupancy mirrors over, and every pose/grant/catalog read the roster needs resolves
    ...
    // The document's own compiled channel shapes (WorldPopulation.CompileFixedTables), fetched once via
    // WorldQuery.PopulationChannels — the same table Fill/OccupySeat always seated a fresh participant with.
    private readonly WorldChannelTable m_populationChannels;
    ...
    private readonly WorldPlayerDefaults m_playerDefaults;
    ...
    private Func<int, Principal, bool>? m_leave;
    ...
    /// <inheritdoc/>
    /// <remarks>The roster is the host's answer to <see cref="IPrincipalResolver"/>: the snapshot mixer asks
    ...
    Principal IPrincipalResolver.PrincipalOf(int slot) {
        return (PrincipalOf(slot: slot));
    }

    /// <summary>Installs the process-local departure router that resolves a roster seat's current world instance.
    /// Exactly one <c>WorldInstanceHost</c> owns this handoff; a second installation is a composition error.</summary>
    ...
    public static int DisplayNumber(int slot) => (slot + 1);
    ...
    public static int SlotFromDisplay(int number) => (number - 1);
    ...
    private WorldIdentity[] QueryCatalog() {
        ...
        m_link.Query(
            query: new WorldQuery.ProfileCatalog(),
            completion: answer => projections = (answer.Payload as IReadOnlyList<WorldIdentityProjection>)
        );
        ...
        var catalog = new WorldIdentity[projections.Count];

        for (var index = 0; (index < projections.Count); index++) {
            catalog[index] = WorldIdentity.FromProjection(
    ...
    // Whether a device holds a name-stable identity-preselection match against the catalog — the query form of
    // Server.WorldOwnedWorlds.PreferredProfile, resolved against THIS roster's own m_catalog by name so the returned
    // identity is the SAME object CycleCandidate/IsProfileActiveElsewhere compare against.
    private WorldIdentity? QueryPreferredProfile(InputDeviceId device) {
        string? name = null;

        m_link.Query(
            query: new WorldQuery.PreferredControllerProfile(Device: device),
    ...
    private GrantVerdict QueryGrantAllows(Principal principal, WorldCapability capability, GrantSubject subject) {
        var verdict = default(GrantVerdict);
        ...
        m_link.Query(
            ...
            completion: answer => verdict = ((answer.Payload as GrantVerdict?) ?? default)
        );

        return verdict;
    }
    // The query form of Server.WorldHandleTable.TryMint, for a claimed slot's own (principal, capability) table —
    // see DriveTarget's own remarks for why a caller caches the returned handle across ticks rather than minting one
    // fresh every call.
    private WorldHandle? QueryHandleMint(Principal principal, WorldCapability capability, int index) {
        WorldHandle? handle = null;

        m_link.Query(
            query: new WorldQuery.GrantHandleMint(
                Capability: capability,
                Index: index,
                Principal: principal
            ),
            completion: answer => handle = (answer.Payload as WorldHandle?)
        );

        return handle;
    }
    // The query form of Server.WorldHandleTable.TryResolve, against the handle's OWN (TablePrincipal,
    // TableCapability) table.
    private GrantSubject? QueryHandleResolve(WorldHandle handle) {
        GrantSubject? subject = null;

        m_link.Query(
            query: new WorldQuery.GrantHandleResolve(Handle: handle),
            completion: answer => subject = (answer.Payload as GrantSubject?)
        );
    ...
    // every other roster op follows. actingPrincipal is the ASKING principal (context.Principal for a
    // console dispatch, SelfProvisioned for a physical press) — mandatory and threaded from Confirm.
    private bool Activate(int slot, Participant participant, Principal actingPrincipal, InputDeviceId? device) {
        var finalProfile = (IsProfileActiveElsewhere(
            profile: participant.Seat.Profile,
            exceptSlot: slot
        )
        ...
        m_link.SubmitSession(
            request: new SessionRequest.SetIdentity(
                Principal: actingPrincipal,
                Slot: slot,
                IdentityName: finalProfile.Name
            ),
            completion: reply => {
                if (!reply.Accepted) {
                ...
                if (device is { } confirmedBy) {
                    m_link.SubmitSession(
                        request: new SessionRequest.RememberPreferredController(
                            Principal: actingPrincipal,
                            Device: confirmedBy,
                            IdentityName: finalProfile.Name
                        ),
    ...
    private void DissolveIfOrphanedDevice(int slot, Principal actingPrincipal) {
        if (
    ...
    private DevicePreselection EvaluatePreselection(InputDeviceId device) {
        ...
        if (device.Persistence != InputDeviceIdentityPersistence.Reconnect) {
            return new DevicePreselection(
                Kind: DevicePreselectionKind.ConnectionOnly,
                Profile: null,
                Seat: -1
            );
        }

        if (QueryPreferredProfile(device: device) is not { } preferred) {
            return new DevicePreselection(
                Kind: DevicePreselectionKind.NoLocalPreference,
                Profile: null,
                Seat: -1
            );
        }
        ...
        if (activeSeat >= 0) {
            return new DevicePreselection(
                Kind: DevicePreselectionKind.AlreadySeated,
                Profile: preferred,
                Seat: activeSeat
            );
        }
        ...
        return new DevicePreselection(
            Kind: ((FirstFreeUnclaimedSlot() >= 0)
            ? DevicePreselectionKind.Preferred
            : DevicePreselectionKind.NoFreeSeat),
            Profile: preferred,
            Seat: -1
        );
    }
    ...
    // minted a body for. actingPrincipal is MANDATORY and EXPLICIT at every call site — context.Principal
    ...
    // value).
    private bool Fill(int slot, WorldIdentity profile, ParticipantState state, ParticipantOrigin origin, Principal actingPrincipal) {
    ...
    private WorldIdentity FirstUnusedProfile(int exceptSlot) {
        foreach (var profile in m_catalog) {
            if (!IsProfileActiveElsewhere(
                exceptSlot: exceptSlot,
                profile: profile
            )) {
                return profile;
            }
    ...
    // The catalog index of a profile (by identity, not by reference — m_catalog is rehydrated from a projection, so
    // two reads of "the same" identity are never the same CLR object), or 0 when it is not found.
    ...
    private bool IsProfileActiveElsewhere(WorldIdentity? profile, int exceptSlot) {
        ...
        for (var slot = 0; (slot < MaxSlots); slot++) {
            if (
                (slot != exceptSlot) &&
                (m_slots[slot] is { State: ParticipantState.Active } participant) &&
                SameProfile(
                a: participant.Seat.Profile,
                b: profile
            )
            ) {
                return true;
    ...
    private JoinResult JoinPendingCandidate(int slot, ParticipantOrigin origin, Principal actingPrincipal, WorldIdentity? candidate) {
        if (
            (((uint)slot) >= MaxSlots) ||
        ...
        var profile = (((candidate is not null) && !IsProfileActiveElsewhere(
            exceptSlot: slot,
            profile: candidate
        ))
            ? candidate
    ...
    private void PendingPicker(Participant participant, int slot, FixedQ4816 stickX) {
        ...
        if (
            ...
            CycleCandidate(
                slot: slot,
                direction: ((stickX < FixedQ4816.Zero)
                ? -1
                : 1)
            );
        }
    ...
    private WorldIdentity? PreferredCandidateFor(int slot) {
        foreach (var device in m_deviceOrder) {
            if (
                ...
                (mapped == slot) &&
                m_devicePreselections.TryGetValue(
                key: device,
    ...
    private Participant? ResolveDeviceSlot(InputDeviceId device) {
        ...
        if (
            ...
            (FirstFreeUnclaimedSlot() is var preferredSlot) &&
            (preferredSlot >= 0)
        ) {
            if (JoinPendingCandidate(
                slot: preferredSlot,
                origin: ParticipantOrigin.Device,
                actingPrincipal: SelfProvisioned(slot: preferredSlot),
                candidate: preferred
            ) != JoinResult.Ok) {
                return null;
            }
        ...
        var (result, slot) = JoinPendingNextFree(
            actingPrincipal: SelfProvisioned,
            origin: ParticipantOrigin.Device
        );
    ...
    private void RouteMove(int slot, Participant participant, FixedVector2 value, SeatMoveBehavior behavior) {
    ...
    /// <c>context.Principal</c> instead, so a third party's action is checked under the real submitter,
    ...
    private static Principal SelfProvisioned(int slot) => Principal.Seat(slot: slot);
    ...
    /// (<c>context.Principal</c>), consumed here, never constructed: Console for a console
    ...
    public AssignOutcome AssignDevice(InputDeviceId device, int targetSlot, Principal actingPrincipal) {
        ...
        var hadCurrent = m_deviceToSlot.TryGetValue(
            key: device,
        ...
        var effectivePrincipal = (hadCurrent
            ? actingPrincipal
            : SelfProvisioned(slot: targetSlot)
        );
        ...
        if (
            hadCurrent &&
            WouldOrphanOnMove(slot: current) &&
            (QueryGrantAllows(
            principal: actingPrincipal,
        ...
        if (m_slots[targetSlot] is not null) {
            ...
            if (JoinPending(
                actingPrincipal: effectivePrincipal,
                origin: ParticipantOrigin.Device,
                slot: targetSlot
            ) != JoinResult.Ok) {
                return AssignOutcome.Denied;
    ...
    public Vector3 BodyColor(int slot) {
        ...
        return ((participant.State == ParticipantState.Active)
            ? color
            : Vector3.Lerp(
                amount: m_pickerNeutralBlend,
                value1: color,
                value2: m_pickerNeutralColor
            )
        );
    }
    ...
    /// this writes is exactly what a downstream Device-origin join (e.g. <see cref="RouteMove(int, FixedVector2, SeatMoveBehavior, Principal)"/>'s or
    ...
    public bool CommitSlot(InputDeviceId device, int slot) {
        if (
            m_deviceToSlot.ContainsKey(key: device) ||
            (ResolveSlot(device: device) != slot)
        ) {
            return false;
        }
    ...
    /// <param name="actingPrincipal">The principal asking to confirm — <c>context.Principal</c>. Only
    ...
    public (ConfirmOutcome Outcome, int Slot) Confirm(InputDeviceId device, Principal actingPrincipal) {
    ...
    /// <param name="actingPrincipal">The principal asking to confirm — <c>context.Principal</c> for a
    ...
    public (ConfirmOutcome Outcome, int Slot) Confirm(int slot, Principal actingPrincipal, InputDeviceId? device) {
        if (
            (((uint)slot) >= MaxSlots) ||
            (m_slots[slot] is not { } participant)
        ) {
            return (Outcome: ConfirmOutcome.Ignored, Slot: -1);
        }
    ...
    /// <param name="actingPrincipal">The principal asking to cycle — <c>context.Principal</c>, the caller's
    ...
    /// <see cref="AssignDevice(InputDeviceId, int, Principal)"/>'s remarks); for a wholly unbound device this
    /// call never reaches <see cref="AssignDevice(InputDeviceId, int, Principal)"/> at all (the first-touch
    ...
    public (AssignOutcome Outcome, int Slot) CycleDevice(InputDeviceId device, Principal actingPrincipal) {
    ...
    public string Describe() {
        ...
        for (var slot = 0; (slot < MaxSlots); slot++) {
            ...
            var state = ((participant.State == ParticipantState.Active)
                ? "active"
                : "PENDING"
            );
            var owners = DeviceTokensFor(slot: slot);
            var inside = ((owners.Length > 0)
                ? owners
                : OriginWord(origin: participant.Origin)
            );
        ...
        return $"[world.players: {string.Join(
            separator: " | ",
            values: segments
        )}]";
    }
    ...
    /// Drive <c>WorldHandleTable</c> (<c>WorldGrants.HandleTable</c>) rather than a raw
    /// grant-table lookup. <see cref="TryClaimSlot"/> itself accepts any <see cref="Principal"/> — it is a
    ...
    /// not fail there — it would throw <see cref="ArgumentException"/> here, out of the <c>WorldGrants.HandleTable</c>
    ...
    /// <see cref="GrantSubject.All"/> wildcard; <c>WorldGrants.ProjectSubjects</c> additionally never
    ...
    /// <c>WorldHandleTable.TryResolve</c>'s generation check load-bearing — the property that a stale
    ...
    /// <c>WorldHandleTable.EnsureFresh</c>, private) and still names the same subject the lease locked in when it
    ...
    public int DriveTarget(int slot) {
        if (
            (((uint)slot) >= MaxSlots) ||
            (m_slotPrincipal[slot] is not { } claimant)
        ) {
            return slot;
        }

        if (
            (m_slotDriveHandle[slot] is { } cached) &&
            (QueryHandleResolve(handle: cached) is { } cachedSubject)
        ) {
            ...
            if (
                (cachedSubject.Kind == GrantSubjectKind.Body) &&
                (cachedSubject == m_slotDriveSubject[slot])
            ) {
        ...
        if (
            ...
            (QueryHandleResolve(handle: handle) is { } subject) &&
    ...
    public JoinResult JoinActive(int slot, WorldIdentity profile, ParticipantOrigin origin, Principal actingPrincipal) {
        if (
            (((uint)slot) >= MaxSlots) ||
            (m_slots[slot] is not null)
        ) {
    ...
    public (JoinResult Result, int Slot) JoinActiveNextFree(WorldIdentity profile, ParticipantOrigin origin, Func<int, Principal> actingPrincipal) {
        ...
        return (Result: JoinActive(
            slot: slot,
            profile: profile,
            origin: origin,
            actingPrincipal: actingPrincipal(arg: slot)
        ), Slot: slot);
    }
    ...
    /// <param name="actingPrincipal">The principal asking to join — <c>context.Principal</c> for a console
    ...
    /// <returns><see cref="JoinResult.Occupied"/> if out of range or already joined, <see cref="JoinResult.Denied"/>
    /// if the server refused the actor, else <see cref="JoinResult.Ok"/>.</returns>
    public JoinResult JoinPending(int slot, ParticipantOrigin origin, Principal actingPrincipal) {
        return JoinPendingCandidate(
            slot: slot,
            origin: origin,
            actingPrincipal: actingPrincipal,
    ...
    /// constant-returning lambda over <c>context.Principal</c> for a console dispatch. Mandatory.</param>
    /// <returns>The result, plus the joined slot index (0-based, valid only when the result is
    /// <see cref="JoinResult.Ok"/>).</returns>
    public (JoinResult Result, int Slot) JoinPendingNextFree(ParticipantOrigin origin, Func<int, Principal> actingPrincipal) {
        var slot = FirstFreeSlot();
    ...
    /// <param name="actingPrincipal">The principal asking to leave — <c>context.Principal</c> for a console
    ...
    public bool Leave(int slot, Principal actingPrincipal) {
        if (
            (slot <= 0) ||
            (slot >= MaxSlots) ||
            (m_slots[slot] is null)
    ...
    /// <summary>The slot's acting identity: the <see cref="Principal"/> a claim recorded via
    /// <see cref="TryClaimSlot"/>, or its ordinary <see cref="Principal.Seat"/> when nothing has claimed it. The
    ...
    /// <param name="slot">The slot index (0-based).</param>
    public Principal PrincipalOf(int slot) {
        return (((((uint)slot) < MaxSlots)
            ? m_slotPrincipal[slot]
            : null) ?? Principal.Seat(slot: slot));
    }
    /// <summary>The profile the slot's participant selects, or <see langword="null"/> for an empty slot.</summary>
    /// <param name="slot">The slot index (0-based).</param>
    public WorldIdentity? ProfileAt(int slot) => ((((uint)slot) < MaxSlots)
        ? m_slots[slot]?.Seat.Profile
        : null
    );
    /// <inheritdoc/>
    ...
    public int ResolveSlot(InputDeviceId device) {
        ...
        if (
            (EvaluatePreselection(device: device) is { Kind: DevicePreselectionKind.Preferred }) &&
            (FirstFreeUnclaimedSlot() is var preferredSlot) &&
            (preferredSlot >= 0)
        ) {
    ...
    /// <param name="slot">The logical player slot recorded in the command snapshot.</param>
    /// <param name="value">The already-quantized stick sample (+X looks right, +Y looks up).</param>
    /// <param name="behavior">How horizontal look affects body facing.</param>
    /// <param name="actingPrincipal">The principal asking — see <see cref="RouteMove(int, FixedVector2, SeatMoveBehavior, Principal)"/>'s
    /// identical remark. Mandatory.</param>
    public void RouteLook(int slot, FixedVector2 value, SeatLookBehavior behavior, Principal actingPrincipal) {
    ...
    /// <param name="actingPrincipal">The principal asking — <c>context.Principal</c>, which already resolves
    ...
    public void RouteMove(int slot, FixedVector2 value, SeatMoveBehavior behavior, Principal actingPrincipal) {
        if (((uint)slot) >= MaxSlots) {
            return;
        }

        // An unmapped device's first touch joins the SAME lane it is arriving on, under its own acting identity —
        // actingPrincipal already resolves through PrincipalOf(slot) at the caller (context.Principal), so
    ...
    /// <param name="slot">The slot index (0-based).</param>
    public SeatController? Seat(int slot) => ((((uint)slot) < MaxSlots)
        ? m_slots[slot]?.Seat
        : null
    );
    ...
    /// <param name="slot">The slot index (0-based).</param>
    /// <param name="profile">The profile to seat on.</param>
    /// <param name="actingPrincipal">The principal asking to set the profile — <c>context.Principal</c> for a
    /// console dispatch, or <see cref="SelfProvisioned"/> for a device/confirm gesture. Mandatory.</param>
    /// <returns>The set outcome.</returns>
    public SetProfileOutcome SetProfile(int slot, WorldIdentity profile, Principal actingPrincipal) {
        if (m_slots[slot] is not { } participant) {
    ...
    /// roster-identity gesture (see <see cref="Confirm(InputDeviceId, Principal)"/>/<see cref="CycleDevice"/>), the same exclusion
    ...
    public bool TryClaimSlot(InputDeviceId device, Principal principal, int? preferredSlot, out int slot, out string? fault) {
    ...
    // means the slot submits under its own Principal.Seat as usual. WorldClient.SubmitAuthorityIntents reads this
    ...
    private readonly Principal?[] m_slotPrincipal = new Principal?[MaxSlots];
    ...
    /// <param name="link">The client→server link session requests, and every pose/grant/catalog read, ride.</param>
    /// <param name="seatBindings">The per-seat input resolver the roster pushes profile binding layers into.</param>
    /// <exception cref="ArgumentNullException">An argument is <see langword="null"/>.</exception>
    public PlayerRoster(WorldDefinition definition, IServerLink link, WorldSeatBindings seatBindings) {
        ...
        WorldChannelTable? channels = null;

        m_link.Query(
            query: new WorldQuery.PopulationChannels(),
            completion: answer => channels = (answer.Payload as WorldChannelTable)
        );
        ...
        // Pending — there is no seat, or nothing to fill it with.
        var seatActivation = definition.Population.SeatActivation;
        ...
        if (
            ...
            Fill(
                ...
                profile: m_catalog[0],
                state: ParticipantState.Active,
                origin: ParticipantOrigin.Permanent,
                actingPrincipal: SelfProvisioned(slot: 0)
            );
        }

        for (var slot = 1; ((slot < MaxSlots) && (slot < seatActivation.Count)); slot++) {
            if (seatActivation[slot] != SeatActivationPolicy.Eager) {
                continue;
            }

            Fill(
                slot: slot,
                profile: FirstUnusedProfile(exceptSlot: -1),
                state: ParticipantState.Active,
                origin: ParticipantOrigin.Permanent,
                actingPrincipal: SelfProvisioned(slot: slot)
            );
        }
    }
