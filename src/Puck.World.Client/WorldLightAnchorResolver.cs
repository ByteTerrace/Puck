...
public static class WorldLightAnchorResolver {
    private static SdfAnchor? ResolvePlacement(WorldAnchor.Placement anchor, WorldClient client, WorldStampPool stamps, ReadOnlySpan<DynamicTransform> transforms) {
        if (
            stamps.TryShapeTransformSlot(
            anchor.PlacementId,
            anchor.ShapeId,
            client,
            out var slot
        ) &&
            (((uint)slot) < ((uint)transforms.Length))
        ) {
            return stamps.ResolvePlacementSlotPose(
                anchor.PlacementId,
                anchor.ShapeId,
                client,
                transforms[slot]
        ...
        var placement = client.Definition.Placements.FirstOrDefault(predicate: p => (p.Id == anchor.PlacementId));
        ...
        if (
            (creation is null) ||
            !WorldPlacementStamper.IsStaticStamp(
            creation: creation,
            placement: placement
        )
        ) { return null; }
        ...
        return new(
            (frame.Position + Vector3.Transform(
                (shape.Position.Value * placement.Scale),
                rotation
            )),
            Quaternion.Normalize(value: (rotation * shape.Rotation.Value))
        );
    }
    ...
    public static SdfAnchor? Resolve(WorldAnchor anchor, WorldClient client, WorldStampPool stamps, ReadOnlySpan<DynamicTransform> transforms) {
        switch (anchor) {
            case WorldAnchor.Entity entity:
                if (!client.IsActive(index: entity.Index)) { return null; }
                if (
                    stamps.TryBodyTransformSlot(
                    bodyIndex: entity.Index,
                    transformSlot: out var root
                ) &&
                    (((uint)root) < ((uint)transforms.Length))
                ) {
                    return new(
                        transforms[root].Position,
                        transforms[root].Orientation
                    );
                }
                return new(
                    client.Position(index: entity.Index),
