...
/// <summary>
/// Deterministically generates a random-but-renderable <see cref="SdfProgram"/> from a seed, for the POST's
/// cross-backend differential-fuzzing stage. VALUES AND ALGORITHM PORTED EXACTLY from the demo's
/// <c>src/Puck.Demo/FuzzSdfProgram.cs</c> (the worked reference), never code-shared, so a POST seed reproduces the
/// identical program the demo's <c>--validate-world --fuzz-seed N</c> gate renders — the standing cross-check. The
/// same seed (and bounds) always yields the same program (so a finding is reproducible), and the program is fed
/// identically to both backends — any divergence in the rendered result is a backend bug (typically shader codegen
/// of a shape SDF, a smooth blend, or a transcendental).
/// <para>
...
/// Every parameter range comes from a <see cref="ShapeBounds"/>, the SAME envelope the scene validator gates
/// authoring against.
/// </para>
/// </summary>
