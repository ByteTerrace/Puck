...
/// <summary>Helpers for comparing the per-tick state-hash traces a determinism/replay stage produces.</summary>
internal static class HashTrace {
    /// <summary>Returns the first tick at which two traces differ, or <c>-1</c> when they are identical (length
    /// included).</summary>
    /// <param name="left">The first trace.</param>
    /// <param name="right">The second trace.</param>
    /// <returns>The index of the first divergence, the shorter length when one is a prefix of the other, or <c>-1</c>
    /// when equal.</returns>
