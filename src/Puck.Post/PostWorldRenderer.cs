...
internal sealed class PostWorldRenderer : IDisposable {
    ...
    private const uint TimingCapacity = 4; // 2 marks used (frame open/close); headroom for the reset range
    private readonly IGpuTimingPool? m_timingPool;
    ...
    public PostWorldRenderer(IGpuComputeServices gpu, IGpuDeviceContext device, string bytecodeExtension, SdfProgram program, uint width, uint height, int dynamicTransformCapacity = 1, IGpuTimingPoolFactory? timingFactory = null, IGpuTimingRecorder? timingRecorder = null) {
        ...
        // Opt-in GPU timing (D4): when a timing factory + recorder are supplied AND the device supports timestamps,
        // each RenderFrame brackets its whole command buffer with a top/bottom-of-pipe timestamp so
        // LastFrameGpuMilliseconds reports the pass's GPU time. Callers that omit them (every other stage) render
        // untimed and unchanged.
        if ((timingFactory is not null) && (timingRecorder is not null)) {
            m_timingCapabilities = timingFactory.GetCapabilities(deviceContext: device);

            if (m_timingCapabilities.IsSupported) {
                m_timingPool = timingFactory.CreateTimestampPool(deviceContext: device, queryCapacity: TimingCapacity);
    ...
    /// enabled at construction (a top/bottom-of-pipe timestamp bracketing the whole beam→cull→views→composite submit),
    /// or <see langword="null"/> when timing is disabled or the timestamps were not yet readable.</summary>
    ...
    public byte[] RenderFrame(SdfFrame frame) {
        ...
        if (m_timingEnabled) {
            ...
            m_lastFrameGpuMilliseconds = ((m_timingRecorder!.ReadTimestamps(deviceHandle: m_deviceHandle, poolHandle: m_timingPool!.PoolHandle, firstQuery: 0, queryCount: 2, rawTicks: ticks) < 2)
                ? null
                : m_timingCapabilities.TicksToMilliseconds(startTicks: ticks[0], endTicks: ticks[1]));
        }
    ...
    private void Record(uint viewportCount) {
        ...
        if (m_timingEnabled) {
            m_timingRecorder!.ResetTimestamps(deviceHandle: m_deviceHandle, commandBufferHandle: commandBuffer, poolHandle: m_timingPool!.PoolHandle, firstQuery: 0, queryCount: TimingCapacity);
            m_timingRecorder.WriteTimestamp(deviceHandle: m_deviceHandle, commandBufferHandle: commandBuffer, poolHandle: m_timingPool.PoolHandle, queryIndex: 0, stageFlags: GpuTimingStage.TopOfPipe);
        }
        ...
        if (m_timingEnabled) {
            m_timingRecorder!.WriteTimestamp(deviceHandle: m_deviceHandle, commandBufferHandle: commandBuffer, poolHandle: m_timingPool!.PoolHandle, queryIndex: 1, stageFlags: GpuTimingStage.BottomOfPipe);
            m_timingRecorder.ResolveTimestamps(deviceHandle: m_deviceHandle, commandBufferHandle: commandBuffer, poolHandle: m_timingPool.PoolHandle, firstQuery: 0, queryCount: 2);
        }
