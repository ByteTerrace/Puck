...
/// <summary>The aggregate outcome of a battery run: the per-stage rows, the folded exit code, and a human-readable
/// table written to the artifacts directory and echoed to the console.</summary>
internal sealed class PostReport {
    /// <summary>Initializes a new instance of the <see cref="PostReport"/> class, folding the per-stage verdicts into
    /// an exit code (any infra → 2, else any fail → 1, else 0; skips are neutral).</summary>
    ...
    public string Render() {
        ...
        _ = builder.AppendLine(value: "Puck.Post — engine power-on self-test");
