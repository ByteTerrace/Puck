...
internal sealed class DriftHuntRenderNode : IRenderNode {
    ...
    private static SdfFrame BuildHuntFrame(SdfProgram program) {
        var camera = CameraSnapshot.LookAt(
            position: new Vector3(0.5f, 3.4f, 9f),
    ...
    private void RenderAndReport(IGpuDeviceContext? gpuDevice) {
        try {
            ...
            using var postContext = new PostContext(services: m_services, gpuDevice: gpuDevice, artifactsDirectory: (m_artifactsDirectory ?? Path.Combine("artifacts", "drift-hunt")));
    ...
    private void WriteChampionArtifacts(string directory, byte[] vulkanPixels, byte[] directXPixels, ParityMetrics metrics, double score) {
        ...
        PngEncoder.Write(height: (int)WorldHeight, path: Path.Combine(directory, $"{stem}-vulkan.png"), rgba: vulkanPixels, width: (int)WorldWidth);
        PngEncoder.Write(height: (int)WorldHeight, path: Path.Combine(directory, $"{stem}-directx.png"), rgba: directXPixels, width: (int)WorldWidth);
        ParityCheck.WriteDiffImage(comparand: directXPixels, height: (int)WorldHeight, path: Path.Combine(directory, $"{stem}-diff-amp{DiffAmplification}x.png"), reference: vulkanPixels, width: (int)WorldWidth);

        // The one-command repro: the seed reproduces the program deterministically (the exotic ops are not authorable
        // from a run-document — backlog item 16 — so a seed recipe, not a run-doc, is the honest repro). Hand-written
        ...
        File.WriteAllText(path: Path.Combine(directory, $"{stem}-repro.json"), contents: json);
    }
}
