...
internal static class DriftHuntProgram {
    ...
    public static SdfProgram Generate(int seed) {
        ...
        var seamA = builder.AddMaterial(material: new SdfMaterial(Albedo: new Vector3(0.96f, 0.9f, 0.72f), Emissive: 0.85f));
        var seamB = builder.AddMaterial(material: new SdfMaterial(Albedo: new Vector3(0.25f, 0.5f, 0.96f), Emissive: 0.85f));
    ...
    private static void EmitMaterialSeam(SdfProgramBuilder builder, Random random, int materialA, int materialB) {
        ...
        var axis = new Vector3(x: MathF.Cos(angle), y: 0f, z: MathF.Sin(angle));
    ...
    private static void EmitLogSphereDroste(SdfProgramBuilder builder, Random random, int[] palette) {
        ...
        if (random.NextDouble() < 0.5) {
            ...
            _ = builder.Box(halfExtents: new Vector3(0.5f, 0.18f, 0.5f), round: 0.05f, material: material);
        }
    }
    ...
    private static void EmitWallpaper(SdfProgramBuilder builder, Random random, int[] palette) {
        ...
        _ = builder
            ...
            .WallpaperFold(group: group, cell: cell, limit: new Vector2(2f, 2f), materialStride: 1)
    ...
    private static void EmitSmoothChamferChain(SdfProgramBuilder builder, Random random, int[] palette) {
        ...
        _ = builder.ResetPoint().Translate(offset: new Vector3(baseX, baseY, baseZ)).Sphere(radius: 0.4f, material: material);
        ...
        for (var link = 1; (link < links); link++) {
            ...
            _ = builder
                .ResetPoint()
                .Translate(offset: new Vector3((baseX + (link * 0.34f)), (baseY + (chamfer ? 0.06f : -0.03f)), baseZ))
