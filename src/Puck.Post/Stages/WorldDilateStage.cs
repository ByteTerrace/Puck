...
internal sealed class WorldDilateStage : IPostStage {
    ...
    internal static SdfProgram BuildDilateScene() {
        ...
        var copper = builder.AddMaterial(material: new SdfMaterial(Albedo: new Vector3(x: 0.82f, y: 0.45f, z: 0.2f)));
        var jade = builder.AddMaterial(material: new SdfMaterial(Albedo: new Vector3(x: 0.25f, y: 0.72f, z: 0.5f)));

        return builder
            // UNSCOPED dilate (the unmaskable path), emitted FIRST so it inflates only its own turret, not the floor.
            .Translate(offset: new Vector3(x: -1.8f, y: 0.6f, z: 0f))
            .Box(halfExtents: new Vector3(x: 0.42f, y: 0.42f, z: 0.42f), round: 0f, material: copper)
            .ResetPoint()
            .Translate(offset: new Vector3(x: -1.8f, y: 1.15f, z: 0f))
            ...
            .Translate(offset: new Vector3(x: 1.8f, y: 0.6f, z: 0f))
            .Box(halfExtents: new Vector3(x: 0.42f, y: 0.42f, z: 0.42f), round: 0f, material: jade)
            .ResetPoint()
            .Translate(offset: new Vector3(x: 1.8f, y: 1.15f, z: 0f))
