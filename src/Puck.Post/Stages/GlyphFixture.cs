...
internal static class GlyphFixture {
    ...
    /// <summary>Builds the DECAL-PROOF variant: RGB carry the true field, but ALPHA is zeroed (fully outside) — a
    /// deliberate violation of the marchable-alpha contract that no geometry may ever bind. The decal tier reconstructs
    /// with median-of-3 over RGB, so it renders this atlas identically to <see cref="Build"/>; an alpha-only
    /// decoder would see empty cells and light zero text pixels, failing the legibility floor. This is the signal that
    /// distinguishes "medians RGB" from "reads alpha" — a threshold no calibration drift can blur.</summary>
    public static FontAtlas BuildDecalProof() {
        var atlas = Build();
        var image = atlas.ImageData!;
        var rgba = (byte[])image.RgbaPixels.Clone();
        ...
        return new FontAtlas(
            ...
            glyphs: atlas.Glyphs,
            height: atlas.Height,
            imageData: new FontAtlasImageData(height: image.Height, rgbaPixels: rgba, width: image.Width),
            imagePath: "post:glyph-fixture-decal-proof",
            kerningPairs: [],
            kind: atlas.Kind,
