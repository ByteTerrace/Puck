...
/// (docs/sdf-bench-notes.md, the 2026-07-09 carve ladder). This stage proves the grid mask EQUALS the flat mask by
...
/// instance classes the frozen grid cannot bin and therefore routes to the ALWAYS-tested list: one DYNAMIC floating
/// sphere (its centre resolves per frame) and one UNMASKABLE intersection sphere (its 1e30 bound passes every tile).
/// So the mask spans <b>7 words</b> (ceil(199/32)) and both the grid cell walk and the always-list are exercised end
/// to end.</para>
...
internal sealed class WorldGridCullStage : IPostStage {
    ...
    private static SdfProgram BuildScene(bool grid) {
        ...
        _ = builder.BeginInstance(boundCenter: new Vector3(-2.6f, 1.4f, -1.2f), boundRadius: 0.5f);
        _ = builder
            .ResetPoint()
            .Translate(offset: new Vector3(-2.6f, 1.4f, -1.2f))
        ...
        _ = builder
            ...
            .Box(halfExtents: new Vector3(3.2f, 0.5f, 3.2f), round: 0.05f, material: azure);
        ...
        for (var row = 0; (row < CarveRows); row++) {
            for (var column = 0; (column < CarveColumns); column++) {
                ...
                var jitterX = (0.12f * MathF.Sin(index * 12.9898f));
                var jitterZ = (0.12f * MathF.Sin(index * 78.233f));
                var position = new Vector3(
                    (-2.9f + ((5.8f / (CarveColumns - 1)) * column) + jitterX),
                    1.02f,
                    (-2.9f + ((5.8f / (CarveRows - 1)) * row) + jitterZ)
                );
    ...
    private static PostStageOutcome RunCore(PostContext context) {
        ...
        DynamicTransform[] transforms = [new DynamicTransform(Position: new Vector3(0.6f, 2.4f, 0.4f), Orientation: Quaternion.Identity)];
        ...
        var diffPath = Path.Combine(context.ArtifactsDirectory, "world-grid-cull-diff.png");

        PngEncoder.Write(height: (int)WorldHeight, path: Path.Combine(context.ArtifactsDirectory, "world-grid-cull-vulkan.png"), rgba: vulkanGridPixels, width: (int)WorldWidth);
        PngEncoder.Write(height: (int)WorldHeight, path: Path.Combine(context.ArtifactsDirectory, "world-grid-cull-directx.png"), rgba: directXGridPixels, width: (int)WorldWidth);
