...
internal sealed class CameraShareStage : IPostStage {
    ...
    private static PostStageOutcome RunCore(PostContext context) {
        ...
        var (varied, firstPixel, sampledPixel) = VulkanImportAndReadback(context: context, sharedHandle: exportable.SharedHandle);

        if (!varied) {
            return PostStageOutcome.Fail(detail: $"the Vulkan host imported the Direct3D 12-produced frame but it was spatially flat (every pixel 0x{firstPixel:X8}) — no content crossed the shared handle");
        }

        return PostStageOutcome.Pass(detail: $"{RenderSize}x{RenderSize} R8G8B8A8 | Direct3D 12 produced a synthetic frame (sdf-child) into a shared image (handle 0x{exportable.SharedHandle:X}); the Vulkan host imported it zero-copy and read back varying content (px0=0x{firstPixel:X8}, px_center=0x{sampledPixel:X8})");
    }
    ...
    private static (bool Varied, uint FirstPixel, uint SampledPixel) VulkanImportAndReadback(PostContext context, nint sharedHandle) {
        ...
        try {
            ...
            // Any pixel differing from pixel 0 proves the frame is not a flat fill — i.e. real produced content
            // crossed the shared handle (an all-same buffer, e.g. a failed/blank import, would not vary).
