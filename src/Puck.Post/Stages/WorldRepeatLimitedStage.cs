...
internal sealed class WorldRepeatLimitedStage : IPostStage {
    ...
    internal static SdfProgram BuildRepeatLimitedScene() {
        ...
        var brick = builder.AddMaterial(material: new SdfMaterial(Albedo: new Vector3(0.8f, 0.4f, 0.28f)));

        return builder
            .Plane(normal: Vector3.UnitY, offset: 0f, material: ground)
            .ResetPoint()
            .RepeatLimited(spacing: new Vector3(1.2f, 6.0f, 1.2f), limit: new Vector3(2f, 0f, 1f))
            .Translate(offset: new Vector3(0f, 0.55f, 0f))
            .Box(halfExtents: new Vector3(0.32f, 0.32f, 0.32f), round: 0.05f, material: brick)
            .Build();
    }
