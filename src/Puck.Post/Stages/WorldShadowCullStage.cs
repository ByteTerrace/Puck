...
internal sealed class WorldShadowCullStage : IPostStage {
    ...
    private static readonly Vector3 CorridorOccluder = new(x: 3.9f, y: 4.2f, z: 3.1f);
    ...
    private static SdfProgram BuildScene(bool withOccluders) {
        ...
        Span<Vector3> occluders = [
            new Vector3(x: -1.6f, y: 1.7f, z: 0.2f),
            new Vector3(x: 0.7f, y: 2.2f, z: 0.7f),
            new Vector3(x: 2.1f, y: 1.5f, z: -0.6f),
            new Vector3(x: -0.4f, y: 2.9f, z: -1.1f),
            CorridorOccluder,
        ];
    ...
    private static PostStageOutcome RunCore(PostContext context) {
        ...
        DynamicTransform[] transforms = [new DynamicTransform(Position: new Vector3(x: -0.9f, y: 2.5f, z: 1.4f), Orientation: Quaternion.Identity)];
        ...
        var diffPath = Path.Combine(path1: context.ArtifactsDirectory, path2: "world-shadow-cull-diff.png");

        PngEncoder.Write(height: (int)WorldHeight, path: Path.Combine(path1: context.ArtifactsDirectory, path2: "world-shadow-cull-vulkan.png"), rgba: vulkanCulled, width: (int)WorldWidth);
        PngEncoder.Write(height: (int)WorldHeight, path: Path.Combine(path1: context.ArtifactsDirectory, path2: "world-shadow-cull-directx.png"), rgba: directXCulled, width: (int)WorldWidth);
