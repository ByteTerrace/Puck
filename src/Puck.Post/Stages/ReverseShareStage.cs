...
internal sealed class ReverseShareStage : IPostStage {
    ...
    private static PostStageOutcome RunCore(PostContext context) {
        ...
        if (lastRed <= (firstRed + 64)) {
            return PostStageOutcome.Fail(detail: $"Direct3D 12 did not read back Vulkan's gradient from the shared resource (firstRed={firstRed}, lastRed={lastRed})");
        }

        return PostStageOutcome.Pass(detail: $"{RenderSize}x{RenderSize} R8G8B8A8 | Vulkan wrote gradient.comp INTO a Direct3D 12-owned shared image (handle 0x{exportable.SharedHandle:X}); Direct3D 12 read it back {firstRed}->{lastRed}");
    }
