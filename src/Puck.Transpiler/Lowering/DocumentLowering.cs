...
public static class DocumentLowering {
    ...
    internal static JsonNode? EvaluateValue(ExpressionNode expr, DocumentScope scope, string? fieldKey = null) {
        ...
        switch (expr) {
            ...
            case CallExpressionNode call: {
                    if (DocumentBuiltins.TryEvaluate(
                        call: call,
                        fieldKey: fieldKey,
            ...
            case RangeExpressionNode range: {
                    ...
                    rangeArr.AppendNode(item: LowerValue(
                        expr: range.Start,
                        scope: scope,
                        fieldKey: fieldKey
                    ));
    ...
    public static void ExpandTemplate(CallExpressionNode call, JsonObject target, DocumentScope scope, Action<StatementNode, JsonObject, DocumentScope> sink) {
        ...
        if (!scope.Templates.TryGetValue(
            ...
            scope.Diagnostics.ReportError(
                code: PuckDiagnosticCodes.InvalidValue,
                message: $"Unknown template '{call.Name}'.",
                span: call.Span
            );
            return;
        }

        var definitionScope = scope.TemplateDefinitionScope(name: call.Name);
        ...
        for (var index = 0; (index < template.Parameters.Count); ++index) {
            ...
            if (boundValue is not null) {
                ...
                scope.Diagnostics.ReportError(
                    code: PuckDiagnosticCodes.InvalidValue,
                    message: $"Template '{call.Name}' requires argument '{param.Name}'.",
                    span: call.Span
                );
                return;
            }
        ...
        if (consumed.Count != call.Arguments.Count) {
            scope.Diagnostics.ReportError(
                code: PuckDiagnosticCodes.InvalidValue,
                message: $"Template '{call.Name}' received an unknown or duplicate argument.",
                span: call.Span
            );
            return;
        }
