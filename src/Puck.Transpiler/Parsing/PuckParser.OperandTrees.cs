...
public static partial class PuckParser {
    ...
    public static OperandExpressionNode CreateOperand(ExpressionNode expression, DocumentValueForm form) {
        ...
        var text = PuckPrinter.PrintExpression(expression: expression);
        ...
        Syntax.SyntaxNode Convert(ExpressionNode node) {
            ...
            return (node.Parenthesized ? new Syntax.SourceGroup(Value: converted) : converted);
        }
        Syntax.SyntaxNode ConvertCore(ExpressionNode node) {
            switch (node) {
                ...
                case LiteralExpressionNode { Unit: null, Value: string value }:
                    return new Syntax.SourceString(Value: value);
                case LiteralExpressionNode { Unit: null, Value: bool value }:
                    return new Syntax.Literal(Value: (value ? 1 : 0));
                case LiteralExpressionNode { Unit: null, Value: long or ulong or int or decimal or double } literal:
                    try {
                        return new Syntax.Literal(Value: DecimalValues.FromLiteral(literal: literal));
                ...
                case ConditionalExpressionNode conditional:
                    return new Syntax.Ternary(Convert(node: conditional.Condition), Convert(node: conditional.WhenTrue), Convert(node: conditional.WhenFalse));
                case CallExpressionNode call:
                    return new Syntax.SourceCall(call.Name, [.. call.Arguments.Select(selector: argument => new Syntax.SourceArgument(Name: argument.Name, Value: Convert(node: argument.Value)))]);
                case LambdaExpressionNode { Parameters.Count: 1 } lambda:
                    return new Syntax.SourceLambda(Binder: lambda.Parameters[0], Body: Convert(node: lambda.Body));
    ...
    internal static OperandExpressionNode RebuildOperand(OperandExpressionNode original, Syntax.SyntaxNode syntax, IReadOnlyList<InterpolatedStringNode> atoms) {
        ...
        var text = Syntax.PrintSyntax(syntax, index => PuckPrinter.PrintExpression(expression: ordered[index]));
        var spans = ScanAtomSpans(text: text);
    ...
    private static Syntax.SyntaxNode Name(string text) {
        ...
        return (text.StartsWith(value: '$') ? new Syntax.SourceName(text) : QualifiedName.Parse(text: text).ToSyntax());
    }
    private static Syntax.SyntaxNode MapAtoms(Syntax.SyntaxNode node, Func<int, int> map) => node switch {
        ...
        Syntax.SourceCall call => call with { Arguments = [.. call.Arguments.Select(selector: argument => argument with { Value = MapAtoms(argument.Value, map) })] },
