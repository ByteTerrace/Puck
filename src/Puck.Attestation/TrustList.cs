...
/// <summary>Whether a trust list entry's key signs claims itself or vouches for others (README.md, "Signed attestation").</summary>
public enum AttestationTrustMode {
    /// <summary>
...
/// A <see cref="AttestationTrustMode.Vouches"/> entry pins the domain's root id, which is what makes the whole
/// chain beneath it trusted (README.md §7, "Trust entries").
/// </summary>
/// <param name="PinnedId">The trusted key's id. For <see cref="AttestationTrustMode.Vouches"/> this must be a root id (<see cref="KeyId.IsRoot"/>); for <see cref="AttestationTrustMode.SignsDirectly"/> it must carry a subject, since only a subject key signs claims.</param>
...
/// <see cref="AttestationVerifyResult.Admits"/> or <see cref="AttestationVerifyResult.TryGetReplayCommit"/>
...
public sealed record TrustList {
    ...
    internal TrustListEntry? FindDirectSignerForVerification(string domain, string? subject) {
        ...
        foreach (var entry in m_entries) {
            if (
                (entry.Mode == AttestationTrustMode.SignsDirectly) &&
                string.Equals(
                a: entry.PinnedId.Domain,
                b: domain,
                comparisonType: StringComparison.Ordinal
