...
/// <summary>
/// The canonical context header that is always part of an attestation's signing input
/// (README.md, "Signed attestation"): the associated-data half of AEAD applied to signatures, so a
/// signature cannot be lifted into a situation it was never minted for. Only the payload differs between a
/// key binding and a claim — see <see cref="AttestationPurposes.KeyBinding"/>.
/// </summary>
...
/// <param name="Purpose">What this attestation is for. <see cref="AttestationPurposes.KeyBinding"/> is reserved; every other value is game-defined and stops a binding being replayed as a claim.</param>
...
public sealed record AttestationHeader(
    string Domain,
    string? Subject,
    string Algorithm,
    string Purpose,
...
public static class AttestationPurposes {
    /// <summary>
    /// Reserved purpose for a key binding — the attestation's payload is a <see cref="KeyBindingPayload"/>
    /// naming the key being vouched for. A key binding is not a separate artifact: it is this purpose value
    /// on an ordinary attestation. No claim may use this purpose, and a binding presented where a claim's
...
public enum AttestationPayloadKind : byte {
    ...
    /// <summary>A <see cref="KeyBindingPayload"/> — used only with <see cref="AttestationPurposes.KeyBinding"/>.</summary>
    KeyBinding = 2,
...
/// associated data (README.md, "Signed attestation"). The recipient id is also inside the signed payload,
...
/// (README.md, "Signed attestation"). This is the one shape used for every purpose — a key binding
/// is an attestation with <see cref="AttestationPurposes.KeyBinding"/> as its purpose, never a separate artifact.
/// </summary>
/// <remarks>
...
/// bytes <i>as they arrived</i>, never against a re-encoding of what it parsed out of them. An attestation
...
/// alternate wire form for one claim. So the arrived bytes travel with the attestation and
/// <see cref="AttestationVerifier"/> verifies against them.</para>
...
/// <c>attestation with { PayloadKind = … }</c> does not compile: there is no way to change what the attestation
...
public sealed record SignedAttestation {
    ...
    public static SignedAttestation Reencode(
