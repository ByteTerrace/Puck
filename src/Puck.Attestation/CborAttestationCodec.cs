...
/// <summary>
/// The CBOR <see cref="IAttestationCodec"/>, built on <see cref="System.Formats.Cbor"/> (Microsoft-authored,
/// but inbox only through the ASP.NET Core shared framework — hence the standalone package reference in
/// this project's csproj). A full attestation is a definite-length 2-element CBOR array
...
public sealed class CborAttestationCodec : IAttestationCodec {
    ...
    public SignedAttestation DecodeAttestation(ReadOnlySpan<byte> wire) {
        if (wire.Length > AttestationResourceLimits.AttestationBytes) {
            throw new FormatException(message: $"The attestation is {wire.Length} bytes; v1 permits at most {AttestationResourceLimits.AttestationBytes} before parsing.");
        }
        ...
        return Decode(
            ...
            body: () => {
                ...
                // carried verbatim into the attestation — never re-derived from what was parsed out of them
                // (README.md §2).
                ...
                RequireCanonical(
                    received: source,
                    reencoded: EncodeAttestation(attestation: attestation),
                    what: "attestation"
                );
