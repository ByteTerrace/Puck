...
public sealed record VolumeDocument(string Kind, DocumentVector3 Position, DocumentQuaternion Rotation,
    ...
    public SdfVolume ToVolume(int dynamicSlot, Vector3 origin, Quaternion rotation, float scale) => new(
        ...
        Quaternion.Normalize(value: (rotation * Rotation.Value)),
        (HalfExtent.Value * scale),
        dynamicSlot,
        ((Axis ?? (2f * HalfExtent.Y)) * scale),
        ((Width ?? (DefaultWidthShare * HalfExtent.X)) * scale),
        ((Speed ?? DefaultSpeed) * scale),
        (Seed ?? 0u),
        (Steps ?? DefaultSteps),
        Ramp.Select(selector: s => new SdfDensityStop(
            s.Density,
            HexColor.Parse(
                value: s.Color,
