...
public static class CreationStampSampling {
    ...
    public static long NoiseInstanceCeiling(int width, int depth) => (((long)Math.Max(
        val1: width,
        val2: 0
    )) * Math.Max(
        val1: depth,
    ...
    public static FixedVector3[] ResolveNoise(FixedQ4816 cellSize, int width, int depth, int frequency, FixedQ4816 threshold, int octaves, uint seed, ulong worldSeed) {
        ...
        var effectiveDepth = Math.Max(
            val1: depth,
            val2: 0
        );
        var effectiveOctaves = Math.Max(
            val1: 1,
            val2: octaves
        );
        var hashSeed = FoldSeed(
            seed: seed,
            worldSeed: worldSeed
        );
        var half = (cellSize / FixedQ4816.FromInteger(value: 2));
        var originX = (-GridHalfExtent(
            cellSize: cellSize,
            count: effectiveWidth
        ));
        var originZ = (-GridHalfExtent(
            cellSize: cellSize,
            count: effectiveDepth
        ));
    ...
    public static FixedVector3[] ResolveScatter(FixedQ4816 cellSize, int width, int depth, int spacing, int radius, uint seed, ulong worldSeed) {
        ...
        var inset = Math.Max(
            val1: 0,
            val2: (effectiveSpacing - (2 * effectiveRadius))
        );
        var originX = (-GridHalfExtent(
            cellSize: cellSize,
            count: effectiveWidth
        ...
        for (var bz = 0; (bz < blocksZ); bz++) {
            for (var bx = 0; (bx < blocksX); bx++) {
                ...
                var px = (((bx * effectiveSpacing) + effectiveRadius) + ((inset > 0)
                    ? (int)(h.X % ((uint)inset))
                    : 0));
                var pz = (((bz * effectiveSpacing) + effectiveRadius) + ((inset > 0)
                    ? (int)(h.Y % ((uint)inset))
                    : 0));
    ...
    public static long ScatterInstanceCeiling(int width, int depth, int spacing) {
        ...
        var blocksX = ((Math.Max(
            val1: width,
            val2: 0
        ) + (effectiveSpacing - 1)) / effectiveSpacing);
        var blocksZ = ((Math.Max(
            val1: depth,
            val2: 0
        ) + (effectiveSpacing - 1)) / effectiveSpacing);
