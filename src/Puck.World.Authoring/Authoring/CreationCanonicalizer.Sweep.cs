...
public static partial class CreationCanonicalizer {
    ...
    private static void ValidateCurve(ShapeDocument shape, List<DocumentValidationError> errors, string path) {
        ...
        if (shape.Type != SdfSolidPrimitive.Sweep) {
            errors.Add(item: new(
                Message: $"a curve is admitted only on a Sweep primitive; this shape is {shape.Type}.",
                Path: path
            ));
        ...
        if (shape.Panel is not null) {
            errors.Add(item: new(
                Message: "a Sweep cannot also carry a panel; it has no local face to inset.",
                Path: path
            ));
        }
        if (shape.Trims is { Count: > 0 }) {
            errors.Add(item: new(
                Message: "a Sweep cannot also carry trims.",
                Path: path
            ));
        }
        if (shape.Flare is not null) {
            errors.Add(item: new(
                Message: "a Sweep cannot also carry a flare; the curve already authors its own silhouette.",
                Path: path
            ));
        }
        if (shape.Shear is not null) {
            errors.Add(item: new(
                Message: "a Sweep cannot also carry a shear; the curve already authors its own silhouette.",
                Path: path
            ));
        }
        if (shape.Bumps is { Count: > 0 }) {
            errors.Add(item: new(
                Message: "a Sweep cannot also carry bumps.",
                Path: path
            ));
        }
        if (shape.Domain is { Count: > 0 }) {
            errors.Add(item: new(
                Message: "a Sweep cannot also carry domain operators.",
                Path: path
            ));
        }
        ...
        if (
            !float.IsFinite(f: radiusStart) ||
            (radiusStart <= 0f)
        ) {
            errors.Add(item: new(
                Message: "radiusStart must be finite and positive.",
                Path: $"{path}.radiusStart"
            ));

            return;
        }
        if (
            !float.IsFinite(f: radiusEnd) ||
            (radiusEnd <= 0f)
        ) {
            errors.Add(item: new(
                Message: "radiusEnd must be finite and positive.",
                Path: $"{path}.radiusEnd"
            ));
        ...
        if (!float.IsFinite(f: bulge)) {
            errors.Add(item: new(
                Message: "bulge is non-finite.",
                Path: $"{path}.bulge"
            ));
        } else if (MathF.Abs(x: bulge) > (SdfProgramBuilder.MaxSweepBulgeRatio * maxRadius)) {
            errors.Add(item: new(
                Message: $"bulge magnitude {MathF.Abs(x: bulge)} exceeds {SdfProgramBuilder.MaxSweepBulgeRatio} times the largest radius {maxRadius}.",
                Path: $"{path}.bulge"
            ));
        }
        ...
        if (!float.IsFinite(f: twist)) {
            errors.Add(item: new(
                Message: "twist is non-finite.",
                Path: $"{path}.twist"
            ));
        }

        if (
            !float.IsFinite(f: strandOffset) ||
            (strandOffset < 0f)
        ) {
            errors.Add(item: new(
                Message: "strandOffset must be finite and non-negative.",
                Path: $"{path}.strandOffset"
            ));
        } else if (strandOffset > (SdfProgramBuilder.MaxSweepStrandOffsetRatio * maxRadius)) {
            errors.Add(item: new(
                Message: $"strandOffset {strandOffset} exceeds {SdfProgramBuilder.MaxSweepStrandOffsetRatio} times the largest radius {maxRadius}.",
                Path: $"{path}.strandOffset"
            ));
        }

        if (
            (strands < SdfProgramBuilder.MinSweepStrands) ||
            (strands > SdfProgramBuilder.MaxSweepStrands)
        ) {
            errors.Add(item: new(
                Message: $"strands must be {SdfProgramBuilder.MinSweepStrands}..{SdfProgramBuilder.MaxSweepStrands}; got {strands}.",
                Path: $"{path}.strands"
            ));
        }
        ...
        if (MathF.Abs(x: (radiusEnd - radiusStart)) > (SdfProgramBuilder.MaxSweepTaperRatio * minRadius)) {
            errors.Add(item: new(
                Message: $"the radius taper |{radiusEnd} - {radiusStart}| exceeds {SdfProgramBuilder.MaxSweepTaperRatio} times the smaller radius {minRadius}.",
                Path: $"{path}.radiusEnd"
            ));
        }
    }
