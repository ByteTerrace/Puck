...
public sealed class CreationBuilder {
    ...
    public static SculptSide Left => new(
        Sign: 1f,
        Suffix: "L"
    );
    ...
    public static SculptSide Right => new(
        Sign: -1f,
        Suffix: "R"
    );
    ...
    private static string DeriveAxisName(Vector3 normalizedAxis, float degrees) {
        ...
        return $"{letter}{((degrees < 0f)
            ? "m"
            : string.Empty)}{magnitudeText}";
    }
    ...
    private static float Round4(float value) => (MathF.Round(
        digits: 4,
    ...
    private static int RoundToInt(float value) => ((int)MathF.Round(
        mode: MidpointRounding.AwayFromZero,
        x: value
    ));
    ...
    public Quaternion Multiply(Quaternion a, Quaternion b, string? explicitName = null) {
        if (!TryRotationName(
            ...
            throw new ArgumentException(
                message: "the left operand is not an interned rotation — build it through AxisAngleDegrees/Multiply first.",
                paramName: nameof(a)
            );
        }
        ...
        if (!TryRotationName(
            ...
            throw new ArgumentException(
                message: "the right operand is not an interned rotation — build it through AxisAngleDegrees/Multiply first.",
                paramName: nameof(b)
            );
        }
    ...
    public SculptShape Shape(
        ...
        if (m_shapeIndexByName.ContainsKey(key: name)) {
            throw new ArgumentException(
                message: $"a shape named '{name}' is already declared.",
        ...
        if (
            (parent is { Length: > 0 }) &&
            !m_shapeIndexByName.ContainsKey(key: parent)
        ) {
            throw new ArgumentException(
                message: $"shape '{name}' names parent '{parent}', which is not yet declared — declare parents before children.",
                paramName: nameof(parent)
            );
        }
        ...
        if (!TryRotationName(
            ...
            throw new ArgumentException(
                message: $"shape '{name}''s rotation is not an interned value — build it through AxisAngleDegrees/Multiply/Identity first.",
        ...
        if (m_shapes.Any(predicate: existing => (existing.Id == resolvedId))) {
            throw new ArgumentException(
                message: $"shape '{name}' reuses id {resolvedId}, already assigned to '{m_shapes.First(predicate: s => (s.Id == resolvedId)).Name}'.",
                paramName: nameof(id)
            );
        }
    ...
    public static IReadOnlyList<ShapeDomainOp> SymmetryX(float offset = 0f) => [new ShapeDomainOp.Symmetry(
    ...
    public bool TryValidateTopology(out string reason) {
        ...
        foreach (var shape in m_shapes) {
            ...
            if (
                (shape.Parent is { Length: > 0 } parent) &&
                !seen.Contains(item: parent)
            ) {
