...
public sealed class AzureConfiguredProvider : IWorldConfiguredProvider, IWorldConfiguredObservationProvider {
    ...
    public static WorldExtensionProviderType Registration { get; } = new(
        "azure.resource",
        settings => new AzureConfiguredProvider(settings: settings)
    );
    ...
    private AzureConfiguredProvider(JsonElement settings) {
        var configuration = (settings.Deserialize(AzureConfiguredProviderJson.Default.AzureConfiguredProviderSettings)
            ?? throw new JsonException(message: "Azure provider settings are required."));
        ...
        m_environment = configuration.Cloud switch {
            ...
            _ => throw new ArgumentException(message: "Azure cloud must be public, government, or china."),
        };
        m_credential = configuration.Authentication switch {
            "managedIdentity" when (configuration.TenantId is null) => new ManagedIdentityCredential(id: ((configuration.ClientId is { } clientId)
            ? ManagedIdentityId.FromUserAssignedClientId(clientId)
            : ManagedIdentityId.SystemAssigned)),
            "azureCli" when (configuration.ClientId is null) => new AzureCliCredential(options: new AzureCliCredentialOptions { TenantId = configuration.TenantId }),
            _ => throw new ArgumentException(message: "Select managedIdentity (optional clientId) or azureCli (optional tenantId). No implicit credential chain is used."),
        };
    }
    ...
    public WorldExtensionOperation Bind(string name, string description, JsonElement settings) {
        ...
        var configuration = (settings.Deserialize(AzureConfiguredProviderJson.Default.AzureConfiguredOperationSettings)
            ?? throw new JsonException(message: "Azure operation settings are required."));

        if (
            !Enum.TryParse<AzureResourceMethod>(
            configuration.Method,
            true,
            out var method
        ) ||
            !Enum.IsDefined(value: method) ||
            !string.Equals(
            method.ToString(),
            configuration.Method,
            ...
            throw new ArgumentException(message: "Azure method must be post, patch, delete, or put.");
        }
        var provider = new AzureResourceOperationProvider(
            m_credential,
            new(
                name,
        ...
        try {
            var registration = provider.Register(description: description);

            m_operations.Add(item: provider);
            return registration;
    ...
    public IWorldExtensionObservationSource BindObservation(JsonElement settings, int maximumItems) {
        ...
        var kind = ((settings.TryGetProperty(
            propertyName: "kind",
        ...
        return kind switch {
            "inventory" or null => new AzureResourceInventory(
            m_credential,
            m_environment,
            settings,
            maximumItems
            ...
            _ => throw new ArgumentException(message: "Azure observation kind must be inventory or metrics."),
        };
    }
