...
public static class AzureSiloExtensions {
    ...
    public static WorldAuthenticationProvider Authentication { get; } = new(
        "azure.api-users",
        (settings, federation) => new EntraWorldAuthenticator(
            settings,
            client: false,
    ...
    public static WorldSiloRetirementProvider Retirement { get; } = new(
        ...
        return new Observer(interval: TimeSpan.FromSeconds(seconds: seconds));
    }
    );
    ...
    private sealed class Observer(TimeSpan interval) : IWorldHostRetirementObserver {
        ...
        public Task RunAsync(Func<DateTimeOffset, CancellationToken, Task> retire, CancellationToken cancellationToken) =>
            new AzureScheduledEvents(client: m_client).RunAsync(
