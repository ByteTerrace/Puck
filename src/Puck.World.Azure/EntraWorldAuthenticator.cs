...
public sealed class EntraWorldAuthenticator : IAuthenticator, IRemoteIdentityVerifier {
    ...
    public EntraWorldAuthenticator(JsonElement settings, bool client, IAuthenticator? federation = null, TokenCredential? credential = null,
        ...
        if (client) {
            ...
            if (
                (m_remoteKey is null) ||
                (m_remoteKey.Length != 64) ||
                !m_remoteKey.All(predicate: static c => char.IsAsciiHexDigitLower(c: c))
            ) { throw new ArgumentException(message: "A client requires the server's lowercase SHA-256 peer-key fingerprint."); }
        ...
        m_configuration = (configuration ?? new ConfigurationManager<OpenIdConnectConfiguration>(
            $"https://login.microsoftonline.com/{m_tenant}/v2.0/.well-known/openid-configuration",
    ...
    private string ReadClientToken() {
        ...
        lock (m_tokenGate) {
            ...
            if (
                ...
                (previous != subject)
            ) { throw new InvalidOperationException(message: "The ambient user changed; reconnect with a fresh world session."); }
    ...
    private string UserId(string token) {
        ...
        ) { throw new InvalidOperationException(message: "The ambient API credential is not a user identity."); }
        return id.ToString(format: "D");
    }

    /// <inheritdoc/>
    public bool AcceptsRemoteIdentity(string keyHash) => ((m_credential is null) || string.Equals(
        a: m_remoteKey,
        b: keyHash,
    ...
    public bool TryVerify(ReadOnlySpan<byte> challenge, ReadOnlySpan<byte> proof, out string? sourceAuthority) {
        ...
        if (
            (parts.Length != 3) ||
            (parts[0] != Convert.ToBase64String(challenge)) ||
            !Guid.TryParseExact(
            parts[1],
            "N",
            out var session
        ...
        try {
            ...
            if (
                (identity.FindAll(type: "tid").SingleOrDefault()?.Value != m_tenant) ||
                !identity.FindAll(type: "groups").Any(predicate: claim => string.Equals(
                a: claim.Value,
                b: m_group,
                comparisonType: StringComparison.OrdinalIgnoreCase
            )) ||
                (identity.FindAll(type: "scp").SingleOrDefault()?.Value.Split(
                ...
                !Guid.TryParse(
                input: identity.FindAll(type: "oid").SingleOrDefault()?.Value,
                result: out var oid
            )
            ) { return false; }
