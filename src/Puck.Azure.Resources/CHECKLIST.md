...
- [ ] set the repo variables `AZURE_TENANT_ID` and `AZURE_SUBSCRIPTION_ID` for the same
      environment. Nothing else needs setting by hand — `azure.yml`'s `Azure/login` step exchanges
...
- [ ] `officialContentBaseUrl` is `https://puck.byteterrace.com/official` — `puck.byteterrace.com`
      is carried on the `api`, `blob-private`, `blob-public`, `configuration`, and `portal` routes' `customDomains`
