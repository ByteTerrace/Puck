...
func blobAction(action string) string =>
  'ActionMatches{\'Microsoft.Storage/storageAccounts/blobServices/containers/blobs/${action}\'}'
func blobListAction() string => '${blobAction('read')} AND SubOperationMatches{\'Blob.List\'}'
func blobPathIsNotUnder(prefix string) string =>
  '@Resource[Microsoft.Storage/storageAccounts/blobServices/containers/blobs:path] StringNotStartsWithIgnoreCase \'${prefix}/\''
func blobPathIsUnder(prefix string) string =>
  '@Resource[Microsoft.Storage/storageAccounts/blobServices/containers/blobs:path] StringLike \'${prefix}/*\''
func blobPathIsUnderAnyOf(prefixes string[]) string =>
  '@Resource[Microsoft.Storage/storageAccounts/blobServices/containers/blobs:path] ForAnyOfAnyValues:StringLike {${join(map(prefixes, prefix => '\'${prefix}/*\''), ', ')}}'
func blobReadAction() string => '${blobAction('read')} AND !SubOperationMatches{\'Blob.List\'}'
func clause(actions string[], branches string[]) string =>
  '((${join(map(actions, action => '!(${action})'), ' AND ')})${(empty(branches) ? '' : ' OR ${join(map(branches, branch => '(${branch})'), ' OR ')}')})'
func containerMetadataIsNotFrozen() string =>
  '((NOT Exists @Resource[Microsoft.Storage/storageAccounts/blobServices/containers/metadata:Frozen]) OR (@Resource[Microsoft.Storage/storageAccounts/blobServices/containers/metadata:Frozen] StringNotEqualsIgnoreCase \'True\'))'
func isNotSystemContainer() string =>
  'NOT @Resource[Microsoft.Storage/storageAccounts/blobServices/containers:name] StringStartsWithIgnoreCase \'$\''
func isOwnContainer() string =>
  '@Resource[Microsoft.Storage/storageAccounts/blobServices/containers:name] StringEqualsIgnoreCase ${principalObjectId()}'
func isOwnQueue() string =>
  '@Resource[Microsoft.Storage/storageAccounts/queueServices/queues:name] StringEqualsIgnoreCase ${principalObjectId()}'
func isPublicPathOrOwnContainer() string => '((${blobPathIsUnder('public')}) OR (${isOwnContainer()}))'
func listPrefixIsUnderAnyOf(prefixes string[]) string =>
  '@Request[Microsoft.Storage/storageAccounts/blobServices/containers/blobs:prefix] ForAnyOfAnyValues:StringLike {${join(map(prefixes, prefix => '\'${prefix}/*\''), ', ')}}'
func principalObjectId() string =>
  '@Principal[Microsoft.Directory/CustomSecurityAttributes/Id:ByteTerraceUsers_ObjectId]'
func queueMessageAction(action string) string =>
  'ActionMatches{\'Microsoft.Storage/storageAccounts/queueServices/queues/messages/${action}\'}'

@export()
func byteTerraceApiHostStorageUserCondition() string =>
  join(
    [
      clause(
        [
...
@export()
func byteTerraceUserStorageUserCondition() string =>
  join(
    [
      clause(
        [
        ...
        [
          allOf([
            isNotSystemContainer()
            isOwnContainer()
            blobPathIsUnderAnyOf(['private', 'public'])
            containerMetadataIsNotFrozen()
          ])
        ]
      )
      ...
      clause([blobReadAction()], [
        allOf([
          isNotSystemContainer()
          isPublicPathOrOwnContainer()
          blobPathIsUnderAnyOf(['private', 'public', 'system'])
        ])
      ])
      ...
      clause([blobAction('tags/read')], [isOwnContainer()])
      clause(
...
@export()
func frontDoorPublicContentReadCondition() string =>
  join(
    [
...
@export()
func containerRepositoryCondition(repositories string[], writable bool) string =>
  clause(
    map(
...
// The CI publishing identity (puck azure publish-static) writes only to the platform's own container
...
@export()
func officialContentPublisherCondition() string =>
  join(
    [
      clause(
        [
