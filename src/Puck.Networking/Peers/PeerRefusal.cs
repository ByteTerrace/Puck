...
public enum PeerRefusal : byte {
    ...
    None = 0,
    /// <summary>The connection closed before the handshake reached an identity decision.</summary>
    ConnectionClosed,
...
/// <summary>One named <see cref="PeerRefusal"/> plus narration.</summary>
...
/// <summary>The exception <see cref="Peer.DialAsync"/> raises when the handshake it ran was refused, carrying the
/// named refusal.</summary>
public sealed class PeerRefusedException : IOException {
    ...
    public PeerRefusedException(PeerFailure failure) : base(message: $"the handshake was refused — {failure}") {
        Failure = failure;
    }
