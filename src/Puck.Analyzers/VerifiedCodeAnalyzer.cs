...
public sealed class VerifiedCodeAnalyzer : DiagnosticAnalyzer {
    ...
    private static void AnalyzeCompilationStart(CompilationStartAnalysisContext context) {
        ...
        context.RegisterSyntaxNodeAction(
            ...
            syntaxKinds: ImmutableArray.Create(
                item1: SyntaxKind.LocalFunctionStatement,
                item2: SyntaxKind.ParenthesizedLambdaExpression
            ));
    ...
    private static void AnalyzeSymbol(SymbolAnalysisContext symbolContext, INamedTypeSymbol? attributeType, ManifestState manifest, ConcurrentDictionary<string, ConcurrentQueue<string>> claims) {
        ...
        var attributeData = symbol.GetAttributes()
            .FirstOrDefault(predicate: candidate => SymbolEqualityComparer.Default.Equals(
            x: candidate.AttributeClass,
            y: attributeType
        ));
        ...
        if (
            ...
            (attributeData.ConstructorArguments[0].Value is not string id) ||
        ...
        if (string.IsNullOrEmpty(value: documentationId)) {
            symbolContext.ReportDiagnostic(diagnostic: Diagnostic.Create(
                ...
                location: location,
                id,
                $"'{symbolDisplay}', a declaration with no documentation-comment id"
            ));
        ...
        claims
            .GetOrAdd(
            key: id,
            valueFactory: _ => new ConcurrentQueue<string>()
        )
        ...
        var entry = ((believable && manifest.Reading.Entries.TryGetValue(
            key: id,
        ...
        if (entry is null) {
            symbolContext.ReportDiagnostic(diagnostic: CreateMismatch(
                location: location,
        ...
        if (!string.Equals(
            ...
            symbolContext.ReportDiagnostic(diagnostic: Diagnostic.Create(
                descriptor: Ver008EntrySymbolMismatch,
                location: location,
                symbolDisplay,
                id,
                entry.Symbol,
    ...
    private static void AnalyzeUnrecordableDeclaration(SyntaxNodeAnalysisContext nodeContext, INamedTypeSymbol? attributeType) {
        ...
        foreach (var attributeList in attributeLists) {
            foreach (var attribute in attributeList.Attributes) {
                var attributeSymbol = nodeContext.SemanticModel.GetTypeInfo(
                    attributeSyntax: attribute,
                    cancellationToken: nodeContext.CancellationToken
                ).Type;
                ...
                nodeContext.ReportDiagnostic(diagnostic: Diagnostic.Create(
                    descriptor: Ver007UnsupportedBrandPlacement,
    ...
    private static string BrandId(SyntaxNodeAnalysisContext nodeContext, AttributeSyntax attribute) {
        ...
        var constant = nodeContext.SemanticModel.GetConstantValue(
        ...
        return ((constant.HasValue
            ...
            : null) ?? argument.Expression.ToString());
    }
    private static void CheckBasisAgreement(SymbolAnalysisContext symbolContext, AttributeData attributeData, VerifiedCodeManifestEntry entry, string symbolDisplay, string id, Location location) {
        ...
        var basisArgument = attributeData.NamedArguments
            .FirstOrDefault(predicate: pair => string.Equals(
            a: pair.Key,
            b: "Basis",
            comparisonType: StringComparison.Ordinal
        ));
        ...
        if (
            (basisArgument.Key is null) ||
            (basisArgument.Value.Value is not string attributeBasis)
        ) {
            return;
        }

        if (BasisSetsAgree(
            attributeBasis: attributeBasis,
            manifestBasis: entry.Basis
        )) {
            return;
        }

        var manifestBasisText = string.Join(
            separator: ", ",
            values: entry.Basis
        );

        symbolContext.ReportDiagnostic(diagnostic: Diagnostic.Create(
            descriptor: Ver004BasisMismatch,
            location: location,
            symbolDisplay,
            id,
            attributeBasis,
    ...
    private static void AnalyzeCompilationEnd(CompilationAnalysisContext endContext, ManifestState manifest, ConcurrentDictionary<string, ConcurrentQueue<string>> claims) {
        ...
        foreach (var fault in manifest.Reading.Faults) {
            endContext.ReportDiagnostic(diagnostic: Diagnostic.Create(
                descriptor: Ver006ManifestUnusable,
        ...
        foreach (var claim in claims.OrderBy(
            keySelector: pair => pair.Key,
            comparer: StringComparer.Ordinal
        )) {
            var claimants = claim.Value.OrderBy(
            ...
            endContext.ReportDiagnostic(diagnostic: Diagnostic.Create(
                descriptor: Ver009DuplicateEntryClaim,
                location: manifest.Location,
                claim.Key,
                claimants.Length,
                string.Join(
        ...
        var assemblyDeclaresOwnNamespace = ((assemblyName.Length == 0) || AssemblyDeclaresNamespace(
            assembly: endContext.Compilation.Assembly,
        ...
        foreach (var entry in manifest.Reading.Entries.Values) {
            ...
            if (!assemblyDeclaresOwnNamespace) {
                endContext.ReportDiagnostic(diagnostic: Diagnostic.Create(
                    descriptor: Ver005AssemblyConventionViolated,
                    location: manifest.Location,
                    entry.Id,
                    entry.Symbol,
                    assemblyName
            ...
            endContext.ReportDiagnostic(diagnostic: Diagnostic.Create(
                descriptor: Ver002UnclaimedManifestEntry,
                location: manifest.Location,
                entry.Id,
                entry.Symbol
            ));
    ...
    private static bool AssemblyDeclaresNamespace(IAssemblySymbol assembly, string dottedName) {
        ...
        foreach (var segment in dottedName.Split('.')) {
            var next = current.GetNamespaceMembers()
                .FirstOrDefault(predicate: candidate => string.Equals(
                a: candidate.Name,
                b: segment,
                comparisonType: StringComparison.Ordinal
            ));
