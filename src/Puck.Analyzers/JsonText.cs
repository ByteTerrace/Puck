...
internal static class JsonText {
    ...
    public static ValueSpan? FindRecordedHash(string json, string id) {
        var root = SkipWhitespace(json: json, index: 0);

        if ((root >= json.Length) || (json[root] != '{')) {
            return null;
        }

        if (FindMember(json: json, objectStart: root, name: "entries") is not int entries) {
            return null;
        }

        if ((entries >= json.Length) || (json[entries] != '{') || (FindMember(json: json, objectStart: entries, name: id) is not int entry)) {
            return null;
        }

        if ((entry >= json.Length) || (json[entry] != '{') || (FindMember(json: json, objectStart: entry, name: "sha256") is not int hash)) {
            return null;
        }

        if ((hash >= json.Length) || (json[hash] != '"')) {
            return null;
        }

        var end = SkipValue(json: json, index: hash);

        return ((end < 0) ? null : new ValueSpan(Start: hash, Length: (end - hash)));
    }
    ...
    public static string Replace(string json, ValueSpan span, string replacement) =>
        ((json.Substring(startIndex: 0, length: span.Start) + replacement) + json.Substring(startIndex: (span.Start + span.Length)));
    ...
    private static int? FindMember(string json, int objectStart, string name) {
        ...
        var index = SkipWhitespace(json: json, index: (objectStart + 1));
        ...
        while (index < json.Length) {
            index = SkipWhitespace(json: json, index: index);

            if (!TryReadString(json: json, index: index, value: out var key, next: out var afterKey)) {
                return null;
            }

            index = SkipWhitespace(json: json, index: afterKey);

            if ((index >= json.Length) || (json[index] != ':')) {
                return null;
            }

            index = SkipWhitespace(json: json, index: (index + 1));

            if (index >= json.Length) {
                return null;
            }

            if (string.Equals(a: key, b: name, comparisonType: StringComparison.Ordinal)) {
                return index;
            }

            var afterValue = SkipValue(json: json, index: index);

            if (afterValue < 0) {
                return null;
            }

            index = SkipWhitespace(json: json, index: afterValue);
    ...
    private static int SkipValue(string json, int index) {
        ...
        if (start == '"') {
            return (TryReadString(json: json, index: index, value: out _, next: out var afterString) ? afterString : -1);
        }
        ...
        if ((start == '{') || (start == '[')) {
            ...
            while (index < json.Length) {
                ...
                if (character == '"') {
                    if (!TryReadString(json: json, index: index, value: out _, next: out var afterString)) {
                        return -1;
                    }
        ...
        while ((index < json.Length) && (json[index] != ',') && (json[index] != '}') && (json[index] != ']') && !char.IsWhiteSpace(c: json[index])) {
            index++;
        }
    ...
    private static bool TryReadString(string json, int index, out string value, out int next) {
        ...
        while (index < json.Length) {
            ...
            if ((escape != 'u') || ((index + 4) > json.Length)) {
                return false;
            }

            if (!ushort.TryParse(s: json.Substring(startIndex: index, length: 4), style: NumberStyles.AllowHexSpecifier, provider: CultureInfo.InvariantCulture, result: out var codeUnit)) {
                return false;
            }
