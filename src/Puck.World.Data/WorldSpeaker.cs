...
public abstract record WorldSpeakerSource {
...
public sealed record WorldSpeakerFeed(WorldSpeakerSource Source, string Channel, float Gain) {
...
/// <param name="Curve">The falloff curve token (<see cref="WorldAudioDefaults.CurveSmoothstep"/> or
/// <see cref="WorldAudioDefaults.CurveLinear"/>), or <see langword="null"/> for the audio-defaults curve.</param>
public sealed record WorldSpeakerAttenuation(float Radius, string? Curve);
...
[JsonPolymorphic(TypeDiscriminatorPropertyName = "$type")]
public abstract record WorldSpeaker(
    string Name,
    ...
    /// <summary>A speaker anchored to a <see cref="WorldAnchor"/> — the entity/part/placement pose it rides supplies
...
public sealed record WorldEmission(string PatchId, float Level, float? Radius = null);
...
/// <param name="DefaultCurve">The falloff curve a point emitter coalesces to: <see cref="CurveSmoothstep"/> or
/// <see cref="CurveLinear"/>.</param>
...
public sealed record WorldAudioDefaults(
    float MasterGain,
    ...
    /// <summary>The squared-smoothstep finite-support falloff.</summary>
    public const string CurveSmoothstep = "smoothstep";
    /// <summary>The distance-linear finite-support falloff.</summary>
