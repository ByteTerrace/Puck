...
public static class WorldFrameIsometry {
    ...
    public static FixedQ4816 YawDelta(in WorldFaceFrame source, in WorldFaceFrame destination) {
        var mappedForward = MapVector(value: s_forward, source: source, destination: destination);
        return FixedQ4816.Atan2(y: mappedForward.X, x: mappedForward.Z);
    }
    ...
    public static WorldFrameArrival MapArrival(
        ...
        var mappedVelocity = MapVector(value: velocity, source: source, destination: destination);

        return new WorldFrameArrival(
            Position: MapPoint(point: travelerPosition, source: source, destination: destination),
            YawRadians: (travelerYawRadians + YawDelta(source: source, destination: destination)),
