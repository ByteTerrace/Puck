...
internal sealed class QrBitWriter {
    ...
    public byte[] FinishAndPad() {
        var terminatorBits = Math.Min(val1: 4, val2: (CapacityBits - m_bitPosition));

        if (terminatorBits > 0) {
            WriteBits(value: 0, bitCount: terminatorBits);
        }
        ...
        for (var byteIndex = (m_bitPosition / 8); (byteIndex < m_buffer.Length); byteIndex++) {
            m_buffer[byteIndex] = (padToggle ? (byte)0xEC : (byte)0x11);
            padToggle = !padToggle;
        }
