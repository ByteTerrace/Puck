...
public static class QrReedSolomon {
    ...
    private static readonly BinaryField<byte> s_field = BinaryField<byte>.Create(degree: 8, reductionTail: ReductionTail);
    // Indexed by (degree - 1): each entry is that degree's generator coefficients, highest-order first, length degree+1.
    private static readonly byte[][] s_generators = BuildGenerators();
    ...
    public static byte[] ComputeCodewords(ReadOnlySpan<byte> data, int eccCount) {
        ...
        ArgumentOutOfRangeException.ThrowIfGreaterThan(value: eccCount, other: MaxEccCodewordsPerBlock);

        var codewords = new byte[eccCount];

        ReedSolomon.ComputeCheckSymbols(
            field: s_field,
            generator: s_generators[(eccCount - 1)],
            message: data,
