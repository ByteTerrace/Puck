...
public static class QrEncoder {
    ...
    public static bool TryFindVersion(int payloadByteCount, QrErrorCorrectionLevel level, out int version, out string? error) {
        ...
        for (version = MinSupportedVersion; (version <= MaxSupportedVersion); version++) {
            ...
            var capacityBits = (QrCapacityTable.BlockPlan(version: version, level: level).TotalDataCodewords * 8);
        ...
        var plan = QrCapacityTable.BlockPlan(version: MaxSupportedVersion, level: level);
    ...
    public static bool TryEncode(string payload, QrErrorCorrectionLevel level, out QrMatrix? matrix, out string? error) {
        ...
        if (!TryFindVersion(payloadByteCount: bytes.Length, level: level, out var version, out error)) {
            matrix = null;

            return false;
        }

        var plan = QrCapacityTable.BlockPlan(version: version, level: level);
        var dataCodewords = BuildDataCodewords(payload: bytes, version: version, plan: plan);
        var finalCodewords = InterleaveBlocks(dataCodewords: dataCodewords, plan: plan);

        matrix = QrMatrix.Build(codewords: finalCodewords, version: version, level: level);
        error = null;
    ...
    public static uint ComputeFormatInfoBits(QrErrorCorrectionLevel level, int mask) {
        ...
        var remainder = ComputeBchRemainder(data: data, dataBits: 5, generator: FormatInfoGenerator, generatorDegree: FormatInfoGeneratorDegree);
    ...
    public static uint ComputeVersionInfoBits(int version) {
        ...
        var remainder = ComputeBchRemainder(data: data, dataBits: 6, generator: VersionInfoGenerator, generatorDegree: VersionInfoGeneratorDegree);
    ...
    private static byte[] BuildDataCodewords(byte[] payload, int version, QrBlockPlan plan) {
        ...
        writer.WriteBits(value: ByteModeIndicator, bitCount: 4);
        writer.WriteBits(value: payload.Length, bitCount: QrCapacityTable.ByteModeCharacterCountBits(version: version));

        foreach (var value in payload) {
            writer.WriteBits(value: value, bitCount: 8);
        }
    ...
    private static byte[] InterleaveBlocks(byte[] dataCodewords, QrBlockPlan plan) {
        ...
        for (var i = 0; (i < blocks.Length); i++) {
            eccBlocks[i] = QrReedSolomon.ComputeCodewords(data: blocks[i], eccCount: plan.EccCodewordsPerBlock);
        }

        var result = new byte[plan.TotalCodewords];
        var position = 0;
        var maxDataLength = Math.Max(val1: plan.Group1DataCodewords, val2: plan.Group2DataCodewords);
