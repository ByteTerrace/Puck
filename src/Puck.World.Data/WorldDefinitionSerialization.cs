...
internal sealed class CommandValueJsonConverter : JsonConverter<CommandValue> {
        ...
        while (reader.Read() && (reader.TokenType != JsonTokenType.EndObject)) {
            ...
            switch (property) {
                case "kind":
                    ...
                    if ((token is null) || !Enum.TryParse<CommandValueKind>(value: token, ignoreCase: false, result: out var parsed) || !Enum.IsDefined(value: parsed)) {
    ...
    private static float ReadComponent(ref Utf8JsonReader reader) {
        ...
        return float.IsFinite(f: value) ? value : throw new JsonException(message: "CommandValue raw components must be finite numbers.");
    }
}
...
internal sealed class MutationKindMaskJsonConverter : JsonConverter<MutationKindMask> {
    ...
    public override MutationKindMask Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options) {
        ...
        if (!MutationKindMask.TryParse(text: token, mask: out var mask, unknown: out var unknown)) {
            throw new JsonException(message: $"a verb mask is a comma-separated list of WorldMutation kind names (e.g. \"UpsertStateCell,RemoveStateCell\"); '{(string.IsNullOrEmpty(value: unknown) ? (token ?? "(absent)") : unknown)}' names no declared mutation kind.");
        }
...
internal sealed class DocumentWriteMaskJsonConverter : JsonConverter<DocumentWriteMask> {
    ...
    public override DocumentWriteMask Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options) {
        ...
        if (!DocumentWriteMask.TryParse(text: token, mask: out var mask, unknown: out var unknown)) {
            throw new JsonException(message: $"a write mask is a comma-separated list of {DocumentWriteMask.All.Describe()}; '{(string.IsNullOrEmpty(value: unknown) ? (token ?? "(absent)") : unknown)}' names no declared operation.");
        }
...
internal sealed class WorldStateRowJsonConverter : JsonConverter<WorldStateRow> {
    ...
    public override WorldStateRow Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options) {
        ...
        while (reader.Read() && (reader.TokenType != JsonTokenType.EndObject)) {
            ...
            switch (property) {
                case "name":
                    ...
                    break;
                case "kind":
                    kind = JsonElement.ParseValue(ref reader);
                    break;
                case "value":
                    value = JsonElement.ParseValue(ref reader);
                    break;
                case "cells":
                    cells = JsonElement.ParseValue(ref reader);
                    break;
                case "min":
                    min = JsonElement.ParseValue(ref reader);
                    break;
                case "max":
                    max = JsonElement.ParseValue(ref reader);
                    break;
                case "capacity":
                    capacity = JsonElement.ParseValue(ref reader);
                    break;
                case "nonNegative":
                    nonNegative = JsonElement.ParseValue(ref reader);
                    break;
                case "gatesDrive":
                    gatesDrive = JsonElement.ParseValue(ref reader);
                    break;
                case "evicts":
                    evicts = JsonElement.ParseValue(ref reader);
                    break;
                case "draw":
                    draw = JsonElement.ParseValue(ref reader);
                    break;
                case "drawCursor":
                    drawCursor = JsonElement.ParseValue(ref reader);
                    break;
                case "drawDecks":
                    drawDecks = JsonElement.ParseValue(ref reader);
                    break;
                case "advance":
                    advance = JsonElement.ParseValue(ref reader);
                    break;
                default:
        ...
        if (!WorldCellName.TryParse(candidate: name, name: out var rowName, reason: out var nameReason)) {
        ...
        if ((value is not null) && (cells is not null)) {
        ...
        if ((value is not null) && (capacity is not null)) {
        ...
        if ((advance is not null) && (draw is not null)) {
        ...
        if ((draw is not null) && (capacity is not null)) {
        ...
        if ((drawCursor is not null) && (draw is null)) {
        ...
        if ((drawDecks is not null) && (draw is null)) {
        ...
        if ((advance is not null) && (capacity is not null)) {
        ...
        if ((advance is not null) && (cells is { ValueKind: JsonValueKind.Array } authored) && (authored.GetArrayLength() > 0)) {
        ...
        var cellKind = RequireCellKind(element: kindElement, context: $"state row '{name}'.kind");

        return new WorldStateRow(
            Name: rowName,
            Kind: cellKind,
            Min: ((min is { } minElement) ? RequireNumeric(kind: cellKind, element: minElement, context: $"state row '{name}'.min") : null),
            Max: ((max is { } maxElement) ? RequireNumeric(kind: cellKind, element: maxElement, context: $"state row '{name}'.max") : null),
            Capacity: ((capacity is { } capacityElement) ? RequireInt32(element: capacityElement, context: $"state row '{name}'.capacity") : null),
            NonNegative: ((nonNegative is { } nonNegativeElement) && RequireBool(element: nonNegativeElement, context: $"state row '{name}'.nonNegative")),
            GatesDrive: ((gatesDrive is { } gatesDriveElement) && RequireBool(element: gatesDriveElement, context: $"state row '{name}'.gatesDrive")),
            Evicts: ((evicts is { } evictsElement) && RequireBool(element: evictsElement, context: $"state row '{name}'.evicts")),
            Cells: ((value is { } valueElement)
                ? [ReadCell(cellKind: cellKind, key: WorldStateRow.SlotKey, element: valueElement, context: $"state row '{name}'.value")]
                : ((cells is { } cellsElement) ? ReadCells(name: name, cellKind: cellKind, cellsElement: cellsElement) : [])),
            ...
            Draw: ((draw is { } drawElement)
                ...
                : null),
            DrawCursor: ((drawCursor is { } drawCursorElement) ? RequireInt64(element: drawCursorElement, context: $"state row '{name}'.drawCursor") : 0L),
            DrawDecks: ((drawDecks is { } drawDecksElement) ? ReadDrawDecks(name: name, element: drawDecksElement) : null)
        );
    }
    ...
    private static List<long> ReadDrawDecks(string name, JsonElement element) {
        ...
        foreach (var entry in element.EnumerateArray()) {
            masks.Add(item: RequireInt64(element: entry, context: $"state row '{name}'.drawDecks[{masks.Count}]"));
        }
    ...
    private static List<WorldStateCell> ReadCells(string name, CellKind cellKind, JsonElement cellsElement) {
        ...
        foreach (var entry in cellsElement.EnumerateArray()) {
            ...
            foreach (var member in entry.EnumerateObject()) {
                switch (member.Name) {
                    case "key":
                        key = ((member.Value.ValueKind == JsonValueKind.String) ? member.Value.GetString() : null);
                        break;
                    case "value":
            ...
            if (!WorldCellName.TryParse(candidate: key, name: out var cellKey, reason: out var keyReason)) {
    ...
    public override void Write(Utf8JsonWriter writer, WorldStateRow value, JsonSerializerOptions options) {
        ...
        writer.WriteStartObject();
        writer.WriteString(propertyName: "name", value: value.Name);
        writer.WriteString(propertyName: "kind", value: DescribeCellKind(cellKind: value.Kind));
        WriteOptionalNumeric(writer: writer, propertyName: "min", kind: value.Kind, raw: value.Min);
        WriteOptionalNumeric(writer: writer, propertyName: "max", kind: value.Kind, raw: value.Max);

        if (value.Capacity is { } declaredCapacity) {
            writer.WriteNumber(propertyName: "capacity", value: declaredCapacity);
        }

        if (value.NonNegative) {
            writer.WriteBoolean(propertyName: "nonNegative", value: true);
        }

        if (value.GatesDrive) {
            writer.WriteBoolean(propertyName: "gatesDrive", value: true);
        }

        if (value.Evicts) {
            writer.WriteBoolean(propertyName: "evicts", value: true);
        }
        ...
        if (value.IsSlot) {
            WriteCellValue(writer: writer, propertyName: "value", kind: value.Kind, cell: value.Cells![0]);
        } else {
            writer.WriteStartArray(propertyName: "cells");

            foreach (var cell in (value.Cells ?? [])) {
                writer.WriteStartObject();
                writer.WriteString(propertyName: "key", value: cell.Key);
                WriteCellValue(writer: writer, propertyName: "value", kind: value.Kind, cell: cell);
        ...
        if (value.Advance is { } advance) {
            writer.WritePropertyName(propertyName: "advance");
            JsonSerializer.Serialize(writer: writer, value: advance, jsonTypeInfo: WorldJsonContext.Default.WorldStateAdvance);
        }

        if (value.Draw is { } draw) {
            writer.WritePropertyName(propertyName: "draw");
            JsonSerializer.Serialize(writer: writer, value: draw, jsonTypeInfo: WorldJsonContext.Default.WorldDraw);
        }

        if (value.DrawCursor != 0L) {
            writer.WriteNumber(propertyName: "drawCursor", value: value.DrawCursor);
        }
    ...
    private static void WriteCellValue(Utf8JsonWriter writer, string propertyName, CellKind kind, WorldStateCell cell) {
        switch (kind) {
            case CellKind.Text:
                writer.WriteString(propertyName: propertyName, value: (cell.Text ?? string.Empty));
                break;
            case CellKind.Bool:
                writer.WriteBoolean(propertyName: propertyName, value: (cell.Value != 0));
                break;
            case CellKind.Fixed:
                writer.WriteString(propertyName: propertyName, value: FixedQ4816.FromRawBits(value: cell.Value).ToString());
                break;
            default:
                writer.WriteNumber(propertyName: propertyName, value: cell.Value);
                break;
        }
    }
    ...
    private static void WriteOptionalNumeric(Utf8JsonWriter writer, string propertyName, CellKind kind, long? raw) {
        ...
        if (kind == CellKind.Fixed) {
            writer.WriteString(propertyName: propertyName, value: FixedQ4816.FromRawBits(value: rawValue).ToString());
        } else {
            writer.WriteNumber(propertyName: propertyName, value: rawValue);
        }
    }
    ...
    private static long RequireNumeric(CellKind kind, JsonElement element, string context) => kind switch {
        CellKind.Fixed => RequireFixed(element: element, context: context),
        _ => RequireInt64(element: element, context: context),
    };

    private static long RequireInt64(JsonElement element, string context) {
        if ((element.ValueKind != JsonValueKind.Number) || !element.TryGetInt64(value: out var parsed)) {
    ...
    private static int RequireInt32(JsonElement element, string context) {
        if ((element.ValueKind != JsonValueKind.Number) || !element.TryGetInt32(value: out var parsed)) {
            throw new JsonException(message: $"{context} must be a whole number.");
        }
    ...
    private static long RequireFixed(JsonElement element, string context) {
        if ((element.ValueKind != JsonValueKind.String) || !FixedQ4816.TryParse(s: element.GetString(), provider: CultureInfo.InvariantCulture, result: out var parsed)) {
...
internal sealed class GrantSubjectJsonConverter : JsonConverter<GrantSubject> {
    ...
    public override GrantSubject Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options) {
        ...
        if ((token is null) || !GrantSubject.TryParse(token: token, subject: out var subject)) {
...
internal sealed class WorldPrincipalJsonConverter : JsonConverter<WorldPrincipal> {
    ...
    public override WorldPrincipal Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options) {
        ...
        if ((token is null) || !WorldPrincipal.TryParse(token: token, principal: out var principal)) {
...
internal sealed class CreationDocumentJsonConverter : JsonConverter<Puck.Forge.Authoring.CreationDocument> {
    ...
    public override Puck.Forge.Authoring.CreationDocument? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options) =>
        JsonSerializer.Deserialize<Puck.Forge.Authoring.CreationDocument>(reader: ref reader, options: Puck.Forge.Authoring.DocumentJsonOptions.Shared);
    ...
    public override void Write(Utf8JsonWriter writer, Puck.Forge.Authoring.CreationDocument value, JsonSerializerOptions options) =>
        JsonSerializer.Serialize(writer: writer, value: value, options: Puck.Forge.Authoring.DocumentJsonOptions.Shared);
}
...
internal sealed class AudioDocumentJsonConverter : JsonConverter<Puck.Forge.Authoring.AudioDocument> {
    ...
    public override Puck.Forge.Authoring.AudioDocument? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options) =>
        JsonSerializer.Deserialize<Puck.Forge.Authoring.AudioDocument>(reader: ref reader, options: Puck.Forge.Authoring.DocumentJsonOptions.Shared);
...
internal sealed class SynthPatchDocumentJsonConverter : JsonConverter<Puck.Forge.Authoring.SynthPatchDocument> {
    ...
    public override Puck.Forge.Authoring.SynthPatchDocument? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options) =>
        JsonSerializer.Deserialize<Puck.Forge.Authoring.SynthPatchDocument>(reader: ref reader, options: Puck.Forge.Authoring.DocumentJsonOptions.Shared);
