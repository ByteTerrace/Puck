...
public sealed class WorldFaceCatalog {
    ...
    private static readonly ConditionalWeakTable<WorldDefinition, WorldFaceCatalog> s_perDefinition = new();
    private static readonly FixedQ4816 s_degreesToRadians = FixedQ4816.FromDouble(value: (Math.PI / 180.0));
    private static readonly FixedQ4816 s_fallbackHalfDepth = FixedQ4816.FromDouble(value: FallbackHalfDepth);
    private static readonly FixedQ4816 s_fallbackHalfHeight = FixedQ4816.FromDouble(value: FallbackHalfHeight);
    private static readonly FixedQ4816 s_fallbackHalfWidth = FixedQ4816.FromDouble(value: FallbackHalfWidth);
    ...
    public static WorldFaceCatalog For(WorldDefinition definition) {
        ...
        return s_perDefinition.GetValue(key: definition, createValueCallback: static built => Derive(definition: built));
    }
    ...
    public bool TryFind(string placementId, string faceName, out WorldFaceRow row) {
        if (m_index.TryGetValue(key: (placementId, faceName), value: out var index)) {
            row = Rows[index];
    ...
    private static WorldFaceCatalog Derive(WorldDefinition definition) {
        ...
        foreach (var placement in definition.Placements) {
            if ((placement is null) || (WorldDefinitionRows.FindCreation(creations: definition.Creations, id: placement.CreationId) is not { } creation)) {
                continue;
            }

            foreach (var face in (creation.Document.Behavior?.Faces ?? [])) {
                var shape = FindShape(document: creation.Document, id: face.ShapeId);
                var source = (FindOverride(faceSources: placement.FaceSources, face: face.Name)
                    ?? ParseDefaultSource(token: face.DefaultSource, cameras: cameras, definition: definition, placementId: placement.Id, faceName: face.Name, notices: notices)
                ...
                rows.Add(item: new WorldFaceRow(
                    ...
                    Frame: DeriveFrame(placement: placement, shape: shape),
        ...
        return new WorldFaceCatalog(rows: rows, notices: notices, claimingFaceCount: claiming, slotCapacity: capacity);
    }
    ...
    private static WorldFaceFrame DeriveFrame(WorldPlacement placement, ShapeDocument? shape) {
        ...
        if (((shape is null) || shape.Rotation.Equals(other: Quaternion.Identity)) &&
            TryCardinalBasis(yawDegrees: yawDegrees, right: out var cardinalRight, normal: out var cardinalNormal)) {
            if (shape is null) {
                return new WorldFaceFrame(
                    ...
                    HalfWidth: s_fallbackHalfWidth,
                    HalfHeight: s_fallbackHalfHeight,
                    HalfDepth: s_fallbackHalfDepth
                );
            }
            ...
            return new WorldFaceFrame(
                Origin: (origin + (cardinalRight * local.X) + (s_unitY * local.Y) + (cardinalNormal * local.Z)),
                Right: cardinalRight,
                Up: s_unitY,
                Normal: cardinalNormal,
        ...
        var placementRotation = FixedQuaternion.FromAxisAngle(
            axis: s_unitY,
            angle: (yawDegrees * s_degreesToRadians)
        );

        if (shape is null) {
            return new WorldFaceFrame(
                Origin: origin,
                Right: placementRotation.Rotate(vector: s_unitX).Normalize(),
                Up: placementRotation.Rotate(vector: s_unitY).Normalize(),
                Normal: placementRotation.Rotate(vector: s_unitZ).Normalize(),
                HalfWidth: s_fallbackHalfWidth,
                HalfHeight: s_fallbackHalfHeight,
                HalfDepth: s_fallbackHalfDepth
            );
        ...
        return new WorldFaceFrame(
            ...
            Right: surfaceRotation.Rotate(vector: s_unitX).Normalize(),
            Up: surfaceRotation.Rotate(vector: s_unitY).Normalize(),
            Normal: surfaceRotation.Rotate(vector: s_unitZ).Normalize(),
    ...
    private static WorldFaceApertureKind ApertureFor(ShapeDocument? shape) =>
        ((shape is { Type: AvatarPrimitive.Box }) ? WorldFaceApertureKind.Box : WorldFaceApertureKind.None);

    private static ShapeDocument? FindShape(CreationDocument document, int? id) {
        if ((id is not { } targetId) || (targetId < 0)) {
            return null;
        }
    ...
    private static WorldScreenSource? FindOverride(IReadOnlyList<WorldPlacementFace>? faceSources, string face) {
        foreach (var entry in (faceSources ?? [])) {
            if ((entry is not null) && string.Equals(a: entry.Face, b: face, comparisonType: StringComparison.Ordinal)) {
                return entry.Source;
            }
        }
