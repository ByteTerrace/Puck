...
public readonly record struct WorldPrincipal(PrincipalKind Kind, int Index, string? Name, int Generation) {
    ...
    public static WorldPrincipal Seat(int slot) => new(Kind: PrincipalKind.Seat, Index: slot, Name: null, Generation: 0);

    /// <summary>Returns the addon principal for a descriptor name.</summary>
    /// <param name="name">The addon descriptor name.</param>
    public static WorldPrincipal Addon(string name) => new(Kind: PrincipalKind.Addon, Index: 0, Name: name, Generation: 0);
    ...
    public static WorldPrincipal Peer(int index, int generation) => new(Kind: PrincipalKind.Peer, Index: index, Name: null, Generation: generation);
    ...
    /// </remarks>
    public static WorldPrincipal World { get; } = new(Kind: PrincipalKind.World, Index: 0, Name: null, Generation: 0);

    /// <summary>Returns the group principal for a stable group id.</summary>
    /// <param name="id">The group's stable id.</param>
    public static WorldPrincipal Group(string id) => new(Kind: PrincipalKind.Group, Index: 0, Name: id, Generation: 0);
    ...
    public static bool TryParse(ReadOnlySpan<char> token, out WorldPrincipal principal) {
        ...
        if (token.Equals(other: "world", comparisonType: StringComparison.OrdinalIgnoreCase)) {
            principal = World;
        ...
        if (token.StartsWith(value: "peer:", comparisonType: StringComparison.OrdinalIgnoreCase)) {
            ...
            if ((separator <= 0) ||
                !int.TryParse(s: remainder[..separator], style: NumberStyles.Integer, provider: CultureInfo.InvariantCulture, result: out var peer) || !WorldPopulationLimits.IsPeerIndex(index: peer) ||
                !int.TryParse(s: remainder[(separator + 1)..], style: NumberStyles.Integer, provider: CultureInfo.InvariantCulture, result: out var generation) || (generation <= 0)) {
                return false;
            }

            principal = Peer(index: peer, generation: generation);

            return true;
        }

        if (token.StartsWith(value: "document:", comparisonType: StringComparison.OrdinalIgnoreCase) && (token.Length > 9)) {
