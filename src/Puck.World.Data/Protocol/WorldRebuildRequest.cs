...
public enum WorldRebuildKind {
    /// <summary>Rebuild from the server's own in-memory base (<c>world.reset</c>) — the last <c>world.save</c>, or
    /// the boot document if never saved. Carries no document of its own; the server resolves it from its own base.</summary>
    Reset,

    /// <summary>Rebuild from a different document read from disk (<c>world.load &lt;path&gt; [force]</c>).</summary>
    Load,

    /// <summary>Re-read the current document origin from disk and rebuild from it (<c>world.reload</c>) — the
    /// artist external-edit loop.</summary>
    Reload,
}
...
/// domain, so this record carries only the result of that resolution (or, for <see cref="WorldRebuildKind.Reset"/>,
/// nothing at all: the base is server state, never client-supplied).</summary>
...
/// to discard the in-session journal on every reload).</param>
/// <param name="ContentHash">The canonical <c>sha256-64/{hex}</c> pin of the exact bytes the console read off disk
/// — required for <see cref="WorldRebuildKind.Load"/>/<see cref="WorldRebuildKind.Reload"/> (computed once, at the
