...
public readonly record struct ChannelReachMask(ulong Bits) {
    ...
    public ChannelHeldMask Meet(ChannelConsentMask consent) =>
        new(Bits: MeetMask64.Meet(left: new MeetMask64(Bits: Bits), right: new MeetMask64(Bits: consent.Bits)).Bits);
...
public readonly struct ChannelCeilings {
    ...
    public ChannelCeilings WithCeiling(ChannelConsentMask channels, long ceiling) {
        ArgumentOutOfRangeException.ThrowIfLessThanOrEqual(value: ceiling, other: 0L);
        ...
        while (remaining != 0UL) {
            var ordinal = BitOperations.TrailingZeroCount(remaining);

            values[ordinal] = ceiling;
            remaining &= (remaining - 1UL);
        }

        return new ChannelCeilings(values: values, support: new ChannelConsentMask(Bits: (Support.Bits | validBits)));
    }
}
