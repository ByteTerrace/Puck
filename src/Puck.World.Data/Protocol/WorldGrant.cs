...
/// <summary>The world-document sections the <see cref="WorldMutation"/> vocabulary targets — the stable-id subject a
...
public enum WorldSection : byte {
    ...
    /// <see cref="WorldMutation.SetProperty"/>.</summary>
    Properties,
...
public readonly record struct GrantSubject(GrantSubjectKind Kind, int Value, string? Id = null) {
    ...
    public static bool TryParseSectionName(ReadOnlySpan<char> name, out WorldSection section) {
        ...
        return (!name.IsEmpty) &&
            ...
            Enum.TryParse<WorldSection>(value: name, ignoreCase: true, result: out section) &&
    ...
    public static GrantSubject Region(string name) => new(Kind: GrantSubjectKind.Region, Value: 0, Id: name);

    /// <summary>Creates a single local seat by 0-based slot index.</summary>
    /// <param name="index">The 0-based seat index.</param>
    public static GrantSubject Seat(int index) => new(Kind: GrantSubjectKind.Seat, Value: index);
    ...
    public static bool TryParse(ReadOnlySpan<char> token, out GrantSubject subject) {
        ...
        if (token.StartsWith(value: "section:", comparisonType: StringComparison.OrdinalIgnoreCase) &&
            (token.Length > 8) &&
            TryParseSectionName(name: token[8..], section: out var section)) {
            subject = Section(section: section);

            return true;
        }

        if (token.StartsWith(value: "state:", comparisonType: StringComparison.OrdinalIgnoreCase) && (token.Length > 6)) {
            subject = State(name: token[6..].ToString());

            return true;
        }

        if (token.StartsWith(value: "region:", comparisonType: StringComparison.OrdinalIgnoreCase) && (token.Length > 7)) {
        ...
        if (token.StartsWith(value: "seat:", comparisonType: StringComparison.OrdinalIgnoreCase) &&
            int.TryParse(s: token[5..], style: NumberStyles.Integer, provider: CultureInfo.InvariantCulture, result: out var seat) && (seat >= 0)) {
