...
public static class WorldAdmissionDoor {
    ...
    /// <param name="Domain">The verified identity's domain, when admitted.</param>
    /// <param name="Subject">The verified identity's subject, when admitted (empty, never null, for a Vouches
    /// entry's own chain-resolved subject — the entry itself pins no subject, but the two-hop chain's leaf always
    /// names one; see <see cref="TryAdmit"/>).</param>
    /// <param name="Grants">The admitting entry's own authored grant templates, when admitted.</param>
    public readonly record struct AdmissionOutcome(bool Admitted, WorldAdmissionRefusal? Refusal, string Detail, string? Domain, string? Subject, IReadOnlyList<WorldAdmissionGrant>? Grants) {
        /// <summary>Builds a refusal outcome.</summary>
        public static AdmissionOutcome Refuse(WorldAdmissionRefusal refusal, string detail) => new(Admitted: false, Refusal: refusal, Detail: detail, Domain: null, Subject: null, Grants: null);

        /// <summary>Builds an admitted outcome.</summary>
        public static AdmissionOutcome Admit(string domain, string subject, IReadOnlyList<WorldAdmissionGrant> grants) => new(Admitted: true, Refusal: null, Detail: string.Empty, Domain: domain, Subject: subject, Grants: grants);
    }
    ...
    public static AdmissionOutcome TryAdmit(
        ...
        if (entries is not { Count: > 0 } rows) {
            return AdmissionOutcome.Refuse(WorldAdmissionRefusal.NoAdmissionEntries, "this world authors no admission entries; no remote peer can ever verify");
        }
        ...
        if (!TryMatchEntry(entries: rows, domain: domain, subject: subject, grants: out var matchedGrants)) {
        ...
        return AdmissionOutcome.Admit(domain: domain, subject: (subject ?? string.Empty), grants: matchedGrants);
    }
    ...
    /// <param name="grants">The matching entry's own current grant templates, when matched; empty when not.</param>
    /// <returns><see langword="true"/> when an entry matches.</returns>
    public static bool TryMatchEntry(IReadOnlyList<WorldAdmissionEntry>? entries, string? domain, string? subject, out IReadOnlyList<WorldAdmissionGrant> grants) {
