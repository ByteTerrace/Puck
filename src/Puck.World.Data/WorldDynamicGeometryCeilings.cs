...
/// <summary>
/// Contributed dynamic-geometry ceilings (ratified 2026-08-02): the per-frame dynamic-instance
/// count a document's contributed dynamic content (e.g. a scripted addon, a creator-mode animated placement pool)
/// may add while keeping its OWN share of the 60 Hz frame budget under the ratified policy — <see cref="GpuBudgetMilliseconds"/>
/// GPU and <see cref="CpuBudgetMilliseconds"/> CPU per frame — measured on the local RTX 4070 and derated
/// <see cref="RdnaDerateFactor"/>x for the RDNA2 floor (the GPU support matrix's weakest of the four calibrated GPUs).
/// <para>
/// <b>Wired 2026-08-02.</b> <see cref="MaxContributedDynamicInstances"/> gates
...
/// program words/instances at apply time; this constant is now consumed at DOCUMENT validation instead of staying an
/// unread number.
/// </para>
/// </summary>
/// <remarks>
/// <para><b>Measurement protocol.</b> Device: the local RTX 4070 (the ONLY hardware this measurement pass covered — the GPU
/// support matrix's other three calibrated GPUs were not re-run; all historical figures were treated as dirty and
/// nothing here cites them). Harness: <c>src/Puck.SdfVm.Bench</c> (new — a real GPU/CPU ceiling harness built from
...
/// Per the owner's pacing ruling for this measurement pass (bisect, don't densify), two single rungs — N=8 then N=1 — were
...
/// default shading quality and 1280x800, the ratified 60 Hz budget for contributed dynamic geometry has no headroom
...
/// (<see cref="Puck.SdfVm.SdfWorldEngine.LastInstanceGridRebuildMilliseconds"/>) at the top of the tested range,
...
public static class WorldDynamicGeometryCeilings {
    /// <summary>The ratified GPU budget for contributed dynamic geometry's OWN share of one frame, at 60 Hz, milliseconds.</summary>
    public const double GpuBudgetMilliseconds = 1.0;
    /// <summary>The ratified CPU budget for contributed dynamic geometry's OWN share of one frame, at 60 Hz, milliseconds.</summary>
    ...
    /// contributed dynamic-instance content against — the CPU/grid bound, NOT the GPU bound. <b>Owner ruling, 2026-08-02:
    /// authors own the frame budget.</b> The measurement above proved GPU cost is per-COVERED-PIXEL (shading
    /// quality x screen coverage), not per-instance — one framed object pays the shading cost nearly in full, so a
    /// per-instance GPU police prices to zero and would gate the feature off while bounding nothing real. The
    /// separate contributed-GPU-share budget is therefore DISSOLVED: GPU cost is the world author's frame-budget
    /// responsibility, like every other content choice they author (the same authors-decide posture as
    /// <c>addons.guestStateCapture</c>). <see cref="MaxContributedDynamicInstancesGpuBound"/> stays recorded as the
    /// measured fact that forced the ruling; the cheaper-shading-tier and smaller-target levers in the remarks
    /// remain available to authors, not mandates.</summary>
