...
public static class WorldSequenceSampling {
    ...
    public static float Scalar(WorldSequence sequence, int index) {
        var adjusted = AdjustedIndex(sequence: sequence, index: index);
        ...
        if (string.Equals(a: sequence.Name, b: WorldSequence.R1, comparisonType: StringComparison.Ordinal)) {
    ...
    public static (FixedQ4816 X, FixedQ4816 Y) FixedPair(WorldSequence sequence, int index) {
        if (!string.Equals(a: sequence.Name, b: WorldSequence.R2, comparisonType: StringComparison.Ordinal)) {
            throw new InvalidOperationException(message: $"Sequence '{sequence.Name}' does not produce a paired sample.");
        }

        var (x, y) = LowDiscrepancy.R2(index: AdjustedIndex(sequence: sequence, index: index));
    ...
    public static FixedQ4816 FixedAngle(WorldSequence sequence, int index) {
        ...
        return (FixedQ4816.FromInteger(value: checked((long)AdjustedIndex(sequence: sequence, index: index))) *
    ...
    public static int Bucket(WorldSequence sequence, int index, int count) {
        ...
        if (string.Equals(a: sequence.Name, b: WorldSequence.Index, comparisonType: StringComparison.Ordinal)) {
        ...
        if (string.Equals(a: sequence.Name, b: WorldSequence.Additive, comparisonType: StringComparison.Ordinal)) {
            return Math.Min(val1: (count - 1), val2: (int)(Scalar(sequence: sequence, index: index) * count));
        }
