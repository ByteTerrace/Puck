...
public static class WorldRuleCompiler {
    ...
    public static CompiledWorldRule[] CompileAll(WorldDefinition definition) {
        ...
        for (var index = 0; (index < rules.Count); index++) {
            ...
            if (string.IsNullOrWhiteSpace(value: name)) {
                throw new WorldRuleException(refusal: WorldRuleRefusal.NameMissing, ruleName: "<unnamed>", detail: "a rule declares a name");
            }
            ...
            if (name.StartsWith(value: WorldStateRow.ReservedNamePrefix, comparisonType: StringComparison.Ordinal)) {
                throw new WorldRuleException(refusal: WorldRuleRefusal.NameReserved, ruleName: name,
            ...
            if (!seen.Add(item: name)) {
                throw new WorldRuleException(refusal: WorldRuleRefusal.NameDuplicated, ruleName: name, detail: "duplicates an earlier rule's name");
            }

            compiled[index] = Compile(rule: rule!, definition: definition);
        }
    ...
    public static CompiledWorldRule[] CompileAllInteractions(WorldDefinition definition) {
        ...
        for (var index = 0; (index < interactions.Count); index++) {
            ...
            if (string.IsNullOrWhiteSpace(value: name)) {
                throw new WorldRuleException(refusal: WorldRuleRefusal.NameMissing, ruleName: "<unnamed>", detail: "an interaction declares a name", subject: "interaction");
            }
            ...
            if (name.StartsWith(value: WorldStateRow.ReservedNamePrefix, comparisonType: StringComparison.Ordinal)) {
                throw new WorldRuleException(refusal: WorldRuleRefusal.NameReserved, ruleName: name,
                    detail: $"carries the reserved character '{WorldStateRow.ReservedNamePrefix}' as its first character — that prefix marks what the ENGINE mints, and nothing mints an interaction", subject: "interaction");
            }
            if (!seen.Add(item: name)) {
                throw new WorldRuleException(refusal: WorldRuleRefusal.NameDuplicated, ruleName: name, detail: "duplicates an earlier interaction's name", subject: "interaction");
            }
            ...
            if (!registry.Contains(item: row.Left)) {
                throw new WorldRuleException(refusal: WorldRuleRefusal.PropertyUnknown, ruleName: name,
                    detail: $"'left' names '{row.Left}', which is not a registered property (see the 'properties' section)", subject: "interaction");
            }
            ...
            switch (row.CoOccurrence) {
                case WorldInteractionCoOccurrence.Distance:
                    if (!registry.Contains(item: row.Right)) {
                        throw new WorldRuleException(refusal: WorldRuleRefusal.PropertyUnknown, ruleName: name,
                            detail: $"'right' names '{row.Right}', which is not a registered property (see the 'properties' section)", subject: "interaction");
                    }
                ...
                case WorldInteractionCoOccurrence.Region:
                    ...
                    gate = new ActionPredicate.All(Predicates: [
                        new ActionPredicate.CompareState(State: $"{WorldRuleFacts.ArgMaxPrefix}{row.Left}", Comparison: ActionStateComparison.NotEqual, Value: -1f),
                        new ActionPredicate.CompareState(State: $"{WorldRuleFacts.RegionPrefix}{row.Right}", Comparison: ActionStateComparison.GreaterOrEqual, Value: 1f),
                    ]);

                    break;
                default:
                    throw new WorldRuleException(refusal: WorldRuleRefusal.PredicateKindInadmissible, ruleName: name,
                        detail: $"'coOccurrence' value '{row.CoOccurrence}' is not a defined WorldInteractionCoOccurrence", subject: "interaction");
            }

            var synthesized = new WorldRule(Name: row.Name, Gate: gate, Effects: row.Effects, Mode: row.Mode);

            compiled[index] = Compile(rule: synthesized, definition: definition);
        }
    ...
    public static CompiledWorldRule Compile(WorldRule rule, WorldDefinition definition) {
        ...
        FlattenPredicate(predicate: rule.Gate, gate: gate, ruleName: rule.Name, definition: definition);

        var effects = new CompiledWorldEffect[rule.Effects.Count];

        for (var index = 0; (index < effects.Length); index++) {
            effects[index] = CompileEffect(effect: rule.Effects[index], ruleName: rule.Name, definition: definition);
        }

        return new CompiledWorldRule(Name: rule.Name, Mode: rule.Mode, Gate: gate.ToArray(), Effects: effects);
    }
        ...
        switch (predicate) {
            ...
            case ActionPredicate.All all:
                foreach (var inner in all.Predicates) {
                    FlattenPredicate(predicate: inner, gate: gate, ruleName: ruleName, definition: definition);
                }

                break;
            case ActionPredicate.CompareState compare:
                gate.Add(item: ResolvePredicate(compare: compare, ruleName: ruleName, definition: definition));

                break;
            default:
                throw new WorldRuleException(refusal: WorldRuleRefusal.PredicateKindInadmissible, ruleName: ruleName,
    ...
    private static ResolvedOperand ResolveOperand(string name, string? key, string ruleName, WorldDefinition definition, string verb, string fieldLabel, string keyFieldLabel) {
        var describe = $"{name}{((key is { } spelledKey) ? $".{spelledKey}" : string.Empty)}";

        if (string.Equals(a: name, b: WorldRuleFacts.Tick, comparisonType: StringComparison.Ordinal)) {
            RefuseKeyOnReservedChannel(key: key, ruleName: ruleName, name: name, keyFieldLabel: keyFieldLabel);

            return new ResolvedOperand(Operand: new CompiledWorldOperand(Kind: WorldRuleFactKind.Tick, Row: null, Key: null), ValueKind: CellKind.Int, Describe: describe);
        }

        if (string.Equals(a: name, b: WorldRuleFacts.Population, comparisonType: StringComparison.Ordinal)) {
            RefuseKeyOnReservedChannel(key: key, ruleName: ruleName, name: name, keyFieldLabel: keyFieldLabel);

            return new ResolvedOperand(Operand: new CompiledWorldOperand(Kind: WorldRuleFactKind.Population, Row: null, Key: null), ValueKind: CellKind.Int, Describe: describe);
        }

        if (name.StartsWith(value: WorldRuleFacts.RegionPrefix, comparisonType: StringComparison.Ordinal)) {
            RefuseKeyOnReservedChannel(key: key, ruleName: ruleName, name: name, keyFieldLabel: keyFieldLabel);

            var placementId = name[WorldRuleFacts.RegionPrefix.Length..];

            if (string.IsNullOrEmpty(value: placementId) || !HasRegion(definition: definition, placementId: placementId)) {
                throw new WorldRuleException(refusal: WorldRuleRefusal.RegionUnknown, ruleName: ruleName, detail: $"'{name}' names no placement carrying a region facet");
            }

            return new ResolvedOperand(Operand: new CompiledWorldOperand(Kind: WorldRuleFactKind.RegionOccupancy, Row: placementId, Key: null), ValueKind: CellKind.Int, Describe: describe);
        }

        if (name.StartsWith(value: WorldRuleFacts.MachinePrefix, comparisonType: StringComparison.Ordinal)) {
            RefuseKeyOnReservedChannel(key: key, ruleName: ruleName, name: name, keyFieldLabel: keyFieldLabel);
            ...
            if ((separator < 0)
                || !int.TryParse(s: suffix[..separator], style: System.Globalization.NumberStyles.Integer, provider: System.Globalization.CultureInfo.InvariantCulture, result: out var screen)
                || !int.TryParse(s: suffix[(separator + 1)..], style: System.Globalization.NumberStyles.Integer, provider: System.Globalization.CultureInfo.InvariantCulture, result: out var address)
                || (screen < 0)
                || (address < 0)) {
                throw new WorldRuleException(refusal: WorldRuleRefusal.MachineChannelMalformed, ruleName: ruleName,
                    detail: $"'{name}' does not spell '{WorldRuleFacts.MachinePrefix}<screen>:<address>' with non-negative integers");
            }

            if (!HasScreen(definition: definition, index: screen)) {
                throw new WorldRuleException(refusal: WorldRuleRefusal.ScreenUnknown, ruleName: ruleName, detail: $"'{name}' names screen {screen}, which the document does not declare");
            }

            return new ResolvedOperand(Operand: new CompiledWorldOperand(Kind: WorldRuleFactKind.MachineMemory, Row: null, Key: null, Screen: screen, Address: address), ValueKind: CellKind.Int, Describe: describe);
        }

        if (name.StartsWith(value: WorldRuleFacts.ReducePrefix, comparisonType: StringComparison.Ordinal)) {
            RefuseKeyOnReservedChannel(key: key, ruleName: ruleName, name: name, keyFieldLabel: keyFieldLabel);
            ...
            if ((separator < 0) || !TryParseReduceOp(text: suffix[..separator], op: out var op) || string.IsNullOrEmpty(value: suffix[(separator + 1)..])) {
                throw new WorldRuleException(refusal: WorldRuleRefusal.ReduceChannelMalformed, ruleName: ruleName,
                    detail: $"'{name}' does not spell '{WorldRuleFacts.ReducePrefix}<max|min|sum|count>:<row>'");
            }

            var rowName = suffix[(separator + 1)..];
            var reduceRow = ResolveNumericRow(name: rowName, ruleName: ruleName, definition: definition, requireKeyed: false, malformed: WorldRuleRefusal.ReduceChannelMalformed, channel: name);
            var reduceValueKind = ((op == WorldStateReduceOp.Count) ? CellKind.Int : reduceRow.Kind);

            return new ResolvedOperand(Operand: new CompiledWorldOperand(Kind: WorldRuleFactKind.Reduction, Row: rowName, Key: null, Reduce: op), ValueKind: reduceValueKind, Describe: describe);
        }

        if (name.StartsWith(value: WorldRuleFacts.ArgMaxPrefix, comparisonType: StringComparison.Ordinal) || name.StartsWith(value: WorldRuleFacts.ArgMinPrefix, comparisonType: StringComparison.Ordinal)) {
            RefuseKeyOnReservedChannel(key: key, ruleName: ruleName, name: name, keyFieldLabel: keyFieldLabel);

            var isMax = name.StartsWith(value: WorldRuleFacts.ArgMaxPrefix, comparisonType: StringComparison.Ordinal);
            var rowName = name[(isMax ? WorldRuleFacts.ArgMaxPrefix.Length : WorldRuleFacts.ArgMinPrefix.Length)..];

            if (string.IsNullOrEmpty(value: rowName)) {
                throw new WorldRuleException(refusal: WorldRuleRefusal.ArgChannelMalformed, ruleName: ruleName,
                    detail: $"'{name}' does not spell '{(isMax ? WorldRuleFacts.ArgMaxPrefix : WorldRuleFacts.ArgMinPrefix)}<row>'");
            }

            _ = ResolveNumericRow(name: rowName, ruleName: ruleName, definition: definition, requireKeyed: true, malformed: WorldRuleRefusal.ArgChannelMalformed, channel: name);

            return new ResolvedOperand(Operand: new CompiledWorldOperand(Kind: WorldRuleFactKind.ArgBody, Row: rowName, Key: null, Reduce: (isMax ? WorldStateReduceOp.Max : WorldStateReduceOp.Min)), ValueKind: CellKind.Int, Describe: describe);
        }

        if (name.StartsWith(value: WorldRuleFacts.DistancePrefix, comparisonType: StringComparison.Ordinal) || name.StartsWith(value: WorldRuleFacts.LineOfSightPrefix, comparisonType: StringComparison.Ordinal)) {
            RefuseKeyOnReservedChannel(key: key, ruleName: ruleName, name: name, keyFieldLabel: keyFieldLabel);

            var isDistance = name.StartsWith(value: WorldRuleFacts.DistancePrefix, comparisonType: StringComparison.Ordinal);
            var suffix = name[(isDistance ? WorldRuleFacts.DistancePrefix.Length : WorldRuleFacts.LineOfSightPrefix.Length)..];
            var tokens = suffix.Split(separator: ':');

            if (tokens.Length != 4) {
                throw new WorldRuleException(refusal: WorldRuleRefusal.SpatialChannelMalformed, ruleName: ruleName,
                    detail: $"'{name}' does not spell '{(isDistance ? WorldRuleFacts.DistancePrefix : WorldRuleFacts.LineOfSightPrefix)}<bodyRefA>:<bodyRefB>' (each a 'body:<n>' or 'argmax:<row>'/'argmin:<row>' pair)");
            }

            var bodyA = ResolveBodyRefToken(tokens: tokens, start: 0, ruleName: ruleName, definition: definition, channel: name);
            var bodyB = ResolveBodyRefToken(tokens: tokens, start: 2, ruleName: ruleName, definition: definition, channel: name);
            var spatialKind = (isDistance ? WorldRuleFactKind.BodyDistance : WorldRuleFactKind.LineOfSight);
            var spatialValueKind = (isDistance ? CellKind.Fixed : CellKind.Bool);

            return new ResolvedOperand(Operand: new CompiledWorldOperand(Kind: spatialKind, Row: null, Key: null, BodyA: bodyA, BodyB: bodyB), ValueKind: spatialValueKind, Describe: describe);
        }
        ...
        if (name.StartsWith(value: WorldRuleFacts.ParkedPrefix, comparisonType: StringComparison.Ordinal)) {
            RefuseKeyOnReservedChannel(key: key, ruleName: ruleName, name: name, keyFieldLabel: keyFieldLabel);
            ...
            if (tokens.Length != 2) {
                throw new WorldRuleException(refusal: WorldRuleRefusal.ParkedChannelMalformed, ruleName: ruleName,
                    detail: $"'{name}' does not spell '{WorldRuleFacts.ParkedPrefix}<bodyRef>' (a 'body:<n>' or 'argmax:<row>'/'argmin:<row>' pair)");
            }

            var parkedBody = ResolveBodyRefToken(tokens: tokens, start: 0, ruleName: ruleName, definition: definition, channel: name);

            return new ResolvedOperand(Operand: new CompiledWorldOperand(Kind: WorldRuleFactKind.Parked, Row: null, Key: null, BodyA: parkedBody), ValueKind: CellKind.Int, Describe: describe);
        }

        if (name.StartsWith(value: WorldStateRow.ReservedNamePrefix, comparisonType: StringComparison.Ordinal)) {
            throw new WorldRuleException(refusal: WorldRuleRefusal.StateRowUnknown, ruleName: ruleName,
        ...
        if (row.Kind == CellKind.Text) {
            throw new WorldRuleException(refusal: WorldRuleRefusal.StateCellUnaddressable, ruleName: ruleName, detail: $"state row '{name}' is kind=text — a rule compares numbers, never text");
        }

        var resolvedKey = ResolveKey(row: row, key: key, ruleName: ruleName, verb: verb, keyFieldLabel: keyFieldLabel);
        ...
        // in.
        if (!row.HasCell(key: resolvedKey) && !(row.IsDraw && (resolvedKey == WorldStateRow.SlotKey.Value))) {
            throw new WorldRuleException(refusal: WorldRuleRefusal.StateCellUndeclared, ruleName: ruleName,
        ...
        return new ResolvedOperand(Operand: new CompiledWorldOperand(Kind: WorldRuleFactKind.StateCell, Row: name, Key: resolvedKey), ValueKind: row.Kind, Describe: describe);
    }
    ...
    private static WorldStateRow ResolveNumericRow(string name, string ruleName, WorldDefinition definition, bool requireKeyed, WorldRuleRefusal malformed, string channel) {
        var row = WorldDefinitionRows.FindStateRow(rows: definition.State, name: name)
            ?? throw new WorldRuleException(refusal: malformed, ruleName: ruleName, detail: $"'{channel}' names row '{name}', which the document does not declare");

        if (row.Kind == CellKind.Text) {
            throw new WorldRuleException(refusal: malformed, ruleName: ruleName, detail: $"'{channel}' names row '{name}', which is kind=text — a reduction/extremum is numeric, never text");
        }

        if (requireKeyed && !row.IsKeyed) {
            throw new WorldRuleException(refusal: WorldRuleRefusal.ArgRowNotKeyed, ruleName: ruleName,
    ...
    private static CompiledBodyRef ResolveBodyRefToken(string[] tokens, int start, string ruleName, WorldDefinition definition, string channel) {
        ...
        if (string.Equals(a: kind, b: "body", comparisonType: StringComparison.Ordinal)) {
            if (!int.TryParse(s: value, style: System.Globalization.NumberStyles.Integer, provider: System.Globalization.CultureInfo.InvariantCulture, result: out var index) || (index < 0)) {
                throw new WorldRuleException(refusal: WorldRuleRefusal.SpatialChannelMalformed, ruleName: ruleName, detail: $"'{channel}' names 'body:{value}', which is not a non-negative integer");
            }

            if (index >= definition.Population.Capacity) {
                throw new WorldRuleException(refusal: WorldRuleRefusal.BodyIndexUnknown, ruleName: ruleName,
        ...
        if (string.Equals(a: kind, b: "argmax", comparisonType: StringComparison.Ordinal) || string.Equals(a: kind, b: "argmin", comparisonType: StringComparison.Ordinal)) {
            if (string.IsNullOrEmpty(value: value)) {
                throw new WorldRuleException(refusal: WorldRuleRefusal.SpatialChannelMalformed, ruleName: ruleName, detail: $"'{channel}' names '{kind}:' with no row");
            }

            _ = ResolveNumericRow(name: value, ruleName: ruleName, definition: definition, requireKeyed: true, malformed: WorldRuleRefusal.SpatialChannelMalformed, channel: channel);

            return new CompiledBodyRef(Kind: ((kind == "argmax") ? CompiledBodyRefKind.ArgMax : CompiledBodyRefKind.ArgMin), Index: -1, Row: value);
        }
    ...
    private static CompiledWorldPredicate ResolvePredicate(ActionPredicate.CompareState compare, string ruleName, WorldDefinition definition) {
        ...
        if ((compare.ComparandKey is not null) && (compare.ComparandState is null)) {
            throw new WorldRuleException(refusal: WorldRuleRefusal.ComparandAmbiguous, ruleName: ruleName,
        ...
        var lhs = ResolveOperand(name: name, key: compare.Key, ruleName: ruleName, definition: definition, verb: "compareState", fieldLabel: "state", keyFieldLabel: "key");
        ...
        if (hasValue) {
            ...
            return new CompiledWorldPredicate(Left: lhs.Operand, Comparison: comparison, Value: value, Comparand: null, Describe: describe);
        }

        var rhs = ResolveOperand(name: compare.ComparandState!, key: compare.ComparandKey, ruleName: ruleName, definition: definition, verb: "compareState", fieldLabel: "comparandState", keyFieldLabel: "comparandKey");
        ...
        if (lhs.ValueKind != rhs.ValueKind) {
            throw new WorldRuleException(refusal: WorldRuleRefusal.ComparandKindMismatch, ruleName: ruleName,
        ...
        return new CompiledWorldPredicate(Left: lhs.Operand, Comparison: comparison, Value: default, Comparand: rhs.Operand, Describe: mixedDescribe);
    }

    private static void RefuseKeyOnReservedChannel(string? key, string ruleName, string name, string keyFieldLabel) {
        if (key is not null) {
            throw new WorldRuleException(refusal: WorldRuleRefusal.StateCellUnaddressable, ruleName: ruleName, detail: $"reserved channel '{name}' is a single quantity and carries no cells — drop the '{keyFieldLabel}'");
        }
    }
    ...
    private static CompiledWorldEffect CompileEffect(ActionEffect effect, string ruleName, WorldDefinition definition) => effect switch {
        ...
        ActionEffect.UpsertPlacement upsertPlacement => ResolveUpsertPlacement(effect: upsertPlacement, ruleName: ruleName),
    ...
    private static CompiledWorldEffect ResolveUpsertHudPanel(ActionEffect.UpsertHudPanel effect, string ruleName) {
        if ((effect.Panel is null) || string.IsNullOrWhiteSpace(value: effect.Panel.Id)) {
            throw new WorldRuleException(refusal: WorldRuleRefusal.HudPanelInvalid, ruleName: ruleName, detail: "'upsertHudPanel' names no panel 'id'");
        }
    ...
    private static CompiledWorldEffect ResolveRemoveHudPanel(ActionEffect.RemoveHudPanel effect, string ruleName) {
        if (string.IsNullOrWhiteSpace(value: effect.Id)) {
            throw new WorldRuleException(refusal: WorldRuleRefusal.HudPanelInvalid, ruleName: ruleName, detail: "'removeHudPanel' names no panel 'id'");
        }
    ...
    private static CompiledWorldEffect ResolveUpsertPlacement(ActionEffect.UpsertPlacement effect, string ruleName) {
        if ((effect.Placement is null) || string.IsNullOrWhiteSpace(value: effect.Placement.Id)) {
            throw new WorldRuleException(refusal: WorldRuleRefusal.PlacementInvalid, ruleName: ruleName, detail: "'upsertPlacement' names no placement 'id'");
        }
    ...
    private static CompiledWorldEffect ResolveRemovePlacement(ActionEffect.RemovePlacement effect, string ruleName) {
        if (string.IsNullOrWhiteSpace(value: effect.Id)) {
            throw new WorldRuleException(refusal: WorldRuleRefusal.PlacementInvalid, ruleName: ruleName, detail: "'removePlacement' names no placement 'id'");
        }
    ...
    private static CompiledWorldEffect ResolveWrite(string rowName, string? key, ActionTarget target, WorldDocumentWriteKind write, float? value, string? fromState, string? fromKey, string ruleName, WorldDefinition definition, string verb) {
        ...
        var row = WorldDefinitionRows.FindStateRow(rows: definition.State, name: rowName)
            ?? throw new WorldRuleException(refusal: WorldRuleRefusal.StateRowUnknown, ruleName: ruleName, detail: $"'{verb}' names no state row '{rowName}' — declare it with world.row.set state <json> first");

        if (row.Kind == CellKind.Text) {
            throw new WorldRuleException(refusal: WorldRuleRefusal.StateCellUnaddressable, ruleName: ruleName, detail: $"state row '{rowName}' is kind=text — '{verb}' writes a number");
        }
        ...
        if (hasValue) {
            ...
            var raw = row.Kind switch {
                ...
                _ => ((literal != 0f) ? 1L : 0L), // Bool — Text already refused above.
            };
        ...
        var source = ResolveOperand(name: fromState!, key: fromKey, ruleName: ruleName, definition: definition, verb: verb, fieldLabel: "fromState", keyFieldLabel: "fromKey");

        if (source.ValueKind != row.Kind) {
            throw new WorldRuleException(refusal: WorldRuleRefusal.EffectSourceKindMismatch, ruleName: ruleName,
    ...
    private static CompiledWorldEffect ResolveGenerate(ActionEffect.Generate generate, string ruleName, WorldDefinition definition) {
        var row = WorldDefinitionRows.FindStateRow(rows: definition.State, name: generate.Row)
            ?? throw new WorldRuleException(refusal: WorldRuleRefusal.StateRowUnknown, ruleName: ruleName, detail: $"'generate' names no state row '{generate.Row}'");

        if (row.Draw is not { } draw) {
            throw new WorldRuleException(refusal: WorldRuleRefusal.GeneratorUnknown, ruleName: ruleName, detail: $"state row '{generate.Row}' declares no draw — 'generate' redraws a draw site");
        }

        if (draw.Timing == WorldDrawTiming.Boot) {
            throw new WorldRuleException(refusal: WorldRuleRefusal.GeneratorUnknown, ruleName: ruleName, detail: $"state row '{generate.Row}' declares timing=boot — it draws once at first fill and is never redrawn");
        }
        ...
        if (!WorldGeneratorEngine.TryResolveSource(generators: definition.Generators, draw: draw, generator: out var generator, reason: out var resolveReason)) {
            throw new WorldRuleException(refusal: WorldRuleRefusal.GeneratorUnknown, ruleName: ruleName, detail: $"state row '{generate.Row}' {resolveReason}");
        }
        ...
        if (!WorldGeneratorEngine.TryCheckTargetKind(source: generator.Source, targetKind: row.Kind, reason: out var kindReason)) {
            throw new WorldRuleException(refusal: WorldRuleRefusal.StateCellUnaddressable, ruleName: ruleName, detail: $"state row '{generate.Row}': {kindReason}");
        }
    ...
    private static string ResolveKey(WorldStateRow row, string? key, string ruleName, string verb, string keyFieldLabel) {
        if (key is { } authored) {
            return (WorldCellName.TryParse(candidate: authored, name: out var parsed, reason: out var reason)
                ? parsed.Value
                : throw new WorldRuleException(refusal: WorldRuleRefusal.StateCellUnaddressable, ruleName: ruleName, detail: $"'{verb}' {keyFieldLabel} '{authored}' {reason}"));
        }

        return (row.IsKeyed
            ? throw new WorldRuleException(refusal: WorldRuleRefusal.StateCellUnaddressable, ruleName: ruleName, detail: $"'{verb}' names keyed row '{row.Name}' without a '{keyFieldLabel}' — a keyed row has no single cell, so name the one you mean")
