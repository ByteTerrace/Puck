...
public static class WorldSimulationTickConversion {
    ...
    public static ulong DurationTicks(float seconds, uint ratePerSecond) {
        return DurationTicks(seconds: FixedQ4816.FromDouble(value: seconds), ratePerSecond: ratePerSecond);
    }
    ...
    public static CompiledTickDuration CompiledDuration(float seconds, uint ratePerSecond) {
        if (ratePerSecond == 0U) {
            return ((seconds > 0f) ? CompiledTickDuration.Never : CompiledTickDuration.FromTicks(ticks: 0));
        }

        return CompiledTickDuration.FromTicks(ticks: checked((int)DurationTicks(seconds: seconds, ratePerSecond: ratePerSecond)));
    }
