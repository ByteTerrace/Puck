...
/// caller serializes the definition verbatim, keeping the sanctioned download hash-identical.</para>
/// </remarks>
public static class WorldProjection {
    ...
    public static WorldProjectionDocument? Compose(WorldDefinition definition, WorldDisclosureTier tier, string authority, int revision) {
        ...
        return new WorldProjectionDocument(
            Provenance: new WorldProjectionProvenance(Authority: authority, DocumentId: definition.DocumentId, Revision: revision, Tier: WorldDisclosureTier.Presentation),
    ...
    public static WorldDefinition ToDefinition(WorldProjectionDocument projection) {
        ...
        return new WorldDefinition(
            ...
            Hud: projection.Hud,
            State: [],
            InputHold: new WorldInputHoldAuthoring(CeilingSeconds: 0f, LowerAfterSeconds: 0f, DefaultSeconds: 0f, EqualizeByDefault: false, Participants: []),
    ...
    public static byte[] Serialize(WorldProjectionDocument projection) {
        ...
        using (var writer = new Utf8JsonWriter(utf8Json: stream, options: s_writerOptions)) {
            JsonSerializer.Serialize(writer: writer, value: projection, jsonTypeInfo: WorldJsonContext.Default.WorldProjectionDocument);
        }
    ...
    public static bool TryDeserialize(ReadOnlySpan<byte> utf8Json, out WorldProjectionDocument? projection, out string reason) {
        ...
        try {
            projection = JsonSerializer.Deserialize(utf8Json: utf8Json, jsonTypeInfo: WorldJsonContext.Default.WorldProjectionDocument);
        ...
        if (!string.Equals(a: projection.Schema, b: WorldProjectionDocument.SchemaVersion, comparisonType: StringComparison.Ordinal)) {
