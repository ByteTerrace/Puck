...
public readonly record struct WorldFaceFrame(
    ...
    public FixedQ4816 PlanarYawRadians => FixedQ4816.Atan2(y: Normal.X, x: Normal.Z);
...
public static class WorldFaceRegion {
    ...
    public static WorldFaceCrossing Sweep(WorldFaceAperture aperture, FixedVector3 from, FixedVector3 to) {
        ...
        return aperture switch {
            WorldFaceAperture.Box box => SweepBox(box: box, from: from, to: to),
            _ => throw new ArgumentOutOfRangeException(paramName: nameof(aperture), actualValue: aperture, message: "The face aperture arm has no swept region test."),
        };
    }
    ...
    private static WorldFaceCrossing SweepBox(WorldFaceAperture.Box box, FixedVector3 from, FixedVector3 to) {
        ...
        var alongNormal = FixedVector3.Dot(left: delta, right: frame.Normal);
        var alongRight = FixedVector3.Dot(left: delta, right: frame.Right);
        var alongUp = FixedVector3.Dot(left: delta, right: frame.Up);
        ...
        var previousAlongNormal = FixedVector3.Dot(left: previousDelta, right: frame.Normal);
        var previousAlongRight = FixedVector3.Dot(left: previousDelta, right: frame.Right);
        var previousAlongUp = FixedVector3.Dot(left: previousDelta, right: frame.Up);

        var enter = FixedQ4816.Zero;
        var exit = FixedQ4816.One;
        var sweptRegion = (ClipSegmentAxis(start: previousAlongNormal, end: alongNormal, lower: FixedQ4816.Zero, upper: box.Depth, enter: ref enter, exit: ref exit)
            && ClipSegmentAxis(start: previousAlongRight, end: alongRight, lower: -frame.HalfWidth, upper: frame.HalfWidth, enter: ref enter, exit: ref exit)
            && ClipSegmentAxis(start: previousAlongUp, end: alongUp, lower: -frame.HalfHeight, upper: frame.HalfHeight, enter: ref enter, exit: ref exit));
        var entersFromFront = (alongNormal <= previousAlongNormal);
        var swept = (sweptRegion && entersFromFront);
        var parameter = (swept ? enter : FixedQ4816.Zero);
...
public readonly record struct WorldFaceCrossingClaim(string PlacementId, string FaceName, FixedQ4816 Parameter)
    ...
    public int CompareTo(WorldFaceCrossingClaim other) {
        ...
        var byPlacement = string.CompareOrdinal(strA: PlacementId, strB: other.PlacementId);

        return ((byPlacement != 0) ? byPlacement : string.CompareOrdinal(strA: FaceName, strB: other.FaceName));
    }
