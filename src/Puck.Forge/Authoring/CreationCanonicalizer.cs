...
public static class CreationCanonicalizer {
    ...
    public static IReadOnlyList<DocumentValidationError> Validate(CreationDocument document) {
        ...
        for (var i = 0; i < (document.Shapes?.Count ?? 0); i++) {
            ...
            if (!Enum.IsDefined(value: shape.Type)) {
                errors.Add(item: new(Path: $"shapes[{i}].type", Message: $"primitive '{shape.Type}' is not recognized."));
            }
            if ((shape.Blend is { } blend) && !Enum.IsDefined(value: blend)) {
                errors.Add(item: new(Path: $"shapes[{i}].blend", Message: $"blend '{blend}' is not recognized."));
            }
        ...
        ValidateParts(document: document, errors: errors, shapeIds: shapeIds);
    ...
    public static CreationDocument Normalize(CreationDocument document) {
        ...
        return (document with {
            ...
            Parts = ((document.Parts is { Count: > 0 } parts) ? parts.ToArray() : null),
    ...
    private static void ValidateParts(CreationDocument document, List<DocumentValidationError> errors, HashSet<int> shapeIds) {
        ...
        for (var index = 0; (index < parts.Count); index++) {
            ...
            if (string.IsNullOrWhiteSpace(value: part.Id)) {
                errors.Add(item: new(Path: $"parts[{index}].id", Message: "a part id is required."));
            } else if (!partIds.Add(item: part.Id)) {
                errors.Add(item: new(Path: $"parts[{index}].id", Message: $"duplicate part id '{part.Id}'."));
            }

            if (!shapeIds.Contains(item: part.ShapeId)) {
                errors.Add(item: new(Path: $"parts[{index}].shapeId", Message: $"references missing shape id {part.ShapeId}."));
            }
        }
    }
