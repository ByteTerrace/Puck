...
/// differential fuzzing of the SDF VM. The ONE generator both the demo's <c>--validate-world --fuzz-seed N</c> gate
/// and the POST's fuzz stage consume, so a seed reproduces the identical program in either harness — the standing
/// cross-check. The same seed (and bounds) always yields the same program (so a finding is reproducible), and the
/// program is fed identically to both backends — any divergence in the rendered result is a backend bug (typically
/// shader codegen of a shape SDF, a smooth blend, or a transcendental).
/// <para>
...
/// </para>
/// </summary>
public static class FuzzSdfProgram {
    ...
    public static SdfProgram Generate(int seed, ShapeBounds bounds) {
        ...
        for (var index = 0; (index < primitiveCount); index++) {
            ...
            // Low-probability WARPS (continuous point ops only — wallpaper's cell-boundary round() flips and the
            // whole-field onion/dilate stay out of the differential path by design). Rates draw from the moderate
            // envelope: warps are not isometries, and large rates amplify benign march noise past the oracle.
            if (random.NextDouble() < 0.15) {
                _ = builder.TwistY(rate: Range(random: random, range: bounds.TwistRate));
            }

            if (random.NextDouble() < 0.15) {
                var bendRate = Range(random: random, range: bounds.BendRate);

                switch (random.Next(minValue: 0, maxValue: 3)) {
                    case 0: { _ = builder.BendX(rate: bendRate); break; }
                    case 1: { _ = builder.BendY(rate: bendRate); break; }
                    default: { _ = builder.BendZ(rate: bendRate); break; }
                }
            }

            if (random.NextDouble() < 0.1) {
                _ = builder.Elongate(extents: RandomVector(random: random, range: bounds.ElongateExtent));
            }

            var blend = (SdfBlendOp)random.Next(minValue: 0, maxValue: 7);
            var smooth = Range(random: random, range: bounds.Smooth);
            var material = materials[random.Next(minValue: 0, maxValue: materialCount)];

            switch (random.Next(minValue: 0, maxValue: 7)) {
                case 0: {
                ...
                case 4: {
                        // The endpoint draws from the SAME per-axis range as the validator, then clamps its length
                        // away from the degenerate point-segment (the validator's 0.05 floor).
                        var endpoint = RandomVector(random: random, range: bounds.CapsuleEndpoint);

                        if (endpoint.LengthSquared() < (0.05f * 0.05f)) {
                            endpoint = new Vector3(x: 0f, y: 0.05f, z: 0f);
                        }

                        _ = builder.Capsule(blend: blend, endpoint: endpoint, material: material, radius: Range(random: random, range: bounds.CapsuleRadius), smooth: smooth);

                        break;
                    }
                case 5: {
                        _ = builder.Cylinder(blend: blend, halfHeight: Range(random: random, range: bounds.CylinderHalfHeight), material: material, radius: Range(random: random, range: bounds.CylinderRadius), smooth: smooth);

                        break;
                    }
                default: {
                        _ = builder.Ellipsoid(blend: blend, material: material, radii: RandomVector(random: random, range: bounds.EllipsoidRadius), smooth: smooth);
