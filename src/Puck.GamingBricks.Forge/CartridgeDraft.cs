...
public sealed class CartridgeDraft {
    ...
    private static string[] Parts(string pointer) {
        ...
        foreach (var part in parts) {
            for (var i = 0; (i < part.Length); ++i) {
                if (
                    (part[i] == '~') &&
                    ((++i == part.Length) || (part[i] is not ('0' or '1')))
                ) { throw Missing(pointer: pointer); }
            }
        }
        return parts.Select(selector: static part => part.Replace(
    ...
    private static JsonNode? Resolve(JsonNode root, string[] parts) {
        ...
        foreach (var part in parts) {
            node = node switch {
                ...
                JsonArray array => array[index: Index(
                key: part,
                count: array.Count
            )],
    ...
    public void Remove(string pointer) => Edit(
        pointer: pointer,
        remove: true,
        value: null
    );
    ...
    public void Set(string pointer, string json) {
        ...
        var value = JsonNode.Parse(
            json: json,
            documentOptions: new JsonDocumentOptions { AllowDuplicateProperties = false, MaxDepth = 32 }
        );
    ...
    public string Show(string pointer = "") => (Resolve(
        root: m_root,
        parts: Parts(pointer: pointer)
    )?.ToJsonString() ?? "null");
