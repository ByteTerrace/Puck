...
public static class CartridgeDocuments {
    ...
    private static CartridgeDocument Read(ReadOnlySpan<byte> utf8) {
        if (utf8.Length > MaximumSourceBytes) {
            throw new ArgumentException(
                message: $"Cartridge source exceeds {MaximumSourceBytes} bytes.",
    ...
    public static CartridgeDocument Create(string target, string title) {
        var palette = new int[((target == "agb")
            ? 16
            : 4)];
        ...
        palette[1] = 0x56B5;
        palette[2] = 0x294A;
        var palettes = new CartridgePalettes(
            Background: [palette],
            Object: [palette]
        );
    ...
    public static (CostBound Frame, long Reservation) Estimate(CartridgeDocument document) {
        ...
        var profile = ((document.Target is "agb" or "cgb")
            ? CartridgeCostProfile.For(target: document.Target)
            : CartridgeCostProfile.Humble
        );

        return (CartridgeCost.Frame(
            document: document,
            profile: profile
        ), profile.FrameUnits);
    }
    ...
    public static CartridgeDocument Parse(ReadOnlySpan<byte> utf8) {
        return Canonicalize(document: Read(utf8: utf8)).Document;
    }
