...
internal static class DirectXResourceStates {
    ...
    internal static void Forget(nint resource) => States.TryRemove(
        key: resource,
        value: out _
    );
    internal static D3D12_RESOURCE_STATES Get(nint resource, D3D12_RESOURCE_STATES fallback) => (States.TryGetValue(
        key: resource,
        value: out var state
    )
        ? state
