...
public sealed record ShaderPipelineResource(
    ...
    /// against the host's counts. A counted buffer is refused, naming the basis, when the host resolves any basis it
    /// scales with to zero units, because a zero-byte buffer cannot be bound.</summary>
    /// <param name="counts">The counts a counted buffer scales with.</param>
    /// <returns>The capacity in bytes.</returns>
    /// <exception cref="InvalidOperationException">The resource is not a buffer, declares neither size, or scales with a
    /// basis <paramref name="counts"/> resolves to zero units.</exception>
    ...
    public ulong ResolveSizeBytes(ShaderPipelineStorageCounts counts) {
        ...
        foreach (var term in count) {
            ...
            foreach (var basis in term.Per) {
                var units = counts.UnitsOf(basis: basis);

                if (units == 0) {
                    throw new InvalidOperationException(message: $"Buffer '{Name}' scales with {basis}, which the host resolves to zero units.");
                }

                product = checked((product * units));
            }
...
/// <summary>The counts a host resolves counted buffers against. A basis the host leaves at zero refuses every buffer
/// that scales with it.</summary>
