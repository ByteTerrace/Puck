...
public sealed class ShaderPipelineParameterLayout {
    ...
    private ShaderPipelineParameterLayout(
        ...
        Slots = new ReadOnlyCollection<ShaderPipelineParameterSlot>(list: slots.ToList());
    ...
    public static ShaderPipelineParameterLayout Resolve(ShaderPipelinePass pass) {
        ...
        if (
            ...
            return new ShaderPipelineParameterLayout(
                slots: [],
                schema: pass.Config,
                sizeBytes: FramePrefixBytes
            );
        }
        ...
        var fields = pass.Config.OrderBy(
            static pair => pair.Key,
            StringComparer.Ordinal
        ).ToArray();
        var types = fields.Select(selector: static pair => pair.Value.Type).ToArray();
        var offsets = ShaderPushConstantLayout.ComputeOffsets(
            sizeBytes: out var configBytes,
            types: types
        );
        var slots = fields.Select(selector: (pair, index) => new ShaderPipelineParameterSlot(
            Name: pair.Key,
            Type: pair.Value.Type,
            Offset: (FramePrefixBytes + offsets[index])
        )).ToArray();

        return new ShaderPipelineParameterLayout(
            slots: slots,
            schema: pass.Config,
            sizeBytes: (FramePrefixBytes + configBytes)
        );
    }
