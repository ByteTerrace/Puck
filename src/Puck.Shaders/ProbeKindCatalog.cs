...
/// source) beside the rest of a deploy's <c>Assets/Probes</c> tree — no code registration. Enumeration reads file
/// names only; a manifest is parsed and validated on <see cref="Load"/>.
/// </summary>
public sealed class ProbeKindCatalog {
    ...
    public static ProbeKindCatalog Scan(string rootDirectory) {
        ...
        var suffix = ProbeKindManifest.FileSuffix;
        ...
        if (Directory.Exists(path: rootDirectory)) {
            foreach (var path in Directory.EnumerateFiles(
                ...
                if (!pathsById.TryAdd(
                    ...
                    throw new InvalidDataException(message: $"The probe kind '{id}' is shipped twice under '{rootDirectory}': '{pathsById[id]}' and '{path}'.");
                }
            }
        }
    ...
    /// <summary>Loads and validates a shipped probe kind's manifest.</summary>
    /// <param name="id">The id.</param>
    ...
    public ProbeKindManifest Load(string id) {
        return (m_pathsById.TryGetValue(
            ...
            : throw new KeyNotFoundException(message: $"No probe kind '{id}' is shipped; the shipped ids are: {string.Join(
                separator: ", ",
                values: Ids
            )}.")
        );
    }
    /// <summary>Gets a shipped probe kind's manifest path.</summary>
    /// <param name="id">The id.</param>
