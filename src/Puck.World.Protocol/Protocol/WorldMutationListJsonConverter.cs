...
public sealed class WorldMutationListJsonConverter : JsonConverter<IReadOnlyList<WorldMutation>> {
            ...
            while (
                ...
                switch (property) {
                    ...
                    case "value": {
                            ...
                            member = ((JsonSerializer.Deserialize(
                                ref reader,
                                returnType: type,
                                options: options
                            ) as WorldMutation) ?? throw new JsonException(message: $"{type.Name} decoded as null"));
                            break;
                        }
                    default:
    ...
    public override void Write(Utf8JsonWriter writer, IReadOnlyList<WorldMutation> value, JsonSerializerOptions options) {
        ...
        foreach (var member in value) {
            ...
            writer.WriteNumber(
                propertyName: "kind",
                value: WorldMutationKindCatalog.OrdinalOf(mutation: member)
            );
