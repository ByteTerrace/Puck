...
/// an authority-entry kind <c>Puck.World.Server.WorldReplaySnapshot.Drive</c>'s switch does not handle, an enum
/// member with no value in the pinned wire set, or a live receipt set the encoder cannot represent. Every one of
/// those is a determinism hole — a recorded input silently dropped from the re-drive, or a state the tape cannot
/// round-trip.
/// </summary>
/// <remarks>
...
/// <see cref="InvalidDataException"/> instead — see <c>Puck.World.Server.WorldReplaySnapshot.Read</c>.</para>
/// </remarks>
