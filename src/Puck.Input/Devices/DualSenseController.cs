...
internal sealed class DualSenseController : IGamepadParser, IRumbleParser, ILedParser, ITriggerEffectParser {
    ...
    // Mode byte + parameter offsets of each trigger block within the 0x02 report (right then left); each block is
    // a mode byte followed by ten parameter bytes, and the two are disjoint (11 + 11 == 22).
    ...
    // The free-running motion-sensor timestamp: a 32-bit LE counter at common 27..30. Its unit is 1/3 µs (the
    // hid-playstation driver divides it by 3 to reach microseconds), so a raw delta converts to seconds by
    // dividing by 3,000,000 — the true inter-report dt the orientation fusion integrates over.
    ...
    // Writes the USB output report 0x02 from the last-applied motor + trigger + lightbar + player-LED state. The
    // DualSense multiplexes the motors, both triggers' force feedback, and the light bar into one report, and the
    // firmware applies only the sections whose valid_flag bits are set — which are disjoint — so always emitting
    // the full state keeps any one write (rumble, trigger, or LED) from clearing the others. (Bluetooth would be
    // report 0x31 with the common block shifted +1 and a trailing CRC32 — not yet implemented.)
    private ValueTask WriteOutputAsync(CancellationToken cancellationToken) {
        ...
        buffer[1] = (FlagCompatibleVibration | FlagHapticsSelect | FlagRightTriggerFeedback | FlagLeftTriggerFeedback); // valid_flag0
        ...
        DualSenseAdaptiveTrigger.WriteTriggerBlock(block: buffer.AsSpan(start: RightTriggerOffset, length: DualSenseAdaptiveTrigger.TriggerBlockLength), spec: in m_rightTriggerEffect);
        DualSenseAdaptiveTrigger.WriteTriggerBlock(block: buffer.AsSpan(start: LeftTriggerOffset, length: DualSenseAdaptiveTrigger.TriggerBlockLength), spec: in m_leftTriggerEffect);
    ...
    public bool TryParse(ReadOnlySpan<byte> report, out GamepadState state) {
        ...
        // The accel, sensor-timestamp, and touchpad blocks sit past the gyro; only read each when the report is
        // long enough (the full USB/BT reports are, but guard so a short report never reads out of bounds).
