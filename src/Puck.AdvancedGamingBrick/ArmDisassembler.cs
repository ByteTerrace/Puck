...
public static class ArmDisassembler {
    ...
    public static string DecodeArm(uint address, uint instruction) {
        ...
        if ((instruction & 0x0E000000u) == 0x0A000000u) {
            ...
            return $"B{(((instruction & 0x01000000u) != 0u)
                ? "L"
                : "")}{condition} {Hex32(value: target)}";
        }

        // Multiply / multiply-long (bits 27-22 select, bits 7-4 == 1001).
        if (
            ((instruction & 0x0FC000F0u) == 0x00000090u) ||
        ...
        if (
            ((instruction & 0x0FB00000u) == 0x03200000u) ||
        ...
        if (
            ((instruction & 0x0E000090u) == 0x00000090u) &&
    ...
    public static string DecodeThumb(uint address, ushort instruction) {
        ...
        if ((op & 0xF800) == 0x1800) {
            ...
            return $"{(subtract
                ? "SUB"
                : "ADD")} {Register(index: op & 7)},{Register(index: (op >> 3) & 7)},{(immediate
                ? $"#{operand}"
        ...
        if ((op & 0xE000) == 0x6000) {
            ...
            var offset = (((op >> 6) & 0x1F) << (byteAccess
                ? 0
                : 2));
            ...
            return $"{(load
                ...
                : "STR")}{(byteAccess
                ? "B"
                : "")} {Register(index: op & 7)},[{Register(index: (op >> 3) & 7)},#0x{offset:X}]";
        }
        ...
        if ((op & 0xF000) == 0x8000) {
            ...
            return $"{(((op & 0x0800) != 0)
                ? "LDRH"
                : "STRH")} {Register(index: op & 7)},[{Register(index: (op >> 3) & 7)},#0x{offset:X}]";
        }
        ...
        if ((op & 0xF000) == 0x9000) {
            return $"{(((op & 0x0800) != 0)
                ? "LDR"
                : "STR")} {Register(index: (op >> 8) & 7)},[SP,#0x{((op & 0xFF) << 2):X}]";
        }

        // Format 12: load address.
        if ((op & 0xF000) == 0xA000) {
            return $"ADD {Register(index: (op >> 8) & 7)},{(((op & 0x0800) != 0)
                ? "SP"
                : "PC")},#0x{((op & 0xFF) << 2):X}";
        }
