...
/// <summary>The interrupt controller, modelled on the cycle-stepped hardware reference (the per-cycle
/// interrupt-step). IE/IF/IME are double-buffered: writes land in the "next" stage [1] and reads return the
...
public sealed partial class AgbInterruptController : IAgbInterruptController {
    ...
    // (written by software and by peripherals). enable/flag/ime[0..1], shifted by the interrupt-step each cycle.
    ...
    public void StepSync(bool stallingCpu) {
        // Interrupt-step: compute the line from the committed stage, then shift the programmed
