...
public sealed partial class AdvancedGamingBrickCore : IMachineHardwareAccess {
    ...
    public static IReadOnlyList<MachineMemorySpaceDescriptor> HardwareSpaces { get; } = [
        new(
            AccessModes: [MachineAccessMode.Inspect, MachineAccessMode.Patch, MachineAccessMode.Bus],
            Description: "AGB bus; naturally aligned scalar accesses. Patch admits EWRAM/IWRAM; bus accesses charge native wait states.",
            FirstAddress: 0,
    ...
    public MachineAccessResult Read(MachineMemoryAddress address, MachineAccessMode mode = MachineAccessMode.Inspect) {
        ...
        if (mode == MachineAccessMode.Inspect) {
            value = address.Width switch {
                1 when ((location >> 24) == 4) => ((uint)(bus.DebugRead16(address: location & ~1U) >> (((int)(location & 1)) * 8))) & 0xFFU,
                1 => bus.DebugRead8(address: location),
                2 => bus.DebugRead16(address: location),
                _ => bus.DebugRead32(address: location),
            };
        } else {
            value = address.Width switch {
                1 => bus.Read8(
    ...
    public MachineAccessResult Write(MachineMemoryAddress address, ulong value, MachineAccessMode mode) {
        ...
        if ((value >> (address.Width * 8)) != 0) {
            return new(
                ...
                Reason: $"Value {value} does not fit {address.Width} byte(s)."
            );
        }
        var bus = ((AgbBus)m_instance.GetRequiredService<IAgbBus>());
        var location = ((uint)address.Address);
        ...
        if (mode == MachineAccessMode.Patch) {
            for (var offset = 0; (offset < address.Width); offset++) {
                bus.DebugWrite8(
                    address: (location + ((uint)offset)),
                    value: ((byte)(value >> (offset * 8)))
                );
            }
        } else {
    ...
    private static MachineAccessResult? ValidateHardwareAccess(MachineMemoryAddress address, MachineAccessMode mode, bool writing) {
        if (
            (address.Space != "bus") ||
            (address.Width is not (1 or 2 or 4))
        ) {
            return new(
                MachineAccessStatus.Unsupported,
                Reason: "AGB hardware access requires space 'bus' and width 1, 2, or 4."
            );
        }
        if (
            (address.Address > uint.MaxValue) ||
            (((ulong)(address.Width - 1)) > (uint.MaxValue - address.Address)) ||
            ((address.Address % ((ulong)address.Width)) != 0)
        ) {
            return new(
                MachineAccessStatus.Refused,
                Reason: $"Address 0x{address.Address:X} with width {address.Width} is outside space 'bus' or is unaligned."
            );
        }
        if (writing
            ? (mode is not (MachineAccessMode.Patch or MachineAccessMode.Bus))
            : (mode is not (MachineAccessMode.Inspect or MachineAccessMode.Bus))) {
            return new(
                MachineAccessStatus.Unsupported,
                Reason: $"Access mode '{mode}' does not support this operation."
            );
        }
        ...
        if (
            ...
            (region is not (2 or 3))
        ) {
            return new(
                MachineAccessStatus.Refused,
                Reason: $"Address 0x{address.Address:X8} is not patchable EWRAM/IWRAM; device registers require bus access."
            );
        }
        if (
            (mode == MachineAccessMode.Inspect) &&
            (region is 0xE or 0xF)
        ) {
            return new(
                MachineAccessStatus.Unsupported,
                Reason: "Save-device inspection is not exposed as a side-effect-free observation."
            );
        }
        return null;
    }
