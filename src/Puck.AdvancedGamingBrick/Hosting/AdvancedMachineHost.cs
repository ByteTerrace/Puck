...
/// <summary>
/// Hosts one native ARM7TDMI AdvancedGamingBrick behind <see cref="IMachineRuntime"/> and
/// <see cref="IQueuedMachineRuntime"/> — a thin adapter that builds an <see cref="AdvancedGamingBrickCore"/> and forwards
...
/// </summary>
public sealed partial class AdvancedMachineHost : QueuedMachineHost {
    ...
    private readonly MachineBootMode m_bootMode;
    ...
    /// <param name="bootMode">Cold startup executes the BIOS; fast startup seeds cartridge handoff without replacing it.</param>
    public AdvancedMachineHost(byte[] biosImage, byte[]? cartridgeRom = null, string? savePath = null, int audioSampleRate = 0, AgbMachineOptions? options = null, MachineBootMode bootMode = MachineBootMode.Fast)
        : base(
        width: ScreenWidth,
        height: ScreenHeight,
