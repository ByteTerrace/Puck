...
public sealed partial class AgbDmaController : IAgbDmaController {
    ...
    public ushort ReadRegister(uint offset) {
        ...
        return local switch {
            // The word-count halfword (CNT_L) is write-only but actively decoded: it drives the bus with zero,
            // unlike the SAD/DAD address registers (which the bus resolves to open bus before reaching here).
            8u => 0,
    ...
    private void ActivateTimed(int timing, IAgbBus bus) {
        ...
        //
        // The internal word counter is NOT re-latched here: hardware captures the count once on the enable rising
        // edge (WriteControl) and again only when a repeat run reloads it (CompleteChannel). A trigger that fires
        // after the game rewrote CNT_L still transfers the count latched at enable — the DMA count-latching edge case
        // (the first transfer uses the enable-time count, the repeat uses the reloaded one).
        _ = bus;
    ...
    private void CompleteChannel(int channel) {
        ...
        if ((control & 0x200) == 0) {
            ...
            // Repeat: re-latch the internal word counter from the (possibly rewritten) CNT_L register for the next
            // trigger. This is the only re-latch after enable, so a mid-run count change takes effect on the repeat,
            // never on the run already in flight (the "DMA count latching" edge case).
            ActivateChannel(channel: channel);
        }
    }
