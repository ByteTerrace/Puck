...
public sealed partial class AgbPpu : IAgbPpu {
    ...
    // OBJ per-line rendering cycle budget: 1210 cycles (304*4-6) normally,
    // or 954 (240*4-6) when DISPCNT bit 5 (H-Blank Interval Free) frees part of the line for CPU OAM access
    // instead. A regular sprite costs 1 cycle per on-screen pixel; an affine (rotate/scale) sprite costs a
    // 10-cycle setup plus 2 cycles per on-screen pixel. Hardware processes OAM in order and simply stops — mid
    // sprite if need be — once the budget runs out, so later sprites (or the tail of the one in flight) drop out.
    private const int SpriteCyclesNormal = 1210;
    private const int SpriteAffinePixelCost = 2;
    private const int SpriteAffineSetupCost = 10;
    private const int SpriteCyclesHBlankFree = 954;
    private const int SpriteRegularPixelCost = 1;

    private readonly AgbScheduler m_scheduler;
    private readonly AgbScheduler.Event m_event;
    ...
    public AgbPpu(AgbScheduler scheduler, IAgbInterruptController interrupts) {
        ...
        m_event = new AgbScheduler.Event { Callback = RunEvent };
        m_scheduler.Schedule(
            e: m_event,
            cyclesFromNow: HDrawLength
        );
    }
    ...
    private void RunEvent(int cyclesLate) {
        if (!m_inHBlank) {
            ...
            m_scheduler.Schedule(
                e: m_event,
                cyclesFromNow: (HBlankLength - cyclesLate)
            );
        } else {
            NextScanline();
            m_inHBlank = false;
            m_scheduler.Schedule(
                e: m_event,
                cyclesFromNow: (HDrawLength - cyclesLate)
            );
        }
    }
    ...
    public ushort ReadRegister(uint offset) {
        if (offset == 0x04u) {
            ...
            return (ushort)((((m_line >= ScreenHeight) && (m_line < (TotalLines - 1)))
                ? 0x1u
                : 0u)
                | (m_hblankFlag
                ? 0x2u
                : 0u)
                | ((m_line == (m_dispStatControl >> 8))
                ? 0x4u
                : 0u)
                | m_dispStatControl);
        }
        ...
        // Unused/reserved bits of these readable PPU registers drive 0 on a read, regardless of what was written
        // (per the hardware I/O-read behavior). The renderer reads m_registers directly, so masking here only shapes the CPU view.
        var raw = m_registers[(offset >> 1)];

        return offset switch {
            0x08u or 0x0Au => (ushort)(raw & 0xDFFFu), // BG0CNT/BG1CNT: bit 13 is BG2/BG3-only
            0x48u or 0x4Au => (ushort)(raw & 0x3F3Fu), // WININ/WINOUT
            0x50u => (ushort)(raw & 0x3FFFu),          // BLDCNT
            0x52u => (ushort)(raw & 0x1F1Fu),          // BLDALPHA
            _ => raw,
        };
    }
    ...
    public bool PramContention {
        get {
            // PRAM contention (scanline renderer): during a rendered visible scanline — not forced blank,
    ...
    public uint ReadVideo(uint address, int width) => (address >> 24) switch {
        0x5u => AgbArrayAccess.Read(
        array: m_palette,
        index: address & 0x3FFu,
        width: width
    ),
        0x6u => AgbArrayAccess.Read(
        array: m_vram,
        index: VramOffset(address: address),
        width: width
    ),
        _ => AgbArrayAccess.Read(
        array: m_oam,
        index: address & 0x3FFu,
        width: width
    ),
    ...
    private void RenderScanline(int line) {
        ...
        if (ForcedBlank) {
            m_framebuffer.AsSpan(
                start: rowBase,
                length: ScreenWidth
            ).Fill(value: 0xFFFFFFFFu);
            ApplyGreenSwap(rowBase: rowBase);
        ...
        switch (mode) {
            ...
            case 2:
                for (var background = 0; (background < 4); ++background) {
                    if (
                        ...
                        ((m_registers[0] & (0x100u << background)) == 0u)
                    ) {
                        continue;
                    }

                    if (IsAffineBackground(
                        mode: mode,
                        background: background
                    )) {
            ...
            default:
                m_framebuffer.AsSpan(
                    start: rowBase,
                    length: ScreenWidth
                ).Fill(value: Color(bgr555: PaletteColor(index: 0)));
                ApplyGreenSwap(rowBase: rowBase);
    ...
    private static void Consider(int color, int priority, int order, int id, ref Layer top, ref Layer second) {
        if (
            ...
            ((priority == top.Priority) && (order < top.Order))
        ) {
            second = top;
            top = new Layer(
                Color: color,
            ...
            ((priority == second.Priority) && (order < second.Order))
        ) {
            second = new Layer(
                Color: color,
                Priority: priority,
    ...
    private uint WindowMaskAt(int x, int line) {
        if (
            ((m_registers[0] & 0x2000u) != 0u) &&
            InWindow(
            horizontalRegister: 0x20,
            verticalRegister: 0x22,
            x: x,
            line: line
        )
        ) {
        ...
        if (
            ((m_registers[0] & 0x4000u) != 0u) &&
            InWindow(
            horizontalRegister: 0x21,
            verticalRegister: 0x23,
            x: x,
            line: line
        )
        ) {
    ...
    private int AlphaBlend(int first, int second) {
        var eva = Math.Min(
            val1: 16,
            val2: m_registers[0x29] & 0x1F
        );        // BLDALPHA EVA
        var evb = Math.Min(
            val1: 16,
            val2: (m_registers[0x29] >> 8) & 0x1F
        ); // BLDALPHA EVB

        var r = Math.Min(
            val1: 31,
            val2: ((((first & 0x1F) * eva) + ((second & 0x1F) * evb)) >> 4)
        );
        var g = Math.Min(
            val1: 31,
            val2: (((((first >> 5) & 0x1F) * eva) + (((second >> 5) & 0x1F) * evb)) >> 4)
        );
        var b = Math.Min(
            val1: 31,
            val2: (((((first >> 10) & 0x1F) * eva) + (((second >> 10) & 0x1F) * evb)) >> 4)
        );
    ...
    private void RenderSprites(int line) {
        ...
        for (var sprite = 0; (sprite < 128); ++sprite) {
            if (cyclesRemaining <= 0) {
                // Budget exhausted: hardware stops processing OAM for the rest of the line, in order — later
                // sprites simply don't appear.
                break;
            }
            ...
            // Affine setup is a fixed per-sprite cost, charged the moment this sprite starts rendering — even if
            // it drives the remaining budget negative (the sprite is then dropped entirely: the column loop below
            // sees an exhausted budget on its very first iteration).
            if (affine) {
                cyclesRemaining -= SpriteAffineSetupCost;
            }
            ...
            for (var column = 0; (column < boxWidth); ++column) {
                ...
                if (cyclesRemaining <= 0) {
                    // Budget exhausted mid-sprite: this column and the rest of the line's OAM don't render. (The
                    // next OAM iteration's own budget check breaks the outer loop too.)
                    break;
                }
                ...
                var colorIndex = FetchSpritePixel(
                    texelX: texelX,
                    texelY: texelY,
                    width: width,
                    tileBase: tileBase,
    ...
    // pixels of the finished, composed line (red and blue stay put): "BGRbgr" becomes "BgRbGr". Applied last,
    // after the line is fully written to the framebuffer, since it operates on the final composed output.
    private void ApplyGreenSwap(int rowBase) {
        if ((m_registers[1] & 0x1) == 0) {
            return;
        }

        for (var x = 0; (x < ScreenWidth); x += 2) {
            var left = m_framebuffer[(rowBase + x)];
            var right = m_framebuffer[((rowBase + x) + 1)];
            var leftGreen = left & 0x0000FF00u;
            var rightGreen = right & 0x0000FF00u;

            m_framebuffer[(rowBase + x)] = (left & 0xFFFF00FFu) | rightGreen;
            m_framebuffer[((rowBase + x) + 1)] = (right & 0xFFFF00FFu) | leftGreen;
        }
    }
