...
public sealed partial class AgbSerialController : IAgbSerialController, IAgbLinkClient {
    ...
    // shift-clock math predicts — the fixed latency between the start-bit write landing and the first shift edge.
    // Derived by diffing the accuracy-suite SIO timing expectations against the mode formulas below: every row is short
    // by exactly 7 cycles before this constant is added back in.
    private const int SioStartLatencyCycles = 7;

    // Multiplayer transfer duration in cycles, indexed [baud rate 0-3][connected partners 0-3 besides this console].
    // These are the real hardware/reference cycle counts for a full 16-bit round at each of the four SIOCNT baud
    // settings (9600/38400/57600/115200 bps); they are not simple bit-time*16 arithmetic because the round also
    // waits on the other stations' shift registers, so the count grows with how many are actually on the link.
    private static readonly int[][] s_multiplayerCyclesPerTransfer = [
    ...
    public void Connect(IAgbLink link) {
        ...
        m_link = link;
        m_node = link.Connect(client: this);
    }

    /// <inheritdoc/>
    public bool IsTransferActive => m_startBit;
    ...
    public void WriteRegister(uint offset, ushort value) {
        switch (offset) {
            ...
            case 0x128u: WriteSioControl(value: value); break;
            ...
            case 0x154u: break;
            case 0x156u: break;
            case 0x158u: WriteJoyStat(value: value); break;
            default: break;
        }
    }
    ...
    private void BeginTransfer() {
        ...
        switch (m_sioMode) {
            ...
            case 1:
                ...
                cycles = ((bitCycles * ((m_sioMode == 1) ? 32 : 8)) + SioStartLatencyCycles);

                break;
            case 2: {
                    // Multiplayer: only the parent drives the round; a child's start bit just marks it busy until the
                    // parent's clock completes the exchange (the cable delivers it via CompleteMultiplayerRound). On a
                    // real cable childness is daisy-chain position (player id != 0); with no cable attached the SI line
                    // (bit 2) is the only evidence, and a cable-less child correctly has no clock source at all — either
                    // way the transfer stays pending here. A lone parent still completes, reading all-ones in the absent
                    // child slots. The round's duration is looked up from the baud rate (SIOCNT bits 0-1 — shared with
                    // the Normal-mode clock-select bits, but a Baud Rate selector here) and how many other consoles are
                    // actually on the link; connectedCount (0 = solo) is clamped to the table's 0-3 range.
                    if (m_link.HasPartner ? (m_node.PlayerId != 0) : m_recvEnable) {
                        return;
                    }

                    var connectedCount = Math.Clamp(max: 3, min: 0, value: (m_link.PlayerCount - 1));

                    cycles = (s_multiplayerCyclesPerTransfer[GetBaudRateIndex()][connectedCount] + SioStartLatencyCycles);

                    break;
                }
            default:
                // UART: no framed payload is modelled (see CompleteTransfer), so this duration is an approximation —
                // the accuracy-suite reference has no UART timing expectations to calibrate against.
                cycles = ((UartBaudCycles() * 8) + SioStartLatencyCycles);
    ...
    // m_shiftClock2MHz.
    private int GetBaudRateIndex() => (m_shiftClockInternal ? 1 : 0) | (m_shiftClock2MHz ? 2 : 0);
    ...
    /// <inheritdoc/>
    bool IAgbLinkClient.TryLatchMultiplayerWord(out ushort word) {
        word = m_dataSend;

        // A console participates in a round only while its SIO is actually in Multiplayer mode with the pins owned
        // by SIO (RCNT bit 15 clear); otherwise its slot reads open-bus all ones on every other console.
        return ((m_sioMode == 2) && ((m_rcntMode & 0x2) == 0));
    }
    ...
    bool IAgbLinkClient.TryCompleteNormalSlave(uint incoming, bool word, out uint outgoing) {
        ...
        // external clock, start bit held, and the pins owned by SIO. Anything else and the master reads the
        // idle-high line, exactly as with no cable (a width/mode mismatch is a broken protocol, not an exchange).
        if ((m_sioMode != (word ? 1 : 0)) || m_shiftClockInternal || !m_startBit || ((m_rcntMode & 0x2) != 0)) {
            outgoing = 0u;
    ...
    private ushort PackRcnt() {
        ...
        // In JOY-bus mode (RCNT mode 3) the SC/SD lines (bits 0-1) are owned by the JOY-bus engine's own clock/data
        // protocol, not the GPIO output latch, and read back 0 regardless of what was written — SI/SO (bits 2-3),
        // their direction-mode bits (4-7), and the SI-IRQ-enable bit (8) are unaffected (JOY RCNT reads 0xC1FC for
        // an all-ones write, vs 0xC1FF without this mask).
        if (m_rcntMode == 3) {
            value &= ~0x0003u;
        }
