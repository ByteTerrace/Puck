    ...
    private void RenderTextRow(Span<int> destination, uint charBase, uint screenBase, bool is8Bpp, int size,
        ...
        for (var x = 0; (x < ScreenWidth);) {
            ...
            if (is8Bpp) {
                ...
                if (address < 0x18000u) {
                    var pixels = BinaryPrimitives.ReadUInt64LittleEndian(source: m_vram.AsSpan(
                        length: 8,
                        start: ((int)address)
                    ));
                    var shift = (first * 8);
                ...
                if (address < 0x18000u) {
                    var pixels = BinaryPrimitives.ReadUInt32LittleEndian(source: m_vram.AsSpan(
                        length: 4,
                        start: ((int)address)
                    ));
