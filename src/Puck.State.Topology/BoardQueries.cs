...
public static class BoardQueries {
    ...
    private static long Component(BoardComponentQuery query, ReadOnlySpan<long> values, int source) {
        ...
        if (
            (values[source] < query.Lower) ||
            (values[source] > query.Upper)
        ) {
            return 0;
        }
        var markPool = System.Buffers.ArrayPool<byte>.Shared.Rent(minimumLength: count);
        var stackPool = System.Buffers.ArrayPool<int>.Shared.Rent(minimumLength: count);

        try {
            var mark = markPool.AsSpan(
                length: count,
                start: 0
            );
            var stack = stackPool.AsSpan(
                length: count,
                start: 0
            );
            ...
            while (size > 0) {
                ...
                for (var direction = 0; (direction < topology.DirectionCount); direction++) {
                    ...
                    if (
                        (values[next] >= query.Lower) &&
                        (values[next] <= query.Upper)
                    ) {
                        mark[next] = 1;
                        stack[size++] = next;
                    } else if (
                        boundary &&
                        (values[next] >= query.BoundaryLower) &&
                        (values[next] <= query.BoundaryUpper)
                    ) {
                        mark[next] = 2;
                        free++;
                    }
    ...
    private static long PathCost(BoardPathCostQuery query, ReadOnlySpan<long> costs, int source, int target) {
        ...
        var distancePool = System.Buffers.ArrayPool<long>.Shared.Rent(minimumLength: (count + capacity));
        var cellPool = System.Buffers.ArrayPool<int>.Shared.Rent(minimumLength: capacity);
        var settledPool = System.Buffers.ArrayPool<byte>.Shared.Rent(minimumLength: count);

        try {
            var distances = distancePool.AsSpan(
                length: count,
                start: 0
            );
            var heapDistance = distancePool.AsSpan(
                length: capacity,
                start: count
            );
            var heapCell = cellPool.AsSpan(
                length: capacity,
                start: 0
            );
            var settled = settledPool.AsSpan(
                length: count,
                start: 0
            );

            distances.Fill(value: long.MaxValue);
            settled.Clear();
            distances[source] = 0;
            var size = 0;
            ...
            for (var visited = 0; (visited <= query.MaxVisits); visited++) {
                ...
                while (size > 0) {
                    Pop(
                        cell: heapCell,
                        distance: heapDistance,
                        ordinal: out var candidate,
                        size: ref size,
                        value: out var candidateDistance
                    );
                    if (
                        (settled[candidate] != 0) ||
                        (candidateDistance != distances[candidate])
                    ) {
                        continue;
                    }
                    best = candidate;
                ...
                for (var direction = 0; (direction < topology.DirectionCount); direction++) {
                    ...
                    if (
                        ...
                        (settled[neighbour] != 0) ||
                        (costs[neighbour] < 0) ||
                        (costs[neighbour] > (query.MaxCost - distance))
                    ) {
                        continue;
                    }
    ...
    private static long Placement(BoardComponentQuery query, ReadOnlySpan<long> values, int source) {
        ...
        if (
            (values[source] < query.BoundaryLower) ||
            (values[source] > query.BoundaryUpper)
        ) {
            return -1;
        }
        var markPool = System.Buffers.ArrayPool<byte>.Shared.Rent(minimumLength: count);
        ...
        try {
            ...
            for (var seed = 0; (seed < topology.DirectionCount); seed++) {
                ...
                if (
                    (start < 0) ||
                    (mark[start] != 0) ||
                    (values[start] < query.Lower) ||
                    (values[start] > query.Upper)
                ) {
                    continue;
                }
    ...
    public static long ClearEnclosed(CompiledTopology topology, Span<long> values, int source, long lower, long upper, long empty) {
        ...
        var markPool = System.Buffers.ArrayPool<byte>.Shared.Rent(minimumLength: count);
        var stackPool = System.Buffers.ArrayPool<int>.Shared.Rent(minimumLength: (2 * count));
    ...
    public static long Evaluate(BoardQuery query, ReadOnlySpan<long> values, long empty, int source, int dynamicTarget = 0) {
        ...
        if (query is BoardMaskQuery mask) {
            ...
            for (var ordinal = 0; ((ordinal < topology.CellCount) && (ordinal < BoardMask.MaxCells)); ordinal++) {
                if (
                    (values[ordinal] >= mask.Lower) &&
                    (values[ordinal] <= mask.Upper)
                ) {
                    result |= (1L << ordinal);
                }
            }
            ...
            for (var directionIndex = 0; (directionIndex < directions.Length); directionIndex++) {
                ...
                for (var distance = 1; (distance < topology.CellCount); distance++) {
                    ...
                    if (values[rayCell] != empty) {
                        if (
                            (values[rayCell] >= attacks.Lower) &&
                            (values[rayCell] <= attacks.Upper)
                        ) {
                            return 1;
                        }
                        break;
    ...
    public static long ImageOfMask(CompiledTopology topology, int element, long mask) {
        ...
        while (remaining != 0UL) {
            var cell = System.Numerics.BitOperations.TrailingZeroCount(value: remaining);

            remaining &= (remaining - 1UL);
            if (cell < topology.CellCount) {
                var carried = topology.Image(
                    cell: cell,
                    element: element
                );
    ...
    public static void Read(StateRow row, CompiledTopology topology, Span<long> values) {
        values[..topology.CellCount].Fill(value: ((StateDomain.CellsOf)row.EffectiveDomain).Empty);
        var cells = row.Cells;
        ...
        for (var cellIndex = 0; (cellIndex < (cells?.Count ?? 0)); cellIndex++) {
            ...
            if (
                (cell.Value.Kind is (CellKind.Bool or CellKind.Fixed or CellKind.Int)) &&
                topology.TryCell(
                    cell.Key.Value,
                    out var index
                )
            ) {
                values[index] = cell.Value.Raw;
            }
        }
    }
