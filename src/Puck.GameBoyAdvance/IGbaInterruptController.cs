...
/// exactly like ARES (gba/cpu/cpu.cpp <c>stepIRQ</c> + io.cpp): writes land in a "next" stage and reads return
...
public interface IGbaInterruptController {
    /// <summary>Gets the level of the IRQ line into the CPU — ARES's <c>irq.synchronizer</c>, recomputed each
    ...
    /// resumes on any enabled+requested interrupt regardless of IME (ARES gba/cpu/cpu.cpp:48).</summary>
    ...
    /// committed stage, then shifts the programmed ("next") stage down into it (ARES <c>stepIRQ</c>). While the
    /// CPU is stalled by DMA the pipeline freezes (the synchronizer holds), matching ARES's <c>stallingCPU</c>.</summary>
    ...
    /// becomes visible to the CPU one <see cref="StepSync"/> later (ARES <c>setInterruptFlag</c>).</summary>
    ...
    /// committed stage, matching ARES io.cpp:187-215.</summary>
