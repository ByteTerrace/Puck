...
public sealed partial class Arm7Tdmi {
    ...
    private static void ArmMultiply(Arm7Tdmi cpu, uint opcode) {
        ...
        var result = cpu.m_gpr[rm] * cpu.m_gpr[rs];
        ...
        cpu.m_bus.Idle(cycles: MultiplyCycles(multiplier: cpu.m_gpr[rs]) + (accumulate ? 1 : 0));
    }
    ...
    private static void ArmMultiplyLong(Arm7Tdmi cpu, uint opcode) {
        ...
        if (signed) {
            var product = (long)(int)cpu.m_gpr[rm] * (int)cpu.m_gpr[rs];
            ...
            result = (ulong)cpu.m_gpr[rm] * cpu.m_gpr[rs];
        ...
        cpu.m_bus.Idle(cycles: MultiplyCycles(multiplier: cpu.m_gpr[rs]) + (accumulate ? 2 : 1));
    }
