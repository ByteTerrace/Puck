...
/// <summary>
/// The physical link-cable transport between Game Boy Advance consoles. A <see cref="GbaSerialController"/> drives
/// all of its serial I/O through this seam, so two or more in-process <c>GameBoyAdvanceMachine</c> instances can be
/// wired together (and a future networked transport can slot in) without the SIO subsystem knowing how the wire is
/// realised. The default <see cref="NullGbaLink"/> models a lone console: the lines idle exactly as hardware leaves
/// them with no cable attached, so a single console behaves correctly without magic auto-completion.
/// </summary>
public interface IGbaLink {
    ...
    IGbaLinkNode Connect();
}

/// <summary>One console's endpoint on an <see cref="IGbaLink"/>. The serial controller exchanges words through it.</summary>
public interface IGbaLinkNode {
