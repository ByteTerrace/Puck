...
/// <summary>
/// The four 16-bit timers, ported one-for-one from ARES's cycle-stepped <c>gba/cpu/timer.cpp</c>. Each timer is a
...
public sealed class GbaTimerController : IGbaTimerController {
    // Prescaler period mask per the 2-bit control field: divide by 1 / 64 / 256 / 1024 (ARES timer.cpp:38).
    ...
    public ushort ReadRegister(uint offset) {
        ...
        if ((offset & 0x2u) == 0u) {
            // CNT_L: the live counter, current because it is stepped every cycle (ARES io.cpp:27-31).
            return (ushort)m_period[timer];
        }

        // CNT_H: the committed control fields (ARES io.cpp:32-36).
    ...
    public void WriteRegister(uint offset, ushort value) {
        ...
        if ((offset & 0x2u) == 0u) {
            // CNT_L write: latch both reload bytes; applied next cycle (ARES io.cpp:304-313).
            m_latchReload[timer] = value;
            m_reloadFlags[timer] = 0b11;
        }
        else {
            // CNT_H write: latch the control word; applied next cycle (ARES io.cpp:317-321).
    ...
    public void RunCycle(long clock) {
        // ARES gba/cpu/cpu.cpp:82-97: run all four, then reload-latch all four, then (if a write is pending)
        // commit the control/reload latch. stepIRQ is handled separately by the interrupt controller.
    ...
    // ARES Timer::run (timer.cpp:35-40): a prescaler-driven timer steps when the global clock hits its boundary.
    ...
    // ARES Timer::step (timer.cpp:42-55): increment; on overflow reload, raise the IRQ, clock Direct Sound, and
    ...
    // ARES Timer::reloadLatch (timer.cpp:28-33): apply a pending enable reload one cycle after it was armed.
    ...
    // ARES Timer::stepLatch (timer.cpp:1-26): commit latched reload bytes and the latched control word; a fresh
