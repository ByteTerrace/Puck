    ...
    public static FixedVector3 LocalSupportPoint(FixedBodyColliderVolume volume, FixedVector3 localDirection) {
        ...
        switch (volume.Kind) {
            ...
            case FixedBodyColliderKind.Capsule: {
                    ...
                    var axisPoint = ((FixedVector3.Dot(
                        left: unit,
                        right: axis
                    ) > FixedQ4816.Zero)
                        ? volume.Endpoint
            ...
            case FixedBodyColliderKind.Box: {
                    ...
                    var corner = new FixedVector3(
                        X: ((boxLocal.X >= FixedQ4816.Zero)
                        ? volume.HalfExtents.X
                        : -volume.HalfExtents.X),
                        Y: ((boxLocal.Y >= FixedQ4816.Zero)
                        ? volume.HalfExtents.Y
                        : -volume.HalfExtents.Y),
                        Z: ((boxLocal.Z >= FixedQ4816.Zero)
        ...
        switch (volume.Kind) {
            case FixedBodyColliderKind.Box: {
                    ...
                    if (
                        ...
                        var x = ((localInto.X >= FixedQ4816.Zero)
                            ? half.X
                            : -half.X
                        );

                        corners[0] = new FixedVector3(
                            X: x,
                            Y: half.Y,
                            Z: half.Z
                        );
                        corners[1] = new FixedVector3(
                            X: x,
                            Y: half.Y,
                            Z: -half.Z
                        );
                        corners[2] = new FixedVector3(
                            X: x,
                            Y: -half.Y,
                            Z: half.Z
                        );
                        corners[3] = new FixedVector3(
                            X: x,
                            Y: -half.Y,
                            Z: -half.Z
                        );
                    } else if (absY >= absZ) {
                        var y = ((localInto.Y >= FixedQ4816.Zero)
                            ? half.Y
                            : -half.Y
                        );
                        ...
                        var z = ((localInto.Z >= FixedQ4816.Zero)
                            ? half.Z
                            : -half.Z
                        );
            ...
            case FixedBodyColliderKind.Capsule: {
                    ...
                    var alignment = FixedQ4816.Abs(value: FixedVector3.Dot(
                        left: axisUnit,
                        right: localInto
                    ));
