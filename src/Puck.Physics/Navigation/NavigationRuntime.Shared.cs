...
public sealed partial class NavigationRuntime {
    ...
    public sealed partial class Domain {
        ...
        private long SharedWorkspaceBytes => (((long)m_sharedTrees.Length) *
            (((CellCount * ((5L * sizeof(int)) + sizeof(byte))) + ((Math.Min(
            val1: CellCount,
            val2: m_capacity.MaxConcurrentRequesters
        ) * 2L) * sizeof(int)))
            + (SharedTree.HashBlockCount(cells: CellCount) * (sizeof(ulong) + sizeof(byte)))));
        private ulong MediumRevision => ((m_mediumField >= 0)
            ? m_fields!.ValueRevision(field: m_mediumField)
            : 0
        );
        ...
        public void AdvanceShared() {
            ...
            while (
                (SharedExpandedLast < Sharing.ExpandedNodesPerTick) &&
                (idleSlots < m_sharedTrees.Length)
            ) {
        ...
        public NavigationStatus RequestShared(int start, int goal, Span<int> path, out int length) {
            ...
            foreach (var tree in m_sharedTrees) {
                ...
                if (
                    !tree.PinnedForStep &&
                    (tree.PendingCount == 0) &&
                    ((victim is null) || (tree.Goal < 0) || ((victim.Goal >= 0) && (tree.Age > victim.Age)))
                ) { victim = tree; }
            }
            if (selected is null) {
                if (victim is null) { SharedCapacityRefusalsLast++; return NavigationStatus.CapacityLimited; }
                selected = victim;
                var previousAge = ((selected.Goal >= 0)
                    ? selected.Age
                    : m_sharedTrees.Length
                );
            ...
            foreach (var tree in m_sharedTrees) {
                ...
                if (
                    (tree.Goal >= 0) &&
                    (tree != selected) &&
                    (tree.Age < selected.Age)
                ) { tree.Age++; }
            }
            selected.Age = 0;
            var status = selected.ReadPath(
                start,
                path[..Math.Min(
                    val1: path.Length,
                    val2: Tuning.MaxPathNodes
                )],
                out length
            );
        ...
        private int Predecessors(int node, Span<SharedEdge> edges) {
            ...
            for (var dy = ((Tuning.Kind == NavigationKind.Surface)
                ? 0
                : -1
            ); (dy <= ((Tuning.Kind == NavigationKind.Surface)
                ? 0
                : 1)); dy++) {
        ...
        public NavigationSharedCheckpoint CaptureShared() => new(
            Cursor: (SharedStale
            ? 0
            : m_sharedCursor),
            Trees: m_sharedTrees.Select(selector: tree => (SharedStale
            ? new NavigationTreeCheckpoint(
                    Age: 0,
                    Goal: -1,
                    Nodes: [],
                    Pending: []
                )
            : tree.Capture())).ToArray()
        );
        public void ValidateShared(NavigationSharedCheckpoint checkpoint) {
            if (
                (checkpoint is null) ||
                (checkpoint.Trees is null) ||
                (checkpoint.Trees.Length != m_sharedTrees.Length) ||
                (checkpoint.Cursor < 0) ||
                (checkpoint.Cursor >= Math.Max(
                val1: 1,
                val2: m_sharedTrees.Length
            ))
            ) {
            ...
            foreach (var tree in checkpoint.Trees) {
                if (
                    ...
                    (tree.Goal >= CellCount) ||
                    (tree.Age < 0) ||
                    (tree.Age >= m_sharedTrees.Length) ||
                    (tree.Nodes is null) ||
                    (tree.Nodes.Length > CellCount) ||
                    (tree.Pending is null) ||
                    (tree.Pending.Length > Math.Min(
                    val1: CellCount,
                    val2: m_capacity.MaxConcurrentRequesters
                ))
                ) {
                ...
                foreach (var node in tree.Nodes) {
                    if (
                        ...
                        (node.Node >= CellCount) ||
                        (node.Cost < 0) ||
                        (node.Cost > (CellCount * SpaceDiagonalCost))
                    ) {
                ...
                foreach (var node in tree.Nodes) {
                    ...
                    if (
                        ...
                        (Math.Abs(value: dy) > 1) ||
                        (Math.Abs(value: dz) > 1) ||
                        ((m_edges[node.Node] & NeighborBit(
                        dx: dx,
                        dy: dy,
                        dz: dz
                    )) == 0) ||
                        ((node.Cost - next.Cost) != ((axes == 1)
                        ? StraightCost
                        : ((axes == 2)
                            ? DiagonalCost
                            : SpaceDiagonalCost)))
        ...
        private sealed class SharedTree {
            ...
            public SharedTree(int cells, int maxConcurrentRequesters) {
                ...
                m_pendingList = new int[Math.Min(
                    val1: cells,
                    val2: maxConcurrentRequesters
                )];
            ...
            private void Open(int node, int cost, int next) {
                ...
                if (m_stamp[node] != m_generation) {
                    m_stamp[node] = m_generation; m_open.Push(
                        node: node,
                        order: order
                    );
            ...
            public void AppendHash(ref Fnv1aHash hash, bool empty) {
                hash.Add(value: (empty
                    ? -1
                    : Goal)); hash.Add(value: (empty
                    ? 0
                    : Age));
                if (
                    empty ||
                    ...
                    for (var block = 0; (block < m_blockHashes.Length); block++) {
                        if (m_dirtyBlocks[block]) {
                            ...
                            for (var node = (block * HashBlockSize); (node < end); node++) {
                                ...
                                blockHash.Add(value: ((byte)(Settled(node: node)
                                    ? 1
                                    : 0)));
                            }
                ...
                if (m_pendingDirty) {
                    ...
                    var pending = m_pendingOrdered.AsSpan(
                        length: count,
                        start: 0
                    );
            ...
            public bool Expand(Domain domain) {
                ...
                for (var index = 0; (index < count); index++) {
                    ...
                    if (
                        !Settled(node: edge.Node) &&
                        ((m_stamp[edge.Node] != m_generation) || (cost < m_cost[edge.Node]))
                    ) {
                        Open(
                            edge.Node,
                            cost,
