...
public sealed partial class NavigationRuntime {
        ...
        for (var index = 0; (index < domains.Count); index++) {
            ...
            if (previous is not null) {
                for (var oldIndex = 0; (oldIndex < previous.m_domains.Length); oldIndex++) {
                    ...
                    if (
                        (candidate.Name == domains[index].Name) &&
                        candidate.TryRebind(
                        row: domains[index],
                        query: query!,
                        fields: fields,
            ...
            m_domains[index] = (reused ?? new Domain(
                row: domains[index],
                query: query!,
                fields: fields,
                capacity: capacity
    ...
    public sealed partial class Domain {
        ...
        public Domain(NavigationDomainInput row, IWorldQuery query, INavigationMediumField? fields, NavigationCapacity capacity) {
            Name = row.Name;
            Tuning = row;
            var up = new FixedVector3(
                X: FixedQ4816.Zero,
                Y: FixedQ4816.One,
                Z: FixedQ4816.Zero
            );

            m_rotation = FixedQuaternion.FromAxisAngle(
                up,
                row.YawRadians
            );
            m_inverseRotation = FixedQuaternion.FromAxisAngle(
                up,
                -row.YawRadians
            );
            m_fields = fields;
            m_query = query;
            m_capacity = capacity;
            m_mediumField = (((Tuning.Kind == NavigationKind.Medium) && (fields is not null) && fields.TryFieldIndex(
                name: Tuning.Medium!,
                field: out var mediumField
            ))
                ? mediumField
                : -1
            );
        ...
        internal bool TryRebind(NavigationDomainInput row, IWorldQuery query, INavigationMediumField? fields, NavigationCapacity capacity) {
            if (
                (row != Tuning) ||
                (capacity != m_capacity) ||
                ((Tuning.Kind == NavigationKind.Medium) && !SameMediumRevision(fields: fields))
            ) {
                return false;
            }
            ...
            if (
                ...
                !HasSameStaticGeometry(query: query)
            ) {
                return false;
            }
            m_query = query;
        ...
        private bool SameMediumRevision(INavigationMediumField? fields) {
            if (
                (Tuning.Kind != NavigationKind.Medium) ||
                (m_fields is null) ||
                (fields is null) ||
                (m_mediumField < 0) ||
                !fields.TryFieldIndex(
                Tuning.Medium!,
                out var field
            ) ||
                (field != m_mediumField)
            ...
            return (
                ...
                (m_sharedFieldRevision == fields.ValueRevision(field: field))
            );
        }
        ...
        private bool TrySampleCell(IWorldQuery query, int node, out FixedQ4816 routeY) {
            ...
            if (Tuning.Kind != NavigationKind.Surface) {
                return !query.Overlap(
                    center: FixedPosition.FromLocal(local: probe),
                    radius: Tuning.AgentRadius
                );
            }
            if (!query.TryGroundHeight(
                FixedPosition.FromLocal(local: probe),
                Tuning.ProbeUp,
                Tuning.ProbeDown,
                out var ground
            )) {
            ...
            if (query.Overlap(
                center: FixedPosition.FromLocal(local: foot),
                radius: Tuning.AgentRadius
            )) { return false; }
            if (headY <= routeY) { return true; }
            var head = new FixedVector3(
                X: probe.X,
                Y: headY,
                Z: probe.Z
            );
            ...
            return (
                !query.Overlap(
                center: FixedPosition.FromLocal(local: head),
                radius: Tuning.AgentRadius
            ) &&
                !query.SphereCast(
                FixedPosition.FromLocal(local: foot),
                core,
                Tuning.AgentRadius,
                core.Length,
                out _
        ...
        private bool HasSameStaticGeometry(IWorldQuery query) {
            for (var node = 0; (node < CellCount); node++) {
                ...
                if (
                    (walkable != m_walkable[node]) ||
                    (walkable && (ground != m_ground[node]))
                ) { return false; }
            }
            for (var current = 0; (current < CellCount); current++) {
                ...
                var minY = ((Tuning.Kind == NavigationKind.Surface)
                    ? 0
                    : -1
                );
                var maxY = ((Tuning.Kind == NavigationKind.Surface)
                    ? 0
                    : 1
                );
                ...
                for (var dy = minY; (dy <= maxY); dy++) {
                    for (var dz = -1; (dz <= 1); dz++) {
                        for (var dx = -1; (dx <= 1); dx++) {
                            ...
                            if (
                                (axes == 0) ||
                                !AdmitsAxes(axes: axes) ||
                                (((uint)nx) >= ((uint)Tuning.Width)) ||
                                (((uint)ny) >= ((uint)Tuning.Layers)) ||
                                (((uint)nz) >= ((uint)Tuning.Depth))
                            ) { continue; }
                            var next = Index(
                                x: nx,
                                y: ny,
                            ...
                            var candidate = (m_walkable[current] && CanTraverseStatic(
                                current: current,
                                dx: dx,
                                dy: dy,
                                dz: dz,
                            ...
                            var currentBit = ((m_edges[current] & NeighborBit(
                                dx: dx,
                                dy: dy,
                                dz: dz
                            )) != 0U);
                            var reverseBit = ((m_edges[next] & NeighborBit(
                                dx: -dx,
                                dy: -dy,
                                dz: -dz
                            )) != 0U);
                            ...
                            if (
                                (candidate != currentBit) ||
                                (candidate != reverseBit)
                            ) { return false; }
                        }
                    }
                }
        ...
        public bool AdmitsLocomotion(in FixedVector3 from, in FixedVector3 to) {
            if (
                (Tuning.Kind == NavigationKind.Surface) ||
                !TryCell(
                node: out _,
                position: from
            ) ||
            ...
            if (
                (Tuning.Kind == NavigationKind.Medium) &&
                ((m_fields is null) || !m_fields.IsSegmentInsideMedium(
                m_mediumField,
                from,
                to,
                Tuning.AgentRadius,
                m_capacity.MaxMediumSegmentSubdivisions
            ))
            ) { return false; }
            if (
                m_query.Overlap(
                center: FixedPosition.FromLocal(local: from),
                radius: Tuning.AgentRadius
            ) ||
                m_query.Overlap(
                center: FixedPosition.FromLocal(local: to),
                radius: Tuning.AgentRadius
            )
            ) { return false; }
            var delta = (to - from);
            ...
            return (
                ...
                !m_query.SphereCast(
                FixedPosition.FromLocal(local: from),
                delta,
                Tuning.AgentRadius,
                delta.Length,
                out _
        ...
        public FixedVector3 Position(int node) {
            ...
            return ((Tuning.Kind == NavigationKind.Surface)
                ? new FixedVector3(
                    X: position.X,
                    Y: m_ground[node],
                    Z: position.Z
        ...
        private FixedVector3 GridPosition(int x, int y, int z) {
            var local = new FixedVector3(
                X: (Tuning.CellSize * FixedQ4816.FromInteger(value: x)),
                Y: (Tuning.CellSize * FixedQ4816.FromInteger(value: y)),
                Z: (Tuning.CellSize * FixedQ4816.FromInteger(value: z))
            );

            return (Tuning.Origin + ((Tuning.YawRadians == FixedQ4816.Zero)
                ? local
                : m_rotation.Rotate(vector: local)));
        }

        public bool TryCell(in FixedVector3 position, out int node) {
            var local = ((Tuning.YawRadians == FixedQ4816.Zero)
                ? position
                : (Tuning.Origin + m_inverseRotation.Rotate(vector: (position - Tuning.Origin)))
            );
            var x = RoundedCell(
                value: (((Int128)local.X.Value) - Tuning.Origin.X.Value),
                cellSize: Tuning.CellSize.Value
            );
            var z = RoundedCell(
                value: (((Int128)local.Z.Value) - Tuning.Origin.Z.Value),
                cellSize: Tuning.CellSize.Value
            );
            var y = ((Tuning.Kind == NavigationKind.Surface)
                ? 0
                : RoundedCell(
                    value: (((Int128)local.Y.Value) - Tuning.Origin.Y.Value),
                    cellSize: Tuning.CellSize.Value
                )
            );
        ...
        public NavigationStatus FindPath(int start, int goal, Span<int> path, out int pathLength, out int expanded) {
            ...
            expanded = 0;
            if (
                !IsWalkable(node: start) ||
                !IsWalkable(node: goal)
            ) {
            ...
            while (m_open.Count != 0) {
                var current = m_open.Pop(order: new GoalOrder(
                    domain: this,
                    goal: goal
                ));
                    ...
                    for (var dz = -1; (dz <= 1); dz++) {
                        for (var dx = -1; (dx <= 1); dx++) {
                            ...
                            if (
                                ...
                                (m_closedStamp[next] == m_searchStamp)
                            ) {
                                continue;
                            }
                            var stepCost = ((axes == 1)
                            ...
                            if (
                                (m_openStamp[next] != m_searchStamp) ||
                                (nextCost < m_cost[next])
                            ) {
                                Open(
                                    cost: nextCost,
                                    goal: goal,
        ...
        private bool CanTraverse(int current, int next, int x, int y, int z, int dx, int dy, int dz) {
            if (
                ((m_edges[current] & NeighborBit(
                dx: dx,
                dy: dy,
                dz: dz
            )) == 0U) ||
                !IsWalkable(node: next)
            ) {
                return false;
            }
            var axes = ((((dx == 0)
        ...
        public bool IsTraversableEdge(int current, int next) {
            EnsureBaked();
            if (
                (((uint)current) >= ((uint)CellCount)) ||
                (((uint)next) >= ((uint)CellCount)) ||
                (current == next)
            ) {
                return false;
            }
        ...
        private bool CanTraverseStatic(IWorldQuery query, int current, int next, int x, int y, int z, int dx, int dy, int dz) {
            ...
            if (Tuning.Kind == NavigationKind.Surface) {
                ...
                if (
                    (rise > Tuning.MaxStepHeight) ||
                    (rise > maximumSlopeRise)
                ) {
                    return false;
                }
            }
        ...
        private bool HasClearTransition(int current, int next) => HasClearTransition(
            current: current,
            next: next,
            query: m_query
        );
        ...
        private int Compare(int left, int right, int goal) {
            ...
            var rightH = Heuristic(
                goal: goal,
                node: right
            );
            var f = (m_cost[left] + leftH).CompareTo(value: (m_cost[right] + rightH));

            return ((f != 0)
                ? f
                : ((leftH != rightH)
                    ? leftH.CompareTo(value: rightH)
                    : left.CompareTo(value: right)
            ));
        }

        private readonly struct GoalOrder(Domain domain, int goal) : INodeOrder {
            public int Compare(int left, int right) => domain.Compare(
                goal: goal,
                left: left,
                right: right
            );
        ...
        private int Heuristic(int node, int goal) {
            ...
            return checked((Math.Max(
                val1: Math.Abs(value: (nx - gx)),
                val2: Math.Max(
                    val1: Math.Abs(value: (ny - gy)),
                    val2: Math.Abs(value: (nz - gz))
                )
            ) * StraightCost));
        }
        ...
        public bool IsWalkable(int node) {
            EnsureBaked();
            if (
                (((uint)node) >= ((uint)m_walkable.Length)) ||
                !m_walkable[node]
            ) {
                return false;
            }
            ...
            return (
                (m_fields is not null) &&
                m_fields.IsInsideMedium(
                field: m_mediumField,
        ...
        private NavigationStatus Reconstruct(int goal, Span<int> path, out int pathLength) {
            ...
            for (var node = goal; (node >= 0); node = m_parent[node]) {
                if (
                    ...
                    (pathLength >= Tuning.MaxPathNodes)
                ) {
                    pathLength = 0;
