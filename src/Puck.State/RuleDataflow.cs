...
public static class RuleDataflow {
    ...
    public static bool ReadsTick(CompiledRule rule) {
        ...
        foreach (var token in rule.Gate) {
            if (
                OperandReadsTick(operand: token.Left) ||
                OperandReadsTick(operand: token.Comparand) ||
                ExpressionReadsTick(tokens: token.LeftExpression) ||
                ExpressionReadsTick(tokens: token.RightExpression)
            ) {
                return true;
            }
        }
