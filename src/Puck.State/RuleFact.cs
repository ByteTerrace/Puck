...
public readonly record struct RuleFact(long Value, CellKind Kind, bool IsForever, bool IsAbsent = false) {
    ...
    public static RuleFact Absent(CellKind kind) => new(
        IsAbsent: true,
        IsForever: false,
        Kind: kind,
        Value: 0L
    ...
    public static RuleFact Finite(FixedQ4816 value) => new(
        Value: value.Value,
        Kind: CellKind.Fixed,
        IsForever: false
    );
    ...
    public static RuleFact Finite(long value, CellKind kind) => new(
        Value: value,
        Kind: kind,
        IsForever: false
    );
    ...
    public static RuleFact Forever(CellKind kind) => new(
        Value: 0L,
        Kind: kind,
        IsForever: true
    );
