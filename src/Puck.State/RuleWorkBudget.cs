...
public static class RuleWorkBudget {
    private static long Worst(ExclusionNode node, IReadOnlyDictionary<string, long> writers) {
        ...
        foreach (var (cell, ranges) in node.Children) {
            ...
            for (var index = 0; (index < ranges.Count); index++) { worsts[index] = Worst(
            ...
            foreach (var (candidate, _) in ranges) {
                ...
                for (var index = 0; (index < ranges.Count); index++) {
                    if (ranges[index].Range.Contains(value: candidate.Low)) { sum = SaturatingAdd(
                        left: sum,
                        right: worsts[index]
                    ); }
                }
            ...
            for (var index = 0; (index < admitted); index++) { total = SaturatingAdd(
                left: total,
                right: sums[index]
            ); }
        }
    ...
    public static Dictionary<string, long> CountWriters(IReadOnlyList<(CompiledRule Rule, long Multiplier)> rules) {
        ...
        foreach (var (rule, multiplier) in rules) {
            foreach (var write in RuleDataflow.Writes(rule: rule)) {
                var into = ((write.Key is null)
                    ? byRow
                    : byCell
                );
                var key = ((write.Key is null)
                    ? write.Row
                    : $"{write.Row}.{write.Key}"
                );

                into[key] = SaturatingAdd(
                    left: into.GetValueOrDefault(key: key),
                    right: multiplier
                );
            }
        }
        ...
        foreach (var (cell, count) in byCell) {
            writers[cell] = SaturatingAdd(
                left: count,
                right: byRow.GetValueOrDefault(key: cell[..cell.LastIndexOf(value: '.')])
            );
        }
    ...
    public static long OperationCost(ExpressionOp operation, CellKind kind, BoardQuery? board = null) => operation switch {
        ...
        ExpressionOp.BoardShift or ExpressionOp.BoardImage => ((board is { } b)
        ? ((b.Topology.CellCount / 2) + b.Visits)
        : 24L),
