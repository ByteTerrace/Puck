...
public sealed class ZoneTable {
    ...
    public string? NameAt(long index) => (((((ulong)index) < ((ulong)m_handles.Length)) && (Names[((int)index)].Length != 0))
        ? Names[((int)index)]
        : null
    );
    ...
    public bool TryHandle(long index, out StateHandle handle) {
        if (
            (((ulong)index) < ((ulong)m_handles.Length)) &&
            m_handles[index].IsValid
        ) {
...
public sealed class LiveZone {
    ...
    private bool TryResolveIndex(IRuleReader reader, out long index) {
        ...
        return RuleEvaluation.TryResolveIndex(
            index: out index,
            reader: reader,
    ...
    public void CollectIndexReads(List<RuleAccess> into) => RuleAccess.CollectReference(
        reference: Index,
        into: into
    );
