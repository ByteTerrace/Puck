...
public sealed class StateVector : IEquatable<StateVector> {
    ...
    /// <summary>Creates a vector without validating dimensions, range, or admissibility. For trusted paths that
    ...
    /// <param name="components">The components to digest.</param>
    /// <returns>The digest.</returns>
    public static uint ComputeDigest(ReadOnlySpan<sbyte> components) {
        var hash = Fnv1aHash.Compute(values: MemoryMarshal.Cast<sbyte, byte>(span: components));
