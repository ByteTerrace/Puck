...
public sealed partial class SearchRuntime {
    private bool Land(Job job, Func<IReadOnlyList<SearchWrite>, bool> apply) {
        var plan = job.Plan;
        var rows = m_live();
        var tokenCells = StateRows.FindStateRow(
            rows: rows,
            name: plan.Tokens
        )?.Cells;
        ...
        if (
            (plan.Legal is { } legal) &&
            (tokenCells is not null)
        ) {
            for (var index = 0; ((index < tokenCells.Count) && (index < job.Legal.Length)); index++) {
                m_outputs.Add(item: new SearchWrite.Cell(
                    Row: legal,
                    Key: tokenCells[index].Key.Value,
                    Value: job.Legal[index]
                ));
            }
        }
        if (
            (plan.Counts is { } counts) &&
            (tokenCells is not null)
        ) {
        ...
        if (
            ...
            var held = ((plan.Held is { } heldName)
                ? Slot(
                    name: heldName,
                    store: m_store
                )
