...
public sealed partial class SearchRuntime {
    ...
    private void Walk(Job job, ulong tick) {
        ...
        if (plan.Chance is { AtDepth: 0 } rootChance) {
            var chanceRow = StateRows.FindStateRow(
                rows: rows,
                name: rootChance.Row
            );

            job.Best = ((chanceRow is not null)
                ? ChanceExpectation(
                    job: job,
                    position: m_base!,
                    chanceRow: chanceRow,
                    remainingDepth: (job.PassDepth - 1),
                    tick: tick
                )
                : 0L
            );
        ...
        var turn = StateRows.FindStateRow(
            rows: rows,
            name: plan.Turn
        );
        var verdict = StateRows.FindStateRow(
            rows: rows,
            name: plan.Verdict
        );
        var cells = plan.CellCount;
        var shapes = plan.Shapes;
        var scoresRow = ((plan.Scores is { } scoresName)
        ...
        if (
            (tokens?.Cells is not { } tokenCells) ||
            (turn is null) ||
            (verdict is null) ||
            (tokenCells.Count != job.Legal.Length) ||
        ...
        while (
            ...
            if (
                ...
                var value = ((chanceRow is not null)
                    ? ChanceExpectation(
                        job: job,
                        position: scratch,
                        chanceRow: chanceRow,
        ...
        switch (shape.Kind) {
            case SearchShapeKind.Transfer: {
                    ...
                    if (
                        ...
                        (endKey != tokenCells[token].Key) ||
                        !frame.ZoneHasRoom(row: zones[candidateIndex])
                    ) {
                        return false;
                    }
            ...
            case SearchShapeKind.Pair: {
                    ...
                    var companionFrom = (frame.TryStoredAt(
                        index: companion,
                        row: tokens,
    ...
    private static void ApplyCandidate(
        ...
        if (shape.Kind == SearchShapeKind.Transfer) {
            _ = scratch.TryTransferToken(
                from: zones![from],
                to: zones[target],
                key: tokenCells[token].Key,
        ...
        switch (shape.Kind) {
            ...
            case SearchShapeKind.Pair:
                _ = scratch.TryWrite(
                    row: tokens,
                    key: tokenCells[companionIndex].Key,
    ...
    private long EvaluateScore(SearchPlan plan, ulong tick) =>
        (m_host!.Evaluator.TryEvaluateExpression(
            program: plan.Score!,
            kind: CellKind.Int,
            tick: tick,
            engineTick: m_engineTick,
    ...
    private static long SeatScore(StateFrame frame, StateRow scores, IReadOnlyList<StateCell> scoreCells, long seat) =>
        (((seat >= 0) && (seat < scoreCells.Count) && frame.TryStoredAt(
            index: ((int)seat),
            row: scores,
            value: out var value
        ))
    ...
    private static void CopySeatScores(StateFrame source, StateFrame destination, StateRow scores, IReadOnlyList<StateCell> scoreCells) {
        for (var seat = 0; (seat < scoreCells.Count); seat++) {
            if (source.TryStoredAt(
                index: seat,
                row: scores,
                value: out var value
            )) {
                _ = destination.TryWrite(
                    row: scores,
                    key: scoreCells[seat].Key,
                    value: value,
                    write: StateWriteKind.Set,
                    reason: out _
                );
    ...
    private static void FoldSeatVector(Job job, int p, StateFrame source, StateRow scores, IReadOnlyList<StateCell> scoreCells, int token, int target) {
        ...
        if (value > CursorBest(
            ...
            if (p > 0) {
                CopySeatScores(
                    source: source,
                    destination: job.Levels[(p - 1)].Frame,
                    scores: scores,
                    scoreCells: scoreCells
                );
            }
    ...
    private StateFrame Position(Job job, int p) => ((p == 0)
        ? m_base!
        : job.Levels[(p - 1)].Frame
    );
