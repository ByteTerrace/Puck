...
public sealed partial class SearchRuntime {
        ...
        switch (job.Phase) {
            case UctSelect: {
                    ...
                    if ((job.PathLength - 1) >= plan.Depth) {
                        Backpropagate(
                            job: job,
                            value: EvaluateOutcome(
                                plan: plan,
                                position: job.UctFrame!,
                                tick: tick
                            )
                        );
                    ...
                    if (TryJudgeCandidate(
                        job: job,
                        from: job.UctFrame!,
                        shape: shapes[job.TreeShape![chosen]],
            ...
            case UctExpand: {
                    if (job.UShape >= shapes.Length) {
                        ...
                        if (TryJudgeCandidate(
                            job: job,
                            from: job.UctFrame!,
                            shape: shapes[job.TreeShape![draw]],
            ...
            default: {
                    ...
                    if (
                        ...
                        if (StateRows.FindStateRow(
                            ...
                            var outcome = SampleChanceOutcome(
                                chance: chance,
                                seed: ref job.Seed
                            );
    ...
    private static int SelectChild(Job job, int node) {
        ...
        var log2 = (63 - System.Numerics.BitOperations.LeadingZeroCount(value: ((ulong)parentVisits)));
        var best = -1;
