...
public readonly record struct ClosedBitset256(ulong Word0 = 0, ulong Word1 = 0, ulong Word2 = 0, ulong Word3 = 0) {
    ...
    public int Count => (((BitOperations.PopCount(value: Word0) + BitOperations.PopCount(value: Word1)) + BitOperations.PopCount(value: Word2)) + BitOperations.PopCount(value: Word3));
    ...
    public bool Contains(int index) => ((((uint)index) < 256) && (((Word(index: (index / 64)) >> (index % 64)) & 1) != 0));
    ...
    public override string ToString() => string.Create(
        ...
        $"{Word3:X16}{Word2:X16}{Word1:X16}{Word0:X16}"
    );
...
public sealed class ClosedBitset256JsonConverter : JsonConverter<ClosedBitset256> {
    ...
    public override ClosedBitset256 Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options) {
        if (
            ...
            throw new JsonException(message: "A 256-bit set must be a 64-digit hexadecimal string.");
        }
        return value;
    }
    /// <inheritdoc/>
    public override void Write(Utf8JsonWriter writer, ClosedBitset256 value, JsonSerializerOptions options) => writer.WriteStringValue(value: value.ToString());
}
