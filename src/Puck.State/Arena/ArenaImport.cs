...
public sealed partial class StateArena {
    ...
    private void LoadRow(int rowOrdinal, in ArenaRowLayout layout, StateRow row, in ArenaTime time) {
        ...
        foreach (var cell in cells) {
            ...
            var slot = (layout.CellStart + ((!m_catalog.IsPoolRow(rowOrdinal: rowOrdinal) && (layout.Shape is (RowShape.Slot or RowShape.Keyed or RowShape.Ordered)))
                ? members
                : position
            ));
    ...
    private int ImportPosition(in ArenaRowLayout layout, StateCell cell) => (layout.Shape switch {
        _ when m_catalog.IsPoolRow(rowOrdinal: layout.Ordinal) => ((m_keys.TryResolve(name: cell.Key, key: out var poolKey) && m_catalog.TryGetPoolSlot(keyOrdinal: poolKey.Ordinal, slot: out var poolSlot) && (((uint)poolSlot) < ((uint)layout.CellCapacity))) ? poolSlot : -1),
    ...
    private bool TryAdmitImport(StateRow? row, HashSet<string> pending, out int rowOrdinal, out long visibilityBytes, out string reason) {
        ...
        if ((m_poolMutationDepth == 0) && m_catalog.IsPoolRow(rowOrdinal: rowOrdinal)) {
            reason = $"row '{row.Name.Value}' is owned by a state pool";
            return false;
        }
        ...
        foreach (var cell in cells) {
            ...
            // Ring slots resolve through catalog symbols and never enter the retained key ledger.
            if ((layout.Shape != RowShape.Ring) && !TryAdmitKey(
                name: cell.Key,
                pending: pending,
                reason: out reason,
                row: row
