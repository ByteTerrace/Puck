...
public static class RuleEvaluation {
        ...
        foreach (var predicate in gate) {
            ...
            if (predicate.LeftExpression is { } leftExpression) {
                ...
                var holds = (rightOk && predicate.Comparison.Holds(
                    value: FixedQ4816.FromRawBits(value: leftValue),
                    valueIsForever: false,
                    expected: FixedQ4816.FromRawBits(value: rightValue),
                ...
                if (trace is not null) {
                    ...
                    trace.Add(item: $"{predicate.Describe}: {left} {DescribeComparison(comparison: predicate.Comparison)} {right} -> {(holds
                        ? "true"
                        : "false")}");
                }
                continue;
            ...
            stack[top++] = holdsHere;
            trace?.Add(item: $"{predicate.Describe}: {DescribeFact(
                value: value.Value,
                kind: value.Kind,
                isForever: value.IsForever,
                isAbsent: value.IsAbsent
            )} {DescribeComparison(comparison: predicate.Comparison)} {DescribeFact(
                value: expected.Value,
                kind: expected.Kind,
                isForever: expected.IsForever,
                isAbsent: expected.IsAbsent
            )} -> {(holdsHere
                ? "true"
                : "false")}");
        }
    ...
    public static string ResolveKey(IRuleReader reader, string? key, CompiledCellRef? keyFrom) {
        ...
        if (indirection.Binding == BoundKey.Token) {
            return (reader.BoundTokenKey ?? throw new InvalidOperationException(message: "a $token key was read outside a pattern value expression"));
        }
        ...
        if (
            (indirection.Binding == BoundKey.Each) &&
            (reader.BoundEachKey is { } eachKey)
        ) {
            return eachKey;
        }
    ...
    public static bool TryResolveIndex(IRuleReader reader, in CompiledCellRef reference, out long index) {
        ...
        index = IntegerOf(value: ReadFixed(
            reader: reader,
