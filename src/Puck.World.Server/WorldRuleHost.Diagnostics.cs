...
public sealed partial class WorldRuleHost {
    ...
    internal IReadOnlyList<RuleRuntimeDiagnostic> RuleRuntimeDiagnostics() => m_evaluator.Diagnostics();
    /// <summary>Formats the runtime refusal summary for <c>world.rule.failures</c>.</summary>
    internal string DescribeRuleRuntimeDiagnostics() {
        ...
        return $"[world.rule.failures: {string.Join(
            separator: " | ",
            values: diagnostics.Select(selector: static value => $"{value.Refusal} count={value.Count} lastTick={value.LastTick} rule='{value.Rule}' effect='{value.Effect}' ({value.Detail})")
        )}]";
    }
}
