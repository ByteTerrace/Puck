...
public sealed partial class WorldGrants {
    ...
    internal void RollbackTransferredEntity(int slot) {
        if (Host.Population.TryCaptureTransferredEntity(
            index: slot,
            peer: out var peer
        )) {
        ...
        _ = Host.Population.TryDetachSeatForTransfer(
            profile: out _,
            slot: slot
        );
    }
    ...
    /// <see cref="WorldServer.Definition"/>'s own <c>Admission</c> list: <c>WorldDefinition</c>'s sections are immutable
    ...
    internal bool TryAdmitPeerConnection(WorldAdmissionVerdict? verdict, IReadOnlyList<WorldAdmissionEntry>? expectedAdmissionEntries, out WorldPeerEventEntry admitted, out string refusal) {
        if (!ReferenceEquals(
            objA: Host.Definition.Admission,
    ...
    private bool TryAdmitVerifiedParticipant(WorldAdmissionVerdict? verdict, int? reservedSlot, IntentSource source, bool authorityTransferred, out WorldPeerEventEntry admitted, out string refusal) {
        ...
        if (
            ...
            Host.Population.TryResumeParkedPeer(
        ...
        var admittedOk = ((reservedSlot is { } slot)
            ? Host.Population.TryAdmitRemotePeerAt(
                slot: slot,
                source: source,
    ...
    internal static IReadOnlyList<WorldAdmissionGrant> AdmissionTemplatesFor(WorldPeerEventEntry peer, IReadOnlyList<WorldGrant> mintedGrants) {
    ...
    internal void ApplyLifecycleEvents(IReadOnlyList<WorldPeerEventEntry> admitted, IReadOnlyList<WorldPeerEventEntry> disconnected, bool ordered, IReadOnlyList<WorldGrant>? mintedGrants = null) {
        if (disconnected.Count > 0) {
            var revoked = new List<WorldGrant>();

            foreach (var peer in disconnected) {
                revoked.AddRange(collection: Rows(principal: peer.Identity));
            }
    ...
    internal void RemintPeerAdmissionGrants(WorldDefinition candidate, IReadOnlyDictionary<int, IReadOnlyList<WorldGrant>> preRebuildPeerRows) {
        foreach (var (index, priorRows) in preRebuildPeerRows) {
            var principal = Host.Population.PeerPrincipal(index: index);
            var baselineTemplates = Host.Population.PeerAdmissionInstalledGrantTemplates(bodyIndex: index);

            var (domain, subject) = Host.Population.PeerIdentity(bodyIndex: index);
            ...
            var revokedKeys = new HashSet<(WorldCapability Capability, GrantSubject Subject)>(collection: Host.Population.PeerAdmissionRevokedKeys(bodyIndex: index));
            ...
            Host.Population.SetPeerAdmissionRevokedKeys(
                bodyIndex: index,
            ...
            var stillTrusted = (Host.Population.PeerAuthorityTransferred(bodyIndex: index)
    ...
    private void StageOwnedState(int slot, WorldIdentity? profile) {
        if (
            (profile is null) ||
            (Host.Body(index: slot) is not { } body)
        ) {
            return;
        }
        ...
        foreach (var declaration in declarations) {
            if (Host.Profiles.TryReadDurableState(
                ownerId: profile.Id,
                sourceDocumentId: (Host.Definition.DocumentId ?? string.Empty),
    ...
    internal SessionReply ApplySession(SessionRequest request) {
        ...
        switch (request) {
            case SessionRequest.Join join: {
                    ...
                    if (((uint)join.Slot) >= Host.Population.LocalSeatCount) {
                        return new SessionReply(
                            Accepted: false,
                            AssignedIndex: -1,
                            RosterEcho: string.Empty,
                    ...
                    if (Host.Population.IsSeatParked(slot: join.Slot)) {
                        if (!Host.Population.TryResumeParkedSeat(
                            slot: join.Slot,
                            profile: profile,
                            mismatch: out _
                        )) {
            ...
            case SessionRequest.Leave leave:
                if (((uint)leave.Slot) >= Host.Population.LocalSeatCount) {
                    return new SessionReply(
                        Accepted: false,
                        AssignedIndex: -1,
                        RosterEcho: string.Empty,
                ...
                Host.Population.DeactivateSeat(
                    slot: leave.Slot,
                    tick: Host.NextInputTick
                );
            ...
            case SessionRequest.SetIdentity setProfile: {
                    if (
                        (((uint)setProfile.Slot) >= Host.Population.LocalSeatCount) ||
                        (Host.Profiles.Find(name: setProfile.IdentityName) is not { } profile)
                    ) {
                        return new SessionReply(
                            Accepted: false,
                            AssignedIndex: -1,
            ...
            case SessionRequest.SetPopulation setPopulation: {
                    ...
                    var applied = Host.Population.SetSimulatedCount(
            ...
            case SessionRequest.SetPeerSource setPeerSource:
                ...
                if (!Host.Population.TrySetPeerSource(
            ...
            case SessionRequest.RememberPreferredController rememberPreferred:
                if (Host.Profiles.Find(name: rememberPreferred.IdentityName) is not { } rememberedIdentity) {
                    return new SessionReply(
                        Accepted: false,
                        AssignedIndex: -1,
                        RosterEcho: string.Empty,
                ...
                Host.Profiles.RememberPreferredController(
    ...
    /// <summary>Applies a live session lever — the same shape as <see cref="WorldServer.ApplyComposition"/> one section over:
    ...
    internal void ApplySessionLever(WorldSessionLever lever, Principal principal, int connectionId = SubmissionEnvelope.LocalConnectionId, long correlationId = 0) {
        ...
        Host.Output.DeliverSessionLever(lever: lever);
    }
}
