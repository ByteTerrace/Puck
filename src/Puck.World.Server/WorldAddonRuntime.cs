...
/// </remarks>
public sealed class WorldAddonRuntime : IDisposable {
        ...
        foreach (var row in definition.Addons) {
            ...
            if (m_host is null) {
                ...
                m_host = new AddonHost(
                    ...
                    engine: engine,
                    loader: new WasmModuleLoader(engine: engine, assetSource: new FileSystemAssetSource())
                );
            }
            ...
            if (!m_host.TryGet(instance: out var instance, name: row.Name)) {
            ...
            ReportCapabilityDisclosure(name: row.Name, requests: row.Requests, grants: server.Grants);
            ...
            m_mounted.Add(item: new MountedAddon(instance: instance, requests: row.Requests, populationCapacity: m_server.Population.Capacity, memoryWatches: row.MemoryWatches));
            ...
            m_receipts.Add(item: new WorldAddonReceipt(Name: instance.Name, Hash: instance.Hash.ToString(), Fuel: (ulong)instance.FuelPerTick));
            ...
            ReportInertChannelDeclarations(bindings: instance.ChannelBindings, name: row.Name);
        }
    }
    ...
    /// <see cref="AddonVerdict.Rejected"/>.</param>
    public void CompleteMutation(int addonIndex, ushort actOrdinal, AddonVerdict verdict) {
    ...
    public static WorldAddonRuntime Create(WorldDefinition definition, WorldServer server) {
        ...
        var runtime = new WorldAddonRuntime(definition: definition, server: server);
    ...
    public IReadOnlyList<AddonCostReport> DescribeCost() {
        ...
        for (var index = 0; (index < m_mounted.Count); ++index) {
            ...
            report[index] = new AddonCostReport(
                ...
                FaultDetail: ((instance.Fault.Kind == AddonFaultKind.None) ? null : instance.Fault.Detail)
            );
        }
    ...
    public string Reload(string name) {
        ...
        var mountedIndex = m_mounted.FindIndex(match: candidate => string.Equals(a: candidate.Instance.Name, b: name, comparisonType: StringComparison.Ordinal));
        ...
        if (!m_host.TryGet(name: name, instance: out var fresh)) {
        ...
        if (ReferenceEquals(objA: fresh, objB: previous.Instance)) {
        ...
        if (fresh.State == AddonState.Enabled) {
            ...
            ReportCapabilityDisclosure(name: name, requests: previous.Requests, grants: m_server.Grants);
            fresh.Admit();

            if (fresh.State == AddonState.Enabled) {
                outcome = ", re-admitted";

                var receiptIndex = m_receipts.FindIndex(match: receipt => string.Equals(a: receipt.Name, b: name, comparisonType: StringComparison.Ordinal));
                ...
                if (receiptIndex >= 0) {
                    ...
                    m_receipts[receiptIndex] = new WorldAddonReceipt(Name: fresh.Name, Hash: fresh.Hash.ToString(), Fuel: (ulong)fresh.FuelPerTick);
                }
            } else {
    ...
    public string SetEnabled(string name, bool enabled) {
        ...
        if (m_host is null) {
            return $"'{name}' — this world enables no addon; there is no host to {(enabled ? "enable" : "disable")}";
        }

        var mountedIndex = m_mounted.FindIndex(match: candidate => string.Equals(a: candidate.Instance.Name, b: name, comparisonType: StringComparison.Ordinal));

        if (mountedIndex < 0) {
            return $"'{name}' is not a mounted guest — {(enabled ? "enable" : "disable")} has nothing to act on (a load-faulted row never joined the tick set)";
        }
        ...
        if (!m_host.SetEnabled(name: name, enabled: enabled)) {
        ...
        ReportCapabilityDisclosure(name: name, requests: addon.Requests, grants: m_server.Grants);
        instance.Admit();
    ...
    public string Mount(string name, string modulePath, string hash, ulong fuel, IReadOnlyList<WorldCapabilityRequest>? requests) {
        ...
        if (m_mounted.Exists(match: candidate => string.Equals(a: candidate.Instance.Name, b: name, comparisonType: StringComparison.Ordinal))) {
        ...
        var descriptor = new AddonDescriptor(
            ...
            ModuleHash: (string.IsNullOrEmpty(value: hash) ? null : hash),
            FuelPerTick: ((fuel == 0UL) ? null : (long)fuel),
            Enabled: true
        );
        ...
        if ((requests is { Count: > 0 }) && (ResolveResponseChannel(instance: instance) < 0)) {
            instance.Disable();

            return $"'{name}' refused — requests {requests.Count} capabilit{((requests.Count == 1) ? "y" : "ies")} but declares no Response channel, so no verdict or disclosure could ever reach it and no requested handle could ever be learned; not mounted";
        }

        ReportCapabilityDisclosure(name: name, requests: requests, grants: m_server.Grants);
        instance.Admit();
        ...
        m_mounted.Add(item: new MountedAddon(instance: instance, requests: requests, populationCapacity: m_server.Population.Capacity));
        m_receipts.Add(item: new WorldAddonReceipt(Name: instance.Name, Hash: instance.Hash.ToString(), Fuel: (ulong)instance.FuelPerTick));
        ReportInertChannelDeclarations(bindings: instance.ChannelBindings, name: name);
        ...
        for (var index = 0; (index < m_mounted.Count); ++index) {
            ...
            if (!instance.Admitted) {
                ...
                if (!addon.DiscrepancyReported) {
                    ReportDiscrepancy(addon: addon, detail: "instance is enabled but was never re-admitted after a reload/enable — skipped every tick (a caller bypassed WorldAddonRuntime.Reload/SetEnabled, which re-admit)");
                }
            ...
            ResolveMutations(addon: addon, addonIndex: index, tick: tick);
        }
    }
    ...
    public void ApplyContributions(ulong tick) {
        for (var index = 0; (index < m_mounted.Count); ++index) {
            ...
            FoldActs(addon: addon, tick: tick);
        }
    }
    ...
    public void ResolveReads(ulong tick) {
        for (var index = 0; (index < m_mounted.Count); ++index) {
            ...
            StageBatch(addon: addon, tick: tick);
        }
    }
    ...
    private void FoldActs(MountedAddon addon, ulong tick) {
        ...
        var handles = m_server.Grants.HandleTable(principal: principal, capability: WorldCapability.Drive);
        ...
        try {
            for (var index = 0; (index < acts.Length); ++index) {
                ...
                if (!act.Resolved) {
                    QueueAnswer(addon: addon, ordinal: act.Ordinal, verdict: AddonVerdict.AttenuatedToEmpty);
                ...
                if (!handles.TryResolve(handle: new WorldHandle(Index: act.HandleIndex, Generation: act.HandleGeneration, TablePrincipal: principal, TableCapability: WorldCapability.Drive), subject: out var subject) ||
                    (subject.Kind != GrantSubjectKind.Body)) {
                    QueueAnswer(addon: addon, ordinal: act.Ordinal, verdict: AddonVerdict.StaleHandle);
                ...
                if (!IsRequested(addon: addon, capability: WorldCapability.Drive, subject: subject)) {
                    QueueAnswer(addon: addon, ordinal: act.Ordinal, verdict: AddonVerdict.AttenuatedToEmpty);
                    ReportUnrequestedAct(addon: addon, subject: subject, via: "drove");
                ...
                if (m_server.Grants.TryGetBudget(principal: principal, capability: WorldCapability.Drive, subject: subject, out var driveBudget)) {
                    ...
                    QueueAnswer(addon: addon, ordinal: act.Ordinal, verdict: AddonVerdict.NoHold);
                ...
                var slot = Contribution(addon: addon, bodyIndex: bodyIndex);

                addon.ActBody[index] = bodyIndex;
                Fold(addon: addon, slot: slot, act: in act);
            }
            ...
            for (var slot = 0; (slot < addon.ContributionCount); ++slot) {
                Submit(addon: addon, slot: slot, tick: tick);
            }
            ...
            for (var index = 0; (index < acts.Length); ++index) {
                ...
                if (outcome != AddonVerdict.None) {
                    QueueAnswer(addon: addon, ordinal: acts[index].Ordinal, verdict: outcome);
                }
            }
        } finally {
    ...
    private static int Contribution(MountedAddon addon, int bodyIndex) {
        ...
        while ((slot < count) && (contributions[slot].BodyIndex < bodyIndex)) {
            ++slot;
        }

        if ((slot < count) && (contributions[slot].BodyIndex == bodyIndex)) {
            return slot;
        }
    ...
    private void Submit(MountedAddon addon, int slot, ulong tick) {
        ...
        var submission = new IntentSubmission(
            ...
            Intent: RoleChannels(values: contribution.Values, channels: m_server.Population.Channels),
            ...
            HeldChannels: CompositionChannels(values: contribution.Values, channels: m_server.Population.Channels)
        );
        var verdict = m_server.ApplyIntentSubmission(body: body, submission: in submission);
    ...
    private bool ContributionAccepted(int bodyIndex, WorldPrincipal principal, in ChannelValues values) {
        ...
        for (var ordinal = 0; (ordinal < ChannelLimits.MaxChannels); ordinal++) {
            if ((values[ordinal].Value != 0L) && reach.Contains(ordinal: ordinal)) {
                return true;
            }
        }
    ...
    private void StageBatch(MountedAddon addon, ulong tick) {
        ...
        EmitDisclosures(addon: addon, budget: (budget - addon.ReservedCount));
        ...
        EmitEvents(addon: addon, budget: budget);
        ResolveAsks(addon: addon);
        ResolveQueries(addon: addon);
        MergeAnswers(addon: addon, budget: budget);
        addon.AnswerCount = 0;
    }
    ...
    private void EmitDisclosures(MountedAddon addon, int budget) {
        ...
        if (sameGeneration && ((addon.DisclosedRevision == grants.Revision) || (addon.OverflowedAtRevision == grants.Revision))) {
            return;
        }

        var drive = grants.ProjectSubjects(principal: addon.Principal, capability: WorldCapability.Drive);
        var observe = grants.ProjectSubjects(principal: addon.Principal, capability: WorldCapability.Observe);

        if (sameGeneration && addon.Disclosed && Same(a: addon.DisclosedDrive, b: drive) && Same(a: addon.DisclosedObserve, b: observe)) {
            addon.DisclosedRevision = grants.Revision;

            return;
        }

        var count = (CountMaterialized(addon: addon, capability: WorldCapability.Drive, subjects: drive) +
            CountMaterialized(addon: addon, capability: WorldCapability.Observe, subjects: observe));
        ...
        EmitDisclosureSet(addon: addon, capability: WorldCapability.Drive, subjects: drive);
        EmitDisclosureSet(addon: addon, capability: WorldCapability.Observe, subjects: observe);
    ...
    private void EmitDisclosureSet(MountedAddon addon, WorldCapability capability, GrantSubject[] subjects) {
        var handles = m_server.Grants.HandleTable(principal: addon.Principal, capability: capability);

        for (var index = 0; (index < subjects.Length); ++index) {
            if (!IsMaterialized(addon: addon, capability: capability, subject: subjects[index]) || !handles.TryMint(index: index, handle: out var handle)) {
                continue;
            }
            ...
            if (!TryPack(addon: addon, handle: handle, index: out var wireIndex, generation: out var wireGeneration)) {
                continue;
            }
    ...
    private void EmitEvents(MountedAddon addon, int budget) {
        ...
        for (var index = 0; (index < edges.Count); ++index) {
            ...
            var gateStatus = SelectEventGate(addon: addon, gateA: edge.GateA, gateB: edge.GateB, chargedSubject: out var chargedSubject);

            if (gateStatus == EventGateStatus.None) {
                continue;
            }

            if ((gateStatus == EventGateStatus.Exhausted) || ((budget - addon.PendingCount) <= 0)) {
                ++dropped;
            ...
            addon.EventCellsDelivered = ((addon.EventCellsDelivered == ulong.MaxValue) ? ulong.MaxValue : (addon.EventCellsDelivered + 1UL));

            if (edge.Family is WorldEventFamily.CollisionBegin or WorldEventFamily.CollisionEnd) {
                addon.CollisionEventsDelivered = ((addon.CollisionEventsDelivered == ulong.MaxValue) ? ulong.MaxValue : (addon.CollisionEventsDelivered + 1UL));
            } else if (edge.Family is WorldEventFamily.RouteEngaged or WorldEventFamily.RouteDisengaged) {
                addon.RouteEventsDelivered = ((addon.RouteEventsDelivered == ulong.MaxValue) ? ulong.MaxValue : (addon.RouteEventsDelivered + 1UL));
            }
        }

        dropped += EmitMemoryWatchEvents(addon: addon, budget: budget);

        if (dropped > 0) {
            addon.EventGapCount = ((addon.EventGapCount > (ulong.MaxValue - (ulong)dropped)) ? ulong.MaxValue : (addon.EventGapCount + (ulong)dropped));
        }

        if ((addon.EventGapCount != addon.LastReportedEventGap) && ((budget - addon.PendingCount) > 0)) {
            addon.Pending[addon.PendingCount++] = new AddonInCell(
                Kind: AddonInCellKind.Observation,
                Channel: (byte)addon.ResponseChannel,
                Ordinal: 0,
    ...
    private int EmitMemoryWatchEvents(MountedAddon addon, int budget) {
        ...
        for (var index = 0; (index < watches.Count); ++index) {
            ...
            var gateStatus = SelectEventGate(addon: addon, gateA: subject, gateB: null, chargedSubject: out var chargedSubject);

            if (gateStatus == EventGateStatus.None) {
                continue;
            }

            if (!TryReadWatch(peek: m_server.Machines, watch: watch, value: out var value)) {
                continue;
            }
            ...
            if (!state.Initialized) {
                ...
                state = new MountedAddon.WatchState(Initialized: true, Value: value);
    ...
    private static bool TryReadWatch(IWorldMachineMemoryPeek peek, WorldAddonMemoryWatch watch, out long value) {
        ...
        for (var offset = 0; (offset < watch.Length); ++offset) {
            if (!peek.TryPeek(screen: watch.Screen, address: (watch.Address + offset), value: out var b)) {
                return false;
            }
    ...
    private EventGateStatus SelectEventGate(MountedAddon addon, GrantSubject gateA, GrantSubject? gateB, out GrantSubject chargedSubject) {
        var statusA = EventGate(addon: addon, subject: gateA);
        ...
        var statusB = (gateB is { } subjectB ? EventGate(addon: addon, subject: subjectB) : EventGateStatus.None);
    ...
    private EventGateStatus EventGate(MountedAddon addon, GrantSubject subject) {
        if (!IsRequested(addon: addon, capability: WorldCapability.Observe, subject: subject)
            || !m_server.Grants.Allows(principal: addon.Principal, capability: WorldCapability.Observe, subject: subject).IsAllowed
            || !m_server.Grants.TryGetEventBudget(principal: addon.Principal, capability: WorldCapability.Observe, subject: subject, out var eventBudget)) {
            return EventGateStatus.None;
        }

        return (addon.EventCounts.GetValueOrDefault(key: subject) < eventBudget ? EventGateStatus.Available : EventGateStatus.Exhausted);
    }
    ...
    private void ResolveAsks(MountedAddon addon) {
        ...
        for (var index = 0; (index < asks.Length); ++index) {
            ...
            if (!WorldAddonWire.TryCapability(mask: ask.CapabilityMask, capability: out var capability)) {
                ...
                if (!addon.DiscrepancyReported) {
                    ReportDiscrepancy(addon: addon, detail: $"ask ordinal {ask.Ordinal} carries capability mask 0x{ask.CapabilityMask:x}, which maps to no engine capability");
                }

                QueueAnswer(addon: addon, ordinal: ask.Ordinal, verdict: AddonVerdict.NoHold);
            ...
            if (ask.SubjectKind == AddonSubjectKind.Body) {
                ...
                subject = (inRange ? GrantSubject.Body(index: (int)ask.SubjectIndex) : GrantSubject.All);
            } else if (ask.SubjectKind == AddonSubjectKind.Section) {
                if (!TryResolveSectionAskName(addon: addon, ask: in ask, name: out var sectionName, refusal: out var copyRefusal, error: out var copyError)) {
                    Console.Error.WriteLine(value: $"[world.addon: {addon.Instance.Name} ask ordinal {ask.Ordinal} refused {copyRefusal} — {copyError}]");
                    QueueAnswer(addon: addon, ordinal: ask.Ordinal, verdict: copyRefusal!.Value);
                ...
                if (!GrantSubject.TryParseSectionName(name: sectionName, section: out var resolvedSection)) {
                    ...
                    QueueAnswer(addon: addon, ordinal: ask.Ordinal, verdict: AddonVerdict.NoSuchSubject);
            ...
            if ((ask.SubjectKind == AddonSubjectKind.Body) && (m_server.Body(index: (int)ask.SubjectIndex) is null)) {
                QueueAnswer(addon: addon, ordinal: ask.Ordinal, verdict: AddonVerdict.NoSuchSubject);

                continue;
            }

            var verdict = grants.Allows(principal: addon.Principal, capability: capability, subject: subject);

            if (!verdict.IsAllowed) {
                QueueAnswer(addon: addon, ordinal: ask.Ordinal, verdict: WorldAddonWire.FromRule(rule: verdict.Rule));
            ...
            if (!grants.HandleTable(principal: addon.Principal, capability: capability).TryMintFor(subject: subject, handle: out var handle)) {
                ...
                if (!addon.DiscrepancyReported) {
                    ReportDiscrepancy(addon: addon, detail: $"holds {capability.ToString().ToLowerInvariant()} over {subject.Describe()} by {verdict.Describe()} but no handle slot projects it — no handle was minted");
                }
            ...
            QueueAnswer(addon: addon, ordinal: ask.Ordinal, verdict: WorldAddonWire.FromRule(rule: verdict.Rule), handleIndex: wireIndex, handleGeneration: wireGeneration);
        }
    }
    ...
    private static bool TryResolveSectionAskName(MountedAddon addon, in AddonAskSubmission ask, out string name, out AddonVerdict? refusal, out string error) {
        ...
        if (!addon.Instance.TryCopyMemory(pointer: ask.SubjectIndex, length: length, destination: buffer, error: out var copyError)) {
            name = "";
    ...
    private static AddonVerdict ToWireVerdict(in WorldMutationAdmission admission) => admission.Rule switch {
        ...
        _ => throw new ArgumentOutOfRangeException(paramName: nameof(admission), actualValue: admission.Rule, message: "unmapped mutation-admission rule at the addon pre-flight — extend the mapping deliberately, never default it"),
    };
    ...
    private void ResolveMutations(MountedAddon addon, int addonIndex, ulong tick) {
        ...
        var grants = m_server.Grants;
        var handles = grants.HandleTable(principal: addon.Principal, capability: WorldCapability.Mutate);
        ...
        try {
            for (var index = 0; (index < queries.Length); ++index) {
                ...
                addon.ReservedAnswers[slot] = new AddonInCell(
                    ...
                    Channel: (byte)((addon.ResponseChannel < 0) ? 0 : addon.ResponseChannel),
                ...
                if (!handles.TryResolve(handle: new WorldHandle(Index: query.HandleIndex, Generation: query.HandleGeneration, TablePrincipal: addon.Principal, TableCapability: WorldCapability.Mutate), subject: out var subject) ||
                    (subject.Kind != GrantSubjectKind.Section)) {
                    SetReservedVerdict(addon: addon, ordinal: query.Ordinal, verdict: AddonMutateRefusals.ToVerdict(reason: AddonMutateRefusal.StaleHandle));
                    ReportStaleHandle(addon: addon);
                ...
                if (!IsRequested(addon: addon, capability: WorldCapability.Mutate, subject: subject)) {
                    SetReservedVerdict(addon: addon, ordinal: query.Ordinal, verdict: AddonMutateRefusals.ToVerdict(reason: AddonMutateRefusal.NotRequested));
                    ReportUnrequestedAct(addon: addon, subject: subject, via: "mutated");
                ...
                if (!m_server.TryAdmitMutation(principal: addon.Principal, section: section, kindOrdinal: kindOrdinal, rowScopedEditSubject: null, meter: true, admission: out var admission)) {
                    SetReservedVerdict(addon: addon, ordinal: query.Ordinal, verdict: ToWireVerdict(admission: in admission));
                    ...
                    } else if ((admission.Rule == WorldMutationAdmissionRule.MissingBudget) && !addon.MutateMissingBudgetReported) {
                ...
                if (lengthUnsigned > (ulong)AddonAbi.MaxMutationPayloadBytes) {
                    SetReservedVerdict(addon: addon, ordinal: query.Ordinal, verdict: AddonMutateRefusals.ToVerdict(reason: AddonMutateRefusal.PayloadTooLarge));
                ...
                if ((addon.MutateBytesThisTick + length) > AddonAbi.MaxMutationBytesPerTickPerAddon) {
                    SetReservedVerdict(addon: addon, ordinal: query.Ordinal, verdict: AddonMutateRefusals.ToVerdict(reason: AddonMutateRefusal.AddonByteBudgetExhausted));
                ...
                if ((m_mutationBytesThisTickAllAddons + length) > AddonAbi.MaxMutationBytesPerTickAllAddons) {
                    SetReservedVerdict(addon: addon, ordinal: query.Ordinal, verdict: AddonMutateRefusals.ToVerdict(reason: AddonMutateRefusal.GlobalByteBudgetExhausted));
                ...
                if (!addon.Instance.TryCopyMemory(pointer: query.B, length: length, destination: addon.MutationPayloadBuffer.AsSpan(start: 0, length: length), error: out var copyError)) {
                    SetReservedVerdict(addon: addon, ordinal: query.Ordinal, verdict: AddonMutateRefusals.ToVerdict(reason: AddonMutateRefusal.PointerOutOfBounds));
                ...
                var payload = addon.MutationPayloadBuffer.AsMemory(start: 0, length: length);
                ...
                if (!WorldAddonMutationDecoder.TryDecode(kindOrdinal: kindOrdinal, section: section, payload: payload, principal: addon.Principal, mutation: out var mutation, error: out var decodeError) || (mutation is null)) {
                    SetReservedVerdict(addon: addon, ordinal: query.Ordinal, verdict: AddonMutateRefusals.ToVerdict(reason: AddonMutateRefusal.DecodeFailed));
                ...
                m_server.EnqueueMutation(mutation: mutation, connectionId: SubmissionEnvelope.LocalConnectionId, correlationId: 0, sourceAddonIndex: addonIndex, actOrdinal: query.Ordinal);
            }
        } finally {
    ...
    private void ResolveQueries(MountedAddon addon) {
        var queries = addon.Pump.Queries;
        var grants = m_server.Grants;
        var handles = grants.HandleTable(principal: addon.Principal, capability: WorldCapability.Observe);
        var driveHandles = grants.HandleTable(principal: addon.Principal, capability: WorldCapability.Drive);
        ...
        try {
            for (var index = 0; (index < queries.Length); ++index) {
                ...
                if (query.Verb == AddonAbi.RequestVerbs.Designate) {
                    var sourceHandle = new WorldHandle(Index: query.HandleIndex, Generation: query.HandleGeneration, TablePrincipal: addon.Principal, TableCapability: WorldCapability.Drive);
                    if (!driveHandles.TryResolve(handle: sourceHandle, subject: out var sourceSubject) || (sourceSubject.Kind != GrantSubjectKind.Body)) {
                        QueueAnswer(addon: addon, ordinal: query.Ordinal, verdict: AddonVerdict.StaleHandle);
                        continue;
                    }
                    if (!IsRequested(addon: addon, capability: WorldCapability.Drive, subject: sourceSubject)) {
                        QueueAnswer(addon: addon, ordinal: query.Ordinal, verdict: AddonVerdict.AttenuatedToEmpty);
                        ReportUnrequestedAct(addon: addon, subject: sourceSubject, via: "designated");
                        continue;
                    }

                    var targetSubject = GrantSubject.Body(index: (int)query.A);
                    if (!IsRequested(addon: addon, capability: WorldCapability.Observe, subject: targetSubject)) {
                        QueueAnswer(addon: addon, ordinal: query.Ordinal, verdict: AddonVerdict.AttenuatedToEmpty);
                        ReportUnrequestedAct(addon: addon, subject: targetSubject, via: "designated");
                        continue;
                    }

                    var registerIndex = (int)query.B;
                    if ((uint)registerIndex >= (uint)m_server.Population.TargetRegisters.Count) {
                        QueueAnswer(addon: addon, ordinal: query.Ordinal, verdict: AddonVerdict.Rejected);
                        continue;
                    }
                    ...
                    QueueAnswer(addon: addon, ordinal: query.Ordinal, verdict: applied ? AddonVerdict.Applied : AddonVerdict.Rejected);
                    continue;
                }

                if (!handles.TryResolve(handle: new WorldHandle(Index: query.HandleIndex, Generation: query.HandleGeneration, TablePrincipal: addon.Principal, TableCapability: WorldCapability.Observe), subject: out var subject) ||
                    (subject.Kind != GrantSubjectKind.Body)) {
                    QueueAnswer(addon: addon, ordinal: query.Ordinal, verdict: AddonVerdict.StaleHandle);
                ...
                if (!IsRequested(addon: addon, capability: WorldCapability.Observe, subject: subject)) {
                    QueueAnswer(addon: addon, ordinal: query.Ordinal, verdict: AddonVerdict.AttenuatedToEmpty);
                    ReportUnrequestedAct(addon: addon, subject: subject, via: "read");
                ...
                var verdict = grants.Allows(principal: addon.Principal, capability: WorldCapability.Observe, subject: subject);

                if (!verdict.IsAllowed) {
                    QueueAnswer(addon: addon, ordinal: query.Ordinal, verdict: WorldAddonWire.FromRule(rule: verdict.Rule));
                ...
                if (grants.TryGetBudget(principal: addon.Principal, capability: WorldCapability.Observe, subject: subject, out var budget)) {
                    ...
                    QueueAnswer(addon: addon, ordinal: query.Ordinal, verdict: AddonVerdict.NoHold);
                ...
                if (m_server.Body(index: subject.Value) is not { } body) {
                    QueueAnswer(addon: addon, ordinal: query.Ordinal, verdict: AddonVerdict.NoSuchSubject);
                ...
                if (query.Verb != AddonAbi.RequestVerbs.BodyPose) {
                    ...
                    if (!addon.DiscrepancyReported) {
                        ReportDiscrepancy(addon: addon, detail: $"request verb {query.Verb} at ordinal {query.Ordinal} is not served by this host — no answer was produced");
                    }
                ...
                QueuePart(addon: addon, ordinal: query.Ordinal, verdict: allowed, part: 0, a: position.X.Value, b: position.Y.Value);
                QueuePart(addon: addon, ordinal: query.Ordinal, verdict: allowed, part: 1, a: position.Z.Value, b: 0L);
                QueuePart(addon: addon, ordinal: query.Ordinal, verdict: allowed, part: 2, a: orientation.X.Value, b: orientation.Y.Value);
                QueuePart(addon: addon, ordinal: query.Ordinal, verdict: allowed, part: 3, a: orientation.Z.Value, b: orientation.W.Value);
            }
        } finally {
    ...
    private static void MergeAnswers(MountedAddon addon, int budget) {
        ...
        try {
            while (index < addon.AnswerCount) {
                ...
                while ((end < addon.AnswerCount) && (addon.Answers[end].Ordinal == ordinal)) {
                    ++end;
                }
    ...
    private static void SortAnswers(AddonInCell[] answers, int count) {
        for (var index = 1; (index < count); ++index) {
            ...
            while ((slot >= 0) && ((answers[slot].Ordinal > candidate.Ordinal) || ((answers[slot].Ordinal == candidate.Ordinal) && (answers[slot].Verb > candidate.Verb)))) {
    ...
    private static bool TryPack(MountedAddon addon, WorldHandle handle, out ushort index, out ushort generation) {
        if (((uint)handle.Index > ushort.MaxValue) || ((uint)handle.Generation > ushort.MaxValue)) {
            index = 0;
            generation = 0;

            if (!addon.DiscrepancyReported) {
                ReportDiscrepancy(addon: addon, detail: $"handle (index {handle.Index}, generation {handle.Generation}) exceeds the wire's 16-bit lanes — it cannot be handed across the ABI without aliasing another handle, so it was withheld");
            }
    ...
    private static bool IsMaterialized(MountedAddon addon, WorldCapability capability, GrantSubject subject) =>
        ((subject.Kind == GrantSubjectKind.Body) && IsRequested(addon: addon, capability: capability, subject: subject));

    private static int CountMaterialized(MountedAddon addon, WorldCapability capability, GrantSubject[] subjects) {
        var count = 0;

        for (var index = 0; (index < subjects.Length); ++index) {
            if (IsMaterialized(addon: addon, capability: capability, subject: subjects[index])) {
                ++count;
            }
        }
    ...
    private static void ReportFault(MountedAddon addon) {
        ...
        if (addon.FaultReported || (addon.Instance.Fault.Kind == AddonFaultKind.None)) {
            return;
        }
    ...
    private static void ReportInertChannelDeclarations(ReadOnlySpan<AddonChannelBinding> bindings, string name) {
        ...
        if (inert.Count > 0) {
            Console.Error.WriteLine(value: $"[world.addon: {name} declares {inert.Count} channel name(s) the host table does not recognize — inert, never faults the mount: {string.Join(separator: ", ", values: inert)}]");
        }
    }
    ...
    public string? DescribeUndeclaredGrantedChannels(WorldPrincipal principal, ChannelReachMask? reach, WorldChannelTable channels) {
        ...
        var index = m_mounted.FindIndex(match: candidate => candidate.Principal.Equals(principal));
    ...
    private static void ReportCapabilityDisclosure(string name, IReadOnlyList<WorldCapabilityRequest>? requests, IWorldGrantsView grants) {
        ...
        if (((requests is null) || (requests.Count == 0)) && (held.Count == 0)) {
            return;
        }
        ...
        if (requests is { Count: > 0 }) {
            foreach (var request in requests) {
                ...
                (heldLabels.Contains(item: label) ? granted : withheld).Add(item: label);
            }
        }
        ...
        Console.Error.WriteLine(value: $"[world.addon: {name} requested {requestCount} capabilit{((requestCount == 1) ? "y" : "ies")} — " +
            $"granted: {((granted.Count > 0) ? string.Join(separator: ", ", values: granted) : "(none)")}; " +
            $"withheld: {((withheld.Count > 0) ? string.Join(separator: ", ", values: withheld) : "(none)")}; " +
            $"holds beyond its manifest (inert — never materialized): {((unrequested.Count > 0) ? string.Join(separator: ", ", values: unrequested) : "(none)")}]");
    }
    ...
    private sealed class MountedAddon {
        public MountedAddon(AddonInstance instance, IReadOnlyList<WorldCapabilityRequest>? requests, int populationCapacity, IReadOnlyList<WorldAddonMemoryWatch>? memoryWatches = null) {
            ...
            MemoryWatchState = (((memoryWatches is { Count: > 0 }) ? new WatchState[memoryWatches.Count] : null));
        }
