...
public readonly record struct WorldAuthorityFence(long Epoch, Guid Token, string RootVersion) {
    ...
    public static WorldAuthorityFence Unowned { get; } = new(
        Epoch: 0,
        RootVersion: string.Empty,
        Token: Guid.Empty
    );
}
...
internal static class WorldAuthorityRootCodec {
    ...
    public static bool TryDecode(ReadOnlySpan<byte> bytes, out WorldAuthorityRoot root, out string reason) {
        try {
            ...
            root = JsonSerializer.Deserialize<WorldAuthorityRoot>(
                options: Options,
                utf8Json: bytes
            );
            if (
            ...
            if (
                (root.Epoch == 0) &&
                (root.FenceToken != Guid.Empty)
            ) { reason = "epoch zero carries a fence"; root = default; return false; }
            ...
            if (
                !IsContentPin(value: root.DefinitionHash) ||
                !IsContentPin(value: root.CheckpointHash) ||
                !IsContentPin(value: root.JournalHash) ||
                !IsContentPin(value: root.ReceiptHash) ||
                !IsContentPin(value: root.ReceiptIndexHash)
            ) { reason = "root content reference is not a canonical sha256-64 pin"; root = default; return false; }
            reason = string.Empty;
            return true;
    ...
    public static bool TryDecodeReceipt(ReadOnlySpan<byte> bytes, out WorldAuthorityOperationReceipt receipt, out string? previousHash, out string reason) {
        try {
            var node = JsonSerializer.Deserialize<ReceiptNode>(
                options: Options,
                utf8Json: bytes
            );
            ...
            if (
                (node is null) ||
                (node.OperationId == Guid.Empty) ||
                string.IsNullOrWhiteSpace(value: node.Actor) ||
                string.IsNullOrWhiteSpace(value: node.PayloadDigest) ||
                string.IsNullOrWhiteSpace(value: node.DecisionCode)
            ) {
                receipt = default;
                previousHash = null;
            ...
            receipt = new WorldAuthorityOperationReceipt(
                node.OperationId,
                node.Actor,
                node.PayloadDigest,
                node.DecisionCode,
