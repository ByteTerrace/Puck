...
public sealed partial class WorldServer : IWorldRuleReader, IRuleHost {
    ...
    StateStore IRuleReader.Store => (m_ruleFrameActive
        ? EnsureRuleFrame()
        : (m_ruleFrameFallbackStore ??= new RowStore(rows: () => m_definition.State))
    );
    ...
    private StateCatalog RuleReadCatalog => (m_ruleFrameActive
        ? m_ruleFrameTickBaseline!.StateCatalog
        : m_definition.StateCatalog
    );
    ...
    void IRuleReader.ReportTableKeyMissing(string table, long key) => m_evaluator.ReportTableKeyMissing(
        key: key,
        table: table
    );
    ...
    bool IRuleHost.TryApply(StateMutation mutation, ulong tick, bool preflight, out string reason) {
        switch (mutation) {
            ...
            case StateMutation.Apply { Transform: StateTransform.WriteSet writeSet }: {
                    if (!EnsureRuleFrame().TryWriteSet(
                        reason: out reason,
                        writeSet: writeSet
                    )) {
                        return false;
            ...
            case StateMutation.Apply { Transform: StateTransform.ClearEnclosed enclosed }: {
                    if (!EnsureRuleFrame().TryClearEnclosed(
                        enclosed: enclosed,
                        reason: out reason
                    )) {
                        return false;
            ...
            case StateMutation.Apply { Transform: StateTransform.Transfer { Selector: ZoneSelector.First or ZoneSelector.Last or ZoneSelector.Key } transfer }: {
                    if (!EnsureRuleFrame().TryTransfer(
                        reason: out reason,
                        transfer: transfer
                    )) {
                        return false;
    ...
    RuleFact IWorldRuleReader.Read(PopulationOperand operand) => RuleFact.Finite(
        value: m_population.ActiveCount(),
        kind: CellKind.Int
    );
    RuleFact IWorldRuleReader.Read(PhysicsQuiescentOperand operand) => RuleFact.Finite(
        value: (m_population.RigidBodiesQuiescent()
        ? 1L
        : 0L),
        kind: CellKind.Bool
    );
    RuleFact IWorldRuleReader.Read(ClockOperand operand) => RuleFact.Finite(
        value: ReadClockPhaseError(),
        kind: CellKind.Int
    );
    RuleFact IWorldRuleReader.Read(RegionOccupancyOperand operand) => RuleFact.Finite(
        value: m_events.OccupantCount(placementId: operand.PlacementId),
        kind: CellKind.Int
    );
    RuleFact IWorldRuleReader.Read(MachineMemoryOperand operand) => RuleFact.Finite(
        value: (Machines.TryPeek(
    ...
    RuleFact IWorldRuleReader.Read(ArgBodyOperand operand) => RuleFact.Finite(
        value: ResolveArgBody(
            handle: operand.StateHandle,
            op: operand.Reduce,
            tick: m_evaluator.Tick,
            filterHandle: operand.FilterHandle,
            hasFilter: (operand.FilterRow is not null)
        ),
        kind: CellKind.Int
    );
    RuleFact IWorldRuleReader.Read(BodyDistanceOperand operand) => RuleFact.Finite(value: ReadBodyDistance(
        bodyA: operand.BodyA,
        bodyB: operand.BodyB,
        tick: m_evaluator.Tick
    ));
    RuleFact IWorldRuleReader.Read(LineOfSightOperand operand) => RuleFact.Finite(
    ...
    RuleFact IWorldRuleReader.Read(ParkedOperand operand) => ((ReadParkedRemaining(
        bodyRef: operand.BodyA,
        tick: m_evaluator.Tick
    ) is { } remaining)
        ? RuleFact.Finite(
            kind: CellKind.Int,
    ...
    RuleFact IWorldRuleReader.Read(UprightOperand operand) => RuleFact.Finite(value: ReadBodyUpright(
        bodyRef: operand.BodyA,
        tick: m_evaluator.Tick
    ));
    RuleFact IWorldRuleReader.Read(BodyFactOperand operand) => RuleFact.Finite(
        value: (((Body(index: ResolveBodyRef(
            bodyRef: operand.Body,
            tick: m_evaluator.Tick
        )) is { } factBody) && BodyFactVocabulary.Holds(
            gate: operand.Fact,
    ...
    RuleFact IWorldRuleReader.Read(LinkStalenessOperand operand) => RuleFact.Finite(
        value: m_events.LinkStalenessTicks(adjacencyName: operand.AdjacencyName),
        kind: CellKind.Int
    );
    RuleFact IWorldRuleReader.Read(ChannelOperand operand) => RuleFact.Finite(value: ReadChannelValue(
        seat: operand.Seat,
        ordinal: operand.ChannelOrdinal
    ));
    RuleFact IWorldRuleReader.Read(NearestOperand operand) => RuleFact.Finite(
        value: ResolveNearestBody(
            from: operand.BodyA,
            tagRowHandle: operand.StateHandle,
            tick: m_evaluator.Tick
        ),
        kind: CellKind.Int
    );
    ...
    RuleFact IWorldRuleReader.Read(BoardCellOfOperand operand) {
        ...
        return RuleFact.Finite(
            value: (((Body(index: index) is { } body) && operand.Topology.TryCellOf(
    ...
    int IWorldRuleReader.ResolveBody(in CompiledBodyRef bodyRef) => ResolveBodyRef(
        bodyRef: bodyRef,
        tick: m_evaluator.Tick
    );
    string IWorldRuleReader.PairKey(PairKeyFact key) => ResolvePairKey(
        a: ResolveBodyRef(
            bodyRef: key.BodyA,
            tick: m_evaluator.Tick
        ),
