...
/// how many in-flight timed presses (<c>body.press</c> holds) it cancelled — <c>body.stop</c>'s synchronous
...
/// <summary>Which cap, if any, decided a timed <c>body.press</c>'s effective hold — <see cref="WorldBody.PressChannel(int, FixedQ4816, float, FixedQ4816)"/>'s
...
/// <summary>The outcome of a timed <c>body.press</c> — the effective hold after both caps apply, and which one
/// (if either) actually bound it. <c>body.press</c>'s synchronous read-back, so its echo can report the true
