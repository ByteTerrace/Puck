...
/// (<see cref="WorldReplaySnapshot.Read"/>'s leading magic/shape-token check) and its mount-pin gate
/// (<see cref="WorldReplaySnapshot"/>'s <c>VerifyMountedAddons</c>), which together are what keeps a re-drive from
/// silently running a world the tape never recorded. NOT the whole codec: the many per-enum "unknown wire value"
...
/// coherent door; left as a natural follow-on increment rather than converted here.</para></summary>
