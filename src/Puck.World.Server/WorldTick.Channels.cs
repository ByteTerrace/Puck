...
public sealed partial class WorldTick {
        ...
        for (var seat = 0; (seat < Host.Population.LocalSeatCount); seat++) {
            if (
                ...
                for (var ordinal = 0; (ordinal < ChannelLimits.MaxChannels); ordinal++) {
                    ...
                    var shape = (Host.Population.Channels.IsDeclared(ordinal: ordinal)
                        ? Host.Population.Channels.Shape(ordinal: ordinal)
                    ...
                    var threshold = Host.Population.Channels.Threshold(ordinal: ordinal);
    ...
    private void StageContribution(int bodyIndex, WorldPrincipal principal, in IntentSubmission submission) {
        ...
        var hasReach = (!isConsoleOrSeat && Host.GrantTable.TryGetChannelReach(
            mask: out reach,
            principal: principal,
            subject: subject
        ));
