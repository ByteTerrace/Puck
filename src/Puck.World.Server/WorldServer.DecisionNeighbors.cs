...
public sealed partial class WorldServer {
    ...
    private void FreezeDecisionPerception(CompiledWorldRule[] rules) {
        if (!rules.Any(predicate: static rule => (rule.Decision?.Options.Any(predicate: static option => (option.Neighbors is not null)) == true))) { return; }
        if (
            (m_decisionPerception is null) ||
            (m_decisionPerception.Active.Length != m_population.Capacity)
        ) {
            m_decisionPerception = new(capacity: m_population.Capacity);
        }
        ...
        foreach (var rule in rules) {
            ...
            foreach (var option in policy.Options) {
                ...
                if (!image.Grids.ContainsKey(key: width)) { image.Grids.Add(
                    key: width,
                    value: new(
        ...
        for (var index = 0; (index < image.Active.Length); index++) {
            ...
            image.Generations[index] = m_population.Generation(index: index);
    ...
    private bool DecisionBodyLive(int index, int generation) => ((((uint)index) < ((uint)m_population.Capacity)) &&
        m_population.IsActive(index: index) && (m_population.EntryBody(index: index) is not null) && (m_population.Generation(index: index) == generation));
    private bool DecisionOptionGate(CompiledWorldDecisionOption option, int observer, int candidate, ulong tick) {
        ...
        m_evaluator.BoundLeft = ((option.Neighbors is null)
            ? -1
            : observer
        ); m_evaluator.BoundRight = candidate;
        try { return RuleGateOpen(
            gate: option.Gate,
            tick: tick
    ...
    private bool DecisionPerceptible(DecisionPerception image, CompiledWorldDecisionNeighbors neighbors, int observer, int candidate) {
        ...
        return m_population.HasLineOfSight(
            from: a,
