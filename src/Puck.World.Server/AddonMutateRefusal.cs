...
/// <c>Server.WorldAddonRuntime.ResolveMutations</c>'s six-stage dispatch gate refuses a <c>SubmitMutation</c> act,
...
public static class AddonMutateRefusals {
    ...
    public static AddonVerdict ToVerdict(AddonMutateRefusal reason) {
        return reason switch {
            ...
            _ => throw new ArgumentOutOfRangeException(paramName: nameof(reason), actualValue: reason, message: "unmapped addon.mutate refusal — the wire mapping must be extended deliberately, never defaulted"),
        };
    }
}
