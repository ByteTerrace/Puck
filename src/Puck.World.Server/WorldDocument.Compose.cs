...
public sealed partial class WorldDocument {
    ...
    internal static bool AffectsAddons(WorldMutation mutation) => (AnyMember(
    ...
    internal static bool AffectsSolidField(WorldMutation mutation) => (AnyMember(
    ...
    private bool ExceedsBootDerivedFaceReservation(WorldDefinition candidate, out string reason) {
        if (candidate.Authoring.DerivedFaceScreens <= Host.BootDerivedFaceScreens) {
            reason = string.Empty;

            return false;
        }

        reason = $"authoring.derivedFaceScreens {candidate.Authoring.DerivedFaceScreens} exceeds the boot-reserved {Host.BootDerivedFaceScreens} derived-face screen slot(s); the binder registers that band once at boot and the render provider key set is frozen there, so restart the host to load a wider one";
    ...
    /// <summary>Returns the row-scoped Mutate subject a creations/placements mutation names, or
    /// <see langword="null"/> for every other kind.</summary>
    /// <param name="mutation">The mutation.</param>
    /// <returns>The row-scoped subject, or <see langword="null"/>.</returns>
    internal static GrantSubject? RowScopedMutateSubjectOf(WorldMutation mutation) => mutation switch {
    ...
    /// <summary>Returns the world-document section a mutation targets — the Mutate-capability subject it is
    /// checked against.</summary>
    /// <param name="mutation">The mutation.</param>
    /// <returns>The targeted section.</returns>
    internal static WorldSection SectionOf(WorldMutation mutation) => mutation switch {
    ...
    internal static bool TryCompose(WorldDefinition current, WorldMutation mutation, ulong tick, ulong engineTick, string instanceIdentity, out WorldDefinition candidate, out string reason, out CellName? evictedKey) {
        if (!TryComposeCore(
    ...
    private static bool TryComposeCore(WorldDefinition current, WorldMutation mutation, ulong tick, ulong engineTick, string instanceIdentity, out WorldDefinition candidate, out string reason, out CellName? evictedKey) {
        reason = string.Empty;
        evictedKey = null;
        ...
        switch (mutation) {
            ...
            case WorldMutation.RemovePlacement m: {
                    ...
                    if (WorldServer.DescribeSpeakersAnchoredTo(
            ...
            case WorldMutation.RemoveTune m: {
                    if (WorldServer.DescribeSpeakersSourcing(
                        speakers: current.Speakers,
            ...
            case WorldMutation.RemovePatch m: {
                    if (WorldServer.DescribePatchDependents(
                        current: current,
                        patchId: m.Name
                    ) is { } dependents) {
                        candidate = current;
            ...
            // A whole-row declaration composes through an arena over the document it declares, which settles every
            ...
            case WorldMutation.RemoveStateRow m:
                ...
                return true;
            // A lone cell write composes through an arena of its own, on exactly the terms a batch's members do:
            // one load, the arena's kernels, one export, and a rewind, so the two doors can never disagree about
            // an envelope, a victim, or a settled clock.
