...
public sealed partial class WorldTick {
    ...
    /// <summary>Seeds and paints every lattice row's authored draw fill through the site ladder.</summary>
    /// <param name="definition">The definition whose lattice rows are painted.</param>
    internal void PaintLatticeDraws(WorldDefinition definition) {
    ...
    // O(rows squared).
    internal void RepaintChangedLatticeDraws(WorldDefinition previous, WorldDefinition current) {
        if (ReferenceEquals(
            objA: previous.State,
            objB: current.State
        )) {
