...
public sealed partial class WorldGrants {
    ...
    internal WorldGrant WithoutAuthoredConsent(WorldGrant grant) {
    ...
    /// against the same administration rule as <see cref="ApplyGrant"/> — enforced only for an <c>Addon</c>/<c>Peer</c> actor,
    ...
    /// permissive.</para>
    /// <para>Every mutating ingress passes this: <see cref="WorldServer.TryApplyMutation"/> for the ordered domain (loopback,
    ...
    internal bool TryAdmitMutation(WorldPrincipal principal, WorldSection section, int kindOrdinal, GrantSubject? rowScopedEditSubject, GrantSubject? rowScopedMutateSubject, bool meter, out WorldMutationAdmission admission) {
        ...
        if (
            ...
            if (!Host.MutationBudget.TryCharge(
                budget: budget,
                principal: principal,
                section: section
            )) {
