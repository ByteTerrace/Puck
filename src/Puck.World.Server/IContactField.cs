...
/// already consulted, plus the last resolved NON-walkable contact's surface normal, an EXISTING solver fact that
/// used to be computed and discarded inside <c>ApplyPush</c>. Read-back only: <see cref="ObstructionNormal"/> feeds
/// no integration and changes no simulation behavior — it is <c>world.contacts</c>' obstruction witness, the fact a
/// body pushed against a WALL (whose normal fails the grounded alignment test) previously had no way to surface at
/// all.</summary>
