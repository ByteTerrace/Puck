...
public sealed partial class WorldBody {
    ...
    private void ConstrainFlockLocomotion(ref BodyMotionScratch scratch) {
        if (
            (m_flockMovementDomain is not { } domain) ||
            (scratch.NextPosition == m_position)
        ) { return; }
        FlockMotionChecked = true;
        if (domain.AdmitsLocomotion(
            from: m_position,
            to: scratch.NextPosition
        )) { return; }
    ...
    private void ProduceFlockIntent(ref BodyMotionScratch scratch) {
        ...
        if (m_bodyMotionProgram.Contains(operation: BodyMotionOp.ComputeLocalTargetVelocity)) {
            ...
            var basis = ((m_tuning.MoveFrame == MotionMoveFrame.World)
                ? m_frame
                : m_orientation
            );
        ...
        scratch.Intent = m_roleOrdinals.Intent(
            moveAdvance: FixedVector3.Dot(
                left: desired,
                right: facing
            ),
            moveStrafe: FixedVector3.Dot(
                left: desired,
                right: right
            ),
            moveUp: FixedVector3.Dot(
                left: desired,
                right: up
            )
        );
