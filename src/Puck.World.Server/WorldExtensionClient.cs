...
public sealed class WorldExtensionClient : IDisposable {
    ...
    internal WorldExtensionClient(WorldExtensionHost host, WorldRecordedExtension runtime, WorldExternalOperationDispatcher dispatcher,
        ...
        Storage = storage?.WithLifetime(isActive: () => runtime.IsActive);
    }
    ...
    public IReadOnlyList<WorldExtensionOperationDescription> Discover() {
        CheckActive();
        return Operations.Values.Select(selector: operation => operation.Description).ToArray();
    }
    ...
    public WorldExtensionOperationHandle GetOperation(string operation, string requestKey) => m_host.GetOperation(
        client: this,
        name: operation,
    ...
    public ValueTask<WorldExtensionOperationHandle> InvokeAsync(string operation, string requestKey, string input = "",
        CancellationToken cancellationToken = default) => m_host.InvokeAsync(
            this,
            operation,
            requestKey,
            input,
    ...
    public QueryAnswer Observe(WorldQuery query) => Runtime.Observe(query: query);
    ...
    public long Submit(WorldMutation mutation) => Runtime.Submit(mutation: mutation);
}
