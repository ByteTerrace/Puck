...
internal static class WorldAddonMutationDecoder {
    private static readonly string[] s_rectMembers = ["x", "y", "width", "height"];
    private static readonly string[] s_elementMembers = ["id", "kind", "rect", "style", "text", "binding"];
    private static readonly string[] s_panelMembers = ["id", "rect", "layer", "style", "elements"];
    private static readonly string[] s_removePanelMembers = ["id"];
    private static readonly string[] s_removeElementMembers = ["panelId", "elementId"];
    private static readonly string[] s_setDefaultsMembers = ["enabled", "cursor"];
    private static readonly string[] s_setDefaultsCursorMembers = ["hoverRadius", "sizePx", "role"];
    ...
    private static readonly string[] s_removeStateRowMembers = ["name"];
    private static readonly string[] s_stateRowMembers = ["name", "kind", "value", "cells", "min", "max", "capacity", "nonNegative"];
    private static readonly string[] s_stateCellMembers = ["key", "value"];

    private static readonly Dictionary<string, CellKind> s_cellKinds = new(comparer: StringComparer.Ordinal) {
        ["int"] = CellKind.Int,
        ["fixed"] = CellKind.Fixed,
        ["bool"] = CellKind.Bool,
        ["text"] = CellKind.Text,
    };
    private static readonly string[] s_inputHoldMembers = ["ceilingTicks", "lowerAfterTicks", "defaultTicks", "equalizeByDefault", "participants"];
    private static readonly string[] s_inputHoldParticipantMembers = ["bodyIndex", "ticks", "equalized"];

    // The placement rows — the FULL WorldPlacement wire shape (every facet), plus each facet's own object shape.
    private static readonly string[] s_placementMembers = ["id", "creationId", "position", "yawDegrees", "scale", "distribution", "mirror", "emission", "solid", "inhabit", "faceSources", "region", "attach"];
    private static readonly string[] s_removePlacementMembers = ["id"];
    private static readonly string[] s_distributionMembers = ["region", "fill"];
    private static readonly string[] s_discRegionMembers = ["$type", "radius", "sampleCount"];
    private static readonly string[] s_pointsRegionMembers = ["$type", "names", "halfExtent"];
    private static readonly string[] s_latticeRegionMembers = ["$type", "stepA", "countA", "stepB", "countB"];
    private static readonly string[] s_sequenceMembers = ["name", "offset", "step"];
    private static readonly string[] s_mirrorMembers = ["normal", "offset"];
    private static readonly string[] s_emissionMembers = ["patchId", "level", "radius"];
    private static readonly string[] s_solidMembers = ["margin"];
    private static readonly string[] s_regionMembers = ["radius"];
    private static readonly string[] s_attachMembers = ["bodyIndex", "localOffset", "localYawDegrees"];
    private static readonly string[] s_inhabitMembers = ["kit", "look", "source", "count", "distribution"];
    private static readonly string[] s_faceSourceMembers = ["face", "source"];
    private static readonly string[] s_feedProfileMembers = ["width", "height", "refreshRateHz"];
    ...
    private static readonly string[] s_screenSourceNoneMembers = ["$type"];
    private static readonly string[] s_screenSourceTestPatternMembers = ["$type", "width", "height"];
    private static readonly string[] s_screenSourceMachineMembers = ["$type", "engine", "contentPath", "options"];
    private static readonly string[] s_screenSourceCameraMembers = ["$type", "profile"];
    private static readonly string[] s_screenSourceViewMembers = ["$type", "cameraName"];
    private static readonly string[] s_screenSourceCaptureMembers = ["$type", "windowTitle", "profile", "monitorIndex"];
    private static readonly string[] s_screenSourceConsoleMembers = ["$type", "rows", "columns", "procedural"];
    private static readonly string[] s_screenSourceQrMembers = ["$type", "payload", "ecLevel", "quietZoneModules"];

    private static readonly Dictionary<string, WorldHudElementKind> s_elementKinds = new(comparer: StringComparer.Ordinal) {
        ["rect"] = WorldHudElementKind.Rect,
        ["text"] = WorldHudElementKind.Text,
        ["gauge"] = WorldHudElementKind.Gauge,
    };

    private static readonly Dictionary<string, WorldHudLayer> s_layers = new(comparer: StringComparer.Ordinal) {
        ["under"] = WorldHudLayer.Under,
        ["over"] = WorldHudLayer.Over,
        ["replace"] = WorldHudLayer.Replace,
    };

    private static readonly Dictionary<string, WorldHudPanelStyle> s_panelStyles = new(comparer: StringComparer.Ordinal) {
        ["panel"] = WorldHudPanelStyle.Panel,
        ["strip"] = WorldHudPanelStyle.Strip,
        ["chip"] = WorldHudPanelStyle.Chip,
    };

    private static readonly Dictionary<string, WorldHudStyleToken> s_styleTokens = new(comparer: StringComparer.Ordinal) {
        ...
        using (document) {
            ...
            try {
                mutation = kindOrdinal switch {
                    19 => DecodeUpsertPlacement(root: document.RootElement, principal: principal),
                    20 => DecodeRemovePlacement(root: document.RootElement, principal: principal),
                    41 => DecodeUpsertHudPanel(root: document.RootElement, principal: principal),
                    42 => DecodeRemoveHudPanel(root: document.RootElement, principal: principal),
                    43 => DecodeUpsertHudElement(root: document.RootElement, principal: principal),
                    44 => DecodeRemoveHudElement(root: document.RootElement, principal: principal),
                    45 => DecodeSetHudDefaults(root: document.RootElement, principal: principal),
                    46 => DecodeUpsertStateRow(root: document.RootElement, principal: principal),
                    47 => DecodeRemoveStateRow(root: document.RootElement, principal: principal),
                    48 => DecodeSetInputHold(root: document.RootElement, principal: principal),
    ...
    private static WorldMutation DecodeSetInputHold(JsonElement root, WorldPrincipal principal) {
        var members = UniqueMembers(element: root, context: "SetInputHold");

        RequireNoUnknownMembers(members: members, allowed: s_inputHoldMembers, context: "SetInputHold");

        if (!members.Remove(key: "participants", value: out var participantsElement) || (participantsElement.ValueKind != JsonValueKind.Array)) {
        ...
        foreach (var element in participantsElement.EnumerateArray()) {
            ...
            var participantMembers = UniqueMembers(element: element, context: context);

            RequireNoUnknownMembers(members: participantMembers, allowed: s_inputHoldParticipantMembers, context: context);
            participants.Add(item: new WorldInputHoldParticipant(
                BodyIndex: RequireInt32(members: participantMembers, name: "bodyIndex", context: context),
                Ticks: RequireInt32(members: participantMembers, name: "ticks", context: context),
                Equalized: RequireBool(members: participantMembers, name: "equalized", context: context)
            ));
            index++;
        }
        ...
        return new WorldMutation.SetInputHold(
            ...
            Settings: new WorldInputHoldSettings(
                CeilingTicks: RequireInt32(members: members, name: "ceilingTicks", context: "SetInputHold"),
                LowerAfterTicks: RequireInt32(members: members, name: "lowerAfterTicks", context: "SetInputHold"),
                DefaultTicks: RequireInt32(members: members, name: "defaultTicks", context: "SetInputHold"),
                EqualizeByDefault: RequireBool(members: members, name: "equalizeByDefault", context: "SetInputHold"),
    ...
    private static WorldMutation DecodeUpsertHudPanel(JsonElement root, WorldPrincipal principal) {
        var members = UniqueMembers(element: root, context: "UpsertHudPanel");

        RequireNoUnknownMembers(members: members, allowed: s_panelMembers, context: "UpsertHudPanel");

        var id = RequireString(members: members, name: "id", context: "UpsertHudPanel");
        var rect = RequireRect(members: members, name: "rect", context: "UpsertHudPanel");
        var layer = RequireEnum(members: members, name: "layer", map: s_layers, context: "UpsertHudPanel");
        var style = RequireEnum(members: members, name: "style", map: s_panelStyles, context: "UpsertHudPanel");
        var elements = new List<WorldHudElement>();

        if (members.TryGetValue(key: "elements", value: out var elementsElement)) {
            ...
            foreach (var element in elementsElement.EnumerateArray()) {
                elements.Add(item: DecodeElement(element: element, context: "UpsertHudPanel.elements[]"));
            }
        }

        return new WorldMutation.UpsertHudPanel(Principal: principal, Panel: new WorldHudPanel(Id: id, Rect: rect, Layer: layer, Style: style, Elements: elements));
    }
    ...
    private static WorldMutation DecodeSingleKeyRemoval(
        ...
        var members = UniqueMembers(element: root, context: context);

        RequireNoUnknownMembers(members: members, allowed: allowed, context: context);

        var key = RequireString(members: members, name: keyName, context: context);

        return create(arg1: principal, arg2: key);
    }
    ...
    private static WorldMutation DecodeRemoveHudPanel(JsonElement root, WorldPrincipal principal) {
        return DecodeSingleKeyRemoval(
            ...
            allowed: s_removePanelMembers,
            keyName: "id",
            create: static (actor, key) => new WorldMutation.RemoveHudPanel(Principal: actor, Id: key));
    }

    private static WorldMutation DecodeUpsertHudElement(JsonElement root, WorldPrincipal principal) {
        var members = UniqueMembers(element: root, context: "UpsertHudElement");

        RequireNoUnknownMembers(members: members, allowed: ["panelId", "element"], context: "UpsertHudElement");

        var panelId = RequireString(members: members, name: "panelId", context: "UpsertHudElement");

        if (!members.TryGetValue(key: "element", value: out var elementValue) || (elementValue.ValueKind != JsonValueKind.Object)) {
            throw new AddonMutationDecodeException(message: "UpsertHudElement: 'element' must be an object");
        }

        var element = DecodeElement(element: elementValue, context: "UpsertHudElement.element");

        return new WorldMutation.UpsertHudElement(Principal: principal, PanelId: panelId, Element: element);
    }

    private static WorldMutation DecodeRemoveHudElement(JsonElement root, WorldPrincipal principal) {
        var members = UniqueMembers(element: root, context: "RemoveHudElement");

        RequireNoUnknownMembers(members: members, allowed: s_removeElementMembers, context: "RemoveHudElement");

        var panelId = RequireString(members: members, name: "panelId", context: "RemoveHudElement");
        var elementId = RequireString(members: members, name: "elementId", context: "RemoveHudElement");

        return new WorldMutation.RemoveHudElement(Principal: principal, PanelId: panelId, ElementId: elementId);
    }

    private static WorldMutation DecodeSetHudDefaults(JsonElement root, WorldPrincipal principal) {
        var members = UniqueMembers(element: root, context: "SetHudDefaults");

        RequireNoUnknownMembers(members: members, allowed: s_setDefaultsMembers, context: "SetHudDefaults");

        if (!members.TryGetValue(key: "enabled", value: out var enabledElement) || (enabledElement.ValueKind is not (JsonValueKind.True or JsonValueKind.False))) {
        ...
        if (members.TryGetValue(key: "cursor", value: out var cursorElement)) {
            ...
            var cursorMembers = UniqueMembers(element: cursorElement, context: "SetHudDefaults.cursor");

            RequireNoUnknownMembers(members: cursorMembers, allowed: s_setDefaultsCursorMembers, context: "SetHudDefaults.cursor");

            var roleToken = RequireString(members: cursorMembers, name: "role", context: "SetHudDefaults.cursor");

            if (!Enum.TryParse<WorldHudCursorRole>(value: roleToken, ignoreCase: false, result: out var role) || !Enum.IsDefined(value: role)) {
            ...
            cursor = new WorldHudCursor(
                HoverRadius: RequireFinite(members: cursorMembers, name: "hoverRadius", context: "SetHudDefaults.cursor"),
                SizePx: RequireFinite(members: cursorMembers, name: "sizePx", context: "SetHudDefaults.cursor"),
                Role: role
            );
        }

        return new WorldMutation.SetHudDefaults(Principal: principal, Defaults: new WorldHudDefaults(Enabled: enabledElement.GetBoolean(), Cursor: cursor));
    }
    ...
    private static WorldMutation DecodeUpsertStateRow(JsonElement root, WorldPrincipal principal) {
        ...
        RequireNoUnknownMembers(members: members, allowed: s_stateRowMembers, context: Context);

        var name = RequireString(members: members, name: "name", context: Context);
        var rowName = RequireCellName(candidate: name, context: $"{Context}.name");
        var kind = RequireEnum(members: members, name: "kind", map: s_cellKinds, context: Context);
        var hasValue = members.TryGetValue(key: "value", value: out var valueElement);
        var hasCells = members.TryGetValue(key: "cells", value: out var cellsElement);
        var hasCapacity = members.TryGetValue(key: "capacity", value: out var capacityElement);
        ...
        return new WorldMutation.UpsertStateRow(Principal: principal, Row: new WorldStateRow(
            Name: rowName,
            Kind: kind,
            Min: OptionalStateNumber(members: members, name: "min", kind: kind, context: Context),
            Max: OptionalStateNumber(members: members, name: "max", kind: kind, context: Context),
            Capacity: (hasCapacity ? (capacityElement.TryGetInt32(value: out var capacity) ? capacity : throw new AddonMutationDecodeException(message: $"{Context}: 'capacity' must be an integer")) : null),
            NonNegative: (members.TryGetValue(key: "nonNegative", value: out var nonNegativeElement) && ((nonNegativeElement.ValueKind is JsonValueKind.True or JsonValueKind.False) ? nonNegativeElement.GetBoolean() : throw new AddonMutationDecodeException(message: $"{Context}: 'nonNegative' must be a boolean"))),
            Cells: (hasValue
                ? [DecodeStateCell(key: WorldStateRow.SlotKey, element: valueElement, kind: kind, context: $"{Context}.value")]
                : (hasCells ? DecodeStateCells(element: cellsElement, kind: kind, context: $"{Context}.cells") : []))
        ));
    }
    ...
    private static WorldMutation DecodeRemoveStateRow(JsonElement root, WorldPrincipal principal) {
        return DecodeSingleKeyRemoval(
            ...
            allowed: s_removeStateRowMembers,
            keyName: "name",
            create: static (actor, key) => new WorldMutation.RemoveStateRow(Principal: actor, Name: key));
    }
    ...
    private static List<WorldStateCell> DecodeStateCells(JsonElement element, CellKind kind, string context) {
        ...
        foreach (var entry in element.EnumerateArray()) {
            ...
            var members = UniqueMembers(element: entry, context: cellContext);

            RequireNoUnknownMembers(members: members, allowed: s_stateCellMembers, context: cellContext);

            var key = RequireString(members: members, name: "key", context: cellContext);
            var cellKey = RequireCellName(candidate: key, context: $"{cellContext}.key");

            if (!members.TryGetValue(key: "value", value: out var valueElement)) {
                throw new AddonMutationDecodeException(message: $"{cellContext}: requires 'value'");
            }

            cells.Add(item: DecodeStateCell(key: cellKey, element: valueElement, kind: kind, context: $"{cellContext}.value"));
            index++;
        }
    ...
    private static WorldCellName RequireCellName(string candidate, string context) =>
        (WorldCellName.TryParse(candidate: candidate, name: out var name, reason: out var reason) ? name : throw new AddonMutationDecodeException(message: $"{context}: '{candidate}' {reason}"));
    ...
    private static WorldStateCell DecodeStateCell(WorldCellName key, JsonElement element, CellKind kind, string context) {
        switch (kind) {
            case CellKind.Text:
                ...
                return new WorldStateCell(Key: key, Text: (element.GetString() ?? string.Empty));
            case CellKind.Bool:
                if (element.ValueKind is not (JsonValueKind.True or JsonValueKind.False)) {
                    throw new AddonMutationDecodeException(message: $"{context}: must be a boolean");
                }

                return new WorldStateCell(Key: key, Value: (element.GetBoolean() ? 1 : 0));
            default:
                return new WorldStateCell(Key: key, Value: RequireStateNumber(element: element, kind: kind, context: context));
        }
    }

    private static long? OptionalStateNumber(Dictionary<string, JsonElement> members, string name, CellKind kind, string context) =>
        (members.TryGetValue(key: name, value: out var element) ? RequireStateNumber(element: element, kind: kind, context: $"{context}.{name}") : null);

    private static long RequireStateNumber(JsonElement element, CellKind kind, string context) {
        if (kind == CellKind.Fixed) {
            if ((element.ValueKind != JsonValueKind.String) || !Puck.Maths.FixedQ4816.TryParse(s: element.GetString(), provider: CultureInfo.InvariantCulture, result: out var parsed)) {
        ...
        if ((element.ValueKind != JsonValueKind.Number) || !element.TryGetInt64(value: out var whole)) {
    ...
    private static WorldMutation DecodeUpsertPlacement(JsonElement root, WorldPrincipal principal) {
        var members = UniqueMembers(element: root, context: "UpsertPlacement");

        RequireNoUnknownMembers(members: members, allowed: s_placementMembers, context: "UpsertPlacement");

        var id = RequireString(members: members, name: "id", context: "UpsertPlacement");
        var creationId = RequireString(members: members, name: "creationId", context: "UpsertPlacement");
        var position = RequireVector3(members: members, name: "position", context: "UpsertPlacement");
        var yawDegrees = RequireFinite(members: members, name: "yawDegrees", context: "UpsertPlacement");
        var scale = RequireFinite(members: members, name: "scale", context: "UpsertPlacement");
        ...
        if (members.TryGetValue(key: "distribution", value: out var distributionElement)) {
            distribution = DecodeDistribution(element: distributionElement, context: "UpsertPlacement.distribution");
        }

        if (members.TryGetValue(key: "mirror", value: out var mirrorElement)) {
            mirror = DecodeMirror(element: mirrorElement, context: "UpsertPlacement.mirror");
        }

        if (members.TryGetValue(key: "emission", value: out var emissionElement)) {
            emission = DecodeEmission(element: emissionElement, context: "UpsertPlacement.emission");
        }

        if (members.TryGetValue(key: "solid", value: out var solidElement)) {
            solid = DecodeSolid(element: solidElement, context: "UpsertPlacement.solid");
        }

        if (members.TryGetValue(key: "inhabit", value: out var inhabitElement)) {
            inhabit = DecodeInhabit(element: inhabitElement, context: "UpsertPlacement.inhabit");
        }

        if (members.TryGetValue(key: "faceSources", value: out var faceSourcesElement)) {
            ...
            foreach (var element in faceSourcesElement.EnumerateArray()) {
                faceSources.Add(item: DecodeFaceSource(element: element, context: "UpsertPlacement.faceSources[]"));
            }
        }

        if (members.TryGetValue(key: "region", value: out var regionElement)) {
            region = DecodeRegion(element: regionElement, context: "UpsertPlacement.region");
        }

        if (members.TryGetValue(key: "attach", value: out var attachElement)) {
            attach = DecodeAttach(element: attachElement, context: "UpsertPlacement.attach");
        }
        ...
        return new WorldMutation.UpsertPlacement(Principal: principal, Placement: placement);
    }
    ...
    private static WorldMutation DecodeRemovePlacement(JsonElement root, WorldPrincipal principal) {
        return DecodeSingleKeyRemoval(
            ...
            allowed: s_removePlacementMembers,
            keyName: "id",
            create: static (actor, key) => new WorldMutation.RemovePlacement(Principal: actor, Id: key));
    }
    ...
    private static WorldDistribution DecodeDistribution(JsonElement element, string context) {
        ...
        RequireNoUnknownMembers(members: members, allowed: s_distributionMembers, context: context);

        if (!members.TryGetValue(key: "region", value: out var region) || !members.TryGetValue(key: "fill", value: out var fill)) {
            throw new AddonMutationDecodeException(message: $"{context}: requires 'region' and 'fill'");
        }

        return new WorldDistribution(Region: DecodeDistributionRegion(element: region, context: $"{context}.region"), Fill: DecodeSequence(element: fill, context: $"{context}.fill"));
    }
    ...
    private static WorldDistributionRegion DecodeDistributionRegion(JsonElement element, string context) {
        ...
        var members = UniqueMembers(element: element, context: context);
        var type = RequireString(members: members, name: "$type", context: context);

        switch (type) {
            case "disc": {
                RequireNoUnknownMembers(members: members, allowed: s_discRegionMembers, context: context);
                int? sampleCount = null;

                if (members.TryGetValue(key: "sampleCount", value: out var sampleElement) && (sampleElement.ValueKind != JsonValueKind.Null)) {
                ...
                return new WorldDistributionRegion.Disc(Radius: RequireFinite(members: members, name: "radius", context: context), SampleCount: sampleCount);
            }
            case "points": {
                RequireNoUnknownMembers(members: members, allowed: s_pointsRegionMembers, context: context);

                if (!members.TryGetValue(key: "names", value: out var namesElement) || (namesElement.ValueKind != JsonValueKind.Array)) {
                ...
                return new WorldDistributionRegion.Points(Names: names, HalfExtent: RequireFinite(members: members, name: "halfExtent", context: context));
            }
            case "lattice":
                RequireNoUnknownMembers(members: members, allowed: s_latticeRegionMembers, context: context);

                return new WorldDistributionRegion.Lattice(
                    StepA: RequireVector3(members: members, name: "stepA", context: context),
                    CountA: RequireInt32(members: members, name: "countA", context: context),
                    StepB: RequireVector3(members: members, name: "stepB", context: context),
                    CountB: RequireInt32(members: members, name: "countB", context: context));
            default:
    ...
    private static WorldSequence DecodeSequence(JsonElement element, string context) {
        ...
        RequireNoUnknownMembers(members: members, allowed: s_sequenceMembers, context: context);

        return new WorldSequence(
            Name: RequireString(members: members, name: "name", context: context),
            Offset: RequireInt32(members: members, name: "offset", context: context),
            Step: RequireFinite(members: members, name: "step", context: context));
    }
    ...
    private static WorldPlacementMirror DecodeMirror(JsonElement element, string context) {
        ...
        RequireNoUnknownMembers(members: members, allowed: s_mirrorMembers, context: context);

        return new WorldPlacementMirror(
            Normal: RequireVector3(members: members, name: "normal", context: context),
            Offset: RequireFinite(members: members, name: "offset", context: context)
        );
    }
    ...
    private static WorldEmission DecodeEmission(JsonElement element, string context) {
        ...
        RequireNoUnknownMembers(members: members, allowed: s_emissionMembers, context: context);

        var patchId = RequireString(members: members, name: "patchId", context: context);
        var level = RequireFinite(members: members, name: "level", context: context);
        float? radius = null;

        if (members.TryGetValue(key: "radius", value: out var radiusElement)) {
            if ((radiusElement.ValueKind != JsonValueKind.Number) || !radiusElement.TryGetSingle(value: out var value) || !float.IsFinite(f: value)) {
        ...
        return new WorldEmission(PatchId: patchId, Level: level, Radius: radius);
    }
    ...
    private static WorldSolid DecodeSolid(JsonElement element, string context) {
        ...
        RequireNoUnknownMembers(members: members, allowed: s_solidMembers, context: context);

        var margin = RequireFinite(members: members, name: "margin", context: context);
    ...
    private static WorldPlacementRegion DecodeRegion(JsonElement element, string context) {
        ...
        RequireNoUnknownMembers(members: members, allowed: s_regionMembers, context: context);

        var radius = RequireFinite(members: members, name: "radius", context: context);
    ...
    private static WorldPlacementAttach DecodeAttach(JsonElement element, string context) {
        ...
        RequireNoUnknownMembers(members: members, allowed: s_attachMembers, context: context);

        var bodyIndex = RequireInt32(members: members, name: "bodyIndex", context: context);
        ...
        var localOffset = RequireVector3(members: members, name: "localOffset", context: context);
        var localYawDegrees = 0f;

        if (members.TryGetValue(key: "localYawDegrees", value: out var yawElement)) {
            if ((yawElement.ValueKind != JsonValueKind.Number) || !yawElement.TryGetSingle(value: out var value) || !float.IsFinite(f: value)) {
        ...
        return new WorldPlacementAttach(BodyIndex: bodyIndex, LocalOffset: localOffset, LocalYawDegrees: localYawDegrees);
    }
    ...
    private static WorldPlacementInhabit DecodeInhabit(JsonElement element, string context) {
        ...
        RequireNoUnknownMembers(members: members, allowed: s_inhabitMembers, context: context);

        string? kit = null;
        string? look = null;

        if (members.TryGetValue(key: "kit", value: out var kitElement)) {
        ...
        if (members.TryGetValue(key: "look", value: out var lookElement)) {
        ...
        var source = RequireIntentSource(members: members, context: context);
        var count = 1;

        if (members.TryGetValue(key: "count", value: out var countElement)) {
            if ((countElement.ValueKind != JsonValueKind.Number) || !countElement.TryGetInt32(value: out count)) {
        ...
        var distribution = (members.TryGetValue(key: "distribution", value: out var distributionElement)
            ? DecodeDistribution(element: distributionElement, context: $"{context}.distribution")
            : null);

        return new WorldPlacementInhabit(Kit: kit, Look: look, Source: source, Count: count, Distribution: distribution);
    }
    ...
    private static WorldPlacementFace DecodeFaceSource(JsonElement element, string context) {
        ...
        RequireNoUnknownMembers(members: members, allowed: s_faceSourceMembers, context: context);

        var face = RequireString(members: members, name: "face", context: context);

        if (!members.TryGetValue(key: "source", value: out var sourceElement) || (sourceElement.ValueKind != JsonValueKind.Object)) {
            throw new AddonMutationDecodeException(message: $"{context}: 'source' must be an object");
        }

        var source = DecodeScreenSource(element: sourceElement, context: $"{context}.source");

        return new WorldPlacementFace(Face: face, Source: source);
    }
    ...
    private static WorldScreenSource DecodeScreenSource(JsonElement element, string context) {
        ...
        return type switch {
            "none" => DecodeScreenSourceNone(members: members, context: context),
            "testPattern" => DecodeScreenSourceTestPattern(members: members, context: context),
            "machine" => DecodeScreenSourceMachine(members: members, context: context),
            "camera" => DecodeScreenSourceCamera(members: members, context: context),
            "view" => DecodeScreenSourceView(members: members, context: context),
            "capture" => DecodeScreenSourceCapture(members: members, context: context),
            "console" => DecodeScreenSourceConsole(members: members, context: context),
            "qr" => DecodeScreenSourceQr(members: members, context: context),
    ...
    private static WorldScreenSource DecodeScreenSourceNone(Dictionary<string, JsonElement> members, string context) {
        RequireNoUnknownMembers(members: members, allowed: s_screenSourceNoneMembers, context: context);
    ...
    private static WorldScreenSource DecodeScreenSourceTestPattern(Dictionary<string, JsonElement> members, string context) {
        RequireNoUnknownMembers(members: members, allowed: s_screenSourceTestPatternMembers, context: context);

        var width = RequireInt32(members: members, name: "width", context: context);
        var height = RequireInt32(members: members, name: "height", context: context);

        return new WorldScreenSource.TestPattern(Width: width, Height: height);
    }

    private static WorldScreenSource DecodeScreenSourceMachine(Dictionary<string, JsonElement> members, string context) {
        RequireNoUnknownMembers(members: members, allowed: s_screenSourceMachineMembers, context: context);

        var engine = RequireString(members: members, name: "engine", context: context);
        var contentPath = RequireString(members: members, name: "contentPath", context: context);
        string? options = null;

        if (members.TryGetValue(key: "options", value: out var optionsElement)) {
        ...
        return new WorldScreenSource.Machine(Engine: engine, ContentPath: contentPath, Options: options);
    }

    private static WorldScreenSource DecodeScreenSourceCamera(Dictionary<string, JsonElement> members, string context) {
        RequireNoUnknownMembers(members: members, allowed: s_screenSourceCameraMembers, context: context);

        if (!members.TryGetValue(key: "profile", value: out var profileElement) || (profileElement.ValueKind != JsonValueKind.Object)) {
            throw new AddonMutationDecodeException(message: $"{context}: 'profile' must be an object");
        }

        var profile = DecodeFeedProfile(element: profileElement, context: $"{context}.profile");
    ...
    private static WorldScreenSource DecodeScreenSourceView(Dictionary<string, JsonElement> members, string context) {
        RequireNoUnknownMembers(members: members, allowed: s_screenSourceViewMembers, context: context);

        var cameraName = RequireString(members: members, name: "cameraName", context: context);
    ...
    private static WorldScreenSource DecodeScreenSourceCapture(Dictionary<string, JsonElement> members, string context) {
        RequireNoUnknownMembers(members: members, allowed: s_screenSourceCaptureMembers, context: context);

        var windowTitle = RequireString(members: members, name: "windowTitle", context: context);
        ...
        if (members.TryGetValue(key: "monitorIndex", value: out var monitorElement)) {
            if ((monitorElement.ValueKind != JsonValueKind.Number) || !monitorElement.TryGetInt32(value: out var value)) {
        ...
        return new WorldScreenSource.Capture(WindowTitle: windowTitle, Profile: profile, MonitorIndex: monitorIndex);
    }

    private static WorldScreenSource DecodeScreenSourceConsole(Dictionary<string, JsonElement> members, string context) {
        RequireNoUnknownMembers(members: members, allowed: s_screenSourceConsoleMembers, context: context);

        var rows = 24;
        var columns = 64;
        var procedural = false;

        if (members.TryGetValue(key: "rows", value: out var rowsElement)) {
            if ((rowsElement.ValueKind != JsonValueKind.Number) || !rowsElement.TryGetInt32(value: out rows)) {
                throw new AddonMutationDecodeException(message: $"{context}: 'rows' must be an integer");
            }
        }

        if (members.TryGetValue(key: "columns", value: out var columnsElement)) {
            if ((columnsElement.ValueKind != JsonValueKind.Number) || !columnsElement.TryGetInt32(value: out columns)) {
                throw new AddonMutationDecodeException(message: $"{context}: 'columns' must be an integer");
            }
        }

        if (members.TryGetValue(key: "procedural", value: out var proceduralElement)) {
        ...
        return new WorldScreenSource.Console(Rows: rows, Columns: columns, Procedural: procedural);
    }

    private static WorldScreenSource DecodeScreenSourceQr(Dictionary<string, JsonElement> members, string context) {
        RequireNoUnknownMembers(members: members, allowed: s_screenSourceQrMembers, context: context);

        var payload = RequireString(members: members, name: "payload", context: context);
        var ecLevel = "M";
        var quietZoneModules = 4;

        if (members.TryGetValue(key: "ecLevel", value: out var ecLevelElement)) {
        ...
        if (members.TryGetValue(key: "quietZoneModules", value: out var quietZoneElement)) {
            if ((quietZoneElement.ValueKind != JsonValueKind.Number) || !quietZoneElement.TryGetInt32(value: out quietZoneModules)) {
        ...
        return new WorldScreenSource.Qr(Payload: payload, EcLevel: ecLevel, QuietZoneModules: quietZoneModules);
    }

    private static WorldFeedProfile DecodeFeedProfile(JsonElement element, string context) {
        var members = UniqueMembers(element: element, context: context);

        RequireNoUnknownMembers(members: members, allowed: s_feedProfileMembers, context: context);

        var width = RequireInt32(members: members, name: "width", context: context);
        var height = RequireInt32(members: members, name: "height", context: context);
        var refreshRateHz = RequireUInt32(members: members, name: "refreshRateHz", context: context);

        return new WorldFeedProfile(Width: width, Height: height, RefreshRateHz: refreshRateHz);
    }

    private static Vector3 RequireVector3(Dictionary<string, JsonElement> members, string name, string context) {
        if (!members.TryGetValue(key: name, value: out var element) || (element.ValueKind != JsonValueKind.Array)) {
        ...
        foreach (var item in element.EnumerateArray()) {
            ...
            if ((item.ValueKind != JsonValueKind.Number) || !item.TryGetSingle(value: out var component) || !float.IsFinite(f: component)) {
    ...
    private static bool RequireBool(Dictionary<string, JsonElement> members, string name, string context) {
        if (!members.TryGetValue(key: name, value: out var element) || (element.ValueKind is not (JsonValueKind.True or JsonValueKind.False))) {
    ...
    private static int RequireInt32(Dictionary<string, JsonElement> members, string name, string context) {
        var element = RequireNumberElement(members: members, name: name, context: context, expected: "an integer");
    ...
    private static uint RequireUInt32(Dictionary<string, JsonElement> members, string name, string context) {
        var element = RequireNumberElement(members: members, name: name, context: context, expected: "a non-negative integer");
    ...
    private static long? OptionalInt64(Dictionary<string, JsonElement> members, string name, string context) {
        if (!members.TryGetValue(key: name, value: out var element)) {
            return null;
        }

        if ((element.ValueKind != JsonValueKind.Number) || !element.TryGetInt64(value: out var value)) {
            throw new AddonMutationDecodeException(message: $"{context}: '{name}' must be an integer");
        }
    ...
    private static WorldHudElement DecodeElement(JsonElement element, string context) {
        ...
        RequireNoUnknownMembers(members: members, allowed: s_elementMembers, context: context);

        var id = RequireString(members: members, name: "id", context: context);
        var kind = RequireEnum(members: members, name: "kind", map: s_elementKinds, context: context);
        var rect = RequireRect(members: members, name: "rect", context: context);
        var style = RequireEnum(members: members, name: "style", map: s_styleTokens, context: context);
        string? text = null;
        string? binding = null;

        if (members.TryGetValue(key: "text", value: out var textElement)) {
        ...
        if (members.TryGetValue(key: "binding", value: out var bindingElement)) {
        ...
        return new WorldHudElement(Id: id, Kind: kind, Rect: rect, Style: style, Text: text, Binding: binding);
    }

    private static WorldHudRect RequireRect(Dictionary<string, JsonElement> members, string name, string context) {
        if (!members.TryGetValue(key: name, value: out var rectElement) || (rectElement.ValueKind != JsonValueKind.Object)) {
            throw new AddonMutationDecodeException(message: $"{context}: '{name}' must be an object");
        }

        var rectMembers = UniqueMembers(element: rectElement, context: $"{context}.{name}");

        RequireNoUnknownMembers(members: rectMembers, allowed: s_rectMembers, context: $"{context}.{name}");

        var x = RequireFinite(members: rectMembers, name: "x", context: $"{context}.{name}");
        var y = RequireFinite(members: rectMembers, name: "y", context: $"{context}.{name}");
        var width = RequireFinite(members: rectMembers, name: "width", context: $"{context}.{name}");
        var height = RequireFinite(members: rectMembers, name: "height", context: $"{context}.{name}");
        ...
        return new WorldHudRect(X: x, Y: y, Width: width, Height: height);
    }

    private static string RequireString(Dictionary<string, JsonElement> members, string name, string context) {
        if (!members.TryGetValue(key: name, value: out var element) || (element.ValueKind != JsonValueKind.String)) {
    ...
    private static float RequireFinite(Dictionary<string, JsonElement> members, string name, string context) {
        if (!members.TryGetValue(key: name, value: out var element) || (element.ValueKind != JsonValueKind.Number) || !element.TryGetSingle(value: out var value)) {
    ...
    private static JsonElement RequireNumberElement(Dictionary<string, JsonElement> members, string name, string context, string expected) {
        if (!members.TryGetValue(key: name, value: out var element) || (element.ValueKind != JsonValueKind.Number)) {
    ...
    private static IntentSource RequireIntentSource(Dictionary<string, JsonElement> members, string context) {
        var token = RequireString(members: members, name: "source", context: context);

        if (string.Equals(a: token, b: "live", comparisonType: StringComparison.Ordinal)) {
            return IntentSource.Live;
        }
        if (string.Equals(a: token, b: "idle", comparisonType: StringComparison.Ordinal)) {
    ...
    private static TEnum RequireEnum<TEnum>(Dictionary<string, JsonElement> members, string name, Dictionary<string, TEnum> map, string context) where TEnum : struct, Enum {
        ...
        if (!map.TryGetValue(key: token, value: out var value)) {
            throw new AddonMutationDecodeException(message: $"{context}: '{name}' names '{token}', which is not one of {{{string.Join(separator: ", ", values: map.Keys)}}}");
        }
