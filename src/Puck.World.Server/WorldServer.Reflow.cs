...
public sealed partial class WorldServer {
    ...
    public bool TryPreviewReflow(WorldPlacementReflowRequest request, WorldPrincipal principal, out WorldPlacementProposal? proposal, out string reason) {
        ...
        if (!TryCaptureReflow(
            ...
            request: request,
            tick: out var tick
        )) { proposal = null; return false; }
        var result = BuildReflowPreview(
    ...
    public bool TryStartReflowPreview(WorldPlacementReflowRequest request, WorldPrincipal principal,
        ...
        try {
            pending = Task.Run(function: () => BuildReflowPreview(
                definition: definition!,
                principal: principal,
                request: request,
                tick: tick
    ...
    private bool TryBuildReflowPreview(WorldDefinition definition, ulong tick, WorldPlacementReflowRequest request, WorldPrincipal principal, out WorldPlacementProposal? proposal, out string reason) {
        ...
        if (request.PlacementIds is { } requestedIds) {
            if (
                ...
                requestedIds.Any(predicate: id => string.IsNullOrWhiteSpace(value: id)) ||
                (requestedIds.Distinct(comparer: StringComparer.Ordinal).Count() != requestedIds.Count)
            ) {
                reason = "explicit reflow placement groups must contain distinct named members";
                return false;
            }
            var knownIds = definition.Placements.Select(selector: placement => placement.Id).ToHashSet(comparer: StringComparer.Ordinal);

            if (requestedIds.Any(predicate: id => !knownIds.Contains(item: id))) {
        ...
        if (
            defaultSelection &&
            (template.Deal.Preserve?.Transform != true)
        ) {
            reason = "dealt-child reflow requires instance-owned transforms";
            return false;
        }
        var children = (defaultSelection
            ? definition.Placements.Where(predicate: p => WorldPlacementDeal.IsChild(
                parent: template,
                placement: p
            ))
            : definition.Placements.Where(predicate: p => request.PlacementIds!.Contains(
                p.Id,
        ...
        if (
            (children.Length == 0) ||
            (children.Length > 64) ||
            children.Any(predicate: p => (!HasReflowBounds(placement: p) || (p.Attach is not null) || (p.Inhabit is not null) || (p.Distribution is not null) || (p.Mirror is not null)))
        ) {
            reason = "reflow requires 1..64 static members with occupation or clearance volumes";
            return false;
        }
        if (children.Any(predicate: p => !string.Equals(
            a: p.Parent,
            b: template.Id,
        ...
        var selectedIds = children.Select(selector: p => p.Id).ToHashSet(comparer: StringComparer.Ordinal);

        if (definition.Placements.Any(predicate: placement => ((placement.Parent is { } parent) && selectedIds.Contains(item: parent)))) {
        ...
        if (
            edits.Any(predicate: edit => ((edit is null) || !selectedIds.Contains(item: edit.PlacementId))) ||
            (edits.Select(selector: edit => edit.PlacementId).Distinct(comparer: StringComparer.Ordinal).Count() != edits.Count)
        ) {
            reason = "seed edits must name distinct members of the bounded placement group";
            return false;
        }
        var editById = edits.ToDictionary(
            edit => edit.PlacementId,
            StringComparer.Ordinal
        );
        var offsetCount = WorldPlacementDeal.InstanceCount(
            template: template,
        ...
        var setupWork = (((((long)children.Length) * (offsetCount + 1L)) * (1 + System.Numerics.BitOperations.Log2(value: ((uint)Math.Max(
            val1: offsetCount,
            val2: 1
        ))))) +
            ((children.Sum(selector: child => ((long)(child.Spatial?.Count ?? 0))) + edits.Sum(selector: edit => ((long)(edit.Spatial?.Count ?? 0)))) * (offsetCount + 1L)));
        ...
        var placementsById = definition.Placements.ToDictionary(
            p => p.Id,
            StringComparer.Ordinal
        );
        ...
        for (var index = 0; (index < children.Length); index++) {
            ...
            if (editById.TryGetValue(
                key: child.Id,
                value: out var edit
            )) {
                if (
                    IsPinned(placement: child) &&
                    ((edit.Position is not null) || (edit.YawDegrees is not null) || (edit.Scale is not null))
                ) {
            ...
            if (!WorldDefinitionValidator.TryValidatePlacementGeometry(
                placement: child,
                reason: out reason
            )) { return false; }
            if (!HasReflowBounds(placement: child)) {
                reason = $"spatial member '{child.Id}' must retain occupation or clearance volumes after edits";
                return false;
            }
            var origin = FixedVector3.FromVector3(value: child.Position);

            options[index] = ((editById.ContainsKey(key: children[index].Id) || IsPinned(placement: child))
                ? [child]
                : [child, .. offsets
                .Where(predicate: offset => (offset != origin))
                .OrderBy(keySelector: offset => (offset - origin).LengthSquared)
                .Select(selector: offset => child with { Position = offset.ToVector3() })]
            );
        }
        var optionVolumes = options
            .Select(selector: optionSet => optionSet.Select(selector: option => WorldSpatialQueryCompilation.CompilePlacement(
            byId: placementsById,
            candidate: option
        )).ToArray())
            .ToArray();

        if (optionVolumes.Any(predicate: optionSet => optionSet.Any(predicate: volumes =>
            !volumes.Any(predicate: volume => (volume.Role is WorldPlacementSpatialRole.Occupation or WorldPlacementSpatialRole.Clearance))))) {
        ...
        foreach (var volumes in optionVolumes.SelectMany(selector: optionsForChild => optionsForChild)
            .Concat(second: children.Select(selector: child => WorldSpatialQueryCompilation.CompilePlacement(
            byId: placementsById,
            candidate: child
        )))) {
            foreach (var volume in volumes) {
                spatialRegion = spatialRegion.Enclose(box: volume.Bounds);
            }
        }
        var localRows = WorldDefinitionFingerprint.SpatialRows(
            definition: definition,
            region: spatialRegion
        );
        var localVolumeCount = localRows.Sum(selector: row => ((long)(row.Spatial?.Count ?? 0)));

        setupWork += IndexWork(count: localVolumeCount);
        ...
        var before = WorldSpatialQueryCompilation.Compile(placements: localRows);
        ...
        var fixedVolumes = before.Volumes.Where(predicate: volume => !selectedIds.Contains(item: volume.PlacementId)).ToArray();

        setupWork += IndexWork(count: fixedVolumes.Length);
        if (setupWork >= policy.CandidateBudget) { reason = "reflow exhausted its candidate budget preparing the search; no changes were made"; return false; }
        ...
        if (
            request.PreserveInfluenceCoverage &&
            !TryPrepareCoverage(
            before,
            optionVolumes.SelectMany(selector: set => set).SelectMany(selector: volumes => volumes),
            spatialRegion,
            ref work,
            policy.CandidateBudget,
            out coverageInputs
        )
        ) {
        ...
        bool Search(int index) {
            if (index == children.Length) {
                ...
                if (PreservesInfluenceCoverage(
                    fixedVolumes,
                    selectedVolumes,
                    coverageInputs,
                    ref work,
                    policy.CandidateBudget,
                    out var coverageFailure,
                    out var coverageBudgetExceeded
                )) { return true; }
            ...
            for (var optionIndex = 0; (optionIndex < options[index].Length); optionIndex++) {
                ...
                foreach (var volume in volumes) {
                    var overlaps = spatialIndex.HasBlockingOverlap(
                        candidate: volume,
                        workUnits: out var queryWork
                    );
                ...
                for (var previous = 0; (clear && (previous < index)); previous++) {
                    foreach (var left in volumes) {
                        foreach (var right in selectedVolumes[previous]) {
                            if (++work > policy.CandidateBudget) { exhausted = true; return false; }
                            if (WorldSpatialQueryIndex.Conflicts(
                                left: left,
                                right: right
                            )) { clear = false; break; }
                        }
                ...
                if (Search(index: (index + 1))) { return true; }
        ...
        if (!Search(index: 0)) {
            reason = (exhausted
                ? "reflow exhausted its candidate budget; no changes were made"
                : (coverageRejected
                    ? coverageReason
                    : "no arrangement satisfies the declared occupation and clearance volumes at the available offsets"
            ));
            return false;
        }
        ...
        try { cost = checked((policy.CostPerMove * moved)); } catch (OverflowException) { reason = "reflow price exceeds the Int range"; return false; }
        if (cost > 0) {
            if (
                ...
                GrantSubject.State(name: payerRow)
            )))
            ) {
            ...
            mutations.Add(item: new WorldMutation.UpsertStateCell(
                principal,
                row,
                (policy.CostKey ?? WorldStateRow.SlotKey.Value),
                -cost,
                WorldDocumentWriteKind.Add
            ));
            expectedCells = [new WorldStateExpectation(
                    row,
                    policy.CostKey,
                    cost,
        ...
        var dependencies = new HashSet<string>(comparer: StringComparer.Ordinal);

        foreach (var placement in localRows.Append(element: template)) {
            WorldStateDocumentValues.CollectReferencedRows(
                graph: placement,
                rows: dependencies
            );
            if (placement.Deal is not { } deal) { continue; }
            dependencies.Add(item: deal.Row);
            if (deal.Variants is { } variants) { dependencies.Add(item: variants.Row); }
        }
        var stateRows = dependencies.Order(comparer: StringComparer.Ordinal).ToArray();
        var placementIds = localRows.Append(element: template)
            .Select(selector: placement => placement.Id)
        ...
        proposal = new WorldPlacementProposal(
            ...
            AffectedIds = [.. children.Select(selector: child => child.Id)],
            Constraints = [$"members<=64", $"candidates<={policy.CandidateBudget}", (request.PreserveInfluenceCoverage
            ? "coverage cannot lose providers"
            : "coverage may change")],
        };
        return true;
    }
    private static bool HasReflowBounds(WorldPlacement placement) =>
        (placement.Spatial?.Any(predicate: volume => (volume?.Role is WorldPlacementSpatialRole.Occupation or WorldPlacementSpatialRole.Clearance)) == true);
    private static bool IsPinned(WorldPlacement placement) =>
        (placement.Spatial?.Any(predicate: volume => (volume?.Pinned == true)) == true);
    private static WorldPlacement? ReflowTemplate(WorldDefinition definition, WorldPlacementReflowRequest request) {
        var templateId = (request.TemplateId ?? ((request.PlacementIds is { Count: > 0 })
            ? definition.Placements.FirstOrDefault(predicate: placement => request.PlacementIds.Contains(
                placement.Id,
                StringComparer.Ordinal
            ))?.Parent
            : null));
    ...
    private static long IndexWork(long count) {
        var levels = (1 + System.Numerics.BitOperations.Log2(value: ((ulong)Math.Max(
            val1: count,
            val2: 1
        ))));
    ...
    private static bool TryPrepareCoverage(WorldSpatialQueryIndex before, IEnumerable<CompiledSpatialVolume> candidates,
        ...
        reads = [];
        var channels = before.Volumes.Concat(second: candidates)
            .Where(predicate: volume => (volume.Role == WorldPlacementSpatialRole.Influence))
            .Select(selector: volume => volume.Channel!).Distinct(comparer: StringComparer.Ordinal).Order(comparer: StringComparer.Ordinal).ToArray();
        var targets = before.Volumes.Where(predicate: volume => (volume.Role == WorldPlacementSpatialRole.Occupation))
            .DistinctBy(
            volume => volume.PlacementId,
            StringComparer.Ordinal
        )
            .Where(predicate: volume => region.Bounds.Intersects(other: new FixedSpatialAabb(
            Center: volume.Shape.Center,
            Extent: FixedVector3.Zero
        )))
            .Select(selector: volume => volume.PlacementId).Order(comparer: StringComparer.Ordinal).ToArray();
        var cost = ((((long)targets.Length) * channels.Length) * Math.Max(
            val1: before.Volumes.Count,
            val2: 1
        ));

        if (cost > (budget - work)) { return false; }
        work += ((int)cost);
        reads = [.. targets.SelectMany(selector: target => channels.Select(selector: channel =>
            new CoverageRead(
                target,
                channel,
                before.CountInfluences(
                    channel: channel,
                    placementId: target
                )
            )))];
    ...
    private static bool PreservesInfluenceCoverage(IReadOnlyList<CompiledSpatialVolume> fixedVolumes,
        ...
        var volumeCount = (fixedVolumes.Count + selectedVolumes.Sum(selector: volumes => ((long)volumes.Count)));
        var cost = (IndexWork(count: volumeCount) + (reads.Count * volumeCount));
        ...
        work += ((int)cost);
        var after = new WorldSpatialQueryIndex([.. fixedVolumes, .. selectedVolumes.SelectMany(selector: volumes => volumes)]);

        foreach (var read in reads) {
            if (after.CountInfluences(
                channel: read.Channel,
                placementId: read.PlacementId
            ) < read.Minimum) {
