...
public static partial class WorldDefinitionValidator {
    ...
    private static void ValidateHistory(WorldStateRow row, string path, List<string> errors) {
        ...
        if (
            (history.Capacity < 1) ||
            (history.Capacity > MaxHistoryCapacity)
        ) {
        ...
        if (
            (row.ValuesFrom is not null) ||
            (row.Phase is not null) ||
        ...
        if (row.ClampToEnvelope(value: history.Empty) != history.Empty) {
        ...
        if (
            (row.HistoryCursor < history.Capacity) &&
            (cells.Count > row.HistoryCursor)
        ) {
