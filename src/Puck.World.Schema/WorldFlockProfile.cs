...
public sealed record WorldFlockProfile(float Range, float SeparationRadius, int CandidateBudget, int MaxNeighbors,
    ...
    [property: JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingNull)] ExpressionProgram? CohesionAffinity = null,
    [property: JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingNull)] ExpressionProgram? AlignmentAffinity = null);
...
public sealed class FixedWorldFlockProfile {
    ...
    public FixedWorldFlockProfile(WorldFlockProfile source, WorldNavigationDomainTable? navigation = null) {
        ...
        MovementDomainIndex = ((source.MovementDomain is { } name)
            ? (((navigation is not null) && navigation.TryGetIndex(
                index: out var index,
                name: name
            ))
                ? index
                : throw new ArgumentException(
                    message: $"Flock movement domain '{name}' is not declared.",
                    paramName: nameof(source)
                ))
            : -1
        );
        Range = FixedQ4816.FromDouble(value: source.Range);
        ArrivalDistance = FixedQ4816.FromDouble(value: source.ArrivalDistance);
        MinimumDot = FixedQ4816.Cos(angle: FixedQ4816.FromDouble(value: (source.HalfAngleDegrees * (Math.PI / 180.0))));
        PeriodEngineTicks = FixedTickConversion.DurationEngineTicks(seconds: FixedQ4816.FromDouble(value: source.UpdateSeconds));
        Weights = new FixedFlockWeights(
            FixedQ4816.FromDouble(value: source.SeparationRadius),
            FixedQ4816.FromDouble(value: source.Separation),
            FixedQ4816.FromDouble(value: source.Alignment),
            FixedQ4816.FromDouble(value: source.Cohesion),
            FixedQ4816.FromDouble(value: source.Goal),
            FixedQ4816.FromDouble(value: source.Inertia)
        );
    }
