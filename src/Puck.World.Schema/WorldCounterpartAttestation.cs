...
public sealed record WorldOverlapTerms(
    ...
    public static bool TryDerive(WorldDefinition definition, out WorldOverlapTerms? terms, out string reason) {
        ...
        if (
            !WorldAdjacencyPolicy.TryBodyReach(
            definition: definition,
            reach: out var bodyReach,
            reason: out reason
        ) ||
...
/// <param name="Document">The resolved document the counterpart's own <paramref name="Destination"/> names, or
...
/// <param name="Hysteresis">The authored minimum ownership deadband.</param>
public sealed record WorldAttestedEdge(SafeName Name, string Counterpart, string Destination, string? Document, WorldAdjacencyBoundary Boundary, float Hysteresis = 0f);
/// <summary>
...
public sealed record WorldCounterpartAttestation(string Document, IReadOnlyList<WorldAttestedEdge> Edges, WorldOverlapTerms Overlap) {
    ...
    public static bool TryCompose(WorldDefinition definition, string document, out WorldCounterpartAttestation? attestation, out string reason) {
        ...
        foreach (var row in (definition.Adjacencies ?? [])) {
            ...
            edges.Add(item: new WorldAttestedEdge(
        ...
        attestation = new WorldCounterpartAttestation(
            Document: document,
            Edges: edges,
            Overlap: terms
        );
...
public static class WorldCounterpartAttestationProtocol {
    ...
    /// <param name="attestation">The attestation on success.</param>
    /// <param name="subject">The verified chain's own subject on success — the caller-side binding a resolver uses
    /// to refuse a validly-signed claim from the wrong owner (an authenticated statement, not a truth claim about
    /// the attested geometry — see the wire spec's own honesty framing); empty on failure.</param>
    /// <param name="reason">The named refusal on failure.</param>
    /// <param name="maximumAge">The verifier-side maximum claim age, or <see langword="null"/> for no ceiling
    /// beyond whatever window the issuer signed. A long-lived directed claim (this door's own kind — no per-claim
    /// nonce, no sequence) has no OTHER replay bound, so a caller reading a claim minted by a party it does not
    /// fully control (a cross-owner API fetch) should pass a short ceiling here rather than trust the issuer's own
    /// window alone.</param>
    ...
    public static bool TryVerify(
        ...
        try {
            attestation = JsonSerializer.Deserialize(
                utf8Json: payload,
                jsonTypeInfo: WorldJsonContext.Default.WorldCounterpartAttestation
            );
        ...
        if (!string.Equals(
            a: attestation.Schema,
            b: WorldCounterpartAttestation.SchemaVersion,
            comparisonType: StringComparison.Ordinal
        )) {
        ...
        subject = (claim.Header.Subject ?? string.Empty);
        reason = string.Empty;
