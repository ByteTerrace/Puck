...
public enum WorldGroupLifetime : byte {
    /// <summary>A runtime group of this kind auto-dissolves the moment <c>WorldMutation.LeaveGroup</c> or
    /// <c>WorldMutation.KickMember</c> empties it (only ever checked once it had at least one member — forming
...
/// <summary>What happens to a seated member's row on <c>WorldMutation.KickMember</c> — the kind decides, never
/// an engine default. Never consulted for <c>WorldMutation.LeaveGroup</c> (voluntary self-departure always
...
public enum OwnershipOwnerKind : byte {
    ...
    /// Entered only by <c>WorldMutation.OfferOwnership</c> and left only by
    /// <c>WorldMutation.SettleOwnership</c> (an accept by the named recipient, or a reclaim to the named
...
/// named party — the recovery-by-timeout half of the escrow/transfer lane (see <c>WorldMutation.SettleOwnership</c>).</summary>
...
/// refusal obligation this type and <c>WorldMutation.OfferOwnership</c>/<c>WorldMutation.SettleOwnership</c>
...
/// section), but its <see cref="Owner"/> moves live through <c>WorldMutation.OfferOwnership</c> (owner ->
/// escrow) and <c>WorldMutation.SettleOwnership</c> (escrow -> recipient, or escrow -> offerer on timeout) —
...
/// <c>WorldMutation.FormGroup</c> adds are wiped the same way every other live-only edit is — see
...
public sealed record WorldGroupsSection(
    ...
    public static WorldGroupsSection Empty { get; } = new(
        Groups: [],
        Kinds: [],
        Ownership: []
    );
