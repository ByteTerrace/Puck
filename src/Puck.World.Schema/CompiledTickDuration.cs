...
public readonly struct CompiledTickDuration : IEquatable<CompiledTickDuration> {
    ...
    public static readonly CompiledTickDuration Never = new(
        isNever: true,
        ticks: 0
    );

    /// <inheritdoc/>
    public bool Equals(CompiledTickDuration other) => (m_isNever
        ? other.m_isNever
        : (!other.m_isNever && (m_ticks == other.m_ticks))
    );
    /// <inheritdoc/>
    ...
    public static CompiledTickDuration FromTicks(int ticks) {
        ...
        return new CompiledTickDuration(
            isNever: false,
            ticks: ticks
        );
    }
    /// <inheritdoc/>
    public override int GetHashCode() => (m_isNever
        ? -1
        : m_ticks
    );
    /// <inheritdoc/>
    public override string ToString() => (m_isNever
        ? "never"
        : $"{m_ticks} ticks"
    );
