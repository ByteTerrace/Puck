...
public static partial class WorldRuleCompiler {
    ...
    private static CompiledWorldDecision? CompileDecision(WorldRule rule, WorldRuleCompileContext context) {
        ...
        if (rule.Mode != ActionTriggerMode.Level) { throw Refuse(detail: "requires Level rule mode"); }
        if (string.Equals(
            a: rule.ForEach,
        ...
        if (decision.IncumbentBonus < 0) { throw Refuse(detail: "incumbentBonus must be non-negative"); }
        ...
        for (var index = 0; (index < options.Length); index++) {
            ...
            if (option.Neighbors is { } source) {
                if (rule.ForEach is null) { throw Refuse(detail: "neighbors requires forEach body keys"); }
                if (
                    ...
                    throw Refuse(detail: "neighbors requires range [1/65536,1000000], halfAngleDegrees (0,180], and 1 <= maxCandidates <= min(32,candidateBudget), with candidateBudget within body capacity ceiling");
                }

                var range = FixedQ4816.FromRawBits(value: RuleCompiler.LiteralToRaw(
                    kind: CellKind.Fixed,
                ...
                var angle = FixedQ4816.FromRawBits(value: RuleCompiler.LiteralToRaw(
                    kind: CellKind.Fixed,
                    literal: radians,
                    ruleName: rule.Name,
                ...
                neighbors = new CompiledWorldDecisionNeighbors(
                    Source: source,
                    Range: range,
                    MinimumDot: FixedQ4816.Cos(angle: angle)
                );
            }
            ...
            try {
                options[index] = new CompiledWorldDecisionOption(
                    ...
                    Effects: CompileDecisionEffects(
