...
public static class WorldDefinitionRows {
    ...
    public static WorldStateRow? FindStateRow(IReadOnlyList<WorldStateRow>? rows, string name) => StateRows.FindStateRow(
        name: name,
        rows: rows
    );
    ...
    public static CompiledPlacementFrame ResolvedFrame(WorldDefinition definition, WorldPlacement placement) {
        ...
        return (definition.PlacementFrames.TryGetValue(
            key: placement.Id,
            value: out var frame
        )
            ? frame
            : new CompiledPlacementFrame(
    ...
    public static System.Numerics.Vector3 ResolvedPosition(WorldDefinition definition, WorldPlacement placement) => ResolvedFrame(
        definition: definition,
        placement: placement
    ).Position;
    /// <summary>Resolves a placement's composed world yaw, degrees (see <see cref="ResolvedFrame"/>).</summary>
    public static float ResolvedYawDegrees(WorldDefinition definition, WorldPlacement placement) => ResolvedFrame(
        definition: definition,
        placement: placement
    ).YawDegrees;
}
