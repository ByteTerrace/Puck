...
/// <see cref="WorldDefinitionValidator"/> defers the answer rather than refusing or silently passing — see
...
public static class WorldProbeVocabularyHook {
    ...
    /// <returns><see langword="true"/> when a kind is registered under that id; <see langword="false"/> when the
    /// installing host has a catalog and the id is not in it; <see langword="null"/> when the installing host
    /// carries no probe-kind catalog at all (the answer is deferred).</returns>
