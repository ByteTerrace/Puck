...
/// <param name="Revision">The instance-revision token: any change from what a currently-mounted guest was prepared
/// under means fresh guest instantiation (a clean reset to the module's initial state), even when every other field
/// is unchanged. NOT promised monotonic: <c>world.undo</c> and a whole-document load legitimately move it backward
/// (configuration undo, not time travel — the pre-undo guest's memory is not part of the document and is never
/// restored), while an author driving it forward is expected to increment with checked overflow so a wrapped value
/// can never alias an earlier revision. Defaults to <c>0</c>.</param>
...
/// WRAM-vs-SRAM shape). The peek provider registers when the server-owned machine host composes a screen's
/// machine, presentation and headless hosts alike (see <c>Server.WorldEventFeed</c>'s own remarks).</summary>
