...
public static partial class WorldDefinitionValidator {
    private static void ValidateStateDisclosure(WorldDefinition definition, List<string> errors) {
        ...
        foreach (var row in definition.State) {
            ...
            storage += row.Capacity ?? (row.EffectiveDomain is StateDomain.CellsOf board ? WorldTopologyCompilation.Find(definition, board.Topology)?.CellCount ?? row.CellCeiling : row.CellCeiling);
            ValidateDisclosureRow(definition, row, errors);
        }
        if (storage > 262_144) {
    ...
    private static void ValidateDisclosureRow(WorldDefinition definition, WorldStateRow row, List<string> errors) {
        ...
        if (row.Draw is { Secret: { } secret } draw && (secret.IsEmpty || row.Kind != CellKind.Int || !GeneratorEngine.TryResolveSource(definition.Generators, draw, out var generator, out _) || generator.Source != GeneratorSource.StreamDraw || generator.Mode != GeneratorMode.WithReplacement)) {
        ...
        if (row.EffectiveDomain is not StateDomain.CellsOf rowBoard || row.Visibility is null ||
            source?.EffectiveDomain is not StateDomain.CellsOf sourceBoard || mask?.EffectiveDomain is not StateDomain.CellsOf maskBoard ||
