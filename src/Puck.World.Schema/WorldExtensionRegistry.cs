...
public sealed class WorldExtensionRegistry<TExtension> where TExtension : class {
        ...
        foreach (var extension in extensions) {
            if (extension is null) {
                throw new ArgumentException(
                    message: $"A null {typeof(TExtension).Name} is registered.",
            ...
            if (string.IsNullOrWhiteSpace(value: key)) {
                throw new ArgumentException(
                    message: $"{Describe(extension: extension)} registers a null or blank {typeof(TExtension).Name} key.",
                    paramName: nameof(extensions)
                );
            }
            ...
            if (!m_byKey.TryAdd(
                ...
                throw new ArgumentException(
                    message: $"{Describe(extension: m_byKey[key: key])} and {Describe(extension: extension)} both register the {typeof(TExtension).Name} key '{key}'.",
                    paramName: nameof(extensions)
                );
            }
        }
    ...
    /// <returns><see langword="true"/> when the key is registered.</returns>
    /// <exception cref="ArgumentNullException"><paramref name="key"/> is <see langword="null"/>.</exception>
    public bool TryGet(string key, out TExtension extension) => m_byKey.TryGetValue(
        key: key,
        value: out extension!
    );
}
