...
public static partial class WorldDefinitionValidator {
    ...
    public static void ValidatePlacementSpatial(WorldPlacement placement, string path, List<string> errors) {
        ...
        var spatialNames = new HashSet<string>(comparer: StringComparer.Ordinal);
        ...
        for (var spatialIndex = 0; (spatialIndex < spatial.Count); spatialIndex++) {
            ...
            if (volume is null) {
                errors.Add(item: $"{spatialPath} is required.");
                continue;
            }
            if (!Enum.IsDefined(value: volume.Role)) { errors.Add(item: $"{spatialPath}.role is unsupported."); }
            if (string.IsNullOrWhiteSpace(value: volume.Name)) {
                errors.Add(item: $"{spatialPath}.name is required.");
            }
            if (!spatialNames.Add(item: volume.Name)) {
                errors.Add(item: $"{spatialPath}.name '{volume.Name}' is duplicated on placement '{placement.Id}'.");
            }
            if (volume.Shape is null) {
                errors.Add(item: $"{spatialPath}.shape is required.");
                continue;
            }
            if (
            ...
            if (!float.IsFinite(f: volume.Shape.YawDegrees)) {
                errors.Add(item: $"{spatialPath}.shape.yawDegrees must be finite.");
            }
            switch (volume.Shape.Kind) {
                case WorldSpatialShapeKind.Box:
                    if (
                        ...
                        RequirePositive(
                            volume.Shape.HalfExtents.X,
                            $"{spatialPath}.shape.halfExtents.x",
                            errors
                        );
                        RequirePositive(
                            volume.Shape.HalfExtents.Y,
                            $"{spatialPath}.shape.halfExtents.y",
                            errors
                        );
                        RequirePositive(
                            volume.Shape.HalfExtents.Z,
                            $"{spatialPath}.shape.halfExtents.z",
                            errors
                        );
                        RequireNavigationFixedPositive(
                            volume.Shape.HalfExtents.X,
                    ...
                    if (volume.Shape.Radius != 0f) {
                        errors.Add(item: $"{spatialPath}.shape.radius must be zero for a box.");
                    }
                    break;
                case WorldSpatialShapeKind.Sphere:
                    RequirePositive(
                        volume.Shape.Radius,
                        $"{spatialPath}.shape.radius",
                        errors
                    );
                    RequireNavigationFixedPositive(
                        volume.Shape.Radius,
                    ...
                    if (
                        (volume.Shape.HalfExtents is null) ||
                        (volume.Shape.HalfExtents.Value != Vector3.Zero)
                    ) {
                        errors.Add(item: $"{spatialPath}.shape.halfExtents must be [0, 0, 0] for a sphere.");
                    }
                    break;
                default:
                    errors.Add(item: $"{spatialPath}.shape.kind '{volume.Shape.Kind}' is unsupported.");
                    break;
            }
            if (volume.Role == WorldPlacementSpatialRole.Influence) {
                if (!CellName.TryParse(
                    candidate: volume.Channel,
                    name: out _,
                    reason: out _
                )) {
                    errors.Add(item: $"{spatialPath}.channel must be a valid cell name for an influence volume.");
                }
            } else if (volume.Channel is not null) {
                errors.Add(item: $"{spatialPath}.channel is only valid for influence volumes.");
            }
        }
    }
