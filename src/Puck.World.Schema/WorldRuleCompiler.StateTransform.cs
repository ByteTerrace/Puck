        ...
        switch (transform) {
            ...
            case StateTransform.Transfer transfer:
                ...
                if (transfer.Draw is { } drawName) {
                    ...
                    if (drawRow.Draw is not { Timing: not DrawTiming.Boot } draw || drawRow.Kind != CellKind.Int ||
                        !GeneratorEngine.TryResolveSource(generators: definition.Generators, draw: draw, generator: out var generator, reason: out _) || generator.Source != GeneratorSource.StreamDraw) {
            ...
            case StateTransform.Shuffle shuffle:
                if (Row(shuffle.Row) is not { IsKeyed: true } ||
                    ...
                    !GeneratorEngine.TryResolveSource(generators: definition.Generators, draw: shuffleDraw, generator: out var shuffleSource, reason: out _) ||
            ...
            case StateTransform.WriteSet writeSet:
                ...
                if (written.EffectiveDomain is not StateDomain.CellsOf writtenBoard || WorldTopologyCompilation.Find(definition, writtenBoard.Topology) is not { } writtenTopology ||
