                ...
                if (Row(sort.Row) is not { } sortedRow ||
                    (sortedRow.Zone is not null
                        ? (!sortedRow.Zone.Ordered || sort.Descending || sort.By is not { Count: >= 1 and <= WorldStateCapacity.MaxSortKeys } ||
                            sort.By.Any(key => key is null || Row(key.Row) is not { IsKeyed: true, Kind: CellKind.Int or CellKind.Fixed }))
                        : (!sortedRow.IsKeyed || sort.By is not null || sortedRow.Kind is not (CellKind.Int or CellKind.Fixed)))) {
                    throw Invalid($"sort requires an ordered zone with 1..{WorldStateCapacity.MaxSortKeys} keyed numeric attribute keys (direction per key), or a keyed numeric row alone");
                }
                break;
