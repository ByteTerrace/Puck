...
public static class WorldDefinitionValidator {
    ...
    private static void RequireSections(WorldDefinition definition) {
        ...
        Require(
            section: definition.SpawnPoints,
            name: "spawnPoints",
            missing: missing
        );
        Require(
            section: definition.Render,
            name: "render",
            missing: missing
        );
        Require(
            section: definition.Screens,
            name: "screens",
            missing: missing
        );
        Require(
            section: definition.Cameras,
            name: "cameras",
            missing: missing
        );
        Require(
            section: definition.PlayerDefaults,
            name: "playerDefaults",
            missing: missing
        );
        Require(
            section: definition.Channels,
            name: "channels",
            missing: missing
        );
        Require(
            section: definition.TargetRegisters,
            name: "targetRegisters",
            missing: missing
        );
        Require(
            section: definition.BodyMotionPrograms,
            name: "bodyMotionPrograms",
            missing: missing
        );
        Require(
            section: definition.Kits,
            name: "kits",
            missing: missing
        );
        Require(
            section: definition.DefaultSeatKit,
            name: "defaultSeatKit",
            missing: missing
        );
        Require(
        ...
        Require(
            section: definition.Addons,
            name: "addons",
            missing: missing
        );
        Require(
            section: definition.BindingOverlays,
            name: "bindingOverlays",
            missing: missing
        );
        Require(
            section: definition.Storage,
            name: "storage",
            missing: missing
        );
        Require(
            section: definition.Creations,
            name: "creations",
            missing: missing
        );
        Require(
            section: definition.Placements,
            name: "placements",
            missing: missing
        );
        Require(
            section: definition.Authoring,
            name: "authoring",
            missing: missing
        );
        Require(
            section: definition.Speakers,
            name: "speakers",
            missing: missing
        );
        Require(
            section: definition.Tunes,
            name: "tunes",
            missing: missing
        );
        Require(
            section: definition.Patches,
            name: "patches",
            missing: missing
        );
        Require(
            section: definition.Audio,
            name: "audio",
            missing: missing
        );
        Require(
            section: definition.Collision,
            name: "collision",
            missing: missing
        );
        Require(
            section: definition.Host,
            name: "host",
            missing: missing
        );
        Require(
            section: definition.Views,
            name: "views",
            missing: missing
        );
        Require(
            section: definition.Looks,
            name: "looks",
            missing: missing
        );
        Require(
            section: definition.LookAssignment,
            name: "lookAssignment",
            missing: missing
        );
        Require(
            section: definition.Links,
            name: "links",
            missing: missing
        );
        Require(
            section: definition.Grants,
            name: "grants",
            missing: missing
        );
        Require(
            section: definition.Hud,
            name: "hud",
            missing: missing
        );
        Require(
            section: definition.State,
            name: "state",
            missing: missing
        );
        ...
        Require(
            section: definition.InputHold,
            name: "inputHold",
            missing: missing
        );

        if (missing.Count > 0) {
            throw new InvalidOperationException(message: $"Incomplete WorldDefinition:{Environment.NewLine} - {string.Join(
    ...
    private static void ValidateActionStateSlot(ActionStateSlot state, string path, List<string> errors) {
        ...
        if (
            (state.Lifetime == ActionStateLifetime.Durable) &&
        ...
        if (string.IsNullOrWhiteSpace(value: listener)) {
            ...
            if (listener.StartsWith(
                ...
                if (
                    ...
                    (seat < 1) ||
                    (seat > WorldPopulationLimits.LocalSeatCount)
                ) {
    ...
    private static void ValidateBindingBar(WorldBindingBarAuthoring? authoring, string path, List<string> errors) {
        ...
        if (
            !float.IsFinite(f: authoring.HideAfterRestSeconds) ||
            (authoring.HideAfterRestSeconds < 0f)
        ) {
    ...
    private static void ValidateCore(WorldDefinition definition, IWorldNeighbourResolver? neighbours, bool validateAdjacencyClaims) {
        ...
        if (
            (definition.Population.Capacity < WorldPopulationLimits.LocalSeatCount) ||
        ...
        var peerCapacity = Math.Max(
            val1: 0,
            val2: (definition.Population.Capacity - WorldPopulationLimits.LocalSeatCount)
        );
    ...
    private static HashSet<string> ValidateCreations(IReadOnlyList<WorldCreation> creations, HashSet<string> fontNames, bool hasTextCatalog, List<string> errors) {
        var ids = new HashSet<string>(comparer: StringComparer.Ordinal);
    ...
    private static void ValidateFaceSources(IReadOnlyList<WorldPlacementFace>? faceSources, WorldPlacement placement, IReadOnlyList<WorldCreation> creations, WorldFaceCatalog faces, HashSet<string> destinationNames, HashSet<string> fontNames, bool hasTextCatalog, string path, List<string> errors) {
    ...
    private static void ValidateGrants(IReadOnlyList<WorldGrant> grants, HashSet<string> addonNames, HashSet<string> groupIds, int populationCapacity, List<string> errors) {
        ...
        for (var index = 0; (index < grants.Count); index++) {
            ...
            if (grant.Principal.Kind == PrincipalKind.World) {
                ...
                (((uint)(grant.Principal.Index - WorldPopulationLimits.LocalSeatCount)) >= ((uint)(populationCapacity - WorldPopulationLimits.LocalSeatCount)))
            ) {
    ...
    private static void ValidateHudCore(WorldHudSection hud, IReadOnlyDictionary<string, WorldStateRow> stateRows, bool isIdentityScope) {
        ...
        if (panels.Count > WorldHudCapacity.MaxWorldPanels) {
            throw new HudValidationException(
                ...
                message: $"hud.panels count {panels.Count} exceeds the maximum of {WorldHudCapacity.MaxWorldPanels}."
            );
        }
    ...
    private static void ValidateMagazine(WorldScreenMagazine? magazine, string path, HashSet<string> cameras, HashSet<string> destinationNames, HashSet<string> fontNames, bool hasTextCatalog, List<string> errors) {
    ...
    private static bool ValidateScreenSource(WorldScreenSource source, string path, HashSet<string> cameras, HashSet<string> destinationNames, HashSet<string> fontNames, bool hasTextCatalog, List<string> errors) {
        switch (source) {
            case null:
                errors.Add(item: $"{path} is required.");
    ...
    private static void ValidateSessionWindowBudget(IReadOnlyList<WorldPlacement> placements, List<string> errors) {
        ...
        if (windows.Count > WorldSessionWindowCapacity.MaxSimultaneousWindows) {
            errors.Add(item: $"placements author {windows.Count} 'window'-projection session face(s) ({string.Join(
                separator: ", ",
                values: windows
            )}), exceeding the simultaneous-window budget of {WorldSessionWindowCapacity.MaxSimultaneousWindows} — a window renders every produced frame regardless of whether anyone is looking through it, so this ceiling is a real GPU cost bound, not a taste one. Drop a window to a plain 'camera' projection or remove a face.");
        }
    }
    ...
    private static void ValidateTextSource(WorldScreenSource.Text text, string path, HashSet<string> fontNames, bool hasTextCatalog, List<string> errors) {
        if (!hasTextCatalog) {
        ...
        if (
            ...
            errors.Add(item: $"{path}.text.foreground must be #RRGGBB.");
        }
        ...
        if (
            ...
            errors.Add(item: $"{path}.text.background must be #RRGGBB.");
        }
    }
