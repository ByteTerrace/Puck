...
public readonly record struct WorldInteractionBound(long Lefts, long Rights, long Evaluations, RuleWork Setup) {
    ...
    // A side holds no more carriers than its tag row has cells or the population has bodies. A name that resolves
    // to no document row gathers nothing.
    ...
    public static WorldInteractionBound Of(CompiledInteraction interaction, WorldFactsCompileContext context) {
        ...
        // A body never pairs with itself. A left drawn from the row the rights are drawn from is one of them, so
        // it has one partner fewer; from another row it may be none of them, and has at most every other body.
