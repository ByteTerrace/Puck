...
public sealed record WorldRenderDefaults(
    ...
    public static WorldRenderDefaults Default { get; } = new WorldRenderDefaults(
        ...
        UpscaleSharpness: 0f,
        Low: new WorldQualityPreset(
            AmbientOcclusion: false,
            RenderScale: WorldRenderScaleTier.Half,
            Shadows: ShadowTier.Off
        ),
        Medium: new WorldQualityPreset(
            AmbientOcclusion: true,
            RenderScale: WorldRenderScaleTier.ThreeQuarter,
            Shadows: ShadowTier.Medium
        ),
        High: new WorldQualityPreset(
            AmbientOcclusion: true,
...
/// <c>WorldMutation.UpsertStateRow</c>/<c>WorldMutation.RemoveStateRow</c>, or — for
/// a per-cell write only —
/// <c>WorldMutation.UpsertStateCell</c>/<c>WorldMutation.RemoveStateCell</c>; the
...
/// <param name="Text">The optional named, hash-pinned font catalog shared by creation text runs and screen text
/// decals.</param>
/// <param name="Metadata">The optional <c>metadata</c> section — free-form author-facing facts (title, description,
/// authors, tags, a <c>custom</c> bag). Nothing here is read by the engine; see <see cref="WorldMetadataSection"/>.</param>
