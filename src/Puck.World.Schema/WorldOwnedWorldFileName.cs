...
/// catalog, and in the cloud as the object key derived from the same name. The mapping takes a <see cref="SafeName"/>
...
/// <see cref="SafeName"/>s addressing one local file (a cloud object namespace is case-SENSITIVE, so the same
...
public static class WorldOwnedWorldFileName {
    ...
    public static string For(SafeName id) => $"{id.Value}{Suffix}";
}
