...
/// <summary>An identity-door refusal — separate from <c>WorldHelloRefusal</c> by construction (see that
...
public static class WorldAdmissionDoor {
    ...
    private static TrustList BuildTrustList(IReadOnlyList<WorldAdmissionEntry> entries, TimeSpan? defaultMaximumAge) {
        ...
        foreach (var entry in entries) {
            ...
            var pinnedId = new KeyId {
                ...
                Subject = ((entry.Mode == WorldAdmissionTrustMode.Vouches)
                ? null
                : entry.Subject),
            };
            ...
            list.Add(item: new TrustListEntry(
                ...
                Mode: ((entry.Mode == WorldAdmissionTrustMode.Vouches)
    ...
    public static SignedAttestation DecodeAttestation(ReadOnlySpan<byte> wire) => Profile.DecodeAttestation(
        codec: Codec,
        wire: wire
    );
    ...
    public static AdmissionOutcome TryAdmit(
        ...
        if (
            ...
            return AdmissionOutcome.Refuse(
                ...
                refusal: WorldAdmissionRefusal.NoAdmissionEntries
            );
        }
        ...
        if (!TryBuildTrustList(
            entries: rows,
            reason: out var trustReason,
            trustList: out var trustList
        )) {
            return AdmissionOutcome.Refuse(
                detail: trustReason,
                refusal: WorldAdmissionRefusal.ChainRefused
            );
        }
        ...
        if (
            ...
            return AdmissionOutcome.Refuse(
                WorldAdmissionRefusal.ChainRefused,
        ...
        if (
            ...
            return AdmissionOutcome.Refuse(
                detail: "the signed claim does not carry the exact challenge this connection was issued",
                refusal: WorldAdmissionRefusal.ChallengeMismatch
            );
        }
        ...
        if (!TryMatchEntry(
            ...
            entries: rows,
            subject: subject,
            verdict: out var matched
        )) {
            return AdmissionOutcome.Refuse(
                detail: $"claim verified for domain '{domain}' subject '{(subject ?? "(none)")}', but no authored admission entry names it",
                refusal: WorldAdmissionRefusal.NoMatchingEntry
            );
        }
    ...
    /// <c>Puck.Networking.IAuthenticator</c>'s signed-claim challenge/proof handshake for <paramref name="sourceAuthority"/>, or be
    ...
    public static WorldAdmissionRefusal? TryAdmitArrival(IReadOnlyList<WorldAdmissionEntry>? entries, string sourceAuthority, out WorldAdmissionVerdict? verdict) {
        ...
        foreach (var entry in rows) {
            ...
            if (string.Equals(
                ...
                verdict = new WorldAdmissionVerdict(
                    identityDomain: sourceAuthority,
                    identitySubject: string.Empty,
                    templates: entry.Grants,
                    tier: entry.Tier
                );
            ...
            wildcard ??= (string.Equals(
                a: entry.Domain,
                b: WorldAdmissionEntry.AnyAuthority,
                comparisonType: StringComparison.Ordinal
            )
                ? entry
                : null
    ...
    /// <param name="defaultMaximumAge">The verifier-side maximum claim age applied to every entry that does not
    /// override it, or <see langword="null"/> for no ceiling beyond the issuer's own signed window (the connection
    /// door's own posture — a fresh per-connection nonce already bounds replay independent of claim age).</param>
    /// <returns><see langword="true"/> when the entries form a valid trust list with at least one key-bearing row.</returns>
    public static bool TryBuildTrustList(IReadOnlyList<WorldAdmissionEntry>? entries, out TrustList? trustList, out string reason, TimeSpan? defaultMaximumAge = null) {
        trustList = null;
    ...
    public static bool TryMatchEntry(IReadOnlyList<WorldAdmissionEntry>? entries, string? domain, string? subject, [System.Diagnostics.CodeAnalysis.NotNullWhen(returnValue: true)] out WorldAdmissionVerdict? verdict) {
        if (entries is { Count: > 0 } rows) {
            foreach (var entry in rows) {
                ...
                var matchesVouching = ((entry.Mode == WorldAdmissionTrustMode.Vouches) && string.Equals(
                    a: entry.Domain,
                    b: domain,
                    comparisonType: StringComparison.Ordinal
                ));
                ...
                if (
                    ...
                    verdict = new WorldAdmissionVerdict(
                        identityDomain: (domain ?? string.Empty),
                        identitySubject: (subject ?? string.Empty),
                        templates: entry.Grants,
                        tier: entry.Tier
                    );
