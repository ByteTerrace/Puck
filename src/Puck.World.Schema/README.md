...
unchanged; an `n`-in-a-row check over a 4x4x4 cube is one 64-bit mask, or a
run-length pattern read with `$match:`.
...
[<cell>]` lists every element and a cell's image under each. `mapBoard` (`target`,
