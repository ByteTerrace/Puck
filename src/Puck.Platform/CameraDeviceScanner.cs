...
public sealed class CameraDeviceScanner : IDisposable {
    ...
    public CameraDeviceScanner(ICameraCaptureService service, TimeSpan interval) {
        ...
        if (
            !double.IsFinite(d: ticks) ||
            (ticks < 1) ||
            (ticks >= long.MaxValue)
        ) {
            throw new ArgumentOutOfRangeException(paramName: nameof(interval));
        }
        m_service = service;
    ...
    private static Task<CameraDeviceScanResult> StartScan(ICameraCaptureService service) => Task.Run(function: () => Scan(service: service));
    ...
    public bool TryPoll(long timestamp, out CameraDeviceScanResult result) {
        ...
        if (m_pending is { } pending) {
            ...
            m_pending = null;
            m_nextTimestamp = ((timestamp > (long.MaxValue - m_intervalTicks))
                ? long.MaxValue
                : (timestamp + m_intervalTicks)
            );
            return true;
        }
        if (timestamp < m_nextTimestamp) { return false; }
        m_pending = StartScan(service: m_service);
        return false;
    }
}
