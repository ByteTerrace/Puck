...
public struct ProbeAxisConditioner {
    ...
    public ProbeAxisConditioner(ProbeAxisPolicy policy) {
        ...
        ArgumentOutOfRangeException.ThrowIfGreaterThan(
            value: policy.Hysteresis,
            other: policy.Deadband
        );
        ArgumentOutOfRangeException.ThrowIfGreaterThanOrEqual(
            value: (policy.Deadband + policy.Hysteresis),
    ...
    private static FixedQ4816 Normalize(FixedQ4816 raw, in ProbeAxisPolicy policy) {
        ...
        return ((lowerSpan > FixedQ4816.Zero)
            ? -FixedQ4816.Clamp(
                value: ((policy.Neutral - raw) / lowerSpan),
                minimum: FixedQ4816.Zero,
                maximum: FixedQ4816.One
            )
            : FixedQ4816.Zero
    ...
    private static FixedQ4816 Quantize(FixedQ4816 value, long quantumRaw) {
        ...
        quantized = Math.Clamp(
            value: quantized,
            min: FixedQ4816.NegativeOne.Value,
    ...
    private FixedQ4816 Smooth(FixedQ4816 gated) {
        if (
            !m_hasSmoothed ||
            (m_policy.Smoothing == FixedQ4816.Zero)
        ) {
            m_hasSmoothed = true;
