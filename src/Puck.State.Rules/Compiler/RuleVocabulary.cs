...
/// <summary>One family of reserved operand spellings — a document project registers one per prefix it answers, and
/// the compiler consults the registered families before its own.</summary>
public abstract class OperandFamily {
    /// <summary>Gets the spellings this family answers, as an author would read them in a refusal, for the refusal
    /// that lists every reserved channel.</summary>
...
/// <remarks>Every firing is one journal scope, so an arm declares nothing about transactions or branches. An arm
/// whose work cannot be rewound declares <see cref="EffectNeeds.Irreversible"/> on the compiled effect instead, and
/// the compiler defers it to commit.</remarks>
public abstract class EffectFamily {
    ...
    public abstract IRuleEffect Compile(ActionEffect effect, string ruleName, RuleCompileContext context);
}
/// <summary>One authored predicate arm a document project owns: the record type and its discriminator, and its
/// compile — which may refuse, for an arm that is authorable only inside another program the same JSON shape
/// serves.</summary>
public abstract class PredicateFamily {
    ...
    /// <param name="ruleName">The rule being compiled.</param>
    /// <param name="context">The compile context.</param>
    /// <returns>The compiled token.</returns>
...
public sealed class RuleVocabulary {
    /// <summary>Gets the library's own vocabulary with nothing registered.</summary>
    ...
    /// <summary>Finds the effect family owning an authored effect's type.</summary>
    /// <param name="effect">The effect.</param>
    /// <returns>The family, or <see langword="null"/>.</returns>
    ...
    /// <see cref="ActionEffect"/> and <see cref="ActionPredicate"/> read and write the registered arms under their
    ...
    /// <summary>Finds the predicate family owning an authored predicate's type.</summary>
    /// <param name="predicate">The predicate.</param>
    /// <returns>The family, or <see langword="null"/>.</returns>
