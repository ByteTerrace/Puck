...
public interface IVulkanFramePresentationApi {
    ...
    /// <returns>A <see cref="VkResult"/> indicating whether the submission succeeded.</returns>
    VkResult Submit(VulkanDeviceCommands device, nint graphicsQueueHandle, nint commandBufferHandle, nint fenceHandle);
    ...
    bool SupportsPresentWait(VulkanDeviceCommands device);
    ...
    VkResult WaitForPresent(VulkanDeviceCommands device, nint swapchainHandle, ulong presentId, ulong timeoutNanoseconds);
}
