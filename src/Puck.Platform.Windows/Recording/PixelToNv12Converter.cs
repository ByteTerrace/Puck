...
internal static class PixelToNv12Converter {
    ...
    private static void WriteChromaPlane(ReadOnlySpan<byte> pixels, bool blueFirst, int width, int height, int chromaWidth, int chromaHeight, Span<byte> chromaPlane) {
        for (var cy = 0; (cy < chromaHeight); cy++) {
            ...
            for (var cx = 0; (cx < chromaWidth); cx++) {
                ...
                chromaPlane[index] = ((byte)Math.Clamp(
                    max: 255,
                    min: 0,
                    value: ((((((CbR * r) + (CbG * g)) + (CbB * b)) + CBias) + Half) >> Shift)
                ));
                chromaPlane[(index + 1)] = ((byte)Math.Clamp(
                    max: 255,
                    min: 0,
                    value: ((((((CrR * r) + (CrG * g)) + (CrB * b)) + CBias) + Half) >> Shift)
                ));
            }
        }
    }
    private static void WriteLumaPlane(ReadOnlySpan<byte> pixels, bool blueFirst, int width, int height, Span<byte> yPlane) {
        for (var row = 0; (row < height); row++) {
            var sourceRow = pixels.Slice(
                length: (width * 4),
                start: ((row * width) * 4)
            );
            var destinationRow = yPlane.Slice(
                length: width,
                start: (row * width)
            );
            var column = 0;
    ...
    private static int WriteLumaRowAvx2(ReadOnlySpan<byte> sourceRow, bool blueFirst, int width, Span<byte> destinationRow) {
        ...
        var kr = Vector256.Create(value: (blueFirst
            ? YB
            : YR));
        var kg = Vector256.Create(value: YG);
        var kb = Vector256.Create(value: (blueFirst
            ? YR
            : YB));
        ...
        for (; (column < vectorWidth); column += 8) {
            var block = MemoryMarshal.Read<Vector256<byte>>(source: sourceRow.Slice(
                length: 32,
                start: (column * 4)
            ));
            // Lane i (int32) holds byte4*i (channel 0), channel 1, channel 2 via masked shifts.
            var packed = block.AsInt32();
            var c0 = Vector256.BitwiseAnd(
                left: packed,
                right: Vector256.Create(value: 0x000000FF)
            );
            var c1 = Vector256.BitwiseAnd(
                left: Vector256.ShiftRightLogical(
                    shiftCount: 8,
                    vector: packed
                ),
                right: Vector256.Create(value: 0x000000FF)
            );
            var c2 = Vector256.BitwiseAnd(
                left: Vector256.ShiftRightLogical(
                    shiftCount: 16,
                    vector: packed
                ),
            ...
            var y = Vector256.Add(
                left: Vector256.Add(
                    left: Vector256.Multiply(
                        left: c0,
                        right: kr
                    ),
                    right: Vector256.Multiply(
                        left: c1,
                        right: kg
                    )
                ),
            ...
            y = Vector256.ShiftRightArithmetic(
                shiftCount: Shift,
                vector: y
            );
    ...
    public static void Convert(ReadOnlySpan<byte> pixels, GpuPixelFormat format, int width, int height, Span<byte> destination) {
        ...
        var chromaPlane = destination.Slice(
            length: ((2 * chromaWidth) * chromaHeight),
            start: ySize
        );
