...
internal static class Av1Bitstream {
    ...
    public static bool? TryReadIsKeyFrame(ReadOnlySpan<byte> temporalUnit) {
        ...
        while (TryReadObu(index: ref index, obuType: out var obuType, payloadLength: out var payloadLength, payloadOffset: out var payloadOffset, temporalUnit: temporalUnit)) {
    ...
    public static byte[] BuildConfigRecord(ReadOnlySpan<byte> temporalUnit) {
        if (!TryFindSequenceHeaderObu(obu: out var sequenceHeaderObu, payload: out var payload, temporalUnit: temporalUnit)) {
            return [];
        }
    ...
    private static bool TryFindSequenceHeaderObu(ReadOnlySpan<byte> temporalUnit, out byte[] obu, out byte[] payload) {
        ...
        while (true) {
            ...
            if (!TryReadObu(index: ref index, obuType: out var obuType, payloadLength: out var payloadLength, payloadOffset: out var payloadOffset, temporalUnit: temporalUnit)) {
                break;
            }

            if (obuType == ObuSequenceHeader) {
                obu = temporalUnit.Slice(length: (index - start), start: start).ToArray();
                payload = temporalUnit.Slice(length: payloadLength, start: payloadOffset).ToArray();
    ...
    private static SequenceHeaderFields ParseSequenceHeader(ReadOnlySpan<byte> payload) {
        ...
        ParseColorConfig(fields: ref fields, reader: ref reader, seqProfile: fields.SeqProfile);
