...
public sealed class XInputGamepadConnection : IGamepadConnection {
    ...
    private static float NormalizeIntensity(float intensity) =>
        (float.IsFinite(f: intensity) ? Math.Clamp(max: 1f, min: 0f, value: intensity) : 0f);
    ...
    private static GamepadState Convert(in XInputGamepad pad) {
        ...
        return new GamepadState(
            Buttons: buttons,
            Gyro: Vector3.Zero,
            LeftStick: NormalizeStick(deadzone: LeftThumbDeadzone, rawX: pad.ThumbLeftX, rawY: pad.ThumbLeftY),
            LeftTrigger: GamepadNormalization.NormalizeTrigger(range: byte.MaxValue, raw: pad.LeftTrigger, threshold: TriggerThreshold),
            Orientation: Quaternion.Identity,
            RightStick: NormalizeStick(deadzone: RightThumbDeadzone, rawX: pad.ThumbRightX, rawY: pad.ThumbRightY),
            RightTrigger: GamepadNormalization.NormalizeTrigger(range: byte.MaxValue, raw: pad.RightTrigger, threshold: TriggerThreshold)
        );
    }
    private static Vector2 NormalizeStick(short rawX, short rawY, short deadzone) {
        ...
        return GamepadNormalization.ApplyRadialDeadzone(deadzone: normalizedDeadzone, stick: stick);
    }
