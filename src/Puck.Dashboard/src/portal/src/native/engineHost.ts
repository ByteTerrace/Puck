...
import type { EngineCore, EngineHostOptions, WorldEngine } from "./engineTypes";
 ...
 * JSON-stringified), so a `bigint` `tick`/`value` argument crosses intact. The language server's `lsp` and
 * `lspIdle` are ordinary calls; the pump that drives them runs on the page's side. */
export type WorkerRequest =
  | { kind: "init"; engineEntryUrl: string }
  | { kind: "boot"; engineFiles: Readonly<Record<string, { url: string; hash: string }>>; wasmModule?: WebAssembly.Module }
  | { kind: "dispose" }
  | { kind: "call"; id: number; method: keyof EngineCore; args: unknown[] };
...
export type WorkerResponse =
  | { kind: "ready"; wasmModule?: WebAssembly.Module }
...
export async function createEngineHost(options: EngineHostOptions): Promise<WorldEngine> {
  if (options.mode === "inline") return withLanguageServer(await createInlineWorldEngine(options));
