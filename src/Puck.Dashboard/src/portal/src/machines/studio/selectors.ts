...
export const selectCanUndo = (snapshot: Snapshot): boolean => snapshot.context.document.past.length > 0;
export const selectCanRedo = (snapshot: Snapshot): boolean => snapshot.context.document.future.length > 0;
export const selectIsDirty = (snapshot: Snapshot): boolean =>
  snapshot.context.document.revision !== snapshot.context.document.cleanRevision;
