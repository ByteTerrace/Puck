...
public static class StubHealth {
    ...
    public static string FilePath(string installRoot) =>
        Path.Combine(
            path1: installRoot,
            path2: StubInstall.HealthDirectoryName,
            path3: StubInstall.HealthFileName
        );
    ...
    public static void IncrementAndFlush(string installRoot, string version) {
        var attempts = (AttemptsFor(
            record: Read(installRoot: installRoot),
            version: version
        ) + 1);

        Write(
            installRoot: installRoot,
            record: new StubHealthRecord(
                Attempts: attempts,
                Version: version
            )
        );
    ...
    public static void Write(string installRoot, StubHealthRecord record) {
        ...
        File.WriteAllText(
            path: tmpPath,
            contents: JsonSerializer.Serialize(value: record)
        );
        File.Move(
            sourceFileName: tmpPath,
            destFileName: FilePath(installRoot: installRoot),
            overwrite: true
        );
    }
}
