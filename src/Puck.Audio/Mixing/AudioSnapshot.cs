...
/// <see cref="AudioEmitter.FadeFrames"/>.</summary>
public enum AudioEmitterKind {
...
public enum AudioAttenuationCurve : byte {
...
public enum AudioSourceKind {
    ...
    Tune,
    /// <summary>The world voice synth, identified by patch id; voices arrive via <see cref="SynthTrigger"/>.</summary>
    Synth,
}
...
public readonly record struct AudioSourceKey(AudioSourceKind Kind, int Slot, string? Id) {
    ...
    public static AudioSourceKey Machine(int slot) => new(
        Id: null,
    ...
    /// <returns>The key.</returns>
    public static AudioSourceKey Synth(string patchId) => new(
        Id: patchId,
        Kind: AudioSourceKind.Synth,
        Slot: 0
    );
    /// <summary>Creates a tune-source key.</summary>
    /// <param name="id">The tune id.</param>
    /// <returns>The key.</returns>
    public static AudioSourceKey Tune(string id) => new(
        Id: id,
        Kind: AudioSourceKind.Tune,
        Slot: 0
    );

    /// <summary>The silent source.</summary>
    public static AudioSourceKey None => new(
        Id: null,
...
public readonly record struct AudioListener(FixedVector3 Position, FixedComplex Yaw);
...
public readonly record struct AudioEmitter(
    ...
    AudioAttenuationCurve Curve,
    int FadeFrames,
    int GainQ16,
    AudioChannel Channel,
    AudioSourceKey Source
);
...
/// <see cref="AudioSourceKind.Synth"/>).</param>
public readonly record struct SynthTrigger(ulong Sequence, string PatchId, ulong Seed, int GainQ16, int EmitterId);
/// <summary>
/// The immutable per-frame record the mixer consumes: listener pose, a fixed-capacity emitter table, and seeded
/// synth trigger events. Built for the <c>PublishBuffer&lt;AudioSnapshot&gt;</c> slab-rotation contract:
...
/// The host's audio director is the live publisher; a proof scripts this builder directly.
/// </summary>
public sealed class AudioSnapshot {
    ...
    private readonly AudioEmitter[] m_emitters;
    private readonly SynthTrigger[] m_triggers;
    ...
    public AudioSnapshot(int maxEmitters = DefaultMaxEmitters, int maxTriggers = DefaultMaxTriggers) {
        ...
        m_emitters = new AudioEmitter[maxEmitters];
        m_triggers = new SynthTrigger[maxTriggers];
    }

    /// <summary>Gets the emitter table.</summary>
    public ReadOnlySpan<AudioEmitter> Emitters => m_emitters.AsSpan(
    ...
    public ReadOnlySpan<SynthTrigger> Triggers => m_triggers.AsSpan(
    ...
    public void Reset(in AudioListener listener) {
        Listener = listener;
        m_emitterCount = 0;
        m_triggerCount = 0;
    }
    ...
    public bool TryAddEmitter(in AudioEmitter emitter) {
    ...
    public bool TryAddTrigger(in SynthTrigger trigger) {
