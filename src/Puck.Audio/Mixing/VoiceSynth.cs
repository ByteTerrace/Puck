...
/// stream). The document-side oscillator enum mirrors these ordinals; the two are separate types (Puck.Audio
/// parses no document) and the host's patch factory maps between them.</summary>
public enum SynthOscillator {
    /// <summary>A rectangular pulse wave; <see cref="VoicePatch.DutyThresholdQ32"/> sets its duty cycle.</summary>
    Pulse,
    ...
    Sine,
    /// <summary>A seeded pseudo-random noise stream; <see cref="VoicePatch.Polynomial"/> selects its
    /// character.</summary>
    Noise,
}
...
public readonly record struct VoicePatch(
...
public sealed class VoiceSynth {
    ...
    private readonly int[] m_monoAccumulator = new int[AudioMixer.MaxBlockFrames];
    ...
    internal void ReleaseUnbound(ReadOnlySpan<AudioEmitter> emitters) {
        for (var i = 0; (i < VoiceCount); i++) {
    ...
    private static int ApplyFilter(ref Voice voice, int sample) {
        if (voice.Patch.FilterMode == VoiceFilterMode.Bypass) {
            return sample;
        }
        ...
        return (voice.Patch.FilterMode switch {
            VoiceFilterMode.LowPass => voice.FilterLow,
            VoiceFilterMode.BandPass => voice.FilterBand,
            _ => high,
        });
    }
    ...
    /// <param name="frames">The frame count (at most <see cref="AudioMixer.MaxBlockFrames"/>).</param>
    public void Render(Span<short> destination, int frames) {
        ArgumentOutOfRangeException.ThrowIfGreaterThan(
            value: frames,
            other: AudioMixer.MaxBlockFrames
        );
    ...
    public int Trigger(in VoicePatch patch, ulong seed, int gainQ16, int emitterId = -1) {
