...
public sealed class AgbDirectSound {
    ...
    private void EmitVoiceMix(int voice) {
        ...
        m_emitter.LoadConstant(
            ...
            value: (VoiceAddress + ((uint)(voice * VoiceByteCount)))
        );
        m_emitter.LoadWord(
        ...
        m_emitter.Branch(
            condition: ThumbCondition.UnsignedHigher,
            label: retire
        );
        m_emitter.Branch(
            condition: ThumbCondition.Equal,
        ...
        m_emitter.Branch(
            condition: ThumbCondition.SignedLess,
            label: low
        );
    ...
    public void EmitBoot() {
        ...
        StoreWord(
            address: DmaSourceAddress,
            value: BufferAddress
        );
        StoreWord(
            address: DmaDestinationAddress,
            value: QueueAddress
        );
        // Enable, queue-request start, word width, repeating, destination fixed on the queue register.
        StoreHalf(
            address: DmaControlAddress,
            value: 0xB640u
        );
        StoreHalf(
            address: TimerReloadAddress,
            value: ((uint)(65536 - CyclesPerSample))
        );
        StoreHalf(
            address: TimerControlAddress,
            value: 0x0080u
        );
    }
