...
objects are 8×8 sprites. Rule arithmetic wraps modulo 256.
...
## Direct boot and BIOS ownership

The document compiler targets direct boot: no SWIs, no IRQ dispatch, no BIOS
dependency. `forge.play` explicitly supplies the engine's `stub` option.
The default header logo is zeroed because direct boot does not validate it.
