...
/// <summary>
/// A bounded, thread-safe pool of parked <see cref="MachineInstance{TMachine, TConfiguration}"/> siblings for a single
/// fork lineage — all built from one configuration and composition. A fork rents a parked instance (or builds one when
/// the pool is empty) and restores fresh state into it, sidestepping the container build; a disposed fork returns here
/// rather than tearing its container down. A parked instance is indistinguishable from a fresh one because a full state
/// restore replaces every byte of emulated state, and each instance owns its own DI container, so no component ever
/// aliases another machine or carries a subscription across a reuse.
/// </summary>
...
internal sealed class MachineInstancePool<TMachine, TConfiguration> : IDisposable
    ...
    internal MachineFork<TMachine, TConfiguration> Rent() {
        lock (m_gate) {
    ...
    internal void Return(MachineInstance<TMachine, TConfiguration> instance, int generation) {
        var release = false;
    ...
    public void Dispose() {
        MachineInstance<TMachine, TConfiguration>[] drained;
