...
public readonly record struct MachineBootOptions(MachineBootMode Mode, string? ImagePath) {
    ...
    public static MachineBootOptions Parse(string? options, out string[] machineTokens, MachineBootOptions? defaults = null) {
        ...
        var imageStart = remaining.IndexOf(
        ...
        if (
            (imageStart >= 0) &&
            ((imageStart == 0) || char.IsWhiteSpace(c: remaining[(imageStart - 1)]))
        ) {
            ...
            if (string.IsNullOrWhiteSpace(value: path)) {
                throw new ArgumentException(
                    message: "bios= requires an image path or puck.",
        ...
        foreach (var token in remaining.Split(
            ...
            if (parsed is null) {
                ...
                throw new ArgumentException(
                    message: "Choose cold or fast startup, not both.",
                    paramName: nameof(options)
                );
            } else {
                mode = parsed;
        ...
        return new(
            Mode: (mode ?? (defaults?.Mode ?? MachineBootMode.Cold)),
            ImagePath: path
        );
    }
}
