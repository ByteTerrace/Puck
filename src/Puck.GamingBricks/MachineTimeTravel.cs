...
public sealed class MachineTimeTravel<TInput> : IDisposable where TInput : unmanaged {
    ...
    private void EnsureRing(int baseLength) {
        ...
        var perSpan = Math.Max(
            val1: (baseLength + PerFrameRecordBytes()),
            val2: 1L
        );

        if (m_budgetBytes < perSpan) {
            throw new InvalidOperationException(message: $"The rewind memory budget ({m_budgetBytes} B) cannot hold a single {perSpan} B keyframe span; raise memoryBudgetBytes or lower keyframeIntervalFrames.");
        }

        var capacity = ((int)Math.Clamp(
            max: 4096L,
            min: 1L,
            value: (m_budgetBytes / perSpan)
        ));
    ...
    private void Locate(long target, out int slot, out int frameIndex) {
        ...
        for (var s = 0; (s < m_segCount); ++s) {
            ...
            if (
                (segment.BaseNativeFrame >= target) &&
                (segment.BaseNativeFrame < current)
            ) {
                slot = index;
                frameIndex = 0;
            ...
            for (var i = 0; (i < segment.DeltaCount); ++i) {
                if (
                    (segment.NativeFrame[i] >= target) &&
                    (segment.NativeFrame[i] < current)
                ) {
                    slot = index;
                    frameIndex = (i + 1);
    ...
    private void StartSegment(int length, long cycle, long nativeFrame, ulong hostAccumulator) {
        ...
        m_captureScratch.AsSpan(
            length: length,
            start: 0
        ).CopyTo(destination: segment.Base);
    ...
    private void UpdateSnapshotLayout(int size) {
        if (
            (size < 0) ||
            (size > m_captureScratch.Length)
        ) {
            throw new InvalidOperationException(message: $"The core reported a {size} B state image for a {m_captureScratch.Length} B capture buffer.");
        }
    ...
    public void AdvanceLookahead(in TInput predicted) {
        ...
        if (
            ...
            lookahead.RestoreState(
                buffer: m_captureScratch,
                length: length
            );
    ...
    public void Record(in TInput input, long budget, ulong hostAccumulator) {
        ...
        if (
            (m_segCount == 0) ||
            (Newest.DeltaCount >= (m_interval - 1))
        ) {
            ...
            StartSegment(
                cycle: cycle,
                hostAccumulator: hostAccumulator,
                length: length,
    ...
    public int RewindBy(int frames, out ulong hostAccumulator) {
        ...
        // future's phase.
        hostAccumulator = ((frameIndex == 0)
            ? segment.BaseAccumulator
            : segment.Accumulator[(frameIndex - 1)]
        );
        ...
        return ((int)Math.Max(
            val1: 0L,
            val2: (current - m_core.NativeFrameIndex)
        ));
    }
    ...
    public void SetFastForward(int factor) =>
        m_fastForward = Math.Clamp(
            max: MaxFastForwardFactor,
            min: 1,
            value: factor
        );
    ...
    public void SetRunahead(int frames) {
        ...
        m_runaheadFrames = Math.Min(
            val1: frames,
            val2: MaxRunaheadFrames
        );
        m_lookahead?.Dispose();
