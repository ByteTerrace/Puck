...
public sealed class StateWriter {
    ...
    public ReadOnlySpan<byte> WrittenSpan => m_buffer.AsSpan(length: m_length, start: 0);
    ...
    public StateReader OpenReader() => new(buffer: m_buffer, length: m_length, start: 0);
    ...
    private Span<byte> Reserve(int count) {
        ...
        var slice = m_buffer.AsSpan(length: count, start: m_length);
