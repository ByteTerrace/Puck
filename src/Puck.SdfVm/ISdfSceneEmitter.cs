...
public interface ISdfSceneEmitter {
    ...
    /// <summary>Gets the number of dynamic-transform slots this emitter owns (0 = none, the default — a purely static
    /// emitter). Read once by the composition host to assign <see cref="SdfEmitContext.SlotBase"/> ranges contiguously;
    /// must stay constant for the emitter's lifetime (it is never re-read mid-session to reshuffle already-assigned
    /// bases).</summary>
    ...
    /// <summary>Gets a value indicating whether this emitter owns its material palette: <see langword="true"/> tells
    /// the composition host to wrap this emitter's <see cref="Emit"/> call in a
    /// <see cref="SdfProgramBuilder.BeginMaterialScope"/> scope, so any positional material recolor it emits —
    /// <see cref="SdfProgramBuilder.WallpaperFold"/>/<see cref="SdfProgramBuilder.RepeatPolar"/> with a nonzero
    /// <c>materialStride</c> — can only ever reach materials this emitter itself registered via
    /// <see cref="SdfProgramBuilder.AddMaterial"/>, instead of leaving that to author discipline.
    /// <para>
