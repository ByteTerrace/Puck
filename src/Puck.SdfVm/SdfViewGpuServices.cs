...
/// Puck.World's <c>WorldScreenBinder</c>'s stashed view factory turned out to be exactly these three members (unit
/// 6b's constructor-chain census). Resolved ONCE, eagerly, at the composition root — Puck.Overlays'
/// <c>OverlayServices.Build</c> precedent: resolve inside the factory, hand out concrete members, the provider
/// itself never escapes — then forwarded UNCHANGED to every construction site instead of a retained
/// <see cref="IServiceProvider"/> each site used to stash and re-resolve from on its own late-construction path.
/// Concrete, read-only, and declared in this consuming layer: the constructor rule's shape for a presentation
/// producer context.
/// </summary>
