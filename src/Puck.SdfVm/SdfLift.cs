...
public enum SdfLift : uint {
    /// <summary>Revolve the 2D profile around the local Y axis (iq's opRevolution): the profile sits in the (radial, Y)
    ...
    Revolve = 0,
    /// <summary>Extrude the 2D profile along the local Z axis (iq's opExtrusion) to the given half-height
