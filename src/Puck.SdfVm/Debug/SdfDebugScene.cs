...
public sealed class SdfDebugScene {
    ...
    public Vector3 Offset2 { get; private set; } = new(
        x: 1.2f,
        y: 0f,
        z: 0f
    );
    ...
    public static string FormatCarve(SdfCarve carve) =>
        string.Create(
            ...
            handler: $"@({carve.Center.X:0.##},{carve.Center.Y:0.##},{carve.Center.Z:0.##}) r={carve.Radius:0.###}{(carve.Smooth
            ? $" smooth k={carve.SmoothK:0.###}"
            : "")}"
        );
    ...
    public SdfCarve? TickMeteor() {
        ...
        var carve = (((i % 3) == 2)
            ...
            ? new SdfCarve(
                Center: new Vector3(
                    x: ((MathF.Cos(x: angle) * MathF.Sqrt(x: MathF.Max(
                        x: 0f,
                        y: (1f - ((1f - (2f * u)) * (1f - (2f * u))))
                    ))) * 1.05f),
                    y: ((1f - (2f * u)) * 0.9f),
                    z: ((MathF.Sin(x: angle) * MathF.Sqrt(x: MathF.Max(
                        x: 0f,
                        y: (1f - ((1f - (2f * u)) * (1f - (2f * u))))
                    ))) * 1.05f)
                ),
