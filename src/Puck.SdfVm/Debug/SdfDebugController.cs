...
public sealed class SdfDebugController {
    ...
    private Vector3 NearSideCarvePoint() =>
        (m_rig.Target + OrbitRig.Offset(
            yaw: m_rig.Yaw,
            pitch: m_rig.Pitch,
            distance: SubjectEnvelopeRadius
        ));
    ...
    public void Advance(in SdfOrbitInput raw, float deltaSeconds) {
        ...
        m_rig.Pitch = Math.Clamp(
            value: (m_rig.Pitch + ((raw.LeftStick.Y * OrbitSpeed) * deltaSeconds)),
            max: MaxPitch,
            min: MinPitch
        );
        m_rig.Distance = Math.Clamp(
            value: (m_rig.Distance * MathF.Exp(x: (((raw.LeftTrigger - raw.RightTrigger) * ZoomRate) * deltaSeconds))),
            max: MaxDistance,
            min: MinDistance
        );
        ...
        if (raw.RightStick != Vector2.Zero) {
            ...
            var forward = new Vector2(
                x: -MathF.Sin(x: m_rig.Yaw),
                y: -MathF.Cos(x: m_rig.Yaw)
            );
