...
/// <summary>Defines the SDF instruction-set contract accepted by the host.</summary>
public static class SdfIsa {
    ...
    internal static void ValidateReport(ReadOnlySpan<byte> report, string viewsVariant) {
        if ((report.Length < 4) || (report[0] != 0x53) || (report[1] != 0x44)) {
        ...
        if (report[2] != Version) {
            throw new InvalidOperationException(message: $"SDF ISA version mismatch: host expects v{Version}, but the loaded beam shader bytecode reports v{report[2]}. Refusing to initialize the SDF world pipeline.");
        }

        if (report[3] != Version) {
            throw new InvalidOperationException(message: $"SDF ISA version mismatch: host expects v{Version}, but the loaded {viewsVariant} shader bytecode reports v{report[3]}. Refusing to initialize the SDF world pipeline.");
        }
    }
