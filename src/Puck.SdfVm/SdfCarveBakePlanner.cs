...
public sealed class SdfCarveBakePlanner {
    ...
    private static volatile bool EnabledFlag = true;
    ...
    private int AcquireSlot(Bin forBin) {
        ...
        for (var slot = 0; (slot < m_slotOwners.Length); slot++) {
            ...
            if (
                (owner is { Phase: BinPhase.Brick }) &&
                ((victim is null) || (owner.AdoptedFrame < victim.AdoptedFrame))
            ) {
                victim = owner;
                victimSlot = slot;
            }
    ...
    private static void EmitBrick(SdfProgramBuilder builder, Bin bin, int slot, int material) {
        var extent = new Vector3(
            x: (bin.DimX * bin.CellSize),
            y: (bin.DimY * bin.CellSize),
            z: (bin.DimZ * bin.CellSize)
        );
        var center = (bin.BoxMin + (0.5f * extent));

        builder.BeginInstance(
            boundCenter: center,
            boundRadius: (0.5f * extent.Length())
        );
    ...
    private bool TryRequestBake(Bin bin, CellAccum accum, IReadOnlyList<SdfCarve> carves, CellKey key, ISdfBrickBakeService bakes) {
        ...
        var cell = MathF.Max(
            ...
            ) / SdfBrickPoolLayout.BrickDim)
        );
        var dimX = Math.Clamp(
            value: ((int)MathF.Ceiling(x: (extent.X / cell))),
            min: 1,
            max: SdfBrickPoolLayout.BrickDim
        );
        var dimY = Math.Clamp(
            value: ((int)MathF.Ceiling(x: (extent.Y / cell))),
            min: 1,
            max: SdfBrickPoolLayout.BrickDim
        );
        var dimZ = Math.Clamp(
            value: ((int)MathF.Ceiling(x: (extent.Z / cell))),
            min: 1,
            max: SdfBrickPoolLayout.BrickDim
        );
        ...
        for (var index = 0; (index < carves.Count); index++) {
            ...
            if (
                ...
                CellKey.For(center: carve.Center).Equals(other: key)
            ) {
                members.Add(item: new Vector4(
                    value: carve.Center,
                    w: carve.Radius
                ));
            }
        ...
        bakes.RequestBrickBake(
            slot: slot,
            request: new BrickBakeRequest(
                BoxMin: boxMin,
                CellSize: cell,
                DimX: dimX,
                DimY: dimY,
    ...
    public bool Advance(IReadOnlyList<SdfCarve> carves, int carveRevision, ISdfBrickBakeService bakes) {
        ...
        var changed = false;
        var effectiveSettle = (m_forceSettle
            ? 0
            : m_settleFrames
        );
        ...
        foreach (var (key, accum) in m_scratch) {
            ...
            switch (bin.Phase) {
                ...
                default: {
                        ...
                        bin.QuietFrames = ((signature == bin.Signature)
                            ? (bin.QuietFrames + 1)
                            : 0
                        );
                        ...
                        if (
                            eligible &&
                            (bin.QuietFrames >= effectiveSettle)
                        ) {
                            _ = TryRequestBake(
                                accum: accum,
                                bakes: bakes,
    ...
    public void Emit(SdfProgramBuilder builder, IReadOnlyList<SdfCarve> carves, int material) {
        ...
        for (var index = 0; (index < carves.Count); index++) {
            ...
            var represented = (!carve.Smooth
                && (carve.Radius >= minRadius)
                && m_bins.TryGetValue(
                key: CellKey.For(center: carve.Center),
                value: out var bin
            )
                && (bin.Phase == BinPhase.Brick));
    ...
    public static void EmitWorstCaseBricks(SdfProgramBuilder builder, int material) {
        ...
        var boundRadius = (0.5f * new Vector3(
            x: extent,
            y: extent,
            z: extent
        ).Length());
        ...
        for (var slot = 0; (slot < SdfBrickPoolLayout.MaxBricks); slot++) {
            builder.BeginInstance(
                boundCenter: new Vector3(value: (0.5f * extent)),
    ...
    private sealed class CellAccum {
        ...
        public void Add(SdfCarve carve) {
            ...
            Min = Vector3.Min(
                value1: Min,
                value2: (carve.Center - new Vector3(value: carve.Radius))
            );
            Max = Vector3.Max(
                value1: Max,
                value2: (carve.Center + new Vector3(value: carve.Radius))
