...
/// <summary>
/// The document-neutral canonicalize+hash core every family canonicalizer rides (<c>CreationCanonicalizer</c>,
/// <c>AudioCanonicalizer</c>, <c>SynthPatchCanonicalizer</c>, <c>ReleaseCanonicalizer</c>): one serializer configuration produces the
...
public static class DocumentCanonicalizer {
    ...
    /// <param name="errors">The violations the family's validation pass collected.</param>
    /// <param name="source">An optional source label (a file path or save handle) prefixed onto the message.</param>
    /// <exception cref="DocumentValidationException"><paramref name="errors"/> is non-empty.</exception>
    public static void ThrowIfInvalid(IReadOnlyList<DocumentValidationError> errors, string? source) {
