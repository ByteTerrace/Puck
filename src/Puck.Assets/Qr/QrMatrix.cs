...
public sealed class QrMatrix {
    ...
    private static int ComputePenalty(bool[] dark, int size) =>
        (((PenaltyRuleAdjacentRuns(
         ...
         PenaltyRuleFinderLikePatterns(
            dark: dark,
            size: size
        )) +
         PenaltyRuleDarkRatio(
            dark: dark,
            size: size
        ));
    ...
    private static bool MaskPredicate(int mask, int row, int col) => mask switch {
        ...
        paramName: nameof(mask),
        actualValue: mask,
        message: $"QR mask pattern must be 0..{(MaskPatternCount - 1)}."
    ),
    };
    ...
    private static int PenaltyRuleDarkRatio(bool[] dark, int size) {
        ...
        return (Math.Min(
            val1: previousDeviation,
            val2: nextDeviation
        ) * 10);
    }
    private static int PenaltyRuleFinderLikePatterns(bool[] dark, int size) {
        var penalty = 0;
        var lastWindow = (size - FinderLikePatternA.Length);
    ...
    private static void ReserveVersionInfoArea(bool[] reserved, int size) {
        for (var i = 0; (i < VersionInfoBitCount); i++) {
            var (area1Row, area1Col) = VersionPositionA(
                bitIndex: i,
                size: size
            );
            var (area2Row, area2Col) = VersionPositionB(
                bitIndex: i,
                size: size
            );
    ...
    private static void WriteFormatInfo(bool[] dark, int size, QrErrorCorrectionLevel level, int mask) {
        var bits = QrEncoder.ComputeFormatInfoBits(
            level: level,
            mask: mask
        );
