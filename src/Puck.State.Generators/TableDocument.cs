...
public static class TableCanonicalizer {
    ...
    public static IReadOnlyList<DocumentValidationError> Validate(TableDocument document) {
        ...
        if (DocumentCanonicalizer.SchemaViolationMessage(
            ...
            recognized: TableDocument.CurrentSchema
        ) is { } schemaViolation) {
        ...
        if (
            ...
            !string.Equals(
            a: document.Kind,
            b: TableDocument.IntKind,
            comparisonType: StringComparison.Ordinal
        )
        ) {
            errors.Add(item: new(
                Message: $"'{document.Kind}' is not '{TableDocument.IntKind}' or '{TableDocument.FixedKind}'.",
                Path: "kind"
            ));
        }
        ...
        for (var index = 0; (index < columns.Count); index++) {
            if (string.IsNullOrWhiteSpace(value: columns[index])) {
                ...
                errors.Add(item: new(
                    Message: $"column '{columns[index]}' is declared twice.",
                    Path: $"columns[{index}]"
                ));
            }
        }
        if (document.Entries is not { Count: > 0 } entries) {
            ...
            for (var index = 0; (index < entries.Count); index++) {
                ...
                if (!keys.Add(item: entry.Key)) {
                    errors.Add(item: new(
                        Message: $"key {entry.Key} is declared twice.",
                ...
                if (columns.Count == 0) {
                    if (entry.Value is not { } value) {
                        errors.Add(item: new(
                            Message: "a single-value table entry carries 'value'.",
                            Path: $"entries[{index}].value"
                        ));
                    } else if (
                        !isFixed &&
                        (value != decimal.Truncate(d: value))
                    ) {
                        errors.Add(item: new(
                    ...
                    if (entry.Values is not null) {
                        errors.Add(item: new(
                            Message: "a table without columns carries no 'values'.",
                    ...
                    if (entry.Value is not null) {
                        errors.Add(item: new(
                            Message: "a column table entry carries 'values', not 'value'.",
                            Path: $"entries[{index}].value"
                        ));
                    }
                    if (
                        ...
                        errors.Add(item: new(
                            Message: $"expected {columns.Count} values, one per column.",
                            Path: $"entries[{index}].values"
                        ));
                    } else if (!isFixed) {
                        for (var column = 0; (column < values.Count); column++) {
                            if (values[column] != decimal.Truncate(d: values[column])) {
                                errors.Add(item: new(
                                    Message: $"{values[column]} is not an integer.",
                                    Path: $"entries[{index}].values[{column}]"
                                ));
                            }
                        }
                    }
