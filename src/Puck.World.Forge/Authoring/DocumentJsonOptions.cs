...
public static class DocumentJsonOptions {
    ...
    /// one shape every document store serializes through. An enum persists BY NAME, never ordinal, and a numeric
    /// enum token (the old, wire-incompatible shape) is REFUSED rather than silently accepted
    /// (<c>allowIntegerValues: false</c>): a reordered enum must never silently reinterpret a persisted value.
    /// <c>PropertyNamingPolicy</c> governs PROPERTY names only, so the enum's exact declared member name is the one
    /// wire spelling regardless.</summary>
