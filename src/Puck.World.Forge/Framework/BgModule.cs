...
internal sealed class BgModule {
    ...
    public void EmitCopyMap(ushort sourceAddress) =>
        FrameworkKernel.EmitBlockCopy(byteCount: 0x400, destinationAddress: Hw.VramBackgroundMap, emitter: m_emitter, sourceAddress: sourceAddress);
    ...
    public void EmitMapClear() =>
        FrameworkKernel.EmitBlockFill(byteCount: 0x400, destinationAddress: Hw.VramBackgroundMap, emitter: m_emitter, value: 0x00);
    ...
    public void EmitPaintRect(ushort sourceAddress, int row, int column, int width, int height) {
        ...
        m_emitter.LoadImmediate(pair: Reg16.Hl, value: Hw.MapCell(column: column, row: row));
