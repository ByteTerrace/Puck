...
/// <c>puck.audio.v1</c> document (see <see cref="Puck.Forge.Authoring.AudioDocument"/>) rather than a cartridge ROM.
/// Its <see cref="Id"/> is <c>tune-instrument</c>; it takes no options. <see cref="Create"/> parses, validates, and
/// normalizes the content through <see cref="Puck.Forge.Authoring.AudioCanonicalizer"/>, compiles it to a jukebox
...
public sealed class TuneInstrumentEngine : IScreenMachineEngine {
    ...
    internal static Puck.Forge.Authoring.AudioDocument ParseContent(byte[] content) {
        Puck.Forge.Authoring.AudioDocument? document;

        try {
            document = JsonSerializer.Deserialize<Puck.Forge.Authoring.AudioDocument>(
                utf8Json: content,
        ...
        try {
            return Puck.Forge.Authoring.AudioCanonicalizer.Canonicalize(document: document).Document;
