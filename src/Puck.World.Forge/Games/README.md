...
Hand-authored SM83 cartridges built directly on `Puck.Forge.Framework
.GameFramework` — the same facade `Puck.Forge.Tune.TuneGame` uses (the
...
```csharp
var rom = Puck.Forge.Games.ArcadeQuestRom.Build();   // 32 KiB image
Puck.Forge.Games.ArcadeQuestRom.Verify(rom: rom);     // throws on any violation
File.WriteAllBytes("arcade-quest.gbc", rom);
```

`ArcadeQuestGame`/`ArcadeQuestVerify`/`ArcadeQuestProtocol` are `internal`
(same publicity-over-`InternalsVisibleTo` discipline as the rest of this
project); only `ArcadeQuestRom`'s `Build`/`Verify` pair is `public`, so a
throwaway build runner outside this assembly needs nothing more than a
`ProjectReference` to `Puck.World.Forge`.
