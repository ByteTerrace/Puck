...
internal static class WorktreeBaseCommand {
    private static int Run(string baseRef, string rawPath) {
        ...
        var toplevel = WorktreeBaseGit.Run(
            path: path,
        ...
        var resolvedBase = WorktreeBaseGit.Run(
            path: worktree,
            arguments: ["rev-parse", "--verify", "--quiet", $"{baseRef}^{{commit}}"]
        );

        if (
            (resolvedBase.ExitCode != 0) ||
            (resolvedBase.Stdout.Length == 0)
        ) {
        ...
        var status = WorktreeBaseGit.Run(
            path: worktree,
        ...
        var reset = WorktreeBaseGit.Run(
            path: worktree,
            arguments: ["reset", "--hard", newBase]
        );
    ...
    public static Command Create() {
        ...
        command.SetAction(action: parseResult => Run(
            ...
            rawPath: parseResult.GetRequiredValue(option: pathOption)
        ));
        return command;
    }
}
