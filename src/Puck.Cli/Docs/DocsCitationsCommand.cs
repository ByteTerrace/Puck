...
// The `puck docs citations` verb: every verb-shaped token a document CITES, checked against vocabularies swept
...
//
//   So this verb refuses to report when it can prove its own input is incomplete (exit 2), and it names
...
internal static class DocsCitationsCommand {
    ...
    private static int Run(string? enumerationPath) {
        ...
        if (enumerationPath is not null) {
            if (!File.Exists(path: enumerationPath)) {
                return CliExit.Refuse(
                    verb: Verb,
                    what: CliPaths.ToDisplay(fullPath: enumerationPath),
                    why: "no enumeration there, so nothing to resolve console verbs against."
                );
            }
        ...
        if (missing.Length > 0) {
            _ = CliExit.Refuse(
                verb: Verb,
                what: enumerationSource,
                why: $"the enumeration is INCOMPLETE: {missing.Length} verb(s) are registered in source but absent from it:"
            );
        ...
        Console.WriteLine(value: $"docs citations: {enumerated.Count} enumerated verb(s), {registered.Count} registered in source, {literals.Count} verb-shaped literal(s); {unresolved.Count} unresolved citation(s).");
        ...
        foreach (var citation in unresolved) {
            Console.WriteLine(value: $"  {citation.File}:{citation.Line}: {citation.Token}");
        }

        Console.WriteLine(value: "docs citations: each token above is cited as a verb but names nothing the code carries — not an enumerated verb, not a registration, not any string literal in src/.");
        Console.WriteLine(value: "  If one names a real mechanism this verb cannot see, the fix is to sweep that vocabulary here, never to suppress the token.");
    ...
    private static bool TryBootLeg(string artifact, bool headless, string runDirectory, HashSet<string> enumerated, out string error) {
        ...
        Console.Error.WriteLine(value: $"docs citations: booting Puck.World {shape} to read its `help` vocabulary.");
        ...
        if (
            ...
            error = $"the {shape} Puck.World boot {(process.TimedOut
                ? "timed out"
                : $"exited {process.ExitCode}")}; transcripts are at {CliPaths.ToDisplay(fullPath: runDirectory)}.";
        ...
        if (found == 0) {
            error = $"the {shape} Puck.World boot exited 0 but its stdout carried no `name - description` help line; transcripts are at {CliPaths.ToDisplay(fullPath: runDirectory)}.";
    ...
    // Resolves the Release Puck.World build (see WorldArtifactBuild) and boots it headless then windowed, piping `help` over stdin to each
    ...
    private static bool TryEnumerateLive(string root, out HashSet<string> enumerated, out string error) {
        ...
        if (enumerated.Count == 0) {
            error = $"both boots exited 0 but produced no `name - description` help line on stdout; transcripts are at {CliPaths.ToDisplay(fullPath: runDirectory)}.";
    ...
    public static Command Create() {
        var enumerationOption = new Option<string?>(name: "--enumeration") {
            Description = "The console verb list, one name per line, as the runtime `help` enumerates them; absent, Puck.World is built and booted to enumerate them live.",
        };
        var command = new Command(
            description: "Check the console-verb tokens skills and XML docs cite against vocabularies swept from the code.",
            name: "citations"
        ) { enumerationOption };
        ...
        command.Detail(detail: """
            ...
            names spelled literally in registrations, every other verb-shaped string literal in src/
            (a refusal door, a HUD binding token: names the code really knows), and every
            world-document field path the generated section schemas declare (`storage.userId`).

            Without --enumeration, Puck.World is built (Release) and booted headless and windowed
            in a scratch run directory, piping `help` over stdin to each and unioning the two
            vocabularies; nothing lands in the working directory.

            If a literally-registered verb is missing from the enumeration, nothing is reported
            and the run is refused: the enumeration provably under-reports the live surface, and
            citations checked against it would accuse correct documentation.

            Exit codes: 0 every citation resolved; 1 unresolved citations, each named on stdout;
            2 a usage error, a refused build or boot, or an incomplete enumeration.
            """);
