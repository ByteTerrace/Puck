...
internal static class OfficialBuildCommand {
    ...
    private static int Run(bool allowDirty, string channel, string engineDirectory, string outRoot, string? worlds) {
        ...
        if (!TryCheckDirty(
            ...
            dirty: out var dirty,
            isRefusal: out var isDirtyRefusal,
            refusal: out var dirtyRefusal,
            repositoryRoot: repositoryRoot
        )) {
            Console.Error.WriteLine(value: $"official build: {dirtyRefusal}");

            return (isDirtyRefusal
                ? 1
                : 2
            );
        }
        ...
        try {
            ...
            var worldProjectRoot = Path.GetFullPath(path: Path.Combine(
    ...
    private static bool TryCheckDirty(string repositoryRoot, bool allowDirty, out bool dirty, out bool isRefusal, out string? refusal) {
        ...
        try {
            result = CliProcess.RunCapturedRaw(
    ...
    public static Command Create() {
        ...
        var command = new Command(
            ...
            """,
            name: "build"
        ) { allowDirtyOption, channelOption, engineOption, outOption, worldsOption };
        ...
        command.SetAction(action: parseResult => Run(
            ...
            outRoot: parseResult.GetRequiredValue(option: outOption),
