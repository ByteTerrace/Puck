...
internal static partial class CanaryAssertions {
    ...
    private static CanaryAssertionResult EvaluateFileDifference(CanaryFileDifferenceAssertion assertion, CanaryTranscript transcript) {
        var beforePath = Path.Combine(
            ...
            path2: assertion.Before
        );
        var afterPath = Path.Combine(
            path1: transcript.RunDirectory,
            path2: assertion.After
        );

        if (
            !File.Exists(path: beforePath) ||
            !File.Exists(path: afterPath)
        ) {
            ...
            return new CanaryAssertionResult(
                Detail: $"{assertion.Name}: missing capture(s) {string.Join(
                    separator: ", ",
                    values: missing
                )}",
                Passed: false
        ...
        var passed = (assertion.Different
            ? !equal
            : equal
        );

        return new CanaryAssertionResult(
            Detail: $"{assertion.Name}: {assertion.Before} and {assertion.After} are {(equal
            ? "byte-identical"
            : "different")}",
            Passed: passed
        );
    ...
    private static CanaryAssertionResult EvaluateFrameAgreement(CanaryFrameAgreementAssertion assertion, CanaryTranscript transcript) {
        ...
        try {
            ...
            return new CanaryAssertionResult(
                Detail: $"{assertion.Name}: a capture could not be decoded ({exception.Message.ReplaceLineEndings(replacementText: " ")})",
                Passed: false
            );
        }
    ...
    private static CanaryAssertionResult EvaluateLine(CanaryLineAssertion assertion, CanaryTranscript transcript, string authorityEndpoint) {
        var text = assertion.Text.Replace(
            comparisonType: StringComparison.Ordinal,
            newValue: authorityEndpoint,
            oldValue: AuthorityToken
        );
        var matched = Lines(
            transcript: transcript,
            stream: assertion.Stream
        ).Any(predicate: line => assertion.Match switch {
            CanaryLineMatch.Exact => string.Equals(
            a: line,
            b: text,
            comparisonType: StringComparison.Ordinal
        ),
            CanaryLineMatch.Contains => line.Contains(
            comparisonType: StringComparison.Ordinal,
            value: text
        ),
            _ => false,
        ...
        var expectation = (assertion.Present
            ? "present"
            : "absent"
        );

        return new CanaryAssertionResult(
            Detail: $"{assertion.Name}: {expectation} on {StreamName(stream: assertion.Stream)}",
            Passed: passed
        );
    }
    private static CanaryAssertionResult EvaluateRelation(CanaryRelationAssertion assertion, Dictionary<string, string> values) {
        if (!TryResolveOperand(
            operand: assertion.Left,
            values: values,
            value: out var left,
            error: out var leftError
        )) {
            return new CanaryAssertionResult(
                Detail: $"{assertion.Name}: {leftError}",
                Passed: false
            );
        }
        ...
        switch (assertion.Operator) {
            ...
            case CanaryRelationOperator.NotEqual: {
                    ...
                    var equal = ValuesEqual(
                        left: left,
                        right: right
                    );
                    var passed = ((assertion.Operator == CanaryRelationOperator.Equal)
                        ? equal
                        : !equal
                    );
            ...
            case CanaryRelationOperator.AtMost: {
                    if (!TryNumber(
                        ...
                        return new CanaryAssertionResult(
                            Detail: $"{assertion.Name}: '{left}' is not a finite number",
                            Passed: false
                        );
                    }
                    ...
                    return new CanaryAssertionResult(
                        Detail: $"{assertion.Name}: {number.ToString(provider: CultureInfo.InvariantCulture)} {bounds}",
                        Passed: passed
                    );
                }
            case CanaryRelationOperator.MinimumMargin: {
                    ...
                    if (
                        ...
                        return new CanaryAssertionResult(
                            Detail: $"{assertion.Name}: minimumMargin needs two finite numbers, got '{left}' and '{right}'",
                            Passed: false
                        );
                    }
            ...
            default:
                return new CanaryAssertionResult(
                    Detail: $"{assertion.Name}: unsupported relation",
                    Passed: false
                );
        }
    }
    ...
    private static CanaryAssertionResult EvaluateSequence(CanarySequenceAssertion assertion, CanaryTranscript transcript) {
        ...
        foreach (var selector in assertion.Responses) {
            var indices = lines
                ...
                .Where(predicate: pair => pair.line.StartsWith(
                value: $"[{selector.Verb}:",
                comparisonType: StringComparison.Ordinal
            ))
            ...
            if (selectedIndex <= priorIndex) {
                return new CanaryAssertionResult(
                    Detail: $"{assertion.Name}: {selector.Verb} occurrence {selector.Occurrence} arrived out of order",
                    Passed: false
                );
            }
        ...
        return new CanaryAssertionResult(
            Detail: $"{assertion.Name}: {assertion.Responses.Count} selected responses arrived in order on {StreamName(stream: assertion.Stream)}",
            Passed: true
        );
    }
    ...
    private static bool TryResolveOperand(CanaryOperand operand, Dictionary<string, string> values, out string value, out string error) {
        ...
        value = operand.NumberLiteral!.Value.ToString(
            format: "R",
            provider: CultureInfo.InvariantCulture
        );
    ...
    public static CanaryEvaluation Evaluate(CanaryLeg leg, CanaryTranscript primaryTranscript, IReadOnlyDictionary<string, CanaryTranscript>? authorityTranscripts = null, string authorityEndpoint = "") {
        ...
        CanaryTranscript Resolve(string? authority) => ((authority is null)
            ? primaryTranscript
            : (authorityTranscripts?.GetValueOrDefault(key: authority) ?? new CanaryTranscript(
                RunDirectory: primaryTranscript.RunDirectory,
                Stderr: [],
                Stdout: []
            ))
        );
        ...
        foreach (var assertion in leg.Assertions) {
            if (assertion is CanaryResponseAssertion response) {
                results.Add(item: EvaluateResponse(
                    assertion: response,
                    transcript: Resolve(authority: response.Authority),
                    values: values
                ));
            }
        }
            ...
            switch (assertion) {
                ...
                case CanarySequenceAssertion sequence:
                    results.Add(item: EvaluateSequence(
                        assertion: sequence,
                        transcript: Resolve(authority: sequence.Authority)
                    ));
                    break;
                case CanaryRelationAssertion relation:
                    results.Add(item: EvaluateRelation(
                        assertion: relation,
                        values: values
                    ));
                    break;
                case CanaryFileDifferenceAssertion files:
                    results.Add(item: EvaluateFileDifference(
                        assertion: files,
