...
internal static partial class CanaryCommand {
    ...
    private static CanaryLegRun RunStubPositiveLegCore(CanaryManifest manifest, string worldArtifact, string stubArtifact, CanaryBudget budget) {
        ...
        MaterializeStubInstall(
            installDirectory: installDirectory,
            stubArtifact: stubArtifact
        );
        StageVersionDirectory(
            installDirectory: installDirectory,
            version: StubBaselineVersion,
            worldArtifact: worldArtifact
        );
        ...
        StubInstallFiles.WritePointer(
            fileName: "current",
            installDirectory: installDirectory,
            value: StubBaselineVersion
        );
        ...
        var channelDirectory = Path.Combine(
            path1: releaseDirectory,
            path2: StubReleaseChannel
        );
        ...
        File.WriteAllBytes(
            ...
            bytes: signedManifestBytes
        );

        var updateConfigPath = Path.Combine(
            path1: runDirectory,
            path2: "update-config.json"
        );

        WriteSelfUpdateConfigFile(
            cacheRoot: installDirectory,
        ...
        var boot1 = LaunchStub(
            ...
            input: ReadScriptWithSubstitution(
                leg: leg,
                runDirectory: runDirectory
            ),
            installDirectory: installDirectory,
        ...
        return BuildStubLegRun(
            ...
            invariants: [
                new(
                    Detail: $"boot 1 (check + apply) exited 0 (actual {boot1.ExitCode})",
                    Passed: (!boot1.TimedOut && (boot1.ExitCode == 0))
                ),
                new(
                    Detail: "boot 1 completed within the leg timeout",
                    Passed: !boot1.TimedOut
                ),
                new(
                    Detail: $"boot 2 (the stub relaunches on the applied version) exited 0 (actual {boot2.ExitCode})",
                    Passed: (!boot2.TimedOut && (boot2.ExitCode == 0))
                ),
                new(
                    Detail: "boot 2 completed within the leg timeout",
                    Passed: !boot2.TimedOut
                ),
            ],
            leg: leg,
            runDirectory: runDirectory
    ...
    private static CanaryLegRun RunStubDiscriminatingLegCore(CanaryManifest manifest, string worldArtifact, string stubArtifact, CanaryBudget budget) {
        ...
        var brokenVersionDirectory = StubInstallFiles.VersionDirectory(
            installDirectory: installDirectory,
            version: StubReleaseVersion
        );
        ...
        return BuildStubLegRun(
            ...
            invariants: [
                new(
                    Detail: $"boot 1 (the broken candidate) refuses to launch (actual exit {boot1.ExitCode})",
                    Passed: (!boot1.TimedOut && (boot1.ExitCode == 2))
                ),
                new(
                    Detail: "boot 1 completed within the leg timeout",
                    Passed: !boot1.TimedOut
                ...
                new(
                    Detail: $"boot 2 (the stub reverts to last-good) exited 0 (actual {boot2.ExitCode})",
                    Passed: (!boot2.TimedOut && (boot2.ExitCode == 0))
                ),
                new(
                    Detail: "boot 2 completed within the leg timeout",
    ...
    private static CanaryLegRun BuildStubLegRun(CliProcessResult boot1, CliProcessResult boot2, IReadOnlyList<CanaryAssertionResult> invariants, CanaryLeg leg, string runDirectory) {
        File.WriteAllText(
            contents: boot1.Stdout,
            path: Path.Combine(
                path1: runDirectory,
                path2: "boot1-stdout.log"
            )
        );
        File.WriteAllText(
            contents: boot1.Stderr,
            path: Path.Combine(
                path1: runDirectory,
                path2: "boot1-stderr.log"
            )
        );
        File.WriteAllText(
            contents: boot2.Stdout,
            path: Path.Combine(
                path1: runDirectory,
                path2: "boot2-stdout.log"
            )
        );
        File.WriteAllText(
            contents: boot2.Stderr,
            path: Path.Combine(
                path1: runDirectory,
                path2: "boot2-stderr.log"
            )
        );
    ...
    private static void MaterializeStubInstall(string installDirectory, string stubArtifact) {
        ...
        CopyDirectoryFiles(
            ...
            recursive: false,
            source: Path.GetDirectoryName(path: stubArtifact)!
        );
        File.WriteAllText(
            contents: JsonSerializer.Serialize(value: new StubConfigurationFile(
                AppExecutableFileName: "Puck.World.exe",
                MaxAttempts: 1
            )),
            path: Path.Combine(
                path1: installDirectory,
    ...
    private static void StageVersionDirectory(string installDirectory, string version, string worldArtifact) {
        var versionDirectory = StubInstallFiles.VersionDirectory(
            installDirectory: installDirectory,
            version: version
        );
        ...
        CopyDirectoryFiles(
            destination: versionDirectory,
            recursive: true,
            source: Path.GetDirectoryName(path: worldArtifact)!
        );
        File.WriteAllText(
            contents: "0",
            path: Path.Combine(
    ...
    private static (byte[] SignedManifestBytes, ReleaseTrustAnchor TrustAnchor) PublishAndSignThrowawayRelease(string releaseDirectory) {
        ...
        File.WriteAllText(
            contents: $"puck.world {StubReleaseVersion}",
            path: Path.Combine(
                path1: payloadDirectory,
                path2: "release-notes.txt"
            )
        );

        var dryRunDirectory = Path.Combine(
            path1: releaseDirectory,
            path2: "dry-run"
        );
        ...
        var unsignedBytes = File.ReadAllBytes(path: Path.Combine(
            path1: dryRunDirectory,
            path2: StubReleaseChannel,
            path3: "manifest.json"
        ));
        var unsigned = (JsonSerializer.Deserialize<ReleaseManifest>(
            utf8Json: unsignedBytes,
    ...
    private static void WriteLegConfirmationFixtures(string installDirectory, string runDirectory) {
        ...
        File.WriteAllText(
            contents: StubBaselineVersion,
            path: Path.Combine(
                path1: runDirectory,
                path2: $"expected-{StubBaselineVersion}.txt"
            )
        );
        File.WriteAllText(
            contents: StubReleaseVersion,
            path: Path.Combine(
                path1: runDirectory,
                path2: $"expected-{StubReleaseVersion}.txt"
            )
        );
    }
