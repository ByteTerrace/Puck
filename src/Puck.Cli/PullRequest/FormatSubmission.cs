...
// The privileged workflow runs this from a CLI built off the default branch only; the PR's artifact
// reaches it as data (validated source bytes), never as code, MSBuild files, or hooks.
    ...
    internal async Task RunAsync(string repository, long runId, string graphUrl) {
        ...
        foreach (var item in files) {
            ...
            if (
                ...
                !seen.Add(item: path) ||
                !FormatSources.Admits(path: path)
            ) {
