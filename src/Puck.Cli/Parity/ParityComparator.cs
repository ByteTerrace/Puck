...
internal static class ParityComparator {
    private static ParityCaptureOutcome CompareCapture(string station, ulong tick, ParityManifestCapture? leftCapture, ParityManifestCapture? rightCapture, string leftDir, string rightDir, ParityStationContract stationContract, int tileSize) {
        if (
            (leftCapture is null) ||
            (rightCapture is null)
        ) {
            var missingSide = ((leftCapture is null)
                ? "left"
                : "right"
            );

            return GateFailure(
                reason: $"missing-frame: capture is absent from the {missingSide} manifest",
                station: station,
                tick: tick
            );
        }
        ...
        var leftFramePath = Path.Combine(
            path1: leftDir,
            path2: leftCapture.Frame!
        );
        var rightFramePath = Path.Combine(
            path1: rightDir,
            path2: rightCapture.Frame!
        );

        if (
            !File.Exists(path: leftFramePath) ||
            !File.Exists(path: rightFramePath)
        ) {
            var missingSide = (!File.Exists(path: leftFramePath)
                ? "left"
                : "right"
            );

            return GateFailure(
                reason: $"missing-frame: {missingSide} frame file is not on disk",
                station: station,
                tick: tick
            );
        }
        ...
        foreach (var (material, floor) in stationContract.CensusFloor) {
            var leftCount = leftCapture.Census!.GetValueOrDefault(
                defaultValue: 0,
                key: material
            );
            var rightCount = rightCapture.Census!.GetValueOrDefault(
                defaultValue: 0,
                key: material
            );
            ...
            if (
                ...
                return GateFailure(
                    reason: $"census-below-floor: material '{material}' floor {floor}, left {leftCount}, right {rightCount}",
                    station: station,
                    tick: tick
                );
            }
        ...
        try {
            ...
            return GateFailure(
                station: station,
                tick: tick,
                reason: $"missing-frame: a frame could not be decoded ({exception.Message.ReplaceLineEndings(replacementText: " ")})"
            );
        }

        var verdicts = new List<ParityCaptureVerdict> {
            new(
            Detail: "content gate held",
            Name: "GATE-OK",
            Passed: true
        ),
        };
        ...
        var stateMatches = string.Equals(
            a: leftCapture.StateHash,
            b: rightCapture.StateHash,
        ...
        verdicts.Add(item: (stateMatches
            ? new ParityCaptureVerdict(
                Name: "STATE-OK",
                Passed: true,
                Detail: $"stateHash {leftCapture.StateHash}"
            )
            : new ParityCaptureVerdict(
                Name: "STATE-DIVERGED",
                Passed: false,
        ...
        if (
            ...
            verdicts.Add(item: new ParityCaptureVerdict(
                Name: "PIXEL-FAILED",
                Passed: false,
                Detail: $"extent disagreement {leftImage.Width}x{leftImage.Height} vulkan-side vs {rightImage.Width}x{rightImage.Height} directx-side"
            ));

            return new ParityCaptureOutcome(
                HeatmapRgba: null,
                LeftFrame: leftImage,
        ...
        var tileComparison = ParityTileComparer.Compare(
            left: leftImage,
            right: rightImage,
            tileMaxDeltaThreshold: stationContract.TileMaxDelta,
            tileMeanDeltaThreshold: stationContract.TileMeanDelta,
            tileSize: tileSize
        );
        ...
        verdicts.Add(item: (tileComparison.Passed
            ? new ParityCaptureVerdict(
                Name: "PIXEL-OK",
                Passed: true,
                Detail: $"worst tile ({worst.TileX},{worst.TileY}) mean={worst.MeanDelta:0.###} max={worst.MaxDelta}"
            )
            : new ParityCaptureVerdict(
                Name: "PIXEL-FAILED",
                Passed: false,
        ...
        var heatmap = ParityTileComparer.BuildHeatmap(
            left: leftImage,
            right: rightImage
        );
    ...
    private static ParityCaptureOutcome GateFailure(string station, ulong tick, string reason) =>
        new(
            Station: station,
            Tick: tick,
            Verdicts: [new ParityCaptureVerdict(
                    Detail: reason,
                    Name: "GATE-FAILED",
                    Passed: false
                )],
    ...
    public static bool TryCompare(ParityManifest left, ParityManifest right, string leftDir, string rightDir, ParityContract contract, out IReadOnlyList<ParityCaptureOutcome> outcomes, out string error) {
        ...
        foreach (var key in orderedKeys) {
            if (!contract.TryResolveStation(
                resolved: out var stationContract,
                station: key.Station
            )) {
            ...
            leftByKey.TryGetValue(
                key: key,
                value: out var leftCapture
            );
            rightByKey.TryGetValue(
                key: key,
                value: out var rightCapture
            );

            results.Add(item: CompareCapture(
                station: key.Station,
                tick: key.Tick,
