...
internal static class ReferencesCommand {
    private static bool Add(
        ...
        records.Add(item: new AnalysisRecord(
            Column: column,
            Detail: null,
            Kind: kind,
            Line: line,
    ...
    private static void AddDeclarations(ISymbol symbol, string relation, List<AnalysisRecord> records, HashSet<(string, int, int, string)> seen) {
        ...
        foreach (var location in symbol.Locations.Where(predicate: static location => location.IsInSource)
            ...
            .OrderBy(
            keySelector: static span => span.Path,
        ...
        if (
            !emitted &&
            !symbol.Locations.Any(predicate: static location => location.IsInSource)
        ) {
            _ = Add(
    ...
    private static async Task AddReferencesAsync(
        ...
        var groups = await SymbolFinder.FindReferencesAsync(
            symbol: symbol,
            solution: solution
        );
        ...
        foreach (var group in groups
            .OrderBy(
            keySelector: static group => group.Definition.ToDisplayString(),
            comparer: StringComparer.Ordinal
        )
            .ThenBy(
            keySelector: static group => (group.Definition.GetDocumentationCommentId() ?? string.Empty),
            comparer: StringComparer.Ordinal
        )) {
    ...
    private static async Task AddRelatedAsync(
        ...
        var related = mode switch {
            ReferencesMode.Implementers => await SymbolFinder.FindImplementationsAsync(
            symbol: symbol,
            solution: solution
        ),
            ReferencesMode.Overrides => await SymbolFinder.FindOverridesAsync(
            symbol: symbol,
            solution: solution
        ),
            _ => await FindDerivedAsync(
            solution: solution,
            symbol: symbol
        ),
        };
        ...
        foreach (var found in related
            .OrderBy(
            keySelector: static found => found.ToDisplayString(),
            comparer: StringComparer.Ordinal
        )
            .ThenBy(
            keySelector: static found => (found.GetDocumentationCommentId() ?? string.Empty),
            comparer: StringComparer.Ordinal
        )) {
            AddDeclarations(
                records: records,
    ...
    private static async Task<List<ISymbol>> FindTargetsAsync(Solution solution, ReferencesOptions options) {
        ...
        return [.. found
            .Where(predicate: symbol => InKind(
                symbol: symbol,
                filter: options.Filter
            ))
            .Where(predicate: symbol => ((options.Containing is null) || symbol.ToDisplayString().Contains(
                value: options.Containing,
                comparisonType: StringComparison.Ordinal
            )))
            .OrderBy(
                keySelector: static symbol => symbol.ToDisplayString(),
                comparer: StringComparer.Ordinal
            )
            .ThenBy(
                keySelector: static symbol => (symbol.GetDocumentationCommentId() ?? string.Empty),
                comparer: StringComparer.Ordinal
            )];
    }
    private static string? FirstWithExtension(DirectoryInfo directory, string extension) {
        try {
            return directory.EnumerateFiles()
                ...
                .Where(predicate: file => Path.GetExtension(path: file).Equals(
    ...
    private static bool IsInDocumentation(Location location) {
        ...
        var token = tree.GetRoot().FindToken(
    ...
    private static async Task<int> RunAsync(ReferencesOptions options) {
        ...
        if (
            ...
            var unrestoredCount = solution.Projects
                ...
                .Count(predicate: static path => !File.Exists(path: Path.Combine(
