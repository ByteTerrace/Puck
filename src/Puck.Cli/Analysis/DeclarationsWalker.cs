...
internal static class DeclarationsWalker {
    public static readonly IReadOnlySet<string> TypeKinds =
        new HashSet<string>(
        collection: ["class", "delegate", "enum", "interface", "record", "struct"],
        comparer: StringComparer.Ordinal
    );
    public static readonly IReadOnlySet<string> MemberKinds =
        new HashSet<string>(
        collection: ["ctor", "event", "field", "method", "property"],
        comparer: StringComparer.Ordinal
    );
    ...
    private static void Add(
        ...
        if (
            (options.Base is { } baseFragment) &&
            ((bases is null) || !bases.Contains(
            comparisonType: StringComparison.Ordinal,
            value: baseFragment
        ))
        ) {
        ...
        if (
            (options.Attribute is { } attributeFragment) &&
            !HasAttribute(
        ...
        records.Add(item: new AnalysisRecord(
            ...
            Name: QualifiedName(
                node: node,
                simpleName: (simpleName + Signature(node: node))
            ),
            Detail: bases
        ));
    }
    ...
    private static string? Bases(SyntaxNode node) =>
        (((node as BaseTypeDeclarationSyntax)?.BaseList is { } list)
            ? string.Join(
                separator: ", ",
                values: list.Types.Select(selector: static type => Flatten(node: type))
            )
            : null
        );
        ...
        switch (node) {
            ...
            case BaseTypeDeclarationSyntax or DelegateDeclarationSyntax when options.WantTypes:
                Add(
                    file: file,
                    node: node,
                    kind: TypeKind(node: node),
                    identifier: Identifier(node: node),
                    simpleName: SimpleName(node: node),
                    options: options,
                    records: records
                );
            ...
            case BaseFieldDeclarationSyntax field when options.WantMembers:
                ...
                foreach (var declarator in field.Declaration.Variables) {
                    Add(
                        file: file,
                        node: field,
                        kind: ((field is EventFieldDeclarationSyntax)
                        ? "event"
                        : "field"),
    ...
    private static int Compare(AnalysisRecord left, AnalysisRecord right) {
        ...
        var byRelation = string.CompareOrdinal(
            strA: left.Relation,
            strB: right.Relation
        );
    ...
    private static string Flatten(SyntaxNode node) {
        ...
        for (var index = 0; (index < tokens.Count); index++) {
            if (
                (index > 0) &&
                Separates(
                left: tokens[(index - 1)].TrailingTrivia,
                right: tokens[index].LeadingTrivia
            )
            ) {
    ...
    private static bool HasAttribute(SyntaxNode node, string fragment) {
        ...
        foreach (var list in member.AttributeLists) {
            foreach (var attribute in list.Attributes) {
                if (attribute.Name.ToString().Contains(
                    comparisonType: StringComparison.Ordinal,
                    value: fragment
                )) {
                    return true;
    ...
    private static string Signature(SyntaxNode node) {
        ...
        var rendered = ((parameters is null)
            ...
            : $"({string.Join(
                separator: ", ",
                values: parameters.Parameters.Select(selector: static parameter => ((parameter.Type is { } type)
                ? Flatten(node: type)
                : string.Empty))
            )})"
        );
