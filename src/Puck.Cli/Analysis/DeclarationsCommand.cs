...
internal static class DeclarationsCommand {
    ...
    private static IReadOnlySet<string>? ParseKinds(string? raw) {
        ...
        foreach (var kind in requested) {
            if (
                !DeclarationsWalker.TypeKinds.Contains(item: kind) &&
                !DeclarationsWalker.MemberKinds.Contains(item: kind)
            ) {
                Console.Error.WriteLine(value: $"declarations: unknown kind '{kind}' (known: {string.Join(
                    separator: ", ",
                    values: DeclarationsWalker.TypeKinds.Concat(second: DeclarationsWalker.MemberKinds).Order(comparer: StringComparer.Ordinal)
                )}).");
