...
internal sealed class LockAnalyzer : ISourceAnalyzer {
    ...
    private static (string Kind, string Text, int StartLine, int EndLine)? Classify(SyntaxNode node) {
        switch (node) {
            ...
            case LockStatementSyntax lockStatement: {
                    var (start, end) = ScanJsonl.LineRange(
                        start: lockStatement.LockKeyword.GetLocation(),
                        end: lockStatement.CloseParenToken.GetLocation()
                    );
            ...
            key: SimpleTypeName(type: variableDeclaration.Type),
            value: out var declaredKind
        ): {
            ...
            case PropertyDeclarationSyntax propertyDeclaration
                when InstanceLockTypes.TryGetValue(
            key: SimpleTypeName(type: propertyDeclaration.Type),
            value: out var propertyKind
        ): {
    ...
    public (string Jsonl, string Grouped) Analyze(SourceCorpus corpus, ScanOptions options) {
        ...
        var breakdown = ((total == 0)
            ...
            : string.Join(
                separator: ", ",
                values: kindCounts.Select(selector: static pair => $"{pair.Value} {pair.Key}")
            )
        );
