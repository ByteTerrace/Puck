...
internal static class ScanCommand {
    ...
    public static Command Create() {
        var groupedOption = new Option<bool>(
            name: "-Grouped",
            aliases: ["--grouped"]
        ) { Description = "Additionally write <name>.grouped.json, the per-file work list." };
        var maxPerChunkOption = new Option<int>(
            name: "-MaxPerChunk",
            aliases: ["--max-per-chunk"]
        ) { DefaultValueFactory = _ => ScanOptions.DefaultMaxPerChunk, Description = "Entries per grouped chunk." };
        var minStatementsOption = new Option<int>(
            name: "-MinStatements",
            aliases: ["--min-statements"]
        ) { DefaultValueFactory = _ => ScanOptions.DefaultMinStatements, Description = "clones: minimum statement count." };
        var minTokensOption = new Option<int>(
            name: "-MinTokens",
            aliases: ["--min-tokens"]
        ) { DefaultValueFactory = _ => ScanOptions.DefaultMinTokens, Description = "clones: minimum token count." };
        var noBlocksOption = new Option<bool>(
            name: "-NoBlocks",
            aliases: ["--no-blocks"]
        ) { Description = "clones: skip the nested-block pass." };
        var onlyOption = new Option<string?>(
            name: "-Only",
            aliases: ["--only"]
        ) { Description = $"Restrict to named analyzers, comma-separated (known: {KnownAnalyzers})." };
        var outDirOption = new Option<string?>(
            name: "-OutDir",
            aliases: ["--out-dir"]
        ) { Description = "Write <name>.jsonl per analyzer here (default <repo>/artifacts/scan)." };
