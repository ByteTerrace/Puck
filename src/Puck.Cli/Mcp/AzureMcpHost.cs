...
internal sealed class AzureMcpHost : RemoteMcpHost, IDisposable {
    ...
    public override bool IsReady => m_host.IsReady(target: m_target);
    ...
    public override async ValueTask<IControlSession> AttachAsync(RemoteMcpCaller caller, CancellationToken cancellationToken) {
        if (m_services.SupportsOnboarding) {
            ...
            } catch (AzureDelegatedAuthenticationException challenge) { throw new RemoteMcpAuthorizationException(claims: challenge.Claims); } catch (Exception error) when ((error is AuthenticationFailedException or HttpRequestException or JsonException or InvalidDataException)) {
    ...
    public override async ValueTask<CallToolResult> CallServiceAsync(RemoteMcpCaller caller, CallToolRequestParams request, CancellationToken cancellationToken) {
        if (
            ...
            } catch (AzureDelegatedAuthenticationException challenge) { throw new RemoteMcpAuthorizationException(claims: challenge.Claims); } catch (Exception error) when ((error is AuthenticationFailedException or HttpRequestException or IOException or JsonException or InvalidOperationException or UnauthorizedAccessException)) {
        ...
        if (
            ...
            (request.Arguments is not { Count: 1 } arguments) ||
            !arguments.TryGetValue(
            key: "observation",
            value: out var name
        ) ||
            (name.ValueKind != JsonValueKind.String) ||
            (name.GetString() is not { Length: > 0 and <= 128 } observation)
        ) {
        ...
        try {
            ...
            return new() { StructuredContent = content, Content = [new TextContentBlock { Text = content.GetRawText() }] };
        } catch (AzureDelegatedAuthenticationException challenge) { throw new RemoteMcpAuthorizationException(claims: challenge.Claims); } catch (AuthenticationFailedException) {
    ...
    public override ValueTask<ControlCapabilities> DescribeControlAsync(RemoteMcpCaller caller, CancellationToken cancellationToken) => m_host.DescribeAsync(
        m_target,
        new(
            Issuer: caller.Issuer,
            Subject: caller.Subject
