...
internal static class SearchCommand {
    ...
    private static void RejectNegative<T>(Option<T> option) {
        option.Validators.Add(item: result => {
            if (
                (result.GetValueOrDefault<T>() is int value) &&
                (value < 0)
            ) {
    ...
    private static int Run(SearchOptions opt) {
        ...
        var finalPattern = (opt.FilesOnly
            ? null
            : (opt.Literal
                ? Regex.Escape(input: opt.Pattern!)
                : opt.Pattern!
        ));

        if (finalPattern is not null) {
            try {
                _ = SearchScanner.MakeRegex(
        ...
        Parallel.For(
            ...
            localInit: () => SearchScanner.MakeRegex(
                pattern: finalPattern!,
                ignoreCase: opt.IgnoreCase
            ),
            body: (i, state, regex) => {
                try {
                    var result = (opt.Span
                        ? SearchScanner.ScanSpan(
                            path: files[i],
                            regex: regex,
                            opt: opt
                        )
                        : SearchScanner.ScanLines(
                            path: files[i],
                            regex: regex,
                            opt: opt
                        )
    ...
    public static Command Create() {
        var afterOption = new Option<int?>(
            name: "--after-context",
            aliases: ["-A"]
        ) { Description = "Print n context lines after each match." };
        var beforeOption = new Option<int?>(
            name: "--before-context",
            aliases: ["-B"]
        ) { Description = "Print n context lines before each match." };
        var contextOption = new Option<int?>(
            name: "--context",
            aliases: ["-C"]
        ) { Description = "Print n context lines before and after each match; an explicit -A or -B overrides that side." };
        var countsOption = new Option<bool>(
            name: "--count",
            aliases: ["-c"]
        ) { Description = "Print per-file matching-line counts." };
        ...
        var filesWithMatchesOption = new Option<bool>(
            name: "--files-with-matches",
            aliases: ["-l"]
        ) { Description = "Print matching file names only; wins over -c." };
        ...
        var includeOption = new Option<string[]>(
            name: "--glob",
            aliases: ["-g"]
        ) { DefaultValueFactory = _ => [], Description = "Include glob (repeatable; no '/' matches the basename)." };
        var lineNumbersOption = new Option<bool>(
            name: "--line-number",
            aliases: ["-n"]
        ) { Description = "Print line numbers; the default, so this only restates it." };
        var literalOption = new Option<bool>(
            name: "--fixed-strings",
            aliases: ["-F"]
        ) { Description = "Treat the pattern as a literal string (escape it)." };
        var maxResultsOption = new Option<int>(
            name: "--max-results",
            aliases: ["-M"]
        ) { DefaultValueFactory = _ => SearchOptions.DefaultMaxResults, Description = "Stop after n results; 0 is unlimited." };
        var noLineNumbersOption = new Option<bool>(
            name: "--no-line-number",
            aliases: ["-N"]
        ) { Description = "Suppress line numbers; wins over -n." };
        ...
        var quietOption = new Option<bool>(
            name: "--quiet",
            aliases: ["-q"]
        ) { Description = "Print nothing; report the verdict in the exit code alone (--files included)." };
        var spanOption = new Option<bool>(
            name: "--span",
            aliases: ["-s"]
        ...
        command.Validators.Add(item: result => {
            if (
                !result.GetValue(option: filesOnlyOption) &&
                (result.GetValue(argument: operandsArgument)!.Length == 0)
            ) {
