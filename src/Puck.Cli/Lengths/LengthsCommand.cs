...
internal static class LengthsCommand {
    ...
    private static bool IsAutoGenerated(byte[] bytes) {
        var head = Encoding.UTF8.GetString(
            bytes: bytes,
            index: 0,
            count: Math.Min(
                val1: bytes.Length,
    ...
    private static Dictionary<string, int> MeasureTree(string repositoryRoot) {
        ...
        var listing = CliProcess.RunCaptured(
            fileName: "git",
            arguments: ["-C", repositoryRoot, "ls-files", "-z", "--", "src/*.cs", "tests/*.cs", "build/*.cs"],
            input: "",
    ...
    private static int Run(bool write) {
        ...
        var ledgerPath = Path.Combine(
            path1: repositoryRoot,
            path2: LedgerFileName
        );

        if (!TryReadLedger(
            ceiling: out var ceiling,
            error: out var error,
            path: ledgerPath,
            recorded: out var recorded
        )) {
        ...
        var refusals = problems.Where(predicate: static problem => (problem.StartsWith(
        ...
        var lowered = recorded.Count(predicate: pair => (next.TryGetValue(
            key: pair.Key,
            value: out var lines
        ) && (lines < pair.Value)));
    ...
    private static bool TryReadLedger(string path, out int ceiling, out Dictionary<string, int> recorded, out string? error) {
        ...
        if (
            ...
            (document["format"]?.GetValue<int>() != 1) ||
            (document["ceiling"] is not JsonValue ceilingValue) ||
            (document["recorded"] is not JsonObject recordedObject)
        ) {
        ...
        foreach (var pair in recordedObject) {
            if (
                ...
                !value.TryGetValue<int>(value: out var lines) ||
                (lines <= ceiling)
            ) {
    ...
    private static void WriteLedger(string path, int ceiling, SortedDictionary<string, int> recorded) {
        ...
        File.WriteAllText(
            path: path,
            contents: (document.ToJsonString(options: WriteOptions).ReplaceLineEndings(replacementText: "\n") + "\n"),
