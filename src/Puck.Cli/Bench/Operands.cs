...
internal static class Operands {
    ...
    public static readonly QuadraticAlgebra<FixedQ4816> ComplexFused = QuadraticAlgebra<FixedQ4816>.Create(
        p: FixedQ4816.Zero,
        q: FixedQ4816.NegativeOne
    );
    public static readonly QuadraticAlgebra<FixedQ4816> SplitFused = QuadraticAlgebra<FixedQ4816>.Create(
        p: FixedQ4816.Zero,
        q: FixedQ4816.One
    );
    public static readonly QuadraticAlgebra<FixedQ4816> DualFix = QuadraticAlgebra<FixedQ4816>.Create(
        p: FixedQ4816.Zero,
        q: FixedQ4816.Zero
    );
    public static readonly QuadraticAlgebra<FixedQuaternion> DualQuat = QuadraticAlgebra<FixedQuaternion>.Create(
        p: FixedQuaternion.AdditiveIdentity,
        q: FixedQuaternion.AdditiveIdentity
    );
    ...
    public static readonly QuadraticAlgebra<ModP> ModAlg = QuadraticAlgebra<ModP>.Create(
        p: new ModP(
            Modulus: Modulus,
            Value: 0UL
        ),
    ...
    public static long WideRaw(Random rng) {
        var magnitude = rng.NextInt64(
            maxValue: (1L << 40),
            minValue: (1L << 31)
        );

        return ((0 == rng.Next(maxValue: 2))
            ? magnitude
            : -magnitude
        );
    }
...
internal readonly record struct ModP(ulong Value, ulong Modulus)
    ...
    static ModP IAdditiveIdentity<ModP, ModP>.AdditiveIdentity => new(
        Modulus: 0UL,
        Value: 0UL
    );
    static ModP IMultiplicativeIdentity<ModP, ModP>.MultiplicativeIdentity => new(
        Modulus: 0UL,
        Value: 1UL
    );

    public static ModP operator +(ModP left, ModP right) {
        var modulus = Operative(
            left: left,
            right: right
        );
        ...
        if (0UL == modulus) {
            return new(
                Value: unchecked((left.Value + right.Value)),
                Modulus: 0UL
            );
        }
    ...
    public static ModP operator -(ModP left, ModP right) {
        ...
        return new(
            ...
            : ((left.Value + modulus) - right.Value)),
            Modulus: modulus
        );
    }
    public static ModP operator *(ModP left, ModP right) {
        ...
        return new(
            Value: ((ulong)((((UInt128)left.Value) * right.Value) % modulus)),
            Modulus: modulus
        );
    }
    public static ModP operator -(ModP value) {
        if (
            ...
            return new(
                Value: unchecked((0UL - value.Value)),
                Modulus: value.Modulus
            );
        }

        return new(
            Value: (value.Modulus - value.Value),
            Modulus: value.Modulus
        );
    }
