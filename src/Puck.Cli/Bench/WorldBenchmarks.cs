...
// against the shipped world costs tens of seconds (WorldNavigationRuntime.BuildEdges sphere-casting through the
// static SDF program), far past what an iteration-based BenchmarkDotNet job could amortize honestly — this lane is
// a plain stopwatch harness instead: build once, measure the steady state, print one row per number.
