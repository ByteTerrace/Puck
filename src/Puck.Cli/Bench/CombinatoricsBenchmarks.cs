...
public class CombinationQueries {
    ...
    public void Setup() {
        ...
        var capacity = Combinatorics.Binomial(
            k: m_k,
            n: m_n
        );
    ...
    public ulong Binomial() {
        ...
        for (var i = 0; (i < Count); ++i) {
            sink ^= Combinatorics.Binomial(
                m_elements[(((i * m_k) + m_k) - 1)],
                m_k
            );
        }
        return sink;
    ...
    public int LargestElement() {
        ...
        for (var i = 0; (i < Count); ++i) {
            sink ^= Combinatorics.CombinationElement(
                m_n,
                m_k,
                m_ranks[i],
                (m_k - 1)
...
public class PermutationQueries {
    ...
    public void Setup() {
        ...
        for (var i = 0; (i < Count); ++i) {
            m_ranks[i] = ((ulong)random.NextInt64(maxValue: ((long)Combinatorics.Factorial(n: Length))));
            var permutation = m_elements.AsSpan(
                (i * Length),
                Length
            );
    ...
    private static ulong QuadraticRank(ReadOnlySpan<int> permutation) {
        ...
        for (var i = 0; (i < permutation.Length); ++i) {
            if (((uint)permutation[i]) >= ((uint)permutation.Length)) {
