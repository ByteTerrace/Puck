...
public class QuaternionKernels {
    ...
    public void Setup() {
        ...
        for (var i = 0; (i < Count); ++i) {
            m_from[i] = new(
                X: FixedQ4816.FromRawBits(value: Operands.NarrowRaw(rng: rng)),
                Y: FixedQ4816.FromRawBits(value: Operands.NarrowRaw(rng: rng)),
                Z: FixedQ4816.FromRawBits(value: Operands.NarrowRaw(rng: rng))
            );
            m_to[i] = new(
                X: FixedQ4816.FromRawBits(value: Operands.NarrowRaw(rng: rng)),
                Y: FixedQ4816.FromRawBits(value: Operands.NarrowRaw(rng: rng)),
            ...
            m_left[i] = FixedQuaternion.FromTo(
                from: m_from[i],
                to: m_to[i]
            );
            m_right[i] = FixedQuaternion.FromTo(
                from: m_to[i],
                to: m_from[((Count - 1) - i)]
            );
            m_amounts[i] = FixedQ4816.FromRawBits(value: rng.NextInt64(
                maxValue: (1L << 16),
                minValue: 0L
            ));
        }
