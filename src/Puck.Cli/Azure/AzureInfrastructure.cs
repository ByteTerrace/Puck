...
internal static partial class AzureCommand {
    ...
    private static async Task DeployInfrastructureAsync(string group, string? infrastructure) {
        ...
        var graph = await AzJsonAsync(
            ...
            "--query",
            "{id:id,roles:appRoles}",
            "-o",
            "json"
        );
        var assigned = await AzJsonAsync(
            "rest",
            "--method",
            "get",
            "--url",
            $"https://graph.microsoft.com/v1.0/servicePrincipals/{owner}/appRoleAssignments",
            "--query",
            "value",
            "-o",
            "json"
        ...
        foreach (var permission in new[] { "Application.ReadWrite.OwnedBy", "Application.Read.All", "AppRoleAssignment.ReadWrite.All", "GroupMember.Read.All" }) {
            ...
            if (
                (roles.Length != 1) ||
                !assigned.AsArray().Any(predicate: role => ((Text(value: role!["resourceId"]) == Text(value: graph["id"])) && (Text(value: role["appRoleId"]) == Text(value: roles[0]!["id"]))))
            ) {
        ...
        if (string.IsNullOrEmpty(value: infrastructure)) {
            ...
            await AzAsync(
                ...
                "--outfile",
                "artifacts/production-infrastructure.json"
            );
            await AzAsync(
                "bicep",
                "build-params",
                ...
                "--outfile",
                "artifacts/production-parameters.json"
            );
        } else {
            if (Text(value: CliFiles.ReadJson(path: Path.Combine(
                path1: infrastructure,
                path2: "source.json"
            ))["commit"]) != await RunAsync(
                arguments: ["rev-parse", "HEAD"],
                capture: true,
                executable: "git"
            )) { throw new InvalidDataException(message: "Infrastructure artifacts belong to another commit."); }
            File.Copy(
                destFileName: "artifacts/production-infrastructure.json",
                overwrite: true,
            ...
            File.Copy(
                destFileName: "artifacts/production-parameters.json",
                overwrite: true,
                sourceFileName: Path.Combine(
                    path1: infrastructure,
            ...
            File.Copy(
                destFileName: "artifacts/production-world-compute.json",
                overwrite: true,
                sourceFileName: Path.Combine(
                    path1: infrastructure,
        ...
        CliFiles.WriteJson(
            path: "artifacts/production-parameters.json",
            value: parameters
        );
        var plan = await AzJsonAsync(
            "deployment",
            "group",
            "what-if",
            "-g",
        ...
        CliFiles.WriteJson(
            path: "artifacts/production-plan.json",
            value: plan
        );
        ...
        CliFiles.WriteJson(
            path: "artifacts/production-outputs.json",
            value: await AzJsonAsync(
                "deployment",
                "group",
                "create",
                "--name",
                "puck-production-platform",
                "-g",
                group,
                "--template-file",
                "artifacts/production-infrastructure.json",
    ...
    private static async Task DeployWorldPlatformAsync(string group) {
        ...
        await AzAsync(
            ...
            "src/Puck.Azure.Resources/main.bicepparam",
            "--outfile",
            "artifacts/world-platform-source.json"
        );
        ...
        WriteParameters(
            path: "artifacts/world-platform-parameters.json",
            values: values
        );
        var outputs = await AzJsonAsync(
            "deployment",
            "group",
            "create",
            "-g",
            ...
            "puck-world-platform",
            "--template-file",
            "src/Puck.Azure.Resources/worldSiloPlatform.bicep",
            "--parameters",
            "@artifacts/world-platform-parameters.json",
            "--query",
            "properties.outputs",
            "-o",
            "json"
        ...
        var mcp = await AzJsonAsync(
            ...
            group,
            "--query",
            "[?(name=='puck-production-platform' || name=='puck-world-mcp') && properties.provisioningState=='Succeeded'] | sort_by(@, &properties.timestamp) | [-1].properties.outputs.worldMcpConfiguration || `{}`",
            "-o",
            "json"
        );
    ...
    private static async Task DeployWorldMcpAsync(string group, bool planOnly) {
        await AzAsync(
            ...
            "src/Puck.Azure.Resources/main.bicepparam",
            "--outfile",
            "artifacts/world-mcp-source.json"
        );
        ...
        var application = await AzJsonAsync(
            "ad",
            "app",
            "show",
            "--id",
            applicationId,
            "--query",
            "{uniqueName:uniqueName,api:api,identifierUris:identifierUris}",
            "-o",
            "json"
        );
        ...
        WriteParameters(
            path: "artifacts/world-mcp-parameters.json",
        ...
        var plan = await AzJsonAsync(
            ...
            group,
            "--template-file",
            "src/Puck.Azure.Resources/worldMcp.bicep",
            "--parameters",
            "@artifacts/world-mcp-parameters.json",
            "--no-pretty-print"
        );

        CliFiles.WriteJson(
            path: "artifacts/world-mcp-plan.json",
            value: plan
        );
        if (
            (((string?)plan["status"]) != "Succeeded") ||
            plan["changes"]!.AsArray().Any(predicate: change => (((string?)change!["changeType"]) == "Delete"))
        ) {
        ...
        if (planOnly) { return; }
        var deployed = await AzJsonAsync(
            "deployment",
            "group",
            "create",
            "-g",
    ...
    private static void WriteParameters(string path, JsonObject values) {
        ...
        CliFiles.WriteJson(
            path: path,
            value: new JsonObject { ["parameters"] = parameters }
        );
    }
        ...
        foreach (var spec in CliFiles.ReadJson(path: $"{Sources}/template-specs.json").AsArray()) {
            ...
            if (versions.AsArray().Any(predicate: value => (Text(value: value) == version))) {
                ...
                using var request = new HttpRequestMessage(
                    ...
                    requestUri: $"https://management.azure.com/subscriptions/{subscription}/resourceGroups/{group}/providers/Microsoft.Resources/templateSpecs/{name}/versions/{version}?api-version=2022-02-01"
                );
                ...
                if (!JsonNode.DeepEquals(
                    node1: published,
                    node2: compiled
                )) { throw new InvalidDataException(message: $"Template Spec {name}:{version} differs from source. Author a new version; published versions are immutable."); }
    ...
    private static void NormalizeTemplate(JsonNode? node) {
        if (node is JsonObject obj) {
            ...
            if (
                ...
                (((string?)link["resourceGroup"]) is { } group)
            ) {
                ...
                ) { link.Remove(propertyName: "resourceGroup"); }
            }
