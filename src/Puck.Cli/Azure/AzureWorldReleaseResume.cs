    ...
    private static async Task<WorldReleaseRunResult> ResumeWorldReleaseCoreAsync(WorldReleaseContext context, CancellationToken token) {
        ...
        try {
            var runtime = new AzureWorldReleaseRuntime(context.ResourceGroup, context.Group, temporary.FullName,
                ...
                context.Groups, context.Authority, context.Archive,
                new WorldReleaseRestore(context.Blobs, Puck.Storage.AzureBlobObjectStorageTarget.FromConnectionStringOrServiceUri(
                    Text(Value(context.Outputs, "worldSiloStorageEndpoint"))), context.Owner), source, candidate,
