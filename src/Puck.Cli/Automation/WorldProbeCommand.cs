...
internal static class WorldProbeCommand {
    public static async Task<int> RunAsync(string[] args) {
        if (args is ["-h" or "--help"]) {
            Console.WriteLine(value: "puck world probe <host> <port> <public-key-file>");
            return 0;
        }
        // Prove UDP reachability and possession of this world's key, rather than just a running container.
        if (args.Length != 3) {
            Console.Error.WriteLine(value: "Usage: puck world probe <host> <port> <public-key-file>");
            return 1;
        }
        ...
        var expectedKey = File.ReadAllBytes(path: args[2]);
        ...
        await using var connection = await QuicConnection.ConnectAsync(new QuicClientConnectionOptions {
            RemoteEndPoint = new DnsEndPoint(host: args[0], port: int.Parse(args[1], System.Globalization.CultureInfo.InvariantCulture)),
