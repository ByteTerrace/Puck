...
internal static class BundleCommand {
    internal static int CreateManifest(string directory, string commit) {
        ...
        if (File.Exists(path: Path.Combine(
            path1: root,
            path2: "release.json"
        ))) { throw new IOException(message: "Bundle already has a release manifest."); }
    ...
    internal static int VerifyManifest(string directory, string commit) {
        ...
        foreach (var file in release["files"]!.AsArray()) {
            var relative = ((string)file!["path"]!).Replace(
                newChar: '/',
                oldChar: '\\'
            );
        ...
        var actual = Directory.EnumerateFiles(
            ...
            .Where(predicate: file => (file != manifestPath)).Select(selector: file => Path.GetRelativePath(
            path: file,
            relativeTo: root
        ).Replace(
            newChar: '/',
        ...
        if (
            (paths.Count == 0) ||
            !paths.SetEquals(other: actual)
        ) { throw new InvalidDataException(message: "The artifact file inventory is incomplete."); }
    ...
    private static void ValidateCommit(string commit) {
        if (!Regex.IsMatch(
            input: commit,
            pattern: "\\A[a-f0-9]{40}\\z"
        )) { throw new ArgumentException(message: "Expected a full lowercase commit SHA."); }
    }
    ...
    public static Command Create() {
        ...
        var create = new Command(
            description: "Write release.json over every file in the directory, then verify it.",
            name: "create"
        ) { directoryArgument, commitArgument };
        var verify = new Command(
            description: "Verify release.json names this commit and matches every file in the directory.",
            name: "verify"
        ) { directoryArgument, commitArgument };
        var command = new Command(
            description: "Create or verify deployment artifact manifests.",
            name: "bundle"
        ) { create, verify };

        create.SetAction(action: parseResult => CreateManifest(
            commit: parseResult.GetRequiredValue(argument: commitArgument),
            directory: parseResult.GetRequiredValue(argument: directoryArgument)
        ));
        verify.SetAction(action: parseResult => VerifyManifest(
            commit: parseResult.GetRequiredValue(argument: commitArgument),
            directory: parseResult.GetRequiredValue(argument: directoryArgument)
        ));
        return command;
