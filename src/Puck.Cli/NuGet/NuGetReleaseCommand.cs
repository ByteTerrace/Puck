...
internal static class NuGetReleaseCommand {
        ...
        new XDocument(new XElement(
            ...
            new XElement(
                ...
                new XElement(
                    ...
                    new XAttribute(
                        name: "value",
                        value: Path.GetFullPath(path: feed)
                    )
                )
            ),
            new XElement(
    ...
    private static async Task<string> InstallAsync(string configFile, string directory, string root, string version) {
        ...
        ) { throw new InvalidDataException(message: $"Installed CLI reports {actual}; expected {version}."); }
    ...
    private static async Task<int> PinPublishedAsync(string packages, string patch) {
        ...
        if (!File.Exists(path: Path.Combine(
            path1: packages,
            path2: $"ByteTerrace.Puck.Cli.{version}.nupkg"
        ))) {
    ...
    private static async Task<int> ReleaseAsync(string manifest) {
        ...
        if (existing.ExitCode != 0) {
            string[] prerelease = (tag.Contains(value: '-')
                ? ["--prerelease"]
                : []
            );
    ...
    public static Command Gate() {
        var command = new Command(
            description: "Refuse a release from any ref other than main or the shared version's own tag, or from a commit outside main's history, or at a version tag bound to another commit.",
            name: "gate"
        );
