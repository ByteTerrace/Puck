...
internal static class NuGetCommand {
    ...
    internal static async Task<JsonObject> PrepareAsync(string input, string output, string selection, string version, string commit, Func<string, Task<string[]>> publishedVersions) {
        ...
        var (ordered, published) = await SelectAsync(
            ReadPackages(
                input: input,
                version: version
            ),
        ...
        foreach (var package in ordered) {
            ...
            foreach (var path in new[] { package.File, package.Symbols }) {
                ...
                files.Add(item: ((JsonNode)new JsonObject { ["name"] = name, ["sha256"] = ContentPin.OfFile(path: path).Hex }));
            }
        ...
        File.WriteAllText(
            ...
            (manifest.ToJsonString(options: JsonOptions) + "\n")
        );
        return manifest;
    }
    internal static IEnumerable<string[]> PushArguments(JsonObject manifest, string directory) {
        foreach (var package in manifest["packages"]!.AsArray()) {
            foreach (var file in package!["files"]!.AsArray()) {
                ...
                List<string> arguments = ["nuget", "push", Path.GetFullPath(path: Path.Combine(
                        path1: directory,
                        path2: name
                ...
                if (name.EndsWith(
                    ...
                    value: ".nupkg"
                )) { arguments.Add(item: "--no-symbols"); }
                yield return arguments.ToArray();
            }
        }
    }
    internal static string ReadVersion(string root) {
        var nodes = (XDocument.Load(uri: Path.Combine(
            path1: root,
            path2: "build",
            path3: "Packaging.targets"
        ))
    ...
    internal static string ValidateVersion(string version) {
        ...
        if (!Regex.IsMatch(
            ...
            pattern: @"\A(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)(?:-([0-9a-z-]+(?:\.[0-9a-z-]+)*))?\z"
        )) {
        ...
        if (
            (version.IndexOf(value: '-') is var dash) &&
            (dash >= 0)
        ) {
    ...
    internal static JsonObject Verify(string directory, string version, string commit) {
        ValidateVersion(version: version);
        ValidateCommit(commit: commit);
        var manifest = JsonNode.Parse(File.ReadAllText(path: Path.Combine(
            path1: directory,
            path2: "release.json"
        )))!.AsObject();
        ...
        if (
            ...
            (((string?)manifest["commit"]) != commit) ||
            (manifest["packages"] is not JsonArray { Count: > 0 } packages)
        ) {
        ...
        foreach (var package in packages) {
            ...
            if (
                !ids.Add(item: id) ||
                (((string?)package["version"]) != version) ||
                (package["files"] is not JsonArray { Count: 2 } files)
            ) {
            ...
            for (var index = 0; (index < files.Count); index++) {
                ...
                if (
                    ...
                    name.Contains(value: '/') ||
                    name.Contains(value: '\\')
                ) { throw new InvalidDataException(message: "Invalid artifact filename."); }
                if (!string.Equals(
                    a: ContentPin.OfFile(path: Path.Combine(
                        path1: directory,
                        path2: name
                    )).Hex,
                    b: ((string?)files[index]!["sha256"]),
                    comparisonType: StringComparison.Ordinal
                )) {
    ...
    private static async Task<int> PrepareBatchAsync(string input, string output, string selection) {
        ...
        var manifest = await PrepareAsync(
            commit: await ExpectedCommitAsync(root: root),
            input: input,
            output: output,
            publishedVersions: id => PublishedVersionsAsync(
                http: http,
                id: id
            ),
            selection: selection,
            version: ReadVersion(root: root)
        );
    ...
    private static int PrintVersion() {
        var version = ReadVersion(root: RepositoryPaths.RequireRoot());
    ...
    private static async Task<int> PushAsync(string directory) {
        ...
        foreach (var push in PushArguments(
            directory: directory,
            manifest: release
        )) {
            await CliProcess.RunCheckedAsync(
    ...
    private static Dictionary<string, Package> ReadPackages(string input, string version) {
        ...
        foreach (var path in Directory.EnumerateFiles(
            ...
            var entries = zip.Entries.Where(predicate: entry => entry.FullName.EndsWith(
                comparisonType: StringComparison.OrdinalIgnoreCase,
                value: ".nuspec"
            )).ToArray();
            ...
            foreach (var name in new[] { "README.md", "LICENSE.md", "icon.png" }) {
                if (zip.GetEntry(entryName: name) is null) { throw new InvalidDataException(message: $"{id} is missing {name}."); }
            }
            // build/Packaging.targets: every package declares its license as an SPDX expression and carries the text.
            var license = metadata.Elements().SingleOrDefault(predicate: element => (element.Name.LocalName == "license"));

            if (license is null) { throw new InvalidDataException(message: $"{id} declares no license."); }
            if (((string?)license.Attribute(name: "type")) != "expression") { throw new InvalidDataException(message: $"{id} must declare its license as an SPDX expression."); }
            var symbols = Path.Combine(
                path1: input,
                path2: $"{id}.{version}.snupkg"
            );
            ...
            var dependencies = metadata.Descendants().Where(predicate: element => (element.Name.LocalName == "dependency"))
                .Select(selector: element => new Dependency(
                Id: (((string?)element.Attribute(name: "id")) ?? ""),
                Version: (((string?)element.Attribute(name: "version")) ?? "")
            )).ToArray();
    ...
    private static void RequireEmpty(string output) {
        if (
            Directory.Exists(path: output) &&
            Directory.EnumerateFileSystemEntries(path: output).Any()
        ) {
    ...
    private static async Task<(Package[] Ordered, Dependency[] Published)> SelectAsync(
        ...
        foreach (var requested in ((selection.Trim() == "all")
            ? catalog.Keys
            : selection.Split(',').Select(selector: id => id.Trim()))) {
            if (!catalog.TryGetValue(
                key: requested,
                value: out var package
            )) {
                throw new InvalidDataException(message: $"Unknown package '{requested}'. Use all or comma-separated package IDs: {string.Join(
                    separator: ", ",
                    values: catalog.Keys.Order(comparer: StringComparer.Ordinal)
                )}.");
            }
            if (!selected.TryAdd(
                key: package.Id,
                value: package
            )) { throw new InvalidDataException(message: $"Duplicate selection: {requested}."); }
        }
        ...
        foreach (var package in selected.Values) {
            foreach (var dependency in package.Dependencies.Where(predicate: dependency => IsInternal(id: dependency.Id))) {
                ...
                if (
                    selected.ContainsKey(key: dependency.Id) ||
                    published.ContainsKey(key: dependency.Id)
                ) { continue; }
                if (!(await publishedVersions(dependency.Id)).Contains(
                    version,
        ...
        var remaining = new Dictionary<string, Package>(
            comparer: PackageComparer,
            dictionary: selected
        );
        ...
        return (ordered.ToArray(), published.Values.OrderBy(
    ...
    private static string Summary(JsonObject manifest) {
        ...
        text.AppendLine(handler: $"\nAlready published dependencies: {string.Join(
            separator: ", ",
            values: manifest["publishedDependencies"]!.AsArray().Select(selector: value => ((string)value!["id"]!))
        )}");
    ...
    public static Command Create() {
        ...
        var outputOption = CliOptions.Output(
            description: "The empty directory to receive the selected batch and its release.json.",
            required: true
        );
        var packOutputOption = CliOptions.Output(description: "The empty directory to pack into; absent, artifacts/packages.");
        ...
        var version = new Command(
            description: "Print the shared version from build/Packaging.targets; set the GITHUB_OUTPUT version when present.",
            name: "version"
        );
        var pack = new Command(
            description: "Pack and validate every opted-in project at the shared version.",
            name: "pack"
        ) { packOutputOption, noBuildOption };
        var prepare = new Command(
            description: "Select a batch, verify its internal dependency closure, and copy it with a release.json manifest.",
            name: "prepare"
        ) { inputArgument, selectionArgument, outputOption };
        var verify = new Command(
            description: "Verify a prepared batch's manifest, source commit, and artifact checksums.",
            name: "verify"
        ) { directoryArgument };
        var push = new Command(
            description: "Verify a prepared batch and push it to NuGet.org with NUGET_API_KEY, packages before symbols.",
            name: "push"
        ) { directoryArgument };
        var command = new Command(
            description: "Pack, select, verify, and push a shared-version NuGet release.",
            name: "nuget"
        ) {
        ...
        command.Detail(detail: "prepare, verify, and push use VERSION (or the shared version) and GITHUB_SHA (or git HEAD).");

        version.SetAction(action: _ => PrintVersion());
        pack.SetAction(action: (parseResult, _) => PackAsync(
            noBuild: parseResult.GetValue(option: noBuildOption),
            output: Path.GetFullPath(path: (parseResult.GetValue(option: packOutputOption) ?? "artifacts/packages")),
            root: RepositoryPaths.RequireRoot()
        ));
        prepare.SetAction(action: (parseResult, _) => PrepareBatchAsync(
            input: parseResult.GetRequiredValue(argument: inputArgument),
            output: parseResult.GetRequiredValue(option: outputOption),
            selection: parseResult.GetRequiredValue(argument: selectionArgument)
        ));
