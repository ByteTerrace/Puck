...
internal sealed class ArchitectureModel {
    ...
    public static ArchitectureModel Load(string repositoryRoot) {
        var ledger = XDocument.Load(uri: Path.Combine(path1: repositoryRoot, path2: "build", path3: "Architecture.props"));
        ...
        foreach (var item in Items(ledger: ledger, name: "PuckArchitectureLayer").OrderBy(keySelector: e => int.Parse(s: (e.Attribute(name: "Rank")?.Value ?? "0")))) {
            layers.Add(item: item.Attribute(name: "Include")!.Value);
        }

        foreach (var item in Items(ledger: ledger, name: "PuckArchitectureKind")) {
            kinds[item.Attribute(name: "Include")!.Value] = string.Equals(a: item.Attribute(name: "Ranked")?.Value, b: "true", comparisonType: StringComparison.OrdinalIgnoreCase);
        }

        foreach (var item in Items(ledger: ledger, name: "PuckArchitectureBackendException")) {
            backendExceptions[item.Attribute(name: "Include")!.Value] = (item.Attribute(name: "Reason")?.Value ?? "");
        }

        foreach (var item in Items(ledger: ledger, name: "PuckArchitectureProfile")) {
            profiles[item.Attribute(name: "Include")!.Value] = Split(value: item.Attribute(name: "Closure")?.Value);
        }

        foreach (var item in Items(ledger: ledger, name: "PuckArchitectureFriends")) {
        ...
        // same reason it is excluded there rather than by a second, quietly-different rule.
        foreach (var root in new[] { "src", "tests" }) {
            var directory = Path.Combine(path1: repositoryRoot, path2: root);

            if (!Directory.Exists(path: directory)) {
                continue;
            }

            foreach (var file in Directory.EnumerateFiles(path: directory, searchPattern: "*.csproj", searchOption: SearchOption.AllDirectories)) {
                if (file.Contains(value: $"{Path.DirectorySeparatorChar}bin{Path.DirectorySeparatorChar}") || file.Contains(value: $"{Path.DirectorySeparatorChar}obj{Path.DirectorySeparatorChar}")) {
                    continue;
                }
    ...
    public static IReadOnlyList<string> ReadFriendsFromAssembly(string assemblyPath) {
        ...
        foreach (var handle in metadata.CustomAttributes) {
            ...
            if ((value.FixedArguments.Length != 0) && (value.FixedArguments[0].Value is string argument)) {
    ...
    public IReadOnlyList<string> Closure(string name) {
        ...
        while (queue.Count != 0) {
            if (!Projects.TryGetValue(key: queue.Dequeue(), value: out var project)) {
                continue;
            }
    ...
    public int RankOf(ArchitectureProject project) {
        if (Kinds.TryGetValue(key: project.Kind, value: out var ranked) && !ranked) {
            return -1;
        }

        var index = Layers.ToList().FindIndex(match: l => string.Equals(a: l, b: project.Layer, comparisonType: StringComparison.OrdinalIgnoreCase));
    ...
    private static ArchitectureProject ReadProject(string file) {
        ...
        foreach (var element in document.Descendants().Where(predicate: e => (e.Name.LocalName == "ProjectReference"))) {
            ...
            edges.Add(item: Path.GetFileNameWithoutExtension(path: include.Value.Replace(oldChar: '\\', newChar: Path.DirectorySeparatorChar)));
        }
    ...
    private static IReadOnlyList<string> Split(string? value) =>
        [.. (value ?? "").Split(options: StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries, separator: ';')];
    ...
    private sealed class StringOnlyAttributeTypeProvider : ICustomAttributeTypeProvider<string> {
        public string GetPrimitiveType(PrimitiveTypeCode typeCode) =>
            ((typeCode == PrimitiveTypeCode.String) ? "string" : throw new NotSupportedException(message: $"unexpected primitive {typeCode} on an InternalsVisibleTo attribute"));
