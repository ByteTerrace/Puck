...
internal static class FormatSubmitCommand {
    private static async Task<int> RunAsync(CancellationToken cancellationToken) {
        ...
        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(
            parameter: CliGitHub.EnvironmentVariable(name: "GH_TOKEN"),
            scheme: "Bearer"
        );
        ...
        await new FormatSubmission(
            ...
            runId: long.Parse(
                ...
                s: CliGitHub.EnvironmentVariable(name: "FORMAT_RUN_ID")
            )
    ...
    public static Command Create() {
        var command = new Command(
            description: "Validate and apply a completed formatting run's artifact to its pull request; the environment names the run and the token.",
            name: "submit"
        );
