...
internal static class FormatCommand {
    ...
    public static Command Create() {
        var filesOption = new Option<string>(
            name: "--files",
            aliases: ["-Files"]
        ) { Description = "Format only this JSON array of paths relative to <root>; standalone C# files use disposable, built SDK projects." };
        var onlyOption = new Option<string>(
            name: "--only",
            aliases: ["-Only"]
        ) { Description = $"Restrict to a comma-separated list of named passes (default: every pass but the three vertical line-wrappers and member-groups). Passes: {FormatPasses.Names}" };
        ...
        var verifyOption = new Option<bool>(
            name: "--verify",
            aliases: ["-Verify"]
        ) { Description = "Write nothing; additionally fail on a rewrite that would introduce syntax errors or a pass that is not a fixed point." };
        var whatIfOption = new Option<bool>(
            name: "--what-if",
            aliases: ["-WhatIf"]
        ) { Description = "Write nothing; exit 1 on any drift, listing the files." };
        var command = new Command(
            ...
            """,
            name: "format"
        ) { rootArgument, filesOption, onlyOption, verifyOption, whatIfOption };
