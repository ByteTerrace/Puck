...
internal sealed class MemberSpacingRewriter : CSharpSyntaxRewriter {
    private static string Bucket(MemberDeclarationSyntax member, HashSet<string> coupled) => member switch {
        FieldDeclarationSyntax field => FieldBucket(
        coupled: coupled,
        field: field
    ),
        EventFieldDeclarationSyntax => "event",
        PropertyDeclarationSyntax property => (coupled.Contains(item: property.Identifier.ValueText)
        ? "member-coupled"
        : "property"),
    ...
    private static TypeDeclarationSyntax Fix(TypeDeclarationSyntax node) =>
        node.WithMembers(members: Normalize(
            members: node.Members,
            normalizeFirst: true
        ));
    private static SyntaxNode Fix(SyntaxNode node) => node switch {
        CompilationUnitSyntax compilationUnit => compilationUnit.WithMembers(members: Normalize(
        members: compilationUnit.Members,
        normalizeFirst: false
    )),
        NamespaceDeclarationSyntax @namespace => @namespace.WithMembers(members: Normalize(
        members: @namespace.Members,
        normalizeFirst: false
    )),
        FileScopedNamespaceDeclarationSyntax @namespace => @namespace.WithMembers(members: Normalize(
        members: @namespace.Members,
        normalizeFirst: false
    )),
        _ => throw new ArgumentException(
        message: "A member container must be a compilation unit or namespace.",
    ...
    private static SyntaxList<MemberDeclarationSyntax> Normalize(SyntaxList<MemberDeclarationSyntax> members, bool normalizeFirst) {
        ...
        var result = new List<MemberDeclarationSyntax> {
            ...
            ? first.WithLeadingTrivia(trivia: RewriteShaping.SetBlankLines(
                desired: 0,
                lead: firstLead
            ))
            : first),
        ...
        for (var index = 1; (index < members.Count); index++) {
            ...
            result.Add(item: current.WithLeadingTrivia(trivia: RewriteShaping.SetBlankLines(
                desired: (sameSubject
                ? 0
                : 1),
                lead: lead
