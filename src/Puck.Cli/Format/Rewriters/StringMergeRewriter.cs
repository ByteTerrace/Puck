...
internal sealed class StringMergeRewriter : CSharpSyntaxRewriter {
    ...
    private static bool TryCollectSegments(ExpressionSyntax expression, List<Segment> segments) {
        switch (expression) {
            case LiteralExpressionSyntax literal when literal.IsKind(kind: SyntaxKind.StringLiteralExpression):
                ...
                if (
                    !literal.Token.IsKind(kind: SyntaxKind.StringLiteralToken) &&
    ...
    public override SyntaxNode? VisitBinaryExpression(BinaryExpressionSyntax node) {
        ...
        if (
            !visited.IsKind(kind: SyntaxKind.AddExpression) ||
            HasInteriorComment(node: visited)
        ) {
            return visited;
        }
        ...
        if (
            !TryCollectSegments(
            expression: Unwrap(expression: visited.Left),
            segments: segments
        ) ||
            !TryCollectSegments(
            expression: Unwrap(expression: visited.Right),
            segments: segments
        )
        ) {
            return visited;
    ...
    public override SyntaxNode? VisitParenthesizedExpression(ParenthesizedExpressionSyntax node) {
        ...
        if (
            ((inner is LiteralExpressionSyntax literal) && literal.IsKind(kind: SyntaxKind.StringLiteralExpression)) ||
