...
internal sealed class LogicalLinesRewriter : CSharpSyntaxRewriter {
    ...
    private static void Flatten(BinaryExpressionSyntax binary, SyntaxKind kind, List<ExpressionSyntax> operands, List<SyntaxToken> operators) {
        if (
            (binary.Left is BinaryExpressionSyntax leftBinary) &&
    ...
    private static bool IsAnnotated(SyntaxToken openParen, BinaryExpressionSyntax binary, SyntaxToken closeParen) {
        if (
            RewriteShaping.HasCommentOrDirective(trivia: openParen.TrailingTrivia) ||
        ...
        return (
            operands.Any(predicate: static operand => RewriteShaping.IsAnnotated(node: operand)) ||
    ...
    private static SyntaxNode LayoutCondition(SyntaxNode original, SyntaxNode visited) {
        var (condition, openParen, closeParen, keyword) = visited switch {
            ...
            message: "A logical condition must belong to an if or while statement.",
            paramName: nameof(visited)
        ),
        };
        ...
        var closed = closeParen.WithLeadingTrivia(
    ...
    public override SyntaxNode? VisitIfStatement(IfStatementSyntax node) =>
        LayoutCondition(
            original: node,
            visited: ((IfStatementSyntax)base.VisitIfStatement(node: node)!)
        );
    public override SyntaxNode? VisitReturnStatement(ReturnStatementSyntax node) {
        var visited = ((ReturnStatementSyntax)base.VisitReturnStatement(node: node)!);

        if (
            (visited.Expression is not ParenthesizedExpressionSyntax paren) ||
            (paren.Expression is not BinaryExpressionSyntax binary) ||
            !IsLogical(binary: binary) ||
            IsAnnotated(
            openParen: paren.OpenParenToken,
            binary: binary,
            closeParen: paren.CloseParenToken
        )
        ) {
            return visited;
        }
        ...
        var indent = LineIndentAt(
            node: node,
            position: node.ReturnKeyword.SpanStart
        );
        var laidOut = paren
            .WithOpenParenToken(openParenToken: paren.OpenParenToken.WithTrailingTrivia())
            .WithExpression(expression: Layout(
            binary: binary,
            innerIndent: (indent + "    ")
        ))
            .WithCloseParenToken(closeParenToken: paren.CloseParenToken.WithLeadingTrivia(
            RewriteShaping.EndOfLine,
            SyntaxFactory.Whitespace(text: indent)
        ));
    ...
    public override SyntaxNode? VisitWhileStatement(WhileStatementSyntax node) =>
        LayoutCondition(
            original: node,
            visited: ((WhileStatementSyntax)base.VisitWhileStatement(node: node)!)
        );
}
