...
internal sealed class AttrRewriter : CSharpSyntaxRewriter {
    ...
    private static T Reorder<T>(T node) where T : MemberDeclarationSyntax {
        ...
        if (
            (lists.Count <= 1) ||
            lists.Any(predicate: static list => (list.Attributes.Count != 1))
        ) {
            return node;
        }
        ...
        var order = Enumerable.Range(
            ...
            .OrderBy(
            keySelector: index => SimpleName(attribute: lists[index].Attributes[0]),
            comparer: StringComparer.Ordinal
        )
            .ToList();

        if (order.SequenceEqual(second: Enumerable.Range(
            start: 0,
            count: lists.Count
        ))) {
            return node;
