...
/// <see cref="StateWriter"/> exactly: every field must be read in the same order and width it was written, which is the
/// discipline that keeps a restore deterministic. Reading past the reader's window faults deterministically rather than
/// misreading — a save/load field-order drift is caught, not silently loaded as wrong state.
/// </summary>
public sealed class StateReader {
    ...
    public StateReader(byte[] buffer, int start = 0, int length = -1) {
