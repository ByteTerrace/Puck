...
public static partial class BrowserExports {
    ...
    public static string Version() => Write(
        value: BrowserEngineInfo.Describe(),
        info: BrowserExportsJsonContext.Default.BrowserVersion
    );
    ...
    [JSExport]
    public static string Parse(string json) => Write(
        value: BrowserParser.Parse(utf8Json: Utf8(text: json)),
    ...
    public static string ParseFragment(string fragmentJson, string hostJson, string alias) => Write(
        value: BrowserParser.ParseFragment(
            fragmentUtf8: Utf8(text: fragmentJson),
            hostUtf8: Utf8(text: hostJson),
            alias: alias
        ),
        info: BrowserExportsJsonContext.Default.BrowserParseResult
    );
    ...
    /// <returns>The same shape as <see cref="Parse"/>.</returns>
    [JSExport]
    public static string Canonicalize(string json) => Write(
    ...
    public static string Compile(string json) {
        ...
        if (!BrowserParser.TryParseAndValidate(
            ...
            return Write(
                ...
                info: BrowserExportsJsonContext.Default.BrowserCompileResult
            );
        }
        ...
        return Write(
            value: new BrowserCompileResult(
                Ok: true,
                Handle: handle.ToString(provider: CultureInfo.InvariantCulture),
                Errors: null
            ),
            info: BrowserExportsJsonContext.Default.BrowserCompileResult
        );
    ...
    public static string Release(string handle) => Write(
        value: new BrowserOutcome(
            Ok: BrowserSessionRegistry.Release(handle: ParseHandle(handle: handle)),
            Error: null
        ),
    ...
    public static string Rows(string handle) {
        if (!BrowserSessionRegistry.TryGet(
            handle: ParseHandle(handle: handle),
            session: out var session
        )) {
            return Write(
                value: new BrowserRowsResult(
                    Ok: false,
                    Rows: null,
                    Error: NoSuchHandle(handle: handle)
                ),
                info: BrowserExportsJsonContext.Default.BrowserRowsResult
            );
        }
    ...
    public static string Judge(string handle, string tick) {
        if (!BrowserSessionRegistry.TryGet(
            ...
            return Write(
                value: new BrowserJudgeExportResult(
                    Ok: false,
                    Trace: null,
                    Error: NoSuchHandle(handle: handle)
                ),
        ...
        var trace = session!.Judge(tick: ulong.Parse(
            s: tick,
    ...
    public static string WriteRow(string handle, string row, string key, string value, string write) {
        ...
        if (write is not ("set" or "add")) {
            return Write(
                value: new BrowserOutcome(
                    Error: $"write '{write}' must be 'set' or 'add'.",
                    Ok: false
                ),
                info: BrowserExportsJsonContext.Default.BrowserOutcome
            );
    ...
    public static string Evaluate(string handle, string expression, string kind, string tick) {
        if (!BrowserSessionRegistry.TryGet(
            ...
            return Write(
                value: new BrowserEvaluateResult(
                    Ok: false,
                    Value: 0L,
                    Error: NoSuchHandle(handle: handle)
                ),
                info: BrowserExportsJsonContext.Default.BrowserEvaluateResult
            );
        }

        if (!Enum.TryParse<CellKind>(
            result: out var parsedKind,
            value: kind
        )) {
            return Write(
                value: new BrowserEvaluateResult(
                    Error: $"kind '{kind}' is not a declared CellKind.",
                    Ok: false,
                    Value: 0L
                ),
                info: BrowserExportsJsonContext.Default.BrowserEvaluateResult
    ...
    public static string BoardMask(string handle, string row) {
        if (!BrowserSessionRegistry.TryGet(
            ...
            return Write(
                value: new BrowserBoardMaskResult(
                    Ok: false,
                    Mask: 0UL,
                    Error: NoSuchHandle(handle: handle)
                ),
        ...
        return Write(
            value: new BrowserBoardMaskResult(
                Error: (ok
            ? null
            : $"'{row}' is not a board row of at most {Puck.State.BoardMask.MaxCells} cells."),
                Mask: mask,
                Ok: ok
            ),
            info: BrowserExportsJsonContext.Default.BrowserBoardMaskResult
    ...
    public static string StateHash(string handle) {
        if (!BrowserSessionRegistry.TryGet(
            ...
            return Write(
                value: new BrowserStateHashResult(
                    Ok: false,
                    Hash: 0UL,
                    Error: NoSuchHandle(handle: handle)
                ),
                info: BrowserExportsJsonContext.Default.BrowserStateHashResult
            );
        }
    ...
    private static long ParseHandle(string handle) => (long.TryParse(
        s: handle,
        provider: CultureInfo.InvariantCulture,
