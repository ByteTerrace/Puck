...
AppBundle (see "mimalloc" below). The native WebAssembly module listed above alone (31.5 MiB, 74%
of the payload) is every assembly's AOT-compiled native code; the Schema assembly module
...
Cost: the published `System.Text.Json` WebAssembly module grows from 327,445 to 425,237 bytes (+30%,
...
drop from ~22.6 s to ~4.3 s. The recorded AOT comparison ran the existing tests and harnesses
(`dotnet test tests/Puck.World.Browser.Tests`, `engine-wasm.test.cjs`,
`native.test.cjs`, `engineBoot.test.cjs`) passed unchanged against the AOT
...
native WebAssembly module compared with the non-SIMD AOT build—the SDK's own incremental
