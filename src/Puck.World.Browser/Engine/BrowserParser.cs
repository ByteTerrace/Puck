    ...
    public static BrowserParseResult ParseFragment(byte[] fragmentUtf8, byte[] hostUtf8, string alias, string catalogFingerprint = "", IMachineValidationCatalog? catalog = null) {
        ...
        if (!WorldDefinitionFileSource.TryComposeFragmentBytes(
            ...
            return new BrowserParseResult(
                Ok: false,
                Document: null,
                Errors: [BrowserErrorPaths.Split(message: composeReason)],
                Deferred: null
            );
        }
        ...
        if (!TryParseAndValidate(
            ...
            return new BrowserParseResult(
                Ok: false,
                Document: null,
                Errors: BrowserErrorPaths.SplitFragment(
                    alias: alias,
                    messages: errors
                ),
                Deferred: null
        ...
        var prefix = $"{alias}_";
        var strippedDeferred = deferred.Select(selector: message => message.Replace(
            comparisonType: StringComparison.Ordinal,
