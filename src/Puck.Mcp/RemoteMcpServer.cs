...
public static partial class RemoteMcpServer {
    ...
    public static WebApplication Build(RemoteMcpOptions options, Action<WebApplicationBuilder>? configureBuilder = null) {
        ...
        var listen = new Uri(uriString: options.ListenUrl!);
        ...
        builder.WebHost.ConfigureKestrel(options: server => {
            ...
            server.Listen(
                IPAddress.Parse(ipString: listen.Host.Trim(
                    '[',
                    ']'
                )),
                listen.Port,
    ...
    public static void AddServices(IServiceCollection services, RemoteMcpOptions options) {
        ...
        services.AddKeyedSingleton(
            "PuckMcp",
            (_, _) => new ConcurrencyLimiter(options: new() { PermitLimit = 4, QueueLimit = 0 })
        );
        services.AddSingleton(implementationInstance: PartitionedRateLimiter.Create<string, string>(subject =>
            RateLimitPartition.GetConcurrencyLimiter(
            subject,
            _ => new() { PermitLimit = 2, QueueLimit = 0 }
        )));
        services.AddCors(setupAction: cors => cors.AddPolicy(
            "PuckMcp",
            policy => policy.WithOrigins([.. origins]).WithMethods(
                "GET",
                "POST",
                "OPTIONS"
    ...
    private static void ConfigureTools(IServiceCollection services, RemoteMcpOptions options, RemoteMcpAccessPolicy access) {
        services.AddAuthorization(configure: authorization => authorization.AddPolicy(
            "PuckMcp",
            policy => policy.AddAuthenticationSchemes(McpAuthenticationDefaults.AuthenticationScheme).RequireAuthenticatedUser().RequireAssertion(handler: context =>
            (HasScope(
            ...
            ) is { } subject) && access.Allows(subject: subject) &&
            ((options.TenantId is null) || (SingleClaim(
                context.User,
                "tid"
            ) == options.TenantId))))
        ));
    ...
    public static RequestDelegate CreateRequestDelegate(IServiceProvider services) {
        ...
        return async context => {
            ...
            try { await pipeline(context).ConfigureAwait(continueOnCapturedContext: false); } finally { context.SetEndpoint(endpoint: endpoint); context.Request.RouteValues = routeValues; }
        };
    }
    ...
    private static void ConfigurePipeline(IApplicationBuilder app) {
        ...
        app.Use(middleware: async (context, next) => {
            if (
                ...
                ((context.Connection.RemoteIpAddress is not { } remote) || !IPAddress.IsLoopback(address: remote))
            ) {
            ...
            if (
                ...
                ((origin.Count != 1) || !origins.Contains(item: origin.ToString()))
            ) {
        ...
        app.Use(middleware: async (context, next) => {
            using var lease = (((SingleClaim(
                context.User,
                options.SubjectClaim
            ) is { } subject) && access.Allows(subject: subject))
                ? context.RequestServices.GetRequiredService<PartitionedRateLimiter<string>>().AttemptAcquire(subject)
                : null
            );
    ...
    private static bool HasScope(ClaimsPrincipal principal, string scope) {
        foreach (var claim in principal.Claims) {
            ...
            foreach (var range in value.Split(separator: ' ')) {
                if (
                    !value[range].IsEmpty &&
                    value[range].SequenceEqual(other: scope)
                ) { return true; }
            }
        }
        return false;
    ...
    private static string? SingleClaim(ClaimsPrincipal principal, string name) {
        ...
        return ((claims.MoveNext() || string.IsNullOrEmpty(value: value))
            ? null
            : value
        );
    }
