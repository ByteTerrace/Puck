...
internal sealed class RemoteMcpTools(RemoteAttachmentPool attachments, string owner, int idleTimeout, RemoteMcpDiagnostics diagnostics, RemoteMcpHost host, RemoteMcpCaller caller) {
    ...
    internal async ValueTask<CallToolResult> CallAsync(CallToolRequestParams? parameters, CancellationToken token) {
        ...
        try {
            ...
            if (result.StructuredContent is { } content) {
                ...
                )) { outcome = (status.GetString() ?? outcome); }
                if (content.TryGetProperty(
    ...
    internal async ValueTask<ListToolsResult> ListAsync(CancellationToken token) {
        ...
        if (host.SupportsAttachments) {
            tools.Add(item: AttachmentTool(detach: false)); tools.Add(item: AttachmentTool(detach: true));
            var capabilities = await host.DescribeControlAsync(
                caller: caller,
                cancellationToken: token
            ).ConfigureAwait(continueOnCapturedContext: false);
    ...
    private CallToolResult AttachmentResult(string? id, string output, bool error) {
        var metadata = JsonSerializer.SerializeToElement(
            new RemoteAttachmentMetadata(
                AttachmentId: id,
                IdleTimeoutSeconds: idleTimeout,
                IsError: error,
                Output: output
            ),
            OperatorMcpJson.Default.RemoteAttachmentMetadata
        );
    ...
    private static Tool AttachmentTool(bool detach) => new() {
        ...
        : "puck_attach"),
        Description = (detach
        ? "Close your delegated attachment and cancel its pending ingress. Already dispatched effects cannot be undone."
        : "Create an isolated delegated attachment to this gateway's World. Returns a caller-bound attachmentId for advertised control tools. At most two attachments per caller and four per host; idle expiry applies. Never automatically reattach and replay uncertain commands."),
        InputSchema = (detach
        ? DetachInput
        : AttachInput),
    ...
    private async ValueTask<CallToolResult> DispatchAsync(CallToolRequestParams? parameters, CancellationToken token) {
        ...
        if (
            ...
            caller: caller,
            cancellationToken: token
        ).ConfigureAwait(continueOnCapturedContext: false)).SupportsCapture
        ) {
            throw new McpProtocolException(
                errorCode: McpErrorCode.InvalidParams,
                message: "Unknown tool."
        ...
        if (parameters.Name == "puck_attach") {
            ...
            try {
                ...
                return AttachmentResult(
                    ...
                    id: id,
                    output: ((id is null)
                    ? "Attachment capacity reached: at most two per caller and four per host. Close an unused attachment before retrying."
                    : "Delegated World attachment created. Keep this handle private and call serially.")
                );
                ...
                return AttachmentResult(
                    error: true,
                    id: null,
                    output: "World attachment unavailable. Check account onboarding, World admission and target readiness."
                );
            }
        }
        var arguments = ((parameters.Arguments is { } supplied)
        ...
        if (
            ...
            return ((parameters.Name == "puck_detach")
                ? AttachmentResult(
                    error: true,
                    id: null,
                    output: "A valid attachmentId is required."
                )
                : OperatorMcpServer.Error(
                    message: "A valid attachmentId is required.",
                    unknown: false
                )
            );
        }
        ...
        if (parameters.Name == "puck_detach") {
            ...
            return AttachmentResult(
                ...
                id: null,
                output: (removed
                ? "Attachment closed. Dispatched effects may remain."
                : "Unknown, closed or expired attachment.")
            );
        }
        return await attachments.CallAsync(
            owner,
            idText,
            new() { Arguments = arguments, Name = parameters.Name },
            token
        ).ConfigureAwait(continueOnCapturedContext: false);
    }
