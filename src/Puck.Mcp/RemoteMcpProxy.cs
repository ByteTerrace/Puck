...
public static partial class RemoteMcpServer {
    ...
    private static void UseProxy(IApplicationBuilder app, RemoteMcpProxyOptions proxy) => app.Use(middleware: async (context, next) => {
        var origin = await context.AuthenticateAsync(scheme: ProxyAuthenticationScheme).ConfigureAwait(continueOnCapturedContext: false);
        ...
        if (
            ...
            ((proxy.TenantId is not null) && (SingleClaim(
            origin.Principal!,
            "tid"
        ) != proxy.TenantId))
        ) {
