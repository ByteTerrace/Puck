    ...
    internal RemoteMcpOptions Validate(bool requireListener = true) {
        static Uri Https(string value, string name) {
            if (
                ...
                (uri.Scheme != "https") ||
                !string.IsNullOrEmpty(value: uri.UserInfo) ||
                !string.IsNullOrEmpty(value: uri.Query) ||
                !string.IsNullOrEmpty(value: uri.Fragment)
            ) {
                throw new ArgumentException(message: $"{name} must be an absolute HTTPS URL without credentials, query or fragment.");
            }
            return uri;
        }
        var resource = Https(
        ...
        if (resource.AbsolutePath != "/mcp") { throw new ArgumentException(message: "publicUrl must end in /mcp."); }
        _ = Https(
            Issuer,
            nameof(Issuer)
        );
        ...
        if (
            ...
            UriKind.Absolute,
            out var listen
        ) || (listen.Scheme is not ("http" or "https")) || !IPAddress.TryParse(
            listen.Host.Trim(
                '[',
                ']'
            ),
            ...
            (listen.AbsolutePath != "/") || !string.IsNullOrEmpty(value: listen.UserInfo) || !string.IsNullOrEmpty(value: listen.Query) || !string.IsNullOrEmpty(value: listen.Fragment) ||
            ((listen.Scheme == "http") && !IPAddress.IsLoopback(address: address)))
        ) { throw new ArgumentException(message: "listenUrl must use an IP literal; plaintext HTTP is restricted to loopback behind a TLS proxy."); }
        if (
            requireListener &&
            (new Uri(uriString: ListenUrl!).Scheme == "https") &&
            string.IsNullOrWhiteSpace(value: CertificatePath)
        ) { throw new ArgumentException(message: "A direct HTTPS listener requires certificatePath."); }
        if (
            string.IsNullOrWhiteSpace(value: Audience) ||
            (string.IsNullOrWhiteSpace(value: Target) && (Services is null))
        ) { throw new ArgumentException(message: "audience and an in-process target or explicit service composition are required."); }
        if (
            string.IsNullOrEmpty(value: Scope) ||
            Scope.Any(predicate: c => (c is < '!' or > '~' or '"' or '\\'))
        ) { throw new ArgumentException(message: "scope must be one OAuth scope token."); }
        if (
            (AuthorizationScope is { } requested) &&
            ((requested.Length == 0) || requested.Any(predicate: c => (c is < '!' or > '~' or '"' or '\\')))
        ) { throw new ArgumentException(message: "authorizationScope must be one OAuth scope token."); }
        if (
            (SubjectClaim is not ("sub" or "oid")) ||
            ((SubjectClaim == "oid") && !Guid.TryParse(
            input: TenantId,
            result: out _
        ))
        ) { throw new ArgumentException(message: "subjectClaim must be sub, or oid with an explicit tenantId UUID."); }
        if (
            (AllowedSubjects is null) ||
            (AllowedSubjects.Length > 64) ||
            AllowedSubjects.Any(predicate: string.IsNullOrWhiteSpace)
        ) { throw new ArgumentException(message: "Explicit allowedSubjects are required (0..64); an empty list denies all callers."); }
        if (
            (AllowedOrigins is null) ||
            (AllowedOrigins.Length > 16)
        ) { throw new ArgumentException(message: "At most 16 additional browser origins are allowed."); }
        foreach (var origin in AllowedOrigins) {
            ...
            if (parsed.AbsolutePath != "/") { throw new ArgumentException(message: "Browser origins cannot contain paths."); }
        }
        if (IdleTimeoutSeconds is < 10 or > 3600) { throw new ArgumentOutOfRangeException(paramName: nameof(IdleTimeoutSeconds)); }
