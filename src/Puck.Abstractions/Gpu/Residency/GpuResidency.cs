...
public static class GpuResidency {
    ...
    /// is written in place on coherent unified memory and through a ring otherwise; a larger region, or any region on a
    /// device reporting no host-visible device-local memory (the default profile included), is staged.</summary>
    ...
    public static GpuResidencyPolicy Select(GpuMemoryProfile profile, ulong byteCount) {
        ...
        return (profile.CoherentUnifiedMemory
