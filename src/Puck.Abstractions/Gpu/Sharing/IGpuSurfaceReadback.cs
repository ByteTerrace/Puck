...
public interface IGpuSurfaceReadback : IDisposable {
    ...
    ReadOnlyMemory<byte> Read(IGpuDeviceContext deviceContext, nint sourceImageHandle, GpuPixelFormat format, uint width, uint height, uint bytesPerPixel, GpuImageLayout sourceLayout);
    ...
    /// on a later frame and only then <see cref="MapPixels"/> maps the result. The source image is restored to
    /// <paramref name="sourceLayout"/>. At most one read may be in flight per instance: this must not be
    ...
    /// <param name="height">The height, in pixels.</param>
    /// <param name="bytesPerPixel">The number of bytes per pixel.</param>
    void SubmitRead(IGpuDeviceContext deviceContext, nint sourceImageHandle, GpuPixelFormat format, uint width, uint height, uint bytesPerPixel, GpuImageLayout sourceLayout);
