...
public readonly record struct GpuComputeBinding {
    ...
    public GpuComputeBinding(uint Binding, GpuComputeBindingKind Kind, uint Count = 1) {
        if (!Enum.IsDefined(value: Kind)) {
            throw new ArgumentOutOfRangeException(nameof(Kind), Kind, "The descriptor kind is not defined.");
        }
    ...
    public static void ValidateSet(IReadOnlyList<GpuComputeBinding> bindings) {
        ...
        foreach (var binding in bindings) {
            if (!Enum.IsDefined(value: binding.Kind) || (0 == binding.Count)) {
                throw new ArgumentException(message: "The descriptor set contains an invalid kind or zero descriptor count.", paramName: nameof(bindings));
            }
            if (!seen.Add(item: binding.Binding)) {
                throw new ArgumentException(message: $"Descriptor binding {binding.Binding} is declared more than once in set 0.", paramName: nameof(bindings));
            }
        }
    }
}
