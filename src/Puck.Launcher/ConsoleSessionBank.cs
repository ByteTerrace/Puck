...
public sealed class ConsoleSessionBank : IConsoleSessions, ICommandObserver {
    ...
    public ConsoleSessionBank(
        ...
        for (var slot = 0; (slot < seatCount); slot++) {
            ...
            tape.VisibilityChanged = visible => VisibilityChanged(
                slot: capturedSlot,
                visible: visible
            );
        }
    ...
    public void RecordAdministrative(string line, CommandResult result) => m_sessions[0].Tape.Record(
        line: line,
        result: result
    );
    ...
    public void RecordAdministrativeEcho(string message, bool refused) => m_sessions[0].Tape.RecordEcho(
        message: message,
        refused: refused
    );
    ...
    public void TrackTextDevice(int slot, InputDeviceId device) {
        ...
        if (
            session.Devices.Add(item: device) &&
            session.Tape.Visible
        ) {
            m_router.SuppressHeld(device: device);
            m_focus.Release(deviceId: device);
        }
