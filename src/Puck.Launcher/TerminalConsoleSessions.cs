...
public sealed class TerminalConsoleSessions : IConsoleSessions {
    ...
    public void RecordAdministrative(string line, Puck.Commands.CommandResult result) {
        m_operatorTape.Record(line: line, result: result);
        m_attached?.RecordAdministrative(line: line, result: result);
    }
    ...
    public void RecordAdministrativeEcho(string message, bool refused) {
        m_operatorTape.RecordEcho(message: message, refused: refused);
        m_attached?.RecordAdministrativeEcho(message: message, refused: refused);
    }
    ...
    public bool TryGetVisible(int slot, out bool visible) {
        if (m_attached is { } attached) {
            return attached.TryGetVisible(slot: slot, visible: out visible);
        }
    ...
    public bool TrySetVisible(int slot, bool? visible, out bool resolved) {
        if (m_attached is { } attached) {
            return attached.TrySetVisible(slot: slot, visible: visible, resolved: out resolved);
        }
