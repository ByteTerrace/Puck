...
public sealed class ShaderProgram {
    ...
    internal ShaderProgram(IReadOnlyList<ShaderInstruction> instructions, IReadOnlyList<Vector4> constants) {
        ArgumentOutOfRangeException.ThrowIfGreaterThan(
            value: instructions.Count,
            other: ShaderIsa.MaxInstructions
        );
        ArgumentOutOfRangeException.ThrowIfGreaterThan(
            value: constants.Count,
            other: ShaderIsa.MaxConstants
        );
    ...
    private static (int Consumed, int Produced) StackEffect(ShaderOp op) => op switch {
        ...
        message: $"Shader operation {op} has no declared stack effect.",
        paramName: nameof(op)
    ),
    };
    ...
    private static void ValidateInstructions(IReadOnlyList<ShaderInstruction> instructions, int constantCount) {
        if (instructions.Count == 0) {
            throw new ArgumentException(
                message: "A Shader VM program must contain at least one instruction.",
        ...
        for (var index = 0; (index < instructions.Count); index++) {
            if (arrivals.Remove(
                ...
                if (
                    ...
                    throw new ArgumentException(
                        message: $"Shader instruction {index} is reached with stack depth {depth} by fallthrough and {arrivalDepth} by jump.",
                        paramName: nameof(instructions)
                    );
                }
            ...
            if (!reachable) {
                throw new ArgumentException(
                    message: $"Shader instruction {index} is unreachable.",
                    paramName: nameof(instructions)
                );
            }
            ...
            if (
                (instruction.Op == ShaderOp.LoadConstant) &&
                (instruction.Operand >= constantCount)
            ) {
                throw new ArgumentException(
                    message: $"Shader instruction {index} loads constant {instruction.Operand}, but the pool contains {constantCount} values.",
                    paramName: nameof(instructions)
                );
            }
            if (
                (instruction.Op == ShaderOp.Pick) &&
                (depth <= instruction.Operand)
            ) {
                throw new ArgumentException(
                    message: $"Shader instruction {index} picks {instruction.Operand} values back through a stack {depth} deep.",
                    paramName: nameof(instructions)
                );
            }
            if (depth < consumed) {
                throw new ArgumentException(
                    message: $"Shader instruction {index} ({instruction.Op}) underflows the value stack.",
                    paramName: nameof(instructions)
                );
            }
            ...
            if (depth > ShaderIsa.MaxStackDepth) {
                throw new ArgumentException(
                    message: $"Shader instruction {index} grows the value stack beyond {ShaderIsa.MaxStackDepth} values.",
                    paramName: nameof(instructions)
                );
            }
            ...
            switch (instruction.Op) {
                case ShaderOp.Jump or ShaderOp.JumpIfZero: {
                        ...
                        if (target <= index) {
                            throw new ArgumentException(
                                message: $"Shader instruction {index} jumps backward to {target}; the Shader VM admits forward jumps only.",
                                paramName: nameof(instructions)
                            );
                        }
                        if (target >= instructions.Count) {
                            throw new ArgumentException(
                                message: $"Shader instruction {index} jumps to {target}, past the {instructions.Count}-instruction stream.",
                                paramName: nameof(instructions)
                            );
                        }
                        if (
                            arrivals.TryGetValue(
                            key: target,
                            out var existing
                        ) &&
                            (existing != depth)
                            ...
                            throw new ArgumentException(
                                message: $"Shader instruction {target} is reached with stack depth {existing} and {depth} by different jumps.",
                                paramName: nameof(instructions)
                            );
                        }
                ...
                case ShaderOp.Halt: {
                        if (depth != 0) {
                            throw new ArgumentException(
                                message: $"Shader instruction {index} halts with {depth} values left beneath its result.",
                                paramName: nameof(instructions)
                            );
                        }
        ...
        if (reachable) {
            throw new ArgumentException(
                message: "A Shader VM program must end with Halt consuming its single result.",
                paramName: nameof(instructions)
            );
        }
        if (!halted) {
            throw new ArgumentException(
                message: "A Shader VM program contains no reachable Halt.",
                paramName: nameof(instructions)
            );
        }
        if (arrivals.Count != 0) {
            throw new ArgumentException(
                message: $"Shader jump target {arrivals.Keys.Min()} lies past the instruction stream.",
                paramName: nameof(instructions)
            );
        }
    }
    ...
    public static ShaderProgram FromWords(ReadOnlySpan<uint> words) {
        if (words.Length < ShaderIsa.HeaderWordCount) {
            throw new ArgumentException(
                message: $"A Shader VM program needs at least {ShaderIsa.HeaderWordCount} header words.",
                paramName: nameof(words)
            );
        }
        if (words[0] != ShaderIsa.Magic) {
            throw new ArgumentException(
                message: $"Shader VM magic 0x{words[0]:X8} does not match 0x{ShaderIsa.Magic:X8}.",
                paramName: nameof(words)
            );
        }
        if (words[1] != ShaderIsa.Version) {
            throw new ArgumentException(
                message: $"Shader VM ISA version {words[1]} does not match host version {ShaderIsa.Version}.",
                paramName: nameof(words)
            );
        }
        ...
        if (instructionCount > ShaderIsa.MaxInstructions) {
            throw new ArgumentException(
                message: $"The packed program declares {instructionCount} instructions; the maximum is {ShaderIsa.MaxInstructions}.",
                paramName: nameof(words)
            );
        }
        if (constantCount > ShaderIsa.MaxConstants) {
            throw new ArgumentException(
                message: $"The packed program declares {constantCount} constants; the maximum is {ShaderIsa.MaxConstants}.",
                paramName: nameof(words)
            );
        }
        ...
        if (words.Length != expectedWordCount) {
            throw new ArgumentException(
                message: $"The packed program declares {instructionCount} instructions and {constantCount} constants, requiring {expectedWordCount} words; it carries {words.Length}.",
                paramName: nameof(words)
            );
        }
