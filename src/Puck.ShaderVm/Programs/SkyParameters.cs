...
public static class SkyParameters {
    ...
    public static void Pack(in SkyFrameSettings settings, Span<Vector4> rows) {
        ...
        rows[Zenith] = new Vector4(
            x: settings.Zenith.X,
            y: settings.Zenith.Y,
            z: settings.Zenith.Z,
            w: settings.FogDensity
        );
        rows[Horizon] = new Vector4(
            x: settings.Horizon.X,
            y: settings.Horizon.Y,
            z: settings.Horizon.Z,
            w: 0f
        );
        rows[Ground] = new Vector4(
            x: settings.Ground.X,
            y: settings.Ground.Y,
            z: settings.Ground.Z,
            w: settings.SunDiscIntensity
        );
        rows[Sun] = new Vector4(
            x: settings.SunDirection.X,
            y: settings.SunDirection.Y,
            z: settings.SunDirection.Z,
            w: SunDiscExponent(discRadians: settings.SunDiscRadians)
        );
        rows[SunColor] = new Vector4(
            x: settings.SunColor.X,
            y: settings.SunColor.Y,
            z: settings.SunColor.Z,
            w: settings.StarBrightness
        );
        rows[Stars] = new Vector4(
            x: MathF.Max(
                x: settings.StarDensity,
                y: 1f
            ),
            y: settings.StarPhase,
            z: settings.TwinkleShare,
            w: settings.TwinkleDepth
        );
        rows[Clouds] = new Vector4(
            x: settings.CloudColor.X,
            y: settings.CloudColor.Y,
            z: settings.CloudColor.Z,
            w: settings.CloudCoverage
        );
        rows[CloudShape] = new Vector4(
            x: settings.CloudSoftness,
            y: MathF.Max(
                x: settings.CloudScale,
                y: 1e-3f
            ),
            z: settings.CloudSpin,
            w: settings.CloudCurl
        );
        rows[CloudWind] = new Vector4(
            x: settings.CloudDrift.X,
            y: settings.CloudDrift.Y,
            z: settings.CloudShear.X,
            w: settings.CloudShear.Y
        );
    }
}
