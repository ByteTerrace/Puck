...
public static class SkyProgram {
    ...
    private static Expression CloudLayer(Expression direction, Expression elevation, uint seed, Expression sun) {
        ...
        var alongX = Thickness(
            point: (point + Expression.Constant(
                x: CloudNormalTap,
                y: 0f
            )),
            seed: seed,
        ...
        var lining = ((CloudSilverLining * Pow(
            exponent: 8f,
            value: Saturate(value: Dot3(
                left: direction,
                right: sun
            ))
        )) * (1f - thickness));
        var shade = ((clouds * (Lerp(
            amount: diffuse,
            from: 0.45f,
            to: 1f
        ) * shadow)) + (Expression.Parameter(index: SkyParameters.SunColor) * lining));
        var alpha = (((1f - Exp(value: -(thickness * CloudOpacity))) * SmoothStep(
            edge0: 0f,
            edge1: CloudHorizonFade,
            value: elevation
        )) * Step(
            edge: 0f,
            value: (clouds.W - 1e-6f)
        ));
    ...
    private static Expression Gradient(Expression direction, Expression elevation) {
        ...
        return Select(
            ...
            whenFalse: Lerp(
                amount: Saturate(value: (elevation + 1f)),
                from: ground,
                to: horizon
            ),
            whenTrue: Lerp(
                amount: Saturate(value: elevation),
                from: horizon,
                to: zenith
            )
        );
    ...
    private static Expression OctahedralDecode(Expression point) {
        ...
        var folded = ((1f - Abs(value: raised.Swizzle(
            w: 0,
            x: 1,
            y: 0,
            z: 1
        ))) * SignPositive(value: raised));

        return Select(
            condition: (raised.Z < 0f),
            whenFalse: raised,
            whenTrue: Expression.Combine(
                x: folded.X,
                y: folded.Y,
                z: raised.Z
            )
    ...
    private static Expression OctahedralEncode(Expression direction) {
        var folded = (direction.Swizzle(
            w: 1,
            x: 0,
            y: 1,
            z: 0
        ) / ((Abs(value: direction.X) + Abs(value: direction.Y)) + Abs(value: direction.Z)));

        return Select(
            condition: (direction.Z < 0f),
            whenFalse: folded,
            whenTrue: ((1f - Abs(value: folded.Swizzle(
                w: 0,
                x: 1,
                y: 0,
                z: 1
            ))) * SignPositive(value: folded))
        );
    }
    ...
    private static Expression SignPositive(Expression value) => ((Step(
        edge: 0f,
        value: value
    ) * 2f) - 1f);
    ...
    private static Expression Spectrum(Expression temperature) {
        var cool = Expression.Constant(
            w: 1f,
            x: 0.71f,
            y: 0.80f,
            z: 1f
        );
        var warm = Expression.Constant(
            w: 1f,
            x: 1f,
            y: 0.71f,
            z: 0.42f
        );
        var white = Expression.Constant(
            w: 1f,
            x: 1f,
            y: 0.98f,
            z: 0.99f
        ...
        return Select(
            ...
            whenFalse: Lerp(
                amount: Saturate(value: (temperature * 2f)),
                from: warm,
                to: white
            ),
            whenTrue: Lerp(
                amount: Saturate(value: ((temperature * 2f) - 1f)),
                from: white,
                to: cool
            )
        );
    ...
    private static Expression StarField(Expression direction, uint seed) {
        ...
        var radius = (((StarRadiusFraction * MathF.PI) / density) * Lerp(
            amount: Sqrt(value: twinkled),
            from: 0.6f,
            to: 1f
        ));
    ...
    private static Expression Thickness(Expression point, Expression shear, uint seed, Expression threshold, Expression softness) {
        ...
        var density = Fbm2(
            octaves: CloudOctaves,
            position: Seeded(
                position: (point + (CloudWarp * (warp - 0.5f))),
                seed: seed
            )
        );
