...
public static class ShaderInterpreter {
    ...
    private static Vector4 Binary(Vector4 left, ShaderOp op, Vector4 right) => op switch {
        ...
        ShaderOp.Minimum => Vector4.Min(
        value1: left,
        value2: right
    ),
        ShaderOp.Maximum => Vector4.Max(
        value1: left,
        value2: right
    ),
        ShaderOp.Power => Zip(
        ...
        ShaderOp.ArcTangent2 => Zip(
        left: left,
        right: right,
        zip: MathF.Atan2
    ),
        _ => throw new ArgumentException(
        message: $"Shader operation {op} is not binary.",
        paramName: nameof(op)
    ),
    };
    ...
    private static float Fbm2(float x, float y, uint seed, int octaves) {
        ...
        for (var octave = 0; (octave < octaves); octave++) {
            value += (amplitude * LatticeNoise2(
                seed: (seed + ((uint)octave)),
                x: x,
                y: y
            ));
    ...
    private static Vector4 Field(ShaderOp op, int octaves, Vector4 value) {
        ...
        return op switch {
            ShaderOp.ValueNoise2 => new Vector4(value: LatticeNoise2(
            seed: seed,
            x: value.X,
            y: value.Y
        )),
            ShaderOp.ValueNoise3 => new Vector4(value: LatticeNoise3(
            seed: seed,
            x: value.X,
            y: value.Y,
            z: value.Z
        )),
            ShaderOp.Fbm2 => new Vector4(value: Fbm2(
            octaves: octaves,
            seed: seed,
            x: value.X,
            y: value.Y
            ...
            _ => throw new ArgumentException(
            message: $"Shader operation {op} is not a field sample.",
            paramName: nameof(op)
        ),
        };
    }
    ...
    private static float LatticeNoise3(float x, float y, float z, uint seed) {
        ...
        var lower = LatticeNoise2(
            x: x,
            y: y,
            seed: seed ^ BitConverter.SingleToUInt32Bits(value: cellZ)
        );
        var upper = LatticeNoise2(
            x: x,
            y: y,
            seed: seed ^ BitConverter.SingleToUInt32Bits(value: (cellZ + 1f))
        );
    ...
    private static Vector4 Ternary(Vector4 a, Vector4 b, Vector4 c, ShaderOp op) => op switch {
        ...
        _ => throw new ArgumentException(
        message: $"Shader operation {op} is not ternary.",
        paramName: nameof(op)
    ),
    };
    private static Vector4 Unary(ShaderOp op, Vector4 value) => op switch {
        ...
        ShaderOp.Ceiling => Map(
        map: MathF.Ceiling,
        value: value
    ),
        ShaderOp.Fraction => Map(
        value: value,
        map: component => (component - MathF.Floor(x: component))
    ),
        ShaderOp.Saturate => Vector4.Clamp(
        value1: value,
        min: Vector4.Zero,
        max: Vector4.One
    ),
        ShaderOp.Truncate => Map(
        map: MathF.Truncate,
        value: value
    ),
        ShaderOp.Round => Map(
        ...
        ShaderOp.Sign => Map(
        value: value,
        map: component => ((float)MathF.Sign(x: component))
    ),
        ...
        ShaderOp.Exponential => Map(
        map: MathF.Exp,
        value: value
    ),
        ShaderOp.NaturalLogarithm => Map(
        map: MathF.Log,
        value: value
    ),
        ShaderOp.Sine => Map(
        ...
        ShaderOp.Normalize2 => Normalize(value: new Vector4(
        w: 0f,
        x: value.X,
        y: value.Y,
        z: 0f
    )),
        ShaderOp.Normalize3 => Normalize(value: new Vector4(
        w: 0f,
        x: value.X,
        y: value.Y,
        z: value.Z
        ...
        ShaderOp.BitsToUnitFloat => Map(
        value: value,
        map: component => (((float)BitConverter.SingleToUInt32Bits(value: component)) * ShaderIsa.InverseTwoPow32)
    ),
        ShaderOp.IntegerBits => Map(
        value: value,
        map: component => BitConverter.UInt32BitsToSingle(value: ((uint)component))
    ),
        _ => throw new ArgumentException(
        message: $"Shader operation {op} is not unary.",
        paramName: nameof(op)
    ),
    };
