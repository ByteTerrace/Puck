...
public sealed class ShaderProgramBuilder {
    ...
    public ShaderProgram Build() {
        if (
            (m_instructions.Count == 0) ||
            (m_instructions[^1].Op != ShaderOp.Halt)
        ) {
    ...
    /// <returns>This builder.</returns>
    /// <exception cref="InvalidOperationException">The program already contains the maximum constant count.</exception>
    public ShaderProgramBuilder LoadConstant(Vector4 value) => Append(
        op: ShaderOp.LoadConstant,
        operand: Intern(value: value)
    );
    ...
    /// <returns>This builder.</returns>
    public ShaderProgramBuilder LoadInput(ShaderInput input) => Append(
    ...
    /// <returns>This builder.</returns>
    public ShaderProgramBuilder LoadLocal(int index) => Append(
        op: ShaderOp.LoadLocal,
        operand: checked((uint)index)
    );
    /// <summary>Appends an operation that loads one caller-supplied parameter.</summary>
    /// <param name="index">The parameter index.</param>
    /// <returns>This builder.</returns>
    public ShaderProgramBuilder LoadParameter(int index) => Append(
    ...
    /// <param name="index">The register index.</param>
    /// <returns>This builder.</returns>
    public ShaderProgramBuilder StoreLocal(int index) => Append(
        op: ShaderOp.StoreLocal,
        operand: checked((uint)index)
    );
