...
public static class ShaderSdf {
    ...
    public static ShaderExpression Intersection(ShaderExpression current, ShaderExpression candidate) => Sample(
        distance: Max(
            left: Distance(sample: current),
            right: Distance(sample: candidate)
        ),
    ...
    /// <returns>The signed distance.</returns>
    public static ShaderExpression Plane(ShaderExpression point, ShaderExpression normal, ShaderExpression offset) => (Dot3(
        left: point,
        right: normal
    ) + offset);
    ...
    public static ShaderExpression Rotate(ShaderExpression point, ShaderExpression quaternion) {
        var axis = quaternion.Swizzle(
            w: 3,
            x: 0,
            y: 1,
            z: 2
        );
        var first = ShaderExpression.Binary(
            left: axis,
            op: ShaderOp.Cross3,
            right: point
        );
        var second = ShaderExpression.Binary(
            left: axis,
            op: ShaderOp.Cross3,
            right: first
        );
    ...
    public static ShaderExpression Sample(ShaderExpression distance, ShaderExpression material) => ShaderExpression.Combine(
        x: distance,
        y: material
    );
    ...
    public static ShaderExpression SmoothUnion(ShaderExpression current, ShaderExpression candidate, ShaderExpression radius) {
        ...
        return Sample(
            distance: (Lerp(
                ...
                from: a,
                to: b
            ) - ((radius * weight) * (1f - weight))),
            material: Material(sample: Union(
                candidate: candidate,
                current: current
            ))
        );
    ...
    public static ShaderExpression WallpaperFoldP1(ShaderExpression point, ShaderExpression cell, out ShaderExpression cellIndex) {
        var lattice = point.Swizzle(
            w: 2,
            x: 0,
            y: 2,
            z: 0
        );
        var index = ShaderExpression.Unary(
            op: ShaderOp.Round,
            value: (lattice / cell)
        );
