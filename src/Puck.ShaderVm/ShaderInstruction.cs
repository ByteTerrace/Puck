...
public readonly record struct ShaderInstruction(ShaderOp Op, uint Operand = 0u) {
    ...
    private static void ValidateEnum<T>(T value, string paramName) where T : struct, Enum {
        if (!Enum.IsDefined(value: value)) {
            throw new ArgumentException(
                message: $"Shader instruction names undeclared {typeof(T).Name} value {Convert.ToUInt64(value: value)}.",
                paramName: paramName
            );
        }
    }
    private static void ValidateOperand(ShaderOp op, uint operand, string paramName) {
        if (!TakesOperand(op: op)) {
            if (operand != 0u) {
                throw new ArgumentException(
                    message: $"Shader operation {op} does not accept an operand.",
                    paramName: paramName
                );
            }
        ...
        switch (op) {
            case ShaderOp.LoadInput when !Enum.IsDefined(value: ((ShaderInput)operand)):
                throw new ArgumentException(
                    message: $"Shader instruction names undeclared {nameof(ShaderInput)} value {operand}.",
                    paramName: paramName
                );
            case ShaderOp.Swizzle when (operand > 0xFFu):
                throw new ArgumentException(
                    message: $"Shader swizzle operand 0x{operand:X} uses bits outside the four two-bit lane selectors.",
                    paramName: paramName
                );
            case ShaderOp.LoadLocal or ShaderOp.StoreLocal when (operand >= ShaderIsa.MaxLocals):
                throw new ArgumentException(
                    message: $"Shader instruction addresses local {operand}; the register file holds {ShaderIsa.MaxLocals}.",
                    paramName: paramName
                );
            case ShaderOp.Pick when (operand >= ShaderIsa.MaxStackDepth):
                throw new ArgumentException(
                    message: $"Shader instruction picks {operand} values back; the stack holds {ShaderIsa.MaxStackDepth}.",
                    paramName: paramName
                );
            case ShaderOp.Fbm2 when ((operand == 0u) || (operand > ShaderIsa.MaxOctaves)):
                throw new ArgumentException(
                    message: $"Shader fbm asks for {operand} octaves; the interpreter evaluates one through {ShaderIsa.MaxOctaves}.",
                    paramName: paramName
                );
            case ShaderOp.Jump or ShaderOp.JumpIfZero when (operand > ShaderIsa.MaxInstructions):
                throw new ArgumentException(
                    message: $"Shader instruction jumps to {operand}; the stream holds at most {ShaderIsa.MaxInstructions}.",
                    paramName: paramName
                );
            default:
                break;
    ...
    public uint Pack() {
        ...
        if (Operand > MaxOperand) {
            throw new ArgumentOutOfRangeException(
                ...
                actualValue: Operand,
                message: $"A Shader VM operand may not exceed 0x{MaxOperand:X6}."
            );
        }
        ValidateOperand(
            op: Op,
