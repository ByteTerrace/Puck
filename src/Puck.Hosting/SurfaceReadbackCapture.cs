...
public static class SurfaceReadbackCapture {
    ...
    /// <param name="deviceContext">The calling backend's device context, whose surface-transfer factory creates the
    /// import and readback objects the caller caches.</param>
    ...
    public static Surface ReadSurface(Surface surface, IGpuDeviceContext deviceContext, Func<SurfaceFormat, GpuPixelFormat> toGpuFormat, ref IGpuSurfaceImport? captureImport, ref IGpuSurfaceReadback? captureReadback) {
        var format = toGpuFormat(surface.Format);
        var imageHandle = surface.ImageHandle;
        var surfaceTransferFactory = deviceContext.Services.SurfaceTransferFactory;
