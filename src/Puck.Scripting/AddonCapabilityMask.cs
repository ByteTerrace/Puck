...
public static class AddonCapabilityMask {
    ...
    /// capability, deleted with the rest of the lane axis, owner ruling 2026-08-02). The bit is never compacted and
    /// never reused: retiring the hole rather than repacking the mask keeps every OTHER bit's meaning fixed, so no
    /// guest rebuild is forced by the deletion itself. No engine capability maps to this bit any more — a guest
    /// naming it in an <c>Ask</c>'s mask resolves to no capability, the same as any other undefined bit.</summary>
