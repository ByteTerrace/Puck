...
public static class SealedCarriage {
    ...
    public static byte[] Unseal(ECDiffieHellman recipientPrivateKey, SealedPayload payload, ReadOnlySpan<byte> associatedData) {
        ...
        if (!CarriageCurves.IsNistP256(curve: recipientPrivateKey.ExportParameters(includePrivateParameters: false).Curve)) {
            throw new FormatException(message: "The sealed carriage recipient key is not on P-256, which is the only curve the sealing algorithm names.");
        }
    ...
    /// calls this only after authenticating the envelope, so an attacker-controlled EC point is not imported
    ...
    internal static void ValidatePayloadStructure(SealedPayload payload) {
        ...
        if (payload.EphemeralPublicKeySubjectPublicKeyInfo.Length > CarriageResourceLimits.SubjectPublicKeyInfoBytes) {
            throw new FormatException(message: $"A sealed payload's ephemeral SPKI is {payload.EphemeralPublicKeySubjectPublicKeyInfo.Length} bytes; the base profile permits at most {CarriageResourceLimits.SubjectPublicKeyInfoBytes}.");
        }
    ...
    private static void ValidateRecipientId(KeyId recipientId, ReadOnlySpan<byte> recipientPublicKeySubjectPublicKeyInfo) {
        ...
        if (!string.Equals(
            ...
            throw new FormatException(message: "The sealed carriage recipient id does not identify the supplied recipient key.");
        }
    }
    ...
    private static void ValidateSealingAlgorithm(KeyId recipientId) {
        ...
        CarriageAlgorithmDescriptor descriptor;

        try {
            descriptor = CarriageAlgorithms.Resolve(algorithm: recipientId.Algorithm);
        } catch (NotSupportedException exception) {
            throw new FormatException(
                message: $"The sealed carriage recipient id names unsupported algorithm '{recipientId.Algorithm}'.",
                innerException: exception
            );
        }

        if (descriptor.Role != CarriageKeyRole.Sealing) {
            throw new FormatException(message: $"The sealed carriage recipient id names '{recipientId.Algorithm}', which is a {descriptor.Role.ToString().ToLowerInvariant()} algorithm rather than a sealing algorithm.");
        }

        if (!string.Equals(
            a: descriptor.Name,
            b: CarriageAlgorithms.EcdhP256HkdfSha256Aes256Gcm,
            comparisonType: StringComparison.Ordinal
        )) {
            throw new FormatException(message: $"The sealed carriage implementation does not support recipient sealing algorithm '{descriptor.Name}'.");
        }
    }
    ...
    private static byte[] DecodeFingerprint(string value, string what) {
        ...
        try {
            ...
            throw new FormatException(
                message: $"The sealed carriage recipient {what} is not a SHA-256 fingerprint.",
                innerException: exception
            );
        }
        ...
        if (
            ...
            throw new FormatException(message: $"The sealed carriage recipient {what} must be a 32-byte lowercase hexadecimal SHA-256 fingerprint.");
        }
