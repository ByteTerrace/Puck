...
/// stamps a <see cref="Principal"/> from.</summary>
...
/// device, a network peer stand-in — so a mixer that synthesized <see cref="Principal.Seat"/> from the slot
...
public interface IPrincipalResolver {
    ...
    /// seat otherwise. Never <see cref="PrincipalKind.Unspecified"/> for a slot the host admits.</summary>
    /// <param name="slot">The logical player slot.</param>
    /// <returns>The slot's acting identity.</returns>
    Principal PrincipalOf(int slot);
}
