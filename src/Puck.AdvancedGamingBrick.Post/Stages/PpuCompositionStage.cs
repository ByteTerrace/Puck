...
internal sealed class PpuCompositionStage : IPostStage<PostContext> {
    ...
    private static AgbPpu BuildPpu(AgbScheduler scheduler, int priorities, int objectPriority) {
        ...
        for (var background = 0; (background < 4); ++background) {
            ...
            for (var tile = 0; (tile < 2); ++tile) {
                ppu.WriteVideo(
                    address: ((0x0600E000u + (((uint)background) * 0x800u)) + (((uint)tile) * 2u)),
                    value: ((uint)((background << 12) | tile)),
                    width: 2
                );
                ...
                for (var y = 0; (y < 8); ++y) {
                    for (var x = 0; (x < 8); x += 4) {
                        ...
                        ppu.WriteVideo(
                            address: ((0x06000000u + (((uint)background) * 0x4000u))
                            + ((uint)(((tile * 32) + (y * 4)) + (x / 2)))),
                            value: pixels,
                            width: 2
                        );
                    }
        ...
        for (var index = 1; (index < 128); ++index) {
            ppu.WriteVideo(
                address: (0x07000000u + (((uint)index) * 8u)),
                value: 0x0200,
                width: 2
            );
        }
        ...
        ppu.WriteVideo(
            address: 0x07000004u,
            value: (((uint)objectPriority) << 10),
            width: 2
        );

        for (var offset = 0u; (offset < 64u); offset += 2u) {
            ppu.WriteVideo(
                address: (0x06010000u + offset),
                value: 0x1111,
                width: 2
            );
        }
    ...
    private static uint ExpectedPixel(int priorities, int objectPriority, int visible, bool effectsAllowed, int effect, bool semiTransparent) {
        ...
        var layers = Enumerable.Range(
            count: 6,
            start: 0
        )
            .Where(predicate: id => ((id == 5) || ((visible & (1 << id)) != 0)))
            .OrderBy(keySelector: id => ((id == 5)
            ? 40
            : ((id == 4)
                ? (objectPriority * 8)
                : (((((priorities >> (id * 2)) & 3) * 8) + id) + 1))))
            .ToArray();
        var top = layers[0];
        var second = ((layers.Length > 1)
            ? layers[1]
            : 5
        );
        ...
        for (var priorities = 0; (priorities < 256); ++priorities) {
            for (var objectPriority = 0; (objectPriority < 4); ++objectPriority) {
                ...
                for (var variant = 0; (variant < Masks.Length); ++variant) {
                    ...
                    ppu.WriteRegister(
                        offset: 0x50u,
                        value: ((ushort)(0x2A15 | (effect << 6)))
                    );
                    ppu.WriteVideo(
                        address: 0x07000000u,
                        value: ((uint)(0x4000 | variant | (semiTransparent
                        ? 0x400
                        : 0))),
                        width: 2
                    );
                    ...
                    for (var x = 0; (x < 16); ++x) {
                        var expected = ExpectedPixel(
                            priorities: priorities,
                            objectPriority: objectPriority,
                            visible: (x | 0x10) & Masks[variant],
                            effectsAllowed: ((Masks[variant] & 0x20) != 0),
                            effect: effect,
