...
internal sealed class HostPersistenceStage : IPostStage<PostContext> {
    ...
    private static void CheckBios(string directory) {
        ...
        using var stub = engine.Create(
            options: "stub",
            contentBytes: SyntheticRom.Create()
        );
        var biosPath = Path.Combine(
            path1: directory,
            path2: "bios.bin"
        );
        ...
        using var supplied = engine.Create(
            options: $"bios={biosPath}",
            contentBytes: SyntheticRom.Create()
        );
    }
    private static void CheckPersistence(string directory) {
        ...
        Encoding.ASCII.GetBytes(s: "SRAM_V").CopyTo(
            array: rom,
            index: (rom.Length - 16)
        );
        ...
        using (var seed = new AdvancedGamingBrickCore(
            bios: bios,
            cartridgeRom: rom,
            savePath: path
        )) {
            ...
            Require(
                ...
                detail: "first forced flush did not create a save"
            );
        }
        ...
        using (var core = new AdvancedGamingBrickCore(
            ...
            using var sibling = PostMachine.BuildInstance(
                bios: bios,
                rom: rom
            );
            ...
            Require(
                condition: (File.ReadAllBytes(path: path)[0] == 0x22),
                detail: "restored state did not flush without a cartridge write"
            );
            core.RestoreState(
                buffer: clean,
                length: cleanLength
            ...
            core.FlushSave(force: false);
            Require(
                condition: (File.ReadAllBytes(path: path)[0] == 0x11),
                detail: "restoring a clean snapshot left the abandoned future on disk"
            );
            var after = new byte[clean.Length];
            var afterLength = core.CaptureState(buffer: ref after);

            Require(
                condition: ((afterLength == cleanLength) && clean.AsSpan(
                    length: cleanLength,
                    start: 0
                ).SequenceEqual(other: after.AsSpan(
                    length: afterLength,
                    start: 0
                ))),
                detail: "host persistence changed clean snapshot bytes"
            );
            ...
            if (OperatingSystem.IsWindows()) {
                ...
                using (var held = new FileStream(
                    ...
                    Require(
                        condition: (File.ReadAllBytes(path: path)[0] == 0x11),
                        detail: "failed save replacement damaged the previous file"
                    );
                }
                core.FlushSave(force: false);
                Require(
                    condition: (File.ReadAllBytes(path: path)[0] == 0x22),
                    detail: "failed replacement was marked clean instead of retried"
                );
            }
            core.RestoreState(
                buffer: clean,
            ...
            if (OperatingSystem.IsWindows()) {
                using (var held = new FileStream(
                    ...
                    core.FlushSave(force: true);
                    Require(
                        condition: (File.ReadAllBytes(path: path)[0] == 0x33),
                        detail: "failed forced flush damaged the previous file"
                    );
                }
                core.FlushSave(force: false);
                Require(
                    condition: (File.ReadAllBytes(path: path)[0] == 0x11),
                    detail: "failed clean forced flush did not retry"
                );
                File.WriteAllBytes(
                    bytes: initial,
                    path: path
            ...
            Require(
                condition: (File.ReadAllBytes(path: path)[0] == 0x11),
                detail: "forced flush skipped a clean existing save"
            );
            core.RestoreState(
                buffer: future,
                length: future.Length
        ...
        Require(
            condition: (File.ReadAllBytes(path: path)[0] == 0x22),
            detail: "disposal did not persist restored state"
        );
        Require(
    ...
    public PostStageOutcome Run(PostContext context) {
        var directory = Path.Combine(
            ...
            path2: $"host-persistence-{Guid.NewGuid():N}"
        );
