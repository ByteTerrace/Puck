...
internal sealed class FirmwareOracleComparison : IDisposable {
    ...
    public FirmwareOracleComparison(byte[] retail) {
        m_puck = FirmwareSwiProbe.Run(
            number: 8,
            thumb: false,
            r0: 0
        );
        try {
            m_retail = FirmwareSwiProbe.Run(
                number: 8,
                thumb: false,
                r0: 0,
                bios: retail
    ...
    private bool Call(byte number, bool thumb, uint r0, uint r1, uint r2, uint r3, Action<AdvancedGamingBrickMachine>? setup, string label) {
        ...
        if (retailReturned != puckReturned) {
            Mismatch(
                detail: $"{label}: Puck {(puckReturned
                ? "returned"
                : "exceeded 2,000,000 instructions")}, retail {(retailReturned
                ? "returned"
                : "exceeded 2,000,000 instructions")}",
                number: number
            );
        } else {
            ++m_bothUnreturned;
    ...
    private void Mismatch(byte number, string detail) {
        ++m_mismatches;
        m_failuresByService.TryGetValue(
            key: number,
            value: out var count
        );
    ...
    private static string? ObserveNonreturn(AdvancedGamingBrickCore core, byte number, bool thumb, uint r0, uint r1) {
        ...
        if (
            (core.CycleCount <= checkpoint.TakenAt) ||
            (core.Instance.Machine.Cpu.GetRegister(index: 15) >= ReplacementBios.ImageSize)
        ) {
    ...
    public void Memory(byte number, bool thumb, byte[] source, int outputLength, string name, uint r2 = 0, uint r3 = 0, byte[]? info = null, bool vram = false) {
        ...
        void Setup(AdvancedGamingBrickMachine machine) {
            ...
            for (var index = 0; (index < comparedLength); index += 2) {
                machine.Bus.Write16(
                    ...
                    address: ((destination - 16) + ((uint)index)),
                    value: 0xCCCC
                );
            }
        }
        ...
        for (var index = 0; (index < comparedLength); ++index) {
            ...
            var expected = m_retail.Instance.Machine.Bus.Read8(
                access: BusAccessType.NonSequential,
                address: address
            );
            var actual = m_puck.Instance.Machine.Bus.Read8(
                access: BusAccessType.NonSequential,
                address: address
            );
            ...
            if (actual != expected) {
                Mismatch(
                    detail: $"{label} output offset {(index - 16)}: Puck={actual:X2}, retail={expected:X2}",
                    number: number
                );
                return;
            }
    ...
    public void NonreturningDivision(byte number, bool thumb, uint r0, uint r1) {
        var numerator = ((int)((number == 6)
            ? r0
            : r1));
        var denominator = ((number == 6)
            ? r1
            : r0
        );

        if (
            (number is not (6 or 7)) ||
            (denominator != 0) ||
            (numerator is >= -1 and <= 1)
        ) {
        ...
        if (
            ...
            Mismatch(
                detail: $"SWI {number:X2} {(thumb
                ? "Thumb"
                : "ARM")} bounded zero divisor in={r0:X8},{r1:X8}: Puck {(puck ?? "remained responsive inside BIOS")}; retail {(retail ?? "remained responsive inside BIOS")}",
                number: number
            );
        }
    }
    ...
    public PostStageOutcome Outcome() {
        ...
        if (m_mismatches != 0) {
            var groups = string.Join(
                separator: ", ",
                values: m_failuresByService.Select(selector: pair => $"SWI {pair.Key:X2}={pair.Value}")
            );

            return PostStageOutcome.Fail(detail: $"{summary}; {groups}; {string.Join(
                separator: "; ",
                values: m_examples
            )}");
        }
        return ((m_bothUnreturned == 0)
            ...
            : PostStageOutcome.Fail(detail: $"{summary}; bounded calls that did not return remain unresolved; {string.Join(
                separator: "; ",
                values: m_examples
            )}")
        );
    ...
    public void Registers(byte number, bool thumb, uint r0, int[] registers, uint r1 = 0, uint mask = uint.MaxValue) {
        ...
        foreach (var register in registers) {
            ...
            if (actual != expected) {
                Mismatch(
                    detail: $"{label} r{register}: Puck={actual:X8}, retail={expected:X8}",
                    number: number
                );
                return;
            }
    ...
    public void SoundBias(bool thumb, ushort initial, uint flag) {
        ...
        var label = $"SWI 19 {(thumb
            ? "Thumb"
            : "ARM")} initial={initial:X4}, flag={flag:X8}";
        ...
        if (!Call(
            ...
            r2: 0,
            r3: 0,
            setup: machine => machine.Bus.Write16(
                access: BusAccessType.NonSequential,
                address: Address,
                value: initial
            ),
        ...
        var expected = m_retail.Instance.Machine.Bus.Read16(
            access: BusAccessType.NonSequential,
            address: Address
        );
        var actual = m_puck.Instance.Machine.Bus.Read16(
            access: BusAccessType.NonSequential,
            address: Address
        );
        ...
        if (actual != expected) {
            Mismatch(
                detail: $"{label} SOUNDBIAS: Puck={actual:X4}, retail={expected:X4}",
                number: 0x19
            );
        }
    }
