        ...
        for (var phase = 0; (phase < 256); ++phase) {
            ...
            for (var index = 0; (index < Scales.Length); ++index) {
                ...
                BinaryPrimitives.WriteInt32LittleEndian(
                    destination: bg,
                    value: (0x123456 + (index * 17))
                );
                BinaryPrimitives.WriteInt32LittleEndian(
                    destination: bg[4..],
                    value: (-0x234567 - (index * 31))
                );
                BinaryPrimitives.WriteInt16LittleEndian(
                    destination: bg[8..],
                    value: ((short)(137 - (index * 19)))
                );
                BinaryPrimitives.WriteInt16LittleEndian(
                    destination: bg[10..],
                    value: ((short)(-49 + (index * 13)))
                );
                var obj = objects.AsSpan(
                    length: 8,
                    start: (index * 8)
                );
                ...
                BinaryPrimitives.WriteInt16LittleEndian(
                    destination: obj,
                    value: Scales[index].X
                );
                BinaryPrimitives.WriteInt16LittleEndian(
                    destination: obj[2..],
                    value: Scales[index].Y
                );
                BinaryPrimitives.WriteUInt16LittleEndian(
                    destination: obj[4..],
                    value: ((ushort)((phase << 8) | ((phase + (index * 37)) & 255)))
                );
            ...
            comparison.Memory(
                ...
                thumb: thumb,
                source: background,
                outputLength: (Scales.Length * 16),
                name: $"BgAffine phase={phase:X2}, eight signed scale/origin records",
                r2: ((uint)Scales.Length)
            );
            ...
            foreach (var stride in new uint[] { 0, 1, 2, 3, 8 }) {
                comparison.Memory(
                    ...
                    outputLength: Math.Max(
                        val1: 2,
                        val2: ((Scales.Length * 4) * ((int)stride))
                    ),
                    name: $"ObjAffine phase={phase:X2}, eight signed scale records, stride={stride}",
                    r2: ((uint)Scales.Length),
                    r3: stride
                );
            }
    ...
    public static void CheckMemory(FirmwareOracleComparison comparison, bool thumb) {
            ...
            foreach (var word in new[] { false, true }) {
                foreach (var fill in new[] { false, true }) {
                    comparison.Memory(
                        ...
                        thumb: thumb,
                        source: data,
                        outputLength: (((int)count) * (word
                        ? 4
                        : 2)),
                        name: $"CpuSet count={count}, word={word}, fill={fill}",
                        r2: count | (word
                        ? (1u << 26)
                        : 0) | (fill
                        ? (1u << 24)
        ...
        foreach (var count in new uint[] { 8, 16, 64 }) {
            foreach (var fill in new[] { false, true }) {
                comparison.Memory(
                    ...
                    thumb: thumb,
                    source: data,
                    outputLength: (((int)count) * 4),
                    name: $"CpuFastSet count={count}, fill={fill}",
                    r2: count | (fill
                    ? (1u << 24)
                    : 0)
                );
        ...
        foreach (var sourceWidth in new byte[] { 1, 2, 4, 8 }) {
            foreach (var destinationWidth in new byte[] { 1, 2, 4, 8, 16, 32 }) {
                ...
                foreach (var offset in ((destinationWidth > sourceWidth)
                    ? new uint[] { 0, 1, 0x80000001 }
                    : [0u])) {
                    byte[] info = [16, 0, sourceWidth, destinationWidth, 0, 0, 0, 0];

                    BinaryPrimitives.WriteUInt32LittleEndian(
                        destination: info.AsSpan(start: 4),
                        value: offset
                    );
                    comparison.Memory(
                        number: 16,
                        thumb: thumb,
                        source: data[..16],
                        outputLength: ((16 * destinationWidth) / sourceWidth),
                        name: $"BitUnpack {sourceWidth}-to-{destinationWidth}, offset={offset:X8}",
        ...
        foreach (var vram in new[] { false, true }) {
            comparison.Memory(
                ...
                outputLength: 64,
                name: "LZ77 overlapping distance-two runs",
                vram: vram
            );
            comparison.Memory(
                number: ((byte)(vram
                ...
                : 0x11)),
                thumb: thumb,
                source: LiteralLz(data: data.AsSpan(
                    length: 72,
                    start: 0
                )),
                outputLength: 72,
                name: "LZ77 multi-flag literals",
                vram: vram
            );
            comparison.Memory(
                number: ((byte)(vram
                ...
                outputLength: 72,
                name: "RL literals and a 64-byte run",
                vram: vram
            );
            ...
            data.AsSpan(
                length: 64,
                start: 0
            ).CopyTo(destination: differential.AsSpan(start: 4));
            comparison.Memory(
                number: ((byte)(vram
                ? 0x17
                : 0x16)),
            ...
            comparison.Memory(
                ...
                source: differential,
                outputLength: 64,
                name: "Diff16 modulo-halfword carry",
                vram: vram
            );
        }
        comparison.Memory(
            number: 0x13,
            thumb: thumb,
            source: [0x28, 32, 0, 0, 1, 0xC0, 0xA5, 0x5A, 0x78, 0x56, 0x34, 0x12],
            outputLength: 32,
            name: "Huffman byte symbols and a full 32-bit bitstream"
        );
        comparison.Memory(
            number: 0x13,
            thumb: thumb,
            source: [0x24, 16, 0, 0, 1, 0xC0, 1, 15, 0x78, 0x56, 0x34, 0x12],
            outputLength: 16,
            name: "Huffman nibble symbols and word packing"
        );
        comparison.Memory(
            number: 0x13,
            thumb: thumb,
            source: [0x28, 8, 0, 0, 3, 0x80, 0x41, 0xC0, 0x42, 0x43, 0, 0, 0, 0, 0xD0, 0x5A],
            outputLength: 8,
            name: "Huffman unequal-depth three-symbol tree"
        );
    }
