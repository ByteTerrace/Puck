...
internal sealed class BundledFirmwareStage : IPostStage<PostContext> {
    ...
    private static void CheckAffine(bool thumb) {
        using var background = FirmwareSwiProbe.Run(
            number: 0x0E,
            thumb: thumb,
            r0: FirmwareSwiProbe.Source,
            ...
            FirmwareSwiProbe.WriteBytes(
                ...
                bytes: [0, 10, 0, 0, 0, 20, 0, 0, 3, 0, 4, 0, 0, 1, 0, 1, 0, 0, 0, 0]
            )
        );

        FirmwareSwiProbe.ExpectBytes(
            bus: background.Instance.Machine.Bus,
            address: FirmwareSwiProbe.Destination,
            expected: [0, 1, 0, 0, 0, 0, 0, 1, 0, 7, 0, 0, 0, 16, 0, 0],
            detail: "BgAffineSet identity scale and translated origin"
        );
        using var objects = FirmwareSwiProbe.Run(
            number: 0x0F,
            thumb: thumb,
            r0: FirmwareSwiProbe.Source,
            r1: FirmwareSwiProbe.Destination,
            ...
            FirmwareSwiProbe.WriteBytes(
                bus: machine.Bus,
                address: FirmwareSwiProbe.Source,
                bytes: [0, 1, 0, 1, 0, 64, 0, 0]
            )
        );

        FirmwareSwiProbe.ExpectBytes(
            bus: objects.Instance.Machine.Bus,
            address: FirmwareSwiProbe.Destination,
            expected: [0, 0, 0, 0, 0, 255, 0, 0, 0, 1, 0, 0, 0, 0],
            detail: "ObjAffineSet quarter turn with stride four"
        );
    }
    private static void CheckCompression(bool thumb) {
        foreach (var (number, source, expected, vram) in new (byte, byte[], byte[], bool)[] {
            ...
            var destination = (vram
                ? 0x06000000u
                : FirmwareSwiProbe.Destination
            );
            using var core = FirmwareSwiProbe.Run(
                ...
                r0: FirmwareSwiProbe.Source,
                r1: destination,
                setup: machine => FirmwareSwiProbe.WriteBytes(
                    bus: machine.Bus,
                    address: FirmwareSwiProbe.Source,
                    bytes: source
                )
            ...
            FirmwareSwiProbe.ExpectBytes(
                bus: core.Instance.Machine.Bus,
                address: destination,
                expected: expected,
                detail: $"decompression {number:X2}"
            );
        }
    }
    private static void CheckIdentity() {
        ...
        FirmwareSwiProbe.Require(
            condition: (AgbFirmware.GetImage()[0] == original),
            detail: "bundled image aliases caller storage"
        );
        FirmwareSwiProbe.Require(
            condition: (AgbBiosProfile.Identify(image: image).Kind == AgbBiosKind.Unknown),
            detail: "modified BIOS identified as bundled"
        );
        var identified = AgbBiosProfile.Identify(image: AgbFirmware.GetImage());

        FirmwareSwiProbe.Require(
            condition: ((identified.Kind == AgbBiosKind.Puck) && !identified.IsCycleParityTrustworthy),
            detail: "Puck image missing identity or falsely claiming retail parity"
        );
        ...
        FirmwareSwiProbe.Require(
            ...
            detail: "startup mode discarded firmware identity"
        );
        FirmwareSwiProbe.Require(
            condition: (cold.Instance.Machine.Cpu.GetRegister(index: 15) < ReplacementBios.ImageSize),
            detail: "cold core skipped its reset vector"
        );
        FirmwareSwiProbe.Require(
            condition: (fast.Instance.Machine.Cpu.GetRegister(index: 15) >= AdvancedGamingBrickMachine.CartridgeEntryPoint),
            detail: "fast core did not seed cartridge handoff"
        );
        ...
        FirmwareSwiProbe.Require(
            condition: cold.Instance.Machine.Snapshot().Data.SequenceEqual(other: fast.Instance.Machine.Snapshot().Data),
            detail: "same-image cross-mode snapshot replay diverged"
        );
    }
    private static void CheckIrqWait(bool thumb) {
        ...
        FirmwareSwiProbe.Require(
            condition: (core.CycleCount >= (160 * 1232)),
            detail: "VBlankIntrWait returned before VBlank"
        );
        FirmwareSwiProbe.Require(
            condition: ((core.Instance.Machine.Cpu.Cpsr & 0x3F) == (thumb
            ? 0x3Fu
            : 0x1Fu)),
            detail: "IRQ/SWI return lost caller execution mode"
        );
        FirmwareSwiProbe.Require(
            condition: (core.Instance.Machine.Bus.Read16(
                access: BusAccessType.NonSequential,
                address: 0x03007FF8
            ) == 0),
    ...
    private static void CheckMath(bool thumb) {
        foreach (var vector in new (byte Number, uint R0, uint R1, uint Quotient, uint Remainder)[] {
            ...
            FirmwareSwiProbe.Require(
                condition: ((cpu.GetRegister(index: 0) == vector.Quotient) && (cpu.GetRegister(index: 1) == vector.Remainder) && (cpu.GetRegister(index: 3) == 14)),
                detail: $"division output mismatch, SWI {vector.Number}, Thumb={thumb}"
            );
        }
        foreach (var (input, expected) in new (uint, uint)[] { (0, 0), (1, 1), (2, 1), (9, 3), (uint.MaxValue, 65535) }) {
            ...
            FirmwareSwiProbe.Require(
                condition: (core.Instance.Machine.Cpu.GetRegister(index: 0) == expected),
                detail: $"sqrt({input}) mismatch"
            );
        }
        foreach (var (x, y, expected) in new (uint, uint, uint)[] { (1, 0, 0), (0, 1, 0x4000), (uint.MaxValue, 0, 0x8000), (0, uint.MaxValue, 0xC000), (1, 1, 0x2000) }) {
            ...
            FirmwareSwiProbe.Require(
                condition: (core.Instance.Machine.Cpu.GetRegister(index: 0) == expected),
                detail: $"ArcTan2({x},{y}) mismatch"
            );
        }
        using var checksum = FirmwareSwiProbe.Run(
            number: 13,
            thumb: thumb
        );

        FirmwareSwiProbe.Require(
            condition: (checksum.Instance.Machine.Cpu.GetRegister(index: 0) == 0xBAAE187F),
            detail: "BIOS compatibility checksum mismatch"
        );
    }
    private static void CheckSound(bool thumb) {
        using var initialized = FirmwareSwiProbe.Run(
            number: 0x1A,
            thumb: thumb,
            r0: FirmwareSwiProbe.Source
        );
        ...
        FirmwareSwiProbe.Require(
            condition: (bus.Read32(
                access: BusAccessType.NonSequential,
                address: 0x03007FF0
            ) == FirmwareSwiProbe.Source),
        ...
        foreach (var initial in new ushort[] { 0, 0x100, 0x200, 0x3FE, 0xC000, 0xC100, 0xC200, 0xC3FE }) {
            foreach (var enabled in new[] { false, true }) {
                using var bias = FirmwareSwiProbe.Run(
                    number: 0x19,
                    thumb: thumb,
                    r0: (enabled
                    ? 1u
                ...
                var actual = bias.Instance.Machine.Bus.Read16(
                    access: BusAccessType.NonSequential,
                    address: 0x04000088
                );
                ...
                var expected = (initial & ~0x3FF) | (enabled
                    ? Math.Max(
                        val1: initial & 0x3FF,
                        val2: 0x200
                    )
        ...
        foreach (var (key, expected) in new (uint, uint)[] { (60, 16384), (72, 32768) }) {
            using var midi = FirmwareSwiProbe.Run(
                number: 0x1F,
                thumb: thumb,
                r0: FirmwareSwiProbe.Source,
                r1: key,
                ...
                machine.Bus.Write32(
                    access: BusAccessType.NonSequential,
                    address: (FirmwareSwiProbe.Source + 4),
                    value: 0x01000000
                )
            );

            FirmwareSwiProbe.Require(
                condition: (midi.Instance.Machine.Cpu.GetRegister(index: 0) == expected),
                detail: "MidiKey2Freq octave vector mismatch"
            );
        }
    }
    private static void CheckTransfers(bool thumb) {
        foreach (var (number, control, length) in new (byte, uint, int)[] { (11, 4, 8), (11, 0x05000002, 8), (12, 1, 32) }) {
            ...
            for (var index = 0; (index < length); ++index) {
                expected[index] = ((byte)(((control == 0x05000002)
                    ? (index % 4)
                    : index) + 1));
            }
            expected[^1] = 0xCC;
            FirmwareSwiProbe.ExpectBytes(
                ...
                address: FirmwareSwiProbe.Destination,
                expected: expected,
                detail: $"transfer {number:X2}/{control:X8}"
            );
        }
    }
    private static void CheckUnpack(bool thumb) {
        ...
        FirmwareSwiProbe.ExpectBytes(
            ...
            address: FirmwareSwiProbe.Destination,
            expected: [0, 17, 18, 19],
            detail: "BitUnpack 2-to-8 with nonzero-only offset"
        );
    }
    private static void CheckWait(bool thumb) {
        ...
        FirmwareSwiProbe.Require(
            ...
            detail: "IntrWait did not consume only the requested pending bit"
        );
    }
