...
internal sealed class SerialClockStage : IPostStage<PostContext> {
    ...
    private static void CheckClock(bool word, bool fast) {
        ...
        var master = new AgbSerialController(
            interrupts: masterIrq,
            scheduler: masterClock
        );
        var slave = new AgbSerialController(
            interrupts: slaveIrq,
            scheduler: slaveClock
        );
        var cable = new AgbLinkCable();

        master.Connect(link: cable);
        slave.Connect(link: cable);
        var mode = ((ushort)(word
            ? 0x1000
            : 0));
        var rate = ((ushort)(fast
            ? 2
            : 0));

        master.WriteRegister(
            offset: 0x128,
            value: ((ushort)(mode | rate | 1))
        );
        slave.WriteRegister(
            offset: 0x128,
            value: mode
        ...
        slave.WriteRegister(
            offset: 0x128,
            value: ((ushort)(mode | rate | 0x4080))
        );
        slaveClock.Advance(cycles: 8192);
        Require(
            condition: (slave.IsTransferActive && (slaveClock.NextWhen == long.MaxValue) && (Flags(irq: slaveIrq) == 0)),
            detail: $"external Normal{(word
            ? 32
            : 8)} completed without master clocks"
        );
        masterClock.Advance(cycles: 8192);
        master.WriteRegister(
            offset: 0x128,
            value: ((ushort)(mode | rate | 0x4081))
        );
        var duration = (((fast
            ? 8
            : 64) * (word
        ...
        Require(
            condition: (master.IsTransferActive && slave.IsTransferActive),
            detail: "normal transfer completed before its last clock"
        );
        masterClock.Advance(cycles: 1);
        Require(
            condition: (!master.IsTransferActive && !slave.IsTransferActive && (Flags(irq: masterIrq) == 128) && (Flags(irq: slaveIrq) == 128)),
            detail: "master completion did not deliver one serial IRQ and release both busy flags"
        );
        if (word) {
            Require(
                condition: ((master.ReadRegister(offset: 0x120) == 0xBEEF) && (master.ReadRegister(offset: 0x122) == 0xABCD)
                && (slave.ReadRegister(offset: 0x120) == 0x2468) && (slave.ReadRegister(offset: 0x122) == 0x1357)),
                detail: "Normal32 exchanged payload mismatch"
            );
        }
        masterIrq.WriteRegister(
            offset: 0x202,
            value: 128
        );
        slaveIrq.WriteRegister(
            offset: 0x202,
            value: 128
        );
        ...
        Require(
            condition: ((Flags(irq: masterIrq) == 0) && (Flags(irq: slaveIrq) == 0)),
            detail: "normal transfer delivered a late duplicate IRQ"
        );
    }
    private static void CheckClockSelection(bool word, bool fast) {
        var clock = new AgbScheduler();
        var irq = new AgbInterruptController();
        var serial = new AgbSerialController(
            interrupts: irq,
            scheduler: clock
        );
        var control = ((ushort)((word
            ? 0x1000
            : 0) | (fast
            ? 2
            : 0) | 0x4080));

        serial.WriteRegister(
            offset: 0x128,
            value: control
        );
        clock.Advance(cycles: 8192);
        Require(
            condition: (serial.IsTransferActive && (Flags(irq: irq) == 0)),
            detail: "lone external transfer fabricated a clock"
        );
        serial.WriteRegister(
            offset: 0x128,
            value: ((ushort)(control | 1))
        );
        clock.Advance(cycles: 4096);
        Require(
            condition: (!serial.IsTransferActive && (Flags(irq: irq) == 128)),
            detail: "selecting the internal clock with start held did not complete"
        );
    }
    private static void CheckSupersededEvent() {
        ...
        Require(
            condition: ((IAgbLinkClient)serial).TryCompleteNormalSlave(
                incoming: 0x12345678,
                outgoing: out _,
                word: true
            ),
        ...
        Require(
            condition: ((Flags(irq: irq) == 128) && (clock.NextWhen == long.MaxValue)),
            detail: "peer completion retained an abandoned local event"
        );
        irq.WriteRegister(
            offset: 0x202,
            value: 128
        ...
        clock.Advance(cycles: 8192);
        Require(
            condition: ((Flags(irq: irq) == 0) && (serial.ReadRegister(offset: 0x120) == 0x5678)
            && (serial.ReadRegister(offset: 0x122) == 0x1234)),
            detail: "abandoned local event rewrote data or raised another IRQ"
        );
    }
