...
internal static class FirmwareMultiBootCartridge {
    ...
    internal static byte[] Download() {
        ...
        Write(
            ...
            value: 0xEA00000E,
            word: (0xC0 / 4)
        ); // Download entry branches over the extended header to 0x100.
        bytes.AsSpan(
            length: 0x3C,
            start: 0xC4
        ).Clear();
    ...
    internal static ushort[] Negotiation(byte[] download, int clients = 1) {
        ...
        for (var offset = 0; (offset < 0xC0); offset += 2) {
            words.Add(item: BinaryPrimitives.ReadUInt16LittleEndian(source: download.AsSpan(
                length: 2,
                start: offset
            )));
        }
    ...
    internal static byte[] Sender(byte[] download, ushort[] negotiation, int mode = 1, int clients = 1) {
        ...
        for (var index = 0; (index < negotiation.Length); ++index) {
            BinaryPrimitives.WriteUInt16LittleEndian(
                ...
                value: negotiation[index]
            );
        }
        var parameters = rom.AsSpan(
            length: 0x4C,
        ...
        parameters[0x1A] = ((clients >= 2)
            ? (byte)0x12
            : (byte)0xFF
        );
        parameters[0x1B] = ((clients >= 3)
            ? (byte)0x13
            : (byte)0xFF
        );
        ...
        BinaryPrimitives.WriteUInt32LittleEndian(
            destination: parameters[0x20..],
            value: ((RomBase + DownloadOffset) + 0xC0)
        );
        BinaryPrimitives.WriteUInt32LittleEndian(
            destination: parameters[0x24..],
            value: ((RomBase + DownloadOffset) + ((uint)download.Length))
        );
        download.CopyTo(
            array: rom,
            index: DownloadOffset
        );
        return rom;
    }
    ...
    private static void Write(byte[] image, int word, uint value) =>
        BinaryPrimitives.WriteUInt32LittleEndian(
            destination: image.AsSpan(start: (word * 4)),
            value: value
        );
}
