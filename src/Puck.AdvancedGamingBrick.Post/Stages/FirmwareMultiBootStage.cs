...
internal sealed class FirmwareMultiBootStage : IPostStage<PostContext> {
    ...
    internal static void CheckDownloadedProgram(AdvancedGamingBrickCore receiver, byte[] download, int mode = 1, int client = 1) {
        Require(
            ...
            detail: "native receiver did not execute the downloaded EWRAM program"
        );
        Require(
            condition: (Read(
                core: receiver,
            ...
            detail: "downloaded program observed an incorrect ARM/System CPSR at handoff"
        );
        Require(
            condition: (Read(
                core: receiver,
            ...
            detail: "downloaded program observed an incorrect system stack at handoff"
        );
        for (var index = 0; (index < download.Length); ++index) {
            var expected = ((index == 0xC4)
                ? (byte)((mode == 1)
                    ? 3
                    : 2)
                : ((index == 0xC5)
            ...
            var actual = receiver.Instance.Machine.Bus.Read8(
                access: BusAccessType.NonSequential,
                address: (0x02000000 + ((uint)index))
            );

            Require(
                condition: (actual == expected),
                detail: $"downloaded EWRAM/header byte {index:X3} differs: {actual:X2} != {expected:X2}"
            );
        }
    }
    internal static void CheckSender(AdvancedGamingBrickCore sender) {
        Require(
            ...
            detail: "native sender SWI25 did not return to its cartridge"
        );
        Require(
            condition: (Read(
                core: sender,
            ...
            detail: "native sender SWI25 reported failure after a valid transfer"
        );
    }
    internal static AdvancedGamingBrickCore PrepareReceiver(bool emptyCartridge, bool erasedCartridge = false, int transferMode = 1) {
        ...
        try {
            ...
            var expectedMode = ((transferMode == 1)
                ? 0x6000
                : 0x5000
            );
            ...
            while (core.CycleCount < (300L * AdvancedGamingBrickMachine.CyclesPerFrame)) {
                ...
                if ((serial.ReadRegister(offset: 0x128) & 0x7000) == expectedMode) {
                    Require(
                        condition: (core.Instance.Machine.Cpu.GetRegister(index: 15) < ReplacementBios.ImageSize),
                        detail: "download receiver readiness came from cartridge code instead of the native BIOS"
                    );
                    return core;
                }
            }
    ...
    private static void Finish(ref AgbLinkSession session, AdvancedGamingBrickCore sender, AdvancedGamingBrickCore receiver,
        ...
        Pump(
            ...
            trace: trace,
            expected: expected,
            untilTransfers: expected.Length,
            churn: churn,
    ...
    private static void Pump(ref AgbLinkSession session, AdvancedGamingBrickCore sender, AdvancedGamingBrickCore receiver,
        ...
        for (var chunk = 0; ((chunk < TransferBudget) && (trace.Count < untilTransfers)); ++chunk) {
            ...
            if (
                ...
                parentSio.WriteRegister(
                    offset: 0x12A,
                    value: ((ushort)(expected[^1].Sent ^ 1))
                );
            }
            if (
                wasActive &&
                ...
                var actual = new FirmwareMultiBootProtocol.Exchange(
                    Sent: parentSio.ReadRegister(offset: 0x120),
                    Reply: parentSio.ReadRegister(offset: 0x122)
                );
                ...
                if (
                    corruptChecksum &&
                    (trace.Count == (expected.Length - 1))
                ) { wanted = wanted with { Sent = ((ushort)(wanted.Sent ^ 1)) }; }
                Require(
                    condition: (actual == wanted),
                    detail: $"MultiBoot transfer {trace.Count}: sent/reply {actual.Sent:X4}/{actual.Reply:X4}, expected {wanted.Sent:X4}/{wanted.Reply:X4}"
                );
                Require(
                    condition: ((parentSio.ReadRegister(offset: 0x124) == 0xFFFF) && (parentSio.ReadRegister(offset: 0x126) == 0xFFFF)),
                    detail: "absent MultiBoot client slots did not stay idle-high"
                );
                trace.Add(item: actual);
                if (
                    churn &&
                    ((trace.Count % 17) == 0) &&
                    !childSio.IsTransferActive
                ) {
        ...
        Require(
            condition: (trace.Count == untilTransfers),
            detail: $"MultiBoot stalled after {trace.Count}/{untilTransfers} expected exchanges within a 60-frame wire budget"
        );
    }
    private static uint Read(AdvancedGamingBrickCore core, uint offset) => core.Instance.Machine.Bus.Read32(
        access: BusAccessType.NonSequential,
        address: (FirmwareMultiBootCartridge.Completion + offset)
    );
    ...
    public PostStageOutcome Run(PostContext context) {
        try {
            ...
            var expected = FirmwareMultiBootProtocol.Expected(
                download: download,
            ...
            try {
                ...
                Pump(
                    ...
                    session: ref session,
                    trace: trace,
                    untilTransfers: checkpointTransfers
                );
                var token = session.Suspend();
                ...
                var replay = trace.GetRange(
                    count: checkpointTransfers,
                    index: 0
                );
                ...
                _ = session.Suspend();
                Require(
                    condition: trace.SequenceEqual(second: replay),
                    detail: "MultiBoot wire transcript changed across snapshot/reconnect replay"
                );
                Require(
                    condition: senderFinal.ContentEquals(other: sender.Instance.Machine.Snapshot()),
                    detail: "MultiBoot sender snapshot diverged across credit-preserving reconnect churn"
                );
                Require(
                    condition: receiverFinal.ContentEquals(other: receiver.Instance.Machine.Snapshot()),
                    detail: "MultiBoot receiver snapshot diverged across credit-preserving reconnect churn"
                );
                ...
                Require(
                    ...
                    detail: "receiver executed a downloaded program after a bad wire checksum"
                );
                Require(
                    condition: (receiver.Instance.Machine.Cpu.GetRegister(index: 15) < ReplacementBios.ImageSize),
                    detail: "checksum refusal did not retain native BIOS control"
                );
            } finally {
                session.Dispose();
            }
            using var empty = PrepareReceiver(emptyCartridge: true);
            using var erased = PrepareReceiver(
                emptyCartridge: true,
                erasedCartridge: true
            );
