...
internal sealed class LifecycleStage : IPostStage<PostContext> {
    ...
    private static void CheckAudio() {
        ...
        for (var iteration = 0; (iteration < 3); ++iteration) {
            ...
            Require(
                condition: (apu.DrainSamples(destination: samples.AsSpan(
                    length: 2,
                    start: 0
                )) == 2),
                detail: "audio did not produce samples"
            );
            apu.ConfigureOutput(sampleRate: 0);
            Require(
                condition: (apu.DrainSamples(destination: samples) == 0),
                detail: "disabled audio retained queued samples"
            );
        }
        apu.ConfigureOutput(sampleRate: 48_000);
        Require(
            condition: (apu.DrainSamples(destination: samples) == 0),
            detail: "re-enabled audio retained old samples"
        );
        apu.Step(cycles: 4096);
        Require(
            condition: (apu.DrainSamples(destination: samples) > 0),
            detail: "re-enabled audio did not produce samples"
        );
    }
    private static void CheckDma(int channel, ushort count) {
        using var instance = PostMachine.BuildInstance(
        ...
        Require(
            condition: ((bus.Read16(
                ...
                address: ((uint)(0x02000000 + (2 * (expected - 1))))
            ) == 0xBEEF)
            && (bus.Read16(
                access: BusAccessType.NonSequential,
                address: ((uint)(0x02000000 + (2 * expected)))
            ) == 0)),
    ...
    private static void CheckHalt(bool stop) {
        ...
        scheduler.Schedule(
            cyclesFromNow: (2 * AdvancedGamingBrickMachine.CyclesPerFrame),
            e: cutoff
        );
        foreach (var budget in new[] { 1, 17, AdvancedGamingBrickMachine.CyclesPerFrame }) {
            ...
            Require(
                condition: ((machine.Cycles == (before + budget)) && bus.Halted && (machine.Cpu.GetRegister(index: 15) == pc)),
                detail: $"{(stop
                ? "STOP"
                : "HALT")} exceeded budget or executed an instruction while asleep"
            );
        }
        ...
        Require(
            ...
            detail: "keypad input did not wake sleeping CPU"
        );
        ...
        Require(
            ...
            detail: "sleep/wake snapshot replay differed"
        );
    }
    private static void CheckHaltedDma() {
        ...
        Require(
            condition: (bus.Halted && (bus.Read16(
                access: BusAccessType.NonSequential,
                address: 0x02000000
            ) == 0xBEEF)),
