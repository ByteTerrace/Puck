...
internal sealed class FirmwareSequenceStage : IPostStage<PostContext> {
    ...
    private static void Equal(uint actual, uint expected, string detail) => Require(
        ...
        detail: $"{detail}: {actual:X8}, expected{expected:X8}"
    );
    private static void Gate(byte command, byte extra, byte duration, byte[]? retail) => Pair(
        program: FirmwareSequenceVectors.Note(
            command: command,
            extra: extra
        ),
        retail: retail,
        ...
        assertion: (fixture, tick) => {
            Require(
                condition: (fixture.Byte(address: (FirmwareSequenceFixture.Channel + 0x10)) == Math.Max(
                    val1: 0,
                    val2: (duration - tick)
                )),
                detail: $"tick{tick} remaining gate duration"
            );
            Require(
                condition: (((fixture.Byte(address: FirmwareSequenceFixture.Channel) & 0x40) != 0) == (tick >= duration)),
                detail: $"tick{tick} gate release edge"
            );
        }
    );
    private static void Pair(byte[] program, byte[]? retail, int ticks, Action<FirmwareSequenceFixture, int> assertion, Action<FirmwareSequenceFixture>? setup = null) {
        using var candidate = new FirmwareSequenceFixture(
            bios: null,
            program: program
        );
        using var reference = ((retail is null)
            ? null
            : new FirmwareSequenceFixture(
                bios: retail,
                program: program
            )
        );
        ...
        for (var tick = 0; (tick < ticks); ++tick) {
            ...
            for (var field = 0; (field < expected.Length); ++field) {
                Require(
                    condition: (actual[field] == expected[field]),
                    detail: $"tick{tick} record{field}: Puck={actual[field]:X8}, retail={expected[field]:X8}"
                );
            }
            Require(
                condition: candidate.Pcm().AsSpan().SequenceEqual(other: samples[tick]),
                detail: $"tick{tick} stereo PCM differs from retail"
            );
        }
    }
    ...
    private static void TickAndAssert(FirmwareSequenceFixture fixture, int tick, Action<FirmwareSequenceFixture, int> assertion, string label) {
        try {
            ...
            throw new InvalidOperationException(
                message: $"{label}: {exception.Message}",
    ...
    private static void Tie(byte[]? retail, bool implicitEnd = false) => Pair(
        ...
        : 7),
        assertion: (fixture, tick) => {
            var releaseTick = (implicitEnd
                ? 6
                : 4
            );

            Require(
                condition: (fixture.Byte(address: (FirmwareSequenceFixture.Channel + 0x10)) == 0),
                detail: $"tick{tick} tied note acquired a finite gate"
            );
            Require(
                condition: (fixture.Byte(address: (FirmwareSequenceFixture.Channel + 0x11)) == 60),
                detail: $"tick{tick} original MIDI key changed"
            );
            Equal(
                actual: fixture.Byte(address: (FirmwareSequenceFixture.Track + 5)),
                expected: (((tick >= 2) && (tick < releaseTick))
                ? 61u
                : 60u),
                detail: $"tick{tick} cached key after explicit/implicit note end"
            );
            Require(
                condition: (((fixture.Byte(address: FirmwareSequenceFixture.Channel) & 0x40) != 0) == (tick >= releaseTick)),
                detail: $"tick{tick} selective end-tie release edge"
            );
            Require(
                condition: (fixture.Word(address: (FirmwareSequenceFixture.Channel + 0x2C)) == ((tick < (releaseTick + 2))
                ? FirmwareSequenceFixture.Track
                : 0)),
                detail: $"tick{tick} releasing-note ownership"
            );
        }
    );
    private static void WaveLoop(byte[]? retail) => Pair(
        program: FirmwareSequenceVectors.Note(command: 0xCF),
        retail: retail,
        ticks: 2,
        setup: fixture => {
            fixture.Word(
                address: (FirmwareSequenceFixture.Wave + 8),
                value: 3
            );
            fixture.Word(
                address: (FirmwareSequenceFixture.Wave + 12),
                value: 8
            );
            fixture.Put(
                address: (FirmwareSequenceFixture.Wave + 16),
                bytes: [0, 16, 32, 48, 64, 80, 96, 112, 0]
            );
        },
        assertion: (fixture, tick) => {
            Require(
                condition: ((fixture.Byte(address: FirmwareSequenceFixture.Channel) & 0x10) != 0),
                detail: "looping channel lost its loop flag"
            );
            var pcm = fixture.Pcm();
            ...
            for (var sample = 0; (sample < 96); ++sample) {
                ...
                Require(
                    condition: ((pcm[sample] == right) && (pcm[(sample + 96)] == left)),
                    detail: $"tick{tick} loop PCM sample{sample}: {pcm[sample]}/{pcm[(sample + 96)]}, expected{right}/{left}"
                );
            }
        }
    );
    ...
    public PostStageOutcome Run(PostContext context) {
        ...
        foreach (var (command, extra, duration) in new (byte, byte, byte)[] { (0xD0, 0, 1), (0xD1, 0, 2), (0xD3, 0, 4), (0xD0, 3, 4) }) {
            Check(
                name: $"gate-{duration}-extra-{extra}",
                action: () => Gate(
                    command: command,
                    duration: duration,
                    extra: extra,
        ...
        Check(
            name: "tie-selective-end",
            action: () => Tie(retail: retail),
            failures: failures
        );
        Check(
            name: "tie-cached-end",
        ...
        Check(
            name: "fixed-wave-loop",
            action: () => WaveLoop(retail: retail),
            failures: failures
        );
        cases += 3;
        return ((failures.Count == 0)
            ? PostStageOutcome.Pass(detail: $"{cases} original native song/sample fixtures: GOTO, pattern depth1/3 and return, repeat0/1/3, finite note gates and optional length, selective tied-note end, fixed-wave loop; caller-owned records and192 PCM bytes per tick; {((retail is null)
                ? "retail comparison unavailable"
                : "identical fixtures matched verified retail records/PCM")}; bounded functional evidence, not full music or timing parity")
