...
internal sealed class FirmwarePresentationStage : IPostStage<PostContext> {
    ...
    private static void CheckHandoff(AdvancedGamingBrickCore core) {
        Require(
            condition: HasEnteredCartridge(core: core),
            detail: "firmware did not hand control to the diagnostic cartridge within 120 frames"
        );
        Require(
            condition: (core.Instance.Machine.Bus.Read32(
                access: BusAccessType.NonSequential,
                address: 0x02000000
            ) == 0x5A),
            detail: "cartridge entry did not write its EWRAM marker"
        );
        ...
        foreach (var pixel in core.Framebuffer) {
            Require(
                condition: (pixel == expected),
                detail: "cartridge did not replace the firmware picture after handoff"
            );
        }
        ...
        foreach (var sample in Drain(apu: core.Instance.Machine.Apu)) {
            Require(
                condition: (sample == 0),
                detail: "firmware startup sound leaked beyond cartridge handoff"
            );
        }
    }
    private static void CheckHeader(bool alternateLogo, bool malformedHeader, uint[] peak) {
        using var core = new AdvancedGamingBrickCore(cartridgeRom: FirmwarePresentationCartridge.Create(
        ...
        while (
            (core.CycleCount < ColdBootBudget) &&
            !HasEnteredCartridge(core: core)
        ) {
            core.RunCycles(cycles: FrameCycles);
            sawWordmark |= core.Framebuffer.SequenceEqual(other: peak);
        }
        Require(
            condition: sawWordmark,
            detail: $"header fields changed native branding: alternate={alternateLogo}, malformed={malformedHeader}"
        );
    ...
    private static void CheckReplay(AdvancedGamingBrickCore core, List<short> startup) {
        ...
        Require(
            condition: (Drain(apu: core.Instance.Machine.Apu).Length == 0),
            detail: "typed mid-boot restore exposed stale queued PCM"
        );
        ...
        for (var frame = 0; (frame < 18); ++frame) {
            foreach (var cycles in slices) {
                ...
                Require(
                    condition: core.Instance.Machine.Snapshot().Data.SequenceEqual(other: fork.Machine.Snapshot().Data),
                    detail: $"mid-boot fork state diverged at frame {frame}, slice {cycles}"
                );
                Require(
                    condition: core.Framebuffer.SequenceEqual(other: fork.Machine.Framebuffer),
                    detail: "mid-boot fork pixels diverged"
                );
                var pcm = Drain(apu: core.Instance.Machine.Apu);

                Require(
                    condition: pcm.AsSpan().SequenceEqual(other: Drain(apu: fork.Machine.Apu)),
                    detail: "mid-boot fork PCM diverged"
                );
        ...
        Require(
            condition: (Drain(apu: core.Instance.Machine.Apu).Length == 0),
            detail: "restoring mid-boot state exposed stale queued PCM"
        );
        ...
        Require(
            condition: core.Instance.Machine.Snapshot().Data.SequenceEqual(other: expectedState.Data),
            detail: "raw mid-boot snapshot replay changed machine bytes"
        );
        Require(
            condition: core.Framebuffer.SequenceEqual(other: expectedPixels),
            detail: "raw mid-boot snapshot replay changed pixels"
        );
        Require(
            condition: CollectionsMarshal.AsSpan(list: replayPcm).SequenceEqual(other: CollectionsMarshal.AsSpan(list: futurePcm)),
            detail: "raw mid-boot snapshot replay changed fresh PCM"
        );
        startup.AddRange(collection: replayPcm);
    }
    private static (int FirstNote, int SecondNote) CheckSound(List<short> samples) {
        Require(
            condition: ((samples.Count > SampleRate) && ((samples.Count % 2) == 0)),
            detail: "startup did not produce complete stereo PCM"
        );
        var previousRise = -1;
        var firstNote = 0;
        var secondNote = 0;
        ...
        for (var index = 0; (index < samples.Count); index += 2) {
            Require(
                condition: (samples[index] == samples[(index + 1)]),
                detail: "startup chime is not centered in stereo"
            );
            Require(
                condition: (Math.Abs(value: ((int)samples[index])) < 4096),
                detail: "startup chime exceeded its quiet amplitude budget"
            );
            if (
                (index >= 2) &&
        ...
        Require(
            condition: ((firstNote >= 20) && (secondNote >= 20)),
            detail: $"startup PCM did not contain both ascending chime notes ({firstNote}/{secondNote} periods)"
        );
        Require(
            condition: (firstLower < firstUpper),
            detail: "startup chime did not play its lower note before its upper note"
        );
    ...
    private static void Save(AdvancedGamingBrickCore core, string directory, string name) =>
        PngEncoder.Write(
            ...
            rgba: MemoryMarshal.AsBytes(span: core.Framebuffer),
            width: 240
        );
    ...
    public PostStageOutcome Run(PostContext context) {
        try {
            ...
            Require(
                condition: (AgbBiosProfile.Identify(image: AgbFirmware.GetImage()).Kind == AgbBiosKind.Puck),
            ...
            for (var frame = 0; ((frame < 64) && !cold.Framebuffer.SequenceEqual(other: peak)); ++frame) {
                ...
                if (
                    !sawFadeIn &&
                    cold.Framebuffer.SequenceEqual(other: half)
                ) {
                    Save(
                        core: cold,
                        directory: artifacts,
            ...
            Require(
                condition: cold.Framebuffer.SequenceEqual(other: peak),
                detail: "native PUCK/BYTETERRACE geometry or palette mismatch; inspect wordmark.png"
            );
            Require(
                condition: sawFadeIn,
                detail: "startup did not render its half-bright fade-in frame"
            );
            Require(
                condition: !HasEnteredCartridge(core: cold),
                detail: "branding was not rendered while the CPU was executing firmware"
            );
            CheckReplay(
                core: cold,
                startup: startup
            ...
            while (
                (cold.CycleCount < ColdBootBudget) &&
                !HasEnteredCartridge(core: cold)
            ) {
                cold.RunCycles(cycles: FrameCycles);
                startup.AddRange(collection: Drain(apu: cold.Instance.Machine.Apu));
                if (
            ...
            Require(
                condition: sawFadeOut,
                detail: "startup did not render its half-bright fade-out frame"
            );
            Require(
                condition: (cold.CycleCount >= (72L * FrameCycles)),
                detail: "cold startup skipped its native presentation interval"
            );
            ...
            using var fast = new AdvancedGamingBrickCore(
                cartridgeRom: rom,
            ...
            Require(
                condition: (cold.Instance.Machine.Identity == fast.Instance.Machine.Identity),
                detail: "fast startup discarded the selected firmware"
            );
