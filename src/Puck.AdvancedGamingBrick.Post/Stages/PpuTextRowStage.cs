...
internal sealed class PpuTextRowStage : IPostStage<PostContext> {
    ...
    private static uint Pixel(byte[] image, ushort[] palette, int depth, int size, int characterBlock,
        ...
        var tile = (entry % 1024);
        var address = ((((characterBlock * 16384) + (tile * ((depth == 1)
            ? 64
            : 32)))
            + (ty * ((depth == 1)
            ? 8
    ...
    public PostStageOutcome Run(PostContext context) {
                ...
                for (var characterBlock = 0; (characterBlock < 4); ++characterBlock) {
                    foreach (var screenBlock in ((ReadOnlySpan<int>)[0, 31])) {
                        ...
                        for (var block = 0; (block < 4); ++block) {
                            for (var entry = 0; (entry < 1024); ++entry) {
                                ...
                                BinaryPrimitives.WriteUInt16LittleEndian(
                                    destination: image.AsSpan(start: (((screenBlock * 2048) + (block * 2048)) + (entry * 2))),
                                    value: value
                                );
                            }
                        }
                        for (var offset = 0; (offset < image.Length); offset += 4) {
                            ppu.WriteVideo(
                                address: (0x06000000u + ((uint)offset)),
                                width: 4,
                        ...
                        for (var index = 0; (index < palette.Length); ++index) {
                            ...
                            ppu.WriteVideo(
                                address: (0x05000000u + (((uint)index) * 2u)),
                                width: 2,
                                value: palette[index]
                            );
                        }
                        ...
                        ppu.WriteRegister(
                            offset: 8,
                            value: ((ushort)((size << 14) | (screenBlock << 8)
                        ...
                        for (var line = 0; (line < 16); ++line) {
                            ...
                            ppu.WriteRegister(
                                offset: 0x10,
                                value: ((ushort)horizontal)
                            );
                            ppu.WriteRegister(
                                offset: 0x12,
                                value: ((ushort)vertical)
                            );
                            ppu.WriteRegister(
                                offset: 0x4C,
                                value: ((ushort)(((mosaicY - 1) << 4) | (mosaicX - 1)))
                            );
                            ...
                            for (var x = 0; (x < 240); ++x) {
                                var expected = Pixel(
                                    ...
                                    size: size,
                                    x: ((x - (x % mosaicX)) + horizontal),
                                    y: ((line - (line % mosaicY)) + vertical)
                                );
                                var actual = ppu.Framebuffer[((line * 240) + x)];

                                ++checkedPixels;
                                if (actual != expected) {
                                    return PostStageOutcome.Fail(detail: ((string)$"depth={depth}, size={size}, char={characterBlock}, screen={screenBlock}, line={line}, x={x}: {actual:X8} != {expected:X8}"));
                                }
                            }
                        }
                    }
        ...
        return PostStageOutcome.Pass(detail: ((string)$"{checkedPixels} pixels matched scalar tile sampling: 4/8 bpp, four map sizes, all character bases, screen-block boundaries, flips, scrolling, palette writes, transparency and mosaic"));
    }
}
