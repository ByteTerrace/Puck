...
internal sealed class FirmwareSequenceFixture : IDisposable {
    ...
    internal FirmwareSequenceFixture(byte[]? bios, byte[] program) {
        ...
        try {
            ...
            Put(
                address: Voice,
                bytes: [8, 60, 0, 0, 0, 0xA0, 0, 2, 255, 255, 128, 128]
            );
            var wave = new byte[(4096 + 16)];

            BinaryPrimitives.WriteUInt16LittleEndian(
                destination: wave.AsSpan(start: 2),
                value: 0x4000
            );
            BinaryPrimitives.WriteUInt32LittleEndian(
                destination: wave.AsSpan(start: 4),
                value: (5734 * 1024)
            );
            BinaryPrimitives.WriteUInt32LittleEndian(
                destination: wave.AsSpan(start: 12),
                value: 4096
            );
    ...
    internal byte Byte(uint address) => Core.Instance.Machine.Bus.Read8(
    ...
    internal void Put(uint address, ReadOnlySpan<byte> bytes) => FirmwareSwiProbe.WriteBytes(
    ...
    internal void Tick() => FirmwareSwiProbe.Call(
        core: Core,
        number: 0x1C,
        thumb: true
    );
    internal uint Word(uint address) => Core.Instance.Machine.Bus.Read32(
        access: BusAccessType.NonSequential,
        address: address
    );
    internal void Word(uint address, uint value) => Core.Instance.Machine.Bus.Write32(
        access: BusAccessType.NonSequential,
        address: address,
        value: value
    );
