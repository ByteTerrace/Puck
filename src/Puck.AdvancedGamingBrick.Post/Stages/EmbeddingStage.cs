...
internal sealed class EmbeddingStage : IPostStage<PostContext> {
    ...
    public PostStageOutcome Run(PostContext context) {
        ...
        Encoding.ASCII.GetBytes(s: "SIIRTC_V").CopyTo(
            array: rom,
            index: (rom.Length - 16)
        );
        ...
        using var normal = new AdvancedGamingBrickCore(
            bios: bios,
            cartridgeRom: rom
        );
        using var overridden = new AdvancedGamingBrickCore(configuration: new AgbMachineConfiguration(
            bios: bios,
            rom: rom,
            options: options
        ));
        using var traced = new AdvancedGamingBrickCore(configuration: new AgbMachineConfiguration(
            bios: bios,
            rom: rom,
            options: new AgbMachineOptions { BusTrace = _ => { } }
        ));

        if (
            !normal.Instance.Machine.Identity.HasRtc ||
            normal.Instance.Machine.Identity.PrefetchDisabled ||
            overridden.Instance.Machine.Identity.HasRtc ||
            !overridden.Instance.Machine.Identity.PrefetchDisabled ||
        ...
        if (
            (traceCount != countBefore) ||
            !normal.Instance.Machine.Snapshot().Data.SequenceEqual(other: traced.Instance.Machine.Snapshot().Data)
        ) {
        ...
        return CoreEmbeddingProbe.Verify(
            core: normal,
            cycleBudget: AdvancedGamingBrickMachine.CyclesPerFrame,
            framebufferLength: (240 * 160)
        );
    }
}
