...
internal sealed class FirmwareResetStage : IPostStage<PostContext> {
    ...
    private static void CheckFifos(bool thumb, uint flags, byte[]? bios) {
        ...
        for (var fifo = 0; (fifo < 2); ++fifo) {
            ...
            Require(
                condition: ((words == 2) && (playing == (reset
                ? 0
                : 3))),
                detail: $"FIFO {fifo}: ring={words}, playing={playing}; expected {(reset
                ? "2/0 after sound reset"
                : "2/3 preserved")}"
            );
            ...
            Require(
                condition: (apu.DebugFifoWordCount(fifo: fifo) == (reset
                ? 2
                : 3)),
                detail: $"FIFO {fifo} partial-word reset/preservation mismatch"
            );
        }
        if (reset) {
            for (var sample = 0; (sample < 8); ++sample) {
                ...
                for (var fifo = 0; (fifo < 2); ++fifo) {
                    Require(
                        condition: (apu.DebugDirectSound(fifo: fifo) == 0),
                        detail: $"FIFO {fifo} reset left stale queued sample {sample}"
                    );
                }
            }
            for (var fifo = 0; (fifo < 2); ++fifo) {
                Require(
                    condition: ((apu.DebugFifoWordCount(fifo: fifo) == 0) && (apu.DebugFifoPlayingBytes(fifo: fifo) == 0)),
        ...
        foreach (var (address, size, flag) in Regions) {
            foreach (var offset in new uint[] { 0, (size / 2), (size - 4) }) {
                ...
                Expect(
                    ...
                    bus: bus,
                    detail: "selective RAM region sentinel",
    ...
    private static void CheckRegisters(bool thumb, uint flags, byte[]? bios) {
        ...
        CheckRegionSentinels(
            bus: bus,
            flags: flags,
            skipFirstEwram: false
        );
        Expect(
            address: 0x03007E00,
            bus: bus,
            detail: "RegisterRamReset reserved IWRAM start preservation",
            expected: 0xC35A96F0
        );
        Expect(
            address: 0x03007FF8,
            bus: bus,
            detail: "RegisterRamReset reserved IWRAM end preservation",
            expected: 0xC35A96F0
        );
        ExpectHalf(
            address: 0x04000000,
            bus: bus,
            detail: "unconditional forced blank",
            expected: 0x80
        );
        ...
        ExpectHalf(
            address: 0x04000204,
            bus: bus,
            detail: "WAITCNT selective reset",
            expected: (other
            ? (ushort)0
            : (ushort)0x4000)
        );
        ...
        ExpectHalf(
            address: 0x04000134,
            bus: bus,
            detail: "RCNT selective serial reset",
            expected: (((flags & 32) != 0)
            ? (ushort)0x8000
            : (ushort)0x8001)
        );
        ExpectHalf(
            address: 0x04000080,
            bus: bus,
            detail: "SOUNDCNT_L selective reset",
            expected: (((flags & 64) != 0)
            ? (ushort)0
            : (ushort)0x1177)
        );
        ExpectHalf(
            address: 0x04000082,
            bus: bus,
            detail: "SOUNDCNT_H selective reset",
            expected: (((flags & 64) != 0)
            ? (ushort)0x000E
            : (ushort)0x030E)
        );
    ...
    private static void CheckReset(bool thumb, byte flag, bool hard, byte[]? bios) {
        using var core = new AdvancedGamingBrickCore(configuration: AgbFirmware.CreateConfiguration(
            cartridgeRom: FirmwareResetCartridge.Create(
                number: (hard
            ? (byte)0x26
            : (byte)0),
                thumb: thumb
        ...
        for (var index = 0; (index <= 12); ++index) {
            machine.Cpu.SetRegister(
                index: index,
                value: (0xABCD1000u + ((uint)index))
            );
        }
        SeedRegions(bus: bus);
        FirmwareSwiProbe.WriteBytes(
            bus: bus,
            address: 0x02000000,
            bytes: FirmwareResetCartridge.Entry(marker: 0xA5)
        );
        ...
        var target = ((hard || (flag == 0))
            ? 0x08000000u
            : 0x02000000u
        );
        var before = machine.Cycles;
        var limit = (hard
            ? (120L * AdvancedGamingBrickMachine.CyclesPerFrame)
            : 2_000_000L
        );
        var artwork = (hard
            ? FirmwarePresentationArtwork.Create()
            : []
        );
        ...
        while (
            ((machine.Cycles - before) < limit) &&
            (machine.Cpu.GetRegister(index: 15) != target)
        ) {
            machine.Step();
            if (
                hard &&
                (machine.Cycles >= nextFrame)
            ) {
        ...
        Require(
            condition: (machine.Cpu.GetRegister(index: 15) == target),
            detail: $"never branched to entry {target:X8}; PC={machine.Cpu.GetRegister(index: 15):X8}, CPSR={machine.Cpu.Cpsr:X8}"
        );
        Require(
            condition: (machine.Cpu.Cpsr == 0x1F),
            detail: $"entry CPSR={machine.Cpu.Cpsr:X8}, expected ARM System with clean flags and IRQ/FIQ enabled"
        );
        for (var index = 0; (index <= 12); ++index) {
            Require(
                condition: (machine.Cpu.GetRegister(index: index) == 0),
                detail: $"entry r{index} was not cleared"
            );
        }
        Require(
            condition: (machine.Cpu.GetRegister(index: 14) == target),
            detail: "entry LR did not identify reset target"
        );
        for (uint address = 0x03007E00; (address < 0x03008000); address += 4) {
            Expect(
                address: address,
                bus: bus,
                detail: "reserved IWRAM clear",
                expected: 0
            );
        }
        if (hard) {
            Require(
                condition: sawArtwork,
                detail: "HardReset never replayed the native PUCK/BYTETERRACE wordmark"
            );
            Require(
                condition: ((machine.Cycles - before) >= (72L * AdvancedGamingBrickMachine.CyclesPerFrame)),
                detail: "HardReset skipped the native cold presentation interval"
            );
            CheckRegionSentinels(
                bus: bus,
                flags: 255,
            ...
            Expect(
                address: 0x03007DFC,
                bus: bus,
                detail: "SoftReset lower boundary preservation",
                expected: 0xC35A96F0
            );
        }
        var marker = ((hard || (flag == 0))
            ? 0x5Au
            : 0xA5u
        );

        for (var instruction = 0; ((instruction < 64) && (Read(
            address: FirmwareResetCartridge.Marker,
            bus: bus
        ) != marker)); ++instruction) {
            machine.Step();
        }
        Expect(
            address: FirmwareResetCartridge.Marker,
            bus: bus,
            detail: "native reset destination marker",
            expected: marker
        );
        foreach (var (offset, expected) in new (uint, uint)[] { (0, 0x03007F00), (4, 0x03007FA0), (8, 0), (12, 0x03007FE0), (16, 0), (24, 0), (28, 0) }) {
            Expect(
                address: (FirmwareResetCartridge.Result + offset),
                bus: bus,
                detail: "native banked stack/SPSR capture",
                expected: expected
            );
        }
    }
    private static void CheckSleep(bool thumb, byte number, uint value, byte[]? bios) {
        ...
        Require(
            condition: bus.Halted,
            detail: "SWI never entered low-power state"
        );
        var pc = machine.Cpu.GetRegister(index: 15);

        _ = machine.RunCycles(cycles: 1234);
        Require(
            condition: (bus.Halted && (machine.Cpu.GetRegister(index: 15) == pc)),
            detail: "sleep executed instructions without a wake source"
        );
        ...
        Require(
            condition: (bus.Halted == stop),
            detail: "timer IRQ wake selected the wrong HALT/STOP mode"
        );
        ...
        Wake();
        Require(
            condition: future.Data.SequenceEqual(other: machine.Snapshot().Data),
            detail: "native sleep/wake snapshot replay diverged"
        );
        ...
        void Wake() {
            ...
            Require(
                condition: (!bus.Halted && (machine.Cpu.GetRegister(index: 7) == 0xEE)),
                detail: "keypad IRQ did not resume the native SWI caller with IME=0"
            );
            Require(
                condition: ((machine.Cpu.Cpsr & 0x3F) == (thumb
                ? 0x3Fu
                : 0x1Fu)),
                detail: "sleep return lost caller ARM/Thumb state"
            );
            ExpectHalf(
                address: 0x04000208,
                bus: bus,
                detail: "sleep changed IME",
                expected: 0
            );
            ExpectHalf(
                address: 0x04000202,
                bus: bus,
                detail: "sleep unexpectedly acknowledged keypad IF",
                expected: 0x1000
            );
        }
    }
    ...
    private static void ExpectHalf(IAgbBus bus, uint address, ushort expected, string detail) {
        ...
        Require(
            condition: (actual == expected),
            detail: $"{detail} at {address:X8}: {actual:X4}, expected {expected:X4}"
        );
    }
    ...
    public PostStageOutcome Run(PostContext context) {
        ...
        return ((failures.Count == 0)
            ? PostStageOutcome.Pass(detail: $"44 asset-free native ARM/Thumb cases: ROM/EWRAM SoftReset entry markers, register/stack hygiene and RAM boundaries; cold HardReset relaunch; selective RAM/I/O groups, forced blank and Direct Sound FIFO queued-silence/playing/partial-word reset; Halt/Stop/CustomHalt timer/keypad wake and snapshot replay. {(hasRetail
                ? "42 verified-retail black-box controls also passed; HardReset is Puck-only (original header)."
                : "Verified-retail controls not run: no verified external BIOS.")} Functional contracts, not retail timing or STOP clock-freeze parity.")
        ...
        void RunImage(string name, byte[]? bios) {
            foreach (var thumb in new[] { false, true }) {
                foreach (var flag in new byte[] { 0, 1, 0xFF }) {
                    RunCase(
                        name: $"{name} SoftReset Thumb={thumb} flag={flag:X2}",
                        test: () => CheckReset(
                            bios: bios,
                            flag: flag,
                            hard: false,
                ...
                if (bios is null) {
                    RunCase(
                        name: $"{name} HardReset Thumb={thumb}",
                        test: () => CheckReset(
                            bios: bios,
                            flag: 1,
                            hard: true,
                ...
                foreach (var flags in new uint[] { 0, 1, 2, 4, 8, 16, 32, 64, 128, 255 }) {
                    RunCase(
                        name: $"{name} RegisterRamReset Thumb={thumb} flags={flags:X2}",
                        test: () => CheckRegisters(
                            bios: bios,
                            flags: flags,
                            thumb: thumb
                ...
                foreach (var flags in new uint[] { 0, 0x40, 0x80, 0xFF }) {
                    RunCase(
                        name: $"{name} FIFO reset Thumb={thumb} flags={flags:X2}",
                        test: () => CheckFifos(
                            bios: bios,
                            flags: flags,
                            thumb: thumb
                ...
                foreach (var (number, value) in new (byte, uint)[] { (2, 0x80), (3, 0), (0x27, 0), (0x27, 0x80) }) {
                    RunCase(
                        name: $"{name} sleep SWI={number:X2} Thumb={thumb} r2={value:X2}",
                        test: () => CheckSleep(
                            bios: bios,
                            number: number,
                            thumb: thumb,
