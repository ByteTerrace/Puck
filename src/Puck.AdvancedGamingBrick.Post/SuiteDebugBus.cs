...
// An IAgbBus decorator that emulates the debug-log register block the menu-driven accuracy-suite ROMs write to, and
// injects controller input, so the suite can be run head-lessly. The suite probes for the debug interface by writing
// 0xC0DE to 0x04FFF780 and expecting 0x1DEA back; once "open" it prints each suite's "BEGIN:"/"END: p/t" line by
// formatting into the 0x04FFF600 buffer and writing the level (| 0x100) to 0x04FFF700. KEYINPUT (0x04000130) is
// overridden so the harness can drive the menu.
internal sealed class SuiteDebugBus : IAgbBus {
