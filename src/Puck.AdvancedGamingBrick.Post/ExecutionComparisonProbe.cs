...
internal static class ExecutionComparisonProbe {
    internal static (bool Pass, string Detail) Run(byte[] rom, ReadOnlyMemory<byte> bios, string label,
        ...
        using var reference = AgbMachineFactory.Create(
        ...
        using var candidate = AgbMachineFactory.Create(configuration: new AgbMachineConfiguration(
            bios: bios,
            rom: rom
        ));
        ...
        if (mutableRam) {
            foreach (var machine in new[] { expected, actual }) {
                ...
                for (var offset = 0; (offset < 32); ++offset) {
                    bus.DebugWrite8(
                        address: (0x03000000u + ((uint)offset)),
                        value: rom[offset]
                    );
                }
        ...
        for (var packet = 0; (packet < packets); ++packet) {
            ...
            if (
                ...
                ((AgbBus)expected.Bus).DebugWrite8(
                    address: 0x03000018u,
                    value: 7
                );
                ((AgbBus)actual.Bus).DebugWrite8(
                    address: 0x03000018u,
                    value: 7
                );
            }
            ...
            var keys = ((ushort)(0x3FF ^ (((packet % 31) == 0)
                ? 9
                : 0)));
            ...
            var budget = (frames
                ? AdvancedGamingBrickMachine.CyclesPerFrame
                : budgets[(packet % budgets.Length)]
            );
            ...
            if (
                ...
                !expected.Snapshot().ContentEquals(other: actual.Snapshot())
            ) {
                return (false, ((string)$"{label}: packet {packet}, budget {budget} diverged (instruction boundaries {expectedSteps}/{actualSteps}; master clocks {expected.Cycles}/{actual.Cycles})"));
            }
            while (true) {
                var samples = expected.Apu.DrainSamples(destination: expectedAudio);

                if (
                    (samples != actual.Apu.DrainSamples(destination: actualAudio)) ||
                    !expectedAudio.AsSpan(
                    length: samples,
                    start: 0
                ).SequenceEqual(other: actualAudio.AsSpan(
                    length: samples,
                    start: 0
                ))
                ) {
        ...
        return (true, (((string)$"{label}: {packets} state/audio checkpoints, {audioSamples:N0} PCM samples; execution {((referenceTicks * 1000.0) / Stopwatch.Frequency):F1} ms stepping / ")
    ...
    private sealed class ReferenceTimerController(AgbTimerController inner) : IAgbTimerController, ISnapshotable {
        ...
        public void WriteRegister(uint offset, ushort value) => inner.WriteRegister(
            offset: offset,
            value: value
        );
    }
