...
internal static class SmokeTests {
    ...
    private static void DataProcessingAndFlags() {
        ...
        Check(
            name: "SUB r2,r0,r1 = 7",
            ok: (cpu.GetRegister(index: 2) == 7u),
            detail: $"got {cpu.GetRegister(index: 2)}"
        );
        Check(
            name: "ADD r3,r0,r1 = 13",
            ok: (cpu.GetRegister(index: 3) == 13u),
            detail: $"got {cpu.GetRegister(index: 3)}"
        );
        ...
        Check(
            name: "ADDS wraps to 0",
            ok: (cpu.GetRegister(index: 1) == 0u),
            detail: $"got {cpu.GetRegister(index: 1)}"
        );
        Check(
            name: "ADDS sets carry",
        ...
        Check(
            name: "ADDS clears negative",
            ok: ((cpu.Cpsr & FlagN) == 0u)
        );
        Check(
            name: "ADDS clears overflow",
    ...
    private static void InterruptsTimersAndDma() {
        ...
        controller.WriteRegister(
            offset: 0x200u,
            value: timer0Bit
        );  // IE: timer 0 (next stage)
        controller.Request(source: InterruptSource.Timer0);          // IF (next stage)

        Check(
            name: "IRQ line still low before pipeline settles",
            ok: !controller.Synchronizer
        );
        ...
        Check(
            name: "IRQ asserts with IME+IE+IF after 2-cycle latency",
            ok: controller.Synchronizer
        );
        ...
        controller.WriteRegister(
            offset: 0x202u,
            value: timer0Bit
        );  // acknowledge (next stage)
        controller.StepSync(stallingCpu: false);
        controller.StepSync(stallingCpu: false);

        Check(
            name: "IRQ IF write-one-to-clear",
            ok: !controller.Synchronizer
        );
        ...
        var timers = new AgbTimerController(
            scheduler: new AgbScheduler(),
            interrupts: timerInterrupts,
            apu: new AgbApu()
        );
        ...
        timers.WriteRegister(
            offset: 0x100u,
            value: 0xFFFE
        );         // reload (overflows after two ticks)
        timers.WriteRegister(
            offset: 0x102u,
            value: 0x00C0
        ...
        Check(
            name: "timer holds off overflow before its count expires",
            ok: !timerInterrupts.HasPendingInterrupt
        );
        ...
        Check(
            name: "timer overflow raises IRQ",
            ok: timerInterrupts.HasPendingInterrupt
        );
    ...
    private static void PpuTiming() {
        ...
        Check(
            name: "PPU H-blank flagged on visible line",
            ok: ppu.ConsumeHBlankStarted()
        );
        ...
        Check(
            name: "PPU reaches V-blank at line 160",
            ok: (ppu.ReadRegister(offset: 0x06u) == 160)
        );
        Check(
            name: "PPU V-blank flag set",
            ok: ((ppu.ReadRegister(offset: 0x04u) & 0x1) != 0)
        );
        Check(
            name: "PPU V-blank started flag",
            ok: ppu.ConsumeVBlankStarted()
        );
        ...
        Check(
            name: "PPU V-blank raised IRQ",
            ok: interrupts.Synchronizer
        );
