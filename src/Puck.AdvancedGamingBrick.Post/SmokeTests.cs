...
internal static class SmokeTests {
    ...
    private static ReadOnlyMemory<byte> s_bios;
    ...
    public static SmokeResult Run(ReadOnlyMemory<byte> bios) {
        ...
        return new SmokeResult(Passed: s_passed, Failed: s_failed, Skipped: s_skipped, Failures: s_failures.ToArray());
    }
    ...
    private static void DataProcessingAndFlags() {
        ...
        Check(name: "SUB r2,r0,r1 = 7", ok: (cpu.GetRegister(index: 2) == 7u), detail: $"got {cpu.GetRegister(index: 2)}");
        Check(name: "ADD r3,r0,r1 = 13", ok: (cpu.GetRegister(index: 3) == 13u), detail: $"got {cpu.GetRegister(index: 3)}");
        ...
        Check(name: "ADDS wraps to 0", ok: (cpu.GetRegister(index: 1) == 0u), detail: $"got {cpu.GetRegister(index: 1)}");
    ...
    private static void BarrelShifterCarry() {
        ...
        Check(name: "LSL#31 produces 0x80000000", ok: (cpu.GetRegister(index: 1) == 0x80000000u), detail: $"got 0x{cpu.GetRegister(index: 1):X8}");
        Check(name: "MOVS LSL#1 result 0", ok: (cpu.GetRegister(index: 2) == 0u), detail: $"got 0x{cpu.GetRegister(index: 2):X8}");
    ...
    private static void MsrFieldMaskFix() {
        ...
        Check(name: "MSR sets NZCV", ok: ((cpu.GetRegister(index: 0) & 0xF0000000u) == 0xF0000000u), detail: $"got 0x{cpu.GetRegister(index: 0):X8}");
        Check(name: "MSR drops reserved bits 24-27", ok: ((cpu.GetRegister(index: 0) & 0x0F000000u) == 0u), detail: $"got 0x{cpu.GetRegister(index: 0):X8}");
    }
    private static void ThumbModeAndExecution() {
        ...
        Check(name: "Thumb MOV/ADD r1 = 8", ok: (cpu.GetRegister(index: 1) == 8u), detail: $"got {cpu.GetRegister(index: 1)}");
    }
    private static void InterruptsTimersAndDma() {
        // Interrupt controller: the IE/IF/IME pipeline is double-buffered. Writes land in the
        ...
        // Timer overflow raises its interrupt, per the per-cycle latch model: reload 0xFFFE, prescaler 1 (steps every
        // cycle). Cycle 0 commits the enable (arms the reload); cycle 1 reload-latches 0xFFFE; cycles 2 and 3 count
        // 0xFFFF then overflow. The overflow arms the request, whose flag is asserted OverflowIrqDelay (2) cycles
        // later — the hardware's overflow→IF latency a timer-IRQ conformance suite pins down — after which it reaches
        // HasPendingInterrupt one StepSync on. No (now+2) heuristic; the delay is a real per-cycle countdown.
        ...
        // Direct per-cycle drive (no bus): the block stays in per-cycle mode, so RunCycle steps every cycle. The
        // scheduler is only a construction dependency here — no overflow events are queued on this path.
        var timers = new AgbTimerController(scheduler: new AgbScheduler(), interrupts: timerInterrupts, apu: new AgbApu());
        ...
        timers.RunCycle(clock: clock);                               // cycle 3: 0xFFFF -> overflow, arms the request
        ++clock;
        timers.RunCycle(clock: clock);                               // cycle 4: overflow-IRQ delay counts down
        ++clock;
        timers.RunCycle(clock: clock);                               // cycle 5: delay elapses, request lands in flag[1]
        ...
        var dmaScheduler = new AgbScheduler();
        var bus = new AgbBus(
            ...
            timers: new AgbTimerController(scheduler: dmaScheduler, interrupts: dmaInterrupts, apu: new AgbApu()),
    ...
    private static void DmaCountLatching() {
        ...
        var bus = new AgbBus(
            ...
            timers: new AgbTimerController(scheduler: latchScheduler, interrupts: interrupts, apu: new AgbApu()),
            dma: dma,
            serial: new AgbSerialController(scheduler: new AgbScheduler(), interrupts: interrupts),
            ppu: new AgbPpu(scheduler: new AgbScheduler(), interrupts: interrupts),
            apu: new AgbApu());

        // src[i] = i as halfwords in IWRAM; the destination is a single fixed halfword.
        for (uint i = 0; (i < 8u); ++i) {
            bus.Write16(address: (0x03000000u + (i * 2u)), value: (ushort)i, access: BusAccessType.NonSequential);
        }

        // DMA3: src 0x03000000 (increment), dst 0x03000100 (fixed), 16-bit, VBlank timing, repeat, count = 2.
        bus.Write16(address: 0x040000D4u, value: 0x0000, access: BusAccessType.NonSequential); // SAD lo
        bus.Write16(address: 0x040000D6u, value: 0x0300, access: BusAccessType.NonSequential); // SAD hi → 0x03000000
        bus.Write16(address: 0x040000D8u, value: 0x0100, access: BusAccessType.NonSequential); // DAD lo
        bus.Write16(address: 0x040000DAu, value: 0x0300, access: BusAccessType.NonSequential); // DAD hi → 0x03000100
        bus.Write16(address: 0x040000DCu, value: 0x0002, access: BusAccessType.NonSequential); // count = 2 (latched here)
        bus.Write16(address: 0x040000DEu, value: 0x9240, access: BusAccessType.NonSequential); // enable+VBlank+repeat+dst-fixed

        // Rewrite the count to 4 AFTER enable — the first run must still transfer the latched 2, not 4.
        bus.Write16(address: 0x040000DCu, value: 0x0004, access: BusAccessType.NonSequential);

        dma.OnVBlank(bus: bus);
        dma.RunPending(bus: bus);
        var first = bus.Read16(address: 0x03000100u, access: BusAccessType.NonSequential);

        Check(name: "DMA count latched at enable (first run transfers 2)", ok: (first == 1u), detail: $"got {first}");

        dma.OnVBlank(bus: bus);
        dma.RunPending(bus: bus);
        var reload = bus.Read16(address: 0x03000100u, access: BusAccessType.NonSequential);

        Check(name: "DMA count reloads from register on repeat (transfers 4)", ok: (reload == 5u), detail: $"got {reload}");
    }
    ...
    // that wins the pixel); a 120th sprite at a fresh x=50 only has 2 cycles left, so its first two columns
    // render and the rest of its box does not (mid-sprite cutoff); a 121st sprite never gets touched at all
    // (whole-sprite dropout), because OAM processing stops the instant the budget hits zero.
    private static void SpriteCycleBudget() {
        var scheduler = new AgbScheduler();
        var ppu = new AgbPpu(scheduler: scheduler, interrupts: new AgbInterruptController());

        // Mode 0, OBJ enabled (bit 12), 1-D tile mapping (bit 6), H-Blank Interval Free (bit 5) → the smaller
        // 954-cycle budget, so a modest sprite count exhausts it deterministically.
        ppu.WriteRegister(offset: 0x00u, value: 0x1060);

        // A single 8×8 4bpp tile (tile 0) of solid colour index 1, shared by every sprite below.
        for (uint i = 0; (i < 32u); i += 2u) {
            ppu.WriteVideo(address: (0x06010000u + i), width: 2, value: 0x1111);
        }
        ...
        ppu.WriteVideo(address: 0x05000000u, width: 2, value: 0x7C00);

        const int burnerCount = 119; // 119 * 8 cycles = 952 of the 954-cycle budget.

        for (var sprite = 0; (sprite < burnerCount); ++sprite) {
            WriteBudgetSprite(ppu: ppu, index: sprite, x: 0);
        }

        WriteBudgetSprite(ppu: ppu, index: burnerCount, x: 50);      // 2 cycles left: columns 0-1 render, 2-7 don't.
        WriteBudgetSprite(ppu: ppu, index: (burnerCount + 1), x: 100); // budget already 0: never processed.

        scheduler.Advance(cycles: 1232); // render scanline 0

        const uint spriteRed = 0xFF0000FFu;
        const uint backdropBlue = 0xFFFF0000u;
        var burnerPixel = ppu.Framebuffer[0];
        var targetEarly = ppu.Framebuffer[50];
        var targetLate = ppu.Framebuffer[55];
        var droppedSprite = ppu.Framebuffer[100];

        Check(name: "OBJ budget: early sprite still renders", ok: (burnerPixel == spriteRed), detail: $"got 0x{burnerPixel:X8}");
        Check(name: "OBJ budget: sprite renders while cycles remain", ok: (targetEarly == spriteRed), detail: $"got 0x{targetEarly:X8}");
        Check(name: "OBJ budget: exhaustion stops mid-sprite", ok: (targetLate == backdropBlue), detail: $"got 0x{targetLate:X8}");
        Check(name: "OBJ budget: later sprite drops out entirely", ok: (droppedSprite == backdropBlue), detail: $"got 0x{droppedSprite:X8}");
    }
    ...
    private static void WriteBudgetSprite(AgbPpu ppu, int index, int x) {
        ...
        ppu.WriteVideo(address: attributeBase, width: 2, value: 0x0000);              // attr0: y=0, normal, 8×8
        ppu.WriteVideo(address: (attributeBase + 2u), width: 2, value: (uint)x);         // attr1: x, 8×8
        ppu.WriteVideo(address: (attributeBase + 4u), width: 2, value: 0x0000);          // attr2: tile 0, priority 0
    }
    ...
    private static void GreenSwap() {
        ...
        var offPpu = new AgbPpu(scheduler: offScheduler, interrupts: new AgbInterruptController());

        offPpu.WriteRegister(offset: 0x00u, value: 0x0403); // Mode 3, BG2 enabled (bit 10)
        offPpu.WriteVideo(address: 0x06000000u, width: 2, value: (uint)colorA);
        offPpu.WriteVideo(address: 0x06000002u, width: 2, value: (uint)colorB);
        offScheduler.Advance(cycles: 1232);

        var offPixel0 = offPpu.Framebuffer[0];
        var offPixel1 = offPpu.Framebuffer[1];

        Check(name: "green swap off leaves pixel 0 untouched", ok: (offPixel0 == colorAUnswapped), detail: $"got 0x{offPixel0:X8}");
        Check(name: "green swap off leaves pixel 1 untouched", ok: (offPixel1 == colorBUnswapped), detail: $"got 0x{offPixel1:X8}");

        var onScheduler = new AgbScheduler();
        var onPpu = new AgbPpu(scheduler: onScheduler, interrupts: new AgbInterruptController());

        onPpu.WriteRegister(offset: 0x00u, value: 0x0403); // Mode 3, BG2 enabled (bit 10)
        onPpu.WriteRegister(offset: 0x02u, value: 0x0001); // GRSWP bit 0
        onPpu.WriteVideo(address: 0x06000000u, width: 2, value: (uint)colorA);
        onPpu.WriteVideo(address: 0x06000002u, width: 2, value: (uint)colorB);
        onScheduler.Advance(cycles: 1232);

        var onPixel0 = onPpu.Framebuffer[0];
        var onPixel1 = onPpu.Framebuffer[1];

        Check(name: "green swap exchanges pixel 0's green", ok: (onPixel0 == colorASwapped), detail: $"got 0x{onPixel0:X8}");
        Check(name: "green swap exchanges pixel 1's green", ok: (onPixel1 == colorBSwapped), detail: $"got 0x{onPixel1:X8}");
    }
    ...
    private static void BiosIrqDispatch() {
        ...
        if (s_bios.Span.IndexOfAnyExcept(value: (byte)0) < 0) {
            ++s_skipped;
    ...
    private static void DependencyInjectionScope() {
        ...
        Check(name: "DI-resolved IArmCpu executes (r0 = 42)", ok: (cpu.GetRegister(index: 0) == 42u), detail: $"got {cpu.GetRegister(index: 0)}");
    }
    ...
    private static void Check(string name, bool ok, string detail = "") {
        if (ok) {
            ...
            ++s_failed;
            s_failures.Add(item: (string.IsNullOrEmpty(value: detail) ? name : $"{name} ({detail})"));
        }
    }
}
...
/// <param name="Failed">The number of checks that failed.</param>
/// <param name="Skipped">The number of checks skipped for want of a prerequisite (a real BIOS).</param>
/// <param name="Failures">The names (with detail) of the failing checks.</param>
internal readonly record struct SmokeResult(int Passed, int Failed, int Skipped, IReadOnlyList<string> Failures);
