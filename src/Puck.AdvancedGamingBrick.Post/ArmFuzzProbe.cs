...
/// <summary>
/// Runs an ARM/Thumb fuzz-corpus ROM (randomized instruction coverage) and reads its verdict from the start of EWRAM: on
/// the first failing instruction the ROM dumps a marker (<c>'AAAA'</c> for ARM, <c>'TTTT'</c> for Thumb) to
/// <c>0x02000000</c> and otherwise leaves it zero. The probe steps the machine until execution settles, then inspects
/// that word.
/// </summary>
internal static class ArmFuzzProbe {
