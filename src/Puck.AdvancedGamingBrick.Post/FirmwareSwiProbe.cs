...
internal static class FirmwareSwiProbe {
    ...
    internal static void Call(AdvancedGamingBrickCore core, byte number, bool thumb, uint r0 = 0, uint r1 = 0, uint r2 = 0, uint r3 = 0, Action<AdvancedGamingBrickMachine>? setup = null, Action<AdvancedGamingBrickMachine>? afterStep = null) {
        ...
        var entry = ((AdvancedGamingBrickMachine.CartridgeEntryPoint + ((uint)(number * 64))) + (thumb
            ? 4096u
            : 0u));
        ...
        for (var instruction = 0; (instruction < MaximumInstructions); ++instruction) {
            ...
            if (
                (machine.Cpu.GetRegister(index: 7) == 0x5A) &&
                (pc >= entry) &&
                (pc < (entry + 64))
            ) {
                return;
        ...
        throw new InvalidOperationException(message: $"BIOS SWI {number:X2} ({(thumb
            ? "Thumb"
            : "ARM")}) did not return; PC={machine.Cpu.GetRegister(index: 15):X8}");
    }
    internal static void ExpectBytes(IAgbBus bus, uint address, ReadOnlySpan<byte> expected, string detail) {
        for (var index = 0; (index < expected.Length); ++index) {
            var actual = bus.Read8(
                access: BusAccessType.NonSequential,
                address: (address + ((uint)index))
            );

            Require(
                condition: (actual == expected[index]),
                detail: $"{detail}: byte {index}, expected {expected[index]:X2}, got {actual:X2}"
            );
        }
    }
    ...
    private static byte[] CreateRom() {
        ...
        for (var number = 0; (number <= 0x2A); ++number) {
            ...
            WriteWord(
                image: rom,
                offset: arm,
                value: 0xEF000000 | (((uint)number) << 16)
            );
            WriteWord(
                image: rom,
                offset: (arm + 4),
            ...
            WriteWord(
                image: rom,
                offset: (thumb + 8),
                value: ((AdvancedGamingBrickMachine.CartridgeEntryPoint + ((uint)thumb)) + 0x21)
            );
            BinaryPrimitives.WriteUInt16LittleEndian(
                destination: rom.AsSpan(start: (thumb + 0x20)),
                value: ((ushort)(0xDF00 | number))
            );
            BinaryPrimitives.WriteUInt16LittleEndian(
                destination: rom.AsSpan(start: (thumb + 0x22)),
                value: 0x275A
            );
            BinaryPrimitives.WriteUInt16LittleEndian(
                destination: rom.AsSpan(start: (thumb + 0x24)),
                value: 0xE7FE
            );
        }
        return rom;
