...
/// <summary>
/// The per-rental owner handle a <see cref="MachineInstance.Fork"/> hands back — a fresh object bound to one pooled
...
/// and disposal is inert. The pooled <see cref="MachineInstance"/> is never exposed directly, which is what closes the
/// ABA hole — two owners can never alias one emulated machine through a lingering reference.
/// </summary>
public sealed class MachineFork : IDisposable {
    private readonly MachineInstance m_instance;
    private readonly int m_generation;

    internal MachineFork(MachineInstance instance, int generation) {
        m_instance = instance;
    ...
    private MachineInstance Current =>
        (m_instance.IsRentalCurrent(generation: m_generation)
            ? m_instance
            : throw new ObjectDisposedException(objectName: nameof(MachineFork), message: "This fork rental was disposed and its machine re-rented; a stale fork handle cannot reach it."));
}
