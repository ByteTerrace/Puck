...
public sealed class CartridgeHeader {
    ...
    private static void DecodeType(byte typeCode, out MapperKind mapper, out bool hasRam, out bool hasBattery, out bool hasRumble) {
        (mapper, hasRam, hasBattery, hasRumble) = typeCode switch {
            ...
            0x06 => (MapperKind.Mbc2, false, true, false),
            0x08 => (MapperKind.RomOnly, true, false, false),
            0x09 => (MapperKind.RomOnly, true, true, false),
            0x0B => (MapperKind.Mmm01, false, false, false),
            0x0C => (MapperKind.Mmm01, true, false, false),
            0x0D => (MapperKind.Mmm01, true, true, false),
            0x0F => (MapperKind.Mbc3, false, true, false),
            0x10 => (MapperKind.Mbc3, true, true, false),
            ...
            0x1A => (MapperKind.Mbc5, true, false, false),
            0x1B => (MapperKind.Mbc5, true, true, false),
            // Rumble variants: RAM-bank select bit 3 drives the motor instead of selecting a bank (Mbc5Cartridge masks
            // it out of the bank number). 0x1C has no RAM at all (rumble-only, e.g. Boktai's forerunners); 0x1D adds RAM;
            // 0x1E adds a battery — the RAM/battery bits mirror 0x1A/0x1B exactly, only the rumble bit differs.
            0x1C => (MapperKind.Mbc5, false, false, true),
            0x1D => (MapperKind.Mbc5, true, false, true),
            0x1E => (MapperKind.Mbc5, true, true, true),
            // The MBC7's save chip is a serial EEPROM behind its register window, not header-described external RAM.
            0x22 => (MapperKind.Mbc7, false, true, false),
            0xFC => (MapperKind.Camera, true, true, false),
            0xFE => (MapperKind.HuC3, true, true, false),
            0xFF => (MapperKind.HuC1, true, true, false),
            _ => (MapperKind.RomOnly, false, false, false),
        };
    }
