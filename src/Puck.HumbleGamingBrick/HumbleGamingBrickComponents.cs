...
public static class HumbleGamingBrickComponents {
    ...
    public static IServiceCollection AddHumbleGamingBrickComponents(this IServiceCollection services) {
        ...
        services.AddScoped<IModeSwitchable>(implementationFactory: static provider => provider.GetRequiredService<SerialComponent>());

        // The one infrared transceiver: the shared IR LED + receiver the Color RP register (bus-gated to Color models) and
        // the HuC1/HuC3 cart IR windows are two views of. Registered on EVERY model — HuC1/HuC3 cart IR works on monochrome
        // hardware too, so only the RP register itself is Color-gated (in the bus). Not clocked (light is a level, no
        // per-cycle work); its register/LED state is snapshotted, the linked peer is host wiring.
        services.TryAddScoped<InfraredPort>();
        services.AddScoped<IInfrared>(implementationFactory: static provider => provider.GetRequiredService<InfraredPort>());
        services.AddScoped<ISnapshotable>(implementationFactory: static provider => provider.GetRequiredService<InfraredPort>());
        services.AddScoped<IModeSwitchable>(implementationFactory: static provider => provider.GetRequiredService<InfraredPort>());
        ...
        // The camera cartridge's image sensor: the deterministic gradient by default, so any camera cartridge has a stable
        ...
        // The MBC7's accelerometer: motionless by default (TiltSensorComponent's fixed centered reading), so any
        // tilt cartridge has stable, bit-identical behavior with no host attached. A queued host that folds recorded
        // per-segment tilt input registers its own ITiltSensor first (TryAdd), on the same seam as the camera above.
        // Also registered as ISnapshotable, since a currently-held recorded reading is machine state.
        services.TryAddScoped<TiltSensorComponent>();
        ...
        services.TryAddScoped(implementationFactory: static provider => {
            ...
            if (cartridge is CameraCartridge camera) {
                camera.Sensor = provider.GetRequiredService<ICameraSensor>();
            }

            // Likewise for an MBC7 tilt cartridge and its accelerometer.
            if (cartridge is Mbc7Cartridge tiltCart) {
                tiltCart.Sensor = provider.GetRequiredService<ITiltSensor>();
            }
            ...
            if (cartridge is IInfraredCartridge infraredCart) {
                ...
                infraredPort.HasHuCCartridge = true;
                infraredCart.Infrared = infraredPort;
            }
