...
public abstract class CartridgeBase : ICartridge {
    // The external-RAM window's byte span (0xA000-0xBFFF inclusive) — the largest a cached window can ever report.
    private const int RamWindowByteCount = 0x2000;
    ...
    /// the populated RAM so an out-of-range tail correctly falls back to the interface path instead of indexing past
    /// the array. A mapper that overrides <see cref="ReadRam"/> or <see cref="WriteRam"/> with side effects or mode
    /// routing overrides this to return <see langword="false"/> instead.</remarks>
    public virtual bool TryComputeRamWindow(out int offset, out int length) {
        if (!RamAccessible) {
            offset = 0;
            length = 0;
    ...
    /// dirty flag. This is the seam a sensor mapper (the camera cartridge) uses to write its freshly captured image into
