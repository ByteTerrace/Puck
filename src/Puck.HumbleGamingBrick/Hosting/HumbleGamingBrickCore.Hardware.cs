...
public sealed partial class HumbleGamingBrickCore : IMachineHardwareAccess {
    ...
    public static IReadOnlyList<MachineMemorySpaceDescriptor> HardwareSpaces { get; } = [
        new(
            ...
            Description: "SM83 bus; ordered byte bus accesses at the current clock boundary. Patch requires mapped RAM or IE.",
            FirstAddress: 0,
            LastAddress: 0xFFFF,
            LittleEndian: true,
            Name: "bus",
    ...
    public MachineAccessResult Read(MachineMemoryAddress address, MachineAccessMode mode = MachineAccessMode.Inspect) {
        ...
        for (var offset = 0; (offset < address.Width); offset++) {
            ...
            var part = ((mode == MachineAccessMode.Inspect)
                ? m_systemBus.DebugReadByte(address: current)
                : m_systemBus.ReadByte(address: current)
            );
    ...
    private MachineAccessResult? ValidateHardwareAccess(MachineMemoryAddress address, MachineAccessMode mode, bool writing) {
        if (
            (address.Space != "bus") ||
            (address.Width is not (1 or 2))
        ) {
            return new(
                MachineAccessStatus.Unsupported,
                Reason: "SM83 hardware access requires space 'bus' and width 1 or 2."
            );
        }
        if (
            (address.Address > 0xFFFFUL) ||
            (((ulong)(address.Width - 1)) > (0xFFFFUL - address.Address))
        ) {
            return new(
                MachineAccessStatus.Refused,
                Reason: $"Address 0x{address.Address:X} with width {address.Width} is outside space 'bus'."
            );
        }
        if (writing
        ...
        for (var offset = 0; (offset < address.Width); offset++) {
            ...
            if (
                ...
                !(current is >= 0x8000 and <= 0xFE9F or >= 0xFF80 and <= 0xFFFF)
            ) {
                return new(
                    MachineAccessStatus.Refused,
                    Reason: $"Address 0x{current:X4} is not patchable RAM or IE; hardware registers require bus access."
                );
            }
            if (
                (mode != MachineAccessMode.Bus) &&
                (current is >= 0xA000 and <= 0xBFFF) &&
                (!m_cartridge.TryComputeRamWindow(
                length: out var length,
                offset: out _
            ) || ((current - 0xA000UL) >= ((ulong)length)))
            ) {
                return new(
                    MachineAccessStatus.Unsupported,
                    Reason: $"Address 0x{current:X4} has no side-effect-free cartridge RAM mapping."
                );
            }
        }
        return null;
